...
bool ProjectExplorerWidget::openProject(const QString& projectPath) {
    if (projectPath.isEmpty() || !QFileInfo(projectPath).isDir()) {
        RAWRXD_LOG_WARN("Invalid project path:") << projectPath;
        return false;
    return true;
}
    ...
    RAWRXD_LOG_DEBUG("Opened project:") << m_projectPath << "Type:" << ProjectDetector::projectTypeName(m_projectMetadata.type);
...
void ProjectExplorerWidget::actionNewFile() {
    ...
    if (result.success) {
        ...
        selectFile(filePath);
        RAWRXD_LOG_DEBUG("Created file:") << filePath;
    } else {
...
void ProjectExplorerWidget::actionNewFolder() {
    ...
    if (result.success) {
        ...
        RAWRXD_LOG_DEBUG("Created folder:") << folderPath;
    } else {
...
void ProjectExplorerWidget::actionRename() {
    ...
    if (result.success) {
        ...
        selectFile(newPath);
        RAWRXD_LOG_DEBUG("Renamed:") << oldPath << "→" << newPath;
    } else {
...
void ProjectExplorerWidget::actionCopy() {
    ...
    m_clipboardIsCut = false;
    m_actionPaste->setEnabled(!m_clipboardPath.isEmpty());
    RAWRXD_LOG_DEBUG("Copied to clipboard:") << m_clipboardPath;
    return true;
}
...
void ProjectExplorerWidget::actionCut() {
    ...
    m_actionPaste->setEnabled(!m_clipboardPath.isEmpty());
    RAWRXD_LOG_DEBUG("Cut to clipboard:") << m_clipboardPath;
    return true;
}
...
void ProjectExplorerWidget::actionPaste() {
    ...
    return true;
        ...
        if (result.success) {
            ...
            RAWRXD_LOG_DEBUG("Moved (cut+paste)");
    return true;
}
    ...
    return true;
        ...
        RAWRXD_LOG_DEBUG("Copied (copy+paste)");
    return true;
}
...
void ProjectExplorerWidget::actionCopyPath() {
    ...
    RAWRXD_LOG_DEBUG("Copied absolute path to clipboard:") << path;
    return true;
}
...
void ProjectExplorerWidget::actionCopyRelativePath() {
    ...
    RAWRXD_LOG_DEBUG("Copied relative path to clipboard:") << relativePath;
    return true;
}
...
void ProjectExplorerWidget::loadGitignorePatterns() {
    ...
    RAWRXD_LOG_DEBUG("Loaded") << m_gitignorePatterns.size() << "gitignore patterns";
    return true;
}
