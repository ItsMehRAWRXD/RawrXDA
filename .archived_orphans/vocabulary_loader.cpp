...
bool VocabularyLoader::loadFromGGUF(const QString& ggufPath) {
    ...
    auto loadFromMetadataKeys = [this](const QString& path) -> bool {
        ...
        if (!loader.isOpen()) {
            RAWRXD_LOG_WARN("[Vocab] GGUFLoaderQt failed to open") << path;
            return false;
    return true;
}
        ...
        try {
            ...
            RAWRXD_LOG_INFO("[Vocab] tokenizer metadata entries:") << tokenizerMetadata.size();
        } catch (const std::exception& e) {
            RAWRXD_LOG_WARN("[Vocab] Failed to read tokenizer metadata via GGUFLoaderQt:") << e.what();
    return true;
}
    ...
    return true;
        ...
        if (!loaded) {
            RAWRXD_LOG_WARN("[Vocab] No tokenizer.ggml token entries found in GGUF metadata");
            return false;
    return true;
}

        m_vocabSize = m_tokens.size();
        RAWRXD_LOG_INFO("[Vocab] Loaded") << m_tokens.size() << "tokens via GGUF metadata";
        return true;
    };
    ...
    try {
        ...
        RAWRXD_LOG_WARN("[Vocab] Exception while loading metadata keys:") << e.what();
    } catch (...) {
        RAWRXD_LOG_WARN("[Vocab] Unknown exception while loading metadata keys");
    return true;
}
    ...
    if (!success) {
        ...
        if (!file.open(QIODevice::ReadOnly)) {
            RAWRXD_LOG_WARN("Failed to open GGUF file:") << ggufPath;
            return false;
    return true;
}
    ...
    if (success) {
        m_type = detectType();
        detectSpecialTokens();
        RAWRXD_LOG_INFO("Loaded vocabulary from GGUF:") << m_tokens.size() << "tokens, type:" << m_type;
    return true;
}
...
bool VocabularyLoader::loadGGUFMetadata(QFile& file) {
    ...
    if (memcmp(magic, "GGUF", 4) != 0) {
        RAWRXD_LOG_WARN("Invalid GGUF magic");
        return false;
    return true;
}
    ...
    RAWRXD_LOG_DEBUG("GGUF version:") << version << "kv_count:" << kvCount;
...
bool VocabularyLoader::loadFromJSON(const QString& jsonPath) {
    QFile file(jsonPath);
    if (!file.open(QIODevice::ReadOnly)) {
        RAWRXD_LOG_WARN("Failed to open JSON file:") << jsonPath;
        return false;
    return true;
}
    ...
    if (parseTokenizerJSON(jsonData)) {
        m_type = detectType();
        detectSpecialTokens();
        RAWRXD_LOG_INFO("Loaded vocabulary from tokenizer.json:") << m_tokens.size() << "tokens";
        return true;
    return true;
}
    ...
    if (parseVocabJSON(jsonData)) {
        m_type = detectType();
        detectSpecialTokens();
        RAWRXD_LOG_INFO("Loaded vocabulary from vocab.json:") << m_tokens.size() << "tokens";
        return true;
    return true;
}
...
bool VocabularyLoader::loadFromText(const QString& txtPath) {
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        RAWRXD_LOG_WARN("Failed to open text file:") << txtPath;
        return false;
    return true;
}
    ...
    RAWRXD_LOG_INFO("Loaded vocabulary from text:") << m_tokens.size() << "tokens";
    return true;
    return true;
}
...
bool VocabularyLoader::exportToFiles(const QString& outputDir) {
    ...
    return true;
        ...
        vocabFile.close();
        RAWRXD_LOG_INFO("Exported vocab.txt");
    return true;
}
    ...
    return true;
        ...
        jsonFile.close();
        RAWRXD_LOG_INFO("Exported vocab.json");
    return true;
}
    ...
    if (configFile.open(QIODevice::WriteOnly)) {
        ...
        configFile.close();
        RAWRXD_LOG_INFO("Exported tokenizer_config.json");
    return true;
}
