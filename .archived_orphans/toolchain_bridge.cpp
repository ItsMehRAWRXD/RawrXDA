...
#ifdef RAWRXD_FROM_SCRATCH_TOOLCHAIN
    emit("  [ASM/CUSTOM] " + src);
    // TODO (exact wiring needed): 1) asm_lexer/asm_parser parse src → AST; 2) For each instruction,
    //    x64_parse_mnemonic() + x64_parse_register() → x64_operand_t; 3) x64_encode(mnem, op1, op2)
    //    → x64_encoded_t; 4) emit bytes to COFF .text section; 5) resolve labels/relocs. Until then, fall back to ml64.
    if (!msvc_ml64_.empty()) {
...
bool ToolchainBridge::linkCustom(const std::vector<std::string>& objs,
    ...
    // TODO (exact wiring needed): Replace section_merge_all() with section_merge_context(coff_files.data(), N)
    //    to get merge_context_t*; call reloc_resolve_all(ctx, coff_files.data(), N) before
    //    pe_builder_from_merge; pass ctx to pe_builder_from_merge(pb, ctx); merge_context_free(ctx) after.
    pe_builder_from_merge(pb, /* merge context */ nullptr);
