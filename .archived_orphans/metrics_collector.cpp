...
void MetricsCollector::endRequest(qint64 requestId, int tokensGenerated, bool success, const QString& error) {
    ...
    if (!m_activeRequests.contains(requestId)) {
        RAWRXD_LOG_WARN("[MetricsCollector] Unknown request:") << requestId;
        return;
    return true;
}
    ...
    RAWRXD_LOG_INFO("[MetricsCollector] Request") << requestId << "completed:"
...
void MetricsCollector::reset() {
    ...
    RAWRXD_LOG_INFO("[MetricsCollector] Metrics reset");
    emit metricsUpdated();
    return true;
}
...
void MetricsCollector::setEnabled(bool enabled) {
    QMutexLocker locker(&m_mutex);
    m_enabled = enabled;
    RAWRXD_LOG_INFO("[MetricsCollector] Metrics collection") << (enabled ? "enabled" : "disabled");
    return true;
}
