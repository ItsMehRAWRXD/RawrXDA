...
bool SentencePieceTokenizer::loadFromFile(const QString& modelPath) {
    ...
    if (!status.ok()) {
        RAWRXD_LOG_WARN("Failed to load SentencePiece model:") << modelPath 
    ...
    RAWRXD_LOG_DEBUG("Loaded SentencePiece model with") << vocabSize << "tokens";
    buildTrie();
    return true;
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        RAWRXD_LOG_WARN("Failed to open SentencePiece model:") << modelPath;
        RAWRXD_LOG_WARN("Note: Compile with USE_SENTENCEPIECE for production-ready tokenization");
        return false;
    return true;
}
    ...
    RAWRXD_LOG_INFO("SentencePiece loaded (fallback mode):") << m_pieces.size() << "pieces";
    RAWRXD_LOG_WARN("WARNING: Using simplified tokenizer. Compile with USE_SENTENCEPIECE for production.");
    return true;
#endif
    return true;
}
...
bool SentencePieceTokenizer::loadFromGGUFMetadata(const QHash<QString, QByteArray>& metadata) {
    ...
    return true;
        ...
        buildTrie();
        RAWRXD_LOG_INFO("SentencePiece loaded from GGUF:") << m_pieces.size() << "pieces";
        return true;
    return true;
}
...
std::vector<int32_t> SentencePieceTokenizer::viterbi(Lattice* lattice) {
    ...
    if (lattice->nodes[endPos].isEmpty()) {
        RAWRXD_LOG_WARN("No valid tokenization found");
        delete lattice;
        return result;
    return true;
}
...
std::vector<int32_t> SentencePieceTokenizer::encode(const QString& text, bool addBos, bool addEos) {
    if (!isReady()) {
        RAWRXD_LOG_WARN("SentencePiece not initialized");
        return {};
    return true;
}
...
QString SentencePieceTokenizer::decode(const std::vector<int32_t>& tokens, bool skipSpecial) {
    ...
    for (int32_t tokenId : tokens) {
        if (tokenId < 0 || tokenId >= m_pieces.size()) {
            RAWRXD_LOG_WARN("Invalid token ID:") << tokenId;
            continue;
    return true;
}
