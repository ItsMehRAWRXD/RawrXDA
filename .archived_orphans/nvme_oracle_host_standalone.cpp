...
// Returns: temperature in Celsius, or negative error codes:
//   -1 = Cannot open drive
//   -2 = IOCTL failed 
//   -3 = InfoCount is 0
...
int QueryNVMeTemp(int driveId) {
    ...
    if (hDrive == INVALID_HANDLE_VALUE) {
        // Try with just GENERIC_READ
        hDrive = CreateFileA(path, GENERIC_READ,
            FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
...
int main() {
    ...
    if (!StartServiceCtrlDispatcherA(table)) {
        ...
        printf("Error codes: -1=Cannot open, -2=IOCTL fail, -3=No sensors, -4=Out of range\n\n");
        for (int i = 0; i < 6; i++) {
            int result = QueryNVMeTemp(i);
            printf("Drive %d: %d %s\n", i, result, 
                result >= 0 ? "C" : 
                result == -1 ? "(cannot open drive)" :
                result == -2 ? "(IOCTL failed - not NVMe?)" :
                result == -3 ? "(no temperature sensors)" :
                "(temp out of range)");
    return true;
}
