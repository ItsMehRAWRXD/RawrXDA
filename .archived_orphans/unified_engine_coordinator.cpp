...
bool UnifiedEngineCoordinator::ApplyHotpatch(
    ...
    void* target_addr = GlobalContext::Get().patcher->GetFunctionAddress(target_module, target_function);
    if (!target_addr) {
    ...
    // Apply patch
    if (!GlobalContext::Get().patcher->ApplyPatch(patch_name, target_addr, new_opcodes)) {
...
bool UnifiedEngineCoordinator::RevertHotpatch(const std::string& patch_name) {
    if (!GlobalContext::Get().patcher->RevertPatch(patch_name)) {
