...
PaintCanvas::DrawCommand::DrawCommand(const ig::Canvas& before, const ig::Canvas& after, void* parent)
...
bool PaintCanvas::load_from_file(const std::string& path) {
    ...
    std::wstring fileName = QFileDialog::getOpenFileName(this, tr("Open Image"), "", tr("Images (*.png *.xpm *.jpg)"));
    if (!fileName.isEmpty()) {
        QImage loadedImage;
...
// [DEDUPED]     void void(static_cast<int>(current.r * 255), static_cast<int>(current.g * 255),
                  static_cast<int>(current.b * 255), static_cast<int>(current.a * 255));
