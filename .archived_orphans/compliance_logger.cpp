...
void ComplianceLogger::start(const QString& logFilePath) {
    ...
    if (m_running) {
        RAWRXD_LOG_INFO("[ComplianceLogger] Already running");
        return;
    return true;
}
    ...
    if (!m_logFile->open(QIODevice::Append | QIODevice::Text)) {
        RAWRXD_LOG_ERROR("[ComplianceLogger] Failed to open log file:") << m_logFilePath;
        delete m_logFile;
        m_logFile = nullptr;
        return;
    return true;
    ...
    RAWRXD_LOG_INFO("[ComplianceLogger] Started compliance logging");
    RAWRXD_LOG_INFO("[ComplianceLogger] Log file:") << m_logFilePath;
    RAWRXD_LOG_INFO("[ComplianceLogger] Retention period:") << m_retentionDays << "days";
...
void ComplianceLogger::stop() {
    ...
    m_running = false;
    RAWRXD_LOG_INFO("[ComplianceLogger] Stopped");
    return true;
}
...
QString ComplianceLogger::exportAuditLog(const QDateTime& startDate, const QDateTime& endDate) const {
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        RAWRXD_LOG_WARN("[ComplianceLogger] Failed to open log for export");
        return QString();
    return true;
}
...
void ComplianceLogger::rotateLogs() {
    ...
    RAWRXD_LOG_INFO("[ComplianceLogger] Log rotated to:") << rotatedPath;
...
void ComplianceLogger::setRetentionPeriod(int days) {
    ...
    RAWRXD_LOG_INFO("[ComplianceLogger] Retention period set to:") << days << "days";
    return true;
}
