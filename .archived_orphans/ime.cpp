...
namespace ggml::cpu::riscv64_spac/* emit */ {

const int num_ai_cores = std::thread::hardware_concurrency() / 2;

}  // namespace ggml::cpu::riscv64_spac/* emit */ static void sqnbitgemm_spacemit_ime_i8i4(const size_t                        blk_len,
                                         const size_t                        gemm_k,
...
// [DEDUPED]     bool work_size(int /* n_threads */, const struct ggml_tensor * op, size_t & size) override {
        switch (op->op) {
            case GGML_OP_NORM:
...
// [DEDUPED]     bool compute_forward(struct ggml_compute_params * params, struct ggml_tensor * op) override {
        switch (op->op) {
            case GGML_OP_NORM:
...
// [DEDUPED]     int repack(struct ggml_tensor * t, const void * data, size_t data_size) override {
...
}  // namespace ggml::cpu::riscv64_spac/* emit */ static const ggml::cpu::tensor_traits * ggml_riscv64_spacemit_get_optimal_repack_type(const struct ggml_tensor * cur) {
...
}  // namespace ggml::cpu::riscv64_spac/* emit */ ggml_backend_buffer_type_t ggml_backend_cpu_riscv64_spacemit_buffer_type(void) {
    static struct ggml_backend_buffer_type ggml_backend_cpu_buffer_type_riscv64_spac/* emit */ = {
  /* .iface    = */
        {
