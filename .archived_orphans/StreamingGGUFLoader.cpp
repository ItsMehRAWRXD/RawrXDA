...
bool StreamingGGUFLoader::BuildTensorIndex() {
    ...
    if (m_totalSize < 24) {
        RAWRXD_LOG_WARN("[StreamingGGUFLoader] File too small for GGUF header");
        return false;
    return true;
}
    ...
    if (magic != 0x46554747) { // "GGUF" little-endian
        RAWRXD_LOG_WARN("[StreamingGGUFLoader] Invalid GGUF magic:") << Qt::hex << magic;
        return false;
    return true;
}
    ...
    RAWRXD_LOG_INFO("[StreamingGGUFLoader] GGUF v") << version
...
bool StreamingGGUFLoader::GetTensorData(const QString& tensorName, std::vector<uint8_t>& outData) {
    ...
    if (offsetInZone + meta.size_bytes > zmem.size_bytes || !zmem.mapped_data) {
        RAWRXD_LOG_WARN("[StreamingGGUFLoader] Tensor") << tensorName
