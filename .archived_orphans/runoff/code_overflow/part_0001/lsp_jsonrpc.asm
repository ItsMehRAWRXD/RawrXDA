...
SECURITY_ATTRIBUTES STRUCT
    nLength              DWORD ?     ; offset 0
    _padA                DWORD ?     ; offset 4  (align QWORD to 8)
    lpSecurityDescriptor QWORD ?     ; offset 8
    bInheritHandle       DWORD ?     ; offset 16
    _padB                DWORD ?     ; offset 20 (total = 24)
...
main PROC FRAME
    ...
    mov     rcx, [hChildStdoutRd]
    mov     edx, HANDLE_FLAG_INHERIT  ; mask = which flags to change
...
@@zero_pi:
    ...
    ;=== Step 5b: Send didOpen to trigger diagnostics ===
    lea     rcx, [szDidOpenTemplate]
    call    jsonrpc_send

    ; Give server time to analyze the document
    mov     ecx, 500
    call    Sleep

    ; Non-blocking check: is diagnostic data available?
    mov     rcx, [hChildStdoutRd]
    xor     edx, edx                ; lpBuffer = NULL (peek only)
    xor     r8d, r8d                ; nBufferSize = 0
    xor     r9d, r9d                ; lpBytesRead = NULL
    lea     rax, [dwBytesRead]
    mov     QWORD PTR [rsp+20h], rax ; lpTotalBytesAvail
    mov     QWORD PTR [rsp+28h], 0   ; lpBytesLeftThisMessage
    call    PeekNamedPipe
    test    eax, eax
    jz      @@do_shutdown            ; peek failed, skip diagnostics
    cmp     DWORD PTR [dwBytesRead], 0
    je      @@do_shutdown            ; no data waiting, skip diagnostics

    ; Read the diagnostic notification
    lea     rcx, [responseBuf]
    mov     edx, 32000
    call    jsonrpc_recv
    test    rax, rax
...
@@no_response:
    ; Server was spawned but unresponsive — force-kill and clean up
...
@@spawn_fail:
    ; Close all pipe handles (server never started, all 4 still open)
    mov     rcx, [hChildStdoutRd]
    call    CloseHandle
    mov     rcx, [hChildStdoutWr]
    call    CloseHandle
    ...
    ; clangd not available, run in demo mode
    lea     rcx, [szSpawnFailDemo]
    mov     edx, szSpawnFailDemoLen
    call    write_con
    mov     DWORD PTR [bServerAlive], 0

    ; Demo: show synthetic capabilities
    lea     rcx, [szGotBody]
    mov     edx, szGotBodyLen
    call    write_con
    lea     rcx, [szDemoResponse]
    call    strlen_s
    mov     edx, eax
    lea     rcx, [szDemoResponse]
    call    write_con
    ...
    ; Demo: parse synthetic diagnostic
