...
; ============================================================================
; Walks section names for known packer signatures + checks entropy
DetectPacker proc USES esi edi ebx
    local i:DWORD

    mov dwDetectedPack, OBF_NONE
...
@@sec_loop:
    ...
    ; Copy 8-byte section name into bSecName
    lea edi, bSecName
    push esi
    mov ecx, 8
@@cpn:
    ...
    ; Check UPX0 / UPX1
    lea eax, bSecName
    cmp dword ptr [eax], 30585055h      ; "UPX0"
    je @@is_upx
    cmp dword ptr [eax], 31585055h      ; "UPX1"
    je @@is_upx

    ; Check VMProtect (.vmp0, .vmp1)
    cmp dword ptr [eax], 706D762Eh      ; ".vmp"
    je @@is_vmp

    ; Check Themida
    push eax
    invoke lstrcmpi, eax, addr szSecThemida
    test eax, eax
    pop eax
    jz @@is_themida

    ; Check ASPack
    cmp dword ptr [eax], 7061732Eh      ; ".spa" reversed = ".aspack" partial
    je @@is_aspack
    push eax
    invoke lstrcmpi, eax, addr szSecASPck2
    test eax, eax
    pop eax
    jz @@is_aspack

    ; Check PECompact
    cmp word ptr [eax], 4550h           ; "PE" as section name
    je @@is_pec

    ; Check MPRESS
    push eax
    invoke lstrcmpi, eax, addr szSecMPR
    test eax, eax
    pop eax
    jz @@is_mpress
...
@@is_upx:
    mov dwDetectedPack, OBF_UPX
    jmp @@check_rich
@@is_vmp:
    mov dwDetectedPack, OBF_VMPROTECT
    jmp @@check_rich
@@is_themida:
    mov dwDetectedPack, OBF_THEMIDA
    jmp @@check_rich
@@is_aspack:
    mov dwDetectedPack, OBF_ASPACK
    jmp @@check_rich
@@is_pec:
    mov dwDetectedPack, OBF_PECOMPACT
    jmp @@check_rich
@@is_mpress:
    mov dwDetectedPack, OBF_MPRESS

@@check_rich:
    ; --- Check for Rich header (MSVC toolchain indicator) ---
    mov bHasRichHeader, 0
    mov esi, pBase
    add esi, 80h                        ; Rich header starts after DOS stub
    mov ecx, 200h                       ; scan first 512 bytes

@@rich_scan:
    cmp ecx, 0
    jle @@check_linker
    cmp dword ptr [esi], 68636952h      ; "Rich"
    je @@rich_found
    inc esi
    dec ecx
    jmp @@rich_scan
...
@@rich_found:
    mov bHasRichHeader, 1

@@check_linker:
    ; --- Extract linker version from Optional Header ---
    mov esi, pOpt
    movzx eax, byte ptr [esi+2]         ; MajorLinkerVersion
    mov dwLinkerMajor, eax
    movzx eax, byte ptr [esi+3]         ; MinorLinkerVersion
    mov dwLinkerMinor, eax

    ret
DetectPacker endp

; ============================================================================
; DETECT SOURCE LANGUAGE (50-language fingerprinting)
; ============================================================================
; Strategy:
;   1. Check COM/.NET descriptor → C#/VB.NET/F#
;   2. Walk import DLL names, match against compiler runtime signatures
;   3. Check section names for language-specific patterns
;   4. Linker version heuristics
;   5. Rich header → MSVC (C/C++)
;   6. Default to C (most common PE language)
DetectLanguage proc USES esi edi ebx
    local pDesc:DWORD, pThunk:DWORD, lpDLL:DWORD

    mov dwDetectedLang, LANG_UNKNOWN
    mov dwDetectedFmt, MODE_PE

    ; ================================================================
    ; PHASE 1: Check .NET COM descriptor (DataDirectory[14])
    ; ================================================================
    mov esi, pOpt
    mov eax, [esi + DD_COM_RVA]
    mov dwCOMDescRVA, eax
    test eax, eax
    jz @@phase2

    ; Has COM descriptor → .NET managed binary
    ; Could be C#, VB.NET, or F# — default C# (most common)
    mov dwDetectedLang, LANG_CSHARP
    ; We'll refine in Phase 2 if we find VB or F# indicators
    jmp @@phase2

    ; ================================================================
    ; PHASE 2: Walk import DLLs for language signatures
    ; ================================================================
@@phase2:
    mov esi, pOpt
    mov eax, [esi + DD_IMPORT_RVA]
...
@@dll_loop:
    mov esi, pDesc
    mov eax, [esi+12]                   ; Import descriptor Name RVA
    test eax, eax
    jz @@phase3                         ; end of descriptors
    ...
    ; --- Match DLL name against known signatures ---

    ; .NET runtime: mscoree.dll → C#/F#/VB.NET
    invoke lstrcmpi, lpDLL, addr szSigMscoree
    test eax, eax
    jz @@match_dotnet

    ; Python runtime
    push lpDLL
    invoke lstrcmpi, lpDLL, addr szSigPython3
    test eax, eax
    pop lpDLL
    jz @@match_python
    ; Check "python" substring in DLL name
    invoke lstrlen, lpDLL
    push eax
    push lpDLL
    call @@check_python_sub
    pop lpDLL
    pop eax

    ; Go runtime — look for Go-specific section names later
    ; Delphi: borlndmm.dll
    invoke lstrcmpi, lpDLL, addr szSigDelphi
    test eax, eax
    jz @@match_delphi

    ; Nim: look for "NimMain" in exports/strings
    ; Dart/Flutter
    push lpDLL
    invoke lstrcmpi, lpDLL, addr szSigDart
    test eax, eax
    pop lpDLL
    jz @@match_dart
    ...
    ; Ruby
    push lpDLL
    call @@check_substr_ruby
    pop lpDLL

    ; Lua
    push lpDLL
    call @@check_substr_lua
    pop lpDLL

    ; PHP
    push lpDLL
    call @@check_substr_php
    pop lpDLL

    ; Perl
    push lpDLL
    call @@check_substr_perl
    pop lpDLL

    ; Julia
    push lpDLL
    call @@check_substr_julia
    pop lpDLL

    ; R
    invoke lstrcmpi, lpDLL, addr szSigR
    test eax, eax
    jz @@match_r

    ; MATLAB
    push lpDLL
    call @@check_substr_matlab
    pop lpDLL

    ; Fortran (gfortran)
    push lpDLL
    call @@check_substr_fortran
    pop lpDLL

    ; Ada (GNAT)
    push lpDLL
    call @@check_substr_ada
    pop lpDLL

    ; Haskell (GHC)
    push lpDLL
    call @@check_substr_haskell
    pop lpDLL

    ; OCaml
    push lpDLL
    call @@check_substr_ocaml
    pop lpDLL

    ; COBOL
    push lpDLL
    call @@check_substr_cobol
    pop lpDLL

    ; UCRT / VCRUNTIME → C/C++ (MSVC)
    push lpDLL
    invoke lstrcmpi, lpDLL, addr szSigUCRT
    test eax, eax
    pop lpDLL
    jz @@match_c_or_cpp

    push lpDLL
    call @@check_substr_vcrt
    pop lpDLL

    ; MFC → C++ (MFC app)
    push lpDLL
    call @@check_substr_mfc
    pop lpDLL
...
@@next_dll:
    ...
    ; --- Internal substring helpers (uses edi as search target) ---
@@check_python_sub:
    mov edi, lpDLL
    push edi
    invoke lstrcmpi, edi, addr szSigPython
    test eax, eax
    pop edi
    jnz @@ret_sub
    mov dwDetectedLang, LANG_PYTHON
@@ret_sub:
    ret
...
@@check_substr_ruby:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigRubyDll
    test eax, eax
    pop edi
    jnz @@ret_sub2
    mov dwDetectedLang, LANG_RUBY
@@ret_sub2:
    ret
...
@@check_substr_lua:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigLua
    test eax, eax
    pop edi
    jnz @@ret_sub3
    mov dwDetectedLang, LANG_LUA
@@ret_sub3:
    ret
...
@@check_substr_php:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigPHP
    test eax, eax
    pop edi
    jnz @@ret_sub4
    mov dwDetectedLang, LANG_PHP
@@ret_sub4:
    ret
...
@@check_substr_perl:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigPerl2
    test eax, eax
    pop edi
    jnz @@ret_sub5
    mov dwDetectedLang, LANG_PERL
@@ret_sub5:
    ret
...
@@check_substr_julia:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigJulia
    test eax, eax
    pop edi
    jnz @@ret_sub6
    mov dwDetectedLang, LANG_JULIA
@@ret_sub6:
    ret
...
@@check_substr_matlab:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigMatlab
    test eax, eax
    pop edi
    jnz @@ret_sub7
    mov dwDetectedLang, LANG_MATLAB
@@ret_sub7:
    ret

@@check_substr_fortran:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigFortran
    test eax, eax
    pop edi
    jnz @@ret_sub8
    mov dwDetectedLang, LANG_FORTRAN
@@ret_sub8:
    ret
...
@@check_substr_ada:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigGNAT
    test eax, eax
    pop edi
    jnz @@ret_sub9
    mov dwDetectedLang, LANG_ADA
@@ret_sub9:
    ret

@@check_substr_haskell:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigHaskellRT
    test eax, eax
    pop edi
    jnz @@ret_sub10
    mov dwDetectedLang, LANG_HASKELL
@@ret_sub10:
    ret
...
@@check_substr_ocaml:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigOCaml
    test eax, eax
    pop edi
    jnz @@ret_sub11
    mov dwDetectedLang, LANG_OCAML
@@ret_sub11:
    ret
...
@@check_substr_cobol:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigCOBOL
    test eax, eax
    pop edi
    jnz @@ret_sub12
    mov dwDetectedLang, LANG_COBOL
@@ret_sub12:
    ret
...
@@check_substr_vcrt:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigVCRT
    test eax, eax
    pop edi
    jnz @@ret_sub13
    mov dwDetectedLang, LANG_CPP
@@ret_sub13:
    ret
...
@@check_substr_mfc:
    mov edi, [esp+4]
    push edi
    invoke lstrcmpi, edi, addr szSigMFC
    test eax, eax
    pop edi
    jnz @@ret_sub14
    mov dwDetectedLang, LANG_CPP
@@ret_sub14:
    ...
    ; --- Match handlers ---
@@match_dotnet:
    ; Already set to LANG_CSHARP if COM descriptor present
    cmp dwDetectedLang, LANG_CSHARP
    je @@next_dll
    mov dwDetectedLang, LANG_CSHARP
    jmp @@next_dll
...
@@match_delphi:
    mov dwDetectedLang, LANG_DELPHI
    jmp @@next_dll

@@match_dart:
    mov dwDetectedLang, LANG_DART
    jmp @@next_dll

@@match_r:
    mov dwDetectedLang, LANG_R
    jmp @@next_dll

@@match_c_or_cpp:
    ; UCRT → could be C or C++, check for VCRUNTIME or MFC to disambiguate
    cmp dwDetectedLang, LANG_UNKNOWN
    jne @@next_dll
    mov dwDetectedLang, LANG_C
    jmp @@next_dll

    ; ================================================================
    ; PHASE 3: Section name heuristics
    ; ================================================================
@@phase3:
    ; Check for Go sections (.symtab, .gosymtab, runtime.*)
    mov esi, pSecHdr
    xor edi, edi
@@sec_scan:
    cmp edi, dwNumSec
...
@@cpn2:
    ...
    ; Check Go runtime sections
    lea eax, bSecName
    cmp dword ptr [eax], 6F672Eh        ; ".go" prefix
    je @@match_go

    ; Check Rust sections (.rdata with Rust patterns)
    ; This is a weak heuristic — we already handle Rust via import names

    add esi, SEC_HDR_SIZE
    inc edi
    jmp @@sec_scan

@@match_go:
    mov dwDetectedLang, LANG_GO
    jmp @@phase4

    ; ================================================================
    ; PHASE 4: Linker version + Rich header fallback
    ; ================================================================
@@phase4:
    cmp dwDetectedLang, LANG_UNKNOWN
    jne @@display

    ; If Rich header present → MSVC → C or C++
    cmp bHasRichHeader, 1
    jne @@default
    mov dwDetectedLang, LANG_C          ; default to C for MSVC without other signals
    jmp @@display

@@default:
    ; MinGW / unknown → default C
    mov dwDetectedLang, LANG_C
    ...
    ; ================================================================
@@display:
    invoke Print, addr szHdrLang

    ; Binary format
    mov eax, dwDetectedFmt
    mov eax, [FmtNames+eax*4]
    invoke wsprintf, addr bBuffer, addr szFmtLang, eax
    invoke Print, addr bBuffer

    ; Source language
    mov eax, dwDetectedLang
    cmp eax, MAX_LANGUAGES
    jl @@lang_ok
    xor eax, eax
@@lang_ok:
    mov eax, [LangNames+eax*4]
    invoke wsprintf, addr bBuffer, addr szFmtComp, eax
    invoke Print, addr bBuffer

    ; Packer
    mov eax, dwDetectedPack
    mov eax, [ObfNames+eax*4]
    invoke wsprintf, addr bBuffer, addr szFmtPack, eax
    invoke Print, addr bBuffer

    ; Linker version
    invoke wsprintf, addr bBuffer, addr szFmtLinker, dwLinkerMajor, dwLinkerMinor
    invoke Print, addr bBuffer

    ; Rich header
    cmp bHasRichHeader, 1
    je @@rich_y
    invoke wsprintf, addr bBuffer, addr szFmtRich, addr szRichNo
    jmp @@rich_show
@@rich_y:
    invoke wsprintf, addr bBuffer, addr szFmtRich, addr szRichYes
@@rich_show:
    invoke Print, addr bBuffer
...
; ============================================================================
; LANGUAGE-AWARE SOURCE RECONSTRUCTION
; ============================================================================
; Emits pseudo-source in the detected language's syntax
SourceReconLang proc USES esi edi ebx
    local dwBytesLeft:DWORD, lpDeclFmt:DWORD, lpEndFmt:DWORD
    local lpComment:DWORD

    invoke Print, addr szHdrSrc

    ; Select language-specific format strings
    mov eax, dwDetectedLang
    ...
    cmp eax, LANG_ADA
    je @@fmt_ada
    cmp eax, LANG_FORTRAN
    je @@fmt_ftn
    cmp eax, LANG_HASKELL
    je @@fmt_hs
    cmp eax, LANG_FSHARP
    je @@fmt_hs
    cmp eax, LANG_OCAML
...
@@fmt_c:
    mov lpComment, offset szSrcC
    mov lpDeclFmt, offset szDeclC
    mov lpEndFmt, offset szEndBrace
    jmp @@do_recon
@@fmt_cpp:
    mov lpComment, offset szSrcCpp
    mov lpDeclFmt, offset szDeclCpp
    mov lpEndFmt, offset szEndBrace
    jmp @@do_recon
@@fmt_cs:
    mov lpComment, offset szSrcCS
    mov lpDeclFmt, offset szDeclCS
    mov lpEndFmt, offset szEndBrace
    jmp @@do_recon
@@fmt_go:
    mov lpComment, offset szSrcGo
    mov lpDeclFmt, offset szDeclGo
    mov lpEndFmt, offset szEndBrace
    jmp @@do_recon
@@fmt_rust:
    mov lpComment, offset szSrcRust
    mov lpDeclFmt, offset szDeclRust
    mov lpEndFmt, offset szEndBrace
    jmp @@do_recon
@@fmt_delphi:
    mov lpComment, offset szSrcDelphi
    mov lpDeclFmt, offset szDeclDelph
    mov lpEndFmt, offset szEndDelph
    jmp @@do_recon
@@fmt_py:
    mov lpComment, offset szSrcPy
    mov lpDeclFmt, offset szDeclPy
    mov lpEndFmt, offset szEndPy
    jmp @@do_recon
@@fmt_java:
    mov lpComment, offset szSrcJava
    mov lpDeclFmt, offset szDeclJava
    mov lpEndFmt, offset szEndBrace
    jmp @@do_recon
@@fmt_nim:
    mov lpComment, offset szSrcNim
    mov lpDeclFmt, offset szDeclNim
    mov lpEndFmt, offset szEndNim
    jmp @@do_recon
@@fmt_zig:
    mov lpComment, offset szSrcZig
    mov lpDeclFmt, offset szDeclZig
    mov lpEndFmt, offset szEndBrace
    jmp @@do_recon
@@fmt_swift:
    mov lpComment, offset szSrcSwift
    mov lpDeclFmt, offset szDeclSwift
    mov lpEndFmt, offset szEndBrace
    jmp @@do_recon
@@fmt_ada:
    mov lpComment, offset szSrcAda
    mov lpDeclFmt, offset szDeclAda
    mov lpEndFmt, offset szEndFtn
    jmp @@do_recon
@@fmt_ftn:
    mov lpComment, offset szSrcFtn
    mov lpDeclFmt, offset szDeclFtn
    mov lpEndFmt, offset szEndFtn
    jmp @@do_recon
@@fmt_hs:
    mov lpComment, offset szSrcHS
    mov lpDeclFmt, offset szDeclHS
    mov lpEndFmt, offset szEndPy
    jmp @@do_recon
@@fmt_ocaml:
    mov lpComment, offset szSrcOCaml
    mov lpDeclFmt, offset szDeclOCaml
    mov lpEndFmt, offset szEndNim

@@do_recon:
    ; Print language comment header
    invoke Print, lpComment
    ...
    ; Function header in target language
    invoke wsprintf, addr bBuffer, lpDeclFmt, dwEntryPt, dwEntryPt
    invoke Print, addr bBuffer
...
@@scan:
    ...
    ; Map opcodes to language-appropriate pseudo-statements
    cmp al, 55h                     ; push ebp
    je @@op_push
    cmp al, 5Dh                     ; pop ebp
    je @@op_pop
    cmp al, 83h                     ; sub/add esp
    je @@op_sub
    cmp al, 8Bh                     ; mov r32, r/m32
    je @@op_lea
    cmp al, 89h                     ; mov r/m32, r32
...
@@op_jmp:
    ...
    invoke wsprintf, addr bBuffer, addr szFmtOp, addr bTemp
    invoke Print, addr bBuffer
    jmp @@endblock                  ; unconditional jump → stop
...
@@endblock:
    invoke Print, lpEndFmt

@@done:
    ret
SourceReconLang endp
...
@@lang:
    ...
    ; --- [14] Language-Aware Source Recon ---
@@langsrc:
    cmp bFileLoaded, 0
    je @@nofile
    call DetectPacker
