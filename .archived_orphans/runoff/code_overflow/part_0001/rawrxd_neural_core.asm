...
@@loop:
    ...
    cmp eax, 256
    jae @@vocab_lookup          ; Token IDs >= 256 need vocab lookup
...
@@vocab_lookup:
    ; Reverse hash lookup: token_id -> string in vocab table
    ; vocab_hash_table layout: [entry_count:4][hash_entries: id(4) + offset(4) + len(2) ...]
    ; Each vocab string is stored at [vocab_base + offset]
    mov rdi, [rbx].TOKENIZER.pVocabTable   ; vocab hash table base
    test rdi, rdi
    jz @@skip_done                          ; no vocab table loaded
    
    mov ecx, [rdi]                          ; entry_count
    lea rdi, [rdi+4]                        ; entries start
    
@@vocab_scan:
    test ecx, ecx
    jz @@skip_done                          ; not found
    cmp eax, [rdi]                          ; compare token ID
    je @@vocab_found
    add rdi, 10                             ; next entry (id:4 + offset:4 + len:2)
    dec ecx
    jmp @@vocab_scan
    
@@vocab_found:
    mov ecx, [rdi+4]                        ; string offset into vocab data
    movzx edx, word ptr [rdi+8]             ; string length
    mov rdi, [rbx].TOKENIZER.pVocabData     ; vocab string data base
    test rdi, rdi
    jz @@skip_done
    add rdi, rcx                            ; pointer to token string
    
    ; Copy token string to output buffer
@@vocab_copy:
    test edx, edx
    jz @@skip_done
    mov cl, [rdi]
