...
// Bridge: route unified dispatch output to IDE output panel (implemented in Win32IDE_Core.cpp)
extern void Win32IDE_AppendOutputFromUnified(void* idePtr, const char* text);

// Output callback type for GUI context — routes to IDE output panel via WM_AGENT_OUTPUT_SAFE
static void gui_status_output(const char* text, void* userData) {
    if (!text || !userData) return;
    Win32IDE_AppendOutputFromUnified(userData, text);
}
