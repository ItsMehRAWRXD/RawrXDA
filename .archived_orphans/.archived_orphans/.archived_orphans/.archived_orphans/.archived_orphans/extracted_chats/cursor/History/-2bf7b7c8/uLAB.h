...
struct OptimizationConfig {
    ...
    // Low-rank factorization settings (SVD-based compression)
    struct {
        bool enable = false;
        double rank_ratio = 0.15;       // Rank = rank_ratio × min(M,N) (0.1-0.3 typical)
        double energy_threshold = 0.95; // Keep rank capturing >95% energy
        std::vector<std::string> target_layers;  // Layers to apply to (empty = all applicable)
        std::map<std::string, double> layer_rank_ratios;  // Per-layer rank ratios
    } low_rank;
    
    // Activation quantization/compression settings
    struct {
        bool enable = false;
        uint8_t activation_bits = 8;    // 8-bit activations (INT8)
        bool dynamic_quantization = true;  // Quantize on-the-fly vs. calibrated
        double calibration_samples = 1000; // Samples for calibration
        std::vector<std::string> skip_layers;  // Layers to skip (sensitive layers)
    } activation_quantization;
    
    // Gradient checkpointing / activation recomputation
    struct {
        bool enable = false;
        size_t checkpoint_interval = 4; // Checkpoint every N layers
        bool selective_checkpointing = true;  // Only checkpoint critical layers
        std::vector<size_t> checkpoint_layers;  // Specific layers to checkpoint
    } checkpointing;
    
    // Structured pruning settings (hardware-friendly)
    struct {
        bool enable = false;
        enum PruningType {
            CHANNEL,      // Remove entire channels
            FILTER,       // Remove entire filters
            BLOCK,        // Remove block structures
            PATTERN       // Remove specific patterns
        } pruning_type = CHANNEL;
        double structured_sparsity = 0.60;  // Target structured sparsity
        std::map<std::string, double> layer_structured_sparsity;  // Per-layer sparsity
    } structured_pruning;
    
    // Hybrid pruning (combines structured + unstructured)
    struct {
        bool enable = false;
        double structured_ratio = 0.50;  // 50% structured, 50% unstructured
        double total_sparsity = 0.85;    // Overall sparsity target
        std::map<std::string, double> layer_hybrid_ratios;  // Per-layer hybrid ratios
    } hybrid_pruning;
    
    // Knowledge distillation preparation
    struct {
        bool enable = false;
        double student_size_ratio = 0.10;  // Student = 10% of teacher size
        bool extract_attention_patterns = true;  // Extract attention for transfer
        bool extract_feature_maps = true;        // Extract intermediate features
        std::vector<std::string> critical_teacher_layers;  // Teacher layers to preserve
    } distillation_prep;
    
    // Layer-wise optimization overrides
    std::map<std::string, OptimizationConfig> layer_configs;  // Per-layer specific configs
};
...
public:
    ...
    /**
     * @brief Apply low-rank factorization to weight matrices
     */
    bool ApplyLowRankFactorization(IGGUFLoader* loader,
                                   const OptimizationConfig& config,
                                   OptimizationStats& stats);
    
    /**
     * @brief Apply activation quantization
     */
    bool ApplyActivationQuantization(IGGUFLoader* loader,
                                    const OptimizationConfig& config,
                                    OptimizationStats& stats);
    
    /**
     * @brief Setup gradient checkpointing
     */
    bool ApplyCheckpointing(IGGUFLoader* loader,
                           const OptimizationConfig& config,
                           OptimizationStats& stats);
    
    /**
     * @brief Apply structured pruning
     */
    bool ApplyStructuredPruning(IGGUFLoader* loader,
                               const OptimizationConfig& config,
                               OptimizationStats& stats);
    
    /**
     * @brief Apply hybrid pruning (structured + unstructured)
     */
    bool ApplyHybridPruning(IGGUFLoader* loader,
                           const OptimizationConfig& config,
                           OptimizationStats& stats);
    
    /**
     * @brief Calculate optimal rank for low-rank factorization
     */
    size_t CalculateOptimalRank(size_t m, size_t n, double rank_ratio, double energy_threshold) const;
    
    /**
     * @brief Estimate memory savings from low-rank factorization
     */
    size_t EstimateLowRankSize(size_t original_size, size_t rank, size_t m, size_t n) const;
