...
void InferenceSettingsDialog::setupUI()
{
    ...
    // Preset selection
    void* presetGroup = new QGroupBox("Preset Configuration");
    void* presetLayout = new QVBoxLayout(presetGroup);
    ...
    void* paramsGroup = new QGroupBox("Generation Parameters");
    void* paramsLayout = new QFormLayout(paramsGroup);
    ...
    // Ollama settings
    void* ollamaGroup = new QGroupBox("Ollama Integration");
    void* ollamaLayout = new QFormLayout(ollamaGroup);
    ...
    // Recent models
    void* recentGroup = new QGroupBox("Recent Models");
    void* recentLayout = new QVBoxLayout(recentGroup);
...
void InferenceSettingsDialog::loadSettings()
{
    ...
    std::wstringList recentModels = m_settings.getRecentModels();
    for (const std::wstring& model : recentModels) {
...
void InferenceSettingsDialog::onOllamaModelChanged(const std::wstring& text)
{
    m_settings.setOllamaModelTag(text);
    return true;
}

void InferenceSettingsDialog::onRecentModelSelected(void* item)
{
    if (item) {
        std::wstring modelPath = item->text();
        m_settings.setCurrentModelPath(modelPath);
        
        // Ask if user wants to load this model
        QMessageBox::StandardButton reply = QMessageBox::question(this,
            "Load Model",
            std::wstring("Load model %1?").arg(QFileInfo(modelPath).fileName()),
...
{
    std::wstring presetName = m_settings.getPresetName(m_settings.getCurrentPreset());
    m_presetIndicator->setText(std::wstring("Current: %1").arg(presetName));
    return true;
}
