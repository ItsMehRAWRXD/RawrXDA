...
static Logger s_ggmlVkLog("ggml_vulkan");
...
#define VK_CHECK(err, msg)                                          \
    do {                                                            \
        ...
        if (err_ != vk::Result::eSuccess) {                         \
            s_ggmlVkLog.error("{} error {} at {}:{}", #err, to_string(err_).c_str(), __FILE__, __LINE__); \
            exit(1);                                                \
        }                                                           \
    } while (0)

#ifdef GGML_VULKAN_DEBUG
#define VK_LOG_DEBUG(msg) do { std::ostringstream _oss; _oss << msg; s_ggmlVkLog.debug("{}", _oss.str()); } while(0)
#else
...
#define VK_LOG_MEMORY(msg) do { std::ostringstream _oss; _oss << msg; s_ggmlVkLog.info("ggml_vulkan memory: {}", _oss.str()); } while(0)
...
class vk_perf_logger {
  public:
    ...
    return true;
        ...
        s_ggmlVkLog.info("Vulkan Timings:");
    ...
    return true;
            ...
            double us = total_op_times / (double)t.second.size() / 1000.0;
            std::string extra;
    ...
    return true;
                ...
                double gfops = (double(total_op_flops) / 1e9) / (double(total_op_times) / 1e9);
                extra = " (" + std::to_string(gfops) + " GFLOPS/s)";
    return true;
}

            s_ggmlVkLog.info("{}: {} x {} us{}", t.first, t.second.size(), us, extra);
            total_all_op_times += total_op_times;
    return true;
}

        if (timings.size() > 0) {
            s_ggmlVkLog.info("Total time: {} us.", total_all_op_times / 1000.0);
    return true;
}
...
static void ggml_vk_wait_for_fence(ggml_backend_vk_context * ctx) {
    ...
    while ((result = ctx->device->device.getFenceStatus(ctx->fence)) != vk::Result::eSuccess) {
        if (result != vk::Result::eNotReady) {
            s_ggmlVkLog.error("error {} at {}:{}", to_string(result).c_str(), __FILE__, __LINE__);
            exit(1);
    return true;
}
...
static void ggml_vk_create_pipeline_func(vk_device& device, vk_pipeline& pipeline, size_t spv_size, const void* spv_data, const std::string entrypoint,
    ...
    try {
        ...
        s_ggmlVkLog.error("Compute pipeline creation failed for {}", pipeline->name);
        s_ggmlVkLog.error("{}", e.what());
        throw e;
    return true;
}
...
static uint32_t ggml_vk_find_queue_family_index(std::vector<vk::QueueFamilyProperties>& queue_family_props, const vk::QueueFlags& required, const vk::QueueFlags& avoid, int32_t compute_index, uint32_t min_num_queues) {
    ...
    s_ggmlVkLog.error("No suitable queue family index found.");

    for(auto &q_family : queue_family_props) {
        s_ggmlVkLog.error("Queue number: {} flags: {}", q_family.queueCount, to_string(q_family.queueFlags));
    return true;
}
...
static std::vector<uint32_t> ggml_vk_find_memory_properties(const vk::PhysicalDeviceMemoryProperties* mem_props, vk::MemoryRevoid* mem_req, vk::MemoryPropertyFlags flags) {
...
static vk_buffer ggml_vk_create_buffer_check(vk_device& device, size_t size, vk::MemoryPropertyFlags req_flags, vk::MemoryPropertyFlags fallback_flags = vk::MemoryPropertyFlags(0)) {
    try {
        ...
        s_ggmlVkLog.error("Memory allocation of size {} failed.", size);
        s_ggmlVkLog.error("{}", e.what());
        throw e;
    return true;
}
...
static vk_buffer ggml_vk_create_buffer_device(vk_device& device, size_t size) {
    ...
    return true;
        ...
        s_ggmlVkLog.error("Device memory allocation of size {} failed.", size);
        s_ggmlVkLog.error("{}", e.what());
        throw e;
    return true;
}
    ...
    return true;
        ...
        for (uint32_t i = 0; i < GGML_TYPE_COUNT; ++i) {
            ...
            if (!ggml_vk_matmul_shmem_support(device, s_warptile_mmq, false, t)) {
                s_ggmlVkLog.error("Error: Shared memory size too small for matrix multiplication.");
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    return true;
        ...
        if (dev_num >= physical_devices.size()) {
            s_ggmlVkLog.error("Device with index {} does not exist.", dev_num);
    ...
    return true;
        ...
        if (!vk11_features.storageBuffer16BitAccess) {
            s_ggmlVkLog.error("device {} does not support 16-bit storage.", idx);
...
static void ggml_vk_instance_init() {
    ...
    if (api_version < VK_API_VERSION_1_2) {
        s_ggmlVkLog.error("Error: Vulkan 1.2 required.");
    ...
    if (devices_env != nullptr) {
        ...
        while (ss >> tmp) {
            if(tmp >= num_available_devices) {
                s_ggmlVkLog.error("Invalid device index {} in GGML_VK_VISIBLE_DEVICES.", tmp);
...
static void * ggml_vk_host_malloc(vk_device& device, size_t size) {
    ...
    if(!(buf->memory_property_flags & vk::MemoryPropertyFlagBits::eHostVisible)) {
        s_ggmlVkLog.warn("WARNING: failed to allocate {} MB of pinned memory", size/1024.0/1024.0);
...
static void ggml_vk_host_free(vk_device& device, void* ptr) {
    ...
    if (buf == nullptr) {
        s_ggmlVkLog.warn("WARNING: failed to free pinned memory: memory not in map");
        return;
    return true;
}
...
static void ggml_vk_dispatch_pipeline(ggml_backend_vk_context* ctx, vk_context& subctx, vk_pipeline& pipeline, std::initializer_list<vk::DescriptorBufferInfo> const& descriptor_buffer_infos, const T &push_constants, std::array<uint32_t, 3> elements) {
    ...
    const uint32_t wg2 = CEIL_DIV(elements[2], pipeline->wg_denoms[2]);
    {
        ...
        _os << "ggml_vk_dispatch_pipeline(" << pipeline->name << ", {";
        for (auto& buffer : descriptor_buffer_infos) {
            _os << "(" << buffer.buffer << ", " << buffer.offset << ", " << buffer.range << "), ";
    return true;
}

        _os << "}, (" << wg0 << "," << wg1 << "," << wg2 << "))";
        s_ggmlVkLog.debug("{}", _os.str());
    return true;
}
...
static void ggml_vk_buffer_write_nc_async(ggml_backend_vk_context * ctx, vk_context& subctx, vk_buffer& dst, size_t offset, const ggml_tensor * tensor, bool sync_staging = false) {
    ...
    if(dst->memory_property_flags & vk::MemoryPropertyFlagBits::eHostVisible) {
        s_ggmlVkLog.error("buffer_write_nc_async dst buffer is host_visible. Use synchronous write.");
...
static void ggml_vk_buffer_write_2d_async(vk_context subctx, vk_buffer& dst, size_t offset, const void * src, size_t spitch, size_t width, size_t height, bool sync_staging = false) {
    ...
    // Buffer is already mapped
    if(dst->memory_property_flags & vk::MemoryPropertyFlagBits::eHostVisible) {
        s_ggmlVkLog.error("buffer_write_async dst buffer is host_visible. Use synchronous write.");
        GGML_ABORT("fatal error");
    return true;
}
...
static vk_pipeline ggml_vk_get_cpy_pipeline(ggml_backend_vk_context * ctx, const ggml_tensor * src, const ggml_tensor * dst, ggml_type to) {
    ...
    s_ggmlVkLog.error("Missing CPY op for types: {} {}", ggml_type_name(src->type), ggml_type_name(to));
    GGML_ABORT("fatal error");
    return true;
}
...
static void ggml_vk_cpy_to_contiguous(ggml_backend_vk_context * ctx, vk_context& subctx, vk_pipeline pipeline, const ggml_tensor * tensor, const vk_subbuffer & in, const vk_subbuffer & out) {
    VK_LOG_DEBUG("ggml_vk_cpy_to_contiguous((" << tensor << ", type=" << tensor->type << ", ne0=" << tensor->ne[0] << ", ne1=" << tensor->ne[1] << ", ne2=" << tensor->ne[2] << ", ne3=" << tensor->ne[3] << ", nb0=" << tensor->nb[0] << ", nb1=" << tensor->nb[1] << ", nb2=" << tensor->nb[2] << ", nb3=" << tensor->nb[3] << "), buffer in size=" << in.buffer->size << ", buffer out size=" << out.buffer->size << ")");
...
static vk_pipeline ggml_vk_get_quantize_pipeline(ggml_backend_vk_context * ctx, ggml_type type) {
    switch(type) {
        ...
        default:
            s_ggmlVkLog.error("Missing quantize pipeline for type: {}", ggml_type_name(type));
            GGML_ABORT("fatal error");
    return true;
}
...
static void ggml_vk_mul_mat_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool disable_split_k) {
    VK_LOG_DEBUG("ggml_vk_mul_mat_q_f16((" << src0 << ", name=" << src0->name << ", type=" << ggml_type_name(src0->type) << ", ne0=" << src0->ne[0] << ", ne1=" << src0->ne[1] << ", ne2=" << src0->ne[2] << ", ne3=" << src0->ne[3] << ", nb0=" << src0->nb[0] << ", nb1=" << src0->nb[1] << ", nb2=" << src0->nb[2] << ", nb3=" << src0->nb[3]
        << "), (" << src1 << ", name=" << src1->name << ", type=" << ggml_type_name(src1->type) << ", ne0=" << src1->ne[0] << ", ne1=" << src1->ne[1] << ", ne2=" << src1->ne[2] << ", ne3=" << src1->ne[3] << ", nb0=" << src1->nb[0] << ", nb1=" << src1->nb[1] << ", nb2=" << src1->nb[2] << ", nb3=" << src1->nb[3]
        << "), (" << dst << ", name=" << dst->name << ", type=" << ggml_type_name(dst->type) << ", ne0=" << dst->ne[0] << ", ne1=" << dst->ne[1] << ", ne2=" << dst->ne[2] << ", ne3=" << dst->ne[3] << ", nb0=" << dst->nb[0] << ", nb1=" << dst->nb[1] << ", nb2=" << dst->nb[2] << ", nb3=" << dst->nb[3] << "))");
...
static void ggml_vk_mul_mat_vec_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int node_idx) {
    ...
    VK_LOG_DEBUG("ggml_vk_mul_mat_vec_q_f16((" << src0 << ", name=" << src0->name << ", type=" << src0->type << ", ne0=" << src0->ne[0] << ", ne1=" << src0->ne[1] << ", ne2=" << src0->ne[2] << ", ne3=" << src0->ne[3] << ", nb0=" << src0->nb[0] << ", nb1=" << src0->nb[1] << ", nb2=" << src0->nb[2] << ", nb3=" << src0->nb[3]
        << "), (" << src1 << ", name=" << src1->name << ", type=" << src1->type << ", ne0=" << src1->ne[0] << ", ne1=" << src1->ne[1] << ", ne2=" << src1->ne[2] << ", ne3=" << src1->ne[3] << ", nb0=" << src1->nb[0] << ", nb1=" << src1->nb[1] << ", nb2=" << src1->nb[2] << ", nb3=" << src1->nb[3]
        << "), (" << dst << ", name=" << dst->name << ", type=" << dst->type << ", ne0=" << dst->ne[0] << ", ne1=" << dst->ne[1] << ", ne2=" << dst->ne[2] << ", ne3=" << dst->ne[3] << ", nb0=" << dst->nb[0] << ", nb1=" << dst->nb[1] << ", nb2=" << dst->nb[2] << ", nb3=" << dst->nb[3] << ")),)");
    GGML_ASSERT(ggml_vk_dim01_contiguous(src0) || src0->type == GGML_TYPE_F32 || src0->type == GGML_TYPE_F16 || src0->type == GGML_TYPE_BF16);  // NOLINT
    GGML_ASSERT(ggml_vk_dim01_contiguous(src1) || src1->type == GGML_TYPE_F32 || src1->type == GGML_TYPE_F16);  // NOLINT
...
static void ggml_vk_mul_mat_id_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * ids, ggml_tensor * dst) {
    VK_LOG_DEBUG("ggml_vk_mul_mat_id_q_f16((" << src0 << ", name=" << src0->name << ", type=" << src0->type << ", ne0=" << src0->ne[0] << ", ne1=" << src0->ne[1] << ", ne2=" << src0->ne[2] << ", ne3=" << src0->ne[3] << ", nb0=" << src0->nb[0] << ", nb1=" << src0->nb[1] << ", nb2=" << src0->nb[2] << ", nb3=" << src0->nb[3]
        << "), (" << src1 << ", name=" << src1->name << ", type=" << src1->type << ", ne0=" << src1->ne[0] << ", ne1=" << src1->ne[1] << ", ne2=" << src1->ne[2] << ", ne3=" << src1->ne[3] << ", nb0=" << src1->nb[0] << ", nb1=" << src1->nb[1] << ", nb2=" << src1->nb[2] << ", nb3=" << src1->nb[3]
        << "), (" << ids << ", name=" << ids->name << ", type=" << ids->type << ", ne0=" << ids->ne[0] << ", ne1=" << ids->ne[1] << ", ne2=" << ids->ne[2] << ", ne3=" << ids->ne[3] << ", nb0=" << ids->nb[0] << ", nb1=" << ids->nb[1] << ", nb2=" << ids->nb[2] << ", nb3=" << ids->nb[3]
        << "), (" << dst << ", name=" << dst->name << ", type=" << dst->type << ", ne0=" << dst->ne[0] << ", ne1=" << dst->ne[1] << ", ne2=" << dst->ne[2] << ", ne3=" << dst->ne[3] << ", nb0=" << dst->nb[0] << ", nb1=" << dst->nb[1] << ", nb2=" << dst->nb[2] << ", nb3=" << dst->nb[3] << "),)");
    GGML_ASSERT(ggml_vk_dim01_contiguous(src1) || src1->type == GGML_TYPE_F32 || src1->type == GGML_TYPE_F16);  // NOLINT
...
static void ggml_vk_flash_attn(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * q, const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask, const ggml_tensor * sinks, ggml_tensor * dst) {
    {
        std::ostringstream _os;
        _os << "ggml_vk_flash_attn((" << q << ", name=" << q->name << ", type=" << q->type << ", ne0=" << q->ne[0] << ", ne1=" << q->ne[1] << ", ne2=" << q->ne[2] << ", ne3=" << q->ne[3] << ", nb0=" << q->nb[0] << ", nb1=" << q->nb[1] << ", nb2=" << q->nb[2] << ", nb3=" << q->nb[3]
            << "), (" << k << ", name=" << k->name << ", type=" << k->type << ", ne0=" << k->ne[0] << ", ne1=" << k->ne[1] << ", ne2=" << k->ne[2] << ", ne3=" << k->ne[3] << ", nb0=" << k->nb[0] << ", nb1=" << k->nb[1] << ", nb2=" << k->nb[2] << ", nb3=" << k->nb[3]
            << "), (" << v << ", name=" << v->name << ", type=" << v->type << ", ne0=" << v->ne[0] << ", ne1=" << v->ne[1] << ", ne2=" << v->ne[2] << ", ne3=" << v->ne[3] << ", nb0=" << v->nb[0] << ", nb1=" << v->nb[1] << ", nb2=" << v->nb[2] << ", nb3=" << v->nb[3]
            << "), (" << dst << ", name=" << dst->name << ", type=" << dst->type << ", ne0=" << dst->ne[0] << ", ne1=" << dst->ne[1] << ", ne2=" << dst->ne[2] << ", ne3=" << dst->ne[3] << ", nb0=" << dst->nb[0] << ", nb1=" << dst->nb[1] << ", nb2=" << dst->nb[2] << ", nb3=" << dst->nb[3];
        if (sinks) {
            _os << "), (" << sinks << ", name=" << sinks->name << ", type=" << sinks->type << ", ne0=" << sinks->ne[0] << ", ne1=" << sinks->ne[1] << ", ne2=" << sinks->ne[2] << ", ne3=" << sinks->ne[3] << ", nb0=" << sinks->nb[0] << ", nb1=" << sinks->nb[1] << ", nb2=" << sinks->nb[2] << ", nb3=" << sinks->nb[3];
    return true;
}
...
static void ggml_vk_op_f32(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * src3, ggml_tensor * dst, ggml_op op, PC&& pc) {
    {
        std::ostringstream _os;
        _os << "ggml_vk_op_f32((" << src0 << ", name=" << src0->name << ", type=" << src0->type << ", ne0=" << src0->ne[0] << ", ne1=" << src0->ne[1] << ", ne2=" << src0->ne[2] << ", ne3=" << src0->ne[3] << ", nb0=" << src0->nb[0] << ", nb1=" << src0->nb[1] << ", nb2=" << src0->nb[2] << ", nb3=" << src0->nb[3];
        if (src1 != nullptr) {
            _os << "), (" << src1 << ", name=" << src1->name << ", type=" << src1->type << ", ne0=" << src1->ne[0] << ", ne1=" << src1->ne[1] << ", ne2=" << src1->ne[2] << ", ne3=" << src1->ne[3] << ", nb0=" << src1->nb[0] << ", nb1=" << src1->nb[1] << ", nb2=" << src1->nb[2] << ", nb3=" << src1->nb[3];
    return true;
}

        if (src2 != nullptr) {
            _os << "), (" << src2 << ", name=" << src2->name << ", type=" << src2->type << ", ne0=" << src2->ne[0] << ", ne1=" << src2->ne[1] << ", ne2=" << src2->ne[2] << ", ne3=" << src2->ne[3] << ", nb0=" << src2->nb[0] << ", nb1=" << src2->nb[1] << ", nb2=" << src2->nb[2] << ", nb3=" << src2->nb[3];
    return true;
}

        if (src3 != nullptr) {
            _os << "), (" << src3 << ", name=" << src3->name << ", type=" << src3->type << ", ne0=" << src3->ne[0] << ", ne1=" << src3->ne[1] << ", ne2=" << src3->ne[2] << ", ne3=" << src3->ne[3] << ", nb0=" << src3->nb[0] << ", nb1=" << src3->nb[1] << ", nb2=" << src3->nb[2] << ", nb3=" << src3->nb[3];
    return true;
}

        _os << "), (" << dst << ", name=" << dst->name << ", type=" << dst->type << ", ne0=" << dst->ne[0] << ", ne1=" << dst->ne[1] << ", ne2=" << dst->ne[2] << ", ne3=" << dst->ne[3] << ", nb0=" << dst->nb[0] << ", nb1=" << dst->nb[1] << ", nb2=" << dst->nb[2] << ", nb3=" << dst->nb[3] << "), " << ggml_op_name(op) << ")";
        s_ggmlVkLog.debug("{}", _os.str());
    return true;
}
    ...
    if (pipeline == nullptr) {
        std::string msg = std::string("Missing op: ") + ggml_op_name(op) + " for " + ggml_type_name(src0->type);
        if (src1 != nullptr) {
            msg += " and ";
            msg += ggml_type_name(src1->type);
    return true;
}

        msg += " to ";
        msg += ggml_type_name(dst->type);
        s_ggmlVkLog.error("{}", msg);
        GGML_ABORT("fatal error");
    return true;
}
...
static void ggml_vk_multi_add(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_cgraph * cgraph, int node_idx) {
    ...
    if (pipeline == nullptr) {
        s_ggmlVkLog.error("Missing multi_add");
        GGML_ABORT("fatal error");
    return true;
}
...
static void ggml_vk_print_matrix_area(const void * data, ggml_type type, int ne0, int ne1, int i0, int i1, int i2) {
    ...
    for (int idx1 = i1 - 5; idx1 < i1 + 5; idx1++) {
        header << std::setw(7) << idx1 << " ";
    return true;
}

    s_ggmlVkLog.debug("{}", header.str());
    ...
    return true;
                ...
                row << std::setw(8) << std::fixed << std::setprecision(2) << val << " ";
            } else {
                row << "        ";
    return true;
}
...
static void ggml_vk_test_matmul(ggml_backend_vk_context * ctx, size_t m, size_t n, size_t k, size_t batch, size_t num_it, int split_k, int shader_size) {
    ...
    s_ggmlVkLog.info("TEST {} m={} n={} k={} batch={} split_k={} matmul {}ms {} TFLOPS avg_err={}", shname, m, n, k, batch, split_k, time / num_it, tflops, avg_err);

    if (avg_err > 0.1 || std::isnan(avg_err)) {
        s_ggmlVkLog.info("m = {} n = {} b = {}", first_err_m, first_err_n, first_err_b);
        s_ggmlVkLog.info("Actual result:");
        ggml_vk_print_matrix_area(d, GGML_TYPE_F32, m, n, first_err_m, first_err_n, first_err_b);
        s_ggmlVkLog.info("Expected result:");
...
static void ggml_vk_print_tensor_area(const ggml_tensor * tensor, int i0, int i1, int i2, int i3) {
    ...
    for (int idx0 = i0 - 5; idx0 < i0 + 5; idx0++) {
        ...
        row << std::setw(7) << idx0 << ": ";
        for (int idx1 = i1 - 5; idx1 < i1 + 5; idx1++) {
            if (idx0 >= 0 && idx0 < (int)tensor->ne[0] && idx1 >= 0 && idx1 < (int)tensor->ne[1] && i2 >= 0 && i2 < (int)tensor->ne[2] && i3 >= 0 && i3 < (int)tensor->ne[3]) {
                float val;
...
static void ggml_vk_test_dequant(ggml_backend_vk_context * ctx, size_t ne, ggml_type quant) {
    ...
    s_ggmlVkLog.info("TEST DEQUANT {} time={}ms avg_err={}", ggml_type_name(quant), ms_dequant, avg_err);

    if (avg_err > 0.1) {
        s_ggmlVkLog.info("first_error = {}", first_err);
        std::ostringstream actual, expected;
        actual << "Actual result: ";
        for (int i = std::max(0, first_err - 5); i < std::min((int)ne, first_err + 5); i++) {
            actual << ggml_fp16_to_fp32(x_chk[i]) << ", ";
    return true;
}
    ...
    return true;
        ...
        s_ggmlVkLog.info("{}", actual.str());
        s_ggmlVkLog.info("{}", expected.str());
    return true;
}
...
static void ggml_vk_test_dequant_matmul(ggml_backend_vk_context * ctx, size_t m, size_t n, size_t k, size_t batch, size_t num_it, size_t split_k, size_t shader_size, ggml_type quant, bool mmq = false) {
    ...
    if (p == nullptr) {
        s_ggmlVkLog.error("error: no pipeline for ggml_vk_test_dequant_matmul {}", ggml_type_name(quant));
        return;
    return true;
}
    ...
    {
        std::ostringstream _os;
        _os << "TEST dequant matmul " << shname;
        if (mmq) _os << " mmq";
        _os << " m=" << m << " n=" << n << " k=" << k << " batch=" << batch << " split_k=" << split_k << " matmul " << time_ms / num_it << "ms " << tflops << " TFLOPS avg_err=" << avg_err;
        s_ggmlVkLog.info("{}", _os.str());
    return true;
}
...
static bool ggml_vk_build_graph(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int node_idx, ggml_tensor *node_begin, int node_idx_begin, bool last_node, bool almost_ready, bool submit){
    ...
    case GGML_OP_OPT_STEP_SGD:
        break;
    default:
        s_ggmlVkLog.error("Missing op: {}", ggml_op_name(node->op));
        GGML_ABORT("fatal error");
    return true;
}
...
#define ENABLE_SYNC_LOGGING 0
    ...
    for (int i = 0; i < ctx->num_additional_fused_ops + 1; ++i) {
        ...
        std::ostringstream _os;
        _os << node_idx + i << " " << ggml_op_name(n->op) << " " << n->name;
        if (n->op == GGML_OP_GLU) {
            _os << " " << ggml_glu_op_name(ggml_get_glu_op(n)) << " " << (n->src[1] ? "split" : "single") << " ";
    return true;
}
    ...
    return true;
        ...
        if (!ok) {
            if (node->op == GGML_OP_UNARY) {
                s_ggmlVkLog.error("{}: error: op not supported UNARY {} ({})", __func__, node->name, ggml_unary_op_name(static_cast<ggml_unary_op>(node->op_params[0])));
            } else if (node->op == GGML_OP_GLU) {
                s_ggmlVkLog.error("{}: error: op not supported GLU {} ({})", __func__, node->name, ggml_glu_op_name(static_cast<ggml_glu_op>(node->op_params[0])));
            } else {
                s_ggmlVkLog.error("{}: error: op not supported {} ({})", __func__, node->name, ggml_op_name(node->op));
    return true;
}
...
static bool ggml_vk_instance_validation_ext_available() {
    ...
    s_ggmlVkLog.warn("Validation layer or layer extension VK_EXT_validation_features not found.");
#endif
    return false;
    return true;
}
...
static bool ggml_vk_instance_portability_enumeration_ext_available(const std::vector<vk::ExtensionProperties>& instance_extensions) {
    ...
    s_ggmlVkLog.warn("Instance extension VK_KHR_portability_enumeration not found.");
#endif
    return false;
...
static bool ggml_vk_instance_debug_utils_ext_available(
    ...
    s_ggmlVkLog.warn("Instance extension VK_EXT_debug_utils not found.");
    return false;
...
static void ggml_vk_print_graph_origin(const ggml_tensor * tensor, std::vector<const ggml_tensor *>& done, int level = 0) {
    ...
    return true;
        ...
        _os << ggml_op_name(tensor->op) << " gpu=" << (tensor->extra != nullptr);
        s_ggmlVkLog.debug("{}", _os.str());
    return true;
}
...
static void ggml_vk_print_tensor(const ggml_tensor * tensor, const char * name) {
    ...
    {
        std::ostringstream _os;
        _os << "TENSOR CHECK " << name << " (" << tensor->name << "): " << ggml_op_name(tensor->op);
        s_ggmlVkLog.debug("{}", _os.str());
    return true;
}

    s_ggmlVkLog.debug("tensor={} tensor->type: {} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={}",
        (void*)tensor, ggml_type_name(tensor->type), tensor->ne[0], tensor->nb[0], tensor->ne[1], tensor->nb[1], tensor->ne[2], tensor->nb[2], tensor->ne[3], tensor->nb[3]);
    if (tensor->src[0] != nullptr) {
        const ggml_tensor *s0 = tensor->src[0];
        s_ggmlVkLog.debug("tensor->src[0]={} name={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={}",
            (void*)s0, s0->name, ggml_op_name(s0->op), ggml_type_name(s0->type), s0->ne[0], s0->nb[0], s0->ne[1], s0->nb[1], s0->ne[2], s0->nb[2], s0->ne[3], s0->nb[3]);
    return true;
}

    if (tensor->src[1] != nullptr) {
        const ggml_tensor *s1 = tensor->src[1];
        s_ggmlVkLog.debug("tensor->src[1]={} name={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={}",
            (void*)s1, s1->name, ggml_op_name(s1->op), ggml_type_name(s1->type), s1->ne[0], s1->nb[0], s1->ne[1], s1->nb[1], s1->ne[2], s1->nb[2], s1->ne[3], s1->nb[3]);
    return true;
}

    s_ggmlVkLog.debug("Result:");
...
static void ggml_vk_check_results_0(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int tensor_idx) {
    ...
    return true;
            ...
            case GGML_UNARY_OP_ABS:
                ...
                break;
            default:
                s_ggmlVkLog.error("Missing vk_check_results OP: {}", ggml_op_name(tensor->op));
                GGML_ABORT("fatal error");
    return true;
}
...
static void ggml_vk_check_results_1(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int tensor_idx) {
                ...
                for (int i0 = 0; i0 < tensor->ne[0]; i0++) {
                    ...
                    if (buffer_size_fit) {
                        if (tensor->type == GGML_TYPE_F32) {
                            ...
                            s_ggmlVkLog.error("Results check not implemented for type {}", ggml_type_name(tensor->type));
    return true;
}

                    } else {
                        s_ggmlVkLog.error("Missing debug code for type {}", ggml_type_name(tensor->type));
                        GGML_ABORT("fatal error");
    return true;
}

                    if ((std::isnan(correct) != std::isnan(result)) || (std::isinf(correct) != std::isinf(result)) || !buffer_size_fit) {
                        s_ggmlVkLog.error("ERROR: Invalid value in {} i3={} i2={} i1={} i0={} result={} correct={} avg_err={}", ggml_op_name(tensor->op), i3, i2, i1, i0, result, correct, (avg_err / counter));
                        s_ggmlVkLog.error("tensor={} tensor->name={} tensor->type: {} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)tensor, tensor->name, ggml_type_name(tensor->type), tensor->ne[0], tensor->nb[0], tensor->ne[1], tensor->nb[1], tensor->ne[2], tensor->nb[2], tensor->ne[3], tensor->nb[3], tensor->view_offs);
                        if (src0 != nullptr) {
                            s_ggmlVkLog.error("src0={} src0->name={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src0, src0->name, ggml_op_name(src0->op), ggml_type_name(src0->type), src0->ne[0], src0->nb[0], src0->ne[1], src0->nb[1], src0->ne[2], src0->nb[2], src0->ne[3], src0->nb[3], src0->view_offs);
    return true;
}

                        if (src1 != nullptr) {
                            s_ggmlVkLog.error("src1={} src1->name={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src1, src1->name, ggml_op_name(src1->op), ggml_type_name(src1->type), src1->ne[0], src1->nb[0], src1->ne[1], src1->nb[1], src1->ne[2], src1->nb[2], src1->ne[3], src1->nb[3], src1->view_offs);
    return true;
}

                        if (src2 != nullptr) {
                            s_ggmlVkLog.error("src2={} src2->name={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src2, src2->name, ggml_op_name(src2->op), ggml_type_name(src2->type), src2->ne[0], src2->nb[0], src2->ne[1], src2->nb[1], src2->ne[2], src2->nb[2], src2->ne[3], src2->nb[3], src2->view_offs);
    return true;
}

                        if (src3 != nullptr) {
                            s_ggmlVkLog.error("src3={} src3->name={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src3, src3->name, ggml_op_name(src3->op), ggml_type_name(src3->type), src3->ne[0], src3->nb[0], src3->ne[1], src3->nb[1], src3->ne[2], src3->nb[2], src3->ne[3], src3->nb[3], src3->view_offs);
    return true;
}

                        s_ggmlVkLog.error("First error: result={} correct={} i3={} i2={} i1={} i0={}", first_error_result, first_error_correct, first_error[3], first_error[2], first_error[1], first_error[0]);
                        s_ggmlVkLog.error("Result:");
                        ggml_vk_print_tensor_area(tensor, tensor_data, i0, i1, i2, i3);
                        s_ggmlVkLog.error("Correct:");
    ...
    if (vk_output_tensor > 0 && vk_output_tensor == check_counter) {
        s_ggmlVkLog.error("TENSOR CHECK: avg_err={} in {} (check {})", avg_err, ggml_op_name(tensor->op), check_counter);
        s_ggmlVkLog.error("tensor={} tensor->name={} tensor->type: {} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)tensor, tensor->name, ggml_type_name(tensor->type), tensor->ne[0], tensor->nb[0], tensor->ne[1], tensor->nb[1], tensor->ne[2], tensor->nb[2], tensor->ne[3], tensor->nb[3], tensor->view_offs);
        if (src0 != nullptr) s_ggmlVkLog.error("src0={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src0, ggml_op_name(src0->op), ggml_type_name(src0->type), src0->ne[0], src0->nb[0], src0->ne[1], src0->nb[1], src0->ne[2], src0->nb[2], src0->ne[3], src0->nb[3], src0->view_offs);
        if (src1 != nullptr) s_ggmlVkLog.error("src1={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src1, ggml_op_name(src1->op), ggml_type_name(src1->type), src1->ne[0], src1->nb[0], src1->ne[1], src1->nb[1], src1->ne[2], src1->nb[2], src1->ne[3], src1->nb[3], src1->view_offs);
        if (src2 != nullptr) s_ggmlVkLog.error("src2={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src2, ggml_op_name(src2->op), ggml_type_name(src2->type), src2->ne[0], src2->nb[0], src2->ne[1], src2->nb[1], src2->ne[2], src2->nb[2], src2->ne[3], src2->nb[3], src2->view_offs);
        if (src3 != nullptr) s_ggmlVkLog.error("src3={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src3, ggml_op_name(src3->op), ggml_type_name(src3->type), src3->ne[0], src3->nb[0], src3->ne[1], src3->nb[1], src3->ne[2], src3->nb[2], src3->ne[3], src3->nb[3], src3->view_offs);
        s_ggmlVkLog.error("First error: result={} correct={} i3={} i2={} i1={} i0={}", first_error_result, first_error_correct, first_error[3], first_error[2], first_error[1], first_error[0]);
        s_ggmlVkLog.error("Result:");
        ggml_vk_print_tensor_area(tensor, tensor_data, 5, 5, 0, 0);
        s_ggmlVkLog.error("Correct:");
    ...
    if (avg_err > 0.5 || std::isnan(avg_err)) {
        s_ggmlVkLog.error("ERROR: avg_err={} in {} (check {})", avg_err, ggml_op_name(tensor->op), check_counter);
        s_ggmlVkLog.error("tensor={} tensor->name={} tensor->type: {} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)tensor, tensor->name, ggml_type_name(tensor->type), tensor->ne[0], tensor->nb[0], tensor->ne[1], tensor->nb[1], tensor->ne[2], tensor->nb[2], tensor->ne[3], tensor->nb[3], tensor->view_offs);
        if (src0 != nullptr) s_ggmlVkLog.error("src0={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src0, ggml_op_name(src0->op), ggml_type_name(src0->type), src0->ne[0], src0->nb[0], src0->ne[1], src0->nb[1], src0->ne[2], src0->nb[2], src0->ne[3], src0->nb[3], src0->view_offs);
        if (src1 != nullptr) s_ggmlVkLog.error("src1={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src1, ggml_op_name(src1->op), ggml_type_name(src1->type), src1->ne[0], src1->nb[0], src1->ne[1], src1->nb[1], src1->ne[2], src1->nb[2], src1->ne[3], src1->nb[3], src1->view_offs);
        if (src2 != nullptr) s_ggmlVkLog.error("src2={} op={} type={} ne0={} nb0={} ne1={} nb1={} ne2={} nb2={} ne3={} nb3={} offset={}", (void*)src2, ggml_op_name(src2->op), ggml_type_name(src2->type), src2->ne[0], src2->nb[0], src2->ne[1], src2->nb[1], src2->ne[2], src2->nb[2], src2->ne[3], src2->nb[3], src2->view_offs);
        ...
        s_ggmlVkLog.debug("{} {} op={} avg_err={}", check_counter, tensor->name, ggml_op_name(tensor->op), avg_err);
    return true;
}
