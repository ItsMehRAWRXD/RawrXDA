...
inline void EmitSyntheticGguf(const std::string& path, const PolyConfig& cfg)
{
    ...
    // alignment
    std::size_t pos = static_cast<std::size_t>(out.tellp());
    std::size_t pad = (32 - (pos % 32)) % 32;
    ByteVec padding(pad, '\0');
    out.write(reinterpret_cast<const char*>(padding.data()), pad);

    // tensor infos (offsets are random but sequential)
    Offset running = static_cast<Offset>(out.tellp());
...
// ---------- random HF stub ----------
inline void EmitRandomStub(const std::string& dir, const PolyConfig& cfg, const char* /*mmfName*/)
{
    std::string config = R"({
