...
short_emit:
    ...
    ; Hash and emit token
    mov rdx, rcx            ; length
    push rcx                ; save token length (ECX clobbered by call)
    mov rcx, r12            ; string
    push r12
    push r13
    call compute_token_hash
    ...
    add r12, rcx            ; advance input by token length
    sub r13, rcx            ; decrease remaining by token length
    jmp tokenize_loop
