...
#include "../security/WindowsDefenderBridge.h"
            ...
            if(GetOpenFileNameW(&ofn)) {
                ...
                if(ctx.GetRBAC().CheckPermission(token, L"hotpatch:apply")) {
                    ...
                    if(hFile != INVALID_HANDLE_VALUE) {
                        ...
                        // Windows Defender scan before applying
                        auto& defender = RawrXD::GetDefenderBridge();
                        if (!defender.IsInitialized()) defender.Initialize();
                        auto scanResult = defender.ScanBuffer(buf.data(), rd, path);
                        if (scanResult.wasBlocked) {
                            MessageBoxW(hWnd,
                                (std::wstring(L"Windows Defender blocked this patch:\n") +
                                 std::wstring(path) + L"\n\nVerdict: " +
                                 std::wstring(RawrXD::VerdictToString(scanResult.verdict),
                                              RawrXD::VerdictToString(scanResult.verdict) +
                                              strlen(RawrXD::VerdictToString(scanResult.verdict)))).c_str(),
                                L"Hotpatch Blocked", MB_ICONWARNING | MB_OK);
                            break;
    return true;
}
