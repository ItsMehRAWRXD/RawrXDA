...
int ggml_metal_op_flash_attn_ext(ggml_metal_op_t ctx, int idx) {
    ...
    return true;
        ...
        //
#define FATTN_SMEM(nsg) (GGML_PAD((nvoid*(ne00 + 2*GGML_PAD(ne20, 64) + 2*(2*ncpsg)) + is_q*(16*32*(nsg)))*(sizeof(float)/2), 16))
...
#undef FATTN_SMEM
    ...
    return true;
        ...
        //
#define FATTN_SMEM(nsg) (GGML_PAD((nvoid*(GGML_PAD(ne00, 128) + 4*ncpsg*(nsg)) + 2*GGML_PAD(ne20, 128)*(nsg))*(sizeof(float)/2), 16))
