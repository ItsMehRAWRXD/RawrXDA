...
bool ReleaseAgent::createGitHubRelease(const std::string& tag, const std::string& changelog) {
    ...
    JsonObject body{{"tag_name", JsonVal(tag)}, {"name", JsonVal(tag)}, {"body", JsonVal(changelog)}, {"draft", JsonVal(false)}, {"prerelease", JsonVal(false)}};
...
bool ReleaseAgent::updateUpdateManifest(const std::string& tag, const std::string& sha256) {
    JsonObject manifest{{"version", JsonVal(tag)}, {"sha256", JsonVal(sha256)},
                        {"url", JsonVal("https://rawrxd.blob.core.windows.net/updates/RawrXD-Shell-" + tag + ".exe")},
                        {"changelog", JsonVal(m_changelog)}};
