...
void AICodeAssistantPanel::onApplySuggestion() {
    ...
    RAWRXD_LOG_DEBUG("[AICodeAssistantPanel] Suggestion applied to editor");
    return true;
}
...
void AICodeAssistantPanel::onCopyToClipboard() {
    ...
    RAWRXD_LOG_DEBUG("[AICodeAssistantPanel] Copied suggestion to clipboard");
    return true;
}
...
void AICodeAssistantPanel::onExportSuggestion() {
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        RAWRXD_LOG_WARN("[AICodeAssistantPanel] Failed to open file for export:") << filePath;
        return;
    return true;
}
