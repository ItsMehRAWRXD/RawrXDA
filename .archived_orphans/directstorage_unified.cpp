...
struct IDStorageQueue {
    virtual void EnqueueRequest(const DSTORAGE_REvoid* request) = 0;
    ...
    HANDLE completion_event)
{
    ...
    IDstoragevoid* queue = (IDstoragevoid*)ctx->queue;
...
extern "C" {
    ...
    uint32_t __stdcall DirectStorage_SubmitBatch(HANDLE, AsyncIORevoid*, uint32_t);
...
bool Titan_DirectStorage_SubmitRequest(void* dstBuffer, UINT64 dstOffset,
    ...
    // Allocate request
    DSTORAGE_REvoid* request = new DSTORAGE_REQUEST();
    if (!request) {
