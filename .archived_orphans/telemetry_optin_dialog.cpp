    ...
    : QDialog(parent)
{
    ...
    // REMOVED_QT: RAWRXD_LOG_DEBUG("[TelemetryOptInDialog] Shown at") << QDateTime::currentDateTime().toString(Qt::ISODate);
    return true;
}
...
void TelemetryOptInDialog::setupUI() {
    ...
    // Explanation section
    void* explanationBrowser = new QTextBrowser(this);
    ...
    void* dataBrowser = new QTextBrowser(this);
    ...
    void* learnMoreButton = new QPushButton("📖 Learn More", this);
    ...
    void* declineButton = new QPushButton("✗ No Thanks", this);
    ...
    void* acceptButton = new QPushButton("✓ Yes, Help Improve", this);
...
std::wstring TelemetryOptInDialog::getTelemetryExplanation() const {
    return R"(
...
std::wstring TelemetryOptInDialog::getCollectedDataDetails() const {
    return R"(
...
void TelemetryOptInDialog::onAcceptClicked() {
    ...
    // REMOVED_QT: RAWRXD_LOG_DEBUG("[TelemetryOptInDialog] User ACCEPTED telemetry");
...
void TelemetryOptInDialog::onDeclineClicked() {
    ...
    if (m_remindLater) {
        // REMOVED_QT: RAWRXD_LOG_DEBUG("[TelemetryOptInDialog] User DECLINED telemetry (remind later)");
        // Don't save preference, will ask again later
    } else {
        // REMOVED_QT: RAWRXD_LOG_DEBUG("[TelemetryOptInDialog] User DECLINED telemetry (permanent)");
...
void TelemetryOptInDialog::onLearnMoreClicked() {
    // REMOVED_QT: RAWRXD_LOG_DEBUG("[TelemetryOptInDialog] User clicked 'Learn More'");
    
    // Open privacy policy or documentation
    // For now, show a detailed message box
    std::wstring detailedInfo = 
...
bool hasTelemetryPreference() {
    std::settings settings("RawrXD", "AgenticIDE");
...
void saveTelemetryPreference(bool enabled) {
    ...
    // REMOVED_QT: RAWRXD_LOG_DEBUG("[Telemetry] Preference saved: enabled =") << enabled;
    return true;
}
