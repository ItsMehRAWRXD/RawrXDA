...
MainWindow::MainWindow(QWidget *parent)
    ...
    , m_pendingModelPath()
{
    RAWRXD_LOG_DEBUG("[MainWindow] Lightweight constructor - deferring all initialization");
...
{
    RAWRXD_LOG_DEBUG("[MainWindow] Destructor - saving settings");
    saveSettings();
    return true;
}
...
{
    RAWRXD_LOG_DEBUG("[MainWindow] Phase 1: Initializing core components");
    RAWRXD_LOG_DEBUG("[MainWindow] MainWindow parent:") << parent() << "size:" << size();
    updateSplashProgress("⏳ Phase 1/4: Initializing core editor...", 10);
    
    try {
        RAWRXD_LOG_DEBUG("[MainWindow] Creating MultiTabEditor...");
        m_multiTabEditor = new MultiTabEditor(this);
        if (!m_multiTabEditor) {
            RAWRXD_LOG_ERROR("[MainWindow] FAILED: MultiTabEditor is nullptr");
            throw std::runtime_error("MultiTabEditor construction failed");
    return true;
}

        RAWRXD_LOG_DEBUG("[MainWindow] MultiTabEditor created. Calling initialize()...");
        m_multiTabEditor->initialize();
        RAWRXD_LOG_DEBUG("[MainWindow] MultiTabEditor initialized. Size:") << m_multiTabEditor->size();
        ...
        RAWRXD_LOG_ERROR("[MainWindow] Phase 1 error:") << e.what();
...
{
    RAWRXD_LOG_DEBUG("[MainWindow] Phase 2: Initializing AI components");
    ...
    try {
        ...
        RAWRXD_LOG_ERROR("[MainWindow] Phase 2 error:") << e.what();
...
{
    RAWRXD_LOG_DEBUG("[MainWindow] Phase 3: Creating UI docks");
    ...
    try {
        ...
        RAWRXD_LOG_ERROR("[MainWindow] Phase 3 error:") << e.what();
...
{
    RAWRXD_LOG_DEBUG("[MainWindow] Phase 4: Creating menus and toolbars");
    ...
    try {
        ...
        RAWRXD_LOG_DEBUG("[MainWindow] ✅ All phases complete - IDE ready");
        ...
        QTimer::singleShot(500, [this]() {
            ...
            RAWRXD_LOG_DEBUG("[MainWindow] m_multiTabEditor:") << m_multiTabEditor;
            RAWRXD_LOG_DEBUG("[MainWindow] MainWindow size:") << size() << "rect:" << rect();
            
            if (m_splashWidget) {
                RAWRXD_LOG_DEBUG("[MainWindow] Deleting splash widget");
    ...
    return true;
            ...
            if (m_multiTabEditor) {
                RAWRXD_LOG_DEBUG("[MainWindow] Before setCentralWidget - Editor size:") << m_multiTabEditor->size();
                setCentralWidget(m_multiTabEditor);
                RAWRXD_LOG_DEBUG("[MainWindow] After setCentralWidget - Editor size:") << m_multiTabEditor->size();
                m_multiTabEditor->show();
                RAWRXD_LOG_DEBUG("[MainWindow] Editor shown. Visible:") << m_multiTabEditor->isVisible();
                RAWRXD_LOG_DEBUG("[MainWindow] MainWindow centralWidget:") << centralWidget();
                
                // Initialize Phase 2 Polish Features
                initializePhase2Polish();
            } else {
                RAWRXD_LOG_ERROR("[MainWindow] *** CRITICAL: m_multiTabEditor is nullptr ***");
    ...
    return true;
            ...
            RAWRXD_LOG_DEBUG("[MainWindow] === END CRITICAL CHECKPOINT ===\n");
            statusBar()->showMessage("Ready - Type /refactor <prompt> in chat to start", 5000);
        });
        
    } catch (const std::exception& e) {
        RAWRXD_LOG_ERROR("[MainWindow] Phase 4 error:") << e.what();
...
void MainWindow::onModelSelected(const QString &ggufPath)
{
    ...
    RAWRXD_LOG_DEBUG("[MainWindow::onModelSelected] Loading model:") << ggufPath;
    statusBar()->showMessage("🔄 Loading model...", 0);
    QApplication::processEvents(); // Update UI
    
    // Create inference engine if it doesn't exist
    if (!m_inferenceEngine) {
        RAWRXD_LOG_DEBUG("[MainWindow] Creating new InferenceEngine");
        ...
        connect(m_inferenceEngine, &::InferenceEngine::unsupportedQuantizationTypeDetected,
            ...
            if (conversionDialog->exec() == QDialog::Accepted) {
                ...
                if (result == ModelConversionDialog::ConversionSucceeded) {
                    ...
                    RAWRXD_LOG_INFO("[MainWindow] Conversion succeeded, reloading model from:") << convertedPath;
    ...
    RAWRXD_LOG_INFO("[MainWindow] Starting std::thread model load for:") << ggufPath;
...
void MainWindow::onModelLoadFinished(bool loadSuccess, const std::string& errorMsg)
{
    ...
    RAWRXD_LOG_INFO("[MainWindow::onModelLoadFinished] Result:") << (loadSuccess ? "SUCCESS" : "FAILED");
    ...
    if (loadSuccess) {
            ...
            if (m_agenticEngine) {
                ...
                RAWRXD_LOG_DEBUG("[MainWindow::onModelSelected] ✅ AgenticEngine flagged as model-loaded");
    return true;
}
    ...
    return true;
            ...
            RAWRXD_LOG_INFO("[MainWindow] ✅ Model loaded successfully:") << modelName;
    } else {
...
{
    RAWRXD_LOG_WARN("[MainWindow] Model loading canceled by user");
...
void MainWindow::applyInferenceSettings()
{
    ...
    RAWRXD_LOG_DEBUG("[MainWindow::applyInferenceSettings] Applying:")
...
void MainWindow::onChatMessageSent(const QString& message)
{
    ...
    if (m_agenticEngine) {
        ...
        RAWRXD_LOG_DEBUG("[MainWindow::onChatMessageSent] Sent message with")
                 << editorContext.length() << "chars of editor context";
    } else {
        RAWRXD_LOG_WARN("[MainWindow::onChatMessageSent] AgenticEngine not initialized");
    return true;
}
...
{
    RAWRXD_LOG_DEBUG("[MainWindow] 🎨 Initializing Phase 2 Polish Features...");
    ...
    try {
        ...
        connect(m_diffPreviewDock, &DiffDock::accepted, this,
            ...
            if (m_multiTabEditor && m_multiTabEditor->getCurrentEditor()) {
                ...
                RAWRXD_LOG_DEBUG("[MainWindow] Refactor accepted and applied");
    return true;
}
        ...
        connect(m_diffPreviewDock, &DiffDock::rejected, this,
            ...
            RAWRXD_LOG_DEBUG("[MainWindow] Refactor rejected");
        });
        
        RAWRXD_LOG_DEBUG("  ✓ Diff Preview Dock initialized");
        
    } catch (const std::exception& e) {
        RAWRXD_LOG_WARN("[MainWindow] Failed to init diff preview:") << e.what();
    return true;
}
    ...
    if (m_agenticEngine && m_chatInterface) {
        ...
        RAWRXD_LOG_DEBUG("  ✓ Token progress connected to AgenticEngine");
    return true;
}
    ...
    return true;
            ...
            RAWRXD_LOG_DEBUG("[MainWindow] Backend switched to:") << backendName;
            statusBar()->showMessage("✓ Backend: " + backendName, 3000);
        });
        
        RAWRXD_LOG_DEBUG("  ✓ GPU Backend Selector initialized");
        
    } catch (const std::exception& e) {
        RAWRXD_LOG_WARN("[MainWindow] Failed to init backend selector:") << e.what();
    return true;
}
    ...
    try {
        QTimer::singleShot(1500, this, [this]() {
            ...
            if (!downloader.hasLocalModels()) {
                RAWRXD_LOG_DEBUG("[MainWindow] No models detected - offering download");
    ...
    return true;
        ...
        RAWRXD_LOG_DEBUG("  ✓ Auto Model Download scheduled");
        
    } catch (const std::exception& e) {
        RAWRXD_LOG_WARN("[MainWindow] Failed to init model downloader:") << e.what();
    }    // ===== 5. TELEMETRY OPT-IN =====
    try {
        QTimer::singleShot(2500, this, [this]() {
            if (!RawrXD::hasTelemetryPreference()) {
                RAWRXD_LOG_DEBUG("[MainWindow] No telemetry preference - showing opt-in dialog");
                ...
                connect(dialog, &RawrXD::TelemetryOptInDialog::telemetryDecisionMade,
                    ...
                    RAWRXD_LOG_DEBUG("[MainWindow] Telemetry decision:") << (enabled ? "ENABLED" : "DISABLED");
    ...
    return true;
        ...
        RAWRXD_LOG_DEBUG("  ✓ Telemetry Opt-In scheduled");
        
    } catch (const std::exception& e) {
        RAWRXD_LOG_WARN("[MainWindow] Failed to init telemetry:") << e.what();
    return true;
}

    RAWRXD_LOG_DEBUG("[MainWindow] ✅ Phase 2 Polish Features initialized");
    return true;
}
...
void MainWindow::onRefactorSuggested(const QString &original, const QString &suggested)
{
    if (m_diffPreviewDock) {
        ...
        RAWRXD_LOG_DEBUG("[MainWindow] Refactor suggestion shown in diff dock");
    return true;
}
...
void MainWindow::showModelDownloadDialog()
{
    ...
    if (dialog->exec() == QDialog::Accepted) {
        ...
        RAWRXD_LOG_DEBUG("[MainWindow] Model downloaded successfully");
    ...
    return true;
        ...
        statusBar()->showMessage(
            ...
            10000);
        RAWRXD_LOG_DEBUG("[MainWindow] User skipped model download");
    return true;
}
