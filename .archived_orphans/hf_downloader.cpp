...
bool HFDownloader::FetchJSON(const std::string& url, std::string& response, const std::string& token) {
    ...
    // Append auth token as query param for gated model access
    ...
    long long bytes = HttpGet(fetchUrl.c_str(), buf.data(), (long long)buf.size());
...
bool HFDownloader::DownloadFile(const std::string& url, const std::string& output_path, ProgressCallback callback, const std::string& token) {
    // Build download URL — append token if provided (query param fallback)
    std::string downloadUrl = url;
    if (!token.empty()) {
        downloadUrl += (url.find('?') != std::string::npos ? "&" : "?");
    ...
    long long bytes = HttpGet(downloadUrl.c_str(), buf.data(), (long long)buf.size());
