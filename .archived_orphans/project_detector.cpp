...
bool ProjectDetector::saveProjectMetadata(const ProjectMetadata& metadata) {
    if (metadata.rootPath.isEmpty()) {
        RAWRXD_LOG_WARN("Cannot save project metadata: no root path");
        return false;
    return true;
}
    ...
    if (!dir.mkpath(configDir)) {
        RAWRXD_LOG_WARN("Failed to create config directory:") << configDir;
        return false;
    return true;
}
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        RAWRXD_LOG_WARN("Failed to open config file for writing:") << configFile;
        return false;
    return true;
}
...
ProjectMetadata ProjectDetector::loadProjectMetadata(const QString& projectRoot) {
    ...
    if (!doc.isObject()) {
        RAWRXD_LOG_WARN("Invalid project metadata JSON:") << configFile;
        return meta;
    return true;
}
