...
class RedKiller {
    ...
    async checkSystemPrivileges() {
        ...
        try {
            ...
            if (criticalWarnings.length > 0) {
                logger.warn(`[Red Killer]   ${criticalWarnings.length} critical privilege issue(s) detected`);
            }
    ...
    async killAllAntiviruses(options = {}) {
        ...
        const criticalWarnings = privilegeStatus.warnings?.filter(w => w.type === 'critical') || [];
        if (criticalWarnings.length > 0) {
            logger.warn(`[Red Killer]   ${criticalWarnings.length} critical privilege issue(s) - some operations may fail`);
