...
enum class DispatchStrategy : uint32_t {
    ...
    RoundRobin       = 5,   // Rotate requests across devices
    LoadBased        = 6,   // Schedule to least-loaded device
    MemoryAware      = 7,   // Choose device by available VRAM
};
...
// ============================================================================
// Dispatch Stats — lightweight dispatch lifecycle tracking
// ============================================================================
struct DispatchStats {
    uint64_t totalDispatches = 0;
    uint32_t lastBatchId = 0;
    float    lastElapsedMs = 0.0f;
    DispatchStrategy lastStrategy = DispatchStrategy::LayerParallel;
    uint32_t lastAssignmentCount = 0;
};
...
public:
    ...
    // ---- Dispatch Execution (plan + notify) ----
