...
#define GGML_RXD_COMMON_IMPL_CPP
...
#define HEX_VERBOSE(...) \
    if (opt_verbose) GGML_RXD_LOG_DEBUG(__VA_ARGS__)

#define HEX_PROFILE(...) \
    if (opt_profile) GGML_RXD_LOG_INFO(__VA_ARGS__)
...
static inline int hex_format_tensor_dims(char * str, const struct ggml_rxd_tensor * t) {
...
static inline void hex_format_op_dims(char * str, const struct ggml_rxd_tensor * t) {
    char * p = str;
    ...
    if (t->src[0]) {
        ...
        for (int i = 1; i < GGML_RXD_MAX_SRC && t->src[i]; i++) {
...
static inline int hex_format_tensor_strides(char * str, const struct ggml_rxd_tensor * t) {
    const char * c = ggml_rxd_is_contiguous(t) ? "" : "!";
...
static inline void hex_format_op_strides(char * str, const struct ggml_rxd_tensor * t) {
    char * p = str;
...
static inline void hex_format_op_types(char * str, const struct ggml_rxd_tensor * t) {
    char * p = str;

    // append src0 and src1 (if any)
    if (t->src[0]) {
        p += snprintf(p, str + 512 - p, "%s", ggml_rxd_type_name(t->src[0]->type));

        for (int i = 1; i < GGML_RXD_MAX_SRC && t->src[i]; i++) {
            p += snprintf(p, str + 512 - p, " x ");
            p += snprintf(p, str + 512 - p, "%s", ggml_rxd_type_name(t->src[i]->type));
        }

        p += snprintf(p, str + 512 - p, " -> ");
    }

    p += snprintf(p, str + 512 - p, "%s", ggml_rxd_type_name(t->type));
}

static inline const char * hex_tensor_buff_name(const struct ggml_rxd_tensor * t) {
    if (t->buffer) {
        return ggml_rxd_backend_buffer_name(t->buffer);
    }
    return "NONE";
}

static inline void hex_format_op_buffs(char * str, const struct ggml_rxd_tensor * t) {
    char * p = str;
...
static inline void hex_format_op_names(char * str, const struct ggml_rxd_tensor * t) {
    char * p = str;
...
struct ggml_rxd_hexagon_session {
    ggml_rxd_hexagon_session(int dev_id, ggml_rxd_backend_dev_t dev) noexcept(false);
    ~ggml_rxd_hexagon_session() noexcept(true);
    ...
    ggml_rxd_backend_buffer_type repack_buffer_type;
...
void ggml_rxd_hexagon_session::enqueue(struct htp_general_req &req, struct dspqueue_buffer *bufs, uint32_t n_bufs, bool sync) {
    ...
    if (err != 0) {
        GGML_RXD_ABORT("ggml-hex: %s dspqueue_write failed: 0x%08x\n", this->name.c_str(), (unsigned) err);
    }
...
void ggml_rxd_hexagon_session::flush() {
    ...
    while (this->op_pending) {
        ...
        if (err != 0) {
            GGML_RXD_ABORT("ggml-hex: dspqueue_read failed: 0x%08x\n", (unsigned) err);
        }

        // Basic sanity checks
        if (rsp_size != sizeof(rsp)) {
            GGML_RXD_ABORT("ggml-hex: dspcall : bad response (size)\n");
        }

        if (rsp.status != HTP_STATUS_OK) {
            GGML_RXD_LOG_ERROR("ggml-hex: dspcall : dsp-rsp: %s\n", status_to_str(rsp.status));
            // TODO: handle errors
        }
...
struct ggml_rxd_backend_hexagon_buffer_type_context {
    ggml_rxd_backend_hexagon_buffer_type_context(const std::string & name, ggml_rxd_hexagon_session * sess) {
        this->sess = sess;
        this->name = name;
    }

    ggml_rxd_hexagon_session * sess;
    std::string            name;
};

struct ggml_rxd_backend_hexagon_buffer_context {
    bool mmap_to(ggml_rxd_hexagon_session * s) {
        ...
        if (err != 0) {
            GGML_RXD_LOG_ERROR("ggml-hex: buffer mapping failed : domain_id %d size %zu fd %d error 0x%08x\n",
    ...
    ggml_rxd_backend_hexagon_buffer_context(ggml_rxd_hexagon_session * sess, size_t size, bool repack) {
        size += 4 * 1024;  // extra page for padding

        if (rpcmem_alloc2) {
            this->base = (uint8_t *) rpcmem_alloc2(RPCMEM_HEAP_ID_SYSTEM, RPCMEM_DEFAULT_FLAGS | RPCMEM_HEAP_NOREG, size);
        } else {
            GGML_RXD_LOG_INFO("ggml-hex: %s rpcmem_alloc2 not found, falling back to rpcmem_alloc\n", sess->name.c_str());
            this->base = (uint8_t *) rpcmem_alloc(RPCMEM_HEAP_ID_SYSTEM, RPCMEM_DEFAULT_FLAGS | RPCMEM_HEAP_NOREG, size);
        }

        if (!this->base) {
            GGML_RXD_LOG_ERROR("ggml-hex: %s failed to allocate buffer : size %zu\n", sess->name.c_str(), size);
            throw std::runtime_error("ggml-hex: rpcmem_alloc failed (see log for details)");
        }

        this->fd = rpcmem_to_fd(this->base);
        if (this->fd < 0) {
            GGML_RXD_LOG_ERROR("ggml-hex: %s failed to get FD for buffer %p\n", sess->name.c_str(), (void *) this->base);
    ...
    ~ggml_rxd_backend_hexagon_buffer_context() {
        munmap();
        if (this->base) {
            rpcmem_free(this->base);
            this->base = NULL;
    ...
    ggml_rxd_hexagon_session * sess;  // primary session
    uint8_t *              base;
    size_t                 size;
    int                    fd;
...
static ggml_rxd_hexagon_session * ggml_rxd_backend_hexagon_buffer_get_sess(ggml_rxd_backend_buffer_t buffer) {
    return static_cast<ggml_rxd_backend_hexagon_buffer_type_context *>(buffer->buft->context)->sess;
}

static void ggml_rxd_backend_hexagon_buffer_free_buffer(ggml_rxd_backend_buffer_t buffer) {
    auto ctx = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(buffer->context);
    delete ctx;
}

static void * ggml_rxd_backend_hexagon_buffer_get_base(ggml_rxd_backend_buffer_t buffer) {
    auto ctx = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(buffer->context);
    return ctx->base;
}

static enum ggml_rxd_status ggml_rxd_backend_hexagon_buffer_init_tensor(ggml_rxd_backend_buffer_t buffer, ggml_rxd_tensor * tensor) {
    auto ctx  = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(buffer->context);
    auto sess = ctx->sess;

    HEX_VERBOSE("ggml-hex: %s init-tensor %s : base %p data %p nbytes %zu usage %d repack %d\n", sess->name.c_str(),
                tensor->name, (void *) ctx->base, tensor->data, ggml_rxd_nbytes(tensor), (int) buffer->usage,
                (int) ctx->repack);
...
static void dump_packed_block_q4x4x2(const uint8_t * v, unsigned int i, size_t k) {
    ...
    const ggml_rxd_half * d = (const ggml_rxd_half *) (v_d + i * dblk_size);
    ...
    HEX_VERBOSE("ggml-hex: repack q4x4x2-%d: %d %d %d %d ... %d %d %d %d ... %d %d %d %d : %.6f %.6f %.6f %.6f\n", i,
                ...
                GGML_RXD_FP16_TO_FP32(d[0]), GGML_RXD_FP16_TO_FP32(d[1]), GGML_RXD_FP16_TO_FP32(d[2]), GGML_RXD_FP16_TO_FP32(d[3]));
    ...
    HEX_VERBOSE("ggml-hex: repack q4x4x2-%d: %d %d %d %d ... %d %d %d %d ... %d %d %d %d : %.6f %.6f %.6f %.6f\n",
                ...
                GGML_RXD_FP16_TO_FP32(d[4]), GGML_RXD_FP16_TO_FP32(d[5]), GGML_RXD_FP16_TO_FP32(d[6]), GGML_RXD_FP16_TO_FP32(d[7]));
}
...
static void repack_row_q4x4x2(uint8_t * y, const block_q4_0 * x, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        // Repack the scales
        ggml_rxd_half * d = (ggml_rxd_half *) (y_d + i * dblk_size);
        d[0]          = x[i * 8 + 0].d;
        d[1]          = x[i * 8 + 1].d;
        d[2]          = x[i * 8 + 2].d;
        d[3]          = x[i * 8 + 3].d;
...
static void unpack_row_q4x4x2(block_q4_0 * x, const uint8_t * y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        // Unpack the scales
        const ggml_rxd_half * d = (const ggml_rxd_half *) (y_d + i * dblk_size);
        x[i * 8 + 0].d      = d[0];
        x[i * 8 + 1].d      = d[1];
        x[i * 8 + 2].d      = d[2];
        x[i * 8 + 3].d      = d[3];
...
static void repack_q4_0_q4x4x2(ggml_rxd_tensor * t, const void * data, size_t size) {
    int64_t nrows = ggml_rxd_nrows(t);

    size_t row_size    = ggml_rxd_row_size(t->type, t->ne[0]);
    size_t row_size_pd = ggml_rxd_row_size(t->type, hex_round_up(t->ne[0], QK_Q4_0x4x2));  // extra elements for the pad
    ...
    void * buf_pd = ggml_rxd_aligned_malloc(row_size_pd);
    GGML_RXD_ASSERT(buf_pd != NULL);

    void * buf_rp = ggml_rxd_aligned_malloc(row_size_rp);
    GGML_RXD_ASSERT(buf_rp != NULL);
    ...
    ggml_rxd_aligned_free(buf_pd, row_size_pd);
    ggml_rxd_aligned_free(buf_rp, row_size_rp);
}

// repack q4x4x2 tensor into q4_0 data
static void repack_q4x4x2_q4_0(void * data, const ggml_rxd_tensor * t, size_t size) {
    int64_t nrows = ggml_rxd_nrows(t);
...
static void dump_block_q8_0(const block_q8_0 * b, int i) {
    HEX_VERBOSE("ggml-hex: repack q8_0 %d: %d %d %d %d ... %d %d %d %d : %.6f\n", i, b->qs[0], b->qs[1], b->qs[2],
                b->qs[3], b->qs[28], b->qs[29], b->qs[30], b->qs[31], GGML_RXD_FP16_TO_FP32(b->d));
}
...
static void repack_q8_0_q8x4x2(ggml_rxd_tensor * t, const void * data, size_t size) {
    int64_t nrows = ggml_rxd_nrows(t);

    size_t row_size    = ggml_rxd_row_size(t->type, t->ne[0]);
    size_t row_size_pd = ggml_rxd_row_size(t->type, hex_round_up(t->ne[0], QK_Q8_0x4x2));  // extra elements for the pad
    size_t row_size_rp = row_size * 2;  // extra space for tmp pad (if any)
...
static void repack_q8x4x2_q8_0(void * data, const ggml_rxd_tensor * t, size_t size) {
    int64_t nrows = ggml_rxd_nrows(t);
...
static void dump_block_mxfp4(const block_mxfp4 * b, int i) {
    HEX_VERBOSE("ggml-hex: repack mxfp4 %d: %d %d %d %d ... %d %d %d %d : %.6f\n", i, unpack_mxfp4(b->qs[0]).v[0],
                ...
                unpack_mxfp4(b->qs[15]).v[1], GGML_RXD_E8M0_TO_FP32_HALF(b->e));
}
...
static void dump_packed_block_mxfp4x4x2(const uint8_t * v, unsigned int i, size_t k) {
    ...
    HEX_VERBOSE("ggml-hex: repack mxfp4x4x2-%d: %d %d %d %d ... %d %d %d %d ... %d %d %d %d : %.6f %.6f %.6f %.6f\n", i,
                ...
                unpack_mxfp4(q[127]).v[0], GGML_RXD_E8M0_TO_FP32_HALF(e[0]), GGML_RXD_E8M0_TO_FP32_HALF(e[1]),
                GGML_RXD_E8M0_TO_FP32_HALF(e[2]), GGML_RXD_E8M0_TO_FP32_HALF(e[3]));
    ...
    HEX_VERBOSE("ggml-hex: repack mxfp4x4x2-%d: %d %d %d %d ... %d %d %d %d ... %d %d %d %d : %.6f %.6f %.6f %.6f\n",
                ...
                unpack_mxfp4(q[126]).v[1], unpack_mxfp4(q[127]).v[1], GGML_RXD_E8M0_TO_FP32_HALF(e[4]),
                GGML_RXD_E8M0_TO_FP32_HALF(e[5]), GGML_RXD_E8M0_TO_FP32_HALF(e[6]), GGML_RXD_E8M0_TO_FP32_HALF(e[7]));
}
...
static void repack_mxfp4_mxfp4x4x2(ggml_rxd_tensor * t, const void * data, size_t size) {
    int64_t nrows = ggml_rxd_nrows(t);

    size_t row_size    = ggml_rxd_row_size(t->type, t->ne[0]);
    size_t row_size_pd = ggml_rxd_row_size(t->type, hex_round_up(t->ne[0], QK_MXFP4x4x2));  // extra elements for the pad
    size_t row_size_rp = row_size * 2;  // extra space for tmp pad (if any)
...
static void repack_mxfp4x4x2_mxfp4(void * data, const ggml_rxd_tensor * t, size_t size) {
    int64_t nrows = ggml_rxd_nrows(t);
...
static void ggml_rxd_backend_hexagon_buffer_set_tensor(ggml_rxd_backend_buffer_t buffer,
                                                   ggml_rxd_tensor *         tensor,
                                                   const void *          data,
                                                   size_t                offset,
                                                   size_t                size) {
    auto ctx  = (ggml_rxd_backend_hexagon_buffer_context *) buffer->context;
    auto sess = ctx->sess;
    ...
    switch (tensor->type) {
        case GGML_RXD_TYPE_Q4_0:
            ...
            GGML_RXD_ASSERT(offset + size <= ggml_rxd_nbytes(tensor));
...
static void ggml_rxd_backend_hexagon_buffer_get_tensor(ggml_rxd_backend_buffer_t buffer,
...
static bool ggml_rxd_backend_hexagon_buffer_cpy_tensor(ggml_rxd_backend_buffer_t      buffer,
...
static void ggml_rxd_backend_hexagon_buffer_clear(ggml_rxd_backend_buffer_t buffer, uint8_t value) {
    auto ctx  = (ggml_rxd_backend_hexagon_buffer_context *) buffer->context;
    auto sess = ctx->sess;
...
static ggml_rxd_backend_buffer_i ggml_rxd_backend_hexagon_buffer_interface = {
    /* .free_buffer     = */ ggml_rxd_backend_hexagon_buffer_free_buffer,
    /* .get_base        = */ ggml_rxd_backend_hexagon_buffer_get_base,
    /* .init_tensor     = */ ggml_rxd_backend_hexagon_buffer_init_tensor,
    /* .memset_tensor   = */ NULL,
    /* .set_tensor      = */ ggml_rxd_backend_hexagon_buffer_set_tensor,
    /* .get_tensor      = */ ggml_rxd_backend_hexagon_buffer_get_tensor,
    /* .cpy_tensor      = */ ggml_rxd_backend_hexagon_buffer_cpy_tensor,
    /* .clear           = */ ggml_rxd_backend_hexagon_buffer_clear,
    /* .reset           = */ NULL,
};

// ** backend buffer type

static const char * ggml_rxd_backend_hexagon_buffer_type_name(ggml_rxd_backend_buffer_type_t buffer_type) {
    return static_cast<ggml_rxd_backend_hexagon_buffer_type_context *>(buffer_type->context)->name.c_str();
}

static ggml_rxd_backend_buffer_t ggml_rxd_backend_hexagon_buffer_type_alloc_buffer(
            ggml_rxd_backend_buffer_type_t buffer_type, size_t size) {
    auto sess = static_cast<ggml_rxd_backend_hexagon_buffer_type_context *>(buffer_type->context)->sess;
    try {
        ggml_rxd_backend_hexagon_buffer_context * ctx = new ggml_rxd_backend_hexagon_buffer_context(sess, size, false /*repack*/);
        return ggml_rxd_backend_buffer_init(buffer_type, ggml_rxd_backend_hexagon_buffer_interface, ctx, size);
    } catch (std::exception const &exc) {
        GGML_RXD_LOG_ERROR("ggml-hex: %s failed to allocate buffer context: %s\n", sess->name.c_str(), exc.what());
        return nullptr;
    }
}

static ggml_rxd_backend_buffer_t ggml_rxd_backend_hexagon_repack_buffer_type_alloc_buffer(
            ggml_rxd_backend_buffer_type_t buffer_type, size_t size) {
    auto sess = static_cast<ggml_rxd_backend_hexagon_buffer_type_context *>(buffer_type->context)->sess;
    try {
        ggml_rxd_backend_hexagon_buffer_context * ctx = new ggml_rxd_backend_hexagon_buffer_context(sess, size, true /*repack*/);
        return ggml_rxd_backend_buffer_init(buffer_type, ggml_rxd_backend_hexagon_buffer_interface, ctx, size);
    } catch (std::exception const &exc) {
        GGML_RXD_LOG_ERROR("ggml-hex: %s failed to allocate buffer context: %s\n", sess->name.c_str(), exc.what());
        return nullptr;
...
static size_t ggml_rxd_backend_hexagon_buffer_type_get_alignment(ggml_rxd_backend_buffer_type_t buffer_type) {
    return 128;  // HVX alignment
    GGML_RXD_UNUSED(buffer_type);
}

static size_t ggml_rxd_backend_hexagon_buffer_type_get_alloc_size(ggml_rxd_backend_buffer_type_t buft, const struct ggml_rxd_tensor * t) {
    return ggml_rxd_nbytes(t);
}

static size_t ggml_rxd_backend_hexagon_buffer_type_get_max_size(ggml_rxd_backend_buffer_type_t buffer_type) {
    return 1 * 1024 * 1024 * 1024;  // 1GB per buffer
    GGML_RXD_UNUSED(buffer_type);
}

static bool ggml_rxd_backend_hexagon_buffer_type_is_host(ggml_rxd_backend_buffer_type_t buft) {
    return opt_hostbuf;
    GGML_RXD_UNUSED(buft);
}

static bool ggml_rxd_backend_hexagon_repack_buffer_type_is_host(ggml_rxd_backend_buffer_type_t buft) {
    return false;
    GGML_RXD_UNUSED(buft);
}

static ggml_rxd_backend_buffer_type_i ggml_rxd_backend_hexagon_buffer_type_interface = {
    /* .get_name         = */ ggml_rxd_backend_hexagon_buffer_type_name,
    /* .alloc_buffer     = */ ggml_rxd_backend_hexagon_buffer_type_alloc_buffer,
    /* .get_alignment    = */ ggml_rxd_backend_hexagon_buffer_type_get_alignment,
    /* .get_max_size     = */ ggml_rxd_backend_hexagon_buffer_type_get_max_size,
    /* .get_alloc_size   = */ ggml_rxd_backend_hexagon_buffer_type_get_alloc_size,
    /* .is_host          = */ ggml_rxd_backend_hexagon_buffer_type_is_host,
};

static ggml_rxd_backend_buffer_type_i ggml_rxd_backend_hexagon_repack_buffer_type_interface = {
    /* .get_name         = */ ggml_rxd_backend_hexagon_buffer_type_name,
    /* .alloc_buffer     = */ ggml_rxd_backend_hexagon_repack_buffer_type_alloc_buffer,
    /* .get_alignment    = */ ggml_rxd_backend_hexagon_buffer_type_get_alignment,
    /* .get_max_size     = */ ggml_rxd_backend_hexagon_buffer_type_get_max_size,
    /* .get_alloc_size   = */ ggml_rxd_backend_hexagon_buffer_type_get_alloc_size,
    /* .is_host          = */ ggml_rxd_backend_hexagon_repack_buffer_type_is_host,
};

void ggml_rxd_hexagon_session::allocate(int dev_id) noexcept(false) {
    ...
    GGML_RXD_LOG_INFO("ggml-hex: allocating new session: %s\n", this->name.c_str());

    domain * my_domain = get_domain(this->domain_id);
    if (my_domain == NULL) {
        GGML_RXD_LOG_ERROR("ggml-hex: unable to get domain struct for CDSP\n");
    ...
    if (dev_id != 0) {
        ...
        if (err != AEE_SUCCESS) {
            GGML_RXD_LOG_ERROR("ggml-hex: failed to reserve new session %d : error 0x%x\n", dev_id, err);
    ...
    char session_uri[256];
    {
        ...
        if (err != AEE_SUCCESS) {
            ...
            GGML_RXD_LOG_WARN("ggml-hex: failed to get URI for session %d : error 0x%x. Falling back to single session URI: %s\n", dev_id, err, session_uri);
        }
    }
    ...
    // Enable Unsigned PD
    {
        ...
        if (err != AEE_SUCCESS) {
            GGML_RXD_LOG_ERROR("ggml-hex: failed to enable unsigned PD for session %d : error 0x%x\n", dev_id, err);
    ...
    if (err != AEE_SUCCESS) {
        GGML_RXD_LOG_ERROR("ggml-hex: failed to open session %d : error 0x%x\n", dev_id, err);
        throw std::runtime_error("ggml-hex: failed to open session (see log for details)");
    }

    this->valid_handle = true;

    GGML_RXD_LOG_INFO("ggml-hex: new session: %s : session-id %d domain-id %d uri %s handle 0x%lx\n", this->name.c_str(),
    ...
    // Enable FastRPC QoS mode
    {
        ...
        if (err != 0) {
            GGML_RXD_LOG_WARN("ggml-hex: failed to enable fastrpc QOS mode: 0x%08x\n", (unsigned) err);
        }
    }
    ...
    err = dspqueue_create(this->domain_id,
                          ...
                          &queue);
    if (err != 0) {
        GGML_RXD_LOG_ERROR("ggml-hex: %s dspqueue_create failed: 0x%08x\n", this->name.c_str(), (unsigned) err);
    ...
    if (err != 0) {
        GGML_RXD_LOG_ERROR("ggml-hex: dspqueue_export failed: 0x%08x\n", (unsigned) err);
    ...
    if (opt_etm) {
        ...
        if (err != 0) {
            GGML_RXD_LOG_ERROR("ggml-hex: failed to enable ETM tracing: 0x%08x\n", (unsigned) err);
        }
    }
    ...
    if (err != 0) {
        GGML_RXD_LOG_ERROR("ggml-hex: failed to start session: 0x%08x\n", (unsigned) err);
        throw std::runtime_error("ggml-hex: iface start failed (see log for details)");
    }
    this->valid_iface = true;
}

void ggml_rxd_hexagon_session::release() noexcept(true) {
    GGML_RXD_LOG_INFO("ggml-hex: releasing session: %s\n", this->name.c_str());
    ...
    if (this->valid_iface) {
        ...
        if (err != 0) {
            GGML_RXD_ABORT("ggml-hex: htp_iface_stop failed: 0x%08x\n", (unsigned) err);
        }
    }
    ...
    if (opt_etm) {
        ...
        if (err != 0) {
            GGML_RXD_LOG_ERROR("ggml-hex: warn : failed to disable ETM tracing: 0x%08x\n", (unsigned) err);
        }
    }
    ...
    if (this->valid_queue) {
        ...
        if (err != 0) {
            GGML_RXD_ABORT("ggml-hex: dspqueue_close failed: 0x%08x\n", (unsigned) err);
        }
    }
...
ggml_rxd_hexagon_session::ggml_rxd_hexagon_session(int dev_id, ggml_rxd_backend_dev_t dev) noexcept(false) {
    ...
    try {
        ...
        buffer_type.iface   = ggml_rxd_backend_hexagon_buffer_type_interface;
        buffer_type.context = new ggml_rxd_backend_hexagon_buffer_type_context(this->name, this);

        repack_buffer_type.iface   = ggml_rxd_backend_hexagon_repack_buffer_type_interface;
        repack_buffer_type.context = new ggml_rxd_backend_hexagon_buffer_type_context(this->name + "-REPACK", this);
    } catch (std::exception const &exc) {
        release();
        throw;
    }
...
ggml_rxd_hexagon_session::~ggml_rxd_hexagon_session() noexcept(true) {
    release();

    delete static_cast<ggml_rxd_backend_hexagon_buffer_type_context*>(buffer_type.context);
    delete static_cast<ggml_rxd_backend_hexagon_buffer_type_context*>(repack_buffer_type.context);
}

// ** backend interface

static bool ggml_rxd_backend_buffer_is_hexagon(const struct ggml_rxd_backend_buffer * b) {
    return b->buft->iface.get_alignment == ggml_rxd_backend_hexagon_buffer_type_get_alignment;
}

static inline bool ggml_rxd_backend_buffer_is_hexagon_repack(const struct ggml_rxd_backend_buffer * b) {
    return b->buft->iface.alloc_buffer == ggml_rxd_backend_hexagon_repack_buffer_type_alloc_buffer;
}

static bool hex_supported_dims2(const struct ggml_rxd_tensor * x, const struct ggml_rxd_tensor * y) {
    if (x->ne[0] != y->ne[0]) {
        return false;
    }
    if (x->ne[1] != y->ne[1]) {
...
static bool hex_supported_src0_type(ggml_rxd_type t) {
    return t == GGML_RXD_TYPE_F32;
}

static bool hex_supported_src1_type(ggml_rxd_type t) {
    return t == GGML_RXD_TYPE_F32;
}

static bool hex_supported_src2_type(ggml_rxd_type t) {
    return t == GGML_RXD_TYPE_F32;
}

static bool hex_supported_src1_type2(ggml_rxd_type t) {
    return t == GGML_RXD_TYPE_F16;
}

static bool hex_supported_src1_type3(ggml_rxd_type t) {
    return t == GGML_RXD_TYPE_I32;
}

static bool hex_supported_dst_type(ggml_rxd_type t) {
    return t == GGML_RXD_TYPE_F32;
}

static bool hex_supported_dims(const struct ggml_rxd_tensor * x, const struct ggml_rxd_tensor * y) {
...
static bool ggml_rxd_hexagon_supported_mul_mat(const struct ggml_rxd_hexagon_session * sess, const struct ggml_rxd_tensor * dst) {
    ...
    if (!ggml_rxd_is_contiguous(src1) || !ggml_rxd_is_contiguous(dst)) {
        return false;
    }
    ...
    switch (src0->type) {
        ...
        case GGML_RXD_TYPE_MXFP4:
            ...
            if (src0->buffer && !ggml_rxd_backend_buffer_is_hexagon_repack(src0->buffer)) {
                return false;
            }
            break;
    ...
    if (src0->buffer &&
        (!ggml_rxd_backend_buffer_is_hexagon(src0->buffer) || ggml_rxd_backend_hexagon_buffer_get_sess(src0->buffer) != sess)) {
        return false;
    }
    if (src1->buffer &&
        (!ggml_rxd_backend_buffer_is_hexagon(src1->buffer) || ggml_rxd_backend_hexagon_buffer_get_sess(src1->buffer) != sess)) {
        return false;
    }
    if (dst->buffer &&
        (!ggml_rxd_backend_buffer_is_hexagon(dst->buffer) || ggml_rxd_backend_hexagon_buffer_get_sess(dst->buffer) != sess)) {
        return false;
    }
...
static bool ggml_rxd_hexagon_supported_mul_mat_id(const struct ggml_rxd_hexagon_session * sess, const struct ggml_rxd_tensor * op) {
    const struct ggml_rxd_tensor * src0 = op->src[0];
    const struct ggml_rxd_tensor * src1 = op->src[1];
    const struct ggml_rxd_tensor * src2 = op->src[2];
    const struct ggml_rxd_tensor * dst  = op;

    if (src1->type != GGML_RXD_TYPE_F32 || dst->type != GGML_RXD_TYPE_F32 || src2->type != GGML_RXD_TYPE_I32) {
        return false;
    }
    ...
    if (src2->buffer &&
        (!ggml_rxd_backend_buffer_is_hexagon(src2->buffer) || ggml_rxd_backend_hexagon_buffer_get_sess(src2->buffer) != sess)) {
        return false;
    }
    if (dst->buffer &&
        (!ggml_rxd_backend_buffer_is_hexagon(dst->buffer) || ggml_rxd_backend_hexagon_buffer_get_sess(dst->buffer) != sess)) {
...
static bool ggml_rxd_hexagon_supported_binary(const struct ggml_rxd_hexagon_session * sess, const struct ggml_rxd_tensor * op) {
    const struct ggml_rxd_tensor * src0 = op->src[0];
    const struct ggml_rxd_tensor * src1 = op->src[1];
    const struct ggml_rxd_tensor * dst  = op;
    ...
    if (!ggml_rxd_can_repeat(src1, src0)) {
        return false;
    }
...
static bool ggml_rxd_hexagon_supported_add_id(const struct ggml_rxd_hexagon_session * sess, const struct ggml_rxd_tensor * op) {
    const struct ggml_rxd_tensor * src0 = op->src[0];
    const struct ggml_rxd_tensor * src1 = op->src[1];
    const struct ggml_rxd_tensor * src2 = op->src[2];
    const struct ggml_rxd_tensor * dst  = op;
...
static bool ggml_rxd_hexagon_supported_unary(const struct ggml_rxd_hexagon_session * sess, const struct ggml_rxd_tensor * op) {
    const struct ggml_rxd_tensor * src0 = op->src[0];
    const struct ggml_rxd_tensor * dst  = op;
...
static bool ggml_rxd_hexagon_supported_activations(const struct ggml_rxd_hexagon_session * sess,
...
static bool ggml_rxd_hexagon_supported_softmax(const struct ggml_rxd_hexagon_session * sess, const struct ggml_rxd_tensor * op) {
    const struct ggml_rxd_tensor * src0 = op->src[0];
    const struct ggml_rxd_tensor * src1 = op->src[1];
    const struct ggml_rxd_tensor * src2 = op->src[2];
    const struct ggml_rxd_tensor * dst  = op;
...
static bool ggml_rxd_hexagon_supported_rope(const struct ggml_rxd_hexagon_session * sess, const struct ggml_rxd_tensor * op) {
    const int32_t * op_params = &op->op_params[0];

    int mode = op_params[2];

    if ((mode & GGML_RXD_ROPE_TYPE_NEOX) || (mode & GGML_RXD_ROPE_TYPE_MROPE) || (mode & GGML_RXD_ROPE_TYPE_VISION)) {
        return false;
    }
    if (mode & 1) {
        return false;
    ...
    if (!hex_supported_src0_type(src0->type)) {
        return false;  // FIXME: add support for GGML_RXD_TYPE_F16 for src0
    }
    if (!hex_supported_dst_type(dst->type)) {
        return false;
    }
    ...
    if (src2) {
        if (!ggml_rxd_is_contiguous(src0) || !ggml_rxd_is_contiguous(src1) || !ggml_rxd_is_contiguous(src2) ||
            !ggml_rxd_is_contiguous(dst)) {
            return false;
        }
    } else {
        if (!ggml_rxd_is_contiguous(src0) || !ggml_rxd_is_contiguous(src1) || !ggml_rxd_is_contiguous(dst)) {
...
static void init_htp_tensor(htp_tensor * h, const ggml_rxd_tensor * t) {
...
static void hex_dump_dspbuf(const struct ggml_rxd_tensor * t, const dspqueue_buffer * d) {
    auto buf  = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(t->buffer->context);
    auto sess = buf->sess;
...
static void ggml_rxd_hexagon_mul_mat(const struct ggml_rxd_tensor * op, uint32_t flags) {
    const struct ggml_rxd_tensor * src0 = op->src[0];
    const struct ggml_rxd_tensor * src1 = op->src[1];
    const struct ggml_rxd_tensor * dst  = op;

    auto src0_buf = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(src0->buffer->context);
    auto src1_buf = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(src1->buffer->context);
    auto dst_buf  = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(dst->buffer->context);
    ...
    bufs[0].size   = ggml_rxd_nbytes(src0);
    bufs[0].flags  = 0;
    ...
    bufs[1].size   = ggml_rxd_nbytes(src1);
    ...
    bufs[2].size   = ggml_rxd_nbytes(dst);
    ...
    if (opt_verbose) {
        char dims[64 * GGML_RXD_MAX_SRC];
        char strides[64 * GGML_RXD_MAX_SRC];
        char types[16 * GGML_RXD_MAX_SRC];
        char buffs[64 * GGML_RXD_MAX_SRC];
        char names[64 * GGML_RXD_MAX_SRC];
    ...
    HEX_PROFILE(
        ...
        "call-usec %llu\n",
        sess->name.c_str(), ggml_rxd_op_name(op->op), src0->name, (uint32_t) src0->ne[0], (uint32_t) src0->ne[1],
...
static void ggml_rxd_hexagon_mul_mat_id(const struct ggml_rxd_tensor * op, uint32_t flags) {
    const struct ggml_rxd_tensor * src0 = op->src[0];
    const struct ggml_rxd_tensor * src1 = op->src[1];
    const struct ggml_rxd_tensor * src2 = op->src[2];
    const struct ggml_rxd_tensor * dst  = op;
    ...
    auto src0_buf = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(src0->buffer->context);
    auto src1_buf = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(src1->buffer->context);
    auto src2_buf = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(src2->buffer->context);
    auto dst_buf  = static_cast<ggml_rxd_backend_hexagon_buffer_context *>(dst->buffer->context);
    ...
    bufs[2].size   = ggml_rxd_nbytes(src2);
    ...
    bufs[3].size   = ggml_rxd_nbytes(dst);
...
static void ggml_rxd_hexagon_binary(const struct ggml_rxd_tensor * op, uint32_t flags) {
    const struct ggml_rxd_tensor * node = op;
    const struct ggml_rxd_tensor * src0 = node->src[0];
    const struct ggml_rxd_tensor * src1 = node->src[1];
    const struct ggml_rxd_tensor * dst  = node;
    ...
    switch (node->op) {
        ...
        case GGML_RXD_OP_SUB:
            ...
            break;
        default:
            GGML_RXD_ABORT("ggml-hex: binary : unsupported op:%d\n", node->op);
    }
    ...
    ggml_rxd_hexagon_session * sess = src0_buf->sess;

    if (opt_verbose) {
        char dims[64 * GGML_RXD_MAX_SRC];
        char strides[16 * GGML_RXD_MAX_SRC];
        char types[16 * GGML_RXD_MAX_SRC];
        char buffs[64 * GGML_RXD_MAX_SRC];
        char names[64 * GGML_RXD_MAX_SRC];
        ...
        HEX_VERBOSE("ggml-hex: %s %s : %s : %s : %s : %s : %s : flags 0x%x\n", sess->name.c_str(),
                    ggml_rxd_op_name(node->op), names, dims, types, strides, buffs, req.flags);
        if (opt_verbose > 1) {
            hex_dump_dspbuf(src0, &bufs[0]);
            hex_dump_dspbuf(src1, &bufs[1]);
            hex_dump_dspbuf(dst, &bufs[2]);
    ...
    HEX_PROFILE(
        "ggml-hex: %s %s %s %u:%u:%u:%u x %s %u:%u:%u:%u -> %s %u:%u:%u:%u : op-usec %u op-cycles %u op-pkts %u (%f) "
        "call-usec %llu\n",
        sess->name.c_str(), ggml_rxd_op_name(node->op), src0->name, (uint32_t) src0->ne[0], (uint32_t) src0->ne[1],
        (uint32_t) src0->ne[2], (uint32_t) src0->ne[3], src1->name, (uint32_t) src1->ne[0], (uint32_t) src1->ne[1],
        (uint32_t) src1->ne[2], (uint32_t) src1->ne[3], dst->name, (uint32_t) dst->ne[0], (uint32_t) dst->ne[1],
        (uint32_t) dst->ne[2], (uint32_t) dst->ne[3], sess->prof_usecs, sess->prof_cycles, sess->prof_pkts,
        (float) sess->prof_cycles / sess->prof_pkts, (unsigned long long) t2 - t1);
...
static void ggml_rxd_hexagon_add_id(const struct ggml_rxd_tensor * op, uint32_t flags) {
    const struct ggml_rxd_tensor * node = op;
    const struct ggml_rxd_tensor * src0 = node->src[0];
    const struct ggml_rxd_tensor * src1 = node->src[1];
    const struct ggml_rxd_tensor * src2 = node->src[2];
    const struct ggml_rxd_tensor * dst  = node;
    ...
    switch (node->op) {
        case GGML_RXD_OP_ADD_ID:
            ...
            break;
        default:
            GGML_RXD_ABORT("ggml-hex: unsupported op:%d\n", node->op);
    }
...
static void ggml_rxd_hexagon_unary(const struct ggml_rxd_tensor * op, uint32_t flags) {
    const struct ggml_rxd_tensor * src0 = op->src[0];
    const struct ggml_rxd_tensor * src1 = op->src[1];
    const struct ggml_rxd_tensor * dst  = op;
    ...
    switch (op->op) {
        ...
        case GGML_RXD_OP_UNARY:
            if (ggml_rxd_get_unary_op(dst) == GGML_RXD_UNARY_OP_SILU) {
        ...
        case GGML_RXD_OP_GLU:
            if (ggml_rxd_get_glu_op(dst) == GGML_RXD_GLU_OP_SWIGLU) {
                req.op    = HTP_OP_GLU_SWIGLU;
                supported = true;
            } else if (ggml_rxd_get_glu_op(dst) == GGML_RXD_GLU_OP_SWIGLU_OAI) {
    ...
    if (!supported) {
        GGML_RXD_ABORT("ggml-hex: unary : unsupported op:%d\n", op->op);
    }
    ...
    bufs[n_bufs].size   = ggml_rxd_nbytes(src0);
    ...
    if (src1) {
        ...
        bufs[n_bufs].size   = ggml_rxd_nbytes(src1);
        bufs[n_bufs].flags  = (DSPQUEUE_BUFFER_FLAG_FLUSH_SENDER |         // Flush CPU
                              DSPQUEUE_BUFFER_FLAG_INVALIDATE_RECIPIENT);  // Invalidate DSP
        ++n_bufs;
    }
    ...
    bufs[n_bufs].size   = ggml_rxd_nbytes(dst);
...
static void ggml_rxd_hexagon_rope(const struct ggml_rxd_tensor * op, uint32_t flags) {
    const struct ggml_rxd_tensor * src0 = op->src[0];
    const struct ggml_rxd_tensor * src1 = op->src[1];
    const struct ggml_rxd_tensor * src2 = op->src[2];
    const struct ggml_rxd_tensor * dst  = op;
    ...
    if (src2) {
        ...
        bufs[n_bufs].size   = ggml_rxd_nbytes(src2);
        bufs[n_bufs].flags  = (DSPQUEUE_BUFFER_FLAG_FLUSH_SENDER |         // Flush CPU
                              DSPQUEUE_BUFFER_FLAG_INVALIDATE_RECIPIENT);  // Invalidate DSP
        ++n_bufs;
    }
...
static const char * ggml_rxd_backend_hexagon_name(ggml_rxd_backend_t backend) {
    auto sess = static_cast<ggml_rxd_hexagon_session *>(backend->context);
    return sess->name.c_str();
}

static void ggml_rxd_backend_hexagon_free(ggml_rxd_backend_t backend) {
    // we just need to delete the backend here
    // the sessions are allocated & freed as part of the registry
    delete backend;
}

static inline bool op_reuse_src1(const ggml_rxd_tensor * op1, const ggml_rxd_tensor * op0) {
    return (op0 && op0->src[1] == op1->src[1]);
}

static inline bool is_compute_op(ggml_rxd_tensor *node)
{
    return !(ggml_rxd_op_is_empty(node->op) || ggml_rxd_is_empty(node));
}

// scan the graph and figure out last compute op index
static inline int last_compute_op(ggml_rxd_cgraph * graph) {
    int last = 0;
...
static ggml_rxd_status ggml_rxd_backend_hexagon_graph_compute(ggml_rxd_backend_t backend, ggml_rxd_cgraph * graph) {
    auto sess = static_cast<ggml_rxd_hexagon_session *>(backend->context);
    ...
    const struct ggml_rxd_tensor * prev_quant_op = nullptr;  // prev executed op with quantizer

    for (int i = 0; i < graph->n_nodes; ++i) {
        ggml_rxd_tensor * node = graph->nodes[i];
        ...
        switch (node->op) {
            case GGML_RXD_OP_MUL_MAT:
                ggml_rxd_hexagon_mul_mat(node, flags);
                prev_quant_op = node;
                break;
            case GGML_RXD_OP_MUL_MAT_ID:
                ggml_rxd_hexagon_mul_mat_id(node, flags);
                prev_quant_op = node;
                break;
            case GGML_RXD_OP_MUL:
            case GGML_RXD_OP_ADD:
            case GGML_RXD_OP_SUB:
                ggml_rxd_hexagon_binary(node, flags);
                break;
            case GGML_RXD_OP_ADD_ID:
                ggml_rxd_hexagon_add_id(node, flags);
                break;
            case GGML_RXD_OP_RMS_NORM:
                ggml_rxd_hexagon_unary(node, flags);
                break;
            case GGML_RXD_OP_UNARY:
                if (ggml_rxd_get_unary_op(node) == GGML_RXD_UNARY_OP_SILU) {
                    ggml_rxd_hexagon_unary(node, flags);
                }
                break;
            case GGML_RXD_OP_GLU:
                if ((ggml_rxd_get_glu_op(node) == GGML_RXD_GLU_OP_SWIGLU) ||
                    (ggml_rxd_get_glu_op(node) == GGML_RXD_GLU_OP_SWIGLU_OAI)) {
                    ggml_rxd_hexagon_unary(node, flags);
                }
                break;
            case GGML_RXD_OP_SOFT_MAX:
            ...
            case GGML_RXD_OP_ROPE:
                ggml_rxd_hexagon_rope(node, flags);
                break;

            default:
                GGML_RXD_ABORT("\nggml-hex: graph-compute %s is not supported\n", ggml_rxd_op_desc(node));
        }
    }
...
static void ggml_rxd_backend_hexagon_synchronize(ggml_rxd_backend_t backend) {
    auto sess = static_cast<ggml_rxd_hexagon_session *>(backend->context);
...
struct node_info {
    ggml_rxd_tensor * node;

    std::vector<ggml_rxd_tensor *> fused;

    ggml_rxd_op op() const {
        return node->op;
    }

    const ggml_rxd_tensor * dst() const {
        return fused.empty() ? node : fused.back();
    }

    const ggml_rxd_tensor * src0() const {
        return node->src[0];
    }

    const ggml_rxd_tensor * src1() const {
        return node->src[1];
    }

    bool is_empty() const {
        return ggml_rxd_op_is_empty(node->op);
    }

    void add_fused(ggml_rxd_tensor * t) {
        fused.push_back(t);
    }
    ...
    bool stackable() const {
        switch (this->op()) {
            case GGML_RXD_OP_MUL_MAT:
            case GGML_RXD_OP_MUL_MAT_ID:
                return ggml_rxd_is_quantized(this->src0()->type);
            default:
                return false;
        }
    }
...
static std::vector<int> ggml_rxd_hexagon_graph_optimize_reorder(const std::vector<node_info> & nodes) {
...
static void ggml_rxd_backend_hexagon_graph_optimize(ggml_rxd_backend_t backend, ggml_rxd_cgraph * gf) {
    const int n = gf->n_nodes;

    constexpr int MAX_FUSE = 16;

    enum ggml_rxd_op ops[MAX_FUSE];
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (node.op() == GGML_RXD_OP_ADD ||
            node.op() == GGML_RXD_OP_NORM ||
            node.op() == GGML_RXD_OP_RMS_NORM) {
            ops[0] = node.op();
            ...
            while (f < n && f < i + MAX_FUSE) {
                ...
                // can be expanded when needed
                if (gf->nodes[f]->op != GGML_RXD_OP_ADD &&
                    gf->nodes[f]->op != GGML_RXD_OP_MUL &&
                    gf->nodes[f]->op != GGML_RXD_OP_NORM &&
                    gf->nodes[f]->op != GGML_RXD_OP_RMS_NORM) {
                    break;
                }
            ...
            f -= i;
            for (; f > 1; f--) {
                if (ggml_rxd_can_fuse(gf, i, ops, f)) {
                    break;
                }
            }
    ...
    const auto order = ggml_rxd_hexagon_graph_optimize_reorder(nodes);
...
static struct ggml_rxd_backend_i hexagon_backend_i = {
    /* .get_name                = */ ggml_rxd_backend_hexagon_name,
    /* .free                    = */ ggml_rxd_backend_hexagon_free,
    /* .set_tensor_async        = */ NULL,
    /* .get_tensor_async        = */ NULL,
    /* .cpy_tensor_async        = */ NULL,
    /* .synchronize             = */ ggml_rxd_backend_hexagon_synchronize,
    /* .graph_plan_create       = */ NULL,
    /* .graph_plan_free         = */ NULL,
    /* .graph_plan_update       = */ NULL,
    /* .graph_plan_compute      = */ NULL,
    /* .graph_compute           = */ ggml_rxd_backend_hexagon_graph_compute,
    /* .event_record            = */ NULL,
    /* .event_wait              = */ NULL,
    /* .graph_optimize          = */ ggml_rxd_backend_hexagon_graph_optimize,
};

static ggml_rxd_guid_t ggml_rxd_backend_hexagon_guid() {
    static ggml_rxd_guid guid = { 0x7b, 0x57, 0xdc, 0xaf, 0xde, 0x12, 0x1d, 0x49,
                              0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11 };
    return &guid;
}

bool ggml_rxd_backend_is_hexagon(ggml_rxd_backend_t backend) {
    return backend && backend->iface.get_name == ggml_rxd_backend_hexagon_name;
}

// device interface

static ggml_rxd_backend_t ggml_rxd_backend_hexagon_device_init(ggml_rxd_backend_dev_t dev, const char * params) {
    auto sess = static_cast<ggml_rxd_hexagon_session *>(dev->context);

    return new ggml_rxd_backend{
        /* .guid      = */ ggml_rxd_backend_hexagon_guid(),
...
static const char * ggml_rxd_backend_hexagon_device_get_name(ggml_rxd_backend_dev_t dev) {
    auto sess = static_cast<ggml_rxd_hexagon_session *>(dev->context);
    return sess->name.c_str();
...
static const char * ggml_rxd_backend_hexagon_device_get_description(ggml_rxd_backend_dev_t dev) {
    return "Hexagon";
    GGML_RXD_UNUSED(dev);
}

static void ggml_rxd_backend_hexagon_device_get_memory(ggml_rxd_backend_dev_t dev, size_t * free, size_t * total) {
...
static enum ggml_rxd_backend_dev_type ggml_rxd_backend_hexagon_device_get_type(ggml_rxd_backend_dev_t dev) {
    return GGML_RXD_BACKEND_DEVICE_TYPE_GPU;

    GGML_RXD_UNUSED(dev);
}

static void ggml_rxd_backend_hexagon_device_get_props(ggml_rxd_backend_dev_t dev, struct ggml_rxd_backend_dev_props * props) {
    props->name        = ggml_rxd_backend_hexagon_device_get_name(dev);
    props->description = ggml_rxd_backend_hexagon_device_get_description(dev);
    props->type        = ggml_rxd_backend_hexagon_device_get_type(dev);
    ggml_rxd_backend_hexagon_device_get_memory(dev, &props->memory_free, &props->memory_total);
    props->caps = {
        /* .async                 = */ true,
...
static ggml_rxd_backend_buffer_type_t ggml_rxd_backend_hexagon_device_get_buffer_type(ggml_rxd_backend_dev_t dev) {
    auto sess = static_cast<ggml_rxd_hexagon_session *>(dev->context);
    return &sess->buffer_type;
}

static ggml_rxd_backend_buffer_type_t ggml_rxd_backend_hexagon_device_get_repack_buffer_type(ggml_rxd_backend_dev_t dev) {
    auto sess = static_cast<ggml_rxd_hexagon_session *>(dev->context);
    return &sess->repack_buffer_type;
}

static bool ggml_rxd_backend_hexagon_device_supports_op(ggml_rxd_backend_dev_t dev, const struct ggml_rxd_tensor * op) {
    auto sess = static_cast<ggml_rxd_hexagon_session *>(dev->context);
    ...
    switch (op->op) {
        ...
        case GGML_RXD_OP_MUL_MAT:
            supp = ggml_rxd_hexagon_supported_mul_mat(sess, op);
            break;

        case GGML_RXD_OP_MUL_MAT_ID:
            supp = ggml_rxd_hexagon_supported_mul_mat_id(sess, op);
            break;

        case GGML_RXD_OP_MUL:
        case GGML_RXD_OP_ADD:
        case GGML_RXD_OP_SUB:
            supp = ggml_rxd_hexagon_supported_binary(sess, op);
            break;

        case GGML_RXD_OP_ADD_ID:
            supp = ggml_rxd_hexagon_supported_add_id(sess, op);
            break;

        case GGML_RXD_OP_RMS_NORM:
            supp = ggml_rxd_hexagon_supported_unary(sess, op);
            break;

        case GGML_RXD_OP_SOFT_MAX:
            supp = ggml_rxd_hexagon_supported_softmax(sess, op);
            break;

        case GGML_RXD_OP_UNARY:
            if (ggml_rxd_get_unary_op(op) == GGML_RXD_UNARY_OP_SILU) {
                supp = ggml_rxd_hexagon_supported_activations(sess, op);
            }
            break;

        case GGML_RXD_OP_GLU:
            if ((ggml_rxd_get_glu_op(op) == GGML_RXD_GLU_OP_SWIGLU) /* || (ggml_rxd_get_glu_op(op) == GGML_RXD_GLU_OP_SWIGLU_OAI) */) {
                supp = ggml_rxd_hexagon_supported_activations(sess, op);
            }
            break;

        case GGML_RXD_OP_ROPE:
            supp = ggml_rxd_hexagon_supported_rope(sess, op);
            break;
    ...
    if (opt_verbose) {
        ...
        HEX_VERBOSE("ggml-hex: %s device-supports-op %s : %s : %s : %s : %s : %s : (%d)\n", sess->name.c_str(),
                    ggml_rxd_op_name(op->op), names, dims, types, strides, buffs, (int) supp);
    }
...
static bool ggml_rxd_backend_hexagon_device_supports_buft(ggml_rxd_backend_dev_t dev, ggml_rxd_backend_buffer_type_t buft) {
    if (buft->iface.get_alignment != ggml_rxd_backend_hexagon_buffer_type_get_alignment) {
        return false;
    }

    auto s0 = static_cast<ggml_rxd_hexagon_session *>(dev->context);
    auto s1 = static_cast<ggml_rxd_backend_hexagon_buffer_type_context *>(buft->context)->sess;
...
static ggml_rxd_backend_buffer_type_t * ggml_rxd_backend_hexagon_device_get_extra_buffers_type(ggml_rxd_backend_dev_t dev) {
    auto s0 = static_cast<ggml_rxd_hexagon_session *>(dev->context);
    HEX_VERBOSE("ggml-hex: device-get-extra-buft : %s \n", s0->name.c_str());

    static ggml_rxd_backend_buffer_type_t bufts[2];
    bufts[0] = ggml_rxd_backend_hexagon_device_get_repack_buffer_type(dev);
    bufts[1] = NULL;
    return bufts;
}

static const struct ggml_rxd_backend_device_i ggml_rxd_backend_hexagon_device_i = {
    /* .get_name             = */ ggml_rxd_backend_hexagon_device_get_name,
    /* .get_description      = */ ggml_rxd_backend_hexagon_device_get_description,
    /* .get_memory           = */ ggml_rxd_backend_hexagon_device_get_memory,
    /* .get_type             = */ ggml_rxd_backend_hexagon_device_get_type,
    /* .get_props            = */ ggml_rxd_backend_hexagon_device_get_props,
    /* .init_backend         = */ ggml_rxd_backend_hexagon_device_init,
    /* .get_buffer_type      = */ ggml_rxd_backend_hexagon_device_get_buffer_type,
    /* .get_host_buffer_type = */ NULL,  // ggml_rxd_backend_hexagon_device_get_host_buffer_type,
    /* .buffer_from_host_ptr = */ NULL,  // ggml_rxd_backend_hexagon_device_buffer_from_ptr,
    /* .supports_op          = */ ggml_rxd_backend_hexagon_device_supports_op,
    /* .supports_buft        = */ ggml_rxd_backend_hexagon_device_supports_buft,
    /* .offload_op           = */ NULL,  // ggml_rxd_backend_hexagon_device_offload_op,
    /* .event_new            = */ NULL,
    /* .event_free           = */ NULL,
...
#define GGML_RXD_HEXAGON_MAX_SESSIONS 16

struct ggml_rxd_hexagon_registry {
    ggml_rxd_hexagon_registry(ggml_rxd_backend_reg_t reg);
    ~ggml_rxd_hexagon_registry();

    ggml_rxd_backend_device devices[GGML_RXD_HEXAGON_MAX_SESSIONS];
};

ggml_rxd_hexagon_registry::ggml_rxd_hexagon_registry(ggml_rxd_backend_reg_t reg) {
    GGML_RXD_LOG_INFO("ggml-hex: Hexagon backend (experimental) : allocating new registry : ndev %zu\n", opt_ndev);

    if (!opt_arch) {
        int err = get_hex_arch_ver(CDSP_DOMAIN_ID, &opt_arch);
        if (err != 0) {
            GGML_RXD_LOG_ERROR("ggml-hex: failed to query HTP version (err %d) defaulting to v73\n", err);
            opt_arch = 73;
        }
    }
    ...
    if(opt_arch < 75) {
        opt_ndev = 1;
        GGML_RXD_LOG_WARN("ggml-hex: forcing ndev to 1 for SoCs archs lower than v75.\n");
    }

    GGML_RXD_LOG_INFO("ggml-hex: Hexagon Arch version v%d\n", opt_arch);

    // Create devices / sessions
    for (size_t i = 0; i < opt_ndev; i++) {
        devices[i].iface   = ggml_rxd_backend_hexagon_device_i;
        devices[i].reg     = reg;
        try {
            devices[i].context = new ggml_rxd_hexagon_session(i, &devices[i]);
        } catch (std::exception const &exc) {
            GGML_RXD_LOG_ERROR("ggml-hex: failed to create device/session %zu\n", i);
            devices[i].context = nullptr;
        }
    }
}

ggml_rxd_hexagon_registry::~ggml_rxd_hexagon_registry() {
    GGML_RXD_LOG_INFO("ggml-hex: releasing registry\n");

    // Release devices / sessions
    for (size_t i = 0; i < opt_ndev; i++) {
        auto sess = static_cast<ggml_rxd_hexagon_session *>(devices[i].context);
        delete sess;
    }
}

static const char * ggml_rxd_backend_hexagon_reg_get_name(ggml_rxd_backend_reg_t reg) {
    return "HTP";
    GGML_RXD_UNUSED(reg);
}

static size_t ggml_rxd_backend_hexagon_reg_get_device_count(ggml_rxd_backend_reg_t reg) {
    return opt_ndev;
    GGML_RXD_UNUSED(reg);
}

static ggml_rxd_backend_dev_t ggml_rxd_backend_hexagon_reg_get_device(ggml_rxd_backend_reg_t reg, size_t index) {
    auto hreg = static_cast<ggml_rxd_hexagon_registry *>(reg->context);
...
static void * ggml_rxd_backend_hexagon_get_proc_address(ggml_rxd_backend_reg_t reg, const char * name) {
    if (strcmp(name, "ggml_rxd_backend_dev_get_extra_bufts") == 0) {
        ggml_rxd_backend_dev_get_extra_bufts_t fct = ggml_rxd_backend_hexagon_device_get_extra_buffers_type;
        return (void *) fct;
    }
...
static void ggml_rxd_hexagon_init(ggml_rxd_backend_reg * reg) {
    // Basic sanity checks to make sure definitions match
    static_assert((unsigned int) HTP_TYPE_Q4_0 == (unsigned int) GGML_RXD_TYPE_Q4_0,
                  "please update hexagon_type to match ggml_rxd_type");
    static_assert((unsigned int) HTP_TYPE_Q8_0 == (unsigned int) GGML_RXD_TYPE_Q8_0,
                  "please update hexagon_type to match ggml_rxd_type");
    static_assert((unsigned int) HTP_TYPE_MXFP4 == (unsigned int) GGML_RXD_TYPE_MXFP4,
                  "please update hexagon_type to match ggml_rxd_type");

    const char * str_verbose = getenv("GGML_RXD_HEXAGON_VERBOSE");
    const char * str_hostbuf = getenv("GGML_RXD_HEXAGON_HOSTBUF");

    opt_verbose      = str_verbose ? atoi(str_verbose) : 0;
    opt_profile      = getenv("GGML_RXD_HEXAGON_PROFILE") != nullptr;
    opt_etm          = getenv("GGML_RXD_HEXAGON_ETM") != nullptr;
    opt_experimental = getenv("GGML_RXD_HEXAGON_EXPERIMENTAL") != nullptr;

    const char * str_opmask = getenv("GGML_RXD_HEXAGON_OPMASK");
    if (str_opmask != nullptr) {
        opt_opmask = strtoul(str_opmask, NULL, 0);
    }
    opt_opsync = getenv("GGML_RXD_HEXAGON_OPSYNC") != nullptr;

    const char * str_ndev = getenv("GGML_RXD_HEXAGON_NDEV");
    if (str_ndev) {
        opt_ndev = strtoul(str_ndev, NULL, 0);
        if (opt_ndev > GGML_RXD_HEXAGON_MAX_SESSIONS) {
            opt_ndev = GGML_RXD_HEXAGON_MAX_SESSIONS;
        }
    }

    const char * str_nhvx = getenv("GGML_RXD_HEXAGON_NHVX");
    if (str_nhvx) {
        opt_nhvx = strtoul(str_nhvx, NULL, 0);
    }

    const char * str_arch = getenv("GGML_RXD_HEXAGON_ARCH");
    if (str_arch) {
        if (str_arch[0] == 'v') {
            str_arch++;
        }
    ...
    reg->context = new ggml_rxd_hexagon_registry(reg);
...
static const struct ggml_rxd_backend_reg_i ggml_rxd_backend_hexagon_reg_i = {
    /* .get_name         = */ ggml_rxd_backend_hexagon_reg_get_name,
    /* .get_device_count = */ ggml_rxd_backend_hexagon_reg_get_device_count,
    /* .get_device       = */ ggml_rxd_backend_hexagon_reg_get_device,
    /* .get_proc_address = */ ggml_rxd_backend_hexagon_get_proc_address,
};

ggml_rxd_backend_reg_t ggml_rxd_backend_hexagon_reg(void) {
    static bool initialized = false;

    static ggml_rxd_backend_reg reg = { /* .api_version = */ GGML_RXD_BACKEND_API_VERSION,
                                    /* .iface       = */ ggml_rxd_backend_hexagon_reg_i,
                                    /* .context     = */ NULL };
...
GGML_RXD_BACKEND_DL_IMPL(ggml_rxd_backend_hexagon_reg)
