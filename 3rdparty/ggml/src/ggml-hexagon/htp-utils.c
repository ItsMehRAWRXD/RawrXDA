...
int get_domains_info(char * domain_type, int * num_domains, fastrpc_domain ** domains_info) {
    ...
    if (remote_system_request) {
        ...
        if (nErr != AEE_SUCCESS) {
            GGML_RXD_LOG_ERROR("Failure in remote_system_request call: %d.\n", nErr);
            goto bail;
        }
        ...
        if ((req.sys.domains = calloc(req.sys.num_domains, sizeof(fastrpc_domain))) == NULL) {
            nErr = AEE_ENOMEMORY;
            GGML_RXD_LOG_ERROR("Unable to allocate memory for req.sys.domains");
            goto bail;
        }
        ...
        for (int i = 0; i < req.sys.num_domains; i++) {
            ...
            if (domain->type != ss_info && domain_type != NULL) {
                nErr = -1;
                GGML_RXD_LOG_ERROR("Incorrect data received from remote_system_request.\n");
                goto bail;
            }
        }
...
int get_effective_domain_id(char * domain_name, int session_id, int * effec_domain_id) {
    ...
    if (err) {
        GGML_RXD_LOG_ERROR("Error 0x%x: failed to get effective domain id for %s, session id %d\n", err, sess.domain_name,
               session_id);
        return err;
    }
...
int get_dsp_support(int * domain) {
    ...
    *domain  = CDSP_DOMAIN_ID;  // DSP domain default value is CDSP_DOMAIN_ID
    ...
    if (remote_handle_control) {
        ...
        GGML_RXD_LOG_ERROR("remote_dsp_capability interface is not supported on this device\n");
    }
...
int get_vtcm_info(int domain, uint32_t * capability, uint32_t attr) {
    ...
    *capability = 0;
    ...
    if (attr == VTCM_PAGE || attr == VTCM_COUNT) {
        ...
        nErr = AEE_EBADPARM;
        GGML_RXD_LOG_ERROR("Unsupported attr. Only VTCM_PAGE and VTCM_COUNT supported\n");
        goto bail;
    }
    if (remote_handle_control) {
        if (domain == ADSP_DOMAIN_ID || domain == CDSP_DOMAIN_ID) {
            ...
            */
            ...
            if ((nErr & 0xFF) == (AEE_EUNSUPPORTEDAPI & 0xFF)) {
                GGML_RXD_LOG_ERROR("\nFastRPC Capability API is not supported on this device\n");
                GGML_RXD_LOG_ERROR("Running the usecase without checking the capability\n");
                nErr = AEE_SUCCESS;
                goto bail;
            ...
            nErr = AEE_EUNSUPPORTED;
            GGML_RXD_LOG_ERROR("Unsupported domain %d\n", domain);
            goto bail;
        }
    } else {
        nErr = AEE_EUNSUPPORTEDAPI;
...
bool is_unsignedpd_supported(int domain_id) {
    ...
    if (remote_handle_control) {
        ...
        nErr = AEE_EUNSUPPORTEDAPI;
        GGML_RXD_LOG_ERROR("remote_dsp_capability interface is not supported on this device. Falling back to signed pd.\n");
        return false;
    }
    return false;
}
    ...
    if (remote_handle_control) {
        if (domain == CDSP_DOMAIN_ID) {
            ...
            */
            ...
            nErr = AEE_EUNSUPPORTED;
            GGML_RXD_LOG_ERROR("Async fastrpc is not supported on domain %d\n", domain);
            goto bail;
        }
    } else {
        nErr = AEE_EUNSUPPORTEDAPI;
...
int get_hmx_support_info(int domain, uint32_t * capability, uint32_t attr) {
    ...
    *capability = 0;
    ...
    if (attr != HMX_SUPPORT_SPATIAL && attr != HMX_SUPPORT_DEPTH) {
        nErr = AEE_EBADPARM;
        GGML_RXD_LOG_ERROR("Unsupported attr. Only HMX_SUPPORT_SPATIAL and HMX_SUPPORT_DEPTH supported\n");
        goto bail;
    }
    if (remote_handle_control) {
        if (domain == CDSP_DOMAIN_ID) {
            ...
            */
            ...
            nErr = AEE_EUNSUPPORTED;
            GGML_RXD_LOG_ERROR("HMX support is not there for domain %d\n", domain);
            goto bail;
        }
    } else {
        nErr = AEE_EUNSUPPORTEDAPI;
...
int get_hex_arch_ver(int domain, int * arch) {
    if (!remote_handle_control) {
        GGML_RXD_LOG_ERROR("ggml-hex: remote_handle_control is not supported on this device\n");
    ...
    if ((err & 0xff) == (AEE_EUNSUPPORTEDAPI & 0xff)) {
        GGML_RXD_LOG_ERROR("ggml-hex: FastRPC capability API is not supported on this device\n");
        return AEE_EUNSUPPORTEDAPI;
    }

    if (err != AEE_SUCCESS) {
        GGML_RXD_LOG_ERROR("ggml-hex: FastRPC capability query failed (err %d)\n", err);
        return err;
    }
    ...
    if (remote_handle_control) {
        if (domain == CDSP_DOMAIN_ID) {
            ...
            */
            ...
            nErr = AEE_EUNSUPPORTED;
            GGML_RXD_LOG_ERROR("HVX support is not available on domain %d\n", domain);
            goto bail;
        }
    } else {
        nErr = AEE_EUNSUPPORTEDAPI;
