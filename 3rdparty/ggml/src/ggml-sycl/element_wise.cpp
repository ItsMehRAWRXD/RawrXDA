...
namespace ggml_rxd_sycl_detail {
...
static inline void dispatch_ggml_sycl_op_unary(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst, KernelInvoker kernel_invoker, Args&&... args) {
    ...
    switch (dst->type) {
        ...
        case GGML_RXD_TYPE_F16:
            {
                ...
                kernel_invoker(data_pts.src, data_pts.dst, (int)ggml_rxd_nelements(dst->src[0]), main_stream, std::forward<Args>(args)...);
                break;
            }
#endif
        ...
        default:
            GGML_RXD_ABORT("GGML tensor type not supported!\n");
    }
}

template<typename KernelInvoker, typename... Args>
static inline void dispatch_ggml_sycl_op_fused_glu(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst, KernelInvoker kernel_invoker, Args&&... args) {
#if defined (GGML_RXD_SYCL_F16)
    GGML_RXD_ASSERT(dst->src[0]->type == GGML_RXD_TYPE_F32 || dst->src[0]->type == GGML_RXD_TYPE_F16);
    GGML_RXD_ASSERT(dst->type == GGML_RXD_TYPE_F32 || dst->type == GGML_RXD_TYPE_F16);
#else
    ...
    GGML_RXD_ASSERT(ggml_rxd_is_contiguous_1(dst->src[0]));
    ...
    if (src1) {
        ...
        GGML_RXD_ASSERT(src1->nb[0] == ggml_rxd_element_size(src1));
        GGML_RXD_ASSERT(src1->ne[0] == nc);
...
template<typename KernelInvoker, typename... Args>
static inline void dispatch_ggml_sycl_op_upscale(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst, KernelInvoker kernel_invoker, Args&&... args) {
#if defined (GGML_RXD_SYCL_F16)
    GGML_RXD_ASSERT(dst->src[0]->type == GGML_RXD_TYPE_F32 || dst->src[0]->type == GGML_RXD_TYPE_F16);
    GGML_RXD_ASSERT(dst->type == GGML_RXD_TYPE_F32 || dst->type == GGML_RXD_TYPE_F16);
#else
...
template<typename F>
static inline void ggml_rxd_sycl_op_unary(
        ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst, F func) {
    ...
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_unary(ctx, dst,
...
static inline void ggml_rxd_sycl_op_arange(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    GGML_RXD_ASSERT(dst->type == GGML_RXD_TYPE_F32);
    ...
    const int k = (int)ggml_rxd_nelements(dst);
...
static inline void ggml_rxd_sycl_op_sgn(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_sgn(x);
    });
}


static inline void ggml_rxd_sycl_op_abs(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_abs(x);
    });
}

static inline void ggml_rxd_sycl_op_elu(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_elu(x);
    });
}
static inline void ggml_rxd_sycl_op_silu(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_silu(x);
    });
}

static inline void ggml_rxd_sycl_op_gelu(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_gelu(x);
    });
}

static inline void ggml_rxd_sycl_op_gelu_quick(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_gelu_quick(x);
    });
}

static inline void ggml_rxd_sycl_op_gelu_erf(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_gelu_erf(x);
    });
}

static inline void ggml_rxd_sycl_op_tanh(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_tanh(x);
    });
}

static inline void ggml_rxd_sycl_op_relu(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_relu(x);
    });
}

static inline void ggml_rxd_sycl_op_hardsigmoid(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_hardsigmoid(x);
    });
}

static inline void ggml_rxd_sycl_op_hardswish(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_hardswish(x);
    });
}

static inline void ggml_rxd_sycl_op_exp(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_exp(x);
    });
}

static inline void ggml_rxd_sycl_op_log(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_unary(ctx, dst,
        [](const auto* src, auto* dst_ptr, int k_elements, queue_ptr stream) {
...
static inline void ggml_rxd_sycl_op_neg(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_neg(x);
    });
}
...
static inline void ggml_rxd_sycl_op_step(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_step(x);
    });
}

static inline void ggml_rxd_sycl_op_sigmoid(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::ggml_rxd_sycl_op_unary(ctx, dst, [](auto x) {
        return op_sigmoid(x);
    });
}

static inline void ggml_rxd_sycl_op_sqrt(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_unary(ctx, dst,
        [](const auto* src, auto* dst_ptr, int k_elements, queue_ptr stream) {
...
static inline void ggml_rxd_sycl_op_sin(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_unary(ctx, dst,
        [](const auto* src, auto* dst_ptr, int k_elements, queue_ptr stream) {
...
static inline void ggml_rxd_sycl_op_cos(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_unary(ctx, dst,
        [](const auto* src, auto* dst_ptr, int k_elements, queue_ptr stream) {
...
static inline void ggml_rxd_sycl_op_leaky_relu(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
...
static inline void ggml_rxd_sycl_op_sqr(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_unary(ctx, dst,
        [](const auto* src, auto* dst_ptr, int k_elements, queue_ptr stream) {
...
static inline void ggml_rxd_sycl_op_upscale(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_upscale(ctx, dst,
        [](const auto* src, auto* dst_ptr, int nb00, int nb01, int nb02, int nb03,
           ...
           queue_ptr stream) {
            ggml_rxd_sycl_detail::upscale_sycl(src, dst_ptr, nb00, nb01, nb02, nb03, ne10, ne11, ne12, ne13, sf0, sf1, sf2, sf3, stream);
        });
}

static inline void ggml_rxd_sycl_op_clamp(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    float min_val;
    float max_val;
...
static inline void ggml_rxd_sycl_op_floor(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_unary(ctx, dst,
        [](const auto* src, auto* dst_ptr, int k_elements, queue_ptr stream) {
            const int num_blocks = ceil_div(k_elements, 256);
            stream->parallel_for(
...
static inline void ggml_rxd_sycl_op_ceil(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_unary(ctx, dst,
        [](const auto* src, auto* dst_ptr, int k_elements, queue_ptr stream) {
            const int num_blocks = ceil_div(k_elements, 256);
            stream->parallel_for(
...
static inline void ggml_rxd_sycl_op_round(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_unary(ctx, dst,
        [](const auto* src, auto* dst_ptr, int k_elements, queue_ptr stream) {
            const int num_blocks = ceil_div(k_elements, 256);
            stream->parallel_for(
...
static inline void ggml_rxd_sycl_op_trunc(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_unary(ctx, dst,
        [](const auto* src, auto* dst_ptr, int k_elements, queue_ptr stream) {
            const int num_blocks = ceil_div(k_elements, 256);
            stream->parallel_for(
...
static inline void ggml_rxd_sycl_op_acc(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor *dst) {
    GGML_RXD_ASSERT(dst->src[0]->type == GGML_RXD_TYPE_F32);
    GGML_RXD_ASSERT(dst->src[1]->type == GGML_RXD_TYPE_F32);
    GGML_RXD_ASSERT( dst->type == GGML_RXD_TYPE_F32);
    GGML_RXD_ASSERT(dst->ne[3] == 1); // just 3D tensors supported
    dpct::queue_ptr main_stream = ctx.stream();
    SYCL_CHECK(ggml_rxd_sycl_set_device(ctx.device));
    ...
    ggml_rxd_sycl_detail::acc_f32_sycl(src0_dd, src1_dd, dst_dd, (int)ggml_rxd_nelements(dst), (int)dst->src[1]->ne[0], (int)dst->src[1]->ne[1], (int)dst->src[1]->ne[2], nb1, nb2, offset, main_stream);
}

static inline void ggml_rxd_sycl_op_geglu(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_fused_glu(ctx, dst,
...
static inline void ggml_rxd_sycl_op_reglu(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_fused_glu(ctx, dst,
        [](const auto* x_ptr, const auto* g_ptr, auto* dst_ptr, uint64_t k, uint64_t n, uint64_t o0, uint64_t o1, queue_ptr main_stream) {
...
static inline void ggml_rxd_sycl_op_swiglu(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_fused_glu(ctx, dst,
        [](const auto* x_ptr, const auto* g_ptr, auto* dst_ptr, uint64_t k, uint64_t n, uint64_t o0, uint64_t o1, queue_ptr main_stream) {
...
static inline void ggml_rxd_sycl_op_geglu_erf(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_fused_glu(ctx, dst,
        [](const auto* x_ptr, const auto* g_ptr, auto* dst_ptr, uint64_t k, uint64_t n, uint64_t o0, uint64_t o1, queue_ptr main_stream) {
            const uint32_t num_blocks = ceil_div(k, SYCL_GELU_BLOCK_SIZE);
            main_stream->parallel_for(
...
static inline void ggml_rxd_sycl_op_geglu_quick(ggml_rxd_backend_sycl_context & ctx, ggml_rxd_tensor * dst) {
    ggml_rxd_sycl_detail::dispatch_ggml_sycl_op_fused_glu(ctx, dst,
        [](const auto* x_ptr, const auto* g_ptr, auto* dst_ptr, uint64_t k, uint64_t n, uint64_t o0, uint64_t o1, queue_ptr main_stream) {
            const uint32_t num_blocks = ceil_div(k, SYCL_GELU_BLOCK_SIZE);
            main_stream->parallel_for(
