...
class RawrXDHandler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_post_ask(self):
        ...
        try:
            ...
            # Append attached files to the question
            if files and isinstance(files, list):
                print(f"[/ask] Processing {len(files)} attached files")
                file_context = "\n\n=== ATTACHED FILES ===\n"
                for i, f in enumerate(files):
                    name = f.get('name', f'file_{i}')
                    content = f.get('content', '')
                    print(f"[/ask] File {i+1}: {name} ({len(content)} bytes)")
                    # Truncate if too huge to prevent massive prompt costs/errors
                    if len(content) > 100000: # Increased to 100KB
                        content = content[:100000] + "\n...[CONTENT TRUNCATED FOR CONTEXT WINDOW]..."
                    file_context += f"\n--- BEGIN ATTACHMENT: {name} ---\n{content}\n--- END ATTACHMENT: {name} ---\n"
                
                question = f"{question}\n\nContext from user files:\n{file_context}"
                print(f"[/ask] Appended {len(file_context)} bytes of file context to prompt")
            
            # Extract model name from path or use as-is
            if '/' in model or '\\' in model:
                # This is a file path from GGUF files
                basename = os.path.basename(model)
                # Try to map common GGUF names to Ollama models
                model_name_lower = basename.lower()
                
                # Map GGUF file names to Ollama model names
                if 'llama' in model_name_lower or 'llama3' in model_name_lower:
                    model = 'llama3.2:3b'
                    ...
                    # Try to extract model name before the first dash/underscore
                    model_base = basename.split('-')[0].split('_')[0].split('.')[0]
                    model = f"{model_base}:latest"
                
                print(f"[/ask] Mapped GGUF file '{basename}' -> Ollama model '{model}'")
            elif not model or model == 'default' or model == 'null':
                model = 'llama3.2:3b'
                print(f"[/ask] Using default model: {model}")
            else:
                print(f"[/ask] Using model: {model}")
            
            # Validate JSON encoding before sending to Ollama
            try:
                test_payload = {
                    'model': model,
                    'prompt': question,
                ...
                print(f"[/ask] JSON payload size: {json_size} bytes")
                
                if json_size > 20 * 1024 * 1024:  # 20 MB limit
                    print(f"[/ask] ⚠️  Payload too large ({json_size} bytes), truncating file context")
                    # Reduce file context
                    max_question_size = 10 * 1024 * 1024  # 10 MB
                    if len(question) > max_question_size:
                        question = question[:max_question_size] + "\n[...FILE CONTEXT TRUNCATED...]"
                        test_payload['prompt'] = question
                        json_str = json.dumps(test_payload)
                        json_size = len(json_str.encode('utf-8'))
                        print(f"[/ask] Reduced JSON payload size: {json_size} bytes")
            except Exception as e:
                print(f"[/ask] ⚠️  Error validating JSON: {e}")
                self.send_json_response({'answer': f'Error encoding request: {str(e)}', 'error': True}, 400)
                return
    ...
    def query_ollama(self, model, system_prompt, question):
        ...
        print(f"[Ollama] Querying model '{model}' with prompt: {question[:50]}...")
        ...
        try:
            ...
            print(f"[Ollama] Waiting for response (timeout: 300s)...")
            with urllib.request.urlopen(req, timeout=300) as response:
                response_data = response.read().decode('utf-8')
                print(f"[Ollama] Received {len(response_data)} bytes")
                result = json.loads(response_data)
                answer = result.get('response', '').strip()
                print(f"[Ollama] ✅ Success! Response length: {len(answer)} chars")
                return answer
                
        except urllib.error.HTTPError as e:
            error_body = e.read().decode('utf-8') if e.fp else 'No error body'
            print(f"[Ollama] ❌ HTTP Error {e.code}: {e.reason}")
            print(f"[Ollama] Error details: {error_body}")
            return None
            
        except urllib.error.URLError as e:
            print(f"[Ollama] ❌ Connection Error: {e.reason}")
            print(f"[Ollama] Is Ollama running on localhost:11434?")
            return None
            
        except json.JSONDecodeError as e:
            print(f"[Ollama] ❌ JSON Decode Error: {e}")
            print(f"[Ollama] Invalid JSON response from Ollama")
            return None
            
        except Exception as e:
            print(f"[Ollama] ❌ Unexpected Error: {type(e).__name__}: {str(e)}")
            print(f"[Ollama] Traceback:")
            traceback.print_exc()
            return None
