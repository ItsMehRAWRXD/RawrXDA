...
FindModelByPath PROC FRAME
    ...
    mov rdi, rcx            ; Input Path
    
    xor ebx, ebx
@search_loop:
    cmp ebx, MODEL_MAX_INSTANCES
    jge @not_found
    
    imul rax, rbx, SIZEOF ModelInstance
    lea rsi, g_ModelRegistry.Instances[rax]
    
    cmp [rsi].ModelInstance.ModelPath, 0
    je @next_search
    
    mov rcx, [rsi].ModelInstance.ModelPath
    mov rdx, rdi
    call RawrXD_StrCmp
    test eax, eax
    jz @found
...
FindFreeSlot PROC FRAME
    xor rdx, rdx
@slot_loop:
    cmp rdx, MODEL_MAX_INSTANCES
    jge @no_slot
    
    imul rax, rdx, SIZEOF ModelInstance
    lea rax, g_ModelRegistry.Instances[rax]
    
    ; Check if UNLOADED and RefCount 0
    cmp [rax].ModelInstance.State, MODEL_STATE_UNLOADED
    jne @next_slot
    cmp [rax].ModelInstance.RefCount, 0
    jne @next_slot
...
NotifyStateChange PROC
    ; Arguments: RCX=Old, RDX=New, R8=Instance
    ; Stub: Iterate callbacks if needed (omitted for speed)
    ret
