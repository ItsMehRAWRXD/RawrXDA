...
# 2d. Copy PowerShield modules (external IDE folder)
Write-Host " -> Copying PowerShield Modules..."
$PowerShieldSource = "D:\Power\Powershield"
$PowerShieldDest = "$DistDir\extras\powershield"
if (Test-Path $PowerShieldSource) {
    New-Item -ItemType Directory -Force -Path $PowerShieldDest | Out-Null
    Copy-Item "$PowerShieldSource\*" -Destination $PowerShieldDest -Recurse -Force
    Write-Host "    [+] PowerShield Included" -ForegroundColor Green
} else {
    Write-Warning "PowerShield folder not found at $PowerShieldSource"
}

# 2e. Copy Cursor Source + Codex Reverse Engine (external)
Write-Host " -> Copying Cursor Source + Codex Reverse Engine..."
$CursorSourceExternal = "D:\Cursor_Source_Complete"
$CursorSourceDest = "$DistDir\extras\cursor_source_complete"
if (Test-Path $CursorSourceExternal) {
    New-Item -ItemType Directory -Force -Path $CursorSourceDest | Out-Null
    Copy-Item "$CursorSourceExternal\*" -Destination $CursorSourceDest -Recurse -Force
    Write-Host "    [+] Cursor Source Complete Included" -ForegroundColor Green
} else {
    Write-Warning "Cursor source folder not found at $CursorSourceExternal"
}

$CodexReverseExternal = "D:\CodexReverseEngine"
$CodexReverseDest = "$DistDir\extras\codex_reverse_engine"
if (Test-Path $CodexReverseExternal) {
    New-Item -ItemType Directory -Force -Path $CodexReverseDest | Out-Null
    Copy-Item "$CodexReverseExternal\*" -Destination $CodexReverseDest -Recurse -Force
    Write-Host "    [+] Codex Reverse Engine Included" -ForegroundColor Green
} else {
    Write-Warning "Codex reverse folder not found at $CodexReverseExternal"
}
