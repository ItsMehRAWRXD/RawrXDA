...
IDEAgentBridgeWithHotPatching::~IDEAgentBridgeWithHotPatching() noexcept
{
    ...
    try {
        if (m_proxyServer && m_proxyServer->isServerRunning()) {
...
void IDEAgentBridgeWithHotPatching::initializeWithHotPatching()
{
    ...
    try {
        ...
        m_hotPatcher->initialize("./gguf_loader", 11435);
        ...
        // Set callbacks (previously signals)
        m_hotPatcher->setHallucinationDetectedCallback([this](const HallucinationDetection& h) {
            this->onHallucinationDetected(h);
        });

        // Load correction patterns from file
...
bool IDEAgentBridgeWithHotPatching::startHotPatchingProxy()
{
    ...
    try {
        // Validate proxy port
        int proxyPort = std::stoi(m_proxyPort);
        if (!isValidPort(proxyPort)) {
            throw std::runtime_error("Invalid proxy port: " + m_proxyPort);
        }

        // Validate GGUF endpoint
        if (!isValidEndpoint(m_ggufEndpoint)) {
            throw std::runtime_error("Invalid GGUF endpoint: " + m_ggufEndpoint);
        }
...
{
    return m_hotPatchingActive && m_hotPatcher && m_proxyServer 
...
{
    if (m_hotPatchingActive == enabled) return;   // no-op

    m_hotPatchingActive = enabled;
...
{
    if (this->getModelInvoker() && m_hotPatchingActive) {
...
void IDEAgentBridgeWithHotPatching::initializeDatabase(const std::string& databasePath)
{
    if (m_hotPatcher) {
        m_hotPatcher->loadDatabase(databasePath);
    }
}

void IDEAgentBridgeWithHotPatching::saveDatabase(const std::string& databasePath)
{
    if (m_hotPatcher) {
        m_hotPatcher->saveDatabase(databasePath);
    }
}
