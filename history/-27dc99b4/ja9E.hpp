...
public:
    ...
    // value with default - template version for proper type returns
    template<typename T>
    T value(const std::string& key, const T& default_val) const {
        if (m_type != object) return default_val;
        auto it = m_object.find(key);
        if (it == m_object.end()) return default_val;
        try {
    ...
    std::string value(const std::string& key, const char* default_val) const {
        return value<std::string>(key, std::string(default_val ? default_val : ""));
    }

    // value returning json (for compatibility)
    json value_json(const std::string& key, const json& default_val) const {
        if (m_type != object) return default_val;
        auto it = m_object.find(key);
    ...
    // get<size_t> - conditionally enabled only if not same as unsigned long long
    #if !defined(_WIN64) || defined(__GNUC__)
    template<>
    size_t get<size_t>() const {
        if (m_type == number) return static_cast<size_t>(m_number);
        throw std::runtime_error("Cannot get size_t from this JSON type");
    }
    #endif
    ...
    long long get<long long>() const {
        if (m_type == number) return static_cast<long long>(m_number);
    ...
    unsigned int get<unsigned int>() const {
        if (m_type == number) return static_cast<unsigned int>(m_number);
        throw std::runtime_error("Cannot get unsigned int from this JSON type");
    }

    // get<unsigned long>
    // On MSVC x64, unsigned long is 32-bit (different from unsigned long long which is 64-bit)
    // On GCC/Clang x64, unsigned long is 64-bit (same as unsigned long long)
    #if defined(_MSC_VER) || !defined(_WIN64)
    template<>
    unsigned long get<unsigned long>() const {
        if (m_type == number) return static_cast<unsigned long>(m_number);
    ...
    unsigned long long get<unsigned long long>() const {
        if (m_type == number) return static_cast<unsigned long long>(m_number);
        throw std::runtime_error("Cannot get unsigned long long from this JSON type");
    }

    #ifdef _WIN32
    // get<DWORD> - only if not already covered by another type
    #if !defined(_WIN64) && !std::is_same_v<DWORD, unsigned long>
    template<>
    DWORD get<DWORD>() const {
        if (m_type == number) return static_cast<DWORD>(m_number);
        throw std::runtime_error("Cannot get DWORD from this JSON type");
    }
    #endif
    #endif
