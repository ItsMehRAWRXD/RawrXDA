...
public:
    ...
    // Array iterators (for range-based for loops over arrays)
    std::vector<json>::iterator array_begin() {
        if (m_type != array) throw std::runtime_error("Cannot iterate non-array as array");
    ...
    // Generic iteration support - prefer array iteration if array, else object
    // Note: For arrays, returns vector iterators; for objects, returns map iterators
    // Use is_array()/is_object() to determine type before iterating
    
    // For array iteration in range-based for
    const std::vector<json>& items() const {
        if (m_type != array) throw std::runtime_error("items() only valid for arrays");
        return m_array;
    }
    ...
    auto begin() {
        if (m_type == array) return reinterpret_cast<std::map<std::string, json>::iterator&>(m_array.begin());
        if (m_type != object) throw std::runtime_error("Cannot iterate non-object/non-array");
        return m_object.begin();
    }

    auto end() {
        if (m_type == array) return reinterpret_cast<std::map<std::string, json>::iterator&>(m_array.end());
        if (m_type != object) throw std::runtime_error("Cannot iterate non-object/non-array");
        return m_object.end();
    }

    auto begin() const {
        if (m_type == array) return reinterpret_cast<const std::map<std::string, json>::const_iterator&>(m_array.begin());
        if (m_type != object) throw std::runtime_error("Cannot iterate non-object/non-array");
        return m_object.begin();
    }

    auto end() const {
        if (m_type == array) return reinterpret_cast<const std::map<std::string, json>::const_iterator&>(m_array.end());
        if (m_type != object) throw std::runtime_error("Cannot iterate non-object/non-array");
        return m_object.end();
    }
