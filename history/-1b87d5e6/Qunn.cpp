...
AgentOrchestra::AgentOrchestra()
    : m_voiceProcessor(nullptr), m_isVoiceActive(false)
{
...
void AgentOrchestra::runCLI()
{
    ...
    std::cout << "\n--- Agent Orchestra CLI ---\n";
    while (true) {
        std::cout << "\nActive Agent: " << m_agents[m_activeAgentId] << std::endl;
        ...
        if (input == "1") {
            ...
            for (const auto& [id, name] : m_agents) {
            ...
            std::cout << "Select Accent: 0=English, 1=Australian, 2=GBE, 3=American\nChoice: ";
            std::getline(std::cin, input);
            int accent = std::stoi(input);
            setVoiceAccent(static_cast<VoiceProcessor::Accent>(accent));
        } else if (input == "6") {
            clearHistory();
        } else if (input == "7") {
            std::cout << "Exiting Agent Orchestra CLI.\n";
            break;
        }
    }
}
...
{
    if (m_agents.count(agentId)) {
        m_activeAgentId = agentId;
        addMessageToChat("System", "Switched to agent: " + m_agents[agentId], true);
    }
}
...
void AgentOrchestra::startVoiceInput()
{
    ...
    if (m_voiceProcessor->startRecording()) {
        ...
        addMessageToChat("System", "Failed to start voice recording", true);
    }
}
...
void AgentOrchestra::stopVoiceInput()
{
    ...
    addMessageToChat("System", "Voice recording stopped.", true);
}
...
void AgentOrchestra::sendMessage(const std::string& message)
{
    ...
    std::string response = generateAgentResponse(m_activeAgentId, message);
    addMessageToChat(m_agents[m_activeAgentId], response, true);
    if (m_voiceProcessor) {
        m_voiceProcessor->speakText(response, m_voiceProcessor->currentAccent());
    }
}
...
{
    // Simple agent response logic (mirrors GUI logic)
    if (agentId == "coder") {
        if (input.find("function") != std::string::npos || input.find("code") != std::string::npos) {
            return "I can help you write that function. Here's a starting template:\nvoid yourFunction() {\n    // TODO: Implement your logic here\n}\nWould you like me to add specific functionality?";
        } else if (input.find("debug") != std::string::npos || input.find("error") != std::string::npos) {
            return "Let me help you debug that. Please share: 1. The error message you're seeing 2. The relevant code snippet 3. What you expected to happen. I'll analyze it and provide a solution.";
        } else {
            return "I'm your Code Assistant. I can help you with: Writing functions and classes, Debugging code issues, Code optimization, Best practices and patterns. What would you like assistance with?";
        }
    } else if (agentId == "designer") {
        if (input.find("UI") != std::string::npos || input.find("design") != std::string::npos) {
            return "For UI design, I recommend considering: 1. Color Scheme: Use consistent branding colors 2. Typography: Clear, readable fonts (14-16pt for body) 3. Spacing: Adequate padding and margins (8px minimum) 4. Accessibility: High contrast ratios (4.5:1 minimum). Would you like specific recommendations for your interface?";
        } else if (input.find("layout") != std::string::npos) {
            return "For layout design: Use vertical stacking for vertical arrangements, horizontal for horizontal, consider resizable panels, add flexible spacing. What type of layout are you creating?";
        } else {
            return "I'm your Design Expert. I can help with: UI/UX design principles, Color schemes and typography, Layout optimization, Accessibility guidelines. What design challenge can I help you solve?";
        }
    } else if (agentId == "analyst") {
        if (input.find("analyze") != std::string::npos || input.find("data") != std::string::npos) {
            return "I'll analyze that data. For thorough analysis, please provide: 1. Data Format: CSV, JSON, or database query? 2. Key Metrics: What are you measuring? 3. Time Range: Historical or real-time? 4. Expected Insights: What patterns are you looking for? I can generate visualizations and statistical summaries.";
        } else if (input.find("performance") != std::string::npos) {
            return "For performance analysis: Metrics to Track: Response time (p50, p95, p99), Memory usage, CPU utilization, Error rates and types. Would you like me to set up monitoring dashboards?";
        } else {
            return "I'm your Data Analyst. I can help with: Data analysis and visualization, Performance metrics tracking, Statistical insights, Trend identification. What data would you like to analyze?";
        }
    } else if (agentId == "orchestrator") {
        if (input.find("task") != std::string::npos || input.find("plan") != std::string::npos) {
            return "I'll orchestrate that task. Here's the breakdown: Phase 1: Requirements gathering, Phase 2: Design and architecture, Phase 3: Implementation, Phase 4: Testing and validation, Phase 5: Deployment. Which agents should I assign to each phase?";
        } else if (input.find("coordinate") != std::string::npos) {
            std::string team = "I'll coordinate the agents. Current team:\n";
            for (const auto& [id, name] : m_agents) {
                team += "- " + id + ": " + name + "\n";
            }
            team += "How should I distribute the workload?";
            return team;
        } else {
            return "I'm your Task Orchestrator. I can help with: Multi-agent coordination, Task planning and distribution, Workflow optimization, Resource allocation. Available agents: " + std::to_string(m_agents.size()) + ". What task should I orchestrate?";
        }
    } else {
        return "I'm ready to assist. Please select a specific agent for specialized help, or describe what you need.";
    }
}

void AgentOrchestra::addMessageToChat(const std::string& sender, const std::string& message, bool isAgent)
{
    std::cout << (isAgent ? "[Agent] " : "[User] ") << sender << ": " << message << std::endl;
    m_chatHistory.push_back({sender, message, isAgent});
}
