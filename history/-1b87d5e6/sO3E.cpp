...
void AgentOrchestra::setupUI()
{
    ...
    // Create volume slider for speech output
    m_volumeSliderQt = new QSlider(Qt::Horizontal, this);
    m_volumeSliderQt->setRange(0, 100);
    m_volumeSliderQt->setValue(100);
    m_volumeSliderQt->setMaximumWidth(120);
    controlBar->addWidget(m_volumeSliderQt);
    ...
    // Connect volume slider to voice processor
    connect(m_volumeSliderQt, &QSlider::valueChanged, this, [this](int value){
        if (m_voiceProcessor) {
            m_voiceProcessor->setVolume(value);
        }
    });
