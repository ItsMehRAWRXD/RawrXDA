...
void ParseExtension(const nlohmann::json& ext, VSCodeMarketplace::MarketplaceEntry& e) {
    ...
    if (!versions.empty() && versions[0].contains("version"))
        e.version = versions[0]["version"].get<std::string>();
    else
        e.version.clear();
...
bool Query(const std::string& searchTerm, int pageSize, int pageNumber,
    ...
    try {
        ...
        auto& exts = results[0].value("extensions", nlohmann::json::array());
