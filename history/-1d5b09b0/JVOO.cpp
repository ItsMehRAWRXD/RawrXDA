...
const char* keywords[] = {
    ...
    "MOV","ADD","SUB","RET","PUSH","POP","XOR","CALL","JMP","LEA","CMP","JE","JNE","SYSCALL","INT",
    nullptr
};
...
TokenType kwTypes[] = {
    ...
    TOK_MOV,TOK_ADD,TOK_SUB,TOK_RET,TOK_PUSH,TOK_POP,TOK_XOR,TOK_CALL,TOK_JMP,TOK_LEA,TOK_CMP,TOK_JE,TOK_JNE,TOK_SYSCALL,TOK_INT
};
...
class CodeGen {
    ...
    const char* tokenTypeToString(TokenType t) {
        switch(t) {
            ...
            case TOK_JNE: return "JNE"; case TOK_SYSCALL: return "SYSCALL";
            default: return "???";
        }
    }
...
public:
    ...
    bool assembleInst(TokenType op, vector<Token>& ops) {
        std::cerr << "[CODEGEN] Opcode: " << (int)op << " OpCount: " << ops.size() << "\n";
        for(size_t i=0; i<ops.size(); i++) {
            std::cerr << "  Op" << i << " Type: " << (int)ops[i].type << " Text: " << ops[i].text << " Val: " << ops[i].value << "\n";
        }
