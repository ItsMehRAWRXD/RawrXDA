...
public:
    ...
    void assembleInst(TokenType op, vector<Token>& ops) {
        switch(op) {
            case TOK_RET:
                emit(0xC3);
                break;
    ...
    void firstPass(AstNode& node) {
        if (node.type == NODE_LABEL) {
            ...
            assembleInst(node.opcode, node.operands);
        }
        for (auto& c : node.children) firstPass(c);
    }
            ...
            if (seg.type == NODE_SEGMENT) {
                for (auto& item : seg.children) {
                    if (item.type == NODE_PROC) {
                        ...
                        for (auto& inst : item.children) firstPass(inst);
                    } else {
                        firstPass(item);
                    }
                }
