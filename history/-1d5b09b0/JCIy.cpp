...
enum TokenType {
    ...
    TOK_XOR, TOK_CALL, TOK_JMP, TOK_LEA, TOK_CMP, TOK_JE, TOK_JNE, TOK_SYSCALL, TOK_INT
};
...
const char* keywords[] = {
    ...
    "MOV","ADD","SUB","RET","PUSH","POP","XOR","CALL","JMP","LEA","CMP","JE","JNE","SYSCALL",
    nullptr
};
...
TokenType kwTypes[] = {
    ...
    TOK_MOV,TOK_ADD,TOK_SUB,TOK_RET,TOK_PUSH,TOK_POP,TOK_XOR,TOK_CALL,TOK_JMP,TOK_LEA,TOK_CMP,TOK_JE,TOK_JNE,TOK_SYSCALL
};
...
class Parser {
    ...
    AstNode parseSegment() {
        ...
        while (cur.type != TOK_ENDS && cur.type != TOK_EOF) {
            ...
            } else if (cur.type >= TOK_MOV && cur.type <= TOK_JNE) {
    ...
    AstNode parseProc() {
        ...
        while (cur.type != TOK_ENDP && cur.type != TOK_EOF) {
            if (cur.type >= TOK_MOV && cur.type <= TOK_JNE) {
                AstNode inst; inst.type = NODE_INSTRUCTION; inst.opcode = cur.type;
                advance();
...
public:
    void reset() { code.clear(); labels.clear(); fixups.clear(); }
    ...
    bool assembleInst(TokenType op, vector<Token>& ops) {
        try {
        ...
        switch(op) {
            ...
            case TOK_PUSH:
                if (ops.size() == 1 && ops[0].type == TOK_NUMBER) {
                    ...
                    if (r) { if(r->rex) emitREX(false,false,false,true); emit(0x50 + r->modrm); }
                }
                return true;
            case TOK_POP:
                if (ops.size() == 1) {
    ...
    bool firstPass(AstNode& node) {
        if (node.type == NODE_LABEL) {
            labels[node.text] = currentOffset();
        } else if (node.type == NODE_INSTRUCTION) {
            if (!assembleInst(node.opcode, node.operands)) {
                std::cerr << "[FATAL] Unsupported instruction: " << node.text << "\n";
                hadError = true;
                return false;
            }
        }
        for (auto& c : node.children) {
            if (!firstPass(c)) return false;
        }
        return true;
    }
    ...
    bool generate(AstNode& root, vector<uint8_t>& output) {
        ...
        for (auto& seg : root.children) {
            if (seg.type == NODE_SEGMENT) {
                std::cerr << "[CODEGEN] Visiting node type: SEGMENT\n";
...
void writePE(const char* filename, vector<uint8_t>& code, size_t entryRVA) {
    ...
    fwrite("\x06\x00\x00\x00", 4, 1, f); // OS major
    fwrite("\x00\x00\x00\x00", 4, 1, f); // OS minor
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Image major
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Image minor
    fwrite("\x06\x00\x00\x00", 4, 1, f); // Subsystem major (Win10)
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Subsystem minor
    ...
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Checksum
    fwrite("\x03\x00", 2, 1, f); // Subsystem (CONSOLE)
    fwrite("\x00\x00", 2, 1, f); // DllCharacteristics
    fwrite("\x00\x00\x10\x00\x00\x00\x00\x00", 8, 1, f); // Stack reserve
    fwrite("\x00\x00\x10\x00\x00\x00\x00\x00", 8, 1, f); // Stack commit
    fwrite("\x00\x00\x10\x00\x00\x00\x00\x00", 8, 1, f); // Heap reserve
    fwrite("\x00\x00\x10\x00\x00\x00\x00\x00", 8, 1, f); // Heap commit
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Loader flags
    fwrite("\x10\x00\x00\x00", 4, 1, f); // Number of RVA and sizes (16 data directories)
    ...
    fwrite(&codesize, 4, 1, f); // Virtual size
    fwrite("\x00\x10\x00\x00", 4, 1, f); // Virtual address (0x1000)
    fwrite(&codesize, 4, 1, f); // Raw size (unpadded for now, should be padded to fileAlign)
    fwrite("\x00\x02\x00\x00", 4, 1, f); // Raw address (0x200)
    fwrite("\x00\x00\x00\x00\x00\x00\x00\x00", 8, 1, f); // Relocations/line nums
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Relocation count
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Line number count
    fwrite("\x60\x00\x00\x60", 4, 1, f); // Characteristics (CODE | EXECUTE | READ)
    
    // Pad headers to fileAlign
    while (ftell(f) < fileAlign) fputc(0, f);
    
    // Code section
    fwrite(code.data(), code.size(), 1, f);
    // Pad to fileAlign
    while ((ftell(f) % fileAlign) != 0) fputc(0, f);
...
int main(int argc, char** argv) {
    ...
    if (entry == 0) entry = gen.getEntryPoint("START");
    if (entry == 0 && !code.empty()) entry = 0; // Default to section start
    
    writePE(argv[2], code, entry);
