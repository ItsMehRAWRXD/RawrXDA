...
// Include Windows AFTER our definitions to avoid MasmTokenType conflict
#include <windows.h>
...
class Parser {
    ...
    AstNode parseSegment() {
        ...
        while (cur.type != TOK_ENDS && cur.type != TOK_EOF) {
            if (cur.type == TOK_CODE || cur.type == TOK_DATA) {
                break; // New segment started
            }
            
            if (cur.type == TOK_IDENT) {
                char id[64]; strcpy(id, cur.text);
                Token next = lex.peek();
                if (next.type == TOK_COLON) {
                    ...
                    continue;
                } else if (next.type == TOK_PROC) {
                    advance();
                    n.children.push_back(parseProc(id));
                    continue;
                }
            }
            ...
            if (cur.type >= TOK_MOV && cur.type <= TOK_INT) {
                ...
                while (cur.type != TOK_EOF && cur.type != TOK_ENDP && cur.type != TOK_ENDS && 
                       !(cur.type == TOK_IDENT && (lex.peek().type == TOK_COLON || lex.peek().type == TOK_PROC)) && 
                       cur.type != TOK_PROC && !(cur.type >= TOK_MOV && cur.type <= TOK_INT)) {
    ...
    AstNode parseProc(const char* name) {
        eat(TOK_PROC);
        AstNode n; n.type = NODE_PROC;
        strcpy(n.text, name);
        while (cur.type != TOK_ENDP && cur.type != TOK_EOF) {
            // Check for name ENDP
            if (cur.type == TOK_IDENT && lex.peek().type == TOK_ENDP) {
                advance(); break; 
            }
            ...
            if (cur.type >= TOK_MOV && cur.type <= TOK_INT) {
                ...
                while (cur.type != TOK_EOF && cur.type != TOK_ENDP && cur.type != TOK_PROC &&
                       !(cur.type == TOK_IDENT && lex.peek().type == TOK_ENDP) &&
                       !(cur.type >= TOK_MOV && cur.type <= TOK_INT)) {
                    if (cur.type == TOK_COMMA) { advance(); continue; }
                    inst.operands.push_back(cur);
                    advance();
                }
...
class CodeGen {
    ...
    const char* tokenTypeToString(MasmTokenType t) {
        switch(t) {
...
public:
    ...
    bool assembleInst(MasmTokenType op, vector<Token>& ops, int lineNum = 0) {
        ...
        switch(op) {
            case TOK_RET:
                ...
                return true;llback: INT3 (0xCC)\n";
            case TOK_SYSCALL:
                emit(0x0F); emit(0x05);  // SYSCALL opcode
                instructionsEmitted++;code
                std::cerr << "       -> Fallback: SYSCALL (0x0F 0x05)\n";
                return true;td::cerr << "       -> Fallback: SYSCALL (0x0F 0x05)\n";
            case TOK_PUSH:
                if (ops.size() == 1 && ops[0].type == TOK_NUMBER) {:
                    emit(0x68); emit32(ops[0].value); // PUSH imm32 ops[0].type == TOK_NUMBER) {
                    instructionsEmitted++;// PUSH imm32
                    std::cerr << "       -> Fallback: PUSH imm32\n";
                } else if (ops.size() == 1) {   std::cerr << "       -> Fallback: PUSH imm32\n";
                    RegInfo* r = findReg(ops[0].text);ps.size() == 1) {
                    if (r) { if(r->rex) emitREX(false,false,false,true); emit(0x50 + r->modrm); instructionsEmitted++; std::cerr << "       -> Fallback: PUSH reg\n"; }fo* r = findReg(ops[0].text);
                }) emitREX(false,false,false,true); emit(0x50 + r->modrm); instructionsEmitted++; std::cerr << "       -> Fallback: PUSH reg\n"; }
                return true;
            case TOK_POP:
                if (ops.size() == 1) {
                    RegInfo* r = findReg(ops[0].text);
                    if (r) { if(r->rex) emitREX(false,false,false,true); emit(0x58 + r->modrm); };
                },false,true); emit(0x58 + r->modrm); }
                return true;
            case TOK_XOR:eturn true;
                if (ops.size() == 2) {
                    RegInfo* d = findReg(ops[0].text);ize() == 2) {
                    RegInfo* s = findReg(ops[1].text);eg(ops[0].text);
                    if (d && s && d->r64) {
            ...
            case TOK_MOV:
                if (ops.size() == 2) {
                    ...
                    if (d && d->r64 && ops[1].type == TOK_NUMBER) {ext);
                        // MOV r64, imm64 (special encoding for A-D, else generic) && d->r64 && ops[1].type == TOK_NUMBER) {
                        if (d->modrm < 8 && !d->rex) { A-D, else generic)
                            emitREX(true,false,false,false);
                            emit(0xC7); emit(0xC0 | d->modrm); // MOV r/m64, imm32 (sign-extended)e);
                            emit32(ops[1].value);xC0 | d->modrm); // MOV r/m64, imm32 (sign-extended)
                        } else {
                            emitREX(true, false, false, d->rex);
                            emit(0xB8 + d->modrm);emitREX(true, false, false, d->rex);
                            emit64(ops[1].value);
                        });
                    } else if (d && ops[1].type == TOK_IDENT) {
                        // MOV reg, label (needs fixup)ENT) {
                        emitREX(true,false,false,d->rex);
                        emit(0xB8 + d->modrm);mitREX(true,false,false,d->rex);
                        fixups.push_back({code.size(), ops[1].text});   emit(0xB8 + d->modrm);
                        emit64(0); // placeholder       fixups.push_back({code.size(), ops[1].text});
                    } else {64(0); // placeholder
                        RegInfo* s = findReg(ops[1].text);e {
                        if (d && s && d->r64) {indReg(ops[1].text);
                            emitREX(true, s->rex, false, d->rex);
            ...
            case TOK_SUB:ue;
                if (ops.size() == 2) {
                    ...
                    if (d && d->r64 && ops[1].type == TOK_NUMBER) {
                        emitREX(true,false,false,d->rex);_NUMBER) {
            ...
            case TOK_ADD:e;
                if (ops.size() == 2) {
                    RegInfo* d = findReg(ops[0].text);
                    if (d && d->r64 && ops[1].type == TOK_NUMBER) {d = findReg(ops[0].text);
                        emitREX(true,false,false,d->rex);{
                        emit(0x81); emit(0xC0 | d->modrm); emit32(ops[1].value);
                    }modrm); emit32(ops[1].value);
                }
                return true;
            case TOK_CALL:eturn true;
            case TOK_JMP: {
                uint8_t opcode = (op == TOK_CALL) ? 0xE8 : 0xE9;
                emit(opcode);de = (op == TOK_CALL) ? 0xE8 : 0xE9;
                if (ops.size() == 1 && ops[0].type == TOK_IDENT) {   emit(opcode);
                    fixups.push_back({code.size(), ops[0].text});ize() == 1 && ops[0].type == TOK_IDENT) {
                    emit32(0); // rel32 placeholder
                } else if (ops.size() == 1 && ops[0].type == TOK_NUMBER) {
                    emit32(ops[0].value);ER) {
                }   emit32(ops[0].value);
                instructionsEmitted++;
            ...
            case TOK_INT:   }
                if (ops.size() == 1 && ops[0].type == TOK_NUMBER) {
                    if (ops[0].value == 3) emit(0xCC);ps.size() == 1 && ops[0].type == TOK_NUMBER) {
                    else { emit(0xCD); emit((uint8_t)ops[0].value); }               if (ops[0].value == 3) emit(0xCC);
                }                else { emit(0xCD); emit((uint8_t)ops[0].value); }
                return true;
            default:ue;
                std::cerr << "       -> [ERROR] Unhandled opcode in fallback: " << tokenTypeToString(op) << "\n";
                break;Unhandled opcode in fallback: " << tokenTypeToString(op) << "\n";
        }
        std::cerr << "       -> [FATAL] Instruction encoding failed completely for " << tokenTypeToString(op) << "\n";
        return false;iled completely for " << tokenTypeToString(op) << "\n";
    }
    
    bool firstPass(AstNode& node, int depth = 0) {
        nodesProcessed++;tPass(AstNode& node, int depth = 0) {
        if (node.type == NODE_LABEL) {odesProcessed++;
            labels[node.text] = currentOffset();
            std::cerr << "[CODEGEN] Label '" << node.text << "' at offset 0x" << std::hex << currentOffset() << std::dec << "\n";
        } else if (node.type == NODE_INSTRUCTION) {   std::cerr << "[CODEGEN] Label '" << node.text << "' at offset 0x" << std::hex << currentOffset() << std::dec << "\n";
            if (!assembleInst(node.opcode, node.operands, 0)) {ode.type == NODE_INSTRUCTION) {
                std::cerr << "[FATAL] Unsupported instruction opcode: " << tokenTypeToString(node.opcode) << "\n";       if (!assembleInst(node.opcode, node.operands, 0)) {
                hadError = true;            std::cerr << "[FATAL] Unsupported instruction opcode: " << tokenTypeToString(node.opcode) << "\n";
                return false; true;
            }
        }
    ...
    void resolveFixups() {
        for (auto& f : fixups) {
            ...
            if (labels.count(name)) {ring& name = f.second;
                size_t target = labels[name];
                size_t nextInstr = patchOffset + 4;   size_t target = labels[name];
                int32_t rel = (int32_t)(target - nextInstr);       size_t nextInstr = patchOffset + 4;
                // Patch in place           int32_t rel = (int32_t)(target - nextInstr);
                code[patchOffset] = rel & 0xFF;            // Patch in place
                code[patchOffset+1] = (rel>>8) & 0xFF;
                code[patchOffset+2] = (rel>>16) & 0xFF;code[patchOffset+1] = (rel>>8) & 0xFF;
                code[patchOffset+3] = (rel>>24) & 0xFF;hOffset+2] = (rel>>16) & 0xFF;
            } else {
    ...
    bool generate(AstNode& root, vector<uint8_t>& output) {
        ...
        .size() << " top-level segments\n";
        // Pass 1: gen code, record labels
        for (auto& seg : root.children) {de, record labels
            if (seg.type == NODE_SEGMENT) {t.children) {
                std::cerr << "[CODEGEN] Processing SEGMENT node: '" << seg.text << "'\n";
                for (auto& item : seg.children) {cerr << "[CODEGEN] Processing SEGMENT node: '" << seg.text << "'\n";
                    if (item.type == NODE_PROC) {or (auto& item : seg.children) {
                        labels[item.text] = currentOffset();       if (item.type == NODE_PROC) {
                        std::cerr << "[CODEGEN] Processing PROC '" << item.text << "' at offset 0x" << std::hex << currentOffset() << std::dec << "\n";               labels[item.text] = currentOffset();
                        for (auto& inst : item.children) { << "[CODEGEN] Processing PROC '" << item.text << "' at offset 0x" << std::hex << currentOffset() << std::dec << "\n";
                            if (!firstPass(inst)) return false;
                        }    if (!firstPass(inst)) return false;
                    } else {                }
                        if (!firstPass(item)) return false;else {
                    }
                }
            }
        }
        ...
        resolveFixups();fixups.size() << " fixups\n";
        ...
        return !hadError;        std::cerr << "[CODEGEN] Primary encoder: " << (instructionsEmitted - fallbackEncodings) << ", Fallback: " << fallbackEncodings << "\n\n";
    }
    
    size_t getEntryPoint(const char* procName="main") {
        string up = procName; _strupr(&up[0]);
        if (labels.count(up)) return labels[up];r(&up[0]);
        return 0;
    }    return 0;
};
...
void writePE(const char* filename, vector<uint8_t>& code, size_t entryRVA) {==============
    ...
    if (!f) { printf("Cannot write output\n"); return; }FILE* f = fopen(filename, "wb");
    ntf("Cannot write output\n"); return; }
    // Alignments
    ...
    int imageSize = ((headerSize + code.size() + sectAlign - 1) & ~(sectAlign-1)) + sectAlign; + fileAlign - 1) & ~(fileAlign-1);
    size() + sectAlign - 1) & ~(sectAlign-1)) + sectAlign;
    // DOS Header
    fwrite("\x4D\x5A\x90\x00\x03\x00\x00\x00\x04\x00\x00\x00\xFF\xFF\x00\x00\xB8\x00\x00\x00", 20, 1, f);// DOS Header
    fwrite("\x00\x00\x00\x00\x40\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 20, 1, f);A\x90\x00\x03\x00\x00\x00\x04\x00\x00\x00\xFF\xFF\x00\x00\xB8\x00\x00\x00", 20, 1, f);
    fwrite("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xF8\x00\x00\x00\x0E\x1F\xBA\x0E", 20, 1, f);\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 20, 1, f);
    fwrite("\x00\xB4\x09\xCD\x21\xB8\x01\x4C\xCD\x21\x54\x68\x69\x73\x20\x70\x72\x6F\x67\x72", 20, 1, f);00\x00\x00\x00\x00\x00\x00\x00\xF8\x00\x00\x00\x0E\x1F\xBA\x0E", 20, 1, f);
    fwrite("\x61\x6D\x20\x63\x61\x6E\x6E\x6F\x74\x20\x62\x65\x20\x72\x75\x6E\x20\x69\x6E\x20", 20, 1, f);fwrite("\x00\xB4\x09\xCD\x21\xB8\x01\x4C\xCD\x21\x54\x68\x69\x73\x20\x70\x72\x6F\x67\x72", 20, 1, f);
    fwrite("\x44\x4F\x53\x20\x6D\x6F\x64\x65\x2E\x0D\x0D\x0A\x24\x00\x00\x00\x00\x00\x00\x00", 20, 1, f);E\x6F\x74\x20\x62\x65\x20\x72\x75\x6E\x20\x69\x6E\x20", 20, 1, f);
    // Pad to PE offset 0x3C+45\x2E\x0D\x0D\x0A\x24\x00\x00\x00\x00\x00\x00\x00", 20, 1, f);
    ...
    fwrite("\xF0\x00", 2, 1, f); // Optional header size (64-bit optional header is 240 bytes)bols
    fwrite("\x22\x00", 2, 1, f); // Characteristics (EXECUTABLE_IMAGE | LARGE_ADDRESS_AWARE)ader size (64-bit optional header is 240 bytes)
    MAGE | LARGE_ADDRESS_AWARE)
    ...
    uint32_t entryVirt = headerSize + entryRVA; data
    fwrite(&entryVirt, 4, 1, f); // Entry point
    fwrite("\x00\x10\x00\x00", 4, 1, f); // Base of code (0x1000)
    fwrite("\x00\x00\x00\x00\x00\x10\x00\x00", 8, 1, f); // Image base (0x100000000 for ASLR, but 1MB for now)de (0x1000)
    fwrite("\x00\x10\x00\x00", 4, 1, f); // Section alignment(0x100000000 for ASLR, but 1MB for now)
    fwrite("\x00\x02\x00\x00", 4, 1, f); // File alignmentignment
    ...
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Checksumvefwrite("\x00\x02\x00\x00", 4, 1, f); // Size of headers
    fwrite("\x03\x00", 2, 1, f); // Subsystem (CONSOLE) // Stack commit f); // Checksumve
    fwrite("\x00\x00", 2, 1, f); // DllCharacteristics
    fwrite("\x00\x00\x10\x00\x00\x00\x00\x00", 8, 1, f); // Stack reservefwrite("\x00\x00\x10\x00\x00\x00\x00\x00", 8, 1, f); // Heap commitfwrite("\x00\x00", 2, 1, f); // DllCharacteristics
    fwrite("\x00\x00\x10\x00\x00\x00\x00\x00", 8, 1, f); // Stack commit f); // Loader flagsx00\x00\x00\x00", 8, 1, f); // Stack reservefwrite("\x00\x00\x10\x00\x00\x00\x00\x00", 8, 1, f); // Heap commit
    fwrite("\x00\x00\x10\x00\x00\x00\x00\x00", 8, 1, f); // Heap reserves)0\x00\x00", 8, 1, f); // Stack commit f); // Loader flags
    fwrite("\x00\x00\x10\x00\x00\x00\x00\x00", 8, 1, f); // Heap commitrves)
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Loader flagsy/null) 1, f); // Heap commit
    ...
    x00\x00\x00\x00", 8, 1, f);
    // Section Header (.text)
    fwrite(".text\0\0\0", 8, 1, f);
    uint32_t codesize = code.size();w, should be padded to fileAlign)fwrite(".text\0\0\0", 8, 1, f);
    fwrite(&codesize, 4, 1, f); // Virtual size0)ze();w, should be padded to fileAlign)
    fwrite("\x00\x10\x00\x00", 4, 1, f); // Virtual address (0x1000)
    fwrite(&codesize, 4, 1, f); // Raw size (unpadded for now, should be padded to fileAlign)fwrite("\x00\x00\x00\x00", 4, 1, f); // Relocation countx00\x10\x00\x00", 4, 1, f); // Virtual address (0x1000)
    fwrite("\x00\x02\x00\x00", 4, 1, f); // Raw address (0x200)4, 1, f); // Line number counte, 4, 1, f); // Raw size (unpadded for now, should be padded to fileAlign)fwrite("\x00\x00\x00\x00", 4, 1, f); // Relocation count
    fwrite("\x00\x00\x00\x00\x00\x00\x00\x00", 8, 1, f); // Relocations/line numsharacteristics (CODE | EXECUTE | READ)w address (0x200)4, 1, f); // Line number count
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Relocation countx00\x00", 8, 1, f); // Relocations/line numsharacteristics (CODE | EXECUTE | READ)
    fwrite("\x00\x00\x00\x00", 4, 1, f); // Line number countto fileAlign
    fwrite("\x60\x00\x00\x60", 4, 1, f); // Characteristics (CODE | EXECUTE | READ));fwrite("\x00\x00\x00\x00", 4, 1, f); // Line number countto fileAlign
    60\x00\x00\x60", 4, 1, f); // Characteristics (CODE | EXECUTE | READ));
    // Pad headers to fileAlign
    while (ftell(f) < fileAlign) fputc(0, f);fwrite(code.data(), code.size(), 1, f);   // Pad headers to fileAlign
    fileAlign    while (ftell(f) < fileAlign) fputc(0, f);fwrite(code.data(), code.size(), 1, f);
    // Code section
    fwrite(code.data(), code.size(), 1, f);   
    // Pad to fileAlign    fclose(f);
    while ((ftell(f) % fileAlign) != 0) fputc(0, f);size(), entryRVA);(f);
    ) % fileAlign) != 0) fputc(0, f);size(), entryRVA);
    fclose(f);
    printf("Generated PE64: %s (%zu bytes code, entry=$%zX)\n", filename, code.size(), entryRVA);==============================================
} code, entry=$%zX)\n", filename, code.size(), entryRVA);==============================================

// ============================================================================, char** argv) {
// MAINf (argc < 3) {=======, char** argv) {
// ============================================================================    printf("RawrXD MASM Solo Compiler\nUsage: %s <input.asm> <output.exe>\n", argv[0]);
int main(int argc, char** argv) {=================================    printf("RawrXD MASM Solo Compiler\nUsage: %s <input.asm> <output.exe>\n", argv[0]);
    if (argc < 3) {argv) {
        printf("RawrXD MASM Solo Compiler\nUsage: %s <input.asm> <output.exe>\n", argv[0]);
        return 1;1], "rb");SM Solo Compiler\nUsage: %s <input.asm> <output.exe>\n", argv[0]);
    }ot open %s\n", argv[1]); return 1; }
    s\n", argv[1]); return 1; }
    FILE* f = fopen(argv[1], "rb");
    if (!f) { printf("Cannot open %s\n", argv[1]); return 1; }SEEK_SET);
    fseek(f, 0, SEEK_END);= new char[sz+1];n %s\n", argv[1]); return 1; }SEEK_SET);
    size_t sz = ftell(f);fread(src, 1, sz, f);r[sz+1];
    fseek(f, 0, SEEK_SET);
    char* src = new char[sz+1];fclose(f);
    fread(src, 1, sz, f);sz+1];fclose(f);
    src[sz] = 0;olo Compiler (RawrXD Edition)\nSource: %s (%zu bytes)\n", argv[1], sz);
    fclose(f);iler (RawrXD Edition)\nSource: %s (%zu bytes)\n", argv[1], sz);
    sis... ");
    printf("MASM Solo Compiler (RawrXD Edition)\nSource: %s (%zu bytes)\n", argv[1], sz);
    printf("Done\nParsing... ");piler (RawrXD Edition)\nSource: %s (%zu bytes)\n", argv[1], sz);
    printf("Lexical Analysis... ");
    Parser p(src); (%zu segments)\n", ast.children.size());
    printf("Done\nParsing... ");dren.size());
    auto ast = p.parse();
    printf("Done (%zu segments)\n", ast.children.size());
    ector<uint8_t> code;ren.size());
    printf("Code Generation... ");
    CodeGen gen;    printf("FAILED\n"); return 1;
    vector<uint8_t> code;rintf("FAILED\n"); return 1;
    if (!gen.generate(ast, code)) {    vector<uint8_t> code;
        printf("FAILED\n"); return 1;    if (!gen.generate(ast, code)) {
    }size_t entry = gen.getEntryPoint("MAIN");        printf("FAILED\n"); return 1;
    printf("Done (%zu bytes)\n", code.size());tEntryPoint("START");    }size_t entry = gen.getEntryPoint("MAIN");
    if (entry == 0 && !code.empty()) entry = 0; // Default to section start    printf("Done (%zu bytes)\n", code.size());tEntryPoint("START");
    size_t entry = gen.getEntryPoint("MAIN");    if (entry == 0 && !code.empty()) entry = 0; // Default to section start
    if (entry == 0) entry = gen.getEntryPoint("START");rgv[2], code, entry);
    if (entry == 0 && !code.empty()) entry = 0; // Default to section start      if (entry == 0) entry = gen.getEntryPoint("START");rgv[2], code, entry);
        delete[] src;    if (entry == 0 && !code.empty()) entry = 0; // Default to section start   
...
}}    return 0;    delete[] src;        writePE(argv[2], code, entry);    return 0;        delete[] src;
