            ...
            case TOK_MOV:
                if (ops.size() == 2) {
                    ...
                    if (d && d->r64 && ops[1].type == TOK_NUMBER) {
                        // Use C7 for 32-bit sign-extended immediates
                        if (ops[1].value >= -2147483648LL && ops[1].value <= 2147483647LL) {
                            emitREX(true, false, false, d->rex);
                            emit(0xC7); encodeModRM(3, 0, d->modrm);
                            emit32((uint32_t)ops[1].value);
                        } else {
                            // Use B8 (MOVABS) for full 64-bit immediates
                            emitREX(true, false, false, d->rex);
                            emit(0xB8 + (d->modrm & 7));
                            emit64(ops[1].value);
                        }
