...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (!tensors.empty()) {
            ...
            try {
                loader.LoadTensorZone(first_tensor_name, data);
                std::cout << "  ✓ Successfully loaded tensor data: " << data.size() << " bytes" << std::endl;
            } catch (const std::exception& e) {
                std::cout << "  Note: Could not load tensor data (expected for some tensor types): " << e.what() << std::endl;
            }
        }
