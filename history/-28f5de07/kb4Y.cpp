...
bool GGUFLoader::LoadTensorRange(size_t start_idx, size_t count, std::vector<uint8_t>& data) {
    if (start_idx + count > tensors_.size()) {
        std::cerr << "Tensor range out of bounds" << std::endl;
        return false;
    }
    ...
    for (size_t i = start_idx; i < start_idx + count; ++i) {
        ...
        if (!file_.good()) {
            std::cerr << "Failed to read tensor range" << std::endl;
            return false;
        }
...
// ---- Compatibility methods for streaming API (no-op / fallback) ----
bool GGUFLoader::BuildTensorIndex() {
    // Non-streaming loader: tensor info is already parsed and present
    return true;
}

bool GGUFLoader::LoadZone(const std::string& zone_name, uint64_t max_memory_mb) {
    // Non-streaming loader doesn't cache zones - for compatibility, attempt to read embedding tensors
    if (!file_.is_open()) return false;
    if (zone_name.find("embedding") != std::string::npos) {
        // Try to find token embedding tensors and read them (temporary read only)
        for (const auto& t : tensors_) {
            if (t.name.find("embed") != std::string::npos || t.name.find("embedding") != std::string::npos) {
                std::vector<uint8_t> tmp;
                file_.seekg(t.offset);
                tmp.resize(t.size_bytes);
                file_.read(reinterpret_cast<char*>(tmp.data()), t.size_bytes);
            }
        }
        return true;
    }
    // For other zones, return true but do nothing
    return true;
}

bool GGUFLoader::UnloadZone(const std::string& zone_name) {
    // No-op for standard loader
    return true;
}
...
uint64_t GGUFLoader::GetCurrentMemoryUsage() const {
    // Non-streaming loader doesn't maintain a streaming cache; return 0
    return 0;
}
