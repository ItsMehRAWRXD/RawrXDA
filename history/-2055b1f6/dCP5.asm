...
AGENT_METRICS STRUCT
    ...
    lock                DB 40 DUP(?)    ; CRITICAL_SECTION (approx 40 bytes)
AGENT_METRICS ENDS
...
g_default_params    GENERATION_PARAMS {DEFAULT_TEMPERATURE, DEFAULT_TOP_P, DEFAULT_TOP_K, DEFAULT_REPEAT_PENALTY, MAX_TOKENS, 2, 0}
...
Phase3Initialize PROC FRAME
    ...
    lea rcx, [rdi].AGENT_CONTEXT.metrics.lock
    call InitializeCriticalSection
...
gen_have_params:
    ...
    ; Update metrics
    lea rcx, [rbx].AGENT_CONTEXT.metrics.lock
...
RegisterTool PROC FRAME
    ...
    push rcx
    lea rcx, [rcx].TOOL_DEFINITION.name
    mov rdx, rsi
    call lstrcpyA
    pop rcx
