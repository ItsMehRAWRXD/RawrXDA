...
void Win32IDE::initLSPServer() {
    ...
    config.rootPath       = m_workspaceRoot;
    config.rootUri        = m_lspServer ? "file:///" + m_workspaceRoot : "";
    ...
    LOG_INFO("LSP Server initialized (in-process mode, root: %s)", m_workspaceRoot.c_str());
}
...
void Win32IDE::cmdLSPServerExportSymbols() {
    ...
    // Write to file
    std::string exportPath = m_workspaceRoot + "\\rawrxd_symbols.json";
    {
