...
void AICodeAssistantPanel::onCommandOutputReceived(const QString &output)
{
    ...
    const qint64 sz = output.toUtf8().size();
    if (sz >= kMaxDisplayBytes) {
        m_suggestionsDisplay->append(QString("=== Command Output ===\n[Output suppressed: %1 bytes exceeds 2GB display limit]").arg(sz));
        return;
    }
    m_suggestionsDisplay->append(QString("=== Command Output ===\n%1").arg(output));
}
...
void AICodeAssistantPanel::displaySuggestion(const QString &title, const QString &suggestion)
{
    ...
    QString safe = sanitize(suggestion);
    const qint64 sz = safe.toUtf8().size();
    if (sz >= kMaxDisplayBytes) {
        m_suggestionsDisplay->setText(QString("=== %1 ===\n[Content suppressed: %2 bytes exceeds 2GB display limit]")
                                      .arg(title).arg(sz));
        m_statusLabel->setText("Suggestion suppressed (>2GB)");
        return;
    }
...
void AICodeAssistantPanel::displayResults(const QStringList &results)
{
    ...
    for (const QString &result : results) {
        total += result.toUtf8().size();
        if (total >= kMaxDisplayBytes) {
            m_resultsWidget->addItem("[Further results suppressed due to 2GB display limit]");
            break;
        }
...
void AICodeAssistantPanel::displayGrepResults(const QJsonArray &results)
{
    ...
    for (const QJsonValue &val : results) {
        ...
        const QString lineOut = QString("[%1:%2] %3").arg(file).arg(line).arg(content);
        total += lineOut.toUtf8().size();
        if (total >= kMaxDisplayBytes) {
            m_resultsWidget->addItem("[Further grep results suppressed due to 2GB display limit]");
            break;
        }
        m_resultsWidget->addItem(lineOut);
        count++;
    }
