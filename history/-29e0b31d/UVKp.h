...
public:
    ...
    // CommandRegistry Integration (Production-Ready Command System)
    void initializeCommandRegistry();
    void registerIDECallbacks();
    bool handleKeyboardShortcutViaRegistry(UINT vkCode, bool ctrl, bool shift, bool alt);
    bool routeCommandViaRegistry(UINT commandId);
    
    // SettingsRegistry Integration (Windows Registry Persistence)
    ...
    // Network Panel (Port Forwarding UI)
    ...
    // Layout schematic integration
    void initializeLayoutLive();
    void registerLayoutTargets();
    void exportLayoutSnapshot(const std::string& path);
    void applyLayoutOverrides(const std::string& path);
    void checkLayoutLiveUpdates();
    void launchSchematicStudio();
...
private:
    ...
    enum class SidebarView {
        ...
        RunDebug = 4,
        Extensions = 5,
        License = 6       // Enterprise Feature Registry
    };
...
private:
    ...
    HFONT m_editorFont;
    HFONT m_iconFont; // For circular activity bar icons
    HFONT m_uiFont;   // For general UI text
    ...
    // Layout schematic live updates
    std::string m_layoutLivePath;
    FILETIME m_layoutLastWrite;
    std::unordered_map<std::string, HWND> m_layoutTargets;
    bool m_layoutWatchEnabled;
    UINT_PTR m_layoutWatchTimer;
    struct OutlineItem {
        std::string name;
...
private:
    ...
    // Enterprise License & Feature Registry
    std::unique_ptr<FeatureRegistryPanel> m_featureRegistryPanel;
    void initEnterpriseLicense();
    void showFeatureRegistryPanel();
    void hideFeatureRegistryPanel();
};
