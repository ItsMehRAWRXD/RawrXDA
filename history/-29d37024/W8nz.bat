...
if %errorlevel% equ 0 (
    ...
    where x86_64-w64-mingw32-ld >nul 2>&1
    if %errorlevel% equ 0 (
        echo [INFO] Using MinGW linker
        x86_64-w64-mingw32-ld -e WinMain -subsystem windows -o bin\nasm_ide_dx.exe ^
            build\dx_ide_main.obj build\file_system.obj build\chat_pane.obj ^
        ...
        echo [ERROR] No linker found (tried link.exe and x86_64-w64-mingw32-ld)
