...
bool OAuth2Manager::registerUser(const QString& username, const QString& email, 
    ...
    user.passwordSalt = generateSalt(); // Generate and assign password salt
...
QString OAuth2Manager::generateJWT(const QString& userId, int expiresIn) {
    ...
    // Store token
    Impl::Token token;
    token.accessToken = QUuid::createUuid().toString();
    token.userId = userId;
...
QString OAuth2Manager::hashPassword(const QString& password) {
