...
{
    m_workflowDock = new QDockWidget("Agentic Workflow", this);
    QWidget* container = new QWidget(m_workflowDock);
    QVBoxLayout* layout = new QVBoxLayout(container);
    
    layout->addWidget(new QLabel("Specification:"));
    m_workflowSpec = new QPlainTextEdit(container);
    m_workflowSpec->setPlaceholderText("Describe the code to generate...");
    layout->addWidget(m_workflowSpec);
    
    QHBoxLayout* pathLayout = new QHBoxLayout();
    m_workflowOutputPath = new QLineEdit(container);
    m_workflowOutputPath->setPlaceholderText("Output file path...");
    m_workflowBrowseButton = new QPushButton("Browse...", container);
    pathLayout->addWidget(m_workflowOutputPath);
    pathLayout->addWidget(m_workflowBrowseButton);
    layout->addLayout(pathLayout);
    
    QHBoxLayout* compLayout = new QHBoxLayout();
    compLayout->addWidget(new QLabel("Compiler:"));
    m_workflowCompiler = new QComboBox(container);
    refreshCompilerCombo();
    compLayout->addWidget(m_workflowCompiler);
    layout->addLayout(compLayout);
    
    m_workflowStartButton = new QPushButton("Run Integrated Workflow", container);
    m_workflowStartButton->setStyleSheet("background-color: #007acc; color: white; font-weight: bold; padding: 8px;");
    layout->addWidget(m_workflowStartButton);
    
    m_workflowProgress = new QProgressBar(container);
    m_workflowProgress->setRange(0, 100);
    m_workflowProgress->setValue(0);
    layout->addWidget(m_workflowProgress);
    
    m_workflowStatus = new QLabel("Ready", container);
    layout->addWidget(m_workflowStatus);
    
    m_workflowLog = new QTextEdit(container);
    m_workflowLog->setReadOnly(true);
    m_workflowLog->setPlaceholderText("Workflow execution logs...");
    m_workflowLog->setFont(QFont("Consolas", 9));
    layout->addWidget(m_workflowLog);
...
void MainWindow_v5::refreshCompilerCombo()
{
    if (!m_workflowCompiler) return;
    m_workflowCompiler->clear();
    m_workflowCompiler->addItems({"auto", "clang", "msvc", "masm", "gcc"});
}

void MainWindow_v5::onWorkflowBrowseOutput()
{
    QString path = QFileDialog::getSaveFileName(this, "Select Output File", QDir::currentPath(), 
                                               "C++ Files (*.cpp);;Assembly Files (*.asm);;All Files (*.*)");
    if (!path.isEmpty()) {
        m_workflowOutputPath->setText(path);
    }
}

void MainWindow_v5::onRunWorkflow()
{
    QString spec = m_workflowSpec->toPlainText();
    QString path = m_workflowOutputPath->text();
    QString compiler = m_workflowCompiler->currentText();
    
    if (spec.isEmpty() || path.isEmpty()) {
        QMessageBox::warning(this, "Input Required", "Please provide both specification and output path.");
        return;
    }
    ...
    if (m_agenticExecutor) {
        m_agenticExecutor->executeFullWorkflow(spec, path, compiler);
    }
}
...
{
    if (m_workflowStatus) {
        m_workflowStatus->setText(QString("Phase: %1 - %2").arg(phase, detail));
    }
    if (m_workflowProgress && total > 0) {
        m_workflowProgress->setValue((step * 100) / total);
    }
    appendWorkflowLog(QString("[%1] %2").arg(phase, detail));
}

void MainWindow_v5::onWorkflowComplete(const QJsonObject& result)
{
    if (m_workflowStartButton) m_workflowStartButton->setEnabled(true);
    if (m_workflowProgress) m_workflowProgress->setValue(100);
    if (m_workflowStatus) m_workflowStatus->setText("Workflow Complete");
    
    if (result["success"].toBool()) {
        QMessageBox::information(this, "Success", "Autonomous workflow completed successfully!");
        
        // Load the generated file
        QString path = result["output_path"].toString();
        QFile file(path);
        if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            m_codeEditor->setPlainText(file.readAll());
            file.close();
            m_tabWidget->setTabText(0, QFileInfo(path).fileName());
        }
    } else {
        QMessageBox::critical(this, "Error", "Workflow failed: " + result["error"].toString());
    }
}
...
{
    if (m_workflowLog) {
        m_workflowLog->append(QString("[%1] %2")
            .arg(QDateTime::currentDateTime().toString("HH:mm:ss"), message));
        m_workflowLog->verticalScrollBar()->setValue(m_workflowLog->verticalScrollBar()->maximum());
    }
    
    if (m_terminal) {
        m_terminal->appendPlainText(message);
    }
}