...
void MainWindow_v5::setupDockWidgets()
{
    ...
    connect(m_modelLoader, &ModelLoaderWidget::modelLoaded, this, [this](const QString& path){
        appendWorkflowLog("Model loaded: " + path);
        statusBar()->showMessage("Model loaded: " + path, 3000);
    });
...
void MainWindow_v5::toggleAIChatPanel()
{
    ...
    if (!m_aiChatPanel->isVisible()) {
        ...
        if (m_agenticExecutor) {
            ...
            // Wire up inference engine if available from main panel
            // Note: In a real multi-threaded scenario, we'd want shared pointers to the engine
        }
