...
#define IDC_CHK_MAX_MODE 1206
#define IDC_CHK_DEEP_THINK 1207
#define IDC_CHK_DEEP_RESEARCH 1208
#define IDC_CHK_NO_REFUSAL 1209
#define IDC_SLIDER_CONTEXT 1210
#define IDC_LBL_CONTEXT 1211
...
// Forward declaration of specific subclass proc
LRESULT CALLBACK SecondarySidebarProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    Win32IDE* pThis = (Win32IDE*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
    switch (uMsg) {
        case WM_COMMAND:
        case WM_HSCROLL: // Forward slider events too!
            // Forward to Main Window
            if (pThis) {
                return SendMessage(pThis->getMainWindow(), uMsg, wParam, lParam);
            }
            break;
        case WM_PAINT: 
            // Optional: Custom background drawing if needed
            break;
    }
    return DefWindowProc(hwnd, uMsg, wParam, lParam);
}
...
void Win32IDE::createSecondarySidebar(HWND hwndParent)
{
    ...
    // Subclass it to handle child messages
    SetWindowLongPtr(m_hwndSecondarySidebar, GWLP_USERDATA, (LONG_PTR)this);
    SetWindowLongPtr(m_hwndSecondarySidebar, GWLP_WNDPROC, (LONG_PTR)SecondarySidebarProc);

    // Header label
    m_hwndSecondarySidebarHeader = CreateWindowExA(
        0, "STATIC", " Inference Control Center",
        WS_CHILD | WS_VISIBLE | SS_LEFT | SS_CENTERIMAGE,
        0, 0, m_secondarySidebarWidth, 24,
        m_hwndSecondarySidebar, (HMENU)IDC_SECONDARY_SIDEBAR_HEADER, m_hInstance, nullptr);
    
    // Advanced Controls Panel (Top) - 80px high
    int y = 28;
    
    // Checkboxes
    m_hwndMaxModeCheckbox = CreateWindowExA(0, "BUTTON", "Max Mode", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 
        5, y, 90, 20, m_hwndSecondarySidebar, (HMENU)IDC_CHK_MAX_MODE, m_hInstance, nullptr);
    m_hwndDeepThinkingCheckbox = CreateWindowExA(0, "BUTTON", "Deep Think", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 
        100, y, 95, 20, m_hwndSecondarySidebar, (HMENU)IDC_CHK_DEEP_THINK, m_hInstance, nullptr);
    m_hwndDeepResearchCheckbox = CreateWindowExA(0, "BUTTON", "Deep Research", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 
        200, y, 110, 20, m_hwndSecondarySidebar, (HMENU)IDC_CHK_DEEP_RESEARCH, m_hInstance, nullptr);
   
    y += 24;
    m_hwndNoRefusalCheckbox = CreateWindowExA(0, "BUTTON", "No Refusal", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 
        5, y, 90, 20, m_hwndSecondarySidebar, (HMENU)IDC_CHK_NO_REFUSAL, m_hInstance, nullptr);
    
    y += 24;
    CreateWindowExA(0, "STATIC", "Context:", WS_CHILD | WS_VISIBLE, 5, y+2, 50, 20, m_hwndSecondarySidebar, nullptr, m_hInstance, nullptr);
    m_hwndContextSlider = CreateWindowExA(0, TRACKBAR_CLASS, "Context", WS_CHILD | WS_VISIBLE | TBS_AUTOTICKS, 
        60, y, 180, 24, m_hwndSecondarySidebar, (HMENU)IDC_SLIDER_CONTEXT, m_hInstance, nullptr);
    m_hwndContextLabel = CreateWindowExA(0, "STATIC", "4K", WS_CHILD | WS_VISIBLE, 
        250, y+2, 60, 20, m_hwndSecondarySidebar, (HMENU)IDC_LBL_CONTEXT, m_hInstance, nullptr);
        
    // Init Slider
    SendMessage(m_hwndContextSlider, TBM_SETRANGE, TRUE, MAKELONG(0, 8)); // 0=4k, 1=8k... 8=1M
    SendMessage(m_hwndContextSlider, TBM_SETPOS, TRUE, 0);

    y += 30; // 106
    
    // Chat output area (read-only rich edit for formatted messages)
    int chatHeight = 350; // Reduced height
    m_hwndCopilotChatOutput = CreateWindowExA(
        WS_EX_CLIENTEDGE, "EDIT", "",
        WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL,
        5, y, m_secondarySidebarWidth - 10, chatHeight,
    ...
    m_hwndCopilotChatInput = CreateWindowExA(
        ...
        5, y, m_secondarySidebarWidth - 10, 60,
    ...
    SetWindowTextA(m_hwndCopilotChatOutput, 
        "RawrXD Inference Engine [MAX MODE AVBL]\r\n"
        "=======================================\r\n\r\n"
        "Engine Online. \r\n"
        "Select modes above for enhanced capability.\r\n"
        "- Max Mode: Full Compute\r\n"
        "- Deep Thinking: Chain of Thought\r\n"
        "- Deep Research: Knowledge Graph Scan\r\n\r\n");
}
    ...
    case WM_HSCROLL:
        {
            ...
            if (hControl == pThis->m_hwndMaxTokensSlider) {
                int pos = SendMessage(hControl, TBM_GETPOS, 0, 0);
                pThis->onContextSliderChange(pos);
            }
        }
        break;
    }
