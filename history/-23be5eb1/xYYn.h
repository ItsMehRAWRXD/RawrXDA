...
// Forward declarations only - no includes to avoid circular dependencies
class AgenticEngine;
class MemoryManager;
class VSIXLoader;

// Don't use namespace here, just declare it nullptr for now
struct ShellConfig {
    bool cli_mode = true;
...
private:
    ...
    void* react_generator_; // Opaque pointer
    ...
    void* react_generator_; // Opaque pointer to avoid dependency
    
public:
    InteractiveShell(const ShellConfig& config = ShellConfig());
    ~InteractiveShell();
