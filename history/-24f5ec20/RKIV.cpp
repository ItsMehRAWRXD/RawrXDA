...
void StreamingEngineRegistry::registerQuadBufferEngine() {
    ...
    desc.description = "Quad-Buffer DMA Streamer: 120B models on 16GB VRAM via demand paging & AVX-512 non-temporal streaming";
    desc.version = "1.0.0";
    ...
    // Wire function pointers to ASM exports
    desc.fnInit = QB_Init;
...
int StreamingEngineRegistry::scoreEngine(const StreamingEngineDescriptor& engine,
    ...
    // ---- Function pointer availability bonus ----
    if (engine.fnInit) score += 25;
    if (engine.fnLoadModel) score += 25;
    if (engine.fnStreamTensor) score += 25;
    if (engine.fnGetStats) score += 10;
