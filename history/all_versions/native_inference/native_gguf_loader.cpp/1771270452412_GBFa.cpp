    ...
    for (uint64_t i = 0; i < header_.metadata_count; ++i) {
        ...
        switch (value_type) {
            ...
            case static_cast<uint32_t>(GGUFUtils::MetadataType::ARRAY): {
                ...
                if (!ReadArray(arr, arr_type, arr_len)) {
...
size_t NativeGGUFLoader::CalculateTensorSize(const NativeGGUFTensorInfo& tensor) const {
    ...
    size_t element_size = GGUFUtils::GetDataTypeSize(tensor.type);
...
uint64_t NativeGGUFLoader::GetFileSize() {
...
std::string NativeGGUFLoader::GetMetadataString(const std::string& key) const {
    ...
    if (it != metadata_.end() && it->second.type == static_cast<uint32_t>(GGUFUtils::MetadataType::STRING)) {
...
uint64_t NativeGGUFLoader::GetMetadataUInt64(const std::string& key) const {
    auto it = metadata_.find(key);
    if (it != metadata_.end() && it->second.type == static_cast<uint32_t>(GGUFUtils::MetadataType::UINT64)) {
...
bool NativeGGUFLoader::GetMetadataBool(const std::string& key) const {
    auto it = metadata_.find(key);
    if (it != metadata_.end() && it->second.type == static_cast<uint32_t>(GGUFUtils::MetadataType::BOOL)) {
...
namespace GGUFUtils {
    size_t GetDataTypeSize(QuantType type) {
        switch (type) {
            case QuantType::F32: return 4;
            case QuantType::F16: return 2;
            case QuantType::Q4_0: case QuantType::Q4_1: case QuantType::Q4_K: return 1;
            case QuantType::Q5_K: case QuantType::Q3_K: case QuantType::Q2_K: case QuantType::Q6_K: case QuantType::Q8_0: case QuantType::Q5_1: return 1;
            default: return 4;
        }
    }

    size_t GetBlockSize(QuantType type) {
        switch (type) {
            case QuantType::Q4_0: case QuantType::Q4_1: return 32;
            case QuantType::Q5_0: case QuantType::Q5_1: return 32;
            case QuantType::Q8_0: case QuantType::Q8_1: return 32;
    ...
    bool IsQuantized(QuantType type) {
        return type >= QuantType::Q4_0;
    }
}
