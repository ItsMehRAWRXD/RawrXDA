...
# Verify cl.exe is available (via vcvars)
Write-Host "  Checking compiler accessibility..." -ForegroundColor DarkGray
$clTest = cmd /c "call `"$vcvarsPath`" > NUL && cl.exe 2>&1" | Select-String "Microsoft"
if (-not $clTest) {
...
# Using direct execution wrapped in vcvars to ensure environment is set
$cmdArgs = $cmakeArgs -join ' '
cmd /c "call `"$vcvarsPath`" > NUL && cmake $cmdArgs" *>> $logFile
...
$bArgs = $buildArgs -join ' '
cmd /c "call `"$vcvarsPath`" > NUL && cmake $bArgs" *>> $logFile
