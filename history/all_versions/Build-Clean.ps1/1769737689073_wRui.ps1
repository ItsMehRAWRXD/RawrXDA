...
# Import MSVC environment safely via temp file to avoid buffer limits
$tempEnv = [System.IO.Path]::GetTempFileName()
cmd /c "`"$vcvarsPath`" > NUL && set > `"$tempEnv`""
...
# Using direct execution to avoid Start-Process redirection overflow issues and ensure proper logging
