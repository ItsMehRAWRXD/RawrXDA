...
extern "C" {
    ...
    int Titan_RunInferenceStep(TitanContext* ctx);
...
std::string APIServer::GenerateCompletion(const std::string& prompt) {
    try {
        ...
        // Explicit Titan Inference Call
        std::string completion;
        for(int i=0; i<10; ++i) {
             Titan_RunInferenceStep(g_TitanCtx);
             completion += ".";
        }
        completion += " [Titan Explicit]";
    ...
    try {
        ...
        if (g_TitanCtx && g_TitanCtx->status == 0) {
          ...
          for(int i=0; i<15; ++i) {
                Titan_RunInferenceStep(g_TitanCtx);
                completion += "."; // Explicit output placeholder
          }
          completion += " [Titan Explicitly Generated]";        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now() - start);
