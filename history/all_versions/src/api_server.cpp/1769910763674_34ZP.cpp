...
std::string APIServer::GenerateCompletion(const std::string& prompt) {
    try {
        ...
        if (!app_state_.loaded_model || !app_state_.inference_engine) {
            LogApiOperation("WARN", "INFERENCE", "No model loaded or engine unavailable");
        ...
        // Real Inference Call
        std::string completion = app_state_.inference_engine->infer(prompt);
