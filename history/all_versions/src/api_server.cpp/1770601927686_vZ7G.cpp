    ...
    try {
        ...
        if (!app_state_.loaded_model || !app_state_.gpu_context) {
            ...
            if (cpuEngine) {
                ...
                std::string result = cpuEngine->Generate(prompt, app_state_.context_size);
                ...
                if (mmf.isInitialized()) {
                    ...
                    modelState.tokensGenerated += result.size() / 4; // Rough estimate
                    if (duration.count() > 0) {
                        modelState.tokensPerSecond = static_cast<float>(result.size() / 4) /
        ...
        auto* cpuEngine = static_cast<CPUInferenceEngine*>(app_state_.inference_engine);
        if (cpuEngine) {
            completion = cpuEngine->Generate(prompt, app_state_.context_size);
        } else {
        ...
        if (mmf.isInitialized()) {
            MmfModelState modelState = mmf.readModelState();
            modelState.tokensGenerated += completion.size() / 4;
            if (duration.count() > 0) {
                modelState.tokensPerSecond = static_cast<float>(completion.size() / 4) /
                    (duration.count() / 1000.0f);
            }
            modelState.isInferring = 0;
            modelState.lastInferenceTimestamp = GetTickCount64();
...
std::string APIServer::GenerateChatCompletion(const std::vector<ChatMessage>& messages) {
    try {
        ...
        auto* cpuEngine = static_cast<CPUInferenceEngine*>(app_state_.inference_engine);
        if (cpuEngine) {
            completion = cpuEngine->Generate(formattedPrompt, app_state_.context_size);
        } else {
