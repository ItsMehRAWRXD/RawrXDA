...
void APIServer::HandlePullRequest(const std::string& request, std::string& response) {
    try {
        ...
        // If we can't download, we should fail or return a clear status.
        LogApiOperation("WARNING", "PULL_REQUEST", "Model download requested but no downloader available. Request: " + model_name);
        response = CreateErrorResponse("Model download not supported in this environment yet.");
