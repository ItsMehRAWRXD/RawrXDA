...
void APIServer::HandleTagsRequest(std::string& response) {
    try {
        ...
        if (!app_state_.model_path.empty()) {
            // Extract filename from path
            size_t lastSlash = app_state_.model_path.find_last_of("/\\");
            if (lastSlash != std::string::npos) {
                modelName = app_state_.model_path.substr(lastSlash + 1);
            } else {
                modelName = app_state_.model_path;
            }
        }

        // Return list of loaded models with proper JSON structure
        std::stringstream json_ss;
        json_ss << "{\"models\":[{\"name\":\"" << modelName << "\",\"modified_at\":\"2026-01-31T12:00:00Z\",\"size\":0}]}";
        response = json_ss.str();
