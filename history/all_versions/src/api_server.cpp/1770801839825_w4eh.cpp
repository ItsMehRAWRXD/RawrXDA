                    ...
                    HandlePullRequest(request.body, response_body);
                     ...
                     if (user_message.empty()) {
                          ...
                          if (cot_ok && !cot_response.empty()) {
                               ...
                               // Fallback: generate directly without CoT analysis
                               LogApiOperation("WARN", "COT_BRIDGE", "CoT engine unreachable — falling back to direct generation");
                               std::string fallback = GenerateCompletion(user_message);
                               response_body = "{\"final_answer\":\"" + fallback + "\",\"trivial\":false,\"total_ms\":0,\"steps\":[],\"fallback\":true}";
                          }
                     }
...
bool APIServer::ProxyToCotEngine(const std::string& request_body, std::string& response,
    ...
    // Set timeouts: 5s connect, 10s send, 15s receive (CoT can be slow)
