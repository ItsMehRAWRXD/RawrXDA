...
@@dglv_key_done:
    ...
    ; Emit dummy variable entries based on stack frame analysis
    ; Variable: RBP-8, RBP-16, etc. (heuristic)
    mov edx, 0                       ; var index
@@dglv_var_loop:
    cmp edx, 8                       ; max 8 locals
    jae @@dglv_empty
    
    ; Check if this stack slot is accessible
    push rdx
    mov rax, rcx
    mov r8d, edx
    inc r8d
    ...
    sub rax, r8
    ; Try to read the value (may fault — protected by SEH in caller)
    mov r9, QWORD PTR [rax]
    
    ; Emit: {"name":"local_N","value":"0xVALUE"}
    cmp edx, 0
    je @@dglv_no_comma
    mov BYTE PTR [rdi], ','
    inc rdi
@@dglv_no_comma:
    ; We use simplified inline JSON building
    mov BYTE PTR [rdi], '{'
    inc rdi
    pop rdx
