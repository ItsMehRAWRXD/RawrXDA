...
void UniversalModelRouter::initializeLocalEngine(const std::string& path) {
    ...
    // Explicit logic to load model
    if (!path.empty()) {
        if (local_engine->loadModel(path)) {
            local_engine_ready = true;
        } else {
            std::cerr << "[Router] Failed to load local model from: " << path << std::endl;
            local_engine_ready = false;
        }
    } else {
        // Just initialized empty, wait for explicit load or default
        local_engine_ready = false;
    }
}
