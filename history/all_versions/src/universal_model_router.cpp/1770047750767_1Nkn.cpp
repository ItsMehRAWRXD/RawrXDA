...
std::string UniversalModelRouter::routeQuery(const std::string& model_name, const std::string& prompt, float temperature) {
    ...
    if (config.backend == ModelBackend::LOCAL_GGUF || config.backend == ModelBackend::LOCAL_TITAN) {
        ...
        std::string neededPath = config.model_id;
        if (neededPath.empty()) {
            // Try to resolve default if not specific
            // ... (keep usage of defaults logic if needed, or fail)
        }
        
        // Reload if path changed or not ready
        if (!local_engine_ready || current_loaded_model_path != neededPath) {
             if (!neededPath.empty() && std::filesystem::exists(neededPath)) {
                 std::cout << "[Router] Switching Local Model to: " << neededPath << std::endl;
                 if (local_engine->loadModel(neededPath).has_value()) {
                     local_engine_ready = true;
                     current_loaded_model_path = neededPath;
                 } else {
                     return "Error: Failed to load local model: " + neededPath;
                 }
             } else {
                 // Try generic initialization if we haven't loaded anything
                 if (!local_engine_ready) {
                    initializeLocalEngine("");
                    if (!local_engine_ready) return "Error: No local model loaded and requested model not found.";
                 }
                 // If we have a model loaded, and request didn't specify path, maybe keep using it?
                 // But strictly, if config is LOCAL, we expect model_id to be valid or we use default.
             }
        }
...
void UniversalModelRouter::routeStreamQuery(const std::string& model_name, const std::string& prompt, StreamCallback callback, float temperature) {
    ...
    if (config.backend == ModelBackend::LOCAL_GGUF || config.backend == ModelBackend::LOCAL_TITAN) {
        ...
        if (!local_engine_ready || current_loaded_model_path != neededPath) { 
             ...
             } else if (!local_engine_ready) {
                 initializeLocalEngine("");
             }
        }

        if (local_engine && local_engine_ready) {
            local_engine->GenerateStreaming(
                local_engine->Tokenize(prompt),
                512,
                callback,
