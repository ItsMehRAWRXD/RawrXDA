...
void Window::create(Window* parent_, const String& title, DWORD style, DWORD exStyle) {
    ...
    // hwnd is set in WndProcRouter WM_NCCREATE
}

// ── Widget-style Create: HWND parent + RECT ─────────────────────────────
void Window::Create(HWND parentHwnd, const RECT& rect, DWORD style) {
    const wchar_t* cls = m_className.empty() ? L"RawrXD_Widget" : m_className.c_str();
    ensureClassRegistered(cls);
    ...
    // WM_NCCREATE already set GWLP_USERDATA, but belt-and-suspenders
    if (hwnd) {
        SetWindowLongPtr(hwnd, GWLP_USERDATA, (LONG_PTR)this);
    }
}

void Window::ensureClassRegistered(const wchar_t* name) {
    // Try to register; ignore failure if class already exists
    WNDCLASSEXW wc = {};
    wc.cbSize = sizeof(wc);
...
LRESULT Window::handleMessage(UINT msg, WPARAM wParam, LPARAM lParam) {
    // Delegate to the virtual WndProc first — widgets override this.
    // If a derived class overrides WndProc AND handleMessage, WndProc gets priority.
    // The default WndProc() calls DefWindowProc, so if nobody overrides it
    // we fall through to the convenience handlers below.

    // Check if WndProc was overridden by comparing vtable
    // Simple approach: call WndProc; if it returns non-default, use that
    LRESULT res = WndProc(msg, wParam, lParam);

    // If a subclass overrode WndProc (and didn't call base), it handled the message.
    // We detect this by checking if the subclass has a different vtable entry.
    // For simplicity: if the class name is set (widget-style), use WndProc path.
    if (!m_className.empty()) {
        return res;
    }

    // Legacy path: convenience event handlers
    switch (msg) {
        case WM_PAINT: {
            PAINTSTRUCT ps;
            BeginPaint(hwnd, &ps);
