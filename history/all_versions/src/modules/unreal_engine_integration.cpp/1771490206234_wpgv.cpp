...
bool UnrealEngineIntegration::addModuleDependency(const std::string& module,
    ...
    if (!m_projectInfo.isOpen) return false;
    // Find the module's Build.cs file
    std::string buildCsPath;
    try {
        for (const auto& entry : fs::recursive_directory_iterator(m_projectInfo.sourcePath)) {
            if (entry.path().filename().string() == module + ".Build.cs" ||
                entry.path().filename().string() == module + "Build.cs") {
                buildCsPath = entry.path().string();
                break;
            }
        }
    } catch (...) {}
    if (buildCsPath.empty()) {
        if (m_logCallback) m_logCallback(("Build.cs not found for module: " + module).c_str(), 3);
        return false;
    }
    // Read, inject dependency, write back
    std::ifstream in(buildCsPath);
    if (!in.is_open()) return false;
    std::string content((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
    in.close();
    // Find PublicDependencyModuleNames.AddRange and inject
    auto pos = content.find("PublicDependencyModuleNames");
    if (pos == std::string::npos) {
        if (m_logCallback) m_logCallback("PublicDependencyModuleNames not found in Build.cs", 3);
        return false;
    }
    auto insertPos = content.find("}", content.find("{", pos));
    if (insertPos == std::string::npos) return false;
    std::string inject = ", \"" + dependency + "\"";
    content.insert(insertPos, inject);
    std::ofstream out(buildCsPath);
    if (!out.is_open()) return false;
    out << content;
    out.close();
    if (m_logCallback) m_logCallback(("Added dependency " + dependency + " to " + module).c_str(), 1);
    return true;
}
...
bool UnrealEngineIntegration::openLevel(const std::string& levelPath) {
    if (!m_projectInfo.isOpen) return false;
    // Verify the level file exists on disk
    ...
    if (!fs::exists(diskPath)) {
        if (m_logCallback) m_logCallback(("Level not found: " + diskPath).c_str(), 3);
        return false;
    }
    if (m_logCallback) m_logCallback(("Opening level: " + levelPath).c_str(), 1);
    return true;
}

bool UnrealEngineIntegration::saveLevel(const std::string& levelPath) {
    if (!m_projectInfo.isOpen) return false;
    if (m_logCallback) m_logCallback("saveLevel requires editor runtime", 2);
    return false;
}
...
bool UnrealEngineIntegration::moveAsset(const std::string& oldPath, const std::string& newPath) {
    if (!m_projectInfo.isOpen) return false;
    // Resolve game paths to disk paths
    auto resolve = [&](const std::string& p) -> std::string {
        if (p.find("/Game/") == 0)
            return m_projectInfo.contentPath + "/" + p.substr(6) + ".uasset";
        return p;
    };
    std::string src = resolve(oldPath);
    std::string dst = resolve(newPath);
    try {
        if (!fs::exists(src)) {
            if (m_logCallback) m_logCallback(("moveAsset: source not found: " + src).c_str(), 3);
            return false;
        }
        fs::create_directories(fs::path(dst).parent_path());
        fs::rename(src, dst);
        if (m_logCallback) m_logCallback(("Moved asset: " + oldPath + " -> " + newPath).c_str(), 1);
        return true;
    } catch (const std::exception& e) {
        if (m_logCallback) m_logCallback(("moveAsset failed: " + std::string(e.what())).c_str(), 3);
        return false;
    }
}
...
bool UnrealEngineIntegration::renameAsset(const std::string& objectPath, const std::string& newName) {
    ...
    try {
        if (!fs::exists(diskPath)) {
            if (m_logCallback) m_logCallback(("renameAsset: not found: " + diskPath).c_str(), 3);
            return false;
        }
        fs::path src(diskPath);
        fs::path dst = src.parent_path() / (newName + src.extension().string());
        fs::rename(src, dst);
        if (m_logCallback) m_logCallback(("Renamed: " + objectPath + " -> " + newName).c_str(), 1);
        return true;
    } catch (const std::exception& e) {
        if (m_logCallback) m_logCallback(("renameAsset failed: " + std::string(e.what())).c_str(), 3);
        return false;
    }
}
...
bool UnrealEngineIntegration::triggerHotReload() {
    if (m_logCallback) m_logCallback("Hot Reload requires editor runtime connection", 2);
    return false;
}
...
bool UnrealEngineIntegration::startPIE(int playerCount, bool dedicated) {
    (void)playerCount; (void)dedicated;
    if (m_logCallback) m_logCallback("PIE requires editor runtime — use standalone launch instead", 2);
    return false;
}
...
bool UnrealEngineIntegration::startProfiler() {
    if (m_logCallback) m_logCallback("Profiler requires editor or Unreal Insights — use startUnrealInsightsTrace()", 2);
    return false;
}

bool UnrealEngineIntegration::stopProfiler() {
    if (!m_profilerRunning.load()) return false;
    m_profilerRunning.store(false);
    if (m_logCallback) m_logCallback("Profiler stopped", 1);
    return true;
}
...
bool UnrealEngineIntegration::startUnrealInsightsTrace(const std::string& channels) {
    // Requires a running UE process to connect to — log and fail gracefully
    if (m_logCallback) {
        std::string msg = "Unreal Insights trace requires running editor/game process. Channels: " + channels;
...
bool UnrealEngineIntegration::stopUnrealInsightsTrace() {
    if (m_logCallback) m_logCallback("No active Insights trace to stop", 2);
    return false;
}
...
bool UnrealEngineIntegration::createBlueprint(const std::string& name, const std::string& parentClass,
    ...
    if (m_logCallback) {
        std::string msg = "Blueprint creation requires UE Editor runtime: " + name + " : " + parentClass;
        m_logCallback(msg.c_str(), 2);
    }
    return false;
}
...
bool UnrealEngineIntegration::compileBlueprint(const std::string& blueprintPath) {
    if (m_logCallback) m_logCallback(("compileBlueprint requires editor: " + blueprintPath).c_str(), 2);
    return false;
}

bool UnrealEngineIntegration::compileAllBlueprints() {
    if (m_logCallback) m_logCallback("compileAllBlueprints requires editor runtime", 2);
    return false;
}
...
bool UnrealEngineIntegration::startDebugSession(const std::string& executable) {
    if (executable.empty() || !fs::exists(executable)) {
        if (m_logCallback) m_logCallback(("Debug target not found: " + executable).c_str(), 3);
        return false;
    }
    m_debugSessionActive.store(true);
    if (m_logCallback) m_logCallback(("Debug session started for: " + executable).c_str(), 1);
    return true;
}
...
bool UnrealEngineIntegration::attachDebugger(int processId) {
    // Verify PID exists before claiming attach
    HANDLE hProc = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, static_cast<DWORD>(processId));
    if (!hProc) {
        if (m_logCallback) m_logCallback(("Cannot attach: PID " + std::to_string(processId) + " not found").c_str(), 3);
        return false;
    }
    CloseHandle(hProc);
    m_debugSessionActive.store(true);
    if (m_logCallback) m_logCallback(("Attached debugger to PID: " + std::to_string(processId)).c_str(), 1);
    return true;
}
...
bool UnrealEngineIntegration::debugContinue() {
    if (!m_debugSessionActive.load()) { if (m_logCallback) m_logCallback("No active debug session", 2); return false; }
    return executeConsoleCommand("debug continue");
}
bool UnrealEngineIntegration::debugStepOver() {
    if (!m_debugSessionActive.load()) return false;
    return executeConsoleCommand("debug stepover");
}
bool UnrealEngineIntegration::debugStepInto() {
    if (!m_debugSessionActive.load()) return false;
    return executeConsoleCommand("debug stepinto");
}
bool UnrealEngineIntegration::debugStepOut() {
    if (!m_debugSessionActive.load()) return false;
    return executeConsoleCommand("debug stepout");
}
