...
void VSCodeExtensionAPI::bridgeOutputToWin32(const VSCodeOutputChannel* channel) {
    ...
    if (!channel->content.empty()) {
        m_host->appendToOutput(channel->content, tabName);
        logDebug("[Bridge] Output channel '%s' flushed %zu bytes", tabName.c_str(), channel->content.size());
    } else {
...
void VSCodeExtensionAPI::bridgeTreeViewToWin32(const char* viewId,
    ...
    // Iterate root children from the provider and add them to the sidebar
    if (provider->rootItems.empty()) {
        logDebug("[Bridge] TreeView '%s' has no root items", viewId);
        return;
    }
    for (const auto& item : provider->rootItems) {
        m_host->addTreeItem(viewId, item.label.c_str(), item.id.c_str(),
                           item.hasChildren ? 1 : 0);
    }
    logDebug("[Bridge] TreeView '%s' bridged %zu root items", viewId, provider->rootItems.size());
}
...
void VSCodeExtensionAPI::bridgeDiagnosticsToLSP(const DiagnosticCollection* collection) {
    ...
    for (const auto& [filePath, diags] : collection->entries) {
        for (const auto& diag : diags) {
            m_host->addProblem(filePath.c_str(), diag.line, diag.column,
                              diag.severity, diag.message.c_str());
            totalDiags++;
        }
    }
