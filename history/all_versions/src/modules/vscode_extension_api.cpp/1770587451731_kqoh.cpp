...
void VSCodeExtensionAPI::bridgeOutputToWin32(const VSCodeOutputChannel* channel) {
    ...
    // Get the channel's pending output (the last line or accumulated buffer)
    // Output channels accumulate text via append(); we flush the latest content
    std::string content;
    {
        // VSCodeOutputChannel stores lines in its internal buffer
        // We grab the full buffer content for display
    ...
    if (!content.empty()) {
        // Route to the IDE's output panel with appropriate severity
        // Default to Info; channels don't have per-message severity
        m_host->appendToOutput(content, tabName);
    }

    logDebug("[Bridge] Output channel '%s' flushed %zu bytes", tabName.c_str(), content.size());
}
...
void VSCodeExtensionAPI::bridgeTreeViewToWin32(const char* viewId,
    ...
    HWND hwndTree = m_host->getSidebar();
    if (!hwndTree) return;

    // Get root elements from the provider
    VSCodeTreeItem rootItems[64];
    size_t rootCount = 0;
    if (provider->getChildren) {
        provider->getChildren(nullptr, rootItems, 64, &rootCount, provider->providerCtx);
    }
    ...
    // Create a root node for this view in the sidebar tree
    HTREEITEM hViewRoot = m_host->addTreeItem(TVI_ROOT, viewId, -1);
    if (!hViewRoot) return;

    // Populate first level of children
    for (size_t i = 0; i < rootCount; i++) {
        const auto& item = rootItems[i];
        std::string label = item.label;
        if (!item.description.empty()) {
            label += " — " + item.description;
        }
        m_host->addTreeItem(hViewRoot, label, -1);
    }

    // Expand the root node so items are visible
    TreeView_Expand(hwndTree, hViewRoot, TVE_EXPAND);

    logDebug("[Bridge] TreeView '%s' populated with %zu items", viewId, rootCount);
}

void VSCodeExtensionAPI::bridgeDiagnosticsToLSP(const DiagnosticCollection* collection) {
    if (!m_host || !collection) return;

    // Route each diagnostic to the Win32IDE Problems panel
    // DiagnosticCollection stores per-URI diagnostics
    for (const auto& [uri, diagnostics] : collection->diagnostics) {
        std::string filePath = uri;

        // Convert URI to filesystem path if needed
        // vscode URIs are typically "file:///C:/path/to/file.ext"
        ...
        for (const auto& diag : diagnostics) {
            // Map DiagnosticSeverity to Win32IDE severity integer
            // VS Code: 0=Error, 1=Warning, 2=Information, 3=Hint
            // Win32IDE addProblem: severity 0=Error, 1=Warning, 2=Info
            int severity = 0;
            switch (diag.severity) {
                case 0: severity = 0; break; // Error
                case 1: severity = 1; break; // Warning
                case 2: severity = 2; break; // Information
                case 3: severity = 2; break; // Hint → Info
                default: severity = 2; break;
            }

            // Build message with source and code if available
            ...
            // Route to Win32IDE Problems panel
            m_host->addProblem(filePath, diag.range.startLine + 1, 
                             diag.range.startCharacter + 1,
                             message, severity);
        }
    }

    // Refresh the Problems panel UI
    m_host->updateProblemsPanel();

    logDebug("[Bridge] Diagnostics collection '%s' published to Problems panel",
