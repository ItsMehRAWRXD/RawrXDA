    ...
    switch (engine) {
        case GameEngineType::Unity: {
            ...
            if (request.platform == "Win64" || request.platform == "Windows")
                ...
                config.target = UnityBuildTarget::StandaloneOSX;
...
bool GameEngineManager::resumePlayMode() {
    switch (m_activeEngine) {
        case GameEngineType::Unity:  return m_unity->resumePlayMode();
...
bool GameEngineManager::refreshInstallations() {
    ...
    // Unity installations
    auto unityVersions = m_unity->getInstalledEditors();
    for (const auto& editor : unityVersions) {
        EngineInstallation inst;
        inst.engine = GameEngineType::Unity;
        inst.version = editor.version;
        inst.path = editor.path;
        inst.isDefault = editor.isDefault;
...
GameProfilerSnapshotUnified GameEngineManager::convertUnitySnapshot(
    ...
    u.cpuTimeMs     = s.cpuTimeMs;
    u.gpuTimeMs     = s.gpuTimeMs;
    u.scriptTimeMs  = s.scriptTimeMs;
    u.physicsTimeMs = s.physicsTimeMs;
    u.renderTimeMs  = s.renderTimeMs;
