...
std::vector<uint8_t> NativeCompiler::GenerateShellcode(const std::string& payload, const std::string& arch) {
    ...
    // Simple single-pass assembler for x64 shellcode
    ...
    while (std::getline(lines, line)) {
        ...
        // Basic instructions
        if (line == "nop") shellcode.push_back(0x90);
        else if (line == "ret") shellcode.push_back(0xC3);
        else if (line == "int3") shellcode.push_back(0xCC);
        else if (line == "syscall") { shellcode.push_back(0x0F); shellcode.push_back(0x05); }
        
        // Push registers
        else if (line == "push rax") shellcode.push_back(0x50);
        else if (line == "push rcx") shellcode.push_back(0x51);
        else if (line == "push rdx") shellcode.push_back(0x52);
        else if (line == "push rbx") shellcode.push_back(0x53);
        else if (line == "push rsp") shellcode.push_back(0x54);
        else if (line == "push rbp") shellcode.push_back(0x55);
        else if (line == "push rsi") shellcode.push_back(0x56);
        else if (line == "push rdi") shellcode.push_back(0x57);
        
        // Pop registers
        else if (line == "pop rax") shellcode.push_back(0x58);
        else if (line == "pop rcx") shellcode.push_back(0x59);
        else if (line == "pop rdx") shellcode.push_back(0x5A);
        else if (line == "pop rbx") shellcode.push_back(0x5B);
        else if (line == "pop rsp") shellcode.push_back(0x5C);
        else if (line == "pop rbp") shellcode.push_back(0x5D);
        else if (line == "pop rsi") shellcode.push_back(0x5E);
        else if (line == "pop rdi") shellcode.push_back(0x5F);
        
        // XOR (Clear) registers
        else if (line == "xor rax, rax") { shellcode.push_back(0x48); shellcode.push_back(0x31); shellcode.push_back(0xC0); }
        else if (line == "xor rcx, rcx") { shellcode.push_back(0x48); shellcode.push_back(0x31); shellcode.push_back(0xC9); }
        else if (line == "xor rdx, rdx") { shellcode.push_back(0x48); shellcode.push_back(0x31); shellcode.push_back(0xD2); }
        else if (line == "xor rsi, rsi") { shellcode.push_back(0x48); shellcode.push_back(0x31); shellcode.push_back(0xF6); }
        else if (line == "xor rdi, rdi") { shellcode.push_back(0x48); shellcode.push_back(0x31); shellcode.push_back(0xFF); }
        
        // MOV support (basic)
        else if (line.find("mov rax, ") == 0) {
            std::string valStr = line.substr(9);
            uint64_t val = std::stoull(valStr, nullptr, 0);
            shellcode.push_back(0x48);
            shellcode.push_back(0xB8);
            for(int i=0; i<8; i++) shellcode.push_back((val >> (i*8)) & 0xFF);
        }
    }
