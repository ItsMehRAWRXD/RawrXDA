...
// Proxy middleware to C++ backend
app.use('/api/inference', createProxyMiddleware({ 
    target: `http://${CPP_BACKEND_HOST}:${CPP_BACKEND_PORT}`, 
    changeOrigin: true,
    pathRewrite: { '^/api/inference': '/inference' },
    onProxyReq: (proxyReq, req, res) => {
        // Add custom header or modify request here if needed
    }
}));
app.use('/api/engines', createProxyMiddleware({ 
    target: `http://${CPP_BACKEND_HOST}:${CPP_BACKEND_PORT}`, 
    changeOrigin: true,
    pathRewrite: { '^/api/engines': '/engines' },
}));
app.use('/api/engine/load', createProxyMiddleware({ 
    target: `http://${CPP_BACKEND_HOST}:${CPP_BACKEND_PORT}`, 
    changeOrigin: true,
    pathRewrite: { '^/api/engine/load': '/engine/load' },
}));
app.use('/api/mode', createProxyMiddleware({ 
    target: `http://${CPP_BACKEND_HOST}:${CPP_BACKEND_PORT}`, 
    changeOrigin: true,
    pathRewrite: { '^/api/mode': '/mode' },
}));
app.use('/api/tools', createProxyMiddleware({ 
    target: `http://${CPP_BACKEND_HOST}:${CPP_BACKEND_PORT}`, 
    changeOrigin: true,
    pathRewrite: { '^/api/tools': '/tools' },
}));
app.use('/api/memory', createProxyMiddleware({ 
    target: `http://${CPP_BACKEND_HOST}:${CPP_BACKEND_PORT}`, 
    changeOrigin: true,
    pathRewrite: { '^/api/memory': '/memory' },
}));
...
std::string ReactServerGenerator::GetAppJsContent(const ReactServerConfig& config) {
    if (config.include_ide_features) {
        std::stringstream ss;
        ss << R"(import React from 'react';
import MainIDE from './MainIDE';
import './App.css';

function App() {
  return (
    <div className="App h-screen overflow-hidden bg-black text-white">
      <MainIDE />
    </div>
  );
}
...
std::string ReactServerGenerator::GetAgentModePanelContent(const ReactServerConfig& config) {
    return R"JS(import React, { useState, useEffect } from 'react';
...
std::string ReactServerGenerator::GetREToolsPanelContent(const ReactServerConfig& config) {
    return R"(import React, { useState } from 'react';
import axios from 'axios';
import { Binary, FileCode, Play, Terminal } from 'lucide-react';

export const RETools = ({ onLog }) => {
    const [target, setTarget] = useState('');
    const [loading, setLoading] = useState(false);

    const runCommand = async (cmd, actionName) => {
        if (!target) return;
        setLoading(true);
        try {
            onLog({ type: 'info', message: `Running ${actionName} on ${target}...`, time: new Date().toLocaleTimeString() });
            const res = await axios.post('/api/command', { command: `!engine ${cmd} ${target}` });
            if (res.data && res.data.output) {
                onLog({ type: 'success', message: res.data.output, time: new Date().toLocaleTimeString() });
            }
        } catch (e) {
            onLog({ type: 'error', message: `Failed to run ${actionName}`, time: new Date().toLocaleTimeString() });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="bg-gray-900 p-4 rounded-lg border border-gray-700 mt-4">
            <h3 className="text-lg font-bold text-blue-400 mb-4 flex items-center gap-2">
                <Binary className="w-5 h-5" /> Reverse Engineering Suite
            </h3>
            
            <input 
                className="w-full bg-gray-800 text-gray-200 p-2 mb-4 text-sm border border-gray-700 rounded font-mono"
                placeholder="Target Binary / Source Path..."
                value={target} onChange={e => setTarget(e.target.value)}
            />

            <div className="grid grid-cols-2 gap-2">
                <button 
                    onClick={() => runCommand('disasm', 'Disassembler')}
                    disabled={loading}
                    className="bg-gray-800 hover:bg-gray-700 text-gray-200 py-2 px-3 rounded text-sm flex items-center justify-center gap-2 border border-gray-600"
                >
                    <FileCode className="w-4 h-4 text-purple-400" /> Disassemble
                </button>
                <button 
                    onClick={() => runCommand('dumpbin', 'PE Analyzer')}
                    disabled={loading}
                    className="bg-gray-800 hover:bg-gray-700 text-gray-200 py-2 px-3 rounded text-sm flex items-center justify-center gap-2 border border-gray-600"
                >
                    <Terminal className="w-4 h-4 text-green-400" /> Dump Headers
                </button>
               <button 
                    onClick={() => runCommand('compile', 'MASM64 Compiler')}
                    disabled={loading}
                    className="col-span-2 bg-blue-900 hover:bg-blue-800 text-blue-100 py-2 px-3 rounded text-sm flex items-center justify-center gap-2 border border-blue-700"
                >
                    <Play className="w-4 h-4" /> Compile (MASM64)
                </button>
            </div>
        </div>
    );
...
std::string ReactServerGenerator::GetMainIDEAppContent(const ReactServerConfig& config) {
    return R"(import React, { useState } from 'react';
import { AgentModes } from './components/ide/AgentModes';
import { EngineManager } from './components/ide/EngineManager';
import { RETools } from './components/ide/RETools';
import { HotpatchControls } from './components/ide/Hotpatch';
import { MemoryViewer } from './components/ide/MemoryViewer';
import { CodeEditor } from './components/ide/CodeEditor';
import { ToolOutput } from './components/ide/ToolOutput';
import { Terminal } from 'lucide-react';

const MainIDE = () => {
  const [logs, setLogs] = useState([]);
  const [code, setCode] = useState("// Welcome to RawrXD IDE\n// Start coding or use the tools on the right.");
import { MemoryViewer } from './components/ide/MemoryViewer';
import { CodeEditor } from './components/ide/CodeEditor';
import { ToolOutput } from './components/ide/ToolOutput';
import { Terminal } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-black text-gray-300 overflow-hidden font-sans">
      ...
      <div className="w-16 bg-gray-900 border-r border-gray-800 flex flex-col items-center py-4 gap-4">
         <div className="p-2 bg-blue-600 rounded-lg text-white font-bold text-xs">XD</div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-12 bg-gray-900 border-b border-gray-800 flex items-center px-4 justify-between">
            <span className="font-semibold text-gray-200">RawrXD Ultimate IDE</span>
            <div className="flex items-center gap-4 text-sm">
                <span>CPP Backend: Connected</span>
            </div>
        </header>
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 flex flex-col bg-gray-900/50">
                ...
                <div className="h-48 border-t border-gray-800 flex flex-col">
                    <div className="bg-gray-900 px-4 py-1 flex items-center gap-2 text-xs border-b border-gray-800">
                        <Terminal className="w-3 h-3" /> Output / Logs
                    </div>
                    <ToolOutput logs={logs} />
                </div>
            </div>

            {/* Right Panel (Tools) */}
            <div className="w-80 border-l border-gray-800 bg-gray-900 flex flex-col overflow-y-auto custom-scrollbar">
                <div className="p-2 space-y-2">
                    <AgentModes />
                    <EngineManager />
                    <RETools onLog={addLog} />
                    <HotpatchControls />
                </div>
            </div>
        </div>
