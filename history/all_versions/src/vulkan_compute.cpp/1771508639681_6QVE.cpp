...
// Vulkan SDK is optional — RAWR_HAS_VULKAN defined by CMake.
// When Vulkan is unavailable, all methods return false/no-op (CPU fallback).
#if RAWR_HAS_VULKAN
#include <vulkan/vulkan.h>
// Logger replaces iostream - see IDELogger.h
#include <algorithm>
#include <fstream>
#include <sstream>
#include <cmath>
...
bool VulkanCompute::Initialize() {
    ...
    Logger::info("Vulkan initialized successfully on device: " + std::string(device_info_.device_name));
    Logger::info(std::string("AMD Device: ") + (IsAMDDevice() ? "Yes" : "No"));
    Logger::info("Compute Queue Family: " + std::to_string(device_info_.compute_queue_family));
...
void VulkanCompute::InitializeCommandBufferPool(uint32_t pool_size) {
    ...
    if (vkAllocateCommandBuffers(device_, &alloc_info, buffers.data()) != VK_SUCCESS) {
        Logger::error("Failed to allocate command buffers for pool");
        return;
    }
    ...
    for (uint32_t i = 0; i < pool_size; ++i) {
        ...
        if (vkCreateFence(device_, &fence_info, nullptr, &command_buffer_pool_[i].fence) != VK_SUCCESS) {
            Logger::error("Failed to create fence for command buffer pool");
            return;
        }
    ...
    Logger::info("Initialized command buffer pool with " + std::to_string(pool_size) + " buffers");
}
...
VkCommandBuffer VulkanCompute::AcquireAsyncCommandBuffer() {
    ...
    Logger::error("WARNING: All command buffers in use, this may cause stalls. Increase pool size.");
    return nullptr;
}
...
bool VulkanCompute::SubmitAsyncCommandBuffer(VkCommandBuffer cmd_buffer) {
    ...
    if (pool_idx < 0) {
        Logger::error("Command buffer not from pool");
        return false;
    }
...
bool VulkanCompute::FlushAsyncCommands() {
    ...
    if (vkWaitForFences(device_, (uint32_t)all_fences.size(), all_fences.data(), VK_TRUE, UINT64_MAX) != VK_SUCCESS) {
        Logger::error("Failed to wait for async command buffers");
        return false;
    }
...
bool VulkanCompute::ExecuteSingleTimeCommands(std::function<void(VkCommandBuffer)> record_func) {
    ...
    if (vkBeginCommandBuffer(command_buffer, &begin_info) != VK_SUCCESS) {
        Logger::error("Failed to begin command buffer recording");
    ...
    if (vkEndCommandBuffer(command_buffer) != VK_SUCCESS) {
        Logger::error("Failed to end command buffer recording");
        vkFreeCommandBuffers(device_, command_pool_, 1, &command_buffer);
        return false;
    }
...
bool VulkanCompute::ExecuteCommandBuffer(VkCommandBuffer cmd_buffer) {
    ...
    if (wait_result != VK_SUCCESS) {
        Logger::error("Command buffer execution timeout or failed");
        vkDestroyFence(device_, fence, nullptr);
        return false;
    }
...
bool VulkanCompute::SelectPhysicalDevice() {
    ...
    for (uint32_t i = 0; i < device_count; ++i) {
        ...
        Logger::info("Found device " + std::to_string(i) + ": " + std::string(props.deviceName) 
    ...
    Logger::info("Selected device: " + std::string(device_info_.device_name));
...
bool VulkanCompute::LoadShader(const std::string& name, const std::string& spirv_path) {
    ...
    if (!LoadSPIRVCode(spirv_path, spirv_code)) {
        Logger::error("Failed to load SPIR-V code: " + std::to_string(spirv_path));
        return false;
    }
    ...
    if (vkCreateShaderModule(device_, &create_info, nullptr, &shader.module) != VK_SUCCESS) {
        Logger::error("Failed to create shader module: " + std::to_string(name));
        return false;
    }

    shaders_[name] = std::move(shader);
    Logger::info("Loaded shader: " + std::to_string(name));
...
bool VulkanCompute::CreateComputePipeline(const std::string& shader_name) {
    ...
    if (it == shaders_.end()) {
        Logger::error("Shader not found: " + std::to_string(shader_name));
        return false;
    }
    ...
    if (vkCreatePipelineLayout(device_, &pipeline_layout_info, nullptr, &it->second.layout) != VK_SUCCESS) {
        Logger::error("Failed to create pipeline layout: " + std::to_string(shader_name));
        return false;
    }
    ...
    if (vkCreateComputePipelines(device_, nullptr, 1, &compute_pipeline_info, nullptr, &it->second.pipeline) != VK_SUCCESS) {
        Logger::error("Failed to create compute pipeline: " + std::to_string(shader_name));
        return false;
    }

    Logger::info("Created compute pipeline: " + std::to_string(shader_name));
    return true;
}
...
bool VulkanCompute::CreateStagingBuffer(size_t size, VkBuffer& buffer, VkDeviceMemory& memory) {
    ...
    if (vkAllocateMemory(device_, &alloc_info, nullptr, &memory) != VK_SUCCESS) {
        Logger::error("Failed to allocate staging memory");
        vkDestroyBuffer(device_, buffer, nullptr);
        return false;
    }
...
bool VulkanCompute::AllocateBuffer(size_t size, uint32_t& buffer_idx, size_t& memory_size) {
    ...
    Logger::info("Allocated buffer " + std::to_string(buffer_idx) + " with " + std::to_string(size) + " bytes");
    return true;
}
...
bool VulkanCompute::CopyBufferToHost(VkBuffer device_buffer, void* host_data, size_t size) {
    ...
    if (!ExecuteSingleTimeCommands([&](VkCommandBuffer cmd_buffer) {
        ...
        Logger::error("Failed to copy buffer from device to host");
    ...
    if (vkMapMemory(device_, staging_memory, 0, size, 0, &mapped_data) != VK_SUCCESS) {
        Logger::error("Failed to map staging memory");
        vkDestroyBuffer(device_, staging_buffer, nullptr);
        vkFreeMemory(device_, staging_memory, nullptr);
        return false;
    }
...
bool VulkanCompute::CopyBufferToHost(uint32_t buffer_idx, void* host_data, size_t size) {
    if (buffer_idx >= allocated_buffers_.size()) {
        Logger::error("Invalid buffer index: " + std::to_string(buffer_idx));
        return false;
    }
...
bool VulkanCompute::CopyHostToBuffer(void* host_data, VkBuffer device_buffer, size_t size) {
    ...
    if (!ExecuteSingleTimeCommands([&](VkCommandBuffer cmd_buffer) {
        ...
        Logger::error("Failed to copy buffer from host to device");
        vkDestroyBuffer(device_, staging_buffer, nullptr);
        vkFreeMemory(device_, staging_memory, nullptr);
        return false;
    }
...
VulkanTensor VulkanCompute::TransferGGUFTensor(const std::string& tensor_name,
    ...
    if (!AllocateBuffer(size_bytes, tensor.device_buffer, tensor.device_memory)) {
        Logger::error("Failed to allocate device buffer for tensor: " + std::to_string(tensor_name));
        return tensor;
    }
    
    // Copy host data to device
    if (!CopyHostToBuffer(tensor.host_data.data(), tensor.device_buffer, size_bytes)) {
        Logger::error("Failed to transfer tensor to device: " + std::to_string(tensor_name));
        return tensor;
    }
    
    uploaded_tensors_.push_back(tensor);
    Logger::info("Transferred tensor " + tensor_name + " (" + std::to_string(size_bytes) + " bytes) to device");
...
void VulkanCompute::ReleaseTensors() {
    ...
    Logger::info("Released all tensors");
}
...
bool VulkanCompute::EnsureMatMulPipeline(const std::string& spirv_path) {
    ...
    if (!LoadShader("matmul", spirv_path)) {
        Logger::error("Failed to load matmul shader from: " + std::to_string(spirv_path));
        return false;
    }
    ...
    if (it == shaders_.end()) {
        Logger::error("Shader not found after loading");
        return false;
    }
    ...
    if (vkCreateDescriptorSetLayout(device_, &layout_info, nullptr, &matmul_descriptor_set_layout_) != VK_SUCCESS) {
        Logger::error("Failed to create MatMul descriptor set layout!");
        return false;
    }
    Logger::info("Created permanent MatMul descriptor set layout");
    ...
    if (vkCreatePipelineLayout(device_, &pipeline_layout_info, nullptr, &it->second.layout) != VK_SUCCESS) {
        Logger::error("Failed to create MatMul pipeline layout");
        vkDestroyDescriptorSetLayout(device_, matmul_descriptor_set_layout_, nullptr);
    ...
    Logger::info("Created MatMul pipeline layout with push constants");
    ...
    if (vkCreateComputePipelines(device_, nullptr, 1, &compute_pipeline_info, nullptr, &it->second.pipeline) != VK_SUCCESS) {
        Logger::error("Failed to create MatMul compute pipeline");
        return false;
    }

    Logger::info("MatMul pipeline and permanent descriptor system initialized successfully");
    return true;
}
...
bool VulkanCompute::CreateDescriptorSetLayout(uint32_t binding_count, VkDescriptorSetLayout& layout) {
    ...
    Logger::error("WARNING: CreateDescriptorSetLayout is deprecated. Use EnsureMatMulPipeline instead.");
    return false;
}
...
bool VulkanCompute::AllocateDescriptorSet(VkDescriptorSetLayout layout, VkDescriptorSet& descriptor_set) {
    ...
    // Kept for backward compatibility but should not be used
    Logger::error("WARNING: AllocateDescriptorSet is deprecated. Use DispatchMatMul instead.");
    return false;
}
...
bool VulkanCompute::UpdateDescriptorSet(VkDescriptorSet descriptor_set, uint32_t binding, 
    ...
    // Kept for backward compatibility but should not be used
    Logger::error("WARNING: UpdateDescriptorSet is deprecated. Use DispatchMatMul instead.");
    return false;
}
...
bool VulkanCompute::DispatchMatMul(uint32_t input_a_idx,
    ...
    if (input_a_idx >= allocated_buffers_.size() || 
        ...
        Logger::error("Invalid buffer indices for MatMul dispatch");
        return false;
    }
    ...
    auto it = shaders_.find("matmul");
    if (it == shaders_.end() || !it->second.pipeline || !matmul_descriptor_set_layout_ || !matmul_descriptor_pool_) {
        Logger::error("MatMul pipeline or descriptor system not initialized. Call EnsureMatMulPipeline first.");
        return false;
    }
    ...
    if (vkAllocateDescriptorSets(device_, &alloc_info, &descriptor_set) != VK_SUCCESS) {
        Logger::error("Failed to allocate descriptor set for MatMul");
        return false;
    }
    ...
    Logger::info("Updated descriptor set with 3 storage buffers (A, B, Output)");
    ...
    bool success = ExecuteSingleTimeCommands([&](VkCommandBuffer cmd_buffer) {
        ...
        Logger::info("Dispatching: " + std::to_string(group_count_x) + "x" + std::to_string(group_count_y) + "x1 workgroups");
    ...
    if (success) {
        Logger::info("MatMul dispatch completed: " + std::to_string(M) + "x" + std::to_string(K) + " * " + std::to_string(K) + "x" + std::to_string(N));
    } else {
        Logger::error("MatMul GPU dispatch failed.");
    }
...
bool VulkanCompute::DispatchMatMulAsync(uint32_t input_a_idx,
    ...
    if (input_a_idx >= allocated_buffers_.size() || 
        input_b_idx >= allocated_buffers_.size() ||
        output_idx >= allocated_buffers_.size()) {
        Logger::error("Invalid buffer indices for MatMul async dispatch");
        return false;
    }
    ...
    auto it = shaders_.find("matmul");
    if (it == shaders_.end() || !it->second.pipeline || !matmul_descriptor_set_layout_ || !matmul_descriptor_pool_) {
        Logger::error("MatMul pipeline not initialized. Call EnsureMatMulPipeline first.");
        return false;
    }
    ...
    VkDescriptorSet descriptor_set = nullptr;
    if (vkAllocateDescriptorSets(device_, &alloc_info, &descriptor_set) != VK_SUCCESS) {
        Logger::error("Failed to allocate descriptor set for async MatMul");
        return false;
    }
    ...
    if (!cmd_buffer) {
        Logger::error("No available command buffers in pool. Consider FlushAsyncCommands().");
        vkFreeDescriptorSets(device_, matmul_descriptor_pool_, 1, &descriptor_set);
        return false;
    }
    ...
    if (vkBeginCommandBuffer(cmd_buffer, &begin_info) != VK_SUCCESS) {
        Logger::error("Failed to begin command buffer for async MatMul");
        vkFreeDescriptorSets(device_, matmul_descriptor_pool_, 1, &descriptor_set);
        return false;
    }
    ...
    if (vkEndCommandBuffer(cmd_buffer) != VK_SUCCESS) {
        Logger::error("Failed to end command buffer for async MatMul");
        vkFreeDescriptorSets(device_, matmul_descriptor_pool_, 1, &descriptor_set);
        return false;
    }
    ...
    if (!SubmitAsyncCommandBuffer(cmd_buffer)) {
        Logger::error("Failed to submit async MatMul command buffer");
        vkFreeDescriptorSets(device_, matmul_descriptor_pool_, 1, &descriptor_set);
        return false;
    }

    Logger::info("Async MatMul queued: " + std::to_string(M) + "x" + std::to_string(K) + " * " + std::to_string(K) + "x" + std::to_string(N)
              + " (" + std::to_string(group_count_x) + "x" + std::to_string(group_count_y) + " workgroups)");
...
bool VulkanCompute::LoadSPIRVCode(const std::string& path, std::vector<uint32_t>& code) {
    ...
    if (!file.is_open()) {
        Logger::error("Failed to open SPIR-V file: " + std::to_string(path));
        return false;
    }

    size_t file_size = file.tellg();
    if (file_size % sizeof(uint32_t) != 0) {
        Logger::error("Invalid SPIR-V file size: " + std::to_string(path));
        return false;
    }
...
bool VulkanCompute::AllocateKVCache(uint32_t num_layers, uint32_t max_seq_len, uint32_t head_dim) {
    if (kv_cache_allocated_) {
        Logger::error("KV cache already allocated. Call ClearKVCache() first.");
        return false;
    }
    ...
    Logger::info("Allocating KV cache: " + std::to_string(num_layers) + " layers, "
              + std::to_string(max_seq_len) + " max tokens, " + std::to_string(head_dim) + " head_dim, "
              + std::to_string(cache_size / 1024 / 1024) + " MB per buffer");
    ...
    for (uint32_t layer = 0; layer < num_layers; ++layer) {
        ...
        if (!AllocateBuffer(cache_size, k_buffer, k_memory)) {
            Logger::error("Failed to allocate K cache for layer " + std::to_string(layer));
            ClearKVCache();
            return false;
        }
        ...
        if (!CopyHostToBuffer(zeros.data(), k_buffer, cache_size)) {
            Logger::error("Failed to zero-init K cache for layer " + std::to_string(layer));
            ClearKVCache();
            return false;
        }
        ...
        if (!AllocateBuffer(cache_size, v_buffer, v_memory)) {
            Logger::error("Failed to allocate V cache for layer " + std::to_string(layer));
            ClearKVCache();
            return false;
        }
        ...
        if (!CopyHostToBuffer(zeros.data(), v_buffer, cache_size)) {
            Logger::error("Failed to zero-init V cache for layer " + std::to_string(layer));
            ClearKVCache();
            return false;
        }
    }
    ...
    Logger::info("KV cache allocated successfully: "
              + std::to_string(num_layers * 2 * cache_size / 1024 / 1024) + " MB total");
...
bool VulkanCompute::AppendToKVCache(uint32_t layer_idx, const float* k_new, 
    ...
    if (!kv_cache_allocated_) {
        Logger::error("KV cache not allocated. Call AllocateKVCache() first.");
        return false;
    }
    
    if (layer_idx >= kv_cache_num_layers_) {
        Logger::error("Invalid layer index: " + std::to_string(layer_idx) + " >= " + std::to_string(kv_cache_num_layers_));
        return false;
    }
    
    if (token_pos >= kv_cache_max_seq_len_) {
        Logger::error("Token position " + std::to_string(token_pos) + " exceeds max_seq_len " + std::to_string(kv_cache_max_seq_len_));
        return false;
    }
    ...
    if (!CopyHostToBufferOffset(k_new, k_buffer, offset, size)) {
        Logger::error("Failed to update K cache at layer " + std::to_string(layer_idx) + ", pos " + std::to_string(token_pos));
        return false;
    }
    
    // Update V cache at token_pos
    if (!CopyHostToBufferOffset(v_new, v_buffer, offset, size)) {
        Logger::error("Failed to update V cache at layer " + std::to_string(layer_idx) + ", pos " + std::to_string(token_pos));
        return false;
    }
...
bool VulkanCompute::GetKVCacheSlice(uint32_t layer_idx, uint32_t start_pos, 
    ...
    if (!kv_cache_allocated_) {
        Logger::error("KV cache not allocated");
        return false;
    }
    
    if (layer_idx >= kv_cache_num_layers_) {
        Logger::error("Invalid layer index: " + std::to_string(layer_idx));
        return false;
    }
    
    if (end_pos > kv_cache_max_seq_len_ || start_pos >= end_pos) {
        Logger::error("Invalid slice range: [" + std::to_string(start_pos) + ", " + std::to_string(end_pos) + ")");
        return false;
    }
    ...
    if (!CopyBufferToHostOffset(k_buffer, offset, k_out, size)) {
        Logger::error("Failed to read K cache slice");
        return false;
    }
    
    // Read V cache slice
    if (!CopyBufferToHostOffset(v_buffer, offset, v_out, size)) {
        Logger::error("Failed to read V cache slice");
        return false;
    }
...
bool VulkanCompute::CopyHostToBufferOffset(const void* host_data, VkBuffer device_buffer, 
    ...
    if (vkCreateBuffer(device_, &buffer_info, nullptr, &staging_buffer) != VK_SUCCESS) {
        Logger::error("Failed to create staging buffer for offset copy");
        return false;
    }
...
void VulkanCompute::Cleanup() {
    ...
    Logger::info("Vulkan resources cleaned up successfully");
}

#else // !RAWR_HAS_VULKAN — CPU-only stub implementations
// When Vulkan SDK is not found, the class from src/vulkan_compute.h (stub header)
// provides inline no-op implementations. Nothing additional needed here.
