...
std::unordered_map<std::string, Tensor> m_weights; 
    ...
    std::unique_ptr<RawrXD::BPETokenizer> m_tokenizer;s> m_layers;
...
} // namespace CPUInference};    std::vector<float*> m_memoryPool;    // Memory pool for tensors        Tensor m_outputWeights;    std::vector<LayerWeights> m_layers;    // Weights organized by layer        std::unordered_map<std::string, Tensor> m_weights; 