...
public:
    ...
    // Execution Scheduler integration (Phase 9.2)
    void InitScheduler();
    ExecutionScheduler* GetScheduler() const { return m_scheduler.get(); }
    bool UseScheduledExecution() const { return m_useScheduler; }
    void SetScheduledExecution(bool enable) { m_useScheduler = enable; }

    // Public for scheduler access
...
private:
    ...
    // Execution Scheduler (Phase 9.2)
    std::unique_ptr<ExecutionScheduler> m_scheduler;
