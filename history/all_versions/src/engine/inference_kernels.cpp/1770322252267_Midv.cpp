...
void InferenceKernels::rope_avx512(float* data, float* data_t, int dim, int pos, float theta, float scale) {
    for (int i = 0; i < dim; i += 2) {
        float rot_freq = 1.0f / std::pow(theta, static_cast<float>(i) / dim);
        float m_theta = pos * rot_freq;
        
        float cos_v = std::cos(m_theta);
        float sin_v = std::sin(m_theta);
        
        float x0 = data[i];
        float x1 = data[i + 1];
        
        data[i] = (x0 * cos_v - x1 * sin_v) * scale;
        data[i + 1] = (x0 * sin_v + x1 * cos_v) * scale;
