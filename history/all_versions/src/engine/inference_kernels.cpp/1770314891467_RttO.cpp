...
void InferenceKernels::matmul_f16_avx512(const uint16_t* A, const uint16_t* B, float* C,
    ...
    for (int m = 0; m < M; m++) {
        for (int n = 0; n < N; n++) {
            float sum = 0.0f;
            for (int k = 0; k < K; k++) {
                uint16_t a_val = A[m * K + k];
                uint16_t b_val = B[n * K + k]; // Careful: B might be transposed or not? Match matmul_f32 logic.
                
                auto f16 = [](uint16_t h) -> float {
                    uint32_t s = (h & 0x8000) << 16;
                    uint32_t e = (h & 0x7C00) >> 10;
                    uint32_t m_val = (h & 0x03FF) << 13;
                    uint32_t v;
                    if(e==0) v = (m_val==0) ? s : (s | 0x3F800000 | (m_val << 10));
                    else if(e==0x1F) v = s | 0x7F800000 | m_val;
                    else v = s | ((e+112)<<23) | m_val;
                    float f; std::memcpy(&f, &v, 4); return f;
                };
                
                sum += f16(a_val) * f16(b_val);
            }
            C[m * N + n] = sum;
        }
    }
