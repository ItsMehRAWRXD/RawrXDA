...
void InferenceKernels::matmul_f32(const float* A, const float* B, float* C, int M, int N, int K) {
    ...
    for (int m = 0; m < M; m++) {
        for (int n = 0; n < N; n++) {
            ...
            for (int k = 0; k < K; k++) {
                sum += A[m * K + k] * B[n * K + k]; // Assumes B is transposed [N, K]
            }
            C[m * N + n] = sum;
        }
    }
            ...
            for (int b = 0; b < blocks_per_row; b++) {
                ...
                for (int q = 0; q < 16; q++) {
                    ...
                    __m512 w_float = _mm512_cvtepi32_ps(_mm512_cvtepi16_epi32(
                        _mm256_cvtepi8_epi16(_mm256_castsi256_si128(w_int))
                    ));
...
void InferenceKernels::softmax_avx512(float* x, int n) {
    ...
    for (int i = 0; i < n; i += 16) {
        ...
        alignas(64) float exp_vals[16];
        alignas(64) float out_vals[16];
        _mm512_storeu_ps(exp_vals, exp_x);
        for (int j = 0; j < remaining; j++) {
            float val = expf(exp_vals[j]);
            out_vals[j] = val;
            sum += val;
        }
        for (int j = remaining; j < 16; j++) out_vals[j] = 0.0f;
        
        __m512 exp_result = _mm512_loadu_ps(out_vals);
