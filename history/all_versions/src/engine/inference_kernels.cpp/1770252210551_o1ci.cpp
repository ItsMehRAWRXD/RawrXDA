...
void InferenceKernels::rmsnorm_avx512(float* o, const float* x, const float* weight, 
    ...
    for (int i = 0; i < n; i += 16) {
        ...
        if (remaining == 16) {
            ...
            __mmask16 mask = (1 << remaining) - 1;
            v = _mm512_maskz_loadu_ps(mask, &x[i]);
        }
        
        __m512 v_sq = _mm512_mul_ps(v, v);
        ss_vec = _mm512_add_ps(ss_vec, v_sq);
    }
    
    float ss = _mm512_reduce_add_ps(ss_vec);
    ss = ss / n + eps;
    float norm_factor = 1.0f / sqrtf(ss);
    ...
    for (int i = 0; i < n; i += 16) {
        ...
        __m512 normalized = _mm512_mul_ps(x_vec, v_norm);
        __m512 result = _mm512_mul_ps(normalized, w_vec);
        
        if (remaining == 16) {
            _mm512_storeu_ps(&o[i], result);
        } else {
            __mmask16 mask = (1 << remaining) - 1;
            _mm512_mask_storeu_ps(&o[i], mask, result);
        }
    }
#else
    // Fallback
...
void InferenceKernels::rope_avx512(float* q, float* k, int head_dim, int pos, 
    ...
    for (int i = 0; i < head_dim; i += 2) {
        ...
        // Frequency for this dimension: theta^(-2i/d) where theta=10000
        float freq = scale / powf(theta, (2.0f * dim_idx) / head_dim);
        float angle = pos * freq;
        
        float cos_angle = cosf(angle);
        float sin_angle = sinf(angle);
        
        // Apply 2D rotation to Q
        if (i + 1 < head_dim) {
            float q0 = q[i];
            float q1 = q[i + 1];
            q[i]     = q0 * cos_angle - q1 * sin_angle;
            q[i + 1] = q0 * sin_angle + q1 * cos_angle;
            
            // Apply same rotation to K
            float k0 = k[i];
            float k1 = k[i + 1];
            k[i]     = k0 * cos_angle - k1 * sin_angle;
            k[i + 1] = k0 * sin_angle + k1 * cos_angle;
        }
    }
}
