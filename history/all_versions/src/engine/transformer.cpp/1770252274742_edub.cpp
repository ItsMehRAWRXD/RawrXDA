...
TransformerLayer::TransformerLayer(int d, int nh, int nkv, int hidden) 
    ...
    // kv_head_dim = dim / n_kv_heads;  // Use local variable instead
...
void TransformerLayer::forward(float* x, int pos, int seq_len) {
    ...
    int kv_head_dim = head_dim;  // For GQA, kv_head_dim can be different
    std::vector<float> q(dim), k(head_dim), v(head_dim);
