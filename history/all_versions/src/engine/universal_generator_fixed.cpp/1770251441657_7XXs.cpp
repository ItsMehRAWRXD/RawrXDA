...
void UniversalGenerator::InitializeTemplates() {
    // Web Application Template
    {
        ...
        tmpl.name = "web-app";
        tmpl.description = "Full-stack web application";
        tmpl.language = LanguageType::JAVASCRIPT;
        tmpl.files = {
        ...
        tmpl.build_commands = {"npm install"};
        tmpl.run_commands = {"npm start"};
        templates["web-app"] = tmpl;
    }
    ...
    // CLI Tool Template
    {
        ProjectTemplate tmpl;
        tmpl.name = "cli-tool";
        tmpl.description = "Command-line interface tool";
        tmpl.language = LanguageType::RUST;
        tmpl.files = {
            {"Cargo.toml", "[package]\nname = \"cli-tool\"\nversion = \"0.1.0\"\nedition = \"2021\"\n\n[dependencies]"},
        ...
        tmpl.build_commands = {"cargo build --release"};
        tmpl.run_commands = {"cargo run --release"};
        templates["cli-tool"] = tmpl;
    }
    ...
    // Library Template
    {
        ProjectTemplate tmpl;
        tmpl.name = "library";
        tmpl.description = "Reusable library package";
        tmpl.language = LanguageType::CPP;
        tmpl.files = {
        ...
        tmpl.build_commands = {"cmake -B build", "cmake --build build"};
        tmpl.run_commands = {"./build/library_test"};
        templates["library"] = tmpl;
    }
}
...
bool UniversalGenerator::GenerateProject(const std::string& name, LanguageType language, 
    ...
    if (config.templates.find("main" + config.file_extension) != config.templates.end()) {
    ...
    if (config.build_system == "Cargo") cmds = {"cargo build"};
    else if (config.build_system == "CMake") cmds = {"cmake -B build", "cmake --build build"};
    else if (config.build_system == "npm") cmds = {"npm install"};
    else if (config.build_system == "Go Modules") cmds = {"go build"};
    else cmds = {"make"};
    ...
    readme << "## Run\n\n";
    readme << "```bash\n";
    readme << (config.build_system == "npm" ? "npm start" : "./" + name) << "\n";
    readme << "```\n";
