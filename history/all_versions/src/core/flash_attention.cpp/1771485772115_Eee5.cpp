...
int32_t FlashAttentionEngine::Forward(FlashAttentionConfig& cfg) {
    if (!m_ready) {
        s_logger.error("[FlashAttention] Forward called but engine not ready. Call Initialize() first.");
        return -2;
    }

    // Validate pointer alignment (ZMM requires 64-byte alignment)
    if (!ValidateAlignment(cfg)) {
        s_logger.error("[FlashAttention] ERROR: Q/K/V/O pointers are not 64-byte aligned. Use _aligned_malloc(size, 64).");
        return -3;
    }
    ...
    if (cfg.headDim % 16 != 0) {
        s_logger.error("[FlashAttention] ERROR: headDim ({}) must be a multiple of 16 for AVX-512.", cfg.headDim);
        return -5;
    }

    // Validate GQA: numHeads must be divisible by numKVHeads
    if (cfg.numHeads % cfg.numKVHeads != 0) {
        s_logger.error("[FlashAttention] ERROR: numHeads ({}) must be divisible by numKVHeads ({}) for GQA.", cfg.numHeads, cfg.numKVHeads);
        return -6;
    }
