            ...
            if (msg) {
                ...
                for (int i = 1; i < argc; i++) {
                    ...
                    if (itemStr) {
                        ...
                        std::strncpy(item.title, itemStr, sizeof(item.title) - 1);
                ...
                if (selectedIndex >= 0 && selectedIndex < static_cast<int>(items.size())) {
                    return JS_NewString(c, items[selectedIndex].title);
                }
            }
            return JS_UNDEFINED;
        ...
        JS_NewCFunction(cx, [](JSContext* c, JSValue this_val, int argc, JSValue* argv) -> JSValue {
            ...
            for (int32_t i = 0; i < len; i++) {
                ...
                if (JS_IsString(elem)) {
                    ...
                    if (str) {
                        std::strncpy(item.label, str, sizeof(item.label) - 1);
                    ...
                    if (label) {
                        std::strncpy(item.label, label, sizeof(item.label) - 1);
            ...
            if (result.success && selectedCount > 0 && selectedIndices[0] >= 0
                ...
                return JS_NewString(c, items[selectedIndices[0]].label);
            }
            return JS_UNDEFINED;
        }, "showQuickPick", 2));
    ...
    JS_SetPropertyStr(cx, window, "showInputBox",
        JS_NewCFunction(cx, [](JSContext* c, JSValue this_val, int argc, JSValue* argv) -> JSValue {
            ...
            if (argc >= 1 && JS_IsObject(argv[0])) {
                ...
                if (prompt) {
                    std::strncpy(options.prompt, prompt, sizeof(options.prompt) - 1);
                ...
                if (ph) {
                    std::strncpy(options.placeHolder, ph, sizeof(options.placeHolder) - 1);
                    JS_FreeCString(c, ph);
                }
                JS_FreeValue(c, phVal);
                ...
                if (val) {
                    std::strncpy(options.value, val, sizeof(options.value) - 1);
        ...
        JS_NewCFunction(cx, [](JSContext* c, JSValue this_val, int argc, JSValue* argv) -> JSValue {
            ...
            JS_SetPropertyStr(c, item, "show",
                JS_NewCFunction(c, [](JSContext* ctx2, JSValue this2, int ac, JSValue* av) -> JSValue {
                    ...
                    if (native && text) {
                        std::strncpy(native->text, text, sizeof(native->text) - 1);
                        native->visible = true;
                    }
