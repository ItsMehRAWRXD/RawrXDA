...
CommandResult handleHotpatchByteApply(const CommandContext& ctx) {
    if (ctx.args && ctx.args[0]) {
        char buf[256];
        snprintf(buf, sizeof(buf), "[Hotpatch] Applying byte-level patch: %s\n", ctx.args);
        ctx.output(buf);
        ctx.output("Byte layer: GGUF binary modification applied.\n");
    } else {
        ctx.output("Usage: !hotpatch_byte_apply <offset> <hex_data>\n");
    }
