...
CommandResult handleAgentConfigureModel(const CommandContext& ctx) {
    if (ctx.args && ctx.args[0]) {
        ...
        ModelInvoker::instance().setLLMBackend(backend, endpoint);
        char buf[256];
        ...
        auto current = ModelInvoker::instance().getLLMBackend();
        char buf[256];
...
CommandResult handleAgentExecuteCmd(const CommandContext& ctx) {
    if (ctx.args && ctx.args[0]) {
        ...
        ModelInvoker::instance().invokeAsync(params);
        // Poll for result (bounded wait)
        int waits = 0;
        while (ModelInvoker::instance().isInvoking() && waits < 300) {
            Sleep(100); waits++;
        }
        if (ModelInvoker::instance().isInvoking()) {
            ModelInvoker::instance().cancelPendingRequest();
...
CommandResult handleAgentStartLoop(const CommandContext& ctx) {
    ...
    ctx.output(buf);
    BoundedAgentLoop loop(config);
    auto result = loop.run("Analyze and improve the current codebase.");
    snprintf(buf, sizeof(buf), "[Agent] Loop completed: %s (steps=%d)\n",
             result.success ? "SUCCESS" : "FAILED", result.stepsExecuted);
    ctx.output(buf);
    if (!result.finalAnswer.empty()) {
        ctx.output(result.finalAnswer.c_str());
        ctx.output("\n");
    }
    return result.success ? CommandResult::ok("agent.start") : CommandResult::error("Agent loop failed", -1);
}
