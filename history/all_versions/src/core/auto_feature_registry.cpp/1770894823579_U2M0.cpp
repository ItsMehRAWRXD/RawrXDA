...
CommandResult handleHotpatchMemoryRevert(const CommandContext& ctx) {
    ...
    snprintf(buf, sizeof(buf), "  Memory patches applied (lifetime): %llu\n",
             (unsigned long long)stats.memoryPatchesApplied.load());
    ctx.output(buf);
...
CommandResult handleHotpatchPresetLoad(const CommandContext& ctx) {
    ...
    bool ok = UnifiedHotpatchManager::instance().load_preset(ctx.args, &preset);
    char buf[256];
    if (ok) {
        snprintf(buf, sizeof(buf), "[Hotpatch] Preset loaded from '%s' (%zu entries)\n",
                 ctx.args, preset.entries.size());
        ctx.output(buf);
...
CommandResult handleHotpatchPresetSave(const CommandContext& ctx) {
    ...
    HotpatchPreset preset{};
    preset.name = "current";
    bool ok = UnifiedHotpatchManager::instance().save_preset(ctx.args, preset);
    char buf[256];
    if (ok) {
...
CommandResult handleHotpatchProxyBias(const CommandContext& ctx) {
    ...
    snprintf(buf, sizeof(buf), "[Hotpatch/Proxy] Token bias set: token=%u bias=%.3f\n",
             tokenId, biasVal);
    ctx.output(buf);
    return CommandResult::ok("proxy.bias");
}
...
CommandResult handleHotpatchProxyValidate(const CommandContext& ctx) {
    ...
    snprintf(buf, sizeof(buf),
             ...
             stats.tokenBiasCount, stats.rewriteRuleCount,
             stats.terminationRuleCount, stats.validatorCount,
             (unsigned long long)stats.totalInvocations);
    ctx.output(buf);
...
CommandResult handleHotpatchServerAdd(const CommandContext& ctx) {
    if (!ctx.args || !ctx.args[0]) {
        ctx.output("Usage: !hotpatch_server_add <patch_name>\n");
        return CommandResult::error("Missing patch name", -1);
    }
    static thread_local ServerHotpatch s_patch{};
    s_patch.name = ctx.args;
    s_patch.transform = [](Request*, Response*) -> bool { return true; };
    s_patch.hit_count = 0;
    auto result = UnifiedHotpatchManager::instance().add_server_patch(&s_patch);
    char buf[256];
    snprintf(buf, sizeof(buf), "[Hotpatch/Server] Added patch '%s': %s\n",
             ctx.args, result.result.success ? "OK" : "FAILED");
    ctx.output(buf);
    return result.result;
}
...
CommandResult handleHotpatchShowProxyStats(const CommandContext& ctx) {
    ...
    snprintf(buf, sizeof(buf),
             ...
             stats.terminationRuleCount, stats.validatorCount,
             (unsigned long long)stats.totalInvocations,
             (unsigned long long)stats.totalRewrites);
    ctx.output(buf);
...
CommandResult handleSwarmAddNode(const CommandContext& ctx) {
    ...
    // Store trace entry as node registration
    auto& dse = DeterministicSwarmEngine::instance();
    dse.recordStep(0, "node_manager", ctx.args, "registered", 0, 1.0f, 0);
    char buf[256];
    snprintf(buf, sizeof(buf), "[Swarm] Node '%s' registered (seed=%llu)\n",
             ctx.args, (unsigned long long)dse.getMasterSeed());
    ctx.output(buf);
...
CommandResult handleSwarmBuildSources(const CommandContext& ctx) {
    ...
    dse.recordStep(0, "coordinator", "sources", "dispatched", 0, 1.0f, 0);
    ctx.output("[Swarm] Source build dispatched (deterministic trace started)\n");
...
CommandResult handleSwarmCacheClear(const CommandContext& ctx) {
    ...
    ctx.output("[Swarm] Trace library and build cache cleared.\n");
...
CommandResult handleSwarmCancelBuild(const CommandContext& ctx) {
    ...
    ctx.output("[Swarm] Build cancelled, trace finalized.\n");
...
CommandResult handleSwarmListNodes(const CommandContext& ctx) {
    auto ids = DeterministicSwarmEngine::instance().listTraceIds();
    ctx.output("[Swarm] Registered node traces:\n");
    if (ids.empty()) {
        ctx.output("  (no traces stored — no nodes registered)\n");
    } else {
        for (size_t i = 0; i < ids.size() && i < 30; ++i) {
            char buf[128];
            snprintf(buf, sizeof(buf), "  node_trace[%zu] id=%llu\n", i, (unsigned long long)ids[i]);
            ctx.output(buf);
        }
    }
...
CommandResult handleSwarmRemoveNode(const CommandContext& ctx) {
    ...
    auto& dse = DeterministicSwarmEngine::instance();
    dse.recordStep(0, "node_manager", ctx.args, "removed", 0, 1.0f, 0);
    char buf[256];
    snprintf(buf, sizeof(buf), "[Swarm] Node '%s' removed\n", ctx.args);
    ctx.output(buf);
...
CommandResult handleSwarmResetStats(const CommandContext& ctx) {
    ...
    ctx.output("[Swarm] All swarm statistics reset.\n");
...
CommandResult handleSwarmStartBuild(const CommandContext& ctx) {
    ...
    dse.recordStep(0, "coordinator", "start_build", "initiated", 0, 1.0f, 0);
    ctx.output("[Swarm] Distributed build started (trace recording active)\n");
...
CommandResult handleSwarmStartHybrid(const CommandContext& ctx) {
    ...
    dse.recordStep(0, "coordinator", "hybrid_start", "local+remote", 0, 1.0f, 0);
    ctx.output("[Swarm] Hybrid mode started (local + remote, trace active)\n");
    return CommandResult::ok("swarm.startHybrid");
}

CommandResult handleSwarmStartLeader(const CommandContext& ctx) {
    auto& dse = DeterministicSwarmEngine::instance();
    dse.setMasterSeed(0x52617772584421ULL); // "RawrXD!" seed
    dse.beginTrace("leader_session", "leader");
    dse.recordStep(0, "leader", "start", "listening", 0, 1.0f, 0);
    char buf[128];
    snprintf(buf, sizeof(buf), "[Swarm] Leader started (seed=%llu)\n",
             (unsigned long long)dse.getMasterSeed());
    ctx.output(buf);
...
CommandResult handleSwarmStartWorker(const CommandContext& ctx) {
    ...
    ctx.output("[Swarm] Worker started, awaiting tasks.\n");
...
CommandResult handleSwarmStop(const CommandContext& ctx) {
    ...
    dse.storeTrace(trace);
    char buf[128];
    snprintf(buf, sizeof(buf), "[Swarm] Stopped. Final trace: %zu steps\n", trace.steps.size());
    ctx.output(buf);
...
CommandResult handleSwarmToggleDiscovery(const CommandContext& ctx) {
    auto& dse = DeterministicSwarmEngine::instance();
    dse.recordStep(0, "discovery", "toggle", "flipped", 0, 1.0f, 0);
    ctx.output("[Swarm] Network discovery toggled (recorded in trace).\n");
...
CommandResult handleSwarmWorkerConnect(const CommandContext& ctx) {
    ...
    auto& dse = DeterministicSwarmEngine::instance();
    dse.recordStep(0, "worker", ctx.args, "connecting", 0, 1.0f, 0);
    char buf[256];
    snprintf(buf, sizeof(buf), "[Swarm] Worker connecting to leader at '%s'\n", ctx.args);
    ctx.output(buf);
...
CommandResult handleVoiceModeContinuous(const CommandContext& ctx) {
    ...
    ctx.output("[Voice] Mode: CONTINUOUS — always listening for voice input.\n");
...
CommandResult handleVoiceModeDisabled(const CommandContext& ctx) {
    ...
    ctx.output("[Voice] Mode: DISABLED — all voice input/output stopped.\n");
...
CommandResult handleVoiceModePtt(const CommandContext& ctx) {
    g_voiceState.mode = 2;
    g_voiceState.enabled = true;
    ctx.output("[Voice] Mode: PUSH-TO-TALK — hold PTT key to transmit.\n");
