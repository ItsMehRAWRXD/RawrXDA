...
CommandResult handleFileSave(const CommandContext& ctx) {
    if (ctx.isGui && ctx.idePtr) {
        ...
        // CLI: write current editor buffer to active file path
        if (!ctx.args || !ctx.args[0]) {
            // No path argument — check CLIState for currentFile
            if (!ctx.cliStatePtr) {
                ctx.output("[File] No CLI state available.\n");
                return CommandResult::error("file.save: no CLI state");
            }
            CLIState* cli = reinterpret_cast<CLIState*>(ctx.cliStatePtr);
            if (cli->currentFile.empty()) {
                ctx.output("[File] No active file path. Use !save_as <filename>.\n");
                return CommandResult::error("file.save: no active path");
            }
            // Save editorBuffer to currentFile
            HANDLE h = CreateFileA(cli->currentFile.c_str(), GENERIC_WRITE, 0, nullptr,
                                   CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
            if (h == INVALID_HANDLE_VALUE) {
                std::string msg = "[File] Cannot write: " + cli->currentFile + " (err " + std::to_string(GetLastError()) + ")\n";
                ctx.output(msg.c_str());
                return CommandResult::error("file.save: write failed");
            }
            DWORD written = 0;
            if (!cli->editorBuffer.empty()) {
                WriteFile(h, cli->editorBuffer.c_str(),
                          static_cast<DWORD>(cli->editorBuffer.size()), &written, nullptr);
            }
            CloseHandle(h);
            std::string msg = "[File] Saved: " + cli->currentFile + " (" + std::to_string(written) + " bytes)\n";
            ctx.output(msg.c_str());
        } else {
            // Path provided as argument — use it as target, pull buffer from CLIState
            if (!ctx.cliStatePtr) {
                ctx.output("[File] No CLI state available for buffer content.\n");
                return CommandResult::error("file.save: no CLI state");
            }
            CLIState* cli = reinterpret_cast<CLIState*>(ctx.cliStatePtr);
            ...
            CloseHandle(h);
            // Update currentFile to reflect the new active path
            cli->currentFile = ctx.args;
            std::string msg = "[File] Saved: " + std::string(ctx.args) + " (" + std::to_string(written) + " bytes)\n";
            ctx.output(msg.c_str());
        }
    }
...
CommandResult handleFileModelFromOllama(const CommandContext& ctx) {
    ...
    try {
        RawrXD::Agent::AgentOllamaClient client("http://127.0.0.1:11434");
...
CommandResult handleServerStart(const CommandContext& ctx) {
    ...
    oss << "[Server] Started: " << launchCmd << " (PID " << g_serverProc.pid << ")\n"
        << "  Active server patches: " << stats.totalApplied << "\n"
        << "  Injection points: PreRequest, PostRequest, PreResponse, PostResponse, StreamChunk\n";
    ctx.output(oss.str().c_str());
    return CommandResult::ok("server.start");
}
