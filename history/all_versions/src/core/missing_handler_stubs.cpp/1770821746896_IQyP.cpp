...
CommandResult handleMultiRespGenerate(const CommandContext& ctx) {
    ...
    auto result = engine.generateAll(sessionId,
        [](const char* text, int index, void* ud) {
            // Per-response callback — responses accumulate in the engine
...
CommandResult handleMultiRespShowStats(const CommandContext& ctx) {
    ...
    report += "  Total responses: " + std::to_string(stats.totalResponses) + "\n";
    report += "  Avg latency: " + std::to_string(static_cast<int>(stats.avgLatencyMs)) + "ms\n";
...
CommandResult handleMultiRespShowTemplates(const CommandContext& ctx) {
    ...
    for (const auto& t : templates) {
        char buf[256];
        snprintf(buf, sizeof(buf), "  [%s] %s  temp=%.2f  enabled=%s\n",
                 t.id.c_str(), t.name.c_str(), t.temperature,
                 t.enabled ? "yes" : "no");
        ctx.output(buf);
    }
...
CommandResult handleMultiRespShowPrefs(const CommandContext& ctx) {
    ...
    for (const auto& p : prefs) {
        char buf[256];
        snprintf(buf, sizeof(buf), "  Session %llu: Response %d — %s\n",
                 static_cast<unsigned long long>(p.sessionId),
                 p.responseIndex, p.reason.c_str());
        ctx.output(buf);
    }
...
CommandResult handleReplayStatus(const CommandContext& ctx) {
    ...
    snprintf(buf, sizeof(buf),
             ...
             static_cast<unsigned long long>(stats.totalActions),
...
CommandResult handleReplayExportSession(const CommandContext& ctx) {
    ...
    if (journal.exportSession(stats.currentSessionId, filename)) {
    ...
    if (result.success) {
        ...
        for (size_t i = 0; i < frames.size(); ++i) {
            ...
            snprintf(buf, sizeof(buf), "  #%zu  0x%016llX  %s+0x%llX  [%s:%d]\n",
                     ...
                     frames[i].functionName.c_str(),
                     static_cast<unsigned long long>(frames[i].offset),
...
CommandResult handleDbgDisasm(const CommandContext& ctx) {
    ...
    auto result = dbg.disassembleAt(addr, 20, lines);
...
CommandResult handleDbgThreads(const CommandContext& ctx) {
    ...
    if (result.success) {
        ...
        for (const auto& t : threads) {
            char buf[256];
            snprintf(buf, sizeof(buf), "  TID=%u  State=%s  RIP=0x%016llX\n",
                     t.threadId, t.state.c_str(),
                     static_cast<unsigned long long>(t.instructionPtr));
            ctx.output(buf);
        }
    } else {
...
CommandResult handleDbgEvaluate(const CommandContext& ctx) {
    ...
    auto& dbg = NativeDebuggerEngine::Instance();
    std::string value;
    auto result = dbg.evaluate(expr, value);

    if (result.success) {
        std::string msg = "[DBG] " + expr + " = " + value + "\n";
        ctx.output(msg.c_str());
    } else {
