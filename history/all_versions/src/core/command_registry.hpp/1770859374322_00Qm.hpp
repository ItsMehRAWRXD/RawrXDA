...
#define COMMAND_TABLE(X) \
    ...
    \
    /* ═══════════════════ MODEL MANAGEMENT (9001-9050) ═══════════════════ */ \
    X(9001,  MODEL_LIST,           "model.list",            "!model_list",       BOTH, "Models",      handleModelList,            CMD_NONE) \
    X(9002,  MODEL_LOAD,           "model.load",            "!model_load",       BOTH, "Models",      handleModelLoad,            CMD_NONE) \
    X(9003,  MODEL_UNLOAD,         "model.unload",          "!model_unload",     BOTH, "Models",      handleModelUnload,          CMD_NONE) \
    X(9004,  MODEL_QUANTIZE,       "model.quantize",        "!model_quantize",   BOTH, "Models",      handleModelQuantize,        CMD_ASYNC) \
    X(9005,  MODEL_FINETUNE,       "model.finetune",        "!model_finetune",   BOTH, "Models",      handleModelFinetune,        CMD_ASYNC) \
    \
    /* ═══════════════════ HOTPATCHING (9100-9150) ═══════════════════ */ \
    X(9100,  HOTPATCH_MEMORY,      "hotpatch.memory",       "!hotpatch_mem",     BOTH, "Hotpatch",    handleHotpatchMemory,       CMD_NONE) \
    X(9101,  HOTPATCH_BYTE,        "hotpatch.byte",         "!hotpatch_byte",    BOTH, "Hotpatch",    handleHotpatchByteLevel,    CMD_NONE) \
    \
    /* ═══════════════════ DISK RECOVERY (9200-9250) ═══════════════════ */ \
    X(9200,  DISK_LIST_DRIVES,     "disk.listDrives",       "!disk_drives",      BOTH, "Disk",        handleDiskListDrives,       CMD_NONE) \
    X(9201,  DISK_SCAN_PARTS,      "disk.scanPartitions",   "!disk_scan",        BOTH, "Disk",        handleDiskScanPartitions,   CMD_ASYNC) \
    \
    /* ═══════════════════ PERFORMANCE GOVERNANCE (9300-9350) ═══════════════════ */ \
    X(9300,  GOVERNOR_STATUS,      "governor.status",       "!governor_status",  BOTH, "Governor",    handleGovernorStatus,       CMD_NONE) \
    X(9301,  GOVERNOR_POWER,       "governor.setPowerLevel","!governor_power",   BOTH, "Governor",    handleGovernorSetPowerLevel,CMD_NONE) \
    \
    /* ═══════════════════ MARKETPLACE & EXTENSIONS (9400-9450) ═══════════════════ */ \
    X(9400,  MARKETPLACE_LIST,     "marketplace.list",      "!marketplace_list", BOTH, "Marketplace", handleMarketplaceList,      CMD_NONE) \
    X(9401,  MARKETPLACE_INSTALL,  "marketplace.install",   "!marketplace_install",BOTH,"Marketplace",handleMarketplaceInstall,   CMD_ASYNC) \
    \
    /* ═══════════════════ EMBEDDINGS & VECTORS (9500-9550) ═══════════════════ */ \
    X(9500,  EMBEDDING_ENCODE,     "embedding.encode",      "!embedding_encode", BOTH, "Embedding",   handleEmbeddingEncode,      CMD_NONE) \
    \
    /* ═══════════════════ VISION & IMAGE ANALYSIS (9600-9650) ═══════════════════ */ \
    X(9600,  VISION_ANALYZE,       "vision.analyze",        "!vision_analyze",   BOTH, "Vision",      handleVisionAnalyzeImage,   CMD_REQUIRES_FILE) \
    \
    /* ═══════════════════ PROMPT ENGINE (9700-9750) ═══════════════════ */ \
    X(9700,  PROMPT_CLASSIFY,      "prompt.classify",       "!prompt_classify",  BOTH, "Prompt",      handlePromptClassifyContext,CMD_NONE) \
    /* ═══════════════════ END OF TABLE ═══════════════════ */
