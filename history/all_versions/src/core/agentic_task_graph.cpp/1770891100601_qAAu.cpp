...
TaskResult AgenticTaskGraph::executeTask(Graph& g, uint64_t taskId) {
    ...
    if (task.executor) {
        ...
        uint64_t timeoutMs = task.timeoutMs > 0 ? task.timeoutMs : 300000;
                        ...
                        if (opLen >= 4) {
                            ...
                            if (opLen >= 4 + pathLen + 8) {
                                ...
                                memcpy(&bp.offset, &task.rollbackData[offset + 4 + pathLen],
                                       sizeof(uint64_t));
                                bp.data = &task.rollbackData[offset + 4 + pathLen + 8];
                                bp.dataSize = opLen - 4 - pathLen - 8;
...
TaskResult AgenticTaskGraph::executor_symbolIndex(TaskNode* self, void* context) {
    ...
    thread_local HNSWIndex hnswIndex(384, 16, 200, DistanceMetric::Cosine);
...
TaskResult AgenticTaskGraph::executor_hotpatch(TaskNode* self, void* context) {
    ...
    if (!self->checkpointData.empty()) {
        ...
        if (patchType == 0) {
            ...
            if (!ur.base.success) {
                self->outputSummary = std::string("Memory patch failed: ") + ur.base.detail;
                return TaskResult::error(ur.base.detail, ur.base.errorCode);
            }

            std::ostringstream ss;
            ss << "Memory patch applied at 0x" << std::hex << addr
               << " (" << std::dec << patchSize << " bytes, seq=" << ur.sequenceNumber << ")";
            self->outputSummary = ss.str();
            ...
            bp.data = &self->checkpointData[headerEnd + 8];
            bp.dataSize = self->checkpointData.size() - headerEnd - 8;

            auto ur = uhm.apply_byte_patch(filename, bp);
            if (!ur.base.success) {
                self->outputSummary = std::string("Byte patch failed: ") + ur.base.detail;
                return TaskResult::error(ur.base.detail, ur.base.errorCode);
            }
            ...
            ss << "Byte patch applied to " << filename
               << " at offset " << bp.offset
               << " (" << bp.dataSize << " bytes, seq=" << ur.sequenceNumber << ")";
            self->outputSummary = ss.str();
            ...
            if (!ur.base.success) {
                self->outputSummary = std::string("Server patch failed: ") + ur.base.detail;
                return TaskResult::error(ur.base.detail, ur.base.errorCode);
            }

            self->outputSummary = std::string("Server patch '") +
                                  (shp->name ? shp->name : "unnamed") +
                                  "' registered (seq=" + std::to_string(ur.sequenceNumber) + ")";
        } else {
...
TaskResult AgenticTaskGraph::executor_swarmCompile(TaskNode* self, void* context) {
    ...
    summary << "Swarm compile complete: " << trace.entries.size()
            ...
            << buildMs << "ms"
            << ", deterministic=" << (replayResult.deterministic ? "yes" : "no")
            << ", trace=" << trace.traceId;
    self->outputSummary = summary.str();
