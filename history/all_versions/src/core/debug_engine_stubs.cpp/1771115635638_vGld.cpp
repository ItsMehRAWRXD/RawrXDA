...
// Helper for Boyer-Moore bad character table (if we want to use it)
// But for simplicity in "production readiness" without external deps, we'll stick to
// a robust optimized scan that handles boundaries correctly and uses larger buffers.
...
uint32_t Dbg_MemoryScan(uint64_t processHandle, uint64_t startAddress,
    ...
    std::unique_ptr<uint8_t[]> chunk(new (std::nothrow) uint8_t[BUFFER_SIZE]);
    if (!chunk) return 2; // OOM
    ...
    while (remaining > 0) {
        ...
        if (!ReadProcessMemory(hProcess, (LPCVOID)(uintptr_t)pos,
                               chunk.get(), toRead, &bytesRead)) {
            // Read failed - might be partial failure or protected page
            // Skip page and continue? Or fail? Usually fail.
            break;
        }
        
        if (bytesRead < patternLen) break;

        // Optimized scan within buffer
        // Using memchr for first byte finding is usually faster than manual loop
        // Boyer-Moore is better for long patterns, but memchr/memcmp is surprisingly fast due to AVX in CRT.
        
        uint8_t firstByte = pat[0];
        uint8_t* ptr = chunk.get();
        size_t searchLen = bytesRead - patternLen + 1;
        
        while (searchLen > 0) {
            void* found = std::memchr(ptr, firstByte, searchLen);
            if (!found) break; // Not found in this chunk
            
            // Candidate found
            size_t offset = (uint8_t*)found - chunk.get();
            if (std::memcmp(found, pat, patternLen) == 0) {
                *outFoundAddress = pos + offset;
                return 0; // Found
            }
            
            // Continue search
            ptr = (uint8_t*)found + 1;
            searchLen = bytesRead - (ptr - chunk.get()) - patternLen + 1;
            // Recalculate searchLen correctly
            if ((size_t)((uint8_t*)found - chunk.get()) >= bytesRead - patternLen) break;
            searchLen = (bytesRead - patternLen + 1) - ((uint8_t*)found - chunk.get()) - 1; // Logic trick, simpler to just use offset
            searchLen = bytesRead - ((uint8_t*)found - chunk.get()) - patternLen + 1; // Wait, this math is getting messy.
            // Let's reset correctly.
            searchLen = (chunk.get() + bytesRead - patternLen + 1) - ((uint8_t*)found + 1);
        }

        // Advance
        // We must backtrack by (patternLen - 1) to handle patterns crossing the buffer boundary
        if (bytesRead < toRead) {
            // Partial read (EOF or error), stop
            break;
        }
