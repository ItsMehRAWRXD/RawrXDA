...
bool computeHMAC_SHA256(const uint8_t* data, size_t dataSize,
    ...
    // Compute inner hash
    uint8_t inner[64 + 32];  // ipad + data + hash space
...
bool encryptLicenseForStorage(const LicenseKeyV2& inKey,
    ...
    // For now, this is a stub - proper encryption would use AES-256-GCM
    // or similar. This is preparation for future hardening.
    if (!outData || !outSize) return false;
    if (*outSize < sizeof(inKey)) return false;

    // Simple XOR with derived key (placeholder)
...
bool decryptLicenseFromStorage(const uint8_t* encData, size_t encSize,
                               const char* password,
                               LicenseKeyV2& outKey) {
    if (!encData || encSize != sizeof(LicenseKeyV2)) return false;
