...
AutotuneResult GPUKernelAutoTuner::initialize() {
    // Enterprise gate: GPUQuant4Bit (0x08) requires Enterprise license
    if (!RawrXD::EnterpriseLicense::isFeatureEnabled(0x08)) {
        return AutotuneResult::error("GPU Kernel Auto-Tuner requires Enterprise license (feature 0x08)");
    }
