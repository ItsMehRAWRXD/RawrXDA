...
void PrintEnterpriseBanner() {
    ...
    if (state == LicenseState::ValidEnterprise || state == LicenseState::ValidOEM) {
        std::cout << "║ 800B Dual-Engine: " << std::left << std::setw(53) 
                  << (lic.Is800BUnlocked() ? "UNLOCKED ✓" : "locked") << " ║\n";
    }
...
std::string GetUpgradePrompt(uint64_t featureMask) {
    ...
    std::ostringstream ss;
     ss << "To unlock " << featName << ":\n"
         << "  1. RawrXD_LicenseCreator.exe --generate-keypair --output <name>\n"
         << "  2. RawrXD_LicenseCreator.exe --create-license --tier enterprise --output license.rawrlic \\\n"
         << "     --private-key <name>_private.blob --public-key <name>_public.blob --hwid "
         << mgr.GetHWIDString() << "\n"
