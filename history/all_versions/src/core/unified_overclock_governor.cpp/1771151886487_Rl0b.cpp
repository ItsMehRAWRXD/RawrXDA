...
// Auto-tune with PID + simulated annealing + adaptive ML strategies
...
ClockResult UnifiedOverclockGovernor::overclock(HardwareDomain domain, int32_t stepMhz) {
    ...
    if (idx >= static_cast<uint32_t>(HardwareDomain::COUNT))
        return ClockResult::error("Invalid domain");
    int32_t newOffset = profiles_[idx].offsetMhz + std::abs(stepMhz);
    newOffset = std::min(newOffset, profiles_[idx].maxFreqMhz - static_cast<int32_t>(baselines_[idx]));
    // Release lock before calling applyOffset which also locks
    mutex_.unlock();
    auto r = applyOffset(domain, newOffset);
    mutex_.lock();
    return r;
}
...
ClockResult UnifiedOverclockGovernor::underclock(HardwareDomain domain, int32_t stepMhz) {
    ...
    if (idx >= static_cast<uint32_t>(HardwareDomain::COUNT))
        return ClockResult::error("Invalid domain");
    int32_t newOffset = profiles_[idx].offsetMhz - std::abs(stepMhz);
    newOffset = std::max(newOffset, profiles_[idx].minFreqMhz - static_cast<int32_t>(baselines_[idx]));
    mutex_.unlock();
    auto r = applyOffset(domain, newOffset);
    mutex_.lock();
    return r;
...
ClockResult UnifiedOverclockGovernor::emergencyThrottleAll() {
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(HardwareDomain::COUNT); ++i) {
        auto domain = static_cast<HardwareDomain>(i);
        // Apply maximum underclock
        applyOffset(domain, profiles_[i].minFreqMhz - static_cast<int32_t>(baselines_[i]));
    }
...
#ifdef _WIN32
    // Use powercfg to set processor performance state
    // Map offset to percentage of max frequency
    float baseline = baselines_[static_cast<uint32_t>(HardwareDomain::CPU)];
    if (baseline <= 0.0f) baseline = 3600.0f; // Default fallback

    int32_t targetFreq = static_cast<int32_t>(baseline) + offsetMhz;
    uint32_t pct = static_cast<uint32_t>(std::clamp(
        (targetFreq * 100.0f) / profiles_[0].maxFreqMhz, 5.0f, 100.0f));

    char cmd[256];
    // Set max processor state
    snprintf(cmd, sizeof(cmd),
             "powercfg /setacvalueindex SCHEME_CURRENT SUB_PROCESSOR PROCTHROTTLEMAX %u",
             pct);
    system(cmd);

    // Set min processor state (for underclock, set min = max to lock)
    if (offsetMhz < 0) {
        snprintf(cmd, sizeof(cmd),
                 "powercfg /setacvalueindex SCHEME_CURRENT SUB_PROCESSOR PROCTHROTTLEMIN %u",
                 pct);
        system(cmd);
    }
...
#endif
    return ClockResult::ok("CPU offset applied");
}
...
ClockResult UnifiedOverclockGovernor::applyGpuOffset(int32_t offsetMhz) {
    ...
    if (offsetMhz >= 0) {
        snprintf(cmd, sizeof(cmd),
                 "nvidia-smi -lgc %d,%d 2>nul",
                 profiles_[1].minFreqMhz,
                 static_cast<int32_t>(baselines_[1]) + offsetMhz);
    } else {
        snprintf(cmd, sizeof(cmd),
                 "nvidia-smi -lgc %d,%d 2>nul",
                 profiles_[1].minFreqMhz,
    ...
    // If nvidia-smi fails, try AMD amd-smi
    if (ret != 0) {
        if (offsetMhz >= 0) {
            snprintf(cmd, sizeof(cmd),
                     "amd-smi set --gpu 0 --clk-freq %d 2>nul",
                     static_cast<int32_t>(baselines_[1]) + offsetMhz);
        } else {
            snprintf(cmd, sizeof(cmd),
                     "amd-smi set --gpu 0 --clk-freq %d 2>nul",
...
#endif
    return ClockResult::ok("GPU offset applied");
}

ClockResult UnifiedOverclockGovernor::applyMemoryOffset(int32_t offsetMhz) {
#ifdef _WIN32
    // Memory OC is limited without BIOS access. Use nvidia-smi for GPU memory
    // or vendor-specific APIs for RAM timing.

    // GPU memory clock via nvidia-smi
    char cmd[256];
    snprintf(cmd, sizeof(cmd),
             "nvidia-smi -lmc %d,%d 2>nul",
             profiles_[2].minFreqMhz,
             static_cast<int32_t>(baselines_[2]) + offsetMhz);
    int ret = system(cmd);

    if (ret != 0) {
        // For system RAM, we can adjust virtual memory settings
        // This is a soft control — real RAM OC requires BIOS/SPD
        // Adjust working set to encourage faster page allocation
        HANDLE hProcess = GetCurrentProcess();
        SIZE_T minWS = 256 * 1024 * 1024;  // 256MB min
        SIZE_T maxWS = 4096ULL * 1024 * 1024; // 4GB max
        if (offsetMhz > 0) {
            // Overclock mode: request larger working set for less page faults
            maxWS = 8192ULL * 1024 * 1024; // 8GB
        } else if (offsetMhz < 0) {
            // Underclock mode: shrink working set to reduce memory pressure
            maxWS = 1024ULL * 1024 * 1024; // 1GB
        }
        SetProcessWorkingSetSize(hProcess, minWS, maxWS);
    }
#endif
    return ClockResult::ok("Memory offset applied");
}

ClockResult UnifiedOverclockGovernor::applyStorageOffset(int32_t offsetMhz) {
    (void)offsetMhz;
#ifdef _WIN32
    // Storage "overclock": adjust power state and queue depth
    // NVMe drives support power states (PS0 = max performance, PS3+ = power saving)
    char cmd[256];
    if (offsetMhz > 0) {
        // Performance mode: disable power saving, enable write cache
        snprintf(cmd, sizeof(cmd),
                 "powercfg /setacvalueindex SCHEME_CURRENT SUB_DISK DISKIDLE 0");
        system(cmd);
        // Disable disk idle timeout
        snprintf(cmd, sizeof(cmd),
                 "powercfg /setacvalueindex SCHEME_CURRENT SUB_DISK 80e3c60e-bb94-4ad8-bbe0-0d3195efc663 0");
        system(cmd);
    } else if (offsetMhz < 0) {
        // Power saving mode: enable aggressive power management
        snprintf(cmd, sizeof(cmd),
                 "powercfg /setacvalueindex SCHEME_CURRENT SUB_DISK DISKIDLE 60");
        system(cmd);
    } else {
        // Reset to default
        snprintf(cmd, sizeof(cmd),
    ...
    system("powercfg /setactive SCHEME_CURRENT");
#endif
    return ClockResult::ok("Storage power state applied");
}
...
float UnifiedOverclockGovernor::readTemperature(HardwareDomain domain) {
    ...
    switch (domain) {
    case HardwareDomain::CPU: {
        // Try WMI via wmic (no deps required)
        pipe = _popen("wmic /namespace:\\\\root\\cimv2 path Win32_PerfFormattedData_Counters_ThermalZoneInformation get Temperature /value 2>nul", "r");
        if (pipe) {
            while (fgets(buf, sizeof(buf), pipe)) {
                if (strstr(buf, "Temperature=")) {
                    float kelvin = static_cast<float>(atof(buf + strlen("Temperature=")));
                    _pclose(pipe);
                    return kelvin - 273.15f; // Kelvin to Celsius
                }
            }
            _pclose(pipe);
        }
    ...
    case HardwareDomain::GPU:
        pipe = _popen("nvidia-smi --query-gpu=temperature.gpu --format=csv,noheader,nounits 2>nul", "r");
        if (pipe) {
            if (fgets(buf, sizeof(buf), pipe)) {
                float temp = static_cast<float>(atof(buf));
                _pclose(pipe);
                return temp;
            }
            _pclose(pipe);
        ...
        return 55.0f;
    case HardwareDomain::Memory:
        // No standard way to read RAM temp without HWiNFO/LibreHardwareMonitor
        return 45.0f;
    case HardwareDomain::Storage:
        // Use smartctl if available
        pipe = _popen("smartctl -A /dev/sda 2>nul | findstr Temperature", "r");
        if (pipe) {
            if (fgets(buf, sizeof(buf), pipe)) {
                ...
                if (p) {
                    _pclose(pipe);
                    return static_cast<float>(atof(p + 2));
                }
            }
            _pclose(pipe);
        }
...
float UnifiedOverclockGovernor::readFrequency(HardwareDomain domain) {
    ...
    switch (domain) {
    case HardwareDomain::CPU: {
        pipe = _popen("wmic cpu get CurrentClockSpeed /value 2>nul", "r");
        if (pipe) {
            while (fgets(buf, sizeof(buf), pipe)) {
                if (strstr(buf, "CurrentClockSpeed=")) {
                    float freq = static_cast<float>(atof(buf + strlen("CurrentClockSpeed=")));
                    _pclose(pipe);
                    return freq;
                }
            }
    ...
    case HardwareDomain::GPU:
        pipe = _popen("nvidia-smi --query-gpu=clocks.gr --format=csv,noheader,nounits 2>nul", "r");
        if (pipe) {
            if (fgets(buf, sizeof(buf), pipe)) {
                float freq = static_cast<float>(atof(buf));
                _pclose(pipe);
                return freq;
            }
            _pclose(pipe);
        ...
        return 1500.0f;
    case HardwareDomain::Memory:
        pipe = _popen("nvidia-smi --query-gpu=clocks.mem --format=csv,noheader,nounits 2>nul", "r");
        if (pipe) {
            if (fgets(buf, sizeof(buf), pipe)) {
                float freq = static_cast<float>(atof(buf));
                _pclose(pipe);
...
float UnifiedOverclockGovernor::readPowerDraw(HardwareDomain domain) {
    ...
    switch (domain) {
    case HardwareDomain::CPU:
        // CPU package power — approximation from frequency and utilization
        return telemetry_[0].utilizationPct * 1.5f; // Rough: 150W at 100%
    case HardwareDomain::GPU:
        pipe = _popen("nvidia-smi --query-gpu=power.draw --format=csv,noheader,nounits 2>nul", "r");
        if (pipe) {
            if (fgets(buf, sizeof(buf), pipe)) {
                float power = static_cast<float>(atof(buf));
                _pclose(pipe);
                return power;
            }
            _pclose(pipe);
        ...
        return 0.0f;
    case HardwareDomain::Memory:
        return 10.0f; // Typical DDR5 per-DIMM
    case HardwareDomain::Storage:
        return 5.0f;  // Typical NVMe active power
    default:
        return 0.0f;
    }
#else
...
float UnifiedOverclockGovernor::readUtilization(HardwareDomain domain) {
    ...
    switch (domain) {
    case HardwareDomain::CPU: {
        pipe = _popen("wmic cpu get LoadPercentage /value 2>nul", "r");
        if (pipe) {
            while (fgets(buf, sizeof(buf), pipe)) {
                if (strstr(buf, "LoadPercentage=")) {
                    float util = static_cast<float>(atof(buf + strlen("LoadPercentage=")));
                    _pclose(pipe);
                    return util;
                }
            }
    ...
    case HardwareDomain::GPU:
        pipe = _popen("nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits 2>nul", "r");
        if (pipe) {
            if (fgets(buf, sizeof(buf), pipe)) {
                float util = static_cast<float>(atof(buf));
                _pclose(pipe);
                return util;
            }
            _pclose(pipe);
        ...
        return 0.0f;
    case HardwareDomain::Memory:
        pipe = _popen("nvidia-smi --query-gpu=utilization.memory --format=csv,noheader,nounits 2>nul", "r");
        if (pipe) {
            if (fgets(buf, sizeof(buf), pipe)) {
                float util = static_cast<float>(atof(buf));
                _pclose(pipe);
