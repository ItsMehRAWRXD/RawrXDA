...
void ModelBruteForceEngine::ProbeWithCPU(ModelProbeResult& result,
    ...
    if (result.architecture.empty() && !meta.architecture.empty())
    ...
    if (result.embedding_dim == 0 && meta.embedding_length > 0)
        result.embedding_dim = meta.embedding_length;
...
void ModelBruteForceEngine::ProbeWithNative(ModelProbeResult& result,
    ...
    pipeConf.maxContextTokens = 512;  // Minimal context for probe
    pipeConf.useGPU = false;          // CPU-only for compatibility check
