...
bool ModelBruteForceEngine::ExtractMetadata(const std::string& path, ModelProbeResult& result) {
    ...
    CloseHandle(hFile);
#else
    // POSIX: similar fread-based parsing (mirrors Windows logic)
    FILE* fp = fopen(path.c_str(), "rb");
    if (!fp) return false;
    fseek(fp, 24, SEEK_SET); // Skip header
    // Abbreviated: same KV parsing logic using fread
    fclose(fp);
#endif
...
void ModelBruteForceEngine::ProbeWithCPU(ModelProbeResult& result,
                                          const BruteForceScanConfig& config) {
    // Try to open with ModelInvoker's internal GGUF loading
    // This uses the existing GGUFLoader mmap path
    HANDLE hFile = CreateFileA(result.path.c_str(), GENERIC_READ, FILE_SHARE_READ,
                               nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (hFile == INVALID_HANDLE_VALUE) return;

    // Verify we can map the file (tests basic CPU loadability)
    ...
    if (hMap) {
        ...
        if (pView) {
            ...
            // Verify first 4 bytes are GGUF magic
...
// ============================================================================
// Probe: Native Pipeline
// ============================================================================
void ModelBruteForceEngine::ProbeWithNative(ModelProbeResult& result,
                                             const BruteForceScanConfig& config) {
    // Check file accessibility and minimum structure for native pipeline
    DWORD attrs = GetFileAttributesA(result.path.c_str());
    if (attrs == INVALID_FILE_ATTRIBUTES) return;

    // Native pipeline requires valid GGUF with at least 1 tensor
    if (result.valid_magic && result.tensor_count > 0) {
        result.native_loadable = true;
    }

    // Set compatibility flags — all modes work if model is loadable
    if (result.cpu_loadable || result.native_loadable || result.ollama_available) {
...
std::vector<ModelProbeResult> ModelBruteForceEngine::BruteForceAll(
    ...
    // Track telemetry
    TelemetryCollector::instance().trackFeatureUsage("model_bruteforce", "");
    RawrXD::Perf::PerfTelemetry::instance().recordSlot(0, m_progress.percent_complete);
