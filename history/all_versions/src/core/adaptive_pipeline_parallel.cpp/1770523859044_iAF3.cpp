...
PipelineResult AdaptivePipelineParallel::profileLocalHardware(NodeHardwareProfile& outProfile) {
    ...
    try {
        ...
        if (gpuBridge.isInitialized()) {
            ...
            // Estimate TFLOPS based on known GPU architectures
            // NVIDIA: vendorId 0x10DE
            // AMD: vendorId 0x1002
            if (caps.vendorId == 0x10DE) { // NVIDIA
                outProfile.hasTensorCores = caps.shaderModelMajor >= 7;
                // Rough estimate: SMs * 128 FP32 cores * 2 (FMA) * clock
                // For RTX 4090: ~82.6 TFLOPS FP32
                outProfile.teraflopsF32 = (float)(caps.dedicatedVRAM / (1024ULL * 1024ULL)) * 0.003f;
                outProfile.teraflopsF16 = outProfile.teraflopsF32 * 2.0f;
            } else if (caps.vendorId == 0x1002) { // AMD
                outProfile.hasTensorCores = false; // AMD Matrix Cores
                outProfile.teraflopsF32 = (float)(caps.dedicatedVRAM / (1024ULL * 1024ULL)) * 0.002f;
                outProfile.teraflopsF16 = outProfile.teraflopsF32 * 2.0f;
            }

            // Memory bandwidth estimate (GB/s)
            // Modern GPUs: ~500-1000 GB/s
            outProfile.memBandwidthGBs = (float)(caps.dedicatedVRAM / (1024ULL * 1024ULL)) * 0.04f;
        } else {
