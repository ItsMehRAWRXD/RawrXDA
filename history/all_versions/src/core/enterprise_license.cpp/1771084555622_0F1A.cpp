...
bool EnterpriseLicense::Initialize() {
    ...
    const auto shieldStart = std::chrono::steady_clock::now();
    int32_t shieldResult = Shield_InitializeDefense();
    const auto shieldEnd = std::chrono::steady_clock::now();
    const int64_t shieldMs = DurationMs(shieldStart, shieldEnd);

    Logger::instance().logInfo("license.shield.duration", {
        {"duration_ms", std::to_string(shieldMs)}
    });
    EmitLicenseSpan(RawrXD::Telemetry::TelemetryLevel::Info,
                    "license.shield", "init", shieldMs);
    ...
    const auto licenseStart = std::chrono::steady_clock::now();
    int64_t result = Enterprise_InitLicenseSystem();
    const auto licenseEnd = std::chrono::steady_clock::now();
    const int64_t licenseMs = DurationMs(licenseStart, licenseEnd);

    Logger::instance().logInfo("license.subsystem.duration", {
        {"duration_ms", std::to_string(licenseMs)}
    });
    EmitLicenseSpan(RawrXD::Telemetry::TelemetryLevel::Info,
                    "license.subsystem", "init", licenseMs);
    ...
    if (result != 0) {
        ...
        Logger::instance().logInfo("license.init.ok", {
            {"duration_ms", std::to_string(licenseMs)}
        });
        EmitLicenseEvent(RawrXD::Telemetry::TelemetryLevel::Info,
                         "license.init", "ok",
    ...
    if (devUnlock != 0) {
        std::cout << "[EnterpriseLicense] Dev unlock active (RAWRXD_ENTERPRISE_DEV) — all features enabled"
                  << std::endl;
        Logger::instance().logInfo("license.dev_unlock", {
            {"active", "true"}
        });
        EmitLicenseEvent(RawrXD::Telemetry::TelemetryLevel::Info,
                         "license.dev_unlock", "active");
    }
    ...
    const int64_t initMs = DurationMs(startTime, initEnd);
    Logger::instance().logInfo("license.init.complete", {
        {"duration_ms", std::to_string(initMs)},
        {"state", LicenseStateName(newState)}
    });
    EmitLicenseSpan(RawrXD::Telemetry::TelemetryLevel::Info,
                    "license.init", "complete", initMs,
                    {{"state", LicenseStateName(newState)}});
...
void EnterpriseLicense::Shutdown() {
    ...
    Enterprise_Shutdown();
    Logger::instance().logInfo("license.shutdown", {
        {"component", "EnterpriseLicense"}
    });
    EmitLicenseEvent(RawrXD::Telemetry::TelemetryLevel::Info,
                     "license.shutdown", "complete");
...
bool EnterpriseLicense::InstallLicense(const void* licenseBlob, size_t blobSize,
    ...
    const int64_t installMs = DurationMs(installStart, installEnd);
    EmitLicenseSpan(RawrXD::Telemetry::TelemetryLevel::Info,
                    "license.install", "call", installMs,
                    {{"size_bytes", std::to_string(blobSize)}});
    
    if (result != 0) {
        std::cerr << "[EnterpriseLicense] License installation failed: 0x"
                  << std::hex << result << std::dec << std::endl;
        Logger::instance().logError("license.install.failed", {
            {"code", std::to_string(result)},
            {"duration_ms", std::to_string(installMs)}
        });
        EmitLicenseEvent(RawrXD::Telemetry::TelemetryLevel::Error,
    ...
    Logger::instance().logInfo("license.install.ok", {
        {"duration_ms", std::to_string(installMs)},
        {"state", LicenseStateName(newState)}
    });
    EmitLicenseEvent(RawrXD::Telemetry::TelemetryLevel::Info,
...
bool EnterpriseLicense::Revalidate() {
    ...
    LicenseState oldState = GetState();
    const auto revalidateStart = std::chrono::steady_clock::now();
    int64_t result = Enterprise_ValidateLicense();
    const auto revalidateEnd = std::chrono::steady_clock::now();
    const int64_t revalidateMs = DurationMs(revalidateStart, revalidateEnd);
    EmitLicenseSpan(RawrXD::Telemetry::TelemetryLevel::Info,
                    "license.revalidate", "call", revalidateMs);
    LicenseState newState = GetState();
    ...
    if (result != 0) {
        Logger::instance().logWarning("license.revalidate.failed", {
            {"code", std::to_string(result)},
            {"duration_ms", std::to_string(revalidateMs)},
            {"state", LicenseStateName(newState)}
        });
        EmitLicenseEvent(RawrXD::Telemetry::TelemetryLevel::Warning,
                         "license.revalidate", "failed",
                         {{"code", std::to_string(result)},
                          {"duration_ms", std::to_string(revalidateMs)},
                          {"state", LicenseStateName(newState)}});
    } else {
        Logger::instance().logInfo("license.revalidate.ok", {
            {"duration_ms", std::to_string(revalidateMs)},
            {"state", LicenseStateName(newState)}
        });
        EmitLicenseEvent(RawrXD::Telemetry::TelemetryLevel::Info,
                         "license.revalidate", "ok",
                         {{"duration_ms", std::to_string(revalidateMs)},
                          {"state", LicenseStateName(newState)}});
    }
...
void EnterpriseLicense::notifyStateChange(LicenseState oldState, LicenseState newState) {
    ...
    Logger::instance().logInfo("license.state.change", {
        {"old_state", LicenseStateName(oldState)},
        {"new_state", LicenseStateName(newState)}
    });
    EmitLicenseEvent(RawrXD::Telemetry::TelemetryLevel::Info,
