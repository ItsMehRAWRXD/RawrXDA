...
AgenticObservability& LicenseObs() {
    static AgenticObservability obs;
    return obs;
}

const char* LicenseStateToString(LicenseState state) {
    switch (state) {
        case LicenseState::ValidEnterprise:  return "ValidEnterprise";
        case LicenseState::ValidPro:         return "ValidPro";
        case LicenseState::ValidTrial:       return "ValidTrial";
...
void LogLicenseEvent(
    AgenticObservability::LogLevel level,
    const char* message,
    const nlohmann::json& context)
{
    LicenseObs().log(level, "EnterpriseLicense", message, context);
}

std::atomic<uint64_t> g_licenseInitCount{0};
std::atomic<uint64_t> g_licenseInitFailCount{0};
std::atomic<uint64_t> g_licenseInstallCount{0};
std::atomic<uint64_t> g_licenseInstallFailCount{0};
std::atomic<uint64_t> g_licenseRevalidateCount{0};
std::atomic<uint64_t> g_licenseRevalidateFailCount{0};
std::atomic<uint64_t> g_licenseStateChangeCount{0};
