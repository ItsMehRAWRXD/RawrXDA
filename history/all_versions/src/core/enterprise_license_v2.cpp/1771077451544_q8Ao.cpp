...
const FeatureDefV2 g_FeatureManifest[TOTAL_FEATURES] = {
    ...
    // ── Community Tier (0–5) ─────────────────────────────────────
    { FeatureID::BasicGGUFLoading,      "Basic GGUF Loading",
      "Local GGUF file loading and parsing",
      LicenseTierV2::Community, true, true, false,
      "src/core/gguf_loader.cpp", "Phase 1" },

    { FeatureID::Q4Quantization,        "Q4_0/Q4_1 Quantization",
      "4-bit quantization for memory-efficient inference",
      LicenseTierV2::Community, true, true, false,
      "src/core/quantization.cpp", "Phase 1" },

    { FeatureID::CPUInference,          "CPU Inference",
      "Single-threaded and multi-threaded CPU inference",
      LicenseTierV2::Community, true, true, false,
      "src/core/inference_engine.cpp", "Phase 1" },

    { FeatureID::BasicChatUI,           "Basic Chat UI",
      "Win32 chat window with input/output panels",
      LicenseTierV2::Community, true, true, false,
      "src/win32app/Win32IDE.cpp", "Phase 1" },

    { FeatureID::ConfigFileSupport,     "Config File Support",
      "JSON configuration file loading and saving",
      LicenseTierV2::Community, true, true, false,
      "src/core/settings.cpp", "Phase 1" },

    { FeatureID::SingleModelSession,    "Single Model Session",
      "Load and interact with a single model at a time",
      LicenseTierV2::Community, true, true, false,
      "src/core/model_session.cpp", "Phase 1" },

    // ── Professional Tier (6–26) ─────────────────────────────────
    { FeatureID::MemoryHotpatching,     "Memory Hotpatching",
      "Direct RAM patching via VirtualProtect/mprotect",
      LicenseTierV2::Professional, true, true, false,
      "src/core/model_memory_hotpatch.cpp", "Phase 6" },
    ...
    { FeatureID::ServerHotpatching,     "Server Hotpatching",
      "Runtime request/response modification at injection points",
      LicenseTierV2::Professional, true, true, false,
      "src/server/gguf_server_hotpatch.cpp", "Phase 6" },

    { FeatureID::UnifiedHotpatchManager,"Unified Hotpatch Manager",
      "Coordinated 3-layer hotpatch routing and statistics",
      LicenseTierV2::Professional, true, true, false,
      "src/core/unified_hotpatch_manager.cpp", "Phase 6" },

    { FeatureID::Q5Q8F16Quantization,   "Q5/Q8/F16 Quantization",
      "Higher-precision quantization formats (Q5_0, Q5_1, Q8_0, F16)",
      LicenseTierV2::Professional, true, true, false,
      "src/core/quantization.cpp", "Phase 7" },

    { FeatureID::MultiModelLoading,     "Multi-Model Loading",
      "Load and switch between multiple GGUF models concurrently",
      LicenseTierV2::Professional, true, true, false,
      "src/core/model_manager.cpp", "Phase 8" },

    { FeatureID::CUDABackend,           "CUDA Backend",
      "NVIDIA GPU acceleration via CUDA compute kernels",
      LicenseTierV2::Professional, false, false, false,
      "src/gpu/cuda_backend.cpp", "Phase 12" },

    { FeatureID::AdvancedSettingsPanel, "Advanced Settings Panel",
      "Extended parameter controls (temperature, top_p, repeat_penalty, etc.)",
      LicenseTierV2::Professional, true, true, false,
      "src/win32app/Win32IDE_Settings.cpp", "Phase 3" },

    { FeatureID::PromptTemplates,       "Prompt Templates",
      "Pre-built and custom prompt templates with variable substitution",
      LicenseTierV2::Professional, true, true, false,
      "src/core/prompt_templates.cpp", "Phase 9" },

    { FeatureID::TokenStreaming,         "Token Streaming",
      "Real-time token-by-token output streaming",
      LicenseTierV2::Professional, true, true, false,
      "src/core/streaming_orchestrator.cpp", "Phase 4" },

    { FeatureID::InferenceStatistics,   "Inference Statistics",
      "Tokens/sec, time-to-first-token, memory usage tracking",
      LicenseTierV2::Professional, true, true, false,
      "src/core/inference_stats.cpp", "Phase 5" },

    { FeatureID::KVCacheManagement,     "KV Cache Management",
      "Key-value cache pruning, rotation, and memory optimization",
      LicenseTierV2::Professional, true, true, false,
      "src/core/kv_cache.cpp", "Phase 10" },

    { FeatureID::ModelComparison,       "Model Comparison",
      "Side-by-side inference output comparison between models",
      LicenseTierV2::Professional, true, false, false,
      "src/core/model_comparison.cpp", "Phase 15" },

    { FeatureID::BatchProcessing,       "Batch Processing",
      "Process multiple prompts in a single batch execution",
      LicenseTierV2::Professional, true, false, false,
      "src/core/batch_processor.cpp", "Phase 15" },

    { FeatureID::CustomStopSequences,   "Custom Stop Sequences",
      "User-defined stop strings for generation termination",
      LicenseTierV2::Professional, true, false, false,
      "src/core/inference_engine.cpp", "Phase 11" },

    { FeatureID::GrammarConstrainedGen, "Grammar-Constrained Generation",
      "BNF/GBNF grammar enforcement during token sampling",
      LicenseTierV2::Professional, true, false, false,
      "src/core/grammar_sampler.cpp", "Phase 14" },

    { FeatureID::LoRAAdapterSupport,    "LoRA Adapter Support",
      "Load and apply LoRA adapters for model fine-tuning",
      LicenseTierV2::Professional, true, false, false,
      "src/core/lora_adapter.cpp", "Phase 16" },

    { FeatureID::ResponseCaching,       "Response Caching",
      "Cache inference results for repeated prompts",
      LicenseTierV2::Professional, true, false, false,
      "src/core/response_cache.cpp", "Phase 13" },

    { FeatureID::PromptLibrary,         "Prompt Library",
      "Organized prompt collection with tags and categories",
      LicenseTierV2::Professional, true, false, false,
      "src/core/prompt_library.cpp", "Phase 13" },

    { FeatureID::ExportImportSessions,  "Export/Import Sessions",
      "Save and restore complete chat sessions to JSON",
      LicenseTierV2::Professional, true, false, false,
      "src/core/session_export.cpp", "Phase 13" },

    { FeatureID::HIPBackend,            "HIP Backend",
      "AMD GPU acceleration via HIP compute kernels",
      LicenseTierV2::Professional, false, false, false,
      "src/gpu/hip_backend.cpp", "Phase 12" },

    // ── Enterprise Tier (27–52) ──────────────────────────────────
    { FeatureID::DualEngine800B,        "800B Dual-Engine",
      "Multi-shard inference orchestrator for 800B+ parameter models",
      LicenseTierV2::Enterprise, true, true, false,
      "src/dual_engine_inference.cpp", "Phase 21" },

    { FeatureID::AgenticFailureDetect,  "Agentic Failure Detection",
      "Automated detection of refusal, hallucination, timeout, safety violations",
      LicenseTierV2::Enterprise, true, true, false,
      "src/agent/agentic_failure_detector.cpp", "Phase 18" },

    { FeatureID::AgenticPuppeteer,      "Agentic Puppeteer",
      "Auto-correction engine for failed agentic responses",
      LicenseTierV2::Enterprise, true, true, false,
      "src/agent/agentic_puppeteer.cpp", "Phase 18" },

    { FeatureID::AgenticSelfCorrection, "Agentic Self-Correction",
      "Iterative self-correction loop with confidence scoring",
      LicenseTierV2::Enterprise, true, true, false,
      "src/agent/agentic_self_corrector.cpp", "Phase 19" },

    { FeatureID::ProxyHotpatching,      "Proxy Hotpatching",
      "Byte-level output rewriting and token bias injection",
      LicenseTierV2::Enterprise, true, true, false,
      "src/core/proxy_hotpatcher.cpp", "Phase 20" },

    { FeatureID::ServerSidePatching,    "Server-Side Patching",
      "Request/response interception at PreRequest/PostResponse/StreamChunk points",
      LicenseTierV2::Enterprise, true, true, false,
      "src/server/gguf_server_hotpatch.cpp", "Phase 20" },

    { FeatureID::SchematicStudioIDE,    "SchematicStudio IDE",
      "Visual schematic editor for model architecture visualization",
      LicenseTierV2::Enterprise, true, true, false,
      "src/win32app/Win32IDE_SchematicStudio.cpp", "Phase 22" },

    { FeatureID::WiringOracleDebug,     "WiringOracle Debug",
      "Connection graph debug and validation tool",
      LicenseTierV2::Enterprise, true, true, false,
      "src/win32app/Win32IDE_WiringOracle.cpp", "Phase 22" },

    { FeatureID::FlashAttention,        "Flash Attention",
      "Memory-efficient attention kernel (O(N) memory, tiled computation)",
      LicenseTierV2::Enterprise, false, false, false,
      "src/core/flash_attention.cpp", "Phase 23" },

    { FeatureID::SpeculativeDecoding,   "Speculative Decoding",
      "Draft-model assisted decoding for faster generation",
      LicenseTierV2::Enterprise, false, false, false,
      "src/core/speculative_decoding.cpp", "Phase 23" },

    { FeatureID::ModelSharding,         "Model Sharding",
      "Split large models across multiple devices or memory regions",
      LicenseTierV2::Enterprise, true, false, false,
      "src/core/model_sharding.cpp", "Phase 21" },

    { FeatureID::TensorParallel,        "Tensor Parallelism",
      "Distribute tensor operations across multiple compute units",
      LicenseTierV2::Enterprise, true, false, false,
      "src/core/tensor_parallel.cpp", "Phase 21" },

    { FeatureID::PipelineParallel,      "Pipeline Parallelism",
      "Stage-based model execution across multiple devices",
      LicenseTierV2::Enterprise, true, false, false,
      "src/core/pipeline_parallel.cpp", "Phase 21" },

    { FeatureID::ContinuousBatching,    "Continuous Batching",
      "Dynamic request batching with continuous scheduling",
      LicenseTierV2::Enterprise, false, false, false,
      "src/core/continuous_batching.cpp", "Phase 24" },

    { FeatureID::GPTQQuantization,      "GPTQ Quantization",
      "Post-training quantization using GPTQ algorithm",
      LicenseTierV2::Enterprise, false, false, false,
      "src/core/gptq_quant.cpp", "Phase 25" },

    { FeatureID::AWQQuantization,       "AWQ Quantization",
      "Activation-aware weight quantization for LLMs",
      LicenseTierV2::Enterprise, false, false, false,
      "src/core/awq_quant.cpp", "Phase 25" },

    { FeatureID::CustomQuantSchemes,    "Custom Quant Schemes",
      "User-defined quantization formats and configuration",
      LicenseTierV2::Enterprise, true, false, false,
      "src/core/custom_quant.cpp", "Phase 25" },

    { FeatureID::MultiGPULoadBalance,   "Multi-GPU Load Balancing",
      "Dynamic work distribution across multiple GPUs",
      LicenseTierV2::Enterprise, true, false, false,
      "src/gpu/multi_gpu_scheduler.cpp", "Phase 24" },

    { FeatureID::DynamicBatchSizing,    "Dynamic Batch Sizing",
      "Automatic batch size adjustment based on available resources",
      LicenseTierV2::Enterprise, true, false, false,
      "src/core/dynamic_batch.cpp", "Phase 24" },

    { FeatureID::PriorityQueuing,       "Priority Queuing",
      "Request priority queue with SLA-based scheduling",
      LicenseTierV2::Enterprise, false, false, false,
      "src/server/priority_queue.cpp", "Phase 26" },

    { FeatureID::RateLimitingEngine,    "Rate Limiting Engine",
      "Token bucket rate limiter with per-client quotas",
      LicenseTierV2::Enterprise, false, false, false,
      "src/server/rate_limiter.cpp", "Phase 26" },

    { FeatureID::AuditLogging,          "Audit Logging",
      "Comprehensive compliance audit trail with structured logging",
      LicenseTierV2::Enterprise, true, true, false,
      "src/core/enterprise_telemetry_compliance.cpp", "Phase 22" },

    { FeatureID::APIKeyManagement,      "API Key Management",
      "Secure API key generation, rotation, and revocation",
      LicenseTierV2::Enterprise, true, false, false,
      "src/server/api_key_manager.cpp", "Phase 26" },

    { FeatureID::ModelSigningVerify,    "Model Signing/Verification",
      "Cryptographic model integrity verification and signing",
      LicenseTierV2::Enterprise, true, true, false,
      "src/core/model_signing.cpp", "Phase 22" },

    { FeatureID::RBAC,                  "Role-Based Access Control",
      "Fine-grained role-based access control for features and models",
      LicenseTierV2::Enterprise, false, false, false,
      "src/core/rbac.cpp", "Phase 27" },
    ...
    { FeatureID::AirGappedDeploy,       "Air-Gapped Deployment",
      "Fully offline deployment with no external network dependencies",
      LicenseTierV2::Sovereign, false, false, false,
      "src/win32app/Win32IDE_AirgappedEnterprise.cpp", "Phase 28" },

    { FeatureID::HSMIntegration,        "HSM Integration",
      "Hardware Security Module integration for key management",
      LicenseTierV2::Sovereign, false, false, false,
      "src/security/hsm_integration.cpp", "Phase 28" },

    { FeatureID::FIPS140_2Compliance,   "FIPS 140-2 Compliance",
      "Federal Information Processing Standards compliant cryptography",
      LicenseTierV2::Sovereign, false, false, false,
      "src/security/fips_compliance.cpp", "Phase 28" },

    { FeatureID::CustomSecurityPolicies,"Custom Security Policies",
      "Organization-specific security policy engine and enforcement",
      LicenseTierV2::Sovereign, false, false, false,
      "src/security/custom_policies.cpp", "Phase 28" },

    { FeatureID::SovereignKeyMgmt,      "Sovereign Key Management",
      "Government-grade key management with escrow and recovery",
      LicenseTierV2::Sovereign, false, false, false,
      "src/security/sovereign_key_mgmt.cpp", "Phase 28" },

    { FeatureID::ClassifiedNetwork,     "Classified Network Support",
      "Deployment on classified/SIPR/JWICS networks",
      LicenseTierV2::Sovereign, false, false, false,
      "src/security/classified_network.cpp", "Phase 28" },

    { FeatureID::TamperDetection,       "Tamper Detection",
      "Runtime tampering detection with integrity verification chain",
      LicenseTierV2::Sovereign, false, false, false,
      "src/security/tamper_detection.cpp", "Phase 28" },

    { FeatureID::SecureBootChain,       "Secure Boot Chain",
      "Cryptographic boot chain verification from UEFI to application",
      LicenseTierV2::Sovereign, false, false, false,
      "src/security/secure_boot.cpp", "Phase 28" },
};
...
LicenseResult EnterpriseLicenseV2::initialize() {
    ...
    if (m_initialized) return LicenseResult::ok("Already initialized");
    ...
    fprintf(stderr, "[LicenseV2] License V2 initialized — Tier: %s, Features: %u/%u\n",
            tierName(m_tier), enabledFeatureCount(), TOTAL_FEATURES);
...
bool EnterpriseLicenseV2::gate(FeatureID id, const char* caller) {
    ...
    if (!enabled) {
        ...
        char detail[256];
        snprintf(detail, sizeof(detail), "Feature '%s' requires %s tier (current: %s)",
                 featName, tierName(g_FeatureManifest[bit].minTier), tierName(m_tier));
...
LicenseResult EnterpriseLicenseV2::loadKeyFromMemory(const void* data, size_t size) {
    ...
    fprintf(stderr, "[LicenseV2] Key loaded — Tier: %s, Features: %u/%u\n",
            tierName(m_tier), m_enabledFeatures.popcount(), TOTAL_FEATURES);
...
LicenseResult EnterpriseLicenseV2::createKey(LicenseTierV2 tier, uint32_t durationDays,
    ...
    if (!outKey) return LicenseResult::error("Null output key", -1);
    if (!signingSecret) return LicenseResult::error("Null signing secret", -2);
...
const FeatureDefV2& EnterpriseLicenseV2::getFeatureDef(FeatureID id) const {
    ...
    if (idx >= TOTAL_FEATURES) {
        static const FeatureDefV2 invalid = {
            FeatureID::COUNT, "Invalid", "Invalid feature",
            LicenseTierV2::Sovereign, false, false, false, "", ""
        };
        return invalid;
    }
...
void EnterpriseLicenseV2::signKey(LicenseKeyV2& key, const char* secret) const {
    ...
    for (size_t i = 0; i < bodyLen; ++i) {
        h1 ^= body[i]; h1 *= 0x01000193ULL;
        h2 ^= body[i]; h2 *= 0x00000100000001B3ULL;
    }

    // Mix in secret
    if (secret) {
        for (size_t i = 0; secret[i]; ++i) {
            h1 ^= static_cast<uint8_t>(secret[i]); h1 *= 0x01000193ULL;
            h2 ^= static_cast<uint8_t>(secret[i]); h2 *= 0x00000100000001B3ULL;
        }
    }
