...
RouterResult AcceleratorRouter::dispatchToARM64NPU(const RouterInferenceTask& task) {
    ...
    if (!arm64.shouldUseNPU()) {
        return RouterResult::error("ARM64 NPU not available or disabled", -41);
    }
    ...
    // Dispatch NPU inference
    ARM64AccelResult npuResult = arm64.dispatchNPUInference(nullptr, nullptr,
                                                              static_cast<uint32_t>(task.inputSizeBytes),
                                                              task.quantType);
...
RouterResult AcceleratorRouter::dispatchToCerebras(const RouterInferenceTask& task) {
    ...
    case DispatchScope::ModelSurgery:     cereScope = CerebrasAccelScope::ModelSurgery; break;
    case DispatchScope::SwarmCompute:     cereScope = CerebrasAccelScope::SwarmCompute; break;
    case DispatchScope::KVCache:          cereScope = CerebrasAccelScope::KVCache; break;
    ...
    r.throughputGFLOPS = inferResult.throughputGFLOPS;
...
RouterResult AcceleratorRouter::dispatchToCPU(const RouterInferenceTask& task) {
    ...
    //   - AVX-512 kernels on x86-64 (SIMD matmul, quantized ops)
    //   - NEON/SVE2 kernels on ARM64 (dot product, FP16 accumulate)
    //   - Pure C fallback on unknown architectures

    m_stats.totalCPUFallbacks.fetch_add(1, std::memory_order_relaxed);

    auto start = std::chrono::steady_clock::now();

    // CPU dispatch — the data is already in host memory
    // The actual computation depends on the kernel name
    // For now this is a placeholder that represents the CPU backend path
    // In production:
    //   matmul → calls into MASM FlashAttention_AVX512.asm or inference_core.asm
    //   quant → calls into requantize kernels
    //   generic → falls through to ggml CPU backend

    if (task.kernelName) {
        std::cout << "[Router-CPU] Executing '" << task.kernelName
                  << "' on CPU (" << task.inputSizeBytes << " bytes)\n";
    }

    // Simulate CPU execution (in production: actual MASM/SIMD dispatch)
    if (task.inputData && task.outputData && task.outputSizeBytes > 0) {
        // Zero-init output as placeholder
        memset(task.outputData, 0, std::min(task.outputSizeBytes, static_cast<uint64_t>(4096)));
    }
...
void AcceleratorRouter::probeARM64() {
    ...
    if (arm64.isInitialized()) {
        ...
        std::cout << "[Router]   ARM64 GPU detected: " << arm64.getGPUName()
                  << " (" << (arm64.getVRAMBytes() / (1024*1024)) << " MB VRAM)\n";
