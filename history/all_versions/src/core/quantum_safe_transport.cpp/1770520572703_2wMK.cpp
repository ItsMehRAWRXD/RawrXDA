...
namespace KyberRef {
    ...
    static const int16_t ZETAS[128] = {
        ...
        3221, 3## 116, 830, 107, 1908, 3082, 2378
    };
...
TransportSecurityResult QuantumSafeTransport::generateX25519KeyPair(X25519KeyPair& outKP) {
    ...
    // Derive public key via scalar multiplication on Curve25519 base point
    // For production: use BCryptGenerateKeyPair(BCRYPT_ECDH_ALGORITHM, ...)
    // Reference: public key = secret * G (base point)
    //
    // Software reference — uses the X25519 scalar multiplication algorithm.
    // For now, generate a random public key stand-in and use BCrypt for actual ECDH.
    auto rr2 = secureRandom(outKP.publicKey.data(), 32);
    ...
    PQAlgorithm algo)
{
    ...
    // Generate ciphertext as encrypted message (simplified reference)
    // Full Kyber encaps uses polynomial encryption with the public matrix
    // For now: ciphertext = BLAKE2b(pk || m) padded to ctBytes
    std::array<uint8_t, 32> ctHash;
    std::vector<uint8_t> ctSeed(pk.size() + 32);
    memcpy(ctSeed.data(), pk.data(), pk.size());
    memcpy(ctSeed.data() + pk.size(), m, 32);
    blake2b256(ctSeed.data(), (uint32_t)ctSeed.size(), ctHash);

    // Fill ciphertext with deterministic pseudo-random expansion from ctHash
    for (uint32_t i = 0; i < ctBytes; i += 32) {
        uint32_t chunkLen = std::min<uint32_t>(32, ctBytes - i);
        // Derive each chunk: BLAKE2b(ctHash || counter)
        uint8_t counterBuf[36];
        memcpy(counterBuf, ctHash.data(), 32);
        uint32_t counter = i / 32;
        memcpy(counterBuf + 32, &counter, 4);
        std::array<uint8_t, 32> chunk;
        blake2b256(counterBuf, 36, chunk);
        memcpy(ciphertext.data() + i, chunk.data(), chunkLen);
    }
