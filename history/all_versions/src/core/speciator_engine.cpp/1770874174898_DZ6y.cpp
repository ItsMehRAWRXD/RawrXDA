...
uint64_t SpeciatorEngine::evaluateFitness(uint32_t genomeIndex, void* testFunction, uint64_t iterations) {
    ...
    return fitness;
#else
    (void)testFunction; (void)iterations;
    m_population[genomeIndex].fitness = 1000; // Stub
    return 1000;
#endif
}
...
#else
        (void)testFunction; (void)iterations;
        m_population[i].fitness = 1000 + i;
#endif
    }
    char msg[128];
    snprintf(msg, sizeof(msg), "Evaluated %zu genomes", m_population.size());
