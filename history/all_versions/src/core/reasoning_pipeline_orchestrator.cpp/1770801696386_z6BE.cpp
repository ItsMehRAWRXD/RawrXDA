...
PipelineResult ReasoningPipelineOrchestrator::execute(
    ...
    if (m_cancelled.load()) {
        m_running.store(false);
        return PipelineResult::error("Pipeline cancelled.");
    }
...
PipelineResult ReasoningPipelineOrchestrator::runDirectLLM(
    ...
    if (!cb) {
        return PipelineResult::error("No inference callback configured.");
    }
    ...
    try {
        ...
        return PipelineResult::error("Inference callback threw an exception.");
    }
...
PipelineResult ReasoningPipelineOrchestrator::runChainedPipeline(
    ...
    for (int i = 0; i < (int)chain.size(); ++i) {
        if (m_cancelled.load()) {
            result.error = "Pipeline cancelled at step " + std::to_string(i);
            break;
        }
        ...
        try {
            ...
            stepResult.error = "Inference failed for role: " + roleName;
        }
...
PipelineResult ReasoningPipelineOrchestrator::runSwarmPipeline(
    ...
    for (int i = 0; i < agentCount; ++i) {
        threads.emplace_back([&, i]() {
            ...
            try {
                ...
                agentResults[i].error = "Agent " + std::to_string(i) + " failed.";
            }
    ...
    if (successResults.empty()) {
        return PipelineResult::error("All swarm agents failed.");
    }
