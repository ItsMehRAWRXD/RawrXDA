...
// ============================================================================
EnterpriseLicenseV2::EnterpriseLicenseV2()
    : m_initialized(false)
...
LicenseResult EnterpriseLicenseV2::initialize() {
    if (m_initialized) return LicenseResult::AlreadyInitialized;
...
const TierLimits::Limits& EnterpriseLicenseV2::currentLimits() const {
    return TierLimits::getTierLimits(m_tier);
}
...
LicenseResult EnterpriseLicenseV2::validateKey(const LicenseKeyV2& key) const {
    // Basic validation
    if (key.magic != 0x5258444C) return LicenseResult::InvalidMagic;
    if (key.version != 2) return LicenseResult::InvalidVersion;
    if (key.tier > static_cast<uint32_t>(LicenseTierV2::Sovereign)) return LicenseResult::InvalidTier;
    
    // Time-based validation
    uint32_t now = static_cast<uint32_t>(std::time(nullptr));
    if (key.issueDate > now) return LicenseResult::InvalidKeyFormat;  // Future issue date
    if (key.expiryDate < now) return LicenseResult::Expired;
...
// HWID Hex
// ============================================================================
void EnterpriseLicenseV2::getHardware IDHex(char* buf, size_t bufLen) const {
    if (buf && bufLen >= 17) {
