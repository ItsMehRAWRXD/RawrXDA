...
public:
    ...
    static SyncConfig getDefaultConfig() {
        ...
        // Set standard paths
        std::strcpy_s(config.cacheLocation, sizeof(config.cacheLocation),
                      "C:\\ProgramData\\RawrXD\\license.cache");
        std::strcpy_s(config.auditLocation, sizeof(config.auditLocation),
        ...
        while (fgets(line, sizeof(line), file)) {
            ...
            *equals = '\0';
            ...
            if (strcmp(key, "grace_period_days") == 0) {
                ...
                std::strncpy_s(config.cacheLocation, sizeof(config.cacheLocation),
                               value, 255);
            } else if (strcmp(key, "audit_location") == 0) {
                std::strncpy_s(config.auditLocation, sizeof(config.auditLocation),
                               value, 255);
            }
        }
    ...
    static const char* getConfigPath() {
        ...
        if (path[0] == '\0') {
            std::strcpy_s(path, sizeof(path), "C:\\ProgramData\\RawrXD\\license.conf");
        }
        return path;
    }
