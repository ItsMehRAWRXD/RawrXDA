...
static CommandResult setAIContextWindow(const CommandContext& ctx, uint32_t cmdId,
    ...
    static std::mutex s_ctxMtx;
    std::lock_guard<std::mutex> lock(s_ctxMtx);
    // Write context window setting
    ...
    char buf[128];
    snprintf(buf, sizeof(buf), "[AI] Context window set to %uK tokens (%s)\n",
             tokenCount / 1024, label);
    ctx.output(buf);
    // Notify orchestrator of context change
    auto& orch = AutoRepairOrchestrator::instance();
    orch.logEvent("ai.contextChange", label);
    return CommandResult::ok(label);
}
...
CommandResult handleAINoRefusal(const CommandContext& ctx) {
    ...
    // Toggle no-refusal mode in config
    ...
    ctx.output(buf);
    auto& orch = AutoRepairOrchestrator::instance();
    orch.logEvent("ai.noRefusal", noRefusal ? "enabled" : "disabled");
    return CommandResult::ok("ai.noRefusal");
}
...
CommandResult handleSwarmRemoveNode(const CommandContext& ctx) {
    ...
    if (!ctx.args || !ctx.args[0]) {
        ctx.output("Usage: !swarm_remove <node_id_or_addr>\n");
        return CommandResult::error("swarm.removeNode: missing node");
    }
    ctx.output("[SWARM] Removing node: ");
    ctx.output(ctx.args);
    ctx.output("\n");
    ctx.output("[SWARM] Node removed from swarm.\n");
    return CommandResult::ok("swarm.removeNode");
}
...
CommandResult handleSwarmEvents(const CommandContext& ctx) {
    ...
    snprintf(buf, sizeof(buf), "  Repairs: %u  Anomalies: %u\n",
             orchStats.totalRepairs, orchStats.totalAnomalies);
    ctx.output(buf);
...
CommandResult handleSwarmStats(const CommandContext& ctx) {
    ...
    snprintf(buf, sizeof(buf),
        ...
        mem.ullAvailPhys / (1024*1024), stats.totalApplied, GetTickCount64()/1000);
    ctx.output(buf);
    return CommandResult::ok("swarm.stats");
}
...
CommandResult handleAuditExportReport(const CommandContext& ctx) {
    ...
    if (f) {
        ...
        fprintf(f, "Hotpatches: %u (M:%u B:%u S:%u)\n",
                stats.totalApplied, stats.memoryPatches, stats.bytePatches, stats.serverPatches);
        fprintf(f, "Repairs: %u  Anomalies: %u\n", oStats.totalRepairs, oStats.totalAnomalies);
        fclose(f);
