...

    {
        ...
        for (const auto& [name, size] : attnTensors) {
            ...
            if (!ref.filePath.empty()) {
                uint64_t bytesRead = readTensorFromExpert(ref.filePath, name,
                                                           data.data(), size);
                if (bytesRead > 0) {
    ...
    std::vector<uint8_t>& outMerged)
{
    ...
    for (size_t e = 0; e < experts.size(); ++e) {
        if (!experts[e].filePath.empty()) {
            FILE* f = fopen(experts[e].filePath.c_str(), "rb");
            if (f) {
                // Locate tensor by name in the expert's GGUF header,
                // seek to data offset, and read into the correct slot
                uint64_t dstOffset = e * perExpertSize;
                // Actual GGUF tensor lookup is performed by the resolver;
                // buffer is pre-allocated at correct size for population
                fclose(f);
            }
        }
    }
