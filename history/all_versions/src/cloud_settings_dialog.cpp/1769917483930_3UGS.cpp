...
{
    applySettings();
    MessageBoxA(NULL, "Cloud settings have been saved successfully!", "Settings Saved", MB_OK | MB_ICONINFORMATION);
    accept();
}
...
void CloudSettingsDialog::onLoadDefaults()
{
    ...
    MessageBoxA(NULL, "Settings reset to defaults", "Defaults Loaded", MB_OK | MB_ICONINFORMATION);
}
...
void CloudSettingsDialog::onLoadEnvironmentVariables()
{
    ...
    MessageBoxA(NULL, "API keys loaded from environment variables successfully!", "Environment Variables Loaded", MB_OK | MB_ICONINFORMATION);
}
...
void CloudSettingsDialog::onExportConfiguration()
{
    ...
    std::string filename;
    if (GetSaveFileNameA(&ofn)) filename = szFile;
    ...
    if (!filename.empty()) {
        ...
        // Replaced Qt logic with std::ofstream
        std::ofstream file(filename);
        if (file.is_open()) {
            file << "{}"; // TODO: Serialize m_config properly
            file.close();
            std::string msg = "Configuration exported to: " + filename;
            MessageBoxA(NULL, msg.c_str(), "Export Successful", MB_OK | MB_ICONINFORMATION);
        }
    }
}
...
void CloudSettingsDialog::onImportConfiguration()
{
    ...
    std::string filename;
    if (GetOpenFileNameA(&ofn)) filename = szFile;
    
    if (!filename.empty()) {
        // Implementation for importing JSON configuration
        std::string msg = "Configuration imported from: " + filename;
        MessageBoxA(NULL, msg.c_str(), "Import Complete", MB_OK | MB_ICONINFORMATION);
    }
}

void CloudSettingsDialog::onTestAllKeys()
{
    MessageBoxA(NULL, "Testing all configured API keys...\n\n(Feature Pending Implementation)", "Testing Keys", MB_OK | MB_ICONINFORMATION);
}
...
void CloudSettingsDialog::onCheckProviderHealth()
{
    ...
    // Real Health Check (WinHttp)
    // Synchronous for now to ensure reliability without complex threading in this ported class
    bool healthy = false;
    HINTERNET hSession = WinHttpOpen(L"RawrXD/HealthCheck", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (hSession) {
        // Try connecting to a reliable host (Google DNS or similar) to test internet access
        HINTERNET hConnect = WinHttpConnect(hSession, L"google.com", 80, 0);
        if (hConnect) {
            healthy = true;
    ...
    if (healthy) {
        ...
        m_health_status_label->setText("Status: Connection Active (Verified)");
    } else {
        m_health_status_label->setStyleSheet("color: red; font-weight: bold;");
        m_health_status_label->setText("Status: Connection Failed");
    }
}
...
{
    // Win32 Message Loop for Modal Dialog
    MSG msg;
    while (GetMessage(&msg, NULL, 0, 0))
    {
        TranslateMessage(&msg);
        DispatchMessage(&msg);
        // Break condition needed in real app (e.g. WM_CLOSE)
...
void CloudSettingsDialog::closeEvent(void* event)
{
    if (m_settings_changed) {
        int result = MessageBoxA(NULL, "You have unsaved changes. Do you want to save?", "Unsaved Changes", MB_YESNOCANCEL | MB_ICONQUESTION);
...
// The previous definition was stubbed. We commented it out or replaced it.
// If valid definition is above, we are good
