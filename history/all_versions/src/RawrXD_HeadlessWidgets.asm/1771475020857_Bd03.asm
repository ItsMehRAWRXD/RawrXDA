...
Widget_CreateSharedMemory PROC FRAME
    ...
    xor  r8d, r8d                       ; dwMaxHigh
    mov  r9d, SHARED_STATE_SIZE         ; dwMaxLow
    mov  qword ptr [rsp+20h], 0         ; lpName (pushed below)
    ; CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, size, name)
    mov  rcx, INVALID_HANDLE_VALUE
    mov  edx, 0                         ; lpAttributes
    mov  r8d, PAGE_READWRITE
    xor  r9d, r9d                       ; dwMaxHigh
    mov  dword ptr [rsp+20h], SHARED_STATE_SIZE
    lea  rax, szStateName
    ...
    mov  r8d, PAGE_READWRITE
    xor  r9d, r9d
    mov  dword ptr [rsp+20h], SHARED_CODE_SIZE
...
dc_references:
    ...
    jz   dc_err
    mov  rdx, SHARED_CODE_SIZE
    lea  r8, [rbp+10h]      ; TODO: parse identifier from payload
