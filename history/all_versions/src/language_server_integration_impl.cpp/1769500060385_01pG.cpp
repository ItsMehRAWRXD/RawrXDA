...
void LanguageServerIntegrationImpl::provideHover(const QString& uri, const Position& pos, 
    ...
    if(symbol) {
        ...
        result.contents.value = QString("**%1** (%2)\n\n%3")
...
void LanguageServerIntegrationImpl::provideDefinition(const QString& uri, const Position& pos,
                                                     const CancellationToken& ct,
                                                     std::function<void(QVector<Location>)> cb) {
...
void LanguageServerIntegrationImpl::provideReferences(const QString& uri, const Position& pos,
                                                     bool includeDecl,
                                                     const CancellationToken& ct,
                                                     std::function<void(QVector<Location>)> cb) {
...
void LanguageServerIntegrationImpl::provideDocumentSymbols(const QString& uri,
                                                          const CancellationToken& ct,
                                                          std::function<void(QVector<DocumentSymbol>)> cb) {
...
void LanguageServerIntegrationImpl::provideCompletion(const QString& uri, const Position& pos,
    ...
    for(auto& s : suggestions) {
        ...
        item.sortText = QString("%1").arg(int(s.score * 1000), 4, 10, QChar('0'));
...
void LanguageServerIntegrationImpl::provideDiagnostics(const QString& uri, 
                                                      std::function<void(QVector<Diagnostic>)> cb) {
...
void LanguageServerIntegrationImpl::provideFormatting(const QString& uri, const Range& range,
                                                     const CancellationToken& ct,
                                                     std::function<void(QVector<TextEdit>)> cb) {
...
void LanguageServerIntegrationImpl::provideCodeActions(const QString& uri, const Range& range,
                                                      const QVector<Diagnostic>& context,
                                                      const CancellationToken& ct,
                                                      std::function<void(QVector<CodeAction>)> cb) {
...
// Incremental Sync
void LanguageServerIntegrationImpl::applyIncrementalSync(const QString& uri, qint64 version,
...
TextEdit LanguageServerIntegrationImpl::calculateTextEditAsm(const QString& uri, 
                                                           const Position& pos,
    ...
    QByteArray utf8 = insertText.toUtf8();
    int normalizedLen = rxd_asm_normalize_completion(utf8.constData(), utf8.length());
    edit.newText = QString::fromUtf8(utf8.constData(), normalizedLen);
    
    return edit;
}

Position LanguageServerIntegrationImpl::offsetToPosition(const QString& text, int offset) const {
    int line = 0, col = 0;
