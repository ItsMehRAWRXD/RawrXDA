...
Lexer_AddToken proc frame pLexer:dq, type:dword, startPos:dq, endPos:dq, pValue:dq, valueLen:dword
    ...
    lea rax, [rdx + rcx * sizeof TOKEN]
    
    mov (TOKEN ptr [rax]).type, type
    mov (TOKEN ptr [rax]).startLoc.offset, startPos
    mov (TOKEN ptr [rax]).endLoc.offset, endPos
    mov (TOKEN ptr [rax]).length, valueLen
    
    .if pValue != 0 && valueLen > 0
        ; Copy value (simplified)
        mov rsi, pValue
        lea rdi, (TOKEN ptr [rax]).value
        mov ecx, valueLen
        rep movsb
    .endif
