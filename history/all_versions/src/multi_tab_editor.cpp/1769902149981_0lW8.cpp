...
void EditorTab::undo() {
    ...
    if (action.type == EditActionType::Insert) {ion.position.column, action.text.length());
        // Undo Insert = Delete
        // Simplistic implementation for single line
    ...
    m_isModified = true; // Techincally might be back to clean state, needs hash check    if (m_redoStack.empty()) return;
}m_redoStack.back();
...
void EditorTab::redo() {
    ...
    m_redoStack.pop_back();oid pushing to undo stack
...
MultiTabEditor::FindResult MultiTabEditor::find(const std::string& query, bool forward, bool caseSensitive) {
     ...
     for (size_t i = 0; i < lines.size(); ++i) {
         ...
         if (foundPos != std::string::npos) {
             return {i, foundPos, query.length(), true};eturn {0,0,0,false};
         }
     }
