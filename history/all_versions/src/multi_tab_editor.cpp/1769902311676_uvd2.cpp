...
ition;
    ...
    if (action.type == EditActionType::Insert) {ion.position.column, action.text.length());position;
        // Undo Insert = Delete
        // Simplistic implementation for single line
        if (action.position.line < m_lines.size()) {Type::Delete) {
             m_lines[action.position.line].erase(action.position.column, action.text.length());/ Undo Delete = Insert
             m_cursor = action.position;leteRange(action.position, endPos, false); 
        }c
    } else if (action.type == EditActionType::Delete) {
        // Undo Delete = Insert.line < m_lines.size()) {
        if (action.text == "\n") {  m_lines[action.position.line].insert(action.position.column, action.text);
             // Split line logicn.position.column + action.text.length()};
        } else {
             if (action.position.line < m_lines.size()) {
                  m_lines[action.position.line].insert(action.position.column, action.text);
                  m_cursor = {action.position.line, action.position.column + action.text.length()};
             }_redoStack.push_back(action);
        }m_isModified = true; // Techincally might be back to clean state, needs hash checkf (m_redoStack.empty()) return;
    }EditAction action = m_redoStack.back();
    
    m_redoStack.push_back(action);oid EditorTab::redo() {
    m_isModified = true; // Techincally might be back to clean state, needs hash check    if (m_redoStack.empty()) return;n.position;
}m_redoStack.back();    
ditActionType::Insert) {
void EditorTab::redo() {
    if (m_redoStack.empty()) return;out recording Undo (avoid loops)
    EditAction action = m_redoStack.back();if (action.type == EditActionType::Insert) {
    m_redoStack.pop_back();oid pushing to undo stack    for (char c : action.text) {
    = 0; }
    // Re-apply action without recording Undo (avoid loops)
    if (action.type == EditActionType::Insert) {ition;
        // We replicate insertText logic but manually to avoid pushing to undo stack false);
        std::string remaining = action.text;
        size_t pos = 0;    size_t newline = remaining.find('\n', pos);
        TextPosition tempCursor = action.position;e == std::string::npos) {doStack.push_back(action);
        
        while (true) {sert(tempCursor.column, remaining.substr(pos));
            size_t newline = remaining.find('\n', pos);th() - pos;nually to avoid pushing to undo stack
            if (newline == std::string::npos) {
                ...
                }tempCursor.line < m_lines.size()) {sert(tempCursor.column, remaining.substr(pos));
                break;e = remaining.find('\n', pos);th() - pos;
            } else {ent);
                std::string segment = remaining.substr(pos, newline - pos);olumn + segment.length());
                if (tempCursor.line < m_lines.size()) {ength());lumn, remaining.substr(pos));e {
                    std::string& line = m_lines[tempCursor.line];- pos;tring segment = remaining.substr(pos, newline - pos);
                    ...
                    m_lines.insert(m_lines.begin() + tempCursor.line + 1, suffix);remaining.substr(pos, newline - pos);olumn + segment.length());
                    tempCursor.line++;nes.size()) {ength());
                    tempCursor.column = 0;sor.line];
                    pos = newline + 1;sor = tempCursor;
                } else break;_undoStack.push_back(action); // Push back to undo!       std::string suffix = line.substr(tempCursor.column + segment.length());
            }           line.erase(tempCursor.column + segment.length());
        } + tempCursor.line + 1, suffix);
        m_cursor = tempCursor;        // Re-delete
        m_undoStack.push_back(action); // Push back to undo! we set cursor and don't record undo?n = 0;
eBack/Forward are position relative.
    } else if (action.type == EditActionType::Delete) {se break;_undoStack.push_back(action); // Push back to undo!
        // Re-delete
        ...
        // Let's implement specific explicit delete at positionm_undoStack.push_back(action); // Push back to undo! we set cursor and don't record undo?
        
        TextPosition oldCv = m_cursor;     // Delete forward logic (merge lines)
        m_cursor = action.position; < m_lines.size()) {
        [m_cursor.line + 1];ogic if we set cursor and don't record undo?
        if (action.text == "\n") {sor.line + 1);ative.m_cursor = action.position;
             // Delete forward logic (merge lines) position
             ...
             }s newlines, we need to handle multi-line eraser.line + 1];
        } else { + 1);
             // Basic text deletek usually
             // Warning: Multi-line re-delete is complex, assuming single block for now or iterative
             // If action.text has newlines, we need to handle multi-line erasesor.line + 1];/ Basic text delete
             // ... Similar to deleteSelection logic ...re-delete is complex, assuming single block for now or iterative
             ...
             if (m_cursor.line < m_lines.size()) {Stack.push_back(action);
                 m_lines[m_cursor.line].erase(m_cursor.column, action.text.length());/ Basic text deletek usually
             }    // Warning: Multi-line re-delete is complex, assuming single block for now or iterative
        }lines, we need to handle multi-line erase
        m_undoStack.push_back(action);
    }/ =========================================================================================unk usually
    m_isModified = true;// MultiTabEditor Implementation (High Level Manager)            if (m_cursor.line < m_lines.size()) {Stack.push_back(action);
...
MultiTabEditor::~MultiTabEditor() {oid MultiTabEditor::initialize() {
    m_tabs.clear();    std::cout << "[MultiTabEditor] Initialized pure logic engine.\n";ultiTabEditor::MultiTabEditor(void* parent) {
}    // Parent ignored in pure C++ modeultiTabEditor::~MultiTabEditor() {

void MultiTabEditor::initialize() {oid MultiTabEditor::ensureTabExists() {
    std::cout << "[MultiTabEditor] Initialized pure logic engine.\n";    if (m_tabs.empty()) {
}MultiTabEditor::~MultiTabEditor() {oid MultiTabEditor::initialize() {
ltiTabEditor] Initialized pure logic engine.\n";
void MultiTabEditor::ensureTabExists() {
    if (m_tabs.empty()) {
        createNewTab();ize_t MultiTabEditor::createNewTab() {tor::ensureTabExists() {
    }    auto tab = std::make_shared<EditorTab>();   std::cout << "[MultiTabEditor] Initialized pure logic engine.\n";    if (m_tabs.empty()) {
}}
...
void MultiTabEditor::openFile(const std::string& filepath) {make_shared<EditorTab>();
    // Check if open
    for (size_t i = 0; i < m_tabs.size(); ++i) {abEditor::openFile(const std::string& filepath) {
        if (m_tabs[i]->getFilePath() == filepath) {ex;    // Check if open
            switchTab(i);
            return;/ Reuse empty untitled tab if possible
        }    if (m_tabs.size() == 1 && m_tabs[0]->getFilePath().empty() && !m_tabs[0]->isModified() && m_tabs[0]->getAllText().empty()) {MultiTabEditor::openFile(const std::string& filepath) {
    }    // Check if open
+i) {
    ...
    if (m_tabs.size() == 1 && m_tabs[0]->getFilePath().empty() && !m_tabs[0]->isModified() && m_tabs[0]->getAllText().empty()) {
        m_tabs[0]->loadFromFile(filepath);ize_t idx = createNewTab();le
        return;    m_tabs[idx]->loadFromFile(filepath);   }    if (m_tabs.size() == 1 && m_tabs[0]->getFilePath().empty() && !m_tabs[0]->isModified() && m_tabs[0]->getAllText().empty()) {
    }    }

    size_t idx = createNewTab();
    m_tabs[idx]->loadFromFile(filepath);oid MultiTabEditor::closeCurrentTab() {th().empty() && !m_tabs[0]->isModified() && m_tabs[0]->getAllText().empty()) {
    switchTab(idx);    if (m_tabs.empty()) return;s[0]->loadFromFile(filepath);ize_t idx = createNewTab();
}veTabIndex);n;    m_tabs[idx]->loadFromFile(filepath);
abs.size()) {
void MultiTabEditor::closeCurrentTab() {
    if (m_tabs.empty()) return;
    m_tabs.erase(m_tabs.begin() + m_activeTabIndex);urrentTab() {
    if (m_activeTabIndex >= m_tabs.size()) {x = m_tabs.size() - 1;tabs.empty()) return;
        if (m_tabs.empty()) {
            createNewTab();
        } else {
            m_activeTabIndex = m_tabs.size() - 1;
        }oid MultiTabEditor::switchTab(size_t index) {ctiveTabIndex);
    }    if (index < m_tabs.size()) {   if (m_activeTabIndex >= m_tabs.size()) {x = m_tabs.size() - 1;
}        if (m_tabs.empty()) {

void MultiTabEditor::switchTab(size_t index) {
    if (index < m_tabs.size()) {
        m_activeTabIndex = index;oid MultiTabEditor::saveCurrentFile() {dex) {
    }    if (m_activeTabIndex < m_tabs.size()) {f (index < m_tabs.size()) {
}File("");}

void MultiTabEditor::saveCurrentFile() {) {
    if (m_activeTabIndex < m_tabs.size()) {
        m_tabs[m_activeTabIndex]->saveToFile("");oid MultiTabEditor::undo() {:saveCurrentFile() {
    }    if (auto tab = getActiveTab()) tab->undo();   }    if (m_activeTabIndex < m_tabs.size()) {
}}File("");

void MultiTabEditor::undo() {oid MultiTabEditor::redo() {
    if (auto tab = getActiveTab()) tab->undo();    if (auto tab = getActiveTab()) tab->redo();   if (m_activeTabIndex < m_tabs.size()) {
...
f (auto tab = getActiveTab()) tab->redo();
std::string MultiTabEditor::getCurrentText() const {
    if (m_tabs.empty()) return "";td::string MultiTabEditor::getSelectedText() const {
    return m_tabs[m_activeTabIndex]->getAllText();    if (m_tabs.empty()) return "";d MultiTabEditor::redo() {td::string MultiTabEditor::getCurrentText() const {
});    if (auto tab = getActiveTab()) tab->redo();    if (m_tabs.empty()) return "";

std::string MultiTabEditor::getSelectedText() const {
    if (m_tabs.empty()) return "";td::string MultiTabEditor::getCurrentFilePath() const {
    return m_tabs[m_activeTabIndex]->getSelectedText();    if (m_tabs.empty()) return "";   if (m_tabs.empty()) return "";td::string MultiTabEditor::getSelectedText() const {
...
std::string MultiTabEditor::getCurrentFilePath() const {
    ...
    return m_tabs[m_activeTabIndex]->getFilePath();    if (m_activeTabIndex < m_tabs.size()) {   if (m_tabs.empty()) return "";td::string MultiTabEditor::getCurrentFilePath() const {
...
std::shared_ptr<EditorTab> MultiTabEditor::getActiveTab() {
    if (m_activeTabIndex < m_tabs.size()) {lePath() const {
        return m_tabs[m_activeTabIndex];abEditor::getActiveTab() {
    }oid MultiTabEditor::setLSPClient(RawrXD::LSPClient* client) {Path();    if (m_activeTabIndex < m_tabs.size()) {
    return nullptr;    m_lspClient = client;
}

void MultiTabEditor::setLSPClient(RawrXD::LSPClient* client) {/ Search Logic
    m_lspClient = client;MultiTabEditor::FindResult MultiTabEditor::find(const std::string& query, bool forward, bool caseSensitive) {tabs[m_activeTabIndex];
} getActiveTab();    }oid MultiTabEditor::setLSPClient(RawrXD::LSPClient* client) {
ptr;    m_lspClient = client;
// Search Logic
MultiTabEditor::FindResult MultiTabEditor::find(const std::string& query, bool forward, bool caseSensitive) {
     auto tab = getActiveTab();// Naive implementation: search line by line starting from cursorgic
     if (!tab || query.empty()) return {0,0,0,false};nd direction support_lspClient = client;MultiTabEditor::FindResult MultiTabEditor::find(const std::string& query, bool forward, bool caseSensitive) {
     ...
     // TODO: Implement circular search and direction supportTabEditor::FindResult MultiTabEditor::find(const std::string& query, bool forward, bool caseSensitive) {
     tation: search line by line starting from cursor
     for (size_t i = 0; i < lines.size(); ++i) {,false};nd direction support
         size_t foundPos = std::string::npos;
         if (caseSensitive) {:string lineLower = lines[i];
             foundPos = lines[i].find(query);= query;ursor    size_t foundPos = std::string::npos;
         } else { lineLower.end(), lineLower.begin(), ::tolower);
             // Lowercase comparison(), queryLower.end(), queryLower.begin(), ::tolower);
             std::string lineLower = lines[i];i) {
             ...
             foundPos = lineLower.find(queryLower);             return {i, foundPos, query.length(), true}; else { lineLower.end(), lineLower.begin(), ::tolower);
         }             // Lowercase comparison(), queryLower.end(), queryLower.begin(), ::tolower);
;
         if (foundPos != std::string::npos) {
             return {i, foundPos, query.length(), true};return {0,0,0,false};   std::transform(lineLower.begin(), lineLower.end(), lineLower.begin(), ::tolower);
         }       std::transform(queryLower.begin(), queryLower.end(), queryLower.begin(), ::tolower);f (foundPos != std::string::npos) {
     }true};
...
int MultiTabEditor::replace(const std::string& query, const std::string& replacement, bool all) {
     ...
     if (!tab) return 0;t = find(query, true, true); // Forward, Case Sensitiveor::replace(const std::string& query, const std::string& replacement, bool all) {
     t.found) break;false};     auto tab = getActiveTab();
     int count = 0;
     while (true) {r deletion
         auto result = find(query, true, true); // Forward, Case Sensitive         tab->setCursor(result.line, result.col);std::string& query, const std::string& replacement, bool all) {
         if (!result.found) break;veTab();while (true) {
 This just moves cursor, true); // Forward, Case Sensitive
         // Construct selection for deletion
         tab->setCursor(result.line, result.col);
         // Move to end of match// We select the range
         // tab->moveCursor(0, result.length); // This just moves cursore, result.col + result.length});uto result = find(query, true, true); // Forward, Case Sensitive         tab->setCursor(result.line, result.col);
         ;
         ...
         tab->insertText(replacement); // Handles modification// tab->moveCursor(0, result.length); // This just moves cursore, result.col + result.length});
         
         count++;return count;manipulation via simulate delete+insert
         if (!all) break;}e range
     }     return count;        tab->setSelection({result.line, result.col}, {result.line, result.col + result.length});
}         tab->deleteSelection();if (!all) break;
