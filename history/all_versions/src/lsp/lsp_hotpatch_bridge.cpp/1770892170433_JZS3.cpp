    ...
    if (layer == HotpatchLayer::Memory) {
        ...
        if (it != m_trackedPatches.end()) {
            ...
            if (addr && size > 0) {
                ...
                memcpy(addr, it->second.originalBytes.data(), size);
                VirtualProtect(addr, size, oldProt, &oldProt);
                FlushInstructionCache(GetCurrentProcess(), addr, size);
#else
                // POSIX: mprotect + memcpy
                uintptr_t page = (uintptr_t)addr & ~0xFFFULL;
                size_t pageSpan = ((uintptr_t)addr + size - page + 0xFFF) & ~0xFFFULL;
                mprotect((void*)page, pageSpan, PROT_READ | PROT_WRITE | PROT_EXEC);
                memcpy(addr, it->second.originalBytes.data(), size);
#endif
                m_trackedPatches.erase(it);
                pr = PatchResult::ok("Memory patch reverted");
            } else {
                pr = PatchResult::error("Tracked patch has invalid address or empty original bytes");
            }
        } else {
