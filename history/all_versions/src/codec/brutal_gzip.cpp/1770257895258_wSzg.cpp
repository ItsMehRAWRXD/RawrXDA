...
std::vector<uint8_t> inflate(const std::vector<uint8_t>& data, bool* success) {
    ...
    size_t pos = 0;
    uint8_t cmf = data[pos++];
    uint8_t flg = data[pos++];

    if ((cmf & 0x0F) != 8) return {};
    if (((cmf << 8) + flg) % 31 != 0) return {};

    std::vector<uint8_t> out;
    while (pos + 5 <= data.size()) {
        uint8_t bfinal = data[pos++] & 0x01;
        uint8_t btype = (data[pos - 1] >> 1) & 0x03;
        if (btype != 0) return {};

        if (pos + 4 > data.size()) return {};
        uint16_t len = static_cast<uint16_t>(data[pos] | (data[pos + 1] << 8));
        uint16_t nlen = static_cast<uint16_t>(data[pos + 2] | (data[pos + 3] << 8));
        pos += 4;
