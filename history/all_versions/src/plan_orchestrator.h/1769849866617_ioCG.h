...
private:
    ...
    bool applyRename(const EditTask& task, bool dryRun);nternal helper methods
    PlanningPrompt(const std::string& userPrompt, const std::vector<std::string>& contextFiles);
    std::string readFileContent(const std::string& filePath);    std::vector<std::string> gatherContextFiles(const std::string& workspaceRoot, int maxFiles);
    bool writeFileContent(const std::string& filePath, const std::string& content);    PlanningResult parsePlanningResponse(const std::string& response);
...
} // namespace RawrXD};    bool writeFileContent(const std::string& filePath, const std::string& content);    std::string readFileContent(const std::string& filePath);    bool applyRename(const EditTask& task, bool dryRun);    bool applyDelete(const EditTask& task, bool dryRun);    bool applyInsert(const EditTask& task, bool dryRun);    bool applyReplace(const EditTask& task, bool dryRun);    bool executeTask(const EditTask& task, bool dryRun);        
