...
@@parse_kv_loop:
    ...
    ; Save key pointer and length for matching after we read the value
    mov r10, r12                           ; r10 = key string pointer
    mov r11, rax                           ; r11 = key length
...
@@kv_u32:
    ...
    ; Match key name (r10=key ptr, r11=key len) to set model params
    ; Check for "embedding_length" suffix (16 chars) -> n_embd
    cmp r11, 16
    jb @@kv_try_block_count
    ; Compare last 16 bytes: "embedding_length"
    mov rdi, r10
    add rdi, r11
    sub rdi, 16
    cmp QWORD PTR [rdi],   'e' OR ('m' SHL 8) OR ('b' SHL 16) OR ('e' SHL 24) OR ('d' SHL 32) OR ('d' SHL 40) OR ('i' SHL 48) OR ('n' SHL 56)
    jne @@kv_try_block_count
    ; Match found -> set n_embd
    mov [r13].TitanContext.n_embd, eax
    jmp @@kv_next
    
@@kv_try_block_count:
    ; Check for "block_count" suffix (11 chars) -> n_layer
    cmp r11, 11
    jb @@kv_try_head_count
    mov rdi, r10
    add rdi, r11
    sub rdi, 11
    cmp DWORD PTR [rdi], 'b' OR ('l' SHL 8) OR ('o' SHL 16) OR ('c' SHL 24)
    jne @@kv_try_head_count
    mov [r13].TitanContext.n_layer, eax
    jmp @@kv_next
    
@@kv_try_head_count:
    ; Check for "head_count_kv" suffix (13 chars) -> n_head_kv (check BEFORE head_count)
    cmp r11, 13
    jb @@kv_try_head_count_simple
    mov rdi, r10
    add rdi, r11
    sub rdi, 13
    cmp DWORD PTR [rdi], 'h' OR ('e' SHL 8) OR ('a' SHL 16) OR ('d' SHL 24)
    jne @@kv_try_head_count_simple
    ; Verify it ends with "_kv"
    mov rdi, r10
    add rdi, r11
    sub rdi, 3
    cmp WORD PTR [rdi], '_' OR ('k' SHL 8)
    jne @@kv_try_head_count_simple
    mov [r13].TitanContext.n_head_kv, eax
    jmp @@kv_next
    
@@kv_try_head_count_simple:
    ; Check for "head_count" suffix (10 chars) -> n_head
    cmp r11, 10
    jb @@kv_next
    mov rdi, r10
    add rdi, r11
    ...
    cmp DWORD PTR [rdi], 'h' OR ('e' SHL 8) OR ('a' SHL 16) OR ('d' SHL 24)
    jne @@kv_next
    mov [r13].TitanContext.n_head, eax
    jmp @@kv_next
...
@@no_header:
    ...
    ; Tokenize prompt via BPE: call RawrXD_Tokenize to convert text to token IDs
    ; RawrXD_Tokenize(pCtx, pPromptStr, pTokenBuf, maxTokens) -> nTokens in EAX
    mov rcx, r13                           ; TitanContext
    mov rdx, r12                           ; prompt string pointer
    lea r8, [r14]                          ; ring buffer = token output destination
    mov r9d, MAX_SEQ_LEN                   ; max tokens
    call RawrXD_Tokenize
    test eax, eax
    jz @@gen_bos_fallback                  ; tokenizer not available, fallback to BOS
    
    ; eax = number of prompt tokens written to ring buffer
    mov [r13].TitanContext.prompt_len, eax
    mov [r13].TitanContext.ring_read_idx, eax
    jmp @@gen_start
    
@@gen_bos_fallback:
    ; Fallback: write BOS token (token 0) as the first token
