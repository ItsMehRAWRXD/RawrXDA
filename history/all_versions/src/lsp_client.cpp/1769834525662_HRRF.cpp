...
void LSPClient::createChildProcess() {
    ...
    if (!bSuccess) {
        ...
        m_hProcess = piProcInfo.hProcess;
        m_hThread = piProcInfo.hThread;
        m_hStdOutRead = hChildStd_OUT_Rd;
        m_hStdInWrite = hChildStd_IN_Wr;
...
void LSPClient::writeToChild(const std::string& message) {
    ...
    WriteFile((HANDLE)m_hStdInWrite, fullMsg.c_str(), (DWORD)fullMsg.length(), &dwWritten, NULL);
}
...
void LSPClient::readFromChild() {
    ...
    while (m_serverRunning) {
        if (!ReadFile((HANDLE)m_hStdOutRead, chBuf, BUFSIZE, &dwRead, NULL) || dwRead == 0) break;
