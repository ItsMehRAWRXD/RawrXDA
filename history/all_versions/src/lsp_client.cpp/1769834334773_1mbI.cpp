...
bool LSPClient::startServer()
{
    ...
    // Create Process
    // Note: We need a mutable buffer for CreateProcessW cmdLine
    ...
    m_stopThread = false;
    m_readThread = std::thread(&LSPClient::onServerReadyRead, this);
    ...
    params["capabilities"] = json::object(); // Basic text capabilities
    
    json request = json::object();
    request["jsonrpc"] = "2.0";
    request["method"] = "initialize";
    request["id"] = 0; // Initialize ID
...
void LSPClient::onServerReadyRead()
{
    ...
    while (m_serverRunning && !m_stopThread) {
        ...
        while (true) {
            ...
            if (headerEnd == std::string::npos) break; // Need more data
            ...
            if (contentLengthPos == std::string::npos || contentLengthPos > headerEnd) {
                // Invalid header, remove it and continue (or error out)
...
void LSPClient::handleCompletionResponse(const json& result, int requestId)
{
    ...
    // result can be Array or Object {items: [], isIncomplete: bool}
    ...
    if (itemList) {
        for (const auto& item : *itemList) {
            ...
            ci.documentation = item.value("documentation", ""); // simple string extract
    ...
    // Retrieve context from pending, but we passed it implicitly via id
    // Ideally we store context in pending map
...
void LSPClient::handleDefinitionResponse(const json& result, int requestId)
{
    if (result.is_array() && !result.empty()) {
        ...
        // Range extraction omitted for brevity
...
std::string LSPClient::buildDocumentUri(const std::string& filePath) const {
    // Basic file URI construction (needs better impl for Windows paths)
