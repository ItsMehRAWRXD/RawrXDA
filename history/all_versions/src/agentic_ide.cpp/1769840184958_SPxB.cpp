...
void AgenticIDE::initialize() {
    ...
    m_orchestrator = std::make_unique<AutonomousIntelligenceOrchestrator>(this); // Initialize Orchestrator
    m_orchestrator->initialize(""); // Determine path later or pass current directory
...
void AgenticIDE::processConsoleInput() {
    ...
    while (m_running && std::getline(std::cin, line)) {
        if (line == "exit" || line == "quit") {
            ...
            auto report = m_orchestrator->getQualityReport();
            std::cout << "Quality Score: " << report["score"] << "\n";
