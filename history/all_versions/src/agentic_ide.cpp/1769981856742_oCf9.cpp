...
Result<void> AgenticIDE::initialize() {
    ...
    // 2.5 Initialize Network Stack
    spdlog::info("Initializing Network Stack (Win32/WinInet)...");
    RawrXD::Net::NetworkConfig netConfig;
    netConfig.userAgent = "RawrXD-Agentic/1.0";
    netConfig.enableSSL = true;
    netConfig.maxConnections = 32;
    
    auto netInit = RawrXD::Net::NetworkManager::instance().initialize(netConfig);
    if (!netInit) {
        spdlog::error("Network Stack Initialization Failed!");
    } else {
        spdlog::info("Network Stack Initialized Successfully.");
    }
...
void AgenticIDE::stop() {
    ...
    RawrXD::Net::NetworkManager::instance().shutdown();
...
nlohmann::json AgenticIDE::getStatus() const {
    ...
    status["network"] = RawrXD::Net::NetworkManager::instance().getNetworkStatus();
    return status;
}
