...
static void ParseBaseUrl(const std::string& url, std::wstring& host_out, INTERNET_PORT& port_out) {
    ...
    if (host_str.empty()) host_str = "127.0.0.1";
    // Avoid IPv6/localhost resolution surprises.
    if (host_str == "localhost") host_str = "127.0.0.1";
    // Convert to wide
    host_out.resize(host_str.size() + 1);
...
bool OllamaClient::makeStreamingPostRequest(const std::string& endpoint,
    ...
    // Streaming: long receive timeout (models may pause before emitting tokens).
    // Use at least 1 hour, or m_timeout_seconds if larger.
    DWORD timeoutMs = (DWORD)((m_timeout_seconds > 0 ? m_timeout_seconds : 300) * 1000);
    DWORD receiveTimeout = (timeoutMs > 3600000 ? timeoutMs : 3600000);
    WinHttpSetTimeouts(hSession, 10000, 15000, 30000, receiveTimeout);
    
    HINTERNET hConnect = WinHttpConnect(hSession, wHost.c_str(), port, 0);
    if (!hConnect) {
        WinHttpCloseHandle(hSession);
        if (on_error) on_error("Failed to connect to Ollama");
        return false;
    }
