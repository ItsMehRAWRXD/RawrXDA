...
FeedbackEntry FeedbackDialog::getFeedback() const
{
    ...
    entry.modified = CurrentTimestamp();
...
void FeedbackDialog::onSubmit()
{
    ...
    entry.submitted = CurrentTimestamp();
    ...
    // Real WinHTTP Submission
    m_statusLabel->setText(tr("Connecting to feedback server..."));
    
    bool success = false;
    HINTERNET hSession = WinHttpOpen(L"RawrXD-Feedback/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (hSession) {
        // Connect to API
        HINTERNET hConnect = WinHttpConnect(hSession, L"api.rawrxd.ai", INTERNET_DEFAULT_HTTPS_PORT, 0);
        if (hConnect) {
            HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"POST", L"/v1/feedback", NULL, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_SECURE);
            if (hRequest) {
                // serialized JSON
                std::stringstream ss;
                ss << "{";
                ss << "\"id\": \"" << entry.id.toStdString() << "\","; // Assuming QString
                ss << "\"title\": \"" << entry.title.toStdString() << "\",";
                ss << "\"description\": \"" << entry.description.toStdString() << "\"";
                ss << "}";
                std::string json = ss.str();
                
                LPCWSTR headers = L"Content-Type: application/json";
                if (WinHttpSendRequest(hRequest, headers, -1, (LPVOID)json.c_str(), (DWORD)json.length(), (DWORD)json.length(), 0)) {
    ...
    if (success) {
        ...
        m_statusLabel->setText(tr("❌ Connection failed. Saved offline."));
         // Log failure but allow user to continue
    }
}
...
TelemetryConsent TelemetryConsentDialog::getConsent() const
{
    ...
    consent.consentDate = CurrentTimestamp();
...
void ContributionDialog::onSubmit()
{
    ...
    m_entry.submitted = CurrentTimestamp();
