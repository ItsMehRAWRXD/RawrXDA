...
RawrXD::IDE::MainWindow::MainWindow(void* parent) {
    // Win32 window creation - Real Native Implementation
    m_hwnd = nullptr;
    m_x = 100;
    m_y = 100;
    m_width = 1024;
    ...
    const char* CLASS_NAME = "RawrXD_IDE_Window";
    WNDCLASSEXA wc = {};
    
    // Check if already registered
    if (!GetClassInfoExA(GetModuleHandle(NULL), CLASS_NAME, &wc)) {
        wc.cbSize = sizeof(WNDCLASSEXA);
        wc.style = CS_HREDRAW | CS_VREDRAW;
        wc.lpfnWndProc = DefWindowProcA; // Placeholder, usually would allow hooking or static forwarding
        wc.cbClsExtra = 0;
        wc.cbWndExtra = 0;
