...
g_RoPECosTable      REAL4 (128 * 32) DUP(?)  ; Reduced size for demo
g_RoPESinTable      REAL4 (128 * 32) DUP(?)
...
@@dim_loop:
    ...
    cvtsi2ss xmm0, r13
    addss xmm0, xmm0                    ; 2*i
    divss xmm0, [rel sixteen]           ; / 16
    fchs                                ; Negate
    
    ; Convert xmm0 to ST(0)
    movss [rsp-4], xmm0
    fld DWORD PTR [rsp-4]
    fmul                                ; ST(0) *= -log2(theta)
    f2xm1                               ; 2^x - 1
    fadd                                ; Reconstruct 2^x
