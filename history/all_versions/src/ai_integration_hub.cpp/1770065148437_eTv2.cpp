...
std::string AIIntegrationHub::chat(const std::string& message) {
    ...
    if (message.rfind("/bugreport", 0) == 0) {
         // Similar to audit
        std::string target = message.length() > 11 ? message.substr(11) : "codebase";
        std::string prompt = "Generate a comprehensive bug report for: " + target;
        
        auto result = m_inferenceEngine->generate(prompt);
         if (result.has_value()) {
            return "[Bug Report]\n" + result.value().text;
        }
        return "Error: Bug report generation failed.";
    }
