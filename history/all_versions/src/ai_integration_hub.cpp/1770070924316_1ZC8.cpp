...
std::vector<GeneratedTestCase> AIIntegrationHub::generateTests(const std::string& function) {
    if (m_codingAgent) {
        auto taskResult = m_codingAgent->generateTests(function, "cpp");
        std::vector<GeneratedTestCase> results;
        if (taskResult.success) {
             GeneratedTestCase tc;
             tc.name = "Generated Tests";
             tc.code = taskResult.generatedCode;
             tc.description = taskResult.explanation; 
             // tc.assertions (empty logic for now)
