...
{
    m_backend = "ollama";
    m_endpoint = "http://localhost:23959";

    // Load cached responses from disk if available
    std::string cacheDir = QStandardPaths::writableLocation(QStandardPaths::CacheLocation);
    if (!cacheDir.empty()) {
        // dir(cacheDir);
 ...
 */
void ModelInvoker::setCodebaseEmbeddings(const QMap<std::string, float>& embeddings)
{
...
LLMResponse ModelInvoker::invoke(const InvocationParams& params)
{
    ...
    try {
        ...
        if (m_backend == "ollama") {
            ...
            response.tokensUsed = llmResponse.value("eval_count") + 
                                  llmResponse.value("prompt_eval_count");
        } else if (m_backend == "claude") {
            auto content = llmResponse.value("content").toArray();
            if (!content.empty()) {
                response.rawOutput = content[0].toObject().value("text").toString();
            }
            response.tokensUsed = llmResponse.value("usage").toObject().value("output_tokens");
        } else if (m_backend == "openai") {
            ...
            response.tokensUsed = llmResponse.value("usage").toObject().value("completion_tokens");
        }
        ...
        response.error = "Exception: " + std::string::fromStdString(e.what());
        response.success = false;
 ...
 */
std::string ModelInvoker::buildSystemPrompt(const std::stringList& tools)
{
 ...
 */
QJsonObject ModelInvoker::sendOllamaRequest(const std::string& model,
                                            ...
                                            double temperature)
{
    ...
    QEventLoop loop;
    QNetworkReply* reply = m_networkManager->post(request, data);  // Signal connection removed\n// Timer operation removed // 30s timeout
 ...
 */
QJsonObject ModelInvoker::sendClaudeRequest(const std::string& prompt,
                                            int maxTokens,
                                            double temperature)
{
 ...
 */
QJsonObject ModelInvoker::sendOpenAIRequest(const std::string& prompt,
                                            int maxTokens,
                                            double temperature)
{
 ...
 */
QJsonArray ModelInvoker::parsePlan(const std::string& llmOutput)
{
