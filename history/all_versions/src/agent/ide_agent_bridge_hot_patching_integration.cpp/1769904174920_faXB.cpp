...
void* IDEAgentBridgeWithHotPatching::getHotPatchingStatistics() const
{
    ...
    json* stats = new json(m_hotPatcher->getCorrectionStatistics());
    ...
    const HallucinationDetection& correction)
{
    ...
    stream << QDateTime::currentDateTimeUtc().toString(Qt::ISODate) << " | "
           << "Type: " << QString::fromStdString(correction.hallucationType) << " | "
           << "Confidence: " << correction.confidence << " | "
           << "Detected: " << QString::fromStdString(correction.detectedContent).left(50) << " | "
           << "Corrected: " << QString::fromStdString(correction.expectedContent).left(50) << "\n";
