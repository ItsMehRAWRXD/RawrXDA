    ...
    const std::string& dbPath)
{
    ...
    try {
        ...
        // Log error "Error fetching correction patterns from " << jsonPath << ": " << e.what() << std::endl;
    }
    ...
    const std::string& dbPath)
{
    ...
    try {
        ...
        std::cerr << "Error fetching behavior patches from " << jsonPath << std::endl;
    }
    ...
    const std::string& databasePath)
{
    ...
    // Load patterns from SQLite databaseerly DB)
    std::vector<CorrectionPatternRecord> patterns = 
    ...
    for (const auto& rec : patterns) {
        try {
            // Log each pattern loaded
                     << "ID:" << rec.id
                     << "Type:" << rec.typering(rec.id);
                     << "Threshold:" << rec.confidenceThreshold;;
            
            HallucinationDetection hd;hd.confidence = rec.confidenceThreshold;
            hd.detectionId = std::to_string(rec.id);nPattern(hd);
            hd.hallucinationType = rec.type;
            hd.detectedContent = rec.pattern;
            hd.confidence = rec.confidenceThreshold;
            m_hotPatcher->addCorrectionPattern(hd);" << rec.id << ":" << ex.what() << std::endl;
...
void IDEAgentBridgeWithHotPatching::loadBehaviorPatches(
    // Log success: Loaded X patterns    const std::string& databasePath)
}
   if (!m_hotPatcher) {
void IDEAgentBridgeWithHotPatching::loadBehaviorPatches(        return;
    const std::string& databasePath)
{
    if (!m_hotPatcher) {   // Load patches from SQLite database
        return;PatchRecord> patches = 
    }haviorPatchesFromDb(databasePath);

    // Load patches from SQLite database    if (patches.empty()) {
    std::vector<BehaviorPatchRecord> patches = iors only
        fetchBehaviorPatchesFromDb(databasePath);

    if (patches.empty()) {
        // Log info: Using default behaviors onlyth the hot patcher
        return;
    }
       BehaviorPatch p;
    // Register patches with the hot patcher            p.patchId = std::to_string(rec.id);
    for (const auto& rec : patches) {
        try {
            BehaviorPatch p;ry {
            p.patchId = std::to_string(rec.id);n::parse(rec.payloadJson);
            p.patchType = rec.patchType;n", "");
            on", "");
            try {    p.affectedModels = j.value("affectedModels", std::vector<std::string>{});
                auto j = json::parse(rec.payloadJson);.successRate = j.value("successRate", 0.0);
                p.condition = j.value("condition", "");
                p.action = j.value("action", "");
                p.affectedModels = j.value("affectedModels", std::vector<std::string>{});parse payload JSON for patch " << rec.id << std::endl;
                p.successRate = j.value("successRate", 0.0);
                p.enabled = j.value("enabled", true);
            } catch (...) {
                std::cerr << "Warning: Failed to parse payload JSON for patch " << rec.id << std::endl;:exception& ex) {
            }
...
void IDEAgentBridgeWithHotPatching::onHallucinationDetected(              << ", Confidence: " << detection.confidence << std::endl;
...
void IDEAgentBridgeWithHotPatching::onHallucinationCorrected(              << ", Original: " << correction.detectedContent
    const HallucinationDetection& correction)t << std::endl;
{
    std::cout << "Hallucination Corrected! Type: " << correction.hallucinationType 
...
void IDEAgentBridgeWithHotPatching::onNavigationErrorFixed(              << " To: " << fix.correctPath << std::endl;
...
void IDEAgentBridgeWithHotPatching::onBehaviorPatchApplied(              << ", Type: " << patch.patchType 
    const BehaviorPatch& patch) std::endl;
{
    std::cout << "Behavior Patch Applied! ID: " << patch.patchId 
...
void IDEAgentBridgeWithHotPatching::onModelInvokerReplaced()        this->getModelInvoker()->setEndpoint(endpoint);
{ endpoint << std::endl;
...
}f (m_proxyServer && m_proxyServer->isListening()) {
       stopHotPatchingProxy();
void IDEAgentBridgeWithHotPatching::proxyPortChanged()        startHotPatchingProxy();
{
    if (m_proxyServer && m_proxyServer->isListening()) {
        stopHotPatchingProxy();
        startHotPatchingProxy();ing::ggufEndpointChanged()
    }
}f (m_proxyServer && m_proxyServer->isListening()) {
       stopHotPatchingProxy();
void IDEAgentBridgeWithHotPatching::ggufEndpointChanged()        startHotPatchingProxy();
{
    if (m_proxyServer && m_proxyServer->isListening()) {
        stopHotPatchingProxy();
        startHotPatchingProxy();ing::logCorrection(
    }& correction)
}
   static std::mutex logMutex;
void IDEAgentBridgeWithHotPatching::logCorrection(    std::lock_guard<std::mutex> locker(&logMutex);
    const HallucinationDetection& correction)
{
    static std::mutex logMutex;
    std::lock_guard<std::mutex> locker(&logMutex);/corrections.log", std::ios::app);

    ensureLogDirectory();        return;
...
har buffer[100];
    auto now = std::chrono::system_clock::now();    std::strftime(buffer, sizeof(buffer), "%Y-%m-%dT%H:%M:%SZ", std::gmtime(&now_c));
    std::time_t now_c = std::chrono::system_clock::to_time_t(now);
    
    char buffer[100];       << "Type: " << correction.hallucationType << " | "
    std::strftime(buffer, sizeof(buffer), "%Y-%m-%dT%H:%M:%SZ", std::gmtime(&now_c));ence: " << correction.confidence << " | "

    logFile << buffer << " | "           << "Corrected: " << correction.expectedContent.substr(0, 50) << "\n";
   ...
   static std::mutex logMutex;
void IDEAgentBridgeWithHotPatching::logNavigationFix(    std::lock_guard<std::mutex> locker(logMutex);
    const NavigationFix& fix)
{
    static std::mutex logMutex;
    std::lock_guard<std::mutex> locker(&logMutex);/navigation_fixes.log", std::ios::app);
...
ogFile << std::put_time(std::localtime(&time), "%Y-%m-%d %H:%M:%S") << " | "
    auto now = std::chrono::system_clock::now();           << "From: " << fix.incorrectPath << " | "
    std::time_t now_c = std::chrono::system_clock::to_time_t(now);
...
";           << "Effectiveness: " << std::fixed << std::setprecision(2) << fix.effectiveness << " | "
    logFile << buffer << " | "
           << "From: " << fix.incorrectPath << " | "           << "To: " << fix.correctPath << " | "           << "Effectiveness: " << fix.effectiveness << " | "           << "Reasoning: " << fix.reasoning << "\n";}
