...
// Keep Qt includes if needed for other parts, but we use std for I/O
#include <QDir>
#include <QDateTime>
    ...
    const std::string& dbPath)
{
    ...
    if (jsonPath.ends_with(".db")) {
        jsonPath = jsonPath.substr(0, jsonPath.length() - 3) + ".json";
    }
...
bool IDEAgentBridgeWithHotPatching::startHotPatchingProxy()
{
    ...
    try {
        int proxyPort = stringToInt(m_proxyPort, 11435);
        if (!isValidPort(proxyPort)) throw std::runtime_error("Invalid proxy port");

        if (!isValidEndpoint(m_ggufEndpoint)) throw std::runtime_error("Invalid GGUF endpoint");
...
{
    return m_hotPatchingEnabled && m_hotPatcher && m_proxyServer && m_proxyServer->isListening();
}
...
void IDEAgentBridgeWithHotPatching::logCorrection(const HallucinationDetection& correction)
{
    ...
    logFile << getCurrentTimeStr() << " | "
            ...
            << "Detected: " << truncateString(correction.detectedContent, 50) << " | "
            << "Corrected: " << truncateString(correction.expectedContent, 50) << "\n";
}
