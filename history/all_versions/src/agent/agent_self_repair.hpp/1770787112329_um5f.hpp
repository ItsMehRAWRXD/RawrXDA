// agent_self_repair.hpp — Agent Self-Repair Bridge (MASM64 ↔ C++)
//
// Connects the agentic failure detection pipeline to the MASM64
// self-patching kernel. When the agent detects its own bugs
// (NOP sleds, bad jumps, corrupted vtables, stale function pointers),
// it uses the ASM kernel to scan, patch, verify, and rollback
// fixes on its own running binary — zero dependencies, zero restart.
//
// This proves the 43MB MASM64 binary can fix its own bugs.
//
// Architecture: C++20 bridge → MASM64 kernel
// Threading: mutex-protected, atomic stats
