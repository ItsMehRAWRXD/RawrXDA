...
extern "C" {
    ...
    // --- Telemetry kernel exports (RawrXD_Telemetry_Kernel.asm) ---
    uint64_t UTC_IncrementCounter(volatile uint64_t* counter);
    uint64_t UTC_ReadCounter(volatile uint64_t* counter);
    void     UTC_LogEvent(const char* message);

    // --- Telemetry counter symbols ---
    extern volatile uint64_t g_Counter_MemPatches;
    extern volatile uint64_t g_Counter_Errors;
}
#endif
