...
CorrectionResult RefusalBypassPuppeteer::bypassRefusal(const std::string& refusedResponse, const std::string& originalPrompt) {
    return CorrectionResult::ok("[Bypass] " + refusedResponse, FailureType::RefusalResponse);
}

std::string RefusalBypassPuppeteer::reframePrompt(const std::string& refusedResponse) {
    return "Reframed: " + refusedResponse;
}
...
CorrectionResult HallucinationCorrectorPuppeteer::detectAndCorrectHallucination(const std::string& response, const std::vector<std::string>& knownFacts) {
...
void FormatEnforcerPuppeteer::setRequiredJsonSchema(const std::string& schemaJson) {
    m_requiredSchema = schemaJson;
}
