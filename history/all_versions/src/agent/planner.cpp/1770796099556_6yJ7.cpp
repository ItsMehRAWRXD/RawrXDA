...
// ============================================================================
// planBulkFix — Batch fix / purge / sweep across codebase
// ============================================================================
json Planner::planBulkFix(const std::string& wish) {
    json tasks = json::array();

    std::string target = "all";
    if (ci_contains(wish, "agent")) target = "agent";
    else if (ci_contains(wish, "server")) target = "server";
    else if (ci_contains(wish, "core")) target = "core";
    else if (ci_contains(wish, "telemetry")) target = "telemetry";
    else if (ci_contains(wish, "terminal")) target = "terminal";

    std::string fixType = "general";
    if (ci_contains(wish, "qt") || ci_contains(wish, "purge qt")) fixType = "qt_purge";
    else if (ci_contains(wish, "lint") || ci_contains(wish, "warning")) fixType = "lint";
    else if (ci_contains(wish, "const") || ci_contains(wish, "correctness")) fixType = "const_correctness";
    else if (ci_contains(wish, "format") || ci_contains(wish, "style")) fixType = "format";

    tasks.push_back({
        {"type", "scan_directory"},
        {"target", target},
        {"fixType", fixType},
        {"description", "Scan " + target + " for " + fixType + " issues"}
    });
    ...
    tasks.push_back({
        ...
        {"fixType", fixType},
        {"dryRun", false},
        {"description", "Apply batch fixes across " + target}
    });

    tasks.push_back({{"type", "build"}, {"target", "RawrXD-Shell"}});
    tasks.push_back({{"type", "self_test"}, {"cases", 50}});

    tasks.push_back({
        {"type", "generate_report"},
        {"format", "markdown"},
        {"description", "Generate bulk fix report"}
    });
