...
private:
    ...
    bool detectUnalignedAccess(const std::string& asm, std::string& evidence);
    bool detectMissingStackAlignment(const std::string& asm, std::string& evidence);
    bool detectLeakedRegisters(const std::string& asm, std::string& evidence);
    bool detectMissingVolatile(const std::string& asm, std::string& evidence);
    bool detectIneffientInstruction(const std::string& asm, std::string& evidence);
    
    // x64 calling conventions
    bool validateCallingConvention(const std::string& asm, const std::string& convention);
    bool checkShadowSpace(const std::string& asm);  // x64 Win64 ABI
    bool checkRegisterPreservation(const std::string& asm, const std::vector<std::string>& nonVolatile);
    
    // SIMD/vectorization
    bool detectMissedVectorization(const std::string& code, std::string& suggestion);
    bool detectSuboptimalSIMD(const std::string& asm, std::string& suggestion);
