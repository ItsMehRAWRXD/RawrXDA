...
bool AgentHotPatcher::loadCorrectionPatterns()
{
    ...
    try {
        ...
        if (root["hallucination_patterns"].type() == JsonValue::Type::Object) {
            auto& obj = root["hallucination_patterns"];
            for (auto& kv : obj.objectEntries()) {
                m_hallucationPatterns[kv.first] = kv.second.asString();
            }
        }
        
        // Load navigation patterns
        if (root["navigation_patterns"].type() == JsonValue::Type::Object) {
            auto& obj = root["navigation_patterns"];
            for (auto& kv : obj.objectEntries()) {
                m_navigationPatterns[kv.first] = kv.second.asString();
            }
        }
    } catch (...) {
