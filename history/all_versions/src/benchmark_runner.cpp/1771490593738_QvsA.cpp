...
bool BenchmarkRunner::testMultiLanguage(BenchmarkTestResult& result) {
    std::vector<std::pair<std::string, std::string>> langTests = {
        {"def hello():\n    ", "python"},
        {"function hello() {\n    ", "javascript"},
        {"void hello() {\n    ", "cpp"}
    };
    std::vector<double> latencies;
    size_t successful = 0;
    for (const auto& [prefix, lang] : langTests) {
        if (shouldStop_) return false;
        auto start = std::chrono::high_resolution_clock::now();
        try {
            auto completions = completionEngine_->getCompletions(prefix, "", lang, "");
            if (!completions.empty()) successful++;
            double ms = std::chrono::duration<double, std::milli>(std::chrono::high_resolution_clock::now() - start).count();
            latencies.push_back(ms);
        } catch (...) {}
    }
    result = calculateStats("Multi-Language", latencies, successful, langTests.size());
    return result.passed = (successful > 0);
}

bool BenchmarkRunner::testContextAware(BenchmarkTestResult& result) {
    std::string context = "class Calculator {\npublic:\n    int add(int a, int b) { return a + b; }\n";
    std::string prefix = "    int sub";
    auto start = std::chrono::high_resolution_clock::now();
    try {
        auto completions = completionEngine_->getCompletions(prefix, context, "cpp", "");
        double ms = std::chrono::duration<double, std::milli>(std::chrono::high_resolution_clock::now() - start).count();
        result = calculateStats("Context-Aware", {ms}, completions.empty() ? 0u : 1u, 1);
        return result.passed = !completions.empty();
    } catch (...) {
        result.passed = false;
        return false;
    }
...
bool BenchmarkRunner::testMultiLine(BenchmarkTestResult& result) {
    std::string prefix = "struct Point {\n    int x;\n    int y;\n";
    auto start = std::chrono::high_resolution_clock::now();
    try {
        auto completions = completionEngine_->getCompletions(prefix, "", "cpp", "");
        double ms = std::chrono::duration<double, std::milli>(std::chrono::high_resolution_clock::now() - start).count();
        // Check if completion spans multiple lines
        bool multiLine = false;
        for (const auto& c : completions) {
            if (c.find('\n') != std::string::npos) { multiLine = true; break; }
        }
        result = calculateStats("Multi-Line", {ms}, multiLine ? 1u : 0u, 1);
        return result.passed = !completions.empty();
    } catch (...) {
        result.passed = false;
        return false;
...
bool BenchmarkRunner::testGPUAcceleration(BenchmarkTestResult& result) {
    // Check if GPU is actually available before claiming pass
    bool hasGPU = false;
    if (engine_) {
        size_t vram = engine_->GetVRAMUsage();
        hasGPU = (vram > 0);
    }
    result.passed = hasGPU;
    result.avgLatencyMs = 0.0;
    if (!hasGPU) {
        log("  GPU acceleration: not available (no VRAM detected)", 2);
    }
    return hasGPU;
}
