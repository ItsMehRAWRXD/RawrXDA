...
public:
    ...
    // ---- Compression Support ----
    // Decompresses data using the brutal compression engine
    bool DecompressZone(const std::vector<uint8_t>& compressed, std::vector<uint8_t>& out);
    ...
    bool GetTensorRef(const std::string& tensor_name, TensorRef& out) const;
