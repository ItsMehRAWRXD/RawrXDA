...
std::string WinHttpRequest(const std::string& url,
    ...
    if (!WinHttpCrackUrlW(url_w.c_str(), 0, 0, &parts)) {
        return "";
    }
    ...
    HINTERNET session = WinHttpOpenW(L"RawrXD/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                                    WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (!session) return "";

    HINTERNET connect = WinHttpConnectW(session, host.c_str(), parts.nPort, 0);
    if (!connect) {
    ...
    HINTERNET request = WinHttpOpenRequestW(connect, method.c_str(), path.c_str(), nullptr,
    ...
    for (const auto& header : headers) {
        WinHttpAddRequestHeadersW(request, header.c_str(), -1L, WINHTTP_ADDREQ_FLAG_ADD);
    }
