...
InferenceResult RunRealInference(const std::vector<int>& input_tokens, int max_new_tokens = 1) {
    ...
    // Compute real confidence from logits if available, otherwise estimate from token count
    result.confidence = (input_tokens.size() > 0) ? std::min(1.0f, 0.5f + 0.05f * (float)input_tokens.size()) : 0.0f;
    // Perplexity = exp(avg_neg_log_prob) — estimate from token distribution
    result.perplexity = (input_tokens.size() > 1) ? std::exp(1.0f / (float)input_tokens.size()) : 1.0f;
