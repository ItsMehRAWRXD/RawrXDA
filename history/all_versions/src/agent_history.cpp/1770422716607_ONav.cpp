...
ReplayResult AgentHistoryRecorder::replay(const ReplayRequest& req, SubAgentManager* mgr) {
    ...
    if (hasChain) {
        // Rebuild chain steps from recorded events
        std::vector<ChainStep> steps;
        for (const auto& e : originalEvents) {
            if (e.eventType == "chain_step" || e.eventType == "agent_spawn") {
                ChainStep step;
                step.prompt = e.input.empty() ? e.description : e.input;
                step.role = "replay";
        ...
        if (!steps.empty()) {
            ...
            std::string chainResult = mgr->executeChain(steps);
            
            result.success = true;
            result.result = chainResult;
            result.eventsReplayed = (int)steps.size();
        }
    } else if (hasSwarm) {
        // Rebuild swarm config from recorded events
        SwarmConfig config;
        for (const auto& e : originalEvents) {
            if (e.eventType == "swarm_task" || e.eventType == "agent_spawn") {
                SwarmTask task;
                task.prompt = e.input.empty() ? e.description : e.input;
                task.role = "replay";
        ...
        if (!config.tasks.empty()) {
            ...
            std::string swarmResult = mgr->executeSwarm(config);
            
            result.success = true;
            result.result = swarmResult;
            result.eventsReplayed = (int)config.tasks.size();
        }
        ...
        std::string agentResult = mgr->spawnSubAgent(replayInput, "replay");
