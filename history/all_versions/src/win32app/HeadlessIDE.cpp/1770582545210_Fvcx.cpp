...
std::string HeadlessIDE::routeInferenceRequest(const std::string& prompt) {
    ...
    if (m_engineManager) {
        auto* engine = m_engineManager->GetActiveEngine();
        if (engine) {
            try {
                std::string result = engine->Infer(prompt, m_config.maxTokens, m_config.temperature);
                if (!result.empty()) {
                    return result;
                }
            } catch (...) {
                m_failureDetections++;
                m_outputSink->appendOutput("Engine inference failed, falling back", OutputSeverity::Warning);
            }
        }
    }

    // Fallback: try CodexUltimate if available
    if (m_codexUltimate) {
        try {
            std::string result = m_codexUltimate->Generate(prompt, m_config.maxTokens);
            if (!result.empty()) {
                return result;
            }
        } catch (...) {
...
void HeadlessIDE::recordSimpleEvent(const std::string& description) {
    m_agentEventCount++;
    if (m_historyRecorder) {
        m_historyRecorder->recordEvent(description);
    }
