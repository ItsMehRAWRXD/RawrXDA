...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    // Initialize agentic bridge after window creation
    LOG_INFO("Initializing agentic bridge");
    if (!Win32IDEBridge::instance().initialize(hInstance, nCmdShow)) {
        LOG_WARN("Agentic bridge initialization failed (continuing without agentic features)");
        // Non-fatal—IDE continues without agentic features
    } else {
    ...
    // Shutdown agentic bridge before exit
    LOG_INFO("Shutting down agentic bridge");
