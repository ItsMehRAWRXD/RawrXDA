...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // ========================================================================
    {
        bool licenseOk = EnterpriseLicense::Instance().Initialize();
        if (licenseOk) {
    ...
    // ========================================================================
    {
        auto& sigVerifier = PluginSignatureVerifier::instance();
    ...
    // ========================================================================
    {
        ...
        if (!sandbox.isInitialized()) {
            SandboxResult sbResult = sandbox.initialize();
            if (sbResult.success) {
    ...
    // ========================================================================
    {
        auto& reconciler = SwarmReconciler::instance();
        if (!reconciler.isInitialized()) {
            // Use first 16 bytes of enterprise HWID as node identity
            uint8_t localNodeId[16] = {0};
            const char* hwid = EnterpriseLicense::Instance().GetHardwareHash();
            if (hwid) {
                for (int i = 0; i < 16 && hwid[i * 2] && hwid[i * 2 + 1]; ++i) {
                    ...
                    localNodeId[i] = (uint8_t)((hexVal(hwid[i * 2]) << 4) | hexVal(hwid[i * 2 + 1]));
                }
            }
    ...
    // ========================================================================
    {
        auto& updater = AutoUpdateSystem::instance();
        updater.setCurrentVersion(RAWRXD_VERSION_STR);
        updater.setRepository("RawrXD", "RawrXD-ModelLoader");
        updater.checkForUpdatesAsync([](const UpdateCheckResult& result, void*) {
            if (result.updateAvailable) {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "[main_win32] Update available: %s (current: %s)\n",
    ...
    // ========================================================================
    {
        EnterpriseLicense::Instance().Shutdown();
