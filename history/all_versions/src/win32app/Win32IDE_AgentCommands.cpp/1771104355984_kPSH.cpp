...
// Run Agentic Smoke Test - Verifies end-to-end tool and model assembly
void Win32IDE::onAgentSmokeTest() {
    LOG_INFO("onAgentSmokeTest called");
    ...
    appendToOutput("🛠 Running Agentic Smoke Test...\n", "Output", OutputSeverity::Info);
    appendToOutput("Goal: Verify the model can execute multiple tools (filesystem + shell) to achieve a multi-step objective.\n", "Output", OutputSeverity::Info);

    // Run in background with a fixed cycle count for the smoke test
    std::thread([this, smokePrompt]() {
        DetachedThreadGuard _guard(m_activeDetachedThreads, m_shuttingDown);
        if (_guard.cancelled) return;
        
        // We use 10 cycles for a robust smoke test
        if (m_agenticBridge->StartAgentLoop(smokePrompt, 10)) {
            LOG_INFO("Smoke test agent loop completed");
            appendToOutput("✅ Agentic Smoke Test sequence finished. Check terminal output for verification.\n", "Output", OutputSeverity::Info);
        } else {
            LOG_ERROR("Smoke test agent loop failed");
            appendToOutput("❌ Agentic Smoke Test failed to complete. Check 'Errors' tab for details.\n", "Errors", OutputSeverity::Error);
        }
    }).detach();
}
