...
void Win32IDE::initializeAgenticBridge() {
    ...
    if (!m_agenticBridge) {
        ...
        if (m_agenticBridge->Initialize("", "bigdaddyg-q2k-ultra-productive:latest")) {
...
void Win32IDE::onAgentStartLoop() {
    ...
    // Start agent loop in background thread (cycle count from config)
    int cycleCount = std::clamp(IDEConfig::getInstance().getInt("agent.cycleCount", 10), 1, 50);
    appendToOutput("🚀 Starting Agent Loop (max " + std::to_string(cycleCount) + " cycles): " + promptStr + "\n", "Output", OutputSeverity::Info);
    
    std::thread([this, promptStr, cycleCount]() {
        DetachedThreadGuard _guard(m_activeDetachedThreads, m_shuttingDown);
        if (_guard.cancelled) return;
        if (m_agenticBridge->StartAgentLoop(promptStr, cycleCount)) {
...
void Win32IDE::onAgentExecuteCommand() {
    ...
    if (m_hwndCopilotChatInput) {
        ...
        if (strlen(input) == 0) {
            MessageBoxA(m_hwndMain, "Enter a command in the AI Chat input box", "Agent", MB_OK | MB_ICONINFORMATION);
            return;
        }
        ...
        MessageBoxA(m_hwndMain, "AI Chat input not available", "Agent Error", MB_OK | MB_ICONERROR);
    }
}
...
void Win32IDE::onAgentConfigureModel() {
    ...
    if (availableModels.empty()) {
        availableModels = {
            "RawrXD-Native (Win32)",
            "bigdaddyg-q2k-ultra-productive:latest",
    ...
    if (accepted && !selectedModel.empty()) {
        ...
        std::string apiModel = getResolvedOllamaModel();
        if (!apiModel.empty()) selectedModel = apiModel;
...
// Set agent cycle count (how many times the agent loops when the model is run)
void Win32IDE::onAgentSetCycleCount() {
    int current = std::clamp(IDEConfig::getInstance().getInt("agent.cycleCount", 10), 1, 50);
    struct { int current; int result; bool ok; } state = { current, current, false };
    if (DialogBoxParamA(m_hInstance, "AGENT_PROMPT_DLG", m_hwndMain,
        [](HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) -> INT_PTR {
            auto* s = (decltype(state)*)lp;
            switch (msg) {
                case WM_INITDIALOG: {
                    SetWindowTextA(GetDlgItem(hwnd, 101), "Agent cycle count (1-50). Number of loops when the model is run:");
                    char buf[16];
                    snprintf(buf, sizeof(buf), "%d", s->current);
                    SetDlgItemTextA(hwnd, 102, buf);
                    return TRUE;
                }
                case WM_COMMAND:
                    if (LOWORD(wp) == IDOK) {
                        char buf[32] = {};
                        GetDlgItemTextA(hwnd, 102, buf, (int)sizeof(buf));
                        int v = atoi(buf);
                        s->result = std::clamp(v, 1, 50);
                        s->ok = true;
                        EndDialog(hwnd, IDOK);
                        return TRUE;
                    }
            ...
            return FALSE;
        }, (LPARAM)&state) != IDOK || !state.ok) {
        return;
    }
    IDEConfig::getInstance().setInt("agent.cycleCount", state.result);
    IDEConfig::getInstance().saveToFile("rawrxd.config.json");
    appendToOutput("[Agent] Cycle count set to " + std::to_string(state.result) +
                   " (model will run up to " + std::to_string(state.result) + " cycles per session).\n",
                   "Output", OutputSeverity::Info);
}
...
void Win32IDE::handleAgentCommand(int commandId) {
    ...
    switch (commandId) {
        ...
        case IDM_AGENT_STOP:
            onAgentStop();
            break;
        case IDM_AGENT_SET_CYCLE_COUNT:
            onAgentSetCycleCount();
            break;
        ...
        case IDM_ENGINE_LOAD_800B:
            appendToOutput("800B Load: Use File > Load Model (GGUF) or Agent > Configure Model for model selection.\n", "Output", OutputSeverity::Info);
            onAgentConfigureModel();
            break;
        case IDM_TITAN_TOGGLE:
            // Alias for IDM_AI_TITAN_TOGGLE so menu can use either ID
            m_useTitanKernel = !m_useTitanKernel;
            CheckMenuItem(m_hMenu, IDM_AI_TITAN_TOGGLE, m_useTitanKernel ? MF_CHECKED : MF_UNCHECKED);
            appendToOutput(std::string("Titan Kernel ") + (m_useTitanKernel ? "ENABLED" : "DISABLED") + "\n", "Output", OutputSeverity::Info);
            break;
...
// ============================================================================
// AGENT COMMAND ROUTER (CRITICAL - WIRES ALL AGENT MENU ITEMS)
// ============================================================================
void Win32IDE::handleAgentCommand(int commandId) {
    LOG_FUNCTION();
    LOG_INFO("handleAgentCommand: " + std::to_string(commandId));
    
    switch (commandId) {
        // Agent Core (4100-4109)
        case IDM_AGENT_START_LOOP:
            onAgentStartLoop();
            break;
        ...
        case IDM_AGENT_BOUNDED_LOOP: {
            // Bounded loop with user-specified cycles
            if (!m_agenticBridge) {
                initializeAgenticBridge();
            }
            if (m_agenticBridge && m_agenticBridge->IsInitialized()) {
                int cycles = IDEConfig::getInstance().getInt("agent.cycleCount", 10);
                std::string prompt = "Continue current task with bounded execution";
                appendToOutput("🔄 Starting Bounded Agent Loop (" + std::to_string(cycles) + " cycles)\n", 
                               "Output", OutputSeverity::Info);
                std::thread([this, prompt, cycles]() {
                    DetachedThreadGuard _guard(m_activeDetachedThreads, m_shuttingDown);
                    if (_guard.cancelled) return;
                    m_agenticBridge->StartAgentLoop(prompt, cycles);
                }).detach();
            }
            break;
        }
        ...
        // AI Context Sizes (4210-4216)
        case IDM_AI_CONTEXT_4K:
            onAIContextSize(IDM_AI_CONTEXT_4K);
            break;
            
        case IDM_AI_CONTEXT_32K:
            onAIContextSize(IDM_AI_CONTEXT_32K);
            break;
            
        case IDM_AI_CONTEXT_64K:
            onAIContextSize(IDM_AI_CONTEXT_64K);
            break;
            
        case IDM_AI_CONTEXT_128K:
            onAIContextSize(IDM_AI_CONTEXT_128K);
            break;
            
        case IDM_AI_CONTEXT_256K:
            onAIContextSize(IDM_AI_CONTEXT_256K);
            break;
            
        case IDM_AI_CONTEXT_512K:
            onAIContextSize(IDM_AI_CONTEXT_512K);
            break;
            
        case IDM_AI_CONTEXT_1M:
            onAIContextSize(IDM_AI_CONTEXT_1M);
            break;
        
        // Multi-Agent & Advanced (4217-4225)
        case IDM_AI_AGENT_CYCLES_SET: {
            // Show dialog to set agent cycle count
            char buf[16] = {0};
            sprintf_s(buf, "%d", IDEConfig::getInstance().getInt("agent.cycleCount", 10));
            if (MessageBoxA(m_hwndMain, "Enter agent cycle count (1-50):", "Set Cycles", 
                            MB_OKCANCEL | MB_ICONQUESTION) == IDOK) {
                int cycles = atoi(buf);
                cycles = std::clamp(cycles, 1, 50);
                IDEConfig::getInstance().setInt("agent.cycleCount", cycles);
                IDEConfig::getInstance().save();
                appendToOutput("Agent cycle count set to " + std::to_string(cycles) + "\n", 
                               "Output", OutputSeverity::Info);
            }
            break;
        }
        ...
        case IDM_AI_AGENT_MULTI_ENABLE:
            appendToOutput("🔗 Multi-Agent mode enabled\n", "Output", OutputSeverity::Info);
            // TODO: Hook into UniversalModelRouter to enable ensemble routing
            break;
        ...
        case IDM_AI_AGENT_MULTI_STATUS:
            appendToOutput("📊 Multi-Agent Status: Not yet implemented\n", "Output", OutputSeverity::Info);
            break;
            
        case IDM_AI_TITAN_TOGGLE:
            appendToOutput("⚡ Titan Kernel toggled\n", "Output", OutputSeverity::Info);
            break;
            
        case IDM_AI_800B_STATUS:
            appendToOutput("📊 800B Dual-Engine Status: Not yet implemented\n", "Output", OutputSeverity::Info);
            break;
        
        // Engine Management (4220-4230)
        case IDM_ENGINE_UNLOCK_800B:
            appendToOutput("🔓 Unlocking 800B Dual-Engine capability...\n", "Output", OutputSeverity::Info);
            // TODO: License check + engine initialization
            break;
            
        case IDM_ENGINE_LOAD_800B:
            appendToOutput("📥 Loading 800B Dual-Engine model...\n", "Output", OutputSeverity::Info);
            // TODO: Call model loader with 800B configuration
            break;
        ...
        default:
            LOG_WARNING(__FUNCTION__, "Unhandled agent command: " + std::to_string(commandId));
            appendToOutput("⚠️ Unhandled agent command: " + std::to_string(commandId) + "\n", 
                           "Output", OutputSeverity::Warning);
            break;
    }
}