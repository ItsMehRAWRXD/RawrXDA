...
AgentResponse AgenticBridge::ExecuteAgentCommand(const std::string& prompt) {
    ...
    else if (prompt.rfind("/bugreport ", 0) == 0) {
            std::string code = prompt.substr(11);
            response.content = m_nativeEngine->bugReport(code, "No specific error provided");
            return response;
    }
    else if (prompt.rfind("/suggest ", 0) == 0) {
            std::string code = prompt.substr(9);
            response.content = m_nativeEngine->codeSuggestions(code);
            return response;
    }
    else if (prompt.rfind("/install_vsix ", 0) == 0) {
            std::string path = prompt.substr(14);
            // Trim quotes if present
            if (path.size() >= 2 && path.front() == '"' && path.back() == '"') {
                path = path.substr(1, path.size() - 2);
            }
            if (RawrXD::VsixNativeConverter::ConvertVsixToNative(path, "d:/rawrxd/extensions")) {
                response.content = "VSIX Installed Successfully: " + path;
            } else {
                response.content = "Failed to install VSIX: " + path;
            }
            return response;
    }
...
void AgenticBridge::SetContextSize(const std::string& sizeName) {
    ...
    int tokens = 4096;
    std::string lowerSize = sizeName;
    std::transform(lowerSize.begin(), lowerSize.end(), lowerSize.begin(), ::tolower);
    
    if (lowerSize.find("1m") != std::string::npos) tokens = 1000000;
    else if (lowerSize.find("128k") != std::string::npos) tokens = 128000;
    else if (lowerSize.find("64k") != std::string::npos) tokens = 64000;
    else if (lowerSize.find("32k") != std::string::npos) tokens = 32000;
    else if (lowerSize.find("16k") != std::string::npos) tokens = 16000;
    else if (lowerSize.find("8k") != std::string::npos) tokens = 8000;
    else if (lowerSize.find("4k") != std::string::npos) tokens = 4096;
    
    m_nativeEngine->setContextWindow(tokens);
    LOG_INFO("Context window set to " + std::to_string(tokens) + " tokens");
    if (m_outputCallback) {
        m_outputCallback("Agent", "Context window updated to " + sizeName);
    }
}
void AgenticBridge::SetNoRefusal(bool enabled) {
