...
AgentResponse AgenticBridge::ExecuteAgentCommand(const std::string& prompt) {
    ...
    if (m_nativeAgent && m_nativeEngine && m_nativeEngine->IsModelLoaded()) {
        ...
        if (prompt.find("/bugreport ") == 0) {
             ...
             m_nativeAgent->BugReport(path); // This will call Ask internally
             // We need to capture the output, but BugReport calls Ask which calls Print which calls callback.
             // So output will be streamed.
             return response; // Return empty/dummy, output goes to callback
        }
