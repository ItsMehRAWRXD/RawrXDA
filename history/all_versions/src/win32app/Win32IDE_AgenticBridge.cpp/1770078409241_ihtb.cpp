...
// Static reference to engine to persist across instances if needed
static std::shared_ptr<AgenticEngine> g_agenticEngine = nullptr;
...
bool AgenticBridge::Initialize(const std::string& frameworkPath, const std::string& modelName) {
    ...
    if (!g_agenticEngine) {
        LOG_INFO("Initializing Global AgenticEngine...");
        g_agenticEngine = std::make_shared<AgenticEngine>();
        
        // Wire up CPU Inference (Real)
        auto cpuEngine = RawrXD::CPUInferenceEngine::getInstance();
        g_agenticEngine->setInferenceEngine(cpuEngine.get());
        
        LOG_INFO("AgenticEngine Ready.");
    }
...
AgentResponse AgenticBridge::ExecuteAgentCommand(const std::string& prompt) {
    if (!m_initialized || !g_agenticEngine) {
        return {AgentResponseType::AGENT_ERROR, "Agent not initialized", "", "", "Error: Not Init"};
    }
    ...
    // Defaults
    cfg.temperature = m_maxMode ? 0.7f : 0.8f;
    cfg.max_tokens = m_maxMode ? 32768 : 2048;
    g_agenticEngine->updateConfig(cfg);

    LOG_INFO("ExecuteNative: " + prompt);
    
    // Call the shared AgenticEngine
    std::string response = g_agenticEngine->chat(prompt);

    // Simple Parse
    AgentResponse r;
    r.rawOutput = response;
    r.content = response;
    
    // Check for thinking tags
    if (response.find("<thinking>") != std::string::npos) {
        r.type = AgentResponseType::THINKING;
    } else {
        r.type = AgentResponseType::ANSWER;
    }

    LOG_INFO("Native Response: " + response.substr(0, 100) + "...");
    return r;
}

// Deprecated / Legacy PowerShell Support (Stubbed)
...
// [FEATURE] Hotpatching / AutoCorrect
// Called by GUI when user accepts a bug fix or refinement
void AgenticBridge::ApplyPatch(const std::string& currentFile, const std::string& patchCode) {
    if (!m_ide) return;
    // Assuming patchCode is a replacement for file content or a sed-like diff
    // For now, simpler: Replace file content with patchCode
    // In full implementation, we'd use AgentHotPatcher::applyDiff
    
    // Write via Win32IDE editor
    // This requires Win32IDE to expose a "SetEditorText" method or direct handle access.
    // Since we are friends or have pointer:
    // m_ide->SetEditorText(patchCode); (Conceptual)
    
    LOG_INFO("Applied Patch to " + currentFile);
}

// [FEATURE] Tasks
void AgenticBridge::GenerateReactServer(const std::string& name) {
    if(!g_agenticEngine) return;
    g_agenticEngine->processQuery("/react-server " + name); 
    // The engine handles generation via RawrXD::ReactServerGenerator
}

void AgenticBridge::AnalyzeCode(const std::string& code) {
    ExecuteAgentCommand("/bugreport " + code);
}
