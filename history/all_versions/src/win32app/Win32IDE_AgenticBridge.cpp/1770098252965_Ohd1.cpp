...
AgentResponse AgenticBridge::ExecuteAgentCommand(const std::string& prompt) {
    ...
    // Command Processing for GUI interaction
    // We can't rely on m_nativeAgent specific methods here because we are using AgenticEngine wrapper
    // But AgenticEngine::chat is what we call.
    
    // Check for specific prefixes that map to Agent actions that need file IO
    // The Engine doesn't know about UI/Filesystem by default unless we enhanced it.
    // However, NativeAgent (which Engine uses) DOES have ReadFile helper. 
    // BUT! AgenticEngine::chat creates a LOCAL NativeAgent. It doesn't persist.
    // So we must handle the file reading here in the bridge pass the CONTENT to the engine.
    
    std::string refinedPrompt = prompt;
    
    if (prompt.find("/bugreport ") == 0) {
        std::string path = prompt.substr(11);
        // We need to read the file content here and wrapping it
        // Or we rely on the Engine to implement these commands.
        // Let's implement reading here for safety as Bridge has FS access context.
        std::ifstream f(path);
        if (f) {
            std::stringstream buffer; buffer << f.rdbuf();
            refinedPrompt = "Analyze the following code for bugs, security vulnerabilities, and logic errors.\n\nCode:\n" + buffer.str();
        } else {
        ...
        if (f) {
            std::stringstream buffer; buffer << f.rdbuf();
            refinedPrompt = "Provide suggestions to improve the following code (performance, readability, style).\n\nCode:\n" + buffer.str();
        } else {
             return {AgentResponseType::ANSWER, "Error: Could not read file " + path};
        }
    }
    else if (prompt.find("/patch ") == 0) { // Hotpatch
        std::string path = prompt.substr(7);
        std::ifstream f(path);
        if (f) {
             std::stringstream buffer; buffer << f.rdbuf();
             refinedPrompt = "Review the following code for hallucinations, invalid paths, and logical contradictions. Rewrite the code to fix these issues immediately.\n\nCode:\n" + buffer.str();
        } else {
             return {AgentResponseType::ANSWER, "Error: Could not read file " + path};
        }
    }
...
// === DYNAMIC REACT GENERATOR TOOL ===
// This allows the agent to generate new React/Web components and apps
    
if (prompt.find("generate_react") == 0) {
   // "generate_react type=component name=MyComponent features=..."
   std::string args = prompt.substr(14);
   
   RawrXD::ReactServerConfig config;
   config.name = "GeneratedApp"; // Default, could be parsed
   config.description = "Dynamically generated by Agent";
   
   // Simple parser for args (key=value)
   std::string type = "app";
   std::string name = "MyGeneratedApp";
   
   std::stringstream ss(args);
   std::string segment;
   while(std::getline(ss, segment, ' ')) {
       if (segment.find("type=") == 0) type = segment.substr(5);
       if (segment.find("name=") == 0) name = segment.substr(5);
       if (segment.find("auth=true") == 0) config.include_auth = true;
       if (segment.find("db=true") == 0) config.include_database = true;
       if (segment.find("ts=true") == 0) config.include_typescript = true;
   }
   
   config.name = name;
   
   bool success = false;
   std::string outputDir = "generated/" + name;
   
   if (type == "app") {
       success = RawrXD::ReactServerGenerator::Generate(outputDir, config);
   } else if (type == "component") {
       // If we just want a component, we might need a dedicated method in generator
       // For now, let's assume generating the whole app structure to get the components
       success = RawrXD::ReactServerGenerator::Generate(outputDir, config); 
   }
   
   if (success) {
       return {AgentResponseType::ANSWER, "React App Generated at: " + outputDir};
   } else {
       return {AgentResponseType::AGENT_ERROR, "Failed to generate React App"};
   }
}

// === END DYNAMIC REACT GENERATOR TOOL ===
...
// === AUTONOMOUS TOOLS BRIDGE ===
// These tools allow the AutonomyManager to interact with the system via Win32IDE
