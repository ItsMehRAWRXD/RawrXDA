...
AgentResponse AgenticBridge::ExecuteAgentCommand(const std::string& prompt) {
    ...
    // ... other toggles handled by bridge setters, but we need to reflect in response

    if (prompt.find("/plan ") == 0) {
        output = m_nativeEngine->planTask(prompt.substr(6));
        // Auto-execute check
        response.content = output;
        // If plan is valid JSON, we might want to execute it:
        // response.content += "\n\nExecuting...\n" + m_nativeEngine->executePlan(output);
    } 
    else if (prompt.find("/edit ") == 0) {
        // Direct edit request
        output = m_nativeEngine->chat(prompt); 
    }
    else {
        // Chat / Ask / etc
        output = m_nativeEngine->chat(prompt);
...
// Replaced by AgenticEngine logic above
            }
            return response; 
    }
    else if (prompt == "/max") {
            SetMaxMode(true);
            m_nativeEngine->updateConfig({1.0f,0.9f,2048,40, m_maxMode, m_deepThinking, m_deepResearch, m_noRefusal}); 
            // Simplified update
            m_outputCallback("", "Max Mode Enabled");
            return response;
    }
