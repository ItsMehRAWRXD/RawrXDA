                ...
                for (const auto& entry : std::filesystem::directory_iterator(jsExtDir, ec)) {
                    if (entry.is_directory()) {
                        ...
                        if (std::filesystem::exists(manifestPath)) {
                            ...
                            // Parse the package.json into a manifest struct
                            vscode::VSCodeExtensionManifest extManifest;
                            if (jsHost.parsePackageJson(entry.path(), extManifest) &&
                                jsHost.validateManifest(extManifest)) {
                                // Register with the C++ API bridge first
                                api.loadExtension(&extManifest);
                                // Then load into QuickJS runtime
                                auto extResult = jsHost.loadJSExtension(
                                    extManifest.id.c_str(), extPath.c_str(), &extManifest);
                                if (extResult.success) {
                                    appendToOutput("[Phase 36] Loaded JS extension: " + extManifest.id +
                                                   " v" + extManifest.version + "\r\n");
                                } else {
                                    appendToOutput("[Phase 36] Failed to load JS extension '" +
                                                   extManifest.id + "': " + extResult.detail + "\r\n");
                                }
                            } else {
                                appendToOutput("[Phase 36] Invalid manifest in: " + extPath + "\r\n");
                            }
                        }
                    }
                }
