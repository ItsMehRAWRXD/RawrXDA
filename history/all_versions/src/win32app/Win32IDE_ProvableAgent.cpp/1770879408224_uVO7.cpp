...
static std::string hmacSha256(const std::string& key, const std::string& msg) {
    ...
    // Simplified: concatenate key+msg and hash (production would use proper HMAC)
...
static ReplayProof generateReplayProof(const CustodyChain& original) {
    ...
    for (auto& att : original.attestations) {
        ...
        if (recomputedSig != att.signature && proof.divergenceStep == -1) {
    ...
    proof.deterministic = (proof.replayHash == proof.originalHash);
...
static LRESULT CALLBACK provablePanelWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        case IDC_PROVE_START:
            PostMessageW(hwndParent, WM_COMMAND, IDM_PROVABLE_START, 0);
            break;
        case IDC_PROVE_VERIFY:
            PostMessageW(hwndParent, WM_COMMAND, IDM_PROVABLE_VERIFY, 0);
            break;
        case IDC_PROVE_REPLAY:
            PostMessageW(hwndParent, WM_COMMAND, IDM_PROVABLE_REPLAY, 0);
            break;
        case IDC_PROVE_EXPORT:
            PostMessageW(hwndParent, WM_COMMAND, IDM_PROVABLE_EXPORT, 0);
            break;
        case IDC_PROVE_RESET:
            PostMessageW(hwndParent, WM_COMMAND, IDM_PROVABLE_RESET, 0);
            break;
        }
        return 0;
    }
...
void Win32IDE::cmdProvableRecord() {
    std::string state = "manual_record_step_" + std::to_string(s_stepCounter.load());
    recordStep("tool_call", state, state + "_output", "[transcript]");
    ...
    appendToOutput("[ProvableAgent] Step recorded and attested.\n");
}
...
void Win32IDE::cmdProvableVerify() {
    ...
    for (auto& att : s_currentChain.attestations) {
        std::string expected = hmacSha256("rawrxd-provable-key-v1",
            att.inputHash + att.outputHash + att.transcriptHash + att.timestamp);
        att.verified = (expected == att.signature);
