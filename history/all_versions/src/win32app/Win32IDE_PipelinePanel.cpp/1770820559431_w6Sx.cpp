...
void Win32IDE::cmdPipelineListNodes() {
    auto& orch = DistributedPipelineOrchestrator::instance();
    auto nodes = orch.getComputeNodes();
    ...
    for (auto& node : nodes) {
        oss << "  ▸ " << node.name
            ...
            << "  cores=" << node.coreCount
            << "  load=" << std::fixed << std::setprecision(1) << node.currentLoad
...
void Win32IDE::cmdPipelineAddNode() {
    ...
    node.port     = 8080;
    node.coreCount = 8;
    node.affinity  = NodeAffinity::CPU;

    auto& orch = DistributedPipelineOrchestrator::instance();
    auto result = orch.registerComputeNode(node);
    if (result.success) {
        appendToOutput("[Pipeline] Node registered: " + node.name + "\n");
    } else {
...
void Win32IDE::cmdPipelineDAGView() {
    ...
    auto pipelines = orch.listPipelines();
    if (pipelines.empty()) {
        oss << "║  No active pipelines.                     ║\n";
    } else {
        for (auto& name : pipelines) {
            oss << "║  Pipeline: " << std::left << std::setw(30) << name << "║\n";
        }
    }
    oss << "╚═══════════════════════════════════════════╝\n";
    appendToOutput(oss.str());
