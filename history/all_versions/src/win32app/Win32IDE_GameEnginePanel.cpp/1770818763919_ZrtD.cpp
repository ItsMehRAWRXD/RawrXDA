...
void Win32IDE::initGameEngines() {
    if (m_gameEnginesInitialized) return;

    m_gameEngineManager = std::make_unique<GameEngineManager>();

    // Set up log callback to route engine messages to IDE output
    m_gameEngineManager->setLogCallback([](const char* message, int severity, GameEngineType engine) {
        // This uses the static instance pattern — log goes to IDE output
        std::string prefix;
        switch (engine) {
            case GameEngineType::Unity:  prefix = "[Unity] ";  break;
            case GameEngineType::Unreal: prefix = "[Unreal] "; break;
            default:                     prefix = "[Engine] "; break;
        }
...
void Win32IDE::handleGameEngineCommand(int commandId) {
    ...
    switch (commandId) {
        // Unified commands
        case IDM_GAME_ENGINE_DETECT:          cmdGameEngineDetect();          break;
        case IDM_GAME_ENGINE_OPEN:            cmdGameEngineOpen();            break;
        case IDM_GAME_ENGINE_CLOSE:           cmdGameEngineClose();           break;
        case IDM_GAME_ENGINE_STATUS:          cmdGameEngineStatus();          break;
        case IDM_GAME_ENGINE_BUILD:           cmdGameEngineBuild();           break;
        case IDM_GAME_ENGINE_PLAY:            cmdGameEnginePlay();            break;
        case IDM_GAME_ENGINE_STOP:            cmdGameEngineStop();            break;
        case IDM_GAME_ENGINE_PAUSE:           cmdGameEnginePause();           break;
        case IDM_GAME_ENGINE_COMPILE:         cmdGameEngineCompile();         break;
        case IDM_GAME_ENGINE_PROFILER_START:  cmdGameEngineProfilerStart();   break;
        case IDM_GAME_ENGINE_PROFILER_STOP:   cmdGameEngineProfilerStop();    break;
        case IDM_GAME_ENGINE_PROFILER_SNAP:   cmdGameEngineProfilerSnapshot(); break;
        case IDM_GAME_ENGINE_DEBUG_START:     cmdGameEngineDebugStart();      break;
        case IDM_GAME_ENGINE_DEBUG_STOP:      cmdGameEngineDebugStop();       break;
        case IDM_GAME_ENGINE_BREAKPOINT:      cmdGameEngineBreakpoint();      break;
        case IDM_GAME_ENGINE_AI_SUMMARY:      cmdGameEngineAISummary();       break;
        case IDM_GAME_ENGINE_AI_SCENE:        cmdGameEngineAIScene();         break;
        case IDM_GAME_ENGINE_INSTALLATIONS:   cmdGameEngineInstallations();   break;
        case IDM_GAME_ENGINE_HELP:            cmdGameEngineHelp();            break;

        // Unity-specific
        case IDM_UNITY_CREATE_SCRIPT:         cmdUnityCreateScript();         break;
        case IDM_UNITY_SCENE_LIST:            cmdUnitySceneList();            break;
        case IDM_UNITY_ASSET_BROWSER:         cmdUnityAssetBrowser();         break;
        case IDM_UNITY_PACKAGE_MANAGER:       cmdUnityPackageManager();       break;

        // Unreal-specific
        case IDM_UNREAL_CREATE_CLASS:         cmdUnrealCreateClass();         break;
        case IDM_UNREAL_CREATE_MODULE:        cmdUnrealCreateModule();        break;
        case IDM_UNREAL_CREATE_PLUGIN:        cmdUnrealCreatePlugin();        break;
        case IDM_UNREAL_GEN_PROJECT_FILES:    cmdUnrealGenProjectFiles();     break;
        case IDM_UNREAL_LIVE_CODING:          cmdUnrealLiveCoding();          break;
        case IDM_UNREAL_COOK_CONTENT:         cmdUnrealCookContent();         break;
        case IDM_UNREAL_PACKAGE_PROJECT:      cmdUnrealPackageProject();      break;
        case IDM_UNREAL_LEVEL_LIST:           cmdUnrealLevelList();           break;
        case IDM_UNREAL_BLUEPRINT_LIST:       cmdUnrealBlueprintList();       break;
...
void Win32IDE::showGameEngineStatusDialog() {
    if (!m_gameEngineManager) return;

    std::string status = m_gameEngineManager->getStatusString();

    MessageBoxA(m_hwnd, status.c_str(), "Game Engine Status — RawrXD IDE",
...
void Win32IDE::cmdGameEngineDetect() {
    ...
    if (result.isValid) {
        std::string msg = "Detected: " + result.detail + "\nVersion: " + result.version +
                          "\nPath: " + result.projectPath;
        appendToOutput("[GameEngine] " + msg + "\n");

        // Ask if user wants to open
        int answer = MessageBoxA(m_hwnd, (msg + "\n\nOpen this project?").c_str(),
                                  "Engine Detected", MB_YESNO | MB_ICONQUESTION);
        if (answer == IDYES) {
        ...
        appendToOutput("[GameEngine] No game engine project detected at: " +
                       std::string(path) + "\n");
        MessageBoxA(m_hwnd, "No Unity or Unreal Engine project found at this location.",
                     "Not Found", MB_OK | MB_ICONWARNING);
    }
}
...
void Win32IDE::cmdGameEngineOpen() {
    ...
    if (m_gameEngineManager->openProject(path)) {
        ...
        switch (m_gameEngineManager->getActiveEngine()) {
            case GameEngineType::Unity:  engineName = "Unity";  break;
            case GameEngineType::Unreal: engineName = "Unreal"; break;
            default:                     engineName = "Unknown"; break;
        }
        appendToOutput("[GameEngine] Opened " + engineName + " project: " +
                       m_gameEngineManager->getActiveProjectName() + "\n");
    } else {
...
void Win32IDE::cmdGameEngineBuild() {
    ...
    std::thread([this, request]() {
        ...
        PostMessageA(m_hwnd, WM_APP + 200, 0, 0);  // signal UI refresh
        appendToOutput(msg);
...
void Win32IDE::cmdGameEnginePause() {
    ...
    if (m_gameEngineManager->getActiveEngine() == GameEngineType::Unity) {
        ...
        if (unity && unity->isPlayModePaused()) {
...
void Win32IDE::cmdUnitySceneList() {
    ...
    for (const auto& scene : scenes) {
        std::string status;
        if (scene.isLoaded) status = " [loaded]";
        if (scene.isDirty) status += " [modified]";
...
void Win32IDE::cmdUnityAssetBrowser() {
    ...
    for (const auto& [type, count] : typeCounts) {
        ...
        switch (static_cast<UnityAssetType>(type)) {
            case UnityAssetType::Scene:         typeName = "Scenes";      break;
            case UnityAssetType::Prefab:        typeName = "Prefabs";     break;
            case UnityAssetType::Script:        typeName = "Scripts";     break;
            case UnityAssetType::Shader:        typeName = "Shaders";     break;
            case UnityAssetType::Material:      typeName = "Materials";   break;
            case UnityAssetType::Texture:       typeName = "Textures";    break;
            case UnityAssetType::Model:         typeName = "Models";      break;
            case UnityAssetType::AudioClip:     typeName = "Audio";       break;
            case UnityAssetType::Animation:     typeName = "Animations";  break;
            case UnityAssetType::AnimatorCtrl:  typeName = "Animators";   break;
            case UnityAssetType::Font:          typeName = "Fonts";       break;
            case UnityAssetType::ScriptableObj: typeName = "ScriptableObjects"; break;
...
void Win32IDE::cmdUnityPackageManager() {
    ...
    for (const auto& pkg : packages) {
        appendToOutput("  " + pkg.first + " @ " + pkg.second + "\n");
    }
}
