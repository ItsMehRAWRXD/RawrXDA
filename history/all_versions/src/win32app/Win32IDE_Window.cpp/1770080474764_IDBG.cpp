...
LRESULT Win32IDE::handleMessage(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        ...
        case WM_HSCROLL:
            // Handle slider movements for Max Tokens
            if ((HWND)lParam == m_hwndMaxTokensSlider) {
                int pos = (int)SendMessage(m_hwndMaxTokensSlider, TBM_GETPOS, 0, 0);
                onMaxTokensChanged(pos);

                // Update Bridge config
                if (m_agentBridge) {
                    AgenticBridge* bridge = reinterpret_cast<AgenticBridge*>(m_agentBridge);
                    bridge->SetMaxTokens(pos);
                    
                    // Auto-toggle Max Mode if high enough?
                    // bridge->SetMaxMode(pos > 8192); 
                }
            }
            return 0;
