...
void Win32IDE::updateVoiceStatusBar()
{
    ...
    if (g_voiceChatInitialized && g_voiceChat) {
        if (g_voiceChat->isRecording()) {
            ...
            clr = RGB(255, 60, 60);
        } else if (g_voiceChat->isProcessing()) {
            stateText = "\xE2\x8F\xB3 Processing...";       // ⏳ Processing...
            clr = RGB(255, 200, 0);
        } else {
...
void Win32IDE::voiceSavePreferences()
{
    ...
    if (g_voiceChat) {
        ptt = g_voiceChat->isPushToTalk();
        autoTranscribe = g_voiceChat->isAutoTranscribe();
        sensitivity = g_voiceChat->getSensitivity();
        deviceId = g_voiceChat->getInputDeviceId();
    }
...
void Win32IDE::voiceLoadPreferences()
{
    ...
    if (g_voiceChat) {
        g_voiceChat->setPushToTalk(ptt);
        g_voiceChat->setAutoTranscribe(autoTranscribe);
        g_voiceChat->setSensitivity(sensitivity);
        g_voiceChat->setInputDeviceId(deviceId);
    }
