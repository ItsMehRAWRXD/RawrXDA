...
// ============================================================================
// Request body: {"path":"D:/rawrxd/src/main.cpp","content":"..."}
// Response: {"success":true,"path":"...","size":1234}
// Security: Rejects ".." traversal. Requires absolute path.
// ============================================================================

void Win32IDE::handleWriteFileEndpoint(SOCKET client, const std::string& body) {
    std::string filePath;
    LocalServerUtil::extractJsonString(body, "path", filePath);
    if (filePath.empty()) {
        std::string resp = LocalServerUtil::buildHttpResponse(400,
            "{\"error\":\"missing_path\",\"message\":\"Request body must contain 'path' field\"}");
        send(client, resp.c_str(), (int)resp.size(), 0);
    ...
    std::string content;
    LocalServerUtil::extractJsonString(body, "content", content);

    // Normalize
    for (auto& ch : filePath) { if (ch == '/') ch = '\\'; }
    ...
    if (filePath.size() < 3 || filePath[1] != ':' || filePath[2] != '\\') {
        std::string resp = LocalServerUtil::buildHttpResponse(400,
            "{\"error\":\"invalid_path\",\"message\":\"Only absolute paths accepted\"}");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    }
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        DWORD err = GetLastError();
        std::string resp = LocalServerUtil::buildHttpResponse(500,
            "{\"error\":\"write_failed\",\"message\":\"Cannot create/write file: " +
            LocalServerUtil::escapeJson(filePath) + "\",\"win32_error\":" + std::to_string(err) + "}");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    }
    ...
    BOOL ok = WriteFile(hFile, content.c_str(), (DWORD)content.size(), &bytesWritten, NULL);
    CloseHandle(hFile);

    if (!ok) {
        std::string resp = LocalServerUtil::buildHttpResponse(500,
            "{\"error\":\"write_failed\",\"message\":\"WriteFile failed\"}");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    }

    std::ostringstream json;
    json << "{\"success\":true,\"path\":\"" << LocalServerUtil::escapeJson(filePath)
         << "\",\"size\":" << bytesWritten << "}";
    std::string resp = LocalServerUtil::buildHttpResponse(200, json.str());
    send(client, resp.c_str(), (int)resp.size(), 0);
    LOG_INFO("write-file: " + filePath + " (" + std::to_string(bytesWritten) + " bytes)");
}

// ============================================================================
// POST /api/list-dir — List directory contents for file tree
// ============================================================================
// Request body: {"path":"D:/rawrxd/src"}
// Response: {"entries":[{"name":"main.cpp","type":"file","size":1234},...],"path":"..."}
// ============================================================================

void Win32IDE::handleListDirEndpoint(SOCKET client, const std::string& body) {
    std::string dirPath;
    LocalServerUtil::extractJsonString(body, "path", dirPath);
    if (dirPath.empty()) dirPath = ".";

    // Normalize
    for (auto& ch : dirPath) { if (ch == '/') ch = '\\'; }
    ...
    // Ensure trailing backslash for FindFirstFile
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        DWORD err = GetLastError();
        std::string resp = LocalServerUtil::buildHttpResponse(404,
            "{\"error\":\"dir_not_found\",\"message\":\"Cannot list: " +
            LocalServerUtil::escapeJson(dirPath) + "\",\"win32_error\":" + std::to_string(err) + "}");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    }
    ...
    do {
        ...
        DWORD sizeLow = fd.nFileSizeLow;

        if (!first) json << ",";
        first = false;
        json << "{\"name\":\"" << LocalServerUtil::escapeJson(name)
             << "\",\"type\":\"" << (isDir ? "dir" : "file")
             << "\",\"size\":" << (isDir ? 0 : (int)sizeLow)
             << "}";
    } while (FindNextFileA(hFind, &fd));
    FindClose(hFind);

    json << "],\"path\":\"" << LocalServerUtil::escapeJson(dirPath) << "\"}";
    std::string resp = LocalServerUtil::buildHttpResponse(200, json.str());
    send(client, resp.c_str(), (int)resp.size(), 0);
}
...
// ============================================================================
// Request body: {"path":"D:/rawrxd/temp/old.txt"}
// ============================================================================
...
void Win32IDE::handleDeleteFileEndpoint(SOCKET client, const std::string& body) {
    ...
    if (attrs == INVALID_FILE_ATTRIBUTES) {
        std::string resp = LocalServerUtil::buildHttpResponse(404,
            ...
            LocalServerUtil::escapeJson(filePath) + "\"}");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    }
    ...
    if (attrs & FILE_ATTRIBUTE_DIRECTORY) {
        ok = RemoveDirectoryA(filePath.c_str());
    } else {
        ok = DeleteFileA(filePath.c_str());
    }

    if (!ok) {
        DWORD err = GetLastError();
        std::string resp = LocalServerUtil::buildHttpResponse(500,
            "{\"error\":\"delete_failed\",\"win32_error\":" + std::to_string(err) + "}");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    }

    std::string resp = LocalServerUtil::buildHttpResponse(200,
        "{\"success\":true,\"deleted\":\"" + LocalServerUtil::escapeJson(filePath) + "\"}");
    send(client, resp.c_str(), (int)resp.size(), 0);
    LOG_INFO("delete-file: " + filePath);
}

// ============================================================================
// POST /api/rename-file — Rename or move a file
// ============================================================================
// Request body: {"old_path":"D:/rawrxd/a.txt","new_path":"D:/rawrxd/b.txt"}
// ============================================================================

void Win32IDE::handleRenameFileEndpoint(SOCKET client, const std::string& body) {
    std::string oldPath, newPath;
    LocalServerUtil::extractJsonString(body, "old_path", oldPath);
    LocalServerUtil::extractJsonString(body, "new_path", newPath);

    if (oldPath.empty() || newPath.empty()) {
        std::string resp = LocalServerUtil::buildHttpResponse(400,
            "{\"error\":\"missing_fields\",\"message\":\"Need 'old_path' and 'new_path'\"}");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    }

    for (auto& ch : oldPath) { if (ch == '/') ch = '\\'; }
    for (auto& ch : newPath) { if (ch == '/') ch = '\\'; }

    if (oldPath.find("..") != std::string::npos || newPath.find("..") != std::string::npos) {
        std::string resp = LocalServerUtil::buildHttpResponse(403,
            "{\"error\":\"forbidden\"}");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    ...
    if (!MoveFileA(oldPath.c_str(), newPath.c_str())) {
        DWORD err = GetLastError();
        std::string resp = LocalServerUtil::buildHttpResponse(500,
            "{\"error\":\"rename_failed\",\"win32_error\":" + std::to_string(err) + "}");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    }

    std::string resp = LocalServerUtil::buildHttpResponse(200,
        "{\"success\":true,\"old_path\":\"" + LocalServerUtil::escapeJson(oldPath) +
        "\",\"new_path\":\"" + LocalServerUtil::escapeJson(newPath) + "\"}");
    send(client, resp.c_str(), (int)resp.size(), 0);
    LOG_INFO("rename-file: " + oldPath + " -> " + newPath);
}

// ============================================================================
// POST /api/mkdir — Create a directory (recursive)
// ============================================================================
// Request body: {"path":"D:/rawrxd/src/new_module"}
// ============================================================================
...
void Win32IDE::handleMkdirEndpoint(SOCKET client, const std::string& body) {
    ...
    for (size_t i = 0; i < dirPath.size(); i++) {
        ...
        if (dirPath[i] == '\\' || i == dirPath.size() - 1) {
            if (built.size() > 2 && built[1] == ':') { // skip "D:"
                CreateDirectoryA(built.c_str(), NULL); // ignore errors on existing dirs
            }
        }
    }
    ...
    if (attrs == INVALID_FILE_ATTRIBUTES || !(attrs & FILE_ATTRIBUTE_DIRECTORY)) {
        DWORD err = GetLastError();
        std::string resp = LocalServerUtil::buildHttpResponse(500,
            "{\"error\":\"mkdir_failed\",\"win32_error\":" + std::to_string(err) + "}");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    }

    std::string resp = LocalServerUtil::buildHttpResponse(200,
        "{\"success\":true,\"path\":\"" + LocalServerUtil::escapeJson(dirPath) + "\"}");
    send(client, resp.c_str(), (int)resp.size(), 0);
    LOG_INFO("mkdir: " + dirPath);
}

// ============================================================================
// POST /api/search-files — Search for text pattern in files
// ============================================================================
// Request body: {"pattern":"TODO","path":"D:/rawrxd/src","file_pattern":"*.cpp","max_results":100}
// Response: {"results":[{"file":"...","line":42,"text":"// TODO: fix this"},...]}
// ============================================================================

void Win32IDE::handleSearchFilesEndpoint(SOCKET client, const std::string& body) {
    std::string pattern, basePath, filePattern;
    LocalServerUtil::extractJsonString(body, "pattern", pattern);
    LocalServerUtil::extractJsonString(body, "path", basePath);
    LocalServerUtil::extractJsonString(body, "file_pattern", filePattern);
    int maxResults = 200;
    LocalServerUtil::extractJsonInt(body, "max_results", maxResults);
    ...
    // Search: iterate directory, read files, find matches
    std::ostringstream json;
    json << "{\"results\":[";
    int count = 0;

    // Simple recursive search using a stack
    std::vector<std::string> dirs;
    dirs.push_back(basePath);

    while (!dirs.empty() && count < maxResults) {
        std::string curDir = dirs.back();
        dirs.pop_back();

        std::string searchStr = curDir + "\\*";
        WIN32_FIND_DATAA fd;
        HANDLE hFind = FindFirstFileA(searchStr.c_str(), &fd);
        ...
        do {
            ...
            if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                ...
                if (name == ".git" || name == "build" || name == "node_modules" || name == ".vs") continue;
                dirs.push_back(fullPath);
            } else {
                // Check file pattern (simple wildcard match for extension)
                bool matchesFilter = (filePattern == "*.*" || filePattern == "*");
                if (!matchesFilter) {
                    // Check extension match: e.g. "*.cpp"
                ...
                if (fSize == INVALID_FILE_SIZE || fSize > 2 * 1024 * 1024) { // 2MB max per file
                    CloseHandle(hFile);
                    continue;
                }
                ...
                DWORD bytesRead = 0;
                ReadFile(hFile, &contents[0], fSize, &bytesRead, NULL);
                CloseHandle(hFile);
                ...
                std::string line;
                int lineNum = 0;
                while (std::getline(stream, line) && count < maxResults) {
                    lineNum++;
                    if (line.find(pattern) != std::string::npos) {
                        if (count > 0) json << ",";
                        // Trim line for output (max 200 chars)
                        std::string trimmed = line;
                        if (trimmed.size() > 200) trimmed = trimmed.substr(0, 200) + "...";
                        json << "{\"file\":\"" << LocalServerUtil::escapeJson(fullPath)
                             << "\",\"line\":" << lineNum
                             << ",\"text\":\"" << LocalServerUtil::escapeJson(trimmed) << "\"}";
                        count++;
                    }
                }
            }
        } while (FindNextFileA(hFind, &fd) && count < maxResults);
        FindClose(hFind);
    }

    json << "],\"count\":" << count
         << ",\"pattern\":\"" << LocalServerUtil::escapeJson(pattern)
         << "\",\"path\":\"" << LocalServerUtil::escapeJson(basePath) << "\"}";

    std::string resp = LocalServerUtil::buildHttpResponse(200, json.str());
    send(client, resp.c_str(), (int)resp.size(), 0);
    LOG_INFO("search-files: '" + pattern + "' in " + basePath + " — " + std::to_string(count) + " results");
}
