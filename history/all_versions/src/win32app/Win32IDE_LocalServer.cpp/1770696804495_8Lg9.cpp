...
void Win32IDE::handleOpenAIChatCompletions(SOCKET client, const std::string& body) {
    ...
    if (activeBackend != AIBackendType::LocalGGUF) {
        ...
        if (stream) {
            ...
            sendAll(client, eventStr);
            std::string doneStr = "data: [DONE]\n\n";
            sendAll(client, doneStr);
        } else {
            std::ostringstream j;
            j << "{\"id\":\"" << requestId
    ...
    if (stream) {
        ...
        for (const auto& tok : generated) {
            ...
            std::string eventStr = event.str();
            bool r = sendAll(client, eventStr);
            if (!r) return;
        }
    ...
    for (const char* candidate : candidates) {
        ...
        if (hFile != INVALID_HANDLE_VALUE) {
            ...
            if (fileSize != INVALID_FILE_SIZE && fileSize > 0) {
                ...
                // Send as HTML — use sendAll() to guarantee the entire
                ...
                if (!sendAll(client, headers)) return;
                sendAll(client, content.c_str(), (int)content.size());
                return;
            }
            CloseHandle(hFile);
        }
