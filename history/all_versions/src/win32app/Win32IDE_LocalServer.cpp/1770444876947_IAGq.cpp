...
void Win32IDE::handleLocalServerClient(SOCKET clientFd) {
    ...
    // ========== Auto-Pin, Heatmap Persistence, Ensemble Delta ==========
    else if (method == "GET" && path == "/api/router/auto-pin") {
        handleRouterAutoPinEndpoint(client);
        closesocket(client);
        return;
    }
    else if (method == "POST" && path == "/api/router/auto-pin/apply") {
        handleRouterAutoPinApplyEndpoint(client, body);
        closesocket(client);
        return;
    }
    else if (method == "POST" && path == "/api/router/heatmap/save") {
        handleRouterHeatmapSaveEndpoint(client);
        closesocket(client);
        return;
    }
    else if (method == "GET" && path == "/api/router/ensemble/delta") {
        handleRouterEnsembleDeltaEndpoint(client);
        closesocket(client);
        return;
    }
    // ========== Phase 9A: LSP Client ==========
    ...
    // ========== Phase 9A: ASM Semantic ==========
    else if (method == "GET" && path.find("/api/asm/symbols") == 0) {
    ...
    else if (method == "GET" && path == "/api/multi-response/results") {
        handleMultiResponseResultsEndpoint(client);
        closesocket(client);
        return;
    }
...
std::string Win32IDE::getLocalServerStatus() const {
    ...
    oss << "  GET  /api/router/why           — Explain last routing decision\r\n";
    oss << "  GET  /api/router/pins          — Show all task pins\r\n";
    oss << "  GET  /api/router/heatmap       — Cost/latency heatmap grid\r\n";
    oss << "  POST /api/router/ensemble      — Ensemble routing (enable/test)\r\n";
    oss << "  POST /api/router/simulate      — Offline routing simulation\r\n";
    oss << "  GET  /api/router/auto-pin      — Auto-pin suggestions from heatmap\r\n";
    oss << "  POST /api/router/auto-pin/apply — Apply auto-pin suggestions\r\n";
    oss << "  POST /api/router/heatmap/save  — Save heatmap data to disk\r\n";
    oss << "  GET  /api/router/ensemble/delta — Ensemble response diff analysis\r\n";
    oss << "  GET  /api/lsp/status           — LSP server status\r\n";
    oss << "  GET  /api/lsp/diagnostics      — LSP diagnostics summary\r\n";
    oss << "  GET  /api/asm/symbols          — ASM symbol table & stats\r\n";
    oss << "  POST /api/asm/navigate         — Navigate to ASM symbol / find refs\r\n";
    oss << "  POST /api/asm/analyze          — Analyze ASM block (convention, registers)\r\n";
    oss << "  GET  /api/multi-response/status — Multi-response engine status\r\n";
    oss << "  GET  /api/multi-response/templates — List response templates\r\n";
    oss << "  POST /api/multi-response/generate  — Generate chain responses\r\n";
    oss << "  GET  /api/multi-response/results   — Latest session results\r\n";
    oss << "  POST /api/multi-response/prefer    — Set preferred response\r\n";
    oss << "  GET  /api/multi-response/stats     — Stats & preference breakdown\r\n";
