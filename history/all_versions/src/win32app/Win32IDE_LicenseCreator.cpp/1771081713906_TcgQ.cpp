...
static bool createV2LicenseInline(RawrXD::License::LicenseTierV2 tier,
    ...
    const char* secret = std::getenv("RAWRXD_LICENSE_SECRET");
    if (!secret || !*secret) secret = "RawrXD-Dev-Secret-2026";
...
LRESULT Win32IDE::LicenseCreatorWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            CreateWindowExW(0, L"EDIT", L"",
                WS_CHILD | WS_VISIBLE | ES_MULTILINE | ES_READONLY | WS_VSCROLL | WS_BORDER,
                12, y, 510, 220, hwnd, (HMENU)(UINT_PTR)IDC_LC_FEATURES_V2, nullptr, nullptr);
        ...
        case WM_COMMAND: {
            ...
            if (id == IDC_LC_V2_GEN_PRO && ide) {
                if (createV2LicenseInline(RawrXD::License::LicenseTierV2::Professional, 180, nullptr)) {
                    ide->appendToOutput("[License] V2 Pro license (180 days) created and installed.\n", "Output", OutputSeverity::Info);
                    refreshLicenseUI(hwnd);
                } else {
            ...
            if (id == IDC_LC_V2_GEN_ENT && ide) {
                if (createV2LicenseInline(RawrXD::License::LicenseTierV2::Enterprise, 365, nullptr)) {
                    ide->appendToOutput("[License] V2 Enterprise license (365 days) created and installed.\n", "Output", OutputSeverity::Info);
                    refreshLicenseUI(hwnd);
                } else {
            ...
            if (id == IDC_LC_V2_GEN_SOV && ide) {
                if (createV2LicenseInline(RawrXD::License::LicenseTierV2::Sovereign, 365, nullptr)) {
                    ide->appendToOutput("[License] V2 Sovereign license (365 days) created and installed.\n", "Output", OutputSeverity::Info);
                    refreshLicenseUI(hwnd);
                } else {
