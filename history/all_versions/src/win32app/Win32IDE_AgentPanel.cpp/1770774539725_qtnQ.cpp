...
void Win32IDE::agentAcceptHunk(size_t fileIndex, size_t hunkIndex) {
    if (!s_agentSession) return;
    if (s_agentSession->AcceptHunk(fileIndex, hunkIndex)) {
...
void Win32IDE::agentRejectHunk(size_t fileIndex, size_t hunkIndex) {
    if (!s_agentSession) return;
    if (s_agentSession->RejectHunk(fileIndex, hunkIndex)) {
...
void Win32IDE::agentAcceptAll() {
    ...
    // Save provenance log
    std::string logDir = m_settings.workingDirectory + "/.rawrxd/agent_logs/";
