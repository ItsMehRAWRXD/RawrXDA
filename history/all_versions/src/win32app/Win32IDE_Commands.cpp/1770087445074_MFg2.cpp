...
bool Win32IDE::routeCommand(int commandId) {
    ...
    if (commandId >= 1000 && commandId < 2000) {
        ...
        return true;
    } else if (commandId >= 4100 && commandId < 4300) {
...
void Win32IDE::handleAgentCommand(int commandId) {
    switch (commandId) {
        ...
        case IDM_REVENG_DUMPBIN: {
            if (m_agenticBridge && !m_currentFile.empty()) {
                std::string res = m_agenticBridge->RunDumpbin(m_currentFile, "headers");
                appendToOutput(res, "Dumpbin", OutputSeverity::Info);
            }
            break;
        }
        case IDM_REVENG_ANALYZE: {
            if (m_agenticBridge && !m_currentFile.empty()) {
                std::string res = m_agenticBridge->RunCodex(m_currentFile);
                appendToOutput(res, "Codex", OutputSeverity::Info);
            }
            break;
        }
        case IDM_REVENG_COMPILE: {
            if (m_agenticBridge && !m_currentFile.empty()) {
                std::string res = m_agenticBridge->RunCompiler(m_currentFile);
                appendToOutput(res, "Compiler", OutputSeverity::Info);
            }
            break;
        }
