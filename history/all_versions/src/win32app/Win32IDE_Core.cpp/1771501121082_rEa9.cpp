...
// ============================================================================
void Win32IDE::onDestroy() {
    OutputDebugStringA("[onDestroy] Starting IDE shutdown sequence...\n");
    
    // Shutdown Circular Beacon System first
    if (m_circularBeaconManager) {
        OutputDebugStringA("[onDestroy] Shutting down CircularBeaconManager...\n");
        try {
            m_circularBeaconManager->shutdownSystem();
            m_circularBeaconManager.reset();
            OutputDebugStringA("[onDestroy] CircularBeaconManager shut down successfully\n");
        } catch (const std::exception& e) {
            OutputDebugStringA(("[onDestroy] CircularBeaconManager shutdown error: " + std::string(e.what()) + "\n").c_str());
        }
    }
    
    // Continue with other cleanup...
    OutputDebugStringA("[onDestroy] IDE shutdown complete\n");
}
