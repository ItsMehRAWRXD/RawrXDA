...
LRESULT Win32IDE::handleMessage(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_TIMER:
        ...
        if (wParam == HOVER_TIMER_ID) { // Hover documentation tooltip
            onHoverTimer();
            return 0;
        }
    ...
    // Hover documentation tooltip ready
    case WM_HOVER_READY: {
        std::lock_guard<std::mutex> lock(m_lspMutex);
        onHoverReady(m_hoverState.content, m_hoverState.line, m_hoverState.column);
        return 0;
    }

    // Signature help ready
    case WM_SIGHELP_READY: {
        int commaCount = (int)wParam;
        m_sigHelpState.activeParameter = commaCount;
...
void Win32IDE::onCreate(HWND hwnd) {
    ...
    // ── Tier 2 Cosmetic Features ───────────────────────────────────────
    OutputDebugStringA("[onCreate] Tier 2 cosmetics init...\n");
    initDiffView();
    initTerminalTabs();
    initHoverTooltips();
    initSignatureHelp();
    ...
    initCodeLens();
    initInlayHints();
    OutputDebugStringA("[onCreate] Tier 2 cosmetics OK\n");
...
void Win32IDE::onDestroy() {
    ...
    // ── Tier 2 Cosmetic Features Shutdown ──────────────────────────────
    shutdownDiffView();
    shutdownTerminalTabs();
    shutdownHoverTooltips();
    shutdownSignatureHelp();
    shutdownReferencesPanel();
