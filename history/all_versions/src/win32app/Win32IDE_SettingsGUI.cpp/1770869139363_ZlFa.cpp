...
void Win32IDE::buildSettingsSchema()
{
    ...
    // General
    {
        ...
        cat.name = "General";
        cat.keys = {
            "autoSave", "autoSaveInterval", "lineNumbers", "wordWrap",
    ...
    // Editor
    {
        ...
        cat.name = "Editor";
        cat.keys = {
            "tabSize", "useSpaces", "encoding", "eolStyle",
            "syntaxColoring", "minimap", "smoothScroll", "caretAnimation",
            "breadcrumbs"
        };
        m_settingsSchema.push_back(cat);
    }
...
static std::string getSettingType(const std::string& key) {
    ...
    if (key == "fontSize" || key == "tabSize" || key == "autoSaveInterval" ||
        key == "aiTopK" || key == "aiMaxTokens" || key == "aiContextWindow" ||
        key == "failureMaxRetries" || key == "localServerPort" ||
        key == "windowAlpha" || key == "themeId" || key == "uiScalePercent") {
        return "int";
    }
...
std::string Win32IDE_GetSettingValue(const IDESettings& s, const std::string& key) {
    if (key == "autoSave") return s.autoSaveEnabled ? "true" : "false";
    if (key == "autoSaveInterval") return std::to_string(s.autoSaveIntervalSec);
    if (key == "lineNumbers") return s.lineNumbersVisible ? "true" : "false";
    if (key == "wordWrap") return s.wordWrapEnabled ? "true" : "false";
    if (key == "fontSize") return std::to_string(s.fontSize);
    if (key == "fontName") return s.fontName;
    if (key == "workingDirectory") return s.workingDirectory;
    if (key == "tabSize") return std::to_string(s.tabSize);
    if (key == "useSpaces") return s.useSpaces ? "true" : "false";
    if (key == "encoding") return s.encoding;
    if (key == "eolStyle") return s.eolStyle;
    if (key == "syntaxColoring") return s.syntaxColoringEnabled ? "true" : "false";
    if (key == "minimap") return s.minimapEnabled ? "true" : "false";
    if (key == "smoothScroll") return s.smoothScrollEnabled ? "true" : "false";
    if (key == "caretAnimation") return s.caretAnimationEnabled ? "true" : "false";
    if (key == "breadcrumbs") return s.breadcrumbsEnabled ? "true" : "false";
    if (key == "aiTemperature") { char b[32]; snprintf(b, 32, "%.2f", s.aiTemperature); return b; }
    if (key == "aiTopP") { char b[32]; snprintf(b, 32, "%.2f", s.aiTopP); return b; }
    if (key == "aiTopK") return std::to_string(s.aiTopK);
    if (key == "aiMaxTokens") return std::to_string(s.aiMaxTokens);
    if (key == "aiContextWindow") return std::to_string(s.aiContextWindow);
    if (key == "aiModelPath") return s.aiModelPath;
    if (key == "aiOllamaUrl") return s.aiOllamaUrl;
    ...
    if (key == "localServer") return s.localServerEnabled ? "true" : "false";
    if (key == "localServerPort") return std::to_string(s.localServerPort);
    if (key == "autoUpdateCheck") return s.autoUpdateCheckEnabled ? "true" : "false";
    if (key == "updateChannel") return s.updateChannel;
    if (key == "showWelcomeOnStartup") return s.showWelcomeOnStartup ? "true" : "false";
    return "";
}
...
static void Win32IDE_SetSettingValue(IDESettings& s, const std::string& key, const std::string& value) {
    ...
    auto toInt = [](const std::string& v) { try { return std::stoi(v); } catch(...) { return 0; } };
    auto toFloat = [](const std::string& v) { try { return std::stof(v); } catch(...) { return 0.0f; } };

    if (key == "autoSave") s.autoSaveEnabled = toBool(value);
    else if (key == "autoSaveInterval") s.autoSaveIntervalSec = toInt(value);
    else if (key == "lineNumbers") s.lineNumbersVisible = toBool(value);
    else if (key == "wordWrap") s.wordWrapEnabled = toBool(value);
    else if (key == "fontSize") s.fontSize = toInt(value);
    else if (key == "fontName") s.fontName = value;
    else if (key == "workingDirectory") s.workingDirectory = value;
    else if (key == "tabSize") s.tabSize = toInt(value);
    else if (key == "useSpaces") s.useSpaces = toBool(value);
    else if (key == "encoding") s.encoding = value;
    else if (key == "eolStyle") s.eolStyle = value;
    else if (key == "syntaxColoring") s.syntaxColoringEnabled = toBool(value);
    else if (key == "minimap") s.minimapEnabled = toBool(value);
    else if (key == "smoothScroll") s.smoothScrollEnabled = toBool(value);
    else if (key == "caretAnimation") s.caretAnimationEnabled = toBool(value);
    else if (key == "breadcrumbs") s.breadcrumbsEnabled = toBool(value);
    else if (key == "aiTemperature") s.aiTemperature = toFloat(value);
    else if (key == "aiTopP") s.aiTopP = toFloat(value);
    else if (key == "aiTopK") s.aiTopK = toInt(value);
    else if (key == "aiMaxTokens") s.aiMaxTokens = toInt(value);
    else if (key == "aiContextWindow") s.aiContextWindow = toInt(value);
    else if (key == "aiModelPath") s.aiModelPath = value;
    else if (key == "aiOllamaUrl") s.aiOllamaUrl = value;
    ...
    else if (key == "localServer") s.localServerEnabled = toBool(value);
    else if (key == "localServerPort") s.localServerPort = toInt(value);
    else if (key == "autoUpdateCheck") s.autoUpdateCheckEnabled = toBool(value);
    else if (key == "updateChannel") s.updateChannel = value;
    else if (key == "showWelcomeOnStartup") s.showWelcomeOnStartup = toBool(value);
}
...
LRESULT CALLBACK Win32IDE::SettingsGUIProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
void Win32IDE::showSettingsGUI()
{
    ...
    m_hwndSettingsGUI = CreateWindowExA(
        WS_EX_TOOLWINDOW,
...
void Win32IDE::createSettingsControls(HWND hwndParent)
{
    ...
    // Search bar at top
    HWND searchLabel = CreateWindowExA(0, "STATIC", "  Search settings:",
        WS_CHILD | WS_VISIBLE | SS_LEFT | SS_CENTERIMAGE,
        10, 10, 120, 24, hwndParent, nullptr, m_hInstance, nullptr);
    SendMessage(searchLabel, WM_SETFONT, (WPARAM)hFont, TRUE);

    m_hwndSettingsSearch = CreateWindowExA(
        WS_EX_CLIENTEDGE, "EDIT", "",
        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
        135, 10, 590, 24,
        hwndParent, (HMENU)IDC_SETTINGS_SEARCH, m_hInstance, nullptr);
    SendMessage(m_hwndSettingsSearch, WM_SETFONT, (WPARAM)hFont, TRUE);

    // Tab control for categories
    m_hwndSettingsTabs = CreateWindowExA(
        0, WC_TABCONTROLA, "",
    ...
    m_hwndSettingsPanel = CreateWindowExA(
        WS_EX_CLIENTEDGE, "STATIC", "",
        WS_CHILD | WS_VISIBLE | WS_VSCROLL,
        10, 75, 715, 440,
        hwndParent, (HMENU)IDC_SETTINGS_PANEL, m_hInstance, nullptr);

    // Bottom buttons
    HWND hApply = CreateWindowExA(0, "BUTTON", "Apply && Save",
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        440, 525, 100, 28,
        hwndParent, (HMENU)IDC_SETTINGS_APPLY, m_hInstance, nullptr);
    HWND hReset = CreateWindowExA(0, "BUTTON", "Reset Defaults",
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        550, 525, 100, 28,
        hwndParent, (HMENU)IDC_SETTINGS_RESET, m_hInstance, nullptr);
...
{
    if (categoryIndex < 0 || categoryIndex >= (int)m_settingsSchema.size()) return;
    if (!m_hwndSettingsPanel) return;
    ...
    HFONT hDescFont = CreateFontA(-11, 0, 0, 0, FW_NORMAL, TRUE, FALSE, FALSE,
        DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_SWISS, "Segoe UI");
    ...
    for (int k = 0; k < (int)cat.keys.size(); k++) {
        ...
        // Label
        HWND hLabel = CreateWindowExA(0, "STATIC", label.c_str(),
        ...
        if (!desc.empty()) {
            HWND hDesc = CreateWindowExA(0, "STATIC", desc.c_str(),
                WS_CHILD | WS_VISIBLE | SS_LEFT,
                10, yPos, 680, 16,
                m_hwndSettingsPanel, nullptr, m_hInstance, nullptr);
        ...
        if (type == "bool") {
            HWND hCheck = CreateWindowExA(0, "BUTTON", "",
                WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
                320, yPos - (desc.empty() ? 20 : 38), 24, 20,
                m_hwndSettingsPanel, (HMENU)(UINT_PTR)controlId, m_hInstance, nullptr);
            ...
            HWND hEdit = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", value.c_str(),
                ...
                320, yPos - (desc.empty() ? 20 : 38), 120, 22,
                m_hwndSettingsPanel, (HMENU)(UINT_PTR)controlId, m_hInstance, nullptr);
            ...
            HWND hEdit = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", value.c_str(),
                WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
                320, yPos - (desc.empty() ? 20 : 38), 360, 22,
                m_hwndSettingsPanel, (HMENU)(UINT_PTR)controlId, m_hInstance, nullptr);
            SendMessage(hEdit, WM_SETFONT, (WPARAM)hFont, TRUE);
        }
...
void Win32IDE::filterSettings(const std::string& query)
{
    ...
    for (const auto& cat : m_settingsSchema) {
        for (const auto& key : cat.keys) {
            ...
            if (lowerLabel.find(lowerQuery) != std::string::npos ||
                lowerKey.find(lowerQuery) != std::string::npos) {
                ...
                HWND hLabel = CreateWindowExA(0, "STATIC", headerText.c_str(),
                    WS_CHILD | WS_VISIBLE | SS_LEFT,
                    10, yPos, 300, 18,
                    m_hwndSettingsPanel, nullptr, m_hInstance, nullptr);
                SendMessage(hLabel, WM_SETFONT, (WPARAM)hFont, TRUE);
                ...
                if (type == "bool") {
                    ...
                    HWND hEdit = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", value.c_str(),
                        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
                        320, yPos - 22, (type == "string") ? 360 : 120, 22,
                        m_hwndSettingsPanel, (HMENU)(UINT_PTR)controlId, m_hInstance, nullptr);
                    SendMessage(hEdit, WM_SETFONT, (WPARAM)hFont, TRUE);
                }
    ...
    if (yPos == 10) {
        // No matches
        HWND hNoMatch = CreateWindowExA(0, "STATIC", "  No settings match your search.",
            WS_CHILD | WS_VISIBLE | SS_LEFT,
            10, 20, 400, 20,
            m_hwndSettingsPanel, nullptr, m_hInstance, nullptr);
