    ...
    std::vector<HybridCompletionItem> lspItems;
    {
        ...
        if (lang < LSPLanguage::Count && getLSPServerState(lang) == LSPServerState::Running) {
            ...
            if (response.contains("result") && response["result"].is_array()) {
                for (auto it = response["result"].begin(); it != response["result"].end(); ++it) {
                    ...
                    if (it->contains("label") && (*it)["label"].is_string())
                        item.label = (*it)["label"].get<std::string>();
                    if (it->contains("detail") && (*it)["detail"].is_string())
                        item.detail = (*it)["detail"].get<std::string>();
                    if (it->contains("insertText") && (*it)["insertText"].is_string())
                        item.insertText = (*it)["insertText"].get<std::string>();
                    else
            ...
            if (response.contains("result") && response["result"].is_object() &&
                ...
                for (auto it = response["result"]["items"].begin();
                     it != response["result"]["items"].end(); ++it) {
                    HybridCompletionItem item;
                    if (it->contains("label") && (*it)["label"].is_string())
                        item.label = (*it)["label"].get<std::string>();
                    if (it->contains("detail") && (*it)["detail"].is_string())
    ...
    if (isAsm) {
        ...
        for (const auto* sym : symbols) {
            ...
            switch (sym->kind) {
                ...
                case AsmSymbolKind::Constant:   item.detail = "ASM Constant";  break;
                case AsmSymbolKind::DataLabel:  item.detail = "ASM Data";      break;
    ...
    const std::string& filePath)
{
    ...
    // ── Step 1: Gather LSP diagnostics ──────────────────────────────────
    {
        ...
        for (const auto& d : lspDiags) {
            ...
            hd.line       = d.range.startLine;
            hd.character  = d.range.startChar;
    ...
    if (isAsm) {
        ...
        auto allRefs = findAsmSymbolReferences("*");
        for (const auto& ref : allRefs) {
            if (ref.filePath == filePath) {
                const auto* sym = findAsmSymbol(ref.symbolName);
                if (!sym) {
                    HybridDiagnostic hd;
                    hd.filePath   = filePath;
                    hd.line       = ref.line;
                    ...
                    hd.message    = "Possibly undefined symbol: " + ref.symbolName;
                    hd.source     = "asm";
                    results.push_back(std::move(hd));
                }
            }
    ...
    if (isAsm) {
        ...
        for (const auto* sym : symbols) {
            ...
            switch (sym->kind) {
                ...
                case AsmSymbolKind::DataLabel: dataCount++;  break;
                default: break;
            }
        }
...
std::vector<Win32IDE::LSPProfileRecommendation> Win32IDE::autoSelectLSPProfile() {
    ...
    static const LangMapping mappings[] = {
        ...
        { LSPLanguage::Go,         "Go",         "gopls",               goExts },
        { LSPLanguage::Rust,       "Rust",       "rust-analyzer",       rsExts },
        { LSPLanguage::Assembly,   "Assembly",   "asm-lsp (built-in)",  asmExts },
        { LSPLanguage::CSharp,     "C#",         "omnisharp",           csExts },
        { LSPLanguage::Java,       "Java",       "jdtls",              javaExts },
    };
    ...
    for (const auto& mapping : mappings) {
        ...
        if (fileCount > 0) {
            ...
            rec.isInstalled = (getLSPServerState(mapping.lang) != LSPServerState::NotConfigured);
    ...
    if (isAsm) {
        ...
        if (asmSym) {
            ...
            switch (asmSym->kind) {
                ...
                case AsmSymbolKind::Constant:   result.kind = "constant";  break;
                case AsmSymbolKind::DataLabel:  result.kind = "data";      break;
    ...
    if (!isAsm || result.referenceCount == 0) {
        ...
        if (lang < LSPLanguage::Count && getLSPServerState(lang) == LSPServerState::Running) {
            ...
            if (!defs.empty()) {
                ...
                result.definitionLine = defs[0].range.startLine + 1;
            ...
            for (const auto& ref : refs) {
                ...
                int refLine = ref.range.startLine + 1;
    ...
    if (isAsm) {
        ...
        if (sym) {
            ...
            switch (sym->kind) {
                ...
                case AsmSymbolKind::Constant:  context += "Constant";       break;
                case AsmSymbolKind::DataLabel: context += "Data Label";     break;
