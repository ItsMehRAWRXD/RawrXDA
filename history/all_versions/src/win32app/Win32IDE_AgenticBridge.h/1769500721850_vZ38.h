...
public:
    ...
    bool InstallMCPTransportHooks();
    void UninstallMCPTransportHooks();
    bool AreMCPHooksInstalled() const { return m_mcpHooksInstalled; }
    void SetMCPReadCallback(ReadMessageCallback callback);
    void SetMCPWriteCallback(WriteMessageCallback callback);
    std::vector<MCPMessage> GetRecentMCPMessages(size_t count = 100) const;
