...
void Win32IDE::cmdSABuildCFG() {
    ...
    oss << "[StaticAnalysis] CFG Status:\n"
        << "  Basic Blocks: " << cfg.blocks.size() << "\n"
        << "  Entry Block:  " << (cfg.entryBlockId >= 0 ? std::to_string(cfg.entryBlockId) : "not set") << "\n";

    if (cfg.blocks.empty()) {
        oss << "  Load instructions first, then build the CFG.\n";
    } else {
...
void Win32IDE::cmdSAComputeDominators() {
    auto& engine = StaticAnalysisEngine::instance();
    auto result = engine.computeDominators();
...
void Win32IDE::cmdSAConvertSSA() {
    auto& engine = StaticAnalysisEngine::instance();
    auto result = engine.convertToSSA();
...
void Win32IDE::cmdSAFullAnalysis() {
    ...
    auto result = engine.runFullAnalysis();
...
void Win32IDE::cmdSAExportDOT() {
    ...
    if (GetSaveFileNameA(&ofn)) {
        auto result = engine.exportCFGDot(filePath);
        if (result.success) {
...
void Win32IDE::cmdSAExportJSON() {
    ...
    if (GetSaveFileNameA(&ofn)) {
        auto result = engine.exportCFGJson(filePath);
        if (result.success) {
