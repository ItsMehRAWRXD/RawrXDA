...
void Win32IDE::initializePowerShell() {
    LOG_FUNCTION();

    std::vector<std::string> modulesToLoad = {
        "SecurityFramework.psm1",
        "language_model_registry.psm1",
        "plugin_craft_room.psm1",
        "PerformanceFramework.psm1",
        "ModuleLifecycleManager.psm1"
    };

    for (const auto& moduleFile : modulesToLoad) {
        std::string modulePath = getScriptPath(moduleFile);
        if (importPowerShellModule(modulePath)) {
            logInfo("PowerShell module loaded: " + moduleFile);
        } else {
            logError(__FUNCTION__, "Failed to load PowerShell module: " + moduleFile);
        }
    }
}

std::string Win32IDE::getScriptPath(const std::string& scriptName) {
    char exePath[MAX_PATH] = {0};
    DWORD len = GetModuleFileNameA(nullptr, exePath, MAX_PATH);
    std::string dir = (len > 0) ? std::string(exePath, len) : std::string(".\\");
...
std::string Win32IDE::runPowerShellModuleCommand(const std::string& moduleFile, const std::string& command) {
    std::string modulePath = getScriptPath(moduleFile);
    std::string psCommand = "Import-Module -Name '" + escapePowerShellString(modulePath) + "' -Force; " + command;
    return executePowerShellCommand(psCommand, false);
}

std::string Win32IDE::runPowerShellModuleTableCommand(const std::string& moduleFile, const std::string& command) {
    std::string tableCommand = command + " | Format-Table -AutoSize | Out-String";
    return runPowerShellModuleCommand(moduleFile, tableCommand);
}
...
std::string Win32IDE::invokeModuleFunction(const std::string& moduleName, 
                                           const std::string& functionName,
                                           const std::vector<std::string>& args) {
    std::string moduleFile = moduleName + ".psm1";
    std::string argsStr;
    
    if (!args.empty()) {
        for (size_t i = 0; i < args.size(); ++i) {
            if (i > 0) argsStr += ", ";
            argsStr += "'" + escapePowerShellString(args[i]) + "'";
        }
    }
    ...
    return runPowerShellModuleCommand(moduleFile, command);
}

std::string Win32IDE::getModuleInventoryFormatted() {
    std::string header = 
        "┌─────────────────────────────────────────────────────────────────┐\n"
        "│                     MODULE INVENTORY                            │\n"
        "└─────────────────────────────────────────────────────────────────┘\n\n";
    
    std::string result = runPowerShellModuleTableCommand("ModuleLifecycleManager.psm1",
        "Get-ModuleInventory | Format-Table @{Label='Module';Expression={$_.Name}}, "
        "@{Label='Modified';Expression={$_.LastWriteTime.ToString('yyyy-MM-dd HH:mm')}}, "
        "@{Label='Size (KB)';Expression={[math]::Round((Get-Item $_.Path).Length/1KB, 2)}} -AutoSize");
    
    return header + result;
}

std::string Win32IDE::createPluginFromTemplate(const std::string& pluginName,
                                               const std::string& templateType) {
    std::vector<std::string> args = {pluginName, templateType};
    ...
    std::string command = "New-CraftRoomPlugin -Name '" + escapePowerShellString(pluginName) + 
                         "' -Type '" + escapePowerShellString(templateType) + "'";
    
    std::string result = runPowerShellModuleCommand("plugin_craft_room.psm1", command);
    
    return header + result;
}

std::string Win32IDE::getPerformanceMetrics() {
    std::string header = 
        "\n⚡ PERFORMANCE METRICS\n"
        "══════════════════════════════════════════════\n\n";
    
    std::string command = 
        "if (Get-Command Get-PerformanceMetrics -ErrorAction SilentlyContinue) { "
        "Get-PerformanceMetrics | Format-Table Metric,Value,Unit -AutoSize | Out-String "
        "} else { "
        "'⚠️  Performance metrics not available. Load PerformanceFramework module first.' "
        "}";
    
    std::string result = runPowerShellModuleCommand("PerformanceFramework.psm1", command);
    
    return header + result;
}

std::string Win32IDE::runSecurityAudit() {
    std::string header = 
        "\n🔐 SECURITY AUDIT\n"
        "══════════════════════════════════════════════\n\n";
    
    std::string command = 
        "if (Get-Command Invoke-SecurityAudit -ErrorAction SilentlyContinue) { "
        "Invoke-SecurityAudit | Format-Table Category,Status,Severity -AutoSize | Out-String "
        "} else { "
        "'⚠️  Security audit function not available. Checking module...'; "
        "Get-Command -Module SecurityFramework | Select-Object -ExpandProperty Name | Out-String "
        "}";
    
    std::string result = runPowerShellModuleCommand("SecurityFramework.psm1", command);
    
    return header + result;
}

std::string Win32IDE::formatPowerShellTable(const std::string& data, const std::string& title) {
    std::string formatted = "\n╔";
    for (size_t i = 0; i < title.length() + 4; ++i) formatted += "═";
    formatted += "╗\n║ " + title + " ║\n╚";
    for (size_t i = 0; i < title.length() + 4; ++i) formatted += "═";
    formatted += "╝\n\n";
