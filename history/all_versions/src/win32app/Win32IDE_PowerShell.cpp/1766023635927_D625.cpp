...
bool Win32IDE::loadRawrXDPowerShellModule() {
    ...
    if (result.find("ERROR") == std::string::npos) {
        ...
        // Load VS Code Extension Manager module
        loadVSCodeExtensionModule();
        ...
        // Cache VS Code extension functions
        m_rawrXDFunctions["Get-VSCodeExtensionStatus"] = "VS Code extension status";
        m_rawrXDFunctions["Search-VSCodeMarketplace"] = "Search VS Code marketplace";
        m_rawrXDFunctions["Install-VSCodeExtension"] = "Install VS Code extension";
        m_rawrXDFunctions["Uninstall-VSCodeExtension"] = "Uninstall VS Code extension";
        m_rawrXDFunctions["Load-VSCodeExtension"] = "Load VS Code extension";
        m_rawrXDFunctions["Get-VSCodeExtensionInfo"] = "Get VS Code extension info";
...
// ============================================================================
// VS CODE EXTENSION MODULE INTEGRATION
// ============================================================================

bool Win32IDE::loadVSCodeExtensionModule() {
    // The VS Code Extension manager is a PowerShell module located at
    // d:\RawrXD\Powershield\VSCodeExtensionManager.psm1
    std::string modulePath = "d:\\RawrXD\\Powershield\\VSCodeExtensionManager.psm1";
    if (!fileExists(modulePath)) {
        return false;
    }
    std::string command = ". '" + modulePath + "'";
    std::string result = executePowerShellCommand(command, false);
    if (result.find("ERROR") == std::string::npos) {
        m_powerShellVSCodeExtensionLoaded = true;
        return true;
    }
    return false;
}

std::string Win32IDE::invokeVSCodeExtensionCommand(const std::string& commandName, const std::vector<std::string>& args) {
    // Build PowerShell command string with arguments escaped
    std::string cmd = commandName;
    for (const auto& a : args) {
        cmd += " \"" + escapePowerShellString(a) + "\"";
    }
    return executePowerShellCommand(cmd, false);
}
...
std::string Win32IDE::invokeRawrXDFunction(const std::string& functionName, 
    ...
    if (functionName == "Get-VSCodeExtensionStatus") {
        return invokeVSCodeExtensionCommand("Get-VSCodeExtensionStatus", args);
    }
    else if (functionName == "Search-VSCodeMarketplace") {
        return invokeVSCodeExtensionCommand("Search-VSCodeMarketplace", args);
    }
    else if (functionName == "Install-VSCodeExtension") {
        return invokeVSCodeExtensionCommand("Install-VSCodeExtension", args);
    }
    else if (functionName == "Uninstall-VSCodeExtension") {
        return invokeVSCodeExtensionCommand("Uninstall-VSCodeExtension", args);
    }
    else if (functionName == "Load-VSCodeExtension") {
        return invokeVSCodeExtensionCommand("Load-VSCodeExtension", args);
    }
    else if (functionName == "Get-VSCodeExtensionInfo") {
        return invokeVSCodeExtensionCommand("Get-VSCodeExtensionInfo", args);
    }
    
    // Handle regular RawrXD functions
...
bool Win32IDE::loadVSCodeExtensionModule() {
    std::string extensionsPath = getExecutableDirectory() + "\\extensions";
    std::string modulePath = extensionsPath + "\\VSCodeExtensionManager.psm1";
    ...
    std::string command = "Import-Module '" + modulePath + "' -Force -DisableNameChecking";
    std::string result = executePowerShellCommand(command, false);
    ...
    if (success) {
        // Verify module is loaded by testing a command
        std::string testResult = executePowerShellCommand("Get-Command Get-VSCodeExtensionStatus -ErrorAction SilentlyContinue", false);
        success = !testResult.empty() && testResult.find("ERROR") == std::string::npos;
    }
    
    return success;
}

std::string Win32IDE::invokeVSCodeExtensionCommand(const std::string& command, const std::vector<std::string>& args) {
    std::string fullCommand = command;
    
    // Build command with parameters
    for (size_t i = 0; i < args.size(); i += 2) {
        if (i + 1 < args.size()) {
            fullCommand += " -" + args[i] + " '" + escapePowerShellString(args[i + 1]) + "'";
        }
    }
    
    return executePowerShellCommand(fullCommand, false);
}

std::string Win32IDE::getExecutableDirectory() {
    char path[MAX_PATH];
...
bool Win32IDE::fileExists(const std::string& filePath) {
