...
void Win32IDE::cmdTelExportOTLP() {
    ...
    if (path.empty()) return;
    auto& exporter = RawrXD::Telemetry::TelemetryExporter::Instance();
    auto result = exporter.ExportToFile(path, RawrXD::Telemetry::ExportFormat::OTLP_JSON);
    if (result.success)
        MessageBoxA(m_hwndMain, "OTLP JSON exported.", "Export", MB_OK);
    else
        MessageBoxA(m_hwndMain, result.error.c_str(), "Export Error", MB_ICONERROR);
}
...
void Win32IDE::cmdTelExportAutoStart() {
    initTelemetryExport();
    auto& exporter = RawrXD::Telemetry::TelemetryExporter::Instance();
    exporter.StartAutoExport(60); // 60 second interval
...
void Win32IDE::cmdComposerNewSession() {
    ...
    auto& composer = RawrXD::Agentic::AgenticComposerUX::Instance();
    auto session = composer.CreateSession("IDE Session");
    RAWRXD_LOG_INFO("Composer session created: " << session.id);
    MessageBoxA(m_hwndMain,
                ("Session created: " + session.id).c_str(),
                "Agentic Composer", MB_OK);
}

void Win32IDE::cmdComposerEndSession() {
    auto& composer = RawrXD::Agentic::AgenticComposerUX::Instance();
    auto* session = composer.GetActiveSession();
    if (!session) {
        MessageBoxA(m_hwndMain, "No active session.", "Composer", MB_ICONWARNING);
        return;
    }
    composer.EndSession(session->id);
...
void Win32IDE::cmdComposerApproveAll() {
    ...
    for (auto& fc : session->fileChanges) {
        composer.ApproveFileChange(session->id, fc.filePath);
    }
...
void Win32IDE::cmdComposerRejectAll() {
    ...
    for (auto& fc : session->fileChanges) {
        composer.RejectFileChange(session->id, fc.filePath);
    }
...
void Win32IDE::cmdComposerShowTranscript() {
    ...
    std::string json = composer.SerializeSession(session->id);
    // Display in output panel
    if (m_hwndOutput) {
        SetWindowTextA(m_hwndOutput, json.c_str());
    }
...
void Win32IDE::cmdComposerShowMetrics() {
    auto& composer = RawrXD::Agentic::AgenticComposerUX::Instance();
    auto metrics = composer.GetRenderMetrics();
    ...
    oss << "Composer UX Metrics:\n"
        ...
        << "  Hunks approved: " << metrics.hunksApproved << "\n"
        << "  Hunks rejected: " << metrics.hunksRejected << "\n";
...
void Win32IDE::initContextMentionParser() {
    ...
    auto& parser = RawrXD::Context::ContextMentionParser::Instance();
    parser.Initialize();
...
void Win32IDE::cmdMentionParse() {
    ...
    char buf[4096] = {};
    if (m_hwndChatInput) {
        GetWindowTextA(m_hwndChatInput, buf, sizeof(buf));
    }
    ...
    auto& parser = RawrXD::Context::ContextMentionParser::Instance();
    auto mentions = parser.Parse(input);

    std::ostringstream oss;
    oss << "Found " << mentions.size() << " mention(s):\n\n";
    for (const auto& m : mentions) {
        oss << "  @" << m.raw << " → type=" << static_cast<int>(m.type)
            << " arg=" << m.argument << "\n";
    }
...
void Win32IDE::cmdMentionSuggest() {
    initContextMentionParser();
    auto& parser = RawrXD::Context::ContextMentionParser::Instance();
    auto suggestions = parser.GetSuggestions("");
    std::ostringstream oss;
    oss << "Available @-mention types (" << suggestions.size() << "):\n\n";
    for (const auto& s : suggestions) {
        oss << "  @" << s.display << " — " << s.description << "\n";
    }
...
void Win32IDE::cmdMentionAssembleContext() {
    ...
    auto& parser = RawrXD::Context::ContextMentionParser::Instance();
    auto mentions = parser.Parse(input);
    auto resolved = parser.Resolve(mentions);
    auto assembled = parser.AssembleContext(resolved, 8192);
    std::ostringstream oss;
...
void Win32IDE::initVisionEncoderUI() {
    ...
    auto& encoder = RawrXD::Multimodal::VisionEncoder::Instance();
    encoder.Initialize();
...
void Win32IDE::cmdVisionLoadFile() {
    ...
    if (result.success) {
        RAWRXD_LOG_INFO("Image loaded: " << path << " (" << result.width << "x" << result.height << ")");
        MessageBoxA(m_hwndMain,
                    (std::string("Loaded: ") + std::to_string(result.width) + "x" +
                     std::to_string(result.height) + " " + result.formatName).c_str(),
...
void Win32IDE::cmdVisionPasteClipboard() {
    ...
    if (result.success) {
        RAWRXD_LOG_INFO("Image pasted from clipboard (" << result.width << "x" << result.height << ")");
        MessageBoxA(m_hwndMain,
                    (std::string("Pasted: ") + std::to_string(result.width) + "x" +
                     std::to_string(result.height)).c_str(),
                    "Vision Encoder", MB_OK);
    } else {
...
void Win32IDE::cmdVisionScreenshot() {
    ...
    if (result.success)
        MessageBoxA(m_hwndMain,
                    (std::string("Screenshot: ") + std::to_string(result.width) + "x" +
                     std::to_string(result.height)).c_str(),
...
void Win32IDE::cmdVisionBuildPayload() {
    initVisionEncoderUI();
    auto& encoder = RawrXD::Multimodal::VisionEncoder::Instance();
    auto payload = encoder.BuildMultimodalPayload("Describe this image.", "auto");
    if (!payload.empty()) {
        RAWRXD_LOG_INFO("Multimodal payload built (" << payload.size() << " chars)");
        if (m_hwndOutput)
            SetWindowTextA(m_hwndOutput, payload.c_str());
        MessageBoxA(m_hwndMain,
                    (std::string("Payload built (") + std::to_string(payload.size()) + " chars). "
                     "Displayed in output panel.").c_str(),
...
void Win32IDE::initRefactoringEngine() {
    ...
    auto& engine = RawrXD::Refactoring::RefactoringEngine::Instance();
    engine.Initialize();
...
static RawrXD::Refactoring::RefactoringContext buildRefactoringContext(HWND hwndEditor, const std::string& filePath) {
    RawrXD::Refactoring::RefactoringContext ctx;
    ...
    if (hwndEditor) {
        ...
        if (len > 0) {
            ctx.sourceCode.resize(len + 1);
            GetWindowTextA(hwndEditor, ctx.sourceCode.data(), len + 1);
            ctx.sourceCode.resize(len);
        }

        // Get selection range
        DWORD selStart = 0, selEnd = 0;
        SendMessageA(hwndEditor, EM_GETSEL, (WPARAM)&selStart, (LPARAM)&selEnd);
        ctx.selectionStart = static_cast<int>(selStart);
        ctx.selectionEnd = static_cast<int>(selEnd);
...
void Win32IDE::cmdRefactorShowAll() {
    ...
    if (m_hwndOutput) SetWindowTextA(m_hwndOutput, oss.str().c_str());
...
void Win32IDE::cmdLanguageDetect() {
    ...
    auto& registry = RawrXD::Language::LanguageRegistry::Instance();
    auto* desc = registry.DetectLanguage(m_currentFile);
    if (desc) {
        std::ostringstream oss;
        oss << "Detected language: " << desc->name << "\n"
            << "  ID: " << desc->id << "\n"
            << "  Extensions: ";
        for (size_t i = 0; i < desc->extensions.size() && i < 5; ++i)
            oss << desc->extensions[i] << " ";
...
void Win32IDE::cmdLanguageListAll() {
    ...
    for (const auto& lang : langs) {
        ...
        if (!lang.extensions.empty()) {
            oss << " — ";
            for (size_t i = 0; i < lang.extensions.size() && i < 4; ++i)
                oss << lang.extensions[i] << " ";
        }
        oss << "\n";
    }
    if (m_hwndOutput) SetWindowTextA(m_hwndOutput, oss.str().c_str());
...
void Win32IDE::cmdSemanticBuildIndex() {
    ...
    // Collect files from current workspace
    std::vector<std::string> files;
    if (!m_workspacePath.empty()) {
        WIN32_FIND_DATAA fd;
        std::string searchPath = m_workspacePath + "\\*";
        HANDLE hFind = FindFirstFileA(searchPath.c_str(), &fd);
        if (hFind != INVALID_HANDLE_VALUE) {
            do {
                if (!(fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
                    files.push_back(m_workspacePath + "\\" + fd.cFileName);
                }
...
void Win32IDE::cmdSemanticFuzzySearch() {
    ...
    char buf[512] = {};
    if (m_hwndChatInput) GetWindowTextA(m_hwndChatInput, buf, sizeof(buf));
    ...
    auto& idx = RawrXD::SemanticIndex::SemanticIndexEngine::Instance();
    auto results = idx.FuzzySearch(query, 20);
    std::ostringstream oss;
    oss << "Search results for '" << query << "' (" << results.results.size() << "):\n\n";
    for (const auto& r : results.results) {
        oss << "  " << r.symbol.name << " (" << r.symbol.file << ":" << r.symbol.line
            << ") score=" << r.score << "\n";
    }
    if (m_hwndOutput) SetWindowTextA(m_hwndOutput, oss.str().c_str());
}
...
void Win32IDE::cmdSemanticShowCallGraph() {
    ...
    for (const auto& c : callers) oss << "    ← " << c.caller << "\n";
    oss << "  Callees (" << callees.size() << "):\n";
    for (const auto& c : callees) oss << "    → " << c.callee << "\n";
    if (m_hwndOutput) SetWindowTextA(m_hwndOutput, oss.str().c_str());
}
...
void Win32IDE::cmdSemanticFindCycles() {
    ...
    if (cycles.empty()) {
        ...
        oss << "Dependency cycles found (" << cycles.size() << "):\n\n";
        for (size_t i = 0; i < cycles.size(); ++i) {
            oss << "  Cycle " << (i + 1) << ": ";
            for (const auto& f : cycles[i]) oss << f << " → ";
            oss << "(loop)\n";
        }
    }
    if (m_hwndOutput) SetWindowTextA(m_hwndOutput, oss.str().c_str());
...
void Win32IDE::cmdResourceGenerate() {
    ...
    if (templateId.empty()) {
        if (m_hwndOutput) SetWindowTextA(m_hwndOutput, picker.str().c_str());
    ...
    if (result.success) {
        if (m_hwndOutput) SetWindowTextA(m_hwndOutput, result.content.c_str());
