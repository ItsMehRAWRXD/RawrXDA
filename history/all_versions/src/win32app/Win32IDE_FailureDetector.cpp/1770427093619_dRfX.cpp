...
AgentResponse Win32IDE::executeWithFailureDetection(const std::string& prompt) {
    // If Failure Intelligence is enabled, delegate to the smarter pipeline
    if (m_failureIntelligenceEnabled && m_agenticBridge) {
        return executeWithFailureIntelligence(prompt);
    }
    ...
    for (int attempt = 0; attempt <= m_failureMaxRetries; attempt++) {
        ...
        // Log failures with Phase 6 classification
        for (auto f : failures) {
            FailureReason reason = classifyFailureReason(f, response.content, prompt);

            appendToOutput("[FailureDetector] Detected: " + failureTypeString(f) +
                           " (" + failureReasonString(reason) + ")" +
                           " (attempt " + std::to_string(attempt + 1) + "/" +
                           std::to_string(m_failureMaxRetries + 1) + ")",
                           "General", OutputSeverity::Warning);

            // Record failure detection event with classification metadata
            ...
            record.failureType   = f;
            record.reason        = reason;
            record.strategyUsed  = RetryStrategyType::None;
            record.attemptNumber = attempt;
            record.retrySucceeded = false;
            record.failureDetail = failureReasonString(reason);
            ...
            // Always record to intelligence history for future matching
