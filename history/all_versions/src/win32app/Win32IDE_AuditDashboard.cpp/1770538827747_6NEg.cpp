...
static void populateAuditListView(AuditDashboardState* state) {
    ...
    for (size_t i = 0; i < features.size(); ++i) {
        ...
        ListView_SetItemText(state->hwndListView, idx, 1, catBuf);

        // Column 2: Status
        wchar_t statusBuf[32] = {};
        MultiByteToWideChar(CP_UTF8, 0, implStatusToString(f.status), -1, statusBuf, 31);
        ListView_SetItemText(state->hwndListView, idx, 2, statusBuf);
        ...
        ListView_SetItemText(state->hwndListView, idx, 3, phaseBuf);

        // Column 4: Menu Wired
        ListView_SetItemText(state->hwndListView, idx, 4,
        ...
        ListView_SetItemText(state->hwndListView, idx, 5, idmBuf);

        // Column 6: Stub?
        ListView_SetItemText(state->hwndListView, idx, 6,
                              const_cast<LPWSTR>(f.stubDetected ? L"STUB" : L"OK"));

        // Column 7: Runtime Test
        ListView_SetItemText(state->hwndListView, idx, 7,
