...
static void populateAuditListView(AuditDashboardState* state) {
    ...
    for (size_t i = 0; i < features.size(); ++i) {
        ...
        ListView_SetItemTextW(state->hwndListView, idx, 3, phaseBuf);

        // Column 4: Menu Wired
        ListView_SetItemTextW(state->hwndListView, idx, 4,
        ...
        ListView_SetItemTextW(state->hwndListView, idx, 5, idmBuf);

        // Column 6: Stub?
        ListView_SetItemTextW(state->hwndListView, idx, 6,
                              const_cast<LPWSTR>(f.stubDetected ? L"STUB" : L"OK"));

        // Column 7: Runtime Test
        ListView_SetItemTextW(state->hwndListView, idx, 7,
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            for (int c = 0; c < 8; ++c) {
                ...
                ListView_InsertColumn(state->hwndListView, c, &lvc);
            }
        ...
        case WM_COMMAND: {
            ...
            if (id == AUDIT_BTN_RUN_ID) {
                ...
                SetWindowTextW(state->hwndStatusBar, L"Audit complete.");
                return 0;
            }
...
void Win32IDE::initAuditSystem() {
    ...
    OutputDebugStringA("[Phase 31] Audit system initialized.\n");
}
...
void Win32IDE::cmdAuditRunFull() {
    OutputDebugStringA("[Phase 31] Running full audit...\n");
