...
void Win32IDE::executeAnnotationAction(const InlineAnnotation& annotation) {
    switch (annotation.action.type) {
        ...
        case AnnotationActionType::JumpToLine: {
            int targetLine = annotation.action.targetLine;
            if (targetLine < 1) targetLine = annotation.line;

            // If a target file is specified and differs from the current file, open it
            if (!annotation.action.targetFile.empty() &&
                annotation.action.targetFile != m_currentFile) {
                // Open the file via the tab system
                int tabIdx = findTabByPath(annotation.action.targetFile);
                if (tabIdx >= 0) {
                    setActiveTab(tabIdx);
                } else {
                    addTab(annotation.action.targetFile,
                           annotation.action.targetFile.substr(
                               annotation.action.targetFile.find_last_of("\\/") + 1));
                }
            }
        ...
        case AnnotationActionType::ApplyFix: {
            if (annotation.action.fixContent.empty()) {
            ...
            int startLine = annotation.action.fixStartLine;
            int endLine   = annotation.action.fixEndLine;
            ...
            confirmMsg += "?\n\nReplacement:\n" + annotation.action.fixContent.substr(0, 200);
            if (annotation.action.fixContent.size() > 200) confirmMsg += "...";
            ...
            SendMessageA(m_hwndEditor, EM_REPLACESEL, TRUE,
                         (LPARAM)annotation.action.fixContent.c_str());
        ...
        case AnnotationActionType::AskAgent: {
            ...
            std::string prompt = annotation.action.agentPrompt;
            if (prompt.empty()) {
        ...
        case AnnotationActionType::OpenFile: {
            if (annotation.action.targetFile.empty()) break;
            ...
            if (annotation.action.targetLine > 0 && m_hwndEditor) {
                int charIdx = (int)SendMessage(m_hwndEditor, EM_LINEINDEX,
                                               annotation.action.targetLine - 1, 0);
                if (charIdx >= 0) {
                    CHARRANGE cr = {charIdx, charIdx};
                    SendMessage(m_hwndEditor, EM_EXSETSEL, 0, (LPARAM)&cr);
                    SendMessage(m_hwndEditor, EM_SCROLLCARET, 0, 0);
            ...
            LOG_INFO("Annotation action: OpenFile " + annotation.action.targetFile);
            break;
        }

        case AnnotationActionType::RunCommand: {
            if (annotation.action.commandId.empty()) break;

            LOG_INFO("Annotation action: RunCommand " + annotation.action.commandId);
            appendToOutput("Executing command: " + annotation.action.commandId + "\n",
...
void Win32IDE::onAnnotationClicked(int line, int clickX, int clickY) {
    ...
    if (anns.size() == 1) {
        if (anns[0].action.type != AnnotationActionType::None) {
...
void Win32IDE::showAnnotationActionMenu(HWND hwndParent, int x, int y,
    ...
    if (cmd >= 1000 && (cmd - 1000) < (int)annotations.size()) {
        ...
        if (chosen.action.type != AnnotationActionType::None) {
...
void Win32IDE::saveSessionAnnotations(nlohmann::json& session) {
    ...
    for (const auto& ann : m_annotations) {
        ...
        // Persist the action if it has one
        j["actionType"] = (int)ann.action.type;
        if (ann.action.type == AnnotationActionType::JumpToLine) {
            j["actionTargetLine"] = ann.action.targetLine;
            j["actionTargetFile"] = ann.action.targetFile;
        } else if (ann.action.type == AnnotationActionType::ApplyFix) {
            j["actionFixContent"] = ann.action.fixContent;
            j["actionFixStartLine"] = ann.action.fixStartLine;
            j["actionFixEndLine"] = ann.action.fixEndLine;
        } else if (ann.action.type == AnnotationActionType::AskAgent) {
            j["actionAgentPrompt"] = ann.action.agentPrompt;
        } else if (ann.action.type == AnnotationActionType::OpenFile) {
            j["actionTargetFile"] = ann.action.targetFile;
            j["actionTargetLine"] = ann.action.targetLine;
        } else if (ann.action.type == AnnotationActionType::RunCommand) {
            j["actionCommandId"] = ann.action.commandId;
        }
...
void Win32IDE::restoreSessionAnnotations(const nlohmann::json& session) {
    ...
    for (size_t i = 0; i < annArray.size(); i++) {
        ...
        // Restore action
        ann.action = {};
        ann.action.type = static_cast<AnnotationActionType>(j.value("actionType", 0));
        ann.action.targetLine   = j.value("actionTargetLine", 0);
        ann.action.targetFile   = j.value("actionTargetFile", "");
        ann.action.fixContent   = j.value("actionFixContent", "");
        ann.action.fixStartLine = j.value("actionFixStartLine", 0);
        ann.action.fixEndLine   = j.value("actionFixEndLine", 0);
        ann.action.agentPrompt  = j.value("actionAgentPrompt", "");
        ann.action.commandId    = j.value("actionCommandId", "");
...
std::string Win32IDE::buildLanguageAwarePrompt(const std::string& basePrompt) const {
    ...
    if (m_syntaxLanguage != SyntaxLanguage::None) {
        enriched += "[Language: " + langName + "] ";
    }

    // File context
    if (!m_currentFile.empty()) {
        enriched += "[File: " + m_currentFile + "] ";
    }
