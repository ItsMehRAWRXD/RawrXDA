...
// ============================================================================
// ANNOTATION LIFECYCLE — clear annotations when a tab/file is closed
//
// Removes all live annotations from m_annotations and also purges the
// per-file stash in m_annotationCache so they don't leak memory.
// Called from removeTab() when the active tab is being closed.
// ============================================================================
void Win32IDE::clearAnnotationsForCurrentFile() {
    // Clear the live annotation vector — these belong to the active editor
    m_annotations.clear();

    // Also purge the stash entry for the current file
    if (!m_currentFile.empty()) {
        m_annotationCache.erase(m_currentFile);
    }

    LOG_INFO("Annotations cleared for file close: " +
             (m_currentFile.empty() ? "<untitled>" : m_currentFile));

    // Trigger repaint so old gutter icons / overlay text disappear
    if (m_hwndLineNumbers) {
        InvalidateRect(m_hwndLineNumbers, nullptr, FALSE);
    }
    if (m_hwndAnnotationOverlay && IsWindow(m_hwndAnnotationOverlay)) {
...
// ============================================================================
// ANNOTATION LIFECYCLE — stash annotations before tab switch
//
// Saves the current live annotations into m_annotationCache keyed by
// the outgoing file path, then clears m_annotations so the incoming
// tab starts with a clean slate (or its own cached set).
// ============================================================================
void Win32IDE::storeAnnotationsForTab() {
    if (m_currentFile.empty() && m_annotations.empty()) return;

    std::string key = m_currentFile.empty() ? "__untitled__" : m_currentFile;

    // Move current annotations into the per-file cache
    m_annotationCache[key] = m_annotations;
    m_annotations.clear();

    LOG_DEBUG("Annotations stashed for tab: " + key +
              " (" + std::to_string(m_annotationCache[key].size()) + " entries)");
}

// ============================================================================
// ANNOTATION LIFECYCLE — restore annotations after tab switch
//
// Looks up the incoming file path in m_annotationCache and repopulates
// m_annotations.  If no cache entry exists, annotations start empty.
// Triggers repaint of gutter and overlay so the restored annotations render.
// ============================================================================
void Win32IDE::restoreAnnotationsForTab() {
    std::string key = m_currentFile.empty() ? "__untitled__" : m_currentFile;

    auto it = m_annotationCache.find(key);
    if (it != m_annotationCache.end()) {
        m_annotations = it->second;
        LOG_DEBUG("Annotations restored for tab: " + key +
                  " (" + std::to_string(m_annotations.size()) + " entries)");
    } else {
        m_annotations.clear();
        LOG_DEBUG("No cached annotations for tab: " + key);
    }

    // Trigger repaint so annotations render for the newly-active tab
    if (m_hwndLineNumbers) {
        InvalidateRect(m_hwndLineNumbers, nullptr, FALSE);
    }
    if (m_hwndAnnotationOverlay && IsWindow(m_hwndAnnotationOverlay)) {
