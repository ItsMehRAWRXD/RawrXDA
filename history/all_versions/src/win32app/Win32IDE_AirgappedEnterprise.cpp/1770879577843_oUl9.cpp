...
static LRESULT CALLBACK entPanelWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        addBtn(15,  140, L"\u2705 Run Compliance",    IDM_AIRGAP_COMPLIANCE);
        addBtn(165, 140, L"\U0001F4E6 Scan Models",   IDM_AIRGAP_MODELS);
        addBtn(315, 140, L"\U0001F6E1 DLP Scan",       IDM_AIRGAP_DLP);
        addBtn(465, 140, L"\U0001F525 Firewall",       IDM_AIRGAP_FIREWALL);
        addBtn(615, 140, L"\U0001F50F License",        IDM_AIRGAP_LICENSE);
        addBtn(765, 140, L"\U0001F4BE Export",          IDM_AIRGAP_EXPORT);
        ...
        if (nmhdr->idFrom == 9200 && nmhdr->code == TCN_SELCHANGE) {
            ...
            switch (tab) {
                case ENT_TAB_COMPLIANCE:
                    PostMessageW(hwndParent, WM_COMMAND, IDM_AIRGAP_COMPLIANCE, 0);
                    break;
                case ENT_TAB_MODELS:
                    PostMessageW(hwndParent, WM_COMMAND, IDM_AIRGAP_MODELS, 0);
                    break;
                case ENT_TAB_AUDIT:
                    PostMessageW(hwndParent, WM_COMMAND, IDM_AIRGAP_AUDIT, 0);
                    break;
                case ENT_TAB_DLP:
                    PostMessageW(hwndParent, WM_COMMAND, IDM_AIRGAP_DLP, 0);
                    break;
                case ENT_TAB_FIREWALL:
                    PostMessageW(hwndParent, WM_COMMAND, IDM_AIRGAP_FIREWALL, 0);
                    break;
                case ENT_TAB_LICENSE:
                    PostMessageW(hwndParent, WM_COMMAND, IDM_AIRGAP_LICENSE, 0);
                    break;
            }
        }
        return 0;
...
void Win32IDE::cmdAirgapDLP() {
    ...
    for (auto& rule : s_dlpRules) {
        ...
        // Just count substring occurrences as approximation
...
// ============================================================================
// Encrypt workspace (placeholder — real impl uses AES-256-GCM via BCrypt)
// ============================================================================
...
void Win32IDE::cmdAirgapEncrypt() {
    ...
    appendToOutput("[Enterprise] Workspace encryption configured:\n"
    ...
    appendAuditLog("user", "encrypt_workspace", s_encWorkspace.workspacePath,
                   "Workspace encryption configured");
}
