...
void Win32IDE::applySyntaxColoring() {
    ...
    // Skip very large files to avoid UI freeze (> 500KB)
    if (textLen > 500000) {
        LOG_WARNING("File too large for full syntax coloring: " + std::to_string(textLen) + " chars");
        return;
    }
    ...
    // Apply colors for non-default tokens
