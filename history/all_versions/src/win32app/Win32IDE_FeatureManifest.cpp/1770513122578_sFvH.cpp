...
static bool testEditorExists(void* idePtr) {
    auto* ide = static_cast<Win32IDE*>(idePtr);
    if (!ide) return false;
    return (ide->getEditorHwnd() != nullptr && IsWindow(ide->getEditorHwnd()));
}
...
static bool testThemeSystem(void* idePtr) {
    auto* ide = static_cast<Win32IDE*>(idePtr);
    if (!ide) return false;
    // We check if we can retrieve theme count
    return (ide->getThemeCount() >= 16);
}
...
static bool testAgenticBridge(void* idePtr) {
    auto* ide = static_cast<Win32IDE*>(idePtr);
    if (!ide) return false;
    return ide->isAgenticBridgeReady();
}
