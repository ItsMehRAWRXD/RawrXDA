...
void Win32IDE::initBackendManager() {
    ...
    auto& nativeasm = m_backendConfigs[(size_t)AIBackendType::NativeASM];
    nativeasm.type      = AIBackendType::NativeASM;
    nativeasm.name      = "Native ASM";
    nativeasm.endpoint  = "";       // No endpoint — uses MASM kernels directly
    nativeasm.model     = "";       // Determined by loaded GGUF + kernel dispatcher
    nativeasm.apiKey    = "";
    nativeasm.enabled   = true;    // Enabled if kernel DLLs available
    nativeasm.timeoutMs = 120000;  // ASM kernels can be slow on first init
    nativeasm.maxTokens = 4096;
    nativeasm.temperature = 0.7f;
...
bool Win32IDE::probeBackendHealth(AIBackendType type) {
    ...
    switch (type) {
        ...
        case AIBackendType::NativeASM: {
#if RAWRXD_HAS_POCKET_LAB || RAWRXD_HAS_PHASE3_KERNEL
            if (m_kernelDispatcher) {
                healthy = m_kernelDispatcher->IsKernelLoaded();
                if (!healthy) {
                    // Try to initialize
                    healthy = m_kernelDispatcher->Initialize();
                    if (!healthy) error = "Kernel DLLs not found or failed to load";
                }
            } else {
                error = "KernelDispatcher not initialized";
            }
#else
            error = "Native ASM kernels not compiled (RAWRXD_HAS_POCKET_LAB/RAWRXD_HAS_PHASE3_KERNEL=0)";
#endif
            break;
        }
        default:
...
std::string Win32IDE::routeInferenceRequest(const std::string& prompt) {
    ...
    switch (active) {
        ...
        case AIBackendType::Gemini:
            ...
            break;
        case AIBackendType::NativeASM:
            result = routeToNativeASM(prompt);
            break;
        default:
...
// ============================================================================
// NATIVE ASM KERNEL ROUTING
// ============================================================================

std::string Win32IDE::routeToNativeASM(const std::string& prompt) {
#if RAWRXD_HAS_POCKET_LAB || RAWRXD_HAS_PHASE3_KERNEL
    if (!m_kernelDispatcher) {
        // Lazy-initialize on first use
        m_kernelDispatcher = std::make_unique<RawrXD::Kernel::KernelDispatcher>();
        if (!m_kernelDispatcher->Initialize()) {
            return "[BackendSwitcher] Error (NativeASM): KernelDispatcher failed to initialize. "
                   "Ensure pocket_lab_turbo.dll and/or Phase3_Agent_Kernel.dll are in bin/.";
        }
        logInfo("[BackendSwitcher] KernelDispatcher initialized — tier: " +
                std::to_string((int)m_kernelDispatcher->GetCurrentSystemTier()));
    }

    // If we have a loaded model path, analyze and configure kernels
    if (!m_kernelDispatcher->IsKernelLoaded()) {
        // Auto-detect and configure based on system capabilities
        RawrXD::Kernel::SystemInfo sys_info;
        m_kernelDispatcher->DetectSystemInfo(sys_info);

        // Build a default model metadata for prompt-only mode
        RawrXD::Kernel::ModelMetadata model_meta{};
        model_meta.param_tier = RawrXD::Kernel::ModelTier::SMALL_3B;
        model_meta.quant_format = RawrXD::Kernel::QuantFormat::Q4_K_M;
        model_meta.context_length = 4096;
        model_meta.vocab_size = 128000;

        // If native engine has a model loaded, use its file size for tier detection
        if (m_nativeEngine && m_nativeEngine->IsModelLoaded()) {
            // Use the model path from native engine if available
            std::string modelPath = m_nativeEngine->GetModelPath();
            if (!modelPath.empty()) {
                std::wstring wpath(modelPath.begin(), modelPath.end());
                m_kernelDispatcher->AnalyzeModel(wpath.c_str(), model_meta);
            }
        }

        RawrXD::Kernel::KernelConfig config;
        m_kernelDispatcher->SelectKernel(sys_info, model_meta, config);
        m_kernelDispatcher->LoadKernel(config);

        auto cfg_out = m_kernelDispatcher->GetCurrentConfig();
        logInfo("[BackendSwitcher] NativeASM kernel loaded — "
                "GPU:" + std::to_string(cfg_out.power_infer_config.gpu_ratio_percent) + "% "
                "CPU:" + std::to_string(cfg_out.power_infer_config.cpu_ratio_percent) + "% "
                "KV:" + std::to_string(cfg_out.kv_config.slot_count) + " slots "
                "Sparse:" + std::to_string(cfg_out.sparse_config.skip_percentage) + "%");
    }

    // Route through Phase3 GenerateTokens kernel
    constexpr size_t OUTPUT_CAP = 32768;
    std::vector<uint8_t> output(OUTPUT_CAP, 0);
    size_t out_len = 0;

    bool ok = m_kernelDispatcher->InvokeGenerateTokens(
        reinterpret_cast<const uint8_t*>(prompt.c_str()),
        prompt.size(),
        output.data(),
        OUTPUT_CAP,
        out_len
    ...
    if (!ok) {
        // Fallback: try PowerInfer loop for token stats
        uint64_t tokens_processed = 0;
        bool power_ok = m_kernelDispatcher->InvokePowerInferLoop(256, tokens_processed);
        if (power_ok && tokens_processed > 0) {
            return "[NativeASM] PowerInfer processed " + std::to_string(tokens_processed) +
                   " tokens (Phase3 GenerateTokens not available — kernel DLL may not be loaded)";
        }
        return "[BackendSwitcher] Error (NativeASM): Kernel invocation failed. "
               "Phase3_Agent_Kernel.dll and pocket_lab_turbo.dll must be in bin/.";
    }

    return std::string(reinterpret_cast<char*>(output.data()), out_len);
#else
    return "[BackendSwitcher] Error (NativeASM): Native ASM kernels not compiled. "
           "Build with RAWRXD_HAS_POCKET_LAB=1 and RAWRXD_HAS_PHASE3_KERNEL=1.";
#endif
}
...
void Win32IDE::showBackendSwitcherDialog() {
    ...
    appendToOutput("[BackendSwitcher] Use Command Palette (Ctrl+Shift+P) to switch backends:\n"
                   ...
                   "  'AI: Switch to Claude'\n"
                   "  'AI: Switch to Gemini'\n"
                   "  'AI: Switch to Native ASM'  (MASM x64 kernels — TurboSparse + PowerInfer)\n",
...
std::string Win32IDE::backendTypeString(AIBackendType type) const {
    switch (type) {
        ...
        case AIBackendType::NativeASM: return "NativeASM";
...
Win32IDE::Win32IDE::AIBackendType Win32IDE::backendTypeFromString(const std::string& name) const {
    ...
    if (name == "NativeASM"  || name == "nativeasm" || name == "Native ASM" || name == "asm") return AIBackendType::NativeASM;
