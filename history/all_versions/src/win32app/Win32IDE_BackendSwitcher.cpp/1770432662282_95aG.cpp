...
// ============================================================================
// nlohmann/json (already available via project includes)
// ============================================================================
...
void Win32IDE::loadBackendConfigs() {
    ...
    try {
        ...
        if (j.contains("backends") && j["backends"].is_array()) {
            for (auto& bj : j["backends"]) {
                std::string typeName = bj.value("type", "");
                ...
                cfg.type      = bt;
                cfg.name      = bj.value("name", cfg.name);
                cfg.endpoint  = bj.value("endpoint", cfg.endpoint);
                cfg.model     = bj.value("model", cfg.model);
                cfg.apiKey    = bj.value("apiKey", cfg.apiKey);
                cfg.enabled   = bj.value("enabled", cfg.enabled);
                cfg.timeoutMs = bj.value("timeoutMs", cfg.timeoutMs);
                cfg.maxTokens = bj.value("maxTokens", cfg.maxTokens);
                cfg.temperature = bj.value("temperature", cfg.temperature);
            }
        }
...
AIBackendType Win32IDE::getActiveBackendType() const {
    return m_activeBackend;
}

const AIBackendConfig& Win32IDE::getActiveBackendConfig() const {
...
std::string Win32IDE::routeToOpenAI(const std::string& prompt) {
    ...
    try {
        ...
        if (rj.contains("choices") && rj["choices"].is_array() && !rj["choices"].empty()) {
            return rj["choices"][0]["message"]["content"].get<std::string>();
        }
        if (rj.contains("error")) {
...
std::string Win32IDE::routeToClaude(const std::string& prompt) {
    ...
    try {
        ...
        if (rj.contains("content") && rj["content"].is_array() && !rj["content"].empty()) {
            return rj["content"][0]["text"].get<std::string>();
        }
        if (rj.contains("error")) {
...
std::string Win32IDE::routeToGemini(const std::string& prompt) {
    ...
    nlohmann::json reqBody;
    reqBody["contents"] = nlohmann::json::array({
    ...
    try {
        ...
        if (rj.contains("candidates") && rj["candidates"].is_array() && !rj["candidates"].empty()) {
            auto& content = rj["candidates"][0]["content"];
...
AIBackendType Win32IDE::backendTypeFromString(const std::string& name) const {
