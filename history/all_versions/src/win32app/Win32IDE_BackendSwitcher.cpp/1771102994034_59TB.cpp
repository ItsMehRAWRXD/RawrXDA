...
bool Win32IDE::probeBackendHealth(AIBackendType type) {
    ...
    switch (type) {
        ...
        case AIBackendType::GitHubCopilot: {
            // Probe GitHub Copilot extension status via VsixLoader registry
            // The extension must be installed and initialized in the compatibility layer
            healthy = m_vsixLoader && m_vsixLoader->isExtensionActive("github.copilot");
            if (!healthy) error = "GitHub Copilot extension not active or not installed";
            break;
        }
        case AIBackendType::AmazonQ: {
            // Probe Amazon Q (AWS Toolkit) extension status
            healthy = m_vsixLoader && m_vsixLoader->isExtensionActive("amazon.q");
            if (!healthy) error = "Amazon Q extension not active or not installed";
            break;
        }
        default:
            break;
...
std::string Win32IDE::routeToGitHubCopilot(const std::string& prompt) {
    ...
    auto apiResult = api.executeCommandWithArgs("github.copilot.chat.proxy", args.dump().c_str());
...
std::string Win32IDE::routeToAmazonQ(const std::string& prompt) {
    ...
    auto apiResult = api.executeCommandWithArgs("amazon.q.chat.proxy", args.dump().c_str());
    if (apiResult.success) {
        if (apiResult.detail && strlen(apiResult.detail) > 0 && strcmp(apiResult.detail, "Success") != 0) {
            return apiResult.detail;
        }
