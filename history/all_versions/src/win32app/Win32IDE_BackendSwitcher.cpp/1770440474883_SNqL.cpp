...
void Win32IDE::initBackendManager() {
    ...
    // ---- Phase 9: Discover and register ASM streaming engines ----
    try {
        auto& registry = RawrXD::getStreamingEngineRegistry();
        registry.discoverEngines();
        auto hw = registry.detectHardware();
        logInfo("[EngineRegistry] Discovered " + std::to_string(registry.getEngineCount()) + " streaming engines");
        logInfo("[EngineRegistry] Hardware: " + hw.cpuName + ", " + std::to_string(hw.totalRAM / (1024*1024*1024)) + "GB RAM, " + std::to_string(hw.cpuCores) + " cores");
        logInfo("[EngineRegistry] AVX2=" + std::string(hw.hasAVX2 ? "YES" : "no") + " AVX-512=" + std::string(hw.hasAVX512 ? "YES" : "no"));
    } catch (...) {
        logWarning("initBackendManager", "Engine discovery failed — local inference will use default engine");
    }
...
std::string Win32IDE::selectStreamingEngine(const std::string& modelPath) {
    ...
    auto profile = registry.detectModelProfile(modelPath);
    
    // Auto-select best engine
    auto result = registry.selectEngine(profile);
    
    if (!result.success) {
        logWarning("selectStreamingEngine", "No engine matched: " + result.reason);
        return "";
    }
    
    // Activate the selected engine
    registry.selectEngineByName(result.selectedEngine);
    
    std::string msg = "[EngineSelector] Selected: " + result.selectedEngine + "\n";
...
bool Win32IDE::switchStreamingEngine(const std::string& engineName) {
    ...
    if (!registry.selectEngineByName(engineName)) {
        logError("switchStreamingEngine", "Engine not found: " + engineName);
        return false;
    }
    
    logInfo("[EngineSelector] Switched streaming engine to: " + engineName);
    appendToOutput("[EngineSelector] Active streaming engine: " + engineName + "\n",
