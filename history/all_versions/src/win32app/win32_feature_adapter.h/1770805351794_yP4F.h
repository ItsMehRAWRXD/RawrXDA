...
static bool routeCommandUnified(int commandId, void* idePtr) {
    ...
    CommandContext ctx{};
    ctx.rawInput = "";            // GUI has no raw text input
    ctx.args = "";                // Will be populated by handler if needed (e.g., dialog input)
    ctx.idePtr = idePtr;          // Win32IDE*
    ctx.cliStatePtr = nullptr;    // No CLI state in GUI mode
    ...
    // Try dispatching by the Win32 command ID
    auto result = registry.dispatchByCommandId(static_cast<uint32_t>(commandId), ctx);
    if (result.success) {
        return true;
    }
    
    // Not in the shared registry — fall through to legacy
    return false;
}
