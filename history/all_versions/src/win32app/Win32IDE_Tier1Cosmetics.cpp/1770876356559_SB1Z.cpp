...
void Win32IDE::showWelcomePage() {
    ...
    // Use WebView2Container if available (Monaco runtime), else fallback dialog
    if (m_webView2 && m_webView2->isReady()) {
        ...
        std::string fileUrl = "file:///" + htmlPath;
        for (auto& c : fileUrl) { if (c == '\\') c = '/'; }
        m_webView2->navigate(fileUrl);

        appendToOutput("[Welcome] Welcome page displayed via WebView2.\n");
    } else {
