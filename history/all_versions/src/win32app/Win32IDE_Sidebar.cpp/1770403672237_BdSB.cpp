...
void Win32IDE::updateDebugVariables()
{
    ...
    // System variables
    vars.push_back({"$DebuggerActive", m_debuggerAttached ? "true" : "false"});
    vars.push_back({"$DebuggerPaused", m_debuggerPaused ? "true" : "false"});
    vars.push_back({"$CurrentFile", m_currentFile.empty() ? "(none)" : m_currentFile});
    vars.push_back({"$CurrentLine", std::to_string(m_debuggerCurrentLine)});
    vars.push_back({"$BreakpointCount", std::to_string(m_breakpoints.size())});

    // Watch expressions
    for (const auto& w : m_watchItems) {
        if (w.enabled) {
            vars.push_back({"[watch] " + w.expression, w.value.empty() ? "<not evaluated>" : w.value});
        }
    }

    // Call stack info
    if (!m_callStack.empty()) {
        vars.push_back({"$CallStackDepth", std::to_string(m_callStack.size())});
        vars.push_back({"$TopFrame", m_callStack[0].function + " @ " + m_callStack[0].file + ":" + std::to_string(m_callStack[0].line)});
    }

    // Model/inference state
    vars.push_back({"$ModelLoaded", m_loadedModelPath.empty() ? "false" : "true"});
    vars.push_back({"$ModelPath", m_loadedModelPath.empty() ? "(none)" : m_loadedModelPath});
    vars.push_back({"$InferenceRunning", m_inferenceRunning ? "true" : "false"});
    vars.push_back({"$WorkingDirectory", m_explorerRootPath});

    // Process environment
    char envBuf[256] = {};
    if (GetEnvironmentVariableA("USERNAME", envBuf, sizeof(envBuf))) {
        vars.push_back({"$USERNAME", envBuf});
    }
    if (GetEnvironmentVariableA("COMPUTERNAME", envBuf, sizeof(envBuf))) {
        vars.push_back({"$COMPUTERNAME", envBuf});
    }
