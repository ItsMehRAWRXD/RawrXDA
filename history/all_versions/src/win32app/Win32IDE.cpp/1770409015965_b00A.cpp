...
Win32IDE::Win32IDE(HINSTANCE hInstance)
        : m_hInstance(hInstance), m_hwndMain(nullptr), m_hwndEditor(nullptr),
            m_hwndLineNumbers(nullptr), m_hwndTabBar(nullptr), m_origLineNumberProc(nullptr),
...
// ===========================================================================
// Stub implementations for functions declared in Win32IDE.h but not yet
// implemented — these prevent linker "undefined reference" errors.
// ===========================================================================
...
void Win32IDE::createTabBar(HWND hwndParent) {
    m_hwndTabBar = CreateWindowExA(
        ...
        0, 0, 800, 28,
        hwndParent, (HMENU)9010, m_hInstance, nullptr);
    if (m_hwndTabBar) {
        SendMessage(m_hwndTabBar, WM_SETFONT, (WPARAM)m_editorFont, TRUE);
    }
}
...
void Win32IDE::createLineNumberGutter(HWND hwndParent) {
    m_hwndLineNumbers = CreateWindowExA(
        0, "STATIC", "",
        WS_CHILD | WS_VISIBLE | SS_OWNERDRAW,
        0, 0, m_lineNumberWidth, 600,
        hwndParent, (HMENU)9011, m_hInstance, nullptr);
}
...
void Win32IDE::addTab(const std::string& filePath, const std::string& displayName) {
    ...
    if (m_hwndTabBar) {
        TCITEMA tie = {};
        tie.mask = TCIF_TEXT;
        tie.pszText = const_cast<char*>(displayName.c_str());
        int newIdx = (int)m_editorTabs.size() - 1;
        TabCtrl_InsertItem(m_hwndTabBar, newIdx, &tie);
        setActiveTab(newIdx);
    }
}
...
void Win32IDE::onTabChanged() {
    ...
    int sel = TabCtrl_GetCurSel(m_hwndTabBar);
    if (sel >= 0 && sel < (int)m_editorTabs.size()) {
        m_activeTabIndex = sel;
        // Load the selected tab's file content if available
        if (!m_editorTabs[sel].filePath.empty()) {
            m_currentFile = m_editorTabs[sel].filePath;
        }
    }
}
...
void Win32IDE::postAgentOutputSafe(const std::string& text) {
    // Post a message to the main window thread for thread-safe output
    char* copy = _strdup(text.c_str());
    PostMessage(m_hwndMain, WM_AGENT_OUTPUT_SAFE, 0, (LPARAM)copy);
}
    HWND hDlg = CreateWindowExA(
        WS_EX_DLGMODALFRAME | WS_EX_TOPMOST,
        "STATIC", "Quick Load GGUF Model",
        WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_VISIBLE,
    ...
    // Label
    HWND hLabel = CreateWindowExA(0, "STATIC",
        "Model (path, HF repo, Ollama name, or URL):",
        WS_CHILD | WS_VISIBLE | SS_LEFT,
        12, 10, 520, 20, hDlg, nullptr, m_hInstance, nullptr);
    SendMessage(hLabel, WM_SETFONT, (WPARAM)m_hFontUI, TRUE);

    // Edit
    HWND hEdit = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "",
        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
        12, 34, 430, 26, hDlg, (HMENU)101, m_hInstance, nullptr);
    SendMessage(hEdit, WM_SETFONT, (WPARAM)m_hFontUI, TRUE);
    SetFocus(hEdit);

    // Go button
    HWND hGoBtn = CreateWindowExA(0, "BUTTON", "Load",
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | BS_DEFPUSHBUTTON,
        450, 34, 80, 26, hDlg, (HMENU)201, m_hInstance, nullptr);
    SendMessage(hGoBtn, WM_SETFONT, (WPARAM)m_hFontUI, TRUE);
    ...
    while (!done && GetMessage(&msg, nullptr, 0, 0)) {
        if (msg.message == WM_COMMAND && msg.hwnd == hDlg) {
            ...
            if (wmId == IDCANCEL || wmId == 2 /*ESC mapped by system*/) {
                done = true;
                continue;
            }
        ...
        // Handle Enter key
        if (msg.message == WM_KEYDOWN && msg.wParam == VK_RETURN && msg.hwnd == hEdit) {
            PostMessage(hDlg, WM_COMMAND, MAKEWPARAM(201, BN_CLICKED), (LPARAM)hGoBtn);
            continue;
        }
