...
void Win32IDE::refreshInlayHints() {
        ...
        if (autoPos != std::string::npos && autoPos < 5) {
            ...
            if (nameEnd > nameStart) {
                ...
                if (!inferredType.empty()) {
                    ...
                    hint.kind = InlayHint::Type;
                    hint.color = INLAY_TYPE_COLOR;
                    m_inlayHints.push_back(hint);
    ...
    LOG_INFO("[InlayHints] Computed " + std::to_string(m_inlayHints.size()) + " hints");
}

// ── Render (called from editor paint) ──────────────────────────────────────
void Win32IDE::renderInlayHints(HDC hdc, int lineY, int lineNumber) {
    if (!m_inlayHintsEnabled || m_inlayHints.empty()) return;

    for (auto& hint : m_inlayHints) {
...
void Win32IDE::parseInlayHintsFromLSP(const std::string& jsonResponse) {
    // Parse LSP textDocument/inlayHint response
    ...
    while ((pos = jsonResponse.find("\"line\":", pos)) != std::string::npos) {
        ...
        hint.text = label;
        hint.kind = (kind == 2) ? InlayHint::Parameter : InlayHint::Type;
        hint.color = inlayHintColor(hint.kind);
        m_inlayHints.push_back(hint);
    }

    if (m_hwndEditor) InvalidateRect(m_hwndEditor, NULL, FALSE);
    LOG_INFO("[InlayHints] Parsed " + std::to_string(m_inlayHints.size()) + " LSP hints");
}

// ── Color helper ───────────────────────────────────────────────────────────
COLORREF Win32IDE::inlayHintColor(InlayHint::Kind kind) const {
    switch (kind) {
    case InlayHint::Type:      return INLAY_TYPE_COLOR;
    case InlayHint::Parameter: return INLAY_PARAM_COLOR;
    default:                   return INLAY_TYPE_COLOR;
    }
}
