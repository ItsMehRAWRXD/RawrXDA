            ...
            if (jsonObj.contains("models")) {
                ...
                if (modelsNode.is_array()) {
                    ...
                    for (auto it = modelsNode.begin(); it != modelsNode.end(); ++it) {
                        const auto& value = it.value();  // Use iterator's value() method
                        if (value.is_object()) {
...
private:
    ...
    void processRequest(const Request& req)
    {
        ...
        if (!WinHttpCrackUrl(endpoint_wide.c_str(), 0, 0, &urlComp)) {
            WinHttpCloseHandle(hSession);
            if (req.onError) req.onError("Failed to parse model endpoint");
            return;
        }
        ...
        std::wstring generatePath = basePath + L"/api/generate";

        HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"POST",
            generatePath.c_str(), NULL, WINHTTP_NO_REFERER,
            WINHTTP_DEFAULT_ACCEPT_TYPES, 0);
