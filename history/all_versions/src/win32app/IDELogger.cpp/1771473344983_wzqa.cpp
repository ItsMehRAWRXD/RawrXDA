...
namespace {
    ...
    const char* levelToString(IDELogLevel level) {
        switch (level) {
            ...
            case IDELogLevel::ERR:      return "ERROR";
...
void IDELogger::initialize(const std::string& logFilePath) {
    ...
    try {
        // If the project has spdlog linked/available, just rely on its defaults.
        // (We don't create sinks here to stay dependency-light.)
        (void)logFilePath;
        return;
    } catch (...) {
        // Fall back below.
...
void IDELogger::log(IDELogLevel level, const std::string& message) {
    ...
    try {
        switch (level) {
            ...
            case IDELogLevel::ERR:      spdlog::error(message); break;
