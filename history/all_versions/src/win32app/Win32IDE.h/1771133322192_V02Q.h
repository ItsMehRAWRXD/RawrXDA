...
#include "../../include/project_context.h"  /* simple context (workingDirectory); agentic one is agentic/project_context.h */
...
#define IDM_AGENT_SMOKE_TEST 4110

// SubAgent / Chain / Swarm / Todo command IDs (4111–4115; 4110 reserved)
...
#define IDM_AGENT_SET_CYCLE_COUNT 4122
#define IDM_AGENT_SET_CYCLE_AGENT_COUNTER 4123

// File menu — Load Model options (1030–1035; 1000–2000 = handleFileCommand)
...
// Engine / 800B Dual-Engine & Titan (4220–4222, 4230)
...
#define IDM_AI_AGENT_MULTI_STATUS 4223
#define IDM_AI_TITAN_TOGGLE       4224
#define IDM_AI_800B_STATUS        4225
...
// ============================================================================
// Agent Failure Detection Enums & Structures (from agentic_failure_detector.hpp)
// ============================================================================
...
private:
    ...
    void showAbout();
public:
    void showSettingsGUIDialog();   // Full property-grid Settings (from summary "Open full Settings...")
...
private:
    ...
    friend LRESULT CALLBACK MultiFileSearchDlgProc(HWND, UINT, WPARAM, LPARAM);
    friend LRESULT CALLBACK CICDSettingsDlgProc(HWND, UINT, WPARAM, LPARAM);
    friend LRESULT CALLBACK ModelRegistryDlgProc(HWND, UINT, WPARAM, LPARAM);
    friend LRESULT CALLBACK SettingsSummaryWndProc(HWND, UINT, WPARAM, LPARAM);
    friend LRESULT CALLBACK auditDashboardWndProc(HWND, UINT, WPARAM, LPARAM);
    ...
    void showLicenseCreatorDialog();  // Tools > License Creator — full enterprise feature dashboard
    void showFeatureRegistryDialog(); // Tools > Feature Registry — V2 manifest grid
    ...
    /** Candidate model root paths for explorer/LocalServer (OLLAMA_MODELS, %LOCALAPPDATA%\\Ollama, D:\\OllamaModels, ...). */
...
private:
    ...
    std::unique_ptr<MultiFileSearchWidget> m_multiFileSearch;
    std::unique_ptr<InterpretabilityPanel>  m_interpretabilityPanel;
    std::unique_ptr<CICDSettings>          m_ciCdSettings;
    std::unique_ptr<RawrXD::GhostTextRenderer> m_ghostTextRendererOverlay;
    ...
    HWND m_hwndSidebarTitle;   // Visible title bar: "File Explorer", "Search", etc.
...
#define WM_AGENT_OUTPUT_SAFE (WM_APP + 102)
    ...
    // Agent Panel HWNDs
    HWND m_hwndAgentDiffPanel   = nullptr;  // Owner-drawn diff rendering area
    HWND m_hwndAgentStatusLabel = nullptr;  // "No active session" / step progress
    HWND m_hwndAgentAcceptAllBtn = nullptr; // Accept All button
    HWND m_hwndAgentRejectAllBtn = nullptr; // Reject All button
    HWND m_hwndAgentNewSessionBtn = nullptr; // New Agent Session button
    HWND m_hwndAgentSummaryEdit = nullptr;  // Read-only session summary text
    ...
    enum class AIBackendType {
        ...
        GitHubCopilot   = 6,   // VS Code Extension: GitHub Copilot
        AmazonQ         = 7,   // VS Code Extension: Amazon Q (AWS Toolkit)
        Count           = 8
    };
    ...
    struct RoutingDecision {
        ...
        int           primaryLatencyMs  = -1;      // Latency of primary attempt (-1 = not yet measured)
    ...
    void handleV1ModelsEndpoint(SOCKET client);  // /v1/models, /api/models — Cursor Settings > Models
...
#define IDM_TOOLS_DIGESTION         5540
    ...
    /** True if coordinator or worker is running; used to enable Swarm Stop and direct next step. */
...
#define IDM_VIEW_MONACO_SETTINGS        9110
#define IDM_VIEW_THERMAL_DASHBOARD      9150
...
public:
    ...
    // 5b. Multi-File Search / CI/CD / Model Registry (real dialogs, no placeholders)
    void showMultiFileSearchDialog();
    void showCICDSettingsDialog();
    void showModelRegistryDialog();
    ModelRegistry* getModelRegistry() const { return m_modelRegistry.get(); }
...
private:
    ...
    // 45b. Extension Toggles (UI for enabling/disabling extensions + model/tool selection)
...
public:
    ...
    int  m_lineEndingMode                = 0;  // 0=CRLF, 1=LF, 2=CR
    bool m_debugWatchInitialized         = false;
    bool m_callStackSymbolsInitialized   = false;
    
    // Phase 50: Quantum Agent Orchestrator (Multi-Model 1x-99x, Auto-Adjusting Timeouts, Balance/Max/Auto)
    bool m_quantumOrchestratorInitialized = false;
    void initQuantumOrchestrator();
    bool handleQuantumCommand(int commandId);
    
    // Quantum command IDs (11610-11629)
    static constexpr int IDM_QUANTUM_MODE_AUTO     = 11610;
    static constexpr int IDM_QUANTUM_MODE_BALANCE  = 11611;
    static constexpr int IDM_QUANTUM_MODE_MAX      = 11612;
    static constexpr int IDM_QUANTUM_MODELS_SET    = 11613;
    static constexpr int IDM_QUANTUM_AGENTS_SET    = 11614;
    static constexpr int IDM_QUANTUM_EXEC          = 11615;
    static constexpr int IDM_QUANTUM_AUDIT         = 11616;
    static constexpr int IDM_QUANTUM_AUDIT_EXEC    = 11617;
    static constexpr int IDM_QUANTUM_STATUS        = 11618;
    static constexpr int IDM_QUANTUM_STATS         = 11619;
private:
};
