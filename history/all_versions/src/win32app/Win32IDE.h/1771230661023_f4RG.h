...
// DISABLED: Agent bridge removed to prevent model loading loops
...
private:
    ...
    // ========================================================================
    // LIVE COMPILER/DUMPBIN OUTPUT - Real Tool Integration
    // ========================================================================
    void ShowCompilerOutput();      // Compile active file, show output
    void ShowDumpbinOutput();       // Run dumpbin on active binary
    void AppendTerminalText(const std::string& text);
    std::string getCurrentFilePath() const;
    
    // ========================================================================
    // SIDEBAR PANELS - Real Implementations
    // ========================================================================
    void createProblemsPanel();
    void addProblem(const std::string& severity, const std::string& file, int line, const std::string& message);
    void clearProblems();
    HWND m_hwndProblemsList;
    
    void createGitPanel();
    void refreshGitStatus();
    HWND m_hwndGitStatus;
    
    void createSearchPanel();
    void performSearch();
    void searchDirectory(const std::string& dir, const char* query, int depth);
    
    void createExtensionsView();
...
private:
    ...
    // Model Loading State (Async)
    bool m_modelLoading;
    std::thread m_modelLoadingThread;
    std::mutex m_modelLoadingMutex;
    std::string m_modelLoadingPath;
    void loadGGUFModelAsync(const std::string& filepath);
    ...
    std::vector<std::pair<std::string, std::string>> getPromptTemplates();
    bool savePromptTemplate(const std::string& name, const std::string& body);
    void clearProblems();
