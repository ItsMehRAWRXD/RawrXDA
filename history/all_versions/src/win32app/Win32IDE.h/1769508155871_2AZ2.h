...
public:
    ...
    void onAgentStop();
    void onAgentToggleWatcher();
    void onAgentToggleConsent();
...
private:
    ...
    void onPowerShellMenuClick(int id);
    void createPowerShellMenu(HMENU parentMenu);
    std::string getScriptPath(const std::string& scriptName);
    std::string runPowerShellModuleCommand(const std::string& moduleFile, const std::string& command);
    std::string runPowerShellModuleTableCommand(const std::string& moduleFile, const std::string& command);
...
private:
    ...
    // Interactive PowerShell module functions
    std::string invokeModuleFunction(const std::string& moduleName, 
                                     const std::string& functionName,
                                     const std::vector<std::string>& args);
    std::string getModuleInventoryFormatted();
    std::string createPluginFromTemplate(const std::string& pluginName,
                                         const std::string& templateType);
    std::string getPerformanceMetrics();
    std::string runSecurityAudit();
    std::string formatPowerShellTable(const std::string& data, const std::string& title);
    
    // Utility function for PowerShell string escaping
    std::string escapePowerShellString(const std::string& input) const {
        std::string result;
        for (char c : input) {
            if (c == '\'' || c == '\\' || c == '$' || c == '`') {
                result += '`';
