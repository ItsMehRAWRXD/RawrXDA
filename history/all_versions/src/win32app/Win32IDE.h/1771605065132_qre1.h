...
// Circular Beacon System - Omnidirectional Agentic Connectivity
#include "../../include/editor_engine.h"
#include "../../include/ide/FileSystemIntegration.hpp"
#include "../../include/kernel_dispatch/KernelDispatcher.hpp"
...
// Layout canon — four panes only; everything else is a pop-up. See Win32IDE_LayoutCanon.h, ARCHITECTURE.md §6.5.
#include "Win32IDE_LayoutCanon.h"
...
// Debug: AI backend test (Ollama connectivity from IDE)
...
// NOTE: 5042–5109 are already allocated to other subsystems; use a safe free ID.
#define IDM_BACKEND_SWITCH_NATIVEASM 5199
...
struct IDESettings
{
    ...
    // Agent / Autonomy (synced with Agent menu: Max Mode, Deep Think, Deep Research, No Refusal)
    bool agentMaxMode = false;
    bool agentDeepThinking = false;
    bool agentDeepResearch = false;
    bool agentNoRefusal = false;
...
class Win32IDE
{
  ...
  public:
    ...
    // Circular Architecture Init — call AFTER construction, BEFORE createWindow
    void InitializeCircularArchitecture();

    // System integration — wire all subsystems together
    void wireAllSystems();
    void enableFileOperations();
    void enableAIFeatures();
    void enableTerminalOperations();
    void enableExtensionSystem();
    void enableDebuggingFeatures();
    void enableGitIntegration();
    ...
    // Exposed for SourceFilePicker dialog (needs path + open by path)
    std::string getCurrentFilePath() const { return m_currentFile; }
    void openFile(const std::string& filePath);

    // Exposed for feature_handlers / RawrXD_InvokeAgentCommandGui
    void handleAgentCommand(int commandId);

  private:EngineManager* m_engineManager = nullptr;
    CodexUltimate* m_codexUltimate = nullptr;

    // Circular Beacon System Manager - Omnidirectional Agentic Connectivity
    std::unique_ptr<RawrXD::CircularBeaconManager> m_circularBeaconManager;
    std::unique_ptr<BeaconClient> m_beaconClient;
    ...
    std::string getExplorerRootPath() const { return m_explorerRootPath; }
...
#if defined(RAWR_HAS_MODEL_ANATOMY)
    /** Build anatomy from current GGUF loader; returns JSON manifest or empty string if no model. */
    std::string getModelAnatomyJson(bool pretty = true) const;
    /** Compare two GGUF files (pathA vs pathB); returns JSON diff or empty on error. */
  ...
  private:// Command routing
    ...
    // Agent-facing command palette (lets agents/models invoke IDE commands)
    std::string getCommandRegistryJson() const;
    bool executeCommandById(int commandId);
    int findCommandIdByName(const std::string& name) const;
    ...
    // Win32 Debug API Integration (Real Implementation)
    void debugEventLoop();                     // Main debug event processor
    void applyDarkMode(HWND hwnd);             // DWM dark mode
    void applyDarkModeToAllSidebarControls();  // Apply to all controls
    void populateTreeAsync();                  // Async tree population for performance
  ...
  private:// ========================================================================
    ...
    struct EditorTab
    {
        ...
        RawrXD::FileSystem::EncodingInfo encodingInfo;  // Detected encoding
        uint32_t editSequence = 0;                      // Dirty tracking: bumped on each edit
        uint32_t savedSequence = 0;                     // Dirty tracking: last saved sequence
        bool IsDirty() const { return editSequence != savedSequence; }
    };
    ...
    bool m_fileModified;
    RawrXD::FileSystem::BufferManager m_bufferManager;       // Encoding-aware file I/O
    RawrXD::FileSystem::EncodingInfo m_currentEncodingInfo;  // Active file's encoding
    ...
    // Win32 Debug API State
    HANDLE m_debugProcessHandle = nullptr;
    HANDLE m_debugThreadHandle = nullptr;
    DWORD m_debugProcessId = 0;
    DWORD m_debugThreadId = 0;
    ...
    void layoutChatPanelContent(int width, int height);
    ...
    /** Verify AI backend (Ollama) reachable; runs off UI thread. Updates status bar via message. */
    bool VerifyAIBackend();
    /** C bridge used by command registry for debug backend test trigger. */
    friend void Win32IDE_runDebugTestAI(void* idePtr);
    /** Run from Debug menu: test ModelConnection in a thread and show MessageBox result. */
    void runDebugTestAI();
    /** Handle async verification result (WM_AI_BACKEND_STATUS). Updates m_aiAvailable + menu. */
    void onAIBackendVerified(bool success);
    /** Dynamically insert/enable/disable 'Test AI Backend' item in Tools menu. */
    void updateToolsMenu();
    /** Deferred AI backend initialization — spawns background Ollama verification. */
    void initializeAIBackend();
    ...
    bool m_aiAvailable = false;         // true after VerifyAIBackend (or on first successful use)
    ...
    HWND m_hwndPSBtnAgentic;
    HWND m_hwndPSBtnHotpatch;
    ...
    void triggerAgenticBeacon();
    void triggerHotpatchBeacon();
...
#define WM_AI_BACKEND_STATUS (WM_APP + 103)
    ...
    enum class AIBackendType
    {
        ...
        NativeASM = 5,  // Native MASM x64 kernel dispatch (Phase3 + PocketLab)
        Count = 6
    };
    ...
    std::string routeToNativeASM(const std::string& prompt);
    ...
    // Native ASM Kernel Dispatcher (Phase3 + PocketLab DLLs)
#if RAWRXD_HAS_POCKET_LAB || RAWRXD_HAS_PHASE3_KERNEL
    std::unique_ptr<RawrXD::Kernel::KernelDispatcher> m_kernelDispatcher;
#endif
...
#define IDM_QW_ALERT_HAPPENINGS 9824
#define IDM_QW_SLO_DASHBOARD 9830
    ...
    // Hotpatch state
    std::unique_ptr<SecureHotpatchOrchestrator> m_securePatcher;
    ...
    void cmdAlertHappenings();

    // Happenings pop-up pane — brings events to user attention
    void showHappening(const char* title, const char* detail, int severity);
    void postHappening(const char* title, const char* detail, int severity);
    void showHappeningsPane();
  ...
  private:// ── OutlineSymbol (enhanced outline model) ──
    ...
    void paintMinimapToDC(HDC hdc, const RECT& rc);
