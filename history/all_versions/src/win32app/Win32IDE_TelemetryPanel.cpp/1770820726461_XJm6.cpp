...
void Win32IDE::handleTelemetryCommand(int commandId) {
    if (!m_telemetryPanelInitialized) initTelemetryPanel();

    switch (commandId) {
        case IDM_TEL_TRACE_STATUS:       cmdTelTraceStatus();          break;
        case IDM_TEL_START_SPAN:         cmdTelStartSpan();            break;
        case IDM_TEL_AUDIT_LOG:          cmdTelAuditLog();             break;
        case IDM_TEL_AUDIT_VERIFY:       cmdTelAuditVerify();          break;
        case IDM_TEL_COMPLIANCE_REPORT:  cmdTelComplianceReport();     break;
        case IDM_TEL_VIOLATIONS:         cmdTelShowViolations();       break;
        case IDM_TEL_LICENSE_STATUS:     cmdTelLicenseStatus();        break;
        case IDM_TEL_USAGE_METER:        cmdTelUsageMeter();           break;
        case IDM_TEL_METRICS_DASHBOARD:  cmdTelMetricsDashboard();     break;
        case IDM_TEL_METRICS_FLUSH:      cmdTelMetricsFlush();         break;
        case IDM_TEL_EXPORT_AUDIT:       cmdTelExportAudit();          break;
        case IDM_TEL_EXPORT_OTLP:        cmdTelExportOTLP();           break;
        case IDM_TEL_GDPR_EXPORT:        cmdTelGDPRExport();           break;
        case IDM_TEL_GDPR_DELETE:        cmdTelGDPRDelete();           break;
        case IDM_TEL_SET_LEVEL:          cmdTelSetLevel();             break;
        case IDM_TEL_STATS:              cmdTelShowStats();            break;
        default:
            appendToOutput("[Telemetry] Unknown command: " + std::to_string(commandId) + "\n");
            break;
    }
}
