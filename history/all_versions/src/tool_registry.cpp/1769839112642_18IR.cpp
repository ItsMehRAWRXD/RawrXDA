...
bool ToolRegistry::registerTool(const ToolDefinition& toolDef) {
    ...
    m_tools[toolDef.name] = std::make_shared<ToolDefinition>(toolDef);
    m_toolConfigs[toolDef.name] = toolDef.config;
    m_statistics[toolDef.name] = ToolStats();
    m_validations[toolDef.name] = toolDef.inputValidation;
    return true;
}
...
ToolResult ToolRegistry::executeWithRetry(const std::string& toolName, const json& params, const ToolConfig& config) {
    ...
    for(int attempt = 0; attempt <= maxRetries; ++attempt) {
        ...
        result.executionContext.metrics.executionTimeMs = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
...
void ToolRegistry::updateStatistics(const ToolResult& result, const std::string& toolName) {
    ...
    if (result.success) m_statistics[toolName].successfulExecutions++;
    else m_statistics[toolName].failedExecutions++;
}
