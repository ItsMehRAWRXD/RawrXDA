...
ToolResult ToolRegistry::executeWithRetry(
    ...
    auto startTime = ToolRegistryHelpers::getCurrentTimeMs();
    ToolResult result;
    result.toolName = toolName;
    result.executionId = ToolRegistryHelpers::generateUUID();
    ...
    if (!config.enableExecution) {
        result.status = ToolExecutionStatus::SkippedByToggle;
        result.error = "Tool disabled by configuration";
        result.success = false;
        return result;
    }
    ...
    try {
        // Simple schema validation stub (check required fields)
        json schema = tool->parameterSchema.value("properties", json::object());
        json required = tool->parameterSchema.value("required", json::array());
        
        for (const auto& req : required) {
            std::string field = req.get<std::string>();
            if (!parameters.contains(field)) {
                throw std::runtime_error("Missing required parameter: " + field);
            }
        }
    } catch (const std::exception& e) {
        result.status = ToolExecutionStatus::Failed;
    ...
    while (true) {
        try {
            // Actual execution
            json output = tool->implementation(parameters);
    ...
    // 4. Record Metrics and Logging
    auto endTime = ToolRegistryHelpers::getCurrentTimeMs();
    result.executionTimeMs = endTime - startTime;
    
    if (m_logger && (config.enableDetailedLogging || !result.success)) {
        if (result.success) 
            m_logger->info("Tool " + toolName + " succeeded (" + std::to_string(result.executionTimeMs) + "ms)");
        else 
            m_logger->error("Tool " + toolName + " failed: " + result.error);
    }
