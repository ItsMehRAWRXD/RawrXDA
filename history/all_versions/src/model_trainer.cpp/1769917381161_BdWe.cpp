...
void ModelTrainer::runTraining() {
    try {
        ...
        if (!cpuEngine && m_inferenceEngine) {
             // Maybe it's not polymorphic or RTTI disabled, try reinterpret or assume
             // Actually, if we use just generic Tokenize/Generate it might be enough?
             // But we want Eval() for loss.
             // If cast fails, we fall back to a "Mock" but ideally we warn.
             logMessage("Warning: Engine does not support training/eval (CPUInferenceEngine required).");
        }
        ...
        for (int i = 0; i < m_config.epochs; ++i) {
            ...
            if (cpuEngine && !m_tokenizedData.empty()) {
                ...
                for (const auto& tokens : m_tokenizedData) {
                 ...
                 // Simulate if no engine
                 totalLoss = (float)(std::rand() % 100) / 100.0f; 
                 samples = 1;
            }
