...
// Real implementation of dataset tokenization
std::vector<std::vector<uint32_t>> ModelTrainer::tokenizeDataset() {
    ...
    if (!cpuEngine && m_inferenceEngine) {
        // Fallback or attempt unsafe cast if we know it is compatible
        // In this codebase, InferenceEngine is likely CPUInferenceEngine or compatible
         cpuEngine = (CPUInferenceEngine*)m_inferenceEngine;
    }

    if (!cpuEngine) {
        trainingError("Inference engine not available for tokenization");
        return m_tokenizedData;
    }

    logMessage("Tokenizing dataset (" + std::to_string(m_textData.size()) + " text, " + std::to_string(m_jsonData.size()) + " json)...");
    
    // Process Plain Text
    for (const auto& line : m_textData) {
        if (line.empty()) continue;
        std::vector<int32_t> tokens = cpuEngine->Tokenize(line);
        // Cast int32 to uint32
        std::vector<uint32_t> unsignedTokens(tokens.begin(), tokens.end());
        m_tokenizedData.push_back(unsignedTokens);
    }
    
    // Process JSON Data (extract "text", "prompt", or "content" fields)
    for (const auto& obj : m_jsonData) {
        std::string content;
        if (obj.contains("text")) content = obj["text"].get<std::string>();
        else if (obj.contains("prompt") && obj.contains("completion")) {
             content = obj["prompt"].get<std::string>() + " " + obj["completion"].get<std::string>();
        }
        else if (obj.contains("instruction") && obj.contains("output")) {
             content = "Instruction: " + obj["instruction"].get<std::string>() + "\nOutput: " + obj["output"].get<std::string>();
        } else {
            // Dump whole object as string fallback
            content = obj.dump();
        }

        if (!content.empty()) {
            std::vector<int32_t> tokens = cpuEngine->Tokenize(content);
            std::vector<uint32_t> unsignedTokens(tokens.begin(), tokens.end());
            m_tokenizedData.push_back(unsignedTokens);
        }
    }
    
    logMessage("Tokenization complete. Total samples: " + std::to_string(m_tokenizedData.size()));
    return m_tokenizedData;
}
void ModelTrainer::trainingStarted() { if(onTrainingStarted) onTrainingStarted(); }
void ModelTrainer::epochStarted(int e, int t) { if(onEpochStarted) onEpochStarted(e, t); }
void ModelTrainer::batchProcessed(int b, int t, float l) { /* ... */ }
void ModelTrainer::epochCompleted(int e, float l, float p) { if(onEpochCompleted) onEpochCompleted(e, l, p); }
void ModelTrainer::trainingCompleted(const std::string& o, float p) { if(onTrainingCompleted) onTrainingCompleted(o, p); }
void ModelTrainer::trainingStopped() { if(onTrainingStopped) onTrainingStopped(); }
