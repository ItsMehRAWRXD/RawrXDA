...
void RawrXDTransformer::Initialize(VkDevice device, VkPhysicalDevice physDevice, Config cfg, RawrXDModelLoader* loader) {
    ...
    printf("[RawrXD] Allocating KV Cache: Layers=%d, Ctx=%d, Dim=%d, Total=%lld elements\n", 
           config.n_layers, config.n_ctx, config.dim, (long long)kv_size);
    
    try {
        kv_cache_k.resize(kv_size);
        kv_cache_v.resize(kv_size);
    } catch (const std::exception& e) {
        printf("[RawrXD] KV Cache allocation failed: %s\n", e.what());
        return;
    }
...
std::vector<float> RawrXDTransformer::Forward(const std::vector<uint32_t>& tokens, int start_pos) {
    ...
    // DEBUG:
    // printf("Dim: %d, Layers: %d, Heads: %d\n", dim, config.n_layers, config.n_heads);
    if (dim == 0 || config.n_layers == 0) return {};
