...
public:
    ...
    void Ask(const std::string& query) {
        ...
        m_engine->GenerateStreaming(input_ids, 2048, 
            ...
            [&]() {
                ...
                // AUTO-CORRECT Logic (disabled until AdvancedFeatures fixed)
    ...
    void Compile(const std::string& filePath, const std::string& type = "auto") {
        ...
        auto result = compiler.CompileSource(filePath);
        if (result.success) {
            Print("[Agent] Compilation successful: " + result.objectFile + "\n");
        } else {
...
private:
    ...
    std::string PerformResearch(const std::string& query) {
        ...
        for (auto it = fs::recursive_directory_iterator(root, ec); it != fs::recursive_directory_iterator(); ++it) {
            ...
            if (ext != ".cpp" && ext != ".h" && ext != ".hpp" && ext != ".md" && ext != ".txt") continue;
            ...
            while (std::getline(f, line)) {
                ...
                if (lower.find(q) != std::string::npos) {
                    ...
                    if (++hits >= max_hits) return oss.str();
                }
            }
        }
