...
public:
    ...
    void Ask(const std::string& query) {
        ...
        if (m_deepThink) {
             fullPrompt = AdvancedFeatures::ChainOfThought(fullPrompt);
        }
        if (m_deepResearch) {
             fullPrompt = AdvancedFeatures::DeepResearch(fullPrompt);
        }
        if (m_noRefusal) {
             fullPrompt = AdvancedFeatures::NoRefusal(fullPrompt);
        }

        Print("[Agent] Generating response...\n");

        if (m_deepThink) Print("<thought>\nThinking process initiated...\n</thought>\n");
        ...
        int maxTok = m_maxMode ? 32768 : 2048;
    ...
    void Plan(const std::string& task) {
        ...
        if (m_deepThink) prompt = "Activate Deep Thinking for Planning.\n" + prompt;
        Ask(prompt);
    }
