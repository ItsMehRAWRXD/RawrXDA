...
private:
    ...
    std::string BuildPrompt(const std::string& userQuery) {
        ...
        if (m_deepResearch) {
             std::string researchPrompt = ::AdvancedFeatures::DeepResearch(userQuery);
             // Standard prompt wrapping
             std::string sys = "You are RawrXD, an expert AI programming assistant with Deep Research capabilities. Use the provided context to answer the user's query.";
        ...
        // Fallback to standard templating
