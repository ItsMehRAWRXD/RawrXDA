...
namespace RawrXD { namespace Telemetry {
    ...
    struct AIMetricsCollector {
        ...
        void recordOllamaRequest(const std::string& model, unsigned long long latency, bool success, unsigned long long promptT.tokens, unsigned long long completionTokens) {
            totalRequests++;
            totalTokens += (promptT.tokens + completionTokens);
        }
