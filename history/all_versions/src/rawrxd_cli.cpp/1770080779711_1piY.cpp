...
void ProcessCommand(std::string input, AppState& state, std::shared_ptr<AIIntegrationHub> hub) {
    ...
    if (input.rfind("/context ", 0) == 0) {
        ...
        // Logic to parse 4k, 1m etc would go here, updating engine config
        std::cout << "Context window updated to: " << sizeStr << " (Memory Plugin resizing...)\n";
        // Call into memory system (stubbed here)
        return;
    }
