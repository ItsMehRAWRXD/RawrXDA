            ...
            if (cmd == "!help") {
                ...
                if(!path.empty()) {
                    ...
                    if(codex.LoadBinary(path)) {
                        std::cout << "[CodexUltimate] Binary Loaded: " << path << std::endl;
                ...
                if(!path.empty()) {
                    path.erase(0, path.find_first_not_of(" \t"));
                    std::cout << "[RawrDumpBin_Final] Analyzing " << path << "...\n";
                    auto info = dumpbin.AnalyzePE(path);
                    std::cout << dumpbin.GenerateReport(info) << "\n";
                }
            }
            else if (cmd == "!disasm") {
                std::string path; std::getline(ss, path);
                if(!path.empty()) {
                    path.erase(0, path.find_first_not_of(" \t"));
                    std::cout << "[RE_NATIVE_DISASM] Disassembling " << path << "...\n";
