...
// Phase 10 Operations
#include "qtapp/ops/IncidentResponseCoordinator.h"
#include "qtapp/ops/RunbookExecutor.h"
#include "qtapp/ops/MetricAnomalyDetector.h"
#include "qtapp/ops/SLOTracker.h"
#include "qtapp/ops/PostmortemGenerator.h"
...
int main(int argc, char** argv) {
    ...
    // Initialize Phase 10 Operations
    IncidentResponseCoordinator incidentCoordinator;
    RunbookExecutor runbookExecutor;
    MetricAnomalyDetector anomalyDetector;
    ...
    std::cout << "          x=analyze file, t=generate tests, y=security scan, o=optimize scan" << std::endl;
    std::cout << "          i=list incidents, m=check SLOs, q=quit" << std::endl;
    ...
    while (running) {
        if (_kbhit()) {
            ...
            case 'i': { // List incidents
                auto incidents = incidentCoordinator.listRecent(10);
                std::cout << "\n=== Recent Incidents ===" << std::endl;
                if (incidents.isEmpty()) {
                    std::cout << "No recent incidents." << std::endl;
                } else {
                    for (const auto& inc : incidents) {
                        std::cout << "[" << inc.id.toStdString() << "] " << inc.title.toStdString() 
                                  << " (Owners: " << inc.owner.toStdString() << ")" << std::endl;
                    }
                }
                break;
            }
            case 'm': { // Check SLOs
                std::cout << "\n=== Service SLOs ===" << std::endl;
                // Example check for 'inference' service
                double avail = sloTracker.availability("inference");
                std::cout << "Inference availability: " << avail << "%" << std::endl;
                break;
            }
            case 'q':
                running = false; break;
