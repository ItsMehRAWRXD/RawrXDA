#include "gui.h"
#include "minimal_hub_stub.hpp" // Use Helper Stub instead of heavy integration
...
void RunInteractiveChat(AppState& state, std::shared_ptr<AIIntegrationHub> hub) {
    ...
    while (true) {
        ...
        AdvancedCodingAgent agent; // Transient agent for CLI commands

        if (input.rfind("/bugreport ", 0) == 0) {
            std::string context = input.substr(11);
            std::cout << "[Agent] Analyzing for bugs: " << context << "...\n";
            AgentTaskResult res = agent.detectBugs(context, "cpp"); // Default to cpp for CLI test
            ...
            // Map suggestion to optimization for now
            ...
            processed = true;
             ...
             // Use feature impl but focus on steps
             ...
             // Use inferCode directly via a helper if exposed, or task wrapper
             // For now, treat as a "feature request" for an answer
             AgentTaskResult res = agent.implementFeature("Answer this question: " + question, "text", "", {});
             std::cout << "[Answer]: " << res.generatedCode << "\n"; // heuristic fallback will catch this
             processed = true;
        }
        ...
        if (input.rfind("/agent ", 0) == 0) {
            ...
            if (!res.generatedCode.empty()) std::cout << res.generatedCode << "\n";
            if (!res.result.empty()) std::cout << res.result << "\n";
            continue;
        }
...
struct CLIConfig {
    bool syntaxHighlighting = false; // Placeholder for now
    bool verbose = false;
