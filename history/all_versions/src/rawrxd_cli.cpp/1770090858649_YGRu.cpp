...
int main(int argc, char** argv) {
    ...
    while (running) {
        std::cout << "\nRawrXD (" << (engine.IsModelLoaded() ? "Ready" : "No Model") << ")> ";
        std::string line;
        
        // Wait for input or handle background tasks (telemetry etc)
        if (!_kbhit()) {
        ...
        if (line[0] == '!' || line[0] == '/') {
            ...
            if (cmd == "!help") {
                ...
                std::cout << "!engine load <path>   : Switch engine/load model" << std::endl;
                std::cout << "!engine status        : Show drive & engine info" << std::endl;
                std::cout << "!engine 800b          : Load 800B distributed model (5-drive)" << std::endl;
                std::cout << "!agent <query>        : Ask the AI agent" << std::endl;
                std::cout << "!think <on/off>       : Toggle Deep Thinking (CoT)" << std::endl;
                std::cout << "!research <on/off>    : Toggle Deep Research (Context-Aware)" << std::endl;
                std::cout << "!maxmode <on/off>     : Toggle Max Mode (Full CPU Scaling)" << std::endl;
                ...
                std::cout << "!dumpbin <file>       : Analyze PE structure" << std::endl;
                std::cout << "!disasm <file>        : Disassemble Native Binary" << std::endl;
                std::cout << "!compile <file>       : Compile source (C/C++/ASM)" << std::endl;
                std::cout << "!status               : Show system telemetry" << std::endl;
                ...
                if (!path.empty()) {
                    ...
                    if (engine.LoadModel(path)) std::cout << "[SUCCESS] Model loaded." << std::endl;
                    else std::cout << "[ERROR] Failed to load model from " << path << std::endl;
                }
            }
            else if (cmd == "!engine") {
                std::string sub;
                ss >> sub;
                if (sub == "800b") {
                    std::cout << "Initializing 5-Drive Distributed Loader for 800B Model..." << std::endl;
                    if (multiEngine.Load800BModel("800B-Base")) {
                        std::cout << "[SUCCESS] 800B Model Online." << std::endl;
                        agent.SetContextLimit(1048576); // Auto-set to 1M
                    }
                } else if (sub == "status") {
                    auto drives = multiEngine.GetDriveInfo();
                    std::cout << "=== Drive Status (800B Multi-Path) ===" << std::endl;
                    for(auto& d : drives) {
                        std::cout << " Path: " << d.drivePath << " | Active: " << (d.isActive?"YES":"NO") << " | Space: " << (d.availableSpace/(1024*1024*1024)) << "GB\n";
                    }
                } else if (sub == "load") {
                ...
                std::cout << "Deep Thinking Strategy: " << (on ? "ENABLED" : "DISABLED") << std::endl;
            }
            else if (cmd == "!research") {
                std::string val; ss >> val;
                bool on = (val == "on" || val == "true");
                agent.SetDeepResearch(on);
                std::cout << "Deep Research Protocol: " << (on ? "ENGAGED" : "DISENGAGED") << std::endl;
            }
            else if (cmd == "!maxmode") {
                std::string val; ss >> val;
                bool on = (val == "on" || val == "true");
                agent.SetMaxMode(on);
                std::cout << "MAX MODE (Full Thread Scaling): " << (on ? "ON" : "OFF") << std::endl;
            }
            else if (cmd == "!norefusal") {
                std::string val; ss >> val;
                bool on = (val == "on" || val == "true");
                agent.SetNoRefusal(on);
                std::cout << "No-Refusal Policy: " << (on ? "ACTIVE" : "INACTIVE") << std::endl;
            }
            else if (cmd == "!autocorrect") {
                std::string val; ss >> val;
                bool on = (val == "on" || val == "true");
                agent.SetAutoCorrect(on);
                std::cout << "Auto-Correct (Hallucination Fix): " << (on ? "ON" : "OFF") << std::endl;
            }
            else if (cmd == "!context") {
                std::string arg; ss >> arg;
                size_t size = 4096;
                if (arg == "32k") size = 32768;
                else if (arg == "128k") size = 131072;
                else if (arg == "1m") size = 1048576;
                else try { size = std::stoull(arg); } catch(...) {}
                
                agent.SetContextLimit(size);
                std::cout << "Context Size Updated: " << size << " tokens." << std::endl;
            }
            else if (cmd == "!dumpbin") {
                std::string path; std::getline(ss, path);
                if(!path.empty()) {
                    path.erase(0, path.find_first_not_of(" \t"));
                    std::cout << dumpbin.DumpAll(path) << std::endl;
                }
            }
            else if (cmd == "!disasm") {
                std::string path; std::getline(ss, path);
                if(!path.empty()) {
                    path.erase(0, path.find_first_not_of(" \t"));
                    std::cout << dumpbin.DumpDisassembly(path, 0x1000, 100) << std::endl;
                }
            }
            else if (cmd == "!compile") {
                std::string path; std::getline(ss, path);
                ...
                std::cout << "CPU: " << snap.cpuTempC << "C | GPU: " << snap.gpuTempC << "C\n";
                std::cout << "Model Loaded: " << (engine.IsModelLoaded() ? "YES" : "NO") << "\n";
                std::cout << "Threads Active: " << (agent.IsMaxMode() ? "ALL" : "Standard") << "\n";
            }
            else if (cmd == "!quit") {
                running = false;
            }
            else {
                std::cout << "Unknown command: " << cmd << ". Type !help for list." << std::endl;
            }
        } else {
            // Default: Direct Chat / Agent Reasoning
            agent.Ask(line);
        }
    }
            }
