...
private:
    ...
    void handleGenerate(const HttpRequest& req, HttpResponse& res) {
         ...
         if (prompt.empty() && body.contains("messages")) {
             // Chat format support (simplified)
         ...
         if (stream) {
             // Fallback for non-streaming backend: Send one big chunk simulating stream for compatibility
             // To implement true streaming, ModelCaller needs a callback overload.
             // We can simulate streaming via NDJSON response if we wanted, but standard HTTP response in chunks is cleaner here.
             
             // NOTE: Simple Server doesn't support Chunked Transfer Encoding robustly in 'sendResponse'. 
             // Production Fix: Just return JSON for now, or update 'ModelCaller' later.
             // Returning valid JSON is better than broken stream.
             ...
             // Emulate OLLAMA stream format (one message)
             std::stringstream ss;
             ss << response.dump() << "\n"; 
             res.body = ss.str();
             res.content_type = "application/x-ndjson"; // Correct type for stream
         } else {
             json response = {
                 {"model", model},
                 {"created_at", getTimestamp()},
