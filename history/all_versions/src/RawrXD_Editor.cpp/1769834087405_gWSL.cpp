...
void Editor::cut() {
    ...
    getSelection(startL, startC, endL, endC);
    
    // Logic for range delete needs strict order
    // Assuming start <= end
    if (startL == endL) {
        // Simple case: single line
        removeInternal(startL, startC, endC - startC);
    } else {
        // Multi-line case: join lines
        String startChar = buffer.substring(buffer.getLineStart(startL) + startC, 1);
        String endChar = buffer.substring(buffer.getLineStart(endL) + endC - 1, 1);
        
        // Remove range
        removeInternal(startL, startC, buffer.getLineLength(startL) - startC); // To end of line
        for (int i = startL + 1; i < endL; ++i) {
            removeInternal(i, 0, buffer.getLineLength(i)); // Whole lines
        }
        removeInternal(endL, 0, endC); // Beginning of last line
        
        // Restore cursor position (naive)
        cursorLine = startL;
        cursorCol = startC;
        if (startChar == L"\n" || startChar == L"\r") {
            cursorLine--;
...
bool Editor::loadFile(const String& path) {
    std::ifstream f(path.toStdWString(), std::ios::binary); // Binary so we handle CRLF manually if needed or let stream do it
    if (!f) return false;
    ...
    // Convert generic string to wstring (assume UTF-8 for source files)
    ...
    if (!s.empty()) {
        ...
        if (len > 0) {
            ...
            MultiByteToWideChar(CP_UTF8, 0, s.c_str(), (int)s.length(), &ws[0], len);
             ...
             // Fallback usually ANSI
             ws = std::wstring(s.begin(), s.end());
        }
    }
    
    setText(String(ws));
    undoStack()->clear(); // Reset undo history on load
    return true;
}

bool Editor::saveFile(const String& path) {
    std::ofstream f(path.toStdWString(), std::ios::binary);
    if (!f) return false;
