...
bool Editor::loadFile(const String& path) {
    HANDLE hFile = CreateFileW(path.toStdWString().c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    // UTF-8 or UTF-16?
    // Assume UTF-8 for now and convert, or auto-detect.
    // Simplifying: Read as bytes, convert simple.
    // For robust editor: Detect BOM.
    
    std::vector<char> bytes(fileSize + 1);
    DWORD readBytes;
    if (ReadFile(hFile, bytes.data(), fileSize, &readBytes, NULL)) {
        bytes[readBytes] = 0;
        
        // Check for BOM
        int skip = 0;
        if (readBytes >= 3 && (unsigned char)bytes[0] == 0xEF && (unsigned char)bytes[1] == 0xBB && (unsigned char)bytes[2] == 0xBF) {
            skip = 3;
        }
        
        // MultiByteToWideChar (UTF8)
        int wlen = MultiByteToWideChar(CP_UTF8, 0, bytes.data() + skip, readBytes - skip, NULL, 0);
        if (wlen > 0) {
            std::wstring wstr(wlen, 0);
            MultiByteToWideChar(CP_UTF8, 0, bytes.data() + skip, readBytes - skip, &wstr[0], wlen);
            setText(String(wstr));
        } else {
            // Fallback ANSI
            std::wstring wstr(readBytes, 0);
            for(size_t i=0; i<readBytes; i++) wstr[i] = (wchar_t)bytes[i];
            setText(String(wstr));
        }
...
bool Editor::saveFile(const String& path) {
    HANDLE hFile = CreateFileW(path.toStdWString().c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (hFile == INVALID_HANDLE_VALUE) return false;
    
    String currentText = buffer.text(); // Full copy
    std::wstring wtext = currentText.toStdWString();
    
    // Convert to UTF-8
    int ulen = WideCharToMultiByte(CP_UTF8, 0, wtext.c_str(), (int)wtext.length(), NULL, 0, NULL, NULL);
    
    std::vector<char> u8buffer(ulen);
    WideCharToMultiByte(CP_UTF8, 0, wtext.c_str(), (int)wtext.length(), u8buffer.data(), ulen, NULL, NULL);
    
    DWORD written;
    bool res = WriteFile(hFile, u8buffer.data(), ulen, &written, NULL);
