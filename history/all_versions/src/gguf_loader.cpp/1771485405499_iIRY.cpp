...
// Note: ReadValue<T> is defined inline in gguf_loader.h
...
uint64_t GGUFLoader::CalculateTensorSize(const std::vector<uint64_t>& shape, GGMLType type) const {
    ...
    switch (type) {
        ...
        // F16_HALF is alias for F16 (value=1), already handled above
...
// Forward declaration for MASM kernel (defined in rawrxd_kernels.cpp)
extern "C" void DequantQ4_0_AVX512(void* src, uint16_t* dst, size_t count);
...
void GGUFLoader::DequantAndUploadQ4_0(TensorInfo& info, void* src, size_t count) {
    ...
    // Call MASM64 kernel for Q4_0 dequantization
    if (staging) {
        DequantQ4_0_AVX512(src, staging, blockCount);
    }
