...
private:
  ...
  "models": [
    ...
    // MASM AGENT_CTX struct layout (4096 bytes per agent)
    struct MasmAgentCtx {
        uint64_t agent_id;
        uint32_t model_profile;
        uint32_t model_tier;
        ...
        uint64_t last_task_ms;
        // zmm_save_area + reserved follow (not accessed from C++)
    };

    // MASM SWARM_STATE struct layout
    struct MasmSwarmState {
        uint32_t initialized;
        uint32_t agent_count;
        uint64_t architect_ctx;     // pointer to AGENT_CTX
        uint64_t coder_ctx;         // pointer to AGENT_CTX
        uint64_t ring_base;
        uint64_t ring_size;
    ...
    static const char* GetAgentStateName(uint32_t state) {
        switch (state) {
            case 0: return "idle";
            case 1: return "inferencing";
            case 2: return "stalled";
            case 0xFFFFFFFF: return "shutdown";
            default: return "unknown";
        }
    }
