...
bool VSIXLoader::LoadPluginFromDirectory(const std::filesystem::path& plugin_dir) {
    ...
    plugin->id = (std::string)manifest["id"];
    plugin->name = (std::string)manifest["name"];
    plugin->version = (std::string)manifest["version"];
    plugin->description = (std::string)manifest["description"];
    plugin->author = (std::string)manifest["author"];
...
Examples:
  !plugin load "plugins/react-advanced.vsix"
