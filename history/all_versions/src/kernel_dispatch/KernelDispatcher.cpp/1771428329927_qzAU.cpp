...
namespace RawrXD::Kernel {
    ...
    bool KernelDispatcher::Initialize() {
        ...
        if (m_pocket_lab_dll) {
            void* proc = nullptr;
            if (GetProcAddress(m_pocket_lab_dll, "PocketLabInit_Export", proc))
                m_pfn_pocket_lab_init = reinterpret_cast<PFN_PocketLabInit>(proc);
            if (GetProcAddress(m_pocket_lab_dll, "PocketLabGetStats_Export", proc))
                m_pfn_pocket_lab_get_stats = reinterpret_cast<PFN_PocketLabGetStats>(proc);
            if (GetProcAddress(m_pocket_lab_dll, "PocketLabRunCycle_Export", proc))
                m_pfn_pocket_lab_run_cycle = reinterpret_cast<PFN_PocketLabRunCycle>(proc);
            if (GetProcAddress(m_pocket_lab_dll, "PocketLabShutdown_Export", proc))
        ...
        if (m_phase3_dll) {
            void* proc = nullptr;
            if (GetProcAddress(m_phase3_dll, "Phase3Initialize_Export", proc))
                m_pfn_phase3_init = reinterpret_cast<PFN_Phase3Initialize>(proc);
            if (GetProcAddress(m_phase3_dll, "GenerateTokens_Export", proc))
                m_pfn_generate_tokens = reinterpret_cast<PFN_GenerateTokens>(proc);
            if (GetProcAddress(m_phase3_dll, "Phase3Shutdown_Export", proc))
    ...
    bool KernelDispatcher::AnalyzeModelFromBuffer(const uint8_t* gguf_header, size_t header_size, ModelMetadata& out_metadata) {
        ...
        // Detect model tier from file size heuristic
        out_metadata.param_tier = DetectModelTierFromSize(out_metadata.file_size_bytes);

        // Detect quantization from GGUF header (simplified)
        // In a real implementation, parse the full GGUF format
    ...
    bool KernelDispatcher::GetProcAddress(HMODULE dll, const char* proc_name, void*& out_proc) {
        if (!dll) {
            return false;
        }
    ...
    QuantFormat KernelDispatcher::DetectQuantFromGGUFHeader(const uint8_t* header) {
        ...
        // Simplified: look for quantization hints in GGUF header
        // In a real implementation, would parse full GGUF format
        // For now, default to Q4_K_M
