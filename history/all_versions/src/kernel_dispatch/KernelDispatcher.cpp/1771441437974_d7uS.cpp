...
namespace RawrXD::Kernel {
    ...
    bool KernelDispatcher::InvokePowerInferLoop(uint32_t max_tokens, uint64_t& out_tokens_processed) {
        ...
        // PocketLabRunCycle takes token_count, returns tokens actually processed
        int64_t result = m_pfn_pocket_lab_run_cycle(static_cast<int64_t>(max_tokens));
        if (result < 0) {
            return false;
        }

        out_tokens_processed = static_cast<uint64_t>(result);

        // Query detailed stats after run
        if (m_pfn_pocket_lab_get_stats) {
            // Stats buffer: [tokens:8][sparse:8][gpu:8][cpu:8][tier:4][spare:28]
            alignas(8) uint8_t stats_buf[64] = {};
            m_pfn_pocket_lab_get_stats(stats_buf);
            // tokens_processed is at offset 0
            uint64_t tokens_from_stats = *reinterpret_cast<uint64_t*>(stats_buf);
            if (tokens_from_stats > out_tokens_processed)
                out_tokens_processed = tokens_from_stats;
        }
