...
AutonomousIntelligenceOrchestrator::AutonomousIntelligenceOrchestrator(void* parent) {
    ...
    if (featureEngine) {
        ...
        // featureEngine should also ideally know about modelManager to run local models
        // But the header didn't show `setModelManager`, maybe it goes through CloudManager as "local" provider?
    }
...
AutonomousIntelligenceOrchestrator::~AutonomousIntelligenceOrchestrator() {
    ...
    std::lock_guard<std::mutex> lock(sessionMutex);
    if (sessions[this]->backgroundThread.joinable()) {
        sessions[this]->backgroundThread.join();
    }
    sessions.erase(this);
}
