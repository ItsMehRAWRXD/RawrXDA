...
check_strings:
    ...
    mov rdx, r13
    sub rdx, r14
    call MatchStringReference
    test eax, eax
    jz next_byte
    
    ; Record string reference (type 4)
    mov rcx, r14
    mov edx, 4                  ; type 4 = string ref
    mov r8d, eax
    push rax
    call RecordPattern
    pop rax
...
;--------------------------------------------------------------------------------
; Match string reference pattern  
; RCX = position
; RDX = remaining bytes
; Returns: RAX = pattern length if matched, 0 otherwise
;--------------------------------------------------------------------------------
MatchStringReference PROC
    cmp rdx, 7
    jb no_string_match
    
    ; Check for lea rcx, [rip+rel32] (48 8D 0D ?? ?? ?? ??)
    cmp byte ptr [rcx], 48h
    jne check_lea_rdx
    cmp byte ptr [rcx+1], 8Dh
    jne check_lea_rdx
    cmp byte ptr [rcx+2], 0Dh
    jne check_lea_rdx
    mov eax, 7
    ret
    
check_lea_rdx:
    ; Check for lea rdx, [rip+rel32] (48 8D 15 ?? ?? ?? ??)
    cmp byte ptr [rcx], 48h
    jne check_mov_imm
    cmp byte ptr [rcx+1], 8Dh
    jne check_mov_imm
    cmp byte ptr [rcx+2], 15h
    jne check_mov_imm
    mov eax, 7
    ret
...
no_string_match:
    xor eax, eax
    ret
MatchStringReference ENDP

;--------------------------------------------------------------------------------
; Record pattern to global pattern array
; RCX = binary offset
; EDX = pattern type (0=entry, 1=exit, 2=call, 3=loop)
; R8D = pattern length
;--------------------------------------------------------------------------------
RecordPattern PROC
    push rbx
    ...
    mov eax, g_pattern_count
    cmp eax, 64
    jae rp_full
    
    ; Calculate array offset
    mov edi, eax
    imul edi, sizeof CODE_PATTERN
    lea rdi, [g_patterns + rdi]
    
    ; Store offset as first 8 bytes of signature
    mov qword ptr [rdi + CODE_PATTERN.signature], rcx
    mov dword ptr [rdi + CODE_PATTERN.sig_length], r8d
    mov dword ptr [rdi + CODE_PATTERN.pattern_type], edx
    mov dword ptr [rdi + CODE_PATTERN.semantic_type], edx
    mov dword ptr [rdi + CODE_PATTERN.confidence], 100
    
    ; Increment counter
    inc eax
    mov g_pattern_count, eax
...
;--------------------------------------------------------------------------------
; PE Import Table Detection
; RCX = binary base
; RDX = binary size
; Returns: RAX = import table RVA (0 if not found)
;--------------------------------------------------------------------------------
DetectImportTable PROC
    push rbx
    sub rsp, 20h
    ...
    ; Get import directory RVA (offset 0x80 in optional header for PE32+)
    mov eax, dword ptr [rax + 24 + 112]  ; Import Directory RVA
...
dit_fail:
    ...
    pop rbx
    ret
DetectImportTable ENDP

;--------------------------------------------------------------------------------
; Data Section Reconstruction
; Scans for data patterns and ASCII strings
; Returns: RAX = data references found
;--------------------------------------------------------------------------------
Reconstructor_ScanData PROC
    push rbx
    push r12
    push r13
    push r14
    ...
    mov r12, g_context.binary_base
    mov r13, g_context.binary_size
    xor r14d, r14d              ; data ref counter
    xor rbx, rbx                ; offset
...
data_scan_loop:
    ...
    ; Check for printable ASCII string (at least 4 chars)
    lea rcx, [r12 + rbx]
    mov rdx, r13
    sub rdx, rbx
    call DetectASCIIString
    test eax, eax
    jz data_check_wide
...
data_check_wide:
    ; Check for wide string (Unicode)
    lea rcx, [r12 + rbx]
    mov rdx, r13
    sub rdx, rbx
    call DetectWideString
    test eax, eax
    jz data_next
...
data_scan_done:
    ...
    pop rbx
    ret
Reconstructor_ScanData ENDP

;--------------------------------------------------------------------------------
; Detect ASCII string (4+ printable chars, null terminated)
; RCX = position
; RDX = remaining bytes
; Returns: RAX = string length including null terminator (0 if not string)
;--------------------------------------------------------------------------------
DetectASCIIString PROC
    cmp rdx, 4
    jb das_not_string
...
das_loop:
    ...
    inc r8d
    inc r9
    cmp r9, 1024                ; Max string length
    jb das_loop
    jmp das_not_string
    
das_check_length:
    cmp r8d, 4                  ; At least 4 printable chars
    jb das_not_string
    
    lea rax, [r9 + 1]          ; Include null terminator
    ret
    
das_not_string:
    xor eax, eax
    ret
DetectASCIIString ENDP

;--------------------------------------------------------------------------------
; Detect wide string (Unicode, 4+ printable chars, null terminated)
; RCX = position
; RDX = remaining bytes
; Returns: RAX = string length in bytes including null terminator (0 if not string)
;--------------------------------------------------------------------------------
DetectWideString PROC
    cmp rdx, 8
    jb dws_not_string
...
dws_loop:
    ...
    ; Check if printable ASCII in low byte, high byte = 0
    cmp ah, 0
    jne dws_not_string
    cmp al, 20h
    jb dws_not_string
...
dws_check_length:
    ...
    lea rax, [r9 + 2]          ; Include null terminator
    ret
    
dws_not_string:
    xor eax, eax
    ret
DetectWideString ENDP

;--------------------------------------------------------------------------------
; PE Export Table Detection
; RCX = binary base
; RDX = binary size
; Returns: RAX = export table RVA (0 if not found)
;--------------------------------------------------------------------------------
DetectExportTable PROC
    push rbx
    sub rsp, 20h
    ...
    ; Get export directory RVA (offset 0x78 in optional header for PE32+)
    mov eax, dword ptr [rax + 24 + 96]  ; Export Directory RVA
...
det_fail:
    ...
    pop rbx
    ret
DetectExportTable ENDP

;--------------------------------------------------------------------------------
; Detect relocations in binary
; RCX = binary base
; RDX = binary size
; Returns: RAX = relocation table RVA (0 if not found)
;--------------------------------------------------------------------------------
DetectRelocations PROC
    push rbx
    sub rsp, 20h
    ...
    ; Get base relocation directory RVA (offset 0x90 in optional header for PE32+)
    mov eax, dword ptr [rax + 24 + 120]  ; BaseReloc Directory RVA
...
drl_fail:
    ...
    pop rbx
    ret
DetectRelocations ENDP

;--------------------------------------------------------------------------------
; Track cross-references (calls to specific function)
; RCX = FUNCTION_INFO* (target function)
; Returns: RAX = number of xrefs found
;--------------------------------------------------------------------------------
TrackCrossReferences PROC
    push rbx
    push r12
    push r13
    push r14
    ...
    mov r12, rcx                            ; Target function
    mov r13, qword ptr [r12 + FUNCTION_INFO.entry_offset]
    mov r14, g_context.binary_base
    mov r15, g_context.binary_size
    xor ebx, ebx                            ; xref counter
...
xref_scan:
    ...
    ; Check for E8 call (direct relative call)
    lea rcx, [r14 + r8]
    mov rdx, r15
    sub rdx, r8
    cmp rdx, 5
    ...
    cmp byte ptr [rcx], 0E8h                ; call rel32
    jne xref_next
    
    ; Calculate call target
    mov eax, dword ptr [rcx + 1]           ; Get rel32
    lea r9, [r8 + 5]                        ; Next instruction offset
    add r9, rax                             ; Add relative offset (signed)
    
    ; Check if it calls our target function
    cmp r9, r13
    jne xref_next
...
xref_done:
    ...
    pop rbx
    ret
TrackCrossReferences ENDP

;--------------------------------------------------------------------------------
; Enhanced BuildASM with data section and xrefs
; Returns: RAX = success (1) or failure (0)
;--------------------------------------------------------------------------------
Reconstructor_BuildASM_Enhanced PROC
    push rbx
    push r12
    push r13
    push r14
    ...
    ; Emit all functions with xref info
    xor ebx, ebx
enh_build_loop:
    cmp ebx, r13d
    jae enh_build_done
    ...
    ; Track xrefs to this function
    mov rcx, r14
    push r15
    call TrackCrossReferences
    pop r15
    
    ; Add xref count comment if any
    test eax, eax
    jz enh_no_xrefs
    
    lea rdx, sz_xref_comment
    mov rcx, r15
    call AppendString
    mov r15, rax
    mov edx, eax
...
enh_build_done:
    ...
    pop rbx
    ret
Reconstructor_BuildASM_Enhanced ENDP
