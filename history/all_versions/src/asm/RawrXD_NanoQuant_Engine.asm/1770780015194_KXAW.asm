...
;   - 1.0625 bits per element (25.8x compression vs FP16)
...
NanoQuant_Init PROC FRAME
    ...
    ; --- Leaf 1: FMA3 (ECX[12]), F16C (ECX[29]) ---
    mov     eax, 1
    cpuid
...
@@b_step_loop:
    ...
    vmovups ymm0, ymmword ptr [rsi + rcx*32]       ; W[8]
    vmovups ymm1, ymmword ptr [r12 + rcx*32]       ; Z[8]
    vmovups ymm2, ymmword ptr [r13 + rcx*32]       ; U[8]
...
@@scale_loop:
    ...
    vmovups ymm1, ymmword ptr [rsi + rcx*32]       ; W[8]
    ...
    vpblendvps ymm5, ymmword ptr [neg_one_f32], ymmword ptr [one_f32], ymm4
...
@@z_step_loop:
    ...
    ; Load U[i] and W[i]
    vmovups ymm2, ymmword ptr [r13 + rcx*32]       ; U[8]
    vmovups ymm3, ymmword ptr [rsi + rcx*32]       ; W[8]
    ...
    vmovups ymmword ptr [r12 + rcx*32], ymm5       ; Store Z
...
@@u_step_loop:
    ...
    vmovups ymm2, ymmword ptr [r12 + rcx*32]       ; Z[8]
    vsubps  ymm5, ymm5, ymm2                       ; alpha*B - Z

    vmovups ymm3, ymmword ptr [r13 + rcx*32]       ; U[8]
    vaddps  ymm3, ymm3, ymm5                       ; U += alpha*B - Z
    vmovups ymmword ptr [r13 + rcx*32], ymm3
...
@@residual_loop:
    ...
    vmovups ymm2, ymmword ptr [r12 + rcx*32]       ; Z
...
@@residual_done:
    ...
    vcomiss xmm0, xmm14                             ; residual vs tolerance
...
; AVX2 fallback: expands bits via vpbroadcastd + vpcmpeqd + vpblendvps.
;
; RCX = src (NQ1 block)
...
@@dq_avx2_loop:
    ...
    ; vpblendvps uses bit 31 of mask: 0xFFFFFFFF has bit 31 set
    vpblendvps ymm3, ymm7, ymm6, ymm2
...
; Uses vpblendvps for bit expansion, then FMA.
;
...
@@vd_avx2_loop:
    ...
    vpblendvps ymm3, ymm4, ymm6, ymm3              ; ±1.0
...
@@mr_sub_col:
    ...
    shr     eax, 3
    mov     r10d, [rsp]
    movzx   r9d, byte ptr [rdi + NQM_HEADER_SIZE + r10 + rax]
    mov     eax, edx
    and     eax, 7
    bt      r9d, eax
    setc    r9b
...
@@rq4k_dq_byte:
    ...
    movzx   r8d, byte ptr [rsi + 68 + rax + rdx]
...
@@disp_dequant:
    mov     rcx, rdx
    mov     rdx, r8
    jmp     NQ1_Dequant                            ; Tail-call (single block)
