...
;   - Apply single-instruction hotfixes in-process (VirtualProtect + patch)
;   - CRC32 integrity verification of patched regions
;   - Trampoline installation for function-level replacement
;   - Atomic compare-and-swap patch application (no torn writes)
;   - Rollback journal: every patch records original bytes for undo
;
; Exports:
;   asm_selfpatch_init          — Initialize self-patch subsystem
;   asm_selfpatch_scan_text     — Scan .text for known bug patterns
;   asm_selfpatch_apply         — Apply single patch with rollback journal
;   asm_selfpatch_verify_crc    — CRC32 verify a patched region
;   asm_selfpatch_install_tramp — Install 14-byte x64 trampoline
;   asm_selfpatch_rollback      — Rollback a patch from journal
;   asm_selfpatch_get_stats     — Get patch statistics
;   asm_selfpatch_scan_nop_sled — Detect accidental NOP slides
;   asm_selfpatch_cas_patch     — Atomic 8-byte compare-and-swap patch
;
...
@@vcrc_found:
    ...
    movzx   edx, DWORD PTR [rbx + JOURNAL_ENTRY.patch_size]
    call    compute_crc32
