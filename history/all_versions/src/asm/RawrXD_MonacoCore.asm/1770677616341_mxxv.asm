...
@ensure_realloc:
    ...
    ; Save newCapacity to shadow space before calls clobber registers
    mov [rsp], edi                  ; [rsp] = newCapacity (preserved across calls)
    ...
    mov r9d, [rsp]                  ; dwBytes = newCapacity from shadow space
    call HeapReAlloc
    ...
    mov [rbx + GB_pBuffer], rax
    mov edi, [rsp]                  ; Restore newCapacity into EDI
    ...
    lea rdi, [r8 + rax]             ; Dest (clobbers EDI/newCapacity)

    ; Save newCapacity and new gapEnd before rep movsb clobbers RDI
    mov [rsp], edi                  ; [rsp] = newCapacity (save again - EDI about to change)
    ; Actually EDI was already clobbered by lea rdi above.
    ; Recalculate: newCapacity is on stack from earlier save.
    ; newGapEnd = newCapacity - bytesAfterGap = eax (computed above, before lea rdi)
    ; Save eax as newGapEnd before it gets used
    mov [rsp + 8], eax              ; [rsp+8] = newGapEnd
