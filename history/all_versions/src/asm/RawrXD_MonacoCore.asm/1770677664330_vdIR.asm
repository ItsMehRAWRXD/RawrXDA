...
@ensure_realloc:
    ...
    mov [rsp], edi                  ; Save newCapacity on shadow space
    ...
    mov r9d, [rsp]                  ; dwBytes = newCapacity
    call HeapReAlloc
    ...
    mov [rsp + 8], eax              ; Save newGapEnd on shadow space
    ...
    mov eax, [rsp + 8]              ; Recover newGapEnd from shadow space
    mov [rbx + GB_gapEnd], eax
    jmp @ensure_update_cap

@ensure_no_move:
    ; No data after gap, so gapEnd = newCapacity
    mov eax, [rsp]                  ; newCapacity
    mov [rbx + GB_gapEnd], eax

@ensure_update_cap:
    mov edi, [rsp]                  ; Recover newCapacity from shadow space
