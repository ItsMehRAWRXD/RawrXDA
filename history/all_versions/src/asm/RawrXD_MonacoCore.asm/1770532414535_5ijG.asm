...
@ensure_realloc:
    ...
    call GetProcessHeap
    mov rcx, rax                    ; hHeap
    xor edx, edx                    ; dwFlags = 0 (HEAP_REALLOC_IN_PLACE not guaranteed)
    ...
    ; Old: [content_before_gap | GAP | content_after_gap ...]
    ; New: [content_before_gap | BIGGER GAP | content_after_gap ... ]
    ...
    ; Dest:   pBuffer + (newCapacity - bytesAfterGap)
    mov r8, [rbx + GB_pBuffer]
    lea rsi, [r8 + rax]             ; Source
    mov eax, edi
    sub eax, ecx                    ; new gapEnd = newCapacity - bytesAfterGap
    lea rdi, [r8 + rax]             ; Dest
...
MC_GapBuffer_Insert proc
    ...
    mov rsi, r13                    ; Source = text
    mov ecx, esi                    ; Count = len
    rep movsb

    ; Update gapStart and used, count newlines
    add [rbx + GB_gapStart], esi
    add [rbx + GB_used], esi

    ; Count newlines in inserted text
    mov rsi, r13
    mov ecx, esi                    ; Wait, ESI was len but now RSI = text pointer after rep
    ; Recalculate: len was saved in local. Let's use the stack value.
    ; Actually ESI still holds len (it wasn't changed by the rep since ECX was the counter)
    ; Correction: ESI was len, ECX was len, after rep movsb ECX=0, RSI/RDI advanced.
    ; Need original text pointer and length.
    mov rsi, r13                    ; Restore text ptr
    xor edx, edx                   ; Newline counter
    mov ecx, esi                   ; Wait, ESI was modified? No — ESI = len was set before rep.
    ; Let me re-check: ESI = len (set from R9D). Then we do mov rsi, r13 which clobbers RSI.
    ; But the len value is still in ESI... no, RSI is the 64-bit reg containing ESI.
    ; After mov rsi, r13, ESI is gone. We need to save len.
    ; Fix: len was saved, let's re-derive from gapStart change
    mov ecx, [rbx + GB_gapStart]
    sub ecx, r12d                   ; len = new gapStart - original pos
