...
vision_project_avx2 PROC
    ...
    mov     r14d, DWORD PTR [rsp + 40 + 56 + 40]  ; r14d = cols (input dim)
                                ; 40 (sub rsp) + 7*8 (pushes) = 96; param at rsp+96+40=136
    ; Fix: re-calculate stack offset for 5th parameter
    ; 7 pushes = 56 bytes, sub rsp 40 = total 96 shifted
    ; 5th param originally at [rsp+40] before pushes/sub = [rsp + 96 + 40] = [rsp+136]
    ; Actually, 5th parameter on stack in x64 is at rsp+40 at call site
    ; After 7 pushes (56 bytes) + sub rsp 40 = 96 bytes shifted
    ; So 5th param is at [rsp + 96 + 40] but we already pushed ret addr (+8)
    ; Total: [rsp + 96 + 8 + 32] = depends on exact frame
    ; Let's use a safer approach: load from correct position
    mov     r14d, DWORD PTR [rsp + 96 + 40]
...
project_avx2_loop:
    ...
    vmovups ymm1, YMMWORD PTR [rbx + ecx*4]    ; 8 weight values
    vmovups ymm2, YMMWORD PTR [rdi + ecx*4]    ; 8 input values
...
project_scalar_loop:
    ...
    vmovss  xmm1, DWORD PTR [rbx + ecx*4]
    vmovss  xmm2, DWORD PTR [rdi + ecx*4]
...
norm_sq_avx2:
    ...
    vmovups ymm1, YMMWORD PTR [rbx + eax*4]
...
norm_sq_scalar:
    ...
    vmovss  xmm1, DWORD PTR [rbx + eax*4]
...
norm_div_avx2:
    ...
    vmovups YMMWORD PTR [rbx + eax*4], ymm1
...
norm_div_scalar:
    ...
    vmovss  DWORD PTR [rbx + eax*4], xmm1
...
dot_avx2:
    ...
    vmovups ymm1, YMMWORD PTR [rbx + eax*4]
    vmovups ymm2, YMMWORD PTR [rcx + eax*4]
...
dot_scalar:
    ...
    vmovss  xmm1, DWORD PTR [rbx + eax*4]
    vmovss  xmm2, DWORD PTR [rcx + eax*4]
    vfmadd231ss xmm0, xmm1, xmm2
...
scale_avx2:
    ...
    vmovups ymm0, YMMWORD PTR [rcx + eax*4]
    vmulps  ymm0, ymm0, ymm2
    vmovups YMMWORD PTR [rcx + eax*4], ymm0
...
scale_scalar:
    ...
    vmovss  xmm0, DWORD PTR [rcx + eax*4]
    vmulss  xmm0, xmm0, xmm1
    vmovss  DWORD PTR [rcx + eax*4], xmm0
...
add_avx2:
    ...
    vmovups ymm0, YMMWORD PTR [rbx + eax*4]    ; dst
    vmovups ymm1, YMMWORD PTR [rcx + eax*4]    ; src
    vfmadd231ps ymm0, ymm1, ymm3               ; dst += src * weight
    vmovups YMMWORD PTR [rbx + eax*4], ymm0
...
add_scalar:
    ...
    vmovss  xmm0, DWORD PTR [rbx + eax*4]
    vmovss  xmm1, DWORD PTR [rcx + eax*4]
    vfmadd231ss xmm0, xmm1, xmm2
    vmovss  DWORD PTR [rbx + eax*4], xmm0
