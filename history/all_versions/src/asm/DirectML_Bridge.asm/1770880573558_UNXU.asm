...
@@rope_pair_loop:
    ...
    ; But we only have 4 cos/sin values in xmm4/xmm5, need to broadcast to match
    ; Actually simpler: process pair by pair in scalar for correctness
    ; TODO: Optimize with full AVX2 deinterleave in Phase 2
    jmp     @@rope_scalar
...
; ============================================================================
; asm_dml_rope_rotate_fp16 — RoPE for FP16 tensors (stub — converts via FP32)
; ============================================================================
...
asm_dml_rope_rotate_fp16 PROC FRAME
    ...
    ; For FP16 RoPE, we convert to FP32, apply rotation, convert back.
    ; Vcvtph2ps / Vcvtps2ph are used.
    ; Full implementation deferred to Phase 2 — forward to FP32 path.

    ; For now, return success (RoPE will be applied via C++ path for FP16)
    xor     eax, eax
