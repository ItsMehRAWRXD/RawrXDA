...
void AIDebugger::requestFixFromModel(const QJsonObject &debugInfo)
{
    ...
    // Use the AI model to generate a fix
    if (m_modelClient) {
        QString suggestedDiff = m_modelClient->complete(prompt);
        emit fixSuggested(suggestedDiff);
    } else {
        emit fixSuggested("Error: No AI model available for fix suggestion");
    }
}