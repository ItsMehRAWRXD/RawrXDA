...
public:
    ...
    std::string generate(const std::string& prompt, const CloudConnectionConfig& config);
    ...
    bool checkProviderHealth(const CloudConnectionConfig& config);
    void checkProviderHealthAsync(const CloudConnectionConfig& config,
                                 std::function<void(bool)> callback);
    
    // Model listing
    std::vector<std::string> listModels(const CloudConnectionConfig& config);
    void listModelsAsync(const CloudConnectionConfig& config,
                        std::function<void(const std::vector<std::string>&)> callback);
    
    // Request building (for debugging/logging)
    nlohmann::json buildRequestBody(const std::string& prompt, const CloudConnectionConfig& config);
...
private:
    ...
    ApiResponse performRequest(const std::string& url, const nlohmann::json& body, const CloudConnectionConfig& config);
};
