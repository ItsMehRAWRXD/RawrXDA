    ...
    DWORD WINAPI Titan_InferenceThread(LPVOID lpParam) {
        ...
        while (!ctx->shouldExit) {
            ...
            if (g_InputState == 1) {
                 ...
                 // Since Generate() currently does one pass of logits, we loop here
                 std::string response;
                 ...
                 for(int i=0; i<max_new_tokens; i++) {
                     ...
                     // Stop token check (eos)
                     if (best_token == 2) break; // Assuming 2 is EOS
                 }
                 
                 response = ctx->engine->Detokenize(tokens); // We need Detokenize implemented too
