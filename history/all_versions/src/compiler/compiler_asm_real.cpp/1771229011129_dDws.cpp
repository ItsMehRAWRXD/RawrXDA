...
private:
    ...
    std::string find_internal_masm() {
        // Check custom compilers directory FIRST (no SDK)
        const char* custom_asm_paths[] = {
            "D:\\rawrxd\\compilers\\nasm\\nasm.exe",
            "D:\\rawrxd\\compilers\\masm_ide\\ml64.exe",
            "D:\\rawrxd\\compilers\\universal_compiler_runtime.exe",
            "C:\\rawrxd\\compilers\\nasm\\nasm.exe",
            "E:\\rawrxd\\compilers\\nasm\\nasm.exe",
            "C:\\nasm\\nasm.exe",
            "E:\\nasm\\nasm.exe"
        ...
        for (const auto& path : custom_asm_paths) {
        ...
        // Fallback: Internal bundled compiler
        const char* internal_paths[] = {
            "src\\masm\\masm_solo_compiler.exe",
            "..\\..\\src\\masm\\masm_solo_compiler.exe",
            "d:\\rawrxd\\src\\masm\\masm_solo_compiler.exe",
            ".\\masm_solo_compiler.exe"
        };
        ...
        if (last_slash != std::string::npos) {
            std::string internal_masm = exe_dir.substr(0, last_slash) + "\\masm_solo_compiler.exe";
            DWORD attrs = GetFileAttributesA(internal_masm.c_str());
            if (attrs != INVALID_FILE_ATTRIBUTES && !(attrs & FILE_ATTRIBUTE_DIRECTORY)) {
                return internal_masm;
            }
        }
    ...
    std::string find_internal_linker() {
        // Check internal linker locations (bundled with app)
        const char* internal_paths[] = {
            "src\\masm\\internal_link.exe",
            "..\\..\\src\\masm\\internal_link.exe",
            "d:\\rawrxd\\src\\masm\\internal_link.exe",
            ".\\internal_link.exe"
        };
        ...
        size_t last_slash = exe_dir.find_last_of("\\/");
        if (last_slash != std::string::npos) {
            std::string internal_linker = exe_dir.substr(0, last_slash) + "\\internal_link.exe";
            DWORD attrs = GetFileAttributesA(internal_linker.c_str());
            if (attrs != INVALID_FILE_ATTRIBUTES && !(attrs & FILE_ATTRIBUTE_DIRECTORY)) {
