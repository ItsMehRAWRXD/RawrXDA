...
CompilerEngine::CompilerEngine(QObject* parent)
    : QObject(parent)
{
    ...
    connect(m_worker, &CompilerWorker::progress, this, &CompilerEngine::onWorkerProgress);
    connect(m_worker, &CompilerWorker::finished, this, &CompilerEngine::onWorkerFinished);
    connect(m_worker, &CompilerWorker::error, this, &CompilerEngine::onWorkerError);
...
CompileResult CompilerEngine::compile(const CompileOptions& options)
{
    ...
    // Read source file
    QString source = readFile(options.inputFile);
    if (source.isEmpty()) {
...
CompileResult CompilerEngine::compileString(const QString& source, const CompileOptions& options)
{
    QElapsedTimer timer;
    timer.start();
...
void CompilerEngine::compileAsync(const CompileOptions& options)
{
    if (m_isCompiling.load()) {
        emit errorOccurred(tr("Compilation already in progress"));
        return;
    }
    ...
    if (source.isEmpty()) {
        m_isCompiling = false;
        emit errorOccurred(tr("Failed to read input file: %1").arg(options.inputFile));
        return;
    }
    
    emit compilationStarted(options);
    
    // Start worker
    QMetaObject::invokeMethod(m_worker, "compile", Qt::QueuedConnection,
...
CompileResult CompilerEngine::runPipeline(const QString& source, const CompileOptions& options)
{
    CompileResult result;
    ...
    // Stage 8: Linking
    QString outputFile = options.outputFile;
    if (outputFile.isEmpty()) {
        QFileInfo fi(options.inputFile);
#ifdef Q_OS_WIN
        outputFile = fi.absolutePath() + "/" + fi.baseName() + ".exe";
#else
        outputFile = fi.absolutePath() + "/" + fi.baseName();
#endif
    }
    ...
    result.outputSize = QFileInfo(outputFile).size();
...
bool CompilerEngine::runLexer(const QString& source, CompileResult& result)
{
    m_tokens.clear();
    ...
    while (pos < source.length()) {
        ...
        QString twoChar = source.mid(pos, 2);
        QString threeChar = source.mid(pos, 3);
        
        static const QHash<QString, int> threeCharOps = {
            {"<<=", 148}, {">>=", 149}, {"...", 132}, {"===", 116}
        };
        
        static const QHash<QString, int> twoCharOps = {
            {"==", 116}, {"!=", 117}, {"<=", 120}, {">=", 121},
            {"<<", 122}, {">>", 123}, {"&&", 124}, {"||", 125},
            {"++", 126}, {"--", 127}, {"->", 128}, {"=>", 129},
            {"::", 130}, {"..", 131}, {"+=", 140}, {"-=", 141},
        ...
        static const QHash<QChar, int> singleCharOps = {
            {'+', 100}, {'-', 101}, {'*', 102}, {'/', 103}, {'%', 104},
            {'&', 105}, {'|', 106}, {'^', 107}, {'~', 108}, {'!', 109},
            {'?', 110}, {':', 111}, {';', 112}, {',', 113}, {'.', 114},
            {'=', 115}, {'<', 118}, {'>', 119},
...
void CompilerEngine::collectSymbols(ASTNode* node, const QString& scope)
{
    if (!node) return;
    ...
    switch (node->type) {
        case 1: // AST_FUNCTION
            sym.name = node->name;
            sym.type = node->typeName.isEmpty() ? "void" : node->typeName;
            sym.kind = "function";
...
bool CompilerEngine::checkSemantics(ASTNode* node, CompileResult& result)
{
    ...
    switch (node->type) {
        ...
        case 14: { // AST_CALL
            ...
            static const QSet<QString> builtins = {"print", "println", "input", "len", "str", "int", "float"};
...
void CompilerEngine::generateIRForFunction(ASTNode* func, QTextStream& ir)
{
    ir << "define @" << func->name << "(";
    ...
    ir << ") -> " << (func->typeName.isEmpty() ? "void" : func->typeName) << " {\n";
    ir << "entry:\n";
...
void CompilerEngine::generateIRForBlock(ASTNode* block, QTextStream& ir, int indent)
{
...
void CompilerEngine::generateIRForStatement(ASTNode* stmt, QTextStream& ir, int indent)
{
    QString prefix(indent * 2, ' ');
    
    switch (stmt->type) {
        case 2: { // AST_VARIABLE
            QString tmp = QString("%%t%1").arg(m_tempCounter++);
            if (!stmt->children.isEmpty()) {
                ir << prefix << tmp << " = alloca " << stmt->typeName << "\n";
                QString val = generateIRForExpression(stmt->children[0].data(), ir, indent);
        ...
        case 5: { // AST_IF
            QString labelThen = QString(".if_then_%1").arg(m_labelCounter++);
            QString labelElse = QString(".if_else_%1").arg(m_labelCounter++);
            QString labelEnd = QString(".if_end_%1").arg(m_labelCounter++);
            
            if (!stmt->children.isEmpty()) {
                QString cond = generateIRForExpression(stmt->children[0].data(), ir, indent);
        ...
        case 6: { // AST_WHILE
            QString labelCond = QString(".while_cond_%1").arg(m_labelCounter++);
            QString labelBody = QString(".while_body_%1").arg(m_labelCounter++);
            QString labelEnd = QString(".while_end_%1").arg(m_labelCounter++);
...
QString CompilerEngine::generateIRForExpression(ASTNode* expr, QTextStream& ir, int indent)
{
    QString prefix(indent * 2, ' ');
    QString result;
    ...
    switch (expr->type) {
        case 17: { // AST_LITERAL
            if (expr->typeName == "int" || expr->typeName == "float") {
                ...
                result = QString("@str_%1").arg(m_stringCounter++);
        ...
        case 12: { // AST_BINARY_OP
            QString left = generateIRForExpression(expr->children[0].data(), ir, indent);
            QString right = generateIRForExpression(expr->children[1].data(), ir, indent);
            QString tmp = QString("%%t%1").arg(m_tempCounter++);
            
            QString op = expr->attributes["operator"].toString();
            QString irOp;
        ...
        case 13: { // AST_UNARY_OP
            QString operand = generateIRForExpression(expr->children[0].data(), ir, indent);
            QString tmp = QString("%%t%1").arg(m_tempCounter++);
        ...
        case 19: { // AST_ASSIGN
            if (expr->children.size() >= 2) {
                QString val = generateIRForExpression(expr->children[1].data(), ir, indent);
                QString var = expr->children[0]->name;
...
void CompilerEngine::optimizeConstantFolding()
{
    ...
    QString ir = QString::fromUtf8(m_irBuffer);
    
    // Find patterns like: %t1 = add 5, 3
    QRegularExpression re(R"((%t\d+) = (add|sub|mul|div) (\d+), (\d+))");
    QRegularExpressionMatchIterator it = re.globalMatch(ir);
    
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        QString var = match.captured(1);
        QString op = match.captured(2);
        int left = match.captured(3).toInt();
        int right = match.captured(4).toInt();
        ...
        ir.replace(match.captured(0), QString("%1 = const %2").arg(var).arg(result));
    }
...
void CompilerEngine::optimizeDeadCodeElimination()
{
    ...
    QString ir = QString::fromUtf8(m_irBuffer);
    QStringList lines = ir.split('\n');
    
    // Find all used variables
    QSet<QString> used;
    QRegularExpression useRe(R"(%t\d+)");
    for (const QString& line : lines) {
        if (line.contains("=")) {
            QString rhs = line.mid(line.indexOf('=') + 1);
            QRegularExpressionMatchIterator it = useRe.globalMatch(rhs);
            while (it.hasNext()) {
    ...
    for (const QString& line : lines) {
        if (line.contains("= const") || line.contains("= add") || 
            ...
            QRegularExpressionMatch match = useRe.match(line);
...
void CompilerEngine::generateX86_64Assembly(QTextStream& out)
{
    ...
    QHash<QString, QString> regMap; // IR variable to register mapping
    int stackOffset = 0;
    ...
    for (const QString& line : lines) {
        ...
        if (trimmed.startsWith("define @")) {
            // Function definition
            QRegularExpression re(R"(define @(\w+))");
            ...
            // IR instruction
            QRegularExpression assignRe(R"((%t\d+) = (\w+) (.+))");
            QRegularExpressionMatch match = assignRe.match(trimmed);
            if (match.hasMatch()) {
                QString dest = match.captured(1);
                QString op = match.captured(2);
                QString args = match.captured(3);
                
                if (op == "alloca") {
                    stackOffset += 8;
                    regMap[dest] = QString("[rbp-%1]").arg(stackOffset);
                    out << "    ; " << dest << " = alloca\n";
                } else if (op == "add" || op == "sub" || op == "mul") {
                    QStringList parts = args.split(", ");
                    QString left = parts.value(0);
                    QString right = parts.value(1);
                    ...
                    QStringList parts = args.split(", ");
                    QString val = parts.value(0);
                    QString loc = parts.value(1);
                    ...
                    QRegularExpression callRe(R"(@(\w+)\(([^)]*)\))");
                    QRegularExpressionMatch callMatch = callRe.match(args);
                    if (callMatch.hasMatch()) {
                        QString funcName = callMatch.captured(1);
                        QString callArgs = callMatch.captured(2);
                        
                        // Push arguments (simplified)
                        if (!callArgs.isEmpty()) {
                            QStringList argList = callArgs.split(", ");
...
QString CompilerEngine::resolveOperand(const QString& op, const QHash<QString, QString>& regMap)
{
    if (op.startsWith("%t")) {
...
void CompilerEngine::generateX86_32Assembly(QTextStream& out)
{
    out << "; Generated by RawrXD Compiler v" << version() << "\n";
    out << "; Target: x86-32\n\n";
...
void CompilerEngine::generateARM64Assembly(QTextStream& out)
{
    out << "// Generated by RawrXD Compiler v" << version() << "\n";
    out << "// Target: ARM64\n\n";
...
bool CompilerEngine::runLinker(CompileResult& result, OutputFormat format, const QString& outputFile)
{
    // Write output file
    return writeFile(outputFile, m_outputBuffer);
}

QVector<Token> CompilerEngine::tokenize(const QString& source, SourceLanguage language)
{
    CompileResult result;
...
QVector<Token> CompilerEngine::getHighlightTokens(const QString& source, SourceLanguage language)
{
    return tokenize(source, language);
}

QSharedPointer<ASTNode> CompilerEngine::parse(const QString& source, SourceLanguage language)
{
    CompileResult result;
    runLexer(source, result);
    if (!runParser(result)) {
...
QVector<Symbol> CompilerEngine::getOutline(const QString& source, SourceLanguage language)
{
    auto ast = parse(source, language);
    if (!ast) return {};
    
    QVector<Symbol> outline;
...
QVector<Diagnostic> CompilerEngine::analyze(const QString& source, SourceLanguage language)
{
    CompileResult result;
    runLexer(source, result);
    runParser(result);
...
SourceLanguage CompilerEngine::detectLanguage(const QString& filePath)
{
    QString ext = QFileInfo(filePath).suffix().toLower();
    
    static const QHash<QString, SourceLanguage> extMap = {
...
QStringList CompilerEngine::supportedLanguages()
{
    return {"Eon", "C", "C++", "Rust", "Go", "Python", "JavaScript", "TypeScript",
            "Java", "C#", "Swift", "Kotlin", "Dart", "Lua", "Ruby", "PHP", "Assembly"};
}

void CompilerEngine::emitProgress(CompilationStage stage, int percent, const QString& message)
{
    emit compilationProgress(stage, percent, message);
    if (m_verbose) {
        emit statusMessage(message);
    }
}

void CompilerEngine::addDiagnostic(DiagnosticSeverity severity, const QString& message,
                                    int line, int column, CompileResult& result)
{
    ...
    diag.line = line;
    diag.column = column;
    diag.timestamp = QDateTime::currentDateTime();
    result.diagnostics.append(diag);
    
    emit compilationDiagnostic(diag);
}

QString CompilerEngine::readFile(const QString& path)
{
    QFile file(path);
...
bool CompilerEngine::writeFile(const QString& path, const QByteArray& data)
{
    QFile file(path);
...
void CompilerEngine::onWorkerProgress(int percent, const QString& message)
{
    emit compilationProgress(CompilationStage::Idle, percent, message);
}
...
void CompilerEngine::onWorkerError(const QString& error)
{
    m_isCompiling = false;
...
void CompilerWorker::compile(const CompileOptions& options, const QString& source)
{
    m_cancelled = false;
...
bool ProjectManager::createProject(const QString& name, const QString& directory, SourceLanguage primaryLanguage)
{
    m_projectName = name;
    m_projectDirectory = directory;
    m_projectFile = QDir(directory).filePath(name + ".rawrxd");
    m_projectOpen = true;
    m_modified = true;
...
bool ProjectManager::addFile(const QString& filePath)
{
    ProjectFile file;
    file.path = filePath;
    file.relativePath = QDir(m_projectDirectory).relativeFilePath(filePath);
...
void ProjectManager::build()
{
    ...
    QVector<ProjectFile> sources = getSourceFiles();
    ...
    for (const auto& file : sources) {
        ...
        emit buildProgress(file.path, ++current, total);
    ...
    m_building = false;
    emit buildFinished(errors == 0, errors, warnings);
}
...
void ProjectManager::buildFile(const ProjectFile& file, const BuildConfiguration& config)
{
    ...
    options.outputFile = QDir(config.intermediateDirectory.isEmpty() ? 
                              m_projectDirectory : config.intermediateDirectory)
                         .filePath(QFileInfo(file.path).baseName() + ".o");
    ...
    for (const auto& diag : result.diagnostics) {
        emit buildDiagnostic(diag);
    }
    
    emit buildOutput(result.success ? 
...
void ProjectManager::linkProject(const BuildConfiguration& config)
{
    ...
    emit buildOutput(tr("Linking %1 object files...").arg(m_objectFiles.size()));
}
...
QVector<ProjectFile> ProjectManager::getSourceFiles() const
{
    QVector<ProjectFile> sources;
...
void ProjectManager::loadProjectFile(const QString& path)
{
    QFile file(path);
    ...
    m_projectDirectory = QFileInfo(path).absolutePath();
    m_projectFile = path;
    ...
    // Load configurations
    m_configurations.clear();
    QJsonArray configsArray = root["configurations"].toArray();
...
void ProjectManager::saveProjectFile(const QString& path)
{
    QJsonObject root;
...
QJsonObject BuildConfiguration::toJson() const
{
    return QJsonObject{
        ...
        {"debugInfo", debugInfo},
        {"defines", QJsonArray::fromStringList(defines)},
        {"includePaths", QJsonArray::fromStringList(includePaths)},
        {"libraryPaths", QJsonArray::fromStringList(libraryPaths)},
        {"libraries", QJsonArray::fromStringList(libraries)},
        {"outputDirectory", outputDirectory},
        {"intermediateDirectory", intermediateDirectory}
    };
}

BuildConfiguration BuildConfiguration::fromJson(const QJsonObject& json)
{
    BuildConfiguration config;
    config.name = json["name"].toString();
    config.target = static_cast<TargetArch>(json["target"].toInt());
    config.format = static_cast<OutputFormat>(json["format"].toInt());
    config.optimization = static_cast<OptimizationLevel>(json["optimization"].toInt());
...
QJsonObject ProjectFile::toJson() const
{
    return QJsonObject{
        ...
        {"compile", compile},
        {"dependencies", QJsonArray::fromStringList(dependencies)}
    };
}
...
ProjectFile ProjectFile::fromJson(const QJsonObject& json)
{
    ...
    file.language = static_cast<SourceLanguage>(json["language"].toInt());
...
void SyntaxHighlighter::loadTheme(const QString& theme)
{
    if (theme == "light") {
        m_keywordFormat.setForeground(QColor("#0000FF"));
        m_typeFormat.setForeground(QColor("#267F99"));
...
void SyntaxHighlighter::highlightEon(const QString& text)
{
    // Keywords
    static QRegularExpression keywordRe(
    ...
    QRegularExpressionMatchIterator it = keywordRe.globalMatch(text);
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
    ...
    // Types
    static QRegularExpression typeRe(
    ...
    // Strings
    static QRegularExpression stringRe(R"("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')");
    ...
    // Numbers
    static QRegularExpression numberRe(R"(\b\d+\.?\d*([eE][+-]?\d+)?[fFlL]?\b|0x[0-9a-fA-F]+|0b[01]+)");
...
void SyntaxHighlighter::highlightC(const QString& text)
{
    // C keywords
    static QRegularExpression keywordRe(
    ...
    // Preprocessor
    static QRegularExpression preprocRe(R"(#\s*(include|define|undef|ifdef|ifndef|if|else|elif|endif|pragma|error|warning)[^\n]*)");
...
void SyntaxHighlighter::highlightCpp(const QString& text)
{
    ...
    // C++ specific keywords
    static QRegularExpression cppKeywordRe(
    ...
    QRegularExpressionMatchIterator it = cppKeywordRe.globalMatch(text);
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        setFormat(match.capturedStart(), match.capturedLength(), m_keywordFormat);
    }
...
void SyntaxHighlighter::highlightRust(const QString& text)
{
    static QRegularExpression keywordRe(
...
void SyntaxHighlighter::highlightGeneric(const QString& text)
{
    // Strings
    static QRegularExpression stringRe(R"("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|`(?:[^`\\]|\\.)*`)");
    ...
    // Numbers
    static QRegularExpression numberRe(R"(\b\d+\.?\d*([eE][+-]?\d+)?[fFlLuU]*\b|0x[0-9a-fA-F]+|0b[01]+|0o[0-7]+)");
    it = numberRe.globalMatch(text);
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        setFormat(match.capturedStart(), match.capturedLength(), m_numberFormat);
...
void DiagnosticsManager::addDiagnostic(const Diagnostic& diagnostic)
{
    ...
    emit diagnosticAdded(diagnostic);
    updateTreeWidget();
}

void DiagnosticsManager::addDiagnostics(const QVector<Diagnostic>& diagnostics)
{
    m_diagnostics.append(diagnostics);
    for (const auto& diag : diagnostics) {
        emit diagnosticAdded(diag);
    }
    updateTreeWidget();
}

QVector<Diagnostic> DiagnosticsManager::getDiagnosticsForFile(const QString& filePath) const
{
...
QVector<Diagnostic> DiagnosticsManager::getErrors() const
{
    QVector<Diagnostic> result;
    for (const auto& diag : m_diagnostics) {
...
QVector<Diagnostic> DiagnosticsManager::getWarnings() const
{
    QVector<Diagnostic> result;
    for (const auto& diag : m_diagnostics) {
...
void DiagnosticsManager::updateTreeWidget()
{
    ...
    for (const auto& diag : m_diagnostics) {
        ...
        item->setText(0, icon);
        item->setText(1, QFileInfo(diag.filePath).fileName());
        item->setText(2, QString::number(diag.line));
        item->setText(3, diag.message);
        item->setData(0, Qt::UserRole, QVariant::fromValue(diag));
    }
}
...
CompletionProvider::CompletionProvider(QObject* parent)
    : QObject(parent)
{
}

QVector<CompletionItem> CompletionProvider::getCompletions(const QString& source, int line, int column)
{
...
QVector<CompletionItem> CompletionProvider::getKeywordCompletions()
{
    QVector<CompletionItem> items;
...
QVector<CompletionItem> CompletionProvider::getSymbolCompletions(const QString& source)
{
    QVector<CompletionItem> items;
    
    if (!m_compiler) return items;
    
    QVector<Symbol> symbols = m_compiler->getOutline(source, m_language);
...
bool DebuggerInterface::startDebugging(const QString& executable, const QStringList& args)
{
...
#ifdef Q_OS_WIN
    m_process->start("gdb", {"--interpreter=mi", executable} + args);
#else
    m_process->start("gdb", {"--interpreter=mi", executable} + args);
#endif
    ...
    for (const auto& bp : m_breakpoints) {
        if (bp.enabled) {
            sendCommand(QString("-break-insert %1:%2").arg(bp.filePath).arg(bp.line));
        }
    }
...
int DebuggerInterface::addBreakpoint(const QString& file, int line, const QString& condition)
{
    Breakpoint bp;
    ...
    if (m_debugging.load()) {
        QString cmd = QString("-break-insert %1:%2").arg(file).arg(line);
        if (!condition.isEmpty()) {
            cmd += QString(" -c \"%1\"").arg(condition);
        }
        sendCommand(cmd);
    }
...
void DebuggerInterface::removeBreakpoint(int id)
{
    for (int i = 0; i < m_breakpoints.size(); i++) {
        if (m_breakpoints[i].id == id) {
            if (m_debugging.load()) {
                sendCommand(QString("-break-delete %1").arg(id));
            }
...
void DebuggerInterface::sendCommand(const QString& command)
{
...
void DebuggerInterface::parseResponse(const QString& response)
{
    ...
    if (response.contains("*stopped")) {
        ...
        QRegularExpression fileRe(R"(fullname="([^"]+)")");
        ...
        if (fileMatch.hasMatch() && lineMatch.hasMatch()) {
            ...
            frame.line = lineMatch.captured(1).toInt();
            emit stepped(frame);
        }
    }
}
...
void CompilerWidget::setupUI()
{
    ...
    // Connect diagnostics tree click
    connect(m_diagnosticsTree, &QTreeWidget::itemDoubleClicked, 
            this, [this](QTreeWidgetItem* item, int) {
        QVariant data = item->data(0, Qt::UserRole);
        if (data.isValid()) {
            Diagnostic diag = data.value<Diagnostic>();
            emit diagnosticClicked(diag.filePath, diag.line, diag.column);
        }
    });
}
...
{
    connect(m_compiler, &CompilerEngine::compilationStarted,
            this, &CompilerWidget::onCompilationStarted);
    connect(m_compiler, &CompilerEngine::compilationProgress,
            this, &CompilerWidget::onCompilationProgress);
    connect(m_compiler, &CompilerEngine::compilationFinished,
            this, &CompilerWidget::onCompilationFinished);
    
    connect(m_projectManager, &ProjectManager::buildStarted,
            this, &CompilerWidget::onBuildStarted);
    connect(m_projectManager, &ProjectManager::buildProgress,
            this, &CompilerWidget::onBuildProgress);
    connect(m_projectManager, &ProjectManager::buildFinished,
            this, &CompilerWidget::onBuildFinished);
    connect(m_projectManager, &ProjectManager::buildOutput,
            this, [this](const QString& msg) {
        m_outputLog->appendPlainText(msg);
    });
    connect(m_projectManager, &ProjectManager::buildDiagnostic,
            this, [this](const Diagnostic& diag) {
        m_diagnosticsManager->addDiagnostic(diag);
    });
}

void CompilerWidget::compileCurrentFile()
{
    emit compilationRequested(QString());
}
...
void CompilerWidget::onCompilationProgress(CompilationStage stage, int percent, const QString& message)
{
    m_progressBar->setValue(percent);
    m_progressBar->setFormat(QString("%1 - %p%").arg(Utils::stageName(stage)));
}
...
void CompilerWidget::onBuildProgress(const QString& file, int current, int total)
{
...
void CompilerWidget::onBuildFinished(bool success, int errors, int warnings)
{
    ...
    QString result = success ? "✅ Build Successful" : "❌ Build Failed";
