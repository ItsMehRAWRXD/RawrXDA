...
namespace nlohmann {
    ...
    public:
        ...
        static json array(std::initializer_list<json> init) {
            json j;
            j.type_ = 5;
            j.array_ = std::make_shared<std::vector<json>>(init);
            return j;
        }
        ...
        auto obj_begin()       { if (!object_) object_ = std::make_shared<std::map<std::string, json>>(); return object_->begin(); }
        auto obj_end()         { if (!object_) object_ = std::make_shared<std::map<std::string, json>>(); return object_->end(); }
        auto obj_begin() const { if (object_) return object_->begin(); static std::map<std::string, json> e; return e.begin(); }
        auto obj_end()   const { if (object_) return object_->end();   static std::map<std::string, json> e; return e.end(); }

        // Array read-only access: for (const auto& item : arr.arr_items())
