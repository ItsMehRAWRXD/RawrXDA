...
static json executeProcessSafely(const std::string& command, const std::vector<std::string>& args, int timeoutMs = 30000, const std::string& cwd = "") {
    ...
    while (true) {
        ...
        if (ReadFile(hChildStd_OUT_Rd, buffer, min(sizeof(buffer), bytesAvailable), &dwRead, NULL) && dwRead != 0) {
    ...
    while (true) {
        ...
        if (ReadFile(hChildStd_ERR_Rd, buffer, min(sizeof(buffer), bytesAvailable), &dwRead, NULL) && dwRead != 0) {
...
void registerSystemTools(ToolRegistry* registry) {
    ...
    runCmd.name = "execute_command";
    runCmd.description = "Execute a shell command";
    runCmd.category = ToolCategory::System;
    runCmd.inputSchema = {
        {"type", "object"},
        {"properties", {
            {"command", {{"type", "string"}}},
    ...
    readFile.category = ToolCategory::FileSystem;
    ...
    listDir.description = "List files in directory";
    listDir.category = ToolCategory::FileSystem;
    listDir.inputSchema = {
         {"type", "object"},
         {"properties", {{"path", {{"type", "string"}}}}},
         {"required", {"path"}}
