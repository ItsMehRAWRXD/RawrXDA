...
int registerFileSystemTools(ToolRegistry* registry) {
    ...
    // listDirectory tool
    {
        ...
        toolDef.handler = [](const json& params) -> json {
            ...
            if (dirPath.isEmpty()) {
                return json{{"success", false}, {"error", "dirPath is required"}};
            }
            
            QDir dir(dirPath);
            if (!dir.exists()) {
                return json{{"success", false}, {"error", "Directory not found: " + dirPath.toStdString()}};
            }
    ...
    // grepSearch tool
    {
        ...
        toolDef.handler = [](const json& params) -> json {
            ...
            if (pattern.isEmpty() || path.isEmpty()) {
                return json{{"success", false}, {"error", "pattern and path are required"}};
            }
...
int registerExecutionTools(ToolRegistry* registry) {
    ...
    // executeCommand tool
    {
        ...
        toolDef.handler = [](const json& params) -> json {
            ...
            if (program.isEmpty()) {
                return json{{"success", false}, {"error", "program is required"}};
            }
