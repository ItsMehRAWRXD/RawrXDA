...
void Settings::setValue(const std::string& key, const SettingsValue& value) {
    {
        std::unique_lock lock(mutex_);
        std::string fullKey = composeKeyLocked(key, true);
        data_[fullKey] = value;
        if (!inReadArray_) {
...
void Settings::remove(const std::string& key) {
    {
        std::unique_lock lock(mutex_);
        std::string prefix = composeKeyLocked(key, true);
...
std::string Settings::composeKeyLocked(const std::string& key, bool includeArrayIndex) const {
    // No lock - assumes caller holds mutex
    std::string fullKey = groupPrefix();
    
    if (includeArrayIndex && !arrayName_.empty()) {
        // We're inside an array context
        fullKey += std::to_string(arrayIndex_);
        fullKey += "/";
    }
...
std::string Settings::composeGroupPrefixLocked() const {
    // No lock - assumes caller holds mutex
    return groupPrefix();
}

std::string Settings::composeArraySizeKeyLocked() const {
    // No lock - assumes caller holds mutex
    return groupPrefix() + arrayName_ + "/size";
}
