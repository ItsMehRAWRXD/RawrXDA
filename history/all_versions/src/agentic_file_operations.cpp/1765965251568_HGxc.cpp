...
AgenticFileOperations::AgenticFileOperations(QObject* parent)
    ...
    , m_maxHistorySize(50)  // Default value
{
    // Read configuration from environment variables
    bool ok = false;
    int historySize = qgetenv("AGENTIC_FILE_OPERATIONS_MAX_HISTORY").toInt(&ok);
    if (ok && historySize > 0) {
        m_maxHistorySize = historySize;
        qDebug() << "[AgenticFileOperations] Configuration loaded: max history size =" << m_maxHistorySize;
    } else {
        qDebug() << "[AgenticFileOperations] Using default configuration: max history size =" << m_maxHistorySize;
    }
