...
; Assemble: ml64 /c /Zi /arch:AVX512 RawrXD_Standalone_Inference.asm
...
GGUFTensorInfo STRUC
    ...
    type           DWORD ?             ; GGML_TYPE
    offset         QWORD ?             ; In file
GGUFTensorInfo ENDS
...
GGUF_LoadModel PROC FRAME
    ...
    mov [rbx].TransformerContext.DataOffset, 010000h ; Simplified
...
@@outer_loop:
    ...
    mov rax, [g_GlobalCtx].TransformerContext.HiddenSize
    shr rax, 5                      ; /32
    
@@inner_loop:
    movzx ecx, WORD PTR [r13]
    ; vcvtph2ps requires AVX-512 F16 or similar, or manual expansion
    ; For now, just a placeholder of the user's intent
    add r13, 18
    add r12, 128
    dec rax
    jnz @@inner_loop
...
MultiHeadAttention PROC FRAME
    ...
    mov r13, [g_GlobalCtx].TransformerContext.LayerWQ
    mov rcx, rdx
    mov rdx, [r13+r12*8]
    mov r8, [g_GlobalCtx].TransformerContext.QBuffer
    call MatMul_Q4
    
    mov rcx, [g_GlobalCtx].TransformerContext.QBuffer
    mov rdx, [g_GlobalCtx].TransformerContext.SeqLen
    call RoPE_Permute
    ...
    mov rcx, [g_GlobalCtx].TransformerContext.LayerWO
    mov rdx, [g_GlobalCtx].TransformerContext.AttnOut
    call MatMul_Q4
...
@@gen_loop:
    ...
    mov eax, [token_buffer] ; dummy next token
