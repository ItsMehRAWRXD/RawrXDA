...
CloudApiClient::CloudApiClient(UniversalModelRouter* parent) {
    // WinHttpOpen is done per request usually or cached session
}
...
ApiResponse CloudApiClient::performRequest(const std::string& url_str, const nlohmann::json& body, const ModelConfig& config) {
    ...
    if (bResults) {
        ...
        response.error_message = "WinHttp Request failed error: " + std::to_string(GetLastError());
    }
