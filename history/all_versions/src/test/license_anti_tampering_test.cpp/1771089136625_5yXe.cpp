...
void test_license_key_valid() {
    ...
    key.features.low = 0xFFFFFFFFFFFFFFFF;
    key.features.high = 0x0000000000FFFFFF;  // All Enterprise features enabled
    ...
    // Verify integrity
    bool valid = verifyLicenseKeyIntegrity(key, hwid);
    printf("  License key integrity: %s\n", valid ? "valid" : "invalid");
    // Note: May fail if HWID doesn't match actual hardware

    printf("  ✓ PASS (subject to hardware HWID match)\n");
}
