...
public:
    ...
    uint32_t GetMetadataInt(const std::string& key) {
        if (key.find("embedding_length") != std::string::npos) return n_embd;
        if (key.find("block_count") != std::string::npos) return n_layers;
        if (key.find("head_count") != std::string::npos && key.find("kv") == std::string::npos) return n_heads;
        if (key.find("head_count_kv") != std::string::npos) return n_heads_kv ? n_heads_kv : n_heads; 
        if (key.find("tokens") != std::string::npos) return n_vocab;
        if (key.find("context_length") != std::string::npos) return n_ctx;
        return 0;
    }
    
    float GetMetadataFloat(const std::string& key) {
        if (key.find("rope.freq_base") != std::string::npos) return f_rope_freq_base;
        if (key.find("layer_norm_rms_epsilon") != std::string::npos) return f_rms_norm_eps;
        return 0.0f;
    }
    ...
    uint8_t* ParseMetadata(uint8_t* ptr, uint64_t count) {
        for (uint64_t i = 0; i < count; i++) {
            ...
            switch (type) {
                ...
                case 5: // INT32
                case 6: // FLOAT32
                    // Capture config - Handle Float vs Int
                    if (type == 6) { 
                         if (key == "llama.rope.freq_base") f_rope_freq_base = *(float*)ptr;
                         if (key == "llama.attention.layer_norm_rms_epsilon") f_rms_norm_eps = *(float*)ptr;
                    } else {
                        if (key == "llama.block_count") n_layers = *(uint32_t*)ptr;
                        if (key == "llama.attention.head_count") n_heads = *(uint32_t*)ptr;
                        if (key == "llama.attention.head_count_kv") n_heads_kv = *(uint32_t*)ptr;
