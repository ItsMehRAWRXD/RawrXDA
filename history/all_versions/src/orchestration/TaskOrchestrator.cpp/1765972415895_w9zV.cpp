...
void TaskOrchestrator::orchestrateTask(const QString& naturalLanguageDescription)
{
    ...
    // Step 1: Parse natural language and split into subtasks
    QList<TaskDefinition> tasks = parseNaturalLanguage(naturalLanguageDescription);
    
    if (tasks.isEmpty()) {
        emit errorOccurred("Failed to parse task description");
        return;
    }
    ...
    // Step 3: Balance workload if needed
