    ...
    if (enable_repl) {
        ...
        while (true) {
            ...
            // ── Phase 26: ReverseEngineered Kernel Commands ──
                ...
                std::cout << "[Scheduler] Work-stealing scheduler: ACTIVE\n"
                          << "  NUMA-aware workers: auto-detected\n"
                          << "  Deadlock detection: enabled (1s interval)\n";
                uint64_t tick = GetHighResTick();
                uint64_t us = TicksToMicroseconds(tick);
                std::cout << "  Timer tick: " << tick << " (" << us << " us)\n";
            }
            else if (input == "/scheduler submit") {
                // Submit a no-op test task to verify scheduler
                auto testFn = [](void* arg) -> void {
                    (void)arg;
                    // Intentionally empty test task
                };
                int64_t taskId = Scheduler_SubmitTask(
                    reinterpret_cast<void*>(+testFn), nullptr, 0, 0, nullptr);
                if (taskId > 0) {
                    std::cout << "[Scheduler] Test task submitted: id=" << taskId << "\n";
                    void* result = Scheduler_WaitForTask(taskId, 5000);
                    std::cout << "[Scheduler] Task completed: "
                              << (result ? "OK" : "timeout") << "\n";
                } else {
                ...
                std::cout << "[ConflictDetector] Wait-for graph deadlock detection: ACTIVE\n"
                          << "  DFS cycle detection: enabled\n"
                          << "  Scan interval: 1000ms\n";
            }
            else if (input == "/heartbeat status") {
                std::cout << "[Heartbeat] UDP gossip protocol monitor\n"
                          << "  Status: check /heartbeat add to enable\n";
            }
                ...
                std::cout << "[GPU DMA] Fence-based DMA transfer engine: ready\n"
                          << "  AllocateDMABuffer: available (VirtualAlloc backend)\n";
            }
