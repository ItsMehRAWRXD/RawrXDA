...
; QuantQ5_1: Real Q5_1 quantization logic (port of ggml-quants.c)
; Input: RCX = src (float*), RDX = dst (block_q5_1*), R8 = k (int64_t)
; Output: RAX = 0 (success)
; Clobbers: RAX, RCX, RDX, R8-R11 (volatile per x64 ABI)
; Preserves: RBX, RSI, RDI, R12, R13, R14, RBP (non-volatile, saved/restored)
; Assumes QK5_1 = 32 (block size)
;
; Stack layout after prologue:
;   push rbp          ; RSP -= 8  (16-aligned after CALL)
;   push rbx          ; RSP -= 8
;   push rsi          ; RSP -= 8
;   push rdi          ; RSP -= 8
;   push r12          ; RSP -= 8
;   push r13          ; RSP -= 8
;   push r14          ; RSP -= 8  (7 pushes = 56 bytes)
;   sub rsp, 40       ; shadow space (32) + local (8) = 40
;   Total: 96 bytes, RSP is 16-aligned (8 from CALL + 56 + 40 = 104 = 8 mod 16? No...)
;   CALL pushes 8 -> RSP is 8 mod 16
;   7 pushes = 56 -> RSP is 8+56 = 64 mod 16 = 0 -> 16-aligned
;   sub rsp, 40 -> RSP = 0+40 = 40 mod 16 = 8 -> misaligned!
;   sub rsp, 48 -> RSP = 0+48 = 48 mod 16 = 0 -> 16-aligned!
;   So: 7 pushes + sub rsp,48 = 104 total after CALL ret addr, 16-aligned.
QuantQ5_1:
    push rbp
    mov rbp, rsp
    push rbx
    push rsi
    push rdi
    push r12                    ; non-volatile: MUST save (used as loop counter)
    push r13                    ; non-volatile: MUST save (used as quant loop counter)
    push r14                    ; non-volatile: MUST save (used for qh accumulator)
    sub rsp, 48                 ; 32-byte shadow space + 16 bytes local storage, 16-byte aligned

    ; Map parameters per Windows x64 calling convention
    mov rsi, rcx                ; rsi = src (float*), was in RCX
    mov rdi, rdx                ; rdi = dst (block_q5_1*), was in RDX
    mov rcx, r8                 ; rcx = k (number of floats), was in R8
...
Q5_1_done:
    xor rax, rax
    add rsp, 48                 ; deallocate local + shadow space
