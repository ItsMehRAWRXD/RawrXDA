...
std::string CICDSettings::deployModel(const std::string& jobId, const std::string& runId)
{
    ...
    // Executes the standard deployment script
    std::cout << "Starting deployment sequence..." << std::endl;
    int result = system("deploy_pipeline.bat");
    if (result != 0) {
        std::cerr << "Deployment script failed with code: " << result << std::endl;
    } else {
