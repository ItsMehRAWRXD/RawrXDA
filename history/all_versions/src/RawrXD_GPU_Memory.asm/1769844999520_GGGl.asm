...
VramHeap STRUCT
    ...
    ; Array of blocks. Defined as bytes to avoid array-of-struct syntax issues
    ; Access using offsets or typed pointers
    HeapBlocks          BYTE 229376 DUP (?) ; 4096 * 56
...
Vram_Initialize PROC
    ...
    ; manual offset calc because HeapBlocks is BYTE array
    lea rax, g_VramHeap.HeapBlocks  ; Base address
...
@init_blocks:
    ...
    ; Access byte offset
    lea rdx, g_VramHeap.HeapBlocks[rax]
...
@search_loop:
    ...
    mov eax, ebx
    imul rax, VRAM_BLOCK_STRUCT_SIZE
    lea rdi, [rsi].VramHeap.HeapBlocks[rax]
...
@find_slot:
    mov eax, ecx
    imul rax, VRAM_BLOCK_STRUCT_SIZE
    lea r8, [rsi].VramHeap.HeapBlocks[rax]
...
@found_block:
    ...
    dec eax
    imul rax, VRAM_BLOCK_STRUCT_SIZE
    lea rcx, [rsi].VramHeap.HeapBlocks[rax]
...
Vram_SubmitUpload PROC
    ...
    inc eax
    and eax, 63 ; VRAM_TRANSFER_QUEUE_DEPTH - 1
    ...
    ; Advance queue tail
    ; safe manual load/store without unary complexity
    mov eax, [rsi].VramHeap.QueueTail
    inc eax
    and eax, 63H
    mov [rsi].VramHeap.QueueTail, eax
