...
bool SemanticIndexEngine::LoadIndex(const std::string& filePath) {
    // Placeholder: would parse the JSON format written by SaveIndex
    // For now, trigger a full rebuild if load fails
    std::ifstream in(filePath);
    if (!in.is_open()) return false;
    
    // Simple validation
    std::string line;
    if (!std::getline(in, line) || line.find("version") == std::string::npos) {
        return false;
    }
    
    // Full implementation would parse all symbols/deps back
    return true;
}
...
void SemanticIndexEngine::extractDependencies(const std::string& filePath,
    ...
    while (std::getline(lines, line)) {
        ...
        if (language == "cpp" || language == "c") {
            ...
            // import "path"
            if (std::regex_search(line, dm, std::regex(R"(import\s+"([^"]+)")"))) {
                DependencyEdge dep;
                dep.sourceFile = filePath;
                dep.targetFile = dm[1].str();
                dep.importExpr = line;
