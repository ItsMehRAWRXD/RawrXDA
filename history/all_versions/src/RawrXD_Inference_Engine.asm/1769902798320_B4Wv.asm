...
@layer_loop:
    cmp r14, [rsi].ModelHandleStruct.LayerCount  ; assuming ModelHandle points to this struct
    jge @layers_done
    ...
    inc eax
    mov (KvCacheSlot PTR [rdi]).ContextLength, eax, (KvCacheSlot PTR [rdi]).ContextLength
    
    inc r14mov (KvCacheSlot PTR [rdi]).ContextLength, eax
    jmp @layer_loop
    
@layers_done:jmp @layer_loop
    ; Final layer norm + output projection to logits
    ; Copy logits to output buffer
    ojection to logits
    inc g_InferenceContext.TokensGenerated; Copy logits to output buffer
    
    add rsp, 40inc g_InferenceContext.TokensGenerated
    pop r14
    pop r13, 40
    pop r12
    pop rdi
    ...
    pop rbx
    ret
Inference_SubmitToken ENDP rbx
...
Inference_SubmitBatch PROC FRAME
    .endprolog═════════════════════════════════════════════════
    ; Group sequences by similar length for efficient batchingitBatch PROC FRAME
...
Inference_ReleaseSequence PROC FRAME
    .endprolog═════════════════════════════════════════════
    aseSequence PROC FRAME
    ; Calc cache ptr.endprolog
    lea rdx, g_InferenceContext.KvCache
    mov eax, SIZEOF KvCacheSlot
    imul rax, rcx.KvCache
    add rdx, raxOF KvCacheSlot
    x
    mov (KvCacheSlot PTR [rdx]).Occupied, 0add rdx, rax
    mov (KvCacheSlot PTR [rdx]).ContextLength, 0
    
    bts g_InferenceContext.FreeKvSlots, ecxmov (KvCacheSlot PTR [rdx]).ContextLength, 0
    
    retbts g_InferenceContext.FreeKvSlots, ecx
