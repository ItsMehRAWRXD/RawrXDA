...
void Telemetry::recordEvent(const std::string &event_name, const TelemetryMetadata &metadata) {
    if (!is_enabled_) return;
...
bool Telemetry::saveTelemetry(const std::string &filepath) {
    ...
    for (size_t i = 0; i < events_.size(); ++i) {
        ...
        file << "      \"timestamp_ms\": " << event.timestampMs << ",\n";
        file << "      \"metadata\": {";

        size_t meta_index = 0;
        for (const auto &kv : event.metadata) {
            file << "\"" << EscapeJsonString(kv.first) << "\": \"" << EscapeJsonString(kv.second) << "\"";
            if (++meta_index < event.metadata.size()) file << ", ";
        }
        file << "}\n";
        file << "    }";
