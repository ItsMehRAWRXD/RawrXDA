...
void ChatInterface::initialize() {
    ...
    // Search Mode toggle
    searchModeToggle_ = new QCheckBox("Search", this);
    searchModeToggle_->setToolTip("Enable web search augmentation");
    connect(searchModeToggle_, &QCheckBox::toggled, this, &ChatInterface::onSearchModeToggled);
    modelLayout->addWidget(searchModeToggle_);
    
    // Deep Research Mode toggle
    deepResearchModeToggle_ = new QCheckBox("Research", this);
    deepResearchModeToggle_->setToolTip("Enable deep research with citations");
    connect(deepResearchModeToggle_, &QCheckBox::toggled, this, &ChatInterface::onDeepResearchModeToggled);
    modelLayout->addWidget(deepResearchModeToggle_);
    
    // Thinking Mode toggle
    thinkingModeToggle_ = new QCheckBox("Thinking", this);
    thinkingModeToggle_->setToolTip("Enable chain-of-thought reasoning");
    connect(thinkingModeToggle_, &QCheckBox::toggled, this, &ChatInterface::onThinkingModeToggled);
    modelLayout->addWidget(thinkingModeToggle_);
...
bool ChatInterface::isSearchMode() const
{
    return searchMode_;
}

bool ChatInterface::isDeepResearchMode() const
{
    return deepResearchMode_;
}

bool ChatInterface::isThinkingMode() const
{
    return thinkingMode_;
}

void ChatInterface::onSearchModeToggled(bool enabled)
{
    searchMode_ = enabled;
    emit searchModeChanged(enabled);
    
    if (m_agenticEngine) {
        m_agenticEngine->setSearchEnabled(enabled);
    }
    
    if (enabled) {
        statusLabel_->setText("Search mode enabled - queries will be augmented with web search");
    } else {
        statusLabel_->setText("Search mode disabled");
    }
    
    qDebug() << "[ChatInterface] Search mode:" << (enabled ? "ON" : "OFF");
}

void ChatInterface::onDeepResearchModeToggled(bool enabled)
{
    deepResearchMode_ = enabled;
    emit deepResearchModeChanged(enabled);
    
    if (m_agenticEngine) {
        m_agenticEngine->setDeepResearchEnabled(enabled);
    }
    
    if (enabled) {
        searchMode_ = true; // Deep research requires search
        if (searchModeToggle_) {
            searchModeToggle_->setChecked(true);
        }
        statusLabel_->setText("Deep research mode enabled - multi-step research with citations");
    } else {
        statusLabel_->setText("Deep research mode disabled");
    }
    
    qDebug() << "[ChatInterface] Deep research mode:" << (enabled ? "ON" : "OFF");
}

void ChatInterface::onThinkingModeToggled(bool enabled)
{
    thinkingMode_ = enabled;
    emit thinkingModeChanged(enabled);
    
    if (m_agenticEngine) {
        m_agenticEngine->setThinkingModeEnabled(enabled);
    }
    
    if (enabled) {
        statusLabel_->setText("Thinking mode enabled - chain-of-thought reasoning active");
    } else {
        statusLabel_->setText("Thinking mode disabled");
    }
    
    qDebug() << "[ChatInterface] Thinking mode:" << (enabled ? "ON" : "OFF");
}
