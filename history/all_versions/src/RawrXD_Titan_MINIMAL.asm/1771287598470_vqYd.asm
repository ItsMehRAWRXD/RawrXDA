...
Quant_Q2K_Deblock PROC FRAME
    ...
    push rdi
    .pushreg rdi
    sub rsp, 32                     ; shadow + alignment (6 pushes = 48, +32 = 80, 80 mod 16 = 0 ✓)
    .allocstack 32
    .endprolog
...
Attention_Forward_GQA PROC FRAME
    ...
    push r14
    .pushreg r14
    sub rsp, 40                     ; shadow(32) + alignment pad (4 pushes=32, +40=72… need 48)
    .allocstack 40
    .endprolog
...
Titan_RunInferenceStep PROC FRAME
    ...
    push r12
    .pushreg r12
    sub rsp, 40                     ; shadow(32) + 8 alignment (2 pushes=16, +40=56… need 48 → use 40+16=56, not aligned)
    .allocstack 40
    .endprolog
    ...
    ; MINIMAL: monotonic counter mod n_vocab as placeholder.
    ; CORE/UNIFIED implement actual logit projection + argmax/sampling.
    mov eax, [rbx].TitanContext.state
    inc eax
    mov ecx, [rbx].TitanContext.n_vocab
    test ecx, ecx
    jz @@step_fail
    xor edx, edx
    div ecx                         ; edx = state % n_vocab
    mov eax, edx
...
Titan_LoadModel PROC FRAME
    ...
    push rdi
    .pushreg rdi
    sub rsp, 48                     ; shadow(32) + 3 stack params (24) = 56, round to 48
                                    ; 3 pushes=24, +48=72. 72 mod 16 = 8. Need +8 more → 48 is wrong.
                                    ; Actually: 3 pushes (24) + return addr (8) = 32 on stack before sub.
                                    ; 32 + 48 = 80. 80 mod 16 = 0 ✓
    .allocstack 48
    .endprolog
...
Titan_InferenceThread PROC FRAME
    ...
    push r13
    .pushreg r13
    sub rsp, 40                     ; shadow(32) + alignment: 3 pushes(24)+ret(8)=32, +40=72.
                                    ; 72 mod 16 = 8... need 32+48=80 → use 48.
    .allocstack 40
    .endprolog
