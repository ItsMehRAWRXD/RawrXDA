...
;   NVMe_PollAllDrives(outArray, count) -> fills array with temps
...
ALIGN 8
g_StorageQuery:
    g_SQ_PropertyId     DD STORAGE_PROPERTY_TEMPERATURE  ; PropertyId
    g_SQ_QueryType      DD 0                              ; PropertyStandardQuery
    g_SQ_AdditionalParams DD 0                            ; AdditionalParameters
...
NVMe_AllocAlignedBuffer PROC
    ...
    add rcx, SECTOR_ALIGN - 1
    and rcx, NOT (SECTOR_ALIGN - 1)
    
    ; VirtualAlloc(NULL, size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)
    xor ecx, ecx                    ; lpAddress = NULL (let system choose)
    ; RDX already has rounded size from above... wait, we clobbered it
    ; Redo:
    mov rdx, rcx                    ; Save size
