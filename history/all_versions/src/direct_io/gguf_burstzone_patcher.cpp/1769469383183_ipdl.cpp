...
public:
    BurstZonePlanner(uint8_t driveCount = 5) : m_driveCount(driveCount) {}
    
    // Assign tensors to drives based on access patterns
    std::vector<TensorZone> PlanZones(const std::vector<GGUFReader::TensorInfo>& tensors) {
        ...
        for (const auto& tensor : tensors) {
            ...
            // Heuristic: Assign based on tensor name pattern
            zone.driveIndex = AssignDrive(tensor.name);
...
private:
    ...
    // Drive assignment heuristics for 5-drive 800B model topology:
    // Drive 0: OS + KV Cache (random access optimized)
    // Drive 1: Q tensors (query projections)
    // Drive 2: K tensors (key projections)
    // Drive 3: V tensors (value projections)
    // Drive 4: Output heads + embeddings
    uint8_t AssignDrive(const std::string& name) {
        ...
        // Default: KV cache drive (fastest random)
        return 0;
    }
