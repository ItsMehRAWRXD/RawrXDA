    ...
    int maxTokens)
{
    try {
        ...
        // Simple tokenizer for streaming simulation (split by space is crude but effective for display)
        // A better approach would be char-by-char or using the engine's detokenizer if available incrementally.
        // We'll simulate 50ms per "word" to mimic network/generation latency.
        ...
        for (const auto& token : tokens) {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(20)); // Faster 20ms delay
        }
