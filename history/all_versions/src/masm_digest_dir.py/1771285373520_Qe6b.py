...
def aggregate_results(results):
    ...
    # Known symbols = PROCs + EXTERNs + PROTOs + DATA labels + CONSTs + STRUCTs
    known_symbols = all_procs | all_externs | all_protos | all_data | all_consts | all_structs
    undefined_calls = sorted(set(all_calls) - known_symbols)

    # Unused PROCs = defined but never called AND not PUBLIC (exported)
    unused_procs = sorted(set(all_procs) - set(all_calls) - all_publics)
