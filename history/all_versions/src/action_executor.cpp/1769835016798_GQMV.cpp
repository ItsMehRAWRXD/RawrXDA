...
bool ActionExecutor::handleQueryUser(Action& action) {
    if (!onQueryUser) {
        action.error = "No user query callback registered";
        return false;
    }
    
    std::string prompt = action.params.value("prompt", "");
    std::string response = onQueryUser(prompt);
    
    action.result = json({{"response", response}}).dump();
    return true;
}
