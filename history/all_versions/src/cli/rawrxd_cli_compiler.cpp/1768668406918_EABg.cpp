...
private:
    bool invokeSystemCompiler(const std::string& inputFile, const std::string& outputFile, const CompileOptions& options) {
        ...
        // Script wrappers (Python, JS, TS)
            ...
            else if (ext == ".ts") interpreter = "ts-node"; // Assuming ts-node
            
            // Check if interpreter exists
            std::string checkCmd = interpreter + (ext == ".py" ? " --version" : " --version") + " >nul 2>nul";
            ...
            wrapperSrc += "    std::string script = R\"(" + Utils::readFile(inputFile) + ")\";\n";
            wrapperSrc += "    std::string cmd = \"" + interpreter + " -c \\\"\" + script + \"\\\"\";\n";
            wrapperSrc += "    return std::system(cmd.c_str());\n";
            wrapperSrc += "}\n";
