                ...
                if (d->tools && d->router) {
                     ...
                     for (const auto& tName : d->tools->getRegisteredTools()) {
                         ...
                         if (tDef) {
                             ...
                             schema["parameters"] = tDef->parameterSchema;
                     ...
                     for(int step=0; step<10 && !taskComplete; step++) {
                         ...
                         try {
                             ...
                             if (toolName == "DONE") {
                                 ...
                                 // Handle output type (ToolResult structure might store json in 'metadata' or 'output' as string)
                                 // Looking at header, output is std::string.
                                 ...
                                 conversation += "\n[Action] " + toolName + " -> " + toolOutput;
                                  ...
                                  conversation += "\n[System] Error: Invalid JSON (missing 'tool').";
                             }
                         } catch(...) {
                             conversation += "\n[System] Error: Could not parse JSON action.";
                         }
