            ...
            case WM_COMMAND:
                switch (LOWORD(wParam)) {
                    ...
                    case IDM_TOOLS_AGENT_MODE:
                        {
                            ...
                            if (!htmlPath.empty() && htmlPath.find("Error") == std::string::npos) {
                                pThis->CreateNewTab(L"AgentMode.txt", L"[Agent] Processing query...\r\n");
                            } else {
                    ...
                    case IDM_TOOLS_ENGINE_MANAGER:
                        {
                            MessageBoxW(pThis->hwnd_, L"Engine Manager requires active runtime connection.", L"Engine Core", MB_OK);
                        }
                        return 0;
                    case IDM_TOOLS_MEMORY_VIEWER:
                        {
                            ...
                            pThis->CreateNewTab(L"Memory.stats", wStats);
                        }
                        return 0;
...
static LRESULT CALLBACK TerminalProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    // Basic subclass for terminal (handle Enter key maybe?)
    if (msg == WM_KEYDOWN && wParam == VK_RETURN) {
        // Execute command logic could go here
    }
    return CallWindowProc(DefWindowProc, hwnd, msg, wParam, lParam);
}

// Remove duplicate implementations if they exist
/*
static std::string WideToUTF8(const std::wstring& w) { ... }
static std::wstring UTF8ToWide(const std::string& s) { ... }
static std::wstring TrimWhitespace(const std::wstring& text) { ... }
static void AppendOutputText(HWND hOutput, const std::wstring& text) { ... }
static std::wstring PromptForText(...) { ... }
*/
