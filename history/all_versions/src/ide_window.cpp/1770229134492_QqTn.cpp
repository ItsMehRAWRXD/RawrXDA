...
static const int IDM_TOOLS_CODE_AUDIT = 3108;
static const int IDM_TOOLS_SECURITY_CHECK = 3109;
static const int IDM_TOOLS_PERFORMANCE_ANALYZE = 3110;
static const int IDM_TOOLS_IDE_HEALTH = 3111;
...
void IDEWindow::ExecutePowerShellCommand(const std::wstring& command)
{
    ...
    std::string pathUTF8 = WideToUTF8(scriptPath);
    std::string cmdUTF8 = WideToUTF8(command);
    std::ofstream tempFile(pathUTF8, std::ios::binary);
    if (tempFile.is_open()) {
        tempFile.write(cmdUTF8.data(), cmdUTF8.size());
        tempFile.close();
...
static LRESULT CALLBACK TerminalProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (msg == WM_KEYDOWN && wParam == VK_RETURN) {
        // Placeholder for terminal input handling
    }
...
void IDEWindow::CreateNewTab(const std::wstring& title, const std::wstring& filePath) {
    ...
    tie.pszText = const_cast<LPWSTR>(title.c_str());
    tie.lParam = nextTabId_;
    
    int tabIndex = TabCtrl_InsertItem(hTabControl_, TabCtrl_GetItemCount(hTabControl_), &tie);
    
    if (tabIndex != -1) {
        openTabs_[nextTabId_] = info;
        activeTabId_ = nextTabId_;
        nextTabId_++;
        ...
        // Update status
        SendMessageW(hStatusBar_, SB_SETTEXTW, 0, (LPARAM)title.c_str());
    }
}
...
void IDEWindow::Shutdown() {
    ...
    // Cleanup windows
    if (hCommandPalette_) DestroyWindow(hCommandPalette_);
    if (hAutocompleteList_) DestroyWindow(hAutocompleteList_);
    if (hParameterHint_) DestroyWindow(hParameterHint_);
...
void IDEWindow::GenerateProject() {
    std::wstring projectName = PromptForText(hwnd_, L"Generate Project", L"Project Name:", L"MyProject");
    if (projectName.empty()) return;
    
    // Project type selection dialog
    const wchar_t* types[] = {L"CLI", L"Win32", L"C++", L"ASM", L"Game"};
    std::wstring typeList = L"Select project type:\n1. CLI Application\n2. Win32 GUI\n3. C++ Library\n4. Assembly\n5. Game Engine";
    
    std::wstring typeChoice = PromptForText(hwnd_, L"Project Type", typeList, L"1");
    
    std::string type = "cpp"; // Default
    if (typeChoice == L"1") type = "cli";
    else if (typeChoice == L"2") type = "win32";
    else if (typeChoice == L"3") type = "cpp";
    else if (typeChoice == L"4") type = "asm";
    else if (typeChoice == L"5") type = "game";
    
    // Get output path
    std::wstring outputPath = PromptForText(hwnd_, L"Output Path", L"Output Directory:", L".");
    
    // Build parameters JSON
    std::string params = "{\"name\":\"" + WideToUTF8(projectName) + 
                        "\",\"type\":\"" + type + 
                        "\",\"path\":\"" + WideToUTF8(outputPath) + "\"}";
    ...
    AppendOutputText(hOutput_, L"[Generator] " + wResult + L"\r\n");
    
    if (result.find("Success") != std::string::npos) {
        MessageBoxW(hwnd_, wResult.c_str(), L"Project Generated", MB_OK | MB_ICONINFORMATION);
    } else {
        MessageBoxW(hwnd_, wResult.c_str(), L"Generation Error", MB_OK | MB_ICONERROR);
    }
}
