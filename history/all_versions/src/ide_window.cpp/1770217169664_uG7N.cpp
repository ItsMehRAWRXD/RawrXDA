...
static const int IDM_TOOLS_GENERATE_PROJECT = 3101;
static const int IDM_TOOLS_GENERATE_GUIDE = 3102;
static const int IDM_TOOLS_HOTPATCH = 3103;
static const int IDM_TOOLS_AGENT_MODE = 3104;
static const int IDM_TOOLS_ENGINE_MANAGER = 3105;
static const int IDM_TOOLS_MEMORY_VIEWER = 3106;
static const int IDM_TOOLS_RE_TOOLS = 3107;
    ...
    , sessionPath_(L"RawrXDSettings.json")
{
    ...
    ideGenerator_ = std::make_unique<ReactIDEGenerator>();
}
        ...
        switch (uMsg) {
            ...
            case WM_COMMAND:
                switch (LOWORD(wParam)) {
                    ...
                    case 5011: // Marketplace Search ID (assumed)
                        if (HIWORD(wParam) == EN_CHANGE && pThis->hMarketplaceSearch_) {
...
static LRESULT CALLBACK TerminalProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    if (msg == WM_KEYDOWN && wParam == VK_RETURN) {
        ...
        // In a real terminal, we'd process the input line.
        // For now, we rely on the editor for input and this is just output/display.
        // But if we wanted to support direct typing:
        if (pThis) {
             // We could capture text from the last line
        }
    }
...
void IDEWindow::SaveSession() {
    std::wofstream file(sessionPath_);
    if (file.is_open()) {
        file << L"{\n";
        file << L"  \"open_files\": [\n";
        
        bool first = true;
        for (const auto& pair : openTabs_) {
            if (!first) file << L",\n";
            file << L"    \"" << pair.second.filePath << L"\"";
            first = false;
        }
        
        file << L"\n  ],\n";
        file << L"  \"active_tab\": " << activeTabId_ << L"\n";
        file << L"}\n";
        file.close();
    }
}
...
void IDEWindow::LoadSession() {
    std::wifstream file(sessionPath_);
    bool loaded = false;
    ...
    if (file.is_open()) {
        ...
        // Simple manual JSON parsing for robustness without external deps
        size_t arrayStart = content.find(L"\"open_files\":");
        if (arrayStart != std::wstring::npos) {
            size_t startBracket = content.find(L"[", arrayStart);
            size_t endBracket = content.find(L"]", startBracket);
            
            if (startBracket != std::wstring::npos && endBracket != std::wstring::npos) {
                std::wstring list = content.substr(startBracket + 1, endBracket - startBracket - 1);
                
                size_t pos = 0;
                while ((pos = list.find(L"\"", pos)) != std::wstring::npos) {
                    size_t endQuote = list.find(L"\"", pos + 1);
                    if (endQuote != std::wstring::npos) {
                        std::wstring path = list.substr(pos + 1, endQuote - pos - 1);
                        if (!path.empty()) {
                            ...
                            if (attrib != INVALID_FILE_ATTRIBUTES && !(attrib & FILE_ATTRIBUTE_DIRECTORY)) {
                                ...
                                if (lastSlash != std::wstring::npos) filename = path.substr(lastSlash + 1);
                                CreateNewTab(filename, path);
                                loaded = true;
                            }
                        }
                        pos = endQuote + 1;
...
void IDEWindow::ToggleCommandPalette() {
    if (hCommandPalette_ && IsWindowVisible(hCommandPalette_)) {
        ...
        if (!hCommandPalette_) {
            ...
            SendMessageW(hCommandPalette_, LB_ADDSTRING, 0, (LPARAM)L"Show Agent Mode");
        }
...
void IDEWindow::ExecutePaletteSelection() {
    ...
    if (cur != LB_ERR) {
        switch(cur) {
            ...
            case 4: PostMessageW(hwnd_, WM_COMMAND, IDM_TOOLS_AGENT_MODE, 0); break; // Agent Mode
        }
        ShowWindow(hCommandPalette_, SW_HIDE);
    }
}
...
void IDEWindow::SearchMarketplace(const std::wstring& searchText) {
    ...
    // Simulate searching via Universal Generator
    std::string result = GenerateAnything("search_extensions", WideToUTF8(searchText));
    // Assume result is comma separated list for now
    std::stringstream ss(result);
    std::string item;
    while (std::getline(ss, item, ',')) {
         SendMessageW(hMarketplaceList_, LB_ADDSTRING, 0, (LPARAM)UTF8ToWide(item).c_str());
    }
}

void IDEWindow::ShowExtensionDetails(const ExtensionInfo& ext) {
    std::wstring details = L"Extension: " + ext.name + L"\nVersion: " + ext.version + L"\n\n" + ext.description;
    MessageBoxW(hwnd_, details.c_str(), L"Extension Details", MB_OK);
}

void IDEWindow::InstallExtension(const ExtensionInfo& ext) {
    std::string result = GenerateAnything("install_extension", WideToUTF8(ext.id));
    MessageBoxW(hwnd_, UTF8ToWide(result).c_str(), L"Install Extension", MB_OK);
}

void IDEWindow::UninstallExtension(const ExtensionInfo& ext) {
     MessageBoxW(hwnd_, L"Uninstall not implemented yet", L"Uninstall Extension", MB_OK);
}
