...
void IDEWindow::SaveSession() {
    // Basic session saving logic to a generic JSON file
    std::ofstream file("RawrXDSession.json");
    if (file.is_open()) {
        file << "{ \"last_tab\": " << activeTabId_ << ", \"open_files\": [";
        bool first = true;
        for (const auto& kv : openTabs_) {
            if (!first) file << ",";
            file << "\"" << WideToUTF8(kv.second.filePath) << "\"";
            first = false;
        }
        file << "] }";
        file.close();
