...
static const int IDM_TOOLS_REAL_TIME_ANALYSIS = 3114;
static const int IDM_TOOLS_SMART_AUTOCOMPLETE = 3115;
static const int IDM_TOOLS_AUTO_REFACTOR = 3116;
static const int IDM_TOOLS_GENERATE_TESTS = 3117;
static const int IDM_TOOLS_PERFORMANCE_PROFILER = 3118;
static const int IDM_TOOLS_IDE_HEALTH = 3119;
            ...
            case WM_COMMAND:
                switch (LOWORD(wParam)) {
                    ...
                    case IDM_TOOLS_GENERATE_TESTS:
                        ...
                        return 0;
                    case IDM_TOOLS_PLAN_TASK:
                        {
                            std::wstring goal = PromptForText(pThis->hwnd_, L"Plan Task", L"Enter task goal:", L"Refactor codebase");
                            if (!goal.empty()) {
                                std::string json = "{\"goal\":\"" + WideToUTF8(goal) + "\"}";
                                std::string plan = GenerateAnything("plan_task", json);
                                pThis->CreateNewTab(L"TaskPlan.txt", L"");
                                SetWindowTextW(pThis->hEditor_, UTF8ToWide(plan).c_str());
                                AppendOutputText(pThis->hOutput_, L"[Task Planner] Plan generated in new tab.\r\n");
                            }
                        }
                        return 0;
                    case IDM_TOOLS_SUMMARIZE_CODE:
                        {
                            int textLen = GetWindowTextLengthW(pThis->hEditor_) + 1;
                            std::wstring wCode(textLen, L'\0');
                            GetWindowTextW(pThis->hEditor_, &wCode[0], textLen);
                            ...
                            std::string code = WideToUTF8(wCode);
                            std::string summary = GenerateAnything("summarize_code", code);
                            
                            AppendOutputText(pThis->hOutput_, L"[Code Summary]\r\n" + UTF8ToWide(summary) + L"\r\n");
                        }
                        return 0;
