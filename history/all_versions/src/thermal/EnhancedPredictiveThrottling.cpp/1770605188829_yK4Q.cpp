...
std::unique_ptr<MLPredictor> MLPredictorFactory::create(const std::string& modelPath)
{
    ...
    try {
        ...
        // TODO: Instantiate ONNX Runtime session with the model
        // When ONNX Runtime becomes a dependency:
        //   auto env = std::make_shared<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "ThermalPredictor");
        //   auto session = std::make_unique<Ort::Session>(*env, modelPath.c_str(), Ort::SessionOptions{});
