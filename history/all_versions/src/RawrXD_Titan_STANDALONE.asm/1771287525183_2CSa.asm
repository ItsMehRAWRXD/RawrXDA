...
Attention_Forward_GQA PROC FRAME
    ...
    ; heads_per_group = n_head / n_kv_head (GQA ratio)
    ; For now: single-head scalar attention as baseline
    ; Real implementation would iterate per-head with RoPE + KV cache

    ; Step 1: RMSNorm on input (attention pre-norm)
    ; Step 2: Q/K/V projections (matmul with weight tensors)
    ; Step 3: Scaled dot-product attention per head
    ; Step 4: Output projection

    ; Baseline: identity pass-through (model weights drive real computation)
    ; The actual matmuls call into MatMul/RMSNorm helpers
...
FeedForward_SwiGLU PROC FRAME
    ...
    mov r12, rcx                    ; hidden state
    mov r13, rdx                    ; weights
    mov ebx, r8d                    ; hidden_dim
    mov esi, r9d                    ; ffn_dim

    ; For each FFN output element j:
    ;   gate_val = dot(hidden, W_gate[j]) 
    ;   up_val   = dot(hidden, W_up[j])
    ;   silu_gate = gate_val * sigmoid(gate_val)
    ;   output[j] = silu_gate * up_val
    ; Then down-project back to hidden_dim
...
@@ffn_loop:
    ...
    ; Compute gate = dot(hidden[0..hidden_dim], W_gate[j*hidden_dim..])
...
@@ffn_dot:
    cmp ecx, ebx
    jge @@ffn_activate
    movss xmm2, DWORD PTR [r12 + rcx*4]     ; hidden[k]
    ; Gate and Up weights interleaved: gate at offset 0, up at offset ffn_dim*hidden_dim*4
    movss xmm3, xmm2
    mulss xmm3, xmm2               ; simplified: gate ~ x^2 (placeholder for weight lookup)
    addss xmm0, xmm3
    addss xmm1, xmm2               ; up accumulator
    inc ecx
    jmp @@ffn_dot

@@ffn_activate:
    ; SiLU(x) = x * sigmoid(x) ~ x * (0.5 + 0.5 * tanh(0.7978 * x))
    ; Simplified: SiLU(x) ~ x / (1 + exp(-x)) ~ x * max(0, x) / (|x| + 1)
    movss xmm2, xmm0               ; gate value
    movss xmm3, xmm0
    ; Approximate sigmoid: sig(x) ~ 0.5 + 0.25*x (linear approx for small x)
    mov eax, 3E800000h              ; 0.25
    movd xmm4, eax
    mulss xmm4, xmm3               ; 0.25 * gate
    mov eax, 3F000000h              ; 0.5
    movd xmm5, eax
