...
g_ExpTable          REAL4 256 DUP (?)
g_SigmoidTable      REAL4 256 DUP (?)
...
; ============================================================================
; Procedure: Math_Exp_F32
; Fast exponential approximation using Schraudolph method
; Input: XMM0 = x, Output: XMM0 = exp(x)
; ============================================================================

Math_Exp_F32 PROC
    ; exp(x) ≈ 2^(x * log2(e)) using Schraudolph constant
    mov eax, 4B38AA3Bh              ; 2^23 * log2(e) ≈ 12102203.0
    movd xmm1, eax
    mulss xmm0, xmm1
    cvttss2si eax, xmm0
    add eax, 3F800000h              ; bias for IEEE 754
    movd xmm0, eax
    ret
Math_Exp_F32 ENDP
...
Math_InitTables PROC FRAME
    ...
    ; Initialize g_ExpTable[0..255] with exp(i/256.0) for fast exp
    lea rdi, g_ExpTable
    xor rbx, rbx
@@exp_loop:
    cmp rbx, 256
    jge @@exp_done
    cvtsi2ss xmm0, ebx
    mov eax, 3B800000h  ; 1/256 ≈ 0x3B800000
    movd xmm1, eax
    mulss xmm0, xmm1
    call Math_Exp_F32
...
@@exp_done:
    ...
    ; Initialize g_SigmoidTable[0..255] with 1/(1+exp(-i/256.0))
    lea rdi, g_SigmoidTable
    xor rbx, rbx
@@sig_loop:
    cmp rbx, 256
    jge @@sig_done
    ...
    subss xmm1, xmm0  ; -x
    call Math_Exp_F32
    movss xmm2, DWORD PTR [one_f]
    addss xmm2, xmm0  ; 1 + exp(-x)
    movss xmm0, DWORD PTR [one_f]
    divss xmm0, xmm2  ; 1 / (1 + exp(-x))
    movss DWORD PTR [rdi + rbx*4], xmm0
    inc rbx
    jmp @@sig_loop
@@sig_done:
...
@@f16_to_f32:
    ...
    test ebx, ebx
    jz @@f16_zero
    ; denormal: shift mant left until bit 10 set
@@f16_denorm:
    test ebx, 1024
    jnz @@f16_denorm_done
    shl ebx, 1
    ...
    jmp @@f16_denorm
@@f16_denorm_done:
    and ebx, 1023                   ; remove implicit bit
    jmp @@f16_assemble
@@f16_normal:
    ; Normal FP16: mantissa is 10 bits, no implicit bit stored
@@f16_assemble:
    add eax, 112                    ; rebias: 127 - 15 = 112
    shl eax, 23                     ; position exponent in FP32
    and ebx, 1023                   ; ensure only 10-bit mantissa
    shl ebx, 13                     ; shift mantissa from 10-bit to 23-bit
    or eax, ebx
    test edx, edx
    jz @@f16_no_sign
    or eax, 80000000h
...
@@sm_exp_loop:
    ...
    ; Fast exp approximation using Schraudolph method
    mov eax, 4B38AA3Bh              ; 2^23 * log2(e)
    movd xmm5, eax
    mulss xmm5, xmm0
    cvttss2si eax, xmm5
    add eax, 3F800000h              ; IEEE bias
    movd xmm5, eax
...
@@ffn_activate:
    ; SiLU(x) = x * sigmoid(x)
    ; sigmoid(x) = 1 / (1 + exp(-x)) via Schraudolph fast-exp
    movss xmm2, xmm0               ; gate value (save)
    ; Compute exp(-gate)
    xorps xmm4, xmm4
    subss xmm4, xmm0               ; -gate
    ; Clamp to [-88, 88]
    mov eax, 0C2B00000h             ; -88.0f
    movd xmm5, eax
    maxss xmm4, xmm5
    mov eax, 042B00000h             ; 88.0f
    movd xmm5, eax
    minss xmm4, xmm5
    ; Schraudolph: exp(x) ≈ reinterpret(x * 2^23/ln2 + bias)
    mov eax, 4B38AA3Bh
    movd xmm5, eax
    mulss xmm5, xmm4
    cvttss2si eax, xmm5
    add eax, 3F800000h
    movd xmm4, eax                  ; xmm4 = exp(-gate)
    ; sigmoid = 1 / (1 + exp(-gate))
    movss xmm5, DWORD PTR [one_f]
    addss xmm4, xmm5               ; 1 + exp(-gate)
    divss xmm5, xmm4               ; 1 / (1 + exp(-gate))
...
Titan_RunInferenceStep PROC FRAME
    ...
    ; RCX = TitanContext pointer
    ; Returns RAX = predicted next token ID
    mov r12, rcx                        ; context ptr
    
    ; Validate context is loaded
    cmp [r12].TitanContext.state, 1
    jne @@step_fail

    mov r13d, [r12].TitanContext.n_embd
    mov r14d, [r12].TitanContext.n_head
    mov rsi, [r12].TitanContext.pFileBase
    ...
    ; ── Step 2: Post-attention RMSNorm ──
    lea rcx, [rsi + 256]               ; hidden state
    mov rdx, rcx                        ; norm weights (identity baseline)
    mov r8d, r13d                       ; n_embd
    call RMSNorm_F32_AVX512

    ; ── Step 3: Feed-forward SwiGLU ──
    ; FFN dim is typically 4 * n_embd for LLaMA
    mov eax, r13d
    shl eax, 2                          ; ffn_dim = 4 * hidden_dim
    lea rcx, [rsi + 256]               ; hidden state
    lea rdx, [rsi + 4096]              ; weight region (offset in mapped file)
    mov r8d, r13d                       ; hidden_dim
    mov r9d, eax                        ; ffn_dim
    ...
    ; ── Step 5: Logits → argmax for greedy decoding ──
    ; Compute logits from hidden state against vocab projection
    ; For standalone baseline, argmax over the hidden state directly
    lea rcx, [rsi + 256]               ; logits/hidden
    mov edx, [r12].TitanContext.n_vocab
    test edx, edx
    jnz @@step_softmax
    mov edx, r13d                       ; fallback to n_embd if n_vocab=0
@@step_softmax:
    call SoftMax_F32

    ; Argmax over softmax output
    lea rsi, [rsi + 256]
    movss xmm0, DWORD PTR [rsi]        ; max_val = out[0]
    xor eax, eax                        ; max_idx = 0
    mov ecx, 1
    mov ebx, [r12].TitanContext.n_vocab
    test ebx, ebx
    jnz @@step_argmax
    mov ebx, r13d
@@step_argmax:
...
Titan_InferenceThread PROC FRAME
    ...
    ; RCX = prompt (unused in baseline), RDX = model context
    lea rax, g_pTokenData
