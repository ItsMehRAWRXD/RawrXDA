...
SecurityIssue AutonomousFeatureEngine::analyzeSecurityIssue(const std::string& c, int l) { 
    // Use Real Model instead of simulating
     // std::this_thread::sleep_for(std::chrono::milliseconds(200)); 
     
    // Call Model Instead (for analyzeSecurityIssue)
    if (ModelCaller::generateCode("Is this code secure?", "cpp", c).find("Vulnerable") != std::string::npos) {
        SecurityIssue issue;
        issue.issueId = GenerateUUID();
        issue.type = "potential_vulnerability";
        issue.severity = "high";
        issue.filePath = "current_buffer"; // Context dependent
        issue.lineNumber = l;
        issue.vulnerableCode = c;
        issue.description = "Potential security vulnerability detected by model.";
        issue.riskScore = 7.5; // Default high risk for detected issues
