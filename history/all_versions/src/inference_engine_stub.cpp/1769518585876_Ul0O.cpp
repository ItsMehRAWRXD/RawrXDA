...
bool InferenceEngine::InitializeLazyPager(const std::string& model_path, uint64_t file_size)
{
    ...
    try {
        ...
        // Attach to the file (creates sparse memory mapping)
        ...
        // Pre-warm first 2 layers for immediate inference (reduces cold-start latency)
        const uint64_t layerSize = file_size / m_layerCount;
        for (uint32_t i = 0; i < std::min(2u, m_layerCount); i++) {
            void* layerData = m_pager->PinLayer(i, i * layerSize, layerSize);
            if (!layerData) {
                qWarning() << "Failed to pre-warm layer" << i;
            }
        }
