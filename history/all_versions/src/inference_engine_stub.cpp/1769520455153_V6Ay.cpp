...
namespace {
    // Lazy pager threshold: models >= 32GB use layer-wise demand paging
    constexpr uint64_t LAZY_PAGER_THRESHOLD = 32ULL * 1024 * 1024 * 1024;
...
bool InferenceEngine::Initialize(const std::string& model_path)
{
    ...
    const bool force_lazy_pager = envEnabled("RAWRXD_USE_LAZY_PAGER");
    ...
    if (metadata_only || m_mode == InferenceMode::METADATA_ONLY) {
        m_mode = InferenceMode::METADATA_ONLY;
        m_initialized = true;
        qInfo() << "InferenceEngine initialized (metadata-only mode)";
        return true;
    }
    
    // Lazy pager mode: skip standard transformer init, use layer-wise paging
    if (m_mode == InferenceMode::LAZY_PAGED) {
        m_initialized = true;
        qInfo() << "InferenceEngine initialized (lazy-paged mode) - layer-wise demand paging active";
        return true;
    }
...
{
    fprintf(stderr, "[DEBUG] LoadModelFromGGUF called for %s\n", model_path.c_str());
    fflush(stderr);
    ...
    try {
        ...
        // Check if we should use lazy pager for large models (>= 32GB)
        const bool force_lazy_pager = envEnabled("RAWRXD_USE_LAZY_PAGER");
        
        fprintf(stderr, "[DEBUG] Checking lazy pager: force=%d, size=%llu, threshold=%llu\n", 
                force_lazy_pager, file_size, LAZY_PAGER_THRESHOLD);
        fflush(stderr);

        if (force_lazy_pager || file_size >= LAZY_PAGER_THRESHOLD) {
            fprintf(stderr, "[DEBUG] Calling InitializeLazyPager...\n");
            fflush(stderr);
            
            if (InitializeLazyPager(model_path, file_size)) {
                const auto load_end = std::chrono::steady_clock::now();
                const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(load_end - load_start).count();
                
                qInfo() << "GGUF model prepared for lazy paging"
                    << "| Vocab:" << m_vocabSize
                    << "| Embedding:" << m_embeddingDim
                    << "| Layers:" << m_layerCount
                    << "| Heads:" << m_headCount
                    << "| FileSize(GB):" << (file_size / (1024.0 * 1024.0 * 1024.0))
                    << "| Load(ms):" << ms
                    << "| Mode: lazy-paged";
                return true;
            }
            qWarning() << "Lazy pager initialization failed, falling back to standard loader";
        }
...
void InferenceEngine::Cleanup()
{
    ...
    m_initialized = false;
    m_modelPath.clear();
    m_mode = InferenceMode::STANDARD;
    
    qInfo() << "InferenceEngine cleaned up";
}

bool InferenceEngine::InitializeLazyPager(const std::string& model_path, uint64_t file_size)
{
    // Create lazy pager from file path
    // The pager will memory-map the file and manage layer-wise demand loading
    fprintf(stderr, "[DEBUG] InitializeLazyPager called for %s (size %llu)\n", model_path.c_str(), file_size);
    
    try {
        m_pager = RawrXD::CreateLazyPagerFromPath(
            model_path.c_str(),
            m_layerCount,
            85  // Thermal threshold (Celsius)
        );
        
        if (!m_pager) {
            fprintf(stderr, "[DEBUG] m_pager is null. CreateLazyPagerFromPath failed.\n");
            qWarning() << "Failed to create lazy pager for" << QString::fromStdString(model_path);
            return false;
        }
        
        fprintf(stderr, "[DEBUG] m_pager created. Attaching...\n");
        // Attach to the file (creates file mapping handle for per-layer mapping)
        if (!m_pager->Attach(file_size)) {
            fprintf(stderr, "[DEBUG] Attach failed.\n");
            qWarning() << "Failed to attach lazy pager to file";
            m_pager.reset();
            return false;
        }
        
        fprintf(stderr, "[DEBUG] Attach success.\n");

        // Skip pre-warming for now - layers will be mapped on-demand during inference
        // The GGUF file doesn't have uniform layer boundaries, so naive offset calculation
        // would map incorrect data. Real implementation needs tensor index parsing.
        
        m_mode = InferenceMode::LAZY_PAGED;
        
        RawrXD::Integration::logInfo(QStringLiteral("InferenceEngine"),
                                     QStringLiteral("lazy_pager_init"),
                                     QStringLiteral("success"),
                                     QJsonObject{
                                         {"file_bytes", static_cast<qint64>(file_size)},
                                         {"layers", static_cast<int>(m_layerCount)},
                                         {"prewarm_layers", 0}
                                     });
        RawrXD::Integration::recordMetric("loader.lazy_pager_activated");
        
        qInfo() << "Lazy tensor pager initialized"
            << "| Layers:" << m_layerCount
            << "| FileSize(GB):" << (file_size / (1024.0 * 1024.0 * 1024.0))
            << "| PreWarm: 0 layers";
        ...
        qCritical() << "Lazy pager initialization exception:" << e.what();
        m_pager.reset();
        return false;
    }
}