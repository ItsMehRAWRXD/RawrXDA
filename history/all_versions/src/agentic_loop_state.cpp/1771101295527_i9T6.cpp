...
nlohmann::json AgenticLoopState::takeSnapshot()
{
    ...
    snapshot["phase"] = phaseToString(m_currentPhase);
    snapshot["status"] = statusToString(m_currentStatus);
    snapshot["goal"] = m_currentGoal;
    snapshot["memory"] = getAllMemory();
    snapshot["timestamp"] = timePointToISO(std::chrono::system_clock::now());
    
    m_lastSnapshot = snapshot;
    m_snapshotHistory.push_back(snapshot);
    if (m_snapshotHistory.size() > 50) {
...
bool AgenticLoopState::restoreFromSnapshot(const nlohmann::json& snapshot)
{
    if (snapshot.empty() || !snapshot.is_object()) return false;
    
    if (snapshot.contains("phase")) m_currentPhase = stringToPhase(snapshot["phase"].get<std::string>());
    if (snapshot.contains("status")) m_currentStatus = stringToStatus(snapshot["status"].get<std::string>());
    if (snapshot.contains("goal")) m_currentGoal = snapshot["goal"].get<std::string>();
    
    if (snapshot.contains("memory") && snapshot["memory"].is_object()) {
        m_memory.clear();
        for (auto it = snapshot["memory"].begin(); it != snapshot["memory"].end(); ++it) {
