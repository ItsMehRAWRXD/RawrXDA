...
SubsystemID LicenseEnforcer::featureToSubsystem(License::FeatureID id) const {
    ...
    // Pro: Quantization (10–11)
    if (idx >= 10 && idx <= 11) return SubsystemID::Quantization;

    // Pro: GPU backends (12, 26)
    if (idx == 12 || idx == 26) return SubsystemID::Inference;

    // Pro: Misc features (13–25)
    if (idx >= 13 && idx <= 25) return SubsystemID::ModelManagement;
    ...
    // Enterprise: Server (31–34)
    if (idx >= 31 && idx <= 34) return SubsystemID::Networking;

    // Enterprise: GPU/Performance (35–47)
    if (idx >= 35 && idx <= 47) return SubsystemID::Inference;

    // Enterprise: Security (48–52)
    if (idx >= 48 && idx <= 52) return SubsystemID::Security;

    // Sovereign (53–60)
    if (idx >= 53 && idx <= 60) return SubsystemID::Sovereign;
...
void LicenseEnforcer::recordEvent(SubsystemID sub, License::FeatureID feat,
    ...
    ev.caller = caller ? caller : "unknown";
    ev.detail = detail ? detail : "";

    m_eventHead = (m_eventHead + 1) % MAX_EVENTS;
    if (m_eventCount < MAX_EVENTS) m_eventCount++;

    // ── Bridge to V2 audit trail (4096-entry ring buffer) ──
    License::EnterpriseLicenseV2::Instance().logEnforcementEvent(
        feat, allowed, caller, detail);
}
