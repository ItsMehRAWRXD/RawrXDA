...
TextBuffer::Node* TextBuffer::createInternal(Node* l, Node* r) {
    ...
    n->left = l;
    n->right = r;
    n->weight = l ? (l->isLeaf ? l->weight : l->weight + (l->right ? (l->right->isLeaf ? l->right->weight : /* wait, weight logic definition */ 0) : 0)) : 0;
    // Actually, weight of internal node = sum of lengths of leaves in LEFT subtree.
    // simpler: helper function for "total length of tree rooted at x"
    
    // Let's redefine Node::weight.
    // Standard Rope definition: weight = sum of lengths of leaves in the left child.
    // If left child is leaf: weight = left->text.length()
    // If left child is internal: weight = total length of left child.
    
    // My previous definition was "Total characters in left subtree".
    
    int leftLen = l ? ((l->isLeaf) ? l->weight : (l->weight + (l->right ? /* recursion needed if not stored */ 0 : 0))) : 0;
    
    // To avoid recursion, let's just store totalLength in every node? 
    // Or just use the standard definition properly.
    // Weight = length of left child.
    
    // Let's use a helper that calculates length using the stored weight property.
    // Length(Node) = Node->isLeaf ? Node->text.length() : Node->weight + Length(Node->right)
    
    n->weight = 0;
    if (l) {
        if (l->isLeaf) n->weight = (int)l->text.length();
        else {
            // Traverse right spine of left child to find length? 
            // Or just store 'totalLength' in every node. Storing totalLength is easier for maintenance.
            // Let's change the struct? No, standard rope avoids it to save space, but for an IDE, space is cheap.
            // But let's stick to standard Weight = Length of Left Subtree.
             
             // Calculating length of a node:
             // int length(node) { 
             //    if(isLeaf) return text.length;
             //    return weight + length(right);
             // }
             
             // So here n->weight should be length(l).
             n->weight = length(l);
    }
    
    n->lineBreakCount = (l ? l->lineBreakCount : 0) + (r ? r->lineBreakCount : 0);
    // Wait, lineBreakCount for Internal Node should probably follow the same pattern (Left subtree count) if we want logN lines?
    // Usually we want "Total lines in this subtree" for line count, unlike weight which is used for split index.
    // Let's store "Total Lines in Left Subtree" too?
    // Actually, simpler: Store Total Lines in THIS subtree.
    // So Internal->lineBreakCount = l->lines + r->lines.
