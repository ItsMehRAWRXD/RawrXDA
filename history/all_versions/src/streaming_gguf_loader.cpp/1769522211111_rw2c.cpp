...
bool StreamingGGUFLoader::ParseMetadata() {
    ...
    for (uint64_t i = 0; i < header_.metadata_kv_count; ++i) {
        ...
        if (value_type == 8) {  // UTF-8 string
            ...
            const bool is_chat_template_key = (key == "tokenizer.chat_template") ||
                                               (key.find("tokenizer.chat_template") != std::string::npos);
            const bool is_merges_key = (key == "tokenizer.ggml.merges") ||
                                       (key.find("tokenizer.ggml.merges") != std::string::npos);
            const bool should_skip = (str_len > MAX_SAFE_STRING_LEN) ||
                                     is_chat_template_key ||
                                     is_merges_key;
