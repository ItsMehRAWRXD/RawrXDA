...
bool StreamingGGUFLoader::Open(const std::string& filepath) {
    ...
    // Enable brutal compression for streaming buffers
    // This pre-allocates zlib structures if needed
...
bool StreamingGGUFLoader::DecompressZone(const std::vector<uint8_t>& compressed, std::vector<uint8_t>& out) {
    bool success = false;
    out = CPUInference::codec::inflate(compressed, &success);
    if (!success) {
        std::cerr << "❌ Failed to decompress zone with brutal_gzip" << std::endl;
        return false;
    }
    return true;
}
    ...
    for (uint64_t i = 0; i < header_.metadata_kv_count; ++i) {
        ...
        if (value_type == GGUFConstants::GGUF_VALUE_TYPE_STRING) {
            ...
            if (key == GGUFConstants::META_GENERAL_ARCHITECTURE) {
                ...
                metadata_.head_count = std::stoul(value);
            }
        ...
        } else if (value_type == GGUFConstants::GGUF_VALUE_TYPE_ARRAY) {  // array
            uint32_t array_type;
            uint64_t array_length;
            if (!ReadValue(array_type)) return false;
            if (!ReadValue(array_length)) return false;

            if (array_type == GGUFConstants::GGUF_VALUE_TYPE_STRING) {
                ...
                if (key == GGUFConstants::META_TOKENIZER_TOKENS) {
                    metadata_.tokens = std::move(elems);
                    metadata_.kv_pairs[key] = std::to_string(metadata_.tokens.size());
                }
            } else if (array_type == GGUFConstants::GGUF_VALUE_TYPE_FLOAT32) {
                ...
                if (key == GGUFConstants::META_TOKENIZER_SCORES) {
                    metadata_.token_scores = std::move(elems);
                    metadata_.kv_pairs[key] = std::to_string(metadata_.token_scores.size());
                }
            } else if (array_type == GGUFConstants::GGUF_VALUE_TYPE_UINT32) {
                ...
                if (key == GGUFConstants::META_TOKENIZER_TOKEN_TYPE) {
                    metadata_.token_types = std::move(elems);
                    metadata_.kv_pairs[key] = std::to_string(metadata_.token_types.size());
                }
            } else {
                for (uint64_t j = 0; j < array_length; ++j) {
                    uint64_t skip_val = 0;
                    if (!ReadValue(skip_val)) return false;
                }
            }
        }
    }
...
bool StreamingGGUFLoader::GetTensorRef(const std::string& tensor_name, TensorRef& out) const {
