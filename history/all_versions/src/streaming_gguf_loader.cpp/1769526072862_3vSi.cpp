...
bool StreamingGGUFLoader::ParseMetadata() {
    ...
    auto on_string = [this](const std::string& k, rawrxd::gguf::GGUFType t, const std::string& v) {
        metadata_.kv_pairs[k] = v;

        // Map standard fields
        if (k == "general.architecture") {
            if (v == "llama") metadata_.architecture_type = 1;
        } else if (k == "llama.block_count") {
            try { metadata_.layer_count = std::stoull(v); } catch(...) {}
        } else if (k == "llama.context_length") {
            try { metadata_.context_length = std::stoull(v); } catch(...) {}
        } else if (k == "llama.embedding_length") {
            try { metadata_.embedding_dim = std::stoull(v); } catch(...) {}
        } else if (k == "llama.vocab_size") {
            try { metadata_.vocab_size = std::stoull(v); } catch(...) {}
        }
    };
...
bool StreamingGGUFLoader::BuildTensorIndex() {
    ...
    for (uint64_t i = 0; i < header_.metadata_kv_count; ++i) {
        ...
        if (value_type == 1) {
            ...
            if (!file_.seekg(skip_offset, std::ios::cur)) return false;
            ...
            auto skip_bytes = [&](uint64_t bytes) -> bool {
                const auto skip_offset = static_cast<std::streamoff>(bytes);
                return static_cast<bool>(file_.seekg(skip_offset, std::ios::cur));
            };

            if (array_type == 8) {
                for (uint64_t j = 0; j < array_length; ++j) {
                    uint64_t elem_len = 0;
                    if (!ReadValue(elem_len)) return false;
                    if (!skip_bytes(elem_len)) return false;
                }
            } else {
                uint64_t elem_size = 0;
                switch (array_type) {
                    case 0: elem_size = sizeof(uint8_t); break;
                    case 1: elem_size = sizeof(int8_t); break;
                    case 2: elem_size = sizeof(uint16_t); break;
                    case 3: elem_size = sizeof(int16_t); break;
                    case 4: elem_size = sizeof(uint32_t); break;
                    case 5: elem_size = sizeof(int32_t); break;
                    case 6: elem_size = sizeof(float); break;
                    case 7: elem_size = sizeof(uint8_t); break;
                    case 10: elem_size = sizeof(uint64_t); break;
                    case 11: elem_size = sizeof(int64_t); break;
                    case 12: elem_size = sizeof(double); break;
                    default: return false;
                }
                const uint64_t total_bytes = array_length * elem_size;
                if (!skip_bytes(total_bytes)) return false;
            }
        } else {
            return false;
        }
