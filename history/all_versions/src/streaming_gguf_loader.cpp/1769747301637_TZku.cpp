...
void StreamingGGUFLoader::TrackMetadataField(const std::string& key, const std::string& value) {
    if (key == "general.architecture") {
        if (value == "llama") metadata_.architecture_type = 1;
    } else if (key == "llama.block_count") {
        metadata_.layer_count = static_cast<uint32_t>(std::stoul(value));
    } else if (key == "llama.context_length") {
        metadata_.context_length = static_cast<uint32_t>(std::stoul(value));
    } else if (key == "llama.embedding_length") {
        metadata_.embedding_dim = static_cast<uint32_t>(std::stoul(value));
    } else if (key == "llama.vocab_size") {
        metadata_.vocab_size = static_cast<uint32_t>(std::stoul(value));
    }
}

bool StreamingGGUFLoader::ShouldSkipMetadataKey(std::string_view key) const {
    if (key == "tokenizer.chat_template") return true;
    if (key.find("tokenizer.ggml.merges") == 0) return true;
    if (key.find("tokenizer.ggml.tokens") == 0) return true;
    if (key.find("tokenizer.ggml.scores") == 0) return true;
    if (key.find("tokenizer.ggml.token_type") == 0) return true;
    return false;
}
