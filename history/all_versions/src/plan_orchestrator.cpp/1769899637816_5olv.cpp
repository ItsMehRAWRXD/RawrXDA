...
PlanningResult PlanOrchestrator::generatePlan(const std::string& prompt, 
    ...
    // 1. Try Swarm DLL (Native High-Performance Path)
    ...
    if (hDll) {
        ...
        if (pfnSwarmInit && pfnSwarmSubmit) {
            pfnSwarmInit();
            std::string root = workspaceRoot.empty() ? m_workspaceRoot : workspaceRoot;
            std::vector<std::string> effContext = contextFiles;
            if (effContext.empty()) effContext = gatherContextFiles(root, 10);
            
            std::string fullPrompt = buildPlanningPrompt(prompt, effContext);
            const size_t SUBS_SIZE = 4 * 1024 * 1024;
            char* encodedResult = new char[SUBS_SIZE];
            memset(encodedResult, 0, SUBS_SIZE);

            SwarmJob job = {};
            job.ModelId = -1; job.Priority = 100;
            job.PromptData = fullPrompt.c_str();
            job.PromptLength = (uint32_t)fullPrompt.length();
            job.MaxTokens = 4096; job.Temperature = 0.2f; job.TopP = 0.9f;
            job.Status = 0; job.ResultBuffer = encodedResult; job.ResultCapacity = SUBS_SIZE;
            
            std::cout << "[PlanOrchestrator] Submitting job to Swarm..." << std::endl;
            pfnSwarmSubmit(&job);
            
            int retries = 0;
            while (job.Status != 4 && job.Status != 6 && retries < 600) { Sleep(100); retries++; }
            
            if (job.Status == 4) {
                 result = parsePlanningResponse(std::string(job.ResultBuffer));
                 if (result.success) {
                     delete[] encodedResult; FreeLibrary(hDll);
                     planningCompleted(result); return result;
                 }
            }
    ...
    // 2. AI-driven planning using Universal Model Router
    if (m_modelRouter) {
...
ExecutionResult PlanOrchestrator::executePlan(const PlanningResult& plan, bool dryRun) {
    ...
    for (const auto& task : plan.tasks) {
        ...
        if (ok) {
            ...
            if (!dryRun) {
                 std::cerr << "[PlanOrchestrator] Task failed. Initiating rollback." << std::endl;
                 rollback();
                 result.success = false;
...
void PlanOrchestrator::rollback() {
    std::cout << "[PlanOrchestrator] Rolling back changes..." << std::endl;
    for (const auto& pair : m_originalFileContents) {
        writeFileContent(pair.first, pair.second);
        std::cout << "Restored " << pair.first << std::endl;
    }
    m_originalFileContents.clear();
}
