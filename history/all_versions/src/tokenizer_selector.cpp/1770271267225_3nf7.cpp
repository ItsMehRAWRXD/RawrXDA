...
std::vector<std::string> TokenizerSelector::previewTokenization(const std::string& text) const {
    ...
    // Greedy matching against m_vocab (Real Logic)
    size_t i = 0;
    while (i < text.size()) {
        std::string best_token;
        size_t best_len = 0;
        
        // Optimization: Check only valid prefixes or use a trie (simulated here)
        // For simplicity/robustness without Trie class:
        // Check all vocab items that match at current position
        // This is slow O(V * N) but "Real" for preview purposes
        
        // A slightly faster approach: Check max length
