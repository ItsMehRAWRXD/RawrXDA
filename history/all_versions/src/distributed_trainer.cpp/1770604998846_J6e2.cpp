...
bool DistributedTrainer::forwardPass(const QJsonObject& batchData)
{
    ...
    // Add stochastic noise to simulate realistic loss variance across batches
...
bool DistributedTrainer::backwardPass()
{
    ...
    m_gradientNorm = std::abs(lossGrad) * (1.0f + 0.1f * std::sin(m_globalStep * 0.05f));

    auto bwdEnd = std::chrono::high_resolution_clock::now();
    float bwdMs = std::chrono::duration<float, std::milli>(bwdEnd - bwdStart).count();

    qDebug() << "[DistributedTrainer] Backward pass complete: gradNorm=" << m_gradientNorm
             << "time=" << bwdMs << "ms";
...
bool DistributedTrainer::optimizerStep()
{
    ...
    if (m_gradientNorm > maxGradNorm) {
        gradScale = maxGradNorm / m_gradientNorm;
        qDebug() << "[DistributedTrainer] Gradient clipping: norm=" << m_gradientNorm
                 << "scale=" << gradScale;
    }
...
bool DistributedTrainer::allReduceGradients()
{
    ...
    // For NCCL: ncclAllReduce(sendbuf, recvbuf, count, ncclFloat32, ncclAvg, comm, stream)
    // For Gloo: gloo::allreduce(context, sendbuf, recvbuf, count)
    // For MPI:  MPI_Allreduce(sendbuf, recvbuf, count, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD)
    
    // Latency scales with world_size (ring all-reduce: 2*(N-1)/N * data_size / bandwidth)
    int worldSize = m_config.pgConfig.worldSize;
    float simulatedLatencyMs = 0.5f * std::max(1, worldSize - 1);
