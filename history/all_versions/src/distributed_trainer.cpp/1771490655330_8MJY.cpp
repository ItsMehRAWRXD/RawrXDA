...
void DistributedTrainer::Checkpoint(const std::string& path) {
    ...
    try {
        ...
        out.write(reinterpret_cast<const char*>(&m_metrics.totalSteps), sizeof(m_metrics.totalSteps));
        out.write(reinterpret_cast<const char*>(&m_metrics.currentLoss), sizeof(m_metrics.currentLoss));
        out.close();
...
void DistributedTrainer::updateMetrics(float stepTimeMs) {
    m_metrics.totalSteps++;
    m_metrics.avgStepTimeMs = (m_metrics.avgStepTimeMs * (m_metrics.totalSteps - 1) + stepTimeMs) / m_metrics.totalSteps;
}
