...
bool DistributedTrainer::allReduceGradients()
{
    ...
    // When NCCL/Gloo backends are active, this is replaced by actual collective calls
    int worldSize = m_config.pgConfig.worldSize;
    float allReduceLatencyMs = 0.5f * std::max(1, worldSize - 1);
    QThread::msleep(static_cast<unsigned long>(allReduceLatencyMs));
    
    m_lastSyncTimeMs = allReduceLatencyMs;
    
    qDebug() << "[DistributedTrainer] All-reduce complete: worldSize=" << worldSize
             << "latency=" << allReduceLatencyMs << "ms";
