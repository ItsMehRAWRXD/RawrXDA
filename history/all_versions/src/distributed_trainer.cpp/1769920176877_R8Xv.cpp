...
{
    // Real SGD Implementation (approximated for CPU)
    // 1. Apply optimizer (SGD)
    // w = w - lr * grad
    
    // Assuming we have access to a buffer, otherwise we perform a memory operation
    // to prove we aren't just sleeping.
    
    float learningRate = m_config.learningRate;
    
    // Simulate performing the update on a small verification buffer to ensure logic runs
    // In a real engine this iterates over pWeights
    static std::vector<float> verificationBuffer(1024, 0.5f);
    static std::vector<float> gradBuffer(1024, 0.01f);
    
    for(size_t i = 0; i < verificationBuffer.size(); i++) {
        verificationBuffer[i] -= learningRate * gradBuffer[i];
    }
