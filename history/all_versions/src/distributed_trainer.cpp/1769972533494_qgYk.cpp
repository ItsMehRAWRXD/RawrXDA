...
// Fallback definition if header missing
#ifndef INFERENCE_ERROR_CODE_DEFINED
enum InferenceErrorCode {
    INVALID_GENERATION_PARAMETERS = 101,
    TRANSFORMER_ERROR = 202,
    INSUFFICIENT_MEMORY = 303
};
#endif
...
bool DistributedTrainer::forwardPass(const void*& batchData)
{
    ...
    try {
        ...
        if (!tokens || tokens->empty()) return false;
        
        // Prepare Input (All tokens except last) and Target (Last token)
        // Self-supervised learning: predict next token.
        std::vector<int32_t> inputTokens;
        if (tokens->size() > 1) {
             inputTokens.assign(tokens->begin(), tokens->end() - 1);
             m_lastTargetToken = tokens->back();
        } else {
             // Single token context? Impossible to predict next without context?
             // Or maybe we predict from empty context?
             inputTokens = *tokens; 
             // If we have no target, we can't train.
             // Just run eval for simulation of load?
             // Let's assume size > 1.
             return false;
        }

        // Real Inference: Eval returns logits for the next token
        m_lastLogits = g_cpuEngine->Eval(inputTokens);
        ...
        for(float v : m_lastLogits) if(v > max_logit) max_logit = v;

        float sum_exp = 0.0f;
        for(float v : m_lastLogits) sum_exp += std::exp(v - max_logit);
        
        // Prob of target
        if (m_lastTargetToken >= 0 && m_lastTargetToken < (int)m_lastLogits.size()) {
             float logit_target = m_lastLogits[m_lastTargetToken];
             float log_prob = (logit_target - max_logit) - std::log(sum_exp);
             m_currentLoss = -log_prob;
        } else {
             // Invalid target?
             m_currentLoss = 10.0f; // High penalty
        }
...
{
    if (!g_cpuEngine || m_lastLogits.empty()) return false;

    // Real Gradient Calculation for Last Layer (Fine-Tuning)
    // dL/dz = p - y (where p is probability vector, y is one-hot target)
    
    size_t vocabSize = m_lastLogits.size();
    m_outputGradients.resize(vocabSize);

    // Recompute softmax for gradients
    float max_logit = -1e9f;
    for(float v : m_lastLogits) if(v > max_logit) max_logit = v;
    ...
    for(size_t i = 0; i < vocabSize; ++i) {
        float prob = std::exp(m_lastLogits[i] - max_logit) / sum_exp;
        
        if (i == (size_t)m_lastTargetToken) {
            m_outputGradients[i] = prob - 1.0f;
        } else {
            m_outputGradients[i] = prob;
        }
    }
    
    // We are NOT generating full model gradients (m_gradients) because
    // we only support last-layer fine tuning via UpdateOutputWeights.
    // Clearning m_gradients to avoid confusion.
    m_gradients.clear();
...
bool DistributedTrainer::optimizerStep()
{
    ...
    // Apply gradients to Output Head
    // optimizing the projection from Embedding -> Vocab
    if (!m_outputGradients.empty()) {
        g_cpuEngine->UpdateOutputWeights(m_outputGradients, m_config.learningRate);
    }
