...
CodeQualityMetrics CodebaseAuditSystem::analyze_cpp_quality(const std::string& content) {
    ...
    metrics.function_count = std::count(content.begin(), content.end(), '(');
    metrics.class_count = std::count(content.begin(), content.end(), '{');
    
    // Estimate complexity (simplified - count control flow keywords)
    metrics.cyclomatic_complexity = 
        std::count(content.begin(), content.end(), '?') +
        std::count(content.begin(), content.end(), ':') +
        (content.find("if") != std::string::npos ? 1 : 0) +
        (content.find("else") != std::string::npos ? 1 : 0) +
        (content.find("for") != std::string::npos ? 1 : 0) +
        (content.find("while") != std::string::npos ? 1 : 0) +
        (content.find("case") != std::string::npos ? 1 : 0);
