...
bool Win32IDEBridge::initializeCapabilities() {
    ...
    for (auto& cap : capabilities) {
        ...
        Wiring::CapabilityFactory factory = [cap]() {
            // Instantiate the capability from the discovered build artifact
            // Each capability self-registers its command handlers and UI hooks
            return cap.createInstance();
        };
