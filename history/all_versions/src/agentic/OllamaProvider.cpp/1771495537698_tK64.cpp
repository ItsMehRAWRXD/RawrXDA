...
void ParseUrl(const std::string& baseUrl, std::wstring& host, INTERNET_PORT& port) {
    ...
    if (colonSlash != std::string::npos) {
        ...
        if (colonPos != std::string::npos) {
            std::string hostStr = hostPort.substr(0, colonPos);
...
void OllamaProvider::PostJsonStreaming(const std::string& endpoint,
    ...
    // Long receive timeout for streaming (Copilot/FIM can run long without output).
    WinHttpSetTimeouts(hRequest, 10000, 15000, 30000, 3600000);
