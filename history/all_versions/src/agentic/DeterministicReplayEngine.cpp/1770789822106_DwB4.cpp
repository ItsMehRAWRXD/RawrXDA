    ...
    if (j.contains("steps") && j["steps"].is_array()) {
        for (const auto& sj : j["steps"]) {
            ...
            if (sj.contains("tool_call")) {
                const auto& tc = sj["tool_call"];
                step.toolCallName = tc.value("name", "");
                ...
                if (sj.contains("tool_result")) {
                    const auto& tr = sj["tool_result"];
                    if (tr.value("success", false)) {
                        step.toolResult = ToolCallResult::Ok(tr.value("output", ""));
                    } else {
                        step.toolResult = ToolCallResult::Error(tr.value("error", ""));
                    }
