...
void ModelGuidedPlanner::invoke_model_for_planning(uint64_t taskId, const std::string& prompt,
    ...
    // Use shared logic to ensure model is loaded
    ensureModelLoaded();
    
    outPlan.modelUsed = currentLoadedModelPath_.empty() ? "Fallback-Internal" : currentLoadedModelPath_;

    if (!currentLoadedModelPath_.empty()) {
        try {
             response = inferenceEngine_->infer(prompt + "\nResponse format: JSON array of steps.");
        } catch (...) {
             response = "[{\"stepId\":0,\"action\":0,\"actionDescription\":\"Inference exception\",\"confidenceScore\":0.0}]";
        }
    } else {
        response = "[{\"stepId\":0,\"action\":0,\"actionDescription\":\"Analyze codebase structure (Default - No Model)\",\"confidenceScore\":1.0}]";
    }
