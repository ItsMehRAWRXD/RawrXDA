...
bool ModelGuidedPlanner::getNextToken(StreamingDecoderState& state, std::string& token) {
    ...
    // Fetch next token from model
    // Attempt to use CPU inference engine if available
    try {
        ...
        if (!initialized) {
             ...
             if (!settings.getCurrentModelPath().empty()) {
                engine.LoadModel(settings.getCurrentModelPath());
                initialized = true;
             }
        }

        if (initialized) {
            // Generating one token at a time involves caching KV, which is complex here.
            // For now, we simulate the token stream by referencing a simpler generation step
            // or just returning a placeholder if state.currentTokenIndex is small
            if (state.currentTokenIndex < 10) {
                 token = " step"; 
                 return true;
            } else {
                 token = "";
...
ModelGuidedPlanner::PlanningStats ModelGuidedPlanner::getStatistics() const {
    ...
    for (const auto& [_, record] : plans_) {
        ...
        if (plan.executionApproved) {
            ...
            failureCount++;
    // Call model via Swarm Orchestrator (or local engine fallback)
    auto& settings = RawrXD::InferenceSettingsManager::getInstance();
    std::string modelPath = settings.getCurrentModelPath();
    ...
    if (!modelPath.empty()) {
        ...
        // Construct a safe default plan if no model
    ...
    outPlan.overallConfidence = outPlan.steps.empty() ? 0.0f : outPlan.steps[0].confidenceScore;
}
...
void ModelGuidedPlanner::invoke_model_for_planning(uint64_t taskId, const std::string& prompt,
    ...
    try {
        auto j = json::parse(modelOutput);
        
        if (j.is_array()) {
            for (const auto& item : j) {
                PlanStep step;
                step.stepId = item.value("stepId", 0);
                step.action = static_cast<PlanAction>(item.value("action", 0));
