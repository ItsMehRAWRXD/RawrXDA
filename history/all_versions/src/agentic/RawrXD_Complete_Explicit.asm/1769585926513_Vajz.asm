...
YTFN_SENTINEL           EQU 7FFFFFFFFFFFFFFFh
...
g_system_tick_freq      DQ  0

g_init_complete         DD  0

g_class_slot_counts     DD  1
                        DD  2
                        DD  4
                        DD  8
                        DD  16
...
Infinity_InitLocks PROC FRAME
    ...
    ; Load address of status_lock (assuming it's at fixed offset in struct)
    mov rcx, rbx
    add rcx, 64                     ; Adjust offset as needed for your struct
    call InitializeSRWLock
    
    ; Load address of stats_lock
    mov rcx, rbx
    add rcx, 128                    ; Adjust offset as needed for your struct
    call InitializeSRWLock
...
Infinity_ClassifyModel PROC FRAME
    ...
    mov r10, 7000000000             ; 7B (load into 64-bit register)
    cmp rcx, r10
    jbe class_a
    mov r10, 13000000000            ; 13B
    cmp rcx, r10
    jbe class_b
    mov r10, 70000000000            ; 70B
    cmp rcx, r10
    jbe class_c
    mov r10, 200000000000           ; 200B
    cmp rcx, r10
    jbe class_d
...
INFINITY_InitializeStream PROC FRAME
    ...
    mov rbx, [rsp + 40 + 56]        ; total_parameters (shadow space + pushed regs)
