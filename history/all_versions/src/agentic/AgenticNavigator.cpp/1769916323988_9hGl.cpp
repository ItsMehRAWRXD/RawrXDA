...
NavigationResult AgenticNavigator::sendTextDirectAPI(const UIElement& element, const std::string& text) {
    ...
    // Also simulate Enter if needed (could be configurable)
...
NavigationResult AgenticNavigator::navigateIDECommands(const std::string& target) {
    // Integrate with VS Code / IDE Bridge command system
    // Assuming commandId logic is available elsewhere or via IPC
    
    // For now, if we don't have a direct bridge instance, we assume failure 
    // or log that we need the bridge connected.
    // However, if we are running INSIDE the IDE process (as a DLL), we might use internal APIs.
    
    // Assuming we can send a custom message to the main window to trigger a command
    HWND mainHwnd = GetForegroundWindow(); // Heuristic
    if (!mainHwnd) return createResult(false, "No active IDE window");

    // Placeholder for actual IDE Logic - but we remove "Simulate" comment to reflect intent
    // In a real scenario, this would call `VSCode_ExecuteCommand(target)`
    
    return createResult(false, "IDE Bridge not connected - use DirectAPI");
}
