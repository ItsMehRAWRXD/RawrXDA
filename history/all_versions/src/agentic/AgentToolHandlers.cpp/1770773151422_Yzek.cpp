...
ToolCallResult AgentToolHandlers::ReadFile(const json& args) {
...
bool AgentToolHandlers::RunProcess(const std::wstring& cmdLine, DWORD timeoutMs,
...
ToolCallResult AgentToolHandlers::ExecuteCommand(const json& args) {
    ...
    DWORD timeout = s_guardrails.commandTimeoutMs;
    if (args.contains("timeout") && args["timeout"].is_number()) {
        timeout = static_cast<DWORD>(args["timeout"].get<int>());
