...
// ---------------------------------------------------------------------------
// Recovery result (PatchResult pattern)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// DiskRecoveryAgent — High-level C++ wrapper with RAII and observability
// ---------------------------------------------------------------------------
class DiskRecoveryAgent {
public:
    using ProgressCallback = std::function<void(const RecoveryStats& stats)>;

    DiskRecoveryAgent() = default;
    ~DiskRecoveryAgent();
    ...
    RecoveryResult Initialize(int driveNum);
