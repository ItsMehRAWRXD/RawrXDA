    ...
    const TelemetrySnapshot& snap)
{
    ...
    // Implement KV-cache swap to disk via memory-mapped files
    {
        ...
        if (hFile != INVALID_HANDLE_VALUE) {
            ...
            uint64_t swapSize = (256ULL * 1024 * 1024); // 256MB default
            if (swapSize > 0) {
                ...
                if (hMapping) {
                    ...
                    if (mapped) {
                        ...
                        // Keep the swap operation bounded and release resources immediately.
                        const size_t warmupBytes = static_cast<size_t>((swapSize > (8ULL * 1024 * 1024))
                            ? (8ULL * 1024 * 1024)
                            : swapSize);
                        std::memset(mapped, 0, warmupBytes);
        ...
        if (fd >= 0) {
            ...
            uint64_t swapSize = (256ULL * 1024 * 1024);
