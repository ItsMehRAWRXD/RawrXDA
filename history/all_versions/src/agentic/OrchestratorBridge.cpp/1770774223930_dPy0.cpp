...
void OrchestratorBridge::WireToolSchemas() {
    // The AgentToolRegistry already generates schemas via GetToolSchemas().
    // The BoundedAgentLoop uses AgentToolHandlers::GetAllSchemas().
    // We need both to produce consistent schemas.
    //
    // Strategy: The X-Macro is the source of truth. We don't override
    // AgentToolHandlers schemas, but ensure the BoundedAgentLoop gets
    // the X-Macro schemas when tools are injected into LLM requests.
}
...
void OrchestratorBridge::WireToolHandlers() {
    ...
    registry.RegisterHandler("read_file", [](const nlohmann::json& args) -> ToolExecResult {
        auto tcr = AgentToolHandlers::ReadFile(args);
    ...
    Prediction::StreamTokenCallback onToken)
{
    ...
    m_ollamaClient->FIMStream(
        ...
        [](const std::string&, uint64_t, uint64_t, double) {},
        [](const std::string&) {}
    );
}
