...
void AgentOllamaClient::InitWinHTTP() {
    ...
    if (m_hSession) {
        ...
        // Resolve/Connect: 15s, Send: 30s, Receive: long (streams can be quiet for a while)
        DWORD receive = timeout;
        if (receive < 3600000) receive = 3600000; // 1 hour minimum
        WinHttpSetTimeouts(m_hSession, 15000, 15000, 30000, receive);
    }
}
