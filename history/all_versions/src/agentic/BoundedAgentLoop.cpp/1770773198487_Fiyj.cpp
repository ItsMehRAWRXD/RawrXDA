...
std::string BoundedAgentLoop::RunLoop(const std::string& userPrompt) {
    ...
    while (step < m_config.maxSteps) {
        ...
        if (response.hasToolCall) {
            ...
            ts.tokensSent = EstimateTokens(request.messages.dump());
...
ToolCallResult BoundedAgentLoop::DispatchTool(const std::string& name, const json& args) {
    if (name == "read_file")        return AgentToolHandlers::ReadFile(args);
...
json BoundedAgentLoop::BuildAssistantToolCallMessage(const LLMChatResponse& response) {
    ...
    msg["tool_calls"] = json::array({
        {
            {"id", response.toolCallId.empty() ? "call_0" : response.toolCallId},
            {"type", "function"},
            {"function", {
    ...
    try {
        ...
        if (respJson.contains("message")) {
            ...
            if (msg.contains("tool_calls") && msg["tool_calls"].is_array() &&
                ...
                auto& tc = msg["tool_calls"][0]; // Handle first tool call
                response.hasToolCall = true;

                if (tc.contains("function")) {
                    if (tc["function"].contains("name")) {
                        response.toolName = tc["function"]["name"].get<std::string>();
                    }
                    if (tc["function"].contains("arguments")) {
                        if (tc["function"]["arguments"].is_string()) {
                            response.toolArgs = json::parse(tc["function"]["arguments"].get<std::string>());
                        } else {
                            response.toolArgs = tc["function"]["arguments"];
                        }
                    }
                }

                if (tc.contains("id")) {
                    response.toolCallId = tc["id"].get<std::string>();
                } else {
