...
bool NeonFabric::initialize(const FabricConfig& config) {
    ...
    // Create real named shared memory for the control block
    static HANDLE hMapFile = NULL;
    static bool isCreator = false;
    
    // Try to open existing first
    hMapFile = OpenFileMappingA(FILE_MAP_ALL_ACCESS, FALSE, "Local\\RawrXDFabricControl");
    ...
    if (!hMapFile) {
        ...
        hMapFile = CreateFileMappingA(
             ...
             PAGE_READWRITE,
             0,
             sizeof(FabricControlBlock),
             "Local\\RawrXDFabricControl");
        isCreator = true;
    }

    if (hMapFile) {
        s_controlBlock = (FabricControlBlock*)MapViewOfFile(hMapFile, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(FabricControlBlock));
    }
    
    // Fallback to local heap if SHM fails
    if (!s_controlBlock) {
        static FabricControlBlock fallbackBlock;
        s_controlBlock = &fallbackBlock;
    } else if (isCreator) {
