...
void AgentToolRegistry::InitDescriptors() {
    // Use X-Macro to populate descriptors
    #define INIT_DESCRIPTOR(name, desc, params) \
    { \
        ToolDescriptor td; \
        td.name = #name; \
        td.description = desc; \
        td.params_schema = params; \
        td.handler = nullptr; \
...
json AgentToolRegistry::GetToolSchemas() const {
    ...
    for (const auto& td : m_tools) {
        ...
        for (auto& [key, val] : td.params_schema.items()) {
...
ToolExecResult AgentToolRegistry::Dispatch(const std::string& tool_name, const json& args) {
    ...
    if (!ValidateArgs(tool_name, args, validationError)) {
        td.error_count.fetch_add(1, std::memory_order_relaxed);
    ...
    td.invocation_count.fetch_add(1, std::memory_order_relaxed);
    if (!result.success) {
        td.error_count.fetch_add(1, std::memory_order_relaxed);
    }
...
uint64_t AgentToolRegistry::GetTotalInvocations() const {
    uint64_t total = 0;
    for (const auto& td : m_tools) {
        total += td.invocation_count.load(std::memory_order_relaxed);
    }
    return total;
}
...
uint64_t AgentToolRegistry::GetTotalErrors() const {
    uint64_t total = 0;
    for (const auto& td : m_tools) {
        total += td.error_count.load(std::memory_order_relaxed);
    }
    return total;
}
    ...
    std::string& error) const
{
    ...
    for (auto& [key, schema] : td.params_schema.items()) {
