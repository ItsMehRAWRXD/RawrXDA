...
// All stubs log once on first call, then silently no-op for performance.
...
// The CPU inference engine (rawrxd_neural_core.asm / cpu_inference_engine.cpp)
// handles all compute in this mode.
//
// Each stub logs a one-time warning so developers know GPU acceleration is off.
// ============================================================================

namespace {
    bool g_stubWarned = false;
    void warnStubOnce() {
        if (!g_stubWarned) {
            fprintf(stderr, "[RawrXD] Vulkan stubs active - GPU acceleration disabled, using CPU inference\n");
            g_stubWarned = true;
        }
    }
}
