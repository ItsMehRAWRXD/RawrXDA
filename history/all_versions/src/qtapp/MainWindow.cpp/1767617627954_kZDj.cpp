...
void MainWindow::setupToolBars()
{
    ...
    connect(m_modelSelector, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int idx) {
        ...
        if (modelData.startsWith("ollama:")) {
            ...
            if (!gguf.isEmpty()) {
                loadGGUFModel(); // will open file dialog; consider implementing overload to accept gguf path
                // Note: for now, user will need to select the resolved GGUF in the file dialog
            } else {
...
void MainWindow::refreshModelSelector()
{
    ...
    for (const QString& dirPath : searchDirs) {
        ...
        while (it.hasNext()) {
            ...
            QString tooltip = QString("<b>%1</b><br/>Path: %2").arg(display).arg(file);
