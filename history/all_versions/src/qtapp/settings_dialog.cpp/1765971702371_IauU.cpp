...
void SettingsDialog::setupUI()
{
    ...
    // Memory Space Tab
    tabWidget->addTab(createMemoryTab(), "Memory");
...
void SettingsDialog::loadSettings()
{
    ...
    // Memory Space
    const auto bytesToValueUnit = [](qint64 bytes, double &outValue, QString &outUnit) {
    ...
    bool memoryEnabled = m_settings->getValue("memory/enabled", false).toBool();
    qint64 memoryLimit = m_settings->getValue("memory/limitBytes", static_cast<qint64>(134217728)).toLongLong();
    m_enableMemorySpace->setChecked(memoryEnabled);

    double limitValue = 128.0;
    QString limitUnit = "MB";
    bytesToValueUnit(memoryLimit, limitValue, limitUnit);
    m_memoryLimitValue->setValue(limitValue);
    m_memoryLimitUnit->setCurrentText(limitUnit);
...
void SettingsDialog::applySettings()
{
    ...
    // Memory Space
    bool memoryEnabled = m_enableMemorySpace->isChecked();
    double limitVal = m_memoryLimitValue->value();
    QString unit = m_memoryLimitUnit->currentText();
    qint64 limitBytes = 0;
    if (unit == "GB") {
        limitBytes = static_cast<qint64>(limitVal * 1024.0 * 1024.0 * 1024.0);
    } else if (unit == "MB") {
        limitBytes = static_cast<qint64>(limitVal * 1024.0 * 1024.0);
    } else {
        limitBytes = static_cast<qint64>(limitVal * 1024.0);
    }

    m_settings->setValue("memory/enabled", memoryEnabled);
    m_settings->setValue("memory/limitBytes", limitBytes);

    MemorySpaceManager::instance().setEnabled(memoryEnabled);
    MemorySpaceManager::instance().setLimitBytes(limitBytes);
    if (!memoryEnabled) {
