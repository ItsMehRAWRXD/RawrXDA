...
{
    // Apply visual settings from sliders
    if (m_transparencySlider) {
        int transparency = m_transparencySlider->value();
        qDebug() << "Applying transparency:" << transparency << "%";
        // Apply transparency to main window
        if (parentWidget()) {
            parentWidget()->setWindowOpacity(transparency / 100.0);
        }
    }
...
void SettingsDialog::resetVisualSettings()
{
    ...
    if (m_brightnessSlider) m_brightnessSlider->setValue(100);
    if (m_contrastSlider) m_contrastSlider->setValue(100);
    if (m_hueRotationSlider) m_hueRotationSlider->setValue(0);
    
    // Reset window opacity
    if (parentWidget()) {
        parentWidget()->setWindowOpacity(1.0);
    }
