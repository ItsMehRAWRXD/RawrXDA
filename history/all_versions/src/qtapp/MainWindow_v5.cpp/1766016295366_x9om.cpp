...
void MainWindow::initializePhase4()
{
    ...
    try {
        ...
        // Replace splash with actual editor (or fallback if editor failed)
        QTimer::singleShot(500, [this]() {
            if (m_splashWidget) {
                m_splashWidget->deleteLater();
                m_splashWidget = nullptr;
            }
            // If the multi‑tab editor was successfully created, use it.
            if (m_multiTabEditor) {
                setCentralWidget(m_multiTabEditor);
                m_multiTabEditor->show();
            } else {
                // Fallback: show a simple placeholder widget so the UI is not just white.
                QWidget *fallback = new QWidget(this);
                QVBoxLayout *layout = new QVBoxLayout(fallback);
                QLabel *msg = new QLabel("<h2>Editor failed to initialize</h2><p>Please check the logs for details.</p>", fallback);
            ...
            // Initialize Phase 2 Polish Features (only if editor exists)
            if (m_multiTabEditor) {
