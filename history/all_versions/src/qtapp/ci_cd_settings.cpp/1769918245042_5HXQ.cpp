...
std::string CICDSettings::deployModel(const std::string& jobId, const std::string& runId)
{
    ...
    // Explicit Deployment Logic: Execute deployment script with parameters
    // We pass jobID and runID as arguments to the script
    std::string command = "powershell.exe -ExecutionPolicy Bypass -File \"deployment/deploy.ps1\" -JobId " + jobId + " -RunId " + runId;
    
    // Log the actual command attempt
    std::cout << "[CI/CD] Executing real deployment: " << command << std::endl;
    
    // Execute synchronous system call
    int ret = std::system(command.c_str());
    
    bool success = (ret == 0);
    deploymentCompleted(deploymentId, success);
