...
namespace {
// Aggregate snapshot helper to avoid double-locking the metrics mutex.
MetricsCollector::AggregateMetrics computeAggregateFromSnapshot(const QList<MetricsCollector::RequestMetrics>& completed) {
    MetricsCollector::AggregateMetrics agg;

    if (completed.isEmpty()) {
        return agg;
    }

    agg.totalRequests = completed.size();
    agg.firstRequest = completed.first().startTime;
    agg.lastRequest = completed.last().endTime;
    ...
    for (const auto& metrics : completed) {
        if (metrics.success) {
    ...
    agg.avgMemoryUsage = agg.totalRequests > 0 ? (totalMemory / agg.totalRequests) : 0;
...
MetricsCollector::AggregateMetrics MetricsCollector::getAggregateMetrics() const {
    QMutexLocker locker(&m_mutex);
    return computeAggregateFromSnapshot(m_completedRequests);
}

QString MetricsCollector::exportToJson() const {
    QList<RequestMetrics> completedSnapshot;
    QHash<QString, qint64> countersSnapshot;

    {
        QMutexLocker locker(&m_mutex);
        completedSnapshot = m_completedRequests;
        countersSnapshot = m_counters;
    }

    AggregateMetrics agg = computeAggregateFromSnapshot(completedSnapshot);
    ...
    if (!countersSnapshot.isEmpty()) {
        QJsonObject countersObj;
        for (auto it = countersSnapshot.constBegin(); it != countersSnapshot.constEnd(); ++it) {
    ...
    for (const auto& metrics : completedSnapshot) {
        QJsonObject reqObj;
