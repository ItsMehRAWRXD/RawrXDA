...
std::vector<int32_t> InferenceEngine::generate(const std::vector<int32_t>& inputTokens, int maxTokens)
{
    ...
    if (connectToAgent()) {
        std::string promptStr = detokenize(inputTokens);
        std::string responseStr = sendToAgent(promptStr);
        if (!responseStr.empty()) {
             std::vector<int32_t> generated = tokenize(responseStr);
    ...
    if (m_transformer.isReady()) {
        ...
        telemetry.recordTiming("inference", "generate.fallback", timer.elapsedMs(), "tokens_out=%1"));
    }
...
std::string InferenceEngine::sendToAgent(const std::string& prompt) {
    ...
    // Read Response
    char buffer[4096]; // Matching Pipe Server Buffer
    DWORD bytesRead;
    success = ReadFile(
        (HANDLE)m_hPipe,
        buffer,
