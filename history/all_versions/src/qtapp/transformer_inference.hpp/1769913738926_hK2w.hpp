...
private:
    ...
    typedef int (*PFN_Titan_RunInferenceStep)(void* pContext);
    typedef int (*PFN_Titan_Shutdown)(void* pContext);
    typedef float* (*PFN_Titan_GetLogits)(void* pContext); // Assuming we can read memory or add this
    
    PFN_Titan_Initialize m_pfnTitanInit{nullptr};
    PFN_Titan_LoadModel m_pfnTitanLoad{nullptr};
    PFN_Titan_RunInferenceStep m_pfnTitanStep{nullptr};
