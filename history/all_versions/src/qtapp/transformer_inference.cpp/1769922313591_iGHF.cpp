...
bool TransformerInference::initTitanBackend(const std::string& modelPath) {
    ...
    // Explicit Implementation of File Mapping for Titan Engine
    HANDLE hFile = CreateFileA(modelPath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    ctx->hFile = (uint64_t)hFile;
    ctx->hMap = (uint64_t)hMap;
    ctx->pFileBase = (uint64_t)pBase;
    ctx->cbFile = (uint64_t)fileSize.QuadPart;
    ctx->status = 1; // Loaded
...
std::vector<float> TransformerInference::forward(const std::vector<int32_t>& tokens) {
    ...
    if (m_ggufDirectMode && m_nVocab > 0) {
        ...
        if (m_titanContext && m_pfnTitanRunInferenceStep) {
            ...
            // EXPLICIT LOGIC: Read real logits from standard Titan context memory
            // The Titan backend writes logits to the mapped memory at ctx->pLogits (offset)
            // We read them back to provide true probability distribution, not just greedy output.
            
            std::vector<float> realLogits(m_nVocab, -INFINITY);
            TitanContext* ctx = (TitanContext*)m_titanContext;
            
            if (ctx && ctx->pLogits) {
                float* logitsBuffer = (float*)ctx->pLogits;
                // Copy real logits
                for (int i = 0; i < m_nVocab; i++) {
                     realLogits[i] = logitsBuffer[i];
                }
            } else if (outLen > 0) {
                // Fallback if logits buffer invalid but we got a token (Greedy approximation)
                int idx = std::abs(resultToken) % m_nVocab;
                realLogits[idx] = 100.0f;
            } else {
                 // No output, initializing (e.g. prompt processing)
                 // Return 0 entropy
                 std::fill(realLogits.begin(), realLogits.end(), 0.0f);
            }
