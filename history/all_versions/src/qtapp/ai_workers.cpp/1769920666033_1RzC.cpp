...
void AITrainingWorker::performEpoch() {
    // Native Training Logic
    std::string modelStr = m_modelPath.toStdString();
    
    // Initialize Engine
    auto engine = std::make_unique<RawrXD::CPUInferenceEngine>();
    if (!modelStr.empty() && std::filesystem::exists(modelStr)) {
        engine->LoadModel(modelStr);
    } 
    
    // Initialize Trainer
    ModelTrainer trainer;
    double epochLoss = 1.0;
    double epochAccuracy = 0.0;
    
    bool trainingRun = false;
    if (trainer.initialize(engine.get(), modelStr)) {
         ModelTrainer::TrainingConfig config;
         config.epochs = 1; 
         config.batchSize = m_config.batchSize > 0 ? m_config.batchSize : 4;
         config.learningRate = m_config.learningRate;
         
         trainer.onEpochCompleted = [&](int ep, float loss, float perplexity) {
             epochLoss = loss;
             epochAccuracy = 1.0 / (perplexity + 1e-9f);
             std::mutexLocker locker(&m_mutex);
             m_progress.loss = loss;
             m_progress.accuracy = epochAccuracy;
         };
         
         {
             std::mutexLocker locker(&m_mutex);
             m_progress.status = std::string("Training epoch " + std::to_string(m_progress.currentEpoch));
         }

         trainingRun = trainer.startTrainingSync(config);
         
         if (trainingRun) {
             if (epochLoss == 1.0 && epochAccuracy == 0.0) {
                 epochLoss = trainer.getCurrentLoss();
                 epochAccuracy = 1.0f / (trainer.getValidationPerplexity() + 1e-9f);
             }
    ...
    if (!trainingRun) {
         std::mutexLocker locker(&m_mutex);
         m_progress.status = "Training step failed.";
         setState(State::Finished);
         trainingCompleted(false, "Failed to execute training step.");
         return;
    }
