...
void MainWindow::loadGGUFModel() {
    ...
    try {
        ...
        // Get model info
        int nLayer = m_ggufLoader->getParam("n_layer", 0);
        int nEmbd = m_ggufLoader->getParam("n_embd", 0);
        int nVocab = m_ggufLoader->getParam("n_vocab", 0);
        int nCtx = m_ggufLoader->getParam("n_ctx", 0);
