...
void MainWindow::createMenuBar()
{
    ...
    AppendMenuA(agentMenu, MF_STRING, IDM_AI_CHAT, "Agent Chat\tCtrl+I");
    AppendMenuA(agentMenu, MF_STRING, IDM_AI_GENERATE, "Generate Code\tCtrl+L");
    AppendMenuA(agentMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenuA(agentMenu, MF_STRING, IDM_AI_AGENT_LOOP, "Start Autonomous Loop");
    AppendMenuA(agentMenu, MF_STRING, IDM_AI_AUTODIGEST, "Auto-Digest Project");
    AppendMenuA(agentMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenuA(agentMenu, MF_STRING, IDM_AI_TITAN_SLOTS, "Titan Kernel Manager");
    AppendMenuA(m_menuBar, MF_POPUP, (UINT_PTR)agentMenu, "&Agentic");
...
void MainWindow::handleMenuCommand(WORD cmdId)
{
    ...
    // ========== AGENTIC COMMANDS ==========
    case IDM_AI_CHAT:
        MessageBoxA(m_hwnd, "Agent Chat triggered. Connecting to RawrXD Native Bridge...", "Agentic", MB_OK | MB_ICONINFORMATION);
        // TODO: Call LoadModelNative/InferenceNative
        break;
    case IDM_AI_GENERATE:
        MessageBoxA(m_hwnd, "Autonomous Code Generation started...", "Agentic", MB_OK | MB_ICONINFORMATION);
        break;
    case IDM_AI_AGENT_LOOP:
        MessageBoxA(m_hwnd, "Titan Kernel: Autonomous Agent Loop Initialized.", "Agentic", MB_OK | MB_ICONINFORMATION);
        break;
    case IDM_AI_AUTODIGEST:
        MessageBoxA(m_hwnd, "Project Auto-Digest: Reading codebase into context-window...", "Agentic", MB_OK | MB_ICONINFORMATION);
        break;
    case IDM_AI_TITAN_SLOTS:
        MessageBoxA(m_hwnd, "Titan Kernel Slot Manager: 0/64 models resident.", "Agentic", MB_OK | MB_ICONINFORMATION);
        break;
...
void MainWindow::updateTelemetry()
{
    ...
    int cpuFreqMhz = static_cast<int>(m_appState->target_all_core_mhz) + m_appState->applied_core_offset_mhz;
    int gpuFreqMhz = m_appState->applied_gpu_offset_mhz;
    bool governorActive = m_appState->enable_overclock_governor && m_appState->governor_status != "stopped";
    std::stringstream ss;
    ss << "Overclock Panel\nCPU: " << cpuFreqMhz << " MHz\n"
       << "GPU: " << gpuFreqMhz << " MHz\n"
       << "Status: " << (governorActive ? "Active" : "Inactive");
#ifdef _WIN32
...
LRESULT CALLBACK MainWindow::FloatingPanelProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT: {
        ...
        if (window && window->m_appState) {
            int cpuFreqMhz = static_cast<int>(window->m_appState->target_all_core_mhz) + window->m_appState->applied_core_offset_mhz;
            int gpuFreqMhz = window->m_appState->applied_gpu_offset_mhz;
            bool governorActive = window->m_appState->enable_overclock_governor && window->m_appState->governor_status != "stopped";
            ss << "CPU: " << cpuFreqMhz << " MHz\n";
            ss << "GPU: " << gpuFreqMhz << " MHz\n\n";
            ss << "Governor: " << (governorActive ? "Active" : "Inactive") << "\n\n";
...
LRESULT CALLBACK GoToLineDialogProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    auto* state = reinterpret_cast<GoToLineDialogState*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
    switch (msg) {
        case WM_CREATE: {
            auto* create = reinterpret_cast<CREATESTRUCT*>(lParam);
            state = reinterpret_cast<GoToLineDialogState*>(create->lpCreateParams);
        ...
        case WM_COMMAND: {
            ...
            if (id == IDOK) {
                char buf[64] = {};
                if (state->edit) GetWindowTextA(state->edit, buf, sizeof(buf));
                state->line = std::atoi(buf);
                state->accepted = state->line > 0;
                state->done = true;
                DestroyWindow(hwnd);
                return 0;
            }
...
bool MainWindow::ShowGoToLineDialog() {
    ...
    if (!classRegistered) {
        WNDCLASSA wc = {};
        wc.lpfnWndProc = GoToLineDialogProc;
        wc.hInstance = GetModuleHandle(nullptr);
        wc.hCursor = LoadCursor(nullptr, IDC_ARROW);
        wc.hbrBackground = reinterpret_cast<HBRUSH>(COLOR_WINDOW + 1);
        wc.lpszClassName = "RawrXDGoToLineDialog";
        RegisterClassA(&wc);
        classRegistered = true;
    }

    GoToLineDialogState state;
    state.owner = this;
    HINSTANCE inst = GetModuleHandle(nullptr);
    HWND dlg = CreateWindowExA(WS_EX_DLGMODALFRAME, "RawrXDGoToLineDialog", "Go to Line",
        WS_CAPTION | WS_SYSMENU | WS_POPUP,
        CW_USEDEFAULT, CW_USEDEFAULT, 260, 140,
        m_hwnd, nullptr, inst, &state);
    if (!dlg) return false;

    CreateWindowExA(0, "STATIC", "Line number:", WS_CHILD | WS_VISIBLE,
        12, 12, 120, 20, dlg, nullptr, inst, nullptr);
    state.edit = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "", WS_CHILD | WS_VISIBLE | ES_NUMBER | ES_AUTOHSCROLL,
        12, 36, 220, 24, dlg, (HMENU)1001, inst, nullptr);
    HWND okBtn = CreateWindowExA(0, "BUTTON", "OK", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON,
        60, 75, 60, 24, dlg, (HMENU)IDOK, inst, nullptr);
    HWND cancelBtn = CreateWindowExA(0, "BUTTON", "Cancel", WS_CHILD | WS_VISIBLE,
        130, 75, 60, 24, dlg, (HMENU)IDCANCEL, inst, nullptr);

    HFONT font = static_cast<HFONT>(GetStockObject(DEFAULT_GUI_FONT));
    SendMessageA(state.edit, WM_SETFONT, (WPARAM)font, TRUE);
    SendMessageA(okBtn, WM_SETFONT, (WPARAM)font, TRUE);
    SendMessageA(cancelBtn, WM_SETFONT, (WPARAM)font, TRUE);

    if (m_lastGoToLine > 0) {
        std::string value = std::to_string(m_lastGoToLine);
        SetWindowTextA(state.edit, value.c_str());
    }

    RECT ownerRect;
    GetWindowRect(m_hwnd, &ownerRect);
    int x = ownerRect.left + ((ownerRect.right - ownerRect.left) - 260) / 2;
    int y = ownerRect.top + ((ownerRect.bottom - ownerRect.top) - 140) / 2;
    ...
    if (state.accepted) {
        m_lastGoToLine = state.line;
        return true;
    }
    return false;
}

int MainWindow::GetUserInputLineNumber() const {
    return m_lastGoToLine;
}

void MainWindow::NavigateToLine(int lineNumber) {
    if (!m_editorHwnd || lineNumber <= 0) return;
    int totalLines = (int)SendMessageA(m_editorHwnd, EM_GETLINECOUNT, 0, 0);
    int targetLine = std::max(1, std::min(lineNumber, totalLines));
    int index = (int)SendMessageA(m_editorHwnd, EM_LINEINDEX, targetLine - 1, 0);
    SendMessageA(m_editorHwnd, EM_SETSEL, (WPARAM)index, (LPARAM)index);
    SendMessageA(m_editorHwnd, EM_SCROLLCARET, 0, 0);
}

void MainWindow::LogNavigationAction(const std::string& action, int lineNumber) {
    std::ostringstream ss;
    ss << "# Navigation: " << action << " -> " << lineNumber << "\n";
...
void MainWindow::ToggleCommentForCurrentLanguage() {
    ...
    GetWindowTextA(m_editorHwnd, buf.data(), len + 1);
    std::string text(buf.data());
    std::string newline = text.find("\r\n") != std::string::npos ? "\r\n" : "\n";

    DWORD selStart = 0, selEnd = 0;
    SendMessageA(m_editorHwnd, EM_GETSEL, (WPARAM)&selStart, (LPARAM)&selEnd);
    int startLine = (int)SendMessageA(m_editorHwnd, EM_LINEFROMCHAR, selStart, 0);
    int endLine = (int)SendMessageA(m_editorHwnd, EM_LINEFROMCHAR, selEnd, 0);
    int endLineIndex = (int)SendMessageA(m_editorHwnd, EM_LINEINDEX, endLine, 0);
    if ((int)selEnd == endLineIndex && endLine > startLine) {
        endLine -= 1;
    }
    ...
    lines.reserve(128);
    size_t pos = 0;
    bool endsWithNewline = !text.empty() && text.back() == '\n';
    while (pos < text.size()) {
        ...
        pos = (next < text.size()) ? next + 1 : text.size();
    }

    if (startLine < 0 || endLine < 0 || lines.empty()) return;
    if (startLine >= (int)lines.size()) return;
    endLine = std::min(endLine, (int)lines.size() - 1);

    std::string ext = fs::path(m_currentFile).extension().string();
    std::string lowerExt;
    lowerExt.reserve(ext.size());
    for (char c : ext) lowerExt.push_back(static_cast<char>(std::tolower((unsigned char)c)));

    std::string prefix = "//";
    if (lowerExt == ".py" || lowerExt == ".ps1" || lowerExt == ".psm1" || lowerExt == ".sh" || lowerExt == ".bat" || lowerExt == ".cmd") {
        prefix = "#";
    } else if (lowerExt == ".lua" || lowerExt == ".sql") {
        prefix = "--";
    } else if (lowerExt == ".asm" || lowerExt == ".s" || lowerExt == ".ini" || lowerExt == ".cfg") {
        prefix = ";";
    }
    ...
    for (int i = startLine; i <= endLine; ++i) {
        ...
        hasNonEmpty = true;
        if (line.compare(idx, prefix.size(), prefix) != 0) {
            allCommented = false;
            break;
        }
    }
    if (!hasNonEmpty) return;
    bool shouldComment = !allCommented;
    ...
    for (int i = startLine; i <= endLine; ++i) {
        ...
        if (idx == std::string::npos) continue;
        if (shouldComment) {
            line.insert(idx, prefix + " ");
        } else {
            if (line.compare(idx, prefix.size(), prefix) == 0) {
                line.erase(idx, prefix.size());
                if (idx < line.size() && line[idx] == ' ') line.erase(idx, 1);
            }
        }
    }
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        out << lines[i];
        if (i + 1 < lines.size() || endsWithNewline) out << newline;
    }
    SetWindowTextA(m_editorHwnd, out.str().c_str());
}
...
void MainWindow::ExpandSelectionIntelligently() {
    ...
    DWORD selStart = 0, selEnd = 0;
    SendMessageA(m_editorHwnd, EM_GETSEL, (WPARAM)&selStart, (LPARAM)&selEnd);
    int line = (int)SendMessageA(m_editorHwnd, EM_LINEFROMCHAR, selStart, 0);
    int lineStart = (int)SendMessageA(m_editorHwnd, EM_LINEINDEX, line, 0);
    int lineLen = (int)SendMessageA(m_editorHwnd, EM_LINELENGTH, lineStart, 0);
    int lineEnd = lineStart + lineLen;
    SendMessageA(m_editorHwnd, EM_SETSEL, (WPARAM)lineStart, (LPARAM)lineEnd);
}
...
void MainWindow::ShrinkSelectionIntelligently() {
    ...
    SendMessageA(m_editorHwnd, EM_GETSEL, (WPARAM)&selStart, (LPARAM)&selEnd);
    if (selStart != selEnd) {
        SendMessageA(m_editorHwnd, EM_SETSEL, (WPARAM)selEnd, (LPARAM)selEnd);
    }
}
...
void MainWindow::loadGGUFModel() {
    ...
    try {
        ...
        m_ggufLoader = std::make_unique<GGUFLoaderQt>(filename);
        ...
        // Get model info
        int nLayer = m_ggufLoader->getParam("n_layer", QVariant(0)).toInt();
        int nEmbd = m_ggufLoader->getParam("n_embd", QVariant(0)).toInt();
        int nVocab = m_ggufLoader->getParam("n_vocab", QVariant(0)).toInt();
        int nCtx = m_ggufLoader->getParam("n_ctx", QVariant(0)).toInt();
...
void MainWindow::showModelInfo() {
    ...
    try {
        ...
        for (const auto& key : metaKeys) {
            QVariant value = m_ggufLoader->getParam(key, QVariant());
            std::string text = value.toStdString();
            if (value.isValid() && !text.empty()) {
                info << "  " << key << ": " << text << "\n";
            }
        }
...
void MainWindow::reloadCurrentModel() {
    ...
    // Recreate loader
    try {
        m_ggufLoader = std::make_unique<GGUFLoaderQt>(modelPath);
