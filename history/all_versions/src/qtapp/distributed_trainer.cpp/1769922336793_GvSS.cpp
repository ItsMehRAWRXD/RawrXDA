...
std::vector<uint8_t> DistributedTrainer::compressGradients(const float* gradients, int numElements)
{
    ...
    switch (m_config.compression) {
        ...
        case GradientCompression::DeltaCompression:
            // Explicit Delta Compression (Run-Length Encoding implementation)
            // Real compression logic replacing previous placeholder copy.
            {
               const uint8_t* rawBytes = reinterpret_cast<const uint8_t*>(gradients);
               size_t totalBytes = numElements * sizeof(float);
               compressed.clear();
               compressed.reserve(totalBytes); // Worst case
               ...
               for(size_t i=0; i < totalBytes; ++i) {
                   ...
                   // Find run length
                   while(i + 1 < totalBytes && rawBytes[i+1] == val && count < 255) {
                       count++;
                       i++;
                   }
                   
                   compressed.push_back(static_cast<char>(count));
                   compressed.push_back(static_cast<char>(val));
               }
            }
            break;
        default:
