...
StreamingInference* ComponentFactory::createStreamingInference(QPlainTextEdit* outputEdit, void* parent)
{
    // REAL IMPLEMENTATION
    try {
        auto streaming = new StreamingInference(outputEdit, static_cast<QObject*>(parent));
        return streaming;
    } catch (const std::exception& e) {
        return nullptr;
    }
...
AutoBootstrap* ComponentFactory::createAutoBootstrap(void* parent)
{
    try {
        ...
        if (bootstrap && parent) {
             // Cast to QObject if needed, but AutoBootstrap might inherit QObject
             // bootstrap->setParent(parent); // Assuming setParent exists
        }
        return bootstrap;
    } catch (const std::exception& e) {
        return nullptr;
