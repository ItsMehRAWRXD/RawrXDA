 ...
 * with std::string, QByteArray, and Qt signal/slot support.
 */
class GGUFLoaderQt {
public:
     ...
     */
    QVariant getParam(const std::string& key, const QVariant& defaultValue) const;
     ...
     */
    QByteArray inflateWeight(const std::string& tensorName);
    
    /**
     * @brief Get metadata for the tokenizer (if available)
     */
    QHash<std::string, QByteArray> getTokenizerMetadata() const;
     ...
     * 
     * @return std::stringList with format: "IQ4_NL (type 39): 145 tensors", etc.
     */
...
private:
    ...
    mutable QHash<std::string, QVariant> m_metadataCache;
