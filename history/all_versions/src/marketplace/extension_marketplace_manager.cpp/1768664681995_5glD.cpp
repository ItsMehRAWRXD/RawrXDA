...
void ExtensionMarketplaceManager::onInstallReplyFinished() {
    ...
    if (reply->error() != QNetworkReply::NoError) {
        ...
        emit installationError(QString(), reply->errorString());
        reply->deleteLater();
        return;
    }
    ...
    QString tempPath = QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/" + extensionId + ".vsix";
    ...
    if (tempFile.open(QIODevice::WriteOnly)) {
        ...
        if (m_vsixInstaller) {
            m_vsixInstaller->installVsix(tempPath);
        }
    } else {
        qWarning() << "[ExtensionMarketplaceManager] Failed to write temp file:" << tempPath;
        emit installationError(extensionId, "Failed to write temporary file");
    }
...
void ExtensionMarketplaceManager::onUpdateCheckReplyFinished() {
    ...
    if (doc.isObject()) {
        ...
        if (!extensionId.isEmpty() && m_installedExtensions.contains(extensionId)) {
            ExtensionInfo& installed = m_installedExtensions[extensionId];
            if (latestVersion != installed.version) {
                ...
                emit updateAvailable(extensionId, latestVersion);
            }
        }
    }
