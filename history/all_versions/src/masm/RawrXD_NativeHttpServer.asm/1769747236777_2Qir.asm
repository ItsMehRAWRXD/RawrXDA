...
WorkerThreadProc PROC
    ...
    mov r15, rcx        ; Server context (use r15 as it is non-volatile and stable)
...
@@request_loop:
    ...
    ; Send HTTP response
    ; HTTP_RESPONSE is large (around 600 bytes). Let's allocate on stack.
    sub rsp, 1024        ; Plenty of space for response + data chunk
    mov rdi, rsp
    add rdi, 128         ; Start of HTTP_RESPONSE struct (slot for stack args below)
    ...
    ; Set reason string based on status
    cmp r14w, 200
    jne @@not_200
    lea rax, g_szOK
    jmp @@set_reason
...
@@set_reason:
    ...
    mov rbx, rax        ; Body length
    
    ; Setup Data Chunk (at offset 800 in our stack allocation)
    lea r8, [rsp + 800]
...
@@send_raw:
    mov rcx, [rbx].SERVER_CONTEXT.hRequestQueue ; Wait, RBX was server context!
    ; RELOAD RBX because I used it for body length!
    ; Actually, let's use another register.
    
    ; Oh, I see. I'll use R12 for body length temporarily and reload R13.
    ; No, I'll just use R15 for server context throughout the loop.
    
    ; Wait, I need to fix the server context register usage.
    ; WorkerThreadProc PROC
    ; mov rbx, rcx (Server context)
    
    mov rcx, [g_ServerContext.hRequestQueue] ; Just use the global for now or R15
    mov rdx, [r12].HTTP_REQUEST.RequestId
    xor r8d, r8d        ; Flags
    mov r9, rdi         ; pResponse
    
    ; Stack arguments for HttpSendHttpResponse
    xor rax, rax
    mov qword ptr [rsp + 32], rax ; pCachePolicy
    mov qword ptr [rsp + 40], rax ; pBytesSent
    mov qword ptr [rsp + 48], rax ; pReserved2
    mov qword ptr [rsp + 56], rax ; Reserved3
    mov qword ptr [rsp + 64], rax ; pOverlapped
    mov qword ptr [rsp + 72], rax ; pLogData
