...
HTTP_INITIALIZE_SERVER          equ 00000002h
HTTP_INITIALIZE_CONFIG          equ 00000001h
...
; HTTP_COOKED_URL structure (from http.h)
...
; HTTP_REQUEST_HEADERS simplified
...
HTTP_REQUEST STRUCT
    ...
    pUnknownVerb                QWORD ?
    pRawUrl                     QWORD ?
    ; CookedUrl fields (flattened instead of nested)
    FullUrlLength               WORD ?
    HostLength                  WORD ?
    AbsPathLength               WORD ?
    QueryStringLength           WORD ?
...
g_szLogThreadFailed     db 'RawrXD HTTP: CreateThread failed', 13, 10, 0
...
FormatErrorResponse PROC
    ...
    lea rcx, g_szErrorTemplate
    mov rdx, rsi
    mov r8d, HTTP_STATUS_INTERNAL_ERROR
...
@@request_loop:
    ...
    sub rsp, 32
    mov rcx, [rbx].SERVER_CONTEXT.hShutdownEvent
    ...
    ; HttpReceiveHttpRequest(hQueue, RequestId, Flags, pRequest, pLength, pOverlapped)
    sub rsp, 64         ; 32 (shadow) + 16 (args 5-6) + 16 (alignment)
    mov rcx, [rbx].SERVER_CONTEXT.hRequestQueue
    xor rdx, rdx        ; RequestId (0 = any)
    mov r8d, HTTP_RECEIVE_REQUEST_FLAG_COPY_BODY
    mov r9, r12         ; pHttpRequest buffer
    mov qword ptr [rsp + 32], 0 ; pBytesReceived (optional)
    mov qword ptr [rsp + 40], 0 ; pOverlapped
    ...
    ; Send HTTP response
    ; HttpSendHttpResponse(hQueue, RequestId, Flags, pResponse, ...)
    sub rsp, 512 + 64   ; Response structure + Shadow + Stack Args
    mov rdi, rsp
    add rdi, 64         ; pResponse buffer starts after stack space
    ...
    mov word ptr [rdi].HTTP_RESPONSE.StatusCode, r14w
    
    mov rcx, [rbx].SERVER_CONTEXT.hRequestQueue
    mov rdx, [r12].HTTP_REQUEST.RequestId
    mov r8d, 0          ; Flags
    mov r9, rdi         ; pHttpResponse
    mov qword ptr [rsp + 32], 0 ; pCachePolicy
    mov qword ptr [rsp + 40], 0 ; pLogData
    mov qword ptr [rsp + 48], 0 ; Reserved1
    mov qword ptr [rsp + 56], 0 ; Reserved2
...
HttpServer_Initialize PROC
    ...
    ; Create version structure on stack (2.0)
    sub rsp, 48         ; 32 (shadow) + 16 (structure)
    mov word ptr [rsp + 32], 2       ; MajorVersion (HTTP 2.0)
    mov word ptr [rsp + 34], 0       ; MinorVersion
    
    lea rcx, [rsp + 32] ; pVersion
    mov edx, HTTP_INITIALIZE_SERVER
    call HttpInitialize
    add rsp, 48
    test eax, eax
    jnz @@init_failed_log
    
    ; Create request queue (Version 2)
    sub rsp, 32
    xor rcx, rcx        ; pVersion = NULL (use default)
    xor edx, edx        ; pName = NULL
    xor r8, r8          ; pSecurityAttributes = NULL
    xor r9, r9          ; Flags = 0
    lea rax, [g_ServerContext.hRequestQueue]
    sub rsp, 32         ; Extra shadow for HttpCreateRequestQueue
    push rax            ; Argument 5: pRequestQueueHandle (on stack)
    call HttpCreateRequestQueue
    add rsp, 40         ; Clean up extra shadow + arg5
    add rsp, 32
    test eax, eax
    ...
    ; Convert port 23959 to wide string "23959"
    mov word ptr [rcx+18], '2'
    mov word ptr [rcx+20], '3'
    mov word ptr [rcx+22], '9'
    mov word ptr [rcx+24], '5'
...
@@init_failed_log:
    mov r8d, eax        ; Save error code (arg 3 for wsprintfA)
    mov r9d, eax        ; Also save for return
    sub rsp, 128
