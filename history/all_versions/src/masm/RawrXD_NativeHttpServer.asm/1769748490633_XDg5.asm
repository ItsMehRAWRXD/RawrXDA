...
HandleHttpRequest PROC
    ...
    ; Use raw URL path directly (ASCII)
...
@@bad_request:
    lea rcx, g_szErrInvalidRequest
    mov rdx, rdi
    mov r8d, MAX_RESPONSE_SIZE
    call FormatErrorResponse
    mov eax, HTTP_STATUS_BAD_REQUEST
    jmp @@cleanup
    
@@verb_ok:
    ; Route to handler based on path
    mov rcx, r13        ; Raw URL pointer
    movzx edx, word ptr [rsi].HTTP_REQUEST.RawUrlLength
...
; ---------------------------------------------------------------------------
; CompareRawUrlPath - Compare raw URL (ASCII) with ASCII endpoint
; RCX = pRawUrl, EDX = RawUrlLength (bytes), R8 = endpoint ASCII
; Returns: EAX = 0 if match, 1 if no match
; ---------------------------------------------------------------------------
CompareRawUrlPath PROC
    push rbx
    push rsi
    push rdi
    
    mov rsi, rcx        ; Raw URL
    mov edi, edx        ; Raw URL length (bytes)
    mov rbx, r8         ; Endpoint ASCII
    
    xor ecx, ecx        ; Path length counter
@@find_end:
    cmp ecx, edi
    jge @@length_found
