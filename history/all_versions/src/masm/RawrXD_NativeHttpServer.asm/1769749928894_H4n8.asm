...
HttpServer_Initialize PROC
    ...
    mov word ptr [rcx+16], ':'
        ...
        ; Convert dynamic port number (in r12d) to wide string
        ; Use wsprintfW to format: L"http://+:PORT/"
        sub rsp, 32
        lea rax, [rsp+32+128+16]  ; Temp buffer for sprintf
        push rax
        lea rdx, L"http://+:%u/"
        mov r8d, r12d             ; Port number
        mov rcx, rax              ; Dest
        call wsprintfW
        pop rax
        add rsp, 32
    
        ; Copy formatted string back to rcx buffer as wide char
        mov rsi, rax              ; Source (formatted string)
        lea rdi, [rsp]            ; Dest
    @@copy_url:
        lodsw                     ; Load 16-bit char
        stosw                     ; Store 16-bit char
        test ax, ax
        jnz @@copy_url
