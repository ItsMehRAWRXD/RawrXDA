...
HandleHttpRequest PROC
    ...
    mov ax, word ptr [rsi + 24]     ; Verb at offset 24
    movzx eax, ax
    push rax
    push rsi
    push rdi
    ...
    ; Get Verb (offset 24 in HTTP_REQUEST for x64)
...
@@verb_ok:
    ; Get RawUrl pointer (offset 40 in HTTP_REQUEST for x64)
    mov rcx, [rsi + 40]     ; pRawUrl (wide string)
    test rcx, rcx
    jz @@not_found
    ...
    mov rdx, rcx        ; Source = pRawUrl (wide)
    mov rcx, r12        ; Dest = ASCII buffer
    call WideToAscii
