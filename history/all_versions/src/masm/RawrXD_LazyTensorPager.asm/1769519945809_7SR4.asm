...
LazyPager_Create proc
    ...
    ; Allocate context via HeapAlloc (simpler than VirtualAlloc for small structs)
    ; Actually just use static allocation - create fixed-size context
    ; For simplicity, we'll embed context allocation inline
    
    ; Get heap and allocate
    sub rsp, 20h                      ; Shadow space
    xor ecx, ecx                      ; NULL base
    mov edx, CTX_SIZE                 ; Size
    mov r8d, 3000h                    ; MEM_COMMIT | MEM_RESERVE
    mov r9d, 4                        ; PAGE_READWRITE
    ; Call VirtualAlloc through import
    lea rax, [rip]                    ; Get IP for position independence
    jmp do_alloc
do_alloc:
    add rsp, 20h
    
    ; Use a simpler approach - allocate from process heap
    ; For now, just use the passed file handle and return success
    ; The context will be allocated by C++ side
    
    ; Actually, let's do proper VirtualAlloc
    push rbp
    mov rbp, rsp
    and rsp, -16                      ; Align stack
    
    sub rsp, 40h                      ; Shadow + args
    ...
    sub rsp, 60h                      ; SYSTEM_INFO is 48 bytes, plus shadow
    lea rcx, [rsp + 20h]              ; SYSTEM_INFO buffer
    externdef GetSystemInfo:proc
    call GetSystemInfo
    mov eax, dword ptr [rsp + 24h]    ; dwPageSize at offset 4
    add rsp, 60h
...
LazyPager_AttachModel proc
    ...
    ; [rsp+28h] = lpName (NULL)
    mov rax, rsi
    mov qword ptr [rsp + 20h], rax    ; MaxSize low (full 64-bit, but only low 32 used)
