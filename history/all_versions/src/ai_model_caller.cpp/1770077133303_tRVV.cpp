    ...
    int numCompletions) 
{
    ...
    try {
        ...
        for (const auto& compText : completions) {
            ...
            result.description = getCompletionDescription(compText, fileType);
...
std::string CallNativeHost(const std::string& prompt) {
    // Try to connect to existing named pipe from CLI Backend
    HANDLE hPipe = CreateFile(L"\\\\.\\pipe\\RawrXD_CLI_Pipe", GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
    
    if (hPipe != INVALID_HANDLE_VALUE) {
        // We found the CLI running! Piggyback on its engine.
        std::string req = "GET /v1/chat/completions HTTP/1.1\r\n\r\n" + prompt; // Simplified Protocol
        DWORD dwWritten;
        WriteFile(hPipe, req.c_str(), req.length(), &dwWritten, NULL);
        
        char buf[4096];
        DWORD dwRead;
        if(ReadFile(hPipe, buf, 4096, &dwRead, NULL)) {
            CloseHandle(hPipe);
            return std::string(buf, dwRead);
        }
        CloseHandle(hPipe);
    }
    return ""; // Fallback to In-Process
}
...
std::string ModelCaller::callModel(const std::string& prompt, const GenerationParams& params) {
    try {
        // [FEATURE] Agentic Parameter Injection - Dynamic Policy Enforcement
        std::string finalPrompt = prompt;
        float activeTemp = params.temperature;
        int activeTokens = params.max_tokens;

        // 1. Detect Max Mode (32k Context)
        if (finalPrompt.find("SYSTEM: MAX MODE ENABLED") != std::string::npos) {
            activeTokens = 32768;
            // We keep the tag in the prompt so the model knows it has permission to be verbose
        }

        // 2. Detect Deep Thinking (Low Temp / Chain of Thought)
        if (finalPrompt.find("SYSTEM: ACTIVATING DEEP THINKING") != std::string::npos || 
            finalPrompt.find("<thinking>") != std::string::npos) {
            activeTemp = 0.2f; // Precision mode
        }

        // 3. Detect Deep Research (Web/File Search - currently mocked via prompting)
        if (finalPrompt.find("SYSTEM: ACTIVATING DEEP RESEARCH") != std::string::npos ||
            finalPrompt.find("[Deep Research Mode Active") != std::string::npos) {
            
            // Actually Scan Files if this flag is present
            std::string researchContext = "\n[AUTO-GENERATED CONTEXT]:\n";
            try {
                int count = 0;
                for(const auto& entry : std::filesystem::recursive_directory_iterator(".")) {
                    ...
                    if(entry.is_regular_file()) {
                        ...
                        if (name.find(".cpp") != std::string::npos || name.find(".h") != std::string::npos) {
                            researchContext += "File: " + name + "\n";
                        }
                    }
                }
            } catch(...) {}
            ...
            activeTemp = 0.4f; // Balanced for synthesis
            if (activeTokens < 8192) activeTokens = 8192;
        }

        // 4. Detect No Refusal (Compliance Mode)
        if (finalPrompt.find("SYSTEM: NO REFUSAL MODE") != std::string::npos) {
            // No specific param change, but logic could go here
        }
        ...
        if (modelType == "local" || modelType == "gguf") {
            ...
            auto outputTokens = engine->Generate(inputTokens, activeTokens);
            return engine->Detokenize(outputTokens);
             ...
             // Inject params into Ollama JSON
             std::string body = "{\"model\": \"" + modelPath + "\", \"prompt\": \"" + safePrompt + "\", \"stream\": false, \"options\": {\"num_predict\": " + std::to_string(activeTokens) + ", \"temperature\": " + std::to_string(activeTemp) + "}}";
