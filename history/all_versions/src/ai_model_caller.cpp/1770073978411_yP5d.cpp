...
std::string ModelCaller::callModel(const std::string& prompt, const GenerationParams& params) {
    try {
        // [FEATURE] Agentic Parameter Injection
        bool isMaxMode = (params.max_tokens > 4096);
        bool isThinking = (prompt.find("<thinking>") != std::string::npos || prompt.find("step-by-step") != std::string::npos);
        float activeTemp = isThinking ? 0.3f : params.temperature;
