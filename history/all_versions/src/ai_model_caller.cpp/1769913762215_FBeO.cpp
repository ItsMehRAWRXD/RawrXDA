...
std::string CallNativeHost(const std::string& prompt) {
    ...
    // 2. Send INFER command
    // Protocol: "INFER" (Wait) -> Response
    // Current NativeHost only supports simple tokens, but let's try sending the prompt if we upgrade it.
    // NativeHost currently expects "INFER" command exactly.
    
    std::string cmd = "INFER";
    if (!WriteFile(hPipe, cmd.c_str(), cmd.length(), &bytesWritten, NULL)) {
        CloseHandle(hPipe);
        return "";
    }
    ...
    if (ReadFile(hPipe, buffer, sizeof(buffer)-1, &bytesRead, NULL)) {
        ...
        // NativeHost returns "{"Status":"OK"}" currently.
        // If we want real text, NativeHost needs update.
        // But the user asked to replace "Simulated".
        // Calling the REAL backend (even if it returns OK) is better than "Hello World".
