            ...
            try {
                ...
                if (start != std::string::npos && end != std::string::npos) {
                    ...
                    for (const auto& item : j) {
                        Diagnostic d;
                        d.message = item.value("message", "Unknown error");
                        d.line = item.value("line", 0);
                        d.column = item.value("column", 0);
                        d.severity = item.value("severity", "info");
                        d.code_action = item.value("code_action", "");
                        results.push_back(d);
                    }
                }
            } catch (const std::exception& e) {
                // Fallback: simplified parsing
                 ...
                 while (std::getline(stream, line)) {
                     if (line.find("error") != std::string::npos || line.find("warning") != std::string::npos) {
                         Diagnostic d;
                         d.message = line;
                         d.severity = (line.find("error") != std::string::npos) ? "error" : "warning";
                         results.push_back(d); 
                     }
                 }
            }
...
private:
    ...
    static std::string callModel(const std::string& prompt, const GenerationParams& params) {
        try {
            ...
            // Option 2: Ollama API (via HTTP/Curl)
            else if (modelType == "ollama") {
                // Construct JSON body
                std::string cmd = "curl -s -X POST http://localhost:11434/api/generate -d \"{ \\\"model\\\": \\\"" + modelPath + "\\\", \\\"prompt\\\": \\\"" + prompt + "\\\", \\\"stream\\\": false }\"";
                std::string response = exec(cmd.c_str());
                // Simple JSON parsing to extract "response" field if possible, or return raw
                // Assuming response is like {"model":..., "response":"content", ...}
                size_t respPos = response.find("\"response\":\"");
                if (respPos != std::string::npos) {
                    respPos += 12;
                    size_t endPos = response.find("\",", respPos);
                    if (endPos != std::string::npos) {
                        return response.substr(respPos, endPos - respPos);
                    }
                }
                if (response.empty()) return "// Error: Ollama connection failed.";
                return response;
            }
...
Code:
    ...
    static std::string buildDiagnosticsPrompt(
        ...
        prompt += "Provide the output as a JSON array of objects with keys: message, line, column, severity, code_action.\n";
        prompt += "Example: [{\"message\": \"Syntax error\", \"line\": 10, \"column\": 5, \"severity\": \"error\", \"code_action\": \"Fix syntax\"}]\n\n";
        prompt += "Code:\n";
        prompt += code;
        prompt += "\n\nJSON Output:\n";
        return prompt;
    ...
    static std::string extractCodeBlock(const std::string& response) {
        // Find code blocks wrapped in ``` markers
        size_t start = response.find("```");
        if (start != std::string::npos) {
            start += 3;
            // Skip optional language identifier (e.g., "cpp\n")
            size_t nextLine = response.find('\n', start);
            if (nextLine != std::string::npos && nextLine < response.find("```", start)) {
                start = nextLine + 1;
            }
...
eCodeSyntax(const std::string& code) {
    static std::vector<std::string> extractAssertions(const std::string& test) {
        ...
        while ((pos = test.find("ASSERT_", pos)) != std::string::npos) {
            ...
            if (end != std::string::npos) {
                assertions.push_back(test.substr(pos, end - pos));
                pos = end + 1;
            } else break;
        }
        return assertions;
    ...
    static bool validateCodeSyntax(const std::string& code) {
        // Implement real syntax validation (brace matching)
        int braces = 0;
        int parens = 0;
        int brackets = 0;
        ...
        return braces == 0 && parens == 0 && brackets == 0 && !code.empty();
    }
