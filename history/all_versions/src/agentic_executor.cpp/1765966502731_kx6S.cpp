...
void AgenticExecutor::clearMemory()
{
    ...
    if (m_memoryEnabled) {
        // Clear any persistent storage if implemented
...
void AgenticExecutor::removeMemoryItem(const QString& key)
{
    ...
    // No persistent storage to delete from
    enforceMemoryLimit();
}
...
void AgenticExecutor::loadMemorySettings()
{
    ...
    m_memoryEnabled = m_settingsManager->getValue("memory/enabled", true).toBool();
    m_memoryLimitBytes = m_settingsManager->getValue("memory/limitBytes", 100 * 1024 * 1024).toLongLong();
...
{
    // Simple in-memory implementation - no disk persistence
    qDebug() << "[AgenticExecutor] Memory loaded (in-memory only)";
}
...
void AgenticExecutor::persistMemoryToDisk()
{
    ...
    // Simple in-memory implementation - no disk persistence
    qDebug() << "[AgenticExecutor] Memory persisted (in-memory only)";
}
