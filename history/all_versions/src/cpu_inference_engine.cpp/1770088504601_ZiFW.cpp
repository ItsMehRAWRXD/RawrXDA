...
void CPUInferenceEngine::InitKVCache() {
    ...
    // Explicit Native Memory Plugin usage for large contexts
    size_t layer_elements = (size_t)m_contextSize * m_embeddingDim;
    
    std::cout << "[Native Memory] Allocating KV Cache for " << m_contextSize << " tokens..." << std::endl;
    
    for (auto& layer : m_kv_cache) {
        // Using standard vector resize, which handles allocation
        // But for HUGE contexts (1M), we might want the plugin logic?
        // Vector max_size is usually sufficient on 64-bit, but let's be aligned.
        ...
        try {
            ...
            std::cerr << "[Native Memory] FATAL: Failed to allocate KV cache for context " << m_contextSize << std::endl;
    ...
    std::cout << "[Native Memory] Allocation Complete." << std::endl;
    m_currentPos = 0;
}
...
void CPUInferenceEngine::SetMaxMode(bool enabled) { 
    ...
    // Max Mode optimizations
    if (enabled) {
        // Increase thread count for maximum performance
        SetThreadCount(std::thread::hardware_concurrency());
        
        // Enable AVX2 optimizations if available
        CPUOps::EnableAVX2(true);
        CPUOps::EnableMultiThreading(true);
        
        // Pre-allocate larger buffers for faster inference
        if (m_contextLimit < 32768) {
            SetContextLimit(32768); // Default to 32k in max mode
        }
    }
}
...
void CPUInferenceEngine::SetDeepThinking(bool enabled) { 
    ...
    // Deep Thinking optimizations
    if (enabled) {
        // Increase context window for deeper reasoning
        if (m_contextLimit < 65536) {
            SetContextLimit(65536); // 64k context for deep thinking
        }
        
        // Enable more thorough attention calculations
        // This would involve modifying the attention mechanism in practice
    }
}
...
void CPUInferenceEngine::SetDeepResearch(bool enabled) { 
    ...
    if (enabled) {
        // Maximum context for research tasks
        if (m_contextLimit < 1048576) {
            SetContextLimit(1048576); // 1M context for research
        }
        
        // Enable extended memory plugins for research
        for (auto& plugin : m_memoryPlugins) {
