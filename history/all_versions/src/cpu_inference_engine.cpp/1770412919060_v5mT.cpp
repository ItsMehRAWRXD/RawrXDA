...
namespace CPUOps {
    ...
    void Softmax(float* data, int size) {
        ...
        for (; i + 7 < size; i += 8) {
            ...
            // Use std::exp per-element for accuracy (fast_exp in inference_kernels)
            alignas(32) float tmp[8], res[8];
            _mm256_store_ps(tmp, _mm256_sub_ps(v, vmax_bc));
            for (int j = 0; j < 8; j++) res[j] = std::exp(tmp[j]);
            __m256 e = _mm256_load_ps(res);
    ...
    // AVX2 SiLU activation: x * sigmoid(x)
    void SiLU(float* data, int size) {
        ...
        for (; i + 7 < size; i += 8) {
            ...
            // exp(-x) via scalar for accuracy
            alignas(32) float tmp[8], res[8];
            _mm256_store_ps(tmp, neg_x);
            for (int j = 0; j < 8; j++) res[j] = std::exp(tmp[j]);
            __m256 vexp = _mm256_load_ps(res);
    ...
    // AVX2 DequantizeQ4_0 — process 32 weights per block, vectorized inner loop
