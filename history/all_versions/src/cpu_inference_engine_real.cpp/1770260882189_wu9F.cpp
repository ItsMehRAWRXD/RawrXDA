...
bool CPUInferenceEngine::LoadModel(const std::string& path) {
    ...
    int hidden_dim = meta.embedding_dim * 4; // Fallback if feed_forward_length not available

    for (int i = 0; i < m_numLayers; i++) {
        LayerWeights layer;
        // Initialize layer weights here
        std::string pre = "blk." + std::to_string(i) + ".";
        
        // Load layer weights
        float* attn_norm = (float*)LoadTensorData(m_loader.get(), m_weight_store, pre + "attn_norm.weight");
        void* wq = LoadTensorData(m_loader.get(), m_weight_store, pre + "attn_q.weight");
        void* wk = LoadTensorData(m_loader.get(), m_weight_store, pre + "attn_k.weight");
        void* wv = LoadTensorData(m_loader.get(), m_weight_store, pre + "attn_v.weight");
        void* wo = LoadTensorData(m_loader.get(), m_weight_store, pre + "attn_output.weight");
        
        float* ffn_norm = (float*)LoadTensorData(m_loader.get(), m_weight_store, pre + "ffn_norm.weight");
        void* w1 = LoadTensorData(m_loader.get(), m_weight_store, pre + "ffn_gate.weight");
        void* w3 = LoadTensorData(m_loader.get(), m_weight_store, pre + "ffn_up.weight");
        void* w2 = LoadTensorData(m_loader.get(), m_weight_store, pre + "ffn_down.weight");
        
        // Store layer weights
        m_transformerLayers.emplace_back();
...
void CPUInferenceEngine::InitKVCache() {
        // layer.cache_pos = 0; // Commented out for now
...
std::vector<float> CPUInferenceEngine::Eval(const std::vector<int32_t>& input_tokens) {
    ...
    // Layers
    for (auto& layer : m_layers) {
        // layer.forward(x.data(), pos, 1); // Commented out for now
    }
...
std::vector<int32_t> CPUInferenceEngine::Tokenize(const std::string& text) {
    ...
    // Simple greedy tokenizer (matches logic in production file)
    size_t i = 0;
    while (i < text.size()) {
        int best_id = -1;
        size_t best_len = 0;
...
void CPUInferenceEngine::SetMaxMode(bool enabled) { m_maxMode = enabled; }
    // Configure sampler
    // m_sampler.temp = temperature; // Commented out for now
    // m_sampler.top_p = top_p; // Commented out for nowsearch = enabled; }
void CPUInferenceEngine::ConfigureSampling(float temperature, float top_p, int top_k, float repeat_penalty) {
    // Forward to Sampler if available, or store in config
    m_sampler.SetTemp(temperature);
    m_sampler.SetTopP(top_p); // Assuming Sampler class has these
}
...
// Math Wrappers - Forwarding to Inference Kernels where possible
void CPUInferenceEngine::MatMul(const float* A, const float* B, float* C, int m, int n, int k) {
    // Assuming F32 for generic wrapper
    // InferenceKernels might expect AVX specific layout, but here we can fallback to generic
    // Or call InferenceKernels::matmul_f32 (if exists).
    // Let's implement generic loop for correctness if kernel not matching
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            float sum = 0.0f;
            for (int l = 0; l < k; l++) {
                sum += A[i*k + l] * B[j*k + l]; // NOTE: B transposed? Implementation dependent.
            }
            C[i*n + j] = sum;
        }
    }
...
void CPUInferenceEngine::RMSNorm(float* data, int size, float epsilon) {
    InferenceKernels::rmsnorm_avx512(data, data, nullptr, size, epsilon); // nullptr weight means just norm
}
void CPUInferenceEngine::LayerNorm(float* data, int size, float epsilon) { 
    // Simplified to RMSNorm
