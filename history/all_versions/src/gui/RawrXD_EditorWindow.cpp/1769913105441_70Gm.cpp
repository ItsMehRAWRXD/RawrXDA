    ...
    if (cursorPos.x != anchorPos.x || cursorPos.y != anchorPos.y) {
         ...
         if (start.y == end.y) {
             ...
             if (start.y < lines.size()) {
                 std::string& line = lines[start.y];
                 if (end.x <= line.length()) {
             ...
             if (end.y < lines.size() && end.x < lines[end.y].size()) {
                 suffix = lines[end.y].substr(end.x);
             }
             
             if (start.y < lines.size()) {
                 lines[start.y] = lines[start.y].substr(0, start.x) + suffix;
             }
             
             // Remove intermediate lines
             auto it = lines.begin() + start.y + 1;
             auto itEnd = lines.begin() + end.y + 1;
             if (it < lines.end() && itEnd <= lines.end()) {
                 lines.erase(it, itEnd);
             }
         }
