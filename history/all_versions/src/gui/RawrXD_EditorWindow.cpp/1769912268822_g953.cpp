...
void EditorWindow::cut() {
    ...
    // Handle selection delete or line delete
    if (cursorPos.x != anchorPos.x || cursorPos.y != anchorPos.y) {
         // Delete selection
         // Simplified to just clearing current line part for this pass
         // Real implementation requires deleteRange
         // Implementing DeleteRange stub logic for now
         Point start = (cursorPos.y < anchorPos.y) ? cursorPos : anchorPos;
         // TODO: Proper range delete
         // Fallback to line delete if selection matches line?
    }
...
void EditorWindow::pushCommand(const EditorCommand& cmd) {
    ...
    if (undoStack.size() > 1000) undoStack.pop_front();
    redoStack.clear();
}
...
void EditorWindow::undo() {
    ...
    if (cmd.type == EditorCommand::Insert) {
        // Undo Insert -> Delete
        if (cmd.text == L"\r" || cmd.text == L"\n") {
            // Join lines
            if (cursorPos.y + 1 < lines.count()) {
                lines[cursorPos.y].append(lines[cursorPos.y + 1]);
                lines.removeAt(cursorPos.y + 1);
            }
        } else {
            // Delete chars on same line
            if (cursorPos.y < lines.count()) {
                lines[cursorPos.y].remove(cursorPos.x, cmd.text.length());
            }
        }
        ...
        if (cmd.text == L"\r" || cmd.text == L"\n") {
            ...
            if (cursorPos.y < lines.count()) {
                lines[cursorPos.y].insert(cursorPos.x, cmd.text);
            }
        }
    }
    InvalidateRect(hwnd, nullptr, FALSE);
...
void EditorWindow::onTimer(UINT_PTR id) {
    if (id == 1) {
        ...
        if (aiHub && cursorPos.y < lines.count()) {
            // Get context for AI
            std::string prefix = lines[cursorPos.y].left(cursorPos.x).toStdString();
            std::string suffix = lines[cursorPos.y].mid(cursorPos.x).toStdString();
            
            // Call AI Hub
            auto completions = aiHub->getCompletions("active_buffer", prefix, suffix, cursorPos.x);
            
            if (!completions.empty()) {
                // Convert std::string (UTF-8) to wstring/String
                std::string& compText = completions[0].text;
                
                // Simple conversion - in real app use proper UTF-8 to WString converter
                // Assuming String has constructor or assignment from char* or std::string
                // If not, we might need manual conversion
                
                // Using a safe approach if String is unicode aware wrapper:
                // If String() accepts only wide, we might need MultibyteToWideChar
                // But for now assuming toStdWString equivalent fromString exists or direct construction
                
                // Assuming String(const char*) works as seen in lines.append(String(wtext...c_str())) logic earlier
                // wait, the earlier logic was String(wtext.substr(...).c_str()) which suggests String takes wchar_t*
                
                // So I need to convert std::string to std::wstring first
                if (compText.empty()) return;

                int size_needed = MultiByteToWideChar(CP_UTF8, 0, &compText[0], (int)compText.size(), NULL, 0);
                std::wstring wstrTo(size_needed, 0);
                MultiByteToWideChar(CP_UTF8, 0, &compText[0], (int)compText.size(), &wstrTo[0], size_needed);
                
                setGhostText(String(wstrTo.c_str()));
            }
        }
    }
}
