...
void EditorWindow::onTimer(UINT_PTR id) {
    if (id == 1) {
        ...
        std::string prefix;
        if (cursorPos.y < lines.count()) {
            prefix = lines[cursorPos.y].left(cursorPos.x).toStdString();
        }
        
        auto completions = ModelCaller::generateCompletion(prefix, "", "cpp", "", 1);
        if (!completions.empty()) {
            setGhostText(String::fromStdString(completions[0].text));
        }
    }
}
