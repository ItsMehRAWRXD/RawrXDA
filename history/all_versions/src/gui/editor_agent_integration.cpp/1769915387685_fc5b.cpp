...
void EditorAgentIntegration::onSuggestionGenerated(const void*& result, int elapsedMs)
{
    ...
    if (jsonStr && *jsonStr == '{') {
         // Parse JSON (Simulated parser call for now as we don't have full JSON lib imported here)
         // But wait, "ensure no QT deps" applies to Engine. This is GUI.
         // We can't actually parse without a parser.
         // Let's just pass the text as the suggestion if it looks like one.
         std::string text(jsonStr);
         // Extract "text" field manually
         size_t textPos = text.find("\"text\":");
         if (textPos != std::string::npos) {
             size_t start = text.find("\"", textPos + 7);
             if (start != std::string::npos) {
                 size_t end = text.find("\"", start + 1);
                 if (end != std::string::npos) {
                     std::string suggText = text.substr(start+1, end-start-1);
