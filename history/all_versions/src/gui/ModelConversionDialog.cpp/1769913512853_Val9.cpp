...
{
    // Explicit Logic: Real Regex Parsing
    static std::regex chunkRegex(R"((\d+)\s*/\s*(\d+))");
    std::smatch chunkMatch;
    if (std::regex_search(output, chunkMatch, chunkRegex)) {
        try {
            int current = std::stoi(chunkMatch[1].str());
            int total = std::stoi(chunkMatch[2].str());
    ...
    // Explicit Logic: Case insensitive check helper
