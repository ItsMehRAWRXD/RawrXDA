...
int main() {
    ...
    try {
        ...
        auto completions = hub.getCompletions("test.cpp", "void main() {", "", 13);
        std::cout << "[IntegrationTest] Completion Response (stub): " << (completions.empty() ? "None" : completions[0].text) << std::endl;
