...
int main() {
    ...
    try {
        ...
        hub.getCompletions("void main() {", [](const std::vector<std::string>& completions) {
            std::cout << "[IntegrationTest] Completion Response (stub): " << (completions.empty() ? "None" : completions[0]) << std::endl;
        });
