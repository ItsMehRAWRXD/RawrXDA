...
@layer_loop:
    ...
    ; 1b. Projections (Q, K, V)
    ; Q = Norm * Wq
    lea rcx, [rsi + 4096*4]   ; Input (Assuming Norm was done to State+4K*4)
    lea rdx, [rdi + 4096*4]   ; Wq
    lea r8,  [rsi + 8192*4]   ; Q Out
    mov r9, 4096
    call VecMatMul_F32_AVX512
    
    ; K
    lea rcx, [rsi + 4096*4]
    lea rdx, [rdi + 4096*4 + 4096*4096*4] ; Wk
    lea r8,  [rsi + 12288*4]  ; K Out
    mov r9, 4096
    call VecMatMul_F32_AVX512
    
    ; V
    lea rcx, [rsi + 4096*4]
    lea rdx, [rdi + 4096*4 + 2*4096*4096*4] ; Wv
    lea r8,  [rsi + 16384*4]  ; V Out
    mov r9, 4096
    call VecMatMul_F32_AVX512
    
    ; 1c. Attention Forward
    lea rcx, [rsi + 8192*4]  ; Q
    lea rdx, [rsi + 12288*4] ; K
    lea r8,  [rsi + 16384*4] ; V
    lea r9,  [rsi + 20480*4] ; Dst
    call Attention_Forward_GQA
    
    ; 1d. Output Projection
    lea rcx, [rsi + 20480*4]
    lea rdx, [rdi + 4096*4 + 3*4096*4096*4] ; Wo
    lea r8,  [rsi + 4096*4] ; Overwrite Scratch with Result
    mov r9, 4096
    call VecMatMul_F32_AVX512
    
    ; 1e. Residual Connection
    ; State += Result
    xor rax, rax
    mov r10, 4096
@res_loop_1:
    ...
    jge @res_loop_1_done
    movss xmm0, REAL4 PTR [rsi + rax*4]
    addss xmm0, REAL4 PTR [rsi + 4096*4 + rax*4]
    movss REAL4 PTR [rsi + rax*4], xmm0
    inc rax
    jmp @res_loop_1
@res_loop_1_done:
    ...
    add rdi, 64 * 1024 * 1024 ; Next layer weights (Explicit ~64MB)
    inc r14
    jmp @layer_loop
