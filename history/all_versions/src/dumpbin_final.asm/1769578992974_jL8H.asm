...
OpcodeTable             OpcodeEntry 256 dup(<0,0,0>)
...
@@string_loop:
    ...
    invoke wsprintf, addr szScratch, addr szFmtYaraString,
        [esi].YARA_STRING._offset, [esi].YARA_STRING._length
    invoke WriteConsole, hStdOut, addr szScratch, eax, NULL, NULL
    popad
...
InitYARARules PROC
    ...
    mov [pRule].YARA_RULE.strings[0]._length, 2
    ...
    mov [pRule].YARA_RULE.strings[0]._length, 4
    mov [pRule].YARA_RULE.conditionType, YARA_COND_ANY
    ...
    mov [pRule].YARA_RULE.strings[0]._length, 6
    ...
    mov [pRule].YARA_RULE.strings[1]._length, 4
    ...
    mov [pRule].YARA_RULE.strings[2]._length, 3
    mov [pRule].YARA_RULE.conditionType, YARA_COND_ANY
...
@@search_loop:
    mov eax, j
    add eax, [edi].YARA_STRING._length
    cmp eax, dwFileSize
    jg @@next_string
    ...
    push esi
    push edi
    mov ecx, [edi].YARA_STRING._length
    mov esi, pMapped
    add esi, j
    ...
    inc dwMatchCount
    mov eax, pString
    mov [eax].YARA_STRING._offset, j
    mov ecx, [eax].YARA_STRING._length
    mov [eax].YARA_STRING._length, ecx
    jmp @@next_string
