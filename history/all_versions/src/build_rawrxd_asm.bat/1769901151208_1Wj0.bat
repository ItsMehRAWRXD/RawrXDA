...
if exist "C:\VS2022Enterprise\VC\Auxiliary\Build\vcvars64.bat" (
    :: Custom path found, attempt to use it
    set "VS_PATH=C:\VS2022Enterprise\VC\Auxiliary\Build\vcvars64.bat"
)
...
:: 2a. Fallback: Manual Path Search (if vcvars failed)
where ml64.exe >nul 2>&1
if %errorlevel% neq 0 (
    echo [Build] ML64 not in path, searching for MSVC tools...
    
    :: Search in C:\VS2022Enterprise
    if exist "C:\VS2022Enterprise\VC\Tools\MSVC" (
         for /d %%i in ("C:\VS2022Enterprise\VC\Tools\MSVC\*") do (
             if exist "%%i\bin\Hostx64\x64\ml64.exe" (
                 echo [Build] Found tools at: %%i
                 set "PATH=%%i\bin\Hostx64\x64;!PATH!"
                 goto :EnvCheck
             )
         )
    )
    ...
    if exist "C:\Program Files\Microsoft Visual Studio\2022\Enterprise\VC\Tools\MSVC" (
         for /d %%i in ("C:\Program Files\Microsoft Visual Studio\2022\Enterprise\VC\Tools\MSVC\*") do (
             if exist "%%i\bin\Hostx64\x64\ml64.exe" (
                 set "PATH=%%i\bin\Hostx64\x64;!PATH!"
                 goto :EnvCheck
             )
...
where ml64.exe >nul 2>&1
if %errorlevel% neq 0 (
    echo [Error] ml64.exe not found in PATH and auto-detection failed.
    echo [Info] Please run 'vcvars64.bat' manually or check your installation.
    exit /b 1
)
if %errorlevel% neq 0 (
