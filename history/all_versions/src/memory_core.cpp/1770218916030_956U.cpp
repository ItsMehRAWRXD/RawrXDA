...
std::string MemoryCore::GetStatsString() {
    std::lock_guard<std::mutex> lock(mem_mutex_);
    std::ostringstream oss;
    
    oss << "=== Memory Core Statistics ===\n";
    oss << "Status: " << (is_allocated_ ? "Active" : "Inactive") << "\n";
    oss << "Tier: " << GetTierName() << "\n";
    oss << "Capacity: " << max_tokens_ << " tokens\n";
    oss << "Usage: " << current_tokens_ << " tokens\n";
    oss << "Utilization: " << std::fixed << std::setprecision(2) 
        << GetUtilizationPercentage() << "%\n";
    oss << "Buffer Size: " << buffer_.size() << " blocks\n";
    oss << "Head Index: " << head_index_ << "\n";
