    ...
    extern "C" {
        ...
        void ggml_set_zero(ggml_tensor* tensor) {
            if(tensor && tensor->data) {
                memset(tensor->data, 0, tensor->_storage_f32.size() * sizeof(float));
            }
        }
        
        void ggml_set_name(ggml_tensor* tensor, const char* name) {
            if(tensor) strncpy(tensor->name, name, 63);
        }
        
        ggml_cgraph* ggml_new_graph(ggml_context* ctx) {
            return new ggml_cgraph(); // Leak in this simple stub/impl, but functional
        }
    }
#endif
