...
; IOCP Functions for high-performance async I/O
EXTRN   CreateIoCompletionPort:PROC
EXTRN   GetQueuedCompletionStatus:PROC
EXTRN   WSARecv:PROC
EXTRN   WSASend:PROC
...
IOCP_THREAD_COUNT         EQU 4           ; Number of IOCP worker threads
WSA_IO_PENDING            EQU 997         ; WSA_IO_PENDING error code

; -----------------------------------------------------------------------------
; IOCP Overlapped Structure (20 bytes + user data)
; -----------------------------------------------------------------------------
OVERLAPPED_Structure      EQU 0           ; 20 bytes (OVERLAPPED)
OVERLAPPED_UserData       EQU 20          ; 8 bytes (pointer to context)
OVERLAPPED_Buffer         EQU 28          ; 8 bytes (WSABUF.lpBuffer)
OVERLAPPED_Length         EQU 36          ; 4 bytes (WSABUF.dwLength)
OVERLAPPED_Operation      EQU 40          ; 4 bytes (0=recv, 1=send)
OVERLAPPED_Socket         EQU 44          ; 8 bytes (socket handle)
SIZEOF_IOCP_OVERLAPPED    EQU 52
...
g_IOCP_Port               DQ    ?          ; IOCP completion port handle
g_IOCP_Threads            DQ IOCP_THREAD_COUNT DUP (?)  ; Thread handles
g_IOCP_OverlappedPool     DQ    ?          ; Pool of overlapped structures
...
RelayEngine_Init PROC FRAME
    ...
    ; Allocate overlapped pool (2 overlapped per connection: recv/send)
    mov     rcx, MAX_CONCURRENT_RELAYS * 2 * SIZEOF_IOCP_OVERLAPPED
    call    AllocateLargePagesMemory
    mov     [g_IOCP_OverlappedPool], rax
    
    ; Create IOCP completion port
    xor     ecx, ecx                    ; FileHandle = INVALID_HANDLE_VALUE
    xor     edx, edx                    ; ExistingCompletionPort = NULL
    mov     r8d, IOCP_THREAD_COUNT      ; NumberOfConcurrentThreads
    xor     r9d, r9d                    ; CompletionKey = 0
    call    CreateIoCompletionPort
    mov     [g_IOCP_Port], rax
    
    ; TODO: Create IOCP worker threads
...
@@do_select:
    ...
    ; Pre-parser sniping: Scan for HTTP headers
    mov     rcx, r12                    ; buffer
    mov     edx, ebx                    ; length
    mov     r8d, 0                      ; direction (0=client->target)
    call    RelayEngine_ScanHTTPHeaders
...
@@check_target:
    ...
    mov     rcx, r12                    ; buffer
    mov     edx, ebx                    ; length
    mov     r8d, 1                      ; direction (1=target->client)
    call    RelayEngine_ScanHTTPHeaders
...
; -----------------------------------------------------------------------------
; RelayEngine_ScanHTTPHeaders — Pre-parser sniping for HTTP traffic inspection
; rcx = Buffer, edx = Length, r8d = Direction (0=client->target, 1=target->client)
; -----------------------------------------------------------------------------
PUBLIC RelayEngine_ScanHTTPHeaders
RelayEngine_ScanHTTPHeaders PROC FRAME
    push    rbx
    .pushreg rbx
    push    rsi
    .pushreg rsi
    ...
    ; Quick check for HTTP headers (starts with GET/POST/PUT/etc.)
    cmp     ebx, 4
    jb      @@exit
    
    ; Check for HTTP method
    mov     eax, [rsi]
    and     eax, 0xDFDFDFDF             ; Case insensitive
    cmp     eax, 'GET '                 ; GET 
    je      @@http_detected
    cmp     eax, 'POST'                 ; POST
    je      @@http_detected
    ...
    cmp     eax, 'HTTP'                 ; HTTP/1.1 200 OK
    je      @@http_detected
    
    jmp     @@exit
    
@@http_detected:
    ; HTTP traffic detected - could trigger sniper callbacks here
    ; For now, just log (would integrate with sniper system)
    ; lock inc QWORD PTR [httpPacketsDetected]
    
@@exit:
    pop     rsi
    pop     rbx
    ret
RelayEngine_ScanHTTPHeaders ENDP

; -----------------------------------------------------------------------------
; SEH Handler for relay exceptions
; -----------------------------------------------------------------------------
PUBLIC RelaySEHHandler
RelaySEHHandler PROC FRAME:RelayUnwind
    ; ExceptionCode in RCX, ContextRecord in RDX
    mov     rax, EXCEPTION_CONTINUE_EXECUTION
    ret
RelaySEHHandler ENDP
