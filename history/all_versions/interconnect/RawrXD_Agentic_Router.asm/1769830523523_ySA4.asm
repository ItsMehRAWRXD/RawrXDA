...
@keyword_loop:
    ...
    ; Search for keyword in input using SSE4.2 string instructions
    mov rdi, r12
    mov rsi, rcx                    ; Keyword
    call strlen                     ; Get keyword length
    ...
    ; Use PCMPESTRI for substring search (SSE4.2)
    ; Simplified: call C runtime for now
    call strstr
    test rax, rax
    setnz al
...
AgentRouter_ExecuteTask PROC FRAME
    ...
    mov rsi, rcx                    ; RSI = AgentTask* (populated with input)
    
    ; Generate task ID
    lock inc qword ptr g_TaskIdCounter
    mov [rsi].AgentTask.TaskId, rax
    
    ; Set timestamps
    call GetTickCount64
    mov [rsi].AgentTask.CreateTick, rax
    
    ; Create cancellation event (manual reset, initially unset)
    xor ecx, ecx
    xor edx, edx                    ; bManualReset = FALSE (auto-reset)
    xor r8, r8                      ; bInitialState = FALSE
    xor r9, r9
    call CreateEventA
    mov [rsi].AgentTask.hCancelEvent, rax
    
    ; Create completion event
    xor ecx, ecx
    xor edx, edx
    mov r8d, 1                      ; bInitialState = FALSE
    xor r9, r9
    call CreateEventA
    mov [rsi].AgentTask.hCompleteEvent, rax
    
    ; Launch worker thread
    lea rcx, AgentWorkerThread
    mov rdx, rsi                    ; Parameter = task pointer
    xor r8, r8
    xor r9, r9
    call CreateThread
    mov [rsi].AgentTask.WorkerThread, rax
...
; ═══════════════════════════════════════════════════════════════════════════════
; AgentWorkerThread
; Main execution pipeline: classify → select tool → execute → format result
; ═══════════════════════════════════════════════════════════════════════════════
AgentWorkerThread PROC FRAME
    push rbx
    push rsi
    
    mov rsi, rcx                    ; RSI = AgentTask*
    
    ; Update status
    mov [rsi].AgentTask.Status, TASK_STATUS_CLASSIFYING
    
    ; Step 1: Classify intent
    mov rcx, [rsi].AgentTask.RawInput
    mov edx, [rsi].AgentTask.InputLength
    call AgentRouter_ClassifyIntent
    
    mov [rsi].AgentTask.PrimaryIntent, eax
    movss [rsi].AgentTask.Confidence, xmm0
    
    ; Check if confidence too low - might need clarification
    comiss xmm0, dword ptr [rel fMinConfidenceThreshold]
    jb @low_confidence
    
    ; Step 2: Select tool
    mov [rsi].AgentTask.Status, TASK_STATUS_ROUTING
    mov ecx, [rsi].AgentTask.PrimaryIntent
    call AgentRouter_SelectTool
    
    cmp eax, -1
    je @no_tool_available
    
    mov [rsi].AgentTask.SelectedTool, eax
    
    ; Step 3: Parse parameters from input
    call ExtractToolParameters
    
    ; Step 4: Execute tool
    mov [rsi].AgentTask.Status, TASK_STATUS_EXECUTING
    call GetTickCount64
    mov [rsi].AgentTask.StartTick, rax
    
    ; Get tool handler
    mov ecx, [rsi].AgentTask.SelectedTool
    call GetToolById
    mov rbx, rax                    ; RBX = AgentTool*
    
    ; Increment running count
    lock inc [rbx].AgentTool.CurrentRunning
    
    ; Call handler with cancellation support
    mov rcx, rsi
    mov rdx, [rsi].AgentTask.hCancelEvent
    call qword ptr [rbx].AgentTool.HandlerFn
    
    ; Decrement running count
    lock dec [rbx].AgentTool.CurrentRunning
    
    ; Update tool metrics
    call GetTickCount64
    sub rax, [rsi].AgentTask.StartTick
    ; Update rolling average...
    
    mov [rsi].AgentTask.CompleteTick, rax
    
    ; Step 5: Mark complete
    mov [rsi].AgentTask.Status, TASK_STATUS_COMPLETE
    mov rcx, [rsi].AgentTask.hCompleteEvent
    call SetEvent
...
@low_confidence:
    ...
    mov [rsi].AgentTask.Status, TASK_STATUS_ERROR
    mov [rsi].AgentTask.ErrorCode, 1    ; ERR_LOW_CONFIDENCE
    jmp @complete_error
    
@no_tool_available:
    mov [rsi].AgentTask.Status, TASK_STATUS_ERROR
    mov [rsi].AgentTask.ErrorCode, 2    ; ERR_NO_TOOL
    jmp @complete_error
...
AgentRouter_CancelTask PROC FRAME
    mov rdx, [rcx].AgentTask.hCancelEvent
    test rdx, rdx
    jz @cancel_done
    ...
    mov ecx, 500                    ; 500ms timeout
    call Sleep
    
    ; Force terminate if still running
    cmp [rcx].AgentTask.Status, TASK_STATUS_EXECUTING
    jne @cancel_done
    
    mov rcx, [rcx].AgentTask.WorkerThread
    mov edx, 0                      ; Exit code
    call TerminateThread            ; Last resort
