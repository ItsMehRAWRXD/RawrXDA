...
TestOptions ParseCommandLine(int argc, wchar_t* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == L"--list" || arg == L"-l") {
            ...
            std::wcout << L"RawrXD Integration Test Runner\n";
            std::wcout << L"Usage: RawrXD_TestRunner [options]\n\n";
            std::wcout << L"Options:\n";
            std::wcout << L"  -l, --list              List all tests\n";
            std::wcout << L"  -c, --category <name>   Run only specified category\n";
            std::wcout << L"  -o, --output <file>     Write results to file\n";
            std::wcout << L"  -x, --xml               Output in JUnit XML format\n";
            std::wcout << L"  -s, --stop-on-failure   Stop on first failure\n";
            std::wcout << L"  -r, --repeat <n>        Repeat tests n times\n";
...
void ListAllTests() {
    ...
    std::wcout << L"\nAvailable Tests (" << tests.size() << L" total):\n";
    std::wcout << L"═══════════════════════════════════════════════════════════════\n";
    ...
    for (const auto& test : tests) {
        ...
        std::wcout << L"  " << std::left << std::setw(40) << test.name 
                  << L" - " << test.description << L"\n";
    }
    std::wcout << L"\n";
}
...
int wmain(int argc, wchar_t* argv[]) {
    try {
        ...
        std::wcout << L"║     Testing 24 Components + Hidden Logic + 44 Tools          ║\n";
        std::wcout << L"╚═══════════════════════════════════════════════════════════════╝\n\n";
        ...
        for (int run = 0; run < opts.repeatCount; ++run) {
            if (opts.repeatCount > 1) {
                std::wcout << L"\n=== Run " << (run + 1) << L"/" << opts.repeatCount << L" ===\n\n";
            }
        ...
        if (!opts.outputFile.empty()) {
            ...
            std::wcout << L"\nResults written to: " << opts.outputFile << L"\n";
        }
