...
;==============================================================================
RMSNorm PROC USES rbx rsi rdi r12 r13, pX:QWORD, pWeight:QWORD, n:DWORD, epsilon:REAL4
    LOCAL scale:REAL4, i:DWORD
...
@@compute_scale:
    ...
    addss xmm0, epsilon     ; + eps
    sqrtss xmm0, xmm0       ; sqrt
    movss xmm1, one_const
    divss xmm1, xmm0        ; 1/sqrt
    movss scale, xmm1
...
SoftMax PROC USES rbx rsi, pX:QWORD, n:DWORD
    LOCAL max_val:REAL4, sum:REAL4, i:DWORD
    
    ; Find max
    movss xmm0, REAL4 PTR [pX]
    mov i, 1
...
@@exp_loop:
    ...
    movss xmm0, REAL4 PTR [pX + i*4]
    subss xmm0, max_val
    
    ; Fast exp using table or approximation
    call FastExp
    
    movss REAL4 PTR [pX + i*4], xmm0
    addss xmm1, xmm0
...
FastExp PROC
    ...
    ; Clamp to [-10, 10]
    minss xmm0, ten_const
    maxss xmm0, neg_ten_const
    
    ; Scale to table index
    addss xmm0, ten_const
    mulss xmm0, exp_scale_const
...
;==============================================================================
; FORWARD PASS (Complete transformer inference)
;==============================================================================
ForwardPass PROC USES rbx rsi rdi r12 r13 r14 r15, pCtx:QWORD, token:DWORD, pos:DWORD, pLogits:QWORD
    LOCAL n_embd:DWORD, n_layer:DWORD, n_head:DWORD, n_ff:DWORD, n_rot:DWORD
    LOCAL n_head_kv:DWORD, layer:DWORD
    ...
    mov rcx, rbx
    mov edx, token
    mov r8, [rbx].ModelContext.embeddings
    call GetTokenEmbedding
    
    ; Copy to residual stream
    mov rsi, [rbx].ModelContext.embeddings
    mov rdi, [rbx].ModelContext.attn_input
    mov ecx, n_embd
    rep movsd
...
@@layer_loop:
    ...
    mov rax, [rbx].ModelContext.layer_attn_norm
    mov rdx, [rax + r8] ; Need to calculate index correctly
    imul r8, layer, 8
    mov rdx, [rax + r8]
    mov r8d, n_embd
    movsd xmm3, [rbx].ModelContext.rms_norm_eps
    cvtsd2ss xmm3, xmm3
    call RMSNorm
    ...
    mov edx, layer
    mov r8d, pos
    call ComputeAttention
    
    ; Output projection
    mov rcx, rbx
    mov edx, layer
    call AttentionOutput
    
    ; Residual
    mov rcx, [rbx].ModelContext.attn_output
    mov rdx, [rbx].ModelContext.attn_input
    mov r8d, n_embd
    call VectorAdd
    
    ; Copy to FFN input
    mov rsi, [rbx].ModelContext.attn_output
    mov rdi, [rbx].ModelContext.attn_input
    mov ecx, n_embd
    rep movsd
    ...
    ; RMSNorm
    mov rcx, [rbx].ModelContext.attn_input
    mov rax, [rbx].ModelContext.layer_ffn_norm
    imul r8, layer, 8
    mov rdx, [rax + r8]
    mov r8d, n_embd
    movsd xmm3, [rbx].ModelContext.rms_norm_eps
    ...
    ; Residual
    mov rcx, [rbx].ModelContext.ffn_down
    mov rdx, [rbx].ModelContext.attn_output
    mov r8d, n_embd
    call VectorAdd
    
    ; Copy back for next layer
    mov rsi, [rbx].ModelContext.ffn_down
    mov rdi, [rbx].ModelContext.attn_input
    mov ecx, n_embd
    rep movsd
...
@@final_norm:
    ; Final RMSNorm
    mov rcx, [rbx].ModelContext.attn_input
    mov rdx, [rbx].ModelContext.norm_final
    mov r8d, n_embd
    movsd xmm3, [rbx].ModelContext.rms_norm_eps
    cvtsd2ss xmm3, xmm3
    call RMSNorm
    ...
    mov rdx, [rbx].ModelContext.attn_input
    mov r8, pLogits
    call LMHeadProjection
    
    ret
ForwardPass ENDP

GetTokenEmbedding PROC USES rbx rsi rdi, pCtx:QWORD, token:DWORD, pOut:QWORD
    LOCAL n_embd:DWORD, pEmb:QWORD, offset:QWORD
    
    mov rbx, pCtx
    mov eax, [rbx].ModelContext.n_embd
    mov n_embd, eax
    
    mov rax, [rbx].ModelContext.tok_embeddings
    mov pEmb, rax
    
    ; Calculate offset: token * n_embd * 4
    mov eax, token
    mul n_embd
    shl rax, 2
...
ComputeQKV PROC USES rbx rsi rdi r12 r13, pCtx:QWORD, layer:DWORD
    LOCAL n_embd:DWORD, n_head:DWORD, n_head_kv:DWORD
    ...
    mov rax, [rbx].ModelContext.layer_wk
    mov r13, [rax + r8]
    
    ; Q = input @ Wq
    mov rcx, [rbx].ModelContext.attn_input
    mov rdx, r12
    mov r8, [rbx].ModelContext.attn_q
    mov r9d, 1
    push n_embd
    push n_embd
    ...
    mov rcx, [rbx].ModelContext.attn_input
    mov rdx, r13
    mov r8, [rbx].ModelContext.attn_k
    mov r9d, 1
    push n_embd
    mov eax, n_head_kv
    mul [rbx].ModelContext.n_rot
    push rax
    call MatMul_QuantizedWrapper
    add rsp, 16
    
    ; V = input @ Wv
    mov rax, [rbx].ModelContext.layer_wv
    imul r8, layer, 8
    mov rdx, [rax + r8]
    mov rcx, [rbx].ModelContext.attn_input
    mov r8, [rbx].ModelContext.attn_v
    mov r9d, 1
    push n_embd
    mov eax, n_head_kv
    mul [rbx].ModelContext.n_rot
...
MatMul_QuantizedWrapper PROC pA:QWORD, pB:QWORD, pC:QWORD, M:DWORD, K:DWORD, N:DWORD
    ; Determine quantization type and dispatch
    pop r11 ; Return address
    pop r10 ; N arg
    pop r9 ; K arg
    push r11
    ; Standard Windows x64 call: M is in r9d, K is in stack, N is in stack
    ; But here we moved them to simplify
    mov r9d, [rsp+8] ; M
    mov eax, [rsp+16] ; K
    mov ecx, [rsp+24] ; N
    ; For now, just forward to MatMul_F32
    mov r9d, r9d
    push rax
    push rcx
    call MatMul_F32
    add rsp, 16
    ret
MatMul_QuantizedWrapper ENDP

ApplyRoPE PROC USES rbx rsi rdi r12 r13 r14, pCtx:QWORD, pos:DWORD
    LOCAL n_head:DWORD, n_head_kv:DWORD, n_rot:DWORD
...
@@q_head_loop:
    ...
    mul n_rot
    shl rax, 2
    mov rsi, [rbx].ModelContext.attn_q
    add rsi, rax
...
@@q_dim_loop:
    ...
    mov rdi, g_rope_cos_table
    movss xmm2, REAL4 PTR [rdi + rax]
    mov rdi, g_rope_sin_table
    movss xmm3, REAL4 PTR [rdi + rax]
