...
__declspec(dllexport)
void SyntaxHL_SetStyle(SyntaxHighlighter* hl, TokenType token, 
                        COLORREF fg, COLORREF bg, BOOL bold, BOOL italic) {
    if (!hl || token >= TOKEN_COUNT) return;
...
__declspec(dllexport)
const TokenStyle* SyntaxHL_GetStyle(const SyntaxHighlighter* hl, TokenType token) {
    if (!hl || token >= TOKEN_COUNT) return NULL;
    return &hl->styles[token];
}

static void InitDefaultStyles(SyntaxHighlighter* hl) {
    // Dark theme defaults
    hl->styles[TOKEN_NONE].foreground = RGB(212, 212, 212);
    hl->styles[TOKEN_KEYWORD].foreground = RGB(86, 156, 214);        // Blue
    hl->styles[TOKEN_KEYWORD].bold = TRUE;
    hl->styles[TOKEN_TYPE].foreground = RGB(78, 201, 176);           // Teal
    hl->styles[TOKEN_FUNCTION].foreground = RGB(220, 220, 170);      // Yellow
    hl->styles[TOKEN_STRING].foreground = RGB(206, 145, 120);        // Orange
    hl->styles[TOKEN_CHAR].foreground = RGB(206, 145, 120);
    hl->styles[TOKEN_NUMBER].foreground = RGB(181, 206, 168);        // Light green
    hl->styles[TOKEN_COMMENT].foreground = RGB(106, 153, 85);        // Green
    hl->styles[TOKEN_COMMENT].italic = TRUE;
    hl->styles[TOKEN_PREPROCESSOR].foreground = RGB(155, 155, 155);  // Gray
    hl->styles[TOKEN_OPERATOR].foreground = RGB(212, 212, 212);
    hl->styles[TOKEN_BRACKET].foreground = RGB(255, 215, 0);         // Gold
    hl->styles[TOKEN_IDENTIFIER].foreground = RGB(156, 220, 254);    // Light blue
    hl->styles[TOKEN_ATTRIBUTE].foreground = RGB(78, 201, 176);
    hl->styles[TOKEN_TAG].foreground = RGB(86, 156, 214);
    hl->styles[TOKEN_ERROR].foreground = RGB(255, 0, 0);
    hl->styles[TOKEN_ERROR].underline = TRUE;
...
int SyntaxHL_TokenizeLine(SyntaxHighlighter* hl, const char* line, int line_len,
                           SyntaxToken* tokens, int max_tokens) {
    ...
    while (i < line_len && token_count < max_tokens) {
        ...
        // String
            ...
            type = (delim == '\'') ? TOKEN_CHAR : TOKEN_STRING;
        }
        // Number
