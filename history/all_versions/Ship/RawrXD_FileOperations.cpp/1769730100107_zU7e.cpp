...
FileOperationResult FileManager::writeFile(const std::wstring& path, const std::wstring& content, bool createBackup) {
    return writeFileRaw(path, wideToUtf8(content), createBackup);
}

FileOperationResult FileManager::writeFileRaw(const std::wstring& path, const std::vector<uint8_t>& data, bool createBackup) {
