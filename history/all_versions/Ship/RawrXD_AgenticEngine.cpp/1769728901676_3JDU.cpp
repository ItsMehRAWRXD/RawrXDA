...
public:
    ...
    bool Initialize(const std::wstring& workspaceRoot = L"") {
        m_workspaceRoot = workspaceRoot.empty() ? GetCurrentDirectory() : workspaceRoot;
        m_running = true;
    ...
    bool ReadFile(const std::wstring& path, std::wstring& content) {
    ...
    bool WriteFile(const std::wstring& path, const std::wstring& content) {
    ...
    void LoadInstructions(const std::wstring& filePath) {
        if (!filePath.empty() && PathFileExistsW(filePath.c_str())) {
            ...
            ReadFile(filePath, m_instructions);
        } else {
