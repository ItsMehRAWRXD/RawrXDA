...
LoadModelNative PROC FRAME
    ...
    ; Store header
    mov eax, [r15]
    mov [rbx + ModelContext.header], eax
    mov eax, [r15+4]
    mov [rbx + ModelContext.header + 4], eax
    mov rax, [r15+8]
    mov [rbx + ModelContext.header + 8], rax
    mov rax, [r15+16]
    mov [rbx + ModelContext.header + 16], rax
...
ForwardPass PROC FRAME
    ...
    ; Load dimensions
    mov r12d, [rbx + ModelContext.n_embd]
    mov r13d, [rbx + ModelContext.n_layer]
    mov r14d, [rbx + ModelContext.n_head]
    
    ; === TOKEN EMBEDDING ===
    mov rcx, rbx
    mov edx, [rbp+10h]      ; token
    mov r8, [rbx + ModelContext.tok_embeddings]
