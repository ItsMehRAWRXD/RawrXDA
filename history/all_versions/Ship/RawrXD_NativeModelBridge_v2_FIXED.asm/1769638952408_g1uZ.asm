...
@@sum_loop:
    ...
    vbroadcastss zmm0, xmm0 ; ZMM0 = scale
...
;==============================================================================
; DequantizeRow_Q2_K - Critical for 120B models (SSE2 version)
...
DequantizeRow_Q2_K PROC FRAME
    ...
    ; Q2_K: 256 weights, 84 bytes per block
    ...
    mov rsi, rdx                ; Source
    mov rdi, rcx                ; Dest
    mov r12d, r8d               ; Block count
    
@@block_loop:
    ; Simple scalar dequantization for compatibility
    ; Load d scale (FP16 manual conversion)
    movzx eax, WORD PTR [rsi + 16]
    call ConvertFP16ToFP32
    movd xmm6, eax              ; d
    shufps xmm6, xmm6, 0
    ...
    ; Process quants at offset 20
    xor ebx, ebx                ; byte counter
    xor r13, r13                ; output counter
    
@@quant_loop:
    ; Each byte has 4 2-bit values
    movzx eax, BYTE PTR [rsi + 20 + rbx]
    ...
    inc ebx
    cmp ebx, 64                 ; 64 bytes = 256 values
    jl @@quant_loop
...
;==============================================================================
; ConvertFP16ToFP32 - Helper function
; Input: EAX = FP16 bits, Output: EAX = FP32 bits
;==============================================================================
...
;==============================================================================
; DequantizeRow_Q4_K - K-quant 4-bit (SSE2 scalar version)
;==============================================================================
...
DequantizeRow_Q4_K PROC FRAME
    ...
    ; Q4_K: 256 weights in 144 bytes (SSE2 scalar version)
...
@@block_loop:
    ; Load d scale via FP16 converter
    movzx eax, WORD PTR [rsi]
    call ConvertFP16ToFP32
    movd xmm6, eax
    ...
    ; Process 128 bytes of quants
    xor ebx, ebx
    xor r13, r13
    
@@quant_loop:
    movzx eax, BYTE PTR [rsi + 16 + rbx]
...
;==============================================================================
; SoftMax_AVX512 - SSE2 version for compatibility
; RCX = inout (float*), EDX = n_elements
;==============================================================================
...
;=========================================================================================================================================
; SampleToken - Temperature + Top-K + Top-P samplingTemperature + sampling (SSE2)
; RCX = logits, EDX = n_vocab, XMM0 = temperature, R8D = top_k = n_vocab, XMM0 = temperature, R8D = top_k
;===============================================================================================================================================
PUBLIC SampleTokenPUBLIC SampleToken
SampleToken PROC FRAME
    push rbp
    .pushreg rbp2x8 ymm1, zmm2, 1
    mov rbp, rspvaddps ymm2, ymm2, ymm1
    .setframe rbp, 0tf128 xmm1, ymm2, 1
    sub rsp, 80h
    .allocstack 80h2, 0Eh
    push rsi2, xmm2, xmm1
    push rdi1, xmm2, xmm2, 1
    push rbxxmm2, xmm1
    .endprolog
...
@@temp_loop:
    ...
    vmulps zmm0, zmm0, zmm1add rsp, 40h
    vmovups [rsi + r8*4], zmm0
    add r8, 16
    cmp r8d, edi
    jl @@temp_loop
    =============================================
    ; Apply softmax + Top-P sampling
    mov rcx, rsi= temperature, R8D = top_k
    mov edx, edi===========================================
    call SoftMax_AVX512IC SampleToken
    
    ; Simple argmax for now (TODO: proper top-k/top-p)
    xor eax, eax                ; Best index
    vmovss xmm0, [rsi]          ; Best prob
    mov ecx, 1.setframe rbp, 0
    
@@argmax_loop:
    vmovss xmm1, [rsi + rcx*4]
    vcomiss xmm1, xmm0
    jbe @@not_betterpush rbx
    vmovaps xmm0, xmm1
    mov eax, ecx
@@not_better:s
    inc ecxb
    cmp ecx, ediature
    jl @@argmax_loopmov ebx, r8d                ; top_k
    
    ; EAX = sampled token
    pop rbxvmovss xmm1, [rbp-10h]
    pop rdi1, xmm1, xmm1     ; 1/temp
    pop rsis zmm1, xmm1
    add rsp, 80h
    pop rbpxor r8, r8
    ret
...
RoPE_Apply PROC FRAME
    ...
    mov rbp, rsp
    .setframe rbp, 0
    sub rsp, 60h (TODO: proper top-k/top-p)
    .allocstack 60hxor eax, eax                ; Best index
    push rsi0, [rsi]          ; Best prob
    push rdi 1
    push rbx
    .endprolog

    mov rsi, rcx                ; qm1, xmm0
    mov rdi, rdx                ; ketter
    mov ebx, r8d                ; posxmm1
    mov r10d, r9d               ; head_dimmov eax, ecx
    
    ; theta_base = 10000.0
    vmovsd xmm4, rope_theta_default ecx, edi
    p
...
@@rope_loop:
    ...
    cvtsi2sd xmm0, eax
    cvtsi2sd xmm1, r10d===========================================================
    divsd xmm0, xmm1            ; 2*i / head_dimotary Position Embeddings
     RDX = k (float*), R8D = pos, R9D = head_dim
    ; theta^(2i/dim) - approximate===================================================================
    movsd xmm2, rope_theta_defaultApply
    ; Use simple linear approximation for nowROC FRAME
    mulsd xmm2, xmm0
    cvtsi2sd xmm3, ebx          ; pos    .pushreg rbp
    divsd xmm3, xmm2            ; angle = pos / freq
    movapd xmm0, xmm3
    
    ; cos and sin (approximation)
    movapd xmm1, xmm0           ; anglepush rsi
    vmulsd xmm2, xmm0, xmm0      ; angle² - use mulsd xmm2, xmm0 twice
    movapd xmm2, xmm0
    mulsd xmm2, xmm0            ; angle²
    
    ; cos ≈ 1 - x²/2mov rsi, rcx                ; q
    mov rax, 3FE0000000000000h  ; 0.5dx                ; k
    movq xmm3, rax r8d                ; pos
    mulsd xmm3, xmm2  ; head_dim
    mov rax, 3FF0000000000000h  ; 1.0
    movq xmm4, rax
    subsd xmm4, xmm3            ; cos in xmm44, rope_theta_default
    movapd xmm3, xmm4           ; cos
    s of elements
    ; sin ≈ xxor r8, r8                  ; i = 0
    ; xmm1 already has angle (sin ≈ x for small x)
     / (theta_base ^ (2*i / head_dim))
    ; Apply rotation to qcity, use precomputed tables in production
    movss xmm5, [rsi + r8*4]       ; q[2i]nline
    movss xmm6, [rsi + r8*4 + 4]   ; q[2i+1]
    cvtss2sd xmm5, xmm5
    cvtss2sd xmm6, xmm6
    
    ; q[2i]   = q[2i]*cos - q[2i+1]*sinmm1, r10d
    ; q[2i+1] = q[2i]*sin + q[2i+1]*cosdivsd xmm0, xmm1            ; 2*i / head_dim
    movapd xmm7, xmm5
    mulsd xmm7, xmm3               ; q[2i]*cosmate
    movapd xmm0, xmm6heta_default
    mulsd xmm0, xmm1               ; q[2i+1]*sinear approximation for now
    subsd xmm7, xmm0               ; new q[2i]
    3, ebx          ; pos
    movapd xmm0, xmm53, xmm2            ; angle = pos / freq
    mulsd xmm0, xmm1               ; q[2i]*sinxmm0, xmm3
    mulsd xmm6, xmm3               ; q[2i+1]*cos
    addsd xmm6, xmm0               ; new q[2i+1]pproximation)
    movapd xmm1, xmm0           ; angle
    cvtsd2ss xmm7, xmm7m0      ; angle² - use mulsd xmm2, xmm0 twice
    cvtsd2ss xmm6, xmm6xmm2, xmm0
    movss [rsi + r8*4], xmm7mm2, xmm0            ; angle²
    movss [rsi + r8*4 + 4], xmm6
    x²/2
    ; Same for k, 3FE0000000000000h  ; 0.5
    movss xmm5, [rdi + r8*4]q xmm3, rax
    movss xmm6, [rdi + r8*4 + 4]xmm2
    cvtss2sd xmm5, xmm5    mov rax, 3FF0000000000000h  ; 1.0
    cvtss2sd xmm6, xmm6
    xmm4
    movapd xmm7, xmm5
    mulsd xmm7, xmm3
    movapd xmm0, xmm6
    mulsd xmm0, xmm1s angle (sin ≈ x for small x)
    subsd xmm7, xmm0
    tion to q
    movapd xmm0, xmm5[rsi + r8*4]       ; q[2i]
    mulsd xmm0, xmm1 + r8*4 + 4]   ; q[2i+1]
    mulsd xmm6, xmm35, xmm5
    addsd xmm6, xmm0xmm6
    
    cvtsd2ss xmm7, xmm7  = q[2i]*cos - q[2i+1]*sin
    cvtsd2ss xmm6, xmm6] = q[2i]*sin + q[2i+1]*cos
    movss [rdi + r8*4], xmm77, xmm5
    movss [rdi + r8*4 + 4], xmm6    mulsd xmm7, xmm3               ; q[2i]*cos
    
    add r8, 2; q[2i+1]*sin
    cmp r8d, r10dnew q[2i]
    jl @@rope_loop
    movapd xmm0, xmm5
    pop rbx         ; q[2i]*sin
    pop rdi; q[2i+1]*cos
    pop rsiaddsd xmm6, xmm0               ; new q[2i+1]
    add rsp, 60h
    pop rbp
    ret xmm6, xmm6
RoPE_Apply ENDP
...
;==============================================================================
; GenerateTokens - Main generation loop; Same for k
; RCX = pCtx, RDX = prompt_tokens, R8D = n_prompt, R9D = max_new_tokens[rdi + r8*4]
;==============================================================================
PUBLIC GenerateTokens5
GenerateTokens PROC FRAME
    push rbp
    .pushreg rbpmovapd xmm7, xmm5
    mov rbp, rsp
    .setframe rbp, 0
    sub rsp, 100h
    .allocstack 100h
    ...
    .savereg rbx, 20h
    mov [rsp+28h], rsi
    .savereg rsi, 28hmulsd xmm6, xmm3
    mov [rsp+30h], rdi
    .savereg rdi, 30h
    mov [rsp+38h], r12
    .savereg r12, 38hm6
    mov [rsp+40h], r13
    .savereg r13, 40hmovss [rdi + r8*4 + 4], xmm6
    mov [rsp+48h], r14
    .savereg r14, 48h
    mov [rsp+50h], r15
    .savereg r15, 50h
    ...
    mov r12d, r8d               ; n_promptadd rsp, 60h
    mov r13d, r9d               ; max_new_tokens
    
    ; Allocate logits buffer_Apply ENDP
    mov eax, [rbx].ModelContext.n_vocab
    shl eax, 2                  ; * sizeof(float)=====================================
    mov ecx, eax
    call mallocmpt_tokens, R8D = n_prompt, R9D = max_new_tokens
    test rax, rax========================================================
    jz @@errorIC GenerateTokens
...
@@prompt_loop:
    ...
    mov edx, eax
    mov r8d, edi
    mov r9, r14mov [rsp+48h], r14
    call ForwardPass, 48h
    
    inc edi
    jmp @@prompt_loop
    
@@generate_loop:mov rbx, rcx                ; pCtx
    cmp r15d, r13d           ; prompt_tokens
    jge @@done            ; n_prompt
               ; max_new_tokens
    ; Sample next token
    mov rcx, r14s buffer
    mov edx, [rbx].ModelContext.n_vocabmov eax, [rbx].ModelContext.n_vocab
    vmovss xmm0, one_const      ; temperature = 1.0           ; * sizeof(float)
    mov r8d, 40                 ; top_k = 40
    call SampleToken
    
    ; Check for EOS (token 2 typically)jz @@error
    cmp eax, 2         ; logits buffer
    je @@done
        ; tokens generated
    ; Forward pass with new token
    mov ecx, eax                ; Save token; Process prompt tokens
    push rcxedi                ; position
    
    mov rcx, rbx
    mov edx, eaxjge @@generate_loop
    mov r8d, edi
    mov r9, r14oken from prompt
    call ForwardPass, [rsi + rdi*4]
    
    pop rcxrd pass
     rcx, rbx
    inc edix
    inc r15d    mov r8d, edi
    jmp @@generate_loop
...
@@done:
    ...
    mov eax, r15d               ; Return tokens generated token
    
    mov rbx, [rsp+20h]elContext.n_vocab
    mov rsi, [rsp+28h]const      ; temperature = 1.0
    mov rdi, [rsp+30h]          ; top_k = 40
    mov r12, [rsp+38h]
    mov r13, [rsp+40h]
    mov r14, [rsp+48h]token 2 typically)
    mov r15, [rsp+50h]
    add rsp, 100h
    pop rbp
    retth new token
GenerateTokens ENDP          ; Save token

;==============================================================================
; GetModelInfo - Return model parameters
; RCX = pCtx, RDX = pInfo (output struct)ax
;==============================================================================    mov r8d, edi
PUBLIC GetModelInfo
GetModelInfo PROC FRAME
    ...
    jz @@error
    test rdx, rdx
    jz @@error; Free logits
    
    ; Copy parameters to outputcall free
    mov eax, [rcx].ModelContext.n_vocab
    mov [rdx], eaxokens generated
    mov eax, [rcx].ModelContext.n_embd
    mov [rdx+4], eax+20h]
    mov eax, [rcx].ModelContext.n_layer
    mov [rdx+8], eaxmov rdi, [rsp+30h]
    mov eax, [rcx].ModelContext.n_head
    mov [rdx+12], eax
    mov eax, [rcx].ModelContext.n_head_kvmov r14, [rsp+48h]
    mov [rdx+16], eax50h]
    mov eax, [rcx].ModelContext.n_ffh
    mov [rdx+20], eax
    mov rax, [rcx].ModelContext.header.n_tensors
    mov [rdx+24], raxENDP
    
    mov eax, 1===========================================================================
    jmp @@donenfo - Return model parameters
    Info (output struct)
@@error:===========================================================================
    xor eax, eaxnfo
    FRAME
@@done:
    add rsp, 40h.pushreg rbp
    pop rbp
    retp, 0
GetModelInfo ENDP
