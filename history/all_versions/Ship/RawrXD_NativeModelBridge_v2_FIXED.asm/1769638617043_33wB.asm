...
@@block_loop:
    ...
    vmovd xmm0, eax
    vcvtph2ps xmm0, xmm0
    vpbroadcastd zmm0, xmm0     ; ZMM0 = scale broadcast
    
    ; Load 16 bytes of quants
    vmovdqu xmm1, [rdx + 2]
    
    ; Expand low nibbles (first 16 values)
    vpmovzxbd zmm2, xmm1        ; Zero-extend bytes to dwords
    vpandd zmm3, zmm2, ZMMWORD PTR [q4_0_mask] ; Low nibbles
    vpsubd zmm3, zmm3, ZMMWORD PTR [q4_0_bias] ; Subtract 8
    vcvtdq2ps zmm3, zmm3        ; Convert to float
    vmulps zmm3, zmm3, zmm0     ; Scale
    vmovups [rcx], zmm3         ; Store 16 floats
    
    ; Expand high nibbles (next 16 values)
    vpsrld zmm4, zmm2, 4
    vpandd zmm4, zmm4, ZMMWORD PTR [q4_0_mask]
    vpsubd zmm4, zmm4, ZMMWORD PTR [q4_0_bias]
    vcvtdq2ps zmm4, zmm4
    vmulps zmm4, zmm4, zmm0
    vmovups [rcx + 64], zmm4    ; Store next 16 floats
    
    add rdx, 18                 ; Next Q4_0 block
    add rcx, 128                ; 32 floats * 4 bytes
    dec r8d
    jnz @@block_loop
...
@@rope_loop:
    ...
    vdivsd xmm0, xmm0, xmm1     ; 2*i / head_dim
    
    ; theta^(2i/dim) - approximate
    vmovsd xmm2, rope_theta_default
    ; Use simple linear approximation for now
    vmulsd xmm2, xmm2, xmm0
    vcvtsi2sd xmm3, xmm3, ebx   ; pos
    vdivsd xmm0, xmm3, xmm2     ; angle = pos / freq
    
    ; cos and sin (approximation)
    vmovsd xmm1, xmm0           ; angle
    vmulsd xmm2, xmm0, xmm0     ; angle²
    ...
    vmovq xmm4, rax
    vsubsd xmm3, xmm4, xmm3     ; cos
    
    ; sin ≈ x
    ; xmm1 already has angle (sin ≈ x for small x)
    
    ; Apply rotation to q
    vmovss xmm5, [rsi + r8*4]       ; q[2i]
    vmovss xmm6, [rsi + r8*4 + 4]   ; q[2i+1]
    vcvtss2sd xmm5, xmm5, xmm5
    vcvtss2sd xmm6, xmm6, xmm6
    
    ; q[2i]   = q[2i]*cos - q[2i+1]*sin
    ; q[2i+1] = q[2i]*sin + q[2i+1]*cos
    vmulsd xmm7, xmm5, xmm3         ; q[2i]*cos
    vmulsd xmm0, xmm6, xmm1         ; q[2i+1]*sin
    vsubsd xmm7, xmm7, xmm0         ; new q[2i]
    
    vmulsd xmm0, xmm5, xmm1         ; q[2i]*sin
    vmulsd xmm6, xmm6, xmm3         ; q[2i+1]*cos
    vaddsd xmm6, xmm0, xmm6         ; new q[2i+1]
    
    vcvtsd2ss xmm7, xmm7, xmm7
    vcvtsd2ss xmm6, xmm6, xmm6
    vmovss [rsi + r8*4], xmm7
    vmovss [rsi + r8*4 + 4], xmm6
    
    ; Same for k
    vmovss xmm5, [rdi + r8*4]
    vmovss xmm6, [rdi + r8*4 + 4]
    vcvtss2sd xmm5, xmm5, xmm5
    vcvtss2sd xmm6, xmm6, xmm6
    
    vmulsd xmm7, xmm5, xmm3
    vmulsd xmm0, xmm6, xmm1
    vsubsd xmm7, xmm7, xmm0
