...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_SIZE: {
        ...
        // Output panel (center bottom, shown when g_bOutputVisible and terminal NOT shown)
        if (g_hwndOutput) {
        ...
        // Terminal panel (center bottom, shown when g_bTerminalVisible)
        if (g_hwndTerminal) {
        ...
        if (g_hwndChatPanel) {
            if (g_bChatVisible) {
                ...
                MoveWindow(g_hwndChatHistory, 5, 5, chatInnerW, histH, TRUE);
                MoveWindow(g_hwndChatInput, 5, histH + 10, chatInnerW - sendW - 5, inputH, TRUE);
                MoveWindow(g_hwndChatSend, chatInnerW - sendW + 5, histH + 10, sendW, inputH, TRUE);
            } else {
                ShowWindow(g_hwndChatPanel, SW_HIDE);
            }
        }
        
        // Issues panel (far right, beside chat if both open)
    ...
    case WM_COMMAND: {
        ...
        case IDM_VIEW_CHAT:
            ...
            SendMessage(hwnd, WM_SIZE, 0, 0);  // Trigger relayout
            if (g_bChatVisible && g_hwndChatHistory) {
        ...
        // === Missing menu wiring (AI) ===
        case IDM_AI_FIX:
            AI_FixCode();
            break;
        
        // === Missing menu wiring (Build) ===
