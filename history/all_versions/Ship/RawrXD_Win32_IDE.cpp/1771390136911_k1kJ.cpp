...
void SendChatToServer(const std::wstring& message) {
    ...
    if (g_modelLoaded) {
        std::wstring prompt = L"User: " + message + L"\nAssistant:";
        QueueInferenceRequest(prompt, 256, 0.7f);
        AppendWindowText(g_hwndOutput, L"[Chat] Queued model response.\r\n");
        return;
    }
...
bool StartChatServer() {
    ...
    AppendWindowText(g_hwndOutput, L"[Chat] Local chat routing enabled.\r\n");
    return true;
}

void StopChatServer() {
    g_chatServerRunning = false;
    AppendWindowText(g_hwndOutput, L"[Chat] Local chat routing disabled.\r\n");
}
...
void AI_LoadModel() {
    if (!LoadTitanKernel()) {
        AppendWindowText(g_hwndOutput, L"[AI] Titan unavailable. Use Load GGUF.\r\n");
        return;
    }
    OPENFILENAMEW ofn = { sizeof(ofn) };
    WCHAR szFile[MAX_PATH] = {0};
    ...
    if (GetOpenFileNameW(&ofn)) {
        ...
        if (pTitan_LoadModel && pTitan_LoadModel(path.c_str(), "default") == 0) {
            g_modelLoaded = true;
            g_loadedModelPath = szFile;
            StartInferenceThread();
            AppendWindowText(g_hwndOutput, L"[AI] Titan model loaded.\r\n");
        } else {
            AppendWindowText(g_hwndOutput, L"[AI] Titan model load failed.\r\n");
        }
    }
}
...
void RefactorSelectedCode() {
    ...
    if (selStart == selEnd) {
        if (g_hwndChatHistory) {
            AppendWindowText(g_hwndChatHistory, L"AI: Select code to refactor.\r\n");
        }
        return;
    }

    int selLen = (int)(selEnd - selStart);
    std::wstring selected(selLen + 1, L'\0');
    SendMessage(g_hwndEditor, EM_SETSEL, selStart, selEnd);
    SendMessage(g_hwndEditor, EM_GETSELTEXT, 0, (LPARAM)selected.c_str());
    selected.resize(wcslen(selected.c_str()));

    std::wstring refactored = selected;
    for (auto& ch : refactored) {
        if (ch == L'\t') ch = L' ';
    }

    SendMessage(g_hwndEditor, EM_SETSEL, selStart, selEnd);
    SendMessage(g_hwndEditor, EM_REPLACESEL, FALSE, (LPARAM)refactored.c_str());
    AppendWindowText(g_hwndOutput, L"[AI] Refactor applied.\r\n");
}
