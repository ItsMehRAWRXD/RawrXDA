...
static AICompletion g_currentCompletion;     // Current active completion
static bool g_completionVisible = false;       // Is completion currently shown
static HWND g_hwndCompletionPopup = nullptr; // Completion popup window
static HFONT g_hFontCompletion = nullptr;    // Font for completion text
static COLORREF g_colorCompletionBg = RGB(45, 45, 45); // Completion bg
static COLORREF g_colorCompletionText = RGB(156, 220, 254); // Completion text
static COLORREF g_colorCompletionGhost = RGB(100, 100, 100); // Ghost text
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        CreateWindowExW(0, L"BUTTON", L"Generate", WS_CHILD | BS_PUSHBUTTON,
            0, 0, 70, 25, hwnd, (HMENU)IDC_CHAT_GENERATE, GetModuleHandle(nullptr), nullptr);
        CreateWindowExW(0, L"BUTTON", L"Explain", WS_CHILD | BS_PUSHBUTTON,
            0, 0, 70, 25, hwnd, (HMENU)IDC_CHAT_EXPLAIN, GetModuleHandle(nullptr), nullptr);
        CreateWindowExW(0, L"BUTTON", L"Refactor", WS_CHILD | BS_PUSHBUTTON,
            0, 0, 70, 25, hwnd, (HMENU)IDC_CHAT_REFACTOR, GetModuleHandle(nullptr), nullptr);
        CreateWindowExW(0, L"BUTTON", L"Send", WS_CHILD | BS_DEFPUSHBUTTON,
            0, 0, 70, 25, hwnd, (HMENU)IDC_CHAT_SEND, GetModuleHandle(nullptr), nullptr);
    ...
    case WM_SIZE: {
        ...
        if (g_bChatVisible) {
            ...
            MoveWindow(GetDlgItem(hwnd, IDC_CHAT_SEND), chatX + btnWidth * 3, btnY, btnWidth, btnHeight, TRUE);
...
void HandleChatSend() {
    ...
    // Process with AI
    if (g_modelLoaded) {
        AppendWindowText(g_hwndChatHistory, L"[AI thinking...]\r\n");
        
        // Convert to UTF-8
        std::string prompt = WideToUtf8(input);
        
        // Use Inference Engine directly if possible
        if (g_modelContext && pForwardPassInfer) {
            // For now, we simulate a response since we need a proper detokenizer
            // In a real scenario, we'd run a loop of ForwardPass/SampleNext
            AppendWindowText(g_hwndChatHistory, L"Assistant: I am simulating a response because the detokenizer is being bridged.\r\n");
            
            // To be replaced with full agent/tokenizer logic soon
        } else {
            AppendWindowText(g_hwndChatHistory, L"[AI] Inference Engine ready but context not initialized.\r\n");
        }
    } else {
        AppendWindowText(g_hwndChatHistory, L"[AI] No model loaded. Use AI > Load GGUF Model.\r\n");
    }
}
