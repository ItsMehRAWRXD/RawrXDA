...
void AI_GenerateCode() {
    ...
    AppendWindowText(g_hwndOutput, L"[AI] Generating code...\r\n");
    
    // Delegate to the real AI completion system (which tries all backends)
    TriggerAICompletion();
}
...
bool LoadTitanKernel() {
    ...
    if (!pTitan_Initialize || !pTitan_LoadModel || !pTitan_RunInference || !pTitan_Shutdown) {
        AppendWindowText(g_hwndOutput, L"[AI] Failed to load Titan Kernel functions.\r\n");
    ...
    if (pTitan_Initialize() != 0) {
        AppendWindowText(g_hwndOutput, L"[AI] Titan Kernel initialization failed.\r\n");
        FreeLibrary(g_hTitanDll);
        g_hTitanDll = nullptr;
        return false;
    }
    
    AppendWindowText(g_hwndOutput, L"[AI] Titan Kernel loaded successfully.\r\n");
    return true;
}
...
bool LoadInferenceEngine() {
    ...
    pForwardPassInfer = (ForwardPassInfer_t)GetProcAddress(g_hInferenceEngine, "ForwardPassInfer");
    ...
    if (!pLoadModel || !pUnloadModel || !pForwardPassInfer || !pSampleNext) {
        AppendWindowText(g_hwndOutput, L"[AI] Failed to load Inference Engine functions.\r\n");
        FreeLibrary(g_hInferenceEngine);
        g_hInferenceEngine = nullptr;
        return false;
    }
    
    AppendWindowText(g_hwndOutput, L"[AI] Inference Engine loaded successfully.\r\n");
    return true;
}
...
bool LoadModelWithInferenceEngine(const std::wstring& modelPath) {
    ...
    if (!g_modelContext) {
        AppendWindowText(g_hwndOutput, L"[AI] Failed to load model with Inference Engine.\r\n");
        return false;
    }
...
void LoadGGUFModel() {
    ...
    if (!g_hInferenceEngine && !LoadInferenceEngine()) {
        AppendWindowText(g_hwndOutput, L"[AI] RawrXD_InferenceEngine.dll not found. Cannot load GGUF.\r\n");
        return;
    }
    OPENFILENAMEW ofn = { sizeof(ofn) };
    WCHAR szFile[MAX_PATH] = {0};
    ...
    if (GetOpenFileNameW(&ofn)) {
        ...
        if (LoadModelWithInferenceEngine(szFile)) {
            AppendWindowText(g_hwndOutput, (L"[AI] GGUF model loaded: " + std::wstring(szFile) + L"\r\n").c_str());
            StartInferenceThread();
        } else {
...
bool StartChatServer() {
    ...
    std::wstring cmd = L"python -c \"import socket, threading, time; s=socket.socket(); s.bind(('localhost',23959)); s.listen(5); print('Chat server started on port 23959'); def handle(c): try: data=c.recv(1024).decode(); print(f'Received: {data}'); c.send(b'Hello from RawrXD Chat Server!'); c.close(); except: pass; while True: c,a=s.accept(); threading.Thread(target=handle,args=(c,)).start()\"";
    ...
    if (CreateProcessW(nullptr, &cmd[0], nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr, nullptr, &si, &pi)) {
        ...
        AppendWindowText(g_hwndOutput, L"[Chat] Failed to start chat server.\r\n");
        return false;
    }
}
...
void StopChatServer() {
    ...
    // For now, just mark as stopped - in a real implementation, we'd need to track the process
...
void SendChatMessage() {
    ...
    int len = GetWindowTextLength(g_hwndChatInput);
    if (len == 0) return;
    
    std::wstring message(len + 1, L'\0');
    GetWindowText(g_hwndChatInput, &message[0], len + 1);
    message.resize(len);
...
// Menu Handlers
// ============================================================================
void HandleAIMenu(WPARAM wParam) {
    switch (LOWORD(wParam)) {
        case ID_AI_COMPLETION:
            TriggerAICompletion();
            break;
        case ID_AI_EXPLAIN:
            ExplainSelectedCode();
...
void HandleViewMenu(WPARAM wParam) {
    switch (LOWORD(wParam)) {
        case ID_VIEW_CHAT:
            if (g_hwndChatPanel) {
                ShowWindow(g_hwndChatPanel, IsWindowVisible(g_hwndChatPanel) ? SW_HIDE : SW_SHOW);
            } else {
                CreateChatPanel();
            }
            break;
        case ID_VIEW_TERMINAL:
            if (g_hwndTerminal) {
                ShowWindow(g_hwndTerminal, IsWindowVisible(g_hwndTerminal) ? SW_HIDE : SW_SHOW);
            } else {
                CreateTerminal();
            }
            break;
        case ID_VIEW_OUTPUT:
            if (g_hwndOutput) {
                ShowWindow(g_hwndOutput, IsWindowVisible(g_hwndOutput) ? SW_HIDE : SW_SHOW);
            }
            break;
        case ID_VIEW_ISSUES:
            if (g_hwndIssuesList) {
                ShowWindow(g_hwndIssuesList, IsWindowVisible(g_hwndIssuesList) ? SW_HIDE : SW_SHOW);
            }
            break;
    }
}

void HandleChatMenu(WPARAM wParam) {
    switch (LOWORD(wParam)) {
...
struct CursorPosition {
    int line;
    int column;
    int charIndex;  // EM_CHARFROMPOS result
    bool isSelection;
    int selectionStart;
    int selectionEnd;
};
...
struct FoldingRegion {
    ...
    int endLine;
    bool isCollapsed;
    std::wstring preview;  // First line preview when collapsed
};
...
static std::vector<Theme> g_themes = {
    {
        ...
        RGB(204, 204, 204), // text
        RGB(51, 153, 255),  // selection
        RGB(128, 128, 128), // lineNumber
        RGB(86, 156, 214),  // keyword (blue)
        RGB(106, 153, 85),  // comment (green)
        RGB(206, 145, 120), // string (orange)
        RGB(181, 206, 168), // number (light green)
        RGB(20, 20, 20),    // terminal bg
        RGB(200, 255, 100), // terminal text
        RGB(100, 255, 100), // terminal success
        RGB(255, 100, 100), // terminal error
        RGB(255, 255, 100)  // terminal warning
    },
    {
        L"Light",
        ...
        RGB(0, 128, 0),     // comment (green)
        RGB(163, 21, 21),   // string (red)
        RGB(9, 134, 88),    // number (teal)
        RGB(240, 240, 240), // terminal bg
        RGB(0, 0, 0),       // terminal text
        RGB(0, 128, 0),     // terminal success
        RGB(255, 0, 0),     // terminal error
        RGB(255, 165, 0)    // terminal warning
    },
    {
        L"Monokai",
        RGB(39, 40, 34),    // background
        RGB(248, 248, 242), // text
        RGB(73, 72, 62),    // selection
        RGB(128, 128, 128), // lineNumber
        RGB(249, 38, 114),  // keyword (pink)
        RGB(117, 113, 94),  // comment (gray)
        RGB(230, 219, 116), // string (yellow)
        RGB(174, 129, 255), // number (purple)
        RGB(39, 40, 34),    // terminal bg
        RGB(248, 248, 242), // terminal text
        RGB(166, 226, 46),  // terminal success
        RGB(249, 38, 114),  // terminal error
        RGB(253, 151, 31)   // terminal warning
    }
};
...
// Menu aliases (IDM_ for menu handlers)
...
void CreateMenuBar(HWND hwnd) {
    ...
    // View Menu - ALL PANELS ENABLED BY DEFAULT
    ...
    AppendMenu(hViewMenu, MF_STRING | MF_CHECKED, ID_VIEW_TERMINAL, L"&Terminal");
    AppendMenu(hViewMenu, MF_STRING | MF_CHECKED, ID_VIEW_CHAT, L"&Chat");
    AppendMenu(hViewMenu, MF_STRING | MF_CHECKED, ID_VIEW_ISSUES, L"&Issues");
...
void HandleViewMenu(HWND hwnd, UINT id) {
    switch (id) {
        ...
        case ID_VIEW_TERMINAL:
            ...
            if (g_bTerminalVisible && !g_hwndTerminal) CreateTerminal();
            if (g_hwndTerminal) ShowWindow(g_hwndTerminal, g_bTerminalVisible ? SW_SHOW : SW_HIDE);
            SendMessage(hwnd, WM_SIZE, 0, 0);
            break;
        case ID_VIEW_CHAT:
            g_bChatVisible = !g_bChatVisible;
            if (g_bChatVisible && !g_hwndChatPanel) CreateChatPanel();
            if (g_hwndChatPanel) ShowWindow(g_hwndChatPanel, g_bChatVisible ? SW_SHOW : SW_HIDE);
            SendMessage(hwnd, WM_SIZE, 0, 0);
            break;
        case ID_VIEW_ISSUES:
...
void ClearCodeIssues() {
    ...
    if (g_hwndIssuesList) {
        SendMessage(g_hwndIssuesList, LB_RESETCONTENT, 0, 0);
    }
}
...
void RunCodeAnalysis() {
    ...
    ClearCodeIssues();
    AppendWindowText(g_hwndOutput, L"[Analysis] Running code analysis...\r\n");
    ...
    std::istringstream stream(WideToUtf8(fullContent));
    std::string line;
    int lineNum = 1;
    int totalLines = 0;
    int emptyLines = 0;
    ...
    while (std::getline(stream, line)) {
        ...
        if (!line.empty() && line.back() != ';' && line.back() != '{' && line.back() != '}' && 
            line.back() != ':' && line.find('#') != 0 && line.find("//") == std::string::npos &&
            line.find("/*") == std::string::npos && !inMultilineComment) {
            
            // Check if it's a declaration/statement that should end with semicolon
            bool isDeclaration = line.find("int ") != std::string::npos ||
            ...
            if (isDeclaration && line.find("if(") == std::string::npos && 
                line.find("for(") == std::string::npos && line.find("while(") == std::string::npos &&
                line.find("switch(") == std::string::npos) {
                AddCodeIssue(lineNum, L"Missing semicolon", L"Error", true);
            }
        }
        ...
        if (line.find("int ") != std::string::npos && line.find('=') == std::string::npos && 
            line.find('(') == std::string::npos) {
            AddCodeIssue(lineNum, L"Uninitialized variable", L"Warning", true);
        }
        
        // Missing includes for common types
        if (line.find("std::string") != std::string::npos) {
            // Check if #include <string> exists in the file
            if (fullContent.find(L"#include <string>") == std::wstring::npos &&
                fullContent.find(L"#include \"string\"") == std::wstring::npos) {
                AddCodeIssue(lineNum, L"Missing #include <string> for std::string", L"Error", true);
            }
        }
        
        if (line.find("std::vector") != std::string::npos) {
            if (fullContent.find(L"#include <vector>") == std::wstring::npos &&
                fullContent.find(L"#include \"vector\"") == std::wstring::npos) {
                AddCodeIssue(lineNum, L"Missing #include <vector> for std::vector", L"Error", true);
            }
        }
        ...
        if (line.find("->") != std::string::npos || line.find("*.") != std::string::npos) {
            AddCodeIssue(lineNum, L"Potential null pointer access", L"Warning", false);
        }
        
        // Unused variables (very basic check)
        if (line.find("int ") != std::string::npos || line.find("bool ") != std::string::npos) {
            size_t varStart = line.find("int ");
            if (varStart == std::string::npos) varStart = line.find("bool ");
            if (varStart != std::string::npos) {
                size_t varEnd = line.find(' ', varStart + 4);
                if (varEnd != std::string::npos) {
                    std::string varName = line.substr(varStart + 4, varEnd - varStart - 4);
                    // Check if variable is used elsewhere (basic check)
                    if (fullContent.find(Utf8ToWide(varName)) == fullContent.find(Utf8ToWide(varName))) {
                        // Variable only appears in its declaration
                        AddCodeIssue(lineNum, L"Potentially unused variable: " + Utf8ToWide(varName), L"Info", false);
                    }
                }
            }
        }
    ...
    double commentRatio = totalLines > 0 ? (double)commentLines / totalLines * 100.0 : 0.0;
    double emptyRatio = totalLines > 0 ? (double)emptyLines / totalLines * 100.0 : 0.0;
    
    if (commentRatio < 10.0) {
        AddCodeIssue(1, L"Low comment ratio (< 10%)", L"Info", false);
    }
    
    if (emptyRatio > 50.0) {
        AddCodeIssue(1, L"High empty line ratio (> 50%)", L"Info", false);
    }
    
    if (functionCount == 0) {
        AddCodeIssue(1, L"No functions detected", L"Info", false);
    }
    ...
    {
        std::lock_guard<std::mutex> lock(g_issuesMutex);
        issueCount = (int)g_codeIssues.size();
    }
    
    if (issueCount > 0) {
        wchar_t status[256];
        swprintf_s(status, L"Analysis complete: %d issues found in %d lines", issueCount, totalLines);
        AppendWindowText(g_hwndOutput, (std::wstring(L"[Analysis] ") + status + L"\r\n").c_str());
    } else {
        AppendWindowText(g_hwndOutput, L"[Analysis] No issues found. Code looks clean!\r\n");
    }
}
...
void FilterIssuesBySeverity(const std::wstring& severity) {
    ...
    std::lock_guard<std::mutex> lock(g_issuesMutex);
    for (const auto& issue : g_codeIssues) {
        if (severity.empty() || issue.severity == severity) {
            std::wstring displayText = std::to_wstring(issue.line) + L": " + issue.severity + L" - " + issue.message;
            SendMessage(g_hwndIssuesList, LB_ADDSTRING, 0, (LPARAM)displayText.c_str());
        }
    }
}
...
void ExportIssues() {
    AppendWindowText(g_hwndOutput, L"[Issues] Exporting issues...\r\n");
    // Implementation for exporting issues to file
}
...
void LoadSettings() {
    ...
    AppendWindowText(g_hwndOutput, L"[Settings] Loading settings...\r\n");
}
...
bool StartTerminal() {
    // Implementation for starting terminal
    AppendWindowText(g_hwndOutput, L"[Terminal] Starting terminal...\r\n");
    return true;
}

void UnloadInferenceModel() {
    // Implementation for unloading inference model
    if (g_modelContext) {
        if (pUnloadModel) {
...
void SaveSettings() {
    ...
    json += "  \"minimapEnabled\": " + std::string(g_settings.minimapEnabled ? "true" : "false") + ",\n";
    json += "  \"codeFoldingEnabled\": " + std::string(g_settings.codeFoldingEnabled ? "true" : "false") + ",\n";
    json += "  \"terminalVisible\": " + std::string(g_settings.terminalVisible ? "true" : "false") + "\n";
    json += "}";
