...
struct CommandPaletteEntry {
    int id;
    std::wstring label;        // e.g. "File: Open..."
    std::wstring shortcut;     // e.g. "Ctrl+O"
    std::wstring category;     // e.g. "File"
};

static HWND g_hwndCmdPalOverlay = nullptr;
static HWND g_hwndCmdPalInput = nullptr;
static HWND g_hwndCmdPalList = nullptr;
static bool g_cmdPalVisible = false;
static std::vector<CommandPaletteEntry> g_allCommands;
static std::vector<int> g_filteredCommandIndices;
...
static bool g_popOutClassRegistered = false;
...
// Sourcefile Dropdown
// ============================================================================
static HWND g_hwndSourcefileCombo = nullptr;
...
// ============================================================================
LRESULT CALLBACK PopOutChatWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    static HWND hHistory = nullptr, hInput = nullptr, hSend = nullptr;
    switch (msg) {
    case WM_CREATE: {
        hHistory = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
            WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY,
            5, 5, 380, 320, hwnd, nullptr, g_hInstance, nullptr);
        SendMessageW(hHistory, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
        hInput = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
            WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
            5, 330, 290, 24, hwnd, nullptr, g_hInstance, nullptr);
        SendMessageW(hInput, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
        hSend = CreateWindowExW(0, L"BUTTON", L"Send",
            WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
            300, 330, 80, 24, hwnd, (HMENU)1, g_hInstance, nullptr);
        SendMessageW(hSend, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
        AppendWindowText(hHistory, L"Pop-out chat ready.\r\n");
        break;
    }
    case WM_SIZE: {
        RECT rc;
        ...
        MoveWindow(hHistory, 5, 5, w - 10, h - 40, TRUE);
        MoveWindow(hInput, 5, h - 30, w - 100, 24, TRUE);
        MoveWindow(hSend, w - 90, h - 30, 80, 24, TRUE);
        break;
    }
    case WM_COMMAND:
        if (LOWORD(wParam) == 1 && HIWORD(wParam) == BN_CLICKED) {
            ...
            if (len > 0) {
                ...
                GetWindowTextW(hInput, &msg[0], len + 1);
                msg.resize(len);
                AppendWindowText(hHistory, L"You: ");
                AppendWindowText(hHistory, msg.c_str());
                AppendWindowText(hHistory, L"\r\n");
                SetWindowTextW(hInput, L"");
                // Also send to main chat if available
                if (g_hwndChatHistory) {
                    AppendWindowText(g_hwndChatHistory, L"[PopOut] ");
                    AppendWindowText(g_hwndChatHistory, msg.c_str());
                    AppendWindowText(g_hwndChatHistory, L"\r\n");
                }
            }
        }
        break;
    case WM_DESTROY:
        hHistory = nullptr; hInput = nullptr; hSend = nullptr;
        break;
    }
    return DefWindowProcW(hwnd, msg, wParam, lParam);
}
...
void PopOutChat() {
    ...
    HWND h = CreateWindowExW(0, L"RawrXD_PopOutChat", L"RawrXD Chat",
