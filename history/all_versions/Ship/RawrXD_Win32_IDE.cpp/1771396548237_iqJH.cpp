...
#define ID_CHAT_POPOUT       1063
#define ID_CHAT_NEW          1064
#define ID_CHAT_MODEL_SELECT 1065
#define ID_SETTINGS          1070
#define ID_SETTINGS_APPLY    1071
#define ID_AI_AGENTIC        1080
#define ID_AI_AUTONOMOUS     1081
...
#define IDM_CHAT_POPOUT     ID_CHAT_POPOUT
#define IDM_CHAT_NEW        ID_CHAT_NEW
#define IDM_SETTINGS        ID_SETTINGS
#define IDM_AI_AGENTIC      ID_AI_AGENTIC
#define IDM_AI_AUTONOMOUS   ID_AI_AUTONOMOUS
...
// ============================================================================
// Pop-Out Chat & Multi-Instance System
// ============================================================================
// Each chat window (including pop-outs) gets its own ChatInstance with
// independent session state, model context, and inference thread so that
// models never cross between windows.

struct ChatInstance {
    int         id;                     // unique instance ID
    std::wstring sessionId;             // unique session identifier
    std::wstring title;                 // window title
    std::vector<std::wstring> messages; // chat history

    // Per-instance model state — independent from global model
    std::wstring modelName;             // human-readable backend name
    std::wstring modelPath;             // path to GGUF/DLL
    HMODULE     hModelDll = nullptr;    // loaded DLL handle
    void*       modelCtx  = nullptr;    // model context for InferenceEngine
    bool        modelLoaded = false;

    // Per-instance inference thread
    HANDLE      hInferenceThread = nullptr;
...
struct PopOutChat {
    HWND        hwnd        = nullptr;  // top-level window
    HWND        hwndHistory = nullptr;  // chat history edit
    HWND        hwndInput   = nullptr;  // chat input edit
    HWND        hwndSend    = nullptr;  // send button
    HWND        hwndModelCombo = nullptr; // model selector dropdown
    HWND        hwndPopOutBtn  = nullptr; // re-dock button (unused for now)
    ChatInstance instance;              // owns its own session
};

static std::vector<PopOutChat*> g_popOutChats;
static int g_nextChatInstanceId = 1;
static ATOM g_popOutChatClassAtom = 0;

// Forward declarations for pop-out system
LRESULT CALLBACK PopOutChatWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
PopOutChat* FindPopOutByHwnd(HWND hwnd);
void PopOutChatSendMessage(PopOutChat* pc);
void PopOutChatRunInference(PopOutChat* pc, const std::wstring& message);
void RegisterPopOutChatClass(HINSTANCE hInst);
PopOutChat* CreatePopOutChatWindow(HINSTANCE hInst, const std::wstring& title);

// Main docked chat also gets a ChatInstance
static ChatInstance g_mainChatInstance;

// ============================================================================
// Agentic / Autonomous DLL Integration
// ============================================================================
// All discovered DLLs in Ship/ that provide agent capabilities.
// We load them on demand and expose through the model selector.

struct AgentDllInfo {
    std::wstring name;          // display name
    std::wstring filename;      // DLL filename
    HMODULE      hModule = nullptr;
    bool         loaded  = false;
};

// Catalog of available agent DLLs (populated at startup)
static std::vector<AgentDllInfo> g_agentDlls;

// Agentic Engine function pointers (from RawrXD_AgenticEngine.dll)
typedef int  (*AgenticEngine_Init_t)();
typedef int  (*AgenticEngine_RunTask_t)(const char* taskJson, char* resultBuf, int bufSize);
typedef void (*AgenticEngine_Shutdown_t)();

static AgenticEngine_Init_t     pAgenticEngine_Init     = nullptr;
static AgenticEngine_RunTask_t  pAgenticEngine_RunTask  = nullptr;
static AgenticEngine_Shutdown_t pAgenticEngine_Shutdown  = nullptr;

// Agent Coordinator
typedef int  (*AgentCoord_Init_t)();
typedef int  (*AgentCoord_Dispatch_t)(const char* prompt, const char* model, char* resultBuf, int bufSize);
typedef void (*AgentCoord_Shutdown_t)();

static AgentCoord_Init_t     pAgentCoord_Init     = nullptr;
static AgentCoord_Dispatch_t pAgentCoord_Dispatch  = nullptr;
static AgentCoord_Shutdown_t pAgentCoord_Shutdown  = nullptr;

// CopilotBridge
typedef int  (*CopilotBridge_Init_t)();
typedef int  (*CopilotBridge_Complete_t)(const char* prefix, const char* suffix, char* resultBuf, int bufSize);
typedef void (*CopilotBridge_Shutdown_t)();

static CopilotBridge_Init_t     pCopilotBridge_Init     = nullptr;
static CopilotBridge_Complete_t pCopilotBridge_Complete  = nullptr;
static CopilotBridge_Shutdown_t pCopilotBridge_Shutdown  = nullptr;

// Forward declarations for agentic
void DiscoverAgentDlls();
bool LoadAgentDll(const std::wstring& dllName);
void RunAgenticTask(const std::wstring& task);
void RunAutonomousMode(const std::wstring& objective);
std::vector<std::wstring> GetAvailableModelNames();
