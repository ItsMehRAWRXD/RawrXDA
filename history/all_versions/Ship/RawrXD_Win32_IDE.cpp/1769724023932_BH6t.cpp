...
bool LoadInferenceEngine() {
    g_hInferenceEngineDll = LoadLibraryW(L"RawrXD_InferenceEngine.dll");
    if (!g_hInferenceEngineDll) {
        wchar_t path[MAX_PATH];
        GetModuleFileNameW(nullptr, path, MAX_PATH);
        std::wstring dir(path);
        size_t pos = dir.find_last_of(L"\\/");
        if (pos != std::wstring::npos) {
            ...
            g_hInferenceEngineDll = LoadLibraryW(dir.c_str());
        }
    }
    if (!g_hInferenceEngineDll) return false;
    
    pLoadModel = (LoadModel_t)GetProcAddress(g_hInferenceEngineDll, "LoadModel");
    pUnloadModel = (UnloadModel_t)GetProcAddress(g_hInferenceEngineDll, "UnloadModel");
    pForwardPass_IE = (ForwardPass_IE_t)GetProcAddress(g_hInferenceEngineDll, "ForwardPass");
    pSampleNext = (SampleNext_t)GetProcAddress(g_hInferenceEngineDll, "SampleNext");
    
    return pLoadModel != nullptr && pUnloadModel != nullptr;
}

bool LoadModelWithInferenceEngine(const std::wstring& filepath) {
    if (!g_hInferenceEngineDll || !pLoadModel) {
