...
CleanupMathTables PROC
    ...
    ; Free sin table
    mov rax, [rel gRopeTableSin]
    test rax, rax
    jz @@skip_sin
    mov rcx, rax
    call free
    mov qword ptr [rel gRopeTableSin], 0
    
@@skip_sin:
    ; Free cos table
    mov rax, [rel gRopeTableCos]
    test rax, rax
    jz @@skip_cos
    mov rcx, rax
    call free
    mov qword ptr [rel gRopeTableCos], 0
    
@@skip_cos:
    ; Free exp table
    mov rax, [rel gExpTable]
    test rax, rax
    jz @@skip_exp
    mov rcx, rax
    call free
    mov qword ptr [rel gExpTable], 0
    
@@skip_exp:
    ; Free log table
    mov rax, [rel gLogTable]
    test rax, rax
    jz @@skip_log
    mov rcx, rax
    call free
    mov qword ptr [rel gLogTable], 0
    
@@skip_log:
    ; Free temp buffer
    mov rax, [rel gTempBuffer]
    test rax, rax
    jz @@skip_temp
    mov rcx, rax
    call free
    mov qword ptr [rel gTempBuffer], 0
