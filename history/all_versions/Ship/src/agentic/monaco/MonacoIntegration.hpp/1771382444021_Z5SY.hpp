...
// Monaco editor implementation (WebView2 or RichEdit fallback)
#ifdef HAS_WEBVIEW2
...
#else
// Fallback RichEdit-based Monaco editor
...
public:
    ...
    ~MonacoRichEditEditor() override;

    // IMonacoEditor implementation
    bool Initialize(HWND parentWindow, const RECT& rect, const MonacoConfig& config) override;
    void Shutdown() override;
    bool IsInitialized() const override { return m_initialized && m_hwndEditor != nullptr; }
...
private:
    ...
    int GetColumnFromChar(int charIndex) const;
