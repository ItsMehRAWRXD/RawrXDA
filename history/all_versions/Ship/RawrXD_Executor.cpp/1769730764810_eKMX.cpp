...
public:
    ...
    EventSystem<TrainingProgress> TrainingProgressUpdated;      // progress
    ...
    std::vector<std::wstring> SearchMemory(const std::wstring& query, size_t maxResults = 10) {
        ...
        while (m_running.load()) {
            if (m_trainingActive.load()) {
                ...
                for (int epoch = 0; epoch < epochs && m_trainingActive.load(); ++epoch) {
                    ...
                    TrainingProgressUpdated.Emit(m_currentTrainingProgress);
