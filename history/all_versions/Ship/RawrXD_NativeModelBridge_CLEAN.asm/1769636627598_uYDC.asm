...
GetTokenEmbedding PROC FRAME pCtx:QWORD, token:DWORD, pEmbeddings:QWORD
    ...
    lea rax, [rsi + rax + 512]  ; RAX = pointer to this token's embedding data
...
@@rope_loop:
    cmp rcx, r13d
    jge @@rope_exit
    cmp rcx, 2          ; Only process first 2 dimensions for demo
    jge @@rope_exit
    
    ; Load value pair
    movss xmm0, DWORD PTR [rbx + rcx*4]      ; XMM0 = val[d]
    movss xmm1, DWORD PTR [rbx + rcx*4 + 4]  ; XMM1 = val[d+1]
    
    ; In production, would lookup sin/cos from tables
    ; sin_angle = sin_table[pos*dim + d]
    ; cos_angle = cos_table[pos*dim + d]
    ; For now: simplified circular rotation (simplified RoPE)
    
    ; Simplified: rotate by angle proportional to position and dimension
    ; This is a placeholder for the full RoPE computation
    
    ; val[d] = val[d] * cos - val[d+1] * sin
    ; val[d+1] = val[d] * sin + val[d+1] * cos
    ; (Simplified: use small angle approximation if needed)
    
    ; For demo, we'll just keep values as-is (no-op RoPE)
    ; Production: Would compute from angle = pos / (base^(2d/dim))
    
    movss DWORD PTR [rbx + rcx*4], xmm0      ; Store back val[d]
    movss DWORD PTR [rbx + rcx*4 + 4], xmm1  ; Store back val[d+1]
...
@@loop_sum:
    cmp rcx, r9          ; If index >= n_embd, break
    jge @@compute_rms
    
    ; Load input[rcx]
    movss xmm1, DWORD PTR [rbx + rcx*4]  ; XMM1 = input[rcx]
...
@@compute_rms:
    ...
    ; Add epsilon: RMS = RMS + eps
    movsd xmm1, [rbp+20h] ; XMM1 = eps (REAL8)
    cvtsd2ss xmm1, xmm1   ; Convert to single precision
    addss xmm0, xmm1    ; XMM0 = RMS + eps
    
    ; Compute 1/RMS for later multiplication
    movss xmm1, 1.0     ; XMM1 = 1.0
...
@@loop_norm:
    cmp rcx, r9         ; If index >= n_embd, done
    jge @@exit
    
    ; output[rcx] = input[rcx] * (1 / RMS)
    movss xmm0, DWORD PTR [rbx + rcx*4]  ; XMM0 = input[rcx]
    mulss xmm0, xmm1    ; XMM0 = input[rcx] * (1/RMS)
    movss DWORD PTR [rsi + rcx*4], xmm0  ; output[rcx] = XMM0
