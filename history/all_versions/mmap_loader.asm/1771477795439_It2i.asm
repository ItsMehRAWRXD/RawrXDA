...
mmap_open PROC FRAME
    ...
    je      @@mmap_fail
    mov     [ggufCtx + GGUF_CTX.hFile], rax
    ...
    lea     rdx, [fileLen]
    call    GetFileSizeEx
    mov     rax, [fileLen + LARGE_INTEGER.QuadPart]
    mov     [ggufCtx + GGUF_CTX.fileSize], rax
    ...
    test    rax, rax
    jz      @@mmap_close_file
    mov     [ggufCtx + GGUF_CTX.hMapping], rax
    ...
    mov     [ggufCtx + GGUF_CTX.pBase], rax
    jmp     @@mmap_ret

@@mmap_close_mapping:
    mov     rcx, [ggufCtx + GGUF_CTX.hMapping]
    call    CloseHandle
@@mmap_close_file:
    mov     rcx, [ggufCtx + GGUF_CTX.hFile]
    call    CloseHandle
@@mmap_fail:
    xor     eax, eax
@@mmap_ret:
...
mmap_close PROC FRAME
    ...
    mov     rcx, [ggufCtx + GGUF_CTX.pBase]
    test    rcx, rcx
    jz      @@mc_skip_unmap
...
gguf_parse_header PROC FRAME
    ...
    mov     rbx, [ggufCtx + GGUF_CTX.pBase]
    ...
    mov     [ggufCtx + GGUF_CTX.magic], eax
    cmp     eax, GGUF_MAGIC
    jne     @@bad_magic

    ; Read version
    mov     eax, DWORD PTR [rbx+4]
    mov     [ggufCtx + GGUF_CTX.version], eax

    ; Read tensor count (uint64)
    mov     rax, QWORD PTR [rbx+8]
    mov     [ggufCtx + GGUF_CTX.tensorCount], rax

    ; Read metadata count (uint64)
    mov     rax, QWORD PTR [rbx+16]
    mov     [ggufCtx + GGUF_CTX.metadataCount], rax
...
gguf_parse_metadata PROC FRAME
    ...
    mov     rsi, [ggufCtx + GGUF_CTX.pBase]    ; base pointer
    mov     r12, rcx            ; r12 = current offset
    mov     r13, [ggufCtx + GGUF_CTX.metadataCount]
...
@@gguf_mapped:
    ...
    call    write_con
    mov     rax, [ggufCtx + GGUF_CTX.fileSize]
    lea     rcx, [numBuf]
    call    itoa_u64
    mov     rcx, rax
    call    write_con
    ...
    mov     edx, szMmapOkLen
    call    write_con
    mov     rax, [ggufCtx + GGUF_CTX.pBase]
    lea     rcx, [numBuf]
    call    itoa_hex
    mov     rcx, rax
    call    write_con
    ...
    mov     edx, 64
    ; Clamp to file size
    cmp     rdx, [ggufCtx + GGUF_CTX.fileSize]
    jbe     @@hex_ok
    mov     rdx, [ggufCtx + GGUF_CTX.fileSize]
@@hex_ok:
    call    hexdump
    ...
    call    write_con
    mov     eax, [ggufCtx + GGUF_CTX.version]
    lea     rcx, [numBuf]
    call    itoa_u64
    mov     rcx, rax
    call    write_con
    ...
    call    write_con
    mov     rax, [ggufCtx + GGUF_CTX.tensorCount]
    lea     rcx, [numBuf]
    call    itoa_u64
    mov     rcx, rax
    call    write_con
    ...
    call    write_con
    mov     rax, [ggufCtx + GGUF_CTX.metadataCount]
    lea     rcx, [numBuf]
    call    itoa_u64
    mov     rcx, rax
    call    write_con
