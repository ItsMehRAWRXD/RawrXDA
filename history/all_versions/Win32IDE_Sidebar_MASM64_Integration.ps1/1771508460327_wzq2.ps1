...
# 1. Copy working minimal ASM source
$MinimalAsmPath = "$PSScriptRoot\src\Win32IDE_Sidebar_minimal.asm"
if (Test-Path $MinimalAsmPath) {
    Copy-Item $MinimalAsmPath $AsmFile -Force
    Write-Host "[+] Copied working minimal ASM to $AsmFile" -ForegroundColor Green
} else {
    Write-Host "[!] Minimal ASM file not found at $MinimalAsmPath" -ForegroundColor Red
    exit 1
}
