...
def scan_model_directories():
    """Scan all model directories recursively for GGUF, BLOB, BIN, and SafeTensors files"""
    ...
    print(f"[Scan] Found {len(models)} models (GGUF, BLOB, BIN, SafeTensors) across {len(MODEL_DIRS)} directories (recursive)")
    return models
