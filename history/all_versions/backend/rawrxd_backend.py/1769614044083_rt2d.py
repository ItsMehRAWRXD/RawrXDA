...
class RawrXDHandler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_post_ask(self):
        ...
        try:
            ...
            # Use a good default model if none specified or invalid
            if not model or model == 'default' or model == 'null':
                model = 'llama3.2:3b'  # Fast, reliable default
            
            # If model looks like a file path, extract just the model name for Ollama
            if '/' in model or '\\' in model:
                # This is a file path, try to find matching Ollama model
                basename = os.path.basename(model).lower()
                # For now, fall back to default
                print(f"[/ask] File path detected ({model}), using default model")
                model = 'llama3.2:3b'
