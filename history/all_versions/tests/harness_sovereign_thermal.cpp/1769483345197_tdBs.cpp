...
struct HarnessConfig {
    ...
    int32_t maxWearPct = 90;                // Wear throttle threshold (0-100)
    double wearWeight = 1.5;                // Wear influence in drive scoring
    double tempWeight = 1.0;                // Temperature influence in drive scoring
};
...
public:
    ...
    std::vector<uint32_t> rankDrives(const std::vector<ThermalSample>& thermals) const;
...
std::vector<uint32_t> VRAMGovernor::rankDrives(const std::vector<ThermalSample>& thermals) const {
    struct DriveScore {
        uint32_t driveId;
        double score;
    };
    ...
    for (const auto& t : thermals) {
        ...
        scores.push_back({t.driveId, score});
    }
...
void SovereignHarness::stressNVMe() {
    ...
    auto driveOrder = governor_.rankDrives(thermals);
    if (driveOrder.empty()) {
