...
public:
    ...
    std::vector<ThermalSample> poll() const {
        ...
        std::cerr << "[DEBUG] Calling NVMe_PollAllDrives with " << drives_.size() << " drives..." << std::endl;
        NVMe_PollAllDrives(temps.data(), drives_.data(), static_cast<uint32_t>(drives_.size()));
        std::cerr << "[DEBUG] NVMe_PollAllDrives returned" << std::endl;
        
        for (size_t i = 0; i < drives_.size(); ++i) {
            samples.push_back({drives_[i], temps[i]});
        }
        return samples;
    }
