...
public:
    struct DriveStatus {
        int32_t driveId;
        int32_t tempC;     // -1 unknown
        int32_t wearPct;   // -1 unknown
        double score;
        bool blacklisted;
    };

    SovereignDriveRanker(double tempWeight, double wearWeight, int32_t maxWearPct)
        : tempWeight_(tempWeight), wearWeight_(wearWeight), maxWearPct_(maxWearPct) {}
    ...
    std::vector<DriveStatus> rankDrives(const std::vector<ThermalSample>& thermals) const {
        ...
        for (const auto& t : thermals) {
            bool blacklisted = (t.wearPct >= 0 && t.wearPct > maxWearPct_);
            ...
            ranked.push_back({static_cast<int32_t>(t.driveId), t.tempC, t.wearPct, score, blacklisted});
        }
