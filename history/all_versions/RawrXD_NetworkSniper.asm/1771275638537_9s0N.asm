...
; -----------------------------------------------------------------------------
; Sniper Context (per-connection)
; -----------------------------------------------------------------------------
SniperContext STRUCT
    Type                DD      ?
    PatternPtr          DQ      ?       ; Pattern to match (e.g., "password=")
    PatternLen          DD      ?
    ReplacementPtr      DQ      ?       ; For MUTATE type
    ReplacementLen      DD      ?
    HitCount            DQ      ?
    ByteCounter         DQ      ?       ; Atomic
    Flags               DD      ?
SniperContext ENDS
...
g_SniperActiveMask      DQ      0           ; Bitmask of active snipers
...
@@handle_inspect:
    ...
    mov     r8, [rsi + 8]               ; PatternPtr
    mov     r9d, [rsi + 16]             ; PatternLen
    call    Sniper_ScanPattern
    test    rax, rax
    jz      @@sniper_loop

    ; Pattern found - increment hit counter
    lock inc QWORD PTR [rsi + 24]       ; HitCount
    jmp     @@sniper_loop
...
@@handle_mutate:
    ...
    mov     r8, [rsi + 8]               ; PatternPtr
    mov     r9d, [rsi + 16]             ; PatternLen
    mov     rax, [rsi + 24]             ; ReplacementPtr
    mov     ebx, [rsi + 32]             ; ReplacementLen
...
@@handle_mirror:
    ...
    mov     rcx, [rsi + 8]              ; Mirror buffer ptr (reusing PatternPtr)
    mov     rdx, r10
    mov     r8d, r11d
...
@@search_loop:
    ...
    movzx   r10d, BYTE PTR [rsi + r9 + rcx - 1]
    cmp     r10d, edx
    jne     @@next_offset
