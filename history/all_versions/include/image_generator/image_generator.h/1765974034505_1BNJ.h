...
bool write_bmp(const Canvas& canvas, const std::string& path) {
...
Canvas create_layer(int width, int height, const Color& clear_color = Color::transparent()) {
    Canvas layer(width, height);
    layer.clear(clear_color);
    return layer;
}

void composite_layer(Canvas& destination, const Canvas& source, int x = 0, int y = 0) {
    destination.composite(source, x, y);
}

// ======================== Filters (Basic) ========================

Canvas apply_brightness(const Canvas& src, float factor) {
...
Canvas apply_contrast(const Canvas& src, float factor) {
    Canvas result(src.width, src.height);
    for (int y = 0; y < src.height; ++y) {
        for (int x = 0; x < src.width; ++x) {
            Color c = src.get(x, y);
...
Canvas apply_saturation(const Canvas& src, float factor) {
    Canvas result(src.width, src.height);
    for (int y = 0; y < src.height; ++y) {
        for (int x = 0; x < src.width; ++x) {
            Color c = src.get(x, y);
...
Canvas apply_blur(const Canvas& src, int radius) {
    Canvas result(src.width, src.height);
