...
inline QByteArray decompress(const QByteArray& compressed)
{
    ...
    const unsigned char* deflate_ptr = data + header_size;
    const size_t deflate_len = data_len - header_size - 8;

    z_stream stream{};
    stream.next_in = const_cast<Bytef*>(reinterpret_cast<const Bytef*>(deflate_ptr));
    stream.avail_in = static_cast<uInt>(deflate_len);
    ...
    std::vector<unsigned char> output(max_uncompressed);
    ...
    inflateEnd(&stream);
    free(out_buf);
    QByteArray decompressed(reinterpret_cast<const char*>(output.data()), static_cast<int>(out_len));
    return decompressed;
#endif
