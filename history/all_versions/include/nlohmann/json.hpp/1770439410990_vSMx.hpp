...
public:
    ...
    // ========================================================================
    // Polymorphic iteration — works for both arrays and objects.
    // For objects: yields std::pair<const std::string, json>.
    // For arrays:  yields json values via a thin index-based iterator.
    // ========================================================================

    // --- Array const_iterator (index-based wrapper) -------------------------
    struct array_const_iterator {
        const std::vector<json>* arr;
        size_t idx;
        array_const_iterator(const std::vector<json>* a, size_t i) : arr(a), idx(i) {}
        const json& operator*() const { return (*arr)[idx]; }
        const json* operator->() const { return &(*arr)[idx]; }
        array_const_iterator& operator++() { ++idx; return *this; }
        bool operator!=(const array_const_iterator& o) const { return idx != o.idx; }
        bool operator==(const array_const_iterator& o) const { return idx == o.idx; }
    };
    ...
    struct array_iterator {
        std::vector<json>* arr;
        size_t idx;
        array_iterator(std::vector<json>* a, size_t i) : arr(a), idx(i) {}
        json& operator*() { return (*arr)[idx]; }
        json* operator->() { return &(*arr)[idx]; }
        array_iterator& operator++() { ++idx; return *this; }
        bool operator!=(const array_iterator& o) const { return idx != o.idx; }
        bool operator==(const array_iterator& o) const { return idx == o.idx; }
    };

    // --- Iteration for objects (map iterators) ------------------------------
    //     NOTE: for objects, range-for yields std::pair<const string, json>.
    //     For arrays, range-for yields json&.
    //     Callers that iterate objects and need key/value should use items().

    // Non-const begin/end
    auto begin() {
        if (m_type == array)  return std::variant<std::map<std::string,json>::iterator, array_iterator>{array_iterator(&m_array, 0)};
        if (m_type == object) return std::variant<std::map<std::string,json>::iterator, array_iterator>{m_object.begin()};
        throw std::runtime_error("Cannot iterate non-container");
    }

    // --- Simplified iteration: expose typed accessors -----------------------
    // Object iteration: for (auto it = obj.obj_begin(); it != obj.obj_end(); ++it)
    //                   it->first = key, it->second = value
    auto obj_begin()       { return m_object.begin(); }
    auto obj_end()         { return m_object.end();   }
    auto obj_begin() const { return m_object.begin(); }
    auto obj_end()   const { return m_object.end();   }

    // Array iteration: for (const auto& item : arr.arr_items())
    const std::vector<json>& arr_items() const {
        if (m_type != array) {
    ...
    std::vector<json>& arr_items() {
        if (m_type != array) throw std::runtime_error("arr_items() on non-array");
        return m_array;
    }

    // items() — for object key-value iteration: for (const auto& [k,v] : obj.items())
    ...
    // static array() factory — compatible with nlohmann::json::array()
    static json array() {
        return array_type();
    }
