...
// ─────────────────────────────────────────────────────────────────────────────
// 3. Standard library includes needed to back the stubs
// ─────────────────────────────────────────────────────────────────────────────
#include <string>
#include <vector>
#include <map>
...
// ─────────────────────────────────────────────────────────────────────────────
// 4. Qt Type Stubs — Empty/minimal structs that let code compile
// ─────────────────────────────────────────────────────────────────────────────
...
public:
    ...
    QString toLower() const { auto s = d; for(auto& c:s) c=(char)tolower((unsigned char)c); return s; }
    QString toUpper() const { auto s = d; for(auto& c:s) c=(char)toupper((unsigned char)c); return s; }
    ...
    QString arg(const T& val) const {
        ...
        if (pos != std::string::npos) {
            std::string rep;
            if constexpr (std::is_same_v<T, QString>) rep = val.d;
            else if constexpr (std::is_same_v<T, std::string>) rep = val;
            else if constexpr (std::is_same_v<T, const char*>) rep = val;
            else if constexpr (std::is_arithmetic_v<T>) rep = std::to_string(val);
            else rep = "?";
            s.replace(pos, 2, rep);
        }
        return s;
...
public:
    ...
    bool isSpace() const { return isspace((unsigned char)c_) != 0; }
    bool isDigit() const { return isdigit((unsigned char)c_) != 0; }
    bool isLetter() const { return isalpha((unsigned char)c_) != 0; }
