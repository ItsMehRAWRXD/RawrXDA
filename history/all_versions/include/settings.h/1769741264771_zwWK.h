...
#include <string>
#include <vector>
#include "common_types.h"  // ChatMessage, DownloadProgress definitions
...
struct AppState {
    ...
    int32_t thermal_headroom_c = 0;
    
    // Telemetry / live hardware state
    ...
    int32_t baseline_stable_offset_mhz = 0;
    ...
    bool show_model_browser_window = false;
    bool show_settings_window = false;
    bool show_download_window = false;
    
    // Download state - stored as raw struct to avoid heap allocation
    // Note: DownloadProgress must be defined before including this header
    // or use incomplete type (which gui.cpp handles via includes)
    struct {
    ...
    // Chat history - stores messages inline
    struct ChatMsg {
        std::string role;
        std::string content;
        std::string name;
    };
    std::vector<ChatMsg> chat_history;
