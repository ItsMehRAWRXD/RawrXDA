...
public:
    ...
    // ---- Registration ----
    // Called once at startup to probe which engines are available
    void registerAvailableEngines();

    // ---- Enumeration ----
    int                     getEngineCount() const;
    EditorEngineInfo        getEngineInfo(int index) const;
    EditorEngineInfo        getEngineInfo(EditorEngineType type) const;
    bool                    isEngineAvailable(EditorEngineType type) const;

    // ---- Factory ----
    // Create a new engine instance. Caller owns the returned pointer.
    IEditorEngine*          createEngine(EditorEngineType type);

    // ---- Active Engine Management ----
    // The "active" engine is the one currently driving the editor area.
    // switchEngine() handles:
    //   1. Getting content from current engine
    //   2. Hiding current engine
    //   3. Creating/showing new engine
    //   4. Setting content in new engine
    //   5. Applying current theme
    EditorEngineResult      switchEngine(EditorEngineType newType,
                                         HWND parentWindow,
                                         const IDETheme& currentTheme);

    IEditorEngine*          getActiveEngine() const;
    EditorEngineType        getActiveEngineType() const;

    // ---- Preferred Engine ----
    // Set/get the user's preferred engine (persisted in config)
    void                    setPreferredEngine(EditorEngineType type);
    EditorEngineType        getPreferredEngine() const;
...
// ============================================================================
#define IDM_EDITOR_ENGINE_RICHEDIT      9200
#define IDM_EDITOR_ENGINE_WEBVIEW2      9201
#define IDM_EDITOR_ENGINE_MONACOCORE    9202
#define IDM_EDITOR_ENGINE_CYCLE         9203    // Cycle to next available
#define IDM_EDITOR_ENGINE_STATUS        9204    // Show engine info dialog

#endif // Intentionally no include guard closure — #pragma once handles it
