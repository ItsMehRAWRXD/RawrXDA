...
#if defined(HAVE_VULKAN) && defined(__has_include)
#if __has_include(<vulkan/vulkan.h>)
#define VULKAN_COMPUTE_HAS_HEADERS 1
#include <vulkan/vulkan.h>
#else
#define VULKAN_COMPUTE_HAS_HEADERS 0
#endif
...
public:
    ...
    // Convenience helper: host pointers in, GPU dispatch out (synchronous)
    bool DispatchMatMulHost(const float* input_a,
                            const float* input_b,
                            float* output,
                            uint32_t M,
                            uint32_t K,
