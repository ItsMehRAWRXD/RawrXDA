...
extern "C" void matmul_kernel_avx512(float* A, float* B, float* C, int N, int M, int K, bool accumulate = false);
...
static inline bool cpu_has_avx512_rt() {
#if defined(_MSC_VER)
    int cpuInfo[4] = {0};
...
#else
    // Use larger tiles for AVX-512 (128x128x128)
    ...
    for (int i0 = 0; i0 < M; i0 += TM) {
        ...
        for (int j0 = 0; j0 < N; j0 += TN) {
            ...
            for (int k0 = 0; k0 < K; k0 += TK) {
                ...
                alignas(64) uint8_t q4_panel[(TN * TK) / 2] = {0};
                ...
                q4_0_unpack_64x64_avx512(q4_panel, Btile, scale);
                ...
                matmul_kernel_avx512(Ablk.data(), Bblk.data(), Cblk.data(), Mb, Kb, Nb, false);
                for (int ii = 0; ii < Mb; ++ii) {
