...
MainDispatcher PROC
    ...
    ; Task 17: Bounds check — valid modes are 1..13
    cmp rax, 13
    ja InvalidMode
    cmp rax, 1
    jb InvalidMode
...
ShowGUIMenu:
    ...
    cmp rax, 0
    je ExitProgram
    ; Task 17: Bounds check for GUI selection (valid: 1-13)
    cmp rax, 13
    ja gui_invalid_mode
    cmp rax, 1
    je gui_compile
...
bbval_check_padding:
    ; ---- Check 4: Block is all INT3/NOP (padding block) ----
    ; Only check blocks with size > 0 and size <= 64 (small blocks)
    mov eax, dword ptr [r12+16]
    test eax, eax
    jz bbval_store_flags
    cmp eax, 64
