...
.data
    ...
    szBBCovHeader   db "{\"basic_block_coverage\":{\"version\":1,",0
    szBBCovSummary  db "\"summary\":{\"total_blocks\":%d,\"text_base\":\"0x%I64X\",\"text_size\":%d},",0
    szBBCovBlocks   db "\"blocks\":[",0
    szBBCovEntry    db "{\"id\":%d,\"offset\":\"0x%I64X\",\"size\":%d,\"type\":\"%s\"},",0
    szBBCovEntryLast db "{\"id\":%d,\"offset\":\"0x%I64X\",\"size\":%d,\"type\":\"%s\"}",0
    szBBCovFooter   db "]}}",0
...
MainDispatcher PROC
    ...
    ; Task 17: Bounds check — valid modes are 1..12
    cmp rax, 12
    ja InvalidMode
    cmp rax, 1
    jb InvalidMode
...
ShowGUIMenu:
    ...
    cmp rax, 0
    je ExitProgram
    ; Task 17: Bounds check for GUI selection (valid: 1-12)
    cmp rax, 12
    ja gui_invalid_mode
    cmp rax, 1
    je gui_compile
...
bbcov_scan_done:
    ...
    ; Format: "BBCov: Found %d basic blocks in %d bytes of .text"
    lea rcx, OutputBuffer
    lea rdx, szBBCovDone
    mov r8d, r12d                     ; block count
    mov r9d, r15d                     ; text size
    call wsprintfA
    lea rcx, OutputBuffer
    call Print
    lea rcx, OutputBuffer
...
bbsort_last_block:
    cmp r15d, 0
    je bbsort_done
    ; Last block: size = text_size - offset
    lea rbx, BBBlockTable
    lea eax, [r15d - 1]
    imul eax, 24
    cdqe
...
BBGenerateJSON PROC
    ...
    push r14
    push r15
    sub rsp, 48h    ; 5 pushes(40) + push rbp(8) + 48h(72) = 120 = 8 mod 16 → aligned

    ; Start building JSON in FileBuffer
    lea rbx, FileBuffer

    ; Header: {"basic_block_coverage":{"version":1,
    mov rcx, rbx
    lea rdx, szBBCovHeader
    call lstrcpyA

    ; Summary: "summary":{"total_blocks":%d,"text_base":"0x%I64X","text_size":%d},
    lea rcx, OutputBuffer
    lea rdx, szBBCovSummary
    mov r8d, dword ptr [BBBlockCount]   ; total_blocks
    mov r9, BBTextBase                  ; text_base (RVA)
    ; text_size is 5th arg — push on stack
    mov eax, dword ptr [BBTextSize]
    mov dword ptr [rsp+20h], eax        ; 5th arg
    call wsprintfA
    ...
    ; Iterate block table and append entries
    mov r15d, dword ptr [BBBlockCount]
...
bbgen_block_loop:
    ...
    ; Get offset, size, type
    mov r14d, dword ptr [r13]          ; offset
    mov ecx, dword ptr [r13+16]        ; size
    ...
    lea rcx, OutputBuffer
    lea rdx, szBBCovEntry
    mov r8d, r12d                      ; id
    movzx r9d, r14w                    ; offset (use full value)
    mov r9d, r14d                      ; offset
