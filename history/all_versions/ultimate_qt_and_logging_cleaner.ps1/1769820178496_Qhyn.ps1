...
foreach ($file in $files) {
    ...
    $content = $content -replace '(\w+->)?logger->[^;]+;', ''
    $content = $content -replace 'LOG_(INFO|DEBUG|ERROR|WARN|TRACE)\([^;]+\);?', ''
    $content = $content -replace '(GGML|VK)_LOG_(INFO|DEBUG|ERROR|WARN|TRACE)\([^;]+\);?', ''
    $content = $content -replace 'fprintf\(stderr, [^;]+\);?', ''
    $content = $content -replace 'std::cerr\s*<<[^;]+;', ''
    $content = $content -replace 'std::cout\s*<<[^;]+;', ''
