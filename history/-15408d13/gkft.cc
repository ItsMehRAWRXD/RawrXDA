...
#ifdef __AVX2__
// Highly optimized AVX2 flash-attention with aggressive optimization hints
__attribute__((always_inline, hot, optimize("O3", "unroll-loops")))
static inline void flash_attn_optimized_avx2(
    ...
    for (int q_idx = 0; q_idx < seq_len; q_idx += 3) {
        ...
        for (int q_local = q_idx; q_local < q_end; ++q_local) {
            const float* __restrict__ q_row = Q + q_local * head_dim;
            float* __restrict__ out_row = O + q_local * head_dim;
            ...
            for (int k_idx = 0; k_idx < seq_len; ++k_idx) {
                const float* __restrict__ k_row = K + k_idx * head_dim;
                const float* __restrict__ v_row = V + k_idx * head_dim;
