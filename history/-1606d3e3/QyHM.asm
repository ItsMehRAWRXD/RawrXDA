...
IDM_EDIT_RENAME     equ 1112
IDM_EDIT_REFERENCES equ 1113
IDM_EDIT_DEFINITION equ 1114
IDM_EDIT_PEEK       equ 1115
...
InitializeIDE proc uses ebx esi edi
    ; Initialize Editor Control Class
    invoke InitEditorControl, ideState.hInstance
...
OnCreate proc uses ebx esi edi hWnd:DWORD
    ...
    invoke CreateWindowEx, 0, CSTR("STATIC"), NULL, \
                          WS_CHILD or WS_VISIBLE or SS_LEFT, \
                          50, 0, rect.right - 170, 20, \
                          hWnd, NULL, ideState.hInstance, NULL
    mov ideState.hBreadcrumbs, eax
    invoke SendMessage, eax, WM_SETFONT, ideState.hMenu, TRUE ; Use menu font
    
    ; Get client area
    invoke GetClientRect, hWnd, addr rect
    
    ; Create edit control
    invoke CreateWindowEx, WS_EX_CLIENTEDGE, CSTR("RawrXDEditor"), NULL, \
                          WS_CHILD or WS_VISIBLE or WS_VSCROLL or WS_HSCROLL, \
                          50, 20, rect.right - 170, rect.bottom - 200, \
    ...
    invoke CreateWindowEx, WS_EX_CLIENTEDGE, CSTR("LISTBOX"), NULL, \
                          ...
                          50, rect.bottom - 180, rect.right - 170, 160, \
                          hWnd, NULL, ideState.hInstance, NULL
    mov ideState.hDebugPane, eax
...
OnSize proc uses ebx esi edi hWnd:DWORD, wParam:DWORD, lParam:DWORD
    ...
    ; Resize edit control
    invoke GetClientRect, hWnd, addr rect
    .if ideState.hBreadcrumbs != 0
        invoke MoveWindow, ideState.hBreadcrumbs, 50, 0, \
                          rect.right - 170, 20, TRUE
    .endif
    .if ideState.hEditControl != 0
        invoke MoveWindow, ideState.hEditControl, 50, 20, \
                          rect.right - 170, rect.bottom - 200, TRUE
    .endif
    .if ideState.hDebugPane != 0
        invoke MoveWindow, ideState.hDebugPane, 50, rect.bottom - 180, \
                          rect.right - 170, 160, TRUE
    .endif
...
OnCommand proc uses ebx esi edi hWnd:DWORD, wParam:DWORD, lParam:DWORD
    ...
    .elseif eax == IDM_EDIT_RENAME
        invoke DoRenameSymbol
        
    .elseif eax == IDM_EDIT_REFERENCES
        invoke DoFindReferences
        
    .elseif eax == IDM_EDIT_DEFINITION
        invoke DoGoToDefinition
        
    .elseif eax == IDM_EDIT_PEEK
        invoke DoPeekDefinition
...
CreateMainMenu proc uses ebx esi edi
    ...
    invoke AppendMenu, hSubMenu, MF_SEPARATOR, 0, NULL
    invoke AppendMenu, hSubMenu, MF_STRING, IDM_EDIT_RENAME, CSTR("Rena&me Symbol\tF2")
    invoke AppendMenu, hSubMenu, MF_STRING, IDM_EDIT_REFERENCES, CSTR("Find All &References\tShift+F12")
    invoke AppendMenu, hSubMenu, MF_STRING, IDM_EDIT_DEFINITION, CSTR("&Go to Definition\tF12")
    invoke AppendMenu, hSubMenu, MF_STRING, IDM_EDIT_PEEK, CSTR("&Peek Definition\tAlt+F12")
...
UpdateDebugUI proc text:DWORD
    .if ideState.hDebugPane != 0
        invoke SendMessage, ideState.hDebugPane, LB_ADDSTRING, 0, text
        invoke SendMessage, ideState.hDebugPane, LB_SETCURSEL, -1, 0 ; Scroll to bottom
    .endif
    ret
UpdateDebugUI endp
...
;==============================================================================
; MONACO FEATURE HANDLERS
;==============================================================================
DoRenameSymbol proc uses ebx esi edi
    local pos:LSP_POSITION
    local newName[64]:BYTE
    ...
    ; Prompt for new name
    ; invoke InputBox, ...
    invoke lstrcpy, addr newName, CSTR("new_symbol_name")
    
    invoke LSP_Rename, pos, addr newName
    .if eax > 0
        invoke UpdateStatusBar, CSTR("Symbol renamed successfully")
        invoke InvalidateRect, ideState.hEditControl, NULL, TRUE
    .else
        invoke UpdateStatusBar, CSTR("Rename failed: No symbol at cursor")
    .endif
    ret
DoRenameSymbol endp

DoFindReferences proc uses ebx esi edi
    local pos:LSP_POSITION
    
    mov pos.line, 0
    mov pos.character, 0
    
    invoke LSP_FindReferences, pos
    .if eax > 0
        invoke UpdateStatusBar, CSTR("References found (see output pane)")
    .else
        invoke UpdateStatusBar, CSTR("No references found")
    .endif
    ret
DoFindReferences endp

DoGoToDefinition proc uses ebx esi edi
    local pos:LSP_POSITION
    
    mov pos.line, 0
    mov pos.character, 0
    
    invoke LSP_Definition, pos
    .if eax != NULL
        mov esi, eax
        assume esi:ptr LSP_LOCATION
        ; Scroll to definition
        invoke UpdateStatusBar, CSTR("Jumping to definition...")
    .else
        invoke UpdateStatusBar, CSTR("Definition not found")
    .endif
    ret
DoGoToDefinition endp

DoPeekDefinition proc uses ebx esi edi
    local pos:LSP_POSITION
    local rect:RECT
    ...
    .if eax != NULL
        ...
        ; Create a peek window
        invoke GetWindowRect, ideState.hEditControl, addr rect
        invoke CreateWindowEx, WS_EX_TOPMOST, CSTR("RawrXDEditor"), NULL, \
                              WS_POPUP or WS_VISIBLE or WS_BORDER, \
                              rect.left + 50, rect.top + 100, \
                              rect.right - rect.left - 100, 200, \
                              ideState.hMainWnd, NULL, ideState.hInstance, NULL
        invoke UpdateStatusBar, CSTR("Peek view opened")
    .else
        invoke UpdateStatusBar, CSTR("Definition not found")
    .endif
    ret
DoPeekDefinition endp
