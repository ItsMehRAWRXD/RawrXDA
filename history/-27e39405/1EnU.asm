...
quantize_loop:
    ...
    vmovss xmm0, dword ptr [r12]    ; load first element
    vmovss xmm1, xmm0                ; min = first
    vmovss xmm2, xmm0                ; max = first
...
find_minmax_loop:
    ...
    vmovss xmm3, dword ptr [r12 + rsi*4]
...
find_minmax_done:
    ...
    vsubss xmm2, xmm2, xmm1          ; max - min
    vmovss xmm3, xmm2
    vdivss xmm3, xmm3, xmmword ptr [fifteen]  ; / 15
    vmovss dword ptr [r15 + r11*4], xmm3      ; store scale
...
quantize_block_loop:
    ...
    vmovups ymm0, ymmword ptr [r12 + rsi*4]    ; load 8 floats
    vsubps ymm0, ymm0, xmm1                    ; subtract min
    vdivps ymm0, ymm0, xmm3                    ; divide by scale

    ; Convert to int and clamp to 0-15
    vcvtps2dq ymm0, ymm0
    vpminsd ymm0, ymm0, ymmword ptr [fifteen_vec]
    vpmaxsd ymm0, ymm0, ymmword ptr [zero_vec]
...
dequantize_loop:
    ...
    vmovss xmm3, dword ptr [r15 + r11*4]    ; scale
...
dequantize_block_loop:
    ...
    ; Store results
    vmovss dword ptr [r13 + rsi*4], xmm0
    cmp rsi, rbx
    jge skip_second
    vmovss dword ptr [r13 + rsi*4 + 4], xmm1
    inc rsi
