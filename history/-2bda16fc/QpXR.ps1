...
# 4) Load codemodel json (first config)
$cmFile = $indexJson.reply.codemodel[0].jsonFile
if (-not $cmFile) { throw "codemodel jsonFile not present in index reply." }
...
foreach ($t in $targets) {
  $md += "- Name: `$($t.name)` — Type: $($t.type) — Langs: $([string]::Join(',', $t.languages)) — Srcs: $($t.sources.Count)"
}
