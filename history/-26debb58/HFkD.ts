...
// GLOBAL MODULE-LEVEL FUNCTIONS (NOT NESTED)
...
async function gatherWorkspaceContext(): Promise<void> {
  console.log('[BigDaddyG] ========================================');
  console.log('[BigDaddyG] GATHERING WORKSPACE CONTEXT');
  console.log('[BigDaddyG] ========================================');
  ...
  try {
    ...
    if (editor) {
      ...
      if (!selection.isEmpty) {
        contextCache.code = editor.document.getText(selection);
        console.log(`[BigDaddyG] Selected code: ${contextCache.code.length} characters (lines ${selection.start.line}-${selection.end.line})`);
      } else {
        contextCache.code = editor.document.getText();
        console.log(`[BigDaddyG] Full document: ${contextCache.code.length} characters`);
      }
    }
    ...
    if (vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
      ...
      contextCache.files = files.map(f => vscode.workspace.asRelativePath(f)).join(', ');
      console.log(`[BigDaddyG] Workspace files: ${files.length} files found`);
    }
    ...
    console.log(`[BigDaddyG] Diagnostics: ${diagCount} issues found`);

    contextCache.workspace = `Files: ${contextCache.files.split(',').length}, Code: ${contextCache.code.length}chars, Issues: ${diagCount}`;
    console.log('[BigDaddyG] Context gathering complete');
    console.log('[BigDaddyG] ========================================');
  } catch (e: any) {
    console.error('[BigDaddyG] ERROR gathering context:', e);
  }
}
