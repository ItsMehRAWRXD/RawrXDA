...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('bigdaddyg.enableInterceptor', async () => {
      ...
      if (!model) return;
      interceptorConfig = { enabled: true, endpoint, backend: backend as any, model };
      rebuildInterceptor();
...
function rebuildInterceptor() {
  if (!interceptorConfig.enabled) {
    ...
    console.log('[BigDaddyG] Interceptor disabled, restored original http/https');
    return;
  }

  http.request = ((url: any, options: any, callback?: any) => {
    const isChat = detectChatRequest(url, options);
    if (isChat && interceptorConfig.enabled && interceptorConfig.model) {
      console.log('[BigDaddyG] Intercepting HTTP request:', { url, isChat });
  ...
  https.request = ((url: any, options: any, callback?: any) => {
    const isChat = detectChatRequest(url, options);
    if (isChat && interceptorConfig.enabled && interceptorConfig.model) {
      console.log('[BigDaddyG] Intercepting HTTPS request:', { url, isChat });
      return proxyToLocal(interceptorConfig.endpoint, interceptorConfig.backend, interceptorConfig.model, url, options, callback);
    }
    return httpsOriginal(url, options, callback);
  }) as any;

  console.log('[BigDaddyG] Interceptor hooks installed');
}
...
function proxyToLocal(endpoint: string, backend: string, model: string, url: any, options: any, callback?: any): http.ClientRequest {
  ...
  const req = http.request(endpoint, { method: 'POST', path: urlForBackend(backend), headers: { 'Content-Type': 'application/json' } }, callback);
...
function transformRequestBody(body: any, backend: string, model: string): any {
  ...
  if (backend === 'ollama-generate') {
    ...
    return { model, messages: body.messages || [{ role: 'user', content: prompt }], stream: false };
