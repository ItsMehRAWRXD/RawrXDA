...
function openChatPanel(mode: 'open' | 'beacon') {
  ...
  panel.webview.onDidReceiveMessage(async (msg) => {
    ...
    if (msg.type === 'send') {
      ...
      console.log('[BigDaddyG] Sending to Ollama:', { model, promptLen: prompt.length, agentMode, ideAccess });

      try {
        const fullPrompt = buildAgentPreface(agentMode) + "\n\n" + prompt;
        panel.webview.postMessage({ type: 'responseStart' });
        let collected = '';

        await streamQueryWithFallback(model, fullPrompt, (chunk) => {
          collected += chunk;
...
function send() {
  ...
  console.log('[UI] Sending:', { model, promptLen: prompt.length, agentMode, ideAccess });
  vscode.postMessage({ type: 'send', model, prompt, mode: agentMode, ideAccess });
}
...
// Streaming helpers
async function streamQueryWithFallback(model: string, prompt: string, onChunk: (t: string) => void): Promise<void> {
  // 1) Ollama /api/generate (stream)
  try {
    await streamEndpoint('/api/generate', { model, prompt, stream: true }, onChunk, parseOllamaGenerateLine);
    return;
  } catch (e: any) {
    console.warn('[Ollama] stream /api/generate failed:', e.message);
  }

  // 2) Ollama /api/chat (stream)
  try {
    await streamEndpoint('/api/chat', { model, messages: [{ role: 'user', content: prompt }], stream: true }, onChunk, parseOllamaChatLine);
    return;
  } catch (e: any) {
    console.warn('[Ollama] stream /api/chat failed:', e.message);
  }

  // 3) OpenAI-compatible /v1/chat/completions (stream)
  try {
    await streamEndpoint('/v1/chat/completions', { model, messages: [{ role: 'user', content: prompt }], stream: true }, onChunk, parseOpenAIStreamLine);
    return;
  } catch (e: any) {
    console.warn('[Ollama] stream /v1/chat/completions failed:', e.message);
  }

  // As a last resort, do non-streaming
  const fallback = await queryOllama(model, prompt);
...
function streamEndpoint(pathname: string, payload: any, onChunk: (t: string) => void, parseLine: LineParser): Promise<void> {
  return new Promise((resolve, reject) => {
    const url = new URL(`${OLLAMA_ENDPOINT}${pathname}`);
    ...
    console.log('[Ollama] STREAM POST', options.hostname + ':' + options.port + options.path);
