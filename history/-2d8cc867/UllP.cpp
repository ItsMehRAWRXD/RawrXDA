...
CommandResult handleFileSave(const CommandContext& ctx) {
    if (ctx.isGui && ctx.idePtr) {
        ...
        if (registry) {
            auto handler = registry->getHandler(SubsystemId::Editor);
            if (handler) {
                // Attempt to get buffer content from editor subsystem
                CommandContext bufCtx = ctx;
                bufCtx.args = "get_buffer";
                auto bufResult = handler(bufCtx);
