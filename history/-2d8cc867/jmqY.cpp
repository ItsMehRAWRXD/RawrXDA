...
CommandResult handleFileSave(const CommandContext& ctx) {
    if (ctx.isGui && ctx.idePtr) {
        ...
        // Write buffer content (placeholder for editor buffer integration)
        const char* placeholder = "";
        DWORD written = 0;
        WriteFile(h, placeholder, 0, &written, nullptr);
        CloseHandle(h);
...
CommandResult handleFileLoadModel(const CommandContext& ctx) {
    ...
    RAWRXD_INVOKE(SubsystemId::ModelManagement, "load", ctx.args);
...
CommandResult handleCOT(const CommandContext& ctx) {
    ...
    if (cotEnabled) {
        ...
        // Suppress premature termination tokens, boost verbose reasoning markers
        TokenBias thinkBias{};
