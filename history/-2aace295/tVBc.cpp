...
bool TaskOrchestrator::shouldDecomposeFurther(const TaskDefinition& task) const
{
    return task.estimatedTokens > 1000;
}

qint64 TaskOrchestrator::calculateMemoryForTask(const TaskDefinition& task) const
{
    // Simple calculation: ~200 bytes per token + base overhead
    return (task.estimatedTokens * 200) + 1048576; // Base 1MB + token-based
}

void TaskOrchestrator::balanceWorkload(QList<TaskDefinition>& tasks)
{
    // Redistribute tasks to balance workload across models
    if (tasks.isEmpty() || m_modelWorkloads.isEmpty()) return;
    
    for (TaskDefinition& task : tasks) {
        // Find model with least current load that supports task type
        QString leastLoadedModel;
        int minLoad = INT_MAX;
        
        for (const QString& model : m_modelCapabilities.keys()) {
            if (m_modelCapabilities[model].contains(task.type) || 
                m_modelCapabilities[model].contains("general")) {
                int currentLoad = m_modelWorkloads.value(model, 0);
                if (currentLoad < minLoad) {
                    minLoad = currentLoad;
                    leastLoadedModel = model;
                }
            }
        }
        
        if (!leastLoadedModel.isEmpty()) {
            m_modelWorkloads[task.model]--;
            task.model = leastLoadedModel;
            m_modelWorkloads[leastLoadedModel]++;
        }
    }
}
