...
void TaskOrchestrator::createExecutionTab(const TaskDefinition& task)
{
    ...
    m_activeTasks[task.id].parameters["progressBar"] = QVariant::fromValue<quintptr>(reinterpret_cast<quintptr>(progressBar));
    m_activeTasks[task.id].parameters["contentArea"] = QVariant::fromValue<quintptr>(reinterpret_cast<quintptr>(contentArea));
}
...
void TaskOrchestrator::handleModelResponse(QNetworkReply* reply, const QString& taskId)
{
    ...
    if (m_activeTasks[taskId].parameters.contains("contentArea")) {
        const quintptr ptr = m_activeTasks[taskId].parameters["contentArea"].value<quintptr>();
        QTextEdit* contentArea = reinterpret_cast<QTextEdit*>(ptr);
        if (contentArea) {
    ...
    if (m_activeTasks[taskId].parameters.contains("progressBar")) {
        const quintptr ptr = m_activeTasks[taskId].parameters["progressBar"].value<quintptr>();
