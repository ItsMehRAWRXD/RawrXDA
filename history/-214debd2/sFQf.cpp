...
QJsonArray Planner::plan(const QString& humanWish) {
    ...
    if (wish.contains("yourself") || wish.contains("itself") || wish.contains("clone") ||
        ...
        wish.contains("same thing it is") || wish.contains("recreate yourself")) {
        return planSelfReplication(humanWish);
    } else if (wish.contains("q8_k") || wish.contains("q6_k") || wish.contains("quant")) {
    ...
    } else if (wish.contains("react") || wish.contains("vue") || wish.contains("angular") || 
               wish.contains("express") || wish.contains("fastapi") || wish.contains("flask") ||
               wish.contains("server") || wish.contains("api") || wish.contains("web app")) {
...
QJsonArray Planner::planSelfReplication(const QString& wish) {
    ...
    tasks.append(QJsonObject{
        ...
        {"path", cloneName},
        {"description", "Create clone project directory"}
    });
...
)").arg(QDir::currentPath(), cloneName, QDateTime::currentDateTime().toString(Qt::ISODate))}
    });
...
It has full autonomous capabilities including self-replication.
    ...
    // Task 10: Optional - Run the clone with a test wish
    if (lowerWish.contains("run") || lowerWish.contains("start")) {
        tasks.append(QJsonObject{
            {"type", "set_environment"},
            {"variable", "RAWRXD_WISH"},
            {"value", "create a react app called clone-test"},
            {"scope", "process"}
        });
        ...
        tasks.append(QJsonObject{
            ...
            {"args", QJsonArray{}},
            {"background", true},
            {"description", "Start clone instance with test wish"}
        });
    }
