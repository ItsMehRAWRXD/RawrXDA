...
@no_font_size:
    ...
    mov rcx, rax
    call parse_float
    movss [rbx + STYLE.transition_duration], xmm0 ; Using transition_duration as proxy for opacity in style if needed
...
ParseStyleJson PROC
    ...
    mov rdi, rcx        ; style_struct
    mov rsi, rdx        ; json_str
    
    ; Initialize default values
    xor eax, eax
    mov [rdi + 0], eax  ; bg_color = 0
    mov [rdi + 4], eax  ; fg_color = 0xFFFFFFFF
    mov [rdi + 8], eax  ; border_color = 0
    mov dword ptr [rdi + 12], 1  ; border_width = 1
    mov dword ptr [rdi + 16], 4  ; border_radius = 4
    
    ; Find and parse bg_color
    lea rcx, [szBgColorKey_Val]
    mov rdx, rsi
    call find_json_key
    test rax, rax
...
; ParseLayoutJson(layout_buffer: rcx, size: edx) -> eax
; Parses pane layout JSON and restores pane positions
PUBLIC ParseLayoutJson
ALIGN 16
ParseLayoutJson PROC
    push rbx
    ...
    mov rsi, rcx        ; layout_buffer
    mov ebx, edx        ; size
    
    ; Initialize pane count to 0
    mov PaneCount, 0
    mov eax, 1          ; success
...
; FindCharInString(char: edx, string: rcx) -> rax
PUBLIC FindCharInString
ALIGN 16
FindCharInString PROC
    xor rax, rax
find_loop:
    mov cl, [rcx]
    test cl, cl
