...
enum class FeatureID : uint32_t {
    ...
    AirGappedDeploy         = 53,
    HSMIntegration          = 54,
    FIPS140_2Compliance     = 55,
    CustomSecurityPolicies  = 56,
    SovereignKeyMgmt        = 57,
    ClassifiedNetwork       = 58,
    TamperDetection         = 59,
    SecureBootChain         = 60,
...
namespace TierPresets {
    ...
    // Enterprise: bits 0–52
    constexpr FeatureMask Enterprise() {
        FeatureMask m;
        for (uint32_t i = 0; i <= 52; ++i) m.set(i);
        return m;
    }
    ...
    constexpr FeatureMask Sovereign() {
        FeatureMask m;
        for (uint32_t i = 0; i <= 60; ++i) m.set(i);
        return m;
    }
...
public:
    ...
    /// Bridge: accept enforcement events from LicenseEnforcer into V2 audit trail
    void logEnforcementEvent(FeatureID id, bool granted,
