...
std::vector<std::string> IntelligentCodebaseEngine::getCircularDependencies() {
    // Detect circular #include dependencies using DFS with gray/black coloring
    std::vector<std::string> cycles;
    enum Color { WHITE, GRAY, BLACK };
    std::unordered_map<std::string, Color> color;
    std::unordered_map<std::string, std::string> parent;

    for (auto& [file, _] : m_fileIndex) color[file] = WHITE;

    std::function<void(const std::string&)> dfs = [&](const std::string& u) {
        color[u] = GRAY;
        auto deps = getDependencies(u);
        for (auto& v : deps) {
            if (color.count(v) && color[v] == GRAY) {
                cycles.push_back(u + " -> " + v + " (cycle)");
            } else if (color.count(v) && color[v] == WHITE) {
                parent[v] = u;
                dfs(v);
            }
        }
