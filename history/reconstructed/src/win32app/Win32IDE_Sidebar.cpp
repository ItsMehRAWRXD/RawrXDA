...
void Win32IDE::createActivityBar(HWND hwndParent)
{
    ...
    RAWRXD_LOG("Activity Bar created with 5 views\n");
}
...
void Win32IDE::createPrimarySidebar(HWND hwndParent)
{
    ...
    RAWRXD_LOG("Primary Sidebar initialized\n");
}
...
void Win32IDE::toggleSidebar()
{
    ...
    RAWRXD_LOG(m_sidebarVisible ? "Sidebar shown (Ctrl+B)\n" : "Sidebar hidden (Ctrl+B)\n");
}
...
void Win32IDE::setSidebarView(SidebarView view)
{
    ...
    case SidebarView::Explorer:
        ...
        refreshFileTree();
        RAWRXD_LOG("Explorer view activated\n");
        break;
    ...
    case SidebarView::Search:
        ...
        RAWRXD_LOG("Search view activated\n");
        break;
    ...
    case SidebarView::Extensions:
        ...
        RAWRXD_LOG("Extensions view activated\n");
        break;
...
{
    RAWRXD_LOG("createExplorerView() called\n");
    ...
    if (!m_hwndExplorerToolbar) {
        RAWRXD_LOG("Failed to create explorer toolbar\n");
        return;
    }
    ...
    RAWRXD_LOG("Creating Explorer TreeView control\n");
    ...
    if (!m_hwndExplorerTree) {
        RAWRXD_LOG("Failed to create Explorer TreeView\n");
        return;
    }
    ...
    RAWRXD_LOG("Explorer view created with file tree at: " + m_explorerRootPath + "\n");
}

void Win32IDE::refreshFileTree()
{
    RAWRXD_LOG("refreshFileTree() called\n");
    if (!m_hwndExplorerTree) {
        RAWRXD_LOG("Cannot refresh file tree - m_hwndExplorerTree is null\n");
        return;
    }
    ...
    if (!hRoot) {
        RAWRXD_LOG("Failed to create tree root\n");
        return;
    }
    ...
    try {
        if (!fs::exists(m_explorerRootPath)) {
            RAWRXD_LOG("Explorer root path does not exist: " + m_explorerRootPath + "\n");
            return;
        }
        
        RAWRXD_LOG("Enumerating directory: " + m_explorerRootPath + "\n");
        ...
        RAWRXD_LOG("File tree refreshed successfully\n");
    }
    catch (const std::exception& e) {
        RAWRXD_LOG("Error refreshing file tree: " + std::string(e.what()) + "\n");
    }
...
void Win32IDE::expandFolder(const std::string& path)
{
    ...
    RAWRXD_LOG("Expanding folder: " + path + "\n");
}
...
void Win32IDE::collapseAllFolders()
{
    ...
    RAWRXD_LOG("All folders collapsed\n");
}
    ...
    if (ShowInputBox(m_hwndMain, &params, folderName)) {
        try {
            ...
            if (fs::create_directory(fullPath)) {
                ...
                RAWRXD_LOG("New folder created: " + fullPath + "\n");
            } else {
        ...
        try {
            ...
            refreshFileTree();
            RAWRXD_LOG("Folder created: " + newPath + "\n");
        }
        catch (const std::exception& e) {
            RAWRXD_LOG("Error creating folder: " + std::string(e.what()) + "\n");
        }
    }
}
...
void Win32IDE::deleteItemInExplorer()
{
    ...
    if (MessageBoxA(m_hwndMain, prompt.c_str(), "Confirm Delete", MB_YESNO | MB_ICONWARNING) == IDYES) {
        try {
            ...
            refreshFileTree();
            RAWRXD_LOG("Deleted: " + fullPath + "\n");
        }
        catch (const std::exception& e) {
            RAWRXD_LOG(std::string("Error deleting: ") + e.what() + "\n");
        }
    }
}
...
void Win32IDE::renameItemInExplorer()
{
    ...
    if (GetSaveFileNameA(&ofn)) {
        ...
        try {
            ...
            refreshFileTree();
            RAWRXD_LOG("Renamed: " + oldPath + " -> " + newPath + "\n");
        }
        catch (const std::exception& e) {
            RAWRXD_LOG(std::string("Error renaming: ") + e.what() + "\n");
        }
    }
}
...
void Win32IDE::revealInExplorer(const std::string& filePath)
{
    ...
    for (const auto& kv : m_treeItemPaths) {
        if (_stricmp(kv.second.c_str(), filePath.c_str()) == 0) {
            ...
            RAWRXD_LOG("Revealed in Explorer: " + filePath + "\n");
            return;
        }
    }
            ...
            if (hItem) {
                ...
                if (TreeView_GetItem(hwnd, &item)) {
                    if (item.lParam == 0) { // File, not folder
                        ...
                        pThis->RAWRXD_LOG("Opening file: " + filePath + "\n");
                    }
                }
            }
        }
...
void Win32IDE::createSearchView(HWND hwndParent)
{
    ...
    RAWRXD_LOG("Search view created with regex/case options\n");
}
                                      ...
                                      bool caseSensitive, bool wholeWord)
{
    ...
    RAWRXD_LOG("Searching for: '" + query + "'\n");
    ...
    try {
        ...
        RAWRXD_LOG("Search error: " + std::string(e.what()) + "\n");
    }

    m_searchInProgress = false;
    RAWRXD_LOG("Search complete: " + std::to_string(m_searchResults.size()) + " results\n");
}
    ...
    try {
        for (const auto& entry : fs::recursive_directory_iterator(m_explorerRootPath)) {
            ...
            if (count > 0) {
                ...
                RAWRXD_LOG("Replaced " + std::to_string(count) + " occurrences in " + entry.path().string() + "\n");
            }
        }
    }
    catch (const std::exception& e) {
        RAWRXD_LOG(std::string("Replace in files error: ") + e.what() + "\n");
        return;
    }

    RAWRXD_LOG("Replace complete: " + std::to_string(totalReplacements) + " total replacements\n");
}
...
void Win32IDE::createSourceControlView(HWND hwndParent)
{
    ...
    RAWRXD_LOG("Source Control view created with Git integration\n");
}
...
void Win32IDE::refreshSourceControlView()
{
    ...
    RAWRXD_LOG("Source Control refreshed: " + std::to_string(files.size()) + " changes\n");
}
...
void Win32IDE::createRunDebugView(HWND hwndParent)
{
    ...
    RAWRXD_LOG("Run and Debug view created\n");
}

void Win32IDE::createLaunchConfiguration()
{
    RAWRXD_LOG("Creating launch configuration...\n");
...
{
    RAWRXD_LOG("Step Over\n");
}

void Win32IDE::stepInto()
{
    RAWRXD_LOG("Step Into\n");
}
...
{
    RAWRXD_LOG("Continue Execution\n");
}

void Win32IDE::showDebugConsole()
{
    RAWRXD_LOG("Debug Console shown\n");
}
...
void Win32IDE::enableExtension(const std::string& extensionId)
{
    for (auto& ext : m_extensions) {
        if (ext.id == extensionId) {
            ext.enabled = true;
            RAWRXD_LOG("Extension enabled: " + extensionId + "\n");
            break;
        }
    }
}
...
void Win32IDE::disableExtension(const std::string& extensionId)
{
    for (auto& ext : m_extensions) {
        if (ext.id == extensionId) {
            ext.enabled = false;
            RAWRXD_LOG("Extension disabled: " + extensionId + "\n");
            break;
        }
    }
}
...
{
    RAWRXD_LOG("Updating extension: " + extensionId + "\n");
    // Placeholder
}

void Win32IDE::showExtensionDetails(const std::string& extensionId)
{
    RAWRXD_LOG("Showing details for: " + extensionId + "\n");
    // Placeholder
}
...
void Win32IDE::loadInstalledExtensions()
{
    ...
    RAWRXD_LOG("Loaded " + std::to_string(m_extensions.size()) + " extensions\n");
}
...
void Win32IDE::createOutlineView(HWND hwndParent)
{
    ...
    RAWRXD_LOG("Outline view created\n");
}
...
void Win32IDE::parseCodeForOutline()
{
    ...
    RAWRXD_LOG("Parsed " + std::to_string(m_outlineItems.size()) + " outline items\n");
}
...
void Win32IDE::goToOutlineItem(int index)
{
    ...
    RAWRXD_LOG("Navigated to: " + item.name + " at line " + std::to_string(item.line) + "\n");
}
...
void Win32IDE::createTimelineView(HWND hwndParent)
{
    ...
    RAWRXD_LOG("Timeline view created\n");
}
...
void Win32IDE::loadGitHistory()
{
    ...
    RAWRXD_LOG("Loaded " + std::to_string(m_timelineEntries.size()) + " timeline entries\n");
}
...
void Win32IDE::goToTimelineEntry(int index)
{
    ...
    if (entry.isGitCommit && !entry.commitHash.empty()) {
        ...
        RAWRXD_LOG("Selected local entry: " + entry.message + "\n");
    }
}
