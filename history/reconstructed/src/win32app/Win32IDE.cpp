...
#include "../agentic/coordination/SwarmOrchestrator.h" // Full type for unique_ptr dtor
...
Win32IDE::~Win32IDE() {
    ...
    // Resource cleanup
    // Win32TerminalManager has no static cleanup — instances clean up in their own destructors
}
#include <richedit.h>
#include <commctrl.h>
#include <shlobj.h>
...
// Build menu IDs — aligned with CommandID enum
...
#define IDM_EDIT_FIND_NEXT 2109
#define IDM_EDIT_FIND_PREV 2110
#define IDM_EDIT_SNIPPET 2111
#define IDM_EDIT_COPY_FORMAT 2112
#define IDM_EDIT_PASTE_PLAIN 2113
#define IDM_EDIT_CLIPBOARD_HISTORY 2114

#define IDM_VIEW_MINIMAP 2201
#define IDM_VIEW_OUTPUT_TABS 2202
#define IDM_VIEW_MODULE_BROWSER 2203
#define IDM_VIEW_THEME_EDITOR 2204
#define IDM_VIEW_FLOATING_PANEL 2205
#define IDM_VIEW_OUTPUT_PANEL 2206
#define IDM_VIEW_TERMINAL 2207
#define IDM_VIEW_SIDEBAR 2208
#define IDM_VIEW_USE_STREAMING_LOADER 2209
#define IDM_VIEW_USE_VULKAN_RENDERER 2210

#define IDM_TERMINAL_POWERSHELL 2301
#define IDM_TERMINAL_CMD 2302
#define IDM_TERMINAL_STOP 2303
#define IDM_TERMINAL_SPLIT_H 2305
#define IDM_TERMINAL_SPLIT_V 2306
#define IDM_TERMINAL_CLEAR_ALL 2304

#define IDM_TOOLS_PROFILE_START 2401
#define IDM_TOOLS_PROFILE_STOP 2402
#define IDM_TOOLS_PROFILE_RESULTS 2403
#define IDM_TOOLS_ANALYZE_SCRIPT 2404

#define IDM_GIT_STATUS 2501
#define IDM_GIT_COMMIT 2502
#define IDM_GIT_PUSH 2503
#define IDM_GIT_PULL 2504
#define IDM_GIT_PANEL 2505

#define IDM_MODULES_REFRESH 2601
#define IDM_MODULES_IMPORT 2602
#define IDM_MODULES_EXPORT 2603

#define IDM_HELP_ABOUT 2701
#define IDM_HELP_CMDREF 2702
#define IDM_HELP_PSDOCS 2703
#define IDM_HELP_SEARCH 2704

// Agent menu IDs
#define IDM_AGENT_START_LOOP 2901
#define IDM_AGENT_EXECUTE_CMD 2902
#define IDM_AGENT_CONFIGURE_MODEL 2903
#define IDM_AGENT_VIEW_TOOLS 2904
#define IDM_AGENT_VIEW_STATUS 2905
#define IDM_AGENT_STOP 2906
...
Win32IDE::Win32IDE(HINSTANCE hInstance)
    ...
    m_hwndHelp(nullptr), m_hMenu(nullptr), m_hAccel(nullptr), m_hwndToolbar(nullptr), 
    ...
    , m_currentMaxTokens(512)
{
    ...
    try {
        IDELogger::getInstance().log(LogLevel::INFO, "Logger started: C:\\RawrXD_IDE.log");
    ...
    try {
        ...
        IDELogger::getInstance().log(LogLevel::CRITICAL, std::string("TransparentRenderer creation failed: ") + e.what());
        OutputDebugStringA("ERROR: TransparentRenderer failed: ");
        OutputDebugStringA(e.what());
        OutputDebugStringA("\n");
        m_renderer = nullptr; // Use null renderer
    } catch (...) {
        IDELogger::getInstance().log(LogLevel::CRITICAL, "TransparentRenderer creation failed with unknown exception");
    ...
    for (int i = 5; i <= 1366; ++i)
        SendMessage(m_hwndStatusBar, SB_SETTEXT, i, (LPARAM)"");
    if (!m_hMenu) return;
    ...
    // Build menu — MASM64 + C++ hybrid toolchain
    HMENU hBuildMenu = CreatePopupMenu();
    AppendMenuA(hBuildMenu, MF_STRING, IDM_BUILD_PROJECT, "&Build Project\tF7");
    AppendMenuA(hBuildMenu, MF_STRING, IDM_BUILD_REBUILD, "&Rebuild All\tCtrl+Shift+B");
    AppendMenuA(hBuildMenu, MF_STRING, IDM_BUILD_CLEAN, "&Clean");
    AppendMenuA(hBuildMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenuA(hBuildMenu, MF_STRING, IDM_BUILD_ASM_CURRENT, "Assemble Current &File\tCtrl+F7");
    AppendMenuA(hBuildMenu, MF_STRING, IDM_BUILD_RUN, "&Run\tF5");
    AppendMenuA(hBuildMenu, MF_STRING, IDM_BUILD_STOP, "&Stop\tShift+F5");
    AppendMenuA(hBuildMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenuA(hBuildMenu, MF_STRING, IDM_BUILD_CONFIG_DEBUG, "Config: &Debug");
    AppendMenuA(hBuildMenu, MF_STRING, IDM_BUILD_CONFIG_RELEASE, "Config: &Release");
    AppendMenuA(hBuildMenu, MF_STRING, IDM_BUILD_SET_TARGET, "Set &Target...");
    AppendMenuA(hBuildMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenuA(hBuildMenu, MF_STRING, IDM_BUILD_SHOW_LOG, "Show Build &Log");
    AppendMenuA(m_hMenu, MF_POPUP, (UINT_PTR)hBuildMenu, "&Build");
...
std::string Win32IDE::getModelInfo() const
{
    ...
    for (size_t i = 0; i < m_modelTensors.size() && i < 10; ++i) {
        ...
        info += "    Size: " + std::to_string(tensor.size / 1024 / 1024) + " MB\n";
...
std::string Win32IDE::sendMessageToModel(const std::string& message)
{
    ...
    if (!m_ollamaBaseUrl.empty()) {
        response = "Error: No model available. Start Ollama at " + m_ollamaBaseUrl + " or load a local GGUF model.\n(Ollama unreachable or native engine not loaded.)";
    } else {
        response = "Error: No model available. Set Ollama URL or load a local GGUF model.";
    }
    return response;
}
...
std::string Win32IDE::generateResponse(const std::string& prompt)
{
    ...
    std::string err = "Model server unavailable.\n";
    err += "• Ollama: " + m_ollamaBaseUrl + " (start Ollama if you use it)\n";
    err += "• Native engine: " + std::string(m_nativeEngineLoaded ? "loaded" : "not loaded") + ", model: " + modelName + "\n";
    err += "Fix: Start Ollama and/or load a GGUF model via Model menu.";
    return err;
}
...
void Win32IDE::generateResponseAsync(const std::string& prompt, std::function<void(const std::string&, bool)> callback)
{
    ...
    m_inferenceThread = std::thread([this, prompt]() {
        ...
        if (m_nativeEngine) {
             RawrXD::CPUInferenceEngine* engine = static_cast<RawrXD::CPUInferenceEngine*>(m_nativeEngine);
             if (engine->isModelLoaded()) {
                 // Run generation with configured sampling parameters
                 ...
                 if (success && m_inferenceCallback) {
                     m_inferenceCallback(result.value().text, false);
                 }
...
void Win32IDE::HandleCopilotSend() {
    ...
    if (userMessage.empty()) {
        IDELogger::getInstance().log(LogLevel::WARNING, "Empty message - ignoring");
        return;
    }
