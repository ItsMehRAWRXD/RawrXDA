...
void Win32IDE::onExternalFileChange(const std::string& changedFile) {
    ...
    // Check if file has actually been externally modified using BufferManager
    if (m_activeTabIndex >= 0 && m_activeTabIndex < static_cast<int>(m_editorTabs.size())) {
        if (m_bufferManager.CheckExternalModification(m_editorTabs[m_activeTabIndex])) {
            m_fileChangedExternally = true;
            // Post to UI thread for toast notification
            if (m_hwndMain) {
...
void Win32IDE::reloadCurrentFile() {
    ...
    // ── Encoding-aware reload ────────────────────────────────────────
    RawrXD::FileSystem::BufferState bufState;
    int wideLen = MultiByteToWideChar(CP_UTF8, 0, m_currentFile.c_str(), -1, nullptr, 0);
    std::wstring widePath(wideLen - 1, L'\0');
    MultiByteToWideChar(CP_UTF8, 0, m_currentFile.c_str(), -1, widePath.data(), wideLen);
    ...
    if (m_bufferManager.OpenFile(widePath, bufState)) {
        ...
        LOG_INFO("File reloaded with encoding: " + std::string(RawrXD::FileSystem::EncodingName(bufState.encoding.encoding)));
    } else {
        ...
        if (file) {
            ...
            updateSaveStatusIndicator();
            updateTitleBarText();
            LOG_INFO("File reloaded (raw): " + m_currentFile);
        } else {
