...
CommandResult handleAiExplainCode(const CommandContext& ctx) {
    if (ctx.args && ctx.args[0]) {
        ...
        ModelInvoker::instance().invokeAsync(params);
        int waits = 0;
        while (ModelInvoker::instance().isInvoking() && waits < 300) { Sleep(100); waits++; }
...
CommandResult handleAiInlineComplete(const CommandContext& ctx) {
    ...
    ModelInvoker::instance().invokeAsync(params);
    int waits = 0;
    while (ModelInvoker::instance().isInvoking() && waits < 50) { Sleep(100); waits++; }
...
CommandResult handleAiModeDeepResearch(const CommandContext& ctx) {
    ...
    ModelInvoker::instance().setSystemPromptTemplate(
