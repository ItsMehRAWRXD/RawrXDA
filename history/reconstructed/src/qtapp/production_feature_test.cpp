...
void testGPUBackend() {
    ...
    if (gpu.isAvailable()) {
        QStringList devices = gpu.availableDevices();
        qInfo() << "  Available Devices:" << devices;
        
        for (int i = 0; i < devices.count(); ++i) {
            qInfo() << "\nGPU Device" << i << "Information:";
            if (!gpu.selectDevice(i)) {
                qWarning() << "  Failed to select device" << i;
                continue;
            }
            ...
            // Real memory allocation test (skip for Vulkan/paged memory constraints)
            const bool skipAlloc = (gpu.backendType() == GPUBackend::Vulkan) || (gpu.availableMemory() == 0);
            if (skipAlloc) {
                qInfo() << "  Memory Allocation: Skipped (using Vulkan budget query)";
            } else {
...
int main(int argc, char *argv[]) {
    // Lightweight Qt core app to satisfy QTimer requirements without GUI loop
