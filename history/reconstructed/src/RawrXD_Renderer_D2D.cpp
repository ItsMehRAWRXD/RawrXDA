...
void Renderer2D::drawStyledText(const Point& p, const String& text, const Font& font, const std::vector<TextRun>& runs, const Color& defaultColor) {
    // Delegate to the full colored implementation
    drawStyledTextColored(p, text, font, runs, defaultColor);
}

void Renderer2D::drawStyledTextColored(const Point& p, const String& text, const Font& font,
                                        const std::vector<TextRun>& runs, const Color& defaultColor) {
    if (!target || !solidBrush || !writeFactory) return;
    ...
    // Apply per-token formatting: bold, italic, AND color via SetDrawingEffect
    for (const auto& run : runs) {
        ...
        // Per-token color via ColorDrawingEffect
        ColorDrawingEffect* effect = new ColorDrawingEffect(ToD2D(run.color));
        layout->SetDrawingEffect(effect, range);
        effect->Release(); // layout holds its own ref
    }

    // Draw with our custom renderer that reads ColorDrawingEffect per glyph run
    CustomTextRenderer* customRenderer = new CustomTextRenderer(target, solidBrush, ToD2D(defaultColor));
    layout->Draw(nullptr, customRenderer, (FLOAT)p.x, (FLOAT)p.y);
    customRenderer->Release();
    layout->Release();
}

void Renderer2D::drawTextAdvanced(const Point& p, const String& text, const Font& font,
                                   const Color& color, float maxWidth, bool enableLigatures) {
    if (!target || !solidBrush || !writeFactory) return;
    const_cast<Font&>(font).updateFormat(writeFactory);
    if (!font.getFormat()) return;
    ...
    // Typography features: ligatures, contextual alternates
    IDWriteTypography* typography = nullptr;
    hr = writeFactory->CreateTypography(&typography);
    if (SUCCEEDED(hr) && typography) {
        if (enableLigatures) {
            ...
            typography->AddFontFeature({ DWRITE_FONT_FEATURE_TAG_DISCRETIONARY_LIGATURES, 1 });
            typography->AddFontFeature({ DWRITE_FONT_FEATURE_TAG_CONTEXTUAL_ALTERNATES, 1 });
        } else {
            typography->AddFontFeature({ DWRITE_FONT_FEATURE_TAG_STANDARD_LIGATURES, 0 });
            typography->AddFontFeature({ DWRITE_FONT_FEATURE_TAG_CONTEXTUAL_LIGATURES, 0 });
        }
        DWRITE_TEXT_RANGE fullRange = { 0, (UINT32)text.length() };
        layout->SetTypography(typography, fullRange);
        typography->Release();
    }

    // Enable font fallback (DirectWrite handles this automatically via its fallback chain,
    // but we can set word-wrapping and trimming for proper Unicode display)
    ...
    target->DrawTextLayout(
        ...
        layout, solidBrush,
        D2D1_DRAW_TEXT_OPTIONS_ENABLE_COLOR_FONT  // enable color emoji/fonts
    );
    layout->Release();
}
