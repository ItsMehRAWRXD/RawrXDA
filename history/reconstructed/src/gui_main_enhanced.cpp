...
bool GUIMainEnhanced::initialize(HINSTANCE hInstance) {
    ...
    m_hub = std::make_shared<AIIntegrationHub>();
    m_hub->initialize();
...
void GUIMainEnhanced::createControls(HWND parent) {
    ...
    // Initial message
    appendToChatOutput("RawrXD Agentic IDE Ready\r\n");
    appendToChatOutput("Modes: Max | Deep Thinking | Deep Research | No Refusal\r\n\r\n");
}
...
void GUIMainEnhanced::executeAgentCommand(const std::string& cmd) {
    ...
    int len = (int)SendMessageA(m_editorWindow, EM_GETTEXTLENGTHEX, (WPARAM)&gtl, 0);
    
    if (len > 0 && len < (int)sizeof(buffer)) {
        GETTEXTEX gt = {(DWORD)(len + 1), GT_DEFAULT, CP_ACP, NULL, NULL};
...
void GUIMainEnhanced::appendToChatOutput(const std::string& text) {
    GETTEXTLENGTHEX gtl = {GTL_DEFAULT, CP_ACP};
    int len = (int)SendMessageA(m_chatOutput, EM_GETTEXTLENGTHEX, (WPARAM)&gtl, 0);
