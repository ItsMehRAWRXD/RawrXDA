...
void Editor::backspace() {
    if (hasSelection()) {
        ...
        if (cursorLine < anchorLine || (cursorLine == anchorLine && cursorCol < anchorCol)) {
            startL = cursorLine; startC = cursorCol;
            endL = anchorLine;   endC = anchorCol;
        } else {
            startL = anchorLine; startC = anchorCol;
            endL = cursorLine;   endC = cursorCol;
        }
        
        int startPos = buffer.getLineStart(startL) + startC;
        int endPos   = buffer.getLineStart(endL) + endC;
        int delLen   = endPos - startPos;
        
        if (delLen > 0) {
            String delText = buffer.substring(startPos, delLen);
            stack.push(std::make_unique<DeleteCmd>(this, startL, startC, delText));
            removeInternal(startL, startC, delLen);
        }
    ...
    if (cursorCol > 0) {
        ...
        // Join with previous line: cursor moves to end of prev line, newline is deleted
        int prevLineLen = buffer.getLineLength(cursorLine - 1);
        // Check for \r\n vs \n at end of previous line
        int prevLineStart = buffer.getLineStart(cursorLine - 1);
        int joinPos = prevLineStart + prevLineLen;
        int delLen = 1; // just \n
        
        // Check if there's a \r before the \n
        if (joinPos > 0) {
            wchar_t prev = buffer.charAt(joinPos - 1);
            // The \n is at joinPos (which is actually the char right before cursorLine start)
            // Recalculate: the newline char(s) sit between previous line content and this line
            int thisLineStart = buffer.getLineStart(cursorLine);
            joinPos = thisLineStart - 1; // the \n
            if (joinPos > 0 && buffer.charAt(joinPos - 1) == L'\r') {
                joinPos--;
                delLen = 2; // \r\n
            }
        }
        
        String delText = buffer.substring(joinPos, delLen);
        stack.push(std::make_unique<DeleteCmd>(this, cursorLine - 1, prevLineLen, delText));
        removeInternal(cursorLine - 1, prevLineLen, delLen);
...
void Editor::deleteKey() {
    if (hasSelection()) {
        backspace(); // same as backspace with selection
        return;
    }
    
    int pos = buffer.getLineStart(cursorLine) + cursorCol;
    if (pos >= buffer.length()) return; // at end of document
    
    // Determine how many chars to delete (1, or 2 for \r\n)
    int delLen = 1;
    wchar_t ch = buffer.charAt(pos);
    if (ch == L'\r' && pos + 1 < buffer.length() && buffer.charAt(pos + 1) == L'\n') {
        delLen = 2;
    }
    
    String delText = buffer.substring(pos, delLen);
    stack.push(std::make_unique<DeleteCmd>(this, cursorLine, cursorCol, delText));
    removeInternal(cursorLine, cursorCol, delLen);
...
void Editor::paintEvent(PAINTSTRUCT& ps) {
    ...
    for (int i = startLine; i < endLine; ++i) {
        ...
        // Selection highlight
        if (hasSelection()) {
            int selStartL, selEndL, selStartC, selEndC;
            if (cursorLine < anchorLine || (cursorLine == anchorLine && cursorCol < anchorCol)) {
                selStartL = cursorLine; selStartC = cursorCol;
                selEndL = anchorLine;   selEndC = anchorCol;
            } else {
                selStartL = anchorLine; selStartC = anchorCol;
                selEndL = cursorLine;   selEndC = cursorCol;
            }
            
            if (i >= selStartL && i <= selEndL) {
                int lineLen = buffer.getLineLength(i);
                // Subtract newline from visible length
                int visLen = lineLen;
                if (visLen > 0) {
                    int lstart = buffer.getLineStart(i);
                    String ls = buffer.substring(lstart, lineLen);
                    if (ls.endsWith(L"\n")) visLen--;
                    if (visLen > 0 && ls[visLen - 1] == L'\r') visLen--;
                }
                
                int hlStart = 0, hlEnd = visLen;
                if (i == selStartL) hlStart = selStartC;
                if (i == selEndL) hlEnd = selEndC;
                
                if (hlEnd > hlStart) {
                    float x1 = gutterWidth + 2 - scrollX + hlStart * charWidth;
                    float x2 = gutterWidth + 2 - scrollX + hlEnd * charWidth;
                    renderer.fillRect(Rect((int)x1, (int)y, (int)(x2 - x1), (int)lineHeight), selectionColor);
                }
            }
        }
        ...
        // Cursor caret
        if (i == cursorLine) {
            float caretX = gutterWidth + 2 - scrollX + cursorCol * charWidth;
            renderer.fillRect(Rect((int)caretX, (int)y, 2, (int)lineHeight), Color::Black);
        }
