...
# PROC: "MyFunc PROC" or "MyFunc PROC FRAME" or "MyFunc PROC arg:QWORD"
# Anchored to start-of-line. Requires PROC as exact keyword boundary (\b)
# to prevent matching "g_pi PROCESS_INFORMATION <>".
# Allows arbitrary text after PROC (parameters, FRAME, USES, etc.)
...
# Label-style definitions: "name:" at start of line — used as call/jmp targets
# in pure-assembly code (no PROC/ENDP wrapper).
# Excludes: local labels (@@), segment directives, known directives.
LABEL_DEF_PATTERN = re.compile(
    r'^([A-Za-z_]\w+):',
    re.MULTILINE
)
# Labels that are NOT callable function labels (data section markers, etc.)
_LABEL_EXCLUDE = {
...
_WIN32_API_BASES = [
    ...
    # Crypt/Cert (crypt32)
    'CertCloseStore', 'CertFreeCertificateChain', 'CertGetCertificateChain',
    'CertNameToStr', 'CertOpenSystemStore', 'CryptDecodeObjectEx',
    'CryptAcquireContext', 'CryptReleaseContext',
    'CryptCreateHash', 'CryptHashData', 'CryptGetHashParam',
    'CryptDestroyHash', 'CryptEncrypt', 'CryptDecrypt',
    # Toolhelp / Process / Thread
    'CreateToolhelp32Snapshot', 'Process32First', 'Process32Next',
    'Thread32First', 'Thread32Next', 'Module32First', 'Module32Next',
    'OpenProcess', 'OpenThread', 'VirtualAllocEx', 'VirtualFreeEx',
    'VirtualProtectEx', 'VirtualQueryEx', 'ReadProcessMemory',
    'WriteProcessMemory', 'CreateRemoteThread', 'QueueUserAPC',
    'NtCreateThreadEx', 'RtlCreateUserThread',
    # Job/Timer/Pipe
    'CreateJobObject', 'CreateNamedPipe', 'SetNamedPipeHandleState',
    'CreateTimerQueue', 'DeleteTimerQueue', 'CreateTimerQueueTimer',
    # File/System extended
    'GetFileAttributes', 'SetFileAttributes',
    'GetFileInformationByHandle', 'FileTimeToSystemTime',
    'GetLocalTime', 'GetSystemTime', 'GetSystemTimePreciseAsFileTime',
    'GetWindowText', 'GetWindowTextLength',
    'FindFirstChangeNotification', 'FindNextChangeNotification',
    'SHCreateDirectoryEx',
    'EnumProcessModules', 'GetModuleInformation', 'QueryWorkingSetEx',
    'InterlockedExchange', 'InterlockedExchangeAdd',
    'InterlockedCompareExchange', 'InterlockedIncrement',
    'InterlockedDecrement',
    'SleepConditionVariableSRW', 'InitializeSRWLock',
    ...
    'vkResetFences', 'vkGetDeviceProcAddr', 'vkGetInstanceProcAddr',
    'vkGetPhysicalDeviceMemoryProperties',
    'vkGetPhysicalDeviceProperties',
    'vkCreatePipelineCache', 'vkDestroyPipelineCache',
    'vkGetImageSparseMemoryRequirements', 'vkQueueBindSparse',
    # Dbghelp
    'MiniDumpWriteDump',
    # Crypt / Certificate
    ...
    # Event Log
    'OpenEventLog', 'ClearEventLog', 'CloseEventLog',
    'RegisterEventSource', 'DeregisterEventSource', 'ReportEvent',
    # Timer Queue
    'DeleteTimerQueueTimer', 'CreateTimerQueueTimer',
    'ChangeTimerQueueTimer',
    # Window Class
    'RegisterClassEx', 'UnregisterClass',
    'GetClassInfoEx', 'GetClassLong', 'SetClassLong',
    # Window Management
    'ShowWindow', 'UpdateWindow', 'DestroyWindow',
    'CreateWindowEx', 'DefWindowProc', 'PostQuitMessage',
    'DispatchMessage', 'TranslateMessage', 'GetMessage', 'PeekMessage',
    'SendMessage', 'PostMessage', 'SetWindowText', 'SetTimer', 'KillTimer',
    'InvalidateRect', 'BeginPaint', 'EndPaint', 'GetDC', 'ReleaseDC',
    'MoveWindow', 'SetWindowPos', 'GetClientRect', 'GetWindowRect',
    'SetFocus', 'SetForegroundWindow', 'EnableWindow',
    'LoadIcon', 'LoadCursor', 'GetSysColorBrush',
    # Dialog
    'DialogBoxParam', 'EndDialog', 'GetDlgItem', 'SetDlgItemText',
    'MessageBox',
    # Misc kernel32
    'Beep', 'GetTickCount', 'GetTickCount64',
    'GetModuleFileName', 'TerminateProcess',
    'GetComputerName', 'GetVersionEx', 'GetSystemInfo',
    'ExpandEnvironmentStrings',
    # Registry
    'RegOpenKeyEx', 'RegSetValueEx', 'RegDeleteValue',
    'RegCloseKey', 'RegQueryValueEx', 'RegCreateKeyEx',
    # GDI
    'CreateFont', 'SelectObject', 'DeleteObject', 'SetBkMode',
    'SetTextColor', 'TextOut', 'FillRect', 'DrawText',
    'CreateSolidBrush', 'CreatePen',
    'LineTo', 'MoveToEx', 'RoundRect', 'SetDCPenColor',
    'SetDCBrushColor', 'Rectangle', 'Ellipse', 'Polygon',
    'Polyline', 'ExtTextOut', 'GetTextExtentPoint32',
    'CreateCompatibleDC', 'CreateCompatibleBitmap', 'BitBlt',
    'StretchBlt', 'SetPixel', 'GetPixel',
    # DWM (dwmapi.dll)
    'DwmFlush', 'DwmExtendFrameIntoClientArea',
    'DwmEnableBlurBehindWindow', 'DwmSetWindowAttribute',
    # Misc kernel32 / system
...
ENTRY_POINTS = {
    ...
    # Timer/callback/hook procs
    'timerproc', 'timerhandler', 'timercallback',
    'enumwindowsproc', 'dialogproc', 'hookproc',
    'subclassproc', 'monitorenumproc',
}
...
def _is_known_external(name):
    ...
    _LIB_PREFIXES = ('ggml_', 'gguf_', 'zlib_', 'sqlite3_', 'curl_',
                     'ssl_', 'lua_', 'pfn')
    for pfx in _LIB_PREFIXES:
    ...
    if lower in {'log', 'relative', 'classify_identifieruffer'}:
        return True
    # Handle wsprintfA / wvsprintfA variants
    if lower.startswith('wv') and lower.endswith(('a', 'w')):
        base = lower[:-1] if lower[-1] in ('a', 'w') else lower
        if base in WIN32_APIS:
            return True
    return False
...
def analyze_file(filepath):
    ...
    endps = set(
        m.group(1) for m in ENDP_PATTERN.finditer(content)
    ...
    # ---- Label-style definitions (name: without PROC) ----
    labels = []
    for m in LABEL_DEF_PATTERN.finditer(content):
        name = m.group(1)
        if name.lower() not in _LABEL_EXCLUDE and name.lower() not in MASM_KEYWORDS:
            labels.append(name)
    ...
    missing_endp = []
    for proc, start in procs:
        endp_match = re.search(
            rf'{re.escape(proc)}\s+ENDP',
            content[start:], re.IGNORECASE
        )
        if not endp_match:
            missing_endp.append(proc)
...
def aggregate_results(results):
    ...
    known_symbols = (
        ...
        all_data | all_consts | all_structs | all_publics | all_labels
    )
...
def main():
    ...
    parser.add_argument('--include-dirs', '-I', nargs='*', default=[],
                        help='Additional directories to scan for .inc symbol resolution')
    args = parser.parse_args()

    print(f'Scanning {args.root} ...')
    results = scan_directory(args.root)

    # Auto-detect sibling include/ directory
    root_parent = os.path.dirname(os.path.abspath(args.root))
    include_dir = os.path.join(root_parent, 'include')
    auto_dirs = [include_dir] if os.path.isdir(include_dir) else []
    extra_dirs = list(set(auto_dirs + args.include_dirs))

    for d in extra_dirs:
        if os.path.isdir(d):
            inc_results = scan_directory(d)
            results.extend(inc_results)
            print(f'  + {len(inc_results)} files from {d}')
