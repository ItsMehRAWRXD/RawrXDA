...
private:
    std::string find_compiler() {
        ...
        const char* custom_compiler_env = getenv("RAWRXD_COMPILER");
        if (custom_compiler_env && custom_compiler_env[0] && file_exists(custom_compiler_env)) {
            return custom_compiler_env;
        }
        
        // Check custom compilers directory
        std::vector<std::string> custom_paths = {
            "D:\\rawrxd\\compilers\\universal_compiler_runtime.exe",
            "D:\\rawrxd\\compilers\\eon_bootstrap_compiler.exe",
            "D:\\rawrxd\\compilers\\bash_compiler_from_scratch.exe",
            "C:\\rawrxd\\compilers\\universal_compiler_runtime.exe",
            "E:\\rawrxd\\compilers\\universal_compiler_runtime.exe"
        };
