...
extern "C" {
    ...
    uint32_t INFINITY_GetSlotState(uint32_t slot_index);
    uint64_t INFINITY_GetSlotVramPtr(uint32_t slot_index);
    uint64_t INFINITY_GetSlotRamPtr(uint32_t slot_index);
...
public:
    ...
    bool Phase4_InitiateDMA(uint32_t slot_index, uint64_t dest_vram) {
        // Get RAM pointer from quad-buffer slot
        uint64_t ram_ptr = INFINITY_GetSlotRamPtr(slot_index);
        uint32_t state = INFINITY_GetSlotState(slot_index);
        
        // Only DMA if slot is READY
    ...
    uint32_t GetSlotState(uint32_t slot_index) {
        if (slot_index >= QUAD_BUFFER_COUNT) return -1;
        return INFINITY_GetSlotState(slot_index);
    }
    
    uint64_t GetSlotVramPtr(uint32_t slot_index) {
        if (slot_index >= QUAD_BUFFER_COUNT) return 0;
        return INFINITY_GetSlotVramPtr(slot_index);
    }
    
    uint64_t GetSlotRamPtr(uint32_t slot_index) {
        if (slot_index >= QUAD_BUFFER_COUNT) return 0;
        return INFINITY_GetSlotRamPtr(slot_index);
    }
    
    // Get current buffer state
    struct BufferStatus {
        uint32_t slots[QUAD_BUFFER_COUNT];
    ...
    BufferStatus GetBufferStatus(void) {
        ...
        for (int i = 0; i < QUAD_BUFFER_COUNT; i++) {
            status.slots[i] = GetSlotState(i);
        }
        
        // Calculate efficiency: READY/COMPUTING slots / total
        uint32_t active = 0;
        for (int i = 0; i < QUAD_BUFFER_COUNT; i++) {
            if (status.slots[i] == BUF_STATE_READY || 
                status.slots[i] == BUF_STATE_COMPUTING) {
                active++;
            }
        }
