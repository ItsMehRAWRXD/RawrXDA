...
// ============================================================================
// Working Bubble — collapsible group showing agent work-in-progress
// ============================================================================

struct WorkingBubble {
    std::string                     title;          // "Working...", "Starting: Implement X"
    std::string                     subtitle;       // "4/6", phase info
    ToolActionState                 state;          // Running, Completed, Failed
    int64_t                         startTimeMs;    // Epoch ms when started
    int64_t                         endTimeMs;      // 0 if still running
    std::vector<ToolActionStatus>   steps;          // Actions inside this bubble
    bool                            isCollapsed;    // UI state hint

    // Nested sub-agent bubbles (for RunSubagent actions)
    struct SubagentBubble {
        std::string                     agentDescription;  // "Research RawrXD ASM project structure"
        std::string                     agentPrompt;       // Truncated prompt
        ToolActionState                 state;
        int64_t                         durationMs;
        std::vector<ToolActionStatus>   innerSteps;        // Steps the subagent performed
        bool                            isCollapsed;

        SubagentBubble() : state(ToolActionState::Pending), durationMs(0), isCollapsed(true) {}
    };
    std::vector<SubagentBubble>     subagents;

    WorkingBubble() : state(ToolActionState::Pending), startTimeMs(0), endTimeMs(0), isCollapsed(false) {}

    // Factory: start a new working bubble
    static WorkingBubble Begin(const std::string& title, const std::string& subtitle = "") {
        WorkingBubble wb;
        wb.title = title;
        wb.subtitle = subtitle;
        wb.state = ToolActionState::Running;
        wb.startTimeMs = static_cast<int64_t>(
    ...
    void finish(bool success = true) {
        state = success ? ToolActionState::Completed : ToolActionState::Failed;
        endTimeMs = static_cast<int64_t>(
            std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::system_clock::now().time_since_epoch()).count());
        isCollapsed = true;  // Auto-collapse when done
    }

    int64_t elapsedMs() const {
        if (endTimeMs > 0) return endTimeMs - startTimeMs;
        auto now = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::system_clock::now().time_since_epoch()).count();
        return static_cast<int64_t>(now) - startTimeMs;
    }
};

// ============================================================================
// Code Block Action — Keep/Undo actions for rendered code blocks
// ============================================================================

struct CodeBlockAction {
    enum class Type : uint8_t {
        Keep    = 0,   // Accept the code edit
        Undo    = 1,   // Revert the code edit
        Copy    = 2,   // Copy to clipboard
        Insert  = 3,   // Insert at cursor
        Apply   = 4,   // Apply diff
        Reject  = 5    // Reject diff
    };

    Type         type;
    std::string  messageId;      // Parent message
    int          blockIndex;     // Code block index
    std::string  filePath;       // Target file (for Keep/Undo)
    std::string  originalCode;   // For Undo: what to restore
    std::string  newCode;        // For Keep: what was applied
    bool         isApplied;      // Has Keep been clicked?

    CodeBlockAction() : type(Type::Copy), blockIndex(0), isApplied(false) {}
};
...
public:
    ...
    // ---- Working Bubble Rendering ----
    static std::string formatWorkingBubblePlainText(const WorkingBubble& bubble);
    static std::string formatWorkingBubbleHtml(const WorkingBubble& bubble);
    static std::string formatSubagentBubbleHtml(const WorkingBubble::SubagentBubble& sub);
    static std::string formatSubagentBubblePlainText(const WorkingBubble::SubagentBubble& sub);

    // ---- Code Block Keep/Undo Button HTML ----
    static std::string generateCodeBlockActionsHtml(const std::string& messageId,
                                                     int blockIndex,
...
public:
    ...
    // Start a new working bubble (groups subsequent actions)
    void beginWorkingBubble(const std::string& title, const std::string& subtitle = "");

    // End the current working bubble
    void endWorkingBubble(bool success = true);

    // Start a subagent bubble inside the current working bubble
    void beginSubagentBubble(const std::string& description, const std::string& prompt = "");

    // End the current subagent bubble
    void endSubagentBubble(bool success = true, int64_t durationMs = 0);

    // Get working bubbles for rendering
    const std::vector<WorkingBubble>& workingBubbles() const { return m_bubbles; }

    // Render working bubbles as plain text
    std::string renderBubblesPlainText() const;

    // Render working bubbles as HTML
    std::string renderBubblesHtml() const;

    // ---- Code Block Action Tracking ----

    void trackCodeBlockAction(const CodeBlockAction& action);
    const std::vector<CodeBlockAction>& codeBlockActions() const { return m_codeActions; }

private:
    std::vector<ToolActionStatus> m_actions;
    std::vector<WorkingBubble>    m_bubbles;
    int                           m_activeBubbleIdx = -1;
    int                           m_activeSubagentIdx = -1;
    std::vector<CodeBlockAction>  m_codeActions;
};
