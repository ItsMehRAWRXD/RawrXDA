...
void GUIMain::createMenus() {
    ...
    // Agentic Mode Submenu
    HMENU modeMenu = CreatePopupMenu();
    AppendMenuW(modeMenu, MF_STRING, 4011, L"&Max Mode (32k)");
    AppendMenuW(modeMenu, MF_STRING, 4012, L"Deep &Thinking");
    AppendMenuW(modeMenu, MF_STRING, 4013, L"Deep &Research");
    AppendMenuW(modeMenu, MF_STRING, 4014, L"Generate Re&act Server");
    AppendMenuW(aiMenu, MF_POPUP, (UINT_PTR)modeMenu, L"Agent &Modes");
...
void GUIMain::handleMenuCommand(int commandId) {
    switch (commandId) {
        ...
        case 4004: // AI Analyze
            ...
            break;
        case 4005: // Run Agent
            MessageBox(m_mainWindow, L"Run Agent Feature - Not fully implemented in UI", L"Agent", MB_OK);
            break;
        case 4006: // Create Plan
            MessageBox(m_mainWindow, L"Create Plan Feature - Not fully implemented in UI", L"Plan", MB_OK);
            break;
        case 4007: // Ask AI
            MessageBox(m_mainWindow, L"Ask AI Feature - Not fully implemented in UI", L"Ask", MB_OK);
            break;
        case 4008: // AI Edit
            MessageBox(m_mainWindow, L"AI Edit Feature - Not fully implemented in UI", L"Edit", MB_OK);
            break;
        case 4009: // Generate Bug Report
            {
                // [Agentic] Generate Bug Report for current file
                // Assuming we get content from editor (stubbed get call)
                std::string content = "void main() { int x = 0; }"; // Stub for GetText
                std::string report = m_ide->getAgent()->processQuery("/bugreport " + content);
                MessageBoxA(m_mainWindow, report.c_str(), "AI Bug Report", MB_OK);
            }
            break;
        case 4010: // Code Suggestions
            {
                 // [Agentic] Generate Suggestions
                std::string content = "void main() { int x = 0; }"; // Stub for GetText
                std::string suggestions = m_ide->getAgent()->processQuery("/suggestions " + content);
                MessageBoxA(m_mainWindow, suggestions.c_str(), "AI Code Suggestions", MB_OK);
            }
            break;
            // New Agentic Commands (Thinking, Max Mode etc)
        case 4011: // Max Mode
             m_ide->getAgent()->processQuery("/maxmode");
             MessageBoxA(m_mainWindow, "Max Mode Enabled (32k Context)", "AI Config", MB_OK);
             break;
        case 4012: // Deep Thinking
             m_ide->getAgent()->processQuery("/think");
             MessageBoxA(m_mainWindow, "Deep Thinking Enabled (Process Mode)", "AI Config", MB_OK);
             break;
        case 4013: // Deep Research
             {
                 // Prompt for topic
                 // For now, trigger generic local scan in background
                 MessageBoxA(m_mainWindow, "Deep Research Started (Scanning background...)", "AI Research", MB_OK);
                 m_ide->getAgent()->processQueryAsync("/research system_scan", [this](std::string res){
                     MessageBoxA(m_mainWindow, "Research Complete. Check Logs.", "AI Research", MB_OK);
                 });
             }
             break;
        case 4014: // React Server
             m_ide->getAgent()->processQueryAsync("/react-server", [this](std::string res){
                 MessageBoxA(m_mainWindow, res.c_str(), "React Generator", MB_OK);
             });
             break;
    }
}
