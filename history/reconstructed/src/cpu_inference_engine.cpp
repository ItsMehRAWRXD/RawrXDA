...
public:
    ...
    bool isLoaded = false;
    std::shared_ptr<IMemoryPlugin> memoryPlugin;
...
RawrXD::Expected<CPUInferenceEngine::GenerationResult, InferenceError> CPUInferenceEngine::generate(
    ...
    res.text = result;
    res.confidence = 1.0f;
    // Count tokens by tokenizing the generated result text
    if (!result.empty()) {
        auto toks = m_impl->engine.Tokenize(result);
        res.tokens_generated = static_cast<int>(toks.size());
    } else {
        res.tokens_generated = 0;
    }
...
nlohmann::json CPUInferenceEngine::getStatus() const {
    nlohmann::json j;
    j["loaded"] = m_impl->isLoaded;
    j["backend"] = "Vulkan/RawrXD_Real";
    j["device"] = "GPU";
    return j;
}

void CPUInferenceEngine::RegisterMemoryPlugin(std::shared_ptr<IMemoryPlugin> plugin) {
    m_impl->memoryPlugin = std::move(plugin);
}
