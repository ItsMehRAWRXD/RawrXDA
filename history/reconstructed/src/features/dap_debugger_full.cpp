...
bool DAPDebugger::stepOver() {
    if (!m_impl->m_debugging) return false;
    // Set TF (trap flag) for single-step, but skip into calls by setting
    // a breakpoint at the next instruction at the same stack depth.
    // Simplified: use DEBUG_EVENT with EXCEPTION_SINGLE_STEP and track
    // call depth. For now, step and track RBP to detect same-frame return.
    CONTEXT ctx = {};
    ctx.ContextFlags = CONTEXT_FULL;
    GetThreadContext(m_impl->m_hThread, &ctx);
    uint64_t currentRBP = ctx.Rbp;
    // Step until we return to the same or higher frame
    do {
        if (!step()) return false;
        GetThreadContext(m_impl->m_hThread, &ctx);
    } while (ctx.Rbp < currentRBP && m_impl->m_debugging);
    return true;
}

bool DAPDebugger::stepOut() {
    if (!m_impl->m_debugging) return false;
    // Step until RBP increases (we've returned from current frame)
    CONTEXT ctx = {};
    ctx.ContextFlags = CONTEXT_FULL;
    GetThreadContext(m_impl->m_hThread, &ctx);
    uint64_t currentRBP = ctx.Rbp;
