...
public:
    ...
    std::string predictNextToken(const std::string& buffer,
        ...
        (void)language;
        if (buffer.empty() || cursorOffset == 0) return "";
        
        // Extract context before cursor (last ~200 chars)
        size_t start = (cursorOffset > 200) ? cursorOffset - 200 : 0;
        std::string ctx = buffer.substr(start, cursorOffset - start);
        
        // Simple heuristic: find the partial token being typed
        // and suggest common continuations based on language keywords
        size_t lastSpace = ctx.find_last_of(" \t\n({;,");
        std::string partial;
        if (lastSpace != std::string::npos)
            partial = ctx.substr(lastSpace + 1);
        else
            partial = ctx;
        
        if (partial.empty()) return "";
        
        // Common C++ keyword completions
        static const char* keywords[] = {
            "auto", "break", "case", "class", "const", "constexpr", "continue",
            "default", "delete", "do", "else", "enum", "explicit", "extern",
            "false", "for", "friend", "if", "include", "inline", "int",
            ...
            "uint32_t", "uint64_t", "unsigned", "using", "virtual", "void",
            "volatile", "while", nullptr
        };
        
        for (int i = 0; keywords[i]; i++) {
            std::string kw = keywords[i];
            if (kw.size() > partial.size() && kw.substr(0, partial.size()) == partial) {
                return kw.substr(partial.size());
            }
        }
        return "";
    }
...
private:
    ...
    std::string getModelCompletion(const std::string& contextWindow,
        ...
        (void)context; (void)temperature;
        // Use the predictNextToken heuristic for now — full model path
        // goes through StreamingCompletionEngine -> AI ModelCaller
        if (contextWindow.empty()) return "";
        size_t cursor = contextWindow.find("<|CURSOR|>");
        if (cursor == std::string::npos) return "";
        return predictNextToken(contextWindow.substr(0, cursor), cursor, "cpp");
    }
};
...
public:
    ...
    std::string generateInlineEdit(const EditorState& editorState,
        ...
        return "";  // Full model inference deferred to AI ModelCaller pipeline
    }
...
public:
    ...
    // IEditorWidget interface implementation
    std::string getContent() const override {
        return editorState_.fileContent;
    }
    ...
    void setContent(const std::string& content) override {
        ...
        editorState_.version++;
        editorState_.line = 0;
        editorState_.column = 0;
    }

    void applyEdits(const std::vector<EditorWidget::TextEdit>& edits) override {
        ...
        const size_t contentSize = editorState_.fileContent.size();
        for (const auto& edit : edits) {
            size_t startPos = getPositionOffset(edit.range.start);
            size_t endPos = getPositionOffset(edit.range.end);
            if (startPos > contentSize) startPos = contentSize;
            if (endPos > contentSize) endPos = contentSize;
            if (endPos < startPos) std::swap(startPos, endPos);
            prepared.push_back({startPos, endPos, edit.newText});
        }

        std::sort(prepared.begin(), prepared.end(), [](const PreparedEdit& a, const PreparedEdit& b) {
            if (a.start != b.start) return a.start > b.start; // descending
            return a.end > b.end;
        });

        for (const auto& edit : prepared) {
            editorState_.fileContent.replace(edit.start, edit.end - edit.start, edit.newText);
        }
    ...
    EditorWidget::Position getCursorPosition() const override {
        return {editorState_.line, editorState_.column};
    }

    void setCursorPosition(EditorWidget::Position pos) override {
        editorState_.line = pos.line;
        editorState_.column = pos.column;
    }

    std::string getSelectedText() const override {
        if (editorState_.selectionStartLine == editorState_.selectionEndLine &&
            editorState_.selectionStartColumn == editorState_.selectionEndColumn) {
            return ""; // No selection
        }

        size_t startPos = getPositionOffset({editorState_.selectionStartLine, editorState_.selectionStartColumn});
        size_t endPos = getPositionOffset({editorState_.selectionEndLine, editorState_.selectionEndColumn});

        if (startPos < endPos && endPos <= editorState_.fileContent.size()) {
            return editorState_.fileContent.substr(startPos, endPos - startPos);
        }
        return "";
    }

    void showInlineCompletion(const EditorWidget::InlineCompletion& completion) override {
        // Store the completion for display
        currentInlineCompletion_ = completion;
    }

    void dismissInlineCompletion() override {
        currentInlineCompletion_.reset();
    }

    void setDiagnostics(const std::vector<EditorWidget::Diagnostic>& diagnostics) override {
        diagnostics_ = diagnostics;
    }

    std::vector<EditorWidget::Diagnostic> getDiagnostics() const override {
        return diagnostics_;
    }
...
private:
    ...
    std::optional<EditorWidget::InlineCompletion> currentInlineCompletion_;
    std::vector<EditorWidget::Diagnostic> diagnostics_;
    ...
    // Helper method to convert Position to offset in file content
    size_t getPositionOffset(EditorWidget::Position pos) const {
        size_t offset = 0;
        int currentLine = 0;
        int currentCol = 0;

        for (size_t i = 0; i < editorState_.fileContent.size(); ++i) {
            if (currentLine == pos.line && currentCol == pos.column) {
                return offset;
            }

            if (editorState_.fileContent[i] == '\n') {
                ++currentLine;
                currentCol = 0;
            } else {
                ++currentCol;
        ...
        return offset; // Return end if position not found
    }
    ...
    CompletionContext buildCompletionContext() {
        ...
        size_t cursorPos = getPositionOffset({editorState_.line, editorState_.column});
        if (cursorPos > editorState_.fileContent.size()) cursorPos = editorState_.fileContent.size();
...
// Factory function to create editor widget
namespace EditorWidget {
std::unique_ptr<IEditorWidget> createEditorWidget() {
    return std::make_unique<RawrXD::IDE::Cursor::CursorEditorWidget>();
}
} // namespace EditorWidget
