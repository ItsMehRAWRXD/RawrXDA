...
// BPETokenizer definition (declared in dml_inference_engine.h)
struct DMLInferenceEngine::BPETokenizer {
    std::vector<std::string> vocab;
    std::vector<float> scores;
    int32_t bosToken = 0, eosToken = 0, padToken = 0, unkToken = 0;
...
bool DMLInferenceEngine::LoadModel(const std::string& modelPath) {
    ...
    if (!dmlCompute.isInitialized()) {
        ...
        if (!r.success) {
            s_logger.error("[DMLEngine] DML init failed: " + r.detail);
            return false;
        }
    }
    ...
    if (!r.success) {
        s_logger.error("[DMLEngine] openModel failed: " + r.detail);
        return false;
    }
...
bool DMLInferenceEngine::LoadSecondModel(const std::string& modelPath) {
    ...
    if (!m_modelLoaded.load()) {
        s_logger.error("[DMLEngine] No primary model loaded");
        return false;
    }

    auto r = m_ggufBridge->loadSecondModel(modelPath.c_str(), m_sessionId + 1);
    if (!r.success) {
        s_logger.error("[DMLEngine] Second model load failed: " + r.detail);
        return false;
    }
...
std::vector<int32_t> DMLInferenceEngine::Generate(const std::vector<int32_t>& inputTokens,
    ...
    for (int step = 0; step < maxTokens; ++step) {
        ...
        if (!r.success) {
            s_logger.error("[DMLEngine] Forward pass failed at step " + std::to_string(step) + ": " + r.detail);
            break;
        }
