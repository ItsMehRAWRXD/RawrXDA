...
public:
    ...
    InterpretabilityPanel() = default;
    ~InterpretabilityPanel() = default;
    
    /**
     * Two-phase initialization
     * Creates all widgets and sets up connections
     */
    void initialize();
     ...
     */
    void updateIntegratedGradients(const std::string& inputName, const std::vector<float>& attributions);
     ...
     */
    bool exportVisualization(const std::string& filePath) const;

    /**
     * @brief Export interpretability data to JSON
     * @return Data as JSON object
     */
    std::string exportInterpretabilityData() const;  // JSON-serialized
     ...
     * @return true if successful
     */
    bool loadInterpretabilityData(const std::string& data);  // JSON string
    ...
    // ===== Callback Types =====
    using LayerSelectionCb = void(*)(void* ctx, int layerIndex);
    using HeadSelectionCb = void(*)(void* ctx, int headIndex);
    using GradientProblemCb = void(*)(void* ctx, const char* problem);
    using CriticalLayerCb = void(*)(void* ctx, int layerIndex, float attribution);
    using VizUpdatedCb = void(*)(void* ctx, int vizType);

    void setLayerSelectionCb(LayerSelectionCb cb, void* ctx) { m_layerSelCb = cb; m_layerSelCtx = ctx; }
    void setHeadSelectionCb(HeadSelectionCb cb, void* ctx) { m_headSelCb = cb; m_headSelCtx = ctx; }
    void setGradientProblemCb(GradientProblemCb cb, void* ctx) { m_gradProbCb = cb; m_gradProbCtx = ctx; }
    void setCriticalLayerCb(CriticalLayerCb cb, void* ctx) { m_critLayerCb = cb; m_critLayerCtx = ctx; }
    void setVizUpdatedCb(VizUpdatedCb cb, void* ctx) { m_vizUpdCb = cb; m_vizUpdCtx = ctx; }
...
private:
    ...
    // ===== UI Components =====
    HWND m_tabWidget = nullptr;
    HWND m_vizTypeCombo = nullptr;
    HWND m_layerSlider = nullptr;
    HWND m_attentionHeadCombo = nullptr;
    HWND m_chartView = nullptr;
    HWND m_statsLabel = nullptr;
    HWND m_problemsLabel = nullptr;

    // ===== Callback Pointers =====
    LayerSelectionCb m_layerSelCb = nullptr;
    void* m_layerSelCtx = nullptr;
    HeadSelectionCb m_headSelCb = nullptr;
    void* m_headSelCtx = nullptr;
    GradientProblemCb m_gradProbCb = nullptr;
    void* m_gradProbCtx = nullptr;
    CriticalLayerCb m_critLayerCb = nullptr;
    void* m_critLayerCtx = nullptr;
    VizUpdatedCb m_vizUpdCb = nullptr;
    void* m_vizUpdCtx = nullptr;
};
