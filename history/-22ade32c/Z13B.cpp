...
void AgenticErrorHandler::initialize(AgenticObservability* obs, AgenticLoopState* state)
{
    ...
    if (m_observability) {
        m_observability->logInfo("AgenticErrorHandler", 
            "Initialized with observability and state management", QJsonObject());
    }
}
...
{
    m_observability->logInfo("AgenticErrorHandler", "handleError", "Handling internal error", {
        {"component", component},
    ...
    const QJsonObject& context)
{
    ...
    if (m_observability) {
        ...
        m_observability->logError(component, "ErrorRecorded", message, logContext);
    }
