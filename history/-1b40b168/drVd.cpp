...
{
    // Enterprise License Gate — Wiring Oracle requires Enterprise tier
    auto __gateResult = LicenseEnforcementGate::getInstance().gateWiringOracle();
    if (!__gateResult.success) {
        WiringReport empty;
        empty.sourceFile = "License gate blocked: " + std::string(__gateResult.detail ? __gateResult.detail : "Enterprise license required");
        return empty;
    }
