...
; ---- Buffers (static .data — no CRT, avoid large stack allocs) ----
ALIGN 16
g_ResponseBuf   BYTE MAX_RESPONSE DUP(0)       ; HTTP response body
g_JsonPayload   BYTE 32768 DUP(0)              ; outgoing JSON POST body
g_ErrorMsg      BYTE MAX_ERROR_MSG DUP(0)       ; formatted error message
g_ExtractBuf    BYTE MAX_EXTRACT_BUF DUP(0)     ; JSON string extraction
...
_sf_process_line:
    ...
    lea  rdi, g_StreamAccum
    add  edi, g_StreamAccLen        ; (note: using 32-bit offset into static buf)
    lea  rsi, g_ExtractBuf          ; (restore rsi temporarily)
    ; Copy extracted text
    mov  ecx, r12d
_sf_copy_loop:
    test ecx, ecx
...
_sf_clear_line:
    ...
    ; Restore rsi to chunk read position (it was clobbered in copy loop)
    ; Actually rsi/ebx were saved on entry — but rsi was modified in copy.
    ; We need to recalculate. The chunk pointer was advanced before calling
    ; _sf_process_line, so the remaining bytes (ebx) and pointer are correct
    ; since we saved them at _sf_byte_loop entry.
    ; Actually we need to fix this: rsi was clobbered during copy.
    ; Fix: save/restore rsi around the copy.
    ; But wait: rsi was pointing into the chunk stream. After the copy loop,
    ; rsi points into g_ExtractBuf. We need to restore it.
    ; The correct approach: calculate remaining from saved state.

    ; Let's use a different register for the copy to avoid clobbering rsi.
    ; This is handled by the stack pushes — rsi was saved at entry.
    ; Actually no, rsi is the live chunk pointer. We clobbered it.
    ; Fix: We'll re-derive from original chunk pointer + bytes consumed.
    ; Since we advanced rsi/ebx in the byte loop and they represent current state,
    ; but we clobbered rsi in the copy loop, we need a fix.
    ;
    ; ACTUAL FIX: The chunk read loop saved rsi/ebx state before jumping to
    ; _sf_process_line. But rsi was clobbered. We should save rsi on stack
    ; before processing. Let me restructure:
    ; -- This is a known issue. The fix is: store chunk pointer in a callee-saved
    ;    register that isn't used for copy. We'll use [rbp-10h] for chunk state.

    ; Since we already processed the line, continue with remaining chunk.
    ; rsi/ebx were the chunk state BEFORE we entered _sf_process_line.
    ; Wait no — rsi and ebx were being used in _sf_byte_loop and were already
    ; updated past the \n character. The issue is only that the copy loop
    ; clobbered rsi. We need to restore it.

    ; The remaining chunk data starts at original rsi (before copy clobber).
    ; We can't recover it here easily. This is an architectural issue.
    ; FIX: Use [rbp-10h/rbp-18h] to save chunk state before processing.
    ; For now, since Stream_Feed is called per-chunk and the chunk is consumed
    ; byte-by-byte, we'll break out to re-enter from the caller with next chunk.
    ; But actually multiple lines per chunk is common.

    ; REAL FIX: save rsi/ebx to locals before processing line
