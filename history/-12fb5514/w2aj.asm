...
;   Http_GetJSON             — GET endpoint, return response body
...
DEFAULT_RECEIVE_TIMEOUT  EQU 300000 ; 300 seconds (5 minutes for slow Ollama streaming)
...
;==============================================================================
; Http_GetJSON — GET JSON from Ollama endpoint
;
; RCX = pResponseOut (buffer to receive response body)
; RDX = responseMaxLen
; R8  = pWideEndpoint (LPCWSTR path, NULL = /api/tags)
;
; Returns: bytes read in EAX (0 on failure)
;==============================================================================
Http_GetJSON PROC FRAME
    push rbp
    .pushreg rbp
    mov  rbp, rsp
    .setframe rbp, 0
    ...
    mov  r12, rcx                   ; pResponseOut
    mov  ebx, edx                   ; responseMaxLen
    mov  rsi, r8                    ; pWideEndpoint

    ; Default endpoint
    test rsi, rsi
    jnz  @F
    lea  rsi, wOllamaTagsPath
@@:
...
@@:
    ...
    ; WinHttpSendRequest — no body for GET
    mov  rcx, rax
    xor  edx, edx
    xor  r8d, r8d
    xor  r9, r9                     ; no body
    mov  qword ptr [rsp+20h], 0     ; optional length
    mov  qword ptr [rsp+28h], 0     ; total length
    mov  qword ptr [rsp+30h], 0
    call WinHttpSendRequest
    test eax, eax
    jz   _hgj_close_fail
