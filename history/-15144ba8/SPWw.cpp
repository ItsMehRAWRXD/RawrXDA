...
void CommandHandler::cmdServerInfo() {
    printInfo("\n=== API Server Status ===");
    printInfo("Server metrics are tracked internally but not exposed to CLI yet.");
    printInfo("The API server runs in the background on your instance port (11434 + PID % 100).");
    printInfo("\nTo monitor server activity:");
    printInfo("  - Check application logs for METRICS entries");
    printInfo("  - Use HTTP client to query /health endpoint");
    printInfo("  - Server stats will be displayed when CLI is stopped");
    printWarning("\nNote: Direct metrics display will be implemented in a future update.");
}
}
