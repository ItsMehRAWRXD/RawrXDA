...
void CommandHandler::cmdInferStream(const std::string& prompt) {
    ...
    // TODO: Implement proper streaming inference using StreamingManager
    printWarning("Streaming inference functionality requires integration with inference engine");
    ...
    // Generate response tokens with streaming (token-by-token output)
    std::vector<int32_t> resultTokens = tokens;
    int maxNewTokens = m_inferenceParams.maxTokens;
    int generatedCount = 0;
    
    for (int i = 0; i < maxNewTokens; ++i) {
        // Get inference for current tokens
        auto prediction = m_modelLoader->Infer(resultTokens);
        if (prediction.empty()) {
            break;
        }
        ...
        // Detokenize this single token and output immediately
        std::vector<int32_t> singleToken = { nextToken };
        std::string tokenText = m_modelLoader->Detokenize(singleToken);
        ...
        // Check for EOS token (typically 2) or PAD token (0)
        if (nextToken == 2 || nextToken == 0) {
            printInfo("\n[Stream ended - EOS token reached]");
            break;
        }
    ...
    std::cout << "\n" << std::endl;
    printSuccess(std::string("Streaming complete: ") + std::to_string(generatedCount) + " tokens generated");
    */
}
...
void CommandHandler::cmdChat() {
    ...
    // TODO: Implement proper chat using inference engine and StreamingManager
    printWarning("Chat functionality requires integration with inference engine");
    
    /* Commented out - GGUFLoader doesn't have Tokenize/Infer/Detokenize methods
    ...
    while (true) {
        ...
        std::vector<int32_t> tokens = m_modelLoader->Tokenize(context);
        if (tokens.empty()) {
            printError("Failed to tokenize input");
            continue;
        }
