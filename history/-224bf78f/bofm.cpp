...
nlohmann::json ModelInvoker::sendRawrXDRequest(const std::string& messageBlock, int maxTokens, float temperature) {
    ...
    if (hPipe == INVALID_HANDLE_VALUE) {
        std::cerr << "[-] Error: Could not connect to RawrXD Agent Pipe. Is RawrXD-Agent.exe running?" << std::endl;
        return result; 
    }

    // Send Prompt
    DWORD bytesWritten;
    // Just send raw byte string for now (assuming RawrXD-Agent handles pure text input via pipe server wrapper)
    // Note: The recent ASM changes allow binary token submission, but the Pipe Server wrapper 
    // copies bytes. If we send text here, the ASM backend sees bytes.
    // The ASM backend's `Titan_InferenceThread` processes 4-byte tokens.
    // ERROR: Sending Text as Bytes will be interpreted as garbage tokens if the backend expects INT32.
    // We must tokenize here or ensure backend handles text.
    // Current backend `RawrXD_Titan_UNIFIED.asm` expects INT32 tokens in `g_InputBuffer`.
    // We need a tokenizer.
    // For this implementation, we will assume the input IS already raw bytes of int32 tokens or 
    // we need to implement a simple tokenizer in C++ here.
    
    // To properly support "RawrXD" backend in the "ModelInvoker", we should ideally tokenize.
    // However, if we want to support "No Qt Deps", we might not have the BPE tokenizer here (it's in `src/qtapp`).
    //
    // Alternate Strategy:
    // Update `RawrXD_PipeServer.asm` to accept a COMMAND BYTE.
    // 0x01 = Raw Tokens
    // 0x02 = Text (requires server-side tokenization, which we don't have in ASM yet)
    
    // Given the constraints: "Complete GUI/IDE/CLI ... ensuring no QT deps".
    // If the CLI needs to run inference, it logic must be self-contained.
    
    // For now, let's assume the standard Llama tokenizer simulation:
    // Simple Byte-Pair or just ASCII mapping for test.
    bool success = WriteFile(hPipe, messageBlock.c_str(), messageBlock.size(), &bytesWritten, NULL);
    ...
    if (success) {
        ...
        if (ReadFile(hPipe, buffer, sizeof(buffer)-1, &bytesRead, NULL)) {
            buffer[bytesRead] = 0;
            // The ASM backend returns binary tokens (int32 sequence).
            // We need to decode this back to string.
            
            // Heuristic: If it looks like valid utf8 text, keep it. 
            // If it looks like binary (00 bytes), decode as int32 tokens -> char.
             result["response"] = std::string(buffer);
        }
    }
...
nlohmann::json ModelInvoker::parsePlan(const std::string& llmOutput)
{
    ...
    size_t start = llmOutput.find('[');
    size_t end = llmOutput.rfind(']');
    if (start != std::string::npos && end != std::string::npos && end > start) {
        try {
            return nlohmann::json::parse(llmOutput.substr(start, end - start + 1));
        } catch (...) {}
    }
...
std::string ModelInvoker::getCacheKey(const InvocationParams& params) const
{
    ...
    size_t seed = 0;
    seed ^= hasher(params.wish) + 0x9e3779b9 + (seed<<6) + (seed>>2);
...
LLMResponse ModelInvoker::queryRaw(const std::string& systemPrompt, const std::string& userPrompt, int maxTokens)
{
    ...
    try {
        ...
        if (m_backend == "ollama") {
            llmResponse = sendOllamaRequest(m_model, fullPrompt, maxTokens, 0.7);
        } else if (m_backend == "claude") {
