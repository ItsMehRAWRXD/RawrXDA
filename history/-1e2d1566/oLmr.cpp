...
void DiskRecoveryAgent::Cleanup() {
    if (m_ctx) {
...
int DiskRecoveryAgent::FindDrive() {
...
RecoveryResult DiskRecoveryAgent::Initialize(int driveNum) {
    if (m_ctx) {
...
RecoveryResult DiskRecoveryAgent::ExtractEncryptionKey() {
    if (!m_ctx) {
...
RecoveryResult DiskRecoveryAgent::RunRecovery() {
    if (!m_ctx) {
        return RecoveryResult::error("Not initialized");
    }
...
RecoveryStats DiskRecoveryAgent::GetStats() const {
    RecoveryStats stats = {};
    if (m_ctx) {
