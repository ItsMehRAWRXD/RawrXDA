...
void Win32IDE::openFileDialog() {
    ...
    if (GetOpenFileNameA(&ofn)) {
        ...
        // Check if it's a GGUF model file - DON'T load as text!
        if (filePath.find(".gguf") != std::string::npos || filePath.find(".GGUF") != std::string::npos) {
            // Load as model, not as text
            if (loadGGUFModel(filePath)) {
                std::string message = "✅ Model loaded: " + filePath + "\n\n" + getModelInfo();
                appendToOutput(message, "Output", OutputSeverity::Info);
                MessageBoxA(m_hwndMain, "Model loaded successfully! Check Output panel for details.", "Model Loaded", MB_OK | MB_ICONINFORMATION);
            }
            return;
        }
        ...
        // Load text file with size check
        try {
            ...
            if (file) {
                ...
                if (fileSize > 10 * 1024 * 1024) { // 10MB limit for text files
                    MessageBoxA(m_hwndMain, "File too large to open in editor (>10MB).\nUse File Explorer to load models.", "File Too Large", MB_OK | MB_ICONWARNING);
                    return;
                }
