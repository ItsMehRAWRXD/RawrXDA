...
LoadModelNative PROC
    ...
    cmp eax, 2175401975         ; GGUF magic
    jne @@load_err
    
    ; Parse GGUF version (offset 4)
    mov eax, DWORD PTR [rsi + 4]
    cmp eax, 3                  ; version 3
    jb @@load_err
    
    ; Parse tensor count (offset 8) - GGUF v3 format
    mov rax, QWORD PTR [rsi + 8]
    ; n_tensors stored for later use
    
    ; Parse KV count (offset 16)
    mov rcx, QWORD PTR [rsi + 16]
    ; n_kv stored for later use
    
    ; Store context: [0]=file handle, [8]=mapping handle, [16]=base ptr
    ; [24]=file size, [32]=n_tensors, [40]=n_kv
    mov [rbx], r13
    mov [rbx + 8], r15
    mov [rbx + 16], rsi
    mov rax, QWORD PTR [rsp]    ; file size from GetFileSizeEx
    mov [rbx + 24], rax
    mov rax, QWORD PTR [rsi + 8]
    mov [rbx + 32], rax
...
;==============================================================================
; DequantizeRow_Q4_K - Dequantize Q4_K quantized data to FP32
; Q4_K format: 256 elements per block with two scales (d, dmin)
; Structure: 2B d (FP16) + 2B dmin (FP16) + 12B scales + 4B mins + 128B quants
; RCX = pSrc, RDX = pDst, R8 = numElements
;==============================================================================

DequantizeRow_Q4_K PROC
    ...
    mov rsi, rcx            ; source Q4_K data
    mov rdi, rdx            ; destination FP32
    mov r12, r8             ; total elements
    
    ; Process in blocks of 256
    mov r13, r12
    shr r13, 8              ; numBlocks = numElements / 256
    test r13, r13
    jz @@q4k_done
    
@@q4k_block_loop:
    ; Load d (FP16 scale at offset 0) - convert to FP32
    movzx eax, word ptr [rsi]
    shl eax, 16
    movd xmm0, eax          ; d in xmm0
    
    ; Load dmin (FP16 at offset 2)
    movzx eax, word ptr [rsi + 2]
    shl eax, 16
    movd xmm1, eax          ; dmin in xmm1
    
    ; Skip header (4 bytes d/dmin + 12 bytes scales + 4 bytes mins = 20)
    ; Quants start at offset 20, 128 bytes for 256 4-bit values
    lea r14, [rsi + 20]     ; quant data pointer
    
    ; Process 128 bytes = 256 quants
    mov r10, 128
...
@@q4k_byte_loop:
    ...
    ; Advance to next block (total block size = 148 bytes for Q4_K)
    add rsi, 148
...
;==============================================================================
; DequantizeRow_Q6_K - Dequantize Q6_K quantized data to FP32
; Q6_K format: 256 elements per block with 6-bit quants
; Structure: complex layout with separate low/high bits
; RCX = pSrc, RDX = pDst, R8 = numElements
;==============================================================================

DequantizeRow_Q6_K PROC
...
@@q6k_block_loop:
    ...
    ; - 128 bytes: low 4 bits (ql)
    ; - 64 bytes: high 2 bits (qh)
    ; - 16 bytes: scales
    ; - 2 bytes: d (FP16)
    ; Total: 210 bytes per block
    
    ; Load d from end of block (offset 208)
    movzx eax, word ptr [rsi + 208]
    shl eax, 16
    movd xmm0, eax          ; d scale
    
    lea r14, [rsi]          ; ql pointer (low 4 bits)
    lea r15, [rsi + 128]    ; qh pointer (high 2 bits)
    
    ; Process 256 elements
    mov r10, 128            ; 128 byte pairs
    xor r11, r11            ; qh bit index
    
@@q6k_elem_loop:
    ; Load low nibble from ql
    movzx eax, byte ptr [r14]
    mov edx, eax
    and edx, 0Fh            ; low 4 bits of first quant
    
    ; Get high 2 bits from qh
    mov r8d, r11d
    shr r8d, 2              ; qh byte index
    movzx ebx, byte ptr [r15 + r8]
    mov ecx, r11d
    and ecx, 3
    shl ecx, 1              ; bit position * 2
    shr ebx, cl             ; shift using cl
    and ebx, 3              ; extract 2 bits
    shl ebx, 4
    or edx, ebx             ; combine to 6-bit value
    
    sub edx, 32             ; center around 0 (-32 to +31)
    mov [rsp], edx
    fild dword ptr [rsp]
    fstp dword ptr [rsp]
    movss xmm1, dword ptr [rsp]
    ...
    ; Advance to next block (210 bytes)
    add rsi, 210
