...
using RawrXD::Recovery::AsmRecoveryResult;
using RawrXD::Recovery::AsmRecoveryStats;
using RawrXD::Recovery::AsmBridgeType;
...
void DiskRecoveryAsmAgent::Cleanup() {
    if (m_ctx) {
        ...
        m_bridgeType = AsmBridgeType::Unknown;
    }
}
...
AsmRecoveryResult DiskRecoveryAsmAgent::Initialize(int driveNum) {
    if (m_ctx) {
        return AsmRecoveryResult::error("Already initialized \u2014 call Cleanup first");
    }
    ...
    return AsmRecoveryResult::ok("Initialized for PhysicalDrive" + std::to_string(driveNum));
}
...
AsmRecoveryResult DiskRecoveryAsmAgent::ExtractEncryptionKey() {
    if (!m_ctx) {
        return AsmRecoveryResult::error("Not initialized");
    }
    ...
    if (result) {
        ...
        return AsmRecoveryResult::ok("Key extracted successfully");
    } else {
        ...
        return AsmRecoveryResult::error("Hardware key extraction failed (bridge unresponsive)");
    }
}
...
AsmRecoveryResult DiskRecoveryAsmAgent::RunRecovery() {
    ...
    return AsmRecoveryResult::ok(summary);
}
...
AsmRecoveryStats DiskRecoveryAsmAgent::GetStats() const {
    AsmRecoveryStats stats = {};
    if (m_ctx) {
