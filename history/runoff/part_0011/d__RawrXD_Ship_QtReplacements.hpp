...
public:
    ...
    int lastIndexOf(const QString& substr) const {
...
public:
    QFile() = default;
    explicit QFile(const QString& name) : m_name(name.toStdWString()) {}

    bool open(const wchar_t* mode) {
        m_stream.open(m_name, std::ios::in | std::ios::binary);
...
public:
    ...
    struct Match {
        bool hasMatch = false;
        Vector<QString> capturedGroups;

        QString group(int index) const {
            return index < static_cast<int>(capturedGroups.size()) ? capturedGroups[index] : QString();
        }
    };
    ...
    Match match(const QString& subject) const {
        ...
        if (std::regex_search(subj, matches, m_pattern)) {
            ...
            for (const auto& m : matches) {
                result.capturedGroups.push_back(QString(m.str()));
            }
        }
        return result;
    }
    ...
    Vector<Match> globalMatch(const QString& subject) const {
        ...
        for (auto it = begin; it != end; ++it) {
            ...
            for (const auto& sub : *it) {
                m.capturedGroups.push_back(QString(sub.str()));
            }
            results.push_back(m);
        }
        return results;
