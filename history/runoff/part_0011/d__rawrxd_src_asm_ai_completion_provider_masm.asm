...
@@score_loop:
    ...
    ; Simple scalar reduction using basic AVX
    ; For now, just work with lower 128 bits  
    vhaddps xmm2, xmm2, xmm2       ; Horizontal add on lower 128 bits
    vhaddps xmm2, xmm2, xmm2       ; Horizontal add again
    
    vmovss  dword ptr [r9+r10*4], xmm2
...
asm_softmax_normalize PROC FRAME
    ...
    vmovss  xmm1, dword ptr [rcx]   ; Max = first element
    
@@find_max:
    cmp     r11, r10
    jge     @@max_done
    vmaxss  xmm1, xmm1, dword ptr [rcx+r11*4]
    inc     r11
    jmp     @@find_max
...
@@max_done:
    ...
    xor     r11, r11
    ; Use movss + shufps for broadcast compatibility
    movss   xmm2, xmm1
    shufps  xmm2, xmm2, 0          ; Broadcast scalar to all lanes
...
asm_topk_sampling PROC FRAME
    ...
    vmovss  xmm0, dword ptr [rcx]   ; Best value
...
@@argmax_loop:
    ...
    vmovss  xmm1, dword ptr [r9+r12*4]
    vcomiss xmm1, xmm0
    jbe     @@not_better
    
    movss   xmm0, xmm1              ; Use consistent non-VEX forms
    mov     rax, r12
