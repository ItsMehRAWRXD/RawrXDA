...
public:
    ...
    MasmOperationResult initialize(uint64_t agent_id, size_t buffer_size,
        ...
        context_.reasoning_buffer = std::malloc(buffer_size);
    ...
    ~AgentMasmBridge() {
        if (context_.reasoning_buffer) {
            std::free(context_.reasoning_buffer);
        }
    }
};
...
public:
    ...
    MasmOperationResult initializeInference(void* model_base, size_t model_size,
        ...
        inference_context_.input_tensor_buffer = std::malloc(tensor_size * batch_size);
        inference_context_.output_tensor_buffer = std::malloc(tensor_size * batch_size);
    ...
    ~AiMasmBridge() {
        if (inference_context_.input_tensor_buffer) {
            std::free(inference_context_.input_tensor_buffer);
        }
        if (inference_context_.output_tensor_buffer) {
            std::free(inference_context_.output_tensor_buffer);
        }
    }
};