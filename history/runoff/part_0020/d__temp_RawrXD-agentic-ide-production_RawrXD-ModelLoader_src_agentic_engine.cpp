...
void AgenticEngine::initialize() {
    ...
    qInfo() << "[AgenticEngine] Inference engine created";
        // Forward inference engine streaming signals so UI can subscribe to token text
        connect(m_inferenceEngine, &InferenceEngine::streamToken,
            this, [this](qint64 reqId, const QString& token) {
            Q_UNUSED(reqId);
            // preserve existing numeric token progress signal
            emit tokenGenerated(1);
            // forward textual token with request id
            emit streamToken(reqId, token);
            });

        connect(m_inferenceEngine, &InferenceEngine::streamFinished,
            this, &AgenticEngine::streamFinished);

        // Forward final resultReady from inference engine to responseReady for backward compatibility
        connect(m_inferenceEngine, QOverload<qint64, const QString&>::of(&InferenceEngine::resultReady),
            this, [this](qint64 /*reqId*/, const QString& result) {
...
QString AgenticEngine::generateTokenizedResponse(const QString& message) {
    ...
    if (m_inferenceEngine && m_inferenceEngine->isModelLoaded()) {
        qDebug() << "Using real inference engine for streaming response generation";

        try {
            // Create a request id for this streaming task
            qint64 reqId = QDateTime::currentMSecsSinceEpoch();

            // Start streaming via the InferenceEngine; tokens will be emitted via streamToken(reqId, token)
            int maxTokens = m_genConfig.maxTokens > 0 ? m_genConfig.maxTokens : 256;
            m_inferenceEngine->generateStreaming(reqId, message, maxTokens);

            // Notify listeners that streaming has started so they can map reqId -> UI panel
            emit streamingStarted(reqId);

            // Final result and completion will be forwarded via the connected signals (resultReady/streamFinished)
            return QString(); // Response will be delivered asynchronously
        } catch (const std::exception& e) {
            qCritical() << "Model streaming aborted:" << e.what();
            return QString("❌ Model error: %1").arg(e.what());
        } catch (...) {
            qCritical() << "Unknown fatal error starting streaming inference";
...
QString AgenticEngine::generateResponseSync(const QString& message) {
    if (m_modelLoaded && m_inferenceEngine && m_inferenceEngine->isModelLoaded()) {
        try {
            auto tokens = m_inferenceEngine->tokenize(message);
            auto generatedTokens = m_inferenceEngine->generate(tokens, m_genConfig.maxTokens);
