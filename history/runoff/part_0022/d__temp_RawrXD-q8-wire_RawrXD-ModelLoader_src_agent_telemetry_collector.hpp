...
public:
     ...
     */
    void trackFeatureUsage(const std::string& featureName, const nlohmann::json& metadata = nlohmann::json());
...
private:
    TelemetryCollector() : m_enabled(false) {}
    bool m_enabled;
    std::string m_sessionId;  ///< Anonymous session identifier (not persistent)
    std::map<std::string, int> m_featureUsage;  ///< Feature name -> usage count
