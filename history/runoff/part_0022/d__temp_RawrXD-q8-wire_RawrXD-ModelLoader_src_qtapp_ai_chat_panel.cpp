...
void AIChatPanel::setupUI()
{
    ...
    // Apply no-highlight styling and enable input
    setStyleSheet("QPushButton:checked { background: none; }");
...
QWidget* AIChatPanel::createQuickActions()
{
    ...
    // Add Options button with Agent mode functionality
    QPushButton* opts = new QPushButton("Options", container);
    opts->setMaximumHeight(26);
    opts->setFlat(true);
    opts->setCursor(Qt::PointingHandCursor);
    
    connect(opts, &QPushButton::clicked, this, [this](){
        auto dlg = new QDialog(this);
        auto lay = new QVBoxLayout(dlg);
        auto tabs = new QTabWidget(dlg);
        tabs->addTab(new QWidget,"Explain");
        tabs->addTab(new QWidget,"Fix");
        tabs->addTab(new QWidget,"Refactor");
        tabs->addTab(new QWidget,"Document");
        tabs->addTab(new QWidget,"Test");
        tabs->addTab(new QWidget,"Agent");
        tabs->addTab(new QWidget,"Plan");
        tabs->addTab(new QWidget,"Ask");
        tabs->addTab(new QWidget,"Edit");
        lay->addWidget(tabs);
        auto smart = new QComboBox;
        smart->addItem("Auto(Smart Selection)");
        smart->addItems(QStringList{"llama2", "mistral", "neural-chat", "dolphin-mixtral"});
        lay->addWidget(smart);
        dlg->exec();
    });
...
void AIChatPanel::clearHistory()
{
    // Alias for clear() - used by MainWindow
    clear();
}
