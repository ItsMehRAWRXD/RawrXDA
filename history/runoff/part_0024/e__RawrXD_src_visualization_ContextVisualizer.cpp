...
void BreadcrumbRenderer::renderBreadcrumbTrail(DrawingContext& ctx, const QList<Breadcrumb>& trail, const Rect& bounds) {
...
QString BreadcrumbRenderer::getIconPath(ContextType type) {
    switch (type) {
...
void ContextPanelRenderer::renderSymbolPanel(DrawingContext& ctx, const QString& filePath, const Rect& bounds) {
    ...
    for (const auto& symbol : symbols) {
        ...
        QString itemText = QString("%1 (%2)").arg(symbol.name, QString::number(symbol.lineNumber));
        renderListItem(ctx, itemText, itemBounds);
        y += 28.0f;
    }
}

void ContextPanelRenderer::renderFilePanel(DrawingContext& ctx, const QString& filePath, const Rect& bounds) {
    ...
    // Display file info
    ctx.drawText(
        QString("Path: %1").arg(fileCtx.fileName),
        Point(bounds.x + 10, y),
    ...
    ctx.drawText(
        QString("Size: %1 bytes").arg(fileCtx.fileSize),
        Point(bounds.x + 10, y),
        "Arial", 11.0f, Color(0, 0, 0, 255)
    );
    y += 20.0f;
    ...
    ctx.drawText(
        QString("Modified: %1").arg(fileCtx.lastModified.toString("yyyy-MM-dd hh:mm")),
        Point(bounds.x + 10, y),
        "Arial", 11.0f, Color(0, 0, 0, 255)
    );
}
...
void ContextPanelRenderer::renderToolsPanel(DrawingContext& ctx, const Rect& bounds) {
    ...
    for (const auto& tool : tools) {
        Rect itemBounds(bounds.x, y, bounds.width, 24.0f);
        QString status = tool.isAvailable ? "✓" : "✗";
        QString itemText = QString("[%1] %2").arg(status, tool.toolName);
        renderListItem(ctx, itemText, itemBounds);
        y += 28.0f;
    }
}
...
void ContextPanelRenderer::renderSourceControlPanel(DrawingContext& ctx, const Rect& bounds) {
    ...
    ctx.drawText(
        QString("Repository: %1").arg(scCtx.repository),
        Point(bounds.x + 10, y),
        "Arial", 11.0f, Color(0, 0, 0, 255)
    );
    y += 20.0f;
    ...
    ctx.drawText(
        QString("Branch: %1").arg(scCtx.branch),
        Point(bounds.x + 10, y),
        "Arial", 11.0f, Color(0, 0, 0, 255)
    );
    y += 20.0f;
    ...
    ctx.drawText(
        QString("Commit: %1...").arg(scCtx.commitHash.left(8)),
        Point(bounds.x + 10, y),
        "Arial", 11.0f, Color(0, 0, 0, 255)
    );
}

void ContextPanelRenderer::renderInstructionsPanel(DrawingContext& ctx, const QString& filePath, const Rect& bounds) {
...
void ContextPanelRenderer::renderRelationshipsPanel(DrawingContext& ctx, const QString& entityId, const Rect& bounds) {
...
void ContextPanelRenderer::renderOpenEditorsPanel(DrawingContext& ctx, const Rect& bounds) {
    ...
    for (const auto& editor : editors) {
        Rect itemBounds(bounds.x, y, bounds.width, 24.0f);
        QString editorText = QString("%1:%2").arg(editor.cursorLine).arg(editor.cursorColumn);
        renderListItem(ctx, editorText, itemBounds);
        y += 28.0f;
    }
}

void ContextPanelRenderer::renderHeader(DrawingContext& ctx, const QString& title, const Rect& bounds) {
...
void ContextPanelRenderer::renderListItem(DrawingContext& ctx, const QString& text, const Rect& bounds, bool selected) {
    if (selected) {
        FillStyle fill;
...
void ContextPanelRenderer::renderStatusBadge(DrawingContext& ctx, const QString& status, const Point& pos, const Color& color) {
...
void ContextGraphRenderer::renderDependencyGraph(DrawingContext& ctx, const QString& centerEntity, const Rect& bounds) {
...
void ContextGraphRenderer::renderFileRelationships(DrawingContext& ctx, const QString& filePath, const Rect& bounds) {
    ...
    centerNode.label = QString(filePath).split("/").last();
    centerNode.position = bounds.center();
    nodes.push_back(centerNode);
...
void ContextGraphRenderer::renderSymbolCallGraph(DrawingContext& ctx, const QString& symbolName, const Rect& bounds) {
...
void ContextGraphRenderer::renderEdge(DrawingContext& ctx, const Point& from, const Point& to, const QString& label) {
    StrokeStyle style;
    style.width = 2.0f;
...
void ContextWindow::navigateToEntity(const QString& entityId) {
    m_currentEntity = entityId;
    m_contextManager.pushContextBreadcrumb(ContextType::Tool, entityId);
}

void ContextWindow::addTab(const QString& tabName) {
    m_tabVisibility[tabName] = true;
}

void ContextWindow::selectTab(const QString& tabName) {
...
void ContextWindow::renderTabs(DrawingContext& ctx) {
    ...
    QStringList tabs = {"Breadcrumbs", "Context", "Graph"};
...
void ScreenshotAnnotator::removeAnnotation(const QString& annotationId) {
...
void InstructionPanel::render(DrawingContext& ctx) {
    ...
    FillStyle bgFill;
    bgFill.solidColor = Color(m_instruction.backgroundColor.isEmpty() ? QColor(240, 240, 240) : QColor(m_instruction.backgroundColor)).rgba();
    ctx.drawRect(m_bounds, bgFill);
    
    StrokeStyle border;
    border.width = 2.0f;
    border.color = Color(m_instruction.borderColor.isEmpty() ? QColor(100, 150, 200) : QColor(m_instruction.borderColor)).rgba();
...
void FileBrowser::expandFolder(const QString& folderPath) {
    m_expandedFolders.insert(folderPath);
}

void FileBrowser::collapseFolder(const QString& folderPath) {
