...
init_application PROC
    ...
    ; ============================================================================
    ; Initialize Zero-Day Agentic Engine
    ; ============================================================================
    
    ; Log startup
    LEA rcx, [rel dbg_init_welcome]
    MOV rdx, LOG_LEVEL_INFO
    CALL Logger_LogStructured
    
    ; Create engine instance
    XOR rcx, rcx        ; router = NULL
    XOR rdx, rdx        ; tools = NULL
    XOR r8, r8          ; planner = NULL
    XOR r9, r9          ; callbacks = NULL
    CALL ZeroDayAgenticEngine_Create
    MOV [engine_handle], rax
    
    ; Check creation success
    CMP rax, 0
    JE init_engine_failed
    
    ; Log successful engine creation
    LEA rcx, [rel dbg_init_ml_init]
    MOV rdx, LOG_LEVEL_INFO
    CALL Logger_LogStructured
    ...
    jmp init_after_engine
    
init_engine_failed:
    ; Log engine creation failure
    LEA rcx, [rel error_no_model]
...
main PROC
    ...
    ; ============================================================================
    ; PHASE 3: Initialize configuration system (external config + env vars)
    ; ============================================================================
    CALL Config_Initialize
    TEST rax, rax
    JZ config_init_failed
...
config_init_failed:
    ; Log configuration initialization failure
    lea rcx, dbg_probe_main0
    call file_log_append
