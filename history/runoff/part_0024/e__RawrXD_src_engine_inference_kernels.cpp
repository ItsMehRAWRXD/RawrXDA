...
void InferenceKernels::gelu_avx512(float* x, int n) {
    ...
    for (int i = 0; i < n; i += 16) {
        ...
        __m512 inner = _mm512_fmadd_ps(_mm512_set1_ps(0.044715f), x3, v);
        inner = _mm512_mul_ps(_mm512_set1_ps(0.7978845608f), inner);
        
        // tanh approximation
        __m512 tanh_val = _mm512_tanh_ps(inner); // Requires AVX512 ER or SVML, simplified for std context
        
        __m512 result = _mm512_mul_ps(v, _mm512_set1_ps(0.5f));
        result = _mm512_mul_ps(result, _mm512_add_ps(_mm512_set1_ps(1.0f), tanh_val));
