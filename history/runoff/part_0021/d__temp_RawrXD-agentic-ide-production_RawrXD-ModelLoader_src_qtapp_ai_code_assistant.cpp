...
AICodeAssistant::AICodeAssistant(QObject *parent)
        : QObject(parent),
            ...
            m_aiRequestTimeout(30000),
            m_searchTimeout(60000),
            m_commandTimeout(30000)
{
...
// Wrapper overload to satisfy panel calls
void AICodeAssistant::getCodeCompletion(const QString &code) {
    getCodeCompletion(code, QString());
}
...
void AICodeAssistant::searchFiles(const QString &pattern, const QString &directory) {
    ...
    try {
        // Parallel file search using Qt Concurrent
        QThreadPool *threadPool = QThreadPool::globalInstance();
        QMutex resultsMutex;
        int totalProcessed = 0;
        
        auto processDirectory = [this, &pattern, &results, &resultsMutex, &totalProcessed](const QString &dirPath) {
            QStringList localResults;
            int localProcessed = 0;
            recursiveSearch(dirPath, pattern, localResults, localProcessed);
        ...
        for (const QFileInfo &entry : entries) {
            threadPool->start([=]() { processDirectory(entry.absoluteFilePath()); });
        }
...
void AICodeAssistant::grepFiles(const QString &pattern, const QString &directory, bool caseSensitive) {
    ...
    try {
        ...
        emitOperationMetrics("grepFiles", elapsed, {
            {"matchesFound", QString::number(results.count())}
        });
    } catch (const std::exception &e) {
...
void AICodeAssistant::executePowerShellCommand(const QString &command) {
    ...
    logStructured("INFO", "PowerShell command completed", {
        ...
        emitOperationMetrics("executePowerShellCommand", elapsed, {
            {"command", command},
            {"exitCode", QString::number(exitCode)}
        });
...
void AICodeAssistant::runBuildCommand(const QString &command) {
    ...
    logStructured("INFO", "Build completed", {
        ...
        emitOperationMetrics("runBuildCommand", elapsed, {
            {"command", command},
            {"exitCode", QString::number(exitCode)}
        });
...
void AICodeAssistant::runTestCommand(const QString &command) {
    ...
    emitOperationMetrics("runTestCommand", elapsed, {
        {"command", command},
        {"exitCode", QString::number(exitCode)}
    });
...
void AICodeAssistant::analyzeAndRecommend(const QString &context) {
    ...
    // Emit metrics after request sent
    qint64 elapsed = getElapsedMilliseconds();
    emitOperationMetrics("analyzeAndRecommend", elapsed, {
        {"contextLength", QString::number(context.length())}
    });
}
...
void AICodeAssistant::autoFixIssue(const QString &issueDescription, const QString &codeContext) {
    ...
    // Emit metrics after request sent
    qint64 elapsed = getElapsedMilliseconds();
    emitOperationMetrics("autoFixIssue", elapsed, {
        {"issueLength", QString::number(issueDescription.length())}
    });
}
...
void AICodeAssistant::sendAIRequest(const QString &prompt, const QString &responseSignal) {
    // Check cache first
    if (m_aiCache.contains(prompt)) {
        m_cacheHits++;
        // Emit cached response
        if (responseSignal == "completionReady") {
            emit completionReady(m_aiCache[prompt]);
        } else if (responseSignal == "refactoringReady") {
            emit refactoringReady(m_aiCache[prompt]);
        } else if (responseSignal == "explanationReady") {
            emit explanationReady(m_aiCache[prompt]);
        } else if (responseSignal == "bugFixReady") {
            emit bugFixReady(m_aiCache[prompt]);
        } else if (responseSignal == "optimizationReady") {
            emit optimizationReady(m_aiCache[prompt]);
        } else if (responseSignal == "analysisReady") {
            // For analysis, we need to parse JSON
            QJsonObject obj = QJsonDocument::fromJson(m_aiCache[prompt].toUtf8()).object();
            emit analysisReady(obj);
        } else if (responseSignal == "autoFixReady") {
            emit autoFixReady(m_aiCache[prompt]);
        }
        return;
    }
    m_cacheMisses++;
...
void AICodeAssistant::preloadModel(const QString &modelName) {
    // Simple async request to Ollama to pull model
    QNetworkRequest request(QUrl(QString("http://%1:%2/api/pull").arg(m_ollamaHost).arg(m_ollamaPort)));
    request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    QJsonObject payload{{"name", modelName}};
    m_networkManager->post(request, QJsonDocument(payload).toJson());
}

void AICodeAssistant::setCacheSize(size_t size) {
    m_cacheSize = size;
    // Trim cache if necessary
    while (m_aiCache.size() > m_cacheSize) {
        auto it = m_aiCache.begin();
        m_aiCache.erase(it);
    }
}

void AICodeAssistant::clearCache() {
    m_aiCache.clear();
    m_cacheHits = 0;
    m_cacheMisses = 0;
}

size_t AICodeAssistant::cacheSize() const { return m_aiCache.size(); }
size_t AICodeAssistant::cacheHits() const { return m_cacheHits; }
size_t AICodeAssistant::cacheMisses() const { return m_cacheMisses; }

void AICodeAssistant::setAIRequestTimeout(int milliseconds) {
    m_aiRequestTimeout = milliseconds;
}
void AICodeAssistant::setSearchTimeout(int milliseconds) {
    m_searchTimeout = milliseconds;
}
void AICodeAssistant::setCommandTimeout(int milliseconds) {
    m_commandTimeout = milliseconds;
}

void AICodeAssistant::emitOperationMetrics(const QString &operation, qint64 durationMs, const QJsonObject &extra) {
    QJsonObject metrics{{"operation", operation}, {"durationMs", QString::number(durationMs)}};
    for (auto it = extra.constBegin(); it != extra.constEnd(); ++it) {
        metrics.insert(it.key(), it.value());
    }
    emit operationMetrics(metrics);
}
...
// Stubbed process handlers (not used in current implementation)
