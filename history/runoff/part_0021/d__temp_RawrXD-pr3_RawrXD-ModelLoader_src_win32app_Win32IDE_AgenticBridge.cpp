...
bool AgenticBridge::Initialize(const std::string& frameworkPath, const std::string& modelName) {
    ...
    // Install MCP transport hooks for raw buffer visibility
    if (InstallMCPTransportHooks()) {
        LOG_INFO("MCP transport hooks installed - full message visibility enabled");
    } else {
        LOG_WARNING("MCP transport hooks not installed - running without intercepts");
    }
...
// =============================================================================
// MCP Transport Hook Integration
// =============================================================================

bool AgenticBridge::InstallMCPTransportHooks() {
    if (m_mcpHooksInstalled) {
        LOG_WARNING("MCP hooks already installed");
        return true;
    }
    
    LOG_INFO("Installing MCP transport hooks for raw buffer visibility...");
    
    if (InstallMCPHooks()) {
        m_mcpHooksInstalled = true;
        
        // Set up logging callback to route MCP messages through IDE output
        auto& mgr = MCPHookManager::GetInstance();
        
        mgr.SetReadMessageCallback([this](const uint8_t* buf, size_t len, const char* src) {
            if (m_outputCallback && len > 0) {
                std::string preview(reinterpret_cast<const char*>(buf), 
                                   std::min(len, size_t(128)));
                m_outputCallback("[MCP-IN]", preview);
            }
        });
        
        mgr.SetWriteMessageCallback([this](const uint8_t* buf, size_t len, const char* dst) {
            if (m_outputCallback && len > 0) {
                std::string preview(reinterpret_cast<const char*>(buf), 
                                   std::min(len, size_t(128)));
                m_outputCallback("[MCP-OUT]", preview);
            }
        });
        
        LOG_INFO("MCP hooks installed: readMessage, writeMessage, onSocketData, WebSocket");
        return true;
    }
    
    LOG_ERROR("Failed to install MCP transport hooks");
    return false;
}

void AgenticBridge::UninstallMCPTransportHooks() {
    if (!m_mcpHooksInstalled) {
        return;
    }
...
void AgenticBridge::SetMCPReadCallback(ReadMessageCallback callback) {
    if (m_mcpHooksInstalled) {
        MCPHookManager::GetInstance().SetReadMessageCallback(callback);
    }
}

void AgenticBridge::SetMCPWriteCallback(WriteMessageCallback callback) {
    if (m_mcpHooksInstalled) {
        MCPHookManager::GetInstance().SetWriteMessageCallback(callback);
    }
}

std::vector<MCPMessage> AgenticBridge::GetRecentMCPMessages(size_t count) const {
    if (m_mcpHooksInstalled) {
        return MCPHookManager::GetInstance().GetRecentMessages(count);
    }
    return {};
}
