...
void AICodeAssistantPanel::onApplySuggestion() {
    ...
    emit applySuggestionRequested(current_suggestion_.suggested_code,
                                  current_suggestion_.type,
                                  current_suggestion_.context);

    explanation_display_->setText(QString("<span style='color: green;'><b>Applied:</b> %1</span>")
                                      .arg(current_suggestion_.type));
    qDebug() << "[AICodeAssistantPanel] Applied suggestion" << current_suggestion_.type;
}
...
void AICodeAssistantPanel::onExportSuggestion() {
    ...
    const QString defaultName = QString("ai_suggestion_%1.txt")
                                    .arg(QDateTime::currentDateTime().toString("yyyyMMdd_HHmmss"));
    const QString targetPath = QFileDialog::getSaveFileName(
        this,
        tr("Export AI Suggestion"),
    ...
    if (!outFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        const QString errorText = QString("<span style='color: red;'><b>Export failed:</b> %1</span>")
                                      .arg(outFile.errorString());
        explanation_display_->setText(errorText);
        qWarning() << "[AICodeAssistantPanel] Export failed" << outFile.errorString();
        return;
    }
    ...
    emit exportSuggestionRequested(current_suggestion_.suggested_code,
                                   current_suggestion_.type,
                                   current_suggestion_.context,
                                   targetPath);

    explanation_display_->setText(QString("<span style='color: green;'><b>Exported:</b> %1</span>")
                                      .arg(QFileInfo(targetPath).fileName()));
    qDebug() << "[AICodeAssistantPanel] Exported suggestion to" << targetPath;
}
