...
// Native UI helper functions (headless or Win32 implementation)
#include "native_ui.h"
...
{
    Logger::instance().info("AgentOrchestra constructor start");
    setupVoiceProcessor();
    ...
    Logger::instance().info("AgentOrchestra UI initialized");
...
{
    // Create main window
    g_mainWindow.create("Agent Orchestra", 800, 600);
    g_mainWindow.show();

    // Chat view and input field
    Native_CreateChatView();
    Native_CreateInputField();

    // Buttons
    void* sendBtn = Native_CreateButton("Send");
    Native_SetButtonCallback(sendBtn, [this]() { onSendButtonClicked(); });

    void* voiceBtn = Native_CreateButton("\xF0\x9F\x8E\xA4 Voice");
    Native_SetButtonCallback(voiceBtn, [this]() { onVoiceButtonClicked(); });

    void* clearBtn = Native_CreateButton("Clear");
    Native_SetButtonCallback(clearBtn, [this]() { clearHistory(); });

    // Agent selector combo
    void* agentCombo = Native_CreateCombo();
    Native_AddComboItem(agentCombo, "coder", 0);
    Native_AddComboItem(agentCombo, "designer", 1);
    Native_AddComboItem(agentCombo, "analyst", 2);
    Native_AddComboItem(agentCombo, "orchestrator", 3);
    Native_SetComboCallback(agentCombo, [this](int idx, const std::string&) { onAgentSelected(idx); });

    // Accent selector combo
    void* accentCombo = Native_CreateCombo();
    ...
    Native_AddComboItem(accentCombo, "GBE", static_cast<int>(VoiceProcessor::GBE));
...
{
    // Run a Win32 message loop so the UI stays responsive.
    MSG msg;
...
void AgentOrchestra::addAgent(const std::string& agentId, const std::string& agentName)
{
    ...
    Logger::instance().debug("Added agent " + agentId + " named " + agentName);
}
...
void AgentOrchestra::setActiveAgent(const std::string& agentId)
{
    if (m_agents.find(agentId) != m_agents.end()) {
        ...
        Logger::instance().info("Active agent set to " + agentId);
    }
}
...
void AgentOrchestra::startVoiceInput()
{
    ...
    if (m_voiceProcessor->startRecording()) {
        ...
        Metrics::instance().incVoiceStart();
...
void AgentOrchestra::sendMessage(const std::string& message)
{
    ...
    Logger::instance().info("User message: " + trimmed);
    addMessageToChat("You", trimmed, false);
    Metrics::instance().incMessagesProcessed();
    ...
    if (!m_activeAgentId.empty()) {
        ...
        Metrics::instance().observeModelCallMs(ms);
        ...
        Logger::instance().info("Agent " + m_activeAgentId + " responded");
...
void AgentOrchestra::addMessageToChat(const std::string& sender, const std::string& message, bool isAgent)
{
    ...
    // Update the UI chat view if available. This mirrors the original Qt implementation
    // where messages were added to a QTextEdit. The native UI provides
    // Native_AppendChatMessage(sender, message, timestamp, isAgent).
    // Guard against missing UI (e.g., headless mode) – the function is a no‑op when
    // the chat view control is not created.
    Native_AppendChatMessage(sender, message, timestamp, isAgent);
}
...
void AgentOrchestra::setupVoiceProcessor()
{
    ...
    m_voiceProcessor->setTextRecognizedCallback([this](const std::string& text) {
        if (m_voiceCallback) m_voiceCallback(text);
        sendMessage(text);
    });

    m_voiceProcessor->setErrorCallback([this](const std::string& error) {
    ...
    m_voiceProcessor->setRecordingStateCallback([this](bool recording) {
        m_isVoiceActive = recording;
    });
}

// ---------------------------------------------------------------------------
// UI event handlers – these forward native UI actions to the core logic.
// ---------------------------------------------------------------------------

void AgentOrchestra::onSendButtonClicked()
{
    std::string txt = Native_GetInputText();
    sendMessage(txt);
...
{
    static const std::vector<std::string> ids = {"coder", "designer", "analyst", "orchestrator"};
    if (index >= 0 && static_cast<size_t>(index) < ids.size()) {
        const std::string &agentId = ids[index];
        setActiveAgent(agentId);
        Native_SelectAgentListRowByIndex(index);
        std::string name = m_agents.count(agentId) ? m_agents[agentId] : "Agent";
        addMessageToChat("System", std::string("Switched to ") + name, true);
    }
}
...
void AgentOrchestra::processAgentResponse(const std::string& agentId, const std::string& message)
{
    ...
    std::string name = m_agents.count(agentId) ? m_agents[agentId] : "Agent";
    addMessageToChat(name, response, true);
}
