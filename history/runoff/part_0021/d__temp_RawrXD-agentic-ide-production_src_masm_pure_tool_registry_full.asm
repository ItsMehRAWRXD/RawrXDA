...
; Original: tool_registry_full.asm
; Conversion Date: 2025-12-25 04:34
; ============================================================================
...
; Tool Registry
MAX_TOOL_COUNT      equ 70  ; Increased from 62 to 70 for meta-agent tools
...
; Meta-agent system exports (pure MASM layer)
EXTERN MetaAgentSystem_Initialize:PROC
EXTERN MetaAgentSystem_CreateAgentFromSpec:PROC
EXTERN MetaAgentSystem_ExecuteAgentRules:PROC
EXTERN MetaAgentSystem_GetAgentCount:PROC
...
.data
    ...
    ; Meta-Agent tools (4)
    szTool_MetaAgentCreate db "meta_agent_create",0
    szTool_MetaAgentModify db "meta_agent_modify",0
    szTool_MetaAgentExecute db "meta_agent_execute",0
    szTool_MetaAgentCount db "meta_agent_count",0
...
; ---------------------------------------------------------------------------
; META-AGENT TOOL REGISTRATION
; ---------------------------------------------------------------------------
RegisterMetaAgentTools proc
    invoke AddToolToRegistry, offset szTool_MetaAgentCreate, offset Tool_MetaAgentCreate_Execute
    invoke AddToolToRegistry, offset szTool_MetaAgentModify, offset Tool_MetaAgentModify_Execute
    invoke AddToolToRegistry, offset szTool_MetaAgentExecute, offset Tool_MetaAgentExecute_Execute
    invoke AddToolToRegistry, offset szTool_MetaAgentCount, offset Tool_MetaAgentCount_Execute
    ret
RegisterMetaAgentTools endp
...
; ---------------------------------------------------------------------------
; META-AGENT TOOL EXECUTION FUNCTIONS
; ---------------------------------------------------------------------------
OP_ADD_RULE          equ 1
...
Tool_MetaAgentCreate_Execute proc pszSpec:DWORD
    LOCAL hRes:DWORD
    invoke GlobalAlloc, GMEM_FIXED, sizeof TOOL_RESULT
    mov hRes, eax

    ; Ensure meta-agent system is initialized
    call MetaAgentSystem_Initialize
    ...
    mov DWORD ptr [hRes+0], 1            ; success
    mov DWORD ptr [hRes+8], eax          ; return agent ID
    mov eax, hRes
    ret
...
@create_failed:
    ...
    mov eax, hRes
    ret
Tool_MetaAgentCreate_Execute endp

Tool_MetaAgentModify_Execute proc pszAgentId:DWORD, pszModType:DWORD, pszValue:DWORD
    LOCAL hRes:DWORD
    invoke GlobalAlloc, GMEM_FIXED, sizeof TOOL_RESULT
    mov hRes, eax
    ...
    ; Dispatch to appropriate self-modification primitive
    mov ecx, pszAgentId
    mov edx, pszModType
    mov r8, pszValue
    cmp edx, OP_ADD_RULE
    je @do_rule
    cmp edx, OP_REMOVE_RULE
    je @do_rule
    cmp edx, OP_ADD_SKILL
    je @do_skill
    cmp edx, OP_REMOVE_SKILL
    je @do_skill
    cmp edx, OP_UPDATE_BACKGROUND
    je @do_background
    jmp @mod_failed

@do_rule:
    call SelfModification_ApplyRuleChange
    test eax, eax
    jz @mod_failed
    jmp @mod_success

@do_skill:
    call SelfModification_ApplySkillChange
    test eax, eax
    jz @mod_failed
    jmp @mod_success

@do_background:
    call SelfModification_UpdateBackground
    test eax, eax
    jz @mod_failed

@mod_success:
    mov DWORD ptr [hRes+0], 1
    mov DWORD ptr [hRes+8], pszAgentId   ; echo back agent ID
    mov eax, hRes
    ret
...
@mod_failed:
    ...
    mov eax, hRes
    ret
Tool_MetaAgentModify_Execute endp

Tool_MetaAgentExecute_Execute proc pszContext:DWORD
    LOCAL hRes:DWORD
    invoke GlobalAlloc, GMEM_FIXED, sizeof TOOL_RESULT
    mov hRes, eax
    ...
    ; Execute rules for all agents
    mov rcx, pszContext
    call MetaAgentSystem_ExecuteAgentRules

    mov DWORD ptr [hRes+0], 1            ; success (0 rules executed still OK)
    mov DWORD ptr [hRes+8], eax          ; rules executed count
    mov eax, hRes
    ret
Tool_MetaAgentExecute_Execute endp

Tool_MetaAgentCount_Execute proc
    LOCAL hRes:DWORD
    invoke GlobalAlloc, GMEM_FIXED, sizeof TOOL_RESULT
    mov hRes, eax
    ...
    mov DWORD ptr [hRes+0], 1
    mov DWORD ptr [hRes+8], eax          ; agent count
    mov eax, hRes
    ret
Tool_MetaAgentCount_Execute endp
