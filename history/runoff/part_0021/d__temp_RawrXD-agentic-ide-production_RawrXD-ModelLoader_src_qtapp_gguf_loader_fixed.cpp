...
QByteArray GGUFLoaderQt::inflateWeight(const QString& tensorName)
{
    ...
    try {
        ...
        QByteArray decompressedData(reinterpret_cast<const char*>(data.data()), static_cast<int>(data.size()));
        
        qint64 totalElapsed = totalTimer.elapsed();
        qDebug() << QString("[GGUFLoaderQt] Tensor %1 loaded in %2ms")
            .arg(tensorName).arg(totalElapsed);
