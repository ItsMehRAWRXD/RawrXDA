...
ModelTestResult ModelTester::testWithOllama(
    ...
    m_logger->info("Testing model: {} with prompt: {}...", modelName, prompt.substr(0, 50));
...
std::vector<LatencyBenchmark> ModelTester::benchmarkModels(
    ...
    for (const auto& model : modelNames) {
        ...
        if (!allLatencies.empty()) {
            ...
            m_logger->info("Benchmark results for {}: avg={}ms, p95={}ms, p99={}ms",
                           model, std::fixed << std::setprecision(2) << bench.avgLatencyMs,
...
std::vector<ParsedCompletion> ModelTester::testResponseParsing(const std::string& modelOutput) {
    m_logger->debug("Testing response parsing on {} chars", modelOutput.length());
...
LatencyBenchmark ModelTester::measureLatencyDistribution(
    ...
    m_logger->info("Measuring latency distribution for {} ({} requests)", modelName, testCount);
...
bool ModelTester::validateModelResponse(
    ...
    m_logger->debug("Validating model response for: {}", modelName);
    ...
    m_logger->info("Validation result: {}", isValid ? "PASS" : "FAIL");
...
void ModelTester::resetResults() {
    m_logger->info("Resetting test results");
    m_testResults.clear();
...
std::string ModelTester::makeOllamaRequest(
    ...
    m_logger->debug("Making Ollama request to {} with {} chars payload", 
    ...
    std::string response = R"(
{
  ...
  "eval_duration": 876543
    ...
    m_logger->debug("Received response: {} chars", response.length());
    return response;
}
