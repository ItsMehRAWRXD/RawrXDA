...
/** Wire format for a hotpatch entry from list/apply/revert */
interface HotpatchEntry {
  name: string;
  layer: HotpatchLayer;
  kind: HotpatchSymbolKind;
  detail: string;
  filePath: string;
  address: string;   // uint64 as hex string
  size: number;
  active: boolean;
  hash: string;      // FNV-1a hash as hex string
}

/** Wire format for a hotpatch diagnostic */
interface HotpatchDiagnostic {
  severity: HotpatchDiagSeverity;
  code: number;
  message: string;
  source: string;
  layer: HotpatchLayer;
...
/** Wire format for GGUF model summary */
interface GGUFModelSummary {
  filePath: string;
  version: number;
  fileSize: number;
  tensorCount: number;
...
/** Wire format for a GGUF tensor entry */
interface GGUFTensorEntry {
  name: string;
  type: number;
  typeName: string;
  offset: string;    // uint64 as hex string
  size: number;
  dims: number[];
  nDims: number;
  hash: string;
...
/** Wire format for workspace stats */
interface WorkspaceStats {
  requestsHandled: number;
  notificationsSent: number;
  diagnosticRefreshes: number;
  symbolRebuilds: number;
  errors: number;
...
/** Hotpatch event notification payload */
interface HotpatchEventPayload {
  type: 'applied' | 'reverted' | 'failed' | 'conflicted';
  patchName: string;
  layer: HotpatchLayer;
  detail: string;
  timestamp: number;
...
/** GGUF load progress notification payload */
interface GGUFLoadProgressPayload {
  filePath: string;
  phase: string;
  percent: number;
  detail: string;
...
let hotpatchViewProvider: HotpatchTreeProvider;
let ggufViewProvider: GGUFTreeProvider;
let statsViewProvider: StatsTreeProvider;
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider('rawrxd.hotpatchView', hotpatchViewProvider),
    vscode.window.registerTreeDataProvider('rawrxd.ggufView', ggufViewProvider),
    vscode.window.registerTreeDataProvider('rawrxd.statsView', statsViewProvider),
  );

  // ---- Register Commands ----
  context.subscriptions.push(
    vscode.commands.registerCommand('rawrxd.hotpatch.list', cmdHotpatchList),
    vscode.commands.registerCommand('rawrxd.hotpatch.apply', cmdHotpatchApply),
    vscode.commands.registerCommand('rawrxd.hotpatch.revert', cmdHotpatchRevert),
    vscode.commands.registerCommand('rawrxd.hotpatch.diagnostics', cmdHotpatchDiagnostics),
    vscode.commands.registerCommand('rawrxd.gguf.modelInfo', cmdGGUFModelInfo),
    vscode.commands.registerCommand('rawrxd.gguf.tensorList', cmdGGUFTensorList),
    vscode.commands.registerCommand('rawrxd.gguf.validate', cmdGGUFValidate),
    vscode.commands.registerCommand('rawrxd.workspace.symbols', cmdWorkspaceSymbols),
    vscode.commands.registerCommand('rawrxd.workspace.stats', cmdWorkspaceStats),
    vscode.commands.registerCommand('rawrxd.hotpatch.refreshView', () => hotpatchViewProvider.refresh()),
    vscode.commands.registerCommand('rawrxd.gguf.refreshView', () => ggufViewProvider.refresh()),
  );
...
async function startClient(context: vscode.ExtensionContext): Promise<void> {
  ...
  const serverOptions: ServerOptions = {
    ...
    debug: { command: serverPath, args: [...serverArgs, '--debug'], transport: TransportKind.stdio },
  };
...
// ---------------------------------------------------------------------------
function resolveServerPath(configPath: string): string | undefined {
...
// Status Bar
// ---------------------------------------------------------------------------
function updateStatusBar(state: 'running' | 'stopped' | 'error' | 'starting'): void {
  switch (state) {
    case 'running':
      statusBarItem.text = '$(beaker) RawrXD';
...
// ---------------------------------------------------------------------------
function registerNotificationHandlers(lc: LanguageClient): void {

  // rawrxd/hotpatch/event — hotpatch state changes
  lc.onNotification(NOTIFY.HOTPATCH_EVENT, (params: HotpatchEventPayload) => {
    outputChannel.appendLine(
      `[HotpatchEvent] ${params.type}: ${params.patchName} (layer=${layerName(params.layer)}) — ${params.detail}`
    );

    switch (params.type) {
      case 'applied':
        vscode.window.showInformationMessage(
          `Hotpatch applied: ${params.patchName} (${layerName(params.layer)})`
        );
        break;
      case 'reverted':
        vscode.window.showInformationMessage(
          `Hotpatch reverted: ${params.patchName}`
        );
        break;
      case 'failed':
        vscode.window.showErrorMessage(
          `Hotpatch failed: ${params.patchName} — ${params.detail}`
        );
        break;
      case 'conflicted':
        vscode.window.showWarningMessage(
          `Hotpatch conflict: ${params.patchName} — ${params.detail}`
        );
        break;
    }
    ...
    const autoRefresh = vscode.workspace.getConfiguration('rawrxd').get<boolean>('diagnostics.autoRefresh', true);
    if (autoRefresh) {
  ...
  // rawrxd/gguf/loadProgress — model loading progress
  lc.onNotification(NOTIFY.GGUF_LOAD_PROGRESS, (params: GGUFLoadProgressPayload) => {
    outputChannel.appendLine(
      `[GGUF] ${params.phase}: ${params.percent}% — ${params.detail} (${path.basename(params.filePath)})`
    );
  ...
  lc.onNotification(NOTIFY.DIAGNOSTIC_REFRESH, (_params: unknown) => {
...
// ---------------------------------------------------------------------------
async function cmdHotpatchList(): Promise<void> {
  if (!ensureClient()) { return; }
  try {
    const patches = await client!.sendRequest<HotpatchEntry[]>(METHOD.HOTPATCH_LIST, {
...
// ---------------------------------------------------------------------------
async function cmdHotpatchApply(): Promise<void> {
  if (!ensureClient()) { return; }
  ...
  try {
    const result = await client!.sendRequest<{ success: boolean; detail: string }>(
      METHOD.HOTPATCH_APPLY,
      {
...
// ---------------------------------------------------------------------------
async function cmdHotpatchRevert(item?: HotpatchTreeItem): Promise<void> {
  if (!ensureClient()) { return; }
...
// ---------------------------------------------------------------------------
async function cmdHotpatchDiagnostics(): Promise<void> {
  if (!ensureClient()) { return; }

  try {
    const diags = await client!.sendRequest<HotpatchDiagnostic[]>(
      METHOD.HOTPATCH_DIAGNOSTICS,
      { layer: HotpatchLayer.All }
    );
      ...
      if (d.relatedPatch) {
        diagnostic.relatedInformation = [
          new vscode.DiagnosticRelatedInformation(
            new vscode.Location(vscode.Uri.file('hotpatch'), range),
            `Patch: ${d.relatedPatch} | Layer: ${layerName(d.layer)} | Addr: ${d.address}`
          ),
        ];
      }
    ...
    const showInline = vscode.workspace.getConfiguration('rawrxd').get<boolean>('diagnostics.showInline', true);
    if (showInline && diags.length > 0) {
      ...
      vscode.window.showInformationMessage(
        `RawrXD Diagnostics: ${errors} errors, ${warnings} warnings, ${infoCount} info`
      );
    }
  } catch (err) {
...
// ---------------------------------------------------------------------------
async function cmdGGUFModelInfo(): Promise<void> {
  if (!ensureClient()) { return; }
  ...
  try {
    const info = await client!.sendRequest<GGUFModelSummary>(
      METHOD.GGUF_MODEL_INFO,
      { filePath }
    );
    ...
    const panel = vscode.window.createWebviewPanel(
      'rawrxd.ggufModelInfo',
      `GGUF: ${path.basename(info.filePath)}`,
...
// ---------------------------------------------------------------------------
async function cmdGGUFTensorList(): Promise<void> {
  if (!ensureClient()) { return; }
  ...
  try {
    const tensors = await client!.sendRequest<GGUFTensorEntry[]>(
      METHOD.GGUF_TENSOR_LIST,
      { filePath }
    );
...
// ---------------------------------------------------------------------------
async function cmdGGUFValidate(): Promise<void> {
  if (!ensureClient()) { return; }
  ...
  try {
    const result = await client!.sendRequest<{
      ...
      magicOk: boolean;
      versionOk: boolean;
      tensorIntegrity: boolean;
    }>(METHOD.GGUF_VALIDATE, { filePath });

    outputChannel.appendLine(`[GGUF] Validation: ${result.valid ? 'PASS' : 'FAIL'} — ${filePath}`);

    if (result.valid) {
      vscode.window.showInformationMessage(
        `✅ GGUF Valid: ${path.basename(filePath)} — magic=${result.magicOk ? 'OK' : 'BAD'}, ` +
      ...
      const diags: vscode.Diagnostic[] = [
        ...result.errors.map(e =>
          new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), e, vscode.DiagnosticSeverity.Error)
        ),
        ...result.warnings.map(w =>
          new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), w, vscode.DiagnosticSeverity.Warning)
        ),
      ];
...
// ---------------------------------------------------------------------------
async function cmdWorkspaceSymbols(): Promise<void> {
  if (!ensureClient()) { return; }
  ...
  try {
    const symbols = await client!.sendRequest<HotpatchEntry[]>(
    ...
    if (selected && selected.symbol.filePath) {
      ...
      const pos = new vscode.Position(
        Math.max(0, (selected.symbol as any).line - 1 || 0),
        (selected.symbol as any).startChar || 0
      );
...
// ---------------------------------------------------------------------------
async function cmdWorkspaceStats(): Promise<void> {
  if (!ensureClient()) { return; }

  try {
    const stats = await client!.sendRequest<WorkspaceStats>(
      METHOD.WORKSPACE_STATS,
      {}
    );
...
class HotpatchTreeItem extends vscode.TreeItem {
  constructor(public readonly patch: HotpatchEntry) {
...
class HotpatchTreeProvider implements vscode.TreeDataProvider<HotpatchTreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<HotpatchTreeItem | undefined>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;
  private patches: HotpatchEntry[] = [];

  setPatches(patches: HotpatchEntry[]): void {
  ...
  refresh(): void {
    if (client) {
      void client.sendRequest<HotpatchEntry[]>(METHOD.HOTPATCH_LIST, { layer: HotpatchLayer.All })
        .then(p => this.setPatches(p))
        .catch(() => { /* server not ready */ });
    }
  }

  getTreeItem(element: HotpatchTreeItem): vscode.TreeItem {
    return element;
  }

  getChildren(): HotpatchTreeItem[] {
...
class GGUFTreeProvider implements vscode.TreeDataProvider<GGUFTreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<GGUFTreeItem | undefined>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;
  private model: GGUFModelSummary | undefined;

  setModelInfo(info: GGUFModelSummary): void {
    this.model = info;
    this._onDidChangeTreeData.fire(undefined);
  }
  ...
  getTreeItem(element: GGUFTreeItem): vscode.TreeItem {
    return element;
  }

  getChildren(): GGUFTreeItem[] {
    if (!this.model) {
...
// Stats Tree Provider
// ---------------------------------------------------------------------------
class StatsTreeProvider implements vscode.TreeDataProvider<GGUFTreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<GGUFTreeItem | undefined>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;
  private stats: WorkspaceStats | undefined;

  setStats(stats: WorkspaceStats): void {
    this.stats = stats;
    this._onDidChangeTreeData.fire(undefined);
  }
  ...
  refresh(): void {
    if (client) {
      void client.sendRequest<WorkspaceStats>(METHOD.WORKSPACE_STATS, {})
...
function showPatchDetail(patch: HotpatchEntry): void {
  const detail = [
    `# Hotpatch: ${patch.name}`,
    '',
    `| Property | Value |`,
...
function mapSeverity(s: HotpatchDiagSeverity): vscode.DiagnosticSeverity {
  switch (s) {
...
function layerName(layer: HotpatchLayer): string {
  switch (layer) {
...
function kindName(kind: HotpatchSymbolKind): string {
  switch (kind) {
...
function symbolIcon(kind: HotpatchSymbolKind): string {
  switch (kind) {
...
function renderModelInfoHTML(info: GGUFModelSummary): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
