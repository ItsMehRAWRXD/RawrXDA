...
StreamFormatter_FlushChunk PROC FRAME
    ...
    ; If index was passed instead of pointer (from WriteToken it was index * sizeof?)
    ; Wait, WriteToken passes `mov rcx, rbx` where rbx is pointer. Correct.
    ; But wait, FlushChunk arg is `rcx`.
    ; In WriteToken: `mov rcx, rbx` (rbx is pointer). `call StreamFormatter_FlushChunk`.
    ; So RCX is pointer.
    ...
    lea rdi, [rsp + 20h]            ; Temp stack buffer for header
    
    ; Format hex size - RawrXD_HexToString(Value, Buffer)
    mov rcx, rsi    ; Value (bufferUsed)
    mov rdx, rdi    ; Dest Buffer
    call RawrXD_HexToString
    
    ; Add CRLF
    mov word ptr [rdi + rax], 0A0Dh       ; \r\n
    add rax, 2
