...
@@skip_array:
    ...
    mov     ebx, DWORD PTR [rsi+r12]    ; element type
    add     r12, 4
    mov     rax, QWORD PTR [rsi+r12]    ; count
    add     r12, 8
    ; Skip elements by size based on element type
    ...
    je      @@arr_strings
    jmp     @@meta_done         ; nested/unknown type — stop parsing
...
@@arr_8:
    ...
    jmp     @@meta_print_nl
@@arr_strings:
    ; String array: each element is [uint64 len][char data]
    test    rax, rax
    jz      @@meta_print_nl
    mov     rcx, rax
@@arr_str_lp:
