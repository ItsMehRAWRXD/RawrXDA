...
void Win32IDE::loadRawrXDModule() {
    ...
    // Also try to load VS Code Extension module
    bool vscodeSuccess = loadVSCodeExtensionModule();
    
    if (vscodeSuccess) {
        m_powerShellVSCodeExtensionLoaded = true;
        if (success) appendPowerShellOutput("\n", RGB(200, 200, 200));
        appendPowerShellOutput("[SUCCESS] VS Code Extension module loaded!\n", RGB(0, 255, 0));
        appendPowerShellOutput("VS Code Extension commands:\n", RGB(255, 255, 0));
        appendPowerShellOutput("  - Get-VSCodeExtensionStatus\n", RGB(150, 150, 150));
        appendPowerShellOutput("  - Install-VSCodeExtension\n", RGB(150, 150, 150));
        appendPowerShellOutput("  - Uninstall-VSCodeExtension\n", RGB(150, 150, 150));
        appendPowerShellOutput("  - Search-VSCodeExtensions\n", RGB(150, 150, 150));
        appendPowerShellOutput("  - Load-VSCodeExtension\n", RGB(150, 150, 150));
    } else {
        if (success) appendPowerShellOutput("\n", RGB(200, 200, 200));
        appendPowerShellOutput("[WARNING] VS Code Extension module not available\n", RGB(255, 255, 0));
        appendPowerShellOutput("VS Code extension commands will not be available\n", RGB(255, 100, 100));
    }
...
void Win32IDE::unloadRawrXDModule() {
    if (!m_powerShellRawrXDLoaded && !m_powerShellVSCodeExtensionLoaded) {
        return;
    }
    ...
    if (m_powerShellVSCodeExtensionLoaded) {
        m_powerShellVSCodeExtensionLoaded = false;
        appendPowerShellOutput("[VS Code Extension module marked as unloaded]\n", RGB(255, 255, 0));
    }
