...
@@topp_done:
    ...
    ; Write token to ring buffer via Titan_SubmitChunk
    ; Ring buffer layout: [header:64 bytes][token_slots: DWORD * RING_SIZE]
    ; g_pRingBuffer points to ring header, data starts at +64
    mov rcx, g_pRingBuffer
    test rcx, rcx
    jz @@ring_skip
    
    ; Calculate write position: (g_BufferIndex % RING_SIZE) * 4 + 64
    mov eax, g_BufferIndex
    and eax, 03FFh                ; RING_MASK (1024-1)
    lea rdx, [rcx + 64]          ; data region past header
    mov DWORD PTR [rdx + rax*4], ebx   ; write token ID
    
    ; Update ring header write pointer (at offset 8)
    inc g_BufferIndex
    mov eax, g_BufferIndex
    mov DWORD PTR [rcx + 8], eax  ; header.write_pos = new index
    
    ; Signal consumer that a new token is available (header.flags at offset 0)
    mov DWORD PTR [rcx], 1        ; FLAG_DATA_AVAILABLE
