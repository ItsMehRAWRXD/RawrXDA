...
void Win32IDE::handleEditCommand(int commandId) {
    switch (commandId) {
        ...
        case IDM_EDIT_CUT:
            ...
            m_fileModified = true;
            // Mark buffer as edited for dirty tracking
            if (m_activeTabIndex >= 0 && m_activeTabIndex < static_cast<int>(m_editorTabs.size())) {
                m_bufferManager.MarkEdited(m_editorTabs[m_activeTabIndex]);
                m_editorTabs[m_activeTabIndex].modified = m_editorTabs[m_activeTabIndex].IsDirty();
            }
...
void Win32IDE::handleToolsCommand(int commandId) {
    switch (commandId) {
        ...
        case 5016: // Settings (full GUI with Agent/Autonomy section)
        ...
        case IDM_BACKEND_SWITCH_NATIVEASM:  // 5199
            setActiveBackend(AIBackendType::NativeASM);
            break;
...
void Win32IDE::buildCommandRegistry()
{
    ...
    // SubAgent: Chain, Swarm, Todo (Phase 19B - command palette)
    ...
    m_commandRegistry.push_back({IDM_SUBAGENT_TODO_LIST, "Agent: View Todo List", "", "Agent"});
    m_commandRegistry.push_back({IDM_SUBAGENT_TODO_CLEAR, "Agent: Clear Todo List", "", "Agent"});
    m_commandRegistry.push_back({IDM_SUBAGENT_STATUS, "Agent: SubAgent Status", "", "Agent"});
    ...
    // Bounded Agent Loop (agentic with safety bounds) — Ctrl+Shift+I wired in Win32IDE_Core
    m_commandRegistry.push_back({IDM_AGENT_BOUNDED_LOOP, "Agent: Bounded Agent Loop", "Ctrl+Shift+I", "Agent"});

    // Autonomy Framework (all wired via unified dispatch → output panel)
    ...
    m_commandRegistry.push_back({IDM_BACKEND_SWITCH_NATIVEASM, "AI: Switch to Native ASM",             "", "AI"});
    ...
    // ================================================================
    // Phase 10: Governor, Safety, Replay, Confidence (5118–5131 — agentic/autonomous)
    // ================================================================
    m_commandRegistry.push_back({IDM_GOV_STATUS,                  "Governor: Show Status",                "", "Governor"});
    m_commandRegistry.push_back({IDM_GOV_SUBMIT_COMMAND,         "Governor: Run Command...",             "", "Governor"});
    m_commandRegistry.push_back({IDM_GOV_KILL_ALL,               "Governor: Kill All Tasks",             "", "Governor"});
    m_commandRegistry.push_back({IDM_GOV_TASK_LIST,              "Governor: List Active Tasks",          "", "Governor"});
    m_commandRegistry.push_back({IDM_SAFETY_STATUS,              "Safety: Show Contract Status",         "", "Safety"});
    m_commandRegistry.push_back({IDM_SAFETY_RESET_BUDGET,        "Safety: Reset Intent Budget",          "", "Safety"});
    m_commandRegistry.push_back({IDM_SAFETY_ROLLBACK_LAST,       "Safety: Rollback Last Action",         "", "Safety"});
    m_commandRegistry.push_back({IDM_SAFETY_SHOW_VIOLATIONS,     "Safety: Show Violations",              "", "Safety"});
    m_commandRegistry.push_back({IDM_REPLAY_STATUS,              "Replay: Show Journal Status",           "", "Replay"});
    m_commandRegistry.push_back({IDM_REPLAY_SHOW_LAST,           "Replay: Show Last Records",             "", "Replay"});
    m_commandRegistry.push_back({IDM_REPLAY_EXPORT_SESSION,      "Replay: Export Session...",            "", "Replay"});
    m_commandRegistry.push_back({IDM_REPLAY_CHECKPOINT,           "Replay: Insert Checkpoint",            "", "Replay"});
    m_commandRegistry.push_back({IDM_CONFIDENCE_STATUS,          "Confidence: Show Gate Status",         "", "Confidence"});
    m_commandRegistry.push_back({IDM_CONFIDENCE_SET_POLICY,       "Confidence: Set Policy...",             "", "Confidence"});
