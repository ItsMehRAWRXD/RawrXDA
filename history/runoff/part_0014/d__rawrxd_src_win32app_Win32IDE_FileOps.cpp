...
void Win32IDE::openRecentFile(int index) {
    if (index >= 0 && index < m_recentFiles.size()) {
        ...
        // Load the file with encoding detection
        RawrXD::FileSystem::BufferState bufState;
        int wideLen = MultiByteToWideChar(CP_UTF8, 0, filePath.c_str(), -1, nullptr, 0);
        std::wstring widePath(wideLen - 1, L'\0');
        MultiByteToWideChar(CP_UTF8, 0, filePath.c_str(), -1, widePath.data(), wideLen);
        ...
        if (m_bufferManager.OpenFile(widePath, bufState)) {
            ...
            LOG_INFO("Recent file opened with encoding: " + std::string(RawrXD::FileSystem::EncodingName(bufState.encoding.encoding)));
        } else {
            ...
            if (file) {
                ...
                MessageBoxA(m_hwndMain, "Failed to open recent file", "Error", MB_OK | MB_ICONERROR);
            }
        }
            file.close();
        } else {
