...
// Lightweight command-line flag check (WinMain style LPSTR)
static bool hasFlag(LPSTR lpCmdLine, const char* flag) {
    if (!lpCmdLine || !flag || !flag[0]) return false;
    return strstr(lpCmdLine, flag) != nullptr;
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // ========================================================================
    {
        ...
        // Production-readiness: do not hard-terminate the whole IDE for recoverable
        // faults (e.g., optional background init). We still write dumps.
        crashCfg.terminateAfterDump = false;
    ...
    // Keep default startup minimal/stable. Experimental subsystems can be enabled
    // explicitly for dev/test without risking MVP usability.
    const bool enableExperimentalMasmSubsystems = hasFlag(lpCmdLine, "--enable-masm");
    ...
    // ========================================================================
    // CIRCULAR ARCHITECTURE INIT — Wire all 7 subsystems into message fabric
    // EventBus ↔ Beacon ↔ GUI panes ↔ AgenticBridge ↔ HotPatcher ↔
    // Compiler ↔ PerfMonitor → single circular routing topology
    // ========================================================================
    ...
    // Show window immediately so the user sees the frame. Heavy init (Camellia,
    // MASM, Swarm) runs on a background thread and posts postShowMsg when complete.
    // Main thread waits up to INIT_POST_SHOW_TIMEOUT_MS with message pump so the
    // window stays responsive and we never block indefinitely.
    ide.showWindow();

    UINT const postShowMsg = WM_APP + 202;
    constexpr unsigned int INIT_POST_SHOW_TIMEOUT_MS = 2000;
    constexpr unsigned int INIT_POLL_MS = 10;
    std::atomic<bool> postShowInitDone{false};

    std::thread postShowThread([&ide, postShowMsg, &postShowInitDone, enableExperimentalMasmSubsystems]() {
        // Keep UI responsive: run heavy init at below-normal priority
        SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_BELOW_NORMAL);

        // NOTE: Workspace encryption-on-startup is intentionally disabled.
        // MVP priority is editor stability + file I/O. Any encryption workflows
        // must be user-initiated and scoped (never implicit at startup).
        OutputDebugStringA("[main_win32] Startup: workspace encryption disabled\n");

        // ========================================================================
        // MASM SUBSYSTEM INITIALIZATION — optional (dev/test)
        // Default OFF for MVP stability. Enable with: --enable-masm
        // ========================================================================
        if (enableExperimentalMasmSubsystems) {
            OutputDebugStringA("[main_win32] Initializing MASM subsystems (--enable-masm)...\n");

            HWND editor = ide.getEditor();
            int r = -1;

            // 1. SelfPatch Engine
            r = asm_spengine_init(nullptr, 0);
            if (r >= 0) {
            ...
            if (editor) {
                ...
                OutputDebugStringA("[main_win32] QuadBuffer skipped: editor HWND not ready\n");
            }
            ...
            OutputDebugStringA("[main_win32] MASM subsystems init complete\n");
        } else {
            OutputDebugStringA("[main_win32] MASM subsystems disabled (default)\n");
        }
    ...
    // ========================================================================
    {
        ...
        // Signal main thread to run layout + SetFocus (must run on UI thread)
        postShowInitDone = true;
        HWND hwndMain = ide.getMainWindow();
        if (hwndMain) {
            PostMessage(hwndMain, postShowMsg, 0, 0);
        }
        OutputDebugStringA("[main_win32] Post-show init complete (background thread)\n");
    });
    postShowThread.detach();

    // Wait for post-show init with timeout and message pump (prevents white screen / "Not Responding")
    {
        using namespace std::chrono;
        auto deadline = steady_clock::now() + milliseconds(INIT_POST_SHOW_TIMEOUT_MS);
        HWND hwndMain = ide.getMainWindow();
        while (steady_clock::now() < deadline) {
            MSG msg = {};
            while (PeekMessageW(&msg, nullptr, 0, 0, PM_REMOVE)) {
                if (msg.message == postShowMsg) {
        ...
        if (!postShowInitDone) {
            OutputDebugStringA("[main_win32] WARNING: ASM/post-show init timeout - continuing with UI\n");
        }
    }

    // Run message loop so WM_PAINT and WM_POST_SHOW_INIT_DONE can dispatch
