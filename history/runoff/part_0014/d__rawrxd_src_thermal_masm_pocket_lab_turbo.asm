...
;---------------------------------------------------------------------
; PocketLabInit — DLL-callable initialization
...
;---------------------------------------------------------------------
; PocketLabShutdown — DLL-callable cleanup
;---------------------------------------------------------------------
PocketLabShutdown PROC
    sub rsp, 28h
    call Cleanup
    add rsp, 28h
    ret
PocketLabShutdown ENDP

;---------------------------------------------------------------------
; ENTRY POINT (standalone console mode)
;---------------------------------------------------------------------
PUBLIC main
main PROC
    sub rsp, 88h
...
DetectTier PROC
    ...
    shr rax, 30                              ; divide by 2^30 = 1 GB
    mov rcx, rax
    call PrintNum
    lea rcx, [szGB]
    mov edx, lenGB
    ...
    ; Determine tier - FIXED: correct branching
    mov rax, [g_PhysicalRAM]
    cmp rax, RAM_8GB
    jbe @TierMobile
    cmp rax, RAM_16GB
    jbe @TierWork
    jmp @TierEnterprise                      ; >= 16GB -> Enterprise
...
BuildSparseBitmap PROC
    ...
    ; Simulate sparse detection (~20% skip)
...
@ScanLoop:
    ...
    ; Check idx % 5 == 0 using reciprocal multiplication
    ; q = (idx * 0xCCCCCCCD) >> 34  (unsigned 32-bit div by 5)
    mov eax, r12d
    mov edx, 0CCCCCCCDh
    mul edx                                  ; edx:eax = idx * magic
    shr edx, 2                               ; edx = idx / 5
    lea ecx, [edx + edx*4]                   ; ecx = (idx/5) * 5
    cmp ecx, r12d
    jne @NextSlab                            ; skip if idx % 5 != 0
...
BuildHotTable PROC
    ...
    ; Fill hot table with LBA indices (4x unrolled)
    xor ebx, ebx
    lea r8, [HotLbaTable]
@FillLoop:
    mov ecx, r12d
    ...
    cmp ecx, 4
    jl @FillRemainder
    ; Write 4 consecutive entries
    lea rax, [rbx]
    mov qword ptr [r8 + rbx*8], rax
    lea rax, [rbx + 1]
    mov qword ptr [r8 + rbx*8 + 8], rax
    lea rax, [rbx + 2]
    mov qword ptr [r8 + rbx*8 + 16], rax
    lea rax, [rbx + 3]
    mov qword ptr [r8 + rbx*8 + 24], rax
    add ebx, 4
    jmp @FillLoop
...
@TokenLoop:
    ...
    ; Prefetch next cache lines (non-temporal, streaming access pattern)
    lea eax, [r12d + 64]                    ; look 64 tokens ahead
    and eax, 2047
    shr eax, 6
    prefetchnta [r13 + rax*8]               ; prefetch sparse bitmap
    test r14, r14
    jz @SkipTokenPrefetch
    mov rax, r12
    add rax, 64
    shr rax, 6
    prefetchnta [r14 + rax*8]               ; prefetch token bitmap
@SkipTokenPrefetch:
...
@ConvertLoop:
    test rcx, rcx
    jz @PrintNumStr
    ; Fast div-by-10: q = (n * 0xCCCCCCCCCCCCCCCD) >> 67
    mov rax, 0CCCCCCCCCCCCCCCDh
    mul rcx                                  ; rdx:rax = rcx * magic
    shr rdx, 3                               ; rdx = rcx / 10
    ; remainder = rcx - (rdx * 10)
    lea r8, [rdx + rdx*4]
    shl r8, 1                                ; r8 = rdx * 10
    mov rax, rcx
    sub rax, r8                              ; rax = remainder
    add al, '0'
    mov [rbx], al
    dec rbx
    mov rcx, rdx                             ; quotient becomes next dividend
    jmp @ConvertLoop

@PrintNumStr:
    lea rcx, [rbx + 1]                      ; start of digit string
    lea rax, [g_NumBuf + 31]                 ; end of buffer
    sub rax, rbx                             ; length = end - rbx
    mov edx, eax
    call PrintStr
    add rsp, 30h
    pop rbx
