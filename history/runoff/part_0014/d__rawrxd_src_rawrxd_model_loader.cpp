...
void RawrXDModelLoader::LoadTensorAsync(Tensor& t) {
    ...
    switch (t.type) {
        case 0:  UploadF32(t, t.data, ne); break;              // F32
        case 1:  DequantAndUploadF16(t, t.data, ne); break;    // F16 (passthrough)
        case 2:  DequantAndUploadQ4_0(t, t.data, ne); break;   // Q4_0
        case 6:  DequantAndUploadQ5_0(t, t.data, ne); break;   // Q5_0
        case 8:  DequantAndUploadQ8_0(t, t.data, ne); break;   // Q8_0
        case 12: DequantAndUploadQ4_K(t, t.data, ne); break;   // Q4_K
        default:
            printf("[RawrXD] Skipping unsupported quant type %u for '%s'\n", 
                   t.type, t.name.c_str());
            break;
    }
}

// F16 passthrough — data is already FP16, upload directly
void RawrXDModelLoader::DequantAndUploadF16(Tensor& t, void* data, size_t N) {
    size_t dstSize = N * sizeof(uint16_t);
    CreateGPUBuffer(t, data, dstSize);
    t.onGPU = true;
}

// Q5_0 dequant → FP16 upload
void RawrXDModelLoader::DequantAndUploadQ5_0(Tensor& t, void* blocks, size_t N) {
    size_t numBlocks = (N + 31) / 32;
    size_t dstSize = N * sizeof(uint16_t);
    uint16_t* staging = (uint16_t*)malloc(dstSize);
    if (!staging) return;

    Q5_0_Block* src = (Q5_0_Block*)blocks;
    for (size_t b = 0; b < numBlocks; b++) {
        float d = HalfToFloat(src[b].d);
        for (int j = 0; j < 32 && b * 32 + j < N; j++) {
            int lo = (src[b].qs[j / 2] >> ((j % 2) * 4)) & 0xF;
            int hi = (src[b].qh[j / 8] >> (j % 8)) & 1;
            int q = lo | (hi << 4);
            staging[b * 32 + j] = FloatToHalf((float)(q - 16) * d);
        }
    }
...
void RawrXDModelLoader::DequantAndUploadQ8_0(Tensor& t, void* blocks, size_t N) {
    ...
    Q8_0_Block* src = (Q8_0_Block*)blocks;
    for (size_t b = 0; b < numBlocks; b++) {
        float d = HalfToFloat(src[b].d);
        for (int j = 0; j < 32 && b * 32 + j < N; j++) {
            staging[b * 32 + j] = FloatToHalf((float)src[b].qs[j] * d);
        }
    }
...
// Q4_K dequant → FP16 upload (256 weights per super-block)
void RawrXDModelLoader::DequantAndUploadQ4_K(Tensor& t, void* blocks, size_t N) {
    ...
    Q4_K_Block* src = (Q4_K_Block*)blocks;
    for (size_t b = 0; b < numBlocks; b++) {
        float d = HalfToFloat(src[b].d);
        float dmin = HalfToFloat(src[b].dmin);
        for (int j = 0; j < 256 && b * 256 + j < N; j++) {
            int q = (src[b].qs[j / 2] >> ((j % 2) * 4)) & 0xF;
            staging[b * 256 + j] = FloatToHalf((float)q * d - dmin);
        }
    }
