...
// Chameleon shader system
static GLuint    gShaderProgram = 0;
static GLuint    gVertexShader = 0;
static GLuint    gFragmentShader = 0;
static GLint     gTimeUniform = -1;
static GLint     gResolutionUniform = -1;
static GLint     gColorUniform = -1;
static bool      gShadersEnabled = true;
static float     gShaderTime = 0.0f;
...
static RECT      gShaderToggle = { 620, 50, 750, 74 };
...
// Built-in shaders
static const char* CHAMELEON_VERTEX_SHADER = R"(
#version 110
...
static const char* CHAMELEON_FRAGMENT_SHADER = R"(
#version 110
uniform float time;
...
void main() {
    vec4 texSample = texture2D(fontTexture, vTexCoord);
    ...
    // Chameleon effect
    float hueShift = time * 0.5 + vPosition.x * 2.0 + vPosition.y * 1.5;
    float saturation = 0.8 + 0.2 * sin(time * 2.0 + vPosition.x * 3.0);
    float brightness = 0.9 + 0.1 * sin(time * 3.0 + vPosition.y * 4.0);
    
    vec3 chameleonColor = hsv2rgb(vec3(hueShift, saturation, brightness));
    
    // Glow effect
    float glow = 1.0 + 0.3 * sin(time * 4.0 + length(vPosition) * 5.0);
    
    gl_FragColor = vec4(chameleonColor * glow, texSample.a * textColor.a);
}
)";
...
static bool InitShaders() {
    gVertexShader = CompileShader(GL_VERTEX_SHADER, CHAMELEON_VERTEX_SHADER);
    gFragmentShader = CompileShader(GL_FRAGMENT_SHADER, CHAMELEON_FRAGMENT_SHADER);
    
    if (!gVertexShader || !gFragmentShader) {
        gShadersEnabled = false;
        return false;
    }
    ...
    gTimeUniform = glGetUniformLocation(gShaderProgram, "time");
    gResolutionUniform = glGetUniformLocation(gShaderProgram, "resolution");
    gColorUniform = glGetUniformLocation(gShaderProgram, "textColor");
...
static void DrawGlyph(int code, float x, float y, float scale, float r, float g, float b, float a, bool useChameleon = false) {
    ...
    if (useChameleon && gShadersEnabled && gShaderProgram) {
        glUseProgram(gShaderProgram);
        
        if (gTimeUniform != -1) glUniform1f(gTimeUniform, gShaderTime);
        if (gResolutionUniform != -1) glUniform2f(gResolutionUniform, (float)gW, (float)gH);
        if (gColorUniform != -1) glUniform4f(gColorUniform, r, g, b, a);
...
static int GetSyntaxColorIndex(wchar_t ch, const std::wstring& context, size_t pos) {
    ...
    if (gCurrentFileType == FILE_GLSL_VERT || gCurrentFileType == FILE_GLSL_FRAG) {
        // Simple keyword detection (would need more sophisticated parsing for real use)
...
static void Render() {
    gShaderTime = (GetTickCount() - gStartTick) / 1000.0f;
    ...
    // Shader toggle
    DrawRect((float)gShaderToggle.left, (float)gShaderToggle.top, (float)gShaderToggle.right, (float)gShaderToggle.bottom,
             gShadersEnabled ? 0.2f : 0.15f, gShadersEnabled ? 0.6f : 0.17f, gShadersEnabled ? 0.3f : 0.19f, 0.8f);
    DrawText(gShadersEnabled ? L"Chameleon ON" : L"Chameleon OFF", 
             (float)gShaderToggle.left + 5, (float)gShaderToggle.top + 2, 2.0f, false);
    ...
    DrawText(gBuffer, (float)gEditorRect.left + 60, contentY, 2.5f, gShadersEnabled, 
    ...
    swprintf_s(status, L"Pos: %zu | Lines: %zu | Chameleon: %s | File: %s", 
               ...
               gShadersEnabled ? L"ON" : L"OFF", gCurrentFilename.c_str());
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_LBUTTONDOWN: {
        ...
        if (PtInRectI(gOpacitySlider, mx, my)) {
            ...
            UpdateWindowAlpha();
        } else if (PtInRectI(gShaderToggle, mx, my)) {
            gShadersEnabled = !gShadersEnabled;
    ...
    case WM_KEYDOWN: {
        ...
        if ((GetKeyState(VK_CONTROL) & 0x8000)) {
            if (wParam == 'S') {
                // Save file dialog would go here
...
int APIENTRY wWinMain(HINSTANCE hInst, HINSTANCE, LPWSTR, int) {
    ...
    if (gVertexShader) glDeleteShader(gVertexShader);
    if (gFragmentShader) glDeleteShader(gFragmentShader);
