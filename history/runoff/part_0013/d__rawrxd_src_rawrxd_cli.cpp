...
#include "modules/ReverseEngineering.hpp" // New native RE modules
#include <iostream>
#include <thread>
#include <atomic>
#include <conio.h>
...
5. Setup Express to serve static assets and API
    ...
    // /disasm - Native disassembler 
    if (input.rfind("/disasm ", 0) == 0) {
        std::string file = input.substr(8);
        if (file.front() == '"') file.erase(0, 1);
        if (file.back() == '"') file.pop_back();
        
        std::cout << "[NativeDisassembler] Processing " << file << "...\n";
        
        std::ifstream binFile(file, std::ios::binary);
        if (binFile) {
            std::vector<uint8_t> data((std::istreambuf_iterator<char>(binFile)), std::istreambuf_iterator<char>());
            auto instructions = RawrXD::ReverseEngineering::NativeDisassembler::DisassembleX64(data.data(), std::min(data.size(), (size_t)1024), 0);
            
            std::cout << "\n=== DISASSEMBLY ===\n";
            for(const auto& inst : instructions) {
                std::cout << std::hex << inst.address << ": " << inst.mnemonic;
                if(!inst.operands.empty()) std::cout << " " << inst.operands;
                std::cout << "\n";
            }
            std::cout << "==================\n";
        } else {
    ...
    // /bininfo - Binary analyzer
    if (input.rfind("/bininfo ", 0) == 0) {
        std::string file = input.substr(9);
        if (file.front() == '"') file.erase(0, 1);
        if (file.back() == '"') file.pop_back();
        
        std::cout << "[BinaryAnalyzer] Processing " << file << "...\n";
        
        auto info = RawrXD::ReverseEngineering::BinaryAnalyzer::AnalyzePE(file);
        std::cout << RawrXD::ReverseEngineering::BinaryAnalyzer::GenerateReport(info);
        return;
    }
    
    // /compile - Native compiler
    if (input.rfind("/compile ", 0) == 0) {
        std::string code = input.substr(9);
        
        std::cout << "[NativeCompiler] Compiling...\n";
        
        auto result = RawrXD::ReverseEngineering::NativeCompiler::CompileToNative(code);
        if(result.success) {
            std::cout << "\n=== COMPILATION SUCCESS ===\n";
            std::cout << "Machine Code Size: " << result.machineCode.size() << " bytes\n";
    ...
    void RunInteractiveChat(AppState& state, std::shared_ptr<AIIntegrationHub> hub) {
        ...
        std::cout << "RE Tools: /disasm <file>, /bininfo <file>, /compile <code>\n";
