...
void IDEMainWindow::setupMenus() {
    ...
    QAction* toggleCompilerAction = viewMenu->addAction("PowerShell &Compilers");
    toggleCompilerAction->setCheckable(true);
    toggleCompilerAction->setChecked(false);
    connect(toggleCompilerAction, &QAction::triggered, this, &IDEMainWindow::onToggleCompilerPanel);
    ...
    // ===== PowerShell Compilers Menu Items =====
    toolsMenu->addSeparator();
    
    QMenu* compilerMenu = toolsMenu->addMenu("PowerShell &Compilers");
    
    QAction* compileAction = compilerMenu->addAction("&Compile Current File");
    compileAction->setShortcut(Qt::CTRL | Qt::SHIFT | Qt::Key_C);
    connect(compileAction, &QAction::triggered, this, &IDEMainWindow::onCompileCurrentFile);
    
    QAction* compileCodeAction = compilerMenu->addAction("Compile &Selected Code");
    compileCodeAction->setShortcut(Qt::CTRL | Qt::SHIFT | Qt::Key_S);
    connect(compileCodeAction, &QAction::triggered, this, &IDEMainWindow::onCompileSelectedCode);
    
    QAction* showCompilerPanelAction = compilerMenu->addAction("Show Compiler &Panel");
    connect(showCompilerPanelAction, &QAction::triggered, this, &IDEMainWindow::onToggleCompilerPanel);
...
void IDEMainWindow::setupDockWidgets() {
    ...
    // Compiler Panel Dock (BOTTOM)
    compilerDock = new QDockWidget("PowerShell Compilers", this);
    compilerPanel = getCompilerPanel();
    compilerDock->setWidget(compilerPanel);
    addDockWidget(Qt::BottomDockWidgetArea, compilerDock);
    compilerDock->hide();  // Hidden by default
    ...
    tabifyDockWidget(modelRouterDock, compilerDock);
...
void IDEMainWindow::onToggleCompilerPanel() {
    compilerDock->setVisible(!compilerDock->isVisible());
}
...
void IDEMainWindow::onSwitchModel() {
    ...
    QStringList models = modelManager->getAvailableModels();
...
// Compiler Menu Implementations
void IDEMainWindow::onCompileCurrentFile() {
    if (currentFilePath.isEmpty()) {
        QMessageBox::warning(this, "Compile Error", "Please save the file first.");
        return;
    }
    ...
    QString language = getCurrentLanguage();
    if (language.isEmpty()) {
        QMessageBox::warning(this, "Compile Error", "Could not detect language from file extension.");
        return;
    }
    
    // Set current file path in compiler panel
    compilerPanel->setCurrentFilePath(currentFilePath);
    
    // Show compiler panel and trigger compilation
    compilerDock->show();
    compilerDock->raise();
    
    // Trigger compilation
    compilerPanel->onCompileFileClicked();
}

void IDEMainWindow::onCompileSelectedCode() {
    QString selectedText = codeEditor->textCursor().selectedText();
    if (selectedText.isEmpty()) {
        QMessageBox::warning(this, "Compile Error", "Please select some code to compile.");
        return;
    }
    
    // Auto-detect language from current file or use default
    QString language = getCurrentLanguage();
    if (language.isEmpty()) {
        language = "cpp"; // Default to C++
    }
    
    // Set current source code in compiler panel
    compilerPanel->setCurrentSourceCode(selectedText);
    ...
    // Trigger compilation
    compilerPanel->onCompileClicked();
}
...
void IDEMainWindow::onModelDownloadProgress(double progress, const QString& message) {
    progressBar->setVisible(true);
    progressBar->setValue(static_cast<int>(progress * 100));
    showMessage(message);
}

void IDEMainWindow::onModelDownloadCompleted(const QString& modelPath) {
    progressBar->setVisible(false);
    
    if (!modelPath.isEmpty()) {
        showMessage("Model downloaded: " + modelPath);
    } else {
        QMessageBox::warning(this, "Download Failed", "Failed to download model");
    }
}
