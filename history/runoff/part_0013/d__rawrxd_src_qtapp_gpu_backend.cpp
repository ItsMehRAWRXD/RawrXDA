...
bool GPUBackend::selectDevice(int deviceIndex) {
    ...
    if (m_backendType == Vulkan) {
        m_vulkanPhysicalDevice = m_vulkanPhysicalDevices[deviceIndex];
        auto physical = reinterpret_cast<VkPhysicalDevice>(m_vulkanPhysicalDevice);

        // Detect VK_EXT_memory_budget support on THIS device
        m_vulkanBudgetSupported = false;
        uint32_t extCount = 0;
        if (vkEnumerateDeviceExtensionProperties(physical, nullptr, &extCount, nullptr) == VK_SUCCESS && extCount > 0) {
            std::vector<VkExtensionProperties> exts(extCount);
            if (vkEnumerateDeviceExtensionProperties(physical, nullptr, &extCount, exts.data()) == VK_SUCCESS) {
        ...
        // Update total memory for THIS device
...
bool GPUBackend::initializeVulkan() {
    ...
    m_deviceList.clear();
    m_vulkanPhysicalDevices.clear();

    for (uint32_t i = 0; i < deviceCount; ++i) {
        VkPhysicalDeviceProperties deviceProperties;
        vkGetPhysicalDeviceProperties(devices[i], &deviceProperties);
        
        QString name = QString("%1 (Vulkan %2.%3)")
        ...
        m_deviceList << name;
        m_vulkanPhysicalDevices << reinterpret_cast<void*>(devices[i]);
    }

    // Default to first device
    m_vulkanPhysicalDevice = m_vulkanPhysicalDevices[0];
    m_deviceIndex = 0;

    // Detect VK_EXT_memory_budget support on selected device
    m_vulkanBudgetSupported = false;
    uint32_t extCount = 0;
    ...
    // Get memory properties for the first device to set m_totalMemory
    VkPhysicalDeviceMemoryProperties memProperties;
    ...
    for (int i = 0; i < m_deviceList.size(); ++i) {
        qInfo() << "[GPUBackend] Device" << i << ":" << m_deviceList[i];
    }
