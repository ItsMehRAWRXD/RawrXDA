...
void OverclockGovernor::RunLoop(AppState* state) {
    ...
    BaselineProfile::LoadProfile(*state);
    ...
    if (state->target_all_core_mhz > 0) {
        ...
        if (baseDetectMhz != state->target_all_core_mhz) {
            // Frequency clamped to safe range (instrumentation disabled)
        }
    ...
    while (running_) {
        ...
        if (!telemetry::Poll(snap)) {
            ...
            continue; // Telemetry unavailable, skip this iteration
        }
        
        if (snap.cpuTempValid) {
            state->current_cpu_temp_c = (uint32_t)std::lround(snap.cpuTempC);
        } else {
            // CPU telemetry unavailable, skip this iteration
            std::this_thread::sleep_for(std::chrono::seconds(1));
    ...
    BaselineProfile::SaveProfile(*state);
