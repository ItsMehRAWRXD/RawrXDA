...
static SlolorisContext g_sloloris = NULL;
static BounceContext g_bounce = NULL;
static HotPatchContext g_hotpatch = NULL;
static BridgeContext g_bridge = NULL;
static DirLoadContext g_dirloader = NULL;
...
static int init_all_engines() {
    ...
    g_sloloris = Sloloris_Init(0);  // Default max tensors
    CHECK(g_sloloris != NULL, "Sloloris_Init succeeded");

    // 2. Bounce TPS
    g_bounce = Bounce_Init(0);  // Default flags
    CHECK(g_bounce != NULL, "Bounce_Init succeeded");

    // 3. HotPatch TPS
    g_hotpatch = HotPatch_Init(0);  // Default flags
    ...
    g_dirloader = DirLoad_Init(75);  // 75% load setpoint
