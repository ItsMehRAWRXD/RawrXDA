...
bool MemoryCore::Allocate(ContextTier tier) {
    ...
    try {
        ...
        LOG_INFO("Heap allocation successful: " + std::to_string(max_tokens_) + " token slots reserved");
        return true;
    } catch (const std::bad_alloc& e) {
        LOG_CRITICAL(std::string("Allocation failed: ") + e.what());
        return false;
    }
}
...
bool MemoryCore::Deallocate() {
    ...
    LOG_INFO("Context window released");
    return true;
}
...
bool MemoryCore::Reallocate(ContextTier new_tier) {
    LOG_INFO("Hot-swapping context window");
