...
// MASM Bridge (C ABI)
// ============================================================================
// These functions provide a stable, C-compatible ABI for the MASM x64 engine.
// They intentionally avoid exceptions crossing the ABI boundary.

namespace {
    bool writeCStrToBuffer(const std::string& s, char* out, size_t outCap) {
        if (!out || outCap == 0) return false;
        const size_t n = (s.size() < (outCap - 1)) ? s.size() : (outCap - 1);
        if (n > 0) {
    ...
    std::string makeBridgeErrorJson(const char* error) {
        json j;
        j["success"] = false;
        j["error"] = (error ? error : "unknown_error");
        return j.dump();
    }
}

extern "C" __declspec(dllexport) int ToolRegistry_InvokeToolSet(
    ToolRegistry* registry,
    const char* toolName,
    const char* paramsJson,
    char* outJson,
    unsigned long long outJsonCap
) {
    const size_t cap = static_cast<size_t>(outJsonCap);

    // Always attempt to write a valid JSON response.
    if (!outJson || cap == 0) {
        return 0;
    }
    outJson[0] = '\0';
    ...
    try {
        if (!registry) {
            writeCStrToBuffer(makeBridgeErrorJson("null_registry"), outJson, cap);
            return 0;
        }
        if (!toolName || *toolName == '\0') {
            writeCStrToBuffer(makeBridgeErrorJson("missing_tool_name"), outJson, cap);
            return 0;
        }
        ...
        if (paramsJson && *paramsJson) {
            try {
                ...
                json j;
                j["success"] = false;
                j["error"] = std::string("invalid_params_json: ") + e.what();
                const bool okWrite = writeCStrToBuffer(j.dump(), outJson, cap);
                return okWrite ? 0 : 0;
            }
        }

        const ToolResult r = registry->executeTool(std::string(toolName), params);
        ...
        json exec;
        exec["executionId"] = r.executionContext.executionId;
        exec["status"] = ToolRegistryHelpers::statusToString(r.executionContext.status);
        exec["executionTimeMs"] = r.executionContext.metrics.executionTimeMs;
        exec["inputBytes"] = r.executionContext.metrics.inputBytes;
        exec["outputBytes"] = r.executionContext.metrics.outputBytes;
        exec["fromCache"] = r.metrics.fromCache;
        exec["wasRetried"] = r.wasRetried;
        exec["retryCount"] = r.retryCount;
        out["execution"] = exec;

        const bool okWrite = writeCStrToBuffer(out.dump(), outJson, cap);
        if (!okWrite) {
            // Fall back to a small deterministic error JSON.
            writeCStrToBuffer(makeBridgeErrorJson("output_buffer_too_small"), outJson, cap);
            return 0;
        }
        ...
        json j;
        j["success"] = false;
        j["error"] = std::string("bridge_exception: ") + e.what();
        writeCStrToBuffer(j.dump(), outJson, cap);
        return 0;
    } catch (...) {
        writeCStrToBuffer(makeBridgeErrorJson("bridge_unknown_exception"), outJson, cap);
        return 0;
    }
}

extern "C" __declspec(dllexport) int ToolRegistry_QueryAvailableTools(
    ToolRegistry* registry,
    char* outJson,
    unsigned long long outJsonCap
) {
    ...
    try {
        ...
        json out;
        out["success"] = true;
        out["tools"] = json::parse("[]");
        for (const auto& name : registry->getRegisteredTools()) {
            out["tools"].push_back(name);
        }
