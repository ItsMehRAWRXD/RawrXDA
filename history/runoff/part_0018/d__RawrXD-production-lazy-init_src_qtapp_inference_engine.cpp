    ...
    : QObject(parent), m_loader(nullptr)
{
    ...
    m_cloudClient = new CloudApiClient(this); // Initialize Cloud Client
...
QString InferenceEngine::generateSync(const QString& prompt, int maxTokens)
{
    ...
    // Cloud Logic
    if (m_useCloud && m_cloudClient) {
        ModelConfig config;
        // Default to OpenAI for now or get from settings
        // In a real app, this should be configured via `setModel` or similar
        config.backend = ModelBackend::OPENAI; 
        config.model_id = "gpt-4"; // Default high quality
        config.api_key = qEnvironmentVariable("OPENAI_API_KEY"); 
        
        // Pass parameters for user settings (Max Mode, Deep Thinking)
        if (maxTokens > 4096) { 
             // Assume "Max Mode" requested via high token count
             config.model_id = "gpt-4-turbo-128k"; // Example switch
             config.parameters["max_tokens"] = QString::number(128000);
        } else {
             config.parameters["max_tokens"] = QString::number(maxTokens);
        }
        
        // Deep Thinking Check (from user request integration)
        // If settings manager has it... but we don't have access to settings here easily unless we include it.
        // But the user prompt said "sliders", so it's likely a setting globally.
        // For now, I'll rely on CloudApiClient's internal updated logic (which I added) to handle "Deep Thinking" if passed in params
        // or just by prompt.
        
        response = m_cloudClient->generate(prompt, config);
        
    } else if (m_useOllama && m_ollamaProxy) {
