...
void InterpretabilityPanelEnhanced::onModelLoaded(const QString& modelPath)
{
    qDebug() << "[InterpretabilityPanelEnhanced] Model loaded:" << modelPath;
    
    // Clear previous visualizations for new model
    clearVisualizations();
    
    QJsonObject event;
    event["event"] = "model_loaded";
    event["model_path"] = modelPath;
    logEvent("model_lifecycle", event);
}

void InterpretabilityPanelEnhanced::onModelUnloaded()
{
    qDebug() << "[InterpretabilityPanelEnhanced] Model unloaded";
    
    // Clear visualizations when model is unloaded
    clearVisualizations();
    
    QJsonObject event;
    event["event"] = "model_unloaded";
    logEvent("model_lifecycle", event);
}
