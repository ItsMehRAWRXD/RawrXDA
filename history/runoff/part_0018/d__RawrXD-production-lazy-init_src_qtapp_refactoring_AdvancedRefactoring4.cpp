    ...
    const QString& className, const QString& interfaceName, const QStringList& methodNames)
{
    ...
    operation.description = QString("Extract interface '%1' from '%2'").arg(interfaceName, className);
    operation.timestamp = QDateTime::currentDateTime();
    operation.canUndo = true;
    ...
    emit refactoringStarted(operation);
    emit refactoringProgress(10, "Reading class definition");
    
    QString code = readFile(operation.sourceRange.filePath);
    if (code.isEmpty()) {
        result.success = false;
        result.message = "Failed to read source file";
        return result;
    ...
    emit refactoringProgress(30, "Generating interface class");
    QString interfaceDef = QString("class %1 {\npublic:\n    virtual ~%1() = default;\n").arg(interfaceName);
    
    for (const QString& methodName : methodNames) {
        interfaceDef += QString("    virtual void %1() = 0;\n").arg(methodName);
    }
    interfaceDef += "};",
    
    if (m_createBackups) createBackup(operation.sourceRange.filePath);
    
    emit refactoringProgress(60, "Updating class inheritance");
    QString oldClassDef = QString("class %1").arg(className);
    QString newClassDef = QString("class %1 : public %2").arg(className, interfaceName);
    code.replace(oldClassDef, newClassDef);
    
    emit refactoringProgress(90, "Writing changes");
    code = interfaceDef + "\n\n" + code;
    
    if (!writeFile(operation.sourceRange.filePath, code)) {
        result.success = false;
        result.message = "Failed to write changes";
        return result;
    }
    ...
    result.success = true;
    result.message = QString("Successfully extracted interface '%1'").arg(interfaceName);
    result.fileChanges.append({operation.sourceRange.filePath, code});
    result.linesChanged = interfaceDef.split('\n').size() + 2;
    ...
    const QString& className, const QString& baseName, const QStringList& memberNames)
{
    ...
    operation.operationId = QUuid::createUuid().toString();
    operation.type = RefactoringType::ExtractInterface;
    operation.description = QString("Extract base class '%1' from '%2'").arg(baseName, className);
    operation.timestamp = QDateTime::currentDateTime();
    operation.canUndo = true;
    
    emit refactoringStarted(operation);
    emit refactoringProgress(10, "Reading source code");
    ...
    emit refactoringProgress(40, "Extracting members");
    QString baseDef = QString("class %1 {\npublic:\n").arg(baseName);
    
    for (const QString& member : memberNames) {
        if (code.contains(member)) {
            baseDef += QString("    int %1;\n").arg(member);
        }
    }
    baseDef += "};",
    ...
    emit refactoringProgress(70, "Updating inheritance");
    QString oldClass = QString("class %1").arg(className);
    QString newClass = QString("class %1 : public %2").arg(className, baseName);
    code.replace(oldClass, newClass);
    ...
    result.success = true;
    result.message = QString("Successfully extracted base class '%1'").arg(baseName);
    result.fileChanges.append({operation.sourceRange.filePath, code});
    result.linesChanged = baseDef.split('\n').size() + 2;
    ...
    const QString& functionName, const QStringList& parameterNames, const QString& objectName)
{
    ...
    operation.operationId = QUuid::createUuid().toString();
    operation.type = RefactoringType::ExtractMethod;
    operation.description = QString("Introduce parameter object '%1' for '%2'").arg(objectName, functionName);
    operation.timestamp = QDateTime::currentDateTime();
    operation.canUndo = true;
    ...
    emit refactoringProgress(30, "Generating parameter object");
    QString structDef = QString("struct %1 {\n").arg(objectName);
    for (const QString& param : parameterNames) {
        structDef += QString("    int %1;\n").arg(param);
    }
    structDef += "};",
    
    if (m_createBackups) createBackup(operation.sourceRange.filePath);
    
    emit refactoringProgress(60, "Updating function signature");
    QString oldSig = QString("%1(").arg(functionName);
    QString newSig = QString("%1(const %2& params").arg(functionName, objectName);
    code.replace(oldSig, newSig);
    ...
    result.fileChanges.append({operation.sourceRange.filePath, code});
    result.linesChanged = structDef.split('\n').size() + 2;
    ...
    const QString& methodName, const CodeRange& scope)
{
    ...
    emit refactoringStarted(operation);
    emit refactoringProgress(20, "Reading source code");
    ...
    emit refactoringProgress(40, "Finding method definition");
    QRegularExpression methodRe(QString(R"(\w+\s+%1\([^)]*\)\s*\{[^}]+\})").arg(QRegularExpression::escape(methodName)));
    ...
    if (m_createBackups) createBackup(scope.filePath);
    
    emit refactoringProgress(60, "Extracting method body");
    QString methodBody = match.captured(0);
    QString bodyContent = methodBody.mid(methodBody.indexOf('{') + 1, methodBody.lastIndexOf('}') - methodBody.indexOf('{') - 1);
    
    emit refactoringProgress(80, "Replacing calls");
    QString methodCall = QString("%1()").arg(methodName);
    code.replace(methodCall, bodyContent);
    code.replace(match.captured(0), "");
    
    if (m_autoFormat) code = formatCode(code);
    
    emit refactoringProgress(95, "Writing changes");
    if (!writeFile(scope.filePath, code)) {
        result.success = false;
        result.message = "Failed to write changes";
        return result;
    }
    ...
    result.fileChanges.append({scope.filePath, code});
    result.linesChanged = 1;
...
AdvancedRefactoring::RefactoringResult AdvancedRefactoring::inlineConstant(const QString& constantName) {
    ...
    emit refactoringStarted(operation);
    emit refactoringProgress(20, "Finding constant definition");
    ...
    emit refactoringProgress(40, "Extracting constant value");
    ...
    emit refactoringProgress(60, "Replacing constant usage");
    QString constantValue = match.captured(1).trimmed();
    code.replace(constantName, constantValue);
    code.replace(match.captured(0), "");
    ...
    result.success = true;
    result.message = QString("Successfully inlined constant '%1'").arg(constantName);
    result.fileChanges.append({scope.filePath, code});
    result.linesChanged = 1;
    ...
    const QString& methodName, const QString& className, const MethodSignature& newSignature)
{
    ...
    emit refactoringStarted(operation);
    CodeRange scope;
    emit refactoringProgress(30, "Reading source code");
    ...
    if (m_createBackups) createBackup(scope.filePath);
    emit refactoringProgress(60, "Updating method signature");
    
    QRegularExpression sigRe(QString(R"(\w+\s+%1\([^)]*\))").arg(QRegularExpression::escape(methodName)));
    QString newSig = QString("%1 %2(%3)").arg(newSignature.returnType, newSignature.methodName, 
                                              newSignature.parameters.join(", "));
    code.replace(sigRe, newSig);
    ...
    result.success = true;
    result.message = QString("Successfully changed method signature");
    result.fileChanges.append({scope.filePath, code});
    result.linesChanged = 1;
    ...
    const QString& paramType, const QString& paramName, const QString& defaultValue)
{
    ...
    operation.operationId = QUuid::createUuid().toString();
    operation.type = RefactoringType::RenameSymbol;
    operation.description = QString("Add parameter '%1' to '%2::%3'").arg(paramName, className, methodName);
    operation.timestamp = QDateTime::currentDateTime();
    operation.canUndo = true;
    ...
    QString newParam = QString("%1 %2").arg(paramType, paramName);
    if (!defaultValue.isEmpty()) newParam += QString(" = %1").arg(defaultValue);
    
    QRegularExpression sigRe(QString(R"(%1\(([^)]*)\))").arg(QRegularExpression::escape(methodName)));
    code.replace(sigRe, QString("%1(%2, %3)").arg(methodName, "\\1", newParam));
    ...
    result.success = true;
    result.message = QString("Successfully added parameter '%1'").arg(paramName);
    result.fileChanges.append({scope.filePath, code});
    result.linesChanged = 1;
    ...
    const QString& methodName, const QString& className, const QString& paramName)
{
    ...
    QRegularExpression paramRe(QString(R"(,?\s*\w+\s+%1(?=,|\)))").arg(QRegularExpression::escape(paramName)));
    code.replace(paramRe, "");
    ...
    result.success = true;
    result.message = QString("Successfully removed parameter '%1'").arg(paramName);
    result.fileChanges.append({scope.filePath, code});
    result.linesChanged = 1;
    ...
    const QString& methodName, const QString& className, const QStringList& newOrder)
{
    ...
    operation.operationId = QUuid::createUuid().toString();
    operation.type = RefactoringType::RenameSymbol;
    operation.description = QString("Reorder parameters in '%1::%2'").arg(className, methodName);
    operation.timestamp = QDateTime::currentDateTime();
    operation.canUndo = true;
    ...
    if (m_createBackups) createBackup(scope.filePath);
    emit refactoringProgress(60, "Reordering parameters");
    
    QString newParams = newOrder.join(", ");
    QRegularExpression sigRe(QString(R"(%1\([^)]*\))").arg(QRegularExpression::escape(methodName)));
    code.replace(sigRe, QString("%1(%2)").arg(methodName, newParams));
    ...
    result.success = true;
    result.message = QString("Successfully reordered parameters");
    result.fileChanges.append({scope.filePath, code});
    result.linesChanged = 1;
    ...
    const CodeRange& loopRange, const QString& targetType)
{
    ...
    if (m_createBackups) createBackup(loopRange.filePath);
    emit refactoringProgress(50, "Converting loop structure");
    
    if (targetType == "while") {
        QRegularExpression forRe(R"(for\s*\([^;]*;[^;]*;[^)]*\))");
        code.replace(forRe, "while (condition)");
    } else if (targetType == "foreach") {
        QRegularExpression forRe(R"(for\s*\(\w+\s+\w+\s*:[^)]*\))");
        code.replace(forRe, "for (auto item : collection)");
    }
    
    if (m_autoFormat) code = formatCode(code);
    emit refactoringProgress(90, "Writing changes");
    
    if (!writeFile(loopRange.filePath, code)) {
        result.success = false;
        result.message = "Failed to write changes";
        return result;
    }
    ...
    result.success = true;
    result.message = QString("Successfully converted loop to %1").arg(targetType);
    result.fileChanges.append({loopRange.filePath, code});
    result.linesChanged = 1;
    ...
    const CodeRange& condRange, bool useTernary)
{
    ...
    operation.operationId = QUuid::createUuid().toString();
    operation.type = RefactoringType::RenameSymbol;
    operation.description = QString("Convert conditional to %1").arg(useTernary ? "ternary" : "if-else");
    ...
    if (m_createBackups) createBackup(condRange.filePath);
    emit refactoringProgress(50, "Converting conditional structure");
    
    if (useTernary) {
        QRegularExpression ifRe(R"(if\s*\([^)]+\)\s*[^;]+;\s*else\s*[^;]+;)");
        code.replace(ifRe, "condition ? trueExpr : falseExpr;");
    } else {
        QRegularExpression ternaryRe(R"(\?.*:)");
        code.replace(ternaryRe, "if () {} else {}");
    }
    
    if (m_autoFormat) code = formatCode(code);
    emit refactoringProgress(90, "Writing changes");
    
    if (!writeFile(condRange.filePath, code)) {
        result.success = false;
        result.message = "Failed to write changes";
        return result;
    }
    ...
    result.success = true;
    result.message = QString("Successfully converted conditional");
    result.fileChanges.append({condRange.filePath, code});
    result.linesChanged = 1;
...
AdvancedRefactoring::RefactoringResult AdvancedRefactoring::optimizeIncludes(const QString& filePath) {
    ...
    emit refactoringStarted(operation);
    emit refactoringProgress(20, "Reading source file");
    ...
    if (m_createBackups) createBackup(filePath);
    emit refactoringProgress(50, "Analyzing includes");
    
    QStringList includes = parseIncludes(code);
    QSet<QString> uniqueIncludes(includes.begin(), includes.end());
    
    emit refactoringProgress(70, "Removing duplicates and sorting");
    QStringList sortedIncludes = uniqueIncludes.values();
    std::sort(sortedIncludes.begin(), sortedIncludes.end());
    
    QString newCode = code;
    for (const QString& include : includes) {
        newCode.remove(QString("#include %1\n").arg(include));
    }
    
    QString includeBlock;
    for (const QString& include : sortedIncludes) {
        includeBlock += QString("#include %1\n").arg(include);
    }
    
    newCode = includeBlock + "\n" + newCode;
    
    if (m_autoFormat) newCode = formatCode(newCode);
    emit refactoringProgress(90, "Writing changes");
    
    if (!writeFile(filePath, newCode)) {
        result.success = false;
        result.message = "Failed to write changes";
        return result;
    }
    ...
    result.success = true;
    result.message = QString("Successfully optimized includes");
    result.fileChanges.append({filePath, newCode});
    result.linesChanged = sortedIncludes.size();
...
AdvancedRefactoring::RefactoringResult AdvancedRefactoring::removeUnusedCode(const QString& filePath) {
    ...
    if (m_createBackups) createBackup(filePath);
    emit refactoringProgress(50, "Analyzing symbol usage");
    
    QRegularExpression varRe(R"(\b(\w+)\s+\w+\s*=\s*[^;]+;)\s*(?!.*\1));
    QStringList matches;
    QRegularExpressionMatchIterator it = varRe.globalMatch(code);
    
    emit refactoringProgress(70, "Removing unused variables");
    int removed = 0;
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        code.remove(match.captured(0));
        removed++;
    }
    ...
    result.success = true;
    result.message = QString("Successfully removed %1 unused symbols").arg(removed);
    result.fileChanges.append({filePath, code});
    result.linesChanged = removed;
...
AdvancedRefactoring::RefactoringResult AdvancedRefactoring::removeDeadCode(const QString& filePath) {
    ...
    if (m_createBackups) createBackup(filePath);
    emit refactoringProgress(50, "Analyzing control flow");
    
    QRegularExpression returnRe(R"(return\s+[^;]*;)\s*(.+?)(?=\n\}|\nelse)");
    QStringList deadCode;
    QRegularExpressionMatchIterator it = returnRe.globalMatch(code);
    
    emit refactoringProgress(70, "Identifying unreachable code");
    int removed = 0;
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        if (match.lastCapturedIndex() > 0) {
            code.remove(match.captured(1));
            removed++;
        }
    }
    ...
    result.success = true;
    result.message = QString("Successfully removed %1 dead code blocks").arg(removed);
    result.fileChanges.append({filePath, code});
    result.linesChanged = removed;
