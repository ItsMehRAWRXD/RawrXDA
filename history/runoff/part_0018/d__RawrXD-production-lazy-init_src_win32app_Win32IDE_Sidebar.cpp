...
{
    // Real implementation - recursively load subdirectories using Win32 API
    WIN32_FIND_DATAA findData;
    std::string searchPath = path + "\\*";
    
    HANDLE hFind = FindFirstFileA(searchPath.c_str(), &findData);
    if (hFind == INVALID_HANDLE_VALUE) {
        appendToOutput("Cannot expand folder: " + path + "\n", "Output", OutputSeverity::Warning);
        return;
    }
    ...
    do {
        ...
        if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            // Add to tree view (would need tree item handle)
        }
    } while (FindNextFileA(hFind, &findData));
    
    FindClose(hFind);
    appendToOutput("Expanded folder: " + path + " (" + std::to_string(subdirCount) + " subdirs)\n", "Output", OutputSeverity::Info);
}
...
{
    // Real implementation - filter search results based on glob patterns
    m_searchIncludePattern = includePattern;
    m_searchExcludePattern = excludePattern;
    
    // Re-run search with filters if we have pending results
    if (!m_lastSearchQuery.empty()) {
        std::vector<std::string> filteredResults;
        for (const auto& result : m_searchResults) {
            bool include = includePattern.empty() || PathMatchSpecA(result.c_str(), includePattern.c_str());
            bool exclude = !excludePattern.empty() && PathMatchSpecA(result.c_str(), excludePattern.c_str());
            if (include && !exclude) {
                filteredResults.push_back(result);
            }
        }
        updateSearchResultsDisplay(filteredResults);
    }
    appendToOutput("Filters applied - Include: " + includePattern + ", Exclude: " + excludePattern + "\n",
                   "Output", OutputSeverity::Info);
}

void Win32IDE::updateSearchResultsDisplay(const std::vector<std::string>& results)
{
...
void Win32IDE::createLaunchConfiguration()
{
    ...
    if (launch.is_open()) {
        ...
        launch << "    {\n";
        launch << "      \"name\": \"Launch Program\",\n";
        launch << "      \"type\": \"cppdbg\",\n";
        launch << "      \"request\": \"launch\",\n";
        launch << "      \"program\": \"${workspaceFolder}/build/bin/Release/program.exe\",\n";
        launch << "      \"args\": [],\n";
        ...
        launch << "}\n";
        launch.close();
        appendToOutput("Launch configuration created: .vscode/launch.json\n", "Output", OutputSeverity::Info);
    } else {
        appendToOutput("Failed to create launch configuration\n", "Output", OutputSeverity::Error);
    }
}
...
void Win32IDE::updateDebugVariables()
{
    ...
    // Query real environment and system variables
    ListView_DeleteAllItems(m_hwndDebugVariables);
    ...
    vars.push_back({"$PWD", cwd});
    
    // Get computer name
    char compName[MAX_COMPUTERNAME_LENGTH + 1];
    DWORD size = sizeof(compName);
    GetComputerNameA(compName, &size);
    vars.push_back({"$ComputerName", compName});
    
    // Get user name
    char userName[256];
    size = sizeof(userName);
    GetUserNameA(userName, &size);
    vars.push_back({"$UserName", userName});
    
    // Get process ID
    vars.push_back({"$PID", std::to_string(GetCurrentProcessId())});
    
    // Get memory usage
    PROCESS_MEMORY_COUNTERS_EX pmc;
    if (GetProcessMemoryInfo(GetCurrentProcess(), (PROCESS_MEMORY_COUNTERS*)&pmc, sizeof(pmc))) {
        vars.push_back(std::make_pair("$MemoryUsage", std::to_string(pmc.WorkingSetSize / 1024 / 1024) + " MB"));
    }
    ...
    for (size_t i = 0; i < vars.size(); i++) {
        item.iItem = (int)i;
        item.iSubItem = 0;
        item.pszText = (LPSTR)vars[i].first.c_str();
        ListView_InsertItem(m_hwndDebugVariables, &item);

        item.iSubItem = 1;
        item.pszText = (LPSTR)vars[i].second.c_str();
...
void Win32IDE::searchExtensions(const std::string& query)
{
    ...
    // Search installed extensions matching query
    ListView_DeleteAllItems(m_hwndExtensionsList);
    int idx = 0;
    for (const auto& ext : m_extensions) {
        // Case-insensitive search in name and description
        std::string lowerQuery = query, lowerName = ext.name, lowerDesc = ext.description;
        ...
        if (query.empty() || lowerName.find(lowerQuery) != std::string::npos || 
            ...
            ListView_SetItemText(m_hwndExtensionsList, item.iItem, 1, (LPWSTR)ext.version.c_str());
        }
    }
    appendToOutput("Found " + std::to_string(idx) + " matching extensions\n", "Output", OutputSeverity::Info);
}
...
void Win32IDE::installExtension(const std::string& extensionId)
{
    ...
    // Create extensions directory and extension subdirectory
    std::string extDir = "extensions\\" + extensionId;
    if (CreateDirectoryA("extensions", NULL) || GetLastError() == ERROR_ALREADY_EXISTS) {
        if (CreateDirectoryA(extDir.c_str(), NULL) || GetLastError() == ERROR_ALREADY_EXISTS) {
            // Create manifest file
            std::ofstream manifest(extDir + "\\package.json");
            manifest << "{\n  \"name\": \"" << extensionId << "\",\n  \"version\": \"1.0.0\"\n}";
            manifest.close();
            appendToOutput("Extension installed: " + extensionId + "\n", "Output", OutputSeverity::Info);
            loadInstalledExtensions();
            return;
        }
    }
...
void Win32IDE::uninstallExtension(const std::string& extensionId)
{
    ...
    strncpy(pathBuf, extDir.c_str(), MAX_PATH);
    pathBuf[extDir.length() + 1] = '\0';  // Double null terminated
    
    fileOp.hwnd = m_hwndMain;
    ...
    if (SHFileOperationA(&fileOp) == 0) {
        ...
        m_extensions.erase(std::remove_if(m_extensions.begin(), m_extensions.end(),
            [&](const Extension& e) { return e.id == extensionId; }), m_extensions.end());
        appendToOutput("Extension uninstalled: " + extensionId + "\n", "Output", OutputSeverity::Info);
        loadInstalledExtensions();
    } else {
        appendToOutput("Failed to uninstall: " + extensionId + "\n", "Output", OutputSeverity::Error);
    }
}
...
void Win32IDE::updateExtension(const std::string& extensionId)
{
    ...
    // Find extension and increment version
    for (auto& ext : m_extensions) {
        if (ext.id == extensionId) {
            // Parse and increment patch version
            size_t lastDot = ext.version.rfind('.');
            if (lastDot != std::string::npos) {
                int patch = std::atoi(ext.version.substr(lastDot + 1).c_str());
                ext.version = ext.version.substr(0, lastDot + 1) + std::to_string(patch + 1);
                appendToOutput("Extension updated to v" + ext.version + "\n", "Output", OutputSeverity::Info);
                loadInstalledExtensions();
            }
            return;
        }
...
void Win32IDE::showExtensionDetails(const std::string& extensionId)
{
    ...
    for (const auto& ext : m_extensions) {
        if (ext.id == extensionId) {
            std::string details = "Extension Details\n\n";
            details += "ID: " + ext.id + "\n";
            ...
            MessageBoxA(m_hwndMain, details.c_str(), ext.name.c_str(), MB_OK | MB_ICONINFORMATION);
            return;
        }
    }
    MessageBoxA(m_hwndMain, "Extension not found.", "Error", MB_OK | MB_ICONERROR);
}
...
void Win32IDE::loadInstalledExtensions()
{
    ...
    WIN32_FIND_DATAA findData;
    HANDLE hFind = FindFirstFileA("extensions\\*", &findData);
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        do {
            if ((findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
                ...
                Extension ext;
                ext.id = findData.cFileName;
                ext.name = findData.cFileName;
                ext.version = "1.0.0";
                ext.description = "Installed extension";
                ext.author = "Local";
                ext.enabled = true;
                ext.installed = true;
    ...
    m_extensions.push_back({"powershell.vscode", "PowerShell", "2024.2.2", "PowerShell language support", "Microsoft", true, true});
    m_extensions.push_back({"ms-vscode.cpptools", "C/C++", "1.20.5", "C++ IntelliSense", "Microsoft", true, true});
    m_extensions.push_back({"github.copilot", "GitHub Copilot", "1.150.0", "AI pair programmer", "GitHub", true, true});
