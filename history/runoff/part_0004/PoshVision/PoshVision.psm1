...
$script:OnnxRuntimeLoaded = $false
try {
    Add-Type -Path $OnnxPkg -ErrorAction Stop
    $script:OnnxRuntimeLoaded = $true
}
catch {
    Write-Warning "Failed to load ONNX Runtime: $($_.Exception.Message)"
}
...
function Get-VisionAnswer {
    ...
    if (-not $script:OnnxRuntimeLoaded) {
        throw "ONNX Runtime failed to load. Reinstall the runtime or repair $OnnxPkg."
    }
