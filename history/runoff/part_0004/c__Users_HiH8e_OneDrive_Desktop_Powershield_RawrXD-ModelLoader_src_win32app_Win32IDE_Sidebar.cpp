...
void Win32IDE::stageSelectedFiles()
{
    ...
    if (idx >= 0) {
        char file[260];
        ListView_GetItemText(m_hwndSCMFileList, idx, 1, file, 260);
        gitStageFile(file);
        refreshSourceControlView();
    }
}
...
void Win32IDE::updateTimelineView()
{
    ...
    for (size_t i = 0; i < m_timelineEntries.size(); ++i) {
        ...
        ListView_SetItemText(m_hwndTimelineList, (int)i, 1, (LPSTR)entry.author.c_str());
        ListView_SetItemText(m_hwndTimelineList, (int)i, 2, (LPSTR)entry.message.c_str());
    }
}
