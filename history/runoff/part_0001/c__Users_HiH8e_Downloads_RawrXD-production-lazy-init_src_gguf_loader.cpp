...
GGUFLoader::GGUFLoader()
    ...
    , is_open_(false)
{
    // compression_provider_ is already initialized by member declaration
    // CompressionFactory::Create() would require additional setup
}
...
bool GGUFLoader::LoadTensorZone(const std::string& tensor_name, std::vector<uint8_t>& data) {
    ...
    if (IsCompressed()) {
        ...
        //     qInfo() << "[GGUFLoader] Decompressed tensor" << tensor_name.c_str() 
        //             << "using" << compression_provider_->GetActiveKernel().c_str();
        //     
        //     // telemetry & safety
        //     GetTelemetry().recordEvent("gguf_tensor_decompress", QJsonObject{{"tensor", QString::fromStdString(tensor_name)}});
        //     if (data.size() > SettingsManager::instance().compressionSettings().max_decomp_bytes) {
        //         qWarning() << "[GGUF] Tensor" << tensor_name.c_str() << "exceeds max decompression size";
        //         return false;
        //     }
        // } else 
...
bool GGUFLoader::LoadTensorRange(size_t start_idx, size_t count, std::vector<uint8_t>& data) {
    ...
    if (IsCompressed()) {
        ...
        //     
        //     // telemetry & safety
        //     GetTelemetry().recordEvent("gguf_tensor_range_decompress", QJsonObject{{"count", (int)count}});
        //     if (data.size() > SettingsManager::instance().compressionSettings().max_decomp_bytes) {
        //         qWarning() << "[GGUF] Tensor range exceeds max decompression size";
        //         return false;
        //     }
        // } else 
        if (!DecompressData(data, decompressed)) {
