...
ToolResult AgenticToolExecutor::readFile(const QString& filePath)
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        return ToolResult{false, "", "Cannot open file: " + filePath, 1, (double)timer.elapsed()};
    }
    ...
    return ToolResult{true, content, "", 0, (double)timer.elapsed()};
}
...
ToolResult AgenticToolExecutor::writeFile(const QString& filePath, const QString& content)
{
    ...
    if (!parentDir.exists()) {
        if (!parentDir.mkpath(".")) {
            return ToolResult{false, "", "Cannot create directory: " + parentDir.absolutePath(), 1, (double)timer.elapsed()};
        }
    }
    
    QFile file(filePath);
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        return ToolResult{false, "", "Cannot open file for writing: " + filePath, 1, (double)timer.elapsed()};
    }
    ...
    return ToolResult{true, "File written successfully: " + filePath, "", 0, (double)timer.elapsed()};
}
...
ToolResult AgenticToolExecutor::listDirectory(const QString& dirPath)
{
    ...
    if (entries.isEmpty()) {
        return ToolResult{true, "No files in directory", "", 0, 0.0};
    }
...
{
    // Safety check: block destructive shell commands when toggle is enabled
    ...
    if (!process.waitForFinished(30000)) {  // 30 second timeout
        process.kill();
        process.waitForFinished();  // Ensure process is killed
...
bool AgenticToolExecutor::isDestructiveCommand(const QString& program, const QStringList& args) const
{
    ...
    // If running a shell, inspect args for destructive tokens
    if (prog == "bash" || prog == "sh" || prog == "pwsh" || prog == "powershell" || prog == "cmd.exe") {
        QString joined = args.join(" ").toLower();
        if (joined.contains("\brm\b") || joined.contains("\bdel\b") || joined.contains("remove-item") || joined.contains("rmdir")) {
            return true;
        }
    }

    // Also consider one-off cases where program contains 'rm' etc.
...
ToolResult AgenticToolExecutor::grepSearch(const QString& pattern, const QString& path)
{
    ...
    if (!regex.isValid()) {
        return ToolResult{false, "", "Invalid regex pattern: " + regex.errorString(), 1, 0.0};
    }
...
ToolResult AgenticToolExecutor::analyzeCode(const QString& filePath)
{
    ...
    if (language == "cpp" || language == "c") {
        ...
        int funcCount = 0;
        auto funcMatches = funcPattern.globalMatch(content, QRegularExpression::MultilineOption);
        while (funcMatches.hasNext()) {
            funcMatches.next();
            funcCount++;
        }
        ...
        int classCount = 0;
        auto classMatches = classPattern.globalMatch(content, QRegularExpression::MultilineOption);
        while (classMatches.hasNext()) {
            classMatches.next();
            classCount++;
        }
