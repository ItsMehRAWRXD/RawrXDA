...
void SettingsManager::setValue(const QString& key, const QVariant& value, bool saveImmediately) {
    ...
    // Build hierarchy of nested objects
    QVector<QJsonObject> hierarchy;
    hierarchy.reserve(parts.size());
    hierarchy.append(m_settings);
    
    for (int i = 0; i < parts.size() - 1; ++i) {
        QJsonObject current = hierarchy.last();
        if (!current.contains(parts[i]) || !current[parts[i]].isObject()) {
            current.insert(parts[i], QJsonObject());
        }
        hierarchy.append(current[parts[i]].toObject());
    }
    
    // Set the value in the deepest object
    QString lastKey = parts.last();
    hierarchy.last()[lastKey] = QJsonValue::fromVariant(value);
    
    // Write back up the hierarchy
    for (int i = hierarchy.size() - 2; i >= 0; --i) {
        hierarchy[i][parts[i]] = hierarchy[i + 1];
    }
    
    m_settings = hierarchy[0];
...
void SettingsManager::remove(const QString& key) {
    ...
    // Remove the value from the deepest object
    hierarchy.last().remove(parts.last());
...
void SettingsManager::setWorkspaceValue(const QString& key, const QVariant& value) {
    ...
    QVector<QJsonObject> hierarchy;
    hierarchy.reserve(parts.size());
    hierarchy.append(m_workspaceSettings);
    ...
    m_workspaceSettings = hierarchy[0];
    saveWorkspace();
}
