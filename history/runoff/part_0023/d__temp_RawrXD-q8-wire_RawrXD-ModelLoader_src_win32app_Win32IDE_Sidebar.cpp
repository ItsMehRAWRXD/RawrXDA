...
// ...existing code...
// Control IDs - Moved to high range to avoid "Circles" / Collisions with CommandRegistry
constexpr int IDC_ACTIVITY_EXPLORER      = 61001;
constexpr int IDC_ACTIVITY_SEARCH        = 61002;
constexpr int IDC_ACTIVITY_SCM           = 61003;
constexpr int IDC_ACTIVITY_DEBUG         = 61004;
constexpr int IDC_ACTIVITY_EXTENSIONS    = 61005;

constexpr int IDC_EXPLORER_TREE          = 61010;
constexpr int IDC_EXPLORER_NEW_FILE      = 61011;
constexpr int IDC_EXPLORER_NEW_FOLDER    = 61012;
constexpr int IDC_EXPLORER_REFRESH       = 61013;
constexpr int IDC_EXPLORER_COLLAPSE      = 61014;

constexpr int IDC_SEARCH_INPUT           = 61020;
constexpr int IDC_SEARCH_BUTTON          = 61021;
constexpr int IDC_SEARCH_RESULTS         = 61022;
constexpr int IDC_SEARCH_REGEX           = 61023;
constexpr int IDC_SEARCH_CASE            = 61024;
constexpr int IDC_SEARCH_WHOLE_WORD      = 61025;
constexpr int IDC_SEARCH_INCLUDE         = 61026;
constexpr int IDC_SEARCH_EXCLUDE         = 61027;

constexpr int IDC_SCM_FILE_LIST         = 61030;
constexpr int IDC_SCM_STAGE             = 61031;
constexpr int IDC_SCM_UNSTAGE           = 61032;
constexpr int IDC_SCM_COMMIT            = 61033;
constexpr int IDC_SCM_SYNC              = 61034;
constexpr int IDC_SCM_MESSAGE           = 61035;

constexpr int IDC_DEBUG_CONFIGS          = 61040;
constexpr int IDC_DEBUG_START            = 61041;
constexpr int IDC_DEBUG_STOP             = 61042;
constexpr int IDC_DEBUG_VARIABLES       = 61043;
constexpr int IDC_DEBUG_CALLSTACK       = 61044;
constexpr int IDC_DEBUG_CONSOLE         = 61045;

constexpr int IDC_EXT_SEARCH             = 61050;
constexpr int IDC_EXT_LIST               = 61051;
constexpr int IDC_EXT_DETAILS            = 61052;
constexpr int IDC_EXT_INSTALL            = 61053;
constexpr int IDC_EXT_UNINSTALL          = 61054;
...
LRESULT CALLBACK Win32IDE::ActivityBarProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        if (pThis) {
            ...
            // Route to setSidebarView which updates active state
            switch (id) {
            ...
            // Invalidate activity bar to redraw circles with new active state
    ...
    case WM_DRAWITEM:
        {
            ...
            if (pdis->CtlID >= 61001 && pdis->CtlID <= 61005) {
                ...
                if (pThis) {
                    if (pThis->m_currentSidebarView == SidebarView::Explorer && pdis->CtlID == IDC_ACTIVITY_EXPLORER) active = true;
                    if (pThis->m_currentSidebarView == SidebarView::Search && pdis->CtlID == IDC_ACTIVITY_SEARCH) active = true;
                    if (pThis->m_currentSidebarView == SidebarView::SourceControl && pdis->CtlID == IDC_ACTIVITY_SCM) active = true;
                    if (pThis->m_currentSidebarView == SidebarView::RunDebug && pdis->CtlID == IDC_ACTIVITY_DEBUG) active = true;
                    if (pThis->m_currentSidebarView == SidebarView::Extensions && pdis->CtlID == IDC_ACTIVITY_EXTENSIONS) active = true;
                }

                // Draw background
                FillRect(hdc, &rc, CreateSolidBrush(RGB(51, 51, 51)));

                // Circle drawing logic - "Everything in a circle" requirement
                int margin = 5;
...
void Win32IDE::createExplorerView(HWND hwndParent)
{
    ...
    // Set current workspace as root (using platform-independent path resolution)
    m_explorerRootPath = PathResolver::getPowershieldPath().toStdString();
...
void Win32IDE::updateDebugVariables()
{
    ...
    // Use static constexpr for compile-time initialization
    static constexpr struct { const char* name; const char* value; } vars[] = {
