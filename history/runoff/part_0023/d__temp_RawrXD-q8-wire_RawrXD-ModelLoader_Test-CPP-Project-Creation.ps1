# Test Model's Ability to Create C++ Project and Compile
# Real-world agentic task: workspace setup, file creation, compilation
...
FIRST, discover what C++ compiler is available on this Windows system.
...
Write-Log "INFO" "asking model to discover available compilers"
...
# Execute discovery
Write-Log "INFO" "executing discovery command"
$discoveryResult = ""
try {
    $discoveryResult = Invoke-Expression $discoveryCommand 2>&1 | Out-String
    Write-Log "INFO" "discovery executed successfully"
} catch {
    Write-Log "WARN" "discovery failed: $($_.Exception.Message)"
    # Fallback
    $discoveryResult = @(
        (where.exe cl 2>&1)
        (where.exe g++ 2>&1)
...
Write-Log "INFO" "PHASE 2: Adaptive Planning Based on Discovery"
...
1. Create a new workspace directory at D:\temp\cpp-project-test
...
WORKSPACE_PATH: D:\temp\cpp-project-test
COMMAND_1: [command]
COMMAND_2: [command]
...

IMPORTANT: 
- For creating the C++ file, use Set-Content with -Value parameter or a here-string @' '@
...
Write-Log "INFO" "querying model for project setup commands"
$response = Invoke-ModelQuery -Prompt $prompt

if (-not $response) {
    Write-Log "ERROR" "no response from model"
    exit 1
}

Write-Host "`n=== MODEL RESPONSE ===" -ForegroundColor Cyan
Write-Host $response
Write-Host "=====================`n" -ForegroundColor Cyan
...
foreach ($line in ($response -split "`n")) {
    if ($line -match '^WORKSPACE_PATH:\s*(.+)$') {
        $workspacePath = $matches[1].Trim()
        Write-Log "INFO" "extracted workspace path: $workspacePath"
    }
    elseif ($line -match '^COMMAND_\d+:\s*(.+)$') {
        $cmd = $matches[1].Trim()
        $commands += $cmd
...
if ($discoveryResult -match 'g\+\+\.exe' -and ($response -match 'g\+\+')) {
    Write-Log "INFO" "✓ Correctly chose g++ (available)"
    $score++
    $usesAvailableCompiler = $true
}
elseif ($discoveryResult -match 'cl\.exe' -and ($response -match '\bcl\b|\bcl\.exe')) {
    Write-Log "INFO" "✓ Correctly chose cl (available)"
    $score++
    $usesAvailableCompiler = $true
}
elseif ($discoveryResult -match 'clang\+\+\.exe' -and ($response -match 'clang\+\+')) {
...
# Check 3: Creates C++ file properly (not with echo)
$maxScore++
if ($commands | Where-Object { $_ -match 'Set-Content|Out-File|@' }) {
    Write-Log "INFO" "✓ Uses proper PowerShell file creation"
    $score++
} else {
    Write-Log "WARN" "✗ May use problematic echo command"
}
...
$workspace = "D:\temp\cpp-project-test-$(Get-Date -Format 'yyyyMMdd-HHmmss')"

if ($workspacePath -and $commands.Count -gt 0) {
    try {
        # Create workspace
        ...
        $fullCommandText = $response -replace $workspacePath, $workspace
        
        if ($fullCommandText -match "@'") {
            Write-Log "INFO" "detected multi-line script, executing as script block"
            
            try {
                # Save to temp script file - remove COMMAND_N: prefixes and fix variables
                $tempScript = "$workspace\temp_execute.ps1"
                $cleanScript = $fullCommandText -replace 'COMMAND_\d+:\s*', '' -replace 'WORKSPACE_PATH:.*\n', ''
                $cleanScript = "`$WORKSPACE_PATH = '$workspace'`n" + $cleanScript
                $cleanScript | Out-File -FilePath $tempScript -Encoding UTF8
                
                Write-Log "INFO" "executing script block"
                Push-Location $workspace
                & $tempScript 2>&1 | ForEach-Object { 
                    if ($_ -and $_ -ne "") { Write-Log "INFO" "output: $_" }
                }
                Pop-Location
            ...
            foreach ($cmd in $commands) {
                ...
                if ($cmd -match 'New-Item.*cpp-project-test|mkdir.*cpp-project-test|md.*cpp-project-test') {
                    Write-Log "INFO" "skipping redundant directory creation"
                    continue
                }
                
                # Modify paths
                $modifiedCmd = $cmd -replace [regex]::Escape($workspacePath), $workspace
        ...
        if ($exeFiles) {
            ...
            $files = Get-ChildItem -Path $workspace -ErrorAction SilentlyContinue
    ...
    Write-Log "WARN" "insufficient information to execute plan"
}
