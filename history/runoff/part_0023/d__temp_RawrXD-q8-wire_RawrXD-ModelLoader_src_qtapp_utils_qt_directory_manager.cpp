...
{
    QString absPath = toAbsolutePath(path);
    QDir dir;
...
QString QtDirectoryManager::toAbsolutePath(const QString& relativePath,
    ...
    if (!basePath.isEmpty()) {
        pathToConvert = QDir(basePath).filePath(relativePath);
    }
    if (QFileInfo(pathToConvert).isAbsolute()) {
        return QDir::cleanPath(pathToConvert);
    }
    return QDir::current().absoluteFilePath(pathToConvert);
}
                                            ...
                                            const QString& basePath) const 
{
    ...
    QString absTarget = absolutePath;
    if (!QFileInfo(absTarget).isAbsolute()) {
        absTarget = toAbsolutePath(absTarget);
    }
    return baseDir.relativeFilePath(absTarget);
}
