    ...
    , m_ownsDirManager(false)
{
    ...
    if (!m_fileWriter) {
        m_fileWriter = new QtFileWriter();
        m_ownsFileWriter = true;
    }
    if (!m_dirManager) {
        m_dirManager = new QtDirectoryManager();
...
void ProjectExplorerWidget::actionCopyRelativePath() {
    ...
    QString relativePath = m_dirManager->toRelativePath(path, m_projectPath);
...
bool ProjectExplorerWidget::isFileIgnored(const QString& filePath) const {
    ...
    QString relativePath = m_dirManager->toRelativePath(filePath, m_projectPath);
...
bool GitignoreFilter::shouldIgnore(const QString& filePath, const QString& basePath) const {
    ...
    if (!basePath.isEmpty()) {
        ...
        checkPath = baseDir.relativeFilePath(filePath);
    }
