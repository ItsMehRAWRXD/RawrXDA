...
private:
    ...
    // ---- Robust Metadata Controls ----
    bool robust_metadata_enabled_;
    bool robust_metadata_verbose_;
    uint64_t max_metadata_string_len_;
    bool masm_metadata_enabled_;
    bool robust_metadata_peek_enabled_;
    HANDLE raw_file_handle_;
    ...
    bool ReadKeyString(std::string& value);
    bool ReadStringLimited(std::string& value, uint64_t max_len, bool allow_skip);
    bool SkipString(uint64_t max_len);
    bool SkipArray(uint64_t max_depth = 8);
    bool SkipValue(uint32_t value_type);
    size_t GetScalarTypeSize(uint32_t value_type) const;
    bool ShouldSkipKey(const std::string& key) const;
    uint64_t CalculateTensorSize(const std::vector<uint64_t>& shape, GGMLType type) const;

    // ---- 800B+ Robust Parser Tools ----
    bool ParseValue(GGUFMetadataValue& val, uint32_t val_depth = 0);
    bool ParseStringRobust(std::string& str, uint32_t max_len = 0xFFFFFF);
    bool ParseArrayRobust(std::vector<GGUFMetadataValue>& arr, uint32_t depth);
