...
class IGGUFLoader; // Changed from GGUFLoader
class VulkanCompute;
...
private:
    ...
    bool AllocateKVCache(uint32_t batch_size);
    double GetMemoryUsageMB();
    
    // State
    std::unique_ptr<IGGUFLoader> m_loader; // Changed from GGUFLoader to allow StreamingGGUFLoader
