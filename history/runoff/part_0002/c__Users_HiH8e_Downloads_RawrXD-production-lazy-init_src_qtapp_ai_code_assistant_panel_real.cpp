...
void AICodeAssistantPanel::onApplySuggestion() {
    ...
    // Emit signal to apply suggestion to editor
    emit suggestionReady(current_suggestion_.suggested_code, current_suggestion_.file_path, current_suggestion_.line_number);
    qDebug() << "[AICodeAssistantPanel] Emitting suggestion for editor";
}
...
void AICodeAssistantPanel::onExportSuggestion() {
    ...
    // Export suggestion to file
    QString fileName = QFileDialog::getSaveFileName(this, "Export Suggestion", 
                                                   QDir::homePath(), "Code Files (*.cpp *.h *.py *.js *.txt)");
    if (!fileName.isEmpty()) {
        QFile file(fileName);
        if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            QTextStream out(&file);
            out << current_suggestion_.suggested_code;
            file.close();
            qDebug() << "[AICodeAssistantPanel] Suggestion exported to:" << fileName;
        }
    }
}
