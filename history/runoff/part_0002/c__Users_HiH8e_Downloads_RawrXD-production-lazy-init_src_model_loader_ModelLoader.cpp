...
bool ModelLoader::loadModel(const std::string& filepath, bool useStreaming)
{
    ...
    try
    {
        ...
        m_isLoaded = true;
        Logger::info(std::string("ModelLoader: loaded model ") + filepath);
        return true;
    }
    catch (const std::exception& e)
    {
        std::string err = std::string("Exception loading GGUF: ") + e.what();
        ErrorReporter::report(err, nullptr);
        Logger::error(err);
        closeModel();
        return false;
    }
    catch (...)
    {
        ...
        Logger::error(std::string("Unknown exception loading GGUF: ") + filepath);
        closeModel();
        return false;
    }
}
