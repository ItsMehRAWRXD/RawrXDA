...
// ---------- PR8-style WASM engine (compile + instantiate, with a tiny cache) ----------
let wasmModule = null;
const nativeCache = new Map();   // mirrors the idea of a NativeModule cache

async function pr8Compile(source) {
  // source is typically a Response from fetch(wasmUrl)
  if (WebAssembly.compileStreaming && source instanceof Response) {
  ...
  // Fallback: buffer the whole payload, then compile
...
async function pr8Instantiate(compiled, imports) {
  // Keep the signature flexible so we can evolve imports without changing callers
...
export async function initLLamaWasm(wasmUrl) {
  // PR8 path: async compile + instantiate, so large wasm doesn’t block the UI thread.
  const resp = await fetch(wasmUrl);
  const compiled = await pr8Compile(resp);
  const { instance: inst } = await pr8Instantiate(compiled, {
