...
CodeCompletionProvider::CodeCompletionProvider(QObject* parent)
    : QObject(parent), m_lspClient(nullptr)
{
}
...
void CodeCompletionProvider::requestCompletions(const QString& filePath, 
                                               int line, 
                                               int column,
                                               CompletionCallback callback)
{
    ...
    const QString uri = "file://" + filePath;
    QMetaObject::Connection conn;
    conn = connect(m_lspClient, &LSPClient::completionsReceived, this,
        [this, callback, uri, line, column, conn](const QString& respUri, int respLine, int respChar, const QVector<RawrXD::CompletionItem>& items) {
            if (respUri == uri && respLine == line && respChar == column) {
                disconnect(conn);
...
void CodeCompletionProvider::requestSignatureHelp(const QString& filePath, 
                                                 int line, 
                                                 int column,
                                                 SignatureCallback callback)
{
    ...
    const QString uri = "file://" + filePath;
    QMetaObject::Connection conn;
    conn = connect(m_lspClient, &LSPClient::signatureHelpReceived, this,
        [this, callback, uri, conn](const QString& respUri, const SignatureHelp& help) {
            if (respUri != uri) {
                return;
            }
            disconnect(conn);
...
QVector<RawrXD::CompletionItem> CodeCompletionProvider::getSnippets(LanguageID language)
{
    QVector<RawrXD::CompletionItem> snippets;
...
RawrXD::CompletionItem CodeCompletionProvider::createSnippet(const QString& label,
...
QString CodeCompletionProvider::getCompletionKindIcon(CompletionKind kind)
{
