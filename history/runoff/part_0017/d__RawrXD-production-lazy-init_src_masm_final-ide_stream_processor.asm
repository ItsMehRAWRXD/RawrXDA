...
STREAM_PROCESSOR_IMPLEMENTATION equ 1
...
.data
; Stream processor structure (x64-aligned)
STREAM_PROCESSOR struct
    processor_mutex QWORD ?      ; Mutex handle for thread safety
    ...
    total_messages QWORD ?       ; Total messages processed
    total_bytes QWORD ?          ; Total bytes processed
    streams_ptr QWORD ?          ; Pointer to streams array
    consumers_ptr QWORD ?        ; Pointer to consumers array
    allocator_ptr QWORD ?        ; Memory allocator
    deallocator_ptr QWORD ?      ; Memory deallocator
    stats_enabled DWORD ?        ; Statistics enabled
    reserved QWORD 14 dup(?)     ; Reserved
STREAM_PROCESSOR ends

; Stream structure (x64-aligned)
STREAM struct
    stream_id DWORD ?            ; Unique stream ID
    name_ptr QWORD ?             ; Stream name
    name_len DWORD ?             ; Name length
    message_count DWORD ?        ; Messages in stream
    first_message_ptr QWORD ?    ; First message (oldest)
    last_message_ptr QWORD ?     ; Last message (newest)
    consumer_count DWORD ?       ; Number of consumers
    ...
    reserved QWORD 4 dup(?)      ; Reserved
STREAM ends

; Message structure (x64-aligned)
MESSAGE struct
    message_id DWORD ?           ; Unique message ID
    stream_id DWORD ?            ; Stream ID
    timestamp QWORD ?            ; Creation timestamp
    data_ptr QWORD ?             ; Message data
    data_len DWORD ?             ; Data length
    next_ptr QWORD ?             ; Next message in stream
    prev_ptr QWORD ?             ; Previous message in stream
...
; Consumer structure (x64-aligned)
CONSUMER struct
    consumer_id DWORD ?          ; Unique consumer ID
    stream_id DWORD ?            ; Stream ID
    name_ptr QWORD ?             ; Consumer name
    name_len DWORD ?             ; Name length
