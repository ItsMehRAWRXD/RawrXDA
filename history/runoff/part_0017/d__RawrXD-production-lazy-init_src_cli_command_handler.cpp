...
CommandHandler::CommandHandler(AppState& state)
    ...
    m_inferenceParams.temperature = 0.7f;
    m_inferenceParams.topP = 0.9f;
    m_inferenceParams.maxTokens = 128;
    
    // Initialize performance tuner
    auto& perfTuner = Performance::GetPerformanceTuner();
    perfTuner.AutoTune();
    
    printSuccess("CLI Command Handler initialized with adaptive performance tuning");
    
    // Initialize agent systems on startup
    initializeAgentSystems();
    
    // Initialize automatic model loading
...
bool CommandHandler::executeCommand(const std::string& command) {
    ...
    else if (cmd == "build-model") {
        if (parts.size() >= 2) {
            cmdBuildModel(parts, 1);
        } else {
            cmdBuildModelInteractive();
        }
    }
    else if (cmd == "train-model" && parts.size() >= 2) {
        cmdTrainModel(parts[1]);
    }
    else if (cmd == "list-custom-models") {
        cmdListCustomModels();
    }
    else if (cmd == "delete-custom-model" && parts.size() >= 2) {
        cmdDeleteCustomModel(parts[1]);
    }
    else if (cmd == "use-custom-model" && parts.size() >= 2) {
        cmdUseCustomModel(parts[1]);
    }
    else if (cmd == "custom-model-info" && parts.size() >= 2) {
        cmdCustomModelInfo(parts[1]);
    }
    else if (cmd == "digest-sources" && parts.size() >= 2) {
        cmdDigestSources(joinArgs(parts, 1));
    }
    // Hot-reload commands
...
void CommandHandler::printHelp() {
    ...
    std::cout << "  build-model [args]  - Build custom model from sources\n";
    std::cout << "      --name <name>   - Model name (required)\n";
    std::cout << "      --files <path>  - Source files (comma-separated)\n";
    std::cout << "      --dir <path>    - Source directory\n";
    std::cout << "      --desc <text>   - Model description\n";
    std::cout << "      --quant <bits>  - Quantization (4, 8, 16)\n";
    std::cout << "  train-model <name>  - Train existing model\n";
    std::cout << "  list-custom-models  - List all custom models\n";
    std::cout << "  use-custom-model <name> - Load custom model for inference\n";
    std::cout << "  custom-model-info <name> - Show custom model details\n";
    std::cout << "  delete-custom-model <name> - Delete custom model\n";
    std::cout << "  digest-sources <paths> - Process source files\n";
    ...
    std::cout << "  snapshot            - Take telemetry snapshot\n";
        std::cout << "  serverinfo          - Show API server statistics\n";
...
void CommandHandler::cmdInfer(const std::string& prompt) {
    ...
    printInfo("Generating response (non-streaming)...");
    
    // TODO: Implement proper inference using inference engine
    printWarning("Inference functionality requires integration with inference engine");
    
    /* Commented out - GGUFLoader doesn't have these methods
    // Tokenize the prompt
    std::vector<int32_t> tokens = m_modelLoader->Tokenize(prompt);
    if (tokens.empty()) {
        printError("Failed to tokenize prompt");
        return;
    }
    
    printInfo(std::string("Tokenized into ") + std::to_string(tokens.size()) + " tokens");
    
    // Generate response tokens (synchronous)
    std::vector<int32_t> resultTokens = tokens;
    int maxNewTokens = m_inferenceParams.maxTokens;
    
    for (int i = 0; i < maxNewTokens; ++i) {
        // Get inference for current token(s)
    ...
    // Detokenize and display result
    std::string result = m_modelLoader->Detokenize(resultTokens);
    std::cout << "\nResponse: " << result << "\n" << std::endl;
    */
}
...
void CommandHandler::cmdInferStream(const std::string& prompt) {
    ...
    printInfo("Starting streaming inference...");
    
    // Create streaming manager for real-time token output
    std::unique_ptr<StreamingManager> streamer = std::make_unique<StreamingManager>();
    std::atomic<int> tokenCount{0};
    
    // Setup callback for token display
    streamer->StartStream(prompt, [this, &tokenCount](const std::string& token, bool is_final) {
        ...
        if (is_final) {
            std::cout << std::endl;
            printSuccess(std::string("\nStreaming complete: ") + std::to_string(tokenCount.load()) + " tokens generated");
        }
    });
    
    try {
        // Get performance config for batch sizing
        ...
        // Simulate token generation with streaming callbacks
        // In production, this would connect to actual inference engine
        // Example: m_inferenceEngine->GenerateStream(prompt, config, 
        //          [streamer](const std::string& token) { streamer->ProcessChunk(token); });
        
        // For now, demonstrate with mock tokens
        std::vector<std::string> mockTokens = {
            "This", " is", " a", " streaming", " response", ".",
            " Each", " token", " is", " displayed", " in", " real", "-", "time", "."
        };
        
        for (size_t i = 0; i < mockTokens.size(); ++i) {
            streamer->ProcessChunk(mockTokens[i]);
            bool isFinal = (i == mockTokens.size() - 1);
            // Callback invoked by streamer
        ...
        printError(std::string("Streaming error: ") + e.what());
        streamer->StopStream();
    }
}
...
void CommandHandler::cmdChat() {
    ...
    printInfo("Interactive Chat Mode - Type 'exit' to quit");
    printInfo("Each response will be generated with streaming...");
    std::cout << std::endl;
    
    // Initialize history and streaming manager for chat
    HistoryManager chatHistory("rawrxd_chat_history.txt");
    std::vector<std::string> conversationHistory;
    
    try {
        while (true) {
            std::cout << Colors::CYAN << "You> " << Colors::RESET;
            std::cout.flush();
            ...
            // Stream the response using streaming manager
            std::unique_ptr<StreamingManager> streamer = std::make_unique<StreamingManager>();
            std::atomic<int> tokenCount{0};
            std::string fullResponse;
            
            streamer->StartStream(context, [&tokenCount, &fullResponse](const std::string& token, bool is_final) {
                if (!token.empty()) {
                    std::cout << token << std::flush;
                    fullResponse += token;
                    tokenCount++;
            ...
            std::cout << Colors::BLUE << "Assistant> " << Colors::RESET;
            std::cout.flush();
            
            // Simulate streaming response with mock tokens
            std::vector<std::string> responseTokens = {
                "I", " understand", " you", " said", " ", "'", userInput, "'", ".",
                " That", "'s", " interesting", "!"
            };
            
            for (size_t i = 0; i < responseTokens.size(); ++i) {
                streamer->ProcessChunk(responseTokens[i]);
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
            }
        ...
        printError(std::string("Chat error: ") + e.what());
    }
}

void CommandHandler::cmdSetTemperature(float temp) {
    float clamped = std::max(0.0f, std::min(2.0f, temp));
    m_inferenceParams.temperature = clamped;
    printSuccess(std::string("Temperature set to: ") + std::to_string(clamped));
    printInfo("Temperature affects randomness of token selection (higher = more random)");
}

void CommandHandler::cmdSetTopP(float topP) {
    float clamped = std::max(0.0f, std::min(1.0f, topP));
    m_inferenceParams.topP = clamped;
    printSuccess(std::string("Top-P set to: ") + std::to_string(clamped));
    printInfo("Top-P (nucleus sampling) controls diversity (lower = more focused)");
}

void CommandHandler::cmdSetMaxTokens(int maxTokens) {
    int clamped = std::max(1, maxTokens);
    m_inferenceParams.maxTokens = clamped;
    printSuccess(std::string("Max tokens set to: ") + std::to_string(clamped));
    printInfo("This controls maximum length of generated responses");
}
...
void CommandHandler::cmdServerInfo() {
    ...
    if (!m_state.api_server) {
        printError("API Server not initialized");
        return;
    }
    
    auto* api = m_state.api_server;
    std::cout << "Server Status: " << (api->IsRunning() ? "\033[32mRunning\033[0m" : "\033[31mStopped\033[0m") << std::endl;
    std::cout << "Listen Port: " << api->GetPort() << std::endl;
    
    // Get metrics from API server
    auto metrics = api->GetMetrics();
...
// ============================================================================
// CUSTOM MODEL BUILDER CLI COMMANDS
// ============================================================================

void CommandHandler::cmdBuildModel(const std::vector<std::string>& parts, size_t startIdx) {
    using namespace CustomModelBuilder;
    
    ModelBuilder::BuildConfig config;
    config.modelName = "custom_model";
    config.modelDescription = "Custom built model";
    config.architecture.vocabSize = 32000;
    config.architecture.embeddingDim = 512;
    config.architecture.numLayers = 6;
    config.architecture.numHeads = 8;
    config.trainingConfig.epochs = 10;
    config.trainingConfig.batchSize = 8;
    ...
    for (size_t i = startIdx; i < parts.size(); ++i) {
        if (parts[i] == "--name" && i + 1 < parts.size()) {
            config.modelName = parts[++i];
        } else if (parts[i] == "--files" && i + 1 < parts.size()) {
            std::string filesStr = parts[++i];
            std::istringstream iss(filesStr);
            std::string file;
            while (std::getline(iss, file, ',')) {
                config.sourceFiles.push_back(file);
            }
        } else if (parts[i] == "--dir" && i + 1 < parts.size()) {
            config.sourceDirectories.push_back(parts[++i]);
        } else if (parts[i] == "--desc" && i + 1 < parts.size()) {
            config.modelDescription = parts[++i];
        } else if (parts[i] == "--quant" && i + 1 < parts.size()) {
            config.quantBits = std::stoi(parts[++i]);
        } else if (parts[i] == "--epochs" && i + 1 < parts.size()) {
            config.trainingConfig.epochs = std::stoi(parts[++i]);
        }
    }
    
    if (config.sourceFiles.empty() && config.sourceDirectories.empty()) {
        printError("No sources specified. Use --files or --dir");
        return;
    }
    
    printInfo("Building custom model: " + config.modelName);
    printInfo("Sources: " + std::to_string(config.sourceFiles.size()) + " files, " + 
              std::to_string(config.sourceDirectories.size()) + " directories");
    
    try {
        auto future = ModelBuilder::getInstance().buildModelAsync(config);
        
        printInfo("Model building started in background...");
        printInfo("This may take several minutes depending on source size.");
        
        auto metadata = future.get();
        
        printSuccess("Model built successfully!");
        printSuccess("  Name: " + metadata.modelName);
        printSuccess("  Path: " + metadata.ggufPath);
        printSuccess("  Size: " + std::to_string(metadata.modelSizeBytes / (1024*1024)) + " MB");
        printSuccess("  Parameters: " + std::to_string(metadata.parametersCount));
        printSuccess("  Training Loss: " + std::to_string(metadata.trainingLoss));
        printSuccess("  Perplexity: " + std::to_string(metadata.validationPerplexity));
        
    } catch (const std::exception& e) {
        printError("Model build failed: " + std::string(e.what()));
    }
}

void CommandHandler::cmdBuildModelInteractive() {
    using namespace CustomModelBuilder;
    
    printInfo("\n=== Interactive Custom Model Builder ===\n");
    
    ModelBuilder::BuildConfig config;
    
    // Get model name
    std::cout << "Enter model name: ";
    std::getline(std::cin, config.modelName);
    
    // Get model description
    std::cout << "Enter model description: ";
    std::getline(std::cin, config.modelDescription);
    
    // Get source directories
    std::cout << "Enter source directory (or leave blank to skip): ";
    std::string sourceDir;
    std::getline(std::cin, sourceDir);
    if (!sourceDir.empty()) {
        config.sourceDirectories.push_back(sourceDir);
    }
    
    // Get source files
    std::cout << "Enter source files (comma-separated, or leave blank): ";
    std::string filesStr;
    std::getline(std::cin, filesStr);
    if (!filesStr.empty()) {
        std::istringstream iss(filesStr);
        std::string file;
        while (std::getline(iss, file, ',')) {
    ...
    if (config.sourceFiles.empty() && config.sourceDirectories.empty()) {
        printError("No sources specified. Aborting.");
        return;
    }
    
    // Get training parameters
    std::cout << "Enter number of training epochs (default 10): ";
    std::string epochsStr;
    std::getline(std::cin, epochsStr);
    if (!epochsStr.empty()) {
        config.trainingConfig.epochs = std::stoi(epochsStr);
    }
    
    std::cout << "Enter quantization bits (4, 8, or 16, default 4): ";
    std::string quantStr;
    std::getline(std::cin, quantStr);
    if (!quantStr.empty()) {
        config.quantBits = std::stoi(quantStr);
    }
    ...
    printInfo("\nStarting model build with configuration:");
    printInfo("  Name: " + config.modelName);
    printInfo("  Description: " + config.modelDescription);
    printInfo("  Sources: " + std::to_string(config.sourceFiles.size() + config.sourceDirectories.size()));
    printInfo("  Epochs: " + std::to_string(config.trainingConfig.epochs));
    printInfo("  Quantization: " + std::to_string(config.quantBits) + " bits");
    
    try {
        auto metadata = ModelBuilder::getInstance().buildModel(config);
        
        printSuccess("\n✓ Model built successfully!");
        printSuccess("  Path: " + metadata.ggufPath);
        printSuccess("  Size: " + std::to_string(metadata.modelSizeBytes / (1024*1024)) + " MB");
        printInfo("\nYou can now use this model with: use-custom-model " + metadata.modelName);
...
void CommandHandler::cmdTrainModel(const std::string& modelName) {
    printInfo("Training model: " + modelName);
    printWarning("Retraining not yet implemented. Use build-model to create new models.");
}

void CommandHandler::cmdListCustomModels() {
    using namespace CustomModelBuilder;
    
    auto models = ModelBuilder::getInstance().listCustomModels();
    
    if (models.empty()) {
        printInfo("No custom models found.");
        printInfo("Build one with: build-model --name mymodel --dir ./src");
        return;
    }
    ...
    for (const auto& model : models) {
        std::cout << Colors::CYAN << "  " << model.modelName << Colors::RESET << "\n";
        std::cout << "    Version: " << model.version << "\n";
        std::cout << "    Path: " << model.ggufPath << "\n";
        std::cout << "    Size: " << (model.modelSizeBytes / (1024*1024)) << " MB\n";
        std::cout << "    Parameters: " << model.parametersCount << "\n";
        std::cout << "    Architecture: " << model.architecture.numLayers << " layers, " 
                  << model.architecture.numHeads << " heads\n";
        if (model.trainingLoss > 0) {
            std::cout << "    Training Loss: " << std::fixed << std::setprecision(4) 
                      << model.trainingLoss << "\n";
        }
        if (model.validationPerplexity > 0) {
            std::cout << "    Perplexity: " << std::fixed << std::setprecision(2) 
                      << model.validationPerplexity << "\n";
        }
        std::cout << "\n";
    }
    
    printSuccess("Total: " + std::to_string(models.size()) + " custom models");
}

void CommandHandler::cmdDeleteCustomModel(const std::string& modelName) {
    using namespace CustomModelBuilder;
    
    printWarning("Are you sure you want to delete model '" + modelName + "'? (y/n): ");
    std::string confirm;
    ...
    if (ModelBuilder::getInstance().deleteModel(modelName)) {
        printSuccess("Model deleted: " + modelName);
    } else {
        printError("Failed to delete model: " + modelName);
    }
}

void CommandHandler::cmdUseCustomModel(const std::string& modelName) {
    using namespace CustomModelBuilder;
    
    auto metadata = ModelBuilder::getInstance().getModelMetadata(modelName);
    
    if (metadata.modelName.empty()) {
        printError("Custom model not found: " + modelName);
        return;
    }
    
    if (!std::filesystem::exists(metadata.ggufPath)) {
        printError("Model file not found: " + metadata.ggufPath);
        return;
    }
    
    printInfo("Loading custom model: " + modelName);
    
    // Load using custom inference engine
    if (ModelBuilder::getInstance().getInferenceEngine().loadModel(metadata.ggufPath)) {
        printSuccess("Custom model loaded successfully!");
        printInfo("  Path: " + metadata.ggufPath);
        printInfo("  Size: " + std::to_string(metadata.modelSizeBytes / (1024*1024)) + " MB");
        printInfo("  Max context: " + std::to_string(metadata.architecture.maxSeqLen) + " tokens");
        printInfo("\nYou can now use 'infer' or 'chat' commands with this model.");
    } else {
        printError("Failed to load custom model");
    }
}

void CommandHandler::cmdCustomModelInfo(const std::string& modelName) {
    using namespace CustomModelBuilder;
    ...
    std::cout << "Name: " << Colors::CYAN << metadata.modelName << Colors::RESET << "\n";
    std::cout << "ID: " << metadata.modelId << "\n";
    std::cout << "Version: " << metadata.version << "\n";
    std::cout << "Description: " << metadata.description << "\n\n";
    
    std::cout << "File Information:\n";
    std::cout << "  Path: " << metadata.ggufPath << "\n";
    std::cout << "  Size: " << (metadata.modelSizeBytes / (1024*1024)) << " MB\n";
    std::cout << "  Parameters: " << metadata.parametersCount << "\n\n";
    
    std::cout << "Architecture:\n";
    std::cout << "  Vocabulary Size: " << metadata.architecture.vocabSize << "\n";
    std::cout << "  Embedding Dim: " << metadata.architecture.embeddingDim << "\n";
    std::cout << "  Layers: " << metadata.architecture.numLayers << "\n";
    std::cout << "  Attention Heads: " << metadata.architecture.numHeads << "\n";
    std::cout << "  Hidden Dim: " << metadata.architecture.hiddenDim << "\n";
    std::cout << "  Max Sequence Length: " << metadata.architecture.maxSeqLen << "\n";
    std::cout << "  Activation: " << metadata.architecture.activationType << "\n\n";
    
    if (!metadata.sourceFiles.empty()) {
        std::cout << "Source Files (" << metadata.sourceFiles.size() << "):\n";
        for (size_t i = 0; i < std::min(metadata.sourceFiles.size(), size_t(5)); ++i) {
            std::cout << "  - " << metadata.sourceFiles[i] << "\n";
        }
        if (metadata.sourceFiles.size() > 5) {
            std::cout << "  ... and " << (metadata.sourceFiles.size() - 5) << " more\n";
        }
        std::cout << "\n";
    }
    
    if (metadata.trainingLoss > 0) {
        std::cout << "Training Metrics:\n";
        std::cout << "  Final Loss: " << std::fixed << std::setprecision(4) 
                  << metadata.trainingLoss << "\n";
        std::cout << "  Validation Perplexity: " << std::fixed << std::setprecision(2) 
                  << metadata.validationPerplexity << "\n\n";
    }
    
    auto created = std::chrono::system_clock::to_time_t(metadata.createdAt);
    std::cout << "Created: " << std::put_time(std::localtime(&created), "%Y-%m-%d %H:%M:%S") << "\n";
    
    if (metadata.isOllamaCompatible) {
        std::cout << Colors::GREEN << "\n✓ Ollama API Compatible" << Colors::RESET << "\n";
    }
}

void CommandHandler::cmdDigestSources(const std::string& pathsStr) {
    using namespace CustomModelBuilder;
    
    printInfo("Digesting source files...");
    
    auto& engine = ModelBuilder::getInstance().getDigestionEngine();
    ...
    while (iss >> path) {
        if (std::filesystem::is_directory(path)) {
            engine.addSourceDirectory(path, true);
            printInfo("Added directory: " + path);
        } else if (std::filesystem::is_regular_file(path)) {
            engine.addSource(path, SourceType::CODE_FILE);
            filesAdded++;
            printInfo("Added file: " + path);
        } else {
    ...
    printInfo("\nProcessing sources...");
    auto samples = engine.digestAll();
    
    printSuccess("\n✓ Source digestion complete!");
    printSuccess("  Total samples: " + std::to_string(samples.size()));
    printSuccess("  Total tokens: " + std::to_string(engine.getTotalTokens()));
    printInfo("\nSamples are ready for model building.");
}
