...
bool NativeInferenceEngine::LoadModel(const std::string& gguf_path) {
    ...
    auto get_uint64 = [&](const std::string& key) -> uint64_t {
        ...
        if (it != metadata_map.end() && it->second.type == GGUFUtils::MetadataType::UINT64) {
            return it->second.value.u64;
        }
        return 0;
    };
    
    vocab_size_ = get_uint64("llama.vocab_size");
    context_length_ = get_uint64("llama.context_length");
    embedding_dim_ = get_uint64("llama.embedding_length");
    num_layers_ = get_uint64("llama.block_count");
...
bool NativeInferenceEngine::LoadTensorAsFloat(const std::string& name, std::vector<float>& data) {
    ...
    auto tensor_info = loader_->GetTensorInfo();
    auto it = tensor_map_.find(name);
    if (it == tensor_map_.end()) return false;
