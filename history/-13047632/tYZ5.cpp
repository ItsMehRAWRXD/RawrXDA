                                     ...
                                     const QJsonObject& initialContext)
{
    ...
    QList<AgentTask> readyToEmit;
    {
        ...
        readyToEmit = scheduleReadyTasks(insertedPlan);
    }
                                    ...
                                    const QString& message)
{
...

    {
        ...
        // Determine whether plan finished
        ...
        if (!runningOrPending) {
            planFinished = true;
            planFailed = planFailed || anyFailed;
    ...
    if (planFinished) {
        if (planFailed) {
            emit planFailed(planId, failureReason.isEmpty() ? QStringLiteral("plan-failed") : failureReason);
        } else {
            emit planCompleted(planId, finalContext);
        }
    }
