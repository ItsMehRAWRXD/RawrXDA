                                     ...
                                     int maxConcurrency)
{
    ...
    logCoordinatorEvent("Agent registered", QString("ID: %1, MaxConcurrency: %2").arg(agentId).arg(maxConcurrency));
    return true;
}
                                     ...
                                     const QJsonObject& initialContext)
{
    ...
    if (!validateTasks(tasks, validationError)) {
        ...
        logCoordinatorEvent("Plan submission failed", QString("Validation error: %1").arg(validationError));
        return {};
    }
    ...
    logCoordinatorEvent("Plan submitted", QString("ID: %1, Tasks: %2, Ready: %3").arg(plan.id).arg(tasks.size()).arg(readyToEmit.size()));
    return plan.id;
}
