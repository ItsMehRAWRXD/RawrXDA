...
void InferenceEngine::request(const QString& prompt, qint64 reqId)
{
    ...
    // Simple tokenization: convert prompt to token IDs (character-based placeholder)
    std::vector<int32_t> tokens;
    for (QChar c : prompt) {
        tokens.push_back(c.unicode() % 50257);  // Limit to vocab size
    }
    
    m_inferenceTimer.start();
    
    // Generate response using transformer (max 50 tokens)
    std::vector<int32_t> generatedTokens = m_transformer.generate(tokens, 50);
    
    // Convert token IDs back to text (simple character-based decoding)
    QString response;
    for (int32_t token : generatedTokens) {
        response.append(QChar(token));
    }
    
    qint64 elapsed = m_inferenceTimer.elapsed();
    m_tokensPerSecond = (50.0 / elapsed) * 1000.0;  // tokens/sec
    
    qInfo() << "Inference completed in" << elapsed << "ms (" 
            << m_tokensPerSecond << "tokens/sec)";
