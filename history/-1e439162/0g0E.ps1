...
function Invoke-PoshLLMTrainHandler {
    ...
    try {
        ...
        Write-Host "Parameters: Merges=$Merges, EmbedDim=$EmbedDim, Heads=$Heads, Layers=$Layers, BlockSize=$BlockSize" -ForegroundColor Gray

        $model = New-PoshLLMModel -Corpus $corpus -Merges $Merges -EmbedDim $EmbedDim -Heads $Heads -Layers $Layers -BlockSize $BlockSize
        Register-PoshLLMModel -Name $Name -Model $model

        Write-Host "✅ Model '$Name' trained successfully!" -ForegroundColor Green
        Write-Host "Vocabulary size: $($model.Tokenizer.VocabSize)" -ForegroundColor Gray
        Write-Host "Use 'poshllm-generate -Name $Name -Prompt "your text here"' to generate text" -ForegroundColor Gray
...
function Invoke-PoshLLMGenerateHandler {
    ...
    try {
        ...
        if (-not $model) {
            Write-Host "Error: Model '$Name' not found. Use poshllm-list to see available models." -ForegroundColor Red
            return 1
        }
        ...
        $generated = Invoke-PoshLLMGenerate -Model $model -Prompt $Prompt -MaxTokens $MaxTokens -Temperature $Temperature
...
function Invoke-PoshLLMListHandler {
    try {
        ...
        foreach ($modelName in $models.Keys) {
            ...
            Write-Host "   Vocab Size: $($model.Tokenizer.VocabSize)" -ForegroundColor Gray
