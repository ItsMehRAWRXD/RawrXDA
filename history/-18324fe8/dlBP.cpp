...
{
    aiHub = std::make_shared<AIIntegrationHub>();
}
...
LRESULT CALLBACK MainWindow::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        ...
        case WM_KEYUP:
            if (wParam == VK_F5 && self && self->aiHub) {
                // Explicit Logic: Trigger Inference Chain
                std::string result = self->aiHub->generateTests("int factorial(int n) { return n<=1?1:n*factorial(n-1); }");
                MessageBoxA(hwnd, result.c_str(), "Titan Inference Result", MB_OK);
            }
            return 0;
