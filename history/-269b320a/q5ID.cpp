...
// Use explicit path to avoid conflict with src/core/enterprise_license.h (V1)
...
LicenseResult EnterpriseLicenseV2::initialize() {
    ...
    // Check for dev unlock environment variable
    ...
    if (licPath) {
        ...
        if (!loadResult.success) {
            // Non-fatal: fall back to community
            recordAudit(FeatureID::BasicGGUFLoading, true, "initialize",
                       "License file load failed, using Community tier");
        }
    }
...
void EnterpriseLicenseV2::logEnforcementEvent(FeatureID id, bool granted,
                                               const char* caller,
                                               const char* detail) {
    std::lock_guard<std::mutex> lock(m_mutex);
    recordAudit(id, granted, caller, detail);
}
