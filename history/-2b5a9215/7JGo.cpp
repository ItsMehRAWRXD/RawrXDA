...
void AgenticEngine::processMessage(const QString& message, const QString& editorContext, bool streaming) {
    ...
    auto resetFlag = [&inferenceInProgress]() {
        inferenceInProgress = false;
        qDebug() << "[AgenticEngine] Inference flag reset";
    };
    
    // Connect to responseReady signal to reset flag
    QMetaObject::Connection* resetConnection = new QMetaObject::Connection();
    *resetConnection = connect(this, &AgenticEngine::responseReady, 
        this, [resetConnection, resetFlag]() {
            resetFlag();
            disconnect(*resetConnection);
            delete resetConnection;
