...
void AgenticEngine::setModelSource(ModelSource source) {
    ...
    qInfo() << "[AgenticEngine] Model source changed to:" << (source == Local ? "Local" : "External");
}

void AgenticEngine::configureExternalModel(const QString& provider, const QString& endpoint, const QString& apiKey, const QString& model) {
    if (!m_externalClient) return;
    
    ExternalModelClient::Provider p = ExternalModelClient::OpenAI;
    if (provider.toLower() == "anthropic") p = ExternalModelClient::Anthropic;
    else if (provider.toLower() == "groq") p = ExternalModelClient::Groq;
    else if (provider.toLower() == "ollama") p = ExternalModelClient::Ollama;
    
    m_externalClient->setConfiguration(p, endpoint, apiKey, model);
    qInfo() << "[AgenticEngine] External model configured:" << provider << model;
}
