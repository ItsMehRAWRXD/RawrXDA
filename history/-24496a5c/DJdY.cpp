...
void TokenGenerator::loadConfigFromJSON(const std::string& path) {
    // Load tokenizer config (vocab_size, model_type, bos/eos token IDs)
    ...
    auto extractInt = [&](const char* key) -> int {
        ...
        if (pos == std::string::npos) return -1;
        while (pos < content.size() && !std::isdigit((unsigned char)content[pos]) && content[pos] != '-') pos++;
        return std::atoi(content.c_str() + pos);
    };
    int vs = extractInt("\"vocab_size\"");
    if (vs > 0) m_config.vocabSize = vs;
    int bos = extractInt("\"bos_token_id\"");
    if (bos >= 0) m_config.bosTokenId = bos;
    int eos = extractInt("\"eos_token_id\"");
    if (eos >= 0) m_config.eosTokenId = eos;
}
void TokenGenerator::loadTokenizerConfigFromJSON(const std::string& path) {
    // Load tokenizer.json — contains merges, added_tokens, normalizer spec
    std::ifstream ifs(path);
    if (!ifs) return;
    // For BPE tokenizers, we'd parse merges here
    // For now, config load is sufficient — vocabulary loaded separately via loadVocabularyFromMemory
    (void)path;
}
...
void TokenGenerator::addToCache(const std::string& key, const std::vector<int>& tokens, bool encode) {
    std::lock_guard l(m_cacheMutex);
    if (encode) m_encodeCache[key] = tokens;
    else {
        std::string decoded;
        for (int id : tokens) { auto it = m_idToToken.find(id); if (it != m_idToToken.end()) decoded += it->second; }
        m_decodeCache[key] = decoded;
    }
...
void TokenGenerator::evictCacheIfNeeded() {
    ...
    if (m_encodeCache.size() > maxCache) m_encodeCache.clear();
    if (m_decodeCache.size() > maxCache) m_decodeCache.clear();
    m_cacheSize = m_encodeCache.size() + m_decodeCache.size();
}

std::string TokenGenerator::detectTokenType(const std::string& tok) const {
    if (tok.empty()) return "unknown";
    if (tok[0] == '<' && tok.back() == '>') return "control";
    if (tok.find_first_of(" \t\n") != std::string::npos) return "whitespace";
    if (tok.size() == 1 && !std::isalnum((unsigned char)tok[0])) return "punctuation";
    return "word";
}
void TokenGenerator::logTokenization(const std::string& input, const std::vector<int>& tokens) {
    (void)input; (void)tokens; // Available for debug builds via OutputDebugStringA if needed
}
void TokenGenerator::logError(const std::string& msg, RawrXD::TokenError err) {
    (void)msg; (void)err; // Available for debug builds
}
