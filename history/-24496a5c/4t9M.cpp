...
Expected<void, TokenError> TokenGenerator::loadVocabularyFromModel(const std::string& path) {
...
Expected<void, TokenError> TokenGenerator::loadVocabularyFromHuggingFace(const std::string& path) {
    std::ifstream file(path);
    if(!file) return Unexpected(TokenError::FileReadFailed);
    json j; 
    try { file >> j; } catch(...) { return Unexpected(TokenError::InvalidFormat); }
