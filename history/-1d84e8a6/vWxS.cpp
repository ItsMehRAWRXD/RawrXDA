...
CorrectionResult AgenticPuppeteer::correctResponse(const QString& originalResponse, const QString& userPrompt)
{
    ...
    if (!m_enabled || originalResponse.isEmpty()) {
        return CorrectionResult::failure(FailureType::None, "Puppeteer disabled or empty response");
    }
    ...
    if (failure == FailureType::None) {
        return CorrectionResult::success(originalResponse, FailureType::None);
    }
    ...
    if (corrected != originalResponse && !corrected.isEmpty()) {
        ...
        return CorrectionResult::success(corrected, failure);
    } else {
        m_stats.failedCorrections++;
        emit correctionFailed(failure, "Could not generate correction");
        return CorrectionResult::failure(failure, "Correction generation failed");
    }
}
...
CorrectionResult RefusalBypassPuppeteer::bypassRefusal(const QString& refusedResponse, const QString& originalPrompt)
{
    ...
    if (!reframed.isEmpty()) {
        return CorrectionResult::success(reframed, FailureType::RefusalResponse);
    }
    
    return CorrectionResult::failure(FailureType::RefusalResponse, "Could not reframe refusal");
}
    ...
    const QString& response, const QStringList& knownFacts)
{
    ...
    if (foundHallucination) {
        ...
        return CorrectionResult::success(corrected, FailureType::Hallucination);
    }
    
    return CorrectionResult::success(response, FailureType::None);
}
...
CorrectionResult FormatEnforcerPuppeteer::enforceJsonFormat(const QString& response)
{
    ...
    if (!fixedDoc.isNull()) {
        return CorrectionResult::success(corrected, FailureType::FormatViolation);
    }
    
    return CorrectionResult::failure(FailureType::FormatViolation, "Could not repair JSON");
}
