...
CorrectionResult AgenticPuppeteer::CorrectResponse(const std::string& original_response, const std::string& user_prompt) {
    // Enterprise license enforcement gate for agentic self-correction
    if (!LicenseEnforcementGate::getInstance().checkAccess(
            EnterpriseFeature::AgenticSelfCorrection, "AgenticPuppeteer::CorrectResponse")) {
        return CorrectionResult::Error(CorrectionFailureType::None, "Feature requires Enterprise license");
    }
