...
std::vector<int32_t> InferenceEngine::generate(const std::vector<int32_t>& inputTokens, int maxTokens)
{
    ...
    if (m_transformer.isReady()) {
        ...
        // Attempt to use Ollama if configured
        if (m_useOllama && !m_ollamaEndpoint.isEmpty()) {
            qInfo() << "[generate] Attempting Ollama fallback generation";
            
            // Build prompt from input tokens using detokenize
            QString prompt = detokenize(inputTokens);
            
            // Use Ollama for generation (synchronous for simplicity)
            QNetworkAccessManager manager;
            QNetworkRequest request(QUrl(m_ollamaEndpoint + "/api/generate"));
            request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
            
            QJsonObject payload;
            payload["model"] = m_ollamaModel.isEmpty() ? "llama2" : m_ollamaModel;
            payload["prompt"] = prompt;
            payload["stream"] = false;
            payload["options"] = QJsonObject{{"num_predict", maxTokens}};
            ...
            if (reply->error() == QNetworkReply::NoError) {
                ...
                QString generatedText = doc.object()["response"].toString();
            ...
            reply->deleteLater();
            qWarning() << "[generate] Ollama fallback failed, using echo mode";
        }
