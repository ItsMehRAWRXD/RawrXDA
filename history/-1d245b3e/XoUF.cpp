...
void InferenceEngine::streamingGenerateWorkerSignals(qint64 reqId, const QString& prompt, int maxTokens)
{
    ...
    if (m_useOllama) {
        ...
        // Ensure OllamaProxy network calls happen in the object's owning thread
        if (m_ollamaProxy->thread() == QThread::currentThread()) {
            m_ollamaProxy->generateResponse(prompt, m_temperature, maxTokens);
        } else {
