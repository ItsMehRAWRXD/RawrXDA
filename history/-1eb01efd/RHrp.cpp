...
app.get('/api/config', (req, res) => {
    res.json({
        ...
        features: {
            ...
            typescript: )" << (config.include_typescript ? "true" : "false") << R"(,
            tailwind: )" << (config.include_tailwind ? "true" : "false") << R"(
        }
    });
});";
...
app.post('/api/auth/login', (req, res) => {
    ...
    res.json({ token: 'jwt-token-here', user: { id: 1, username } });
});
...
app.get('/api/auth/me', (req, res) => {
    ...
    res.json({ user: { id: 1, username: 'demo' } });
});";
    }
