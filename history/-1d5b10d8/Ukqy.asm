...
codegen_node:
    ...
    push rbp
    mov rbp, rsp
    push rcx                        ; save AST node pointer
    
    ; Get node type
    mov rcx, [rsp+8]                ; restore AST node pointer
    movzx rax, byte [rcx]
    
    ; Print simple debug message
    lea rcx, [debug_codegen_started]
    call print_string
    
    ; Restore AST node pointer and check type
    mov rcx, [rsp+8]
    movzx rax, byte [rcx]
    cmp al, AST_PROGRAM
    je .done
...
.entry_ok:
    ...
    ; TEMPORARY HACK: Emit NOP for all instructions to test the rest of the pipeline
    mov byte [rdi], OP_NOP
    inc rdi
