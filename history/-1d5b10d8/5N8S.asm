...
.entry_ok:
    ...
    mov rcx, [rbx+8]
    call get_token_ptr
    mov r8, rax                    ; token pointer

    ; Debug: emit mnemonic being encoded
    lea rcx, [r8+1]
    call print_string
    lea rcx, [newline]
    call print_string
...
print_error_message:
    ...
    cmp rax, ERR_FILE_READ
    je .err_file_read
    cmp rax, ERR_LEXER
    je .err_lexer
    cmp rax, ERR_PARSER
    je .err_parser
    cmp rax, ERR_SEMANTIC
    je .err_semantic
    cmp rax, ERR_CODEGEN
    je .err_codegen
    cmp rax, ERR_PEWRITER
    je .err_pewriter
    jmp .done

.err_file_read:
    lea rcx, [err_file_read]
    call print_string
    jmp .done
...
section .data
    ...
    err_file_read           db "File not found", 13, 10, 0
    err_lexer               db "Lexical error", 13, 10, 0
    err_parser              db "Parser error", 13, 10, 0
    err_semantic            db "Semantic error", 13, 10, 0
    err_codegen             db "Code generation error", 13, 10, 0
    err_pewriter            db "PE write error", 13, 10, 0
