...
{
    if (index < 0) return;
    const QString modelName = m_detectedModelsCombo->currentText();
    // Resolve blob path via proxy helper
    QString blobPath;
    if (m_ollamaProxy) {
        // Prefer exact lookup if available
        blobPath = m_ollamaProxy->blobPathForModel(modelName);
        if (blobPath.isEmpty()) {
            // Fallback: try heuristic
            blobPath = m_ollamaProxy->resolveBlobToModel(modelName);
        }
    }
    if (!blobPath.isEmpty() && QFile::exists(blobPath)) {
