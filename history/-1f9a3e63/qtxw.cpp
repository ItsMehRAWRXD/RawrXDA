...
void BlobConverterPanel::onDetectedModelSelected(int index)
{
    ...
    if (!m_detectedModelCombo || index < 0) {
        return;
    }
    
    QString selectedModel = m_detectedModelCombo->currentText();
    if (selectedModel.isEmpty()) {
        return;
    }
    
    // Try to auto-populate metadata based on detected model
    addLog(QString("[BlobConvert] Selected detected model: %1").arg(selectedModel));
    
    // If we have a blob path associated with this model, use it
    // The model names in the combo should correspond to detected blob files
    if (m_detectedBlobs.contains(selectedModel)) {
        QString blobPath = m_detectedBlobs[selectedModel];
        m_blobPathEdit->setText(blobPath);
        
        // Trigger auto-output path generation
        QFileInfo fi(blobPath);
        QString outputName = fi.baseName() + ".gguf";
        m_outputPathEdit->setText(fi.absolutePath() + "/" + outputName);
    }
