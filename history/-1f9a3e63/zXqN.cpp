...
{
    // Feature flag check
    if (!SettingsManager::instance().getValue("EnableOllamaBlobScan", true).toBool()) {
        addLog("[BlobScan] Feature disabled in settings", true);
        return;
    }
...
void BlobConverterPanel::onModelSelected(int index)
{
    ...
    if (!modelName.isEmpty()) {
        QString blobPath = m_ollamaProxy->getBlobPath(modelName);
        if (!blobPath.isEmpty()) {
