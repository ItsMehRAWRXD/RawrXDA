    ...
    if (!initialized) {
        ...
        QTimer::singleShot(0, this, [this] {
            ...
            if (!m_toolRegistry) {
                ...
                // Wire the existing backend tool implementations into ToolRegistry so
                // ToolRegistry_InvokeToolSet/QueryAvailableTools reflect real tools.
                try {
                    RawrXD::registerBackendAgenticTools(*m_toolRegistry, QDir::currentPath().toStdString());
                } catch (const std::exception& e) {
                    if (logger) {
                        logger->error("AgenticIDE", std::string("Failed to register backend tools: ") + e.what());
                    }
                }
            }
