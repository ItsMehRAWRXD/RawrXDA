...
bool GGUFLoader::Open(const std::string& filepath) {
    ...
    if (!file_.is_open()) {
        // DIAGNOSTIC: Log exact reason for file open failure
        std::cerr << "[GGUFLoader] ERROR: Failed to open file: " << filepath << std::endl;
        std::cerr << "[GGUFLoader] Check file exists, permissions, and path encoding" << std::endl;
        return false;
    }
    
    std::cout << "[GGUFLoader] File opened successfully: " << filepath << std::endl;
    is_open_ = true;
    
    // ParseHeader now throws exceptions on error (consistent error handling)
    try {
        if (!ParseHeader()) {
            std::cerr << "[GGUFLoader] ERROR: ParseHeader returned false" << std::endl;
            Close();
            return false;
        }
        std::cout << "[GGUFLoader] Header parsed successfully" << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "[GGUFLoader] EXCEPTION during ParseHeader: " << e.what() << std::endl;
        Close();
...
bool GGUFLoader::ParseHeader() {
    ...
    // DIAGNOSTIC: Log actual magic bytes read
    std::cout << "[GGUFLoader] Magic bytes: 0x" << std::hex << header_.magic << std::dec;
    std::cout << " (" << (char)(header_.magic & 0xFF) 
              << (char)((header_.magic >> 8) & 0xFF)
              << (char)((header_.magic >> 16) & 0xFF)
              << (char)((header_.magic >> 24) & 0xFF) << ")" << std::endl;
    
    if (header_.magic != 0x46554747) {  // "GGUF"
        std::stringstream ss;
        ss << "Invalid GGUF magic: 0x" << std::hex << header_.magic 
           << " (expected 0x46554747 = 'GGUF')";
    ...
    // DIAGNOSTIC: Log actual version read
    std::cout << "[GGUFLoader] GGUF version: " << header_.version << std::endl;
    
    if (header_.version != 3) {
        std::stringstream ss;
        ss << "Unsupported GGUF version: " << header_.version 
           << " (only version 3 supported - found in our GGML build)";
        throw std::runtime_error(ss.str());
    }
...
bool GGUFLoader::ParseMetadata() {
    std::cout << "[GGUFLoader] ParseMetadata() called" << std::endl;
    
    if (!file_.is_open()) {
        std::cerr << "[GGUFLoader] ERROR: Cannot parse metadata: file not open" << std::endl;
    ...
    if (header_.metadata_kv_count == 0) {
        ...
        std::cout << "[GGUFLoader] No metadata in file (metadata_kv_count=0)" << std::endl;
        return true;
    }
    
    std::cout << "[GGUFLoader] Parsing " << header_.metadata_kv_count << " metadata entries..." << std::endl;
    ...
    // Read tensor info
    std::cout << "[GGUFLoader] Parsing " << header_.tensor_count << " tensor definitions..." << std::endl;
    for (uint64_t i = 0; i < header_.tensor_count; ++i) {
        if (i % 100 == 0) {
            std::cout << "[GGUFLoader] Parsed " << i << "/" << header_.tensor_count << " tensors..." << std::endl;
        }
        
        TensorInfo tensor;
        
        if (!ReadString(tensor.name)) {
            std::cerr << "[GGUFLoader] ERROR: Failed to read tensor name at index " << i << std::endl;
            throw std::runtime_error("Failed to read tensor name at index " + std::to_string(i));
        }
        
        uint32_t n_dims;
        if (!ReadValue(n_dims)) {
            std::cerr << "[GGUFLoader] ERROR: Failed to read dimension count for tensor: " << tensor.name << std::endl;
        ...
        for (uint32_t d = 0; d < n_dims; ++d) {
            if (!ReadValue(tensor.shape[d])) {
                std::cerr << "[GGUFLoader] ERROR: Failed to read dimension " << d << " for tensor: " << tensor.name << std::endl;
        ...
        if (!ReadValue(type_val)) {
            std::cerr << "[GGUFLoader] ERROR: Failed to read type for tensor: " << tensor.name << std::endl;
        ...
        if (!ReadValue(tensor.offset)) {
            std::cerr << "[GGUFLoader] ERROR: Failed to read offset for tensor: " << tensor.name << std::endl;
    ...
    std::cout << "[GGUFLoader] All " << header_.tensor_count << " tensors parsed successfully" << std::endl;
