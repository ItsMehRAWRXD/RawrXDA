...
; Usage: Place CANARY_PROLOGUE *after* sub rsp (frame must cover [rbp-8]).
;        Place CANARY_EPILOGUE *before* restoring rsp / pop rbp.
; Both use rbp (not rsp) for XOR so the check value is frame-position-independent.
CANARY_PROLOGUE MACRO
    ;; Store canary at [rbp-8]: cookie XOR rbp
...
CANARY_EPILOGUE MACRO
    ;; Validate canary at [rbp-8]: must equal cookie XOR rbp
