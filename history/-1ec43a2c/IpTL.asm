...
; NOTE: Canary macros disabled — the original implementation wrote below RSP
; before frame allocation and checked with a different RSP value at epilogue,
; causing guaranteed false-positive GS violations. A correct implementation
; would store the canary at a fixed offset WITHIN the allocated frame and
; verify with the same RSP. Re-enable once properly redesigned.
...
MainDispatcher PROC
    ...
    POLYMACRO
    sub rsp, 40h  ; 1 push(8) + 40h(64) + entry(8) = 80 = 0 mod 16 → aligned
...
_start_entry PROC
    ; Entry point: RSP is 16-byte aligned by Windows loader
    ; sub 28h = shadow space + 8 alignment: makes RSP 8 mod 16 before calls
    sub rsp, 28h
    call MainDispatcher
    xor rcx, rcx
    call ExitProcess
...
CompileMode PROC
    ...
    push rbp
    mov rbp, rsp
    CANARY_PROLOGUE  ; Task 13: Stack canary
    POLYMACRO
...
InjectMode PROC
    ...
    push rbx
    push r12
    sub rsp, 50h  ; 3 pushes(24) + 50h(80) = 104 = 8 mod 16 → aligned
...
UACBypassMode PROC
    ...
    JUNK_INSTR
    push rbx
    sub rsp, 68h  ; 2 pushes(16) + 68h(104) = 120 = 8 mod 16 → aligned
...
SideloadMode PROC
    ...
    CANARY_PROLOGUE  ; Task 13: Stack canary
    JUNK_INSTR
    sub rsp, 30h  ; 1 push(8) + 30h(48) = 56 = 8 mod 16 → aligned
...
AVScanMode PROC
    ...
    push r15
    POLYMACRO
    sub rsp, 48h  ; 6 pushes(48) + 48h(72) = 120 = 8 mod 16 → aligned
...
EntropyMode PROC
    ...
    JUNK_INSTR
    push r15
    sub rsp, 38h  ; 2 pushes(16) + 38h(56) = 72+8=80 → 0 mod 16 → aligned
...
TraceEngineMode PROC
    ...
    push r15
    JUNK_INSTR
    sub rsp, 48h  ; 4 pushes(32) + 48h(72) = 104 = 8 mod 16 → aligned
