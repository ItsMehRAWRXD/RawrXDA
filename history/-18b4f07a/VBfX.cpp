...
void MetricsCollector::startRequest(qint64 requestId, const QString& modelName, int promptTokens) {
    // Professional License Gate — Metrics Collector requires Professional tier
    if (!LicenseEnforcementGate::getInstance().checkAccess(EnterpriseFeature::MetricsCollector, "startRequest")) {
        return;
    }
    if (!m_enabled) return;
