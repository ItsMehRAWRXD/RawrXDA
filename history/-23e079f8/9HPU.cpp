...
PatchResult LSPHotpatchBridge::refreshDiagnostics() {
    ...
    if (m_server) {
        ...
        m_server->writeNotification(NOTIFY_DIAGNOSTIC_REFRESH, notifParams);
    }
...
void LSPHotpatchBridge::onHotpatchEvent(const struct HotpatchEvent* event, void* userData) {
    ...
    if (event) {
        ...
        params["layerName"]  = event->layerName ? event->layerName : "";
        params["patchName"]  = event->patchName ? event->patchName : "";
        params["success"]    = event->success;
    ...
    bridge.m_server->writeNotification(NOTIFY_HOTPATCH_EVENT, params);
}
...
void LSPHotpatchBridge::handleHotpatchApply(int /*id*/, const json& params) {
    ...
    if (layer == HotpatchLayer::Memory) {
        ...
        if (params.contains("filename") && params.contains("offset") && params.contains("data")) {
            ...
            if (!bytes.empty()) {
                pr = direct_write(filename.c_str(), offset, bytes.size(), bytes.data());
            } else {
                pr = PatchResult::error("Empty patch data");
            }
        } else {
...
void LSPHotpatchBridge::handleHotpatchRevert(int /*id*/, const json& params) {
    ...
    if (layer == HotpatchLayer::Memory) {
        // Memory revert: requires the patch address (looks up original bytes)
        if (params.contains("address")) {
            ...
            pr = revert_memory_patch(
                reinterpret_cast<void*>(static_cast<uintptr_t>(addr)));
        } else {
            pr = PatchResult::error("Memory revert requires 'address' param");
        }
    } else if (layer == HotpatchLayer::Byte) {
