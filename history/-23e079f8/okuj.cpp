...
void LSPHotpatchBridge::handleHotpatchRevert(int /*id*/, const json& params) {
    ...
    if (layer == HotpatchLayer::Memory) {
        // Memory revert: tracked patch lookup not available
        // (LSPHotpatchBridge has no m_trackedPatches member)
        pr = PatchResult::error(
            "Memory patch revert requires tracked patch data. "
            "This bridge does not currently track applied patches.");
    } else if (layer == HotpatchLayer::Byte) {
