    ...
    if (layer == HotpatchLayer::Memory) {
        ...
        if (params.contains("address") && params.contains("data")) {
            ...
            if (!bytes.empty()) {
                ...
                tracked.address = reinterpret_cast<void*>(static_cast<uintptr_t>(addr));
                tracked.originalBytes = std::move(originalBytes);
                tracked.name = name;
        ...
        if (params.contains("filename") && params.contains("offset") && params.contains("data")) {
            ...
            if (!bytes.empty()) {
                 ...
                 pr = UnifiedHotpatchManager::instance().apply_byte_patch(filename.c_str(), bp);
            } else {
                pr = PatchResult::error("Empty patch data");
            }
        } else {
...
void LSPHotpatchBridge::handleHotpatchRevert(int /*id*/, const json& params) {
    ...
    if (layer == HotpatchLayer::Memory) {
        // Memory revert: look up tracked patch entry by name
        auto it = m_trackedPatches.find(name);
        if (it != m_trackedPatches.end()) {
            // Restore original bytes at the patched address
            pr = UnifiedHotpatchManager::instance().apply_memory_patch(
                it->second.address,
                it->second.originalBytes.size(),
                it->second.originalBytes.data()
            );
            if (pr.success) {
                m_trackedPatches.erase(it);
                pr = PatchResult::ok("Memory patch reverted by reapplying original bytes.");
            } else {
                pr = PatchResult::error("Failed to reapply original bytes during revert.");
            }
        } else {
            pr = PatchResult::error(
                "No tracked memory patch named '" + name + "'. "
                "Memory patches must be applied via this bridge to be revertible.");
        }
    } else if (layer == HotpatchLayer::Byte) {
