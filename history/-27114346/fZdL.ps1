...
while ($attempt -le $maxAttempts) {
    ...
    $batchContent = "@echo off`r`n`"$Linker`" $($linkArgs -join ' ') 1>`"$stdoutFile`" 2>`"$stderrFile`"`r`nexit /b %errorlevel%"
    Set-Content -Path $batchFile -Value $batchContent -Encoding ASCII
    
    $cmdProc = Start-Process -FilePath "cmd.exe" -ArgumentList "/c", "`"$batchFile`"" -NoNewWindow -Wait -PassThru
