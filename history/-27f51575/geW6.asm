...
@write_content:
    ; Store content size before calling WriteFile (which modifies EAX)
    mov ecx, g_FileState.dwBytesRead
    mov dwContentSize, ecx
    
    ; Get buffer pointer (avoiding EAX which gets clobbered)
    mov esi, g_FileState.pBuffer
    invoke WriteFile, hFile, esi, dwContentSize, addr dwBytesWritten, NULL
    test eax, eax
    jz @write_fail
