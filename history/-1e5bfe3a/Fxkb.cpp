...
void MainWindow::initializeAgenticSystem()
{
    ...
    if (m_failureDetector && m_selfCorrector) {
        connect(m_failureDetector, &AgenticFailureDetector::failureDetected,
                this, [this](FailureType type, const QString& description, double confidence) {
...
void MainWindow::onAgenticModeChanged(int mode)
{
    ...
    if (m_hexMagConsole) {
        m_hexMagConsole->appendPlainText(QString("Switched to: %1").arg(modeName));
    }
...
void MainWindow::handleAskMode(const QString& question)
{
    ...
    if (m_hexMagConsole) {
        m_hexMagConsole->appendPlainText(QString("Ask Mode Query: %1").arg(question));
    }
...
void MainWindow::handlePlanMode(const QString& task)
{
    ...
    if (m_hexMagConsole) {
        m_hexMagConsole->appendPlainText(QString("Plan Mode Task: %1").arg(task));
    }
...
void MainWindow::handleAgentMode(const QString& goal)
{
    ...
    if (m_hexMagConsole) {
        m_hexMagConsole->appendPlainText(QString("Agent Mode Goal: %1").arg(goal));
    }
...
void MainWindow::onModelSelectionChanged(const QString& modelPath)
{
    ...
    if (m_hexMagConsole) {
        m_hexMagConsole->appendPlainText(QString("Model selected: %1").arg(modelPath));
    }
...
void MainWindow::onUnloadModel()
{
    ...
    if (m_hexMagConsole) {
        m_hexMagConsole->appendPlainText("Model unloaded");
    }
