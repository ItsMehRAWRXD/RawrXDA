...
Vram_SubmitUpload PROC FRAME
    ...
    mov dword ptr [rbx + GpuTransferOp.OperationType], 0 ; UPLOAD
    mov rcx, rcx ; Dummy to match length if needed? No
    mov qword ptr [rbx + GpuTransferOp.SrcHostPtr], rcx
    mov qword ptr [rbx + GpuTransferOp.DstVramOffset], rdx
    mov qword ptr [rbx + GpuTransferOp.OperationSize], r8
    ...
    xor r9, r9
    call CreateEventA
    mov qword ptr [rbx + GpuTransferOp.CompletionEvent], rax
    
    ; Submit to GPU driver
    mov rcx, qword ptr [rbx + GpuTransferOp.SrcHostPtr]
    mov rdx, qword ptr [rbx + GpuTransferOp.DstVramOffset]
    mov r8, qword ptr [rbx + GpuTransferOp.OperationSize]
    call qword ptr [pfnGpuSubmitTransfer]
    mov qword ptr [rbx + GpuTransferOp.FenceValue], rax
    
    ; Advance queue tail
    inc [rsi].VramHeap.QueueTail
    and dword ptr [rsi].VramHeap.QueueTail, VRAM_TRANSFER_QUEUE_DEPTH - 1
    
    mov rax, qword ptr [rbx + GpuTransferOp.FenceValue]    add rsp, 40
    pop rsi
    pop rbx
    ret
...
@compact_loop:
    ...
    mov dword ptr [r15 + GpuTransferOp.OperationType], 2 ; COPY
    ...
    mov qword ptr [r15 + GpuTransferOp.OperationSize], r8
