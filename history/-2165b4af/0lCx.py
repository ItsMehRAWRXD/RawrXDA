...
def _generate_code_explanation(text: str, fname: str, lang: str) -> str:
    ...
    # Find function definitions (C/C++) - only scan first 50KB to avoid regex issues
    if lang in ("cpp", "c", "c_cpp"):
        sample = text[:50000]
        # Simple line-by-line scan instead of regex over whole file
        funcs = []
        for line in sample.split('\n'):
            line = line.strip()
            if line.endswith('{') and '(' in line and not line.startswith('//') and not line.startswith('#'):
                # Extract function name before the opening paren
                paren_idx = line.index('(')
                before_paren = line[:paren_idx].strip().split()
                if before_paren:
                    name = before_paren[-1].lstrip('*&')
                    if name and len(name) > 1 and name not in ('if', 'for', 'while', 'switch', 'catch'):
                        funcs.append(name)
        if funcs:
