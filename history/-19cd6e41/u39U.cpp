...
void cmd_model(const std::string& args) {
    ...
    if (sub == "load" || sub == "model_load") {
        ...
        if (ok) {
            auto info = hotpatcher.getModelInfo();
            std::cout << "✅ Model analyzed successfully\n";
            std::cout << "  File: " << rest << "\n";
            std::cout << "  Layers: " << info.totalLayers << "\n";
            std::cout << "  Parameters: " << (info.totalParams / 1e9) << "B\n";
            std::cout << "  Total size: " << (info.totalSizeBytes / (1024ULL * 1024ULL)) << " MB\n";
            std::cout << "  Current quant: " << info.currentQuant << "\n";

            cli_record_action(
                static_cast<int>(ReplayActionType::ToolCall),
                "model", "analyze", rest,
                "Analyzed: " + std::to_string(info.totalLayers) + " layers, " +
                std::to_string(info.totalParams) + " params",
                0, 1.0f, 0.0);
        } else {
        ...
        std::cout << "  Target layers: " << plan.layers.size() << "\n";
        std::cout << "  Estimated VRAM savings: " << (plan.estimatedSavingsBytes / (1024ULL * 1024ULL)) << " MB\n";
        std::cout << "  Estimated quality loss: " << std::fixed << std::setprecision(2)
                  << (plan.estimatedQualityLoss * 100.0f) << "%\n";
        std::cout << "\n  Plan JSON:\n" << hotpatcher.quantPlanToJson() << "\n";
        ...
        hotpatcher.setSurgeryProgressCallback(
            [](SurgeryOp op, uint32_t layerIdx, uint32_t totalLayers,
               float progress, void* /*userData*/) {
                ...
                switch (op) {
                    case SurgeryOp::Requantize: opStr = "Requantize"; break;
                    case SurgeryOp::Evict:      opStr = "Evict";      break;
                    case SurgeryOp::Reload:     opStr = "Reload";     break;
                    case SurgeryOp::Split:      opStr = "Split";      break;
                    case SurgeryOp::Merge:      opStr = "Merge";      break;
                }
        ...
        bool ok = hotpatcher.applyQuantPlan(plan);
        double elapsed = nowEpochMs() - startMs;

        std::cout << "\n";
        if (ok) {
            std::cout << "✅ Model surgery completed in " << formatDuration(elapsed) << "\n";
            std::cout << "  Layers modified: " << plan.layers.size() << "\n";
            std::cout << "  VRAM saved: " << (plan.estimatedSavingsBytes / (1024ULL * 1024ULL)) << " MB\n";

            cli_record_action(
                static_cast<int>(ReplayActionType::ToolCall),
                "model", "surgery", "",
                "Surgery completed: " + std::to_string(plan.layers.size()) + " layers",
                0, 1.0f, elapsed);
        } else {
            std::cout << "❌ Model surgery failed\n";
        }
        ...
        std::cout << "    Pressure:      " << static_cast<int>(budget.pressure)
                  << " (" << (budget.pressure == VRAMPressure::None ? "None" :
                              budget.pressure == VRAMPressure::Low ? "Low" :
                              budget.pressure == VRAMPressure::Medium ? "Medium" :
                              budget.pressure == VRAMPressure::High ? "High" : "Critical") << ")\n";
        ...
        bool ok = hotpatcher.evictLayer(idx);
        std::cout << (ok ? "✅ Layer evicted" : "❌ Failed to evict layer") << "\n";

    } else if (sub == "reload") {
        if (rest.empty()) { std::cout << "Usage: !model reload <layer_index>\n"; return; }
        uint32_t idx = static_cast<uint32_t>(parseInt(rest, -1));
        bool ok = hotpatcher.reloadLayer(idx);
        std::cout << (ok ? "✅ Layer reloaded" : "❌ Failed to reload layer") << "\n";
