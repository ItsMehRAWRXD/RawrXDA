...
void AIDigestionPanel::updateProgressDisplays() {
    ...
    if (m_digestionWorker && m_digestionWorker->isRunning()) {
        auto progress = m_digestionWorker->currentProgress();
        m_digestionProgressBar->setValue(static_cast<int>(progress.percentage));
        m_digestionStatusLabel->setText(progress.status);
    }
    
    // Update training progress
    if (m_trainingWorker && m_trainingWorker->isRunning()) {
        auto progress = m_trainingWorker->currentProgress();
        m_trainingProgressBar->setValue(static_cast<int>(progress.percentage));
        m_trainingStatusLabel->setText(progress.status);
    }
}

void AIDigestionPanel::updateParameterWidgets() {
    // Update model size-dependent parameters
    QString sizeText = m_modelSizeCombo->currentText();
    if (sizeText.contains("3GB")) {
        m_batchSizeSpin->setValue(8);
        m_maxTokensSpin->setValue(2048);
    } else if (sizeText.contains("7GB")) {
        m_batchSizeSpin->setValue(4);
        m_maxTokensSpin->setValue(4096);
    } else if (sizeText.contains("13GB")) {
        m_batchSizeSpin->setValue(2);
        m_maxTokensSpin->setValue(8192);
    } else if (sizeText.contains("30GB")) {
...
void AIDigestionPanel::updateModelList() {
    ...
    QDir outputDir(m_outputDirectoryEdit->text());
    if (!outputDir.exists()) return;
    
    QStringList filters;
    filters << "*.gguf";
    QFileInfoList modelFiles = outputDir.entryInfoList(filters, QDir::Files, QDir::Time);
    
    for (const QFileInfo& fileInfo : modelFiles) {
        int row = m_modelsTable->rowCount();
        m_modelsTable->insertRow(row);
        
        m_modelsTable->setItem(row, 0, new QTableWidgetItem(fileInfo.baseName()));
        m_modelsTable->setItem(row, 1, new QTableWidgetItem(formatFileSize(fileInfo.size())));
        m_modelsTable->setItem(row, 2, new QTableWidgetItem(fileInfo.lastModified().toString("yyyy-MM-dd hh:mm")));
        m_modelsTable->setItem(row, 3, new QTableWidgetItem("Ready"));
        m_modelsTable->setItem(row, 4, new QTableWidgetItem(fileInfo.absoluteFilePath()));
    }
}
...
void AIDigestionPanel::applyModelPreset(const QString& presetName) {
    ...
    if (config.contains("modelSize")) {
        m_modelSizeCombo->setCurrentText(config["modelSize"].toString());
    }
    if (config.contains("quantization")) {
        m_quantizationCombo->setCurrentText(config["quantization"].toString());
    }
    if (config.contains("maxTokens")) {
        m_maxTokensSpin->setValue(config["maxTokens"].toInt());
    }
    if (config.contains("chunkSize")) {
        m_chunkSizeSpin->setValue(config["chunkSize"].toInt());
    }
...
// Digestion worker event handlers
void AIDigestionPanel::onDigestionProgress(double progress) {
    m_digestionProgressBar->setValue(static_cast<int>(progress * 100));
}

void AIDigestionPanel::onDigestionStatusChanged(const QString& status) {
    m_digestionStatusLabel->setText(status);
    logMessage(QString("Digestion: %1").arg(status));
}

void AIDigestionPanel::onFileProcessed(const QString& filePath, int processedCount, int totalCount) {
    QString fileName = QFileInfo(filePath).fileName();
    logMessage(QString("Processed [%1/%2]: %3").arg(processedCount).arg(totalCount).arg(fileName));
    
    double progress = totalCount > 0 ? static_cast<double>(processedCount) / totalCount : 0.0;
    m_digestionProgressBar->setValue(static_cast<int>(progress * 100));
}
...
void AIDigestionPanel::onDigestionDatasetReady(const TrainingDataset& dataset) {
    ...
    logMessage(QString("Dataset ready: %1 samples, %2 tokens")
        .arg(dataset.totalSamples)
        .arg(dataset.totalTokens));
}
...
// Training worker event handlers
void AIDigestionPanel::onTrainingProgress(double progress, const QJsonObject& metrics) {
    m_trainingProgressBar->setValue(static_cast<int>(progress * 100));
    
    if (metrics.contains("loss")) {
        logMessage(QString("Training progress: %1% (loss: %2)")
            .arg(progress * 100, 0, 'f', 1)
            .arg(metrics["loss"].toDouble(), 0, 'f', 4));
    }
}

void AIDigestionPanel::onTrainingStatusChanged(const QString& status) {
    m_trainingStatusLabel->setText(status);
    logMessage(QString("Training: %1").arg(status));
}
...
void AIDigestionPanel::onTrainingFinished(const QString& modelPath) {
    ...
    logMessage(QString("Training completed - Model saved: %1").arg(modelPath));
    showSuccessMessage("Training Complete", QString("Model saved to:\n%1").arg(modelPath));
    
    updateModelList();
    emit trainingCompleted(modelPath);
}

void AIDigestionPanel::onTrainingErrorWorker(const QString& error) {
