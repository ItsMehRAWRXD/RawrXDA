...
void InteractiveShell::ProcessCommand(const std::string& input) {
    ...
    if (trimmed == "/status") {
        ...
        status += "  Model: " + std::string(CPUInference::CPUInferenceEngine::getInstance()->isModelLoaded() ? "Loaded" : "Not loaded") + "\n";
...
void InteractiveShell::ProcessAgenticCommand(const std::string& input) {
    ...
    if (input.rfind("/plan ", 0) == 0) {
        ...
        auto result = CPUInference::CPUInferenceEngine::getInstance()->generate(prompt, 0.7f, 0.9f, 2048);
        if (result.has_value()) {
...
void InteractiveShell::ProcessRegularInput(const std::string& input) {
    ...
    // Generate response (using default parameters for now)
