...
void AICodeAssistant::onNetworkReply()
{
    ...
    for (const QString &line : lines) {
        ...
        if (doc.isObject()) {
            ...
            if (obj.contains("response")) {
                ...
                emit suggestionStreaming(chunk);
            }
        }
    }
    ...
    if (!fullResponse.isEmpty()) {
        ...
        AICodeAssistant::CodeSuggestion enriched;
        enriched.suggested_code = suggestion;
        enriched.type = m_currentSuggestionType;
        enriched.context = QString();
        enriched.original_code = QString();
        enriched.explanation = QString();
        enriched.confidence = 1.0f;
        enriched.latency_ms = m_timer.elapsed();
        enriched.timestamp = QDateTime::currentDateTime();
        emit suggestionReady(enriched);
