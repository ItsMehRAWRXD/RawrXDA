...
RegistryManager::RegistryManager() : m_hSCManager(nullptr) {
}
...
bool ServiceManager::StartService(const std::wstring& serviceName, const std::vector<std::wstring>& args) {
    ...
    if (hService) {
        ...
        bool result = StartServiceW(hService, args.size(), lpServiceArgVectors) == TRUE;
...
bool ServiceManager::DeleteService(const std::wstring& serviceName) {
    ...
    if (hService) {
        bool result = DeleteServiceW(hService) == TRUE;
        CloseServiceHandle(hService);
        return result;
    }
    return false;
...
std::vector<WindowInfo> WindowManager::EnumerateWindows(bool visibleOnly) {
    ...
    std::vector<WindowInfo>* pWindows = &windows;
    
    EnumWindowsW([](HWND hwnd, LPARAM lParam) -> BOOL {
        ...
        pWindowList->push_back(info);
        return TRUE;
    }, reinterpret_cast<LPARAM>(pWindows));
