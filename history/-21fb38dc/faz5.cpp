...
void SettingsManager::setValue(const QString& key, const QVariant& value, bool saveImmediately) {
    ...
    // Set the value
    QString lastKey = parts.last();
    current[lastKey] = QJsonValue::fromVariant(value);
    
    // Write back the changes to m_settings
    QJsonObject* obj = &m_settings;
    for (int i = 0; i < parts.size() - 1; ++i) {
        QJsonObject nested = (*obj)[parts[i]].toObject();
        if (i == parts.size() - 2) {
            nested[lastKey] = QJsonValue::fromVariant(value);
        }
        (*obj)[parts[i]] = nested;
        obj = &m_settings;
        for (int j = 0; j <= i; ++j) {
            if (j == i) {
                QJsonValueRef ref = (*obj)[parts[j]];
                QJsonObject temp = ref.toObject();
                obj = &m_settings;
                for (int k = 0; k < j; ++k) {
                    // Navigate down
                }
    ...
    // Simpler approach: rebuild from root
    QJsonObject root = m_settings;
    QJsonObject* ptr = &root;
    QVector<QJsonObject> hierarchy;
    hierarchy.reserve(parts.size());
    
    for (int i = 0; i < parts.size() - 1; ++i) {
        if (!ptr->contains(parts[i]) || !(*ptr)[parts[i]].isObject()) {
            ptr->insert(parts[i], QJsonObject());
        }
        QJsonObject nested = (*ptr)[parts[i]].toObject();
        hierarchy.append(nested);
        ptr = &hierarchy.last();
    }
    
    (*ptr)[lastKey] = QJsonValue::fromVariant(value);
    
    // Write back up the hierarchy
    for (int i = hierarchy.size() - 1; i >= 0; --i) {
        if (i == 0) {
            root[parts[i]] = hierarchy[i];
        } else {
            hierarchy[i - 1][parts[i]] = hierarchy[i];
        }
    }
