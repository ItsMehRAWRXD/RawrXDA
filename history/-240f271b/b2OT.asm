...
PATCH_RESULT struct
    ...
    error_code      dword 0
    detail_len      dword 0
    detail          qword 0              ; pointer to error message
PATCH_RESULT ends

; Memory patch structure
MEMORY_PATCH struct
    target_address  qword 0              ; Address to patch
    source_data     qword 0              ; New data
    patch_size      qword 0              ; Bytes to replace
    original_data   qword 0              ; Backup of original
    metadata        qword 0              ; Custom metadata
MEMORY_PATCH ends
...
PROT_CONTEXT struct
    address         qword 0
    size            qword 0
    original_protect dword 0
    temp_protect    dword 0
PROT_CONTEXT ends
...
MemoryHotpatch_ProtectMemory proc
    ...
    ; Fill context
    mov [rdi + PROT_CONTEXT.address], rsi
    mov [rdi + PROT_CONTEXT.size], r9
    mov eax, [old_protect]
    mov [rdi + PROT_CONTEXT.original_protect], eax
...
skip_verify_after:
    ...
    mov rcx, r14
    mov rdx, [rcx + PROT_CONTEXT.original_protect]
    mov rcx, [rcx + PROT_CONTEXT.address]
    mov r8, [rcx + PROT_CONTEXT.size]
    mov r9d, edx
    call VirtualProtect
    test eax, eax
    jz patch_prot_fail
