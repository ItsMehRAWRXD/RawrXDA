...
CommandResult handleUnrealAttach(const CommandContext& ctx) {
    ...
    ctx.output("[UNREAL] Attaching to process ");
    ctx.output(std::to_string(pid).c_str());
    ctx.output("...\n");
...
CommandResult handleUnityAttach(const CommandContext& ctx) {
    ...
    ctx.output("[UNITY] Attaching to process ");
    ctx.output(pidStr.c_str());
    ctx.output("...\n");
...
CommandResult handleModelUnload(const CommandContext& ctx) {
    ...
    // Signal server to release model memory
    auto& hpm = UnifiedHotpatchManager::instance();
    auto stats = hpm.getStats();
    ctx.output("  Releasing model memory...\n");
    // Force GC of model tensors
    MEMORYSTATUSEX mem = {};
...
CommandResult handleGovernorSetPowerLevel(const CommandContext& ctx) {
    ...
    ctx.output("[GOVERNOR] Setting power level: ");
    ctx.outputLine(level);
...
CommandResult handleEmbeddingEncode(const CommandContext& ctx) {
    ...
    // Generate a deterministic hash-based embedding vector (128-dim)
    // Real implementation would call model inference endpoint
    const int dims = 128;
    ctx.output("  Dimensions: 128\n  Vector: [");
    uint32_t hash = 0x811c9dc5; // FNV-1a init
    std::ostringstream oss;
    for (int i = 0; i < dims; i++) {
        // FNV-1a hash with dimension salt
        uint32_t h = hash;
        for (char c : text) { h ^= static_cast<uint8_t>(c); h *= 0x01000193; }
        h ^= i;
        h *= 0x01000193;
        float val = static_cast<float>(static_cast<int32_t>(h)) / static_cast<float>(INT32_MAX);
        if (i > 0) oss << ", ";
        if (i < 8 || i >= dims - 2) oss << std::fixed << std::setprecision(4) << val;
        else if (i == 8) oss << "...";
        hash = h;
    }
    oss << "]\n";
...
CommandResult handleVisionAnalyzeImage(const CommandContext& ctx) {
    ...
    ctx.outputLine(path);
    // Read image header to detect format and dimensions
    FILE* f = fopen(path.c_str(), "rb");
    if (!f) {
        ctx.output("  File not found.\n");
        return CommandResult::error("vision: file not found");
    }
    ...
    if (hdr[0] == 0x89 && hdr[1] == 'P' && hdr[2] == 'N' && hdr[3] == 'G') {
        ...
        uint32_t h = (hdr[20]<<24)|(hdr[21]<<16)|(hdr[22]<<8)|hdr[23];
        oss << "  Format:    PNG\n"
            << "  Size:      " << w << "x" << h << "\n"
            << "  Bit depth: " << (int)hdr[24] << "\n";
    } else if (hdr[0] == 0xFF && hdr[1] == 0xD8) {
        oss << "  Format:    JPEG\n";
    } else if (hdr[0] == 'B' && hdr[1] == 'M') {
        uint32_t w = *(uint32_t*)(hdr+18);
        uint32_t h = *(uint32_t*)(hdr+22);
        oss << "  Format:    BMP\n"
            << "  Size:      " << w << "x" << h << "\n";
    } else {
        oss << "  Format:    Unknown (magic: 0x" << std::hex << (int)hdr[0] << (int)hdr[1] << std::dec << ")\n";
    }
    oss << "  Note: Full vision analysis requires model inference endpoint.\n";
    ctx.output(oss.str().c_str());
