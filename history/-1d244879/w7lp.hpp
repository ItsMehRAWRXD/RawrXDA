...
class InferenceEngine : public QObject {
    ...
    // New property for Cloud integration
    Q_PROPERTY(bool useCloud READ useCloud WRITE setUseCloud NOTIFY cloudUsageChanged)
...
public slots:
    ...
    void setUseCloud(bool use) { 
        if (m_useCloud != use) {
            m_useCloud = use;
            emit cloudUsageChanged(use);
        }
    }
    bool useCloud() const { return m_useCloud; }

    CloudApiClient* cloudClient() const { return m_cloudClient; }
...
signals:
    ...
    void cloudUsageChanged(bool useCloud);
