...
// Guard against broken Windows SDK configurations
#if __has_include(<d3d11.h>) && !defined(NO_D3D10_EFFECT_COMPAT)
...
// ------------------------------------------------------------------------------
// Simple local descriptor to avoid SDK dependencies
...
public:
    SimpleTypeDesc typeDesc;
    HRESULT GetDesc(SimpleTypeDesc* out) const { if (out) *out = typeDesc; return S_OK; }
    static D3D10EffectType* make(const SimpleTypeDesc& d) { auto p = new D3D10EffectType; p->typeDesc = d; return p; }
};
...
//  Pass / Technique
// ------------------------------------------------------------------------------
// Stub for constant buffer (not fully implemented)
class D3D10EffectConstantBuffer : public IUnknownImpl {
public:
    std::string name;
    ID3D11Buffer* buffer = nullptr;
    ~D3D10EffectConstantBuffer() { if (buffer) buffer->Release(); }
};
...
inline HRESULT D3DX10CreateEffectFromMemory(
    const void* data, SIZE_T dataSize, const char*, const void*, const void*, DWORD, DWORD,
    D3D10Effect** effect, void** errors)
{
...
#else
// D3D10 Effect compatibility layer disabled - no-op stubs
typedef void* ID3D10Effect;
inline HRESULT D3D10CompileEffectFromMemory(const void*, SIZE_T, const char*, const void*, const void*, UINT, UINT, ID3D10Effect**, void**) { return E_NOTIMPL; }
#endif // !NO_D3D10_EFFECT_COMPAT
