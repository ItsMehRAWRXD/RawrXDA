...
void RawrXDModelLoader::UploadViaStaging(void* data, size_t size, VkBuffer dstBuffer) {
    ...
    // Copy command (Simulated immediate submission via hypothetical one-shot-helper)
    // In real code: BeginCommandBuffer -> CmdCopyBuffer -> End -> Submit -> Wait
    // Implementation omitted for brevity as per reverse-engineering constraints
    // Assuming command pool exists in context
