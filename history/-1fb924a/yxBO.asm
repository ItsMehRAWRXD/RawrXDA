...
TerminalExecParams STRUCT
    dwProfile       dd ?    ; 1=cmd, 2=cmd(admin), 3=pwsh, 4=pwsh(admin)
    lpCommand       dd ?    ; LPSTR
    pfnOnChunk      dd ?    ; optional callback: stdcall (lpChunk, cbChunk)
TerminalExecParams ENDS

CloudDownloadParams STRUCT
    lpUrl           dd ?
    lpOutPath       dd ?
    lpBearerToken   dd ?
CloudDownloadParams ENDS

ModelActionParams STRUCT
    dwSource        dd ?    ; MODEL_SOURCE_*
    lpModelOrUrl    dd ?    ; model name (ollama) or URL (hf/azure/custom)
    lpArg           dd ?    ; prompt (infer) or output path (download)
    lpToken         dd ?    ; bearer token (hf/azure)
ModelActionParams ENDS
...
.data
    ...
    ; Terminal profiles
    TERMINAL_PROFILE_CMD          dd 1
    TERMINAL_PROFILE_CMD_ADMIN    dd 2
    TERMINAL_PROFILE_PWSH         dd 3
    TERMINAL_PROFILE_PWSH_ADMIN   dd 4

    szCmdPrefix   db "cmd.exe /c ", 0
    szPwshPrefix  db "pwsh.exe -NoProfile -NonInteractive -Command ", 0
    szQuote       db 34, 0
    szAdminNotCaptured db "[admin] command launched (output not captured)", 0
    szTruncated   db "\r\n...(truncated)...\r\n", 0

    ; Elevated launcher helpers
    szRunAsVerb   db "runas", 0
    szCmdExe      db "cmd.exe", 0
    szTempPrefix  db "RXD", 0
    szCmdSlashC   db "/c ", 0
    szRedirectPre db " 1> ", 34, 0
    szRedirectPost db 34, " 2>&1", 0

    ; Model/CLI strings
    szUnsupportedModelSource db "Unsupported model source", 0
    szOllamaPullCmd db "ollama pull ", 0
    szOllamaListCmd db "ollama list", 0
    szOllamaRunCmd db "ollama run ", 0
    szSpaceA db " ", 0
...
AgenticIDE_LoadSettings PROC lpProfileName:DWORD
    ...
    invoke ReadFile, hFile, addr g_ToolConfigs, edi, addr bytesRead, NULL
    test eax, eax
    jz @CloseFile
...
AgenticIDE_ExecuteTool PROC dwToolID:DWORD, lpParams:DWORD, pResult:DWORD
    ...
    ; Terminal exec locals (kept here because MASM locals must be declared at PROC scope)
    LOCAL hRead:DWORD
    LOCAL hWrite:DWORD
    LOCAL sa:SECURITY_ATTRIBUTES
    LOCAL stSI:STARTUPINFOA
    LOCAL stPI:PROCESS_INFORMATION
    LOCAL bytesRead:DWORD
    LOCAL outBuf:DWORD
    LOCAL outLen:DWORD
    LOCAL maxOut:DWORD
    LOCAL chunk[4096]:BYTE
...
@DispatchTool:
    ...
    je @Tool_WebNavigate
    cmp eax, TOOL_MODEL_LOAD
    je @Tool_ModelLoad
    cmp eax, TOOL_MODEL_SWAP
    je @Tool_ModelSwap
    cmp eax, TOOL_MODEL_LIST
    je @Tool_ModelList
    cmp eax, TOOL_MODEL_INFER
    je @Tool_ModelInfer
    ...
    cmp eax, TOOL_CLOUD_DOWNLOAD
    je @Tool_CloudDownload
    ...
    je @Tool_SetTrace
    cmp eax, TOOL_TERMINAL_EXEC
    je @Tool_TerminalExec
...
@Tool_ModelLoad:
    push esi
    push lpParams
    call AgenticIDE_Tool_ModelLoad
    add esp, 8
    test eax, eax
    jz @ToolFailed
    jmp @ToolSuccess
...
@Tool_ModelList:
    push esi
    push lpParams
    call AgenticIDE_Tool_ModelList
    add esp, 8
    test eax, eax
    jz @ToolFailed
    jmp @ToolSuccess
...
@Tool_ModelInfer:
    push esi
    push lpParams
    call AgenticIDE_Tool_ModelInfer
    add esp, 8
    test eax, eax
    jz @ToolFailed
    jmp @ToolSuccess
...
@Tool_TerminalExec:
    ; lpParams -> TerminalExecParams
    mov outBuf, 0
    mov outLen, 0
    mov maxOut, 1048576  ; 1MB cap

    ; Build command line based on profile
    mov ebx, lpParams
    mov eax, [ebx].TerminalExecParams.dwProfile
    cmp eax, 3
    je @BuildPwsh
    cmp eax, 1
    je @BuildCmd
...
@BuildCmd:
    invoke lstrcpyA, addr cmdLine, addr szCmdPrefix
    mov eax, [ebx].TerminalExecParams.lpCommand
    invoke lstrcatA, addr cmdLine, eax
    jmp @SpawnCapture

@BuildPwsh:
    invoke lstrcpyA, addr cmdLine, addr szPwshPrefix
    invoke lstrcatA, addr cmdLine, addr szQuote
    mov eax, [ebx].TerminalExecParams.lpCommand
    invoke lstrcatA, addr cmdLine, eax
    invoke lstrcatA, addr cmdLine, addr szQuote
    jmp @SpawnCapture
...
@AdminLaunch:
    push esi
    push lpParams
    call AgenticIDE_TerminalExec_Admin
    add esp, 8
    test eax, eax
    jz @ToolFailed
    jmp @ToolSuccess
...
@SpawnCapture:
    mov edx, [ebx].TerminalExecParams.pfnOnChunk
    push esi
    push edx
    lea eax, cmdLine
    push eax
    call AgenticIDE_RunProcessCapture
    add esp, 12
    test eax, eax
    jz @ToolFailed
    jmp @ToolSuccess
...
; ============================================================================
; AgenticIDE_RunProcessCapture - Run command line (non-elevated) and capture stdout/stderr
; Input: lpCmdLine (LPSTR), pfnOnChunk (optional), pResult (ToolResult*)
; Output: eax = 1 success (pResult->lpResultData points to GlobalAlloc buffer)
; ============================================================================
AgenticIDE_RunProcessCapture PROC lpCmdLine:DWORD, pfnOnChunk:DWORD, pResult:DWORD
    LOCAL hRead:DWORD
    LOCAL hWrite:DWORD
    LOCAL sa:SECURITY_ATTRIBUTES
    LOCAL stSI:STARTUPINFOA
    ...
    invoke GlobalAlloc, GMEM_FIXED or GMEM_ZEROINIT, maxOut
    mov outBuf, eax
    test eax, eax
    jz @Fail
    ...
    xor eax, eax
    rep stosb
    mov stSI.cb, SIZEOF STARTUPINFOA
    mov stSI.dwFlags, STARTF_USESTDHANDLES or STARTF_USESHOWWINDOW
    mov stSI.wShowWindow, SW_HIDE
    mov eax, hWrite
    mov stSI.hStdOutput, eax
    mov stSI.hStdError, eax
    mov stSI.hStdInput, NULL
    ...
    invoke CreateProcessA, NULL, lpCmdLine, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, addr stSI, addr stPI
    test eax, eax
    jz @Fail
...
@ReadLoop:
    invoke ReadFile, hRead, addr chunk, 4096, addr bytesRead, NULL
    test eax, eax
    jz @ReadDone
    mov eax, bytesRead
    test eax, eax
...
@ReadDone:
    ...
    mov esi, pResult
    mov eax, outBuf
    mov [esi].ToolResult.lpResultData, eax
    mov eax, outLen
    mov [esi].ToolResult.cbResultSize, eax
    mov eax, 1
    ret
...
@FailRet:
    xor eax, eax
    ret
AgenticIDE_RunProcessCapture ENDP

; ============================================================================
; AgenticIDE_Tool_ModelLoad
; - OLLAMA: runs `ollama pull <model>` and returns output
; - HUGGINGFACE/CUSTOM URL: downloads to lpArg using bearer lpToken
; Input: lpParams -> ModelActionParams, pResult -> ToolResult
; ============================================================================
AgenticIDE_Tool_ModelLoad PROC lpParams:DWORD, pResult:DWORD
    LOCAL cmdLine[1024]:BYTE
    mov ebx, lpParams

    mov eax, [ebx].ModelActionParams.dwSource
    cmp eax, MODEL_SOURCE_OLLAMA
    je @OllamaPull
    cmp eax, MODEL_SOURCE_HUGGINGFACE
    je @UrlDownload
    cmp eax, MODEL_SOURCE_CUSTOM
    je @UrlDownload

    ; unsupported
    mov esi, pResult
    lea eax, [esi].ToolResult.szErrorMsg
    invoke lstrcpyA, eax, addr szUnsupportedModelSource
    xor eax, eax
    ret

@UrlDownload:
    ; lpModelOrUrl=url, lpArg=outPath, lpToken=bearer
    push [ebx].ModelActionParams.lpToken
    push [ebx].ModelActionParams.lpArg
    push [ebx].ModelActionParams.lpModelOrUrl
    call WinHTTP_DownloadToFileBearerA
    add esp, 12
    ret
...
@OllamaPull:
    ...
    invoke lstrcpyA, addr cmdLine, addr szCmdPrefix
    invoke lstrcatA, addr cmdLine, addr szOllamaPullCmd
    mov eax, [ebx].ModelActionParams.lpModelOrUrl
    invoke lstrcatA, addr cmdLine, eax
    push pResult
    push 0
    lea eax, cmdLine
    ...
    add esp, 12
    ret
AgenticIDE_Tool_ModelLoad ENDP

; ============================================================================
; AgenticIDE_Tool_ModelList
; - OLLAMA: runs `ollama list`
; ============================================================================
AgenticIDE_Tool_ModelList PROC lpParams:DWORD, pResult:DWORD
...
@OllamaList:
    invoke lstrcpyA, addr cmdLine, addr szCmdPrefix
    invoke lstrcatA, addr cmdLine, addr szOllamaListCmd
    push pResult
    push 0
    lea eax, cmdLine
    push eax
    ...
    add esp, 12
    ret
AgenticIDE_Tool_ModelList ENDP

; ============================================================================
; AgenticIDE_Tool_ModelInfer
; - OLLAMA: runs `ollama run <model> "<prompt>"`
; Input: lpParams -> ModelActionParams (lpArg=prompt)
; ============================================================================
AgenticIDE_Tool_ModelInfer PROC lpParams:DWORD, pResult:DWORD
    LOCAL cmdLine[2048]:BYTE
    mov ebx, lpParams
    mov eax, [ebx].ModelActionParams.dwSource
    cmp eax, MODEL_SOURCE_OLLAMA
    je @OllamaRun
...
@OllamaRun:
    invoke lstrcpyA, addr cmdLine, addr szCmdPrefix
    invoke lstrcatA, addr cmdLine, addr szOllamaRunCmd
    mov eax, [ebx].ModelActionParams.lpModelOrUrl
    invoke lstrcatA, addr cmdLine, eax
    invoke lstrcatA, addr cmdLine, addr szSpaceA
    invoke lstrcatA, addr cmdLine, addr szQuote
    mov eax, [ebx].ModelActionParams.lpArg
    invoke lstrcatA, addr cmdLine, eax
    invoke lstrcatA, addr cmdLine, addr szQuote
    push pResult
    push 0
    ...
    add esp, 12
    ret
AgenticIDE_Tool_ModelInfer ENDP

; ============================================================================
; AgenticIDE_TerminalExec_Admin - Run cmd/pwsh elevated and stream output via temp file
; Input:  lpParams -> TerminalExecParams, pResult -> ToolResult
; Output: eax = 1 success
; Notes: Uses ShellExecuteExA(runAs) to launch elevated cmd.exe that redirects
;        stdout/stderr to a temp file; parent polls and streams file contents.
; ============================================================================
AgenticIDE_TerminalExec_Admin PROC lpParams:DWORD, pResult:DWORD
    ; Stubbed admin terminal exec: mark disabled and return 0
    push ebp
    mov ebp, esp
    mov esi, pResult
    mov [esi].ToolResult.dwResultCode, 2 ; disabled
    mov [esi].ToolResult.lpResultData, 0
    mov [esi].ToolResult.cbResultSize, 0
    xor eax, eax
    pop ebp
    ret
AgenticIDE_TerminalExec_Admin ENDP
END
