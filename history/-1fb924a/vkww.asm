...
@SpawnCapture:
    ...
    xor eax, eax
    rep stosb
    invoke CreateProcessA, NULL, addr cmdLine, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, addr stSI, addr stPI
    test eax, eax
    jz @ToolFailed
...
@Trunc:
    ...
    mov edi, outBuf
    add edi, outLen
    invoke lstrcpyA, edi, addr szTruncated
    invoke lstrlenA, addr szTruncated
    add outLen, eax
    jmp @ReadDone
...
AgenticIDE_TerminalExec_Admin PROC lpParams:DWORD, pResult:DWORD
    LOCAL se:SHELLEXECUTEINFOA
    LOCAL tmpPath[MAX_PATH]:BYTE
    LOCAL tmpFile[MAX_PATH]:BYTE
    LOCAL cmdLine[1024]:BYTE
    ...
    invoke GetTempPathA, MAX_PATH, addr tmpPath
    test eax, eax
    jz @Fail
    invoke GetTempFileNameA, addr tmpPath, addr szTempPrefix, 0, addr tmpFile
    test eax, eax
    jz @Fail

    ; Ensure file exists/empty
    invoke CreateFileA, addr tmpFile, GENERIC_WRITE, FILE_SHARE_READ or FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL
    mov hTmp, eax
    cmp eax, INVALID_HANDLE_VALUE
    je @Fail
    invoke CloseHandle, hTmp

    ; Build elevated command arguments to cmd.exe
    ; cmd.exe /c <command> 1> "tmp" 2>&1
    invoke lstrcpyA, addr args, addr szCmdSlashC
    ...
    ; cmd admin
    mov eax, [ebx].TerminalExecParams.lpCommand
    invoke lstrcatA, addr args, eax
    jmp @AddRedirect

@BuildPwsh:
    ; confirmed pwsh admin
    invoke lstrcatA, addr args, addr szPwshPrefix
    invoke lstrcatA, addr args, addr szQuote
    mov eax, [ebx].TerminalExecParams.lpCommand
    invoke lstrcatA, addr args, eax
    invoke lstrcatA, addr args, addr szQuote

@AddRedirect:
    invoke lstrcatA, addr args, addr szRedirectPre
    invoke lstrcatA, addr args, addr tmpFile
    invoke lstrcatA, addr args, addr szRedirectPost

    ; Launch elevated cmd.exe
    lea edi, se
    mov ecx, SIZEOF SHELLEXECUTEINFOA
    xor eax, eax
    rep stosb
    mov se.cbSize, SIZEOF SHELLEXECUTEINFOA
    mov se.fMask, SEE_MASK_NOCLOSEPROCESS
    mov se.lpVerb, OFFSET szRunAsVerb
    mov se.lpFile, OFFSET szCmdExe
    lea eax, args
    mov se.lpParameters, eax
    mov se.nShow, SW_HIDE
    invoke ShellExecuteExA, addr se
    test eax, eax
    jz @Fail

    ; Open temp file for streaming reads
    invoke CreateFileA, addr tmpFile, GENERIC_READ, FILE_SHARE_READ or FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL
    mov hTmp, eax
    cmp eax, INVALID_HANDLE_VALUE
    je @FailWait

@PollLoop:
    invoke WaitForSingleObject, se.hProcess, 50
    cmp eax, WAIT_TIMEOUT
    jne @DrainAndDone

    ; Read any newly written bytes
    invoke SetFilePointer, hTmp, offset, NULL, FILE_BEGIN
    invoke ReadFile, hTmp, addr chunk, 4096, addr bytesRead, NULL
    test eax, eax
    jz @PollLoop
    mov eax, bytesRead
    test eax, eax
...
@Done:
    invoke CloseHandle, hTmp
@FailWait:
    invoke WaitForSingleObject, se.hProcess, INFINITE
    invoke CloseHandle, se.hProcess
    ; best-effort delete temp file
    invoke DeleteFileA, addr tmpFile
...
; verb/file strings used by elevated launcher
.data
    szRunAsVerb db "runas",0
    szCmdExe db "cmd.exe",0
