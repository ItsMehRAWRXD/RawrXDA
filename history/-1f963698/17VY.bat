...
if %errorlevel% neq 0 (
    ...
    set "LIB_MSVC=C:\VS2022Enterprise\VC\Tools\MSVC\14.50.35717\lib\x64"
    set "LIB_UM=C:\Program Files (x86)\Windows Kits\10\Lib\10.0.26100.0\um\x64"
    set "LIB_UCRT=C:\Program Files (x86)\Windows Kits\10\Lib\10.0.26100.0\ucrt\x64"
    
    if exist "!ML64_PATH!" (
        set "ML64="!ML64_PATH!""
        set "LINKER="!LINK_PATH!""
        set "LIB=!LIB_MSVC!;!LIB_UM!;!LIB_UCRT!;!LIB!"
        set "LIBPATH_FLAGS=/LIBPATH:"!LIB_MSVC!" /LIBPATH:"!LIB_UM!" /LIBPATH:"!LIB_UCRT!""
    ) else (
...
%LINKER% /DLL /OUT:RawrXD_Interconnect.dll /DEF:RawrXD_Interconnect.def /OPT:REF /LTCG /DEBUG:FULL /NOLOGO /NODEFAULTLIB ^
    !LIBPATH_FLAGS! ^
    !CORE_OBJS! ^
    kernel32.lib ntdll.lib ws2_32.lib user32.lib gdi32.lib
if %errorlevel% neq 0 exit /b %errorlevel%

:: 3. Compile Titan Unified (Merged into Core)
:: echo [Build] 2/5: Titan Unified Inference Engine (DLL)...
:: Merged into Interconnect.dll
...
%LINKER% /SUBSYSTEM:CONSOLE /ENTRY:main /OUT:RawrXD_NativeHost.exe /OPT:REF /LTCG /DEBUG:FULL /NOLOGO /NODEFAULTLIB ^
    !LIBPATH_FLAGS! ^
    RawrXD_NativeHost.obj ^
    RawrXD_Interconnect.lib ^
    kernel32.lib user32.lib RawrXD_Interconnect.lib
if %errorlevel% neq 0 exit /b %errorlevel%
...
%LINKER% /DLL /ENTRY:DllMain /OUT:RawrXD_PatternBridge.dll /OPT:REF /LTCG /DEBUG:FULL /NOLOGO ^
    !LIBPATH_FLAGS! ^
