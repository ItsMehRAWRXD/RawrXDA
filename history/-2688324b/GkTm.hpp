...
struct AgentResult {
    ...
    AgentResult() = default;
    AgentResult(bool s, const std::string& c, const std::vector<std::string>& st, const std::string& t)
        : success(s), content(c), steps(st), type(t) {}
};
...
private:
    ...
    AgentResult RunInference(const std::string& prompt, const std::string& type) {
        ...
        auto genResult = m_engine->generate(prompt);
        if (!genResult.has_value()) {
            AgentResult errorResult;
            errorResult.success = false;
            errorResult.content = "Inference failed.";
            errorResult.type = type;
            return errorResult;
        }
        std::string rawOutput = genResult.value().text;
        ...
        finalResult.content = rawOutput;
        finalResult.success = !rawOutput.empty();
