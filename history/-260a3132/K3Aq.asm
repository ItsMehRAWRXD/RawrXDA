...
Handler PROC
    ...
    ; Report STOP_PENDING if needed
    ; For now, main loop will exit shortly
.ignore:
    ret
Handler ENDP
...
ServiceMain PROC FRAME
    ...
    test rax, rax
    jz .report_fail
    mov rbx, rax    ; Save HMODULE in NON-VOLATILE rbx

    lea rcx, QuerySymbol
    ; GetProcAddress(hModule (RCX), lpProcName (RDX))
    mov rcx, rbx
    lea rdx, QuerySymbol
    call GetProcAddress
...
.loop:
    ...
    ; Loop over 5 drives
    xor r15, r15     ; r15 is Non-Volatile loop counter
.next:
    mov rcx, r15            ; Drive ID param for QueryNVMeTemp
    mov rax, [queryTempPtr]
    call rax
    
    mov rdx, [gMMFPtr]
    ; Write to offset 16 + index*4
