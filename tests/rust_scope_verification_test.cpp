// rust_scope_verification_test.cpp
// "Access Modifier Sovereignty" tests for Rust — proves the parser correctly
// identifies pub(crate) vs pub vs private scope boundaries.
// Compile: g++ -O2 -std=c++20 -I d:\rawrxd\src\core -I d:\rawrxd\include rust_scope_verification_test.cpp rust_parser_v2.o -o rust_scope_test.exe
...
// Helper: find first node whose text contains substring
...
// Test 1: pub(crate) function — should be visible within crate, not outside
static void test_pub_crate_visibility() {
    ...
    // Sovereignty: pub(crate) is distinct from pub and private
...
// Test 2: struct with pub fields vs private fields
...
// Test 3: trait + impl with method visibility
static void test_trait_impl_visibility() {
    ...
    // Impl should have children (methods parsed recursively)
    if (impl_node) {
        CHECK(impl_node->getChildCount() >= 1, "impl has child methods");
    }
}

// Test 4: unsafe + async modifiers
...
// Test 6: raw string literals + doc comments
