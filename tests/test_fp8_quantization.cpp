...
double calculate_rmse(const std::vector<float>& original, const std::vector<float>& reconstructed)
{
...
bool test_e4m3_basic()
{
    ...
    std::vector<float> test_values = {0.0f,   1.0f,    -1.0f, 10.0f, -10.0f, 100.0f, -100.0f,
                                      448.0f, -448.0f, 0.5f,  -0.5f, 0.1f,   -0.1f};
    ...
    for (float val : test_values)
    {
        ...
        {
            std::cout << "  FAIL: " << val << " -> " << (int)quantized << " -> " << reconstructed
...
bool test_e5m2_basic()
{
    ...
    std::vector<float> test_values = {0.0f,    1.0f,    -1.0f,    10.0f, -10.0f, 100.0f,
                                      -100.0f, 1000.0f, -1000.0f, 0.5f,  -0.5f};
...
bool test_batch_quantization()
{
    ...
    // E4M3 has ~3 mantissa bits: wide uniform [-A,A] absmax scaling yields RMSE ~1–2% of A
    // (encoder/decoder are consistent; this bounds numerical spread, not "quality vs FP32")
    const float halfSpan = 400.0f;
    const double relRmse = rmse / static_cast<double>(halfSpan);
    bool passed = relRmse < 0.02;  // <2% of half-range

    std::cout << "  RMSE: " << rmse << " (rel vs span: " << relRmse << ", gate: <0.02)" << std::endl;
...
bool test_batch_elementwise_parity()
{
    std::cout << "[TEST] Batch vs element-wise parity (stochastic off, E4M3)..." << std::endl;

    SovereignFP8Quantizer q(FP8Format::E4M3);
    q.enableStochasticRounding(false);
    q.setDitherSeed(0xCAFEBABEu);

    constexpr size_t n = 256;
    std::mt19937 rng(99);
    std::uniform_real_distribution<float> dist(-200.0f, 200.0f);
    std::vector<float> input(n);
    std::vector<uint8_t> batchOut(n);
    std::vector<uint8_t> elemOut(n);
    for (size_t i = 0; i < n; ++i)
    {
        input[i] = dist(rng);
    }

    const float scale = q.estimateScale(input.data(), n);
    q.quantizeBatch(input.data(), batchOut.data(), n, scale);

    // Match batch pre-scale: double multiply then float, same as quantizeBatch implementation
    const double sc = static_cast<double>(scale);
    for (size_t i = 0; i < n; ++i)
    {
        const float scaled = static_cast<float>(static_cast<double>(input[i]) * sc);
        elemOut[i] = q.quantize(scaled);  // stochastic off => same as quantizeFinite
    }

    const bool passed = std::memcmp(batchOut.data(), elemOut.data(), n) == 0;
    if (!passed)
    {
        for (size_t i = 0; i < n; ++i)
        {
            if (batchOut[i] != elemOut[i])
            {
                std::cout << "  First mismatch at i=" << i << " batch=" << static_cast<int>(batchOut[i])
                          << " elem=" << static_cast<int>(elemOut[i]) << std::endl;
                break;
            }
        }
    }

    g_results.push_back({"Batch/Element Parity", passed, 0.0, 0.0, n});
    return passed;
}
...
bool test_special_values()
{
    ...
    // NaN encode (0x7F) round-trips to quiet NaN
    uint8_t qnan = quantizer.quantize(std::numeric_limits<float>::quiet_NaN());
    float dnan = quantizer.dequantize(qnan);
    if (!std::isnan(dnan))
    {
        std::cout << "  FAIL: NaN round-trip (code=" << static_cast<int>(qnan) << ", got " << dnan << ")" << std::endl;
        passed = false;
    }

    g_results.push_back({"Special Values", passed, 0.0, 0.0, 4});
    return passed;
}
...
bool test_compression_ratio()
{
    ...
    const double withScales = KVCacheFP8Manager::getCompressionRatioWithScaleOverhead(num_elements, 1u);
    std::cout << "  Effective ratio (1 KV block, incl. k_scale+v_scale): " << withScales << std::endl;
...
int main(int argc, char** argv)
{
    ...
    if (test_batch_elementwise_parity())
        passed++;
    else
        failed++;
    std::cout << std::endl;
    ...
    for (const auto& result : g_results)
    {
        ...
        if (result.samples > 0)
        {
            std::cout << " (RMSE: " << result.rmse << ", samples: " << result.samples << ")";
        }
        std::cout << std::endl;
    }
