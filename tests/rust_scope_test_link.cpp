// rust_scope_test_link.cpp
// Test harness that links against pre-compiled rust_parser_v2_stub.o
// Compile: g++ -O2 -std=c++20 -I d:\rawrxd\tests -I d:\rawrxd\src\core -I d:\rawrxd\include -c rust_scope_test_link.cpp -o rust_scope_test_link.o
// Link:    g++ rust_scope_test_link.o rust_parser_v2_stub.o -o rust_scope_test.exe
...
static void test_modifiers_preserved() {
    ...
    auto unsafe_fn = findNodeWithText(result.nodes, "pub unsafe fn raw_ptr");
    auto async_fn  = findNodeWithText(result.nodes, "pub async fn fetch");
    auto const_fn  = findNodeWithText(result.nodes, "pub const fn max");
...
static void test_lifetime_tokenization() {
    ...
    CHECK(result.success, "parse succeeded");
    printf("  [DEBUG] nodes.size() = %zu\n", result.nodes.size());
    for (size_t i = 0; i < result.nodes.size(); ++i) {
        printf("  [DEBUG] node[%zu] = \"%s\"\n", i, result.nodes[i]->getText().c_str());
    }
