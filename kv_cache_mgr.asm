...
kv_cache_get PROC FRAME
    ...
    mov     ebx, ecx            ; layer
    mov     r12d, edx           ; position (preserved in r12)
    mov     rdi, r8             ; K output dest
    mov     r13, r9             ; V output dest

    ; layer_base = base + layer * kvLayerSize
    mov     rax, QWORD PTR [kvCache + KV_CACHE.pBase]
    mov     ecx, DWORD PTR [kvCache + KV_CACHE.kvLayerSize]
    imul    ecx, ebx
    add     rax, rcx            ; rax = layer_base

    ; pos_offset = pos * kvRowBytes
    mov     ecx, DWORD PTR [kvCache + KV_CACHE.kvRowBytes]
    mov     edx, r12d
    imul    edx, ecx            ; edx = pos * kvRowBytes

    ; K source = layer_base + pos_offset
    lea     rsi, [rax+rdx]

    ; Copy K (kvRowBytes bytes)
    mov     ecx, DWORD PTR [kvCache + KV_CACHE.kvRowBytes]
@@get_k:
    ...
    ; V source = layer_base + maxSeq*kvRowBytes + pos*kvRowBytes
    mov     rax, QWORD PTR [kvCache + KV_CACHE.pBase]
    mov     ecx, DWORD PTR [kvCache + KV_CACHE.kvLayerSize]
    imul    ecx, ebx
    add     rax, rcx            ; layer_base
    mov     ecx, DWORD PTR [kvCache + KV_CACHE.maxSeq]
    mov     edx, DWORD PTR [kvCache + KV_CACHE.kvRowBytes]
    imul    ecx, edx
    add     rax, rcx            ; V section start
    mov     ecx, r12d           ; position
    imul    ecx, edx            ; pos * kvRowBytes
    add     rax, rcx
    mov     rsi, rax            ; V source

    ; Copy V (kvRowBytes bytes)
    mov     ecx, DWORD PTR [kvCache + KV_CACHE.kvRowBytes]
    mov     rdi, r13            ; V dest
@@get_v:
    mov     r8b, BYTE PTR [rsi]
...
main PROC FRAME
    ...
    xor     ebx, ebx
    lea     r10, [tempK]        ; avoid ADDR32: load symbol addr into reg
    lea     r11, [tempV]
@@fill_temp:
    ...
    inc     eax                 ; 1-based
    cvtsi2ss xmm0, eax
    movss   DWORD PTR [r10+rbx*4], xmm0
