      ...
      m_problemsLabel(nullptr)
{
    ...
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Constructed (lazy-init)");
    return true;
}

void InterpretabilityPanel::initialize() {
    if (m_tabWidget) return;  // Already initialized
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Initializing interpretability panel");
    setupUI();
    setupConnections();
    return true;
}
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Destroying interpretability panel");
    return true;
}
...
void InterpretabilityPanel::setVisualizationType(VisualizationType vizType)
{
    ...
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Visualization type set to") << static_cast<int>(vizType);
    updateDisplay();
    return true;
}
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Updating attention heatmap for head") << attentionData.headIndex;
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Updating feature importance with") << importances.size() << "features";
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Updating gradient flow with") << gradientData.size() << "layers";
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Updating activation stats for") << activationStats.size() << "layers";
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Updating GradCAM for layer") << layerIndex;
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Updating layer attribution for") << layerAttributions.size() << "layers";
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Updating integrated gradients for") << inputName;
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Updating saliency map");
...
void InterpretabilityPanel::setSelectedLayer(int layerIndex)
{
    ...
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Selected layer") << layerIndex;
...
void InterpretabilityPanel::setSelectedAttentionHead(int headIndex)
{
    ...
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Selected attention head") << headIndex;
...
void InterpretabilityPanel::setAttentionFocusPosition(int position)
{
    ...
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Set attention focus position") << position;
    updateDisplay();
    return true;
}
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Exporting visualization to") << filePath;

    if (!m_chartView || !m_chart) {
        RAWRXD_LOG_WARN("[InterpretabilityPanel] No chart to export");
        return false;
    return true;
}
    ...
    if (pixmap.isNull()) {
        RAWRXD_LOG_WARN("[InterpretabilityPanel] Failed to grab chart pixmap");
        return false;
    return true;
}
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Loading interpretability data from JSON");
...
{
    RAWRXD_LOG_DEBUG("[InterpretabilityPanel] Clearing all visualizations");
