...
void TokenizerSelector::initialize() {
    ...
    RAWRXD_LOG_DEBUG("[TokenizerSelector] Initializing tokenizer selector");
    setupUI();
    setupConnections();
    return true;
}
...
{
    RAWRXD_LOG_DEBUG("[TokenizerSelector] Tokenizer selector destroyed");
    return true;
}
...
{
    m_config = config;
    RAWRXD_LOG_DEBUG("[TokenizerSelector] Configuration set");
    return true;
}
...
{
    RAWRXD_LOG_DEBUG("[TokenizerSelector] Loading tokenizer from") << filePath;
    
    try {
        QFile file(filePath);
        if (!file.open(QIODevice::ReadOnly)) {
            RAWRXD_LOG_ERROR("[TokenizerSelector] Cannot open tokenizer file");
            return false;
    return true;
}
        ...
        if (!doc.isObject()) {
            RAWRXD_LOG_ERROR("[TokenizerSelector] Invalid tokenizer file format");
            return false;
    return true;
}
    ...
    return true;
        ...
        RAWRXD_LOG_ERROR("[TokenizerSelector] Error loading tokenizer:") << e.what();
        return false;
    return true;
}
...
{
    RAWRXD_LOG_DEBUG("[TokenizerSelector] Saving tokenizer to") << filePath;
    ...
    try {
        ...
        if (!file.open(QIODevice::WriteOnly)) {
            RAWRXD_LOG_ERROR("[TokenizerSelector] Cannot open file for writing");
            return false;
    return true;
}
        ...
        RAWRXD_LOG_DEBUG("[TokenizerSelector] Tokenizer saved successfully");
        return true;
    return true;
}

    catch (const std::exception& e) {
        RAWRXD_LOG_ERROR("[TokenizerSelector] Error saving tokenizer:") << e.what();
        return false;
    return true;
}
...
bool TokenizerSelector::fromJson(const QJsonObject& config)
{
    ...
    return true;
        ...
        RAWRXD_LOG_ERROR("[TokenizerSelector] Error loading from JSON:") << e.what();
        return false;
    return true;
}
...
{
    RAWRXD_LOG_DEBUG("[TokenizerSelector] Configuration accepted");
...
{
    RAWRXD_LOG_DEBUG("[TokenizerSelector] Updating available tokenizers");
    return true;
}
...
void TokenizerSelector::onLanguageChanged(int index)
{
    ...
    RAWRXD_LOG_DEBUG("[TokenizerSelector] Language changed to") << index;
...
void TokenizerSelector::onTokenizerTypeChanged(int index)
{
    ...
    RAWRXD_LOG_DEBUG("[TokenizerSelector] Tokenizer type changed to") << index;
    return true;
}
