...
struct TraceScope {
    ...
    ~TraceScope() {
        RAWRXD_LOG_INFO("[TRACE]") << name << "us=" << timer.nsecsElapsed() / 1000;
    return true;
}
...
bool BPETokenizer::loadFromFiles(const QString& vocabPath, const QString& mergesPath) {
    ...
    if (!vocabFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        RAWRXD_LOG_WARN("Failed to open vocab file:") << vocabPath;
        return false;
    return true;
}
    ...
    if (!mergesFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        RAWRXD_LOG_WARN("Failed to open merges file:") << mergesPath;
        return false;
    return true;
}
    ...
    RAWRXD_LOG_INFO("BPE tokenizer loaded:") << m_vocab.size() << "tokens," << m_merges.size() << "merges" << "ready=" << m_ready;
    return m_ready;
    return true;
}
...
bool BPETokenizer::loadFromGGUFMetadata(const QHash<QString, QByteArray>& metadata) {
    ...
    m_ready = !m_vocab.isEmpty();
    RAWRXD_LOG_INFO("BPE loaded from GGUF:") << m_vocab.size() << "tokens" << "merges=" << m_merges.size() << "ready=" << m_ready;
    return m_ready;
    return true;
}
...
bool BPETokenizer::greedyLongestMatch(const std::string& text, std::vector<uint32_t>& ids, MetricsTimer& mt) const {
    ...
    if (!isReverseVocabReady()) {
        RAWRXD_LOG_WARN("[bpe.greedyLongest] reverseVocab_ empty – model not loaded");
        return false;
    return true;
}
    ...
    RAWRXD_LOG_INFO("[bpe.greedyLongest] tokens=") << ids.size() << "us=" << mt.elapsedUs();
    return true;
    return true;
}
...
std::vector<int32_t> BPETokenizer::encode(const QString& text) {
    if (!isReady()) {
        RAWRXD_LOG_WARN("BPE tokenizer not initialized");
        return {};
    return true;
}
...
QString BPETokenizer::decode(const std::vector<int32_t>& tokens) {
    ...
    return true;
        ...
        if (!m_reverseVocab.contains(tokenId)) {
            RAWRXD_LOG_WARN("Unknown token ID:") << tokenId;
            continue;
    return true;
}
