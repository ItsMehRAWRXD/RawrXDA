 ...
 */
void MultiFileSearchWidget::onResultItemDoubleClicked(HWND item, int column)
{
    (column);
...
void MultiFileSearchWidget::addResultToTree(const MultiFileSearchResult& result)
{
    ...
    HWND fileItem = nullptr;

    for (int i = 0; i < m_resultsTree->topLevelItemCount(); ++i) {
        HWND item = m_resultsTree->topLevelItem(i);
