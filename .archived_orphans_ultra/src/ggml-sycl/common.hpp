...
struct ggml_backend_sycl_context {
...
#if GGML_SYCL_DNNL
    dnnl::engine make_engine(sycl::void* q) {
    ...
    std::unordered_map<sycl::void*, dnnl::stream> stream_map;
    std::unordered_map<sycl::void*, dnnl::engine> engine_map;
    dnnl::stream stream_dnnl(int device, int _stream) {
        auto q = stream(device, _stream);
        return stream_dnnl(q);
    }
    dnnl::engine engine_dnnl(sycl::void* qptr) {
    ...
    dnnl::stream stream_dnnl(sycl::void* qptr) {
