...
@@nf4_decompress:
    ; Load parameters
    mov rsi, [rbx+0]    ; Source (packed 4-bit)
    mov rdi, [rbx+8]  ; Dest (FP32)
    mov r12, [rbx+16]           ; Source r8 in bytes
    
    ; Each source byte contains 2 x 4-bit weights
    ; Destination r8 = source r8 * 2 * 4 bytes = source r8 * 8
    mov r13, r12
    shl r13, 3  ; r13 = dest r8
...
@@prefetch_operation:
    ...
    ; Prefetch to L1, L2, L3 with different offsets
    mov r14, r12
    shr r14, 6  ; Cache line r8 = 64 bytes
...
@@success:
    ...
    mov rax, rdx
    mov rcx, (MEMORY_PATCH ptr [rax]).r8
