...
@@nf4_decompress:
    ...
    mov r12, (MEM_PATCH_DATA ptr [rbx]).PatchSize           ; Source patch_size_var in bytes
    
    ; Each source byte contains 2 x 4-bit weights
    ; Destination patch_size_var = source patch_size_var * 2 * 4 bytes = source patch_size_var * 8
    mov r13, r12
    shl r13, 3  ; r13 = dest patch_size_var
    
    ; Load NF4 lookup table into xmm20-xmm21
    vmovups xmm20, [NF4_LOOKUP_TABLE]      ; First 16 floats
    
    ; Load nibble mask
    vmovdqu8 xmm22, [NIBBLE_MASK]
...
@@nf4_block_loop:
    ...
    vmovdqu8 xmm0, [rsi]      ; xmm0 = 256 bytes = 512 x 4-bit weights
    
    ; Extract low nibbles (even indices)
    vpandq xmm1, xmm0, xmm22  ; xmm1 = low nibbles (bytes 0, 2, 4...)
    
    ; Extract high nibbles (odd indices)
    vpsrlw xmm2, xmm0, 4      ; Shift right by 4
    vpandq xmm2, xmm2, xmm22  ; xmm2 = high nibbles
...
@@prefetch_operation:
    ...
    ; Prefetch to L1, L2, L3 with different offsets
    mov r14, r12
    shr r14, 6  ; Cache line patch_size_var = 64 bytes
...
@@nt_block_loop:
    ...
    ; Load 256 bytes
    vmovdqu8 xmm0, [rsi]
    vmovdqu8 xmm1, [rsi + 64]
    vmovdqu8 xmm2, [rsi + 128]
    vmovdqu8 xmm3, [rsi + 192]
    
    ; Non-temporal store
    vmovntdq [rdi], xmm0
    vmovntdq [rdi + 64], xmm1
    vmovntdq [rdi + 128], xmm2
    vmovntdq [rdi + 192], xmm3
...
@@temp_block_loop:
    ...
    vmovdqa64 [rdi], xmm0
    vmovdqa64 [rdi + 64], xmm1
    vmovdqa64 [rdi + 128], xmm2
    vmovdqa64 [rdi + 192], xmm3
...
@@success:
    ; Update statistics
    add g_TotalBytesCopied, patch_size_var
