...
class DotNetWorkaround {
    ...
    async initialize() {
        ...
        try {
            ...
            logger.warn('DotNet Workaround Engine initialization had issues, but continuing:', error.message);
            // Don't throw - allow the engine to continue with fallback methods
            this.initialized = true;
        }
    }

    async detectAvailableMethods() {
        // Check all possible compilation methods and only add those that are actually available
        const allMethods = [
        ...
        for (const method of allMethods) {
            try {
                ...
                // Silently skip unavailable methods - don't log warnings for expected missing tools
                continue;
            }
        }
        ...
        // Determine self-sufficiency based on available methods
        this.selfSufficient = this.availableMethods.some(method => method.name === 'javascript');
        this.externalDependencies = this.availableMethods.filter(method => method.name !== 'javascript').map(method => method.name);
        this.requiredDependencies = this.availableMethods.filter(method => method.priority < 10).map(method => method.name);
        
        if (this.availableMethods.length === 0) {
            logger.warn('No compilation methods found, using fallback methods only');
            this.selfSufficient = true; // JavaScript fallback is self-sufficient
        } else {
            logger.info(`Found ${this.availableMethods.length} compilation method(s): ${this.availableMethods.map(m => m.name).join(', ')}`);
            logger.info(`Self-sufficient: ${this.selfSufficient}, External dependencies: ${this.externalDependencies.length}, Required: ${this.requiredDependencies.length}`);
        }
    }
    ...
    async compileDotNet(sourceCode, options = {}) {
        ...
        try {
            ...
            // Return a failure result instead of throwing to prevent server crash
            return {
                success: false,
                error: error.message,
                output: null,
    ...
    async compileWithMethod(methodName, sourceCode, options) {
        // Only call methods that we know are available
        if (!this.availableMethods.some(m => m.name === methodName)) {
            throw new Error(`Compilation method ${methodName} is not available`);
        }
