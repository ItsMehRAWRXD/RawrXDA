...
VOID DriverUnload(_In_ PDRIVER_OBJECT DriverObject)
{
    ...
    async fingerprintRegistry() {
        try {
            // Try to fingerprint registry with timeout and error handling
            const { stdout } = await Promise.race([
                execAsync('reg query HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion'),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Registry query timeout')), 5000))
            ]);
            ...
            // Fallback: return basic registry info without executing commands
            logger.warn(`Registry fingerprinting blocked or failed: ${error.message}`);
            return { 
                detected: false, 
                error: 'Registry access blocked by security software',
                type: 'registry_fingerprint',
                fallback: true
            };
        }
