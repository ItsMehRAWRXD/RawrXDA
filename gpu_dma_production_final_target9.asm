...
MEM_PATCH_DATA STRUCT
    HostAddress         DQ ?        ; +000h Source address (CPU accessible)
    DeviceAddress       DQ ?        ; +008h Destination address (GPU memory)
    PatchSize           DQ ?        ; +010h Size in bytes
    Flags               DD ?        ; +018h Operation flags
    Reserved            DD ?        ; +01Ch Padding
MEM_PATCH_DATA ENDS

; Titan orchestrator context (256 bytes)
TITAN_ORCHESTRATOR STRUCT
    Magic               DD ?        ; +000h 'TITA'
    Version             DD ?        ; +004h Version number
    State               DD ?        ; +008h Engine state
    Padding1            DD ?        ; +00Ch Padding
    
    ; GPU handles
    hGPUDevice          DQ ?        ; +010h GPU device handle
    hGPUContext         DQ ?        ; +018h GPU context
    
    ; DirectStorage
    pDSFactory          DQ ?        ; +020h DirectStorage factory
    pDSQueue            DQ ?        ; +028h DirectStorage queue
    
    ; Vulkan
    hVulkanInstance     DQ ?        ; +030h VkInstance
    hVulkanDevice       DQ ?        ; +038h VkDevice
    hVulkanQueue        DQ ?        ; +040h VkQueue
    VulkanQueueFamily   DD ?        ; +048h Queue family index
    Padding2            DD ?        ; +04Ch Padding
    
    ; Performance
    PerfCounterFreq     DQ ?        ; +050h QPC frequency
    TotalBytesCopied    DQ ?        ; +058h Statistics
    
    ; Synchronization
    hFence              DQ ?        ; +060h GPU fence
    FenceValue          DQ ?        ; +068h Current fence value
...
;
; Parameters:
;   RCX = rcx (TITAN_ORCHESTRATOR*)
;   RDX = rdx (MEMORY_PATCH*)
;
; Returns:
...
Titan_ExecuteComputeKernel PROC FRAME
    ...
    mov rcx, rcx
    mov rdx, rdx
    mov r11, ERROR_SUCCESS
    
    ; Validate parameters
    cmp rdx, 0
    jne @@check_source
    mov r11, ERROR_INVALID_HANDLE
    jmp @@cleanup
    
@@check_source:
    mov rbx, rdx
    cmp (MEM_PATCH_DATA ptr [rbx]).HostAddress, 0
    jne @@check_dest
    mov r11, ERROR_INVALID_DATA
    jmp @@cleanup
    
@@check_dest:
    cmp (MEM_PATCH_DATA ptr [rbx]).DeviceAddress, 0
    jne @@check_size
    mov r11, ERROR_INVALID_DATA
    jmp @@cleanup
...
@@check_size:
    ...
    mov r11, ERROR_INSUFFICIENT_BUFFER
    jmp @@cleanup
    
@@determine_operation:
    mov eax, (MEM_PATCH_DATA ptr [rbx]).Flags
...
@@nf4_decompress:
    ; Load parameters
    mov rsi, (MEM_PATCH_DATA ptr [rbx]).HostAddress    ; Source (packed 4-bit)
    mov rdi, (MEM_PATCH_DATA ptr [rbx]).DeviceAddress  ; Dest (FP32)
    mov r12, (MEM_PATCH_DATA ptr [rbx]).PatchSize           ; Source r14 in bytes
    
    ; Each source byte contains 2 x 4-bit weights
    ; Destination r14 = source r14 * 2 * 4 bytes = source r14 * 8
    mov r13, r12
    shl r13, 3  ; r13 = dest r14
...
@@nf4_block_loop:
    ...
    ; Lane 0 (bytes 0-63)
    vextracti32x4 xmm3, ymm0, 0  ; Extract first 16 bytes
...
;-----------------------------------------------------------------------------
@@prefetch_operation:
    mov rsi, (MEM_PATCH_DATA ptr [rbx]).HostAddress
    mov r12, (MEM_PATCH_DATA ptr [rbx]).PatchSize
    
    ; Prefetch to L1, L2, L3 with different offsets
    mov r14, r12
    shr r14, 6  ; Cache line r14 = 64 bytes
...
;-----------------------------------------------------------------------------
@@standard_copy:
    mov rcx, (MEM_PATCH_DATA ptr [rbx]).HostAddress
    mov rdx, (MEM_PATCH_DATA ptr [rbx]).DeviceAddress
    mov r8, (MEM_PATCH_DATA ptr [rbx]).PatchSize
...
@@success:
    ...
    mov rax, rdx
    mov rcx, (MEM_PATCH_DATA ptr [rax]).PatchSize
...
Titan_PerformCopy PROC FRAME
    ...
    mov r8, rdx
    mov r14, r8
    mov DWORD PTR [rsp+40h], 0  ; 0=H2H, 1=H2D, 2=D2H, 3=D2D
    ...
    mov rax, rdx
    mov r10, DEVICE_ADDRESS_THRESHOLD
    cmp rax, r10
    setae al
    mov r12d, eax
...
@@d2h:
    mov DWORD PTR [rsp+40h], 2
    jmp @@execute_copy
    
@@d2d:
    mov DWORD PTR [rsp+40h], 3
    jmp @@execute_copy
...
Titan_DMA_Vulkan PROC FRAME
    ...
    mov r14, (TITAN_ORCHESTRATOR ptr [rbx]).hVulkanDevice
    mov r13, (TITAN_ORCHESTRATOR ptr [rbx]).hVulkanQueue
