...
test(2, 't2-chat-nonstream', 'POST /v1/chat/completions (non-streaming)', async () => {
  const r = await httpRequest(CONFIG.url + '/v1/chat/completions', 'POST',
    { model: 'ministral-3:latest', messages: [{ role: 'user', content: 'Say OK' }], stream: false },
    CONFIG.timeout * 6);
  if (!r.ok) return { pass: false, detail: `Error: ${r.error || 'HTTP ' + r.status}` };
...
test(2, 't2-legacy-ask', 'POST /ask legacy endpoint', async () => {
  const r = await httpRequest(CONFIG.url + '/ask', 'POST', { question: 'Say OK', model: 'ministral-3:latest' }, CONFIG.timeout * 3);
  if (!r.ok) return { pass: false, detail: `Error: ${r.error || 'HTTP ' + r.status}` };
...
async function run() {
  ...
  // Quick backend probe with retries for slow startup
  let backendAlive = false;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      const probe = await httpRequest(CONFIG.url + '/health', 'GET', null, 5000);
      if (probe.ok) { backendAlive = true; break; }
    } catch (_) { }
    if (attempt < 3) {
      log(`${C.dim}  Backend probe attempt ${attempt}/3 failed, retrying in 2s...${C.reset}`);
