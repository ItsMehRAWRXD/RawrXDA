  ...
  const renderTree = (items: any[], depth: number = 0) => (
    items.map((item: any) => (
      <div key={item.name}>
        <button
          type="button"
          className="w-full text-left py-0.5 px-2 hover:bg-white/5 flex items-center gap-1 text-xs explorer-tree-item"
...
export const Win32IDELayout: React.FC<Win32IDELayoutProps> = ({
  ...
  const [panelHeight, _setPanelHeight] = useState(250);
  const [sidebarWidth, _setSidebarWidth] = useState(250);
  const [terminalInput, setTerminalInput] = useState('');
  const terminalRef = useRef<HTMLDivElement>(null);
  const layoutRootRef = useRef<HTMLDivElement>(null);

  // Sync layout CSS variables (avoids inline styles)
  useEffect(() => {
    const root = layoutRootRef.current;
    if (!root) return;
    root.style.setProperty('--layout-sidebar-width', `${sidebarWidth}px`);
    root.style.setProperty('--layout-panel-height', `${panelHeight}px`);
  ...
  return (
    <div ref={layoutRootRef} className="win32-layout-root h-screen w-screen flex flex-col bg-[#1e1e1e] text-[#cccccc]">
      {/* Title Bar */}
      <div className="h-[30px] bg-[#323233] flex items-center justify-between px-2 select-none drag-region">
        ...
        <div className="flex items-center gap-1">
          <button type="button" className="w-[46px] h-[30px] hover:bg-white/10 flex items-center justify-center" title="Minimize">
            <Minimize2 className="w-3 h-3" aria-hidden />
          </button>
          <button type="button" className="w-[46px] h-[30px] hover:bg-white/10 flex items-center justify-center" title="Maximize">
            <Maximize2 className="w-3 h-3" aria-hidden />
          </button>
          <button type="button" className="w-[46px] h-[30px] hover:bg-red-600 flex items-center justify-center" title="Close">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[48px] bg-[#333333] border-r border-[#2d2d30] flex flex-col items-center py-2 gap-1">
          ...
          <button type="button" className="w-[48px] h-[48px] flex items-center justify-center hover:bg-white/10 text-gray-400" title="Account">
        ...
        {/* Primary Sidebar */}
        {sidebarVisible && (
          <div className="win32-layout-sidebar bg-[#252526] border-r border-[#2d2d30] flex flex-col">
            {/* Sidebar Header */}
            <div className="h-[35px] flex items-center justify-between px-4 border-b border-[#2d2d30]">
              ...
              <div className="flex gap-1">
                <button type="button" className="w-6 h-6 hover:bg-white/10 flex items-center justify-center rounded" title="More options">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="h-[35px] bg-[#252526] border-b border-[#2d2d30] flex items-center">
            <div className="flex items-center h-full">
              <div className="h-full px-3 flex items-center gap-2 text-xs bg-[#1e1e1e] border-t-2 border-purple-600 text-white">
                ...
                {activeFile}
                <button type="button" className="ml-1 hover:bg-white/10 rounded p-0.5" title="Close tab">
                  <X className="w-3 h-3" aria-hidden />
                </button>
              </div>
            </div>
          ...
          <div
            className={`flex-1 bg-[#1e1e1e] overflow-hidden win32-layout-editor-area ${panelVisible ? 'with-panel' : ''}`}
          >
            {editor || (
          ...
          {/* Bottom Panel */}
          {panelVisible && (
            <div className="win32-layout-panel bg-[#1e1e1e] border-t border-[#2d2d30] flex flex-col">
              {/* Panel Tabs */}
              <div className="h-[35px] flex items-center justify-between bg-[#252526] border-b border-[#2d2d30]">
                ...
                <div className="flex gap-1 px-2">
                  <button type="button" className="w-6 h-6 hover:bg-white/10 flex items-center justify-center rounded" title="New">
                    <Plus className="w-4 h-4" aria-hidden />
                  </button>
                  <button type="button" className="w-6 h-6 hover:bg-white/10 flex items-center justify-center rounded" title="Panel options">
