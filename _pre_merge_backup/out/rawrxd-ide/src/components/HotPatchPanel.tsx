      ...
      {showCreateForm && (
        <div className="bg-card border border-border rounded-lg p-3 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" aria-hidden /> : 'Apply'}
            </button>
            <button
              type="button"
            ...
            {patches.length === 0 ? (
              ...
              patches.map((patch) => (
                <tr key={patch.id} className="border-t border-border hover:bg-secondary/30">
                  ...
                  <td className="p-2">
                    <span className={`px-1.5 py-0.5 rounded text-xs ${patch.layer === 'memory' ? 'bg-blue-900 text-blue-300' :
                        patch.layer === 'byte' ? 'bg-green-900 text-green-300' :
                          'bg-purple-900 text-purple-300'
                      }`}>{patch.layer}</span>
                  </td>
                  <td className="p-2">{patch.target}</td>
                  <td className={`p-2 ${patch.status === 'active' ? 'text-green-500' :
                      patch.status === 'failed' ? 'text-red-500' :
                        'text-gray-500'
                    }`}>{patch.status}</td>
                  <td className="p-2 text-right flex gap-1 justify-end">
                    {patch.status === 'active' && (
                      <button type="button" onClick={() => handleRevert(patch.id)} className="p-1 hover:bg-secondary rounded" title="Revert patch">
                        <RotateCcw className="w-4 h-4" aria-hidden />
                      </button>
                    )}
                    {patch.status === 'reverted' && (
                      <button type="button" onClick={() => handleApplyPatch(patch.id)} className="p-1 hover:bg-secondary rounded" title="Re-apply patch">
                        <Zap className="w-4 h-4" aria-hidden />
                      </button>
                    )}
                    <button type="button" onClick={() => handleRemovePatch(patch.id)} className="p-1 hover:bg-secondary rounded text-red-400" title="Remove patch">
