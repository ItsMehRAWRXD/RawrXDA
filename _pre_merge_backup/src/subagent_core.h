...
//
// Agents/models are not tied to a fixed pane or map: they can register a
// surface (e.g. via BeaconHub::registerDynamicBeacon) when they need to appear
// and unregister when done, so they appear where needed and disappear when done.
// ============================================================================
...
public:
    ...
    // ---- IDE Command Palette (for list_ide_commands, execute_ide_command) ----
    // Lets agents/models browse and invoke IDE commands (their own command palette)
    using ListIDECommandsCallback = std::function<std::string()>;
    using ExecuteIDECommandCallback = std::function<std::string(int id, const std::string& name)>;
    void setListIDECommandsCallback(ListIDECommandsCallback cb) { m_listIDECommandsCallback = cb; }
    void setExecuteIDECommandCallback(ExecuteIDECommandCallback cb) { m_executeIDECommandCallback = cb; }
...
private:
    ...
    FailureDetector* m_failureDetector = nullptr;
    Puppeteer* m_puppeteer = nullptr;
    ...
    ListIDECommandsCallback m_listIDECommandsCallback;
    ExecuteIDECommandCallback m_executeIDECommandCallback;
