...
import secrets
from flask import Flask, request, jsonify, make_response, Response, after_this_request
import logging
...
class UniversalAccessGateway:
    ...
    def __init__(self,
        ...
        self.allowed_origins = allowed_origins or ["http://localhost",
                                                   "http://127.0.0.1",
                                                   "https://rawrxd.local",
                                                   "app://rawrxd"]
    ...
    def init_app(self, app: Flask):
        ...
        def handle_cors_preflight():
            ...
            if request.method == 'OPTIONS':
                ...
                response.headers.add('Access-Control-Allow-Origin', origin if allowed else '')
            ...
            if allowed:
                after_this_request(lambda response: self._add_cors_headers(
                    response, origin
                ))
        ...
        def validate_auth():
            ...
            if not self.require_auth and request.remote_addr in ('127.0.0.1', 'localhost'):
                return
