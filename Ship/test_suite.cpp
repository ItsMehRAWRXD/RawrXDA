...
inline std::vector<TestResult>& GetResults() {
    static std::vector<TestResult> results;
    return results;
}

inline int& GetPassed() {
    static int passed = 0;
    return passed;
}

inline int& GetFailed() {
    static int failed = 0;
    return failed;
}
...
inline std::vector<TestRegistration>& GetRegistrations() {
    static std::vector<TestRegistration> regs;
    return regs;
}

inline void RegisterTest(const wchar_t* name, TestFunc func) {
    GetRegistrations().push_back({name, func});
}

#define TEST_CASE(name) \
    void test_##name(); \
    struct TestRegistrar_##name { \
        TestRegistrar_##name() { Test::RegisterTest(L#name, test_##name); } \
    }; \
    static TestRegistrar_##name g_registrar_##name; \
    void test_##name()
...
inline void RunAllTests() {
    for (const auto& reg : GetRegistrations()) {
        ...
        try {
            ...
            result.passed = false;
            result.message = L"Exception thrown";
            GetFailed()++;
        } catch (...) {
            result.passed = false;
...
inline void PrintResults() {
    ...
    std::wcout << L"\n───────────────────────────────────────────────────────────────────\n";
    std::wcout << L"  Total: " << GetResults().size() << L"  Passed: " << GetPassed() << L"  Failed: " << GetFailed() << L"\n";
    std::wcout << L"═══════════════════════════════════════════════════════════════════\n";
}
