...
DllMain PROC hInstDLL:QWORD, fdwReason:DWORD, lpvReserved:QWORD
    ...
    je @@dm_thread_attach
    ; DLL_THREAD_DETACH (3) or unknown
    jmp @@dm_ok
    
@@dm_attach:
    ; Save module handle
    mov QWORD PTR [g_hInstance], rbx
    ; Disable thread notifications (optimization)
    mov rcx, rbx
    call DisableThreadLibraryCalls
    ; Initialize the interceptor subsystem
    call InitOSInterceptor
    jmp @@dm_ok
    
@@dm_detach:
    ; Cleanup: stop interceptor if running
