...
; =============================================================================
; asm_spengine_quant_switch_adaptive
; v14.2.1 Cathedral Build: VRAM-pressure-aware dynamic quantization switching.
; Inspects current VRAM utilization and caps the quantization kernel to
; prevent OOM under memory pressure.
;
; ECX = desired kernel (QKERNEL_Q4_K_M, QKERNEL_Q5_K_M, QKERNEL_Q8_0)
; EDX = current VRAM usage percentage (0-100, from Vulkan memory budget)
; R8  = new kernel function pointer
; Returns: RAX = actual kernel selected (may differ from requested under pressure)
;          RDX = previous kernel pointer
; =============================================================================
asm_spengine_quant_switch_adaptive PROC FRAME
    push    rbx
    .pushreg rbx
    push    rsi
    .pushreg rsi
    ...
    mov     ebx, ecx                        ; desired kernel
    mov     esi, edx                        ; VRAM usage %
    mov     rdi, r8                         ; new kernel func ptr

    ; Store current VRAM pressure
    mov     DWORD PTR [g_spe_ctx.vramPressure], esi

    ; Apply pressure override policy (v14.2.1 Cathedral Build heuristic)
    cmp     esi, VRAM_PRESSURE_HIGH
    jge     @@force_q4

    cmp     esi, VRAM_PRESSURE_MEDIUM
    jge     @@cap_q5

    ; Low pressure: allow requested kernel as-is
    jmp     @@apply_kernel

@@force_q4:
    ; High pressure (>=85%): force Q4_K_M regardless of request
    cmp     ebx, QKERNEL_Q4_K_M
    je      @@apply_kernel                  ; Already Q4, proceed
    mov     ebx, QKERNEL_Q4_K_M            ; Override to Q4
    jmp     @@apply_kernel

@@cap_q5:
    ; Medium pressure (>=65%): cap at Q5_K_M (disallow Q8_0)
    cmp     ebx, QKERNEL_Q8_0
    jne     @@apply_kernel                  ; Q4 or Q5 OK
    mov     ebx, QKERNEL_Q5_K_M            ; Override Q8 to Q5

@@apply_kernel:
    ; Validate kernel ID
    cmp     ebx, QKERNEL_COUNT
    jge     @@aqsw_fail

    ; Atomic swap in dispatch table
    lea     rcx, g_quant_dispatch
    mov     rax, rdi
    xchg    QWORD PTR [rcx + rbx*8], rax
    mov     rdx, rax                        ; old kernel pointer (return in RDX)

    ; Record active quantization
    mov     DWORD PTR [g_spe_ctx.activeQuant], ebx
    ...
    mov     eax, ebx                        ; return actual kernel ID
    jmp     @@aqsw_exit
...
@@aqsw_exit:
    ...
    pop     rbx
    ret
asm_spengine_quant_switch_adaptive ENDP
