...
struct BenchmarkResult {
    float inferenceLatency = 0.0f;      // ms per token
    float tokenGenerationRate = 0.0f;   // tokens per second
    float memoryEfficiency = 0.0f;      // memory usage ratio
    float codeCompletionAccuracy = 0.0f; // percentage
    float bugDetectionRate = 0.0f;      // percentage
    float securityScanRate = 0.0f;      // percentage
    float refactoringSuccessRate = 0.0f; // percentage
    float documentationQuality = 0.0f;  // score 0-1
};
...
struct CompetitiveResult {
    ...
    float performanceRatio = 0.0f; // ourResult / theirResult
    float accuracyRatio = 0.0f;
    float featureCoverage = 0.0f;  // percentage of their features we have
...
struct CodeMetrics {
    ...
    size_t cognitiveComplexity = 0;
    size_t halsteadVolume = 0;
    size_t maintainabilityIndex = 0;
    size_t securityIssues = 0;
    size_t performanceBottlenecks = 0;
    size_t codeSmells = 0;
    float testCoverage = 0.0f;
    float documentationCoverage = 0.0f;
};
