...
public:
     ...
     */
    void analyzeProject(const std::filesystem::path& projectRootPath);
...
private:
    void scanProjectFiles(const std::filesystem::path& projectRootPath);
    void analyzeFileContent(const std::filesystem::path& filePath);
    void identifyDependencies(const std::filesystem::path& projectRootPath);
    void recognizePatterns();
    void inferCodeStyle();
    void calculateMetrics();
    std::string generateUniqueId();
    std::string getFileExtension(const std::filesystem::path& filePath) const;
    bool isSourceFile(const std::filesystem::path& filePath) const;
    bool isConfigFile(const std::filesystem::path& filePath) const;
