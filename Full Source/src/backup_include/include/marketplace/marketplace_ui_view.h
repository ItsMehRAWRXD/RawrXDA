...
class MarketplaceUIView
{
  ...
  private:
    ...
    std::vector<std::pair<std::string, std::string>> m_searchResultsData;  // id, display text
    std::vector<std::pair<std::string, std::string>> m_installedExtensionsData;
    int m_selectedIndex = -1;
    int m_tabIndex = 0;
    bool m_offlineMode = false;
    std::vector<std::string> m_allowList;
    std::vector<std::string> m_denyList;
    bool m_requireSignature = false;
};
