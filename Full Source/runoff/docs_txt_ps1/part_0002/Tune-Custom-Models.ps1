# 🎯 Custom Model Tuning & Optimization Script
...
Write-Host "  🎯 Custom Model Tuning & Optimization Suite" -ForegroundColor Cyan
...
Write-Host "`n📊 Current Model Analysis:" -ForegroundColor Yellow
...
foreach ($model in $CustomModels) {
  ...
  Write-Host "`n🔹 Analyzing: " -NoNewline -ForegroundColor Cyan
  ...
  foreach ($test in $tests) {
    ...
    if ($result.Success) {
      ...
      Write-Host "   ⚠️ $($test.Name) - Needs tuning" -ForegroundColor Yellow
    }
  }
  
  $score = [math]::Round(($passedTests / $tests.Count) * 100, 0)
  Write-Host "   📈 Capability Score: $score%" -ForegroundColor $(if ($score -ge 75) { "Green" } elseif ($score -ge 50) { "Yellow" } else { "Red" })
}

Write-Host "`n`n🔧 Optimization Recommendations:" -ForegroundColor Yellow
Write-Host "════════════════════════════════════════════════════════" -ForegroundColor Yellow
...
foreach ($model in $CustomModels) {
  ...
  Write-Host "`n🎯 $($model.Name) → $($model.NewName)" -ForegroundColor Cyan
...
if ($ApplyTuning) {
  Write-Host "`n`n⚡ APPLYING OPTIMIZATIONS..." -ForegroundColor Magenta
  ...
  foreach ($model in $CustomModels) {
    ...
    Write-Host "`n🔄 Creating optimized model: $($model.NewName)" -ForegroundColor Yellow
    ...
    Write-Host "   📝 Modelfile saved: $modelfilePath" -ForegroundColor Gray
    
    # Create the model in Ollama
    Write-Host "   🚀 Building model in Ollama..." -ForegroundColor Gray
    try {
      ...
      if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "   ⚠️ Model creation had issues: $createResult" -ForegroundColor Yellow
      }
    }
    catch {
  ...
  if ($TestAfterTuning) {
    Write-Host "`n`n🧪 TESTING TUNED MODELS..." -ForegroundColor Magenta
    Write-Host "════════════════════════════════════════════════════════" -ForegroundColor Magenta
    
    foreach ($model in $CustomModels) {
      Write-Host "`n🔹 Testing: $($model.NewName)" -ForegroundColor Cyan
      ...
      if ($testResult.Success) {
        Write-Host "   ✅ Agentic response verified!" -ForegroundColor Green
        Write-Host "   Response preview: $($testResult.Response.Substring(0, [Math]::Min(100, $testResult.Response.Length)))..." -ForegroundColor Gray
      }
      else {
        Write-Host "   ⚠️ Response needs review" -ForegroundColor Yellow
      }
    }
  }
}
else {
  Write-Host "`n`n💡 To apply these optimizations, run:" -ForegroundColor Yellow
  Write-Host "   .\Tune-Custom-Models.ps1 -ApplyTuning" -ForegroundColor Cyan
  Write-Host "`n💡 To apply and test, run:" -ForegroundColor Yellow
  Write-Host "   .\Tune-Custom-Models.ps1 -ApplyTuning -TestAfterTuning" -ForegroundColor Cyan
}

Write-Host "`n`n📋 Quick Reference - Optimal Parameters:" -ForegroundColor Yellow
Write-Host "════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host @"

┌─────────────────────┬─────────────┬───────────┬───────────┐
│ Model               │ Temperature │ Context   │ Use Case  │
├─────────────────────┼─────────────┼───────────┼───────────┤
│ bigdaddyg-fast      │ 0.3         │ 4096      │ Fast tasks│
│ bigdaddyg           │ 0.4         │ 8192      │ Complex   │
│ cheetah-stealth     │ 0.2         │ 4096      │ Security  │
└─────────────────────┴─────────────┴───────────┴───────────┘
