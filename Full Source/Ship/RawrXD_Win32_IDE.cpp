...
#define ID_COMMAND_PALETTE   1200
#define ID_SETTINGS_DIALOG   1201

// Pop-out Chat
#define ID_CHAT_POPOUT       1210
#define ID_CHAT_NEW_SESSION  1211

// Agentic DLL controls
#define ID_AGENT_AUTONOMOUS  1220
#define ID_AGENT_SELECT_DLL  1221

// Sourcefile Dropdown
#define ID_SOURCEFILE_DROPDOWN 1230

// Quick Open (Ctrl+P)
#define ID_QUICK_OPEN          1240

// AI Backend Switcher
#define ID_AI_BACKEND_LOCAL    1250
#define ID_AI_BACKEND_OLLAMA   1251
#define ID_AI_BACKEND_OPENAI   1252
#define ID_AI_BACKEND_CLAUDE   1253
#define ID_AI_BACKEND_GEMINI   1254
#define ID_AI_BACKEND_CONFIG   1255
#define ID_AI_SET_API_KEY      1256

// Inline Edit
#define ID_AI_INLINE_EDIT      1260

// Source File Picker
#define ID_SOURCEFILE_PICKER   1270

// Source File Registry dynamic IDs (60000+)
#define IDM_SRCFILE_BASE       60000
#define IDM_SRCFILE_MAX        65000
...
#define IDM_COMMAND_PALETTE ID_COMMAND_PALETTE
#define IDM_SETTINGS_DIALOG ID_SETTINGS_DIALOG
...
#define IDC_CMDPAL_OVERLAY   2100
#define IDC_CMDPAL_INPUT     2101
#define IDC_CMDPAL_LIST      2102

// Settings Dialog controls
#define IDC_SETTINGS_THEME   2110
#define IDC_SETTINGS_FONT    2111
#define IDC_SETTINGS_FONTSIZE 2112
#define IDC_SETTINGS_MINIMAP 2113
#define IDC_SETTINGS_FOLDING 2114
#define IDC_SETTINGS_WORDWRAP 2115
#define IDC_SETTINGS_AUTOCOMPLETE 2116
#define IDC_SETTINGS_APPLY   2117
#define IDC_SETTINGS_RESET   2118

// Sourcefile Dropdown
#define IDC_SOURCEFILE_COMBO 2130

// Pop-out Chat controls
#define IDC_POPCHAT_HISTORY  2140
#define IDC_POPCHAT_INPUT    2141
#define IDC_POPCHAT_SEND     2142
#define IDC_POPCHAT_MODEL    2143

// Quick Open controls
#define IDC_QUICKOPEN_INPUT  2150
#define IDC_QUICKOPEN_LIST   2151

// Source File Picker controls
#define IDC_SF_FILTER        2160
#define IDC_SF_LIST          2161
#define IDC_SF_OK            2162
#define IDC_SF_CANCEL        2163

// Inline Edit controls
#define IDC_INLINE_EDIT_INPUT 2170
#define IDC_INLINE_EDIT_APPLY 2171
#define IDC_INLINE_EDIT_CANCEL 2172

// Progress bar controls
#define IDC_PROGRESS_BAR     2180
#define IDC_PROGRESS_LABEL   2181
#define IDC_PROGRESS_CANCEL  2182
...
void FilterCommandPalette(const std::wstring& query);
void ExecuteCommandFromPalette(int index);
LRESULT CALLBACK CommandPaletteOverlayProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
LRESULT CALLBACK CommandPaletteInputProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData);

// Pop-out Chat
void PopOutChat();
LRESULT CALLBACK PopOutChatWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);

// Settings Dialog
void ShowSettingsDialogModal();
INT_PTR CALLBACK SettingsDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);

// Agentic DLL discovery
void DiscoverAgentDlls();
void RunAutonomousMode();

// Sourcefile Dropdown
void CreateSourcefileDropdown(HWND hwndParent);
...
LRESULT CALLBACK QuickOpenWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);

// Source File Picker / Registry
void ShowSourceFilePicker();
LRESULT CALLBACK SourceFilePickerWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
void CollectSourceFiles(const fs::path& root, std::vector<std::wstring>& out, int maxDepth, int depth);
bool IsSourceExt(const std::wstring& path);
HMENU BuildSourceFileMenu();

// AI Backend Switcher (OpenAI/Claude/Gemini/Ollama/Local)
enum class AIBackendType { LocalGGUF, Ollama, OpenAI, Claude, Gemini, Count };
void InitBackendManager();
void ShowBackendSwitcherDialog();
void SetActiveBackend(AIBackendType type);
...
void ShowApiKeyDialog(AIBackendType type);

// Real-Time Streaming UX
void ShowProgressBar(const std::wstring& operation);
void UpdateProgress(float percent, const std::wstring& status);
...
LRESULT CALLBACK InlineEditWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);

// Unrestricted Terminal (real subprocess)
void CreateRealTerminal();
void TerminalWriteInput(const std::string& data);
void TerminalReadOutputThread();
void TerminalReadErrorThread();
void TerminalMonitorThread();
void StopRealTerminal();

// Multi-Agent Parallelism
void RunMultiAgentTask(const std::string& task);
void ShowAgentProgress(int completed, int total, const std::string& current);
...
static std::vector<PopOutChatInstance> g_popOutChats;
static int g_nextPopOutId = 1;

// ============================================================================
// Agentic DLL Registry
// ============================================================================
struct AgentDllInfo {
    std::wstring name;
    std::wstring path;
    HMODULE hModule;
    bool loaded;
    std::wstring category; // "engine", "agent", "bridge", "utility"
};

static std::vector<AgentDllInfo> g_agentDlls;
static bool g_autonomousMode = false;

// ============================================================================
// Sourcefile Dropdown
// ============================================================================
static std::vector<std::wstring> g_sourcefileList;

// ============================================================================
// AI Backend Switcher State
// ============================================================================
...
static AIBackendConfig g_backendConfigs[(size_t)AIBackendType::Count];
static AIBackendType g_activeBackend = AIBackendType::LocalGGUF;
static std::mutex g_backendMutex;
static bool g_backendManagerInit = false;

// ============================================================================
// Real Terminal (subprocess) State
// ============================================================================
static HANDLE g_hTermProcess = nullptr;
static HANDLE g_hTermThread = nullptr;
static HANDLE g_hTermStdInRead = nullptr;
static HANDLE g_hTermStdInWrite = nullptr;
static HANDLE g_hTermStdOutRead = nullptr;
static HANDLE g_hTermStdOutWrite = nullptr;
static HANDLE g_hTermStdErrRead = nullptr;
static HANDLE g_hTermStdErrWrite = nullptr;
static std::atomic<bool> g_termRunning{false};
static std::thread g_termOutputThread;
static std::thread g_termErrorThread;
static std::thread g_termMonitorThread;

// ============================================================================
// Progress Bar / Streaming UX State
// ============================================================================
static HWND g_hwndProgressContainer = nullptr;
static HWND g_hwndProgressBar = nullptr;
static HWND g_hwndProgressLabel = nullptr;
static HWND g_hwndProgressCancel = nullptr;
static std::atomic<bool> g_operationActive{false};
static std::atomic<bool> g_operationCancelled{false};
static std::atomic<float> g_progressPercent{0.0f};
static std::mutex g_progressMutex;
static std::string g_progressStatus;

// ============================================================================
// Inline Edit State
// ============================================================================
static HWND g_hwndInlineEditOverlay = nullptr;
static HWND g_hwndInlineEditInput = nullptr;
static bool g_inlineEditActive = false;
static int g_inlineEditSelStart = 0;
static int g_inlineEditSelEnd = 0;

// ============================================================================
// Source File Registry (dynamic scan at runtime)
// ============================================================================
...
static std::vector<SourceFileEntry> g_sourceFileRegistry;
static UINT g_nextSrcFileId = IDM_SRCFILE_BASE;
...
void SendChatToServer(const std::wstring& message) {
    ...
    // ── Tier 1: HTTP Chat Server (localhost:23959) via WinHTTP ──
    if (g_chatServerRunning) {
        HINTERNET hSession = WinHttpOpen(L"RawrXD IDE/1.0",
            WINHTTP_ACCESS_TYPE_NO_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
        if (hSession) {
            HINTERNET hConnect = WinHttpConnect(hSession, L"localhost", 23959, 0);
            if (hConnect) {
                HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"POST", L"/chat",
                    nullptr, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, 0);
                if (hRequest) {
                    std::string postData = WideToUtf8(message);
                    LPCWSTR hdrs = L"Content-Type: text/plain\r\n";
                    BOOL sent = WinHttpSendRequest(hRequest, hdrs, (DWORD)-1L,
...
HMENU CreateMainMenu() {
    ...
    AppendMenu(hFileMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenu(hFileMenu, MF_STRING, ID_QUICK_OPEN, L"&Quick Open...\tCtrl+P");
    AppendMenu(hFileMenu, MF_STRING, ID_SOURCEFILE_PICKER, L"Source File &Browser...");
    AppendMenu(hFileMenu, MF_SEPARATOR, 0, nullptr);
    ...
    AppendMenu(hViewMenu, MF_STRING | (g_bFileTreeVisible ? MF_CHECKED : MF_UNCHECKED), ID_VIEW_FILETREE, L"&File Explorer\tCtrl+B");
    AppendMenu(hViewMenu, MF_STRING | (g_bOutputVisible ? MF_CHECKED : MF_UNCHECKED), ID_VIEW_OUTPUT, L"&Output");
    AppendMenu(hViewMenu, MF_STRING | (g_bTerminalVisible ? MF_CHECKED : MF_UNCHECKED), ID_VIEW_TERMINAL, L"&Terminal\tCtrl+`");
    AppendMenu(hViewMenu, MF_STRING | (g_bChatVisible ? MF_CHECKED : MF_UNCHECKED), ID_VIEW_CHAT, L"&Chat");
    AppendMenu(hViewMenu, MF_STRING | (g_bIssuesVisible ? MF_CHECKED : MF_UNCHECKED), ID_VIEW_ISSUES, L"&Issues");
    AppendMenu(hViewMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenu(hViewMenu, MF_STRING | (g_minimapVisible ? MF_CHECKED : MF_UNCHECKED), ID_VIEW_MINIMAP, L"&Minimap\tCtrl+M");
    AppendMenu(hViewMenu, MF_STRING | (g_codeFoldingEnabled ? MF_CHECKED : MF_UNCHECKED), ID_VIEW_FOLDING, L"Code &Folding\tCtrl+K");
    AppendMenu(hViewMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenu(hViewMenu, MF_STRING, ID_COMMAND_PALETTE, L"Command &Palette...\tCtrl+Shift+P");
    AppendMenu(hViewMenu, MF_STRING, ID_SETTINGS_DIALOG, L"&Settings...\tCtrl+,");
    AppendMenu(hViewMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenu(hViewMenu, MF_STRING, ID_CHAT_POPOUT, L"Pop-Out C&hat Window");
    ...
    AppendMenu(hAIMenu, MF_STRING, ID_AI_INLINE_EDIT, L"&Inline Edit\tCtrl+I");
    ...
    HMENU hBackendMenu = CreatePopupMenu();
    AppendMenu(hBackendMenu, MF_STRING, ID_AI_BACKEND_LOCAL, L"&Local GGUF (Native)");
    AppendMenu(hBackendMenu, MF_STRING, ID_AI_BACKEND_OLLAMA, L"&Ollama (localhost:11434)");
    AppendMenu(hBackendMenu, MF_STRING, ID_AI_BACKEND_OPENAI, L"&OpenAI (GPT-4o)");
    AppendMenu(hBackendMenu, MF_STRING, ID_AI_BACKEND_CLAUDE, L"&Claude (Anthropic)");
    AppendMenu(hBackendMenu, MF_STRING, ID_AI_BACKEND_GEMINI, L"&Gemini (Google)");
    AppendMenu(hBackendMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenu(hBackendMenu, MF_STRING, ID_AI_BACKEND_CONFIG, L"Backend &Status...");
    AppendMenu(hBackendMenu, MF_STRING, ID_AI_SET_API_KEY, L"Set &API Key...");
    AppendMenu(hAIMenu, MF_POPUP, (UINT_PTR)hBackendMenu, L"Switch &Backend");
    AppendMenu(hAIMenu, MF_SEPARATOR, 0, nullptr);
    
    AppendMenu(hAIMenu, MF_STRING, ID_AGENT_AUTONOMOUS, L"Agentic &Autonomous Mode");
    AppendMenu(hAIMenu, MF_STRING, ID_AGENT_SELECT_DLL, L"&Discover Agent DLLs...");
    AppendMenu(hMenuBar, MF_POPUP, (UINT_PTR)hAIMenu, L"&AI");
    
    // Source Files menu (dynamically populated from workspace)
    if (!g_workspaceRoot.empty()) {
        HMENU hSrcMenu = BuildSourceFileMenu();
        if (hSrcMenu) {
            AppendMenu(hMenuBar, MF_POPUP, (UINT_PTR)hSrcMenu, L"&Source Files");
        }
    }
...
void HandleViewMenu(HWND hwnd, UINT id) {
    ...
    switch (id) {
        case ID_VIEW_FILETREE:
            ...
            CheckMenuItem(hMenu, ID_VIEW_FILETREE, g_bFileTreeVisible ? MF_CHECKED : MF_UNCHECKED);
            SendMessage(hwnd, WM_SIZE, 0, 0);
            break;
        case ID_VIEW_OUTPUT:
            g_bOutputVisible = !g_bOutputVisible;
            if (g_bOutputVisible) g_bTerminalVisible = false;
            CheckMenuItem(hMenu, ID_VIEW_OUTPUT, g_bOutputVisible ? MF_CHECKED : MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_VIEW_TERMINAL, g_bTerminalVisible ? MF_CHECKED : MF_UNCHECKED);
            SendMessage(hwnd, WM_SIZE, 0, 0);
            break;
        case ID_VIEW_TERMINAL:
            g_bTerminalVisible = !g_bTerminalVisible;
            if (g_bTerminalVisible) g_bOutputVisible = false;
            CheckMenuItem(hMenu, ID_VIEW_TERMINAL, g_bTerminalVisible ? MF_CHECKED : MF_UNCHECKED);
            CheckMenuItem(hMenu, ID_VIEW_OUTPUT, g_bOutputVisible ? MF_CHECKED : MF_UNCHECKED);
            SendMessage(hwnd, WM_SIZE, 0, 0);
            break;
        case ID_VIEW_CHAT:
            ...
            CheckMenuItem(hMenu, ID_VIEW_CHAT, g_bChatVisible ? MF_CHECKED : MF_UNCHECKED);
            SendMessage(hwnd, WM_SIZE, 0, 0);
            break;
        case ID_VIEW_ISSUES:
            g_bIssuesVisible = !g_bIssuesVisible;
            CheckMenuItem(hMenu, ID_VIEW_ISSUES, g_bIssuesVisible ? MF_CHECKED : MF_UNCHECKED);
            SendMessage(hwnd, WM_SIZE, 0, 0);
            if (g_bIssuesVisible) RefreshCodeIssues();
            break;
        case ID_VIEW_MINIMAP:
            ToggleMinimap();
            CheckMenuItem(hMenu, ID_VIEW_MINIMAP, g_minimapVisible ? MF_CHECKED : MF_UNCHECKED);
            break;
        case ID_VIEW_FOLDING:
            ...
            CheckMenuItem(hMenu, ID_VIEW_FOLDING, g_codeFoldingEnabled ? MF_CHECKED : MF_UNCHECKED);
            break;
    }
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        case IDM_VIEW_OUTPUT:
            HandleViewMenu(hwnd, ID_VIEW_OUTPUT);
            break;
            
        case IDM_VIEW_TERMINAL:
            HandleViewMenu(hwnd, ID_VIEW_TERMINAL);
            break;
            
        case IDM_VIEW_CHAT:
            HandleViewMenu(hwnd, ID_VIEW_CHAT);
            break;
            
        case IDM_VIEW_ISSUES:
            HandleViewMenu(hwnd, ID_VIEW_ISSUES);
            break;
        ...
        // --- View menu (routed through HandleViewMenu) ---
        case IDM_VIEW_FILETREE:
            HandleViewMenu(hwnd, ID_VIEW_FILETREE);
            break;
        case ID_VIEW_MINIMAP:
            HandleViewMenu(hwnd, ID_VIEW_MINIMAP);
            break;
        case ID_VIEW_FOLDING:
            HandleViewMenu(hwnd, ID_VIEW_FOLDING);
            break;
        
        // --- Command Palette ---
        case ID_COMMAND_PALETTE:
            ShowCommandPalette();
            break;
        
        // --- Settings Dialog ---
        case ID_SETTINGS_DIALOG:
            ShowSettingsDialogModal();
            break;
        ...
        // Source File Picker
        case ID_SOURCEFILE_PICKER:
            ShowSourceFilePicker();
            break;
        
        // Inline Edit
        case ID_AI_INLINE_EDIT:
            StartInlineEdit();
            break;
        
        // Backend Switcher
        case ID_AI_BACKEND_LOCAL:
            SetActiveBackend(AIBackendType::LocalGGUF);
            break;
        case ID_AI_BACKEND_OLLAMA:
            SetActiveBackend(AIBackendType::Ollama);
            break;
        case ID_AI_BACKEND_OPENAI:
            SetActiveBackend(AIBackendType::OpenAI);
            break;
        case ID_AI_BACKEND_CLAUDE:
            SetActiveBackend(AIBackendType::Claude);
            break;
        case ID_AI_BACKEND_GEMINI:
            SetActiveBackend(AIBackendType::Gemini);
            break;
        case ID_AI_BACKEND_CONFIG:
            ShowBackendSwitcherDialog();
            break;
        case ID_AI_SET_API_KEY:
            ShowApiKeyDialog(g_activeBackend);
            break;
            
        default:
            // Source File Registry dynamic IDs
            if (wmId >= IDM_SRCFILE_BASE && wmId <= IDM_SRCFILE_MAX) {
                UINT idx = wmId - IDM_SRCFILE_BASE;
                if (idx < g_sourceFileRegistry.size()) {
                    LoadFile(g_sourceFileRegistry[idx].absolutePath);
                }
            }
            break;
        }
...
LRESULT CALLBACK EditorSubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData) {
    ...
    case WM_KEYDOWN:
        if (GetKeyState(VK_CONTROL) & 0x8000) {
            if ((GetKeyState(VK_SHIFT) & 0x8000) && wParam == 'P') {
                ShowCommandPalette();
                return 0;
            }
            switch (wParam) {
            case 'P': // Ctrl+P = Quick Open
                ShowQuickOpenDialog();
                return 0;
            case 'I': // Ctrl+I = Inline Edit
                StartInlineEdit();
                return 0;
            case 'G':
                TriggerAICompletion();
...
// Command Palette
// ============================================================================
static HWND g_hwndPaletteOverlay = nullptr;
static HWND g_hwndPaletteInput = nullptr;
static HWND g_hwndPaletteList = nullptr;
...
static std::vector<PaletteCommand> g_paletteCommands;
static std::vector<int> g_paletteFiltered;

static void BuildPaletteCommands() {
    g_paletteCommands.clear();
    g_paletteCommands.push_back({L"File: New",               ID_FILE_NEW});
    g_paletteCommands.push_back({L"File: Open...",            ID_FILE_OPEN});
    g_paletteCommands.push_back({L"File: Open Folder...",     ID_FILE_OPENFOLDER});
    g_paletteCommands.push_back({L"File: Save",               ID_FILE_SAVE});
    g_paletteCommands.push_back({L"File: Save As...",          ID_FILE_SAVEAS});
    g_paletteCommands.push_back({L"File: Close",              ID_FILE_CLOSE});
    g_paletteCommands.push_back({L"Edit: Undo",               ID_EDIT_UNDO});
    g_paletteCommands.push_back({L"Edit: Redo",               ID_EDIT_REDO});
    g_paletteCommands.push_back({L"Edit: Cut",                ID_EDIT_CUT});
    g_paletteCommands.push_back({L"Edit: Copy",               ID_EDIT_COPY});
    g_paletteCommands.push_back({L"Edit: Paste",              ID_EDIT_PASTE});
    g_paletteCommands.push_back({L"Edit: Find...",            ID_EDIT_FIND});
    g_paletteCommands.push_back({L"Edit: Replace...",         ID_EDIT_REPLACE});
    g_paletteCommands.push_back({L"Edit: Select All",         ID_EDIT_SELECT_ALL});
    g_paletteCommands.push_back({L"View: Toggle File Explorer", ID_VIEW_FILETREE});
    g_paletteCommands.push_back({L"View: Toggle Output",      ID_VIEW_OUTPUT});
    g_paletteCommands.push_back({L"View: Toggle Terminal",     ID_VIEW_TERMINAL});
    g_paletteCommands.push_back({L"View: Toggle Chat",        ID_VIEW_CHAT});
    g_paletteCommands.push_back({L"View: Toggle Issues",      ID_VIEW_ISSUES});
    g_paletteCommands.push_back({L"View: Toggle Minimap",     ID_VIEW_MINIMAP});
    g_paletteCommands.push_back({L"View: Toggle Code Folding", ID_VIEW_FOLDING});
    g_paletteCommands.push_back({L"AI: Code Completion",      ID_AI_COMPLETE});
    g_paletteCommands.push_back({L"AI: Explain Code",         ID_AI_EXPLAIN});
    g_paletteCommands.push_back({L"AI: Refactor",             ID_AI_REFACTOR});
    g_paletteCommands.push_back({L"AI: Fix Code",             ID_AI_FIX});
    g_paletteCommands.push_back({L"AI: Load Model...",        ID_AI_LOADMODEL});
    g_paletteCommands.push_back({L"Build: Compile",           ID_BUILD_COMPILE});
    g_paletteCommands.push_back({L"Build: Run",               ID_BUILD_RUN});
    g_paletteCommands.push_back({L"Build: Debug",             ID_BUILD_DEBUG});
    g_paletteCommands.push_back({L"Help: About",              ID_HELP_ABOUT});
}

void FilterCommandPalette(const std::wstring& query) {
    g_paletteFiltered.clear();
    std::wstring lq = query;
    for (auto& c : lq) c = towlower(c);
    for (int i = 0; i < (int)g_paletteCommands.size(); i++) {
        std::wstring ll = g_paletteCommands[i].label;
        for (auto& c : ll) c = towlower(c);
        if (lq.empty() || ll.find(lq) != std::wstring::npos) {
            g_paletteFiltered.push_back(i);
        }
    }
    // Repopulate list
    if (g_hwndPaletteList) {
        SendMessageW(g_hwndPaletteList, LB_RESETCONTENT, 0, 0);
        for (int idx : g_paletteFiltered) {
            SendMessageW(g_hwndPaletteList, LB_ADDSTRING, 0, (LPARAM)g_paletteCommands[idx].label.c_str());
        }
        if (!g_paletteFiltered.empty()) {
            SendMessageW(g_hwndPaletteList, LB_SETCURSEL, 0, 0);
        }
    }
}

void ExecuteCommandFromPalette(int index) {
    if (index < 0 || index >= (int)g_paletteFiltered.size()) return;
    int cmdIdx = g_paletteFiltered[index];
    UINT cmdId = g_paletteCommands[cmdIdx].cmdId;
    HideCommandPalette();
    PostMessageW(g_hwndMain, WM_COMMAND, MAKEWPARAM(cmdId, 0), 0);
}

LRESULT CALLBACK CommandPaletteInputProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR) {
    if (msg == WM_KEYDOWN) {
        if (wParam == VK_ESCAPE) {
            HideCommandPalette();
            return 0;
        ...
        if (wParam == VK_RETURN) {
            int sel = (int)SendMessageW(g_hwndPaletteList, LB_GETCURSEL, 0, 0);
            if (sel >= 0) ExecuteCommandFromPalette(sel);
            return 0;
        }
        if (wParam == VK_DOWN) {
            int sel = (int)SendMessageW(g_hwndPaletteList, LB_GETCURSEL, 0, 0);
            int cnt = (int)SendMessageW(g_hwndPaletteList, LB_GETCOUNT, 0, 0);
            if (sel + 1 < cnt) SendMessageW(g_hwndPaletteList, LB_SETCURSEL, sel + 1, 0);
            return 0;
        }
        if (wParam == VK_UP) {
            int sel = (int)SendMessageW(g_hwndPaletteList, LB_GETCURSEL, 0, 0);
            if (sel > 0) SendMessageW(g_hwndPaletteList, LB_SETCURSEL, sel - 1, 0);
            return 0;
        }
    }
    if (msg == WM_CHAR && wParam != VK_RETURN && wParam != VK_ESCAPE) {
        ...
        int len = GetWindowTextLengthW(hwnd);
        std::wstring q(len + 1, L'\0');
        GetWindowTextW(hwnd, &q[0], len + 1);
        q.resize(len);
        FilterCommandPalette(q);
        return r;
    }
    return DefSubclassProc(hwnd, msg, wParam, lParam);
}
...
LRESULT CALLBACK CommandPaletteOverlayProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_DESTROY:
        g_hwndPaletteOverlay = nullptr;
        g_hwndPaletteInput = nullptr;
        g_hwndPaletteList = nullptr;
        break;
    }
    return DefWindowProcW(hwnd, msg, wParam, lParam);
}
...
void ShowCommandPalette() {
    if (g_hwndPaletteOverlay) {
        SetFocus(g_hwndPaletteInput);
        return;
    }
    // Build command list
    ...
    if (!classRegistered) {
        WNDCLASSEXW wc = {};
        wc.cbSize = sizeof(wc);
        wc.lpfnWndProc = CommandPaletteOverlayProc;
        wc.hInstance = g_hInstance;
        wc.hCursor = LoadCursor(nullptr, IDC_ARROW);
        wc.hbrBackground = (HBRUSH)GetStockObject(NULL_BRUSH);
        wc.lpszClassName = L"RawrXD_CmdPalette";
        RegisterClassExW(&wc);
        classRegistered = true;
    }
    
    // Position centered at top of main window
    RECT mainRc;
    GetClientRect(g_hwndMain, &mainRc);
    MapWindowPoints(g_hwndMain, nullptr, (POINT*)&mainRc, 2);
    int palW = 500, palH = 350;
    int palX = mainRc.left + (mainRc.right - mainRc.left - palW) / 2;
    int palY = mainRc.top + 50;
    
    g_hwndPaletteOverlay = CreateWindowExW(WS_EX_TOPMOST | WS_EX_TOOLWINDOW,
        L"RawrXD_CmdPalette", L"",
        WS_POPUP | WS_VISIBLE | WS_BORDER,
        palX, palY, palW, palH,
        g_hwndMain, nullptr, g_hInstance, nullptr);
    
    g_hwndPaletteInput = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
        10, 10, palW - 20, 24,
        g_hwndPaletteOverlay, nullptr, g_hInstance, nullptr);
    SendMessageW(g_hwndPaletteInput, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
    SetWindowSubclass(g_hwndPaletteInput, CommandPaletteInputProc, 0, 0);
    
    g_hwndPaletteList = CreateWindowExW(0, L"LISTBOX", L"",
        WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY | LBS_NOINTEGRALHEIGHT,
        10, 40, palW - 20, palH - 50,
        g_hwndPaletteOverlay, (HMENU)9001, g_hInstance, nullptr);
    SendMessageW(g_hwndPaletteList, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
    
    FilterCommandPalette(L"");
    SetFocus(g_hwndPaletteInput);
}

void HideCommandPalette() {
    if (g_hwndPaletteOverlay) {
        DestroyWindow(g_hwndPaletteOverlay);
        g_hwndPaletteOverlay = nullptr;
        g_hwndPaletteInput = nullptr;
        g_hwndPaletteList = nullptr;
    }
...
// ============================================================================
// Pop-out Chat Window (Independent Instances with Model Selectors)
// ============================================================================
LRESULT CALLBACK PopOutChatWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    PopOutChatInstance* inst = (PopOutChatInstance*)GetWindowLongPtrW(hwnd, GWLP_USERDATA);
    
    switch (msg) {
    case WM_CREATE: {
        CREATESTRUCT* cs = (CREATESTRUCT*)lParam;
        inst = (PopOutChatInstance*)cs->lpCreateParams;
        ...
        // Model selector combo at top
        inst->hwndModelCombo = CreateWindowExW(0, L"COMBOBOX", L"",
            WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL,
            5, 5, w - 10, 200,
            hwnd, (HMENU)IDC_POPCHAT_MODEL, g_hInstance, nullptr);
        SendMessageW(inst->hwndModelCombo, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
        SendMessageW(inst->hwndModelCombo, CB_ADDSTRING, 0, (LPARAM)L"Default (Auto)");
        SendMessageW(inst->hwndModelCombo, CB_ADDSTRING, 0, (LPARAM)L"Titan Kernel");
        SendMessageW(inst->hwndModelCombo, CB_ADDSTRING, 0, (LPARAM)L"Inference Engine (GGUF)");
        SendMessageW(inst->hwndModelCombo, CB_ADDSTRING, 0, (LPARAM)L"Native Model Bridge (ASM)");
        for (const auto& dll : g_agentDlls) {
            SendMessageW(inst->hwndModelCombo, CB_ADDSTRING, 0, (LPARAM)dll.name.c_str());
        }
        SendMessageW(inst->hwndModelCombo, CB_SETCURSEL, 0, 0);
        
        // Chat history (RichEdit for better formatting)
        LoadLibraryW(L"msftedit.dll");
        inst->hwndHistory = CreateWindowExW(WS_EX_CLIENTEDGE, MSFTEDIT_CLASS, L"",
            WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY,
            5, 35, w - 10, h - 75,
            hwnd, (HMENU)IDC_POPCHAT_HISTORY, g_hInstance, nullptr);
        SendMessageW(inst->hwndHistory, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
        SendMessageW(inst->hwndHistory, EM_SETBKGNDCOLOR, 0, RGB(30, 30, 30));
        {
            CHARFORMAT2W cf = {};
            cf.cbSize = sizeof(cf);
            cf.dwMask = CFM_COLOR;
            cf.crTextColor = RGB(220, 220, 220);
            SendMessageW(inst->hwndHistory, EM_SETCHARFORMAT, SCF_ALL, (LPARAM)&cf);
        }
        
        // Input
        inst->hwndInput = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
            WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
            5, h - 35, w - 95, 28,
            hwnd, (HMENU)IDC_POPCHAT_INPUT, g_hInstance, nullptr);
        SendMessageW(inst->hwndInput, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
        
        // Send button
        inst->hwndSend = CreateWindowExW(0, L"BUTTON", L"Send",
            WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
            w - 85, h - 35, 80, 28,
            hwnd, (HMENU)IDC_POPCHAT_SEND, g_hInstance, nullptr);
        SendMessageW(inst->hwndSend, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
        
        // Welcome message
        AppendWindowText(inst->hwndHistory, 
            (L"[Chat #" + std::to_wstring(inst->instanceId) + L"] Independent session. Select a model above.\r\n").c_str());
    ...
    case WM_SIZE: {
        ...
        GetClientRect(hwnd, &rc);
        int w = rc.right, h = rc.bottom;
        MoveWindow(inst->hwndModelCombo, 5, 5, w - 10, 200, TRUE);
        MoveWindow(inst->hwndHistory, 5, 35, w - 10, h - 75, TRUE);
        MoveWindow(inst->hwndInput, 5, h - 35, w - 95, 28, TRUE);
        MoveWindow(inst->hwndSend, w - 85, h - 35, 80, 28, TRUE);
        break;
    }
    ...
    case WM_COMMAND: {
        ...
        if (LOWORD(wParam) == IDC_POPCHAT_SEND) {
            int len = GetWindowTextLengthW(inst->hwndInput);
            if (len <= 0) break;
            std::wstring userMsg(len + 1, 0);
            GetWindowTextW(inst->hwndInput, &userMsg[0], len + 1);
            userMsg.resize(len);
            
            // Display user message
            AppendWindowText(inst->hwndHistory, (L"\r\nYou: " + userMsg + L"\r\n").c_str());
            SetWindowTextW(inst->hwndInput, L"");
            
            // Store in session
            inst->session.messages.push_back(L"You: " + userMsg);
            
            // Queue inference - result goes to output (not to other chat windows)
            QueueInferenceRequest(userMsg, g_settings.maxTokens, g_settings.temperature,
                                  nullptr, 0);
            
            AppendWindowText(inst->hwndHistory, L"AI: [Processing...]\r\n");
        }
        break;
    }
    ...
    case WM_DESTROY: {
        if (inst) {
            for (auto it = g_popOutChats.begin(); it != g_popOutChats.end(); ++it) {
                if (it->instanceId == inst->instanceId) {
                    g_popOutChats.erase(it);
                    break;
                }
            }
        }
...
void PopOutChat() {
    ...
    if (!classRegistered) {
        WNDCLASSEXW wc = {};
        wc.cbSize = sizeof(wc);
        wc.lpfnWndProc = PopOutChatWndProc;
        wc.hInstance = g_hInstance;
        wc.hCursor = LoadCursor(nullptr, IDC_ARROW);
        wc.hbrBackground = (HBRUSH)(COLOR_BTNFACE + 1);
        wc.lpszClassName = L"RawrXD_PopOutChat";
        RegisterClassExW(&wc);
        classRegistered = true;
    }
    
    // Create new instance
    PopOutChatInstance inst = {};
    inst.instanceId = g_nextPopOutId++;
    inst.session.id = L"popout_" + std::to_wstring(inst.instanceId);
    inst.session.title = L"Chat #" + std::to_wstring(inst.instanceId);
    GetSystemTime(&inst.session.created);
    inst.session.lastUsed = inst.session.created;
    
    g_popOutChats.push_back(inst);
    PopOutChatInstance* pInst = &g_popOutChats.back();
    
    // Offset each new window
    int offset = (inst.instanceId - 1) * 30;
    
    std::wstring title = L"RawrXD Chat #" + std::to_wstring(inst.instanceId);
    
    HWND hwndChat = CreateWindowExW(0,
        L"RawrXD_PopOutChat", title.c_str(),
        WS_OVERLAPPEDWINDOW | WS_VISIBLE,
        CW_USEDEFAULT + offset, CW_USEDEFAULT + offset, 420, 550,
        nullptr, nullptr, g_hInstance, pInst);
    
    if (hwndChat) {
        pInst->hwndWindow = hwndChat;
        ShowWindow(hwndChat, SW_SHOW);
        UpdateWindow(hwndChat);
        
        AppendWindowText(g_hwndOutput,
            (L"[Chat] Opened independent pop-out chat #" + std::to_wstring(inst.instanceId) + L"\r\n").c_str());
    }
}
...
// Settings Dialog
// ============================================================================
INT_PTR CALLBACK SettingsDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM) {
    switch (msg) {
    case WM_INITDIALOG:
        return TRUE;
    case WM_COMMAND:
...
void ShowSettingsDialogModal() {
    // Create a simple settings window since we don't have a dialog resource
    static bool classRegistered = false;
    if (!classRegistered) {
        WNDCLASSEXW wc = {};
        wc.cbSize = sizeof(wc);
        ...
        wc.lpszClassName = L"RawrXD_Settings";
        RegisterClassExW(&wc);
        classRegistered = true;
    }
    
    HWND hSettings = CreateWindowExW(WS_EX_DLGMODALFRAME,
        L"RawrXD_Settings", L"RawrXD Settings",
    ...
    CreateWindowExW(0, L"STATIC", L"Font Size:",
        WS_CHILD | WS_VISIBLE, 20, 20, 80, 20, hSettings, nullptr, g_hInstance, nullptr);
    HWND hFontSize = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"16",
        WS_CHILD | WS_VISIBLE | ES_NUMBER, 110, 18, 50, 22, hSettings, nullptr, g_hInstance, nullptr);
    
    // Tab size label + edit
    CreateWindowExW(0, L"STATIC", L"Tab Size:",
        WS_CHILD | WS_VISIBLE, 20, 50, 80, 20, hSettings, nullptr, g_hInstance, nullptr);
    CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"4",
        WS_CHILD | WS_VISIBLE | ES_NUMBER, 110, 48, 50, 22, hSettings, nullptr, g_hInstance, nullptr);
    
    // Theme label
    CreateWindowExW(0, L"STATIC", L"Theme:",
        WS_CHILD | WS_VISIBLE, 20, 80, 80, 20, hSettings, nullptr, g_hInstance, nullptr);
    HWND hTheme = CreateWindowExW(0, WC_COMBOBOXW, L"",
        WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST, 110, 78, 150, 200, hSettings, nullptr, g_hInstance, nullptr);
    SendMessageW(hTheme, CB_ADDSTRING, 0, (LPARAM)L"Dark (Default)");
    SendMessageW(hTheme, CB_ADDSTRING, 0, (LPARAM)L"Light");
    SendMessageW(hTheme, CB_ADDSTRING, 0, (LPARAM)L"High Contrast");
    SendMessageW(hTheme, CB_SETCURSEL, 0, 0);
    
    // Checkboxes
    CreateWindowExW(0, L"BUTTON", L"Enable Minimap",
        WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 20, 115, 200, 20, hSettings, nullptr, g_hInstance, nullptr);
    CreateWindowExW(0, L"BUTTON", L"Enable Code Folding",
        WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 20, 140, 200, 20, hSettings, nullptr, g_hInstance, nullptr);
    CreateWindowExW(0, L"BUTTON", L"Auto-start Chat Server",
        WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 20, 165, 200, 20, hSettings, nullptr, g_hInstance, nullptr);
    
    // OK/Cancel buttons
    HWND hOK = CreateWindowExW(0, L"BUTTON", L"OK",
        WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 240, 280, 80, 28, hSettings, (HMENU)IDOK, g_hInstance, nullptr);
    CreateWindowExW(0, L"BUTTON", L"Cancel",
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 330, 280, 80, 28, hSettings, (HMENU)IDCANCEL, g_hInstance, nullptr);
    
    SetFocus(hFontSize);
    AppendWindowText(g_hwndOutput, L"[Settings] Settings dialog opened.\r\n");
}

// ============================================================================
// Agent DLL Discovery & Autonomous Mode
// ============================================================================
void DiscoverAgentDlls() {
    AppendWindowText(g_hwndOutput, L"[Agent] Scanning for agent DLLs...\r\n");
    
    WIN32_FIND_DATAW fd;
    HANDLE hFind = FindFirstFileW(L"*.dll", &fd);
    int count = 0;
    if (hFind != INVALID_HANDLE_VALUE) {
        do {
            AppendWindowText(g_hwndOutput, L"  Found: ");
            AppendWindowText(g_hwndOutput, fd.cFileName);
            AppendWindowText(g_hwndOutput, L"\r\n");
            count++;
        } while (FindNextFileW(hFind, &fd));
        FindClose(hFind);
    ...
    wchar_t buf[128];
    swprintf_s(buf, L"[Agent] Scan complete. Found %d DLL(s).\r\n", count);
    AppendWindowText(g_hwndOutput, buf);
}

void RunAutonomousMode() {
    int result = MessageBoxW(g_hwndMain,
        L"Autonomous mode will let the AI agent analyze and modify code independently.\n\n"
        L"This requires a loaded AI model. Continue?",
    ...
    if (!g_modelLoaded) {
        MessageBoxW(g_hwndMain,
            L"No AI model is loaded. Use AI > Load Model first.",
            L"RawrXD - Autonomous Mode", MB_OK | MB_ICONWARNING);
        return;
    }
    
    AppendWindowText(g_hwndOutput, L"[Agent] Autonomous mode activated.\r\n");
    AppendWindowText(g_hwndOutput, L"[Agent] Analyzing current file...\r\n");
    
    // Get current editor content and queue analysis
    int len = GetWindowTextLengthW(g_hwndEditor);
    if (len > 0) {
        std::wstring content(len + 1, L'\0');
        GetWindowTextW(g_hwndEditor, &content[0], len + 1);
        content.resize(len);
        QueueInferenceRequest(L"Analyze this code for issues and suggest fixes:\n" + content);
    } else {
        AppendWindowText(g_hwndOutput, L"[Agent] No code in editor to analyze.\r\n");
    }
}
...
// Sourcefile Dropdown
// ============================================================================
static HWND g_hwndSourceDropdown = nullptr;

void CreateSourcefileDropdown(HWND hwndParent) {
    if (g_hwndSourceDropdown) return;
    g_hwndSourceDropdown = CreateWindowExW(0, WC_COMBOBOXW, L"",
        WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | CBS_SORT,
        0, 0, 300, 200, hwndParent, nullptr, g_hInstance, nullptr);
    SendMessageW(g_hwndSourceDropdown, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
    RefreshSourcefileDropdown();
}

void RefreshSourcefileDropdown() {
    if (!g_hwndSourceDropdown) return;
    SendMessageW(g_hwndSourceDropdown, CB_RESETCONTENT, 0, 0);
    
    if (g_workspaceRoot.empty()) return;
    
    // Add source files from workspace
    WIN32_FIND_DATAW fd;
    std::wstring pattern = g_workspaceRoot + L"\\*.*";
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        do {
            ...
            // Only add source/text files
            size_t dot = name.find_last_of(L'.');
            if (dot != std::wstring::npos) {
                std::wstring ext = name.substr(dot);
                for (auto& c : ext) c = towlower(c);
                if (ext == L".cpp" || ext == L".c" || ext == L".h" || ext == L".hpp" ||
                    ext == L".py" || ext == L".js" || ext == L".ts" || ext == L".txt" ||
                    ext == L".md" || ext == L".json" || ext == L".xml" || ext == L".html") {
                    SendMessageW(g_hwndSourceDropdown, CB_ADDSTRING, 0, (LPARAM)name.c_str());
                }
            }
        } while (FindNextFileW(hFind, &fd));
        FindClose(hFind);
...
// ============================================================================
// PRODUCTION FEATURE 1: Quick Open Dialog (Ctrl+P)
// Fuzzy file search across workspace — opens file on Enter/dblclick
// ============================================================================
...
static void CollectAllFiles(const fs::path& root, std::vector<std::wstring>& out, int maxDepth, int depth) {
    ...
    try {
        for (auto& e : fs::directory_iterator(root, fs::directory_options::skip_permission_denied)) {
            ...
            if (fn[0] == L'.') continue;
            if (fn == L"node_modules" || fn == L"__pycache__" || fn == L".git" || fn == L"3rdparty") continue;
            if (e.is_directory()) {
                CollectAllFiles(e.path(), out, maxDepth, depth + 1);
...
static void FilterQuickOpen(HWND hFilter, HWND hList, QuickOpenState* qs) {
    if (!qs) return;
    wchar_t buf[256] = {};
    ...
    qs->filtered.clear();
    for (const auto& p : qs->allFiles) {
        std::wstring lower = p;
        std::transform(lower.begin(), lower.end(), lower.begin(), ::towlower);
        if (q.empty() || lower.find(q) != std::wstring::npos) {
            qs->filtered.push_back(p);
        }
        if (qs->filtered.size() >= 500) break; // cap for performance
    }
    SendMessageW(hList, LB_RESETCONTENT, 0, 0);
    for (const auto& p : qs->filtered) {
        // Show relative path if possible
        std::wstring display = p;
        if (!g_workspaceRoot.empty() && display.find(g_workspaceRoot) == 0) {
            display = display.substr(g_workspaceRoot.size());
            if (!display.empty() && (display[0] == L'\\' || display[0] == L'/')) display = display.substr(1);
        }
...
LRESULT CALLBACK QuickOpenWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    QuickOpenState* qs = (QuickOpenState*)GetWindowLongPtrW(hwnd, GWLP_USERDATA);
    switch (msg) {
    case WM_CREATE: {
        qs = new QuickOpenState();
        SetWindowLongPtrW(hwnd, GWLP_USERDATA, (LONG_PTR)qs);
        
        fs::path root = g_workspaceRoot.empty() ? fs::current_path() : fs::path(g_workspaceRoot);
        CollectAllFiles(root, qs->allFiles, 8, 0);
        std::sort(qs->allFiles.begin(), qs->allFiles.end());
        qs->filtered = qs->allFiles;
        
        RECT rc; GetClientRect(hwnd, &rc);
        int cw = rc.right > 0 ? rc.right : 620;
        int ch = rc.bottom > 0 ? rc.bottom : 440;
        
        HWND hFilter = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
            WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 10, 10, cw - 20, 24,
            hwnd, (HMENU)(UINT_PTR)IDC_QUICKOPEN_INPUT, nullptr, nullptr);
        HWND hList = CreateWindowExW(WS_EX_CLIENTEDGE, L"LISTBOX", L"",
            WS_CHILD | WS_VISIBLE | LBS_NOTIFY | LBS_HASSTRINGS | WS_VSCROLL,
            10, 40, cw - 20, ch - 50,
            hwnd, (HMENU)(UINT_PTR)IDC_QUICKOPEN_LIST, nullptr, nullptr);
        
        if (g_hFontUI) {
            SendMessage(hFilter, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
            SendMessage(hList, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
        }
        ...
        wchar_t title[128];
        swprintf_s(title, L"Quick Open — %d files", (int)qs->allFiles.size());
    ...
    case WM_SIZE: {
        ...
        HWND hF = GetDlgItem(hwnd, IDC_QUICKOPEN_INPUT);
        HWND hL = GetDlgItem(hwnd, IDC_QUICKOPEN_LIST);
        if (hF) SetWindowPos(hF, nullptr, 10, 10, cw - 20, 24, SWP_NOZORDER);
        if (hL) SetWindowPos(hL, nullptr, 10, 40, cw - 20, ch - 50, SWP_NOZORDER);
        break;
    }
    case WM_COMMAND:
        if (LOWORD(wParam) == IDC_QUICKOPEN_INPUT && HIWORD(wParam) == EN_CHANGE && qs) {
            FilterQuickOpen(GetDlgItem(hwnd, IDC_QUICKOPEN_INPUT), GetDlgItem(hwnd, IDC_QUICKOPEN_LIST), qs);
        } else if (LOWORD(wParam) == IDC_QUICKOPEN_LIST && HIWORD(wParam) == LBN_DBLCLK && qs) {
            int sel = (int)SendMessageW(GetDlgItem(hwnd, IDC_QUICKOPEN_LIST), LB_GETCURSEL, 0, 0);
            if (sel >= 0 && sel < (int)qs->filtered.size()) {
                LoadFile(qs->filtered[sel]);
            }
            DestroyWindow(hwnd);
        }
        break;
    case WM_KEYDOWN:
        ...
        if (wParam == VK_RETURN && qs) {
            int sel = (int)SendMessageW(GetDlgItem(hwnd, IDC_QUICKOPEN_LIST), LB_GETCURSEL, 0, 0);
            if (sel >= 0 && sel < (int)qs->filtered.size()) {
                LoadFile(qs->filtered[sel]);
            } else if (!qs->filtered.empty()) {
                LoadFile(qs->filtered[0]);
            }
            DestroyWindow(hwnd);
            return 0;
        }
        break;
    case WM_DESTROY:
        if (qs) { delete qs; SetWindowLongPtrW(hwnd, GWLP_USERDATA, 0); }
        break;
    default:
        return DefWindowProcW(hwnd, msg, wParam, lParam);
    }
...
void ShowQuickOpenDialog() {
    ...
    if (!registered) {
        WNDCLASSEXW wc = {};
        wc.cbSize = sizeof(wc);
        wc.lpfnWndProc = QuickOpenWndProc;
        wc.hInstance = g_hInstance;
        wc.lpszClassName = L"RawrXD_QuickOpen";
        wc.hCursor = LoadCursor(nullptr, IDC_ARROW);
        wc.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
        RegisterClassExW(&wc);
        registered = true;
    }
    RECT r; GetWindowRect(g_hwndMain, &r);
    int w = 640, h = 480;
    int x = r.left + (r.right - r.left - w) / 2;
    int y = r.top + (r.bottom - r.top - h) / 2;
    CreateWindowExW(WS_EX_DLGMODALFRAME, L"RawrXD_QuickOpen", L"Quick Open",
        WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_VISIBLE | WS_SIZEBOX,
        x, y, w, h, g_hwndMain, nullptr, g_hInstance, nullptr);
    AppendWindowText(g_hwndOutput, L"[UI] Quick Open dialog opened.\r\n");
}

// ============================================================================
// PRODUCTION FEATURE 2: Source File Picker / Registry
// Scans workspace, maps every source file to a dynamic menu item
// ============================================================================
bool IsSourceExt(const std::wstring& path) {
    static const wchar_t* exts[] = {
        L".cpp", L".c", L".h", L".hpp", L".asm", L".py", L".ps1",
        L".psm1", L".js", L".ts", L".json", L".xml", L".md", L".txt",
        L".bat", L".cmd", L".sh", L".cmake", nullptr
    };
    std::wstring ext = fs::path(path).extension().wstring();
    for (int i = 0; exts[i]; ++i) {
        if (_wcsicmp(ext.c_str(), exts[i]) == 0) return true;
    }
    return false;
}
...
HMENU BuildSourceFileMenu() {
    g_sourceFileRegistry.clear();
    g_nextSrcFileId = IDM_SRCFILE_BASE;
    
    if (g_workspaceRoot.empty()) return nullptr;
    
    std::vector<std::wstring> files;
    CollectSourceFiles(fs::path(g_workspaceRoot), files, 6, 0);
    std::sort(files.begin(), files.end());
    
    // Build hierarchical menu by directory
    HMENU hRoot = CreatePopupMenu();
    std::map<std::wstring, HMENU> dirMenus;
    
    for (const auto& f : files) {
        if (g_nextSrcFileId > IDM_SRCFILE_MAX) break;
        
        std::wstring rel = f;
        if (rel.find(g_workspaceRoot) == 0) {
            rel = rel.substr(g_workspaceRoot.size());
            if (!rel.empty() && (rel[0] == L'\\' || rel[0] == L'/')) rel = rel.substr(1);
        }
        
        SourceFileEntry entry;
        entry.relativePath = rel;
        entry.absolutePath = f;
        entry.menuId = g_nextSrcFileId++;
        g_sourceFileRegistry.push_back(entry);
        
        // Find or create submenu for directory
        fs::path relPath(rel);
        std::wstring dir = relPath.has_parent_path() ? relPath.parent_path().wstring() : L"";
        
        HMENU targetMenu = hRoot;
        if (!dir.empty()) {
            if (dirMenus.find(dir) == dirMenus.end()) {
                HMENU sub = CreatePopupMenu();
                dirMenus[dir] = sub;
                // Attach to root (simplified — flat directory list)
                std::wstring dirName = fs::path(dir).filename().wstring();
                if (dirName.empty()) dirName = dir;
                AppendMenuW(hRoot, MF_POPUP, (UINT_PTR)sub, dirName.c_str());
            }
            targetMenu = dirMenus[dir];
        }
        
        std::wstring fileName = relPath.filename().wstring();
        AppendMenuW(targetMenu, MF_STRING, entry.menuId, fileName.c_str());
    }
    
    if (files.empty()) {
        AppendMenuW(hRoot, MF_STRING | MF_GRAYED, 0, L"(no source files found)");
    }
    
    return hRoot;
}

// Source File Picker window (like production SourceFilePicker)
struct PickerState {
    std::vector<std::wstring> paths;
    std::vector<std::wstring> filtered;
};

static void FilterPicker(HWND hFilter, HWND hList, PickerState* ps) {
    if (!ps) return;
    wchar_t buf[256] = {};
    GetWindowTextW(hFilter, buf, 255);
    std::wstring q(buf);
...
LRESULT CALLBACK SourceFilePickerWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        std::sort(ps->paths.begin(), ps->paths.end());
        ...
        CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
            WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 10, 10, cw - 20, 24,
            hwnd, (HMENU)(UINT_PTR)IDC_SF_FILTER, nullptr, nullptr);
        CreateWindowExW(WS_EX_CLIENTEDGE, L"LISTBOX", L"",
            WS_CHILD | WS_VISIBLE | LBS_NOTIFY | LBS_HASSTRINGS | WS_VSCROLL,
            10, 40, cw - 20, ch - 100,
            hwnd, (HMENU)(UINT_PTR)IDC_SF_LIST, nullptr, nullptr);
        CreateWindowExW(0, L"BUTTON", L"Open", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON,
            cw - 180, ch - 50, 80, 28, hwnd, (HMENU)(UINT_PTR)IDC_SF_OK, nullptr, nullptr);
        CreateWindowExW(0, L"BUTTON", L"Cancel", WS_CHILD | WS_VISIBLE,
            cw - 90, ch - 50, 80, 28, hwnd, (HMENU)(UINT_PTR)IDC_SF_CANCEL, nullptr, nullptr);
        ...
        wchar_t title[128];
        swprintf_s(title, L"Source File Browser — %d files", (int)ps->paths.size());
        SetWindowTextW(hwnd, title);
        SetFocus(GetDlgItem(hwnd, IDC_SF_FILTER));
        break;
    }
    case WM_SIZE: {
        int cw = LOWORD(lParam), ch = HIWORD(lParam);
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == IDC_SF_FILTER && HIWORD(wParam) == EN_CHANGE && ps) {
            FilterPicker(GetDlgItem(hwnd, IDC_SF_FILTER), GetDlgItem(hwnd, IDC_SF_LIST), ps);
            ...
            if (sel >= 0 && sel < (int)ps->filtered.size()) {
                LoadFile(ps->filtered[sel]);
            }
            DestroyWindow(hwnd);
...
void ShowSourceFilePicker() {
    ...
    int x = r.left + (r.right - r.left - w) / 2;
    int y = r.top + (r.bottom - r.top - h) / 2;
    CreateWindowExW(WS_EX_DLGMODALFRAME, L"RawrXD_SourceFilePicker",
        L"Source File Browser", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_VISIBLE | WS_SIZEBOX,
        x, y, w, h, g_hwndMain, nullptr, g_hInstance, nullptr);
    AppendWindowText(g_hwndOutput, L"[UI] Source File Picker opened.\r\n");
}

// ============================================================================
// PRODUCTION FEATURE 3: AI Backend Switcher
// Runtime switching between Local GGUF, Ollama, OpenAI, Claude, Gemini
// ============================================================================
void InitBackendManager() {
    if (g_backendManagerInit) return;
    
    auto& local = g_backendConfigs[(size_t)AIBackendType::LocalGGUF];
    local.type = AIBackendType::LocalGGUF; local.name = "Local GGUF";
    local.endpoint = ""; local.model = ""; local.apiKey = "";
    local.enabled = true; local.timeoutMs = 60000; local.maxTokens = 2048; local.temperature = 0.7f;
    
    auto& ollama = g_backendConfigs[(size_t)AIBackendType::Ollama];
    ollama.type = AIBackendType::Ollama; ollama.name = "Ollama";
    ollama.endpoint = "http://localhost:11434"; ollama.model = "llama3.2"; ollama.apiKey = "";
    ollama.enabled = true; ollama.timeoutMs = 30000; ollama.maxTokens = 2048; ollama.temperature = 0.7f;
    
    auto& openai = g_backendConfigs[(size_t)AIBackendType::OpenAI];
    openai.type = AIBackendType::OpenAI; openai.name = "OpenAI";
    openai.endpoint = "https://api.openai.com"; openai.model = "gpt-4o"; openai.apiKey = "";
    openai.enabled = false; openai.timeoutMs = 30000; openai.maxTokens = 4096; openai.temperature = 0.7f;
    
    auto& claude = g_backendConfigs[(size_t)AIBackendType::Claude];
    claude.type = AIBackendType::Claude; claude.name = "Claude";
    claude.endpoint = "https://api.anthropic.com"; claude.model = "claude-sonnet-4-20250514"; claude.apiKey = "";
    claude.enabled = false; claude.timeoutMs = 30000; claude.maxTokens = 4096; claude.temperature = 0.7f;
    
    auto& gemini = g_backendConfigs[(size_t)AIBackendType::Gemini];
    gemini.type = AIBackendType::Gemini; gemini.name = "Gemini";
    gemini.endpoint = "https://generativelanguage.googleapis.com"; gemini.model = "gemini-2.0-flash"; gemini.apiKey = "";
    gemini.enabled = false; gemini.timeoutMs = 30000; gemini.maxTokens = 4096; gemini.temperature = 0.7f;
    
    g_activeBackend = AIBackendType::LocalGGUF;
    g_backendManagerInit = true;
    AppendWindowText(g_hwndOutput, L"[BackendSwitcher] Initialized — active: Local GGUF\r\n");
}

std::string GetActiveBackendName() {
    if (!g_backendManagerInit) InitBackendManager();
    return g_backendConfigs[(size_t)g_activeBackend].name;
}
...
void SetActiveBackend(AIBackendType type) {
    ...
    const auto& cfg = g_backendConfigs[(size_t)type];
    if (!cfg.enabled && type != AIBackendType::LocalGGUF && type != AIBackendType::Ollama) {
        std::wstring msg = L"[BackendSwitcher] Cannot switch to " + Utf8ToWide(cfg.name) + L" — no API key set.\r\n"
                          L"  Use AI > Switch Backend > Set API Key to configure.\r\n";
        AppendWindowText(g_hwndOutput, msg.c_str());
        MessageBoxW(g_hwndMain, (L"Backend '" + Utf8ToWide(cfg.name) + L"' requires an API key.\nUse AI > Switch Backend > Set API Key.").c_str(),
                    L"Backend Not Configured", MB_OK | MB_ICONWARNING);
        return;
    }
    
    std::lock_guard<std::mutex> lock(g_backendMutex);
    AIBackendType prev = g_activeBackend;
    g_activeBackend = type;
    
    std::wstring msg = L"[BackendSwitcher] Switched from " + Utf8ToWide(g_backendConfigs[(size_t)prev].name) +
                       L" to " + Utf8ToWide(cfg.name) + L" (model: " + Utf8ToWide(cfg.model) + L")\r\n";
    AppendWindowText(g_hwndOutput, msg.c_str());
    
    // Update status bar
    if (g_hwndStatusBar) {
        std::wstring status = L"Backend: " + Utf8ToWide(cfg.name);
        SendMessageW(g_hwndStatusBar, SB_SETTEXTW, 2, (LPARAM)status.c_str());
    }
}
...
void ShowBackendSwitcherDialog() {
    ...
    std::wstring status = L"[BackendSwitcher] Status:\r\n";
    for (size_t i = 0; i < (size_t)AIBackendType::Count; ++i) {
        const auto& cfg = g_backendConfigs[i];
        bool isActive = ((AIBackendType)i == g_activeBackend);
        status += isActive ? L"  ▶ " : L"    ";
        status += Utf8ToWide(cfg.name);
        status += cfg.enabled ? L" [enabled]" : L" [DISABLED]";
        if (!cfg.endpoint.empty()) status += L" — " + Utf8ToWide(cfg.endpoint);
        if (!cfg.model.empty()) status += L" (" + Utf8ToWide(cfg.model) + L")";
        status += L"\r\n";
    }
    AppendWindowText(g_hwndOutput, status.c_str());
    
    if (g_hwndChatHistory) {
        AppendWindowText(g_hwndChatHistory, status.c_str());
    }
}
...
void ShowApiKeyDialog(AIBackendType type) {
    ...
    wchar_t apiKey[512] = {};
    std::wstring prompt = L"Enter API key for " + Utf8ToWide(g_backendConfigs[(size_t)type].name) + L":";
    
    // Use a simple dialog approach
    HWND hDlg = CreateWindowExW(WS_EX_DLGMODALFRAME | WS_EX_TOPMOST, L"STATIC", prompt.c_str(),
    ...
    if (hDlg) {
        HWND hEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
            ...
            10, 40, 470, 24, hDlg, (HMENU)1, nullptr, nullptr);
        HWND hOk = CreateWindowExW(0, L"BUTTON", L"OK",
            WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON,
            300, 80, 80, 28, hDlg, (HMENU)IDOK, nullptr, nullptr);
        HWND hCancel = CreateWindowExW(0, L"BUTTON", L"Cancel",
            WS_CHILD | WS_VISIBLE, 390, 80, 80, 28, hDlg, (HMENU)IDCANCEL, nullptr, nullptr);
        ...
        while (!done && GetMessageW(&msg, nullptr, 0, 0)) {
            if (msg.message == WM_COMMAND) {
                if (LOWORD(msg.wParam) == IDOK) {
                    GetWindowTextW(hEdit, apiKey, 511);
                    std::string key = WideToUtf8(apiKey);
                    if (!key.empty()) {
                        std::lock_guard<std::mutex> lock(g_backendMutex);
                        g_backendConfigs[(size_t)type].apiKey = key;
                        g_backendConfigs[(size_t)type].enabled = true;
                        AppendWindowText(g_hwndOutput, (L"[BackendSwitcher] API key set for " + 
                            Utf8ToWide(g_backendConfigs[(size_t)type].name) + L"\r\n").c_str());
                    }
                    done = true;
...
// HTTP POST helper using WinHTTP (from production BackendSwitcher)
std::string HttpPost(const std::string& url, const std::string& body,
                     const std::vector<std::string>& headers, int timeoutMs) {
...
// Per-backend routing
std::string RouteToLocalGGUF(const std::string& prompt) {
    if (!g_modelLoaded) return "[Error] No local GGUF model loaded. Use File > Load Model.";
    // Delegate to existing inference pipeline
    QueueInferenceRequest(Utf8ToWide(prompt), 512, 0.5f);
    return "[LocalGGUF] Inference request queued.";
}

std::string RouteToOllama(const std::string& prompt) {
    const auto& cfg = g_backendConfigs[(size_t)AIBackendType::Ollama];
    if (cfg.endpoint.empty()) return "[Error] Ollama endpoint not configured.";
    
    std::string reqBody = "{\"model\":\"" + cfg.model + "\",\"prompt\":\"" + prompt + "\",\"stream\":false}";
    std::string resp = HttpPost(cfg.endpoint + "/api/generate", reqBody, {"Content-Type: application/json"}, cfg.timeoutMs);
    
    // Parse response (simple extraction)
    size_t pos = resp.find("\"response\":\"");
    if (pos != std::string::npos) {
        pos += 12;
        size_t end = resp.find("\"", pos);
        if (end != std::string::npos) return resp.substr(pos, end - pos);
    }
    if (resp.empty()) return "[Error] Ollama not responding at " + cfg.endpoint;
    return resp;
}

std::string RouteToOpenAI(const std::string& prompt) {
    const auto& cfg = g_backendConfigs[(size_t)AIBackendType::OpenAI];
    if (cfg.apiKey.empty()) return "[Error] OpenAI API key not set.";
    
    std::string reqBody = "{\"model\":\"" + cfg.model + "\",\"max_tokens\":" + std::to_string(cfg.maxTokens) +
    ...
    std::string resp = HttpPost(cfg.endpoint + "/v1/chat/completions", reqBody, hdrs, cfg.timeoutMs);
    ...
    if (resp.empty()) return "[Error] OpenAI API not responding.";
    return resp;
}

std::string RouteToClaude(const std::string& prompt) {
    const auto& cfg = g_backendConfigs[(size_t)AIBackendType::Claude];
    if (cfg.apiKey.empty()) return "[Error] Claude API key not set.";
    ...
    std::string resp = HttpPost(cfg.endpoint + "/v1/messages", reqBody, hdrs, cfg.timeoutMs);
    ...
    if (resp.empty()) return "[Error] Claude API not responding.";
    return resp;
}

std::string RouteToGemini(const std::string& prompt) {
    const auto& cfg = g_backendConfigs[(size_t)AIBackendType::Gemini];
    if (cfg.apiKey.empty()) return "[Error] Gemini API key not set.";
    
    std::string reqBody = "{\"contents\":[{\"parts\":[{\"text\":\"" + prompt + "\"}]}],"
        "\"generationConfig\":{\"temperature\":" + std::to_string(cfg.temperature) +
        ",\"maxOutputTokens\":" + std::to_string(cfg.maxTokens) + "}}";
    
    std::string url = cfg.endpoint + "/v1beta/models/" + cfg.model + ":generateContent?key=" + cfg.apiKey;
    std::string resp = HttpPost(url, reqBody, {"Content-Type: application/json"}, cfg.timeoutMs);
    ...
    if (resp.empty()) return "[Error] Gemini API not responding.";
    return resp;
}
...
std::string RouteInferenceRequest(const std::string& prompt) {
    ...
    { std::lock_guard<std::mutex> lock(g_backendMutex); active = g_activeBackend; }
    
    switch (active) {
        case AIBackendType::LocalGGUF: return RouteToLocalGGUF(prompt);
        case AIBackendType::Ollama:    return RouteToOllama(prompt);
        case AIBackendType::OpenAI:    return RouteToOpenAI(prompt);
        case AIBackendType::Claude:    return RouteToClaude(prompt);
        case AIBackendType::Gemini:    return RouteToGemini(prompt);
        default: return "[Error] Unknown backend.";
    }
}

// ============================================================================
// PRODUCTION FEATURE 4: Real-Time Streaming UX (Progress Bar)
// ============================================================================
void ShowProgressBar(const std::wstring& operation) {
    g_operationActive.store(true);
    g_operationCancelled.store(false);
    g_progressPercent.store(0.0f);
    { std::lock_guard<std::mutex> lock(g_progressMutex); g_progressStatus = WideToUtf8(operation); }
    
    if (!g_hwndMain) return;
    
    RECT mainRC; GetClientRect(g_hwndMain, &mainRC);
    int barHeight = 32;
    
    if (!g_hwndProgressContainer) {
        g_hwndProgressContainer = CreateWindowExW(0, L"STATIC", L"",
            WS_CHILD | WS_VISIBLE, 0, 0, mainRC.right, barHeight,
            g_hwndMain, nullptr, g_hInstance, nullptr);
    }
    SetWindowPos(g_hwndProgressContainer, HWND_TOP, 0, 0, mainRC.right, barHeight, SWP_SHOWWINDOW);
    
    if (!g_hwndProgressBar) {
        g_hwndProgressBar = CreateWindowExW(0, PROGRESS_CLASSW, L"",
            WS_CHILD | WS_VISIBLE | PBS_SMOOTH, 8, 4, mainRC.right - 180, 24,
            g_hwndProgressContainer, nullptr, g_hInstance, nullptr);
        SendMessage(g_hwndProgressBar, PBM_SETBARCOLOR, 0, (LPARAM)RGB(0, 122, 204));
        SendMessage(g_hwndProgressBar, PBM_SETBKCOLOR, 0, (LPARAM)RGB(37, 37, 38));
        SendMessage(g_hwndProgressBar, PBM_SETRANGE32, 0, 1000);
    }
    SendMessage(g_hwndProgressBar, PBM_SETPOS, 0, 0);
    
    if (!g_hwndProgressLabel) {
        g_hwndProgressLabel = CreateWindowExW(0, L"STATIC", operation.c_str(),
            WS_CHILD | WS_VISIBLE | SS_LEFT | SS_CENTERIMAGE, 12, 4, mainRC.right - 184, 24,
            g_hwndProgressContainer, nullptr, g_hInstance, nullptr);
        if (g_hFontUI) SendMessage(g_hwndProgressLabel, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
    }
    SetWindowTextW(g_hwndProgressLabel, operation.c_str());
    
    if (!g_hwndProgressCancel) {
        g_hwndProgressCancel = CreateWindowExW(0, L"BUTTON", L"Cancel",
            WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, mainRC.right - 160, 4, 80, 24,
            g_hwndProgressContainer, (HMENU)(UINT_PTR)IDC_PROGRESS_CANCEL, g_hInstance, nullptr);
        if (g_hFontUI) SendMessage(g_hwndProgressCancel, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
    }
    
    ShowWindow(g_hwndProgressContainer, SW_SHOW);
    AppendWindowText(g_hwndOutput, (L"[Progress] " + operation + L"\r\n").c_str());
}

void UpdateProgress(float percent, const std::wstring& status) {
    g_progressPercent.store(percent);
    { std::lock_guard<std::mutex> lock(g_progressMutex); g_progressStatus = WideToUtf8(status); }
    
    if (g_hwndProgressBar) SendMessage(g_hwndProgressBar, PBM_SETPOS, (WPARAM)(int)(percent * 10.0f), 0);
    if (g_hwndProgressLabel) SetWindowTextW(g_hwndProgressLabel, status.c_str());
}

void HideProgressBar() {
    g_operationActive.store(false);
    if (g_hwndProgressContainer) ShowWindow(g_hwndProgressContainer, SW_HIDE);
}

void CancelOperation() {
    if (!g_operationActive.load()) return;
    g_operationCancelled.store(true);
    if (g_hwndProgressLabel) SetWindowTextW(g_hwndProgressLabel, L"Cancelling...");
    if (g_hwndProgressCancel) EnableWindow(g_hwndProgressCancel, FALSE);
    AppendWindowText(g_hwndOutput, L"[Progress] Operation cancelled.\r\n");
}

// ============================================================================
// PRODUCTION FEATURE 5: Inline Edit Mode
// Select code, press Ctrl+I, type instruction, AI applies edit
// ============================================================================
void StartInlineEdit() {
    if (!g_hwndEditor || g_inlineEditActive) return;
    
    DWORD selStart = 0, selEnd = 0;
    SendMessage(g_hwndEditor, EM_GETSEL, (WPARAM)&selStart, (LPARAM)&selEnd);
    
    if (selStart == selEnd) {
        AppendWindowText(g_hwndOutput, L"[InlineEdit] Select code first, then press Ctrl+I.\r\n");
        return;
    }
    
    g_inlineEditSelStart = selStart;
    g_inlineEditSelEnd = selEnd;
    g_inlineEditActive = true;
    
    // Get position of selection in screen coords
    POINT pt;
    SendMessage(g_hwndEditor, EM_POSFROMCHAR, (WPARAM)&pt, selEnd);
    ClientToScreen(g_hwndEditor, &pt);
    ScreenToClient(g_hwndMain, &pt);
    
    // Create inline edit overlay
    if (g_hwndInlineEditOverlay) DestroyWindow(g_hwndInlineEditOverlay);
    
    int inputW = 400, inputH = 28;
    g_hwndInlineEditOverlay = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
        pt.x, pt.y + 20, inputW, inputH,
        g_hwndMain, (HMENU)(UINT_PTR)IDC_INLINE_EDIT_INPUT, g_hInstance, nullptr);
    
    if (g_hFontUI) SendMessage(g_hwndInlineEditOverlay, WM_SETFONT, (WPARAM)g_hFontUI, TRUE);
    
    // Set placeholder-like text
    SendMessageW(g_hwndInlineEditOverlay, EM_SETCUEBANNER, TRUE, (LPARAM)L"Describe the edit (Enter to apply, Esc to cancel)...");
    SetFocus(g_hwndInlineEditOverlay);
    
    // Subclass the input to catch Enter/Escape
    SetWindowSubclass(g_hwndInlineEditOverlay, [](HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR, DWORD_PTR) -> LRESULT {
        if (msg == WM_KEYDOWN) {
            if (wParam == VK_RETURN) {
                wchar_t buf[1024] = {};
    ...
    AppendWindowText(g_hwndOutput, L"[InlineEdit] Type your edit instruction and press Enter.\r\n");
}
...
void ApplyInlineEdit(const std::wstring& instruction) {
    ...
    // Get selected text
    int len = g_inlineEditSelEnd - g_inlineEditSelStart;
    std::vector<wchar_t> buf(len + 1, 0);
    SendMessage(g_hwndEditor, EM_SETSEL, g_inlineEditSelStart, g_inlineEditSelEnd);
    
    // Route to active backend
    std::string selectedText = WideToUtf8(std::wstring(buf.data()));
    std::string prompt = "Apply this edit to the selected code: " + WideToUtf8(instruction) +
                         "\n\nSelected code:\n" + selectedText + "\n\nReturn only the modified code:";
    
    // Send to backend asynchronously
    ShowProgressBar(L"Applying inline edit...");
    
    std::thread([prompt]() {
        std::string result = RouteInferenceRequest(prompt);
        // Post result back to main thread
        PostMessage(g_hwndMain, WM_USER + 50, 0, 0);
        
        std::wstring wResult = Utf8ToWide(result);
        AppendWindowText(g_hwndOutput, (L"[InlineEdit] Result:\r\n" + wResult + L"\r\n").c_str());
        HideProgressBar();
    }).detach();
...
void CancelInlineEdit() {
    g_inlineEditActive = false;
    if (g_hwndInlineEditOverlay) {
        DestroyWindow(g_hwndInlineEditOverlay);
        g_hwndInlineEditOverlay = nullptr;
    }
    SetFocus(g_hwndEditor);
}

// ============================================================================
// PRODUCTION FEATURE 6: Unrestricted Terminal (Real subprocess)
// Creates cmd.exe/powershell with stdin/stdout pipes
// ============================================================================
void CreateRealTerminal() {
    if (g_termRunning.load()) return;
    ...
    if (!CreatePipe(&g_hTermStdOutRead, &g_hTermStdOutWrite, &sa, 0) ||
        !CreatePipe(&g_hTermStdErrRead, &g_hTermStdErrWrite, &sa, 0) ||
        !CreatePipe(&g_hTermStdInRead, &g_hTermStdInWrite, &sa, 0)) {
        AppendWindowText(g_hwndOutput, L"[Terminal] Failed to create pipes.\r\n");
        return;
    }
    
    SetHandleInformation(g_hTermStdOutRead, HANDLE_FLAG_INHERIT, 0);
    SetHandleInformation(g_hTermStdErrRead, HANDLE_FLAG_INHERIT, 0);
    SetHandleInformation(g_hTermStdInWrite, HANDLE_FLAG_INHERIT, 0);
    
    STARTUPINFOA si = {};
    si.cb = sizeof(si);
    si.dwFlags = STARTF_USESTDHANDLES;
    si.hStdInput = g_hTermStdInRead;
    si.hStdOutput = g_hTermStdOutWrite;
    si.hStdError = g_hTermStdErrWrite;
    
    PROCESS_INFORMATION pi = {};
    
    // Try pwsh.exe first, fall back to powershell.exe, then cmd.exe
    char cmd[MAX_PATH] = {};
    char pwshPath[MAX_PATH];
    if (SearchPathA(nullptr, "pwsh.exe", nullptr, MAX_PATH, pwshPath, nullptr)) {
        strcpy_s(cmd, "pwsh.exe -NoExit -NoLogo -NoProfile");
    } else {
        strcpy_s(cmd, "powershell.exe -NoExit -NoLogo -NoProfile");
    }
    
    if (!CreateProcessA(nullptr, cmd, nullptr, nullptr, TRUE, CREATE_NO_WINDOW, nullptr, nullptr, &si, &pi)) {
        AppendWindowText(g_hwndOutput, L"[Terminal] Failed to create subprocess.\r\n");
        return;
    }
    
    g_hTermProcess = pi.hProcess;
    g_hTermThread = pi.hThread;
    g_termRunning.store(true);
    
    // Close child-side pipe handles
    CloseHandle(g_hTermStdOutWrite); g_hTermStdOutWrite = nullptr;
    CloseHandle(g_hTermStdErrWrite); g_hTermStdErrWrite = nullptr;
    CloseHandle(g_hTermStdInRead); g_hTermStdInRead = nullptr;
    
    // Start reader threads
    g_termOutputThread = std::thread(TerminalReadOutputThread);
    g_termErrorThread = std::thread(TerminalReadErrorThread);
    g_termMonitorThread = std::thread(TerminalMonitorThread);
    
    // Make terminal panel an editable input field
    if (g_hwndTerminal) {
        // Remove ES_READONLY to allow typing
        LONG_PTR style = GetWindowLongPtrW(g_hwndTerminal, GWL_STYLE);
        style &= ~ES_READONLY;
        SetWindowLongPtrW(g_hwndTerminal, GWL_STYLE, style);
        SetWindowTextW(g_hwndTerminal, L"");
        
        // Subclass terminal for Enter key
        SetWindowSubclass(g_hwndTerminal, [](HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR, DWORD_PTR) -> LRESULT {
            if (msg == WM_KEYDOWN && wParam == VK_RETURN) {
                // Get last line as command
                int lineCount = (int)SendMessage(hwnd, EM_GETLINECOUNT, 0, 0);
                if (lineCount > 0) {
                    int lineIdx = lineCount - 1;
                    int lineLen = (int)SendMessage(hwnd, EM_LINELENGTH, SendMessage(hwnd, EM_LINEINDEX, lineIdx, 0), 0);
                    if (lineLen > 0) {
                        std::vector<wchar_t> buf(lineLen + 2, 0);
                        *(WORD*)buf.data() = (WORD)(lineLen + 1);
                        SendMessageW(hwnd, EM_GETLINE, lineIdx, (LPARAM)buf.data());
                        std::string cmd = WideToUtf8(std::wstring(buf.data(), lineLen));
                        cmd += "\n";
                        TerminalWriteInput(cmd);
                    }
                }
                return 0;
            }
    ...
    AppendWindowText(g_hwndOutput, L"[Terminal] Real terminal subprocess started.\r\n");
    
    // Show terminal panel
    g_bTerminalVisible = true;
    CheckMenuItem(GetMenu(g_hwndMain), ID_VIEW_TERMINAL, MF_CHECKED);
    SendMessage(g_hwndMain, WM_SIZE, 0, 0);
}

void TerminalWriteInput(const std::string& data) {
    if (!g_termRunning.load() || !g_hTermStdInWrite) return;
    DWORD written;
    WriteFile(g_hTermStdInWrite, data.c_str(), (DWORD)data.size(), &written, nullptr);
}

void TerminalReadOutputThread() {
    char buffer[4096];
    DWORD bytesRead;
    while (g_termRunning.load()) {
        if (ReadFile(g_hTermStdOutRead, buffer, sizeof(buffer) - 1, &bytesRead, nullptr) && bytesRead > 0) {
            ...
            // Post to UI thread
            if (g_hwndTerminal) {
                PostMessage(g_hwndMain, WM_USER + 51, 0, 0);
                AppendWindowText(g_hwndTerminal, wOut.c_str());
            }
        } else {
            break;
        }
...
void TerminalReadErrorThread() {
    ...
    DWORD bytesRead;
    while (g_termRunning.load()) {
        if (ReadFile(g_hTermStdErrRead, buffer, sizeof(buffer) - 1, &bytesRead, nullptr) && bytesRead > 0) {
            buffer[bytesRead] = '\0';
            std::wstring wErr = Utf8ToWide(std::string(buffer, bytesRead));
            if (g_hwndTerminal) {
                AppendWindowText(g_hwndTerminal, wErr.c_str());
            }
        } else {
            break;
        }
...
void TerminalMonitorThread() {
    WaitForSingleObject(g_hTermProcess, INFINITE);
    g_termRunning.store(false);
    DWORD exitCode;
    GetExitCodeProcess(g_hTermProcess, &exitCode);
    AppendWindowText(g_hwndOutput, L"[Terminal] Process exited.\r\n");
}

void StopRealTerminal() {
    if (g_termRunning.load()) {
        g_termRunning.store(false);
        if (g_hTermProcess) TerminateProcess(g_hTermProcess, 0);
        if (g_termOutputThread.joinable()) g_termOutputThread.join();
        if (g_termErrorThread.joinable()) g_termErrorThread.join();
        if (g_termMonitorThread.joinable()) g_termMonitorThread.join();
        if (g_hTermProcess) { CloseHandle(g_hTermProcess); g_hTermProcess = nullptr; }
        if (g_hTermThread) { CloseHandle(g_hTermThread); g_hTermThread = nullptr; }
        ...
        if (g_hTermStdErrRead) { CloseHandle(g_hTermStdErrRead); g_hTermStdErrRead = nullptr; }
    }
}

// ============================================================================
// PRODUCTION FEATURE 7: Multi-Agent Parallelism
// Dispatches multiple inference tasks in parallel threads
// ============================================================================
void RunMultiAgentTask(const std::string& task) {
    if (!g_backendManagerInit) InitBackendManager();
    
    AppendWindowText(g_hwndOutput, L"[MultiAgent] Starting parallel task dispatch...\r\n");
    ShowProgressBar(L"Multi-Agent: Processing...");
    
    // Split task into sub-tasks (simple approach: by newlines)
    std::vector<std::string> subTasks;
    std::istringstream iss(task);
    std::string line;
    while (std::getline(iss, line)) {
        if (!line.empty()) subTasks.push_back(line);
    }
    
    if (subTasks.empty()) subTasks.push_back(task);
    
    int total = (int)subTasks.size();
    ...
    for (int i = 0; i < total; ++i) {
        threads.emplace_back([i, &subTasks, &results, &completed, total]() {
            results[i] = RouteInferenceRequest(subTasks[i]);
            int done = ++completed;
            ShowAgentProgress(done, total, subTasks[i].substr(0, 50));
        });
    }
    ...
    // Collect results
    AppendWindowText(g_hwndOutput, L"[MultiAgent] All sub-tasks completed:\r\n");
    for (int i = 0; i < total; ++i) {
        std::wstring result = L"  [" + std::to_wstring(i + 1) + L"/" + std::to_wstring(total) + L"] " +
                              Utf8ToWide(results[i].substr(0, 200)) + L"\r\n";
        AppendWindowText(g_hwndOutput, result.c_str());
    }
...
void ShowAgentProgress(int completed, int total, const std::string& current) {
    if (total <= 0) return;
    float pct = (float)completed / (float)total * 100.0f;
    std::wstring status = L"Multi-Agent [" + std::to_wstring(completed) + L"/" + std::to_wstring(total) + L"] " + Utf8ToWide(current);
