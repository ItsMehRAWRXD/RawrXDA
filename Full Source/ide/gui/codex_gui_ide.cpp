...
// Block sizes for each quant type (bytes per block) — polarity-matched with CLI
...
static BFResult BruteForceLoadGUI(const char* path) {
    ...
    for (uint32_t q = 0; q < GGML_TYPE_COUNT; q++) {
        ...
        // Block-size validation — polarity-matched with CLI IDE
        bool valid = true;
        if (q == 4 || q == 5) valid = false; // unused slots
        uint32_t bsz = (q < GGML_TYPE_COUNT) ? QuantBlockSizes[q] : 0;
        if (bsz > 0 && hdr->tensor_count > 0) {
            size_t dataAvail = mf.size / 2;
            if (dataAvail < bsz * hdr->tensor_count) valid = false;
        }
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        case IDM_ANALYZE_PE: {
            if (g_currentPath[0]) {
                ClearOutput();
                AppendOutput("PE BINARY ANALYSIS\r\n");
                AppendOutput("==================\r\n\r\n");
                MappedFile pmf = MapFileRO(g_currentPath);
                if (pmf.base && pmf.size >= 64) {
                    const uint8_t* pb = (const uint8_t*)pmf.base;
                    if (pb[0] == 'M' && pb[1] == 'Z') {
                        uint32_t pe_off = *(const uint32_t*)(pb + 0x3C);
                        if (pe_off + 24 < pmf.size && memcmp(pb + pe_off, "PE\0\0", 4) == 0) {
                            uint16_t machine = *(const uint16_t*)(pb + pe_off + 4);
                            uint16_t sections = *(const uint16_t*)(pb + pe_off + 6);
                            uint16_t optSize = *(const uint16_t*)(pb + pe_off + 20);
                            uint16_t chars = *(const uint16_t*)(pb + pe_off + 22);
                            AppendOutputFmt("[+] Valid PE file\r\n");
                            AppendOutputFmt("    Machine: 0x%04X (%s)\r\n", machine,
                                machine==0x8664?"AMD64":machine==0x014C?"x86":machine==0xAA64?"ARM64":"Other");
                            AppendOutputFmt("    Sections: %u\r\n", sections);
                            AppendOutputFmt("    Characteristics: 0x%04X\r\n", chars);
                            if (chars & 0x0002) AppendOutput("      EXECUTABLE\r\n");
                            if (chars & 0x2000) AppendOutput("      DLL\r\n");
                            if (chars & 0x0020) AppendOutput("      LARGE_ADDRESS_AWARE\r\n");
                            // Optional header
                            if (optSize > 0 && pe_off + 26 < pmf.size) {
                                uint16_t optMagic = *(const uint16_t*)(pb + pe_off + 24);
                                AppendOutputFmt("    Format: %s\r\n", optMagic==0x020B?"PE32+ (64-bit)":optMagic==0x010B?"PE32 (32-bit)":"Unknown");
                                if (pe_off + 24 + 20 < pmf.size) {
                                    uint32_t entry = *(const uint32_t*)(pb + pe_off + 24 + 16);
                                    AppendOutputFmt("    Entry Point: 0x%08X\r\n", entry);
                                }
                            }
                            // Section headers
                            uint32_t secStart = pe_off + 24 + optSize;
                            if (secStart + sections * 40 <= pmf.size) {
                                AppendOutput("\r\n    Sections:\r\n");
                                for (int si = 0; si < sections; si++) {
                                    uint32_t so = secStart + si * 40;
                                    char sname[9] = {};
                                    memcpy(sname, pb + so, 8);
                                    uint32_t vsize = *(const uint32_t*)(pb + so + 8);
                                    uint32_t rva = *(const uint32_t*)(pb + so + 12);
                                    AppendOutputFmt("      %-8s VirtSize=0x%X RVA=0x%X\r\n", sname, vsize, rva);
                                }
                            }
                        } else {
                            AppendOutput("[-] MZ header found but PE signature invalid\r\n");
                        }
                    } else {
                        AppendOutput("[-] Not a PE file\r\n");
                    }
                    // Entropy
                    double pe_ent = ShannonEntropy((const uint8_t*)pmf.base, pmf.size > 65536 ? 65536 : pmf.size);
                    AppendOutputFmt("\r\n[*] Entropy: %.4f bits/byte\r\n", pe_ent);
                    UnmapF(pmf);
                } else {
                    AppendOutput("[-] Cannot open file for PE analysis\r\n");
                    if (pmf.base) UnmapF(pmf);
                }
            } else {
                AppendOutput("[-] No file loaded. Use Load Model first.\r\n");
            }
            break;
        }

        case IDM_TOOLS_ENTROPY: {
            if (g_currentPath[0]) {
                MappedFile emf = MapFileRO(g_currentPath);
                if (emf.base) {
                    double e = ShannonEntropy((const uint8_t*)emf.base, emf.size > 65536 ? 65536 : emf.size);
                    ClearOutput();
                    AppendOutputFmt("Shannon Entropy of %s\r\n", g_currentPath);
                    AppendOutputFmt("Entropy: %.6f bits/byte\r\n\r\n", e);
                    if (e > 7.5) AppendOutput("[!] HIGH — likely compressed/encrypted\r\n");
                    else if (e > 6.0) AppendOutput("[*] MEDIUM — typical binary/model data\r\n");
                    else AppendOutput("[*] LOW — likely text or sparse data\r\n");
                    UnmapF(emf);
                } else {
                    AppendOutputFmt("[-] Cannot open: %s\r\n", g_currentPath);
                }
            } else {
                AppendOutput("[-] No file loaded. Use Load Model first.\r\n");
            }
        ...
        case 1040: {
            ...
            if (g_lastResult.success) {
                OPENFILENAMEA sfn = {};
                char savePath[MAX_PATH] = "codex_report.txt";
                ...
                sfn.lpstrTitle = "Export Model Report";
                sfn.lpstrDefExt = "txt";
                if (GetSaveFileNameA(&sfn)) {
                    FILE* fp = fopen(savePath, "w");
                    if (fp) {
                        fprintf(fp, "CODEX AI REVERSE ENGINE v7.0 — MODEL ANALYSIS REPORT\n");
                        fprintf(fp, "============================================================\n");
                        fprintf(fp, "File: %s\n", g_currentPath);
                        fprintf(fp, "Architecture: %s\n", g_lastResult.architecture);
                        fprintf(fp, "Model Name: %s\n", g_lastResult.model_name[0] ? g_lastResult.model_name : "(unknown)");
                        fprintf(fp, "GGUF Version: %u\n", g_lastResult.gguf_version);
                        fprintf(fp, "Tensors: %u\n", g_lastResult.tensor_count);
                        fprintf(fp, "Vocabulary: %u\n", g_lastResult.vocab_size);
                        fprintf(fp, "Context Len: %u\n", g_lastResult.context_length);
                        fprintf(fp, "Embedding: %u\n", g_lastResult.embedding_dim);
                        fprintf(fp, "Heads: %u\n", g_lastResult.head_count);
                        fprintf(fp, "Layers: %u\n", g_lastResult.layer_count);
                        fprintf(fp, "Quant Type: %s\n", g_lastResult.quant_type < GGML_TYPE_COUNT ? QuantNames[g_lastResult.quant_type] : "unknown");
                        fprintf(fp, "Tokenizer: %s\n", g_lastResult.tokenizer_type < 6 ? TokenizerNames[g_lastResult.tokenizer_type] : "unknown");
                        fprintf(fp, "BF Retries: %u\n", g_lastResult.retry_count);
                        fclose(fp);
                ...
                AppendOutput("[-] No model loaded to export.\r\n");
            }
            break;
        }
...
static HMENU CreateMainMenu() {
    ...
    AppendMenuA(hTools, MF_STRING, IDM_TOOLS_ENTROPY, "Calculate &Entropy\tCtrl+E");
    AppendMenuA(hMenu, MF_POPUP, (UINT_PTR)hTools, "&Tools");

    HMENU hExport = CreatePopupMenu();
    AppendMenuA(hExport, MF_STRING, 1040, "&Export Report\tCtrl+S");
    AppendMenuA(hMenu, MF_POPUP, (UINT_PTR)hExport, "E&xport");
