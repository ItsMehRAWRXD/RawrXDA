...
# Export autonomous agent state to JSON for CLI consumption
function Export-AutonomousAgentState {
    param(
        [string]$OutputPath = $null
    )
    
    $state = $script:AutonomousAgentState | ConvertTo-Json -Depth 10
    
    if ($OutputPath) {
        $state | Out-File -FilePath $OutputPath -Encoding UTF8
        Write-AutonomousAgentLog -Message "Autonomous agent state exported to $OutputPath" -Level Success
    }
    
    return $state
}

# Import autonomous agent state from JSON for CLI restoration
function Import-AutonomousAgentState {
    param(
        [Parameter(Mandatory=$true)][string]$InputPath
    )
    
    if (-not (Test-Path $InputPath)) {
        throw "State file not found: $InputPath"
    }
    
    $importedState = Get-Content -Path $InputPath -Raw | ConvertFrom-Json
    
    # Merge imported state with current state
    $script:AutonomousAgentState.Version = $importedState.Version
    $script:AutonomousAgentState.Status = $importedState.Status
    $script:AutonomousAgentState.Mode = $importedState.Mode
    $script:AutonomousAgentState.Paths = $importedState.Paths
    $script:AutonomousAgentState.IDEDigest = $importedState.IDEDigest
    
    Write-AutonomousAgentLog -Message "Autonomous agent state imported from $InputPath" -Level Success
}
...
function Initialize-AutonomousAgentState {
    ...
    # Load IDE digest if available
    try {
        Import-Module (Join-Path $PSScriptRoot "RawrXD.Win32Deployment.psm1") -Force -ErrorAction Stop
        $script:AutonomousAgentState.IDEDigest = Get-Win32IDEDigest -AsObject
        Write-AutonomousAgentLog -Message "IDE digest loaded for autonomous operations" -Level Info
    } catch {
        Write-AutonomousAgentLog -Message "IDE digest not available: $($_.Exception.Message)" -Level Warning
    }
    ...
    Write-AutonomousAgentLog -Message "Autonomous agent state initialized" -Level Info -Data @{
        ...
        Log = $LogPath
        Backup = $BackupPath
        IDEDigestLoaded = ($null -ne $script:AutonomousAgentState.IDEDigest)
    }
}
...
function Start-SelfAnalysis {
    ...
    try {
        ...
        # Analyze IDE digest if available
        if ($script:AutonomousAgentState.IDEDigest) {
            Write-AutonomousAgentLog -Message "Analyzing IDE digest for autonomous operations" -Level Info
            
            # Extract key subsystems from digest
            $subsystems = $script:AutonomousAgentState.IDEDigest.Subsystems
            
            # Check for critical subsystems
            $criticalSubsystems = @('Win32 window lifecycle', 'Digestion engine', 'Renderer', 'Terminal manager', 'GGUF streaming loader')
            foreach ($subsystem in $criticalSubsystems) {
                if ($subsystems -notcontains $subsystem) {
                    $analysis.MissingFeatures.Add(@{
                        Type = "CriticalSubsystem"
                        Subsystem = $subsystem
                        Severity = "Critical"
                        Details = "Missing critical subsystem: $subsystem"
                    })
                    Write-AutonomousAgentLog -Message "Critical subsystem missing: $subsystem" -Level Critical
                }
            }
            
            # Analyze performance characteristics
            if ($subsystems -contains 'Performance bottlenecks') {
                $analysis.PerformanceBottlenecks.Add(@{
                    Type = "Performance"
                    Source = "IDE Digest"
                    Severity = "Medium"
                    Details = "Performance bottlenecks identified in IDE"
                })
            }
            
            # Check for security surface
            if ($subsystems -contains 'Security surface') {
                $analysis.SecurityVulnerabilities.Add(@{
                    Type = "Security"
                    Source = "IDE Digest"
                    Severity = "High"
                    Details = "Security vulnerabilities identified in IDE"
                })
            }
            
            Write-AutonomousAgentLog -Message "IDE digest analysis complete" -Level Success -Data @{
                SubsystemsFound = $subsystems.Count
                CriticalSubsystems = $criticalSubsystems.Count
            }
        }
