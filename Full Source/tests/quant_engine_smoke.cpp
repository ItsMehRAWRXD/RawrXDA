...
static Logger s_logger("quant_engine_smoke");
...
int main() {
    ...
    if ((int)u5.size() != n) { s_logger.error( "Q5 unpack size mismatch\n"; return 1; }
    float maxErr5 = 0.f;
    for (int i = 0; i < n; ++i) maxErr5 = std::max(maxErr5, fabs(u5[i] - vals[i]));
    s_logger.info("Q5 maxErr=");

    // Q6
    QByteArray q6 = apply_quant(raw, "Q6_K");
    auto u6 = unpack_generic_bits(q6, 6);
    if ((int)u6.size() != n) { s_logger.error( "Q6 unpack size mismatch\n"; return 1; }
    float maxErr6 = 0.f;
    for (int i = 0; i < n; ++i) maxErr6 = std::max(maxErr6, fabs(u6[i] - vals[i]));
    s_logger.info("Q6 maxErr=");

    // F16
    QByteArray f16 = apply_quant(raw, "F16");
    auto u16 = unpack_f16(f16);
    if ((int)u16.size() != n) { s_logger.error( "F16 unpack size mismatch\n"; return 1; }
    float maxErr16 = 0.f;
    for (int i = 0; i < n; ++i) maxErr16 = std::max(maxErr16, fabs(u16[i] - vals[i]));
    s_logger.info("F16 maxErr=");

    // Check error bounds: heuristics based on scale
    if (maxErr5 > 1.5f) {
        s_logger.error( "Q5 error too high" << std::endl;
        return 2;
    }
    if (maxErr6 > 1.0f) {
        s_logger.error( "Q6 error too high" << std::endl;
        return 3;
    }
    if (maxErr16 > 1e-3f * 3.0f) { // product dependant on test range
        s_logger.error( "F16 error too high" << std::endl;
        return 4;
    }

    s_logger.info("quant_engine_smoke: PASS");
    return 0;
}
