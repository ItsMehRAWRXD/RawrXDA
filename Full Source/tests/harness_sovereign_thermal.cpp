...
constexpr const char* kSidecarGlobalName = "Global\\SOVEREIGN_NVME_TEMPS";
constexpr const char* kSidecarLocalName = "Local\\SOVEREIGN_NVME_TEMPS";
constexpr const char* kSidecarBareName = "SOVEREIGN_NVME_TEMPS";
...
bool NvmeSidecarReader::ensureOpen() {
    ...
    bool bareFallback = false;
    mapping_ = OpenFileMappingA(FILE_MAP_READ, FALSE, kSidecarGlobalName);
    if (!mapping_) {
        mapping_ = OpenFileMappingA(FILE_MAP_READ, FALSE, kSidecarLocalName);
        if (!mapping_) {
            mapping_ = OpenFileMappingA(FILE_MAP_READ, FALSE, kSidecarBareName);
            if (!mapping_) {
    ...
    if (bareFallback) {
        status_ = "sidecar_active_bare";
    } else {
        status_ = localFallback ? "sidecar_active_local" : "sidecar_active";
    }
    return true;
}
...
// ════════════════════════════════════════════════════════════════════════════════
// Governor Status Reader (Global\SOVEREIGN_GOVERNOR_STATUS)
// ════════════════════════════════════════════════════════════════════════════════
...
class GovernorSidecarReader {
public:
    ~GovernorSidecarReader();
    bool read(SovereignGovernorStatus& outStatus);
    std::string status() const { return status_; }
...
private:
    ...
    HANDLE mapping_ = nullptr;
    SovereignGovernorStatus* view_ = nullptr;
    std::string status_ = "governor_missing";
};

GovernorSidecarReader::~GovernorSidecarReader() {
    close();
}

void GovernorSidecarReader::close() {
    if (view_) {
        UnmapViewOfFile(view_);
        view_ = nullptr;
    }
...
bool GovernorSidecarReader::ensureOpen() {
    if (view_) {
        return true;
    }

    mapping_ = OpenFileMappingA(FILE_MAP_READ, FALSE, "Global\\SOVEREIGN_GOVERNOR_STATUS");
    if (!mapping_) {
        status_ = "governor_missing";
        return false;
    }

    view_ = static_cast<SovereignGovernorStatus*>(MapViewOfFile(mapping_, FILE_MAP_READ, 0, 0, sizeof(SovereignGovernorStatus)));
    if (!view_) {
        status_ = "governor_unmapped";
        close();
        return false;
    }

    status_ = "governor_active";
    return true;
}

bool GovernorSidecarReader::read(SovereignGovernorStatus& outStatus) {
    if (!ensureOpen()) {
        return false;
    }
    outStatus = *view_;
...
// ════════════════════════════════════════════════════════════════════════════════
// SovereignDriveRanker - wear/thermal weighted ranking + blacklist
// ════════════════════════════════════════════════════════════════════════════════

class SovereignDriveRanker {
public:
    struct DriveStatus {
        int32_t driveId;
        int32_t tempC;     // -1 unknown, < -1 = IOCTL failure
        int32_t wearPct;   // -1 unknown
        double score;
        bool blacklisted;
        std::string blacklistReason;  // "wear", "overheat", "invalid", or combo
    };

    SovereignDriveRanker(double tempWeight, double wearWeight, int32_t maxWearPct, int32_t maxTempC = 70)
        : tempWeight_(tempWeight), wearWeight_(wearWeight), maxWearPct_(maxWearPct), maxTempC_(maxTempC) {}

    std::vector<DriveStatus> rankDrives(const std::vector<ThermalSample>& thermals) const {
        std::vector<DriveStatus> ranked;
        ranked.reserve(thermals.size());

        for (const auto& t : thermals) {
            // Blacklist conditions:
            // 1. Wear level exceeds maxWearPct (end of life)
            // 2. Temperature exceeds maxTempC (thermal throttling)
            // 3. Temperature < -1 (IOCTL failure - not NVMe)
            bool blacklisted = false;
            std::string reason;
            
            if (t.wearPct >= 0 && t.wearPct > maxWearPct_) {
                blacklisted = true;
                reason = "wear";
            }
            if (t.tempC >= maxTempC_) {
                blacklisted = true;
                reason = reason.empty() ? "overheat" : reason + "+overheat";
            }
            if (t.tempC < -1) {
                blacklisted = true;
                reason = "invalid";
            ...
            double effectiveTemp = (t.tempC >= 0) ? static_cast<double>(t.tempC) : 50.0;
            double effectiveWear = (t.wearPct >= 0) ? static_cast<double>(t.wearPct) : 50.0;
            double score = blacklisted ? std::numeric_limits<double>::infinity()
                                       : (tempWeight_ * effectiveTemp + wearWeight_ * effectiveWear);
            ranked.push_back({static_cast<int32_t>(t.driveId), t.tempC, t.wearPct, score, blacklisted, reason});
        }

        std::sort(ranked.begin(), ranked.end(), [](const DriveStatus& a, const DriveStatus& b) {
    ...
    std::string formatRanking(const std::vector<DriveStatus>& ranked, uint64_t cycle) const {
        std::ostringstream log;
        log << "[Governor] Cycle #" << cycle << " | Drive Ranking:";
        for (size_t i = 0; i < ranked.size(); ++i) {
            log << "\n    #" << i << ": Device" << ranked[i].driveId
                << " (Temp:" << ranked[i].tempC << "C Wear:" << ranked[i].wearPct
                << "% Score:" << std::fixed << std::setprecision(1) << ranked[i].score;
            if (ranked[i].blacklisted) {
                log << " BLACKLISTED[" << ranked[i].blacklistReason << "]";
            }
            log << ")";
        }
        return log.str();
...
std::vector<VRAMGovernor::DriveScore> VRAMGovernor::scoreDrives(const std::vector<ThermalSample>& thermals) const {
    ...
    for (const auto& t : thermals) {
        // Blacklist conditions:
        // 1. Wear level exceeds maxWearPct (end of life)
        // 2. Temperature exceeds maxTempC (thermal throttling risk)
        // 3. Temperature is negative (except -1 which means unknown) - indicates IOCTL failure
        bool blacklisted = false;
        std::string blacklistReason;
        
        if (t.wearPct >= 0 && t.wearPct > cfg_.maxWearPct) {
            blacklisted = true;
            blacklistReason = "wear";
        }
        if (t.tempC >= cfg_.maxTempC) {
            blacklisted = true;
            blacklistReason = blacklistReason.empty() ? "overheat" : blacklistReason + "+overheat";
        }
        if (t.tempC < -1) {
            // Negative temps (except -1=unknown) indicate IOCTL failure (not NVMe)
            blacklisted = true;
            blacklistReason = "invalid";
        }
        
        // Effective values for scoring (use defaults for unknowns)
        double temp = (t.tempC >= 0) ? static_cast<double>(t.tempC) : 50.0;
        double wear = (t.wearPct >= 0) ? static_cast<double>(t.wearPct) : 50.0;
        double score = blacklisted ? std::numeric_limits<double>::infinity()
                                   : (temp * cfg_.tempWeight + wear * cfg_.wearWeight);
...
private:
    ...
    GovernorSidecarReader governorSidecar_;
...
void SovereignHarness::stressNVMe() {
    ...
    // Read MMF Governor Status
    uint32_t governorAllowedMask = 0xFFFFFFFF;
    SovereignGovernorStatus govStatus;
    if (governorSidecar_.read(govStatus)) {
        governorAllowedMask = govStatus.allowedMask;
        std::ostringstream gs;
        gs << "governor_mmf_active allowed=" << std::hex << governorAllowedMask 
           << " throttled=" << govStatus.throttledMask << std::dec;
        emit("governor_sync", gs.str());
    } else {
        emit("governor_sync", "governor_mmf_missing");
    }
    
    size_t totalBytesPerDrive = cfg_.nvmeTotalGB * 1024ull * 1024ull * 1024ull;
    
    auto thermals = thermal_.poll();
    SovereignDriveRanker ranker(cfg_.tempWeight, cfg_.wearWeight, cfg_.maxWearPct);
    auto ranked = ranker.rankDrives(thermals);
    std::vector<uint32_t> driveOrder;
    driveOrder.reserve(ranked.size());
    for (const auto& entry : ranked) {
        if (entry.blacklisted) {
            continue;
        }
        driveOrder.push_back(static_cast<uint32_t>(entry.driveId));
    }
    emit("governor_rank", ranker.formatRanking(ranked, ++governorCycle_));
    if (driveOrder.empty()) {
        driveOrder = cfg_.driveIds;
    }

    for (uint32_t driveId : driveOrder) {
        // Enforce Official Governor MMF Policy
        if (!((governorAllowedMask >> driveId) & 1)) {
             std::ostringstream deny;
             deny << "denied_by_sovereign_governor drive=" << driveId;
             emit("nvme_skipped", deny.str());
             continue;
        }
