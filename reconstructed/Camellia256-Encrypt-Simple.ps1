#Simplified Camellia-style encryption (XOR keystream derived from key expansion)
param(
...
$data = [IO.File]::ReadAllBytes($Input)
...
[IO.File]::WriteAllBytes($Output, $cipher)
Write-Host "✓ Encrypted: $Output ($($cipher.Length) bytes)" -ForegroundColor Green
