...
// Stub ASM kernel (delegates to intrinsics for now - ASM impl pending MASM syntax conversion)
#ifndef HAVE_MASM_FLASH_ATTN
extern "C" void flash_attn_asm_avx2(
    const float* Q, const void* K, const float* V, float* O,
    int seqLen, int headDim, int quantType
) {
    // Dequantize Q8_0 K-matrix to FP32
    const BlockQ8_0* K_q8 = (const BlockQ8_0*)K;
    int num_blocks = (seqLen * headDim) / 32;
    std::vector<float> K_fp32(seqLen * headDim);
    
    for (int b = 0; b < num_blocks; ++b) {
        float scale = K_q8[b].scale;
        for (int i = 0; i < 32; ++i) {
            K_fp32[b * 32 + i] = static_cast<float>(K_q8[b].qs[i]) * scale;
        }
    }
    
    // Call intrinsics flash-attention on dequantized K
    flash_attn_forward(Q, K_fp32.data(), V, O, seqLen, headDim, false);
}
#else
// External ASM implementation
extern "C" void flash_attn_asm_avx2(
    const float* Q, const void* K, const float* V, float* O,
    int seqLen, int headDim, int quantType
);
