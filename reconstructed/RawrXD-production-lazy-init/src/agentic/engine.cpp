...
#include <QFileInfo>
#include <algorithm>
#include "../RawrXD-ModelLoader/src/llm_adapter/complete_model_loader_system.h"
...
QString AgenticEngine::generateCode(const QString& prompt) {
    if (m_modelLoader) {
        auto result = m_modelLoader->generateAutonomous(prompt.toStdString(), 512, "quality");
        return QString::fromStdString(result.text);
    }
    
    if (m_inferenceEngine) {
        return m_inferenceEngine->generateSync(prompt, 512);
    }
    
    return "// Error: No model loader or inference engine available";
}
...
QString AgenticEngine::generateFunction(const QString& signature, const QString& description)
{
    ...
    if (!signature.contains("{")) {
        ...
        if (description.contains("validate", Qt::CaseInsensitive)) {
            functionCode += "    if (!isValid()) return false;\n";
            functionCode += "    return true;\n";
        } else if (description.contains("calculate", Qt::CaseInsensitive)) {
            functionCode += "    double result = 0.0;\n";
            functionCode += "    // Perform calculation\n";
            functionCode += "    return result;\n";
        } else {
            functionCode += "    // Add implementation here\n";
        }
        functionCode += "}\n";
    }
...
QString AgenticEngine::generateTests(const QString& code)
{
    ...
    while (matches.hasNext() && testCount < 5) {
        ...
        testCode += "        // Arrange\n";
        testCode += "        // Setup test preconditions\n";
        testCode += "        \n";
        testCode += "        // Act\n";
        testCode += "        auto result = " + functionName + "();\n";
        testCode += "        \n";
        testCode += "        // Assert\n";
        testCode += "        ASSERT_TRUE(result);\n";
        testCode += "    }\n\n";
        testCount++;
    }
...
{
    QString prompt = QString("As an expert AI agent, plan the steps to achieve this goal: %1\nRespond with a bulleted list of actionable steps.").arg(goal);
    QString planText = generateCode(prompt); // Reuse generation logic
    
    QJsonArray plan;
    QStringList lines = planText.split('\n');
    for (const QString& line : lines) {
        QString trimmed = line.trimmed();
        if (trimmed.startsWith('-') || trimmed.startsWith('*') || (trimmed.size() > 0 && trimmed[0].isDigit())) {
            plan.append(trimmed);
        }
    }
    
    if (plan.isEmpty() && !planText.isEmpty()) {
...
void AgenticEngine::setModelLoader(rawr_xd::CompleteModelLoaderSystem* loader) {
