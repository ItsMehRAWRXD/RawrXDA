...
std::vector<uint8_t> TencentCompressionProvider::Compress(
    ...
    for (size_t block_idx = 0; block_idx < num_blocks; ++block_idx) {
        ...
        std::vector<uint8_t> is_nonzero(block_count);
...
void TencentCompressionProvider::QuantizeBlock_Q4_0_AVX512(
    ...
    for (; i <= count - simd_width; i += simd_width) {
        ...
        alignas(64) int32_t temp_q[16];
        _mm512_store_si512(reinterpret_cast<__m512i*>(temp_q), q32);
        for (int j = 0; j < 16; ++j) {
            output[i + j] = static_cast<int8_t>(std::max(-128, std::min(127, temp_q[j])));
        }
    }
