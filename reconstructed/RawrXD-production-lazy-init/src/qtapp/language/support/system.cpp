...
LanguageSupportManager::LanguageSupportManager(QObject* parent)
    : QObject(parent)
{
...
bool LanguageSupportManager::initialize()
{
    ...
    const QVector<LanguageID> lspLanguages = {
    ...
    for (LanguageID id : lspLanguages) {
        if (startLSPServer(id)) {
            successCount++;
            emit languageSupported(id);
        } else {
            emit languageNotSupported(id);
        }
    }
...
bool LanguageSupportManager::startLSPServer(LanguageID id)
{
    ...
    if (!config || config->lspServerCommand.isEmpty()) {
        return false;
    }
    ...
    m_lspServers[id] = process.release();  // Transfer ownership to QMap
...
void LanguageSupportManager::stopLSPServer(LanguageID id)
{
    ...
    if (it != m_lspServers.end() && it.value() && it.value()->state() == QProcess::Running) {
        it.value()->terminate();
...
bool LanguageSupportManager::isToolAvailable(const QString& command)
{
    QProcess process;
...
QString LanguageSupportManager::findToolInPath(const QString& toolName)
{
    QProcess process;
...
LanguageID LanguageSupportManager::detectLanguageFromFile(const QString& filePath) const
{
    QFileInfo fileInfo(filePath);
    QString suffix = fileInfo.suffix().toLower();
    
    // Map extension to language
    static const QMap<QString, LanguageID> extensionMap = {
        {"c", LanguageID::C},
        {"cpp", LanguageID::CPP},
        {"cc", LanguageID::CPP},
        {"cxx", LanguageID::CPP},
...
LanguageID LanguageSupportManager::detectLanguageFromContent(const QString& filePath) const
{
    QFile file(filePath);
    ...
    QTextStream in(&file);
    QString firstLine = in.readLine().toLower();
    file.close();
...
const LanguageConfiguration* LanguageSupportManager::getLanguageConfig(const QString& fileName) const
{
...
{
    const auto* config = getLanguageConfig(id);
    return config ? config->name.toStdString() : "Unknown";
}
...
void LanguageSupportManager::requestCompletion(const QString& filePath, int line, int column,
...
void LanguageSupportManager::requestFormatting(const QString& filePath,
                                              std::function<void(const QString&)> callback)
{
    // Will be implemented in CodeFormatter
}

void LanguageSupportManager::requestHover(const QString& filePath, int line, int column,
                                         std::function<void(const QString&)> callback)
{
    // LSP hover request
}

void LanguageSupportManager::requestDefinition(const QString& filePath, int line, int column,
                                              std::function<void(const QString&, int, int)> callback)
{
    // LSP definition request
}

void LanguageSupportManager::requestRename(const QString& filePath, int line, int column,
...
void LanguageSupportManager::requestReferences(const QString& filePath, int line, int column,
                                              std::function<void(const QVector<QPair<QString, QVector<QPair<int, int>>>>&)> callback)
{
    // LSP references request
}

QVector<LanguageConfiguration> LanguageSupportManager::getSupportedLanguages() const
{
    QVector<LanguageConfiguration> result;
...
void LanguageSupportManager::onLSPServerStarted(LanguageID id)
{
    ...
    if (config) {
        ...
        emit lspServerStarted(id);
    }
}

void LanguageSupportManager::onLSPServerError(LanguageID id, const QString& error)
{
    const auto* config = getLanguageConfig(id);
    if (config) {
...
void LanguageSupportManager::onFormatterFinished(LanguageID id, const QString& output)
{
    emit formattingCompleted(id);
}
