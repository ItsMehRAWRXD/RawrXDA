 ...
 * \date 2025-12-05
 * 
 * STABLE API — GitignoreFilter frozen as of v1.0.0 (January 22, 2026)
...
public:
    ...
    explicit ProjectExplorerWidget(QWidget* parent = nullptr,
                                   RawrXD::IFileWriter* fileWriter = nullptr,
                                   RawrXD::IDirectoryManager* dirManager = nullptr);
     ...
     */
    RawrXD::ProjectMetadata currentProjectMetadata() const;
...
private:
    ...
    RawrXD::ProjectMetadata m_projectMetadata;
    RawrXD::ProjectDetector m_projectDetector;
    // Abstracted file and directory managers injected via constructor
    RawrXD::IFileWriter *m_fileWriter = nullptr;
    RawrXD::IDirectoryManager *m_dirManager = nullptr;
    ...
    // .gitignore patterns (order-preserving per spec)
...
public:
    ...
    void setDirectoryManager(RawrXD::IDirectoryManager* dirManager);
...
private:
    /**
     * \brief Match path against .gitignore pattern (full spec)
     * \param path Relative path to check
     * \param pattern .gitignore pattern
     * \return true if matches
     */
    bool matchGitignorePattern(const QString& path, const QString& pattern) const;
    bool isParentDirectoryExcluded(const QString& path) const;
...
public:
    ...
    /**
     * \brief Get loaded patterns
     */
    QStringList patterns() const;
    
    /**
     * \brief Parse gitignore line handling trailing spaces and escapes per spec
     * \param line Raw line from .gitignore file
     * \return Parsed pattern, or empty if invalid/comment
     */
    static QString parseGitignoreLine(const QString& line);
    
private:
    QStringList m_patterns;
    
    /**
     * \brief Match file against single .gitignore pattern (full spec)
    ...
    /**
     * \brief Convert .gitignore pattern to regex pattern
     * \param pattern .gitignore pattern
     * \param isAnchored Whether pattern is anchored to base path
     * \return Regular expression pattern string
     */
    static QString gitignorePatternToRegex(const QString& pattern, bool isAnchored);
};
