...
QString AdvancedRefactoring::parseSymbolType(const QString& code, int position) {
    // Extract symbol type at given position
    QRegularExpression typeRe(R"((?:void|int|bool|double|float|QString|QStringList|QList|QMap|QVector|class|struct)\s+)");
    
    int searchStart = qMax(0, position - 100);
    QString snippet = code.mid(searchStart, position - searchStart);
    
    QRegularExpressionMatch match = typeRe.match(snippet);
    if (match.hasMatch()) {
...
QList<AdvancedRefactoring::SymbolInfo> AdvancedRefactoring::parseSymbols(const QString& code, const QString& filePath) {
    QList<SymbolInfo> symbols;
    
    // Parse classes
    QRegularExpression classRe(R"((?:class|struct)\s+(\w+))");
    ...
    while (it.hasNext()) {
        ...
        info.type = match.captured(0).contains("class") ? SymbolType::Class : SymbolType::Struct;
    ...
    QRegularExpression funcRe(R"((?:void|int|bool|QString|QList|QMap)\s+(\w+)\s*\()");
    it = funcRe.globalMatch(code);
...
AdvancedRefactoring::MethodSignature AdvancedRefactoring::parseMethodSignature(const QString& code, int startPos) {
    MethodSignature sig;
    
    // Extract method signature from code starting at position
    int endPos = code.indexOf('{', startPos);
    if (endPos == -1) endPos = code.indexOf(';', startPos);
    
    if (endPos != -1) {
        QString sigStr = code.mid(startPos, endPos - startPos).trimmed();
        sig = MethodSignature::fromString(sigStr);
    }
    
    return sig;
}

QStringList AdvancedRefactoring::parseMethodParameters(const QString& code, int startPos) {
    QStringList params;
    
    int parenStart = code.indexOf('(', startPos);
    int parenEnd = code.indexOf(')', parenStart);
    
    if (parenStart != -1 && parenEnd != -1) {
        QString paramStr = code.mid(parenStart + 1, parenEnd - parenStart - 1);
        
        for (const QString& param : paramStr.split(',')) {
            QString p = param.trimmed();
            if (!p.isEmpty()) {
                // Extract parameter name (last word)
                QStringList parts = p.split(' ', Qt::SkipEmptyParts);
                if (!parts.isEmpty()) {
                    params.append(parts.last());
                }
            }
        }
    }
...
QString AdvancedRefactoring::parseMethodBody(const QString& code, int startPos) {
    int braceStart = code.indexOf('{', startPos);
    int braceEnd = code.lastIndexOf('}');
    
    if (braceStart != -1 && braceEnd != -1) {
        return code.mid(braceStart + 1, braceEnd - braceStart - 1);
    }
...
QMap<QString, QString> AdvancedRefactoring::findVariableUsages(const QString& varName, const QString& code) {
    QMap<QString, QString> usages;
    
    QRegularExpression usageRe(QString(R"(\b%1\b)").arg(QRegularExpression::escape(varName)));
    QRegularExpressionMatchIterator it = usageRe.globalMatch(code);
    
    int count = 0;
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        QString lineNum = QString::number(code.left(match.capturedStart()).count('\n') + 1);
        usages[lineNum] = match.captured(0);
        count++;
    }
...
QList<AdvancedRefactoring::CodeRange> AdvancedRefactoring::findAllOccurrences(const QString& symbolName, const QString& filePath) {
    QList<CodeRange> occurrences;
    ...
    QRegularExpression re(QString(R"(\b%1\b)").arg(QRegularExpression::escape(symbolName)));
    ...
    while (it.hasNext()) {
        ...
        range.startLine = code.left(match.capturedStart()).count('\n') + 1;
        range.startColumn = match.capturedStart() - code.lastIndexOf('\n', match.capturedStart() - 1) - 1;
        range.endLine = range.startLine;
        range.endColumn = range.startColumn + match.capturedLength();
...
bool AdvancedRefactoring::isRenameSafe(const QString& oldName, const QString& newName, const CodeRange& scope) {
    // Check if renaming would create name conflicts
    QString code = readFile(scope.filePath);
    if (code.isEmpty()) {
        return false;
    }
    ...
    if (code.contains(QRegularExpression(QString(R"(\b%1\b)").arg(QRegularExpression::escape(newName))))) {
        return false;
    }
...
QString AdvancedRefactoring::updateIncludePaths(const QString& code, const QString& oldPath,
                                              const QString& newPath) {
    QString result = code;
    
    // Update include paths
    QRegularExpression includeRe(QString("#include.*%1").arg(QRegularExpression::escape(oldPath)));
    result.replace(includeRe, QString("#include \"%1\"").arg(newPath));
...
QString AdvancedRefactoring::detectLoopType(const QString& code, const CodeRange& range) {
    QStringList lines = code.split('\n');
    
    for (int i = range.startLine - 1; i < range.endLine && i < lines.size(); ++i) {
...
bool AdvancedRefactoring::isCodeUnreachable(const QString& code, int line) {
    QStringList lines = code.split('\n');
    ...
    // Simple heuristic: code after return/break/continue is unreachable
    if (line > 1) {
        QString prevLine = lines[line - 2].trimmed();
        if (prevLine.endsWith("return;") || prevLine.endsWith("return;") || 
            prevLine.endsWith("break;") || prevLine.endsWith("continue;")) {
            return true;
        }
    }
...
QStringList AdvancedRefactoring::findSymbolDependencies(const QString& symbolName, const QString& code) {
    QStringList dependencies;
    
    // Find all symbols used within the given symbol's code
    QRegularExpression usageRe(QString(R"(\b(\w+)\s*\()").arg(QRegularExpression::escape(symbolName)));
    QRegularExpressionMatchIterator it = usageRe.globalMatch(code);
    
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        QString dep = match.captured(1);
        if (dep != symbolName && !dependencies.contains(dep)) {
