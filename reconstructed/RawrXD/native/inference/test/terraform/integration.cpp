...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "  Metadata: " << header.metadata_count << std::endl;
        std::cout << std::endl;
        ...
        for (size_t i = 0; i < std::min(tensors.size(), size_t(5)); ++i) {
            ...
            std::cout << " (" << GGUFUtils::GetDataTypeSize(tensor.type)
                      << " bytes per element, " << tensor.size_bytes << " total bytes)" << std::endl;
        }
