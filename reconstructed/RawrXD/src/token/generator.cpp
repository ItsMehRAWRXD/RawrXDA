...
void TokenGenerator::loadConfigFromJSON(const std::string& path) {
    ...
    try {
        ...
        if (j.contains("vocab_size")) m_config.vocabSize = j["vocab_size"].get<int>();
        if (j.contains("bos_token_id")) m_config.bosTokenId = j["bos_token_id"].get<int>();
        if (j.contains("eos_token_id")) m_config.eosTokenId = j["eos_token_id"].get<int>();
    } catch (...) {
    ...
    try {
        ...
        if (j.contains("model") && j["model"].contains("merges")) {
            ...
            for (auto& m : merges) {
                ...
                if (sp != std::string::npos) {
                    m_mergeRanks[{merge.substr(0, sp), merge.substr(sp + 1)}] = rank++;
                }
            }
        }
        // Load added tokens
        if (j.contains("added_tokens")) {
            for (auto& at : j["added_tokens"]) {
                if (at.contains("content") && at.contains("id")) {
