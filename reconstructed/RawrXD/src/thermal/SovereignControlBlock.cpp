...
bool SharedMemoryManager::create(const std::string& name)
{
    ...
    m_hMapping = CreateFileMappingW(
        ...
        PAGE_READWRITE,
        0,
        SovereignControlBlock::TOTAL_SIZE,
        wname.c_str()
    );
    ...
    if (ftruncate(m_fd, SovereignControlBlock::TOTAL_SIZE) == -1) {
        ::close(m_fd);
        m_fd = -1;
        return false;
    }
    ...
    m_controlBlock = static_cast<SovereignControlBlock*>(
        mmap(nullptr, SovereignControlBlock::TOTAL_SIZE,
...
#else
    if (m_controlBlock != nullptr) {
        munmap(m_controlBlock, SovereignControlBlock::TOTAL_SIZE);
        m_controlBlock = nullptr;
    }
...
void SharedMemoryManager::initialize()
{
    ...
    std::memset(m_controlBlock, 0, SovereignControlBlock::TOTAL_SIZE);
