...
bool EnterpriseLicense::Initialize() {
    ...
    if (result != 0) {
        s_logger.error("[EnterpriseLicense] License init failed with status: 0x{:x}", result);
...
bool EnterpriseLicense::InstallLicense(const void* licenseBlob, size_t blobSize,
    ...
    if (result != 0) {
        s_logger.error("[EnterpriseLicense] License installation failed: 0x{:x}", result);
        return false;
    }
...
bool EnterpriseLicense::InstallLicenseFromFile(const std::wstring& path) {
    ...
    if (!file.is_open()) {
        s_logger.error("[EnterpriseLicense] Cannot open license file");
        return false;
    }

    auto fileSize = static_cast<size_t>(file.tellg());
    if (fileSize <= RSA_SIG_SIZE) {
        s_logger.error("[EnterpriseLicense] License file too small (need > {} bytes, got {})", RSA_SIG_SIZE, fileSize);
        return false;
    }
