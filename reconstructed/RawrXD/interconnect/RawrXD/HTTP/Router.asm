...
szJsonHealthOK          BYTE '{ "status": "ok" }', 0
...
WaitForInferenceCompletion PROC FRAME
    ...
    ; Stub: In a real system, this would wait on an event in InferenceJob
    ; For now, we simulate a delay or check status loop
    mov rbx, rcx    ; InferenceJob*
    
    ; Loop until Status == COMPLETE (4) or ERROR (6)
@wait_loop:
    cmp dword ptr [rbx].InferenceJob.Status, 4 ; COMPLETE
    je @done
    cmp dword ptr [rbx].InferenceJob.Status, 6 ; ERROR
    je @done
...
FormatJsonResponse PROC FRAME
    ; Wrap the result string into a JSON field
    ; rcx = buffer, rdx = capacity, r8 = template, r9 = value
    ; Simple wrapper around wsprintfA for now
    ...
    ; wsprintfA(buffer, template, value)
    mov rcx, rcx    ; Buffer (already there)
    mov rdx, r8     ; Template
    mov r8, r9      ; Value
    call wsprintfA
