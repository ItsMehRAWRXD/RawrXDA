...
### Loader Profiles (Environment Flags)

The inference engine selects the GGUF loader based on model size or explicit flags.

**Selection order:**
1. `RAWRXD_USE_ENHANCED_LOADER=1` → `EnhancedStreamingGGUFLoader`
2. `RAWRXD_USE_STREAMING_LOADER=1` → `StreamingGGUFLoader`
3. Large models (≥ 20GB) or `RAWRXD_LOADER_PROFILE=ultra|800b|mini` → `EnhancedStreamingGGUFLoader`
4. `RAWRXD_LOADER_PROFILE=tight|70b|120b` → `StreamingGGUFLoader`
5. Default → `GGUFLoader`

If no profile is set, the engine will also try to read Sovereign telemetry from
`Global\SOVEREIGN_NVME_TEMPS` and map tier `0/1/2` to `70b/120b/800b`.

**Optional tuning:**
- `RAWRXD_STREAMING_ZONE_MB` (e.g., `128`, `384`) overrides the per‑zone memory budget.

Example:
```powershell
$env:RAWRXD_USE_ENHANCED_LOADER=1
$env:RAWRXD_LOADER_PROFILE="800b"
$env:RAWRXD_STREAMING_ZONE_MB="128"
```
