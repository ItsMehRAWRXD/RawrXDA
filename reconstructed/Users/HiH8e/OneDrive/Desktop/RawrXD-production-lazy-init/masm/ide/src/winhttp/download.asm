...
.data
    ...
    szGET                db "GET",0
    szAuthPrefixA         db "Authorization: Bearer ",0
    szCRLF_A              db 13,10,0
...
WinHTTP_DownloadToFileBearerA PROC lpUrl:DWORD, lpOutPath:DWORD, lpBearerToken:DWORD
    ...
    LOCAL hdrA[MAX_HEADER_CHARS]:BYTE
    ...
    push OFFSET wHost
    push hSession
    call dword ptr [pWinHttpConnect]
    mov hConnect, eax
    test eax, eax
    jz @Fail
...
@HaveFlags:
    ...
    push 0            ; pwszReferrer
    push 0            ; pwszVersion
    push OFFSET wPath ; pwszObjectName
    push OFFSET wGET  ; pwszVerb
    push hConnect
    call dword ptr [pWinHttpOpenRequest]
    mov hRequest, eax
    test eax, eax
    jz @Fail
    ...
    ; Build ANSI header then convert to wide: "Authorization: Bearer <token>\r\n"
    invoke lstrcpyA, addr hdrA, addr szAuthPrefixA
    invoke lstrcatA, addr hdrA, lpBearerToken
    invoke lstrcatA, addr hdrA, addr szCRLF_A
    invoke MultiByteToWideChar, CP_ACP, 0, addr hdrA, -1, addr wHeader, MAX_HEADER_CHARS
    ...
    push OFFSET wHeader
    push hRequest
    call dword ptr [pWinHttpAddRequestHeaders]
...
@Send:
    ...
    push 0
    push hRequest
    call dword ptr [pWinHttpSendRequest]
    test eax, eax
    jz @Fail

    push 0
    push hRequest
    call dword ptr [pWinHttpReceiveResponse]
    test eax, eax
    jz @Fail
...
@ReadLoop:
    ...
    push eax
    push hRequest
    call dword ptr [pWinHttpQueryDataAvailable]
    test eax, eax
    jz @Done
    mov eax, dwAvail
    test eax, eax
...
@SzOk:
    ...
    push ecx
    push hRequest
    call dword ptr [pWinHttpReadData]
    test eax, eax
    jz @Done
...
@NoFile:
    ...
    je @NoReq
    push hRequest
    call dword ptr [pWinHttpCloseHandle]
@NoReq:
