...
// INTERVAL MANAGEMENT - TRACK AND CLEANUP INTERVALS
if (!window.managedIntervals) {
    window.managedIntervals = new Set();
}
if (!window.registerManagedInterval) {
    function registerManagedInterval(intervalId) {
        if (intervalId && typeof intervalId === 'number') {
            window.managedIntervals.add(intervalId);
            return intervalId;
        }
        return intervalId;
    }
    function unregisterManagedInterval(intervalId) {
        if (intervalId && typeof intervalId === 'number') {
            window.managedIntervals.delete(intervalId);
        }
    }
    function clearAllManagedIntervals() {
        window.managedIntervals.forEach(id => clearInterval(id));
        window.managedIntervals.clear();
    }
    window.registerManagedInterval = registerManagedInterval;
    window.unregisterManagedInterval = unregisterManagedInterval;
    window.clearAllManagedIntervals = clearAllManagedIntervals;
}
