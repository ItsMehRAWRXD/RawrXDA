...
void Win32IDE::addProblem(const std::string& file, int line, int col, const std::string& msg, int severity)
{
    ...
    ListView_SetItemText(m_hwndProblemsListView, lvi.iItem, 1, const_cast<char*>(msg.c_str()));
    ListView_SetItemText(m_hwndProblemsListView, lvi.iItem, 2, const_cast<char*>(file.c_str()));
    
    std::string lineStr = std::to_string(line);
    ListView_SetItemText(m_hwndProblemsListView, lvi.iItem, 3, const_cast<char*>(lineStr.c_str()));
