...
void handle_found(int fd)
{
    ...
    fprintf(outfd, "%d.%d.%d.%d:%s:%s:%s:%d:%d:%d\n",
        (get_in_addr(&name.sin_addr) & 0xff), 
        ((get_in_addr(&name.sin_addr) & (0xff << 8)) >> 8), 
        ((get_in_addr(&name.sin_addr) & (0xff << 16)) >> 16),
        ((get_in_addr(&name.sin_addr) & (0xff << 24)) >> 24), 
...
void *loader(void *threadCount)
{
    ...
    set_in_addr(&dest_addr.sin_addr, inet_addr(token));
        ...
        do
        {
            ...
            set_in_addr(&my_addr.sin_addr, inet_addr(bind_ip));
