...
static bool IsLoopbackEndpoint(const std::string& endpoint)
{
    ...
    return StartsWith(hostPort, "localhost") || StartsWith(hostPort, "127.0.0.1") || StartsWith(hostPort, "[::1]") ||
...
bool EnhancedModelLoader::preflightMemory(const std::string& modelPath, std::string& reason, bool& gpuUsable) const
{
    ...
    const char* bypassA = std::getenv("RAWRXD_BYPASS_RAM_GATES");
    const char* bypassB = std::getenv("RAWRXD_SKIP_RAM_PREFLIGHT");
    const bool bypass = (bypassA && (bypassA[0] == '1' || bypassA[0] == 'y' || bypassA[0] == 'Y')) ||
                        (bypassB && (bypassB[0] == '1' || bypassB[0] == 'y' || bypassB[0] == 'Y'));
    if (!bypass && modelBytes > ramLimit)
    {
    ...
    if (SUCCEEDED(CreateDXGIFactory1(__uuidof(IDXGIFactory1), reinterpret_cast<void**>(&factory))))
    {
        ...
        if (SUCCEEDED(factory->EnumAdapters1(0, &adapter)))
        {
            ...
            {
                const uint64_t vramLimit = static_cast<uint64_t>(static_cast<double>(desc.DedicatedVideoMemory) *
                                                                 (1.0 - m_vramReserveFraction));
...
bool EnhancedModelLoader::loadModel(const std::string& modelInput)
{
    ...
    if (m_onLoadingStage)
        m_onLoadingStage("Detecting model format...");
...
bool EnhancedModelLoader::loadModelAsync(const std::string& modelInput)
{
    ...
    if (ok)
        logLoadSuccess(modelInput, src.format, ms);
    else
        logLoadError(modelInput, src.format, m_lastError);
...
{
    if (m_onLoadingStage)
        m_onLoadingStage("Loading local GGUF...");
    ...
    if (m_onLoadingProgress)
        m_onLoadingProgress(100);
    if (m_onModelLoaded)
        m_onModelLoaded(m_modelPath);
    return true;
}
...
{
    if (m_onLoadingStage)
        m_onLoadingStage("Resolving Hugging Face model...");
    if (!m_hfDownloader)
        m_hfDownloader = std::make_unique<HFDownloader>();
    ...
    if (m_onLoadingStage)
        m_onLoadingStage("Downloading HF artifact: " + gguf_file);
...
{
    if (m_onLoadingStage)
        m_onLoadingStage("Connecting to Ollama...");
    if (!m_ollamaProxy)
        m_ollamaProxy = std::make_unique<OllamaProxy>();
    ...
    if (m_onLoadingStage)
        m_onLoadingStage("Injecting model metadata...");

    // Fetch actual metadata from Ollama API
    auto meta = m_ollamaProxy->getModelMetadata(modelName);
    RawrXD::ModelMetadataBuffer metaBuf = RawrXD::ModelMetadataHotpatch::fromOllamaModel(
        modelName, meta.found ? meta.family : "", meta.found ? meta.parameter_size : "",
    ...
    std::cout << "[EnhancedModelLoader] metadata hotpatch for '" << modelName
              << "': complete=" << (metaComplete ? "yes" : "no") << " flags=0x" << std::hex << metaBuf.flags << std::dec
...
bool EnhancedModelLoader::startServer(uint16_t port)
{
    ...
    if (!m_server)
        m_server = std::make_unique<GGUFServer>(m_engine.get());
...
std::string EnhancedModelLoader::getModelInfo() const
{
    ...
    if (m_engine)
        s += " engine=" + std::string(m_engine->GetEngineName());
    return s;
}
...
bool EnhancedModelLoader::decompressAndLoad(const std::string& compressedPath, CompressionType compression)
{
    ...
    m_lastError =
        "Compressed model loading not implemented for compression=" + FormatRouter::compressionToString(compression);
    if (m_onError)
        m_onError(m_lastError);
    return false;
}
...
{
    std::cout << "[EnhancedModelLoader] load start: " << input << " (" << FormatRouter::formatToString(format) << ")\n";
}

void EnhancedModelLoader::logLoadSuccess(const std::string& input, ModelFormat format, int64_t durationMs)
{
    std::cout << "[EnhancedModelLoader] load ok: " << input << " (" << FormatRouter::formatToString(format) << ")"
              << " in " << durationMs << "ms\n";
}

void EnhancedModelLoader::logLoadError(const std::string& input, ModelFormat format, const std::string& error)
{
    std::cerr << "[EnhancedModelLoader] load failed: " << input << " (" << FormatRouter::formatToString(format)
              << "): " << error << "\n";
}
