...
ReplayFixture ReplayFixture::load(const std::string& fixtureDir) {
    ...
    if (j.contains("targets") && j["targets"].is_array()) {
        const json& targetsArr = j["targets"];
        for (size_t i = 0; i < targetsArr.size(); ++i) {
            const json& tj = targetsArr[i];
...
FixtureManifest FixtureManifest::load(const std::string& manifestPath) {
    ...
    std::string manifestContent((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
    json j = json::parse(manifestContent);
    if (j.is_discarded()) return m;

    if (j.contains("fixtures") && j["fixtures"].is_array()) {
        const json& fixturesArr = j["fixtures"];
        for (size_t fi = 0; fi < fixturesArr.size(); ++fi) {
            const json& fj = fixturesArr[fi];
            FixtureManifestEntry e;
            e.id          = fj.contains("id")          ? fj["id"].get<std::string>()          : "";
            e.path        = fj.contains("path")        ? fj["path"].get<std::string>()        : "";
            e.description = fj.contains("description") ? fj["description"].get<std::string>() : "";
            e.tag         = fj.contains("tag")          ? fj["tag"].get<std::string>()          : "full";
            e.enabled     = fj.contains("enabled")     ? fj["enabled"].get<bool>()             : true;
