...
MERGE_SORT_THRESHOLD    EQU     32          ; v14.2.1: >32 symbols \u2192 merge sort
...
; =============================================================================
; merge_sort_ranked — v14.2.1: O(n log n) merge sort for RANKED_SYMBOL arrays
; Bottom-up iterative merge sort (no recursion, stack-safe).
; RCX = array base (RANKED_SYMBOL*)
; EDX = count
; Uses a temporary buffer allocated via VirtualAlloc.
; =============================================================================
merge_sort_ranked PROC FRAME
    push    rbx
    .pushreg rbx
    push    rsi
    .pushreg rsi
    ...
    ; Allocate temp buffer (same size as input)
    xor     ecx, ecx
    imul    edx, r12d, SIZEOF RANKED_SYMBOL
    mov     r8d, MEM_COMMIT OR MEM_RESERVE
    mov     r9d, PAGE_READWRITE
    call    VirtualAlloc
    test    rax, rax
    jz      @@merge_done                    ; Fallback: already partially sorted
    mov     r13, rax                        ; temp buffer

    ; Bottom-up merge sort: width = 1, 2, 4, 8, ...
    mov     ebx, 1                          ; width
...
@@width_loop:
    ...
    xor     r14d, r14d                      ; i = 0 (start of left run)
...
@@run_loop:
    ...
    mov     edi, r14d                       ; left = i
    lea     r15d, [r14d + ebx]              ; mid = i + width
    cmp     r15d, r12d
    jle     @@mid_ok
    mov     r15d, r12d
@@mid_ok:
    lea     ecx, [r14d + ebx*2]             ; right = i + 2*width
    cmp     ecx, r12d
    jle     @@right_ok
    mov     ecx, r12d
@@right_ok:
    ...
    ; Merge [left..mid) and [mid..right) into temp[left..right)
    push    rcx                             ; save right
    mov     eax, edi                        ; l = left
    mov     edx, r15d                       ; r = mid

    ; Output pointer in temp buffer
    imul    r8, rdi, SIZEOF RANKED_SYMBOL
    lea     r8, [r13+r8]                    ; dest = temp[left]
...
@@merge_loop:
    ...
    ; Compare importance (descending: higher first)
    push    rax
    push    rdx
    imul    rax, rax, SIZEOF RANKED_SYMBOL
    mov     eax, DWORD PTR [rsi+rax].RANKED_SYMBOL.importance
    push    rax                             ; left importance
    mov     rax, rdx
    imul    rax, rax, SIZEOF RANKED_SYMBOL
    mov     edx, DWORD PTR [rsi+rax].RANKED_SYMBOL.importance
    pop     rcx                             ; left importance in ecx
    cmp     ecx, edx                        ; left >= right? take left
    pop     rdx
    pop     rax
    jge     @@copy_left
...
@@copy_right:
    ...
    push    rdx
    push    rcx
    ; Copy array[r] to dest
    mov     rcx, r8                         ; dest
    imul    rax, rdx, SIZEOF RANKED_SYMBOL
    lea     rdx, [rsi+rax]                  ; src = array[r]
    mov     r8d, SIZEOF RANKED_SYMBOL
    call    memcpy
    pop     rcx
    pop     rdx
    pop     rax
    add     r8, SIZEOF RANKED_SYMBOL
    inc     edx
    jmp     @@merge_loop
...
@@merge_run_done:
    pop     rcx                             ; discard saved right
    ; Advance to next pair of runs
    lea     r14d, [r14d + ebx*2]
    jmp     @@run_loop

@@swap_buffers:
    ; Copy temp back to array for this pass
    mov     rcx, rsi                        ; dest = array
    mov     rdx, r13                        ; src = temp
    imul    r8, r12, SIZEOF RANKED_SYMBOL
    call    memcpy
...
@@sync_sort:
    ; v14.2.1: Adaptive sort — merge sort for large sets, insertion for small
    mov     rcx, rbx                        ; buffer base
    mov     edx, r15d                       ; count
    cmp     r15d, MERGE_SORT_THRESHOLD
    jle     @@use_insertion_sort
    call    merge_sort_ranked               ; O(n log n) for large sets
    jmp     @@sync_sorted
@@use_insertion_sort:
    call    insertion_sort_ranked           ; O(n^2) fast for small sets
@@sync_sorted:
