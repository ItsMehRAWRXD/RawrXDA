...
DEFAULT_STAGING_SIZE    EQU     10000000h       ; 256 MB
...
asm_gguf_loader_init PROC FRAME
    ...
    ; v14.2.1: Set default GPU residency threshold (Cathedral Build)
    mov     QWORD PTR [g_loader_ctx.gpu_threshold], DEFAULT_GPU_THRESHOLD
    mov     QWORD PTR [g_loader_ctx.tensors_gpu], 0
    mov     QWORD PTR [g_loader_ctx.tensors_mapped], 0
    mov     QWORD PTR [g_loader_ctx.dma_pending], 0
...
@@read_dims:
    ...
    jge     @@skip_extra_dim
    mov     rax, QWORD PTR [rbx]
    mov     DWORD PTR [rdi].GGUF_TENSOR_INFO.dims[rdx*4], eax
@@skip_extra_dim:
    add     rbx, 8
    inc     edx
    jmp     @@read_dims
...
@@zero_dims:
    cmp     ecx, 4
    jge     @@type_field
    mov     DWORD PTR [rdi].GGUF_TENSOR_INFO.dims[rcx*4], 0
    inc     ecx
    jmp     @@zero_dims
...
@@type_field:
    ...
    ; v14.2.1: Classify residency by GPU threshold heuristic
    ; Tensors >= gpu_threshold bytes → mark for GPU upload
    mov     rax, QWORD PTR [rdi].GGUF_TENSOR_INFO.data_size
    cmp     rax, QWORD PTR [g_loader_ctx.gpu_threshold]
    jl      @@tensor_cpu_resident
    mov     DWORD PTR [rdi].GGUF_TENSOR_INFO.residency, RESIDENCY_STAGED
    lock inc QWORD PTR [g_loader_ctx.tensors_gpu]
    jmp     @@tensor_classified
@@tensor_cpu_resident:
    lock inc QWORD PTR [g_loader_ctx.tensors_mapped]
@@tensor_classified:
...
; =============================================================================
; asm_gguf_loader_configure_gpu
; v14.2.1: Set GPU residency threshold for tensor classification.
; Tensors with data_size >= threshold will be marked RESIDENCY_STAGED
; for GPU upload via the DMA ring buffer pipeline.
; RCX = threshold in bytes (0 = all tensors to GPU, -1 = all CPU-mapped)
; Returns: RAX = 0
; =============================================================================
asm_gguf_loader_configure_gpu PROC FRAME
    sub     rsp, 8
    .allocstack 8
    .endprolog

    mov     QWORD PTR [g_loader_ctx.gpu_threshold], rcx

    xor     eax, eax
    add     rsp, 8
    ret
asm_gguf_loader_configure_gpu ENDP

; =============================================================================
; asm_gguf_loader_get_residency
; v14.2.1: Get residency classification summary.
; RCX = output buffer (at least 32 bytes: 4 QWORDs)
;   [0]  = gpu_threshold
;   [8]  = tensors_gpu (marked for GPU upload)
;   [16] = tensors_mapped (CPU zero-copy)
;   [24] = dma_pending (DMA uploads in-flight)
; Returns: RAX = 0
; =============================================================================
asm_gguf_loader_get_residency PROC FRAME
    sub     rsp, 8
    .allocstack 8
    .endprolog

    mov     rax, QWORD PTR [g_loader_ctx.gpu_threshold]
    mov     QWORD PTR [rcx], rax
    mov     rax, QWORD PTR [g_loader_ctx.tensors_gpu]
    mov     QWORD PTR [rcx+8], rax
    mov     rax, QWORD PTR [g_loader_ctx.tensors_mapped]
    mov     QWORD PTR [rcx+16], rax
    mov     rax, QWORD PTR [g_loader_ctx.dma_pending]
    mov     QWORD PTR [rcx+24], rax

    xor     eax, eax
    add     rsp, 8
    ret
asm_gguf_loader_get_residency ENDP
