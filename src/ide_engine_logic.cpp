...
// Forward declarations of IDE helper functions
...
static std::string WideToUTF8(const std::wstring& w) {
    ...
    int sizeNeeded = WideCharToMultiByte(CP_UTF8, 0, &w[0], (int)w.size(), nullptr, 0, nullptr, nullptr);
    std::string result(sizeNeeded, 0);
    WideCharToMultiByte(CP_UTF8, 0, &w[0], (int)w.size(), &result[0], sizeNeeded, nullptr, nullptr);
    return result;
}

static std::wstring UTF8ToWide(const std::string& s) {
    if (s.empty()) return std::wstring();
    int sizeNeeded = MultiByteToWideChar(CP_UTF8, 0, &s[0], (int)s.size(), nullptr, 0);
    std::wstring result(sizeNeeded, 0);
    MultiByteToWideChar(CP_UTF8, 0, &s[0], (int)s.size(), &result[0], sizeNeeded);
    return result;
}

static void AppendOutputText(HWND hOutput, const std::wstring& text) {
    if (!IsWindow(hOutput)) return;
    
    int len = GetWindowTextLength(hOutput);
    SendMessage(hOutput, EM_SETSEL, (WPARAM)len, (LPARAM)len);
    SendMessage(hOutput, EM_REPLACESEL, (WPARAM)FALSE, (LPARAM)text.c_str());
}
...
public:
    static void DisplayMemoryStatus(HWND hOutput) {
        if (!GlobalContext::Get().memory.get()) {
            AppendOutputText(hOutput, L"[Memory] No memory context available\r\n");
            return;
        }

        MemoryCore* mem = GlobalContext::Get().memory.get();
...
private:
    ...
    static void ExecuteGenerationMode(HWND hOutput, HWND hEditor, const std::wstring& params) {
        ...
        if (len > 0) {
            ...
            GetWindowTextW(hEditor, buffer.data(), len + 1);
...
extern "C" {
    ...
    void IDE_AnalyzePatches(HWND hOutput) {
        IDEEngine::ReverseEngineeringTools::AnalyzePatches(hOutput);
    }
}
