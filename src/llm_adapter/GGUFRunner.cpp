...
bool GGUFRunner::runInference(const std::string& prompt, float* outputBuffer)
{
    ...
    for (int t = 0; t < maxTokens; ++t)
    {
        ...
        // Pre-inference check for KV cache allocation
        if (context_.keyCache.empty() || context_.valueCache.empty()) {
            RawrXD::Logging::Logger::instance().error(
                "[GGUFRunner] KV cache not allocated before inference. This is a critical error.", "GGUFRunner");
            inferenceComplete(false);
            return false;
        }
...
void GGUFRunner::loadGGUFModel(const std::string& filePath)
{
    ...
    {
        RawrXD::Logging::Logger::instance().error(
            "[GGUFRunner] Failed to parse GGUF tensor table or tensors. The model file may be corrupt or incomplete.",
            "GGUFRunner");
    }
    else
    {
