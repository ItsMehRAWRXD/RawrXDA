...
; CRC32 hashes for APIs — computed via SSE 4.2 CRC32C (iSCSI polynomial)
; Verified: python3 -c "import crcmod; f=crcmod.predefined.mkCrcFun('crc-32c'); print(hex(f(b'CreateNamedPipeA')))"
CRC32_CreateNamedPipeA      EQU 06B32AE62h
CRC32_ConnectNamedPipe      EQU 04F92D981h
CRC32_DisconnectNamedPipe   EQU 0E81C7437h
CRC32_OpenFileMappingA      EQU 0A327F1D3h

; Global API pointers (resolved at runtime)
pCreateNamedPipeA           DQ 0
pConnectNamedPipe           DQ 0
pDisconnectNamedPipe        DQ 0
pOpenFileMappingA           DQ 0
...
Resolve_API_By_CRC32 PROC
    ; RCX = CRC32 hash to find
    ; Returns RAX = function pointer or 0 if not found
    push rbx
    push rsi
    push rdi
    push r12                ; save target hash in non-volatile r12
    push r13                ; save NumberOfNames
    push r14                ; save export table pointers
    push r15
    
    mov  r12d, ecx          ; r12d = target CRC32 hash (preserve across calls)
    ...
    mov  r13d, [rsi + 18h]  ; r13d = NumberOfNames
    mov  r14d, [rsi + 20h]  ; AddressOfNames RVA
    add  r14, rbx           ; r14 = AddressOfNames VA
    mov  r15d, [rsi + 24h]  ; AddressOfNameOrdinals RVA
    add  r15, rbx           ; r15 = AddressOfNameOrdinals VA
    mov  eax, [rsi + 1Ch]   ; AddressOfFunctions RVA
    add  rax, rbx
    mov  rsi, rax           ; rsi = AddressOfFunctions VA (reuse rsi)
    
    xor  edi, edi           ; edi = index
resolve_loop:
    cmp  edi, r13d          ; compare index vs NumberOfNames (was: cmp rdi, rcx — BUG!)
    jae  not_found
    
    ; Get name RVA
    mov  edx, [r14 + rdi*4]
    add  rdx, rbx           ; rdx = name VA
    
    ; Compute CRC32 of name — clobbers RCX, returns hash in RAX
    mov  rcx, rdx
    call Compute_CRC32_Name ; returns hash in EAX, length in RDX
    cmp  eax, r12d          ; compare hash vs preserved target (was: cmp rax, rcx — BUG!)
    je   found
...
Initialize_Sovereign_APIs PROC
    ...
    mov rcx, CRC32_CreateNamedPipeA
    call Resolve_API_By_CRC32
    mov pCreateNamedPipeA, rax
    
    mov rcx, CRC32_ConnectNamedPipe
    call Resolve_API_By_CRC32
    mov pConnectNamedPipe, rax
    
    mov rcx, CRC32_DisconnectNamedPipe
    call Resolve_API_By_CRC32
    mov pDisconnectNamedPipe, rax
    
    mov rcx, CRC32_OpenFileMappingA
    call Resolve_API_By_CRC32
    mov pOpenFileMappingA, rax
...
PIPE_UNLIMITED_INSTANCES    EQU 4
ENDIF
...
; SDDL for pipe security: Admins + SYSTEM + Creator/Owner
ALIGN 8
szHW_PipeSDDL       BYTE "D:(A;;GA;;;BA)(A;;GA;;;SY)(A;;GA;;;CO)",0
...
sl_loop:
    ...
    ; ---- Build SECURITY_ATTRIBUTES with DACL: Admins + SYSTEM + Creator/Owner ----
    lea  rcx, szHW_PipeSDDL
    mov  edx, 1                      ; SDDL_REVISION_1
    lea  r8, [rbp-40h]              ; &pSD (receives allocated SD)
    xor  r9d, r9d
    call ConvertStringSecurityDescriptorToSecurityDescriptorA
    test eax, eax
    jz   sl_pipe_sd_fail
    ; SECURITY_ATTRIBUTES at [rbp-58h]
    lea  rax, [rbp-58h]
    mov  dword ptr [rax], 18h       ; nLength
    mov  rcx, qword ptr [rbp-40h]
    mov  qword ptr [rax+8], rcx     ; lpSecurityDescriptor
    mov  dword ptr [rax+10h], 0     ; bInheritHandle = FALSE
    jmp  sl_pipe_create
sl_pipe_sd_fail:
    ; Fallback: NULL security (better than crashing)
...
sl_pipe_create:
    ...
    lea  rax, [rbp-58h]
    mov  qword ptr [rsp+38h], rax   ; lpSecurityAttributes
