...
bool TransformerInference::loadWeights(const QHash<QString, QByteArray>& tensorCache,
    ...
    if (!m_ctx) {
        ...
        m_tokenEmbed = createTensorFromCache(embdData, GGML_RXD_TYPE_F32, {m_nVocab, m_nEmbd});
        if (!m_tokenEmbed) {
            // Try alternative name
            const QByteArray& altData = tensorCache.value("model.embed_tokens.weight");
            m_tokenEmbed = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nVocab, m_nEmbd});
        }
        
        // Load output projection: [n_embd, vocab_size]
        const QByteArray& outData = tensorCache.value("output.weight");
        m_outputWeight = createTensorFromCache(outData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nVocab});
        if (!m_outputWeight) {
            const QByteArray& altData = tensorCache.value("lm_head.weight");
            m_outputWeight = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nVocab});
        }
    ...
    for (int i = 0; i < m_nLayers; ++i) {
        ...
        layer.attn_q = createTensorFromCache(qData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nEmbd});
        if (!layer.attn_q) {
            const QByteArray& altData = tensorCache.value(altPrefix + "self_attn.q_proj.weight");
            layer.attn_q = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nEmbd});
        }
        
        const QByteArray& kData = tensorCache.value(prefix + "attn_k.weight");
        layer.attn_k = createTensorFromCache(kData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nEmbd});
        if (!layer.attn_k) {
            const QByteArray& altData = tensorCache.value(altPrefix + "self_attn.k_proj.weight");
            layer.attn_k = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nEmbd});
        }
        
        const QByteArray& vData = tensorCache.value(prefix + "attn_v.weight");
        layer.attn_v = createTensorFromCache(vData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nEmbd});
        if (!layer.attn_v) {
            const QByteArray& altData = tensorCache.value(altPrefix + "self_attn.v_proj.weight");
            layer.attn_v = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nEmbd});
        }
        
        const QByteArray& projData = tensorCache.value(prefix + "attn_output.weight");
        layer.attn_proj = createTensorFromCache(projData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nEmbd});
        if (!layer.attn_proj) {
            const QByteArray& altData = tensorCache.value(altPrefix + "self_attn.o_proj.weight");
            layer.attn_proj = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nEmbd});
        }
        
        // FIX 4: Attention Biases
        const QByteArray& qBiasData = tensorCache.value(prefix + "attn_q.bias");
        layer.attn_q_bias = createTensorFromCache(qBiasData, GGML_RXD_TYPE_F32, {m_nEmbd});
        if (!layer.attn_q_bias) {
            const QByteArray& altData = tensorCache.value(altPrefix + "self_attn.q_proj.bias");
            layer.attn_q_bias = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd});
        }
        
        const QByteArray& kBiasData = tensorCache.value(prefix + "attn_k.bias");
        layer.attn_k_bias = createTensorFromCache(kBiasData, GGML_RXD_TYPE_F32, {m_nEmbd});
        if (!layer.attn_k_bias) {
            const QByteArray& altData = tensorCache.value(altPrefix + "self_attn.k_proj.bias");
            layer.attn_k_bias = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd});
        }
        
        const QByteArray& vBiasData = tensorCache.value(prefix + "attn_v.bias");
        layer.attn_v_bias = createTensorFromCache(vBiasData, GGML_RXD_TYPE_F32, {m_nEmbd});
        if (!layer.attn_v_bias) {
            const QByteArray& altData = tensorCache.value(altPrefix + "self_attn.v_proj.bias");
            layer.attn_v_bias = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd});
        }
        
        const QByteArray& projBiasData = tensorCache.value(prefix + "attn_output.bias");
        layer.attn_output_bias = createTensorFromCache(projBiasData, GGML_RXD_TYPE_F32, {m_nEmbd});
        if (!layer.attn_output_bias) {
            const QByteArray& altData = tensorCache.value(altPrefix + "self_attn.o_proj.bias");
            layer.attn_output_bias = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd});
        }
        
        // Layer norm
        const QByteArray& ln1Data = tensorCache.value(prefix + "attn_norm.weight");
        layer.ln1_weight = createTensorFromCache(ln1Data, GGML_RXD_TYPE_F32, {m_nEmbd});
        if (!layer.ln1_weight) {
            const QByteArray& altData = tensorCache.value(altPrefix + "input_layernorm.weight");
            layer.ln1_weight = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd});
        }
        
        // MLP
        const QByteArray& mlp1Data = tensorCache.value(prefix + "ffn_up.weight");
        layer.mlp_fc1 = createTensorFromCache(mlp1Data, GGML_RXD_TYPE_F32, {m_nEmbd, m_nEmbd * 4});
        if (!layer.mlp_fc1) {
            const QByteArray& altData = tensorCache.value(altPrefix + "mlp.up_proj.weight");
            layer.mlp_fc1 = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd, m_nEmbd * 4});
        }
        
        const QByteArray& mlp2Data = tensorCache.value(prefix + "ffn_down.weight");
        layer.mlp_fc2 = createTensorFromCache(mlp2Data, GGML_RXD_TYPE_F32, {m_nEmbd * 4, m_nEmbd});
        if (!layer.mlp_fc2) {
            const QByteArray& altData = tensorCache.value(altPrefix + "mlp.down_proj.weight");
            layer.mlp_fc2 = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd * 4, m_nEmbd});
        }
        
        // FIX 4: MLP Biases
        const QByteArray& mlp1BiasData = tensorCache.value(prefix + "ffn_up.bias");
        layer.mlp_fc1_bias = createTensorFromCache(mlp1BiasData, GGML_RXD_TYPE_F32, {m_nEmbd * 4});
        if (!layer.mlp_fc1_bias) {
            const QByteArray& altData = tensorCache.value(altPrefix + "mlp.up_proj.bias");
            layer.mlp_fc1_bias = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd * 4});
        }
        
        const QByteArray& mlp2BiasData = tensorCache.value(prefix + "ffn_down.bias");
        layer.mlp_fc2_bias = createTensorFromCache(mlp2BiasData, GGML_RXD_TYPE_F32, {m_nEmbd});
        if (!layer.mlp_fc2_bias) {
            const QByteArray& altData = tensorCache.value(altPrefix + "mlp.down_proj.bias");
            layer.mlp_fc2_bias = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd});
        }
        
        const QByteArray& ln2Data = tensorCache.value(prefix + "ffn_norm.weight");
        layer.ln2_weight = createTensorFromCache(ln2Data, GGML_RXD_TYPE_F32, {m_nEmbd});
        if (!layer.ln2_weight) {
            const QByteArray& altData = tensorCache.value(altPrefix + "post_attention_layernorm.weight");
            layer.ln2_weight = createTensorFromCache(altData, GGML_RXD_TYPE_F32, {m_nEmbd});
        }
    }
...
void TransformerInference::initKVCache() {
    ...
    for (int i = 0; i < m_nLayers; ++i) {
        m_kCache[i] = ggml_rxd_new_tensor_2d(m_kvCtx, GGML_RXD_TYPE_F32, m_nEmbd, m_ctxSize);
        m_vCache[i] = ggml_rxd_new_tensor_2d(m_kvCtx, GGML_RXD_TYPE_F32, m_nEmbd, m_ctxSize);
        
        // Zero initialize
        ggml_rxd_set_zero(m_kCache[i]);
        ggml_rxd_set_zero(m_vCache[i]);
    }
}
...
ggml_rxd_tensor* TransformerInference::createTensorFromCache(
    const QString& name,
    ...
    ggml_rxd_tensor* tensor = nullptr;
    if (nDims == 1) {
        tensor = ggml_rxd_new_tensor_1d(m_ctx, GGML_RXD_TYPE_F32, shape[0]);
    } else if (nDims == 2) {
        tensor = ggml_rxd_new_tensor_2d(m_ctx, GGML_RXD_TYPE_F32, shape[0], shape[1]);
    } else {
    ...
    size_t expectedSize = ggml_rxd_nbytes(tensor);
...
struct ggml_rxd_tensor* TransformerInference::createTensorFromCache(
    const QByteArray& data,
    int typeId,
    const std::vector<qint64>& dimensions)
{
    // Use the retrieved typeId to cast to the correct ggml_rxd_type enum
    enum ggml_rxd_type type = (enum ggml_rxd_type)typeId;

    struct ggml_rxd_tensor* tensor = nullptr;
    
    if (dimensions.size() == 1) {
        tensor = ggml_rxd_new_tensor_1d(m_ctx, type, dimensions[0]);
    } else if (dimensions.size() == 2) {
        // Dimensions in ggml are stored in reverse order
        tensor = ggml_rxd_new_tensor_2d(m_ctx, type, dimensions[1], dimensions[0]);
    } else if (dimensions.size() == 3) {
        tensor = ggml_rxd_new_tensor_3d(m_ctx, type, dimensions[2], dimensions[1], dimensions[0]);
    } else {
    ...
    size_t expected_size = ggml_rxd_nbytes(tensor);
    size_t actual_size = static_cast<size_t>(data.size());
    
    // Get quantization type info
    enum ggml_rxd_type dtype = static_cast<enum ggml_rxd_type>(typeId);
    const char* type_name = ggml_rxd_type_name(dtype);
    bool is_quantized = ggml_rxd_is_quantized(dtype);
    ...
    if (is_quantized) {
        ...
        // GGML's ggml_rxd_nbytes() might calculate differently for some quant types
...
std::vector<float> TransformerInference::forward(const std::vector<int32_t>& tokens) {
    ...
    ggml_rxd_context* gfCtx = ggml_rxd_init(params);
    if (!gfCtx) {
    ...
    ggml_rxd_tensor* logitsTensor = buildGraph(gfCtx, tokens);
    ...
    // Execute graph
    struct ggml_rxd_cgraph* gf = ggml_rxd_new_graph(gfCtx);
    ggml_rxd_build_forward_expand(gf, logitsTensor);
    
    // Create CPU backend for graph execution
    ggml_rxd_backend_t backend = ggml_rxd_backend_cpu_init();
    if (!backend) {
    ...
    enum ggml_rxd_status status = ggml_rxd_backend_graph_compute(backend, gf);
    ...
    ggml_rxd_backend_tensor_get(logitsTensor, logits.data(), 0, m_nVocab * sizeof(float));
...
ggml_rxd_tensor* TransformerInference::buildGraph(ggml_rxd_context* ctx, const std::vector<int32_t>& tokens) {
    int nTokens = tokens.size();
    
    // Create input tensor for token IDs
    ggml_rxd_tensor* inp = ggml_rxd_new_tensor_1d(ctx, GGML_RXD_TYPE_I32, nTokens);
    std::memcpy(inp->data, tokens.data(), nTokens * sizeof(int32_t));
    
    // Token embedding lookup: [n_tokens, n_embd]
    ggml_rxd_tensor* cur = ggml_rxd_get_rows(ctx, m_tokenEmbed, inp);
    ...
    for (int il = 0; il < m_nLayers; ++il) {
        ...
        ggml_rxd_tensor* inpL = cur;
        if (layer.ln1_weight) {
            cur = ggml_rxd_norm(ctx, cur, 1e-5f);
            cur = ggml_rxd_mul(ctx, cur, layer.ln1_weight);
            if (layer.ln1_bias) {
                cur = ggml_rxd_add(ctx, cur, layer.ln1_bias);
            }
        }
        
        // Self-attention with proper multi-head handling
        // Project to Q, K, V: [n_tokens, n_embd] -> [n_tokens, n_embd]
        ggml_rxd_tensor* Q = ggml_rxd_mul_mat(ctx, layer.attn_q, cur);
        ggml_rxd_tensor* K = ggml_rxd_mul_mat(ctx, layer.attn_k, cur);
        ggml_rxd_tensor* V = ggml_rxd_mul_mat(ctx, layer.attn_v, cur);
        
        // Scaled dot-product attention: softmax((Q @ K^T) / sqrt(d_k)) @ V
        // Note: ggml_rxd_mul_mat(A, B) computes A @ B^T for proper matrix mult
        ggml_rxd_tensor* KQ = ggml_rxd_mul_mat(ctx, K, Q);  // [n_tokens, n_tokens] attention scores
        
        // Scale by 1/sqrt(d_k) where d_k = n_embd / n_head
        float scale = 1.0f / sqrtf((float)(m_nEmbd / m_nHead));
        KQ = ggml_rxd_scale(ctx, KQ, scale);
        
        // Apply causal mask for autoregressive generation (if needed)
        // For simplicity, just softmax without mask
        KQ = ggml_rxd_soft_max(ctx, KQ);
        
        // Attention output: [n_tokens, n_tokens] @ [n_tokens, n_embd] -> [n_tokens, n_embd]
        ggml_rxd_tensor* attnOut = ggml_rxd_mul_mat(ctx, V, KQ);
        
        // Attention projection back to embedding dimension
        if (layer.attn_proj) {
            attnOut = ggml_rxd_mul_mat(ctx, layer.attn_proj, attnOut);
        }
        
        // Residual connection: x = attn(ln(x)) + x
        cur = ggml_rxd_add(ctx, attnOut, inpL);
        
        // Layer norm 2 (pre-MLP normalization)
        inpL = cur;
        if (layer.ln2_weight) {
            cur = ggml_rxd_norm(ctx, cur, 1e-5f);
            cur = ggml_rxd_mul(ctx, cur, layer.ln2_weight);
            if (layer.ln2_bias) {
                cur = ggml_rxd_add(ctx, cur, layer.ln2_bias);
            }
        }
        
        // MLP: FC1 -> GELU -> FC2
        if (layer.mlp_fc1 && layer.mlp_fc2) {
            cur = ggml_rxd_mul_mat(ctx, layer.mlp_fc1, cur);
            cur = ggml_rxd_gelu(ctx, cur);
            cur = ggml_rxd_mul_mat(ctx, layer.mlp_fc2, cur);
        }
        
        // Residual connection: x = mlp(ln(x)) + x
        cur = ggml_rxd_add(ctx, cur, inpL);
    }
    
    // Final layer norm (typically uses dedicated final norm weights)
    if (!m_layers.empty() && m_layers.back().ln2_weight) {
        cur = ggml_rxd_norm(ctx, cur, 1e-5f);
        cur = ggml_rxd_mul(ctx, cur, m_layers.back().ln2_weight);
        if (m_layers.back().ln2_bias) {
            cur = ggml_rxd_add(ctx, cur, m_layers.back().ln2_bias);
        }
    }
    
    // Output projection to vocabulary: [n_tokens, n_embd] -> [n_tokens, vocab]
    if (m_outputWeight) {
        cur = ggml_rxd_mul_mat(ctx, m_outputWeight, cur);
    }
    
    // Extract last token logits for next-token prediction
    // ggml_rxd_view_1d uses element offsets, not byte offsets
    int lastTokenOffset = (nTokens - 1) * m_nVocab;
    cur = ggml_rxd_view_1d(ctx, cur, m_nVocab, lastTokenOffset * sizeof(float));
