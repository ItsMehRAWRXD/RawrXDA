...
nlohmann::json AlertSystem::getSystemHealthStatus() const
{
    ...
    int64_t uptimeSeconds = std::chrono::duration_cast<std::chrono::seconds>(now - startTime).count();
    status["uptime"] = uptimeSeconds;
    status["pid"] = static_cast<int64_t>(GetCurrentProcessId());
...
double AlertSystem::getCpuUsagePercent() const
{
    ...
    auto toULL = [](const FILETIME& ft) -> uint64_t {
        return (static_cast<uint64_t>(ft.dwHighDateTime) << 32) | ft.dwLowDateTime;
    };

    uint64_t idle   = toULL(idleTime)   - toULL(prevIdleTime);
    uint64_t kernel = toULL(kernelTime) - toULL(prevKernelTime);
    uint64_t user   = toULL(userTime)   - toULL(prevUserTime);
    ...
    double cpuPercent = (1.0 - static_cast<double>(idle) / static_cast<double>(total)) * 100.0;
    return std::max(0.0, std::min(100.0, cpuPercent));
#else
    // POSIX: read /proc/stat
    ...
    if (std::getline(stat, line) && line.substr(0, 3) == "cpu") {
        ...
        return (1.0 - static_cast<double>(dIdle) / static_cast<double>(dTotal)) * 100.0;
    }
    return 0.0;
#endif
}
...
{
    // Iterate tracked operations and alert if any exceed the response time threshold
    qint64 now = QDateTime::currentMSecsSinceEpoch();
    double thresholdMs = m_thresholds.responseTimeMs;

    for (auto it = m_trackedOperations.constBegin(); it != m_trackedOperations.constEnd(); ++it) {
        qint64 startTime = it.value().startTimestamp;
        double elapsed = static_cast<double>(now - startTime);
        if (elapsed > thresholdMs) {
            createAlert(AlertType::Warning, AlertCategory::Performance,
                std::string("Long-running operation: ") + it.key().toStdString() +
                " (" + std::to_string(static_cast<int>(elapsed)) + "ms, threshold: " +
                std::to_string(static_cast<int>(thresholdMs)) + "ms)",
                AlertPriority::High);
        }
    }
}
...
{
    // Check thread heartbeats — any thread that hasn't reported in > 30s is considered stale
    qint64 now = QDateTime::currentMSecsSinceEpoch();
    constexpr qint64 HEARTBEAT_TIMEOUT_MS = 30000; // 30 seconds

    for (auto it = m_threadHeartbeats.constBegin(); it != m_threadHeartbeats.constEnd(); ++it) {
        qint64 lastBeat = it.value();
        qint64 elapsed = now - lastBeat;
        if (elapsed > HEARTBEAT_TIMEOUT_MS) {
            createAlert(AlertType::Critical, AlertCategory::System,
                std::string("Thread unresponsive: ") + it.key().toStdString() +
                " (last heartbeat " + std::to_string(elapsed / 1000) + "s ago)",
                AlertPriority::Critical);
        }
    }

    // Also check main thread responsiveness via QCoreApplication::processEvents latency
    ...
    if (latencyMs > 500) {
        createAlert(AlertType::Warning, AlertCategory::Performance,
            std::string("Main event loop latency: ") + std::to_string(latencyMs) + "ms",
            AlertPriority::Medium);
    }
}
