...
#include <algorithm>
#include "../../utils/sovereign_bridge.hpp" // Bridge included
...
{
    auto* groupBox = new QGroupBox("Sovereign Kernel HUD");
    auto* layout = new QGridLayout(groupBox);
    
    // Use QLabel instead of Charts for 0-dependency HUD
    auto* thermalLabel = new QLabel("Thermal Status: Initializing...");
    thermalLabel->setObjectName("lblThermal");
    thermalLabel->setStyleSheet("font-size: 14pt; font-weight: bold; color: cyan;");
    
    auto* statsLabel = new QLabel("Sovereign Stats: Initializing...");
    statsLabel->setObjectName("lblStats"); 
    statsLabel->setStyleSheet("font-size: 12pt;");
    
    layout->addWidget(thermalLabel, 0, 0);
    layout->addWidget(statsLabel, 1, 0);
    
    qobject_cast<QVBoxLayout*>(this->layout())->addWidget(groupBox);
    
    /* 
    // Legacy Chart Code disabled for Sovereign Integration
    // Latency chart
...
{
    // Override standard chart update to update Sovereign HUD
    auto thermalLabel = findChild<QLabel*>("lblThermal");
    auto statsLabel = findChild<QLabel*>("lblStats");
    
    if (thermalLabel && statsLabel) {
        auto stats = SovereignBridge::getStats();
        
        QString color = stats.temps[0] < 45 ? "green" : (stats.temps[0] < 60 ? "orange" : "red");
        thermalLabel->setStyleSheet(QString("font-size: 14pt; font-weight: bold; color: %1;").arg(color));
        thermalLabel->setText(QString("NVMe Temp: %1°C (Max: %2°C) %3")
            .arg(stats.temps[0], 0, 'f', 1)
            .arg(stats.temps[4], 0, 'f', 1) // heuristics
            .arg(stats.isThrottled ? "[THROTTLED]" : ""));
            
        statsLabel->setText(QString("Tier: %1 | Sparse Skip: %2% | GPU Split: %3% | Yielding: %4")
            .arg(stats.tier == 2 ? "800B" : (stats.tier == 1 ? "120B" : "70B"))
            .arg(stats.sparsePct)
            .arg(stats.gpuSplit)
            .arg(SovereignBridge::shouldYield() ? "YES" : "NO"));
    }
