...
QPair<QByteArray, int> apply_quant_with_type(const QByteArray& raw, const QString& mode)
{
    ...
    int ggml_rxd_type_id = 0;  // Default to F32 (0)
    
    if (mode == "Q4_0") {
        quantized = quantize_q4_0(raw);
        ggml_rxd_type_id = 2;  // GGML_RXD_TYPE_Q4_0
    } else if (mode == "Q8_0") {
        quantized = quantize_q8k(raw);
        ggml_rxd_type_id = 7;  // GGML_RXD_TYPE_Q8_0
    } else if (mode == "F16") {
        quantized = to_f16(raw);
        ggml_rxd_type_id = 1;  // GGML_RXD_TYPE_F16
    } else if (mode == "F32") {
        quantized = raw;  // No quantization for F32
        ggml_rxd_type_id = 0;  // GGML_RXD_TYPE_F32
    } else {
    ...
    return qMakePair(quantized, ggml_rxd_type_id);
}
