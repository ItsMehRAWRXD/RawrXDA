...
static uint8_t getTierByte(const std::string& tier) {
    if (tier == "community")  return 0;  // LicenseState::Invalid (Community)
    if (tier == "trial")      return 1;  // LicenseState::ValidTrial
    if (tier == "pro")        return 7;  // LicenseState::ValidPro
    if (tier == "enterprise") return 2;  // LicenseState::ValidEnterprise
    if (tier == "oem")        return 3;  // LicenseState::ValidOEM
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    if (command == "create") {
        ...
        for (const auto& f : g_features) {
            std::cout << "  0x" << std::right << std::hex << std::setw(2) 
                      << std::setfill('0') << std::uppercase << f.mask 
                      << std::dec << std::setfill(' ')
                      << "  | "
