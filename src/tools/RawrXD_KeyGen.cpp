...
static int cmd_issue(int argc, char* argv[], int startIdx) {
    ...
    hdr.issueDate       = (uint32_t)time(NULL);
    hdr.expiryDate      = (days > 0) ? (uint32_t)(hdr.issueDate + (uint32_t)(days * 86400)) : 0;
    hdr.tier            = licenseType;
    ...
    printf("  Issued:    %s", ctime((time_t*)&hdr.issueDate));
    if (hdr.expiryDate) {
        printf("  Expires:   %s", ctime((time_t*)&hdr.expiryDate));
    } else {
