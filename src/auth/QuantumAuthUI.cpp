...
void* KeyMetadata::toJson() const
{
    ...
    obj["expires"] = expires.toString(ISODate);
    obj["lastUsed"] = lastUsed.toString(ISODate);
    ...
    obj["revocationDate"] = revocationDate.toString(ISODate);
    obj["customMetadata"] = void*::fromVariantMap(customMetadata);
    return obj;
}

KeyMetadata KeyMetadata::fromJson(const void*& obj)
{
    KeyMetadata meta;
    meta.keyId = obj["keyId"].toString();
    meta.keyName = obj["keyName"].toString();
    meta.algorithm = static_cast<KeyAlgorithm>(obj["algorithm"]);
    meta.strength = static_cast<KeyStrength>(obj["strength"]);
    meta.purposes = KeyPurposes(obj["purposes"]);
    meta.created = // DateTime::fromString(obj["created"].toString(), ISODate);
    meta.expires = // DateTime::fromString(obj["expires"].toString(), ISODate);
    meta.lastUsed = // DateTime::fromString(obj["lastUsed"].toString(), ISODate);
    ...
    meta.usageCount = obj["usageCount"];
    meta.maxUsages = obj["maxUsages"];
    meta.isRevoked = obj["isRevoked"].toBool();
    meta.revocationReason = obj["revocationReason"].toString();
    meta.revocationDate = // DateTime::fromString(obj["revocationDate"].toString(), ISODate);
...
EntropyVisualizer::EntropyVisualizer(void* parent)
    ...
    , m_entropyLevel(0.0)
    , m_targetSamples(256)
    , m_animationTimer(std::make_unique<std::chrono::system_clock::time_pointr>(this))
{
    setMinimumSize(300, 200);
    setMaximumHeight(200);  // Signal connection removed\n});
