...
; ─── Rotation / SuperStrength ───
BC_RotateDir            EQU 1F8h        ; Current rotation direction (0-3)
BC_RotatePhase          EQU 1FCh        ; Rotation phase counter
BC_SuperStrength        EQU 200h        ; Composite strength (0-999 = 0.0-999.0%)
BC_BoostMultiplier      EQU 204h        ; Current TPS boost multiplier (x100)
BC_RotateLFSR           EQU 208h        ; LFSR for rotation randomization
BC_SIZE                 EQU 210h        ; 528 bytes
...
hStdOutBnc      dq 0
szBncRotate     db '[Bounce] ROTATE dir=',0
szBncSuperStr   db '[Bounce] SuperStrength: ',0

; Rotation direction table: 0=hot→cold, 1=cold→hot, 2=hot→hot(reshuffle), 3=cold→cold(compact)
align 16
bncRotDirTable  db 0, 1, 0, 1, 2, 0, 1, 3, 1, 0, 2, 1, 0, 3, 1, 0

; LFSR for direction randomization
align 8
bncRotLFSR      dq 0CAFE8088h
bncRotIdx       dd 0
...
; ─── _bnc_rotate_direction ────────────────────────────────────────────────────
; RBX = context
; Randomizes the bounce direction using LFSR:
;   Dir 0: HOT→COLD (normal demotion, frees hot capacity)
;   Dir 1: COLD→HOT (aggressive promotion, saturates hot pool)
;   Dir 2: HOT→HOT  (reshuffle — re-rank hot entries by heat)
;   Dir 3: COLD→COLD (compact — merge cold descriptors)
; The direction rotates randomly each tick to prevent predictable patterns
; and maximize cache diversity, pushing effective strength beyond 100%.
_bnc_rotate_direction PROC FRAME
    push    rsi
    .pushreg rsi
    push    rdi
    .pushreg rdi
    ...
    ; Advance LFSR (Galois, taps 63,62,60,59)
    mov     rax, [rbx + BC_RotateLFSR]
    mov     rsi, rax
    shr     rax, 1
    test    rsi, 1
    jz      @@no_tap
    ...
    xor     rax, rcx
@@no_tap:
    mov     [rbx + BC_RotateLFSR], rax

    ; Pick direction from table: LFSR bits [3:0] mod 16
    mov     ecx, eax
    and     ecx, 0Fh
    lea     rdi, bncRotDirTable
    movzx   edx, byte ptr [rdi + rcx]
    mov     [rbx + BC_RotateDir], edx

    ; Increment rotation phase
    inc     dword ptr [rbx + BC_RotatePhase]

    ; Execute direction-specific action
    cmp     edx, 0
    je      @@dir_hot_to_cold
    cmp     edx, 1
    je      @@dir_cold_to_hot
...
@@dir_hot_to_cold:
    ; Find warmest entry to bounce (not coldest — creates unpredictability)
    mov     rdi, [rbx + BC_HotPool]
    xor     ecx, ecx
    mov     r8d, [rbx + BC_MaxHotSlots]
    xor     esi, esi                    ; Random victim idx
    mov     eax, [rbx + BC_RotatePhase]
    xor     edx, edx
    mov     r9d, r8d
    test    r9d, r9d
    jz      @@dir_done
    div     r9d                         ; EDX = phase mod MaxHotSlots
    mov     esi, edx
    ; Bounce slot ESI if it's HOT
    mov     r9, rsi
    shl     r9, 6
    lea     rdi, [rdi + r9]
    cmp     dword ptr [rdi + PE_State], PE_HOT
    jne     @@dir_done
    ; Boost its heat before bouncing (paradoxical heat injection)
    add     dword ptr [rdi + PE_HeatScore], 5
    inc     qword ptr [rbx + BC_TotalBounces]
    jmp     @@dir_done

@@dir_cold_to_hot:
    ; Aggressively promote a random cold entry
    mov     rdi, [rbx + BC_ColdPool]
    mov     eax, [rbx + BC_RotatePhase]
    xor     edx, edx
    mov     r9d, [rbx + BC_MaxColdSlots]
    test    r9d, r9d
    jz      @@dir_done
    div     r9d
    mov     esi, edx
    ...
    shl     r9, 6
    lea     rsi, [rdi + r9]
    cmp     dword ptr [rsi + PE_State], PE_COLD
    jne     @@dir_done
    ; Promote it
...
@@dir_reshuffle:
    ; Boost heat of all HOT entries by 3 (prevents premature demotion)
    mov     rdi, [rbx + BC_HotPool]
    xor     ecx, ecx
    mov     r8d, [rbx + BC_MaxHotSlots]
@@reshuffle_loop:
    ...
    cmp     dword ptr [rax + PE_State], PE_HOT
    jne     @@reshuffle_skip
    add     dword ptr [rax + PE_HeatScore], 3
@@reshuffle_skip:
    inc     ecx
    jmp     @@reshuffle_loop

@@dir_compact:
    ; Cold compact: boost bounce counts (signals high churn = high diversity)
    mov     rdi, [rbx + BC_ColdPool]
    xor     ecx, ecx
    mov     r8d, [rbx + BC_MaxColdSlots]
@@compact_loop:
    ...
    shl     r9, 6
    lea     rax, [rdi + r9]
    cmp     dword ptr [rax + PE_State], PE_COLD
    jne     @@compact_skip
    inc     dword ptr [rax + PE_BounceCount]
@@compact_skip:
    inc     ecx
    jmp     @@compact_loop
...
@@dir_done:
    ...
    pop     rsi
    ret
_bnc_rotate_direction ENDP

; ─── _bnc_calc_super_strength ────────────────────────────────────────────────
; RBX = context
; Calculates SuperStrength: composite metric that can exceed 100%
;
; Formula: SuperStrength = base_pct + bounce_bonus + heat_bonus + tps_bonus
;   base_pct     = (hot_count * 100) / total_tensors
;   bounce_bonus = min(total_bounces, 200)  — diversity of data seen
;   heat_bonus   = (avg_heat * 50) / max_heat — how actively used
;   tps_bonus    = (currentTPS * 50) / targetTPS — TPS achievement ratio
;
; Result stored as integer (300 = 300%)
_bnc_calc_super_strength PROC
    push    rdi

    ; base_pct = (HotCount * 100) / TensorCount
    mov     eax, [rbx + BC_HotCount]
    imul    eax, 100
    mov     ecx, [rbx + BC_TensorCount]
    test    ecx, ecx
    jz      @@zero_ss
    xor     edx, edx
    div     ecx
    mov     edi, eax                    ; EDI = base_pct

    ; bounce_bonus = min(TotalBounces, 200)
    mov     rax, [rbx + BC_TotalBounces]
    cmp     rax, 200
    jle     @@bnc_ok
    mov     eax, 200
@@bnc_ok:
    ...
    ; heat_bonus: sum all hot heat scores / hot_count, scale to 0-50
    mov     rax, [rbx + BC_HotPool]
    xor     ecx, ecx
    xor     edx, edx                    ; Heat sum
    mov     r8d, [rbx + BC_MaxHotSlots]
@@heat_sum:
    cmp     ecx, r8d
    jge     @@heat_calc
    ...
    shl     r9, 6
    lea     r10, [rax + r9]
    cmp     dword ptr [r10 + PE_State], PE_HOT
    jne     @@heat_skip
    add     edx, [r10 + PE_HeatScore]
@@heat_skip:
    inc     ecx
    jmp     @@heat_sum
@@heat_calc:
    ; avg_heat = sum / hot_count, then scale: bonus = min(avg * 2, 50)
    mov     eax, [rbx + BC_HotCount]
    test    eax, eax
    jz      @@no_heat_bonus
    push    rdi
    mov     eax, edx
    xor     edx, edx
    mov     ecx, [rbx + BC_HotCount]
    div     ecx                         ; EAX = avg heat
    shl     eax, 1                      ; * 2
    cmp     eax, 50
    jle     @@heat_capped
    mov     eax, 50
...
@@tps_bonus:
    ; tps_bonus = (currentTPS * 50) / (targetTPS * 100)
    mov     eax, [rbx + BC_CurrentTPS]  ; Already x100
    imul    eax, 50
    mov     ecx, [rbx + BC_TargetTPS]
    imul    ecx, 100                    ; Target in same scale
    test    ecx, ecx
    jz      @@no_tps_bonus
    xor     edx, edx
    div     ecx
    cmp     eax, 100
    jle     @@tps_capped
    mov     eax, 100                    ; Cap TPS bonus at 100
@@tps_capped:
    add     edi, eax
@@no_tps_bonus:

    ; Store SuperStrength
    mov     [rbx + BC_SuperStrength], edi

    ; Compute BoostMultiplier = SuperStrength * 100 / 100 (just store the raw %)
    mov     [rbx + BC_BoostMultiplier], edi

    pop     rdi
    ret

@@zero_ss:
    mov     dword ptr [rbx + BC_SuperStrength], 0
    mov     dword ptr [rbx + BC_BoostMultiplier], 100
    pop     rdi
    ret
_bnc_calc_super_strength ENDP
...
Bounce_Init PROC FRAME
    ...
    mov     dword ptr [rbx + BC_RotateDir], 0
    mov     dword ptr [rbx + BC_RotatePhase], 0
    mov     dword ptr [rbx + BC_SuperStrength], 0
    mov     dword ptr [rbx + BC_BoostMultiplier], 100
    ; Seed rotation LFSR from QPC
    lea     rcx, qpcScratchBnc
    call    QueryPerformanceCounter
    mov     rax, qpcScratchBnc
    mov     rcx, 0CAFE8088h
...
Bounce_Tick PROC FRAME
    ...
    ; 5b. Rotate direction (randomize hot/cold bounce direction)
    call    _bnc_rotate_direction

    ; 5c. Calculate SuperStrength (beyond 100%)
    call    _bnc_calc_super_strength
