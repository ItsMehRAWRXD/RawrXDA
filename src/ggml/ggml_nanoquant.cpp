...
static void nq_r4_from_float(const float* src, void* dst, int64_t n_elements) {
    if (!src || !dst || n_elements <= 0) return;
    // NQ_R4 requires M, N, rank — cannot be determined from n_elements alone.
    // Use NQ_MatrixFactor_MultiRank directly for production quantization.
    // This stub exists for GGML type-system compatibility only.
    (void)src;
    (void)dst;
    (void)n_elements;
}

/// NQ_R4 vec_dot — not supported at block level.
