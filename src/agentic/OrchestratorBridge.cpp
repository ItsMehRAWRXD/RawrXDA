...
using RawrXD::Agent::LLMChatFunction;
using RawrXD::Agent::ToolExecResult;
using LLMChatFunction = RawrXD::Agent::LLMChatFunction;
using RawrXD::Agent::ChatMessage;
using RawrXD::Agent::InferenceResult;
using RawrXD::Agent::EditorContext;
...
using RawrXD::Agent::AgentLoopConfig;
...
void OrchestratorBridge::WireToolSchemas() {
    ...
    for (size_t i = 0; i < xMacroSchemas.size(); ++i) {
        const auto& xTool = xMacroSchemas[i];
        if (!xTool.contains("function") || !xTool["function"].contains("name")) {
            unifiedTools.push_back(xTool);
            continue;
        }

        std::string toolName = xTool["function"]["name"].get<std::string>();
        auto it = handlerMap.find(toolName);
    ...
    for (auto it = handlerMap.begin(); it != handlerMap.end(); ++it) {
        unifiedTools.push_back(it->second);
    }
...
void OrchestratorBridge::WireToolHandlers() {
    ...
    // Disk Recovery tools (8 tools: scan, probe, start, status, pause, abort, key, badmap)
    DiskRecoveryToolHandler::RegisterTools(registry);
}
