...
INFINITY_STREAM_STATE STRUCT
    ...
    ; Core subsystems
    ai_model            AI_MODEL_STATE <>
    gpu_device          GPU_DEVICE_STATE <>
    dstorage_state      DSTORAGE_STATE <>
    
    ; Synchronization
    stream_lock         SRW LOCK <>
...
.DATA
    ...
    ; Global INFINITY_STREAM singleton
    g_infinity_stream   INFINITY_STREAM_STATE <>
...
; RCX = error_code, RDX = file_ptr, R8D = line, R9 = function_ptr
SetError PROC
    PUSH RBP
    MOV  RBP, RSP
    SUB  RSP, 32
    
    ; Validate error code is explicit (not magic number)
    CMP  ECX, ERROR_SUCCESS
    JE   .error_set_invalid
    CMP  ECX, ERROR_AI_INFERENCE_FAILED
    JGE  .error_set_valid
    CMP  ECX, ERROR_AI_INFERENCE_FAILED
    JL   .error_set_invalid
    
.error_set_valid:
    ; Store in global error history
    LEA  RAX, g_infinity_stream
    MOV  RDX, [RAX + OFFSET INFINITY_STREAM_STATE.error_history_ptr]
    MOV  R10D, [RAX + OFFSET INFINITY_STREAM_STATE.error_count]
    
    ; Wrap around if buffer full
    CMP  R10D, ERROR_HISTORY_SIZE
    JL   .error_store_direct
    MOV  R10D, 0
    
.error_store_direct:
    ; Calculate offset in error history
    IMUL R10, SIZEOF ERROR_RECORD
    ADD  RDX, R10
    
    ; Fill in error record
    MOV  [RDX + OFFSET ERROR_RECORD.code], ECX
    MOV  [RDX + OFFSET ERROR_RECORD.file_ptr], RDX
    MOV  [RDX + OFFSET ERROR_RECORD.line], R8D
    MOV  [RDX + OFFSET ERROR_RECORD.function_ptr], R9
    
    ; Get current timestamp (simplified: just use counter)
    RDTSC
    ...
    TEST R10, R10
    JZ   .error_no_callback
    
    MOV  RCX, [RAX + OFFSET INFINITY_STREAM_STATE.last_error_code]
    CALL R10
...
.error_set_invalid:
    ; Attempt to set invalid error code; log and use ERROR_SUCCESS
