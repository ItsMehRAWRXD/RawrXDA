...
; SDDL string for pipe security: Admins + SYSTEM + Creator/Owner get full access
ALIGN 8
szPipeSDDL          BYTE "D:(A;;GA;;;BA)(A;;GA;;;SY)(A;;GA;;;CO)",0

; Externs for security descriptor APIs
...
Parse_WidgetIntent PROC FRAME
    ...
    ; --- Length validation: ensure at least 10 bytes ("WIDGET:T:X") ---
    xor  ecx, ecx
@@pw_len:
    cmp  byte ptr [rsi + rcx], 0
    je   @@pw_len_done
    inc  ecx
    cmp  ecx, PIPE_BUFFER_SIZE
    jae  @@pw_len_done
@@pw_len_done:
    cmp  ecx, 10            ; minimum "WIDGET:T:H..."
    jb   @@pw_bail
...
adv_sym_done:
    ...
    jmp  adv_exit
adv_no_sym:
    mov  eax, 1001h         ; no symbol found in context
adv_exit:
...
do_codelens:
    ...
    ; Scan the context string for the identifier; count occurrences in the active region
...
do_quickfix:
    ; Dispatch quick-fix request to HeadacheEliminator engine
    mov  rcx, r14
...
engine_loop:
    ...
    ; ---- Build SECURITY_ATTRIBUTES restricting to current user + admins ----
    ; ConvertStringSecurityDescriptorToSecurityDescriptorA creates a full SD from SDDL
    ;   "D:(A;;GA;;;BA)(A;;GA;;;SY)(A;;GA;;;CO)"  — Admins + System + Creator/Owner
    lea  rcx, szPipeSDDL
    mov  edx, 1                      ; SDDL_REVISION_1
    lea  r8, [rbp-0C0h]             ; &pSD (receives allocated SD)
    xor  r9d, r9d
    call ConvertStringSecurityDescriptorToSecurityDescriptorA
    test eax, eax
    jz   engine_pipe_fail
    ; Build SECURITY_ATTRIBUTES on stack
    lea  rax, [rbp-0D8h]            ; SECURITY_ATTRIBUTES (18h bytes)
    mov  dword ptr [rax], 18h       ; nLength = sizeof(SECURITY_ATTRIBUTES)
    mov  rcx, qword ptr [rbp-0C0h]  ; pSD from conversion
