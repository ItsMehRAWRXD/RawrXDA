...
bool TextBuffer::validateTree(Node* node, int& totalChars, int& totalLines) const {
    if (!node) {
        totalChars = 0;
        totalLines = 0;
        return true;
    ...
    if (node->isLeaf) {
        totalChars = (int)node->text.length();
        totalLines = countNewlines(node->text);
        if (node->weight != totalChars) return false;
        if (node->lineBreakCount != totalLines) return false;
        return true;
    }

    int leftChars = 0, leftLines = 0;
    int rightChars = 0, rightLines = 0;

    bool lValid = validateTree(node->left, leftChars, leftLines);
    bool rValid = validateTree(node->right, rightChars, rightLines);

    if (!lValid || !rValid) return false;

    totalChars = leftChars + rightChars;
    totalLines = leftLines + rightLines;

    if (node->weight != leftChars && node->weight != length(node->left)) return false; 
    if (node->lineBreakCount != totalLines) return false;
    
    // Check height balance
    int h1 = node->left ? node->left->height : 0;
    int h2 = node->right ? node->right->height : 0;
    if (node->height != 1 + std::max(h1, h2)) return false;

    return true;
}

bool TextBuffer::checkConsistency() const {
    int chars = 0, lines = 0;
    if (!validateTree(root, chars, lines)) return false;
    if (chars != totalLength) return false;
    if (lines != totalLineCount) return false;
    return true;
}

void TextBuffer::dumpTree(Node* node, int depth, std::wstringstream& ss) const {
    if (!node) return;
    for(int i=0; i<depth; i++) ss << L"  ";
    if (node->isLeaf) {
        ss << L"Leaf (w=" << node->weight << L", h=" << node->height << L"): " 
           << node->text.substr(0, std::min((size_t)10, node->text.length())) << L"...\n";
    } else {
        ss << L"Internal (w=" << node->weight << L", h=" << node->height << L")\n";
        dumpTree(node->left, depth + 1, ss);
        dumpTree(node->right, depth + 1, ss);
    }
}

String TextBuffer::getDebugTree() const {
    std::wstringstream ss;
    dumpTree(root, 0, ss);
    return String(ss.str());
}

bool TextBuffer::isValidCodePoint(int pos) const {
    if (pos < 0 || pos >= totalLength) return false;
    wchar_t c = charAt(pos);
    if (isLowSurrogate(c)) {
        if (pos > 0 && isHighSurrogate(charAt(pos - 1))) return true; // Part of pair
        return false; // Orphan low surrogate
    }
    return true; 
}
} // namespace RawrXD
