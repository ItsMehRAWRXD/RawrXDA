...
QJsonArray AgenticExecutor::decomposeTask(const QString& goal)
{
    ...
    QString prompt = QString(
        ...
        "2. The type of action (create_directory, create_file, compile, run, train_model, execute_command, win32_powershell, generate_react_app, etc.)\n"
        ...
        "- execute_command: Run any system command (git, curl, zip, etc.)\n"
        "- win32_powershell: Execute a PowerShell script or command for Windows automation\n"
        "- generate_react_app: Generate a full React/Express stack with optional IDE components\n"
        "- train_model: Fine-tune a GGUF model with dataset\n"
        "- is_training: Check if model training is in progress\n\n"
        "Return as JSON array:\n"
        "[\n"
        "  {\"step\": 1, \"action\": \"generate_react_app\", \"description\": \"...\", \"params\": {\"path\": \"./new-ide\", \"config\": {\"name\": \"MyIDE\", \"ide_features\": true}}, \"criteria\": \"...\" },\n"
        "  {\"step\": 2, \"action\": \"win32_powershell\", \"description\": \"...\", \"params\": {\"command\": \"Get-Process | Where-Object ...\"}, \"criteria\": \"...\" }\n"
        "]\n\n"
...
bool AgenticExecutor::executeStep(const QJsonObject& step)
{
    ...
    try {
        if (action == "create_directory") {
            ...
            QStringList args = params["args"].toVariant().toStringList();
            // If command contains spaces and no args, might need split? 
            // Better to rely on params separation, but for safety:
            if (args.isEmpty() && cmd.contains(" ")) {
                // simple split might be naive but okay for basic cases
                QStringList parts = cmd.split(" ", Qt::SkipEmptyParts);
                cmd = parts.takeFirst();
                args = parts;
            }
            
            QJsonObject runResult = runExecutable(cmd, args);
            
            // Store output in memory for subsequent steps
            addToMemory("last_command_stdout", runResult["stdout"]);
            addToMemory("last_command_stderr", runResult["stderr"]);
            
            return runResult["success"].toBool();
        }
        else if (action == "win32_powershell") {
            QString cmd = params["command"].toString();
            qInfo() << "[AgenticExecutor] Executing PowerShell: " << cmd;
            
            QStringList args;
            args << "-NoProfile" << "-ExecutionPolicy" << "Bypass" << "-Command" << cmd;
            
            QJsonObject runResult = runExecutable("powershell.exe", args);
            
            // Store output
            addToMemory("last_powershell_stdout", runResult["stdout"]);
            addToMemory("last_powershell_stderr", runResult["stderr"]);
            
            if (!runResult["success"].toBool()) {
                qWarning() << "PowerShell error:" << runResult["stderr"].toString();
            }
            ...
            QString dataset = params["dataset"].toString();
            QString model = params["model"].toString();
            QJsonObject config = params["config"].toObject();
            QJsonObject trainResult = trainModel(dataset, model, config);
            return trainResult["success"].toBool();
        }
        else if (action == "is_training") {
            return isTrainingModel();
        }
        else if (action == "generate_react_app") {
            QString path = params["path"].toString();
            QJsonObject configParams = params["config"].toObject();
            
            RawrXD::ReactServerConfig config;
            config.name = configParams["name"].toString("rawrxd-app");
            config.description = configParams["description"].toString("Generated by RawrXD");
            config.port = configParams["port"].toString("3000");
            config.include_auth = configParams["auth"].toBool(false);
            config.include_database = configParams["database"].toBool(false);
            config.include_typescript = configParams["typescript"].toBool(true);
            config.include_tailwind = configParams["tailwind"].toBool(true);
            config.include_docker = configParams["docker"].toBool(false);
            config.include_tests = configParams["tests"].toBool(true);
            
            // IDE Features
            config.include_ide_features = configParams["ide_features"].toBool(true);
            config.include_monaco_editor = configParams["monaco"].toBool(true);
            config.include_agent_modes = configParams["agent_modes"].toBool(true);
            config.include_re_tools = configParams["re_tools"].toBool(true);
            
            qInfo() << "[AgenticExecutor] Generating React App at" << path;
            emit logMessage("Generating React App: " + config.name);
            
            bool success = RawrXD::ReactServerGenerator::Generate(path.toStdString(), config);
            
            if (success && configParams["install_deps"].toBool(true)) {
                emit logMessage("Installing dependencies...");
                runExecutable("npm", QStringList() << "install" << "--prefix" << path);
            }
