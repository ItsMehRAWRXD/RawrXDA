...
; ============================================================================
; Tokenize - Simple byte-level BPE tokenization (fallback)
; Input:  RCX = input UTF-8 string, EDX = max output tokens
;         R8  = output token_id buffer (DWORD array)
...
Tokenize PROC PRIVATE
    ...
    mov r12d, edx                    ; R12D = max tokens
    mov rdi, r8                      ; RDI = output token array
    xor r13d, r13d                   ; R13D = token count

    ; Fallback tokenizer: byte-level encoding
    ; Each byte becomes its own token ID (BPE vocabulary offset 3+)
    ; Special tokens: 0=PAD, 1=BOS, 2=EOS

    ; Emit BOS token first
    cmp r13d, r12d
    jge @@tok_done
    mov DWORD PTR [rdi], 1           ; BOS = token 1
    add rdi, 4
    inc r13d
...
@@tok_loop:
    ...
    test al, al
    jz @@tok_eos
    ; Map byte to token: token_id = byte_value + 3 (skip PAD/BOS/EOS)
    add eax, 3
...
@@tok_eos:
    ...
    cmp r13d, r12d
    jge @@tok_done
    mov DWORD PTR [rdi], 2           ; EOS = token 2
    add rdi, 4
    inc r13d
...
; ============================================================================
; Detokenize - Convert token IDs back to UTF-8 string
; Input:  RCX = token_id buffer (DWORD array), EDX = token count
;         R8  = output UTF-8 string buffer
; Output: EAX = bytes written (excluding null terminator)
; ============================================================================
Detokenize PROC PRIVATE
    ...
    mov rsi, rcx                     ; RSI = token array
    mov ebx, edx                     ; EBX = token count
    mov rdi, r8                      ; RDI = output string buffer
    xor ecx, ecx                     ; ECX = output byte count
...
@@detok_loop:
    ...
    ; Skip special tokens (PAD=0, BOS=1, EOS=2)
    cmp eax, 3
    jb @@detok_loop

    ; Reverse byte-level mapping: byte = token_id - 3
    sub eax, 3
    cmp eax, 255
    ja @@detok_loop                  ; invalid token, skip
    mov BYTE PTR [rdi], al
    inc rdi
    inc ecx
    jmp @@detok_loop
