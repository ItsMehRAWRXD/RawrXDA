...
SupportResult SupportTierManager::CreateTicket(TicketPriority priority,
    ...
    uint64_t now = nowMs();
    uint64_t slaDeadline = (m_slaConfig.responseTimeMinutes == 0)
        ? UINT64_MAX  // Community: no SLA — deadline effectively never
        : now + (static_cast<uint64_t>(m_slaConfig.responseTimeMinutes) * 60000);
