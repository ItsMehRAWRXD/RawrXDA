...
extern "C" {
    ...
    // ---- Authenticated Encrypt/Decrypt (RawrXD_Camellia256_Auth.asm) ----

    // Authenticated file encryption (Encrypt-then-MAC, RCM2 format)
    int asm_camellia256_auth_encrypt_file(const char* inputPath,
                                          const char* outputPath);
    // Authenticated file decryption (HMAC-SHA256 verification + CTR decrypt)
    int asm_camellia256_auth_decrypt_file(const char* inputPath,
                                          const char* outputPath);
    // In-memory authenticated encryption (produces RCM2 container)
    int asm_camellia256_auth_encrypt_buf(uint8_t* plaintext, uint32_t plaintextLen,
                                         uint8_t* output, uint32_t* outputLen);
    // In-memory authenticated decryption (verifies HMAC before decrypt)
    int asm_camellia256_auth_decrypt_buf(const uint8_t* authData, uint32_t authDataLen,
                                         uint8_t* plaintext, uint32_t* plaintextLen);

    // ---- Bare-Metal K-Quant Kernels (RawrXD_KQuant_Kernel.asm) ----

    // Q4_K dequantization — per-block AVX2 (256 elements from 144 bytes)
    size_t asm_dequant_q4_k_avx2(float* output, const void* input);
    // Q4_K dequantization — per-block AVX-512 (256 elements, requires AVX-512F)
    size_t asm_dequant_q4_k_avx512(float* output, const void* input);
    // Q4_K dequantization — multi-block with auto-dispatch (AVX-512 or AVX2)
    size_t asm_dequant_q4_k_batch(float* output, const void* input, size_t numElements);
    // Runtime AVX-512 feature detection (cached result)
