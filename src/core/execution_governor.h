...
struct GovernoredTask {
    ...
    // Move constructor — std::atomic is not moveable, so load+store explicitly
    GovernoredTask(GovernoredTask&& other) noexcept
        : id(other.id), type(other.type), risk(other.risk),
          description(std::move(other.description)),
          command(std::move(other.command)),
          timeoutMs(other.timeoutMs),
          state(other.state.load()),
          hProcess(other.hProcess), hReadPipe(other.hReadPipe), hThread(other.hThread),
          outputBuffer(std::move(other.outputBuffer)),
          bytesRead(other.bytesRead),
          startTime(other.startTime), endTime(other.endTime),
          cancelFn(std::move(other.cancelFn)),
          onComplete(std::move(other.onComplete)),
          rollbackEnabled(other.rollbackEnabled),
          rollbackFn(std::move(other.rollbackFn))
    {
        other.hProcess = nullptr;
        other.hReadPipe = nullptr;
        other.hThread = nullptr;
    }

    // Move assignment
    GovernoredTask& operator=(GovernoredTask&& other) noexcept {
        if (this != &other) {
            id = other.id; type = other.type; risk = other.risk;
            description = std::move(other.description);
            command = std::move(other.command);
            ...
            hProcess = other.hProcess; hReadPipe = other.hReadPipe; hThread = other.hThread;
            other.hProcess = nullptr; other.hReadPipe = nullptr; other.hThread = nullptr;
            outputBuffer = std::move(other.outputBuffer);
            bytesRead = other.bytesRead;
            startTime = other.startTime; endTime = other.endTime;
            cancelFn = std::move(other.cancelFn);
    ...
    // No copy
    GovernoredTask(const GovernoredTask&) = delete;
    GovernoredTask& operator=(const GovernoredTask&) = delete;
};
...
private:
    ...
    std::map<GovernorTaskId, GovernoredTask>     m_tasks;
