...
// ================================================
// Configuration Constants (some moved to header)
// ================================================
constexpr size_t MODEL_SCAN_INTERVAL_MS = 30000;  // 30 seconds

// Helper function to create RegistryResult
inline RegistryResult make_result(RegistryStatus status, const char* message) {
...
RegistryResult registry_load_model(const char* name, void** handle) {
    ...
    for (size_t i = 0; i < g_model_count.load(); ++i) {
        if (strcmp(g_models[i].name, name) == 0) {
            ...
            // Opaque handle: real loading is done by IDE/GGUF loader; registry tracks state.
            entry.model_handle = reinterpret_cast<void*>(i + 1);
...
void registry_set_loaded_handle(const char* name, void* handle) {
    if (!name) return;
    std::lock_guard<std::mutex> lock(g_registry_mutex);
    for (size_t i = 0; i < g_model_count.load(); ++i) {
        if (strcmp(g_models[i].name, name) == 0 && g_models[i].is_loaded) {
            g_models[i].model_handle = handle;
            return;
        }
    }
}
...
void registry_cleanup() {
    std::lock_guard<std::mutex> lock(g_registry_mutex);
    // Clear loaded state; caller (IDE/loader) is responsible for releasing model resources.
    for (size_t i = 0; i < g_model_count.load(); ++i) {
        if (g_models[i].is_loaded) {
            g_models[i].is_loaded = false;
            g_models[i].model_handle = nullptr;
