...
struct NodeHardwareProfile {
    ...
    float       networkBandwidthGBps;   // Inter-node bandwidth (GB/s) — alias
    ...
    NodeHardwareProfile()
        ...
        , networkBandwidthGBs(0), networkBandwidthGBps(0), networkLatencyMs(0)
...
struct BatchSplitDescriptor {
    ...
    struct Split {
        ...
        uint32_t    batchStart;         // Alias: start index in global batch
...
struct ExecutionSchedule {
    ...
    bool        isOptimal;
    bool        needsRebalance;
    bool        forcedStrategy;         // True when strategy was explicitly forced
    ...
    ExecutionSchedule()
        ...
        , isOptimal(false), needsRebalance(false), forcedStrategy(false)
    {}
};
