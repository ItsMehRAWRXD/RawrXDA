...
// ================================================
// Request / Result Types (used by routing)
// ================================================
struct ModelRequest {
    char prompt[4096];
...
enum class RouteStatus { Ok = 0, Error, Timeout, NoRoute };
constexpr RouteStatus ROUTE_SUCCESS = RouteStatus::Ok;
constexpr RouteStatus ROUTE_ERROR = RouteStatus::Error;

struct RouteResult {
    RouteStatus status = ROUTE_ERROR;
    char message[512] = {};
    std::string response;
...
#else
// POSIX: link with libcurl and implement send_http_request via curl_easy_* for non-Win32 builds.
static RouteResult send_http_request(const ModelRoute& route, const ModelRequest& request) {
    (void)route;
    (void)request;
...
RouteResult router_route_request(const ModelRequest& request) {
    ...
    do {
        if (selected_route.is_local) {
            // Local route: wire to IDE inference engine when available; until then return synthetic success.
...
RouteResult router_get_metrics(const char* model_name, RouteMetrics* metrics) {
    ...
    for (size_t i = 0; i < g_route_count.load(); ++i) {
        if (strcmp(g_routes[i].model_name, model_name) == 0) {
            RouteMetrics& src = g_routes[i].metrics;
            metrics->request_count.store(src.request_count.load());
            metrics->success_count.store(src.success_count.load());
            metrics->error_count.store(src.error_count.load());
            metrics->total_latency_ms.store(src.total_latency_ms.load());
            metrics->last_success_time.store(src.last_success_time.load());
            metrics->is_healthy.store(src.is_healthy.load());
