...
struct PatchManifest {
    uint64_t            patchId;
    uint64_t            manifestId{0};        // Alias for UI — maps to patchId
    std::string         name;
...
struct HotpatchAuditEntry {
    ...
    uint64_t                                  patchId;
    uint64_t                                  transactionId;
    uint64_t                                  timestampUs{0};  // Microsecond epoch timestamp
...
struct ControlPlaneStats {
    ...
    std::atomic<uint64_t> totalManifests{0};
    std::atomic<uint64_t> rollbacksPerformed{0};
    std::atomic<uint64_t> validationsRun{0};
    std::atomic<uint64_t> validationFailures{0};
    std::atomic<uint64_t> transactionsCommitted{0};
    std::atomic<uint64_t> transactionsRolledBack{0};
};
