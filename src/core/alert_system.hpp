...
private:
    ...
    float queryGpuVramUsage() const;
    float queryNetworkUsage() const;
    float queryTokenRate() const;
    void logAlert(const AlertEvent& evt) const;
    void dispatchAlert(const AlertEvent& evt);

    // Token rate tracking
    mutable std::atomic<uint64_t> m_tokenCounter{0};
    mutable std::atomic<uint64_t> m_lastTokenSnapshot{0};
    mutable std::atomic<uint64_t> m_lastTokenSnapshotTimeMs{0};

public:
    // Called externally to feed token count into the rate monitor
    void recordTokensProcessed(uint64_t count) {
        m_tokenCounter.fetch_add(count, std::memory_order_relaxed);
    }
};
