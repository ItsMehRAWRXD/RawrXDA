...
std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::initialize() {
    ...
    if (bar0Result) {
        ...
        if (mapResult) {
            ...
            if (heapResult) {
                ...
                RawrXD::Logging::Logger::instance().info(
                    "[UnifiedMemory] Hardware SAM path active (~" + std::to_string(gbAvailable) + "GB heap)");
                return {};
            }
        }
    }
...
void UnifiedMemoryExecutor::shutdown() {
    ...
    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] Shutting down unified memory executor...");
    ...
    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] Unified memory executor shut down");
}
...
std::expected<UnifiedBuffer, UnifiedMemoryError> UnifiedMemoryExecutor::allocate(
    uint64_t sizeBytes, uint64_t alignment) {
    
    if (!m_initialized.load()) {
        return std::unexpected(UnifiedMemoryError::NotInitialized);
    }

    if (sizeBytes == 0) {
        return std::unexpected(UnifiedMemoryError::InvalidParameter);
    }
    ...
    if (alignedPtr + sizeBytes > m_heapLimit) {
        return std::unexpected(UnifiedMemoryError::InsufficientMemory);
    }
    ...
    buffer.ptr = reinterpret_cast<void*>(
        reinterpret_cast<uintptr_t>(m_heapBase) + alignedPtr);
    ...
    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] Allocated " + std::to_string(sizeBytes) + 
        " bytes at unified address " + std::to_string(reinterpret_cast<uintptr_t>(buffer.ptr)));

    return buffer;
}

std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::free(UnifiedBuffer& buffer) {
    if (!m_initialized.load()) {
        return std::unexpected(UnifiedMemoryError::NotInitialized);
    }
...
std::expected<UnifiedBuffer, UnifiedMemoryError> UnifiedMemoryExecutor::loadModelUnified(
    ...
    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] Loading model into unified memory: " + filePath + 
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        RawrXD::Logging::Logger::instance().error(
            "[UnifiedMemory] Failed to open model file: " + filePath);
        return std::unexpected(UnifiedMemoryError::ModelLoadFailed);
    }
    ...
    if (!hMapping) {
        CloseHandle(hFile);
        RawrXD::Logging::Logger::instance().error(
            "[UnifiedMemory] Failed to create file mapping");
        return std::unexpected(UnifiedMemoryError::ModelLoadFailed);
    }
    ...
    if (!mappedView) {
        ...
        CloseHandle(hFile);
        RawrXD::Logging::Logger::instance().error(
            "[UnifiedMemory] Failed to map view of file");
        return std::unexpected(UnifiedMemoryError::ModelLoadFailed);
    }
    ...
    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] Model loaded into unified memory at " + 
...
std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::executeLayerUnified(
    ...
    void* layerWeights = reinterpret_cast<void*>(
        reinterpret_cast<uintptr_t>(m_modelBase) + layerOffset);
    ...
    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] Executing layer " + std::to_string(layerIndex) + 
        " with unified memory");
...
std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::streamingExecutorUnified() {
    if (!m_initialized.load()) {
        return std::unexpected(UnifiedMemoryError::NotInitialized);
    }

    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] Starting streaming executor (unified memory)");
...
std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::heterogeneousScheduler() {
    if (!m_initialized.load()) {
        return std::unexpected(UnifiedMemoryError::NotInitialized);
    }

    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] Starting heterogeneous scheduler (CPU + GPU)");
...
std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::testUnifiedCoherency() {
    ...
    for (size_t i = 0; i < testSize / sizeof(uint32_t); ++i) {
        if (pattern[i] != static_cast<uint32_t>(i)) {
            RawrXD::Logging::Logger::instance().error(
                "[UnifiedMemory] Coherency test failed at offset " + std::to_string(i));
            return std::unexpected(UnifiedMemoryError::CoherencyTestFailed);
        }
    }
...
std::expected<UnifiedMemoryExecutor::BAR0Info, UnifiedMemoryError> 
UnifiedMemoryExecutor::getBAR0Info() const {
    if (!m_initialized.load()) {
        return std::unexpected(UnifiedMemoryError::NotInitialized);
    }
...
std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::initializeBAR0() {
    ...
    if (bar0 == 0 || (bar0 & 0xFFFFFFFFFFFFFFF0ULL) == 0) {
        ...
        return std::unexpected(UnifiedMemoryError::SAMNotEnabled);
    }
    ...
    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] BAR0 physical address: 0x" + 
        std::to_string(m_bar0Physical));
    return {};
}

std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::mapBAR0ToCPU() {
    if (m_bar0Physical == 0) {
        return std::unexpected(UnifiedMemoryError::SAMNotEnabled);
    }
    ...
    if (!m_bar0Virtual) {
        RawrXD::Logging::Logger::instance().error(
            "[UnifiedMemory] Failed to map BAR0 to CPU address space");
        return std::unexpected(UnifiedMemoryError::BARMappingFailed);
    }

    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] BAR0 mapped to virtual address: " + 
        std::to_string(reinterpret_cast<uintptr_t>(m_bar0Virtual)));
    return {};
}

std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::establishUnifiedHeap() {
    if (!m_bar0Virtual) {
        return std::unexpected(UnifiedMemoryError::BARMappingFailed);
    }

    // Establish unified heap (after system reserve)
    m_heapBase = reinterpret_cast<void*>(
        reinterpret_cast<uintptr_t>(m_bar0Virtual) + SYSTEM_RAM_RESERVE);
    m_heapPtr.store(0);
    ...
    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] Unified heap established: base=" + 
        std::to_string(reinterpret_cast<uintptr_t>(m_heapBase)) + 
        ", limit=" + std::to_string(limitGB) + "GB");

    return {};
}

std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::initializeGPUUnifiedMemory() {
    if (!m_bar0Virtual) {
        return std::unexpected(UnifiedMemoryError::BARMappingFailed);
    }
    ...
    if (result != 0) {
        RawrXD::Logging::Logger::instance().warning(
            "[UnifiedMemory] GPU unified memory init returned " + std::to_string(result));
        // Not fatal - continue with CPU-only unified memory
    } else {
        RawrXD::Logging::Logger::instance().info(
            "[UnifiedMemory] GPU unified memory initialized");
    }

    return {};
}

std::expected<void, UnifiedMemoryError> UnifiedMemoryExecutor::initializeHostBackedUnified() {
    ...
    if (!arena) {
        RawrXD::Logging::Logger::instance().error(
            "[UnifiedMemory] Host-backed VirtualAlloc failed");
        return std::unexpected(UnifiedMemoryError::AllocationFailed);
    }
    ...
    RawrXD::Logging::Logger::instance().info(
        "[UnifiedMemory] Host-backed unified arena ready: " + std::to_string(mib) + " MiB bump heap");
    return {};
}
