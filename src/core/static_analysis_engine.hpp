...
struct BasicBlock {
    ...
    std::vector<uint64_t> instructions;         // Alias for UI panel access
    ...
    int32_t             immediateDominator{-1}; // Alias for UI panel access
...
struct LoopInfo {
    ...
    uint32_t                headerBlockId{0};   // Alias for UI panel access
    std::unordered_set<uint32_t> bodyBlocks;    // All blocks in the loop
    std::vector<uint32_t>   bodyBlockIds;       // Indexed access for UI panel
    std::vector<uint32_t>   backEdges;          // Back edge sources
    uint32_t                depth;              // Nesting depth
    uint32_t                nestingDepth{0};    // Alias for UI panel access
...
struct AnalysisStats {
    ...
    // Extended stats — UI panel counters
    std::atomic<uint64_t> edgesCreated{0};
    std::atomic<uint64_t> phiNodesInserted{0};
    std::atomic<uint64_t> optimizationsApplied{0};
    std::atomic<uint64_t> analysesRun{0};
};
