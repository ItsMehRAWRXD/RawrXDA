...
void Win32IDE::initMCP() {
    ...
    // Prompt: Code Review
    {
        ...
        m_mcpServer->registerPrompt(tmpl, [](const std::string& argsJson) -> std::vector<RawrXD::MCP::PromptMessage> {
            // Extract language argument from JSON
            std::string language;
            {
                auto langPos = argsJson.find("\"language\"");
                if (langPos != std::string::npos) {
                    auto vs = argsJson.find('"', langPos + 10);
                    if (vs != std::string::npos) { vs++;
                        auto ve = argsJson.find('"', vs);
                        if (ve != std::string::npos) language = argsJson.substr(vs, ve - vs);
                    }
                }
            }
            // Extract code argument from JSON
            std::string code;
