...
public:
    PeekOverlayWindow(HWND hwndParent, const std::vector<PeekItem>& items, int triggerLine, int triggerCol);
    ~PeekOverlayWindow();
...
void PeekOverlayWindowDeleter::operator()(PeekOverlayWindow* p) noexcept { delete p; }

PeekOverlayWindow::PeekOverlayWindow(HWND hwndParent, const std::vector<PeekItem>& items, 
                                   int triggerLine, int triggerCol)
    : m_hwnd(nullptr), m_hwndParent(hwndParent), m_items(items), m_currentIndex(0),
    ...
    m_hwnd = CreateWindowExW(WS_EX_TOPMOST | WS_EX_TOOLWINDOW,
                            L"RawrXD_PeekOverlay", L"Peek",
                            WS_POPUP | WS_BORDER | WS_VISIBLE,
                            x, y, width, height,
                            m_hwndParent, nullptr, GetModuleHandle(nullptr), this);
    ...
    switch (vk) {
        ...
        case VK_RETURN:
            ...
            if (m_currentIndex >= 0 && m_currentIndex < (int)m_items.size()) {
                ...
                // TODO: Navigate to item.file, item.line, item.column
                hide();
            }
            break;
    }
...
void PeekOverlayWindow::renderItem(HDC hdc, const PeekItem& item, int x, int y, int width, int height) {
    ...
    HFONT contentFont = CreateFontW(-12, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                                   OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
...
void Win32IDE::showPeekDefinition(int line, int col) {
    // Find definitions at position
    std::vector<PeekItem> items = findDefinitionsAt(line, col);
    if (items.empty()) return;
    
    m_peekOverlayWindow.reset(new PeekOverlayWindow(m_hwndMain, items, line, col));
...
std::vector<PeekItem> Win32IDE::findDefinitionsAt(int line, int col) {
    ...
    if (m_currentFile.empty()) return items;
    
    std::string uri = filePathToUri(m_currentFile);
    auto locations = lspGotoDefinition(uri, line - 1, col - 1); // Convert to 0-based
    
    for (const auto& loc : locations) {
        PeekItem item;
        item.file = uriToFilePath(loc.uri);
        item.line = loc.range.start.line + 1; // Convert to 1-based
        item.column = loc.range.start.character + 1; // Convert to 1-based
        ...
        if (file.is_open()) {
            ...
            // Get context around the definition (3 lines before and after)
            int startLine = std::max(0, item.line - 4); // 0-based for vector access
            int endLine = std::min((int)lines.size() - 1, item.line + 2);
            
            std::string content;
            for (int i = startLine; i <= endLine; ++i) {
                if (i == item.line - 1) { // Highlight the definition line
                    content += ">>> " + lines[i] + "\n";
                } else {
                    content += "    " + lines[i] + "\n";
                }
            ...
            if (item.line - 1 < (int)lines.size()) {
                std::string defLine = lines[item.line - 1];
                // Extract function/class name (simple heuristic)
            ...
            item.title = "Definition at line " + std::to_string(item.line);
...
std::vector<PeekItem> Win32IDE::findReferencesAt(int line, int col) {
    ...
    std::string uri = filePathToUri(m_currentFile);
    auto locations = lspFindReferences(uri, line - 1, col - 1); // Convert to 0-based
    ...
    for (const auto& loc : locations) {
        ...
        if (file.is_open()) {
            ...
            // Get context around the reference (2 lines before and after)
            int startLine = std::max(0, item.line - 3); // 0-based for vector access
            int endLine = std::min((int)lines.size() - 1, item.line + 1);
            
            std::string content;
            for (int i = startLine; i <= endLine; ++i) {
                if (i == item.line - 1) { // Highlight the reference line
                    content += ">>> " + lines[i] + "\n";
                } else {
                    content += "    " + lines[i] + "\n";
                }
            ...
            // Create a meaningful title
            if (item.line - 1 < (int)lines.size()) {
                std::string refLine = lines[item.line - 1];
                // Extract the reference (simple heuristic - could be improved)
...
void Win32IDE::handlePeekOverlayKey(UINT vk, bool ctrl, bool alt, bool shift) {
    if (!m_peekOverlayActive || !m_peekOverlayWindow) return;
