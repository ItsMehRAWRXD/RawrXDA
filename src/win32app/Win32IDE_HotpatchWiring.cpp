...
static BackendRequestType classifyBackendRequestType(const std::string& prompt)
{
    if (prompt.find("\"request_type\":\"LSP_COMPLETION\"") != std::string::npos ||
        prompt.find("request_type=LSP_COMPLETION") != std::string::npos || prompt.rfind("LSP_COMPLETION:", 0) == 0 ||
    ...
    if (prompt.find("\"request_type\":\"ARCH_REASONING\"") != std::string::npos ||
        prompt.find("request_type=ARCH_REASONING") != std::string::npos || prompt.find("swarm") != std::string::npos)
    {
...
static bool tryLspNamedPipeFastPath(const std::string& prompt, std::string& outResponse, std::string& outError)
{
    if (prompt.empty())
    {
    ...
    HANDLE hPipe = CreateFileA(kLspFastPathPipeName, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING,
    ...
    {
        std::string payload =
            "{\"request_type\":\"LSP_COMPLETION\",\"prompt\":\"" + jsonEscapeFastPath(prompt) + "\"}\n";
...
using InferenceTransportDetourFn = bool (*)(HeadlessIDE* ide, const std::string& prompt, bool cloudSelected,
                                            const HeadlessConfig& config, bool modelLoaded,
                                            const std::string& loadedModelName, InferenceTransportTelemetry& telemetry,
                                            std::string& outResponse);

static bool inferenceTransportPrimary(HeadlessIDE* ide, const std::string& prompt, bool cloudSelected,
                                      const HeadlessConfig& config, bool modelLoaded,
                                      const std::string& loadedModelName, InferenceTransportTelemetry& telemetry,
                                      std::string& outResponse);

static bool inferenceTransportRecovery(HeadlessIDE* ide, const std::string& prompt, bool cloudSelected,
                                       const HeadlessConfig& config, bool modelLoaded,
                                       const std::string& loadedModelName, InferenceTransportTelemetry& telemetry,
                                       std::string& outResponse);
...
static bool performNativeTransport(HeadlessIDE* ide, const std::string& prompt, bool cloudSelected,
                                   const HeadlessConfig& config, bool modelLoaded, const std::string& loadedModelName,
                                   int timeoutMs, int maxTokens, float temperature,
                                   InferenceTransportTelemetry& telemetry, std::string& outResponse)
{
    ...
    {
        sink->appendOutput("Cloud backend selected in headless mode: routing through embedded native transport",
    ...
    messages.push_back({"system",
                        "You are RawrXD IDE's embedded AI assistant. "
    ...
    if (result.success)
    {
        ...
        {
            char perf[320];
            snprintf(perf, sizeof(perf), "[inference] %llu prompt + %llu completion tokens, %.1f tok/s, %.0f ms",
                     (unsigned long long)result.prompt_tokens, (unsigned long long)result.completion_tokens,
    ...
    if (sink)
    {
        sink->appendOutput(("Native transport failed: " + result.error_message).c_str(), OutputSeverity::Warning);
    }
    return true;
}
...
static bool inferenceTransportPrimary(HeadlessIDE* ide, const std::string& prompt, bool cloudSelected,
                                      ...
                                      std::string& outResponse)
{
    return performNativeTransport(ide, prompt, cloudSelected, config, modelLoaded, loadedModelName,
                                  static_cast<int>(kTitanMaxLatencyMs), std::max(1, config.maxTokens),
                                  config.temperature, telemetry, outResponse);
}
                                       ...
                                       std::string& outResponse)
{
    ...
    {
        ide->getOutputSink()->appendOutput("Titan recovery detour active (2000ms/3.5 tok/s guardrails)",
                                           OutputSeverity::Warning);
    }
    return performNativeTransport(ide, prompt, cloudSelected, config, modelLoaded, loadedModelName,
                                  static_cast<int>(kTitanMaxLatencyMs), recoveryMaxTokens, recoveryTemp, telemetry,
                                  outResponse);
}
...
static void engageRecoveryDetour(IOutputSink* sink, const char* reason)
{
    ...
    if (g_activeInferenceTransport.compare_exchange_strong(expected, &inferenceTransportRecovery,
                                                           std::memory_order_acq_rel, std::memory_order_acquire))
    {
        if (sink)
        {
            const std::string msg =
                std::string("[titan-guard] Recovery detour engaged: ") + (reason ? reason : "threshold breach");
...
static void restorePrimaryDetour(IOutputSink* sink)
{
    ...
    if (g_activeInferenceTransport.compare_exchange_strong(expected, &inferenceTransportPrimary,
                                                           std::memory_order_acq_rel, std::memory_order_acquire))
    {
        if (sink)
        {
...
static void tryLoadOptionalEngineLogged(EngineManager* manager, const char* enginePath, const char* engineId)
{
    ...
    try
    {
        ...
        if (!loaded)
        {
            std::cerr << "[HeadlessIDE] Optional engine load failed: id=" << engineId << " path=" << enginePath
                      << std::endl;
        }
    }
    catch (const std::exception& ex)
    {
        std::cerr << "[HeadlessIDE] Optional engine load threw: id=" << engineId << " path=" << enginePath
...
HeadlessResult HeadlessIDE::initBackendManager()
{
    ...
    // Probe the embedded native transport (primary backend).
    RawrXD::Agent::NativeInferenceConfig nativeCfg;
    nativeCfg.host = "127.0.0.1";
    nativeCfg.port = 11435;
    nativeCfg.timeout_ms = 3000;
    RawrXD::Agent::NativeInferenceClient probeClient(nativeCfg);
    bool nativeAvailable = probeClient.TestConnection();
    ...
    if (nativeAvailable)
    {
        ...
        statusMsg << " | native: online (" << models.size() << " models)";
        // Default to the embedded native transport if available and no explicit backend set.
        if (m_config.backend.empty())
        {
            m_activeBackend = AIBackendType::native;
        }
...
HeadlessResult HeadlessIDE::initLLMRouter()
{
    ...
    // Priority: embedded native transport > LocalGGUF > Cloud backends
    struct RouterEntry
    {
        AIBackendType type;
        const char* name;
                                        ...
                                        std::function<void(const StreamChunk&)> chunkCallback)
{
    ...
    // Use NativeInferenceClient streaming API for real per-token delivery.
    if (m_activeBackend == AIBackendType::native || m_activeBackend == AIBackendType::LocalGGUF)
    {
        RawrXD::Agent::NativeInferenceConfig cfg;
        cfg.host = "127.0.0.1";
        cfg.port = 11435;
        ...
        // Same routing as HeadlessIDE: direct embedded-native NDJSON unless
        // RAWRXD_STREAM_VIA_ORCHESTRATOR / FALLBACK set.
...
bool HeadlessIDE::probeBackendHealth(AIBackendType type)
{
    switch (type)
    {
        ...
        case AIBackendType::native:
        {
            // Probe the embedded native transport.
            RawrXD::Agent::NativeInferenceConfig cfg;
            cfg.host = "127.0.0.1";
            cfg.port = 11435;
            ...
            if (connected)
            {
                ...
                m_outputSink->appendOutput(("native: " + std::to_string(models.size()) + " models available").c_str(),
                                           OutputSeverity::Debug);
            }
            return connected;
        }
        ...
        case AIBackendType::Gemini:
            ...
            // The request path can still fall back to the embedded native transport when cloud transport is unavailable.
...
std::string HeadlessIDE::routeInferenceRequest(const std::string& prompt)
{
    ...
    // Cloud modes currently reuse the embedded native transport as a practical fallback.
    const bool cloudSelected = (m_activeBackend == AIBackendType::OpenAI || m_activeBackend == AIBackendType::Claude ||
                                m_activeBackend == AIBackendType::Gemini);
    const bool canUseLocalTransport =
        (m_activeBackend == AIBackendType::native || m_activeBackend == AIBackendType::LocalGGUF ||
         (cloudSelected && m_backendManagerInitialized));

    if (canUseLocalTransport)
    {
        InferenceTransportDetourFn detour = g_activeInferenceTransport.load(std::memory_order_acquire);
        if (!detour)
        {
        ...
        const bool attempted = detour(this, prompt, cloudSelected, m_config, m_modelLoaded, m_loadedModelName,
                                      telemetry, transportResponse);

        if (attempted)
        {
            g_titanLastLatencyMs.store(telemetry.durationMs > 0.0 ? static_cast<uint64_t>(telemetry.durationMs) : 0,
                                       std::memory_order_release);
            g_titanLastTpsMilli.store(telemetry.tps > 0.0 ? static_cast<uint64_t>(telemetry.tps * 1000.0) : 0,
                                      std::memory_order_release);
            ...
            if (m_phase10Initialized)
            {
                ...
                const ConfidenceEvaluation eval = confidence.evaluateAndRecord(
                    confidenceScore, ActionClass::QueryModel, SafetyRiskTier::High, "titan_inference_guard");
            ...
            if (breach)
            {
                const uint32_t breaches = g_titanConsecutiveBreaches.fetch_add(1, std::memory_order_acq_rel) + 1;
                g_titanConsecutiveHealthy.store(0, std::memory_order_release);
                ...
                if (m_hotpatchInitialized)
                {
                    ...
                    if (hp.isInitialized())
                    {
                        ...
                        if (!surgery.success)
                        {
                            m_outputSink->appendOutput(("[titan-guard] Hotpatch pressure response failed: " +
                                                        std::string(surgery.detail ? surgery.detail : "unknown"))
                                                           .c_str(),
                                                       OutputSeverity::Warning);
                        }
                    }
                ...
                char breachBuf[320];
                snprintf(breachBuf, sizeof(breachBuf),
                         "[titan-guard] breach #%u latency=%.0fms tps=%.2f (budget %.0fms / %.1f tok/s)", breaches,
                         telemetry.durationMs, telemetry.tps, kTitanMaxLatencyMs, kTitanMinTokensPerSecond);
            ...
            {
                g_titanConsecutiveBreaches.store(0, std::memory_order_release);
                const uint32_t healthy = g_titanConsecutiveHealthy.fetch_add(1, std::memory_order_acq_rel) + 1;
            ...
            {
                m_outputSink->appendOutput(("Native transport failed: " + telemetry.error).c_str(),
                                           OutputSeverity::Warning);
            }
        }
    }
    ...
    // Final fallback: provide actionable error
        return "[error] Inference unavailable — ensure the embedded native transport is running on port 11435 "
...
std::string HeadlessIDE::getHotpatchStatus() const
{
    ...
    oss << "Titan guard: " << (detour == &inferenceTransportRecovery ? "RECOVERY" : "PRIMARY")
        << " (budget=" << static_cast<int>(kTitanMaxLatencyMs) << "ms/" << kTitanMinTokensPerSecond << " tok/s)\n";
...
void HeadlessIDE::handleClient(SOCKET clientFd)
{
    ...
    const unsigned long requestId =
        static_cast<unsigned long>(requestStartMs ^ static_cast<uint64_t>(GetCurrentThreadId()));
    ...
    auto stripQueryAndFragment = [](const std::string& value)
    {
        ...
        const size_t cut = (q == std::string::npos) ? h : ((h == std::string::npos) ? q : std::min(q, h));
    ...
    {
        return route == "/" || route == "/health" || route == "/api/health" || route == "/api/status" ||
               route == "/api/headless/status" || route == "/api/version" || route == "/api/features" ||
               route == "/api/manifest";
    };
    ...
    else if (path == "/api/version")
    {
        ...
        {
            responseBody = "{\"version\":\"" + std::string(VERSION) + "\",\"phase\":\"" + BUILD_PHASE +
                           "\",\"mode\":\"headless\"}";
        }
    }
...
int HeadlessIDE::runServerMode()
{
    ...
    // Block until shutdown (tight poll so stop is responsive)
