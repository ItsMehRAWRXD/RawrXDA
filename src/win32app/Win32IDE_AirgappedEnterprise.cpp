...
static const char* complianceToString(ComplianceFramework f)
{
    switch (f)
    {
        ...
        case ComplianceFramework::FedRAMP:
            return "FedRAMP";
        case ComplianceFramework::ITAR:
            return "ITAR";
        case ComplianceFramework::EAR:
            return "EAR";
        case ComplianceFramework::NIST_800_171:
            return "NIST 800-171";
    }
    return "Unknown";
}
...
static const char* checkStatusToString(ComplianceCheckStatus s)
{
    switch (s)
    {
        ...
        case ComplianceCheckStatus::Fail:
            return "FAIL";
        case ComplianceCheckStatus::Warning:
            return "WARN";
        case ComplianceCheckStatus::NotApplicable:
            return "N/A";
...
static std::string getTimestampEnt()
{
    ...
    char buf[64];
    snprintf(buf, sizeof(buf), "%04d-%02d-%02dT%02d:%02d:%02d", local.tm_year + 1900, local.tm_mon + 1, local.tm_mday,
...
static void appendAuditLog(const std::string& actor, const std::string& action, const std::string& target,
                           const std::string& detail)
{
    ...
    std::string hashInput = std::to_string(entry.id) + entry.timestamp + entry.actor + entry.action + entry.target +
                            entry.detail + prevHash;
...
static void generateComplianceChecks()
{
    ...
    CheckDef checks[] = {
        ...
        {ComplianceFramework::FedRAMP, "AC-2", "Account Management",
         "Information system accounts are managed, authorized, and monitored", licenseValid && hasHWIDLock, true,
         licenseValid ? "License validated, HWID=" + s_licenseStatus.hwid : "License NOT validated — no HWID binding"},
        {ComplianceFramework::FedRAMP, "AC-17", "Remote Access",
         "Remote access connections are controlled and monitored", airgapVerified, true,
         airgapVerified
             ? "Airgap verified — 0 external connections"
             : "FAIL: " + std::to_string(s_firewallStatus.activeConnections) + " active connections detected"},
        {ComplianceFramework::FedRAMP, "AU-2", "Audit Events", "System generates audit records for defined events",
         auditActive && auditChainIntact, true,
         auditActive ? "Hash-chained audit log active (" + std::to_string(auditCount) + " entries, chain " +
                           (auditChainIntact ? "intact" : "BROKEN") + ")"
                     : "FAIL: Audit log is empty — no events recorded"},
        {ComplianceFramework::FedRAMP, "SC-8", "Transmission Confidentiality", "Information in transit is protected",
         airgapVerified, true,
         airgapVerified ? "No network transmission — fully offline"
                        : "FAIL: Network connections detected — data may be in transit"},
        {ComplianceFramework::FedRAMP, "SC-28", "Protection of Information at Rest", "Data at rest is encrypted",
        ...
        {ComplianceFramework::FedRAMP, "SI-4", "Information System Monitoring",
         "System is monitored for attacks and anomalies", auditActive && dlpActive, true,
         std::string("Audit log: ") + (auditActive ? "active" : "inactive") +
             ", DLP: " + (dlpActive ? "active" : "inactive")},

        // ITAR
        {ComplianceFramework::ITAR, "ITAR-120", "Export Controls", "Technical data is not exported to non-US persons",
         airgapVerified, true,
         airgapVerified ? "Airgap prevents all network egress"
                        : "FAIL: Network egress detected — export control may be violated"},
        {ComplianceFramework::ITAR, "ITAR-121", "Data Handling", "Classified technical data handling procedures",
         dlpActive, true,
         dlpActive ? "DLP rules active (" + std::to_string(s_dlpRules.size()) + " rules) for classified markers"
                   : "FAIL: DLP rules not initialized"},
        {ComplianceFramework::ITAR, "ITAR-122", "Registration", "Manufacturer registration with DDTC", false, false,
         "Responsibility of deploying organization"},

        // NIST 800-171
        {ComplianceFramework::NIST_800_171, "3.1.1", "Authorized Access Control",
         "Limit system access to authorized users", licenseValid && hasHWIDLock, true,
        ...
        {ComplianceFramework::NIST_800_171, "3.1.2", "Transaction Control",
         "Limit system access to authorized transactions", true, true, "Agent safety contracts enforce boundaries"},
        {ComplianceFramework::NIST_800_171, "3.3.1", "Audit Log Creation", "Create and retain system audit logs",
         auditActive && auditChainIntact, true,
         auditActive ? "Tamper-evident audit log: " + std::to_string(auditCount) + " entries"
                     : "FAIL: No audit log entries"},
        {ComplianceFramework::NIST_800_171, "3.5.1", "Identification", "Identify system users and processes",
         licenseValid, true, "Session identity: " + s_licenseStatus.licensee + " (tier: " + s_licenseStatus.tier + ")"},
        {ComplianceFramework::NIST_800_171, "3.13.1", "Boundary Protection",
         "Monitor and control communications at system boundary", airgapVerified, true,
        ...
        {ComplianceFramework::GDPR, "Art.25", "Data Protection by Design",
         "Data protection measures built into processing activities", airgapVerified, true,
         airgapVerified ? "Local-only processing, no cloud dependency"
                        : "WARNING: Network activity detected — verify no PII egress"},
        {ComplianceFramework::GDPR, "Art.30", "Records of Processing", "Maintain records of processing activities",
         auditActive, true,
        ...
        {ComplianceFramework::GDPR, "Art.32", "Security of Processing",
         "Appropriate technical measures for data security", encryptionReady, true,
        ...
        {ComplianceFramework::SOX, "SOX-302", "Management Assessment",
         "Management certifies accuracy of financial reports", false, false, "IDE is a development tool — N/A"},
        {ComplianceFramework::SOX, "SOX-404", "Internal Controls",
         "Assessment of internal control over financial reporting", auditActive, true,
         auditActive ? "Change tracking via provable agent + audit log" : "FAIL: No audit trail for change tracking"},

        // HIPAA
        {ComplianceFramework::HIPAA, "164.312(a)", "Access Control",
         "Implement technical policies for electronic PHI access", licenseValid, true,
        ...
        {ComplianceFramework::HIPAA, "164.312(c)", "Integrity Controls",
         "Protect ePHI from improper alteration or destruction", auditActive && auditChainIntact, true,
         auditChainIntact ? "Hash-chain audit trail intact" : "FAIL: Audit chain integrity compromised"},
        {ComplianceFramework::HIPAA, "164.312(e)", "Transmission Security",
         "Guard against unauthorized access to ePHI during transmission", airgapVerified, true,
         airgapVerified ? "No network transmission — airgapped" : "FAIL: Network connections detected"},
    };
        ...
        if (pTcpTable && GetTcpTable2(pTcpTable, &size, TRUE) == NO_ERROR)
        {
            ...
            for (DWORD i = 0; i < pTcpTable->dwNumEntries; ++i)
            {
                if (pTcpTable->table[i].dwState == MIB_TCP_STATE_ESTAB)
                {
                    ...
                    char ipBuf[32];
                    snprintf(ipBuf, sizeof(ipBuf), "%d.%d.%d.%d:%d", b1, b2, (remoteAddr >> 16) & 0xFF,
                             (remoteAddr >> 24) & 0xFF, ntohs((u_short)pTcpTable->table[i].dwRemotePort));
...
static void scanModelVault(const std::string& vaultDir = "models")
{
    ...
    if (hFind != INVALID_HANDLE_VALUE)
    {
        do
        {
            ...
            static const char* quantNames[] = {"Q2_K",  "Q3_K_S", "Q3_K_M", "Q3_K_L", "Q4_0",  "Q4_K_S", "Q4_K_M",
            ...
            char timeBuf[64];
            snprintf(timeBuf, sizeof(timeBuf), "%04d-%02d-%02dT%02d:%02d:%02d", stLocal.wYear, stLocal.wMonth,
...
static void validateLicenseOffline()
{
    ...
    if (licFile.is_open())
    {
        ...
        if (licFields.count("signature") && !licFields["signature"].empty())
        {
            ...
            std::string payload = s_licenseStatus.licensee + "|" + s_licenseStatus.tier + "|" +
                                  std::to_string(s_licenseStatus.seatCount) + "|" + s_licenseStatus.expiryDate + "|" +
                                  licFields["features"] + "|" + (licFields.count("hwid") ? licFields["hwid"] : "");
...
static LRESULT CALLBACK entPanelWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
        case WM_CREATE:
        {
            ...
            HFONT hBold = CreateFontW(-16, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                                      CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH, L"Segoe UI");
            HFONT hMono = CreateFontW(-12, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                                      CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, FIXED_PITCH, L"Cascadia Mono");

            // Header
            HWND hTitle =
                CreateWindowExW(0, L"STATIC", L"\U0001F512  Airgapped Enterprise AI Dev Environment",
                                WS_CHILD | WS_VISIBLE | SS_LEFT, 15, 8, 800, 24, hwnd, nullptr, hInst, nullptr);
            SendMessageW(hTitle, WM_SETFONT, (WPARAM)hBold, TRUE);

            // Status bar
            wchar_t statusBuf[256];
            swprintf(statusBuf, 256, L"License: %hs (%hs)  |  Airgap: %s  |  Models: %zu  |  Audit: %zu entries",
                     s_licenseStatus.tier.c_str(), s_licenseStatus.licensee.c_str(),
                     s_firewallStatus.airgapVerified ? L"\u2705" : L"\u274C", s_modelVault.size(), s_auditLog.size());
            HWND hStatus = CreateWindowExW(0, L"STATIC", statusBuf, WS_CHILD | WS_VISIBLE | SS_LEFT, 15, 34, 900, 18,
                                           hwnd, (HMENU)9210, hInst, nullptr);
            SendMessageW(hStatus, WM_SETFONT, (WPARAM)hFont, TRUE);

            // Tab control
            s_hwndEntTab = CreateWindowExW(0, WC_TABCONTROLW, L"", WS_CHILD | WS_VISIBLE | TCS_FIXEDWIDTH, 15, 56, 940,
                                           28, hwnd, (HMENU)9200, hInst, nullptr);
            ...
            s_hwndEntList = CreateWindowExW(WS_EX_CLIENTEDGE, WC_LISTVIEWW, L"",
                                            WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS, 15,
                                            90, 940, 340, hwnd, (HMENU)9220, hInst, nullptr);
            ...
            s_hwndEntOutput = CreateWindowExW(WS_EX_CLIENTEDGE, MSFTEDIT_CLASS, L"",
                                              WS_CHILD | WS_VSCROLL | ES_MULTILINE | ES_READONLY, 15, 90, 940, 340,
                                              hwnd, (HMENU)9221, hInst, nullptr);
            ...
            {
                HWND h = CreateWindowExW(0, L"BUTTON", label, WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, x, btnY, w, 30,
        ...
        case WM_COMMAND:
        {
            HWND hwndParent = GetParent(hwnd);
            if (hwndParent)
                PostMessageW(hwndParent, WM_COMMAND, LOWORD(wParam), 0);
            return 0;
        }
...
{
    if (m_airgappedEnterpriseInitialized)
        return;
    ...
    appendAuditLog("system", "startup", "RawrXD-IDE", "Airgapped enterprise environment initialized");
...
bool Win32IDE::handleAirgappedCommand(int commandId)
{
    ...
    switch (commandId)
    {
        case IDM_AIRGAP_SHOW:
            cmdAirgapShow();
            return true;
        case IDM_AIRGAP_COMPLIANCE:
            cmdAirgapCompliance();
            return true;
        case IDM_AIRGAP_MODELS:
            cmdAirgapModels();
            return true;
        case IDM_AIRGAP_AUDIT:
            cmdAirgapAudit();
            return true;
        case IDM_AIRGAP_DLP:
            cmdAirgapDLP();
            return true;
        case IDM_AIRGAP_FIREWALL:
            cmdAirgapFirewall();
            return true;
        case IDM_AIRGAP_LICENSE:
            cmdAirgapLicense();
            return true;
        case IDM_AIRGAP_ENCRYPT:
            cmdAirgapEncrypt();
            return true;
        case IDM_AIRGAP_EXPORT:
            cmdAirgapExport();
            return true;
        case IDM_AIRGAP_STATS:
            cmdAirgapStats();
            return true;
        case IDM_AIRGAP_DIODE_SCAN:
            cmdAirgapDiodeScan();
            return true;
        case IDM_AIRGAP_SNEAKER_PACK:
            cmdAirgapSneakerPack();
            return true;
        case IDM_AIRGAP_SNEAKER_UNPACK:
            cmdAirgapSneakerUnpack();
            return true;
        default:
            return false;
    }
...
void Win32IDE::cmdAirgapShow()
{
    ...
    s_hwndEntPanel =
        CreateWindowExW(WS_EX_APPWINDOW, ENT_PANEL_CLASS, L"RawrXD — Airgapped Enterprise AI Dev Environment",
                        WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_CLIPCHILDREN, CW_USEDEFAULT,
                        CW_USEDEFAULT, 990, 560, m_hwndMain, nullptr, GetModuleHandleW(nullptr), nullptr);
    if (s_hwndEntPanel)
    {
...
void Win32IDE::cmdAirgapCompliance()
{
    ...
    std::ostringstream oss;
    oss << "[Enterprise] Compliance scan — " << s_complianceChecks.size() << " controls checked:\n"
        << "  PASS: " << pass << "  FAIL: " << fail << "  WARN: " << warn << "  N/A: " << na << "\n";
    appendToOutput(oss.str());
...
void Win32IDE::cmdAirgapModels()
{
    ...
    appendToOutput("[Enterprise] Model vault: " + std::to_string(s_modelVault.size()) + " models available offline.\n");
}
...
void Win32IDE::cmdAirgapAudit()
{
    if (s_hwndEntList && IsWindowVisible(s_hwndEntList))
    {
        ...
        for (size_t i = 0; i < s_auditLog.size(); ++i)
        {
            ...
            wchar_t idBuf[16];
            swprintf(idBuf, 16, L"%d", e.id);
            lvi.pszText = idBuf;
            SendMessageW(s_hwndEntList, LVM_INSERTITEMW, 0, (LPARAM)&lvi);

            wchar_t tsBuf[32];
            MultiByteToWideChar(CP_UTF8, 0, e.timestamp.c_str(), -1, tsBuf, 32);
            lvi.iSubItem = 1;
            lvi.pszText = tsBuf;
            SendMessageW(s_hwndEntList, LVM_SETITEMTEXTW, i, (LPARAM)&lvi);

            wchar_t actorBuf[16];
            MultiByteToWideChar(CP_UTF8, 0, e.actor.c_str(), -1, actorBuf, 16);
            lvi.iSubItem = 2;
            lvi.pszText = actorBuf;
            SendMessageW(s_hwndEntList, LVM_SETITEMTEXTW, i, (LPARAM)&lvi);

            wchar_t actionBuf[32];
            MultiByteToWideChar(CP_UTF8, 0, e.action.c_str(), -1, actionBuf, 32);
            lvi.iSubItem = 3;
            lvi.pszText = actionBuf;
            SendMessageW(s_hwndEntList, LVM_SETITEMTEXTW, i, (LPARAM)&lvi);

            wchar_t targetBuf[64];
            MultiByteToWideChar(CP_UTF8, 0, e.target.c_str(), -1, targetBuf, 64);
            lvi.iSubItem = 4;
            lvi.pszText = targetBuf;
            SendMessageW(s_hwndEntList, LVM_SETITEMTEXTW, i, (LPARAM)&lvi);
    ...
    appendToOutput("[Enterprise] Audit log: " + std::to_string(s_auditLog.size()) + " tamper-evident entries.\n");
}
...
void Win32IDE::cmdAirgapDLP()
{
    ...
    appendAuditLog("system", "dlp_scan", "editor_content", "Total DLP hits: " + std::to_string(totalHits));
}
...
void Win32IDE::cmdAirgapEncrypt()
{
    ...
    static const uint8_t pbkdf2Salt[16] = {0xCA, 0x3E, 0x11, 0xA2, 0x56, 0xBE, 0xEF, 0x01,
    ...
    NTSTATUS ntStatus =
        BCryptOpenAlgorithmProvider(&hPrf, BCRYPT_SHA256_ALGORITHM, nullptr, BCRYPT_ALG_HANDLE_HMAC_FLAG);
    if (BCRYPT_SUCCESS(ntStatus))
    {
        ntStatus = BCryptDeriveKeyPBKDF2(hPrf, (PUCHAR)password.data(), (ULONG)password.size(), (PUCHAR)pbkdf2Salt,
                                         sizeof(pbkdf2Salt),
    ...
    {
        oss << "[Enterprise] ERROR: Camellia-256 block encrypt failed (code " << blockResult << ")\n"
            << "  Encryption NOT active.\n";
    }
...
void Win32IDE::cmdAirgapExport()
{
    ...
    for (auto& c : s_complianceChecks)
    {
        if (c.status == ComplianceCheckStatus::Pass)
            pass++;
        if (c.status == ComplianceCheckStatus::Fail)
            fail++;
    }
    ofs << "    \"pass\": " << pass << ",\n"
        << "    \"fail\": " << fail << "\n"
...
void Win32IDE::cmdAirgapStats()
{
    ...
    for (auto& m : s_modelVault)
        totalModelSize += m.sizeBytes;

    int dlpHits = 0;
    for (auto& r : s_dlpRules)
        dlpHits += r.hitCount;
    ...
    oss << "╔══════════════════════════════════════════════════════════════╗\n"
        ...
        << "║  Compliance checks:  " << s_complianceChecks.size() << " (pass=" << pass << ", fail=" << fail << ")\n"
        << "║  Frameworks:\n";
    for (auto& [fw, count] : byFramework)
    {
        oss << "║    " << fw << ": " << count << " controls\n";
    }
    oss << "║  Models in vault:    " << s_modelVault.size() << " (" << std::fixed << std::setprecision(1)
        << (totalModelSize / 1000000000.0) << " GB total)\n"
        << "║  Audit log entries:  " << s_auditLog.size() << "\n"
        << "║  DLP rules:          " << s_dlpRules.size() << " (" << dlpHits << " total hits)\n"
        << "║  Active connections: " << s_firewallStatus.activeConnections << "\n"
        << "╚══════════════════════════════════════════════════════════════╝\n";
    appendToOutput(oss.str());
}

void Win32IDE::cmdAirgapDiodeScan()
{
    const char* envStaging = std::getenv("RAWRXD_DIODE_STAGING");
    std::string path =
        (envStaging && envStaging[0]) ? std::string(envStaging) : rawrxd::data_diode::defaultDiodeIngestPathUtf8();
    if (auto ensured = rawrxd::data_diode::ensureDirectoryUtf8(path); !ensured)
    {
        appendToOutput(std::string("[Enterprise][Diode] Could not create staging directory: ") + path + " (" +
                       rawrxd::data_diode::diodeErrorMessage(ensured.error()) + ")\n");
        return;
    }
    auto scanned = rawrxd::data_diode::scanDiodeStagingDirectory(path);
    if (!scanned)
    {
        appendToOutput(std::string("[Enterprise][Diode] Staging scan failed: ") +
                       rawrxd::data_diode::diodeErrorMessage(scanned.error()) + "\n");
        appendAuditLog("system", "diode_staging_scan", path, "scan failed");
        return;
    }
    std::ostringstream oss;
    oss << "[Enterprise][Diode] Staging directory (uni-directional ingest): " << path << "\n"
        << "  Files: " << scanned->size() << "\n";
    for (const auto& e : *scanned)
    {
        oss << "  - " << e.relativePath << "  (" << e.sizeBytes << " bytes)  sha256=" << e.sha256Hex << "\n";
    }
    appendToOutput(oss.str());
    appendAuditLog("system", "diode_staging_scan", path, "scan ok, " + std::to_string(scanned->size()) + " files");
}

void Win32IDE::cmdAirgapSneakerPack()
{
    const char* secret = std::getenv("RAWRXD_SNEAKER_SECRET");
    if (!secret || !secret[0])
    {
        appendToOutput("[Enterprise][Sneaker] Set environment variable RAWRXD_SNEAKER_SECRET (UTF-8 passphrase) "
                       "before packing.\n"
                       "  Optional: RAWRXD_SNEAKER_SRC = source folder (default: current directory)\n"
                       "            RAWRXD_SNEAKER_OUT = output bundle path (default: .\\sovereign_sneaker.rxdsk)\n");
        return;
    }
    const char* srcEnv = std::getenv("RAWRXD_SNEAKER_SRC");
    char cwdBuf[MAX_PATH]{};
    std::string src = (srcEnv && srcEnv[0]) ? std::string(srcEnv) : std::string();
    if (src.empty())
    {
        ...
        {
            appendToOutput("[Enterprise][Sneaker] Could not resolve current directory.\n");
            return;
        }
    }
    std::string outPath;
    const char* outEnv = std::getenv("RAWRXD_SNEAKER_OUT");
    if (outEnv && outEnv[0])
        outPath = outEnv;
    else
        outPath = src + "\\sovereign_sneaker.rxdsk";

    auto packed = rawrxd::data_diode::packSneakernetBundle(src, outPath, secret);
    if (!packed)
    {
        appendToOutput(std::string("[Enterprise][Sneaker] Pack failed: ") +
                       rawrxd::data_diode::diodeErrorMessage(packed.error()) + "\n");
        appendAuditLog("user", "sneaker_pack", outPath, "failed");
        return;
    }
    appendToOutput("[Enterprise][Sneaker] Bundle written: " + outPath + "\n");
    appendAuditLog("user", "sneaker_pack", outPath, "success");
}

void Win32IDE::cmdAirgapSneakerUnpack()
{
    const char* secret = std::getenv("RAWRXD_SNEAKER_SECRET");
    if (!secret || !secret[0])
    {
        appendToOutput("[Enterprise][Sneaker] Set RAWRXD_SNEAKER_SECRET to the same passphrase used when packing.\n"
                       "  Required: RAWRXD_SNEAKER_BUNDLE = path to .rxdsk file\n"
                       "  Optional: RAWRXD_SNEAKER_DEST = extract directory (default: .\\sneaker_unpack)\n");
        return;
    }
    const char* bundleEnv = std::getenv("RAWRXD_SNEAKER_BUNDLE");
    if (!bundleEnv || !bundleEnv[0])
    {
        appendToOutput("[Enterprise][Sneaker] Set RAWRXD_SNEAKER_BUNDLE to the bundle path.\n");
        return;
    }
    char cwdBuf[MAX_PATH]{};
    std::string dest;
    const char* destEnv = std::getenv("RAWRXD_SNEAKER_DEST");
    if (destEnv && destEnv[0])
        dest = destEnv;
    else if (GetCurrentDirectoryA(MAX_PATH, cwdBuf))
        dest = std::string(cwdBuf) + "\\sneaker_unpack";
    else
    {
        appendToOutput("[Enterprise][Sneaker] Could not resolve destination directory.\n");
        return;
    }

    auto unpacked = rawrxd::data_diode::unpackSneakernetBundle(std::string(bundleEnv), dest, secret);
    if (!unpacked)
    {
        appendToOutput(std::string("[Enterprise][Sneaker] Unpack failed: ") +
                       rawrxd::data_diode::diodeErrorMessage(unpacked.error()) + "\n");
        appendAuditLog("user", "sneaker_unpack", dest, "failed");
        return;
    }
    appendToOutput("[Enterprise][Sneaker] Extracted to: " + dest + "\n");
    appendAuditLog("user", "sneaker_unpack", dest, "success");
}
