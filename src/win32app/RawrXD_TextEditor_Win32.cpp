 ...
 * Pure Win32 replacement for Qt QPlainTextEdit/QTextEdit (integrated into Win32 IDE)
...
public:
    ...
    void GetText(char* buffer, size_t bufSize) const {
        ...
        size_t copyLen = (bufSize - 1) < result.size() ? (bufSize - 1) : result.size();
        if (copyLen > 0) {
            memcpy(buffer, result.c_str(), copyLen);
        }
        buffer[copyLen] = '\0';
    ...
    bool GetLine(size_t lineNum, char* buffer, size_t bufSize) const {
        ...
        if (lineNum >= m_lines.size() || !buffer || bufSize == 0) {
            LeaveCriticalSection(&m_criticalSection);
            return false;
        }

        const LineInfo& line = m_lines[lineNum];
        size_t copyLen = (bufSize - 1) < line.text.size() ? (bufSize - 1) : line.text.size();
        if (copyLen > 0) {
            memcpy(buffer, line.text.c_str(), copyLen);
        }
        buffer[copyLen] = '\0';
