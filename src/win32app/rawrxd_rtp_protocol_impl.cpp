...
// Static descriptor table and protocol state
...
static const char g_rtp_desc_dispatch[] = "RTP dispatcher";

// Initialise the descriptor table with the built-in dispatch descriptor.
...
// Return a pointer to the descriptor table.
...
// Validate a raw packet buffer. Returns 0 on success, -1 on failure.
...
// Dispatch a packet. Writes a result string and returns 0 on success, -1 on error.
...
// Build a JSON context blob from current protocol state.
...
// Return a pointer to the most recently built context blob.
const void* RTP_GetContextBlobPtr(void) {
    return g_rtp_context_blob;
}

// Return the byte size of the most recently built context blob.
uint32_t RTP_GetContextBlobSize(void) {
    return g_rtp_context_blob_size;
}

// Return a pointer to the internal telemetry counter array.
const void* RTP_GetTelemetrySnapshot(void) {
    return g_rtp_telemetry;
}

// Run the agent loop for up to max_iters iterations.
int32_t RTP_AgentLoop_Run(const char* user_prompt_utf8, char* out_buf, uint32_t out_cap, uint32_t max_iters) {
    if (user_prompt_utf8 == nullptr || max_iters == 0) {
        ...
        g_rtp_telemetry[2] += 1; // invalid input
        return -1;
    }
    if (out_buf != nullptr && out_cap > 0) {
...
// Stream parser: initialise state.
void RTP_StreamParser_Init(void) {
    g_rtp_stream_state = 0;
    g_rtp_stream_len = 0;
}

// Stream parser: reset to idle state.
void RTP_StreamParser_Reset(void) {
    g_rtp_stream_state = 0;
    g_rtp_stream_len = 0;
}

// Stream parser: push one byte into the receive buffer.
...
// Stream parser: copy the pending packet out if available.
...
// Return the current stream parser state.
uint32_t RTP_StreamParser_GetState(void) {
    return g_rtp_stream_state;
}

// Encode a tool-result frame into out_buf.
