...
// ---------------------------------------------------------------------------
static constexpr int IDC_RECOVERY_SCAN       = 10301;
static constexpr int IDC_RECOVERY_PROBE      = 10302;
static constexpr int IDC_RECOVERY_START      = 10303;
static constexpr int IDC_RECOVERY_PAUSE      = 10304;
static constexpr int IDC_RECOVERY_ABORT      = 10305;
static constexpr int IDC_RECOVERY_KEY_EXTRACT= 10306;
static constexpr int IDC_RECOVERY_BADMAP     = 10307;
static constexpr int IDC_RECOVERY_DRIVELIST  = 10308;
static constexpr int IDC_RECOVERY_LOG        = 10309;
static constexpr int IDC_RECOVERY_OUTPATH    = 10310;
static constexpr int IDC_RECOVERY_BROWSE     = 10311;
static constexpr int IDC_RECOVERY_PROGRESS   = 10312;

// ---------------------------------------------------------------------------
// Singleton recovery agent for IDE + cached drive list
// ---------------------------------------------------------------------------
...
static std::vector<DriveInfo> s_cachedDrives;
...
void Win32IDE::onRecoveryScan()
{
    ...
    s_cachedDrives.clear();
    auto result = agent.ScanForDyingDrives(s_cachedDrives);
    if (!result.success) {
        char buf[256];
    ...
    for (size_t i = 0; i < s_cachedDrives.size(); i++) {
        char entry[256];
        snprintf(entry, sizeof(entry), "Drive %d: %s (%s, %.1f GB)",
                 s_cachedDrives[i].driveNumber,
                 s_cachedDrives[i].productId,
                 s_cachedDrives[i].serialNumber,
                 s_cachedDrives[i].totalBytes / (1024.0 * 1024.0 * 1024.0));
...
void Win32IDE::onRecoveryProbe()
{
    ...
    if (sel >= (int)s_cachedDrives.size()) return;

    auto& agent = ideRecoveryAgent();
    int driveNum = s_cachedDrives[sel].driveNumber;

    recoveryAppendLog("[PROBE] Probing USB bridge...\r\n");
    DriveInfo probeInfo{};
    auto result = agent.ProbeDrive(driveNum, probeInfo);
    if (!result.success) {
        char buf[256];
    ...
    // Update cached entry with probe results
    s_cachedDrives[sel] = probeInfo;

    const char* bridgeName = "Unknown";
    switch (probeInfo.bridgeType) {
...
void Win32IDE::onRecoveryStart()
{
    ...
    // Configure agent before starting
    RecoveryConfig config;
    config.targetDriveNumber = driveNum;
    config.outputDir         = outPath;
    config.maxRetries        = 3;
    config.sparseImage       = true;
    agent.SetConfig(config);

    recoveryAppendLog("[START] Beginning recovery imaging...\r\n");
    auto result = agent.StartRecovery(driveNum);
    if (!result.success) {
        char buf[256];
...
{
    int sel = (int)SendMessageA(m_hwndRecoveryDriveList, LB_GETCURSEL, 0, 0);
    if (sel == LB_ERR || sel >= (int)s_cachedDrives.size()) {
        recoveryAppendLog("[KEY] No drive selected. Click Scan first.\r\n");
        return;
    }
    ...
    auto& agent = ideRecoveryAgent();
    uint8_t keyOut[32] = {};
    auto result = agent.ExtractEncryptionKey(driveNum, keyOut);
    if (!result.success) {
        char buf[256];
    ...
    char hexBuf[128];
    for (int i = 0; i < 32; i++) {
        snprintf(hexBuf + i * 2, 3, "%02X", keyOut[i]);
    }
    char logBuf[256];
    snprintf(logBuf, sizeof(logBuf), "[KEY] AES-256 key: %s\r\n", hexBuf);
    recoveryAppendLog(logBuf);
...
void Win32IDE::onRecoveryBadMapExport()
{
    ...
    auto& agent = ideRecoveryAgent();
    auto result = agent.ExportBadSectorMap(mapFile);
    if (!result.success) {
        char buf[256];
...
void Win32IDE::onRecoveryTimer()
{
    auto& agent = ideRecoveryAgent();
    auto state  = agent.GetState();
    const auto& stats = agent.GetStats();

    uint64_t good  = stats.goodSectors.load();
    uint64_t total = stats.totalSectors.load();
    uint64_t bad   = stats.badSectors.load();

    // Update progress bar (0..1000 range)
    int progress = 0;
    if (total > 0) {
        progress = (int)((good * 1000ULL) / total);
    }
