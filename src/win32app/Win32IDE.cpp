...
// ---------------------------------------------------------------------------
// Auto-configure RAM preflight reserve fraction based on model file size.
// Called after model selection but before any loader/streaming path runs.
// Sets the RAWRXD_RAM_PREFLIGHT_RESERVE_FRAC env var so that all three
// preflight implementations (RawrXDModelLoader, ai_implementation,
// ai_assistant_engine) see a consistent, model-aware fraction.
// ---------------------------------------------------------------------------
static void autoConfigureRamPreflight(const std::string& modelPath)
{
    std::error_code ec;
    const auto modelBytes = std::filesystem::file_size(modelPath, ec);
    if (ec || modelBytes == 0)
        return;  // can't stat — let the loader's own error handling deal with it
    ...
    const double ratio = static_cast<double>(modelBytes) / static_cast<double>(mem.ullTotalPhys);

    // Scale reserve fraction so the model always fits within (1 - frac) of total RAM.
    //   ratio < 0.70  → default 0.20 (plenty of headroom)
    //   ratio 0.70-0.80 → 0.15
    //   ratio 0.80-0.90 → 0.08
    //   ratio > 0.90  → 0.05 (mmap streaming; minimal reserve)
    double frac;
    if (ratio < 0.70)
        frac = 0.20;
    else if (ratio < 0.80)
    ...
    _putenv_s("RAWRXD_RAM_PREFLIGHT_RESERVE_FRAC", buf);

    // Automatically bypass software RAM gates for large models before loading/streaming
    // This accommodates the streaming loader handling massive (e.g. 800B) models
    if (ratio > 0.70)
    {
        _putenv_s("RAWRXD_BYPASS_RAM_GATES", "1");
        printf("[RawrXD][PREFLIGHT-AUTO] RAWRXD_BYPASS_RAM_GATES automatically set to 1 for large model.\n");
    }

    printf("[RawrXD][PREFLIGHT-AUTO] model=%.2f GB  totalRAM=%.2f GB  ratio=%.2f  reserveFrac=%s\n",
           modelBytes / (1024.0 * 1024.0 * 1024.0), mem.ullTotalPhys / (1024.0 * 1024.0 * 1024.0), ratio, buf);
}
...
#define IDM_EDIT_PREFERENCES 2016
#define IDM_EDIT_FIND 2017
#define IDM_EDIT_REPLACE 2018
#define IDM_EDIT_FIND_NEXT 2019
#define IDM_EDIT_FIND_PREV 2020

#define IDM_VIEW_MINIMAP 2021
#define IDM_VIEW_OUTPUT_TABS 2022
#define IDM_VIEW_MODULE_BROWSER 2023
#define IDM_VIEW_THEME_EDITOR 2024
#define IDM_VIEW_FLOATING_PANEL 2025
#define IDM_VIEW_OUTPUT_PANEL 2026
#define IDM_VIEW_USE_STREAMING_LOADER 2027
#define IDM_VIEW_USE_VULKAN_RENDERER 2028
#define IDM_VIEW_SIDEBAR 2029
#define IDM_VIEW_TERMINAL 2030
...
Win32IDE::Win32IDE(HINSTANCE hInstance)
    : m_hInstance(hInstance), m_hwndMain(nullptr), m_hwndEditor(nullptr), m_hwndLineNumbers(nullptr),
      ...
      m_hwndAnnotationOverlay(nullptr), m_nativePipelineReady(false)
{
    // ============================================================
    ...
    // Token used by async callbacks to detect shutdown/destruction.
    m_callbackLifetimeToken = std::make_shared<int>(0);
    ...
    m_userModelDirectories.push_back("F:\\OllamaModels");
...
void Win32IDE::createMenuBar(HWND hwnd)
{
    ...
    AppendMenuW(hEditMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenuW(hEditMenu, MF_STRING, IDM_EDIT_PREFERENCES, L"&Preferences...");
    ...
    AppendMenuW(hTerminalMenu, MF_STRING, IDM_VIEW_TERMINAL, L"Show &Terminal\tCtrl+`");
    AppendMenuW(hTerminalMenu, MF_STRING, IDM_VIEW_OLLAMA_SERVICE, L"&Ollama Service...");
    AppendMenuW(hTerminalMenu, MF_SEPARATOR, 0, nullptr);
    ...
    AppendMenuW(hAIOptionsMenu, MF_STRING, IDM_AI_MODE_WORKFLOW_EXECUTOR, L"&Workflow Executor");
    ...
    // Agent operations exposed outside Hotpatch menu
    {
        HMENU hAgentOpsMenu = CreatePopupMenu();
        AppendMenuW(hAgentOpsMenu, MF_STRING, IDM_AGENT_COMPACT_CONVERSATION, L"&Compact Conversation");
        AppendMenuW(hAgentOpsMenu, MF_STRING, IDM_AGENT_OPTIMIZE_TOOL_SELECTION, L"Optimize &Tool Selection");
        AppendMenuW(hAgentOpsMenu, MF_STRING, IDM_AGENT_RESOLVING_STATUS, L"&Resolving Status");
        AppendMenuW(hAgentOpsMenu, MF_STRING, IDM_AGENT_READ_LINES, L"&Read Lines...");
        AppendMenuW(hAgentOpsMenu, MF_STRING, IDM_AGENT_PLANNING_EXPLORATION, L"&Planning Exploration");
        AppendMenuW(hAgentOpsMenu, MF_STRING, IDM_AGENT_SEARCH_FILES, L"&Search Files...");
        AppendMenuW(hAgentOpsMenu, MF_STRING, IDM_AGENT_EVALUATE_INTEGRATION, L"&Evaluate Integration");
        AppendMenuW(hAgentMenu, MF_POPUP, (UINT_PTR)hAgentOpsMenu, L"Agent &Operations");
        AppendMenuW(hAgentMenu, MF_SEPARATOR, 0, nullptr);
    }
                ...
                if (isModelExt(path))
                {
                    ...
                    std::thread(
                        [this, path]()
                        {
                            ...
                            bool ggufLoaded = loadGGUFModel(path);
                            bool inferenceLoaded = false;
                            bool nativeLoaded = false;

                            if (ggufLoaded)
                            {
                                inferenceLoaded = loadModelForInference(path);
                                if (!inferenceLoaded && m_nativeEngine)
                                {
                                    nativeLoaded = m_nativeEngine->LoadModel(path);
                                }
                            }
                            ...
                            {
                                std::string message;
                                if (ggufLoaded && (inferenceLoaded || nativeLoaded))
                                {
                                    message = "Model loaded successfully; chat and agentic use this model.\n";
                                    appendToOutput(message, "Agent", OutputSeverity::Info);
                                }
                                else
                                {
                                    message = "Model loading failed.\n";
                                    appendToOutput(message, "Agent", OutputSeverity::Error);
                                }
                            }
                        })
                        .detach();
    ...
    if (m_chatMode && m_agenticBridge && m_agenticBridge->IsInitialized())
    {
        ...
        std::thread(
            [this, command]()
            {
                ...
                // Try agentic bridge first to get executor metadata
                AgentResponse r = m_agenticBridge->ExecuteAgentCommand(command);
                std::string response = r.content;
                if (response.empty() || r.type == AgentResponseType::AGENT_ERROR)
                {
                    // Fallback to full routing path (preserves executor metadata)
                    r = sendMessageToModel(command);
                    response = r.content;
                }

                if (!response.empty() && !isShuttingDown())
                {
                    // Show executor status line
                    if (!r.executorLabel.empty())
                    {
                        appendToOutput("[Executor: " + r.executorLabel + "]\n", "Output", OutputSeverity::Info);
                    }

                    appendChatMessage("Model", response);
                    appendToOutput("[Chat] " + response + "\n", "Output", OutputSeverity::Info);

                    // Per-agent telemetry: show each child agent result
                    if (!r.agentResults.empty())
                    {
                        appendToOutput("--- Per-Agent Telemetry ---\n", "Output", OutputSeverity::Info);
                        for (size_t i = 0; i < r.agentResults.size(); ++i)
                        {
                            const auto& [role, result] = r.agentResults[i];
                            std::string snippet = result.substr(0, 200);
                            if (result.size() > 200)
                                snippet += "...";
                            appendToOutput("[Agent " + std::to_string(i + 1) + " / " + role + "] " + snippet + "\n",
                                           "Output", OutputSeverity::Info);
                        }
                        appendToOutput("--- End Telemetry ---\n", "Output", OutputSeverity::Info);
                    }
                }
            })
            .detach();
...
void Win32IDE::copyWithFormatting()
{
    ...
    if (OpenClipboard(m_hwndMain))
    {
        ...
        if (hMem)
        {
            ...
            if (dest)
            {
                memcpy_s(dest, text.size() + 1, text.c_str(), text.size() + 1);
                GlobalUnlock(hMem);
...
void Win32IDE::copyLineNumbers()
{
    ...
    if (OpenClipboard(m_hwndMain))
    {
        ...
        if (hMem)
        {
            ...
            if (dest)
            {
                memcpy_s(dest, lineNumbers.size() + 1, lineNumbers.c_str(), lineNumbers.size() + 1);
                GlobalUnlock(hMem);
                SetClipboardData(CF_TEXT, hMem);
            }
            else
...
void Win32IDE::showPreferencesDialog()
{
    if (m_hwndPreferencesDialog && IsWindow(m_hwndPreferencesDialog))
    {
        SetForegroundWindow(m_hwndPreferencesDialog);
        return;
    }

    m_hwndPreferencesDialog = CreateWindowExW(
        WS_EX_DLGMODALFRAME, L"STATIC", L"Preferences", 
        WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_VISIBLE | WS_CLIPCHILDREN,
        150, 150, 500, 400, m_hwndMain, nullptr, m_hInstance, nullptr);

    if (!m_hwndPreferencesDialog) return;

    // Load current preferences
    loadPreferences();

    // Create tab control
    m_hwndPrefsTab = CreateWindowExW(0, WC_TABCONTROLW, L"", 
        WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS, 
        10, 10, 470, 320, m_hwndPreferencesDialog, nullptr, m_hInstance, nullptr);

    // Add tabs
    TCITEMW tie;
    tie.mask = TCIF_TEXT;
    tie.pszText = const_cast<LPWSTR>(L"LSP");
    TabCtrl_InsertItem(m_hwndPrefsTab, 0, &tie);
    tie.pszText = const_cast<LPWSTR>(L"AI Model");
    TabCtrl_InsertItem(m_hwndPrefsTab, 1, &tie);
    tie.pszText = const_cast<LPWSTR>(L"Terminal");
    TabCtrl_InsertItem(m_hwndPrefsTab, 2, &tie);
    tie.pszText = const_cast<LPWSTR>(L"Editor");
    TabCtrl_InsertItem(m_hwndPrefsTab, 3, &tie);

    // Create tab content panels
    createPreferencesTabContent();

    // OK/Cancel buttons
    CreateWindowExW(0, L"BUTTON", L"OK", 
        WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 
        300, 340, 80, 25, m_hwndPreferencesDialog, (HMENU)IDOK, m_hInstance, nullptr);
    CreateWindowExW(0, L"BUTTON", L"Cancel", 
        WS_CHILD | WS_VISIBLE, 
        390, 340, 80, 25, m_hwndPreferencesDialog, (HMENU)IDCANCEL, m_hInstance, nullptr);

    ShowWindow(m_hwndPreferencesDialog, SW_SHOW);
}

void Win32IDE::createPreferencesTabContent()
{
    // LSP Tab
    m_hwndLspTab = CreateWindowExW(0, L"STATIC", L"", 
        WS_CHILD | WS_VISIBLE, 20, 40, 450, 280, m_hwndPreferencesDialog, nullptr, m_hInstance, nullptr);
    
    CreateWindowExW(0, L"STATIC", L"LSP Command:", 
        WS_CHILD | WS_VISIBLE, 10, 10, 100, 20, m_hwndLspTab, nullptr, m_hInstance, nullptr);
    m_hwndLspCommand = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"clangd", 
        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 
        120, 8, 200, 22, m_hwndLspTab, nullptr, m_hInstance, nullptr);
    
    m_hwndLspAutoStart = CreateWindowExW(0, L"BUTTON", L"Auto-start LSP server", 
        WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 
        10, 40, 200, 20, m_hwndLspTab, nullptr, m_hInstance, nullptr);

    // AI Model Tab
    m_hwndAiTab = CreateWindowExW(0, L"STATIC", L"", 
        WS_CHILD, 20, 40, 450, 280, m_hwndPreferencesDialog, nullptr, m_hInstance, nullptr);
    
    CreateWindowExW(0, L"STATIC", L"Default Model:", 
        WS_CHILD | WS_VISIBLE, 10, 10, 100, 20, m_hwndAiTab, nullptr, m_hInstance, nullptr);
    m_hwndDefaultModel = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", 
        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 
        120, 8, 250, 22, m_hwndAiTab, nullptr, m_hInstance, nullptr);
    CreateWindowExW(0, L"BUTTON", L"Browse...", 
        WS_CHILD | WS_VISIBLE, 380, 6, 60, 25, m_hwndAiTab, (HMENU)IDC_BROWSE_MODEL, m_hInstance, nullptr);

    // Terminal Tab
    m_hwndTerminalTab = CreateWindowExW(0, L"STATIC", L"", 
        WS_CHILD, 20, 40, 450, 280, m_hwndPreferencesDialog, nullptr, m_hInstance, nullptr);
    
    CreateWindowExW(0, L"STATIC", L"Shell:", 
        WS_CHILD | WS_VISIBLE, 10, 10, 50, 20, m_hwndTerminalTab, nullptr, m_hInstance, nullptr);
    m_hwndShellType = CreateWindowExW(0, L"COMBOBOX", L"", 
        WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST, 
        70, 8, 150, 200, m_hwndTerminalTab, nullptr, m_hInstance, nullptr);
    SendMessageW(m_hwndShellType, CB_ADDSTRING, 0, (LPARAM)L"PowerShell");
    SendMessageW(m_hwndShellType, CB_ADDSTRING, 0, (LPARAM)L"Cmd");
    SendMessageW(m_hwndShellType, CB_ADDSTRING, 0, (LPARAM)L"Bash");
    SendMessageW(m_hwndShellType, CB_ADDSTRING, 0, (LPARAM)L"Custom");

    // Editor Tab
    m_hwndEditorTab = CreateWindowExW(0, L"STATIC", L"", 
        WS_CHILD, 20, 40, 450, 280, m_hwndPreferencesDialog, nullptr, m_hInstance, nullptr);
    
    CreateWindowExW(0, L"STATIC", L"Font Size:", 
        WS_CHILD | WS_VISIBLE, 10, 10, 70, 20, m_hwndEditorTab, nullptr, m_hInstance, nullptr);
    m_hwndFontSize = CreateWindowExW(0, UPDOWN_CLASSW, L"", 
        WS_CHILD | WS_VISIBLE | UDS_SETBUDDYINT | UDS_ALIGNRIGHT, 
        140, 8, 50, 22, m_hwndEditorTab, nullptr, m_hInstance, nullptr);
    m_hwndFontSizeEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"12", 
        WS_CHILD | WS_VISIBLE | ES_NUMBER, 
        90, 8, 50, 22, m_hwndEditorTab, nullptr, m_hInstance, nullptr);
    SendMessageW(m_hwndFontSize, UDM_SETBUDDY, (WPARAM)m_hwndFontSizeEdit, 0);
    SendMessageW(m_hwndFontSize, UDM_SETRANGE, 0, MAKELPARAM(24, 8));
    SendMessageW(m_hwndFontSize, UDM_SETPOS, 0, 12);

    m_hwndShowLineNumbers = CreateWindowExW(0, L"BUTTON", L"Show line numbers", 
        WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 
        10, 40, 150, 20, m_hwndEditorTab, nullptr, m_hInstance, nullptr);
    CheckDlgButton(m_hwndEditorTab, (int)m_hwndShowLineNumbers, BST_CHECKED);

    m_hwndWordWrap = CreateWindowExW(0, L"BUTTON", L"Word wrap", 
        WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 
        10, 65, 100, 20, m_hwndEditorTab, nullptr, m_hInstance, nullptr);

    // Set initial values from loaded preferences
    SetWindowTextW(m_hwndLspCommand, utf8ToWide(m_prefs.lspCommand).c_str());
    CheckDlgButton(m_hwndLspTab, (int)m_hwndLspAutoStart, m_prefs.lspAutoStart ? BST_CHECKED : BST_UNCHECKED);
    SetWindowTextW(m_hwndDefaultModel, utf8ToWide(m_prefs.defaultModel).c_str());
    SendMessageW(m_hwndShellType, CB_SETCURSEL, (WPARAM)m_prefs.shellType, 0);
    SendMessageW(m_hwndFontSize, UDM_SETPOS, 0, m_prefs.fontSize);
    CheckDlgButton(m_hwndEditorTab, (int)m_hwndShowLineNumbers, m_prefs.showLineNumbers ? BST_CHECKED : BST_UNCHECKED);
    CheckDlgButton(m_hwndEditorTab, (int)m_hwndWordWrap, m_prefs.wordWrap ? BST_CHECKED : BST_UNCHECKED);

    // Show LSP tab initially
    ShowWindow(m_hwndLspTab, SW_SHOW);
}

INT_PTR CALLBACK Win32IDE::PreferencesDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    Win32IDE* pThis = reinterpret_cast<Win32IDE*>(GetWindowLongPtrW(hwndDlg, GWLP_USERDATA));
    
    switch (uMsg)
    {
        case WM_INITDIALOG:
            pThis = reinterpret_cast<Win32IDE*>(lParam);
            SetWindowLongPtrW(hwndDlg, GWLP_USERDATA, (LONG_PTR)pThis);
            return TRUE;
        ...
        case WM_COMMAND:
            ...
            {
                // Save preferences
                pThis->savePreferencesFromDialog();
                pThis->savePreferences();
                DestroyWindow(hwndDlg);
                pThis->m_hwndPreferencesDialog = nullptr;
                return TRUE;
            }
            ...
            else if (LOWORD(wParam) == IDC_BROWSE_MODEL)
            {
                pThis->browseForModel();
                return TRUE;
            }
            break;
...
void Win32IDE::switchPreferencesTab()
{
    int tab = TabCtrl_GetCurSel(m_hwndPrefsTab);
    
    ShowWindow(m_hwndLspTab, tab == 0 ? SW_SHOW : SW_HIDE);
    ShowWindow(m_hwndAiTab, tab == 1 ? SW_SHOW : SW_HIDE);
    ShowWindow(m_hwndTerminalTab, tab == 2 ? SW_SHOW : SW_HIDE);
    ShowWindow(m_hwndEditorTab, tab == 3 ? SW_SHOW : SW_HIDE);
}

void Win32IDE::savePreferencesFromDialog()
{
    // LSP settings
    wchar_t buf[256];
    GetWindowTextW(m_hwndLspCommand, buf, 256);
    m_prefs.lspCommand = wideToUtf8(buf);
    m_prefs.lspAutoStart = IsDlgButtonChecked(m_hwndLspTab, (int)m_hwndLspAutoStart) == BST_CHECKED;
    
    // AI settings
    GetWindowTextW(m_hwndDefaultModel, buf, 256);
    m_prefs.defaultModel = wideToUtf8(buf);
    
    // Terminal settings
    m_prefs.shellType = (ShellType)SendMessageW(m_hwndShellType, CB_GETCURSEL, 0, 0);
    
    // Editor settings
    m_prefs.fontSize = SendMessageW(m_hwndFontSize, UDM_GETPOS, 0, 0);
    m_prefs.showLineNumbers = IsDlgButtonChecked(m_hwndEditorTab, (int)m_hwndShowLineNumbers) == BST_CHECKED;
    m_prefs.wordWrap = IsDlgButtonChecked(m_hwndEditorTab, (int)m_hwndWordWrap) == BST_CHECKED;
}

void Win32IDE::browseForModel()
{
    OPENFILENAMEW ofn = {0};
    wchar_t szFile[260] = {0};
    
    ofn.lStructSize = sizeof(ofn);
    ofn.hwndOwner = m_hwndPreferencesDialog;
    ofn.lpstrFile = szFile;
    ...
    if (GetOpenFileNameW(&ofn))
    {
        SetWindowTextW(m_hwndDefaultModel, szFile);
    }
}

void Win32IDE::loadPreferences()
{
    ...
    auto parseString = [&](const char* key, std::string& out) -> bool {
        ...
        if (!end) return false;
        out.assign(pos, end - pos);
        return true;
    };

    auto parseBool = [&](const char* key, bool& out) -> bool {
        const char* pos = strstr(buf, key);
        if (!pos) return false;
        pos += strlen(key);
        pos = strchr(pos, ':');
    ...
    parseString("\"lspCommand\"", m_prefs.lspCommand);
    parseBool("\"lspAutoStart\"", m_prefs.lspAutoStart);
    parseString("\"defaultModel\"", m_prefs.defaultModel);
    
    int shellTypeInt;
    if (parseInt("\"shellType\"", shellTypeInt)) {
        m_prefs.shellType = static_cast<ShellType>(shellTypeInt);
    }
    
    parseInt("\"fontSize\"", m_prefs.fontSize);
    parseBool("\"showLineNumbers\"", m_prefs.showLineNumbers);
    parseBool("\"wordWrap\"", m_prefs.wordWrap);

    OutputDebugStringA("[Preferences] Loaded from file\n");
}

void Win32IDE::savePreferences()
{
    std::string path = getPreferencesPath();
    if (path.empty()) return;
    ...
    // Write JSON manually
    fprintf(f, "{\n");
    fprintf(f, "  \"lspCommand\": \"%s\",\n", m_prefs.lspCommand.c_str());
    fprintf(f, "  \"lspAutoStart\": %s,\n", m_prefs.lspAutoStart ? "true" : "false");
    fprintf(f, "  \"defaultModel\": \"%s\",\n", m_prefs.defaultModel.c_str());
    fprintf(f, "  \"shellType\": %d,\n", static_cast<int>(m_prefs.shellType));
    fprintf(f, "  \"fontSize\": %d,\n", m_prefs.fontSize);
    fprintf(f, "  \"showLineNumbers\": %s,\n", m_prefs.showLineNumbers ? "true" : "false");
    fprintf(f, "  \"wordWrap\": %s\n", m_prefs.wordWrap ? "true" : "false");
    fprintf(f, "}\n");

    fclose(f);
    OutputDebugStringA("[Preferences] Saved to file\n");
}

std::string Win32IDE::getPreferencesPath()
{
...
void Win32IDE::loadModelFromPath(const std::string& filepath)
{
    ...
    // Auto-tune RAM preflight reserve fraction for this model before any loader runs.
    autoConfigureRamPreflight(filepath);
    ...
    std::string loadPathUsed = "none";
    if (!ggufOk)
    {
        appendToOutput("GGUF streaming load failed; attempting bridge/native fallback path.\n", "Output",
                       OutputSeverity::Warning);
    }
    else
    {
        loadPathUsed = "streaming-gguf";
    }
    if (ggufOk)
    {
        initializeInference();
    ...
    if (bridgeOk && !ggufOk)
    {
        loadPathUsed = "agentic-bridge";
    }
    bool nativeFallbackOk = false;
    if (!ggufOk && !bridgeOk)
    {
        try
        {
            ...
            auto* engine = static_cast<RawrXD::CPUInferenceEngine*>(m_nativeEngine.get());
            if (engine && engine->LoadModel(filepath))
            {
                m_loadedModelPath = filepath;
                m_nativeEngineLoaded = true;
                nativeFallbackOk = true;
                loadPathUsed = "native-fallback";
                appendToOutput("Native fallback model load succeeded after GGUF/bridge failure.\n", "Output",
                               OutputSeverity::Warning);
            }
        }
        catch (const std::exception& e)
        {
            appendToOutput(std::string("Native fallback load failed: ") + e.what() + "\n", "Errors",
                           OutputSeverity::Error);
        }
    }
    if (bridgeOk && !ggufOk)
        appendToOutput("Model loaded into Agentic Bridge (streaming GGUF skipped).\n", "Output", OutputSeverity::Info);
    if (ggufOk || bridgeOk || nativeFallbackOk)
    {
        initializeInference();
        SendMessageW(m_hwndStatusBar, SB_SETTEXT, 0,
                     (LPARAM)utf8ToWide("Model ready [" + loadPathUsed + "]: " + filepath).c_str());
...
bool Win32IDE::loadGGUFModel(const std::string& filepath)
{
    ...
    try
    {
        ...
        const bool embeddingZoneLoaded = m_ggufLoader->LoadZone("embedding");
        if (!embeddingZoneLoaded)
        {
    ...
    const bool metadataOnlyState = (m_ggufLoader && m_ggufLoader->IsMetadataOnly());
    const bool metadataOnlyHeuristic = zones.empty() && currentMemory == 0 && !m_modelTensors.empty();
    if (metadataOnlyState || metadataOnlyHeuristic)
    {
        setLocalMetadataOnlyBlocked(
            "The loaded model is currently in metadata-only mode (insufficient memory commit headroom). "
            "Local inference is blocked to prevent repeated MapViewOfFile failures. "
            "Provision more pagefile/RAM or load a smaller quantized model.");
    }
    else
    {
        clearLocalMetadataOnlyBlocked();
...
AgentResponse Win32IDE::sendMessageToModel(const std::string& message)
{
    ...
    if (!canChat)
    {
        return {AgentResponseType::AGENT_ERROR,
                "Error: No model loaded. Load a GGUF (File > Open / Load Model) or set up Ollama/backend in Backend "
                "Switcher.",
                "",
                "",
                "",
    ...
    if (m_backendManagerInitialized)
    {
        ...
        if (resp.find("MapViewOfFile failed") != std::string::npos)
        {
            setLocalMetadataOnlyBlocked(
                "The loaded model is in metadata-only mode due to memory commit limits. "
                "Local inference is blocked until you load a smaller model or increase pagefile/RAM.");
            return {AgentResponseType::AGENT_ERROR,
                    "The loaded model is too large for your system's memory commit limits (MapViewOfFile failed). "
        ...
        if (resp.find("Native inference API unavailable") != std::string::npos)
        {
            // Do not hard-stop here; allow downstream fallbacks (MASM HTTP / native CPU path) to run.
        }
        if (!resp.empty() && resp.find("[Backend Error]") != 0 && resp.find("[ERROR]") != 0)
        {
            return {AgentResponseType::ANSWER, resp, "", "", "", "BackendManager"};
        }
    }
    ...
    {
        return {AgentResponseType::ANSWER, llmResponse, "", "", "", "MASMHttpAPI"};
    }
    ...
    if (m_ggufLoader)
    {
        ...
        if (m_nativeEngine)
        {
            ...
            return {AgentResponseType::ANSWER, response, "", "", "", "NativeCPU"};
        }
    }
    ...
    if (m_agenticBridge && m_agenticBridge->IsInitialized())
    {
        ...
        {
            // Continue to final fallback handling instead of returning a terminal error here.
        }
        return r;
    }

    return {AgentResponseType::AGENT_ERROR,
            "Error: Local model loaded but Native Inference Engine not initialized.\n",
            "",
            "",
            "",
            "None"};
...
bool Win32IDE::loadModelForInference(const std::string& filepath)
{
    ...
    // Auto-tune RAM preflight reserve fraction for this model before the bridge loads it.
    autoConfigureRamPreflight(filepath);
    appendToOutput("Loading model: " + filepath + "\n", "System", OutputSeverity::Info);

    // Keep model selector in sync with directories chosen via File->Load/Open model paths.
    {
        std::error_code parentEc;
        const std::string parentDir = std::filesystem::path(filepath).parent_path().string();
        if (!parentDir.empty() && std::filesystem::exists(parentDir, parentEc) &&
            std::find(m_userModelDirectories.begin(), m_userModelDirectories.end(), parentDir) ==
                m_userModelDirectories.end())
        {
            m_userModelDirectories.push_back(parentDir);
        }
    }
    ...
    if (m_agenticBridge)
    {
        if (m_agenticBridge->LoadModel(filepath))
        {
            m_loadedModelPath = filepath;
            clearLocalMetadataOnlyBlocked();
            resetAutoDowngradeState(filepath);
        ...
        std::string detail = m_agenticBridge->GetLastModelLoadError();
        std::string detailLower = detail;
        std::transform(detailLower.begin(), detailLower.end(), detailLower.begin(), ::tolower);
        if (detailLower.find("memory map") != std::string::npos ||
            detailLower.find("mapping failed") != std::string::npos)
        {
            // Keep local GGUF path usable even if bridge-side memory mapping fails.
            m_loadedModelPath = filepath;
            resetAutoDowngradeState(filepath);
            setLocalMetadataOnlyBlocked(
                "The loaded model exceeds current memory commit limits and was placed in metadata-only mode. "
                "Inference is blocked to avoid repeated MapViewOfFile failures.");
            initializeInference();
            if (m_hwndModelSelector)
            {
                populateModelSelector();
            }
            appendToOutput("Model bridge load hit memory-mapping limits; using local fallback inference path.\n",
                           "Output", OutputSeverity::Warning);
            return true;
        }
    }
...
bool Win32IDE::initializeInference()
{
    ...
    if (m_nativeEngine && !m_loadedModelPath.empty())
    {
        ...
        if (!engine->IsModelLoaded())
        {
            ...
            {
                appendToOutput(std::string("❌ Native Engine Model Load Exception: ") + e.what(), "Errors",
                               OutputSeverity::Error);
            }
            catch (...)
            {
                appendToOutput("❌ Native Engine Model Load Exception: unknown error", "Errors", OutputSeverity::Error);
            }
        }
    }
...
std::string Win32IDE::generateResponse(const std::string& prompt)
{
    ...
    if (m_backendManagerInitialized)
    {
        if (m_ggufLoader && m_ggufLoader->IsMetadataOnly())
        {
            setLocalMetadataOnlyBlocked(
                "The loaded model is in metadata-only mode due to memory commit limits. "
                "Local inference is blocked until you load a smaller model or increase pagefile/RAM.");
            return "The loaded model is currently in metadata-only mode due to memory commit limits. "
                   "Local inference is blocked. Load a smaller quantized model or increase pagefile/RAM.";
        }
        std::string resp = routeWithIntelligence(prompt);
        if (resp.find("MapViewOfFile failed") != std::string::npos)
        {
            setLocalMetadataOnlyBlocked(
                "The loaded model is in metadata-only mode due to memory commit limits. "
        ...
        if (resp.find("Native inference API unavailable") != std::string::npos)
        {
            // Allow downstream fallback routes (Ollama/native path) before surfacing a terminal error.
        }
        return resp;
    }
    ...
    // Fallback: Agentic Bridge / Streaming Loader
    if (m_agenticBridge && m_agenticBridge->IsInitialized())
    {
...
bool Win32IDE::isLocalMetadataOnlyBlocked(std::string* reason) const
{
    if (!m_localMetadataOnlyInferenceBlocked.load(std::memory_order_acquire))
    {
        if (reason)
            reason->clear();
        return false;
    ...
    if (reason)
    {
        std::lock_guard<std::mutex> lock(const_cast<Win32IDE*>(this)->m_inferenceMutex);
        *reason = m_localMetadataOnlyBlockReason;
    }
    return true;
}

void Win32IDE::setLocalMetadataOnlyBlocked(const std::string& reason)
{
    {
        std::lock_guard<std::mutex> lock(m_inferenceMutex);
        m_localMetadataOnlyBlockReason = reason;
    }
    m_localMetadataOnlyInferenceBlocked.store(true, std::memory_order_release);
}

void Win32IDE::clearLocalMetadataOnlyBlocked()
{
    {
        std::lock_guard<std::mutex> lock(m_inferenceMutex);
        m_localMetadataOnlyBlockReason.clear();
    }
    m_localMetadataOnlyInferenceBlocked.store(false, std::memory_order_release);
}

void Win32IDE::resetAutoDowngradeState(const std::string& loadedPath)
{
    std::lock_guard<std::mutex> lock(m_autoDowngradeMutex);
    m_autoDowngradeAttemptedSourcePath.clear();
    m_autoDowngradeLoadedPath = loadedPath;
}
...
void Win32IDE::generateResponseAsync(const std::string& prompt, std::function<void(const std::string&, bool)> callback)
{
    ...
    if (m_inferenceRunning)
    {
        ...
        appendToOutput("⏳ Inference already in progress. Please wait...", "Output", OutputSeverity::Warning);
        if (callback)
        ...
        [this, prompt]()
        {
            ...
            if (m_loadedModelPath.empty() && m_ollamaModelOverride.empty())
            {
                if (m_inferenceCallback)
                    m_inferenceCallback("Error: No model selected or loaded.", true);
                m_inferenceRunning = false;
                return;
            }
            ...
            if (!m_agenticBridge)
            {
                ...
                else if (!m_ollamaModelOverride.empty())
                    initializeAgenticBridge();  // Initialize with default, will use override

                if (!m_agenticBridge)
                {
                    if (m_inferenceCallback)
                        m_inferenceCallback("Error: Agentic Bridge failed to initialize. Check logs.", true);
                    m_inferenceRunning = false;
                    return;
                }
            }

            // Ensure bridge has the correct model loaded
            if (!m_loadedModelPath.empty() && m_agenticBridge->GetCurrentModel() != m_loadedModelPath)
            {
                if (!m_agenticBridge->LoadModel(m_loadedModelPath))
                {
                    if (m_inferenceCallback)
                        m_inferenceCallback("Warning: Could not load model into bridge, attempting inference anyway...",
                                            false);
                }
            }
...
void Win32IDE::createChatPanel()
{
    ...
    // Install SidebarProcImpl — the chat panel's full WM_COMMAND/WM_HSCROLL router.
    // SidebarProc (the left-sidebar proc) has no handler for chat controls and must not
    // be placed here; SidebarProcImpl chains to m_oldSidebarProc at the end.
    ...
    if (m_hwndModelSelector)
    {
        ...
        SendMessage(m_hwndModelSelector, CB_SETMINVISIBLE, 12, 0);
        SendMessage(m_hwndModelSelector, CB_SETDROPPEDWIDTH, 420, 0);
        populateModelSelector();
    }
    ...
    m_hwndMaxTokensSlider =
        CreateWindowExW(0, TRACKBAR_CLASSW, L"", WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_NOTICKS, 5, 80, 290, 25,
                        m_hwndSecondarySidebar, (HMENU)IDC_AI_CONTEXT_LABEL, m_hInstance, nullptr);
    ...
    toggleY += 28;
    m_hwndChkWorkflowExecutor =
        CreateWindowExW(0, L"BUTTON", L"Workflow Exec", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, toggleX, toggleY, 140,
                        20, m_hwndSecondarySidebar, (HMENU)IDC_AI_WORKFLOW_EXECUTOR, m_hInstance, nullptr);
    CreateWindowExW(0, L"STATIC", L"Agents:", WS_CHILD | WS_VISIBLE | SS_LEFT, toggleX + 150, toggleY + 2, 48, 18,
                    m_hwndSecondarySidebar, nullptr, m_hInstance, nullptr);
    m_hwndWorkflowAgentCountCombo = CreateWindowExW(
        0, L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL, toggleX + 205, toggleY - 2, 85, 220,
        m_hwndSecondarySidebar, (HMENU)IDC_AI_WORKFLOW_EXECUTOR_AGENT_COUNT, m_hInstance, nullptr);

    if (m_hwndWorkflowAgentCountCombo)
    {
        SendMessage(m_hwndWorkflowAgentCountCombo, WM_SETFONT, (WPARAM)hFont, TRUE);
        for (int agentCount = 1; agentCount <= 32; ++agentCount)
        {
            std::wstring label = std::to_wstring(agentCount);
            SendMessageW(m_hwndWorkflowAgentCountCombo, CB_ADDSTRING, 0, (LPARAM)label.c_str());
        }
        SendMessage(m_hwndWorkflowAgentCountCombo, CB_SETCURSEL, 3, 0);
    }
    ...
    if (m_hwndChkWorkflowExecutor)
        SendMessage(m_hwndChkWorkflowExecutor, WM_SETFONT, (WPARAM)hFont, TRUE);

    m_hwndCopilotChatOutput =
        CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
                        WS_CHILD | WS_VISIBLE | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL | WS_VSCROLL, 5, 230, 290,
...
void Win32IDE::populateModelSelector()
{
    ...
    {
        // Preserve backing model path (not display label) so restore works with friendly labels.
        previousSelection = m_availableModels[prevIdx];
    }
    ...
    // Drop stale/non-existent directories so we can re-seed from valid locations.
    ...
    // Ensure common local model directories are included when they exist.
    ...
    size_t localModelCount = 0;
    std::string discoveryLog;

    // Debug: Log directories being scanned
    discoveryLog = "Scanning " + std::to_string(m_userModelDirectories.size()) + " model directories:\n";
    LOG_INFO(discoveryLog);
    appendToOutput(discoveryLog, "ModelDiscovery", OutputSeverity::Info);

    for (const auto& dir : m_userModelDirectories)
    {
        std::vector<std::string> modelsFromDir = getModelsFromDirectory(dir);
        std::string dirLog = "  - " + dir + ": found " + std::to_string(modelsFromDir.size()) + " model(s)\n";
        LOG_INFO(dirLog);
        appendToOutput(dirLog, "ModelDiscovery", OutputSeverity::Info);
    ...
    // Keep alias fallback models only when no local files are discovered.
    ...
    // Group non-blob files first, then blobs; sort each group by filename.
    ...
    auto isBlobPath = [&normalizeForSort](const std::string& value)
    {
        const std::string normalized = normalizeForSort(value);
        return normalized.find("\\blobs\\") != std::string::npos;
    };

    std::sort(m_availableModels.begin(), m_availableModels.end(),
              [&normalizeForSort, &isBlobPath](const std::string& lhs, const std::string& rhs)
              {
                  const bool lhsBlob = isBlobPath(lhs);
                  const bool rhsBlob = isBlobPath(rhs);
                  if (lhsBlob != rhsBlob)
                  {
                      return !lhsBlob;
                  }

                  const std::string lhsName = normalizeForSort(std::filesystem::path(lhs).filename().string());
                  const std::string rhsName = normalizeForSort(std::filesystem::path(rhs).filename().string());
                  if (lhsName != rhsName)
                  {
                      return lhsName < rhsName;
                  }

                  return normalizeForSort(lhs) < normalizeForSort(rhs);
              });

    // Remove duplicate absolute paths after ordering.
    ...
    {
        appendToOutput("No local models discovered and no fallback aliases available.\n", "Output",
                       OutputSeverity::Warning);
    }
    else
    {
        appendToOutput("Model selector refreshed: " + std::to_string(localModelCount) + " local files discovered.\n",
                       "Output", OutputSeverity::Info);
    }

    // Populate combobox with friendly labels while preserving index mapping to m_availableModels.
    int droppedWidth = 420;
    for (const auto& model : m_availableModels)
    {
        std::string displayLabel = model;
        std::error_code ec;
        if (std::filesystem::exists(model, ec))
        {
            std::filesystem::path p(model);
            const std::string name = p.filename().string();
            const uintmax_t sizeBytes = std::filesystem::file_size(p, ec);
            if (!ec && sizeBytes > 0)
            {
                const double sizeGB = static_cast<double>(sizeBytes) / (1024.0 * 1024.0 * 1024.0);
                char sizeBuf[64] = {0};
                std::snprintf(sizeBuf, sizeof(sizeBuf), "%.2f GB", sizeGB);
                displayLabel = name + " (" + sizeBuf + ")";
            }
            else
            {
                displayLabel = name;
        ...
        droppedWidth = std::max(droppedWidth, 32 + static_cast<int>(displayLabel.size()) * 7);
        SendMessageW(m_hwndModelSelector, CB_ADDSTRING, 0, (LPARAM)utf8ToWide(displayLabel).c_str());
    }

    if (!m_availableModels.empty())
    {
        SendMessage(m_hwndModelSelector, CB_SETMINVISIBLE,
                    (WPARAM)std::min<size_t>(12, std::max<size_t>(1, m_availableModels.size())), 0);
        SendMessage(m_hwndModelSelector, CB_SETDROPPEDWIDTH, droppedWidth, 0);
    }
...
std::vector<std::string> Win32IDE::getModelsFromDirectory(const std::string& directory)
{
    ...
    try
    {
        ...
        const std::string normalizedDir = normalizePath(directory);
        const std::string ollamaModelsSuffix = "\\ollamamodels";
        const bool includeAllRegularFiles =
            (normalizedDir == "d:\\ollamamodels" ||
             (normalizedDir.size() > ollamaModelsSuffix.size() &&
              normalizedDir.rfind(ollamaModelsSuffix) == normalizedDir.size() - ollamaModelsSuffix.size()));
        ...
        while (it != end)
        {
            ...
            try
            {
                ...
                if (entry.is_regular_file(ec))
                {
                    ...
                    if (includeAllRegularFiles || ext == ".gguf" || ext == ".bin" || ext == ".ggml")
                    {
                        models.push_back(fullPath);
                        LOG_DEBUG("Found model: " + fullPath);
                    }
                }
                else if (ec)
                {
                    // Clear error code to continue iteration
                    ec.clear();
                }
...
// ========================================================================
// Missing Chat Display Methods (Critical Gap Implementations)
// ========================================================================

void Win32IDE::displayResponse(const std::string& response)
{
    if (!m_hwndCopilotChatOutput)
        return;

    // Append response to chat display
    std::wstring wideResponse = std::wstring(response.begin(), response.end());
    static std::wstring chatHistory;
    chatHistory += L"Agent: " + wideResponse + L"\r\n\r\n";

    // Update window text with chat history
    SetWindowTextW(m_hwndCopilotChatOutput, chatHistory.c_str());

    // Scroll to end
    SendMessage(m_hwndCopilotChatOutput, EM_SETSEL, -1, -1);
    SendMessage(m_hwndCopilotChatOutput, EM_SCROLLCARET, 0, 0);

    METRICS.increment("chat.responses_displayed");
}

void Win32IDE::addMessage(const std::string& sender, const std::string& message)
{
    if (!m_hwndCopilotChatOutput)
        return;

    // Append message to chat display with sender prefix
    static std::wstring chatHistory;
    std::wstring wideSender(sender.begin(), sender.end());
    std::wstring wideMessage(message.begin(), message.end());
    
    chatHistory += wideSender + L": " + wideMessage + L"\r\n";
    ...
    METRICS.increment("chat.messages_displayed");
}

void Win32IDE::focusInput()
{
    if (!m_hwndCopilotChatInput)
        return;
    ...
    SendMessage(m_hwndCopilotChatInput, EM_SETSEL, -1, -1);

    METRICS.increment("chat.input_focused");
}

void Win32IDE::setVis(bool visible)
{
    if (m_hwndSecondarySidebar)
    {
        ShowWindow(m_hwndSecondarySidebar, visible ? SW_SHOW : SW_HIDE);
    }
    
    if (m_hwndCopilotChatOutput)
    {
        ShowWindow(m_hwndCopilotChatOutput, visible ? SW_SHOW : SW_HIDE);
    }
    
    if (m_hwndCopilotChatInput)
    {
        ShowWindow(m_hwndCopilotChatInput, visible ? SW_SHOW : SW_HIDE);
        if (visible) focusInput();
    }

    METRICS.gauge("chat.visible", visible ? 1 : 0);
}
...
void Win32IDE::HandleCopilotSend()
{
    ...
    // If the dropdown has a valid selection, configure the backend before sending.
    int modelIdx = (int)SendMessage(m_hwndModelSelector, CB_GETCURSEL, 0, 0);
    if (modelIdx >= 0 && modelIdx < (int)m_availableModels.size())
    {
        const std::string& selectedModel = m_availableModels[modelIdx];
        std::error_code ec;
        if (std::filesystem::exists(selectedModel, ec) && !ec)
        {
            std::string ext = std::filesystem::path(selectedModel).extension().string();
            std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);
            if (ext == ".gguf" || ext == ".bin" || ext == ".ggml")
            {
                if (m_loadedModelPath != selectedModel)
                    loadModelFromPath(selectedModel);
                m_ollamaModelOverride.clear();
            }
        }
        else
        {
            // Alias (e.g. "mistral") — configure the bridge
    ...
    // Always forward to sendCopilotMessage — it writes all output directly to the chat pane.
    sendCopilotMessage(userMessage);
}
...
void Win32IDE::onModelSelectionChanged()
{
    ...
    {
        const std::string& selected = m_availableModels[idx];
        ...
        {
            m_ollamaModelOverride = selected;
        }
    }
}
    ...
    switch (uMsg)
    {
        ...
        case WM_COMMAND:
        {
            if (pThis)
            {
                ...
                if (controlId == IDC_COPILOT_SEND_BTN && notifyCode == BN_CLICKED)
                {
                    OutputDebugStringA("[SidebarProcImpl] Send clicked\n");
                    pThis->HandleCopilotSend();
                    return 0;
                }
                else if (controlId == IDC_COPILOT_CLEAR_BTN && notifyCode == BN_CLICKED)
                {
                    OutputDebugStringA("[SidebarProcImpl] Clear clicked\n");
                    pThis->HandleCopilotClear();
                    return 0;
                }
                else if (controlId == IDC_MODEL_BROWSE_BTN && notifyCode == BN_CLICKED)
                {
                ...
                else if (controlId == 6040 && notifyCode == CBN_SELCHANGE)
                {
                    OutputDebugStringA("[SidebarProcImpl] Debug configuration changed\n");
                    pThis->onDebugConfigurationChanged();
                    return 0;
                }
                else if (controlId == 4200 && notifyCode == CBN_SELCHANGE)
                {
                    HWND hContextCombo = (HWND)lParam;
                    int sel = (int)SendMessage(hContextCombo, CB_GETCURSEL, 0, 0);
                    if (sel >= 0)
                    {
                        wchar_t text[128] = {0};
                        SendMessageW(hContextCombo, CB_GETLBTEXT, sel, (LPARAM)text);
                        std::string s = wideToUtf8(text);
                        size_t endDigits = 0;
                        while (endDigits < s.size() && std::isdigit((unsigned char)s[endDigits]))
                            ++endDigits;
                        if (endDigits > 0)
                        {
                            int ctx = std::atoi(s.substr(0, endDigits).c_str());
                            if (ctx > 0)
                                pThis->onAIContextSize(ctx);
                        }
                    }
                    return 0;
                }
                ...
                else if (controlId == IDC_AI_WORKFLOW_EXECUTOR && notifyCode == BN_CLICKED)
                {
                    pThis->onAIModeWorkflowExecutor();
                }
                else if (controlId == IDC_AI_WORKFLOW_EXECUTOR_AGENT_COUNT && notifyCode == CBN_SELCHANGE)
                {
                    HWND hAgentCountCombo = (HWND)lParam;
                    int sel = (int)SendMessage(hAgentCountCombo, CB_GETCURSEL, 0, 0);
                    if (sel >= 0)
                        pThis->onAIWorkflowAgentCountChanged(sel + 1);
                    return 0;
                }
            }
            return 0;
        ...
        case WM_HSCROLL:
        {
            if (pThis)
            {
                ...
                if (hSlider == pThis->m_hwndMaxTokensSlider)
                {
                    int pos = (int)SendMessage(hSlider, TBM_GETPOS, 0, 0);
                    pThis->onMaxTokensChanged(pos);
                    return 0;
                }
                if (hSlider == pThis->m_hwndContextSlider)
                {
                    static const int kContexts[] = {4096, 8192, 16384, 32768, 65536, 131072, 262144};
                    int idx = (int)SendMessage(hSlider, TBM_GETPOS, 0, 0);
                    if (idx < 0)
                        idx = 0;
                    if (idx >= (int)(sizeof(kContexts) / sizeof(kContexts[0])))
                        idx = (int)(sizeof(kContexts) / sizeof(kContexts[0])) - 1;
                    pThis->onAIContextSize(kContexts[idx]);
                    return 0;
                }
            }
            break;
        ...
        case WM_SIZE:
        {
            if (pThis)
            {
                // Dynamically reposition chat controls to fill the resized sidebar.
                // Header section (model row, sliders, checkboxes) is fixed at top ~200px;
                // chat output expands in the middle; input + buttons pin to the bottom.
                ...
                if (w > 10 && h > 50)
                {
                    ...
                    const int INPUT_H = 85;
                    const int BTN_H = 30;
                    int ctrlW = w - MARGIN * 2;
                    int outH = h - HEADER - MARGIN - INPUT_H - MARGIN - BTN_H - MARGIN;
                    if (outH < 50)
                        outH = 50;
                    int inputY = HEADER + outH + MARGIN;
                    int btnsY = inputY + INPUT_H + MARGIN;
                    int halfW = ctrlW / 2 - MARGIN;
                    if (pThis->m_hwndCopilotChatOutput && IsWindow(pThis->m_hwndCopilotChatOutput))
                        MoveWindow(pThis->m_hwndCopilotChatOutput, MARGIN, HEADER, ctrlW, outH, TRUE);
                    if (pThis->m_hwndCopilotChatInput && IsWindow(pThis->m_hwndCopilotChatInput))
                        MoveWindow(pThis->m_hwndCopilotChatInput, MARGIN, inputY, ctrlW, INPUT_H, TRUE);
                    if (pThis->m_hwndCopilotSendBtn && IsWindow(pThis->m_hwndCopilotSendBtn))
                        MoveWindow(pThis->m_hwndCopilotSendBtn, MARGIN, btnsY, halfW, BTN_H, TRUE);
                    if (pThis->m_hwndCopilotClearBtn && IsWindow(pThis->m_hwndCopilotClearBtn))
                        MoveWindow(pThis->m_hwndCopilotClearBtn, MARGIN + halfW + MARGIN * 2, btnsY, halfW, BTN_H,
                                   TRUE);
                }
        ...
        default:
        {
            // WM_CHAT_RESPONSE (WM_APP+550): background inference thread finished.
            // lParam is a heap-allocated std::string* that the handler must delete.
            if (uMsg == (WM_APP + 550))
            {
                ...
                if (pThis && resp)
                {
                    ...
                    if (!pThis->m_chatHistory.empty() && pThis->m_chatHistory.back().first == "assistant" &&
                        pThis->m_chatHistory.back().second.compare(0, kThinking.size(), kThinking) == 0)
                    {
                        pThis->m_chatHistory.back().second = *resp;
                    }
                    else
                    {
                        pThis->m_chatHistory.push_back({"assistant", *resp});
                    }
                    if (pThis->m_currentToolActions.totalActions() > 0)
                    {
                        size_t msgIdx = pThis->m_chatHistory.size() - 1;
                        pThis->m_chatToolActions[msgIdx] = pThis->m_currentToolActions.actions();
                        pThis->m_chatToolActions[msgIdx].push_back(
                            RawrXD::UI::ToolActionStatus::FinishedAction(pThis->m_currentToolActions.totalActions()));
