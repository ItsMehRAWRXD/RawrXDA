 ...
 * Pure Win32 replacement for Qt resource system (integrated into Win32 IDE)
...
public:
    ...
    void LoadResourceFromFile(const wchar_t* filePath, const char* resourceId) {
        ...
        if (fileSize == 0 || fileSize == (DWORD)-1) {
            CloseHandle(hFile);
            return;
        }
        ...
        std::string key(resourceId ? resourceId : "");
        m_resources[key] = std::move(data);
    ...
    void RegisterResource(const char* resourceId, const char* data, size_t size) {
        ...
        m_resources[resourceId ? resourceId : ""] = std::move(res);
    ...
    bool GetResource(const char* resourceId, char** outData, size_t* outSize) {
        ...
        auto it = m_resources.find(resourceId ? resourceId : "");
    ...
    bool ResourceExists(const char* resourceId) const {
        EnterCriticalSection(&m_criticalSection);
        bool exists = m_resources.find(resourceId ? resourceId : "") != m_resources.end();
        LeaveCriticalSection(&m_criticalSection);
        return exists;
    }
