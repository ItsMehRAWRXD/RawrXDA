...
void Win32IDE::initializeAgenticBridge() {
    ...
    if (!m_agenticBridge) {
        m_agenticBridge = new AgenticBridge(this);
...
void Win32IDE::onAIModeWorkflowExecutor() {
    LOG_INFO("onAIModeWorkflowExecutor toggled");

    bool current = (GetMenuState(m_hMenu, IDM_AI_AGENT_MULTI_ENABLE, MF_BYCOMMAND) & MF_CHECKED) != 0;
    bool newState = !current;

    if (m_agenticBridge) {
        m_agenticBridge->SetWorkflowExecutorEnabled(newState);
    }

    if (m_hwndChkWorkflowExecutor) {
        SendMessage(m_hwndChkWorkflowExecutor, BM_SETCHECK, newState ? BST_CHECKED : BST_UNCHECKED, 0);
    }

    appendToOutput(std::string("Workflow Executor ") + (newState ? "ENABLED" : "DISABLED") + "\n", "Output",
                   OutputSeverity::Info);
}
...
void Win32IDE::syncAgentModeUiFromBridge() {
    ...
    if (m_hwndChkMaxMode) {
        SendMessage(m_hwndChkMaxMode, BM_SETCHECK, m_agenticBridge->GetMaxMode() ? BST_CHECKED : BST_UNCHECKED, 0);
    }
    if (m_hwndChkDeepThink) {
        SendMessage(m_hwndChkDeepThink, BM_SETCHECK,
                    m_agenticBridge->GetDeepThinking() ? BST_CHECKED : BST_UNCHECKED, 0);
    }
    if (m_hwndChkDeepResearch) {
        SendMessage(m_hwndChkDeepResearch, BM_SETCHECK,
                    m_agenticBridge->GetDeepResearch() ? BST_CHECKED : BST_UNCHECKED, 0);
    }
    if (m_hwndChkNoRefusal) {
        SendMessage(m_hwndChkNoRefusal, BM_SETCHECK,
                    m_agenticBridge->GetNoRefusal() ? BST_CHECKED : BST_UNCHECKED, 0);
    }
    if (m_hwndChkWorkflowExecutor) {
        SendMessage(m_hwndChkWorkflowExecutor, BM_SETCHECK,
                    m_agenticBridge->GetWorkflowExecutorEnabled() ? BST_CHECKED : BST_UNCHECKED, 0);
    }
    if (m_hwndWorkflowAgentCountCombo) {
        int count = m_agenticBridge->GetWorkflowExecutorAgentCount();
        if (count < 1) count = 1;
        SendMessage(m_hwndWorkflowAgentCountCombo, CB_SETCURSEL, count - 1, 0);
    }
}

void Win32IDE::onAIWorkflowAgentCountChanged(int agentCount) {
    if (agentCount < 1) agentCount = 1;
    if (agentCount > 16) agentCount = 16;

    if (m_agenticBridge) {
        m_agenticBridge->SetWorkflowExecutorAgentCount(agentCount);
    }

    if (m_hwndWorkflowAgentCountCombo) {
        SendMessage(m_hwndWorkflowAgentCountCombo, CB_SETCURSEL, agentCount - 1, 0);
    }

    appendToOutput("Workflow Executor agent count set to " + std::to_string(agentCount) + "\n", "Output",
                   OutputSeverity::Info);
}
