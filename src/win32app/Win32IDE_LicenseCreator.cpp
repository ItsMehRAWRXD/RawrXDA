...
static bool createLicenseInline(uint8_t tier, int days, uint64_t features, const wchar_t* outputPath)
{
...
static bool createV2LicenseInline(RawrXD::License::LicenseTierV2 tier, uint32_t days, const wchar_t* outputPath,
                                  const char* signingSecret, bool bindMachine)
{
...
static bool createV2LicenseFromDialog(HWND hwnd, Win32IDE* ide, RawrXD::License::LicenseTierV2 tier,
...
static void refreshLicenseUI(HWND hwnd)
{
    ...
    const char* tierName = RawrXD::License::tierName(RawrXD::License::EnterpriseLicenseV2::Instance().currentTier());
...
static void buildFeatureText(std::wstringstream& ss)
{
    ...
    ss << L"Max Model: " << lic.GetMaxModelSizeGB() << L"GB | Max Context: " << lic.GetMaxContextLength()
       << L" tokens\r\n\r\n";
    ...
    for (const auto& f : s_features)
    {
        ...
        if (f.mask == 0x01u)
            enabled = enabled || lic.Is800BUnlocked() ||
                      RawrXD::License::EnterpriseLicenseV2::Instance().isFeatureEnabled(
    ...
    for (const auto& f : s_features)
    {
        bool on = (mask & f.mask) != 0;
        if (f.mask == 0x01u)
            on = on || lic.Is800BUnlocked() ||
                 RawrXD::License::EnterpriseLicenseV2::Instance().isFeatureEnabled(
                     RawrXD::License::FeatureID::DualEngine800B);
        if (on)
            active++;
...
static void buildFeatureTextV2(std::wstringstream& ss)
{
    ...
    ss << L"Enabled: " << lic.enabledFeatureCount() << L" / " << TOTAL_FEATURES << L" | Implemented: "
       << lic.countImplemented() << L" | UI Wired: " << lic.countWiredToUI() << L" | Tested: " << lic.countTested()
       << L"\r\n";
    ss << L"Max Model: " << limits.maxModelGB << L"GB | Max Context: " << limits.maxContextTokens << L" tokens\r\n\r\n";
    ...
    for (uint32_t i = 0; i < TOTAL_FEATURES; ++i)
    {
        ...
        ss << (enabled ? L"  [ON]  " : (licensed ? L"  [LCK] " : L"  [OFF] "))
           << utf8ToWide(def.name ? def.name : "Unknown") << L" | " << utf8ToWide(tierName(def.minTier)) << L" | "
           << (def.implemented ? L"IMPL" : L"----") << L" | " << (def.wiredToUI ? L"UI" : L"--") << L" | "
           << (def.tested ? L"TEST" : L"----") << L"\r\n";
    }
...
LRESULT Win32IDE::LicenseCreatorWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    switch (msg)
    {
        case WM_CREATE:
        {
            ...
            CreateWindowExW(0, L"STATIC", L"Enterprise License Creator", WS_CHILD | WS_VISIBLE | SS_LEFT, 12, 10, 400,
                            20, hwnd, nullptr, nullptr, nullptr);

            CreateWindowExW(0, L"STATIC", L"Status:", WS_CHILD | WS_VISIBLE | SS_RIGHT, 12, 38, 80, 18, hwnd, nullptr,
                            nullptr, nullptr);

            {
                const char* ed = RawrXD::EnterpriseLicense::Instance().GetEditionName();
                CreateWindowExW(0, L"EDIT", utf8ToWide(ed ? ed : "Unknown").c_str(),
                                WS_CHILD | WS_VISIBLE | ES_READONLY | WS_BORDER, 100, 35, 300, 22, hwnd,
                                (HMENU)(UINT_PTR)IDC_LC_STATUS, nullptr, nullptr);
            }

            CreateWindowExW(0, L"EDIT", L"",
                            WS_CHILD | WS_VISIBLE | ES_MULTILINE | ES_READONLY | WS_VSCROLL | WS_BORDER, 12, 65, 510,
                            180, hwnd, (HMENU)(UINT_PTR)IDC_LC_FEATURES, nullptr, nullptr);

            int y = 255;
            if (getenv("RAWRXD_ENTERPRISE_DEV"))
            {
                CreateWindowExW(0, L"BUTTON", L"Dev Unlock", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 12, y, 100, 26,
                                hwnd, (HMENU)(UINT_PTR)IDC_LC_DEV_UNLOCK, nullptr, nullptr);
                y += 32;
            }

            CreateWindowExW(0, L"BUTTON", L"Install License...", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 12, y, 120, 26,
                            hwnd, (HMENU)(UINT_PTR)IDC_LC_INSTALL, nullptr, nullptr);

            CreateWindowExW(0, L"BUTTON", L"Copy HWID", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 140, y, 90, 26, hwnd,
                            (HMENU)(UINT_PTR)IDC_LC_COPY_HWID, nullptr, nullptr);

            CreateWindowExW(0, L"BUTTON", L"Refresh", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 240, y, 80, 26, hwnd,
                            (HMENU)(UINT_PTR)IDC_LC_REFRESH, nullptr, nullptr);

            CreateWindowExW(0, L"BUTTON", L"Close", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 430, y, 80, 26, hwnd,
                            (HMENU)(UINT_PTR)IDC_LC_CLOSE, nullptr, nullptr);

            // Row 2: License creation buttons (V1)
            y += 32;
            CreateWindowExW(0, L"BUTTON", L"Create Trial (30d)", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 12, y, 130, 26,
                            hwnd, (HMENU)(UINT_PTR)IDC_LC_GEN_TRIAL, nullptr, nullptr);

            CreateWindowExW(0, L"BUTTON", L"Create Pro (180d)", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 148, y, 126, 26,
                            hwnd, (HMENU)(UINT_PTR)IDC_LC_GEN_PRO, nullptr, nullptr);

            CreateWindowExW(0, L"BUTTON", L"Create Enterprise (365d)", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 280, y,
                            160, 26, hwnd, (HMENU)(UINT_PTR)IDC_LC_GEN_ENTERPRISE, nullptr, nullptr);

            // V2 section
            y += 40;
            CreateWindowExW(0, L"STATIC", L"Enterprise V2 License Status:", WS_CHILD | WS_VISIBLE | SS_LEFT, 12, y, 240,
                            18, hwnd, nullptr, nullptr, nullptr);

            CreateWindowExW(0, L"EDIT", L"", WS_CHILD | WS_VISIBLE | ES_READONLY | WS_BORDER, 250, y - 3, 272, 22, hwnd,
                            (HMENU)(UINT_PTR)IDC_LC_STATUS_V2, nullptr, nullptr);

            y += 26;
            CreateWindowExW(0, L"STATIC", L"V2 Secret:", WS_CHILD | WS_VISIBLE | SS_LEFT, 12, y, 80, 18, hwnd, nullptr,
                            nullptr, nullptr);
            CreateWindowExW(0, L"EDIT", L"", WS_CHILD | WS_VISIBLE | ES_PASSWORD | WS_BORDER, 95, y - 3, 220, 22, hwnd,
                            (HMENU)(UINT_PTR)IDC_LC_V2_SECRET, nullptr, nullptr);
            CreateWindowExW(0, L"STATIC", L"Days:", WS_CHILD | WS_VISIBLE | SS_LEFT, 325, y, 40, 18, hwnd, nullptr,
                            nullptr, nullptr);
            CreateWindowExW(0, L"EDIT", L"365", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_NUMBER, 370, y - 3, 60, 22, hwnd,
                            (HMENU)(UINT_PTR)IDC_LC_V2_DAYS, nullptr, nullptr);

            y += 26;
            CreateWindowExW(0, L"STATIC", L"Output:", WS_CHILD | WS_VISIBLE | SS_LEFT, 12, y, 80, 18, hwnd, nullptr,
                            nullptr, nullptr);
            CreateWindowExW(0, L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 95, y - 3, 300, 22,
                            hwnd, (HMENU)(UINT_PTR)IDC_LC_V2_OUTPUT, nullptr, nullptr);
            CreateWindowExW(0, L"BUTTON", L"Browse...", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 405, y - 3, 90, 22, hwnd,
                            (HMENU)(UINT_PTR)IDC_LC_V2_BROWSE, nullptr, nullptr);

            y += 26;
            HWND hBind =
                CreateWindowExW(0, L"BUTTON", L"Bind to this machine (HWID)", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
                                12, y, 220, 20, hwnd, (HMENU)(UINT_PTR)IDC_LC_V2_BIND, nullptr, nullptr);
            if (hBind)
                SendMessageW(hBind, BM_SETCHECK, BST_CHECKED, 0);

            y += 30;
            CreateWindowExW(0, L"EDIT", L"",
                            WS_CHILD | WS_VISIBLE | ES_MULTILINE | ES_READONLY | WS_VSCROLL | WS_BORDER, 12, y, 510,
                            180, hwnd, (HMENU)(UINT_PTR)IDC_LC_FEATURES_V2, nullptr, nullptr);

            y += 190;
            if (getenv("RAWRXD_ENTERPRISE_DEV"))
            {
                CreateWindowExW(0, L"BUTTON", L"V2 Dev Unlock", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 12, y, 110, 26,
                                hwnd, (HMENU)(UINT_PTR)IDC_LC_V2_DEV_UNLOCK, nullptr, nullptr);
            }

            CreateWindowExW(0, L"BUTTON", L"V2 Install Key...", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 130, y, 120, 26,
                            hwnd, (HMENU)(UINT_PTR)IDC_LC_V2_INSTALL, nullptr, nullptr);

            CreateWindowExW(0, L"BUTTON", L"V2 Refresh", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 260, y, 90, 26, hwnd,
                            (HMENU)(UINT_PTR)IDC_LC_V2_REFRESH, nullptr, nullptr);

            y += 32;
            CreateWindowExW(0, L"BUTTON", L"V2 Create Pro (180d)", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 12, y, 160,
                            26, hwnd, (HMENU)(UINT_PTR)IDC_LC_V2_GEN_PRO, nullptr, nullptr);

            CreateWindowExW(0, L"BUTTON", L"V2 Create Ent (365d)", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 180, y, 170,
                            26, hwnd, (HMENU)(UINT_PTR)IDC_LC_V2_GEN_ENT, nullptr, nullptr);

            CreateWindowExW(0, L"BUTTON", L"V2 Create Sov (365d)", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 360, y, 170,
                            26, hwnd, (HMENU)(UINT_PTR)IDC_LC_V2_GEN_SOV, nullptr, nullptr);
        ...
        case WM_COMMAND:
        {
            ...
            if (id == IDC_LC_GEN_ENTERPRISE && ide)
            {
                ...
                {
                    ide->appendToOutput(
                        "[License] Enterprise license (365 days, all features) created and installed.\n", "Output",
                        OutputSeverity::Info);
                    refreshLicenseUI(hwnd);
                }
                else
            ...
            if (id == IDC_LC_LAUNCH_KEYGEN)
            {
                ...
                while (lastSlash > exePath && *lastSlash != L'\\')
                    --lastSlash;
                                   ...
                                   dirPath[0] ? dirPath : nullptr, &si, &pi))
                {
                    ...
                    if (ide)
                        ide->appendToOutput("[License] RawrXD_KeyGen launched.\n", "Output", OutputSeverity::Info);
                }
                else
                {
                    wcscpy_s(exePath, L"RawrXD_KeyGen.exe");
                    if (CreateProcessW(nullptr, exePath, nullptr, nullptr, FALSE, 0, nullptr, nullptr, &si, &pi))
                    {
                        ...
                        ide->appendToOutput(
                            "[License] RawrXD_KeyGen not found. Build from src/tools/RawrXD_KeyGen.cpp\n", "Output",
                            OutputSeverity::Warning);
                }
                return 0;
            }
...
void Win32IDE::showLicenseCreatorDialog()
{
    ...
    HWND child = CreateWindowExW(WS_EX_DLGMODALFRAME, L"RawrXD_LicenseCreator", L"Enterprise License Creator",
                                 WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_VISIBLE, x, y, w, h, m_hwndMain, nullptr,
                                 m_hInstance, this);
    if (child)
    {
...
void Win32IDE::showFeatureRegistryDialog()
{
    ...
    m_hwndFeatureRegistryHost =
        CreateWindowExW(WS_EX_APPWINDOW, L"RawrXD_FeatureRegistryHost", L"Enterprise Feature Registry",
                        WS_OVERLAPPEDWINDOW | WS_VISIBLE, x, y, w, h, m_hwndMain, nullptr, m_hInstance, this);
