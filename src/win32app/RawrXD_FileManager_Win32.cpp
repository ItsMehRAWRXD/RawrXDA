 ...
 * Pure Win32 replacement for Qt file operations (integrated into Win32 IDE)
...
extern "C" {
    ...
    RAWRXD_SHIP_EXPORT bool __stdcall FileManager_FileExists(void* mgr, const wchar_t* filePath) {
        RawrXDFileManager* m = static_cast<RawrXDFileManager*>(mgr);
        return m ? m->FileExists(filePath) : false;
    }

    RAWRXD_SHIP_EXPORT bool __stdcall FileManager_DeleteFile(void* mgr, const wchar_t* filePath) {
        RawrXDFileManager* m = static_cast<RawrXDFileManager*>(mgr);
        return m ? m->DeleteFile(filePath) : false;
    }

    RAWRXD_SHIP_EXPORT bool __stdcall FileManager_CreateDirectory(void* mgr, const wchar_t* dirPath) {
        RawrXDFileManager* m = static_cast<RawrXDFileManager*>(mgr);
        return m ? m->CreateDirectory(dirPath) : false;
    }
}
