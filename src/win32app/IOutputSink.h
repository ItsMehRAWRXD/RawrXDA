...
// NOTE: Cannot use 'TokenOrigin' — conflicts with Windows winnt.h
//       TOKEN_INFORMATION_CLASS::TokenOrigin enum value.
enum class StreamTokenOrigin : int {
...
public:
    ...
    virtual void onStreamingToken(const char* token, size_t length, StreamTokenOrigin origin) noexcept = 0;
...
public:
    ...
    void onStreamingToken(const char* token, size_t length, StreamTokenOrigin origin) noexcept override;
...
public:
    ...
    void onStreamingToken(const char*, size_t, StreamTokenOrigin) noexcept override {}
