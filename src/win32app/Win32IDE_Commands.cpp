...
void Win32IDE::handleViewCommand(int commandId)
{
    switch (commandId)
    {
        ...
        case 2032:  // IDM_VIEW_OLLAMA_SERVICE — show/hide Ollama service pane
        {
            if (m_hwndOllamaPane)
            {
                bool isVisible = IsWindowVisible(m_hwndOllamaPane);
                ShowWindow(m_hwndOllamaPane, isVisible ? SW_HIDE : SW_SHOW);
                
                if (!isVisible && m_hwndMain)
                {
                    // Update layout when showing
                    RECT clientRect;
                    if (GetClientRect(m_hwndMain, &clientRect))
                    {
            ...
            SendMessage(m_hwndStatusBar, SB_SETTEXT, 0, (LPARAM) "Ollama Service");
            break;
        }
...
void Win32IDE::handleToolsCommand(int commandId)
{
    switch (commandId)
    {
        ...
        case IDM_ASM_LINT_CURRENT_FILE:  // 5999
        ...
        case IDM_AI_BENCHMARK_MENU:
            ...
            break;
        case IDM_INFERENCE_DEBUGGER_PANEL:
            showInferenceDebuggerPanel();
            break;
...
void Win32IDE::buildCommandRegistry()
{
    ...
    m_commandRegistry.push_back({IDM_ASM_LINT_CURRENT_FILE, "ASM: Lint Current File (ml64)", "", "ASM"});
    ...
    m_commandRegistry.push_back(
        {IDM_INFERENCE_DEBUGGER_PANEL, "AI: Inference Debugger (Sovereign latent probe)", "", "AI"});
