...
void Win32IDE::cmdHotpatchMemoryApply() {
    ...
    appendToOutput("[Hotpatch] Memory patch dialog ready. Use CLI '!hotpatch_apply' for scripted access.\n");
...
void Win32IDE::cmdHotpatchMemoryRevert() {
    ...
    appendToOutput("[Hotpatch] Memory revert: Use CLI '!hotpatch_revert <index>' to revert tracked patches.\n");
    appendToOutput("[Hotpatch] View tracked patches with '!hotpatch_status'.\n");
}
...
void Win32IDE::cmdHotpatchByteApply() {
    ...
    if (GetOpenFileNameA(&ofn)) {
        ...
        appendToOutput("[Hotpatch] Use CLI '!hotpatch_byte <offset> <hex_bytes>' to apply byte patches.\n");
        appendToOutput("[Hotpatch] Uses CreateFileMapping for zero-copy I/O — no full file load.\n");
    }
}
...
void Win32IDE::cmdHotpatchByteSearch() {
    ...
    if (GetOpenFileNameA(&ofn)) {
        ...
        appendToOutput("[Hotpatch] Use CLI '!hotpatch_search <hex_pattern> <hex_replacement>'\n");
        appendToOutput("[Hotpatch] Uses SIMD-accelerated pattern matching (SSE2 + Boyer-Moore ASM).\n");
    }
}
...
void Win32IDE::cmdHotpatchServerAdd() {
    ...
    appendToOutput("  PreRequest  — Modify request before inference\n");
    appendToOutput("  PostRequest — Modify request after preprocessing\n");
    appendToOutput("  PreResponse — Modify response before delivery\n");
    appendToOutput("  PostResponse— Modify response after delivery\n");
    appendToOutput("  StreamChunk — Intercept streaming tokens\n");
    appendToOutput("[Hotpatch] Use CLI '!hotpatch_server add <name> <point>' to register a patch.\n");
}
...
void Win32IDE::cmdHotpatchServerRemove() {
    ...
    appendToOutput("[Hotpatch] Use CLI '!hotpatch_server remove <name>' to unregister a patch.\n");
}
...
void Win32IDE::cmdHotpatchProxyBias() {
    ...
    appendToOutput("[Hotpatch] Use CLI '!hotpatch_bias <token_id> <bias_value> [permanent]'\n");
}
...
void Win32IDE::cmdHotpatchProxyRewrite() {
    ...
    appendToOutput("[Hotpatch] Use CLI '!hotpatch_rewrite add <pattern> <replacement>'\n");
    appendToOutput("[Hotpatch] Use CLI '!hotpatch_rewrite remove <name>'\n");
}
...
void Win32IDE::cmdHotpatchProxyTerminate() {
    ...
    appendToOutput("[Hotpatch] Use CLI '!hotpatch_terminate add <name> <stop_seq> [max_tokens]'\n");
    appendToOutput("[Hotpatch] Use CLI '!hotpatch_terminate remove <name>'\n");
}
...
void Win32IDE::cmdHotpatchProxyValidate() {
    ...
    appendToOutput("[Hotpatch] Built-in validators: length_check, json_syntax, safety_filter\n");
    appendToOutput("[Hotpatch] Use CLI '!hotpatch_validate add <name>' to enable a built-in.\n");
}
