...
public:
    SQLite3DatabaseManager(Win32IDE* ide)
        : m_ide(ide), m_db(nullptr), m_initialized(false) {}
    ...
    bool executeParameterizedQuery(const std::string& sql,
    ...
    bool storeTelemetryEvent(const std::string& eventType, const std::string& eventData) {
        std::string sql = "INSERT INTO telemetry_events (event_type, event_data, timestamp) VALUES (?, ?, datetime('now'));";
    ...
    bool saveAgentState(const std::string& agentId, const std::string& stateData) {
        std::string sql = "INSERT OR REPLACE INTO agent_states (agent_id, state_data, updated_at) VALUES (?, ?, datetime('now'));";
...
bool Win32IDE::saveSetting(const std::string& key, const std::string& value) {
    if (!m_sqliteManager) initSQLite3Core();
