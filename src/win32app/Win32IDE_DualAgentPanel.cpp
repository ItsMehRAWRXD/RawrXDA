...
void Win32IDE::handleDualAgentInitEndpoint(SOCKET client, const std::string& body) {
    ...
    LOG_INFO("[Phase41] Dual-agent init: architect_profile=" + std::to_string(archIdx) + ", coder_profile=" + std::to_string(coderIdx));
    ...
    if (!s_bridgeInit) {
        ...
        if (!s_bridgeInit) {
            LOG_ERROR("[Phase41] ModelBridge_Init failed");
    ...
    if (result != SWARM_OK) {
        LOG_ERROR("[Phase41] Swarm_Init failed: code=" + std::to_string(result) + " (" + GetSwarmErrorName(result) + ")");
        char buf[512];
    ...
    LOG_INFO("[Phase41] Swarm initialized: ring=" + std::to_string(state ? state->ring_size / (1024ULL * 1024ULL) : 0ULL) + "MB");
...
void Win32IDE::handleDualAgentShutdownEndpoint(SOCKET client) {
    ...
    LOG_INFO("[Phase41] Dual-agent shutdown requested");
    ...
    if (result != SWARM_OK) {
        LOG_ERROR("[Phase41] Swarm_Shutdown failed: code=" + std::to_string(result) + " (" + GetSwarmErrorName(result) + ")");
        char buf[256];
        std::snprintf(buf, sizeof(buf),
    ...
    LOG_INFO("[Phase41] Swarm shut down cleanly");
    std::string resp = buildHttpResponse(200,
...
void Win32IDE::handleDualAgentHandoffEndpoint(SOCKET client, const std::string& body) {
    ...
    LOG_INFO("[Phase41] Handoff: " + std::to_string(context.size()) + " bytes context");
    ...
    if (result != SWARM_OK) {
        LOG_WARNING("[Phase41] Handoff failed: ring_full (code=" + std::to_string(result) + ")");
    ...
    LOG_INFO("[Phase41] Handoff OK: " + std::to_string(dataLen) + " -> " + std::to_string(alignedLen) + " bytes (aligned)");
    std::string resp = buildHttpResponse(200, buf);
    send(client, resp.c_str(), (int)resp.size(), 0);
#else
    char buf[256];
...
void Win32IDE::handleDualAgentSubmitEndpoint(SOCKET client, const std::string& body) {
    ...
    if (!extractString(body, "agent", agentName)) {
        std::string resp = buildHttpResponse(400,
            R"json({"error":"Missing 'agent' field (expected 'architect' or 'coder')"})json");
        send(client, resp.c_str(), (int)resp.size(), 0);
        return;
    }
    ...
    LOG_INFO("[Phase41] Submit: agent=" + agentName + ", task_type=" + std::to_string(taskTypeInt) + ", data=" + std::to_string(data.size()) + " bytes");
    ...
    if (result != SWARM_OK) {
        LOG_ERROR("[Phase41] SubmitTask failed: code=" + std::to_string(result));
        char buf[256];
        std::snprintf(buf, sizeof(buf),
    ...
    LOG_INFO("[Phase41] Task submitted: id=" + std::to_string(task.task_id) + " -> " + agentName);
    std::string resp = buildHttpResponse(200, buf);
    send(client, resp.c_str(), (int)resp.size(), 0);
#else
    char buf[256];
