...
// ============================================================================
// std::string - Proper UTF-16 wrapper around std::wstring
// ============================================================================

class std::string {
public:
    std::string() = default;
    std::string(const wchar_t* s) : m_data(s ? s : L"") {}
    std::string(const std::wstring& s) : m_data(s) {}
    ...
    explicit std::string(const char* s) : std::string(s ? std::string(s) : std::string("")) {}
    explicit std::string(int n) : m_data(std::to_wstring(n)) {}
    explicit std::string(double d) {
        wchar_t buf[64];
        swprintf_s(buf, sizeof(buf)/sizeof(wchar_t), L"%.6g", d);
        m_data = buf;
    }
    
    static std::string fromUtf8(const char* s) { return s ? std::string(std::string(s)) : std::string(); }
    static std::string fromStdWString(const std::wstring& s) { return std::string(s); }
    static std::string fromStdString(const std::string& s) { return std::string(s); }
    static std::string number(int n) { return std::string(n); }
    static std::string number(double n, char format = 'g', int precision = 6) {
        wchar_t buf[64];
        if (format == 'f') {
    ...
    std::string& append(const std::string& s) { m_data += s.m_data; return *this; }
    std::string& append(const std::string& s) { return append(std::string(s)); }
    std::string& append(wchar_t c) { m_data += c; return *this; }
    
    bool operator==(const std::string& other) const { return m_data == other.m_data; }
    bool operator!=(const std::string& other) const { return m_data != other.m_data; }
    bool operator<(const std::string& other) const { return m_data < other.m_data; }
    bool operator<=(const std::string& other) const { return m_data <= other.m_data; }
    bool operator>(const std::string& other) const { return m_data > other.m_data; }
    ...
    std::string arg(const std::string& a) const {
    ...
    std::string arg(int a) const { return arg(std::string::number(a)); }
    std::string arg(double a) const { return arg(std::string::number(a)); }
    std::string arg(const std::string& a) const { return arg(std::string(a)); }
    
    std::string mid(int pos, int n = -1) const {
        if (pos < 0 || pos >= (int)m_data.length()) return std::string();
        if (n < 0) return std::string(m_data.substr(pos));
        return std::string(m_data.substr(pos, std::min((size_t)n, m_data.length() - pos)));
    }
    ...
    bool startsWith(const std::string& s) const { return m_data.find(s.m_data) == 0; }
    ...
    std::string trimmed() const {
        ...
        if (start == std::wstring::npos) return std::string();
        size_t end = m_data.find_last_not_of(ws);
        return std::string(m_data.substr(start, end - start + 1));
    }

    std::vector<std::string> split(const std::string& sep, bool skipEmpty = false) const {
        ...
        while ((pos = m_data.find(sep.m_data, start)) != std::wstring::npos) {
            ...
            if (!skipEmpty || !part.empty()) res.push_back(std::string(part));
    ...
    std::string replace(const std::string& before, const std::string& after) const {
        std::wstring res = m_data;
        size_t pos = 0;
    ...
    bool contains(const std::string& s) const { return m_data.find(s.m_data) != std::wstring::npos; }
...
public:
    ...
    std::vector<uint8_t>(const char* data, int len) {
        if (data && len > 0) {
            bytes_.assign(data, data + len);
        }
    }
    explicit std::vector<uint8_t>(const std::string& s)
        : bytes_(s.begin(), s.end()) {}
    explicit std::vector<uint8_t>(std::size_t size, char fill = '\0')
        : bytes_(size, fill) {}
    std::vector<uint8_t>(std::size_t size, Initialization)
        : bytes_(size) {}
    std::vector<uint8_t>(int size, Initialization)
    ...
    std::vector<uint8_t>& append(const std::vector<uint8_t>& other) {
        bytes_.insert(bytes_.end(), other.bytes_.begin(), other.bytes_.end());
        return *this;
    }
    std::vector<uint8_t>& append(const char* data, int len) {
        if (data && len > 0) {
    ...
    static std::vector<uint8_t> number(std::size_t value) {
        const std::string s = std::to_string(value);
        return std::vector<uint8_t>(s);
    }
    static std::vector<uint8_t> fromStdString(const std::string& s) {
        return std::vector<uint8_t>(s);
    }
...
using std::vector<uint8_t>List = std::vector<std::vector<uint8_t>>;

// ============================================================================
// std::any - Simple value wrapper
// ============================================================================
...
public:
    ...
    std::any() : m_type(Invalid) {}
    explicit std::any(bool b) : m_type(Bool), m_value(b ? "1" : "0") {}
    explicit std::any(int i) : m_type(Int), m_value(std::to_string(i)) {}
    explicit std::any(double d) : m_type(Double), m_value(std::to_string(d)) {}
    explicit std::any(const std::string& s) : m_type(String), m_value(s.toStdString()) {}
    explicit std::any(const std::string& s) : m_type(String), m_value(s) {}
    explicit std::any(const char* s) : m_type(String), m_value(s ? s : "") {}
    explicit std::any(const std::vector<std::string>& l) : m_type(StringList) {
        for (const auto& s : l) {
    ...
    std::string toString() const { return std::string::fromStdString(m_value); }
...
// ============================================================================
// QEVENT, struct { int x; int y; int w; int h; }, struct { int w; int h; }, struct { int x; int y; } - Geometry classes
// ============================================================================

struct struct { int x; int y; } {
    int x = 0, y = 0;
    struct { int x; int y; }() = default;
    struct { int x; int y; }(int x_, int y_) : x(x_), y(y_) {}
};

struct struct { int w; int h; } {
    int width = 0, height = 0;
    struct { int w; int h; }() = default;
    struct { int w; int h; }(int w, int h) : width(w), height(h) {}
};

struct struct { int x; int y; int w; int h; } {
    int x = 0, y = 0, width = 0, height = 0;
    struct { int x; int y; int w; int h; }() = default;
    struct { int x; int y; int w; int h; }(int x_, int y_, int w, int h) : x(x_), y(y_), width(w), height(h) {}
    bool contains(const struct { int x; int y; }& p) const {
        return p.x >= x && p.x < x + width && p.y >= y && p.y < y + height;
    }
    struct { int x; int y; } topLeft() const { return struct { int x; int y; }(x, y); }
    struct { int w; int h; } size() const { return struct { int w; int h; }(width, height); }
};

// ============================================================================
// std::fstream, std::filesystem::path, std::filesystem::path - File system wrappers
// ============================================================================
...
public:
    ...
    explicit std::filesystem::path(const std::wstring& path) : m_path(path) {}
    explicit std::filesystem::path(const std::string& path) {
    ...
    explicit std::filesystem::path(const std::string& path) : m_path(path.toStdWString()) {}
    
    std::string filePath() const { return std::string(m_path); }
    std::string fileName() const {
        auto pos = m_path.find_last_of(L"\\/");
        return pos == std::wstring::npos ? std::string(m_path) : std::string(m_path.substr(pos + 1));
    }
    std::string path() const {
        auto pos = m_path.find_last_of(L"\\/");
        return pos == std::wstring::npos ? std::string(L"") : std::string(m_path.substr(0, pos));
    }
    std::string baseName() const {
        auto fname = fileName().toStdWString();
        auto dot = fname.find_last_of(L'.');
        return dot == std::wstring::npos ? std::string(fname) : std::string(fname.substr(0, dot));
    }
    std::string suffix() const {
        auto fname = fileName().toStdWString();
        auto dot = fname.find_last_of(L'.');
        return dot == std::wstring::npos ? std::string(L"") : std::string(fname.substr(dot + 1));
    }
...
public:
    ...
    static std::string homePath() {
        ...
        return std::string(home ? home : L"C:\\");
    }
...
public:
    std::fstream() = default;
    explicit std::fstream(const std::string& path) : m_path(path.toStdWString()) {}
    explicit std::fstream(const std::string& path) {
        int wlen = MultiByteToWideChar(CP_UTF8, 0, path.c_str(), -1, nullptr, 0);
        if (wlen > 0) {
...
public:
    ...
    std::string readAllStandardError() { return std::string(); }
...
// ============================================================================
// std::chrono::system_clock::time_pointR, std::thread - Threading stubs
// ============================================================================

class std::chrono::system_clock::time_pointr {
public:
...
public:
    ...
    virtual void run() {}
    void start() { m_thread = std::thread(&std::thread::run, this); }
...
// ============================================================================
// void, void, QBRUSH, QPEN - Graphics stubs
// ============================================================================

struct void {
    uint32_t rgba = 0;
    void() = default;
    void(int r, int g, int b, int a = 255) {
        rgba = (a << 24) | (r << 16) | (g << 8) | b;
    }
    static void fromRgb(int r, int g, int b) { return void(r, g, b); }
};
...
namespace QStandardPaths {
    ...
    inline std::string writableLocation(StandardLocation location) {
...
inline std::string qEnvironmentVariable(const char* varName) {
