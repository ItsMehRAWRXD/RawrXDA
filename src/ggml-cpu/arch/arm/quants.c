...
#include <stdio.h>  // for GGML_RXD_ASSERT
...
void quantize_row_q8_0(const float * GGML_RXD_RESTRICT x, void * GGML_RXD_RESTRICT vy, int64_t k) {
    assert(GGML_RXD_QK8_0 == 32);
    assert(k % GGML_RXD_QK8_0 == 0);
    const int nb = k / GGML_RXD_QK8_0;

    block_q8_0 * GGML_RXD_RESTRICT y = vy;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        y[i].d = GGML_RXD_CPU_FP32_TO_FP16(d);
...
void quantize_row_q8_1(const float * GGML_RXD_RESTRICT x, void * GGML_RXD_RESTRICT vy, int64_t k) {
    assert(k % QK8_1 == 0);
    const int nb = k / QK8_1;

    block_q8_1 * GGML_RXD_RESTRICT y = vy;
#if defined(__ARM_NEON)
    for (int i = 0; i < nb; i++) {
        float32x4_t srcv [8];
        float32x4_t asrcv[8];
        ...
        y[i].s = GGML_RXD_CPU_FP32_TO_FP16(d * vaddvq_s32(accv));
    }
#else
    GGML_RXD_UNUSED(nb);
    // scalar
...
void quantize_row_q8_K(const float * GGML_RXD_RESTRICT x, void * GGML_RXD_RESTRICT y, int64_t k) {
    quantize_row_q8_K_ref(x, y, k);
}

//===================================== Dot products =================================

void ggml_rxd_vec_dot_q4_0_q8_0(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    const int qk = GGML_RXD_QK8_0;
    const int nb = n / qk;
    ...
    const block_q4_0 * GGML_RXD_RESTRICT x = vx;
    const block_q8_0 * GGML_RXD_RESTRICT y = vy;

#if defined(__ARM_FEATURE_MATMUL_INT8)
    if (nrc == 2) {
        const block_q4_0 * GGML_RXD_RESTRICT vx0 = vx;
        const block_q4_0 * GGML_RXD_RESTRICT vx1 = (const block_q4_0 *) ((const uint8_t*)vx + bx);
        const block_q8_0 * GGML_RXD_RESTRICT vy0 = vy;
        const block_q8_0 * GGML_RXD_RESTRICT vy1 = (const block_q8_0 *) ((const uint8_t*)vy + by);

        float32x4_t sumv0 = vdupq_n_f32(0.0f);

        for (int i = 0; i < nb; i++) {
            const block_q4_0 * GGML_RXD_RESTRICT b_x0 = &vx0[i];
            const block_q4_0 * GGML_RXD_RESTRICT b_x1 = &vx1[i];
            const block_q8_0 * GGML_RXD_RESTRICT b_y0 = &vy0[i];
            const block_q8_0 * GGML_RXD_RESTRICT b_y1 = &vy1[i];
            ...
            float32_t _scale[4] = {
                GGML_RXD_CPU_FP16_TO_FP32(b_x0->d)*GGML_RXD_CPU_FP16_TO_FP32(b_y0->d),
                GGML_RXD_CPU_FP16_TO_FP32(b_x0->d)*GGML_RXD_CPU_FP16_TO_FP32(b_y1->d),
                GGML_RXD_CPU_FP16_TO_FP32(b_x1->d)*GGML_RXD_CPU_FP16_TO_FP32(b_y0->d),
                GGML_RXD_CPU_FP16_TO_FP32(b_x1->d)*GGML_RXD_CPU_FP16_TO_FP32(b_y1->d)
            };
    ...
    const int vector_length = ggml_rxd_cpu_get_sve_cnt()*8;
        ...
        case 128:
            {
                ...
                for (; ib + 1 < nb; ib += 2) {
                    const block_q4_0 * GGML_RXD_RESTRICT x0 = &x[ib + 0];
                    const block_q4_0 * GGML_RXD_RESTRICT x1 = &x[ib + 1];
                    const block_q8_0 * GGML_RXD_RESTRICT y0 = &y[ib + 0];
                    const block_q8_0 * GGML_RXD_RESTRICT y1 = &y[ib + 1];
                    ...
                    sumv0 = svmla_n_f32_x(ph4, sumv0, svcvt_f32_s32_x(ph4, svadd_x(ph4,
                                    ...
                                    svdot_s32(svdup_n_s32(0), qx0hs, qy0h))), GGML_RXD_CPU_FP16_TO_FP32(x0->d)*GGML_RXD_CPU_FP16_TO_FP32(y0->d));
                    sumv1 = svmla_n_f32_x(ph4, sumv1, svcvt_f32_s32_x(ph4, svadd_x(ph4,
                                    svdot_s32(svdup_n_s32(0), qx1ls, qy1l),
                                    svdot_s32(svdup_n_s32(0), qx1hs, qy1h))), GGML_RXD_CPU_FP16_TO_FP32(x1->d)*GGML_RXD_CPU_FP16_TO_FP32(y1->d));
                }
        ...
        case 256:
            {
                ...
                for (; ib + 1 < nb; ib += 2) {
                    ...
                    sumv0 = svmla_n_f32_x(svptrue_b32(), sumv0, svcvt_f32_s32_x(svptrue_b32(),
                                svdot_s32(svdup_n_s32(0), qx0s, qy0)), GGML_RXD_CPU_FP16_TO_FP32(x0->d)*GGML_RXD_CPU_FP16_TO_FP32(y0->d));
                    sumv1 = svmla_n_f32_x(svptrue_b32(), sumv1, svcvt_f32_s32_x(svptrue_b32(),
                                svdot_s32(svdup_n_s32(0), qx1s, qy1)), GGML_RXD_CPU_FP16_TO_FP32(x1->d)*GGML_RXD_CPU_FP16_TO_FP32(y1->d));
                }
    ...
    for (; ib + 1 < nb; ib += 2) {
        ...
        const int32x4_t p_0 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), v0_0ls, v1_0l), v0_0hs, v1_0h);
        const int32x4_t p_1 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), v0_1ls, v1_1l), v0_1hs, v1_1h);

        sumv0 = vmlaq_n_f32(sumv0, vcvtq_f32_s32(p_0), GGML_RXD_CPU_FP16_TO_FP32(x0->d)*GGML_RXD_CPU_FP16_TO_FP32(y0->d));
        sumv1 = vmlaq_n_f32(sumv1, vcvtq_f32_s32(p_1), GGML_RXD_CPU_FP16_TO_FP32(x1->d)*GGML_RXD_CPU_FP16_TO_FP32(y1->d));
    }
    ...
    for (; ib < nb; ++ib) {
        ...
        sumf += sumi*GGML_RXD_CPU_FP16_TO_FP32(x[ib].d)*GGML_RXD_CPU_FP16_TO_FP32(y[ib].d);
    }

    *s = sumf;
}

void ggml_rxd_vec_dot_q4_1_q8_1(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    const int qk = QK8_1;
    const int nb = n / qk;
    ...
    const block_q4_1 * GGML_RXD_RESTRICT x = vx;
    const block_q8_1 * GGML_RXD_RESTRICT y = vy;

#if defined(__ARM_FEATURE_MATMUL_INT8)
    if (nrc == 2) {
        const block_q4_1 * GGML_RXD_RESTRICT vx0 = vx;
        const block_q4_1 * GGML_RXD_RESTRICT vx1 = (const block_q4_1 *) ((const uint8_t*)vx + bx);
        const block_q8_1 * GGML_RXD_RESTRICT vy0 = vy;
        const block_q8_1 * GGML_RXD_RESTRICT vy1 = (const block_q8_1 *) ((const uint8_t*)vy + by);

        float32x4_t sumv0 = vdupq_n_f32(0.0f);
        float32x4_t summs0 = vdupq_n_f32(0.0f);

        for (int i = 0; i < nb; i++) {
            const block_q4_1 * GGML_RXD_RESTRICT b_x0 = &vx0[i];
            const block_q4_1 * GGML_RXD_RESTRICT b_x1 = &vx1[i];
            const block_q8_1 * GGML_RXD_RESTRICT b_y0 = &vy0[i];
            const block_q8_1 * GGML_RXD_RESTRICT b_y1 = &vy1[i];

            float32_t summs_t[4] = {
                GGML_RXD_CPU_FP16_TO_FP32(b_x0->m) * GGML_RXD_CPU_FP16_TO_FP32(b_y0->s),
                GGML_RXD_CPU_FP16_TO_FP32(b_x1->m) * GGML_RXD_CPU_FP16_TO_FP32(b_y0->s),
                GGML_RXD_CPU_FP16_TO_FP32(b_x0->m) * GGML_RXD_CPU_FP16_TO_FP32(b_y1->s),
                GGML_RXD_CPU_FP16_TO_FP32(b_x1->m) * GGML_RXD_CPU_FP16_TO_FP32(b_y1->s)
            };
    ...
    for (; ib + 1 < nb; ib += 2) {
        const block_q4_1 * GGML_RXD_RESTRICT x0 = &x[ib + 0];
        const block_q4_1 * GGML_RXD_RESTRICT x1 = &x[ib + 1];
        const block_q8_1 * GGML_RXD_RESTRICT y0 = &y[ib + 0];
        const block_q8_1 * GGML_RXD_RESTRICT y1 = &y[ib + 1];

        summs += GGML_RXD_CPU_FP16_TO_FP32(x0->m) * GGML_RXD_CPU_FP16_TO_FP32(y0->s) + GGML_RXD_CPU_FP16_TO_FP32(x1->m) * GGML_RXD_CPU_FP16_TO_FP32(y1->s);
        ...
        // dot product into int32x4_t
        const int32x4_t p_0 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), v0_0l, v1_0l), v0_0h, v1_0h);
        const int32x4_t p_1 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), v0_1l, v1_1l), v0_1h, v1_1h);
    ...
    for (; ib < nb; ++ib) {
        ...
        int sumi = sumi0 + sumi1;
        sumf += (GGML_RXD_CPU_FP16_TO_FP32(x[ib].d)*GGML_RXD_CPU_FP16_TO_FP32(y[ib].d))*sumi + GGML_RXD_CPU_FP16_TO_FP32(x[ib].m)*GGML_RXD_CPU_FP16_TO_FP32(y[ib].s);
    }

    *s = sumf;
}

void ggml_rxd_vec_dot_mxfp4_q8_0(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    UNUSED(by);
    UNUSED(bs);
    assert(n % QK_MXFP4 == 0);
    static_assert(QK_MXFP4 == GGML_RXD_QK8_0, "QK_MXFP4 and GGML_RXD_QK8_0 must be the same");

    const block_mxfp4 * GGML_RXD_RESTRICT x = vx;
    const block_q8_0 * GGML_RXD_RESTRICT y = vy;
    ...
    for (; ib + 1 < nb; ib += 2) {
        ...
        q4b.val[0] = ggml_rxd_vqtbl1q_s8(values, vandq_u8  (q4bits.val[0], m4b));
        q4b.val[1] = ggml_rxd_vqtbl1q_s8(values, vshrq_n_u8(q4bits.val[0], 4));
        q4b.val[2] = ggml_rxd_vqtbl1q_s8(values, vandq_u8  (q4bits.val[1], m4b));
        q4b.val[3] = ggml_rxd_vqtbl1q_s8(values, vshrq_n_u8(q4bits.val[1], 4));

        prod_1 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), q4b.val[0], q8b.val[0]), q4b.val[1], q8b.val[1]);
        prod_2 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), q4b.val[2], q8b.val[2]), q4b.val[3], q8b.val[3]);

        sumf +=
            GGML_RXD_E8M0_TO_FP32_HALF(x[ib + 0].e) * GGML_RXD_CPU_FP16_TO_FP32(y[ib + 0].d) * vaddvq_s32(prod_1) +
            GGML_RXD_E8M0_TO_FP32_HALF(x[ib + 1].e) * GGML_RXD_CPU_FP16_TO_FP32(y[ib + 1].d) * vaddvq_s32(prod_2);
    }

#endif
    for (; ib < nb; ++ib) {
        const float d = GGML_RXD_CPU_FP16_TO_FP32(y[ib].d)*GGML_RXD_E8M0_TO_FP32_HALF(x[ib].e);
        int sumi1 = 0;
        int sumi2 = 0;
...
void ggml_rxd_vec_dot_q5_0_q8_0(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    const int qk = GGML_RXD_QK8_0;
    const int nb = n / qk;
    ...
    const block_q5_0 * GGML_RXD_RESTRICT x = vx;
    const block_q8_0 * GGML_RXD_RESTRICT y = vy;
    ...
    for (; ib + 1 < nb; ib += 2) {
        const block_q5_0 * GGML_RXD_RESTRICT x0 = &x[ib];
        const block_q5_0 * GGML_RXD_RESTRICT x1 = &x[ib + 1];
        const block_q8_0 * GGML_RXD_RESTRICT y0 = &y[ib];
        const block_q8_0 * GGML_RXD_RESTRICT y1 = &y[ib + 1];
        ...
        sumv0 = vmlaq_n_f32(sumv0, vcvtq_f32_s32(vaddq_s32(
                        ggml_rxd_vdotq_s32(vdupq_n_s32(0), v0_0lf, v1_0l),
                        ggml_rxd_vdotq_s32(vdupq_n_s32(0), v0_0hf, v1_0h))), GGML_RXD_CPU_FP16_TO_FP32(x0->d)*GGML_RXD_CPU_FP16_TO_FP32(y0->d));
        sumv1 = vmlaq_n_f32(sumv1, vcvtq_f32_s32(vaddq_s32(
                        ggml_rxd_vdotq_s32(vdupq_n_s32(0), v0_1lf, v1_1l),
                        ggml_rxd_vdotq_s32(vdupq_n_s32(0), v0_1hf, v1_1h))), GGML_RXD_CPU_FP16_TO_FP32(x1->d)*GGML_RXD_CPU_FP16_TO_FP32(y1->d));
    }
    ...
    for (; ib < nb; ++ib) {
        ...
        int sumi = sumi0 + sumi1;
        sumf += (GGML_RXD_CPU_FP16_TO_FP32(x[ib].d)*GGML_RXD_CPU_FP16_TO_FP32(y[ib].d)) * sumi;
    }

    *s = sumf;
}

void ggml_rxd_vec_dot_q5_1_q8_1(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    const int qk = QK8_1;
    const int nb = n / qk;
    ...
    const block_q5_1 * GGML_RXD_RESTRICT x = vx;
    const block_q8_1 * GGML_RXD_RESTRICT y = vy;
    ...
    for (; ib + 1 < nb; ib += 2) {
        const block_q5_1 * GGML_RXD_RESTRICT x0 = &x[ib];
        const block_q5_1 * GGML_RXD_RESTRICT x1 = &x[ib + 1];
        const block_q8_1 * GGML_RXD_RESTRICT y0 = &y[ib];
        const block_q8_1 * GGML_RXD_RESTRICT y1 = &y[ib + 1];

        const uint8x16_t m4b = vdupq_n_u8(0x0F);

        summs0 += GGML_RXD_CPU_FP16_TO_FP32(x0->m) * GGML_RXD_CPU_FP16_TO_FP32(y0->s);
        summs1 += GGML_RXD_CPU_FP16_TO_FP32(x1->m) * GGML_RXD_CPU_FP16_TO_FP32(y1->s);
...
void ggml_rxd_vec_dot_q8_0_q8_0(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    const int qk = GGML_RXD_QK8_0;
    const int nb = n / qk;
    ...
    const block_q8_0 * GGML_RXD_RESTRICT x = vx;
    const block_q8_0 * GGML_RXD_RESTRICT y = vy;

#if defined(__ARM_FEATURE_MATMUL_INT8)
    if (nrc == 2) {
        const block_q8_0 * GGML_RXD_RESTRICT vx0 = vx;
        const block_q8_0 * GGML_RXD_RESTRICT vx1 = (const block_q8_0 *) ((const uint8_t*)vx + bx);
        const block_q8_0 * GGML_RXD_RESTRICT vy0 = vy;
        const block_q8_0 * GGML_RXD_RESTRICT vy1 = (const block_q8_0 *) ((const uint8_t*)vy + by);
        ...
        for (int i = 0; i < nb; i++) {
            const block_q8_0 * GGML_RXD_RESTRICT b_x0 = &vx0[i];
            const block_q8_0 * GGML_RXD_RESTRICT b_y0 = &vy0[i];

            const block_q8_0 * GGML_RXD_RESTRICT b_x1 = &vx1[i];
            const block_q8_0 * GGML_RXD_RESTRICT b_y1 = &vy1[i];
        ...
        case 128:
            {
                ...
                for (; ib + 1 < nb; ib += 2) {
                    const block_q8_0 * GGML_RXD_RESTRICT x0 = &x[ib + 0];
                    const block_q8_0 * GGML_RXD_RESTRICT x1 = &x[ib + 1];
                    const block_q8_0 * GGML_RXD_RESTRICT y0 = &y[ib + 0];
                    const block_q8_0 * GGML_RXD_RESTRICT y1 = &y[ib + 1];
                    ...
                    sumv0 = svmla_n_f32_x(pl16, sumv0, svcvt_f32_s32_x(pl16, svadd_x(pl16,
                                    ...
                                    svdot_s32(svdup_n_s32(0), qx0_1, qy0_1))), GGML_RXD_CPU_FP16_TO_FP32(x0->d)*GGML_RXD_CPU_FP16_TO_FP32(y0->d));
                    sumv1 = svmla_n_f32_x(pl16, sumv1, svcvt_f32_s32_x(pl16, svadd_x(pl16,
                                    svdot_s32(svdup_n_s32(0), qx1_0, qy1_0),
                                    svdot_s32(svdup_n_s32(0), qx1_1, qy1_1))), GGML_RXD_CPU_FP16_TO_FP32(x1->d)*GGML_RXD_CPU_FP16_TO_FP32(y1->d));
                }
        ...
        case 256:
            {
                ...
                for (; ib + 1 < nb; ib += 2) {
                    ...
                    sumv0 = svmla_n_f32_x(svptrue_b32(), sumv0, svcvt_f32_s32_x(svptrue_b32(),
                                svdot_s32(svdup_n_s32(0), qx0, qy0)), GGML_RXD_CPU_FP16_TO_FP32(x0->d)*GGML_RXD_CPU_FP16_TO_FP32(y0->d));
                    sumv1 = svmla_n_f32_x(svptrue_b32(), sumv1, svcvt_f32_s32_x(svptrue_b32(),
                                svdot_s32(svdup_n_s32(0), qx1, qy1)), GGML_RXD_CPU_FP16_TO_FP32(x1->d)*GGML_RXD_CPU_FP16_TO_FP32(y1->d));
                }
        ...
        case 512:
            {
                ...
                for (; ib + 1 < nb; ib += 2) {
                    ...
                    // scale creation
                    const float32_t deq1 = GGML_RXD_CPU_FP16_TO_FP32(x0->d)*GGML_RXD_CPU_FP16_TO_FP32(y0->d);
                    const float32_t deq2 = GGML_RXD_CPU_FP16_TO_FP32(x1->d)*GGML_RXD_CPU_FP16_TO_FP32(y1->d);
    ...
    for (; ib + 1 < nb; ib += 2) {
        ...
        sumv0 = vmlaq_n_f32(sumv0, vcvtq_f32_s32(vaddq_s32(
                        ggml_rxd_vdotq_s32(vdupq_n_s32(0), x0_0, y0_0),
                        ggml_rxd_vdotq_s32(vdupq_n_s32(0), x0_1, y0_1))), GGML_RXD_CPU_FP16_TO_FP32(x0->d)*GGML_RXD_CPU_FP16_TO_FP32(y0->d));

        sumv1 = vmlaq_n_f32(sumv1, vcvtq_f32_s32(vaddq_s32(
                        ggml_rxd_vdotq_s32(vdupq_n_s32(0), x1_0, y1_0),
                        ggml_rxd_vdotq_s32(vdupq_n_s32(0), x1_1, y1_1))), GGML_RXD_CPU_FP16_TO_FP32(x1->d)*GGML_RXD_CPU_FP16_TO_FP32(y1->d));
    }
...
void ggml_rxd_vec_dot_tq1_0_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    UNUSED(by);
    ...
    const block_tq1_0 * GGML_RXD_RESTRICT x = vx;
    const block_q8_K  * GGML_RXD_RESTRICT y = vy;

    const int nb = n / GGML_RXD_QK_K;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const float d = GGML_RXD_CPU_FP16_TO_FP32(x[i].d) * y[i].d;
    ...
    *s = sumf;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_tq1_0_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_tq2_0_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    UNUSED(by);
    ...
    const block_tq2_0 * GGML_RXD_RESTRICT x = vx;
    const block_q8_K  * GGML_RXD_RESTRICT y = vy;
    ...
    *s = sumf;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_tq2_0_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_q2_K_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    UNUSED(by);
    ...
    const block_q2_K * GGML_RXD_RESTRICT x = vx;
    const block_q8_K * GGML_RXD_RESTRICT y = vy;
    ...
    switch (vector_length) {
        case 128:
            for (int i = 0; i < nb; ++i) {
                const float d = y[i].d * GGML_RXD_CPU_FP16_TO_FP32(x[i].d);
                svfloat32_t d_broad = svdup_n_f32((float32_t)d);
                const float dmin = -y[i].d * GGML_RXD_CPU_FP16_TO_FP32(x[i].dmin);
                svfloat32_t dmin_broad = svdup_n_f32((float32_t)dmin);

                const uint8_t * GGML_RXD_RESTRICT q2 = x[i].qs;
                const int8_t  * GGML_RXD_RESTRICT q8_sv = y[i].qs;
                const uint8_t * GGML_RXD_RESTRICT sc = x[i].scales;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * GGML_RXD_RESTRICT q2 = x[i].qs;
        const int8_t  * GGML_RXD_RESTRICT q8 = y[i].qs;
        const uint8_t * GGML_RXD_RESTRICT sc = x[i].scales;
        ...
        const ggml_rxd_int16x8x2_t q8sums = ggml_rxd_vld1q_s16_x2(y[i].bsums);
        const ggml_rxd_int16x8x2_t mins16 = {{vreinterpretq_s16_u16(vmovl_u8(vget_low_u8(mins))), vreinterpretq_s16_u16(vmovl_u8(vget_high_u8(mins)))}};
...
#define MULTIPLY_ACCUM_WITH_SCALE(index)\
        isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q2bytes.val[0], q8bytes.val[0])) * aux[is+(index)];\
        isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q2bytes.val[1], q8bytes.val[1])) * aux[is+1+(index)];

#define SHIFT_MULTIPLY_ACCUM_WITH_SCALE(shift, index)\
        q8bytes = ggml_rxd_vld1q_s8_x2(q8); q8 += 32;\
        q2bytes.val[0] = vreinterpretq_s8_u8(vandq_u8(vshrq_n_u8(q2bits.val[0], (shift)), m3));\
        q2bytes.val[1] = vreinterpretq_s8_u8(vandq_u8(vshrq_n_u8(q2bits.val[1], (shift)), m3));\
        MULTIPLY_ACCUM_WITH_SCALE((index));

        for (int j = 0; j < GGML_RXD_QK_K/128; ++j) {
            const ggml_rxd_uint8x16x2_t q2bits = ggml_rxd_vld1q_u8_x2(q2); q2 += 32;

            ggml_rxd_int8x16x2_t q8bytes = ggml_rxd_vld1q_s8_x2(q8); q8 += 32;
    ...
    *s = sum;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_q2_K_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_q3_K_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    UNUSED(by);
    ...
    const block_q3_K * GGML_RXD_RESTRICT x = vx;
    const block_q8_K * GGML_RXD_RESTRICT y = vy;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * GGML_RXD_RESTRICT q3_sv = x[i].qs;
        const uint8_t * GGML_RXD_RESTRICT qh_sv = x[i].hmask;
        const int8_t  * GGML_RXD_RESTRICT q8_sv = y[i].qs;
    ...
    *s = sum;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * GGML_RXD_RESTRICT q3 = x[i].qs;
        const uint8_t * GGML_RXD_RESTRICT qh = x[i].hmask;
        const int8_t  * GGML_RXD_RESTRICT q8 = y[i].qs;

        ggml_rxd_uint8x16x2_t qhbits = ggml_rxd_vld1q_u8_x2(qh);

        ggml_rxd_uint8x16x4_t q3h;
        ...
        for (int j = 0; j < GGML_RXD_QK_K/128; ++j) {
            ...
            const ggml_rxd_uint8x16x2_t q3bits = ggml_rxd_vld1q_u8_x2(q3); q3 += 32;
            const ggml_rxd_int8x16x4_t q8bytes_1 = ggml_rxd_vld1q_s8_x4(q8); q8 += 64;
            const ggml_rxd_int8x16x4_t q8bytes_2 = ggml_rxd_vld1q_s8_x4(q8); q8 += 64;
            ...
            isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q3bytes.val[0], q8bytes_1.val[0])) * scale[0];
            isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q3bytes.val[1], q8bytes_1.val[1])) * scale[1];
            isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q3bytes.val[2], q8bytes_1.val[2])) * scale[2];
            isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q3bytes.val[3], q8bytes_1.val[3])) * scale[3];
            ...
            isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q3bytes.val[0], q8bytes_2.val[0])) * scale[0];
            isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q3bytes.val[1], q8bytes_2.val[1])) * scale[1];
            isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q3bytes.val[2], q8bytes_2.val[2])) * scale[2];
            isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q3bytes.val[3], q8bytes_2.val[3])) * scale[3];
    ...
    *s = sum;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_q3_K_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif

}

#ifdef __ARM_FEATURE_SVE
static inline svuint32_t ggml_rxd_decode_q4scales_and_mins_for_mmla(const uint32_t * vx_scales) {
...
void ggml_rxd_vec_dot_q4_K_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
    ...
    const block_q4_K * GGML_RXD_RESTRICT x = vx;
    const block_q8_K * GGML_RXD_RESTRICT y = vy;
    ...
    if (nrc == 2) {
        ...
        const block_q4_K * GGML_RXD_RESTRICT vx0 = vx;
        const block_q8_K * GGML_RXD_RESTRICT vy0 = vy;
        const block_q4_K * GGML_RXD_RESTRICT vx1 = (const block_q4_K *) ((const uint8_t*)vx + bx);
        const block_q8_K * GGML_RXD_RESTRICT vy1 = (const block_q8_K *) ((const uint8_t*)vy + by);
            ...
            case 128:
                {
                    ...
                    for (int i = 0; i < nb; ++i) {
                        ...
                        svfloat32_t vx_d = svzip1_f32(svdup_n_f32(GGML_RXD_FP16_TO_FP32(vx0[i].d)), svdup_n_f32(GGML_RXD_FP16_TO_FP32(vx1[i].d)));
                        svfloat32_t svsuper_block_scales = svmul_f32_x(pg128_all, vy_d, vx_d);
                        svfloat32_t vx_dmins = svzip1_f32(svdup_n_f32(GGML_RXD_FP16_TO_FP32(vx0[i].dmin)), svdup_n_f32(GGML_RXD_FP16_TO_FP32(vx1[i].dmin)));
                        ...
                        const uint8_t * GGML_RXD_RESTRICT q4_0 = vx0[i].qs;
                        const int8_t  * GGML_RXD_RESTRICT q8_0 = vy0[i].qs;
                        const uint8_t * GGML_RXD_RESTRICT q4_1 = vx1[i].qs;
                        const int8_t  * GGML_RXD_RESTRICT q8_1 = vy1[i].qs;
                        ...
                        svuint32_t decoded_scales0 = ggml_rxd_decode_q4scales_and_mins_for_mmla((const uint32_t *)vx0[i].scales);
                        svuint32_t decoded_scales1 = ggml_rxd_decode_q4scales_and_mins_for_mmla((const uint32_t *)vx1[i].scales);
...
#pragma GCC unroll 1
                        for (int j = 0; j < GGML_RXD_QK_K/64; ++j) {
...
#pragma GCC unroll 1
    ...
    if (nrc == 2) {
        const block_q4_K * GGML_RXD_RESTRICT x0 = x;
        const block_q4_K * GGML_RXD_RESTRICT x1 = (const block_q4_K *) ((const uint8_t *)vx + bx);
        const block_q8_K * GGML_RXD_RESTRICT y0 = y;
        const block_q8_K * GGML_RXD_RESTRICT y1 = (const block_q8_K *) ((const uint8_t *)vy + by);
        ...
        for (int i = 0; i < nb; ++i, ++x0, ++x1, ++y0, ++y1) {
            const uint8_t * GGML_RXD_RESTRICT qx0 = x0->qs;
            const uint8_t * GGML_RXD_RESTRICT qx1 = x1->qs;
            const  int8_t * GGML_RXD_RESTRICT qy0 = y0->qs;
            const  int8_t * GGML_RXD_RESTRICT qy1 = y1->qs;
            ...
            for (int j = 0; j < GGML_RXD_QK_K / 64; ++j, qx0 += 32, qx1 += 32, qy0 += 64, qy1 += 64) {
            ...
            // adjust bias, apply superblock scale
            {
                ...
                const float32x4_t dmins = {
                    GGML_RXD_CPU_FP16_TO_FP32(x0->dmin) * y0->d,
                    GGML_RXD_CPU_FP16_TO_FP32(x0->dmin) * y1->d,
                    GGML_RXD_CPU_FP16_TO_FP32(x1->dmin) * y0->d,
                    GGML_RXD_CPU_FP16_TO_FP32(x1->dmin) * y1->d,
                };
                vfsum = vmlsq_f32(vfsum, vcvtq_f32_s32(vld1q_s32(bias)), dmins);

                const float32x4_t superblock_scale = {
                    GGML_RXD_CPU_FP16_TO_FP32(x0->d) * y0->d,
                    GGML_RXD_CPU_FP16_TO_FP32(x0->d) * y1->d,
                    GGML_RXD_CPU_FP16_TO_FP32(x1->d) * y0->d,
                    GGML_RXD_CPU_FP16_TO_FP32(x1->d) * y1->d,
                };
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        memcpy(utmp, x[i].scales, GGML_RXD_K_SCALE_SIZE);
        ...
        const uint8_t * GGML_RXD_RESTRICT q4 = x[i].qs;
        const int8_t  * GGML_RXD_RESTRICT q8 = y[i].qs;
    ...
    *s = sumf;
    ...
    ggml_rxd_int8x16x2_t q4bytes;
    ggml_rxd_int8x16x2_t q8bytes;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        for (int j = 0; j < GGML_RXD_QK_K/64; ++j) {
            const ggml_rxd_uint8x16x2_t q4bits = ggml_rxd_vld1q_u8_x2(q4); q4 += 32;

            q8bytes = ggml_rxd_vld1q_s8_x2(q8); q8 += 32;
            q4bytes.val[0] = vreinterpretq_s8_u8(vandq_u8  (q4bits.val[0], m4b));
            q4bytes.val[1] = vreinterpretq_s8_u8(vandq_u8  (q4bits.val[1], m4b));

            const int32x4_t p1 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(mzero, q4bytes.val[0], q8bytes.val[0]), q4bytes.val[1], q8bytes.val[1]);
            ...
            const int32x4_t p2 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(mzero, q4bytes.val[0], q8bytes.val[0]), q4bytes.val[1], q8bytes.val[1]);
    ...
    *s = sumf;
    ...
    UNUSED(kmask3);
    UNUSED(utmp);
    ggml_rxd_vec_dot_q4_K_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_q5_K_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy,  size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    ...
    const block_q5_K * GGML_RXD_RESTRICT x = vx;
    const block_q8_K * GGML_RXD_RESTRICT y = vy;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * GGML_RXD_RESTRICT q5 = x[i].qs;
        const uint8_t * GGML_RXD_RESTRICT qh = x[i].qh;
        const int8_t  * GGML_RXD_RESTRICT q8 = y[i].qs;

        ggml_rxd_uint8x16x2_t qhbits = ggml_rxd_vld1q_u8_x2(qh);

        ggml_rxd_uint8x16x4_t q5h;

        int32_t sumi = 0;

        for (int j = 0; j < GGML_RXD_QK_K/64; ++j) {

            const ggml_rxd_uint8x16x2_t q5bits = ggml_rxd_vld1q_u8_x2(q5); q5 += 32;
            const ggml_rxd_int8x16x4_t q8bytes = ggml_rxd_vld1q_s8_x4(q8); q8 += 64;
            ...
            sumi += vaddvq_s32(ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(mzero, q5bytes.val[0], q8bytes.val[0]), q5bytes.val[1], q8bytes.val[1])) * *scales++;
            sumi += vaddvq_s32(ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(mzero, q5bytes.val[2], q8bytes.val[2]), q5bytes.val[3], q8bytes.val[3])) * *scales++;
        }
    ...
    *s = sumf;
    ...
    UNUSED(kmask3);
    UNUSED(utmp);
    ggml_rxd_vec_dot_q5_K_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_q6_K_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
#ifdef __ARM_FEATURE_MATMUL_INT8
    assert((nrc == 2) || (nrc == 1));
#else
    ...
    const block_q6_K * GGML_RXD_RESTRICT x = vx;
    const block_q8_K * GGML_RXD_RESTRICT y = vy;
    ...
    if (nrc == 2) {
        ...
        const block_q6_K * GGML_RXD_RESTRICT vx0 = vx;
        const block_q8_K * GGML_RXD_RESTRICT vy0 = vy;
        const block_q6_K * GGML_RXD_RESTRICT vx1 = (const block_q6_K *) ((const uint8_t*)vx + bx);
        const block_q8_K * GGML_RXD_RESTRICT vy1 = (const block_q8_K *) ((const uint8_t*)vy + by);
            ...
            case 128:
                {
                    ...
                    for (int i = 0; i < nb; ++i) {
                        const uint8_t * GGML_RXD_RESTRICT ql0 = vx0[i].ql;
                        const uint8_t * GGML_RXD_RESTRICT qh0 = vx0[i].qh;
                        const uint8_t * GGML_RXD_RESTRICT ql1 = vx1[i].ql;
                        const uint8_t * GGML_RXD_RESTRICT qh1 = vx1[i].qh;
                        const int8_t  * GGML_RXD_RESTRICT q80 = vy0[i].qs;
                        const int8_t  * GGML_RXD_RESTRICT q81 = vy1[i].qs;

                        const int8_t * GGML_RXD_RESTRICT scale0 = vx0[i].scales;
                        const int8_t * GGML_RXD_RESTRICT scale1 = vx1[i].scales;
...
#elif defined(__ARM_FEATURE_MATMUL_INT8)
    if (nrc == 2) {
        const block_q6_K * GGML_RXD_RESTRICT x0 = x;
        const block_q6_K * GGML_RXD_RESTRICT x1 = (const block_q6_K *) ((const uint8_t *)vx + bx);
        const block_q8_K * GGML_RXD_RESTRICT y0 = y;
        const block_q8_K * GGML_RXD_RESTRICT y1 = (const block_q8_K *) ((const uint8_t *)vy + by);
        ...
        for (int i = 0; i < nb; ++i, ++x0, ++x1, ++y0, ++y1) {
            const uint8_t * GGML_RXD_RESTRICT ql0 = x0->ql;
            const uint8_t * GGML_RXD_RESTRICT ql1 = x1->ql;
            const uint8_t * GGML_RXD_RESTRICT qh0 = x0->qh;
            const uint8_t * GGML_RXD_RESTRICT qh1 = x1->qh;
            const  int8_t * GGML_RXD_RESTRICT qy0 = y0->qs;
            const  int8_t * GGML_RXD_RESTRICT qy1 = y1->qs;
    ...
    for (int i = 0; i < nb; ++i) {
        const float d_all = GGML_RXD_CPU_FP16_TO_FP32(x[i].d);

        const uint8_t * GGML_RXD_RESTRICT q6 = x[i].ql;
        const uint8_t * GGML_RXD_RESTRICT qh = x[i].qh;
        const int8_t  * GGML_RXD_RESTRICT q8 = y[i].qs;

        const int8_t * GGML_RXD_RESTRICT scale = x[i].scales;
    ...
    *s = sum;
    ...
    ggml_rxd_int8x16x4_t q6bytes;
    ggml_rxd_uint8x16x4_t q6h;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const ggml_rxd_int16x8x2_t q6scales = {{vmovl_s8(vget_low_s8(scales)), vmovl_s8(vget_high_s8(scales))}};
        ...
        for (int j = 0; j < GGML_RXD_QK_K/128; ++j) {
            ...
            ggml_rxd_uint8x16x2_t qhbits = ggml_rxd_vld1q_u8_x2(qh); qh += 32;
            ggml_rxd_uint8x16x4_t q6bits = ggml_rxd_vld1q_u8_x4(q6); q6 += 64;
            ggml_rxd_int8x16x4_t q8bytes = ggml_rxd_vld1q_s8_x4(q8); q8 += 64;
            ...
            isum += vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q6bytes.val[0], q8bytes.val[0])) * scale[0] +
                    vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q6bytes.val[1], q8bytes.val[1])) * scale[1] +
                    vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q6bytes.val[2], q8bytes.val[2])) * scale[2] +
                    vaddvq_s32(ggml_rxd_vdotq_s32(vzero, q6bytes.val[3], q8bytes.val[3])) * scale[3];

            scale += 4;

            q8bytes = ggml_rxd_vld1q_s8_x4(q8); q8 += 64;
    ...
    *s = sum;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_q6_K_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}
...
void ggml_rxd_vec_dot_iq2_xxs_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    ...
    const block_iq2_xxs * GGML_RXD_RESTRICT x = vx;
    const block_q8_K    * GGML_RXD_RESTRICT y = vy;
    ...
    ggml_rxd_int8x16x4_t q2u;
    ggml_rxd_int8x16x4_t q2s;
    ggml_rxd_int8x16x4_t q8b;

    float sumf = 0;
    for (int i = 0; i < nb; ++i) {
        const float d = GGML_RXD_CPU_FP16_TO_FP32(x[i].d) * y[i].d;
        const uint16_t * GGML_RXD_RESTRICT q2 = x[i].qs;
        const int8_t   * GGML_RXD_RESTRICT q8 = y[i].qs;
        float sumf1 = 0, sumf2 = 0;
        for (int ib32 = 0; ib32 < GGML_RXD_QK_K/32; ib32 += 2) {
            q8b = ggml_rxd_vld1q_s8_x4(q8); q8 += 64;
            ...
            const int32x4_t p1 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), q2u.val[0], q8b.val[0]), q2u.val[1], q8b.val[1]);
            const int32x4_t p2 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), q2u.val[2], q8b.val[2]), q2u.val[3], q8b.val[3]);
    ...
    *s = 0.25f * sumf;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_iq2_xxs_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_iq2_xs_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    ...
    const block_iq2_xs * GGML_RXD_RESTRICT x = vx;
    const block_q8_K   * GGML_RXD_RESTRICT y = vy;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        for (int ib64 = 0; ib64 < GGML_RXD_QK_K/64; ++ib64) {
            q8b = ggml_rxd_vld1q_s8_x4(q8); q8 += 64;
            ...
            q2u.val[2] = vmulq_s8(q2u.val[2], q2s.val[2]);
            q2u.val[3] = vmulq_s8(q2u.val[3], q2s.val[3]);
            const int32x4_t p1 = ggml_rxd_vdotq_s32(vdupq_n_s32(0), q2u.val[0], q8b.val[0]);
            const int32x4_t p2 = ggml_rxd_vdotq_s32(vdupq_n_s32(0), q2u.val[1], q8b.val[1]);
            const int32x4_t p3 = ggml_rxd_vdotq_s32(vdupq_n_s32(0), q2u.val[2], q8b.val[2]);
            const int32x4_t p4 = ggml_rxd_vdotq_s32(vdupq_n_s32(0), q2u.val[3], q8b.val[3]);
    ...
    *s = 0.125f * sumf;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_iq2_xs_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_iq2_s_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    ...
    const block_iq2_s * GGML_RXD_RESTRICT x = vx;
    const block_q8_K  * GGML_RXD_RESTRICT y = vy;
   ...
   static const uint8_t k_mask1[32] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    ...
    const ggml_rxd_uint8x16x2_t mask1 = ggml_rxd_vld1q_u8_x2(k_mask1);
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * GGML_RXD_RESTRICT qs = x[i].qs;
        const uint8_t * GGML_RXD_RESTRICT qh = x[i].qh;
        const uint16_t * GGML_RXD_RESTRICT signs = (const uint16_t *)(x[i].qs + GGML_RXD_QK_K/8);
        const int8_t  * GGML_RXD_RESTRICT q8 = y[i].qs;
        ...
        for (int ib32 = 0; ib32 < GGML_RXD_QK_K/32; ib32 += 2) {
            ...
            vs.val[1] = vandq_u8(ggml_rxd_vqtbl1q_u8(vs.val[0], mask1.val[1]), mask2);
            vs.val[0] = vandq_u8(ggml_rxd_vqtbl1q_u8(vs.val[0], mask1.val[0]), mask2);
            ...
            const int32x4_t p1 = ggml_rxd_vdotq_s32(vzero, q2s.val[0], q8b.val[0]);
            const int32x4_t p2 = ggml_rxd_vdotq_s32(vzero, q2s.val[1], q8b.val[1]);
            const int32x4_t p3 = ggml_rxd_vdotq_s32(vzero, q2s.val[2], q8b.val[2]);
            const int32x4_t p4 = ggml_rxd_vdotq_s32(vzero, q2s.val[3], q8b.val[3]);
    ...
    *s = 0.125f * sumf;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_iq2_s_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif

}

void ggml_rxd_vec_dot_iq3_xxs_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    ...
    const block_iq3_xxs * GGML_RXD_RESTRICT x = vx;
    const block_q8_K    * GGML_RXD_RESTRICT y = vy;
    ...
    for (int i = 0; i < nb; ++i) {
        const float d = GGML_RXD_CPU_FP16_TO_FP32(x[i].d) * y[i].d;
        const uint8_t * GGML_RXD_RESTRICT q3 = x[i].qs;
        const uint8_t * GGML_RXD_RESTRICT gas = x[i].qs + GGML_RXD_QK_K/4;
        const int8_t   * GGML_RXD_RESTRICT q8 = y[i].qs;
        float sumf1 = 0, sumf2 = 0;
        for (int ib32 = 0; ib32 < GGML_RXD_QK_K/32; ib32 += 2) {
            q8b = ggml_rxd_vld1q_s8_x4(q8); q8 += 64;
            memcpy(aux32, gas, 2*sizeof(uint32_t)); gas += 2*sizeof(uint32_t);
            const uint32x4_t aux32x4_0 = ggml_rxd_vld1q_u32(iq3xxs_grid[q3[ 0]], iq3xxs_grid[q3[ 1]], iq3xxs_grid[q3[ 2]], iq3xxs_grid[q3[ 3]]);
            const uint32x4_t aux32x4_1 = ggml_rxd_vld1q_u32(iq3xxs_grid[q3[ 4]], iq3xxs_grid[q3[ 5]], iq3xxs_grid[q3[ 6]], iq3xxs_grid[q3[ 7]]);
            const uint32x4_t aux32x4_2 = ggml_rxd_vld1q_u32(iq3xxs_grid[q3[ 8]], iq3xxs_grid[q3[ 9]], iq3xxs_grid[q3[10]], iq3xxs_grid[q3[11]]);
            const uint32x4_t aux32x4_3 = ggml_rxd_vld1q_u32(iq3xxs_grid[q3[12]], iq3xxs_grid[q3[13]], iq3xxs_grid[q3[14]], iq3xxs_grid[q3[15]]);
            q3 += 16;
            ...
            const int32x4_t p1 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), q3s.val[0], q8b.val[0]), q3s.val[1], q8b.val[1]);
            const int32x4_t p2 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), q3s.val[2], q8b.val[2]), q3s.val[3], q8b.val[3]);
    ...
    *s = 0.5f * sumf;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_iq3_xxs_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_iq3_s_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    ...
    const block_iq3_s * GGML_RXD_RESTRICT x = vx;
    const block_q8_K  * GGML_RXD_RESTRICT y = vy;
   ...
   static const uint8_t k_mask1[32] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * GGML_RXD_RESTRICT qs = x[i].qs;
        const uint8_t * GGML_RXD_RESTRICT qh = x[i].qh;
        const uint16_t * GGML_RXD_RESTRICT signs = (const uint16_t *)x[i].signs;
        const int8_t   * GGML_RXD_RESTRICT q8 = y[i].qs;
        ...
        for (int ib32 = 0; ib32 < GGML_RXD_QK_K/32; ib32 += 2) {
            ...
            const uint32x4_t aux32x4_0 = ggml_rxd_vld1q_u32(iq3s_grid[idx.index[0]], iq3s_grid[idx.index[1]],
                                                        iq3s_grid[idx.index[2]], iq3s_grid[idx.index[3]]);
            const uint32x4_t aux32x4_1 = ggml_rxd_vld1q_u32(iq3s_grid[idx.index[4]], iq3s_grid[idx.index[5]],
                                                        iq3s_grid[idx.index[6]], iq3s_grid[idx.index[7]]);
            idx.vec_index = vorrq_u16(vmovl_u8(vget_high_u8(idx_l)), vandq_u16(vshlq_u16(vdupq_n_u16(qh[ib32+1]), hshift), m256));
            const uint32x4_t aux32x4_2 = ggml_rxd_vld1q_u32(iq3s_grid[idx.index[0]], iq3s_grid[idx.index[1]],
                                                        iq3s_grid[idx.index[2]], iq3s_grid[idx.index[3]]);
            const uint32x4_t aux32x4_3 = ggml_rxd_vld1q_u32(iq3s_grid[idx.index[4]], iq3s_grid[idx.index[5]],
                                                        iq3s_grid[idx.index[6]], iq3s_grid[idx.index[7]]);
    ...
    *s = sumf;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_iq3_s_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_iq1_s_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    ...
    const block_iq1_s * GGML_RXD_RESTRICT x = vx;
    const block_q8_K  * GGML_RXD_RESTRICT y = vy;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        for (int ib = 0; ib < GGML_RXD_QK_K/32; ib += 2) {
            ...
            const int32x4_t p1 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), q1b.val[0], q8b.val[0]), q1b.val[1], q8b.val[1]);
            const int32x4_t p2 = ggml_rxd_vdotq_s32(ggml_rxd_vdotq_s32(vdupq_n_s32(0), q1b.val[2], q8b.val[2]), q1b.val[3], q8b.val[3]);
        ...
        sumf += y[i].d * GGML_RXD_CPU_FP16_TO_FP32(x[i].d) * (sumi1 + sumi2 + IQ1S_DELTA * sumi3);
    }
    ...
    *s = sumf;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_iq1_s_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_iq1_m_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(n % GGML_RXD_QK_K == 0);
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    ...
    const block_iq1_m * GGML_RXD_RESTRICT x = vx;
    const block_q8_K  * GGML_RXD_RESTRICT y = vy;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        for (int ib = 0; ib < GGML_RXD_QK_K/32; ib += 2) {
            ...
            const int32x4_t p1 = vpaddq_s32(ggml_rxd_vdotq_s32(mzero, q1b.val[0], q8b.val[0]), ggml_rxd_vdotq_s32(mzero, q1b.val[1], q8b.val[1]));
            const int32x4_t p2 = vpaddq_s32(ggml_rxd_vdotq_s32(mzero, q1b.val[2], q8b.val[2]), ggml_rxd_vdotq_s32(mzero, q1b.val[3], q8b.val[3]));
            ...
            const int32x4_t p3 = vpaddq_s32(ggml_rxd_vdotq_s32(mzero, deltas.val[aux8[0]], q8b.val[0]), ggml_rxd_vdotq_s32(mzero, deltas.val[aux8[1]], q8b.val[1]));
            const int32x4_t p4 = vpaddq_s32(ggml_rxd_vdotq_s32(mzero, deltas.val[aux8[2]], q8b.val[2]), ggml_rxd_vdotq_s32(mzero, deltas.val[aux8[3]], q8b.val[3]));
            const int32x4_t p34 = vpaddq_s32(p3, p4);

            int32x4_t scales_4 = ggml_rxd_vld1q_u32(sc[ib/2] >> 0, sc[ib/2] >> 3, sc[ib/2] >> 6, sc[ib/2] >> 9);
        ...
        sumf += y[i].d * GGML_RXD_CPU_FP16_TO_FP32(scale.f16) * (vaddvq_s32(sumi1) + IQ1M_DELTA * vaddvq_s32(sumi2));
    }
    ...
    *s = sumf;
    ...
    UNUSED(nb);
    UNUSED(scale);
    ggml_rxd_vec_dot_iq1_m_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}

void ggml_rxd_vec_dot_iq4_nl_q8_0(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    UNUSED(by);
    UNUSED(bs);
    assert(n % GGML_RXD_QK4_NL == 0);
    static_assert(GGML_RXD_QK4_NL == GGML_RXD_QK8_0, "GGML_RXD_QK4_NL and GGML_RXD_QK8_0 must be the same");

    const block_iq4_nl * GGML_RXD_RESTRICT x = vx;
    const block_q8_0   * GGML_RXD_RESTRICT y = vy;

    const int nb = n / GGML_RXD_QK4_NL;
    ...
    for (; ib + 1 < nb; ib += 2) {
        ...
        sumf +=
            GGML_RXD_CPU_FP16_TO_FP32(x[ib+0].d) * GGML_RXD_CPU_FP16_TO_FP32(y[ib + 0].d) * vaddvq_s32(prod_1) +
            GGML_RXD_CPU_FP16_TO_FP32(x[ib+1].d) * GGML_RXD_CPU_FP16_TO_FP32(y[ib + 1].d) * vaddvq_s32(prod_2);
    }

#endif
    for (; ib < nb; ++ib) {
        const float d = GGML_RXD_CPU_FP16_TO_FP32(y[ib].d)*GGML_RXD_CPU_FP16_TO_FP32(x[ib].d);
        int sumi1 = 0, sumi2 = 0;
        for (int j = 0; j < GGML_RXD_QK4_NL/2; ++j) {
            sumi1 += y[ib].qs[j+       0] * kvalues_iq4nl[x[ib].qs[j] & 0xf];
            sumi2 += y[ib].qs[j+GGML_RXD_QK4_NL/2] * kvalues_iq4nl[x[ib].qs[j] >>  4];
        }
        sumf += d * (sumi1 + sumi2);
    }
    *s = sumf;
...
void ggml_rxd_vec_dot_iq4_xs_q8_K(int n, float * GGML_RXD_RESTRICT s, size_t bs, const void * GGML_RXD_RESTRICT vx, size_t bx, const void * GGML_RXD_RESTRICT vy, size_t by, int nrc) {
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    UNUSED(by);
    ...
    const block_iq4_xs * GGML_RXD_RESTRICT x = vx;
    const block_q8_K   * GGML_RXD_RESTRICT y = vy;
    ...
    const int8x16_t values = vld1q_s8(kvalues_iq4nl);
    const uint8x16_t m4b = vdupq_n_u8(0x0f);
    ggml_rxd_uint8x16x2_t q4bits;
    ggml_rxd_int8x16x4_t q4b;
    ggml_rxd_int8x16x4_t q8b;
    int32x4_t prod_1, prod_2;
    ...
    for (int ibl = 0; ibl < nb; ++ibl) {
        ...
        int sumi1 = 0, sumi2 = 0;
        for (int ib = 0; ib < GGML_RXD_QK_K/64; ++ib) {

            q4bits = ggml_rxd_vld1q_u8_x2(q4); q4 += 32;
            q8b    = ggml_rxd_vld1q_s8_x4(q8); q8 += 64;
        ...
        sumf += GGML_RXD_CPU_FP16_TO_FP32(x[ibl].d) * y[ibl].d * (sumi1 + sumi2);
    }
    ...
    *s = sumf;
    ...
    UNUSED(y);
    UNUSED(nb);
    ggml_rxd_vec_dot_iq4_xs_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}
