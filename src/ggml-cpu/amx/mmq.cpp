...
template <typename Fn> ALWAYS_INLINE auto ggml_select_bool_constexpr(bool value, Fn&& fn)
{
    if (value)
    {
...
template <typename Fn> ALWAYS_INLINE auto ggml_select_floating_type(enum ggml_type type, Fn&& fn)
{
    switch (type)
    {
        case GGML_TYPE_F16:
            return fn(TypeTag<ggml_fp16_t>{}, std::integral_constant<int, 16>{});
        case GGML_TYPE_BF16:
            return fn(TypeTag<ggml_bf16_t>{}, std::integral_constant<int, 32>{});
        default:
            GGML_ABORT("Unsupported floating data type");
            return fn(TypeTag<ggml_fp16_t>{}, std::integral_constant<int, 16>{});
    }
}

template <typename Fn> ALWAYS_INLINE auto ggml_select_qtype(enum ggml_type qtype, Fn&& fn)
{
    switch (qtype)
    {
        case GGML_TYPE_Q4_0:
            return fn(TypeTag<block_q4_0>{}, TypeTag<block_q8_0>{}, std::integral_constant<int, QK4_0>{});
        case GGML_TYPE_Q4_1:
            return fn(TypeTag<block_q4_1>{}, TypeTag<block_q8_1>{}, std::integral_constant<int, QK4_1>{});
        case GGML_TYPE_Q8_0:
            return fn(TypeTag<block_q8_0>{}, TypeTag<block_q8_0>{}, std::integral_constant<int, QK8_0>{});
        case GGML_TYPE_Q4_K:
            return fn(TypeTag<block_q4_K>{}, TypeTag<block_q8_K>{}, std::integral_constant<int, QK_K>{});
        case GGML_TYPE_Q5_K:
            return fn(TypeTag<block_q5_K>{}, TypeTag<block_q8_K>{}, std::integral_constant<int, QK_K>{});
        case GGML_TYPE_Q6_K:
            return fn(TypeTag<block_q6_K>{}, TypeTag<block_q8_K>{}, std::integral_constant<int, QK_K>{});
        case GGML_TYPE_IQ4_XS:
            return fn(TypeTag<block_iq4_xs>{}, TypeTag<block_q8_K>{}, std::integral_constant<int, QK_K>{});
        default:
            GGML_ABORT("Unsupported quantized data type");
            return fn(TypeTag<block_q4_0>{}, TypeTag<block_q8_0>{}, std::integral_constant<int, QK4_0>{});
    }
}
...
struct tinygemm_kernel_avx<float, ggml_fp16_t, float, BLOCK_M, BLOCK_N, BLOCK_K>
{
    static void apply(int K, const float* RESTRICT A, const ggml_fp16_t* RESTRICT B, float* RESTRICT C, int ldc)
    {
        ...
        auto loadc = [&](auto idx) { vc[idx] = _mm512_setzero_ps(); };
...
struct tinygemm_kernel_vnni<block_q8_0, block_q4_0, float, BLOCK_M, BLOCK_N, BLOCK_K>
{
    static void apply(int KB, const void* RESTRICT _A, const void* RESTRICT _B, float* RESTRICT C, int ldc)
    {
        ...
        auto loadc = [&](auto col) { vc[col] = _mm512_setzero_ps(); };
        Unroll<COLS>{}(loadc);
        ...
        // store to C
        auto storec = [&](auto col) { _mm512_storeu_ps((__m512i*)(C + 0 * ldc + col * 16), vc[col]); };
        Unroll<COLS>{}(storec);
    }
};
...
#define LAUNCH_TINYGEMM_KERNEL_VNNI(NB_SIZE)                                                                           \
    tinygemm_kernel_vnni<vec_dot_type, type, float, 1, NB_SIZE, blck_size>::apply(                                     \
        ...
        (const char*)src0->data + PACKED_INDEX(nb * kTilesN, 0, KB, TILE_SIZE), (float*)dst->data + 0 * N + nb_start,  \
        ldc)
                         ...
                         int ldc)
{
    ...
    if (M == 2 * TILE_M)
    {
        ...
        for (int i = 1; i < KB; ++i)
        {
            ...
            ggml_select_bool_constexpr(
                ii > 0,
                [&](auto is_acc_tag)
                {
                    constexpr bool is_acc = decltype(is_acc_tag)::value;
                    if (need_unpack)
                    {
                        unpack_B<TB>(Tile0, B_blk0);
                        _tile_loadd(TMM0, Tile0, TILE_N * VNNI_BLK);
                    ...
                    acc_C<TA, TB, is_acc>::apply(C, ldc, Tile4(C_pre), &A[ii], KB,
    ...
    else
    {
        for (int i = 0; i < KB; ++i)
        {
            ...
            ggml_select_bool_constexpr(i > 0,
                                       [&](auto is_acc_tag)
                                       {
                                           constexpr bool is_acc = decltype(is_acc_tag)::value;
                                           acc_C<TA, TB, is_acc>::apply(C, ldc, Tile4(C_cur), &A[i], KB,
                                                                        B + PACKED_INDEX(0, i, KB, TILE_SIZE), m0);
            ...
            if (m1 != 0)
            {
                ...
                ggml_select_bool_constexpr(
                    ...
                    [&](auto is_acc_tag)
                    {
                        ...
                        acc_C<TA, TB, is_acc>::apply(C + TILE_M * ldc + TILE_N, ldc, Tile7(C_cur), &A[TILE_M * KB + i],
                                                     KB, B + PACKED_INDEX(1, i, KB, TILE_SIZE), m1);
                    });
            }
        }
    }
    ...
    for (int i = 0; i < KB; ++i)
    {
        ...
        ggml_select_bool_constexpr(
            ...
            [&](auto is_acc_tag)
            {
                ...
                acc_C<TA, TB, is_acc>::apply(C + TILE_N, ldc, Sumi6, &A[i], KB, B + PACKED_INDEX(1, i, KB, TILE_SIZE),
                                             m0);
                if (m1 != 0)
                {
...
size_t ggml_backend_amx_get_alloc_size(const struct ggml_tensor* tensor)
{
    ...
    {
        size_t row_size_B{0};
        ggml_select_qtype(TYPE,
                          [&](auto type_tag, auto vec_dot_type_tag, auto blck_size_tag)
                          {
                              using type = typename decltype(type_tag)::type;
                              constexpr int blck_size = decltype(blck_size_tag)::value;
                              (void)vec_dot_type_tag;
...
void ggml_backend_amx_convert_weight(struct ggml_tensor* tensor, const void* data, size_t offset, size_t size)
{
    ...
    ggml_select_qtype(TYPE,
                      [&](auto type_tag, auto vec_dot_type_tag, auto blck_size_tag)
                      {
                          ...
                          constexpr int blck_size = decltype(blck_size_tag)::value;
                          (void)vec_dot_type_tag;
                          convert_B_packed_format<type, blck_size>((void*)((char*)tensor->data + offset),
                                                                   (const type*)data, N, K);
                      });
}
...
size_t ggml_backend_amx_desired_wsize(const struct ggml_tensor* dst)
{
    ...
    ggml_select_qtype(TYPE,
                      [&](auto type_tag, auto vec_dot_type_tag, auto blck_size_tag)
                      {
                          using vec_dot_type = typename decltype(vec_dot_type_tag)::type;
                          constexpr int blck_size = decltype(blck_size_tag)::value;
                          (void)type_tag;
...
void ggml_backend_amx_mul_mat(const ggml_compute_params* params, struct ggml_tensor* dst)
{
    ...
    if (is_floating_type)
    {
        ...
        parallel_for_ggml(params, MB * NB,
                          [&](int begin, int end)
                          {
                              ggml_select_floating_type(TYPE,
                                                        [&](auto type_tag, auto blck_size_tag)
                                                        {
                                                            using type = typename decltype(type_tag)::type;
                                                            constexpr int blck_size = decltype(blck_size_tag)::value;
