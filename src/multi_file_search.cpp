...
void MultiFileSearchWidget::startSearch()
{
    ...
    if (m_projectRoot.empty() || m_searchQuery.empty())
    {
        ...
        if (m_completedCb)
            m_completedCb(m_completedCtx, 0);
        return;
    }
...
void MultiFileSearchWidget::onSearchFinished()
{
    m_isSearching = false;
    if (m_completedCb)
        m_completedCb(m_completedCtx, m_totalResultCount);
}

static bool globMatchExtList(const std::string& name, const std::string& patternCsv)
{
    if (patternCsv.empty() || patternCsv == "*")
        return true;

    size_t p = 0;
    while (p < patternCsv.size())
    {
        while (p < patternCsv.size() && (patternCsv[p] == ' ' || patternCsv[p] == '\t' || patternCsv[p] == ','))
            p++;
        if (p >= patternCsv.size())
            break;
        ...
        while (!pat.empty() && (pat.back() == ' ' || pat.back() == '\t'))
            pat.pop_back();
        while (!pat.empty() && (pat.front() == ' ' || pat.front() == '\t'))
            pat.erase(pat.begin());
...
void MultiFileSearchWidget::performSearch(const std::string& searchText, const std::string& rootPath, bool useRegex,
                                          bool caseSensitive, const std::string& fileFilter)
{
    ...
    auto findMatch = [&](const std::string& line, size_t startPos) -> std::pair<size_t, size_t>
    {
        ...
        if (caseSensitive)
        {
            ...
            return (p == std::string::npos) ? std::pair<size_t, size_t>{std::string::npos, (size_t)0}
    ...
    try
    {
             ...
             fs::recursive_directory_iterator(rootPath, fs::directory_options::skip_permission_denied, ec))
        {
            ...
            if (isIgnored(path, gitignorePatterns))
                continue;
            if (!globMatchExtList(name, fileFilter.empty() ? "*" : fileFilter))
                continue;

            filesSearched++;
            if (m_progressCb)
                m_progressCb(m_progressCtx, filesSearched, m_totalResultCount);
    ...
    updateStatus("Found " + std::to_string(m_totalResultCount) + " matches in " + std::to_string(filesSearched) +
                 " files.");
    onSearchFinished();
}
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_CREATE:
        {
            ...
            st->edtQuery = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 8, 8,
                                           520, 24, hwnd, (HMENU)1001, GetModuleHandleW(nullptr), nullptr);
            SendMessageW(st->edtQuery, WM_SETFONT, (WPARAM)font, TRUE);

            st->edtFilter =
                CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"*.cpp,*.h,*.hpp,*.c,*.py,*.js,*.ts,*.json,*.md",
                                WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 8, 38, 520, 24, hwnd, (HMENU)1002,
                                GetModuleHandleW(nullptr), nullptr);
            SendMessageW(st->edtFilter, WM_SETFONT, (WPARAM)font, TRUE);

            st->chkRegex = CreateWindowW(L"BUTTON", L"Regex", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 540, 8, 120, 20,
                                         hwnd, (HMENU)1003, GetModuleHandleW(nullptr), nullptr);
            SendMessageW(st->chkRegex, WM_SETFONT, (WPARAM)font, TRUE);

            st->chkCase = CreateWindowW(L"BUTTON", L"Case", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 540, 28, 120, 20,
                                        hwnd, (HMENU)1004, GetModuleHandleW(nullptr), nullptr);
            SendMessageW(st->chkCase, WM_SETFONT, (WPARAM)font, TRUE);

            st->btnSearch = CreateWindowW(L"BUTTON", L"Search", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 540, 52, 120, 26,
            ...
            st->lst = CreateWindowExW(WS_EX_CLIENTEDGE, L"LISTBOX", L"",
                                      WS_CHILD | WS_VISIBLE | LBS_NOINTEGRALHEIGHT | WS_VSCROLL | WS_HSCROLL, 8, 70,
                                      652, 320, hwnd, (HMENU)1006, GetModuleHandleW(nullptr), nullptr);
            SendMessageW(st->lst, WM_SETFONT, (WPARAM)font, TRUE);

            st->lbl = CreateWindowW(L"STATIC", L"Matches: 0", WS_CHILD | WS_VISIBLE, 8, 398, 300, 20, hwnd, (HMENU)1007,
                                    GetModuleHandleW(nullptr), nullptr);
...
void MultiFileSearchWidget_ShowDialog(void* ctx)
{
    ...
    HWND hwnd = CreateWindowExW(0, kWndClass, L"Multi-File Search", WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT,
                                CW_USEDEFAULT, 690, 470, nullptr, nullptr, GetModuleHandleW(nullptr), &st);
