#!/usr/bin/env bash
# RawrXD IDE — Linux launcher (bootable space). Requires Wine and a Windows build.
set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
[ -f "$SCRIPT_DIR/rawrxd-space.env" ] && set -a && source "$SCRIPT_DIR/rawrxd-space.env" && set +a
RAWRXD_SPACE_ROOT="${RAWRXD_SPACE_ROOT:-$HOME/.rawrxd-space}"
export WINEPREFIX="${WINEPREFIX:-$RAWRXD_SPACE_ROOT}"
WINE="${WINE:-wine}"
for c in wine wine64; do command -v $c >/dev/null 2>&1 && WINE=$c && break; done
...
if [ -z "$IDE_EXE" ] || [ ! -f "$IDE_EXE" ]; then
  echo "Set RAWRXD_IDE_EXE or copy RawrXD-Win32IDE.exe into the space. See README.md." >&2
  exit 1
fi
