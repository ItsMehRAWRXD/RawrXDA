...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('bigdaddyg-copilot.openChat', () => {
    ...
    }), vscode.commands.registerCommand('bigdaddyg-copilot.insertCode', async (code) => {
    ...
    }), vscode.commands.registerCommand('bigdaddyg-copilot.applyDiff', async (fileUri, newContent) => {
        ...
        const agentMode = await vscode.window.showQuickPick(modes, { placeHolder: 'Select mode' });
        if (!agentMode)
            return;
        const ideAccessStr = await vscode.window.showQuickPick(['Yes', 'No'], { placeHolder: 'IDE Access?' });
        const ideAccess = ideAccessStr === 'Yes';
        interceptorConfig = { enabled: true, endpoint, backend: backend, model, agentMode, ideAccess };
        rebuildInterceptor();
...
function rebuildInterceptor() {
    if (!interceptorConfig.enabled) {
        http.request = httpOriginal;
        https.request = httpsOriginal;
        return;
    }
...
function detectChatRequest(url, options) {
...
function proxyToLocal(endpoint, backend, model, url, options, callback) {
...
async function handleEditDirective(text) {
...
function urlForBackend(backend) {
...
function transformRequestBody(body, backend, model) {
...
function buildAgentPreface(mode) {
    if (mode === 'edit') {
...
function addContextToPrompt(prompt) {
    if (!interceptorConfig.ideAccess)
        return prompt;
    return prompt + `\n\n[Context]\nFiles: ${contextCache.files}\nCode: ${contextCache.code}`;
}
function addContextToMessages(messages) {
    if (!interceptorConfig.ideAccess || messages.length === 0)
        return messages;
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
        if (msg.type === 'send') {
            ...
            try {
                ...
                const backend = msg.backend;
                if (!endpoint || !backend)
                    throw new Error('Endpoint and backend required');
...
async function streamQuerySingle(endpoint, backend, model, prompt, onChunk) {
    if (backend === 'ollama-generate') {
...
function parseOllamaGenerateLine(line) {
    if (!line)
        return;
...
function streamEndpoint(base, pathname, payload, onChunk, parseLine) {
...
async function loadModels(base, backend) {
    if (backend === 'ollama-generate' || backend === 'ollama-chat') {
        const tags = await httpJsonBase(base, 'GET', '/api/tags');
        if (tags.status !== 200)
            return [];
        const models = (tags.json?.models || []).map((m) => m.name).filter(Boolean);
        return models;
    }
    if (backend === 'openai-chat') {
        ...
        if (m.status !== 200)
            return [];
        return (m.json?.data || []).map((x) => x.id).filter(Boolean);
    }
    return [];
}
async function tryApplyAgentEdit(text) {
    const m = /AGENT_EDIT:\s*(\{[\s\S]*?\})/i.exec(text);
    if (!m)
        return false;
    const payload = JSON.parse(m[1]);
...
function httpJsonBase(base, method, path, payload) {
    return new Promise((resolve, reject) => {
