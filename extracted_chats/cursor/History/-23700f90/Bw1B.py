...
try:
    ...
    Lang, name, show_help_flag, list_flag, preview_flag = "py", "example", False, False, False
...
if __name__ == "__main__":
    try:
        ...
        # Validate language support (skip validation for role-only)
        if Lang != "role" and not validate_language(Lang):
        ...
        if Lang == "role":
            ...
            if input("Add role template (y/n)? ").lower().startswith("y"):
                with open(f"{name}.role", "w") as fh:
                    fh.write("\n".join(lang_role()))
                print(f"✅ Role template written to {name}.role")
