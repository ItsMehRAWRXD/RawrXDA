...
function startOrchestraServer(options = {}) {
  ...
  if (orchestraServer && !orchestraServer.killed) {
    console.log([BigDaddyG] ⚠️ Orchestra server already running (requested by ));
  ...
  logToRenderer('info', 🎼 Starting Orchestra server ()...);
  ...
  try {
    ...
    const registerLifecycleHandlers = (target) => {
      ...
      const errorHandler = (error) => {
        ...
        logToRenderer('error', ❌ Orchestra server error: );
    ...
    logToRenderer('error', ❌ Failed to load Orchestra: );
    notifyOrchestraStatus({ running: false, error: error.message, source: invocationSource });
