            ...
            if ($data.messages) {
                foreach ($msg in $data.messages) {
                    $conversation.messages += @{
                        role = if ($msg.role) { $msg.role } else { "user" }
                        content = if ($msg.content) { $msg.content } elseif ($msg.text) { $msg.text } elseif ($msg.message) { $msg.message } else { "" }
                        timestamp = if ($msg.timestamp) { $msg.timestamp } elseif ($msg.created_at) { $msg.created_at } else { (Get-Date).ToString("yyyy-MM-ddTHH:mm:ss.fffZ") }
                    }
                }
