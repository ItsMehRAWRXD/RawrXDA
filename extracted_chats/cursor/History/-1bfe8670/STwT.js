...
async function testIDEHTML() {
    ...
    try {
        ...
        const features = [
            ...
            { name: 'Omega Compiler', pattern: 'OmegaCompilerUI|omegaCompiler|Omega.*Universal.*Compiler|compile-handcrafted' },
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                try {
                    ...
                    if (langsResponse.status === 200 && langsResponse.data && langsResponse.data.languages) {
                        logPass(`(${langsResponse.data.languages.length} languages)`);
                        recordTest('Languages endpoint', true, `${langsResponse.data.languages.length} languages available`);
                    } else {
                        logFail('Invalid response format');
                        recordTest('Languages endpoint', false, 'Invalid response format');
                    }
                } catch (error) {
                    logFail(error.message);
                ...
                // Test Omega compile endpoint availability (just check it exists, don't actually compile)
                logTest('Testing /api/omega/compile endpoint');
                try {
                    // Send a test request with invalid data to check if endpoint exists
                    const compileResponse = await httpRequest({
                        hostname: 'localhost',
                        port: 9000,
                        path: '/api/omega/compile',
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' }
                    }, JSON.stringify({ inputFile: 'test.invalid' }));
                    
                    // If we get any response (even 400), endpoint exists
                    if (compileResponse.status === 400 || compileResponse.status === 500) {
                        logPass('Endpoint exists');
                        recordTest('Compile endpoint', true);
                    } else if (compileResponse.status === 200) {
                        logPass('Endpoint working');
                        recordTest('Compile endpoint', true);
                    } else {
                        logWarn(`Unexpected status: ${compileResponse.status}`);
                        recordTest('Compile endpoint', true, `Status: ${compileResponse.status}`, true);
                    }
                } catch (error) {
                    logWarn(`Endpoint check failed: ${error.message}`);
                    recordTest('Compile endpoint', true, error.message, true);
                }
            ...
            if (error.code === 'ECONNREFUSED' || error.message.includes('ECONNREFUSED')) {
                logInfo('Server not running (expected if not started)');
                logInfo('  To start: npm start or START-IDE.bat');
                recordTest('Backend server running', false, 'Server not started (start with: npm start)', true);
            } else {
                logWarn(`Connection check failed: ${error.message}`);
                recordTest('Backend server running', false, error.message, true);
            }
        }
}
