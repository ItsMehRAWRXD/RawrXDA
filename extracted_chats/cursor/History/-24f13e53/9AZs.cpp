...
// ============================================================================
// Forward WM_COMMAND from Explorer toolbar to main window so New/Refresh/Collapse work
static LRESULT CALLBACK ExplorerToolbarProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    if (uMsg == WM_COMMAND && hwnd) {
        Win32IDE* pThis = (Win32IDE*)GetWindowLongPtrA(hwnd, GWLP_USERDATA);
        if (pThis && pThis->getMainWindow())
            SendMessageA(pThis->getMainWindow(), WM_COMMAND, wParam, lParam);
        return 0;
    }
    return DefWindowProcA(hwnd, uMsg, wParam, lParam);
}
...
void Win32IDE::createExplorerView(HWND hwndParent)
{
    ...
    SetWindowLongPtrA(m_hwndExplorerToolbar, GWLP_USERDATA, (LONG_PTR)this);
    SetWindowLongPtrA(m_hwndExplorerToolbar, GWLP_WNDPROC, (LONG_PTR)ExplorerToolbarProc);
