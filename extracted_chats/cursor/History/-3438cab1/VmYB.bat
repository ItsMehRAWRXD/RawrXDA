...
echo ☕ Java Projects:
for /r %%d in (.) do (
    echo "%%d" | findstr /i "node_modules\|\.vscode\|\.git\|target\|\.m2" >nul
    if errorlevel 1 (
        pushd "%%d"
        if exist "*.java" (
            set "has_main=0"
...
echo 🔷 C# Projects:
for /r %%d in (.) do (
    echo "%%d" | findstr /i "node_modules\|\.vscode\|\.git\|bin\|obj" >nul
    if errorlevel 1 (
        pushd "%%d"
        if exist "*.cs" (
            set "has_main=0"
...
echo ⚡ C++ Projects:
for /r %%d in (.) do (
    echo "%%d" | findstr /i "node_modules\|\.vscode\|\.git\|build\|Debug\|Release" >nul
    if errorlevel 1 (
        pushd "%%d"
        if exist "*.cpp" (
            set "has_main=0"
...
echo 🌐 Web Projects:
for /r %%d in (.) do (
    echo "%%d" | findstr /i "node_modules\|\.vscode\|\.git\|dist\|build" >nul
    if errorlevel 1 (
        pushd "%%d"
        if exist "*.html" (
...
echo   Opening project folders...
for /r %%d in (.) do (
    echo "%%d" | findstr /i "node_modules\|\.vscode\|\.git\|venv\|env\|__pycache__\|\.pytest_cache\|target\|\.m2\|bin\|obj\|build\|Debug\|Release\|dist" >nul
    if errorlevel 1 (
        if exist "%%d\*.py" (
            start "" "%%d"
