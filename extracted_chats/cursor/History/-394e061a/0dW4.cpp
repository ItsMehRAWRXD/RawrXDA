...
void BracketMatcher::highlightMatches(int openPos, int closePos)
{
    ...
    highlightFormat.setBackground(QColor(255, 255, 0, 150)); // Semi-transparent yellow
    highlightFormat.setForeground(Qt::black);

    // Highlight opening bracket using mergeCharFormat to avoid permanent changes
    if (openPos >= 0 && openPos < doc->characterCount()) {
        QTextCursor cursor(doc);
        cursor.setPosition(openPos);
        cursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor);
        // Store original format before applying highlight
        m_originalOpenFormat = cursor.charFormat();
        // Merge highlight format (less intrusive than setCharFormat)
        cursor.mergeCharFormat(highlightFormat);
        m_hasHighlighting = true;
        m_lastOpenPos = openPos;
    }

    // Highlight closing bracket
    if (closePos >= 0 && closePos < doc->characterCount()) {
        QTextCursor cursor(doc);
        cursor.setPosition(closePos);
        cursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor);
        // Store original format before applying highlight
        m_originalCloseFormat = cursor.charFormat();
        // Merge highlight format
        cursor.mergeCharFormat(highlightFormat);
        m_hasHighlighting = true;
        m_lastClosePos = closePos;
    }

    // Set up timer to clear highlighting after 1 second
    if (!m_highlightTimer) {
        m_highlightTimer = new QTimer(this);
        m_highlightTimer->setSingleShot(true);
        connect(m_highlightTimer, &QTimer::timeout, this, &BracketMatcher::clearHighlighting);
    }
    m_highlightTimer->stop(); // Stop any existing timer
    m_highlightTimer->start(1000); // Clear after 1 second
}

void BracketMatcher::clearHighlighting()
{
    if (!m_editor || !m_hasHighlighting) {
        return;
    }

    QTextDocument* doc = m_editor->document();

    // Restore original format for opening bracket
    if (m_lastOpenPos >= 0 && m_lastOpenPos < doc->characterCount()) {
        QTextCursor cursor(doc);
        cursor.setPosition(m_lastOpenPos);
        cursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor);
        // Restore original format
        cursor.setCharFormat(m_originalOpenFormat);
    }

    // Restore original format for closing bracket
    if (m_lastClosePos >= 0 && m_lastClosePos < doc->characterCount()) {
        QTextCursor cursor(doc);
        cursor.setPosition(m_lastClosePos);
        cursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor);
        // Restore original format
        cursor.setCharFormat(m_originalCloseFormat);
    }
