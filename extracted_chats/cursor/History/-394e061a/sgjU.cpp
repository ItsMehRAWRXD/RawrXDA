...
void BracketMatcher::highlightMatches(int openPos, int closePos)
{
    ...
    format.setBackground(QColor(255, 255, 0, 100)); // Semi-transparent yellow
    format.setProperty(QTextFormat::FullWidthSelection, false);

    // Highlight opening bracket
    if (openPos >= 0 && openPos < doc->characterCount()) {
        QTextEdit::ExtraSelection openSelection;
        QTextCursor cursor(doc);
        cursor.setPosition(openPos);
        cursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor);
        openSelection.cursor = cursor;
        openSelection.format = format;
        m_extraSelections.append(openSelection);
    }

    // Highlight closing bracket
    if (closePos >= 0 && closePos < doc->characterCount()) {
        QTextEdit::ExtraSelection closeSelection;
        QTextCursor cursor(doc);
        cursor.setPosition(closePos);
        cursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor);
        closeSelection.cursor = cursor;
        closeSelection.format = format;
        m_extraSelections.append(closeSelection);
    }

    // Apply extra selections to editor
    if (QTextEdit* textEdit = qobject_cast<QTextEdit*>(m_editor)) {
        textEdit->setExtraSelections(m_extraSelections);
    }
}
