...
try {
    ...
    $modelExists = $availableModels | Where-Object { $_ -eq $ModelName -or $_ -eq "$ModelName`:latest" }
    ...
    $actualModelName = if ($availableModels -contains "$ModelName`:latest") { "$ModelName`:latest" } else { $ModelName }
    Write-Host "✅ Model found: $actualModelName" -ForegroundColor Green
