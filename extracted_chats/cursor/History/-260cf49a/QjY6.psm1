...
function Protect-SensitiveString {
    <#
    .SYNOPSIS
        Encrypts sensitive string using AES-256 encryption
    .DESCRIPTION
        Uses AES-256-CBC encryption to protect sensitive data. The encryption key
        is derived from the session or uses a secure default. Returns base64-encoded
        encrypted string with IV prepended.
    .PARAMETER Data
        The sensitive string to encrypt
    .PARAMETER Key
        Optional encryption key (32 bytes for AES-256). If not provided, uses session key or generates one.
    .OUTPUTS
        [string] Base64-encoded encrypted string with IV
    .EXAMPLE
        $encrypted = Protect-SensitiveString -Data "MySecretPassword"
    #>
    param(
        [Parameter(Mandatory = $true)]
        [string]$Data,
    ...
    if ($script:SecurityConfig -and -not $script:SecurityConfig.EncryptSensitiveData) {
        Write-SecurityLog "Encryption disabled in config, returning plain text" "WARNING"
        return $Data
    }
    
    try {
        # Use existing StealthCrypto if available
        if ([System.Type]::GetType("StealthCrypto") -ne $null) {
            $sessionKey = if ($script:CurrentSession -and $script:CurrentSession.EncryptionKey) {
                $script:CurrentSession.EncryptionKey
            } else {
                $null
            }
            $encrypted = [StealthCrypto]::Encrypt($Data, $sessionKey)
            Write-SecurityLog "Data encrypted using StealthCrypto" "DEBUG" "Length: $($Data.Length)"
            return $encrypted
        }
        
        # Fallback to AES-256-CBC implementation
        ...
        # Use provided key or generate/derive one
        if ($Key -and $Key.Length -eq 32) {
            $aes.Key = $Key
        } else {
            # Derive key from session or use secure random
            if ($script:CurrentSession -and $script:CurrentSession.EncryptionKey) {
                $keyBytes = [System.Text.Encoding]::UTF8.GetBytes($script:CurrentSession.EncryptionKey)
        ...
        Write-SecurityLog "Data encrypted using AES-256" "DEBUG" "Length: $($Data.Length)"
        return $encrypted
    }
    catch {
        Write-SecurityLog "Encryption failed: $($_.Exception.Message)" "ERROR"
        # Return original data on failure (or throw based on security requirements)
        return $Data
    }
}
...
function Unprotect-SensitiveString {
    <#
    .SYNOPSIS
        Decrypts sensitive string that was encrypted with Protect-SensitiveString
    .DESCRIPTION
        Decrypts AES-256-CBC encrypted data. Expects base64-encoded string with IV prepended.
    .PARAMETER EncryptedData
        Base64-encoded encrypted string with IV
    .PARAMETER Key
        Optional decryption key (must match encryption key)
    .OUTPUTS
    ...
    # Check if encryption is enabled
    if ($script:SecurityConfig -and -not $script:SecurityConfig.EncryptSensitiveData) {
        Write-SecurityLog "Encryption disabled in config, returning as-is" "WARNING"
        return $EncryptedData
    }
    ...
    try {
        ...
        if ([System.Type]::GetType("StealthCrypto") -ne $null) {
            ...
            $decrypted = [StealthCrypto]::Decrypt($EncryptedData, $sessionKey)
            Write-SecurityLog "Data decrypted using StealthCrypto" "DEBUG" "Success"
            return $decrypted
        }
        ...
        [System.Array]::Copy($encryptedBytes, 0, $iv, 0, 16)
        $aes.IV = $iv
        ...
        if ($Key -and $Key.Length -eq 32) {
            ...
            if ($script:CurrentSession -and $script:CurrentSession.EncryptionKey) {
                ...
                throw "Decryption key not available"
            }
        }
        
        # Extract encrypted data (after IV)
        $encryptedDataOnly = New-Object byte[] ($encryptedBytes.Length - 16)
        [System.Array]::Copy($encryptedBytes, 16, $encryptedDataOnly, 0, $encryptedDataOnly.Length)
        
        # Decrypt
        $decryptor = $aes.CreateDecryptor()
        $decryptedBytes = $decryptor.TransformFinalBlock($encryptedDataOnly, 0, $encryptedDataOnly.Length)
        ...
        Write-SecurityLog "Data decrypted using AES-256" "DEBUG" "Success"
        return $decrypted
    }
    catch {
        Write-SecurityLog "Decryption failed: $($_.Exception.Message)" "ERROR"
        # Return original data on failure (or throw based on security requirements)
        return $EncryptedData
    }
}
...
function Update-AIErrorStatistics {
    <#
    .SYNOPSIS
        Updates AI error statistics with error information
    .DESCRIPTION
        Tracks AI-related errors by category, severity, model, type, and count.
        Supports both incremental updates (single error) and batch updates (error count).
    .PARAMETER ErrorCategory
        Category of the error (e.g., "AI", "Model", "API")
    .PARAMETER Severity
        Severity level (Critical, Error, Warning, Info)
    .PARAMETER AIModel
        Name of the AI model that generated the error
    .PARAMETER ErrorCount
        Number of errors to add (for batch updates)
    .PARAMETER ErrorType
        Type of error (e.g., "Timeout", "RateLimit", "InvalidResponse")
    .EXAMPLE
        Update-AIErrorStatistics -ErrorCategory "AI" -Severity "Error" -AIModel "llama2"
    .EXAMPLE
        Update-AIErrorStatistics -ErrorCount 5 -ErrorType "Timeout"
    #>
    param(
        [Parameter(Mandatory = $false)]
        [string]$ErrorCategory = "AI",
    ...
    try {
        ...
        # Ensure directory exists
        $statsDir = Split-Path -Path $statsFile -Parent
        ...
        if (Test-Path $statsFile) {
            try {
                ...
                $stats.TotalErrors = if ($existingStats.TotalErrors) { $existingStats.TotalErrors } else { 0 }
                $stats.ErrorsByCategory = if ($existingStats.ErrorsByCategory) { $existingStats.ErrorsByCategory | ConvertTo-Hashtable } else { @{} }
                $stats.ErrorsBySeverity = if ($existingStats.ErrorsBySeverity) { $existingStats.ErrorsBySeverity | ConvertTo-Hashtable } else { @{} }
                $stats.ErrorsByModel = if ($existingStats.ErrorsByModel) { $existingStats.ErrorsByModel | ConvertTo-Hashtable } else { @{} }
                $stats.ErrorsByType = if ($existingStats.ErrorsByType) { $existingStats.ErrorsByType | ConvertTo-Hashtable } else { @{} }
            }
            catch {
                Write-StartupLog "Failed to parse existing stats file, creating new: $($_.Exception.Message)" "WARNING"
            }
        }
        ...
        # Category statistics
        if ($ErrorCategory) {
            if (-not $stats.ErrorsByCategory.ContainsKey($ErrorCategory)) {
                $stats.ErrorsByCategory[$ErrorCategory] = 0
            }
            $stats.ErrorsByCategory[$ErrorCategory] += $ErrorCount
        }
        
        # Severity statistics
        if ($Severity) {
            if (-not $stats.ErrorsBySeverity.ContainsKey($Severity)) {
                $stats.ErrorsBySeverity[$Severity] = 0
            }
            $stats.ErrorsBySeverity[$Severity] += $ErrorCount
        }
        
        # Model statistics
        if ($AIModel) {
            if (-not $stats.ErrorsByModel.ContainsKey($AIModel)) {
                $stats.ErrorsByModel[$AIModel] = 0
            }
            $stats.ErrorsByModel[$AIModel] += $ErrorCount
        }
        
        # Type statistics
        if ($ErrorType) {
            if (-not $stats.ErrorsByType.ContainsKey($ErrorType)) {
                $stats.ErrorsByType[$ErrorType] = 0
            }
            $stats.ErrorsByType[$ErrorType] += $ErrorCount
        }
        
        # Save updated statistics
        $stats | ConvertTo-Json -Depth 3 | Set-Content $statsFile -Encoding UTF8
        
        Write-StartupLog "AI error statistics updated: +$ErrorCount errors" "DEBUG"
        return $true
    }
    catch {
...
# ============================================
# Function: Create-DocumentTemplate
# ============================================
function Create-DocumentTemplate {
    <#
    .SYNOPSIS
        Creates a new document template with specified content
    .DESCRIPTION
        Creates document templates that can be used for generating standardized reports,
        documentation, or other structured content. Templates support various formats
        including text, markdown, JSON, and XML.
    .PARAMETER TemplateName
        Name of the template (used for file naming)
    .PARAMETER TemplateType
        Type of template: Text, Markdown, JSON, XML, HTML
    .PARAMETER TemplateContent
        Content of the template
    .PARAMETER TemplateDirectory
        Directory to store templates (defaults to Templates subdirectory)
    .OUTPUTS
        [string] Path to the created template file
    .EXAMPLE
        $templatePath = Create-DocumentTemplate -TemplateName "ErrorReport" -TemplateType "Markdown" -TemplateContent $markdownContent
    #>
    param(
        [Parameter(Mandatory = $true)]
        [string]$TemplateName,
        ...
        [Parameter(Mandatory = $false)]
        [ValidateSet("Text", "Markdown", "JSON", "XML", "HTML")]
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($TemplateDirectory)) {
            $baseDir = if ($script:EmergencyLogPath) { $script:EmergencyLogPath } else { $env:TEMP }
            $TemplateDirectory = Join-Path $baseDir "Templates"
        }
        
        # Ensure template directory exists
        if (-not (Test-Path $TemplateDirectory)) {
            New-Item -ItemType Directory -Path $TemplateDirectory -Force | Out-Null
            Write-StartupLog "Created template directory: $TemplateDirectory" "INFO"
        }
        
        # Determine file extension based on type
        $extension = switch ($TemplateType.ToLower()) {
            "markdown" { ".md" }
            "json" { ".json" }
            "xml" { ".xml" }
            "html" { ".html" }
        ...
        $sanitizedName = $TemplateName -replace '[^\w\-_]', '_'
        $templateFileName = "$sanitizedName$extension"
...
<!-- Type: $TemplateType -->
<!-- Created: $timestamp -->
<!-- Generator: RawrXD Logging Module -->

"@
        
        # Combine metadata with content for certain types
        $finalContent = switch ($TemplateType.ToLower()) {
            "markdown" { $metadata + $TemplateContent }
            "html" { "<!-- $metadata -->`n" + $TemplateContent }
            "xml" { "<!-- $metadata -->`n" + $TemplateContent }
            default { $TemplateContent }
        }
        
        # Write template to file
        Set-Content -Path $templatePath -Value $finalContent -Encoding UTF8 -Force
        
        Write-StartupLog "Document template created: $templatePath" "SUCCESS"
        return $templatePath
    }
    catch {
        $errorMsg = "Failed to create document template '$TemplateName': $($_.Exception.Message)"
        Write-StartupLog $errorMsg "ERROR"
        Write-ErrorToFile -ErrorMessage $errorMsg -ErrorCategory "TEMPLATE" -SeverityLevel "Error"
        return $null
    }
}
...
# ============================================
# Function: Generate-DocumentSummary
# ============================================
function Generate-DocumentSummary {
    <#
    .SYNOPSIS
        Generates a summary document from a template and data
    .DESCRIPTION
        Takes a document template and applies data to generate a formatted summary.
        Supports variable substitution, conditional content, and multiple output formats.
    ...
    .PARAMETER SummaryData
        Hashtable containing data to insert into the template
    .PARAMETER OutputDirectory
        Directory for output files (defaults to Documents subdirectory)
    .OUTPUTS
        [string] Path to the generated summary document
    .EXAMPLE
        $summaryPath = Generate-DocumentSummary -TemplatePath $templatePath -DocumentVersion 1 -SummaryData @{Title="Error Report"; Date=(Get-Date)}
    #>
    param(
        [Parameter(Mandatory = $true)]
        [string]$TemplatePath,
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($OutputDirectory)) {
            $baseDir = if ($script:EmergencyLogPath) { $script:EmergencyLogPath } else { $env:TEMP }
            $OutputDirectory = Join-Path $baseDir "Documents"
        }
        ...
        # Get template name and extension
        ...
        # Create output file path with version
        $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
        $outputFileName = "${templateFileName}_v${DocumentVersion}_$timestamp$templateExtension"
        ...
        foreach ($key in $defaultData.Keys) {
            if (-not $SummaryData.ContainsKey($key)) {
                $SummaryData[$key] = $defaultData[$key]
            }
        }
        
        # Perform variable substitution
        foreach ($variable in $SummaryData.GetEnumerator()) {
            $placeholder = "{{$($variable.Key)}}"
            $processedContent = $processedContent -replace [regex]::Escape($placeholder), $variable.Value
        }
        
        # Write processed content to output file
        Set-Content -Path $outputPath -Value $processedContent -Encoding UTF8 -Force
        
        Write-StartupLog "Document summary generated: $outputPath" "SUCCESS"
        return $outputPath
    }
    catch {
        $errorMsg = "Failed to generate document summary from '$TemplatePath': $($_.Exception.Message)"
        Write-StartupLog $errorMsg "ERROR"
        Write-ErrorToFile -ErrorMessage $errorMsg -ErrorCategory "TEMPLATE" -SeverityLevel "Error"
        return $null
    }
...
function Send-DocumentNotification {
    <#
    .SYNOPSIS
        Sends notification about a generated document
    .DESCRIPTION
        Sends email notification or other alerts when documents are created or updated.
        Supports multiple notification methods including email, file system events, and logging.
    ...
    .PARAMETER NotificationMethod
        Method of notification: Email, Log, FileSystem, All
    ...
    .EXAMPLE
        Send-DocumentNotification -DocumentPath $summaryPath -RecipientEmail "admin@company.com" -NotificationMethod "Email"
    #>
    param(
        [Parameter(Mandatory = $true)]
        [string]$DocumentPath,
        ...
        [Parameter(Mandatory = $false)]
        [ValidateSet("Email", "Log", "FileSystem", "All")]
        ...
        [Parameter(Mandatory = $false)]
        [System.Management.Automation.PSCredential]$SMTPCredentials = $null
    )
    ...
    try {
        ...
        # Get document info
        $documentInfo = Get-Item $DocumentPath
        $documentName = $documentInfo.Name
        $documentSize = [math]::Round($documentInfo.Length / 1KB, 2)
        
        # Set default subject if not provided
        if ([string]::IsNullOrEmpty($NotificationSubject)) {
            $NotificationSubject = "Document Generated: $documentName"
        }
        
        $success = $true
        
        # Log notification (always performed)
        $logMessage = "Document notification: $documentName (${documentSize}KB) at $DocumentPath"
        Write-StartupLog $logMessage "INFO"
        
        # Email notification
        if ($NotificationMethod -in @("Email", "All") -and -not [string]::IsNullOrEmpty($RecipientEmail)) {
            try {
                # Create email body
                $emailBody = @"
A new document has been generated:

Document: $documentName
Size: ${documentSize}KB
Path: $DocumentPath
Generated: $(Get-Date -Format "yyyy-MM-dd HH:mm:ss")
System: $env:COMPUTERNAME
User: $env:USERNAME

This is an automated notification from RawrXD.
"@
                
                # Send email using Send-MailMessage or alternative method
                if (Get-Command Send-MailMessage -ErrorAction SilentlyContinue) {
                    $mailParams = @{
                        ...
                        Port = $SMTPPort
                        From = "rawrxd-noreply@system.local"
                    }
                    ...
                    Write-StartupLog "Email notification sent to $RecipientEmail" "SUCCESS"
                }
                else {
                    Write-StartupLog "Send-MailMessage not available, skipping email notification" "WARNING"
                }
            }
            catch {
                Write-StartupLog "Failed to send email notification: $($_.Exception.Message)" "ERROR"
                $success = $false
            }
        }
        
        # File system notification (create a notification file)
        if ($NotificationMethod -in @("FileSystem", "All")) {
            try {
                $notificationDir = Join-Path (Split-Path $DocumentPath) "Notifications"
                if (-not (Test-Path $notificationDir)) {
                    New-Item -ItemType Directory -Path $notificationDir -Force | Out-Null
                }
                
                $notificationFile = Join-Path $notificationDir "notification_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt"
                $notificationContent = @"
...
"@
                ...
                Set-Content -Path $notificationFile -Value $notificationContent -Encoding UTF8
                Write-StartupLog "File system notification created: $notificationFile" "SUCCESS"
            }
            catch {
                Write-StartupLog "Failed to create file system notification: $($_.Exception.Message)" "ERROR"
                $success = $false
            }
        }
        ...
        $errorMsg = "Failed to send document notification for '$DocumentPath': $($_.Exception.Message)"
        Write-StartupLog $errorMsg "ERROR"
        Write-ErrorToFile -ErrorMessage $errorMsg -ErrorCategory "NOTIFICATION" -SeverityLevel "Error"
        return $false
    }
}
...
# ============================================
# Function: Set-SafeRichText
# ============================================
function Set-SafeRichText {
    <#
    .SYNOPSIS
        Sets rich text content with safe RTF formatting
    .DESCRIPTION
        Safely sets rich text content with RTF formatting, preventing parsing errors
        and security issues. Supports basic formatting like bold, italic, colors.
    .PARAMETER RichTextBox
        The RichTextBox control to update
    .PARAMETER Content
        The content to set (plain text)
    .PARAMETER Format
        Formatting options: Plain, Bold, Italic, Colored
    .PARAMETER Color
        Text color (for Colored format)
    .PARAMETER BackgroundColor
        Background color
    .EXAMPLE
        Set-SafeRichText -RichTextBox $rtb -Content "Error occurred" -Format "Bold" -Color "Red"
    #>
    param(
        [Parameter(Mandatory = $true)]
        $RichTextBox,
        ...
        [Parameter(Mandatory = $false)]
        [ValidateSet("Plain", "Bold", "Italic", "Colored")]
        [string]$Format = "Plain",
    ...
    try {
        if (-not $RichTextBox -or $RichTextBox.IsDisposed) { 
            Write-StartupLog "RichTextBox is null or disposed" "WARNING"
            return 
        }
        ...
        switch ($Format) {
            ...
            "Bold" {
                ...
                $RichTextBox.Select(0, 0) # Deselect
            }
            "Italic" {
                $RichTextBox.Text = $Content
                $RichTextBox.SelectAll()
                $RichTextBox.SelectionFont = New-Object System.Drawing.Font($RichTextBox.Font, [System.Drawing.FontStyle]::Italic)
                $RichTextBox.Select(0, 0) # Deselect
            }
            "Colored" {
                $RichTextBox.Text = $Content
                ...
                # Set text color
                try {
                    $textColor = [System.Drawing.Color]::FromName($Color)
                    if ($textColor.IsKnownColor -or $textColor.IsNamedColor) {
                        $RichTextBox.SelectionColor = $textColor
                    }
                }
                catch {
                    Write-StartupLog "Invalid text color: $Color" "WARNING"
                }
                
                # Set background color
                try {
                    $bgColor = [System.Drawing.Color]::FromName($BackgroundColor)
                    if ($bgColor.IsKnownColor -or $bgColor.IsNamedColor) {
                        $RichTextBox.SelectionBackColor = $bgColor
                    }
                }
                catch {
                    Write-StartupLog "Invalid background color: $BackgroundColor" "WARNING"
                }
        ...
        Write-StartupLog "RichTextBox content updated with format: $Format" "DEBUG"
    }
    catch {
        Write-StartupLog "Failed to set rich text content: $($_.Exception.Message)" "ERROR"
        ...
        try {
            ...
            Write-StartupLog "Fallback text assignment also failed: $($_.Exception.Message)" "ERROR"
        }
    }
}


# Helper function for hashtable conversion (used internally)
