...
LRESULT Win32IDE::handleMessage(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_NOTIFY: {
        ...
        if (pNMHDR) {
            ...
            // Debugger Call Stack list: selection change → update Variables and highlight source line
            if (pNMHDR->hwndFrom == m_hwndDebuggerStackTrace && pNMHDR->code == LVN_ITEMCHANGED) {
                ...
                if (pnm->uNewState & LVIS_SELECTED) {
                    int idx = (int)ListView_GetNextItem(m_hwndDebuggerStackTrace, -1, LVNI_SELECTED);
                    if (idx >= 0 && (size_t)idx < m_callStack.size()) {
                        m_selectedStackFrameIndex = idx;
                        updateVariables();
                        const StackFrame& frame = m_callStack[static_cast<size_t>(idx)];
...
void Win32IDE::onCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify) {
    ...
    // Debugger toolbar (IDs from Win32IDE_Debugger.cpp: 2103–2108)
    if (id >= 2103 && id <= 2108) {
        handleDebuggerToolbarCommand(id);
        return;
    }
