...
try {
    ...
    if (Get-Command Write-DevConsole -ErrorAction SilentlyContinue) {
        ...
        Write-Host "[WARNING] ⚠️ System.Web.Extensions not available, using native JSON handling" -ForegroundColor Yellow
    }
}
...
function Initialize-RawrXDChatBotExtension {
    ...
    try {
        if (Get-Command Write-DevConsole -ErrorAction SilentlyContinue) {
            ...
            Write-Host "[INFO] 🤖 Initializing RawrXD ChatBot Extension..." -ForegroundColor Cyan
        }
        ...
        if (Get-Command Write-DevConsole -ErrorAction SilentlyContinue) {
            ...
            Write-Host "[SUCCESS] ✅ ChatBot Extension initialized successfully" -ForegroundColor Green
            Write-Host "[INFO]    - Language: $($script:supportedLanguages[$script:chatBotLanguage].Name)" -ForegroundColor Cyan
            Write-Host "[INFO]    - Intents registered: $($script:chatBotIntents.Count)" -ForegroundColor Cyan
            Write-Host "[INFO]    - Entities registered: $($script:chatBotEntities.Count)" -ForegroundColor Cyan
        }
        ...
        if (Get-Command Write-DevConsole -ErrorAction SilentlyContinue) {
            ...
            Write-Host "[ERROR] ❌ Failed to initialize ChatBot Extension: $($_.Exception.Message)" -ForegroundColor Red
        }
        return $false
    }
}
