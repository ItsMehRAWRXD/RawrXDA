...
class AIProviderManager {
  ...
  async loadApiKeys() {
    try {
      if (window.electron?.apiKeys?.getAll) {
        const result = await window.electron.apiKeys.getAll();
        if (result && result.success !== false) {
          this.apiKeys = result.keys || result || {};
          return;
        }
      }
    } catch (error) {
  ...
  async saveApiKey(provider, key) {
    ...
    try {
      if (window.electron?.apiKeys?.set) {
        const result = await window.electron.apiKeys.set(provider, key);
        if (result && result.success === false) {
  ...
  async deleteApiKey(provider) {
    ...
    try {
      if (window.electron?.apiKeys?.remove) {
        const result = await window.electron.apiKeys.remove(provider);
        if (result && result.success === false) {
  ...
  async refreshModelCatalog() {
    ...
    try {
      ...
      if (result && result.success !== false) {
        ...
        this.ollamaDiskModels = catalog.ollama || [];
