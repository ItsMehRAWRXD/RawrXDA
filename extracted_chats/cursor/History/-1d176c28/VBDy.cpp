...
#define IDC_MAX_TOKENS_SLIDER 1209
#define IDC_CONTEXT_MEM_COMBO 1210
...
void Win32IDE::createSecondarySidebar(HWND hwndParent)
{
    ...
    // Subclass so WM_COMMAND from Send/Clear buttons reaches main window
    SetWindowLongPtrA(m_hwndSecondarySidebar, GWLP_USERDATA, (LONG_PTR)this);
    SetWindowLongPtrA(m_hwndSecondarySidebar, GWLP_WNDPROC, (LONG_PTR)SecondarySidebarProc);
    
    // ====================================================================
    // Phase 37: Merged rich chat panel — Model selector, sliders, AI modes
    // ====================================================================
    
    HFONT hBoldFont = CreateFontA(-14, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, ANSI_CHARSET,
                                  OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
                                  DEFAULT_PITCH | FF_DONTCARE, "Segoe UI");
    HFONT hNormalFont = CreateFontA(-13, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET,
                                    OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
                                    DEFAULT_PITCH | FF_DONTCARE, "Segoe UI");
    
    int w = m_secondarySidebarWidth;
    int y = 0;
    ...
    if (m_hwndSecondarySidebarHeader) SendMessage(m_hwndSecondarySidebarHeader, WM_SETFONT, (WPARAM)hBoldFont, TRUE);
    y += 30;
    ...
    if (m_hwndModelSelector) {
        SendMessage(m_hwndModelSelector, WM_SETFONT, (WPARAM)hNormalFont, TRUE);
    ...
    HWND hContextCombo = CreateWindowExA(
        ...
        65, y, w - 75, 200,
        m_hwndSecondarySidebar, (HMENU)IDC_CONTEXT_MEM_COMBO, m_hInstance, nullptr);
    if (hContextCombo) {
        SendMessage(hContextCombo, WM_SETFONT, (WPARAM)hNormalFont, TRUE);
        ...
        SendMessage(hContextCombo, CB_SETCURSEL, 1, 0); // Default 4K
        m_currentContextSize = 4096;
    }
    y += 28;
    
    // AI Mode checkboxes (2x2 grid)
    m_hwndChkMaxMode = CreateWindowExA(0, "BUTTON", "Max Mode",
        WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
        5, y, 145, 20,
        m_hwndSecondarySidebar, (HMENU)IDC_AI_MAX_MODE, m_hInstance, nullptr);
    m_hwndChkDeepThink = CreateWindowExA(0, "BUTTON", "Deep Think",
        WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
        155, y, 145, 20,
        m_hwndSecondarySidebar, (HMENU)IDC_AI_DEEP_THINK, m_hInstance, nullptr);
    y += 22;
    
    m_hwndChkDeepResearch = CreateWindowExA(0, "BUTTON", "Deep Research",
        WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
        5, y, 145, 20,
        m_hwndSecondarySidebar, (HMENU)IDC_AI_DEEP_RESEARCH, m_hInstance, nullptr);
    m_hwndChkNoRefusal = CreateWindowExA(0, "BUTTON", "No Refusal",
        WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
        155, y, 145, 20,
        m_hwndSecondarySidebar, (HMENU)IDC_AI_NO_REFUSAL, m_hInstance, nullptr);
    
    if (m_hwndChkMaxMode) SendMessage(m_hwndChkMaxMode, WM_SETFONT, (WPARAM)hNormalFont, TRUE);
    if (m_hwndChkDeepThink) SendMessage(m_hwndChkDeepThink, WM_SETFONT, (WPARAM)hNormalFont, TRUE);
    if (m_hwndChkDeepResearch) SendMessage(m_hwndChkDeepResearch, WM_SETFONT, (WPARAM)hNormalFont, TRUE);
    if (m_hwndChkNoRefusal) SendMessage(m_hwndChkNoRefusal, WM_SETFONT, (WPARAM)hNormalFont, TRUE);
    y += 26;
    
    // ====================================================================
    // Chat output area (read-only multiline edit for formatted messages)
    // ====================================================================
    // Calculate remaining space: leave room for input (70px) + buttons (32px) + padding
    int chatOutputHeight = 600 - y - 70 - 36 - 10;
    if (chatOutputHeight < 100) chatOutputHeight = 100;
    
    m_hwndCopilotChatOutput = CreateWindowExA(
        WS_EX_CLIENTEDGE, "EDIT", "",
        WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL,
        5, y, w - 10, chatOutputHeight,
        m_hwndSecondarySidebar, (HMENU)IDC_COPILOT_CHAT_OUTPUT, m_hInstance, nullptr);
    if (m_hwndCopilotChatOutput) {
        SendMessage(m_hwndCopilotChatOutput, WM_SETFONT, (WPARAM)hNormalFont, TRUE);
    }
    y += chatOutputHeight + 4;
    ...
    if (m_hwndCopilotChatInput) {
        SendMessage(m_hwndCopilotChatInput, WM_SETFONT, (WPARAM)hNormalFont, TRUE);
    }
    y += 70;
    ...
    if (m_hwndCopilotSendBtn) SendMessage(m_hwndCopilotSendBtn, WM_SETFONT, (WPARAM)hNormalFont, TRUE);
    ...
    if (m_hwndCopilotClearBtn) SendMessage(m_hwndCopilotClearBtn, WM_SETFONT, (WPARAM)hNormalFont, TRUE);
...
// toggleSecondarySidebar() — defined in Win32IDE_Sidebar.cpp (with null check + logging)
...
void Win32IDE::sendCopilotMessage(const std::string& message)
{
    ...
    if (isModelLoaded()) {
        ...
        std::string enrichedPrompt = buildCopilotPrompt(message);
        response = generateResponse(enrichedPrompt);
...
LRESULT CALLBACK Win32IDE::SecondarySidebarProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg) {
    case WM_COMMAND:
        // Forward Send/Clear and other child control clicks to main window so onCommand handles them
...
void Win32IDE::updateEnhancedStatusBar()
{
    ...
    // Part 11: AI Chat status (was Copilot; label aligned with View > AI Chat)
    std::string copilotText = m_statusBarInfo.copilotActive ? "AI" : "AI (off)";
