...
foreach ($guideModel in $guideModels) {
    ...
    if ($installedModels -contains $guideModel.Name) {
        ...
        # Check partial matches (Ollama might store with different naming)
        $modelBase = $guideModel.Name -replace '^hf\.co/bartowski/', '' -replace ':.*$', ''
        $modelBase = $modelBase -replace '-GGUF$', ''
        
        foreach ($installed in $installedModels) {
            if ($installed -like "*$modelBase*" -or $modelBase -like "*$installed*") {
                $found = $true
