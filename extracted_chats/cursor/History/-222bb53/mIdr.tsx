...
interface DashboardState {
  activeTab: 'inference' | 'orchestration' | 'introspection' | 'comparison' | 'config';
  ...
  const handleRunOrchestration = useCallback(async () => {
    ...
    if (task) {
      try {
        ...
        logEvent('orchestration_error', { task, error: error instanceof Error ? error.message : String(error) });
      }
    }
  }, [logEvent]);
    ...
    <ConfigProvider>
      ...
      <nav className="dashboard-nav">
        <div className="nav-tabs">
          ...
          <button 
            className={`nav-tab ${state.activeTab === 'config' ? 'active' : ''}`}
      ...
      <style>{`
        ...
        .inference-tab, .orchestration-tab, .introspection-tab, .comparison-tab, .config-tab {
          flex: 1;
          display: flex;
          flex-direction: column;
          overflow: hidden;
