...
import { bigDaddyGRuntime, BigDaddyGRuntime } from './runtime';
...
class BigDaddyGAgent {
  ...
  private engine: BigDaddyGRuntime;

  constructor(config: AgentConfig, engine: BigDaddyGRuntime = bigDaddyGRuntime) {
    this.config = config;
    this.engine = engine;
    this.state = {
      status: 'idle',
  ...
  async run(task: string): Promise<AgentResult> {
    ...
    try {
      ...
      for await (const streamingToken of this.engine.streamTokens(task, this.config.maxTokens)) {
...
export function createAgent(config: AgentConfig, engine?: BigDaddyGRuntime): BigDaddyGAgent {
  return new BigDaddyGAgent(config, engine);
}
...
// React Hook Wrapper for proper hook usage
export function useBigDaddyGAgent(config: AgentConfig) {
  // This would be used in a React component where useBigDaddyGEngine is available
  // const engine = useBigDaddyGEngine();
  // return useMemo(() => new BigDaddyGAgent(config, engine), [engine, config]);
  
  // For now, return a factory function that can be called with an engine
  return (engine: BigDaddyGRuntime) => new BigDaddyGAgent(config, engine);
}