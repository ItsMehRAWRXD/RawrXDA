...
QJsonObject AgenticExecutor::executeUserRequest(const QString& request)
{
    ...
    addToMemory("last_user_request", QVariant::fromValue(request));
    addToMemory("last_request_timestamp", result["timestamp"].toVariant());
...
QJsonArray AgenticExecutor::getAvailableTools()
{
    ...
    compressTool["description"] = "Compress data using MASM-optimized compression (brutal_gzip or deflate)";
    compressTool["parameters"] = QJsonObject{
        {"data", QJsonObject{{"type", "string"}, {"description", "Base64 encoded data to compress"}}},
        {"method", QJsonObject{{"type", "string"}, {"enum", QJsonArray{"brutal_gzip", "deflate"}}, {"description", "Compression method (default: brutal_gzip)"}}}
    };
    ...
    decompressTool["parameters"] = QJsonObject{
        {"data", QJsonObject{{"type", "string"}, {"description", "Base64 encoded compressed data"}}}
    };
    tools.append(decompressTool);
    
    QJsonObject compressionStatsTool;
    compressionStatsTool["name"] = "compression_stats";
    compressionStatsTool["description"] = "Get compression statistics and active kernel information";
    tools.append(compressionStatsTool);
...
QJsonObject AgenticExecutor::callTool(const QString& toolName, const QJsonObject& params)
{
    ...
    if (toolName == "create_directory") {
        ...
        if (dataBase64.isEmpty()) {
            result["success"] = false;
            result["error"] = "Data parameter is required";
            return result;
        }
        ...
        uint32_t preferType = (method == "brutal_gzip") ? 2 : 1;
        auto provider = CompressionFactory::Create(preferType);
        
        if (!provider || !provider->IsSupported()) {
            result["success"] = false;
            result["error"] = "Compression provider not available";
            return result;
        }
        ...
        bool success = provider->Compress(raw, compressed);
        qint64 timeMs = timer.elapsed();
        
        if (success) {
            QByteArray output(reinterpret_cast<const char*>(compressed.data()), compressed.size());
            double ratio = (1.0 - (double)compressed.size() / raw.size()) * 100.0;
            
            result["success"] = true;
            result["compressed_data"] = QString(output.toBase64());
            result["compression_ratio"] = ratio;
            result["original_size"] = static_cast<qint64>(raw.size());
            result["compressed_size"] = static_cast<qint64>(compressed.size());
            result["time_ms"] = timeMs;
            result["method"] = method;
            
            qInfo() << "[AgenticExecutor] Compressed" << raw.size() << "bytes to" << compressed.size() 
                    << "bytes (" << QString::number(ratio, 'f', 2) << "%) in" << timeMs << "ms";
        } else {
            result["success"] = false;
        ...
        // Try both providers
        auto provider = CompressionFactory::Create(2); // Try BRUTAL_GZIP first
        if (!provider || !provider->IsSupported()) {
            provider = CompressionFactory::Create(1); // Fallback to DEFLATE
        }
        
        if (!provider || !provider->IsSupported()) {
            result["success"] = false;
            result["error"] = "Decompression provider not available";
            return result;
        }
        ...
        bool success = provider->Decompress(compressed, raw);
        qint64 timeMs = timer.elapsed();
        
        if (success) {
            QByteArray output(reinterpret_cast<const char*>(raw.data()), raw.size());
            
            result["success"] = true;
            result["decompressed_data"] = QString(output.toBase64());
            result["compressed_size"] = static_cast<qint64>(compressed.size());
            result["decompressed_size"] = static_cast<qint64>(raw.size());
            result["time_ms"] = timeMs;
            
            qInfo() << "[AgenticExecutor] Decompressed" << compressed.size() << "bytes to" << raw.size() 
                    << "bytes in" << timeMs << "ms";
        } else {
            result["success"] = false;
    ...
    else if (toolName == "compression_stats") {
        ...
        if (provider && provider->IsSupported()) {
            ...
            if (auto gzip = std::dynamic_pointer_cast<BrutalGzipWrapper>(provider)) {
                kernel = QString::fromStdString(gzip->GetActiveKernel());
            } else if (auto deflate = std::dynamic_pointer_cast<DeflateWrapper>(provider)) {
                kernel = "Deflate: " + QString::fromStdString(deflate->GetDecompressionStats());
            }
            ...
            result["error"] = "No compression provider available";
        }
    }
    else {
        result["success"] = false;
...
{
    // Alias for saveMemoryToDisk - maintain backward compatibility
    saveMemoryToDisk();
}

void AgenticExecutor::saveMemoryToDisk()
{
    // Alias for persistMemoryToDisk - maintain backward compatibility
    persistMemoryToDisk();
}
