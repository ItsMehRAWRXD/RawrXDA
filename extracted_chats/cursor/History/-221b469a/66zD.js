...
if (window.bottomDock && window.webBrowser) {
    console.log('[BrowserPanel] 🌐 Using bottom dock browser integration');
    window.browserPanel = {
        toggle: () => window.webBrowser.toggleBrowser(),
        show: (url) => window.webBrowser.openBrowser(url),
        hide: () => window.webBrowser.closeBrowser(),
        navigate: (url) => window.webBrowser.navigate(url),
        openYouTube: () => window.webBrowser.navigate('https://www.youtube.com'),
        refresh: () => window.webBrowser.refresh?.(),
        togglePlayback: () => console.warn('[BrowserPanel] ⚠️ Playback controls not available in dock mode'),
        playFirstVideo: () => console.warn('[BrowserPanel] ⚠️ Play-first-video not available in dock mode'),
        enterPictureInPicture: () => console.warn('[BrowserPanel] ⚠️ PiP not available in dock mode'),
        tabs: [],
        activeTabId: null,
        get history() {
            return window.webBrowser?.history || [];
        },
        get bookmarks() {
            return window.webBrowser?.bookmarks || [];
        },
    };
} else {
