...
LRESULT CALLBACK Win32IDE::BreadcrumbProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_RBUTTONUP:
    {
        if (pThis) {
            ...
            if (hMenu) {
                AppendMenuW(hMenu, MF_STRING, 1, L"Command Palette (Ctrl+Shift+P)");
                AppendMenuW(hMenu, MF_SEPARATOR, 0, nullptr);
                AppendMenuW(hMenu, MF_STRING, 2, L"Toggle Breadcrumbs");
                ...
                if (cmd == 1) {
                    ...
                    PostMessageW(pThis->getMainWindow(), WM_COMMAND, MAKEWPARAM(12020, 0), (LPARAM)hwnd);
                }
            }
        }
        return 0;
