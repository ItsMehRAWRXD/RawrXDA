...
// ============================================================================
// BREADCRUMB HIT-TEST — same layout as paint for accurate click targeting
// ============================================================================

int Win32IDE::getBreadcrumbHitIndex(HWND hwnd, int clientX)
{
    if (m_breadcrumbPath.empty()) return -1;
    RECT rc;
    ...
    for (size_t i = 0; i < m_breadcrumbPath.size(); i++) {
        const auto& item = m_breadcrumbPath[i];
        if (i > 0) xPos += 22; // separator " > "
        const char* icon = "";
        if (item.symbolKind == "class" || item.symbolKind == "struct") icon = "C ";
        else if (item.symbolKind == "method" || item.symbolKind == "function") icon = "f ";
        else if (item.symbolKind == "namespace") icon = "N ";
        if (icon[0]) xPos += 14;
        SIZE textSize;
        GetTextExtentPoint32A(hdc, item.label.c_str(), static_cast<int>(item.label.size()), &textSize);
        int itemRight = xPos + textSize.cx + 4;
        if (clientX >= xPos && clientX < itemRight) {
            SelectObject(hdc, oldFont);
            ReleaseDC(hwnd, hdc);
...
LRESULT CALLBACK Win32IDE::BreadcrumbProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONUP:
    {
        if (pThis) {
            ...
            int clickedIndex = pThis->getBreadcrumbHitIndex(hwnd, mouseX);
            if (clickedIndex >= 0)
