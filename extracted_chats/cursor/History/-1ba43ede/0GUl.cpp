...
void Win32IDE::onCreate(HWND hwnd) {
    ...
    OutputDebugStringA("[onCreate] createSecondarySidebar...\n");
...
void Win32IDE::onCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify) {
    ...
    switch (id) {
        ...
        case 3007:  // View > AI Chat — toggle secondary sidebar (Ctrl+Alt+B)
            ...
            if (m_hwndStatusBar) SendMessage(m_hwndStatusBar, SB_SETTEXT, 0, (LPARAM)(m_secondarySidebarVisible ? "AI Chat shown" : "AI Chat hidden"));
            return;
        case 6011:  // Explorer toolbar: New File
            newFileInExplorer();
            return;
        case 6012:  // Explorer toolbar: New Folder
            newFolderInExplorer();
            return;
        case 6013:  // Explorer toolbar: Refresh
            refreshFileTree();
            return;
        case 6014:  // Explorer toolbar: Collapse All
            collapseAllFolders();
            return;
        default: break;
    }
    ...
    // Command not found in SSOT registry — direct user so they know the command wasn't handled
