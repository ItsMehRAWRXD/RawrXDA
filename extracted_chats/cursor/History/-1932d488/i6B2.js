...
class PluginMarketplace {
    ...
    renderPlugin(plugin) {
        ...
        return `
            <div style="margin-bottom: 16px; padding: 16px; background: rgba(119, 221, 190, 0.05); border: 1px solid var(--cursor-border); border-radius: 12px; transition: all 0.2s; cursor: pointer;" onmouseover="this.style.borderColor='var(--cursor-jade-light)'; this.style.background='rgba(119, 221, 190, 0.08)'" onmouseout="this.style.borderColor='var(--cursor-border)'; this.style.background='rgba(119, 221, 190, 0.05)'"><div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 10px;"><div style="display: flex; align-items: center; gap: 12px; flex: 1;">${iconContent}<div style="flex: 1;"><div style="font-weight: 600; font-size: 14px; color: var(--cursor-jade-dark); margin-bottom: 4px;">${plugin.name} <span style="font-size: 11px; color: var(--cursor-text-muted);">v${plugin.version}</span>${featured}${stateBadge}</div><div style="font-size: 11px; color: var(--cursor-text-secondary); margin-bottom: 6px;">by ${plugin.author}</div><div style="font-size: 12px; color: var(--cursor-text); margin-bottom: 8px;">${plugin.description}</div><div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">${(plugin.tags || []).map(tag => `<span style="background: rgba(119, 221, 190, 0.15); padding: 3px 8px; border-radius: 10px; font-size: 10px; color: var(--cursor-jade-dark);">${tag}</span>`).join('')}</div></div></div><div style="display: flex; flex-direction: column; gap: 8px; align-items: flex-end; min-width: 140px;">${installed ? `<button onclick="pluginMarketplace.uninstallPlugin('${plugin.id}')" style="background: rgba(255, 71, 87, 0.1); border: 1px solid #ff4757; color: #ff4757; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-size: 11px; font-weight: 600; white-space: nowrap;">🗑️ Uninstall</button>` : `<button onclick="pluginMarketplace.installPlugin('${plugin.id}')" style="background: linear-gradient(135deg, var(--cursor-jade-dark), var(--cursor-accent)); border: none; color: white; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-size: 11px; font-weight: 600; box-shadow: 0 2px 8px rgba(119, 221, 190, 0.3); white-space: nowrap;">⬇️ Install</button>`}<div style="display: flex; gap: 12px; font-size: 10px; color: var(--cursor-text-muted);">${downloadsDisplay !== '0' ? `<span title="Downloads">⬇️ ${downloadsDisplay}</span>` : ''}<span title="Rating">⭐ ${ratingDisplay}</span></div></div></div></div>`;
    }
