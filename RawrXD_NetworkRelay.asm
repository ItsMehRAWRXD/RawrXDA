...
@@found_slot:
    ...
    ; Release pool lock
    mov     QWORD PTR [g_RelayPoolLock], 0
    jmp     @@exit
...
@@relay_loop:
    ...
    ; Set timeout: 1ms = {0, 1000}
    mov     DWORD PTR [rsp + 48], 0      ; tv_sec
    mov     DWORD PTR [rsp + 52], 1000   ; tv_usec

    xor     ecx, ecx
    lea     rdx, [rsp + 64]
    xor     r8d, r8d
    xor     r9d, r9d                     ; exceptfds = NULL
    lea     rax, [rsp + 48]
    mov     QWORD PTR [rsp + 32], rax    ; timeout
    call    select
