...
// Externs from MASM - Note: g_textBuf is a pointer, g_totalChars is an int
extern "C" const wchar_t* g_textBuf; 
...
extern "C" void Bridge_OnSuggestionComplete(const uint32_t* tokens, int count, float conf, int lat) {
    ...
    memcpy(g_latestResponse.suggestion_tokens, tokens, count * sizeof(uint32_t));
    g_latestResponse.token_count = count;
    g_latestResponse.confidence = conf;
    g_latestResponse.latency_ms = lat;
...
extern "C" int Bridge_SubmitCompletion(int command) {
    if (command == 1 && g_responseReady) { g_responseReady = false; return 1; }
    return 0;
}
